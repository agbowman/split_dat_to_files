CREATE PROGRAM ce_group_events_backfill:dba
 ...
 DECLARE batch_size = i4 WITH protect, constant(500000)
 DECLARE current_max_person_id = f8 WITH protect, noconstant(0.0)
 DECLARE max_person_id = f8 WITH protect, noconstant(0.0)
 DECLARE min_person_id = f8 WITH protect, noconstant(0.0)
 DECLARE current_row_id = f8 WITH protect, noconstant(0.0)
 DECLARE added_rows = f8 WITH protect, noconstant(0.0)
 DECLARE added_shadow_rows = f8 WITH protect, noconstant(0.0)
 DECLARE errmsg = vc WITH protect, noconstant("")
 DECLARE previous_percent = f8 WITH protect, noconstant(0.0)
 SET readme_data->status = "F"
 SET readme_data->message = "Readme Failed: Starting ce_group_events_install"
 SET dm_err->eproc = "Find max person_id"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  max_id = max(ce.person_id)
  FROM clinical_event ce
  DETAIL
   max_person_id = max_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Find min person_id"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  min_id = min(ce.person_id)
  FROM clinical_event ce
  DETAIL
   min_person_id = min_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 WHILE (current_max_person_id < max_person_id)
   SET current_max_person_id = (current_row_id+ batch_size)
   IF (current_max_person_id > max_person_id)
    SET current_max_person_id = max_person_id
   ENDIF
   SET string_tflg = "pending"
   INSERT  FROM ce_grouping_queue cgq
    (cgq.person_id, cgq.ce_grouping_queue_id, cgq.queue_status_tflg,
    cgq.create_dt_tm, cgq.remaining_retry_cnt)(SELECT
     x.person_id, seq(ocf_seq,nextval), string_tflg,
     cnvtdatetime(sysdate), 10
     FROM (
      (
      (SELECT DISTINCT
       ce.person_id
       FROM clinical_event ce,
        (inner JOIN person p ON p.person_id=ce.person_id
         AND p.active_ind=1)
       WHERE ce.event_class_cd IN (223, 233, 236)
        AND ce.valid_until_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00")
        AND ce.person_id BETWEEN current_row_id AND current_max_person_id
       ORDER BY ce.person_id))
      x))
   ;end insert
   SET added_rows += curqual
   IF (error(errmsg,0) != 0)
    ROLLBACK
    GO TO exit_program
   ELSE
    COMMIT
   ENDIF
   SET current_row_id += batch_size
   SET temp_percent = (round((current_max_person_id/ max_person_id),5) * 100)
   IF (temp_percent > previous_percent)
    SET previous_percent = temp_percent
   ENDIF
 ENDWHILE
 SET stat = alterlist(drr_validate_table->list,2)
 SET drr_validate_table->list[1].table_name = "CLINICAL_EVENT1333DRR"
 SET drr_validate_table->list[2].table_name = "CE_GROUPING_QUEUE7119DRR"
 SET drr_shadow_cnt = drr_table_and_ccldef_exists(null)
 IF (drr_shadow_cnt=2)
  SET dm_err->eproc = "Find max person_id from CLINICAL_EVENT1333DRR"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
   max_id = max(ce.person_id)
   FROM clinical_event1333drr ce
   DETAIL
    max_person_id = max_id
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   ...
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Find min person_id from CLINICAL_EVENT1333DRR"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
  ...
  WHILE (current_max_person_id < max_person_id)
    ...
    ENDIF
    SET string_tflg = "pending"
    INSERT  FROM ce_grouping_queue7119drr cgq
     (cgq.person_id, cgq.ce_grouping_queue_id, cgq.queue_status_tflg,
     cgq.create_dt_tm, cgq.remaining_retry_cnt)(SELECT
      x.person_id, seq(ocf_seq,nextval), string_tflg,
      cnvtdatetime(sysdate), 10
 ...
 SET readme_data->message = "Success: ce_group_events_backfill completed with success"
 GO TO exit_program
#exit_program
 IF ((dm_err->err_ind=1))
 ...
 SET dm_err->eproc = "Ending ce_group_events_backfill"
 CALL final_disp_msg("ce_group_events_backfill")
END GO
