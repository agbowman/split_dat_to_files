CREATE PROGRAM dm_rmc_mover_6980:dba
 ...
 DECLARE cust_type_cd = f8
 DECLARE cust_type_pos = i4
 DECLARE cust_type_mean = vc
 DECLARE cust_cat_or_pos = i4
 DECLARE cust_cat_or_cd = vc
 DECLARE cust_to_cat_or_cd = vc
 DECLARE cust_rpt_missing = vc
 DECLARE cust_err_val = i4
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_cat_or_cd = get_value(cust_tab_name,"CAT_OR_CATTYPE_CD","FROM")
 SET cust_type_pos = get_col_pos(cust_tab_name,"ASSOCIATION_TYPE_CD")
 SET cust_cat_or_pos = get_col_pos(cust_tab_name,"CAT_OR_CATTYPE_CD")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_type_pos].translated=0))
  SET idcd_check = is_translated(cust_tab_name,"ALL")
 ENDIF
 SET cust_type_cd = cnvtreal(get_value(cust_tab_name,"ASSOCIATION_TYPE_CD","TO"))
 ...
 IF (cust_type_cd > 0)
  SELECT INTO "nl:"
   FROM code_value cv
   WHERE cv.code_value=cust_type_cd
   DETAIL
    cust_type_mean = cv.cdf_meaning
   WITH nocounter
  ;end select
  ...
  ENDIF
 ENDIF
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_cat_or_pos].translated = 0
 IF (((cust_type_mean IN ("ORDERS", "POWERNOTE")) OR (cust_type_cd=0)) )
  SET stat = alterlist(rdds_exception->qual,1)
  SET rdds_exception->qual[1].tab_col_name = "CAT_OR_CATTYPE_CD"
  SET rdds_exception->qual[1].tru_tab_name = "CODE_VALUE"
  SET rdds_exception->qual[1].tru_col_name = "CODE_VALUE"
  IF (cnvtreal(cust_cat_or_cd) > 0)
   SET cust_to_cat_or_cd = select_merge_translate(cust_cat_or_cd,"CODE_VALUE")
  ELSE
   CALL put_value(cust_tab_name,"CAT_OR_CATTYPE_CD","0")
  ENDIF
 ELSEIF (cust_type_mean="POWERFORM")
  SET stat = alterlist(rdds_exception->qual,1)
  SET rdds_exception->qual[1].tab_col_name = "CAT_OR_CATTYPE_CD"
  SET rdds_exception->qual[1].tru_tab_name = "DCP_FORMS_REF"
  SET rdds_exception->qual[1].tru_col_name = "DCP_FORMS_REF_ID"
  IF (cnvtreal(cust_cat_or_cd) > 0)
   SET cust_to_cat_or_cd = select_merge_translate(cust_cat_or_cd,"DCP_FORMS_REF")
  ELSE
   CALL put_value(cust_tab_name,"CAT_OR_CATTYPE_CD","0")
  ENDIF
 ELSE
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "NOMV32"
  CALL merge_audit("FAILREASON","ERROR: An unknown ASSOCIATION_TYPE_CD was found",2)
  GO TO exit_6980
 ENDIF
 IF (cnvtreal(cust_cat_or_cd) > 0)
  IF (isnumeric(cust_to_cat_or_cd))
   CALL put_value(cust_tab_name,"CAT_OR_CATTYPE_CD",cust_to_cat_or_cd)
  ELSE
   SET cust_rpt_missing = report_missing(rdds_exception->qual[1].tru_tab_name,rdds_exception->qual[1]
    .tru_col_name,cnvtreal(cust_cat_or_cd))
...
#exit_6980
 ...
 FREE SET cust_type_cd
 FREE SET cust_type_pos
 FREE SET cust_type_mean
 FREE SET cust_cat_or_pos
 FREE SET cust_cat_or_cd
 FREE SET cust_to_cat_or_cd
END GO
