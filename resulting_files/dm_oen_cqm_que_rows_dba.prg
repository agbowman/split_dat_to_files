CREATE PROGRAM dm_oen_cqm_que_rows:dba
 ...
 SET reply->table_name = "CQM_OENINTERFACE_QUE"
 SET reply->rows_between_commit = minval(10000,request->max_rows)
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(coq.queue_id)
   FROM cqm_oeninterface_que coq
   WHERE coq.queue_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    q.rowid
    FROM cqm_oeninterface_que q
    WHERE parser(sbr_getrowidnotexists("q.queue_id between curMinID and curMaxID","q"))
     AND q.create_dt_tm < cnvtdatetime((curdate - 1),0)
     AND  NOT ( EXISTS (
    (SELECT
     t.queue_id
    ...
    WITH nocounter, maxqual(q,value(rowsleft))
   ;end select
   SET reply->err_code = error(reply->err_msg,1)
   IF ((reply->err_code != 0))
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1","Failed in row collections: %1","s",
