CREATE PROGRAM dm2_dup_chg_rows:dba
 ...
 DECLARE ddcr_error_ind = i2
 DECLARE ddcr_min_log = f8
 DECLARE ddcr_max_log = f8
 DECLARE ddcr_loop = i4
 DECLARE ddcr_cnt = i4
 DECLARE ddcr_sa_run_ind = i2
 DECLARE ddcr_instance = i4
 DECLARE ddcr_high_val = i4
 DECLARE ddcr_colon_pos = i4
 DECLARE ddcr_old_dt_tm_ind = i2
 DECLARE ddcr_qual_ind = i2
 DECLARE ddcr_cur_log = f8
 DECLARE ddcr_batch = i4 WITH constant(2000)
 FREE RECORD instance_val
 RECORD instance_val(
   1 qual[*]
     2 name = vc
     2 num = i4
 )
 FREE RECORD dup_holder
 RECORD dup_holder(
   1 qual[1]
     2 pk_value = f8
     2 pk_where = vc
 ...
 IF (check_logfile("dm2_del_dups_",".log","DM2_DEL_DUPS LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET readme_data->message = "A log file couldn't be created."
  SET readme_data->status = "F"
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_dup_chg_rows"
 CALL disp_msg("",dm_err->logfile,0)
 IF ((readme_data->readme_id > 0))
  SET ddcr_sa_run_ind = 0
  DELETE  FROM dm_info
   WHERE info_domain="DATA MANAGEMENT"
    AND info_name="CHG ROW INSTANCE:*"
   WITH nocounter
  ;end delete
  DELETE  FROM dm_info
   WHERE info_domain="DATA MANAGEMENT"
    AND info_name="CHG ROW BATCH"
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Error cleaning out dm_info batch rows."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET readme_data->status = "F"
   SET readme_data->message = dm_err->emsg
   SET ddcr_error_ind = 1
   GO TO exit_script
  ENDIF
  COMMIT
 ELSE
  SET ddcr_sa_run_ind = 1
 ENDIF
 SET ddcr_cnt = 0
 IF (ddcr_sa_run_ind=1)
  SELECT INTO "NL:"
   FROM dm_info d
   WHERE d.info_domain="DATA MANAGEMENT"
    AND d.info_name="CHG ROW INSTANCE:*"
   DETAIL
    ddcr_cnt = (ddcr_cnt+ 1), stat = alterlist(instance_val->qual,ddcr_cnt), instance_val->qual[
    ddcr_cnt].name = d.info_name
   WITH nocounter, forupdatewait(d)
  ;end select
  SET ddcr_high_val = 0
  FOR (ddcr_loop = 1 TO ddcr_cnt)
    SET ddcr_colon_pos = 0
    SET ddcr_colon_pos = findstring(":",instance_val->qual[ddcr_loop].name,1,0)
    IF (ddcr_colon_pos != 0)
     SET instance_val->qual[ddcr_loop].num = cnvtint(substring((ddcr_colon_pos+ 1),(size(instance_val
        ->qual[ddcr_loop].name,1) - ddcr_colon_pos),instance_val->qual[ddcr_loop].name))
     IF ((ddcr_high_val < instance_val->qual[ddcr_loop].num))
      SET ddcr_high_val = instance_val->qual[ddcr_loop].num
     ENDIF
    ENDIF
  ENDFOR
  SET ddcr_instance = (ddcr_high_val+ 1)
  INSERT  FROM dm_info d
   SET d.info_domain = "DATA MANAGEMENT", d.info_name = concat("CHG ROW INSTANCE:",cnvtstring(
      ddcr_instance)), d.updt_dt_tm = cnvtdatetime(curdate,curtime3),
    d.info_number = 0
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Error inserting dm_info instance row"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ROLLBACK
   GO TO exit_script
  ENDIF
 ...
 SELECT INTO "nl:"
  ...
  FROM dm_chg_log d
  DETAIL
   rptbuf = reportinfo(2), ddcr_max_log = cnvtreal(substring(1,15,rptbuf))
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error selecting max Log_ID row"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET readme_data->status = "F"
  SET readme_data->message = dm_err->emsg
  SET ddcr_error_ind = 1
...
#redo
 SET ddcr_cnt = 0
#try_again
 SET ddcr_cnt = (ddcr_cnt+ 1)
 IF (ddcr_cnt < 5)
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain="DATA MANAGEMENT"
    AND d.info_name="CHG ROW BATCH"
   DETAIL
    ddcr_min_log = d.info_number
   WITH nocounter, forupdatewait(d)
  ;end select
  IF (curqual=0)
   SELECT INTO "nl:"
    ...
    FROM dm_chg_log d
    DETAIL
     rptbuf = reportinfo(2), ddcr_min_log = cnvtreal(substring(1,15,rptbuf))
    WITH nocounter
   ;end select
   IF (ddcr_min_log=ddcr_max_log)
    CALL disp_msg("There are no rows in the DM_CHG_LOG table.",dm_err->logfile,1)
    SET readme_data->message = "There are no rows in the DM_CHG_LOG table."
    SET readme_data->status = "S"
    GO TO exit_script
   ENDIF
   INSERT  FROM dm_info d
    SET d.info_domain = "DATA MANAGEMENT", d.info_name = "CHG ROW BATCH", d.info_number =
     ddcr_min_log,
     d.updt_dt_tm = cnvtdatetime(curdate,curtime3)
    WITH nocounter
   ;end insert
   IF (check_error("Inserting into DM_INFO")=1)
    SET dm_err->emsg = "Error insert into DM_INFO"
    GO TO try_again
   ELSE
    COMMIT
    SET ddcr_cnt = 0
  ...
  ELSE
   UPDATE  FROM dm_info d
    SET d.info_number = (ddcr_min_log+ ddcr_batch), d.updt_dt_tm = cnvtdatetime(curdate,curtime3)
    WHERE d.info_name="CHG ROW BATCH"
   ...
   IF (ddcr_sa_run_ind=1)
    UPDATE  FROM dm_info
     ...
     WHERE info_domain="DATA MANAGEMENT"
      AND info_name=concat("CHG ROW INSTANCE:",cnvtstring(ddcr_instance))
     WITH nocounter
    ;end update
    IF (curqual=0)
     INSERT  FROM dm_info d
   ...
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Error occurred when updating batch number"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET readme_data->status = "F"
    SET readme_data->message = dm_err->emsg
    SET ddcr_error_ind = 1
  ...
  ENDIF
 ELSE
  SET dm_err->emsg = "Second error on insert into DM_INFO"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET readme_data->message = "A new batch number can not be obtained"
  SET readme_data->status = "F"
  GO TO exit_script
 ENDIF
 IF (ddcr_min_log <= ddcr_max_log)
  SET ddcr_error_ind = 0
  CALL echo("_________ ")
  CALL echo("New Batch")
  CALL echo(concat("Processing ",trim(cnvtstring(ddcr_batch))," starting with log_id ",trim(
     cnvtstring(ddcr_min_log)),"."))
  CALL echo(
   "This program can be run in multiple CCL sessions to decrease the amount of processing time")
  CALL echo("To do so run: DM2_DUP_CHG_ROWS GO")
  SET ddcr_cur_log = ddcr_min_log
  WHILE ((ddcr_cur_log <= (ddcr_min_log+ ddcr_batch)))
    SELECT INTO "NL:"
     FROM dm_chg_log c
     WHERE c.log_id=ddcr_cur_log
      AND c.log_type="REFCHG"
     DETAIL
      dup_holder->qual[1].pk_value = c.pk_where_value, dup_holder->qual[1].pk_where = c.pk_where,
      dup_holder->qual[1].table_name = c.table_name,
      dup_holder->qual[1].target_env_id = c.target_env_id, dup_holder->qual[1].del_ind = c.delete_ind
     WITH nocounter, forupdatewait(c)
    ;end select
    IF (check_error(dm_err->eproc)=1)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Error selecting DM_CHG_LOG row"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET readme_data->status = "F"
     SET readme_data->message = dm_err->emsg
     SET ddcr_error_ind = 1
    ...
    IF (ddcr_error_ind=0)
     IF (curqual > 0)
      EXECUTE dm2_dup_chg_child ddcr_cur_log
     ENDIF
    ENDIF
    IF (ddcr_error_ind=1)
     SET ddcr_cur_log = (ddcr_min_log+ ddcr_batch)
    ENDIF
    SET ddcr_cur_log = (ddcr_cur_log+ 1)
  ENDWHILE
  IF (ddcr_error_ind=1)
   GO TO exit_script
  ELSE
 ...
 ELSE
  IF (ddcr_sa_run_ind=1)
   UPDATE  FROM dm_info
    SET info_number = 1, updt_dt_tm = cnvtdatetime(curdate,curtime3)
    WHERE info_domain="DATA MANAGEMENT"
     AND info_name=concat("CHG ROW INSTANCE:",cnvtstring(ddcr_instance))
    WITH nocounter
   ;end update
   ...
   ENDIF
   COMMIT
   CALL echo("This stand-alone instance of DM_DUP_CHG_ROWS has finished")
  ELSE
   SET ddcr_qual_ind = 1
   SET ddcr_old_dt_tm_ind = 0
   WHILE (ddcr_qual_ind != 0
    AND ddcr_old_dt_tm_ind=0)
     CALL echo("Waiting for other instances to finish")
     CALL pause(30)
     SELECT INTO "NL:"
      FROM dm_info d
      WHERE d.info_domain="DATA MANAGEMENT"
      ...
      DETAIL
       IF (datetimediff(cnvtdatetime(curdate,curtime3),d.updt_dt_tm,4) > 10)
        ddcr_old_dt_tm_ind = 1
       ENDIF
      WITH nocounter
     ;end select
     IF (curqual=0)
     ...
     ENDIF
   ENDWHILE
   IF (ddcr_old_dt_tm_ind=0)
    SET readme_data->message = "Finished deleting all duplicate rows in DM_CHG_LOG"
    SET readme_data->status = "S"
   ELSE
    SET readme_data->message = "One of the stand-alone instances has errored."
    SET readme_data->status = "F"
   ENDIF
  ENDIF
 ENDIF
#exit_script
 IF (ddcr_sa_run_ind=0)
