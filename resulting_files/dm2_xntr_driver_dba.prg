CREATE PROGRAM dm2_xntr_driver:dba
 ...
 SUBROUTINE dxrc_requeue_errors(dre_job_error_id,dre_extract_error_id)
   ...
   RETURN(0)
 END ;Subroutine
 IF (check_logfile("dm2_xntr_driver",".log","DM2_XNTR_DRIVER Log...") != 1)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_xntr_driver."
 CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 DECLARE dxd_retrieve_extract(dre_job_id=f8,dre_extract=vc(ref),dre_idx=i4) = i2
 IF ((validate(xxcclseclogin->loggedin,- (99)) != - (99)))
  IF ((xxcclseclogin->loggedin != 1))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "User is not logged into CCL Security"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_script
  ENDIF
 ENDIF
 IF (validate(dxd_extract)=0)
  FREE RECORD dxd_extract
  RECORD dxd_extract(
    1 person_id = f8
    1 job_id = f8
    1 cnt = i4
 ...
 IF (((checkprg("DM2_XNT_DM_RETRIEVE_XML")=0) OR (checkprg("DM2_XNT_DM_REMOVE_XML")=0)) )
  ...
  GO TO exit_script
 ENDIF
 DECLARE dxd_schema_dt_tm = f8 WITH protect, noconstant(cnvtdatetime("31-DEC-2100 23:59:59"))
 DECLARE dxd_plsql_dt_tm = f8 WITH protect, noconstant(cnvtdatetime("01-JAN-1800 00:00:00"))
 DECLARE dxd_person_id = f8 WITH protect, noconstant(0.0)
 DECLARE dxd_job_id = f8 WITH protect, noconstant(0.0)
 DECLARE dxd_idx = i4 WITH protect, noconstant(0)
 DECLARE dxd_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dxd_err_msg = vc WITH protect, noconstant(" ")
 DECLARE dxd_start_time = f8 WITH protect, noconstant(cnvtdatetime(curdate,curtime3))
 DECLARE dxd_stop_time_ind = i2 WITH protect, noconstant(0)
 EXECUTE dm2_set_context "FIRE_REFCHG_TRG", "NO"
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking last time schema was updated."
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DATA MANAGEMENT"
   AND d.info_name="USERLASTUPDT"
  DETAIL
   dxd_schema_dt_tm = d.info_date
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking the last time insert procedures were created."
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DATA MANAGEMENT"
   AND d.info_name="XNTR_PLSQL_GEN"
  DETAIL
   dxd_plsql_dt_tm = d.info_date
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 IF (dxd_plsql_dt_tm < dxd_schema_dt_tm)
  EXECUTE dm2_xntr_create_ins_proc
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = "Querying for next person to retrieve."
 SELECT INTO "nl:"
  FROM dm_xntr_job d
  WHERE d.status="QUEUED"
  DETAIL
   dxd_person_id = d.person_id, dxd_job_id = d.dm_xntr_job_id
  WITH nocounter, maxqual(d,1), forupdatewait(d)
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  ROLLBACK
  GO TO exit_script
 ENDIF
 WHILE (dxd_person_id > 0)
   SET dxd_stop_time_ind = dxrc_check_stop_time(dxd_start_time)
   IF ((((dm_err->err_ind=1)) OR (dxd_stop_time_ind=1)) )
    GO TO exit_script
   ENDIF
   SET dxd_err_ind = 0
   SET dxd_err_msg = "An error occurred during extract(s):"
   IF (dxrc_update_job(dxd_job_id,"RUNNING","",1) > 0)
    GO TO exit_script
   ENDIF
   SET dxd_extract->job_id = dxd_job_id
   SET dxd_extract->person_id = dxd_person_id
   EXECUTE dm2_xntr_find_extracts  WITH replace("DXFE_EXTRACT","DXD_EXTRACT")
   IF (check_error(dm_err->eproc) > 0)
    CALL dxrc_update_job(dxd_job_id,"ERROR",dm_err->emsg,1)
    SET stat = initrec(dxd_extract)
    SET dm_err->err_ind = 0
   ENDIF
   IF ((dxd_extract->cnt > 0))
    FOR (dxd_idx = 1 TO dxd_extract->cnt)
      IF (dxd_retrieve_extract(dxd_job_id,dxd_extract,dxd_idx) < 0)
       SET dxd_err_ind = 1
       SET dxd_err_msg = concat(dxd_err_msg," ",trim(cnvtstring(dxd_extract->qual[dxd_idx].extract_id,
          20),3))
       CALL echo(concat("DXD_ERR_MSG:",dxd_err_msg))
       SET dm_err->err_ind = 0
      ENDIF
      SET dxd_stop_time_ind = dxrc_check_stop_time(dxd_start_time)
      IF ((((dm_err->err_ind=1)) OR (dxd_stop_time_ind=1)) )
       CALL dxrc_requeue_errors(dxd_job_id,0.0)
       GO TO exit_script
      ENDIF
    ENDFOR
    IF (dxd_err_ind=1)
     CALL dxrc_update_job(dxd_job_id,"ERROR",dxd_err_msg,1)
    ELSE
     CALL dxrc_update_job(dxd_job_id,"FINISHED","",1)
    ENDIF
   ENDIF
   SET dm_err->eproc = "Querying for next person to retrieve."
   SET dxd_job_id = 0.0
   SET dxd_person_id = 0.0
   SELECT INTO "nl:"
    FROM dm_xntr_job d
    WHERE d.status="QUEUED"
    DETAIL
   ...
   ENDIF
 ENDWHILE
 SUBROUTINE dxd_retrieve_extract(dre_job_id,dre_extract,dre_idx)
   DECLARE dre_extract_id = f8 WITH protect, noconstant(0.0)
   FREE RECORD dre_stmt
   RECORD dre_stmt(
     1 stmt[*]
       2 str = vc
   )
   FREE RECORD dre_cmb_ids
   RECORD dre_cmb_ids(
     1 extract_dt_tm = f8
     1 extract_id = f8
     1 person_cnt = i4
   ...
   FREE RECORD dre_request
   RECORD dre_request(
     1 extract_uuid = vc
     1 extract_version = i4
     1 extract_id = f8
   )
   FREE RECORD dre_dxed_reply
   RECORD dre_dxed_reply(
     1 status = vc
     1 status_msg = vc
     1 detail_id = f8
   )
   SET dre_extract_id = dre_extract->qual[dre_idx].extract_id
   IF (dxrc_update_extract(dre_extract_id,"RETRIEVE"," ",1) > 0)
    CALL dxrc_rollback_extract(dre_extract_id,0.0,1)
    RETURN(- (1))
   ENDIF
   SET dre_request->extract_uuid = dre_extract->qual[dre_idx].extract_uuid
   SET dre_request->extract_version = dre_extract->qual[dre_idx].extract_version
   SET dre_request->extract_id = dre_extract->qual[dre_idx].extract_id
   EXECUTE dm2_xnt_dm_retrieve_xml  WITH replace("DXRX_REQUEST","DRE_REQUEST")
   IF (check_error(dm_err->eproc)=1)
    CALL dxrc_rollback_extract(dre_extract_id,0.0,1)
    RETURN(- (1))
   ENDIF
   IF (dxrc_update_extract(dre_extract_id,"INSERT"," ",0) > 0)
    CALL dxrc_rollback_extract(dre_extract_id,0.0,1)
    RETURN(- (1))
   ENDIF
   SET stat = alterlist(dre_stmt->stmt,1)
   SET dre_stmt->stmt[1].str = "rdb asis(^ BEGIN XNTR_PARSE_XML_PKG.RETRIEVE_DATA; END; ^) go"
   EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DRE_STMT")
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    CALL dxrc_rollback_extract(dre_extract_id,0.0,1)
    RETURN(- (1))
   ENDIF
   EXECUTE dm2_xntr_get_cmb_ids  WITH replace("DXGCI_CMB_IDS","DRE_CMB_IDS")
   IF (check_error(dm_err->eproc)=1)
    CALL dxrc_rollback_extract(dre_extract_id,0.0,1)
    RETURN(- (1))
   ENDIF
   EXECUTE dm2_xnt_insert_data dre_extract_id
   IF (check_error(dm_err->eproc)=1)
    CALL dxrc_rollback_extract(dre_extract_id,0.0,1)
    RETURN(- (1))
   ENDIF
   IF (dxrc_update_extract(dre_extract_id,"SYNCHRONIZE"," ",1) > 0)
    CALL dxrc_rollback_extract(dre_extract_id,0.0,1)
    RETURN(- (1))
   ENDIF
   SET dre_cmb_ids->extract_id = dre_extract_id
   SET dre_cmb_ids->extract_dt_tm = dre_extract->qual[dre_idx].extract_dt_tm
   EXECUTE dm2_xntr_load_combine  WITH replace("DXLC_CMB_IDS","DRE_CMB_IDS")
   IF (check_error(dm_err->eproc)=1)
    CALL dxrc_rollback_extract(dre_extract_id,0.0,1)
    RETURN(- (1))
   ENDIF
   EXECUTE dm2_xntr_load_readme dre_extract_id, dre_extract->qual[dre_idx].extract_dt_tm
   IF (check_error(dm_err->eproc)=1)
    CALL dxrc_rollback_extract(dre_extract_id,0.0,1)
    RETURN(- (1))
   ENDIF
   EXECUTE dm2_xntr_execute_detail dre_extract_id WITH replace("DXED_REPLY","DRE_DXED_REPLY")
   IF (((check_error(dm_err->eproc)=1) OR ((dre_dxed_reply->status="F"))) )
    SET dm_err->emsg = dre_dxed_reply->status_msg
    CALL dxrc_rollback_extract(dre_extract_id,dre_dxed_reply->detail_id,1)
    RETURN(- (1))
   ENDIF
   EXECUTE dm2_xnt_dm_remove_xml dre_extract->qual[dre_idx].extract_uuid, dre_extract->qual[dre_idx].
   extract_version
   IF (dxrc_update_extract(dre_extract->qual[dre_idx].extract_id,"FINISHED"," ",1) > 0)
    CALL dxrc_rollback_extract(dre_extract_id,0.0,1)
    RETURN(- (1))
   ENDIF
   RETURN(1)
...
#exit_script
 ...
 SET dm_err->eproc = "Dm2_xntr_driver finished"
 CALL final_disp_msg("dm2_xntr_driver")
END GO
