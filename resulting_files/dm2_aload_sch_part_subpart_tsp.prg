CREATE PROGRAM dm2_aload_sch_part_subpart_tsp
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_sch_part_subpart_tsp",".log","dm2_aload_sch_part_subpart_tsp")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_sch_part_subpart_tsP"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daspst_cnt = i4 WITH protect, noconstant(0)
 DECLARE daspst_str = vc WITH protect, noconstant("")
 DECLARE daspst_req_size = i4 WITH protect, noconstant(0)
 DECLARE daspst_ndx = i4 WITH protect, noconstant(0)
 FREE RECORD daspst_maint
 RECORD daspst_maint(
   1 qual[*]
     2 maint_type = c1
   1 num_of_inserts = i4
 ...
 SET daspst_maint->num_of_inserts = 0
 SET daspst_maint->num_of_updates = 0
 SET daspst_maint->num_of_none = 0
 SET daspst_req_size = size(requestin->list_0,5)
 SET stat = alterlist(daspst_maint->qual,daspst_req_size)
 IF (dac_pop_coldic_rec("DM_AFD_PART_SUBPART_TSPACES")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Determining maintenance type for DM_AFD_PART_SUBPART_TSPACES."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(daspst_req_size))
  DETAIL
   daspst_maint->qual[d.seq].maint_type = "I", daspst_maint->num_of_inserts = (daspst_maint->
   num_of_inserts+ 1)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Match record from the package to table"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_afd_tables dat,
   ...
   (dummyt d  WITH seq = value(daspst_req_size))
  PLAN (d
   WHERE d.seq > 0)
   JOIN (dat
   WHERE dat.owner=currdbuser
    AND dat.alpha_feature_nbr=cnvtint(requestin->list_0[d.seq].alpha_feature_nbr)
    AND (dat.table_name=requestin->list_0[d.seq].table_name))
   JOIN (dapst
   WHERE dapst.owner=dat.owner
    AND dapst.alpha_feature_nbr=cnvtint(requestin->list_0[d.seq].alpha_feature_nbr)
    AND (dapst.table_name=requestin->list_0[d.seq].table_name)
    AND (dapst.object_name=requestin->list_0[d.seq].object_name)
    AND dapst.table_name=dat.table_name)
  ORDER BY dapst.table_name, dapst.object_name
  DETAIL
   IF (cnvtint(requestin->list_0[d.seq].schema_instance) < dat.schema_instance)
    daspst_maint->qual[d.seq].maint_type = "N", daspst_maint->num_of_none = (daspst_maint->
    num_of_none+ 1), daspst_maint->num_of_inserts = (daspst_maint->num_of_inserts - 1)
   ELSEIF (cnvtint(requestin->list_0[d.seq].schema_instance)=dat.schema_instance
    AND cnvtdatetime(requestin->list_0[d.seq].updt_dt_tm)=cnvtdatetime(dapst.updt_dt_tm))
    daspst_maint->qual[d.seq].maint_type = "N", daspst_maint->num_of_none = (daspst_maint->
    num_of_none+ 1), daspst_maint->num_of_inserts = (daspst_maint->num_of_inserts - 1)
   ELSEIF (cnvtint(requestin->list_0[d.seq].schema_instance)=dat.schema_instance
    AND cnvtdatetime(requestin->list_0[d.seq].updt_dt_tm) != cnvtdatetime(dapst.updt_dt_tm))
    daspst_maint->qual[d.seq].maint_type = "U", daspst_maint->num_of_updates = (daspst_maint->
    num_of_updates+ 1), daspst_maint->num_of_inserts = (daspst_maint->num_of_inserts - 1)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 2))
  CALL echorecord(daspst_maint)
  CALL echorecord(requestin)
 ENDIF
 IF ((daspst_maint->num_of_inserts > 0))
  ...
  CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
    " dcv ,(dummyt d with seq = value(daspst_req_size) )"),0)
  ...
  CALL dm2_push_cmd("plan d where daspst_maint->qual[d.seq].maint_type = 'I' ",0)
  CALL dm2_push_cmd("join dcv ",0)
  IF (dm2_push_cmd("with nocounter go",1)=0)
   ROLLBACK
   GO TO exit_program
  ENDIF
  CALL echo(build("curqual value->",curqual))
  IF ((curqual != daspst_maint->num_of_inserts))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = build("Rows Inserted:",curqual,". Does not match expected rows inserted:",
    daspst_maint->num_of_inserts)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ROLLBACK
   GO TO exit_program
  ENDIF
  COMMIT
 ENDIF
 IF ((daspst_maint->num_of_updates > 0))
  ...
  IF (dac_build_col_list(null)=0)
   ...
   GO TO exit_program
  ENDIF
  CALL dm2_push_cmd("plan d where daspst_maint->qual[d.seq].maint_type = 'U' ",0)
  ...
  CALL dm2_push_cmd(" and requestin->list_0[d.seq].object_name = dcv.object_name ",0)
  IF (dm2_push_cmd(
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_sch_part_subpart_tsp completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_sch_part_subpart_tsp")
END GO
