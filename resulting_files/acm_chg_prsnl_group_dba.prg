CREATE PROGRAM acm_chg_prsnl_group:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->prsnl_group_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
   prsnl_group p
  SET p.prsnl_group_id = acm_request->prsnl_group_qual[xref->chg[d.seq].idx].prsnl_group_id, p
   .prsnl_group_name = acm_request->prsnl_group_qual[xref->chg[d.seq].idx].prsnl_group_name, p
   .prsnl_group_name_key = trim(cnvtupper(cnvtalphanum(acm_request->prsnl_group_qual[xref->add[d.seq]
      .idx].prsnl_group_name)),3),
   p.prsnl_group_type_cd = acm_request->prsnl_group_qual[xref->chg[d.seq].idx].prsnl_group_type_cd, p
   .prsnl_group_desc = acm_request->prsnl_group_qual[xref->chg[d.seq].idx].prsnl_group_desc, p
   .prsnl_group_class_cd = acm_request->prsnl_group_qual[xref->chg[d.seq].idx].prsnl_group_class_cd,
   p.contributor_system_cd =
   IF ((acm_request->prsnl_group_qual[xref->chg[d.seq].idx].contributor_system_cd > 0.0)) acm_request
    ->prsnl_group_qual[xref->chg[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   , p.beg_effective_dt_tm = cnvtdatetime(acm_request->prsnl_group_qual[xref->chg[d.seq].idx].
    beg_effective_dt_tm), p.end_effective_dt_tm = cnvtdatetime(acm_request->prsnl_group_qual[xref->
    chg[d.seq].idx].end_effective_dt_tm),
   p.service_resource_cd = acm_request->prsnl_group_qual[xref->chg[d.seq].idx].service_resource_cd, p
   .active_ind = acm_request->prsnl_group_qual[xref->chg[d.seq].idx].active_ind, p.active_status_cd
    = acm_request->prsnl_group_qual[xref->chg[d.seq].idx].active_status_cd,
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.prsnl_group_id=acm_request->prsnl_group_qual[xref->chg[d.seq].idx].prsnl_group_id))
  WITH nocounter, status(reply->prsnl_group_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->prsnl_group_qual[xref->chg[index].idx].status != 1))
    SET failed = update_error
    SET table_name = "PRSNL_GROUP"
    GO TO exit_script
   ENDIF
 ENDFOR
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    FROM prsnl_group p
    WHERE expand(t1,f_val,t_val,p.prsnl_group_id,acm_request->prsnl_group_qual[xref->chg[t1].idx].
     prsnl_group_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.prsnl_group_id,acm_request->prsnl_group_qual[xref->chg[t1].idx].
      prsnl_group_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((p.updt_cnt=acm_request->prsnl_group_qual[idx].updt_cnt)) OR ((acm_request->force_updt_ind
     =1))) )
      reply->prsnl_group_qual[idx].status = - (1), reply->prsnl_group_qual[idx].prsnl_group_id =
      acm_request->prsnl_group_qual[idx].prsnl_group_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->prsnl_group_qual[idx].chg_str
     IF (findstring("PRSNL_GROUP_NAME,",chg_str)=0)
      acm_request->prsnl_group_qual[idx].prsnl_group_name = p.prsnl_group_name
     ENDIF
     IF (findstring("PRSNL_GROUP_TYPE_CD,",chg_str)=0)
      acm_request->prsnl_group_qual[idx].prsnl_group_type_cd = p.prsnl_group_type_cd
     ENDIF
     IF (findstring("PRSNL_GROUP_DESC,",chg_str)=0)
      acm_request->prsnl_group_qual[idx].prsnl_group_desc = p.prsnl_group_desc
     ENDIF
     IF (findstring("PRSNL_GROUP_CLASS_CD,",chg_str)=0)
      acm_request->prsnl_group_qual[idx].prsnl_group_class_cd = p.prsnl_group_class_cd
     ENDIF
     IF (findstring("CONTRIBUTOR_SYSTEM_CD,",chg_str)=0)
      acm_request->prsnl_group_qual[idx].contributor_system_cd = p.contributor_system_cd
     ENDIF
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->prsnl_group_qual[idx].beg_effective_dt_tm = p.beg_effective_dt_tm
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->prsnl_group_qual[idx].end_effective_dt_tm = p.end_effective_dt_tm
     ENDIF
     IF (findstring("SERVICE_RESOURCE_CD,",chg_str)=0)
      acm_request->prsnl_group_qual[idx].service_resource_cd = p.service_resource_cd
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->prsnl_group_qual[idx].active_ind = p.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->prsnl_group_qual[idx].active_status_cd = p.active_status_cd
     ENDIF
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->prsnl_group_qual[xref->chg[index].idx].status=0))
