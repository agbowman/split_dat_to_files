CREATE PROGRAM dm2_dbimport_ts_precedence:dba
 ...
 ENDIF
 IF (check_logfile("dm2_dbimport_ts_pre",".log","DM2_DBIMPORT_TS_PRECEDENCE LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 DECLARE ddtp_curqual = i4 WITH noconstant(0)
 ...
 ENDIF
 SET ddtp_curqual = 0
 EXECUTE dm_dbimport "cer_install:dm2_mrg_ts_precedence.csv", "dm2_mrg_load_ts_precedence", 1000
 IF ((dm_err->err_ind=0)
 ...
 IF (checkprg("DM_RMC_AUTO_VERIFY_SETUP") > 0
  ...
  SET dm_err->eproc = "Import complete, logging event data..."
  ...
  SET auto_ver_request->qual[1].event_reason = "DM_TS_PRECEDENCE change by RDDS readme"
