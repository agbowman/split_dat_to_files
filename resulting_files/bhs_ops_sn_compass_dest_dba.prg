CREATE PROGRAM bhs_ops_sn_compass_dest:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 SET frec->file_name = concat("/cerner/d_p627/bhscust/surginet/extract/daily/","case_detail.csv")
 ...
 SELECT INTO "nl:"
  ...
  PLAN (sc
   WHERE sc.surg_start_dt_tm BETWEEN cnvtdatetime(mf_start_dt) AND cnvtdatetime(mf_stop_dt)
    AND expand(ml_idx1,1,m_loc->l_cnt,sc.surg_area_cd,m_loc->qual[ml_idx1].f_loc_cd)
 ...
 SET frec->file_buf = build('"',"PRIMARY_SURGEON",'","',"SURGICAL_SPECIALTY",'","',
  "OR_CASE_NUMBER",'","',"PATIENT_TYPE",'","',"SCHEDULED_PATIENT_TYPE",
  '","',"SCHEDULE_PRIORITY",'","',"PRIMARY_PROCEDURE",'","',
  "CASE_START_DATE",'","',"SCHEDULED_START_TIME",'","',"DELAY_REASONS",
  '","',"OPERATING_ROOM",'","',"SURGICAL_AREA",'","',
  "TOT_SURGERY_MINUTES",'","',"TOT_PAT_IN_RM_MINUTES",'","',"CASE_START_DAY",
  '","',"CASE_CREATED_DATE_TIME",'","',"ADD_ON_INDICATOR",'","',
  "SCHEDULED_CASE_DURATION",'","',"DISCHARGE_DATE_TIME",'","',"DELAY_COMMENTS",
  '","',"POSTOP_LOCATION",'","',"POSTOP_COMMENTS",'","',
  "IN_RM_DT_TM",'","',"OUT_OF_RM_DT_TM",'","',"SURGERY_START_DT_TM",
  '","',"SURGERY_STOP_DT_TM",'","',"PATIENT_NAME",'","',
  "PUBLIC_SCHED_COMMENT",'","',"FIN",'","',"MRN",
  '","',"DOB",'","',"AGE",'","',
  "ADMIT_DT_TM",'","',"ASA",'","',"CASE_LEVEL",
  '","',"PT_FIN_CLASS",'","',"GENDER",'"',
  char(13),char(10))
 SET stat = cclio("WRITE",frec)
 FOR (ml_idx1 = 1 TO m_rec->l_cnt)
  SET frec->file_buf = build('"',trim(m_rec->qual[ml_idx1].s_prim_surgeon,3),'","',trim(m_rec->qual[
    ml_idx1].s_prim_surgeon_specialty,3),'","',
   trim(m_rec->qual[ml_idx1].s_case_nbr,3),'","',trim(m_rec->qual[ml_idx1].s_pat_type,3),'","',trim(
    m_rec->qual[ml_idx1].s_case_sched_patient_type,3),
   '","',trim(m_rec->qual[ml_idx1].s_sched_priority,3),'","',trim(m_rec->qual[ml_idx1].
    s_prim_procedure,3),'","',
   trim(m_rec->qual[ml_idx1].s_surg_start_dt,3),'","',trim(m_rec->qual[ml_idx1].s_surg_sched_start_tm,
    3),'","',replace(trim(m_rec->qual[ml_idx1].s_delay_reason,3),'"','""'),
   '","',trim(m_rec->qual[ml_idx1].s_operating_room,3),'","',trim(m_rec->qual[ml_idx1].s_surg_area,3),
   '","',
   trim(m_rec->qual[ml_idx1].s_surg_tot_time,3),'","',trim(m_rec->qual[ml_idx1].s_rm_tot_time,3),
   '","',trim(m_rec->qual[ml_idx1].s_case_start_day,3),
   '","',trim(m_rec->qual[ml_idx1].s_case_craeted_dt_tm,3),'","',trim(m_rec->qual[ml_idx1].
    s_add_on_indicator,3),'","',
   trim(m_rec->qual[ml_idx1].s_case_sched_duration,3),'","',trim(m_rec->qual[ml_idx1].s_disch_dt,3),
   '","',replace(replace(trim(m_rec->qual[ml_idx1].s_del_desc,3),'"','""'),char(13),""),
   '","',trim(m_rec->qual[ml_idx1].s_postop_dest,3),'","',replace(trim(m_rec->qual[ml_idx1].
     s_postop_comment,3),'"','""'),'","',
   trim(format(cnvtdatetime(m_rec->qual[ml_idx1].f_in_rm),"MM/DD/YYYY HH:mm;;q"),3),'","',trim(format
    (cnvtdatetime(m_rec->qual[ml_idx1].f_out_rm),"MM/DD/YYYY HH:mm;;q"),3),'","',trim(format(
     cnvtdatetime(m_rec->qual[ml_idx1].f_surg_start),"MM/DD/YYYY HH:mm;;q"),3),
   '","',trim(format(cnvtdatetime(m_rec->qual[ml_idx1].f_surg_stop),"MM/DD/YYYY HH:mm;;q"),3),'","',
   trim(m_rec->qual[ml_idx1].s_pat_name,3),'","',
   replace(trim(m_rec->qual[ml_idx1].s_pub_sch_comment,3),'"','""'),'","',trim(m_rec->qual[ml_idx1].
    s_fin,3),'","',trim(m_rec->qual[ml_idx1].s_mrn,3),
   '","',trim(m_rec->qual[ml_idx1].s_dob,3),'","',trim(m_rec->qual[ml_idx1].s_age,3),'","',
   trim(m_rec->qual[ml_idx1].s_admit_dt_tm,3),'","',trim(m_rec->qual[ml_idx1].s_asa,3),'","',trim(
    m_rec->qual[ml_idx1].s_case_level,3),
   '","',trim(m_rec->qual[ml_idx1].s_fin_class,3),'","',trim(m_rec->qual[ml_idx1].s_gender,3),'"',
   char(13),char(10))
  SET stat = cclio("WRITE",frec)
 ENDFOR
