CREATE PROGRAM cpm_audit_get_info:dba
 EXECUTE sacrtl
 EXECUTE srvcore
 ...
 SUBROUTINE (decorateauditsource(auditsource=vc) =vc)
   ...
   IF (auditinfo->logical_domain_enabled)
    DECLARE logicaldomainname = vc WITH protect, noconstant("")
    DECLARE tempuserid = f8 WITH protect, noconstant(reqinfo->updt_id)
    SET acm_get_curr_logical_domain_req->concept = ld_concept_prsnl
    SET reqinfo->updt_id = request->prsnl_id
    EXECUTE acm_get_curr_logical_domain  WITH replace("REQUEST","acm_get_curr_logical_domain_req"),
    replace("REPLY","acm_get_curr_logical_domain_rep")
    SET reqinfo->updt_id = tempuserid
    IF ((acm_get_curr_logical_domain_rep->status_block.status_ind=true))
     SELECT INTO "nl:"
      FROM logical_domain ld
      WHERE (ld.logical_domain_id=acm_get_curr_logical_domain_rep->logical_domain_id)
      ...
      WITH nocounter
     ;end select
     IF ((acm_get_curr_logical_domain_rep->logical_domain_id=0)
      AND trim(logicaldomainname,3) IN ("", null))
      SET logicaldomainname = "Default Logical Domain"
     ENDIF
 ...
 IF (prsnl_alias_key != "")
  ...
  SELECT INTO "nl:"
   ...
   HEAD REPORT
    ...
    trim(pas.alias),
    reply->prsnl_alias_type = uar_get_code_meaning(prsnl_alias_cd)
     ...
     IF (addtoreply=1)
      ...
      IF (sizediff > 0)
       ...
       SET reply->event_list[eventitr].participants[replypartitr].participant_role_cd = substring(1,
        maxparticipantrolecd,request->event_list[eventitr].participants[partitr].participant_role_cd)
      ELSE
      ...
      IF (sizediff > 0)
       ...
       SET reply->event_list[eventitr].participants[replypartitr].participant_id_type = substring(1,
        maxparticipantidtype,request->event_list[eventitr].participants[partitr].participant_id_type)
      ELSE
      ...
      IF (sizediff > 0)
       ...
       SET reply->event_list[eventitr].participants[replypartitr].external_source = trim(substring(1,
         maxexternalsource,request->event_list[eventitr].participants[partitr].external_source))
      ELSE
      ...
      IF ((reply->event_list[eventitr].participants[replypartitr].participant_id > 0)
       ...
       ELSEIF (trim(idtype,4)="PATIENT")
        ...
        SET encntrlist->participant[encntrcnt].person_id = reply->event_list[eventitr].participants[
        replypartitr].participant_id
       ...
       ELSEIF (trim(idtype,4)="ENCOUNTER")
        ...
        SET encntrlist->participant[encntrcnt].encntr_id = reply->event_list[eventitr].participants[
        replypartitr].participant_id
        SET encntrlist->participant[encntrcnt].event_pos = eventitr
        SET encntrlist->participant[encntrcnt].part_pos = replypartitr
       ELSE
        ...
        IF (checkprg(scriptname) <= 0)
         ...
         ENDIF
        ELSE
         SET scriptreply->participant[1].participant_id = request->event_list[eventitr].participants[
         partitr].participant_id
         ...
         IF (partnamesize <= 0)
          ...
          IF (sizediff > 0)
           CALL logtruncationmsg("participant_name",sizediff)
           SET reply->event_list[eventitr].participants[replypartitr].participant_name = substring(1,
            maxparticipantname,scriptreply->participant[1].participant_name)
          ELSE
           SET reply->event_list[eventitr].participants[replypartitr].participant_name = scriptreply
           ->participant[1].participant_name
          ENDIF
         ENDIF
         ...
         cnvtdatetimeutc(cnvtdatetimeutc(scriptreply->participant[1].relationship_creation_dt_tm,2),3
          )
         ...
         IF (sizediff > 0)
          ...
          substring(1,maxrelationcreationtype,scriptreply->participant[1].relationship_creation_type)
         ELSE
          SET reply->event_list[eventitr].participants[replypartitr].relationship_creation_type =
         ...
         IF (sizediff > 0)
          ...
          SET reply->event_list[eventitr].participants[replypartitr].relationship_type = substring(1,
           maxrelationtype,scriptreply->participant[1].relationship_type)
         ELSE
          SET reply->event_list[eventitr].participants[replypartitr].relationship_type = scriptreply
          ->participant[1].relationship_type
         ENDIF
        ENDIF
 ...
 IF (encntrcnt > 0)
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    encntrcaocnt += 1, stat = alterlist(encntrorgdetails->encounters,encntrcaocnt), encntrorgdetails
    ->encounters[encntrcaocnt].cao_id = e.chart_access_organization_id,
  ...
  IF (checkprg("CPM_AUDIT_GET_ORG_DETAILS") > 0)
   ...
   FOR (script_reply_index = 1 TO script_reply_cnt)
     SET encntrorgdetails->encounters[script_reply_index].cao_org_name = encntrorgdetailsreply->orgs[
     script_reply_index].chart_access_org.org_name
     ...
     SET encntrorgdetails->encounters[script_reply_index].care_giver_org_name = encntrorgdetailsreply
     ->orgs[script_reply_index].care_giver.org_name
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    reply->event_list[eventidx].participants[partidx].encounter_id = e.encntr_id, reply->event_list[
    eventidx].participants[partidx].encounter_org = o.org_name,
    ...
    reply->event_list[eventidx].participants[partidx].admit_dt_tm = cnvtdatetimeutc(cnvtdatetimeutc(e
      .beg_effective_dt_tm,2),3), reply->event_list[eventidx].participants[partidx].discharge_dt_tm
     = cnvtdatetimeutc(cnvtdatetimeutc(e.disch_dt_tm,2),3), reply->event_list[eventidx].participants[
    partidx].encounter_type = uar_get_code_display(e.encntr_type_cd),
    ...
    IF (validate(reply->event_list[eventidx].participants[partidx].participant_care_giver.org_id)=1)
     ...
     eventidx].participants[partidx].participant_care_giver.org_alias = encntrorgdetails->encounters[
     encntrorgdetailsidx].care_giver_alias,
     ...
     eventidx].participants[partidx].participant_care_giver.org_name = encntrorgdetails->encounters[
     encntrorgdetailsidx].care_giver_org_name, reply->event_list[eventidx].participants[partidx].
     participant_chart_access_org.org_id = encntrorgdetails->encounters[encntrorgdetailsidx].cao_id,
     ...
     encounters[encntrorgdetailsidx].cao_alias_type, reply->event_list[eventidx].participants[partidx
     ].participant_chart_access_org.org_name = encntrorgdetails->encounters[encntrorgdetailsidx].
     cao_org_name
    ENDIF
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
   ...
   FROM person p,
    (dummyt d  WITH seq = value(encntrcnt))
   PLAN (d
    WHERE (encntrlist->participant[d.seq].person_id > 0))
    JOIN (p
    WHERE (p.person_id=encntrlist->participant[d.seq].person_id))
   DETAIL
    eventidx = encntrlist->participant[d.seq].event_pos, partidx = encntrlist->participant[d.seq].
    part_pos, reply->event_list[eventidx].participants[partidx].person_id = p.person_id,
    reply->event_list[eventidx].participants[partidx].person_name = p.name_full_formatted
    IF ((encntrlist->participant[d.seq].participant_type_flag=etype_person))
 ...
 IF (person_alias_key != "")
  SET person_alias_source = cnvtupper(trim(getkeystring(build(person_alias_key_path,person_alias_key,
      "/source"))))
  IF (person_alias_source="PERSON_ALIAS")
   SET person_alias_cd = uar_get_code_by("DISPLAYKEY",person_alias_code_set,nullterm(person_alias_key
     ))
   SELECT INTO "nl:"
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1
   ...
   JOIN (d2)
   JOIN (pa
   WHERE (pa.alias=reply->event_list[d1.seq].participants[d2.seq].person_alias)
    AND pa.person_id != 0
    AND pa.beg_effective_dt_tm <= cnvtdatetime(sysdate)
    AND pa.end_effective_dt_tm >= cnvtdatetime(sysdate))
   JOIN (so
  ...
  DETAIL
   reply->event_list[d1.seq].participants[d2.seq].oid = so.oid_txt
  WITH nocounter
 ;end select
 ...
 SELECT
  IF (check_active_ind_person_cvr)
   PLAN (pr
    WHERE expand(idx1,1,size(personid_nonzero->list,5),pr.person_id,personid_nonzero->list[idx1].pid)
   ...
   FOR (idx1 = 1 TO size(reply->event_list[idx].participants,5))
     IF ((reply->event_list[idx].participants[idx1].person_id != 0))
      ...
      IF (sensitivity_pos != 0)
       SET reply->event_list[idx].participants[idx1].sensitivity_codes = build(reply->event_list[idx]
        .participants[idx1].sensitivity_codes,";",personid_nonzero->list[sensitivity_pos].
        code_display)
      ENDIF
     ENDIF
   ENDFOR
