CREATE PROGRAM dm2_tgt_chg_log_summary:dba
 ...
 DECLARE post = vc
 DECLARE pre = vc
 IF (check_logfile("Target Summary",".log","DM2_TGT_CHG_LOG_SUM LOGFILE")=0)
  SET dm_err->err_ind = 1
...
#redo
 ...
 IF (currdb="ORACLE")
  ...
  SELECT INTO "NL:"
   ...
   DETAIL
    target = d.info_number
   WITH nocounter
  ;end select
  SET post = concat("@MERGE",value(trim(cnvtstring(source))),value(trim(cnvtstring(target))))
  SET drdm_srch_str = concat("DM_CHG_LOG",post)
  SELECT INTO "NL:"
   FROM (parser(drdm_srch_str) d)
   WHERE d.target_env_id=target
    AND log_type="REFCHG"
   DETAIL
    valid_input_ind = 1
   WITH nocounter, maxqual(d,1)
 ...
 ELSE
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "This script is not operational in DB2"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ...
 CALL echo("*")
 SELECT INTO "mine"
  d.table_name, d.log_id, d.chg_dt_tm,
  t.text
  FROM (parser(drdm_srch_str) d),
   dm_chg_log_audit t
  PLAN (d
   ...
   JOIN (t
   WHERE d.log_id=t.log_id
    AND t.action="FAILREASON")
  ORDER BY d.table_name, d.log_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 1
 ...
 ENDIF
#exit_program
 SET dm_err->eproc = "EXITING THE DM2_TGT_CHG_LOG_SUMMARY"
 CALL echo("*")
 CALL echo("*")
 CALL echo("*")
 CALL echo("********************************************************************************")
 CALL echo("*")
 CALL echo("Exiting the DM2_TGT_CHG_LOG_SUMMARY")
 CALL echo("*")
