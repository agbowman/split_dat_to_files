CREATE PROGRAM dm2_chg_dcp_rows:dba
 ...
 DECLARE dfc_loop = i4
 DECLARE dfc_error_ind = i2
 DECLARE dfc_table_cnt = i4
 DECLARE dfc_col_cnt = i4
 DECLARE dfc_min_log = f8
 DECLARE dfc_max_log = f8
 DECLARE dfc_batch = i4 WITH constant(2000)
 IF (check_logfile("dm2_chg_dcp_",".log","dm2_chg_dcp LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_chg_dcp_rows"
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "NL:"
  FROM dm_tables_doc d
  WHERE d.table_name IN ("DCP_INPUT_REF", "DCP_FORMS_DEF")
  HEAD REPORT
   dfc_table_cnt = 0
  DETAIL
   dfc_table_cnt = (dfc_table_cnt+ 1), stat = alterlist(dcp_tables->qual,dfc_table_cnt), dcp_tables->
   qual[dfc_table_cnt].table_name = d.table_name,
   dcp_tables->qual[dfc_table_cnt].suffix = d.table_suffix
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error selecting from dm_tables_doc."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = dm_err->emsg
 ...
 SELECT INTO "nl:"
  ...
  FROM dm_chg_log d
  DETAIL
   rptbuf = reportinfo(2), dfc_min_log = cnvtreal(substring(1,15,rptbuf))
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  the = max(log_id)"#############.##"
  FROM dm_chg_log d
  DETAIL
   rptbuf = reportinfo(2), dfc_max_log = cnvtreal(substring(1,15,rptbuf))
  WITH nocounter
 ;end select
 WHILE (dfc_min_log < dfc_max_log)
  EXECUTE dm2_chg_dcp_child dfc_min_log
  IF ((dm_err->err_ind=1))
   SET dfc_min_log = dfc_max_log
  ELSE
   SET dfc_min_log = (dfc_min_log+ dfc_batch)
  ENDIF
 ENDWHILE
 FOR (dfc_loop = 1 TO size(dcp_tables->qual,5))
   IF ((dcp_tables->qual[dfc_loop].add_trig_ind=1))
    EXECUTE dm2_drop_chg_log_triggers dcp_tables->qual[dfc_loop].table_name, "REFCHG"
   ENDIF
 ENDFOR
 IF (dfc_error_ind=0)
  SET readme_data->message = "SUCCESS - All the DCP* rows in dm_chg_log have been corrected"
