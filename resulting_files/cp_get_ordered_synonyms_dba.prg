CREATE PROGRAM cp_get_ordered_synonyms:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Person ID:" = 0.0,
  "Encounter ID:" = 0.0,
  "Personnel ID:" = 0.0,
  "Provider Patient Relation Code:" = 0.0,
  "Lookback Units:" = 0,
  "Lookback Unit Type Flag" = 0,
  "Updated Order List" = ""
  WITH outdev, person_id, encntr_id,
  prsnl_id, ppr_cd, lookback_units,
  lookback_type, updated_orders
 ...
 DECLARE ordered_cnt = i4 WITH noconstant(0), protect
 ...
 ENDIF
 SET stat = cnvtjsontorec( $UPDATED_ORDERS)
 DECLARE main(null) = null
 DECLARE loadorderedregimens(null) = null
 DECLARE loadorderedplans(null) = null
 IF ( NOT (validate(order_history_reply)))
 ...
 ENDIF
 IF ( NOT (validate(flat_order_list)))
 ...
 SUBROUTINE main(null)
   ...
   IF ((flat_order_list->regimen_cnt > 0))
    CALL loadorderedregimens(null)
   ENDIF
   IF ((flat_order_list->plan_cnt > 0))
 ...
 SUBROUTINE loadorderedregimens(null)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE cancelled_status_cd = f8 WITH constant(uar_get_code_by("MEANING",4002501,"CANCELLED")),
   protect
   CALL log_message("Begin loadOrderedRegimens()",log_level_debug)
   SELECT INTO "nl:"
    FROM regimen r
    PLAN (r
     WHERE expand(idx,1,flat_order_list->regimen_cnt,r.regimen_catalog_id,cnvtreal(flat_order_list->
       regimens[idx].id))
      AND (r.person_id= $PERSON_ID)
      AND r.order_dt_tm >= cnvtdatetime(look_back_dt_tm)
      AND  NOT (r.regimen_status_cd IN (cancelled_status_cd)))
    ORDER BY r.regimen_catalog_id, r.order_dt_tm DESC
    HEAD r.regimen_catalog_id
     ordered_cnt += 1
     IF (ordered_cnt > size(order_history_reply->qual,5))
      stat = alterlist(order_history_reply->qual,(ordered_cnt+ 10))
     ENDIF
     order_history_reply->qual[ordered_cnt].parent_entity_name = "REGIMEN_CATALOG",
     order_history_reply->qual[ordered_cnt].parent_entity_id = r.regimen_catalog_id,
     order_history_reply->qual[ordered_cnt].last_ordered_dt_tm = r.order_dt_tm,
     order_history_reply->qual[ordered_cnt].last_ordered_dt_tm_disp = format(r.order_dt_tm,
      "@SHORTDATE")
    FOOT REPORT
     order_history_reply->cnt = ordered_cnt, stat = alterlist(order_history_reply->qual,ordered_cnt)
 ...
 SUBROUTINE loadorderedplans(null)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE void_status_cd = f8 WITH constant(uar_get_code_by("MEANING",16769,"VOID")), protect
   DECLARE entity_name = vc WITH noconstant(""), protect
   DECLARE entity_id = f8 WITH noconstant(0.0), protect
   CALL log_message("Begin loadOrderedPlans()",log_level_debug)
   SELECT INTO "nl:"
    FROM pathway p
    PLAN (p
     WHERE expand(idx,1,flat_order_list->plan_cnt,p.pw_cat_group_id,cnvtreal(flat_order_list->plans[
       idx].id))
      AND (p.person_id= $PERSON_ID)
      AND p.order_dt_tm >= cnvtdatetime(look_back_dt_tm)
      AND  NOT (p.pw_status_cd IN (void_status_cd)))
    ORDER BY p.pw_cat_group_id, p.pathway_customized_plan_id, p.order_dt_tm DESC
    HEAD p.pathway_customized_plan_id
     ...
     IF (p.pathway_customized_plan_id > 0)
      entity_name = "PATHWAY_CUSTOMIZED_PLAN", entity_id = p.pathway_customized_plan_id
     ELSE
      entity_name = "PATHWAY_CATALOG", entity_id = p.pw_cat_group_id
     ENDIF
     order_history_reply->qual[ordered_cnt].parent_entity_name = entity_name, order_history_reply->
     qual[ordered_cnt].parent_entity_id = entity_id, order_history_reply->qual[ordered_cnt].
     last_ordered_dt_tm = p.order_dt_tm,
     order_history_reply->qual[ordered_cnt].last_ordered_dt_tm_disp = format(p.order_dt_tm,
      "@SHORTDATE")
    FOOT REPORT
     order_history_reply->cnt = ordered_cnt, stat = alterlist(order_history_reply->qual,ordered_cnt)
    WITH nocounter, expand = 1
 ...
 SET order_history_reply->status_data.status = "F"
 CALL main(null)
 SET order_history_reply->status_data.status = "S"
#exit_script
 ...
 CALL log_message(build("Total time in seconds:",((curtime3 - script_start_curtime3)/ 100.0)),
  log_level_debug)
 IF (( $OUTDEV != "NOFORMS"))
  CALL putjsonrecordtofile(order_history_reply)
 ENDIF
END GO
