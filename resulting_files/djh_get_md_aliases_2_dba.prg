CREATE PROGRAM djh_get_md_aliases_2:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (pr
   WHERE pr.physician_ind >= 0
    ...
    AND pr.name_last_key="ABARE*"
    AND pr.name_first_key="NATHAN*")
   JOIN (pa
 ...
 SELECT INTO  $1
  ...
  DETAIL
   ...
   FOR (y = 1 TO size(md_list->qual[d.seq].phone,5))
     col 10, "Phone: ", md_list->qual[d.seq].phone[y].p_phone_type_disp,
     md_list->qual[d.seq].phone[y].phone_num, row + 1
   ENDFOR
   IF (row > 52)
    BREAK
   ENDIF
