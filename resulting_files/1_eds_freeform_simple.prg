CREATE PROGRAM 1_eds_freeform_simple
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Enter label count:" = "",
  "Enter Name" = "",
  "Enter Person ID" = ""
  WITH outdev, label_count, name,
  id
 EXECUTE reportrtl
 ...
 SUBROUTINE layoutsection0abs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Cerner Hospital ",char(0)))
    SET rptsd->m_flags = 4
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "1_eds_freeform_simple"
 ...
 SET _nrows = 1
 SET _nlabelcnt = 0
 SET _nmaxlabels = cnvtint(value( $LABEL_COUNT))
 IF (_nmaxlabels < 1)
  SET _nmaxlabels = (_ncols * _nrows)
 ENDIF
 WHILE (_nlabelcnt < _nmaxlabels)
   SET y = 0
   WHILE (y < _nrows
    AND _nlabelcnt < _nmaxlabels)
     SET x = 0
     WHILE (x < _ncols
      AND _nlabelcnt < _nmaxlabels)
       SET _xoffset = ((_flmargin+ (_flabelwidth * x))+ (_fcolgutter * x))
       SET _yoffset = ((_ftmargin+ (_flabelheight * y))+ (_frowgutter * y))
       SET dummy_val = layoutsection0(rpt_render)
       SET _nlabelcnt = (_nlabelcnt+ 1)
       SET x = (x+ 1)
     ENDWHILE
     SET y = (y+ 1)
   ENDWHILE
   IF (_nlabelcnt < _nmaxlabels)
    CALL pagebreak(0)
   ENDIF
 ENDWHILE
