CREATE PROGRAM dcp_bmi_zscore_calc:dba
 ...
 DECLARE calculatebmizscore(null) = null
 ...
 SUBROUTINE getstatsdata(null)
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     ...
     bmi_chart_source_cd = c.chart_source_cd, bmi_chart_source_disp = concat("^~:!",
      zscore_source_string," ",uar_get_code_display(bmi_chart_source_cd))
    WITH nocounter
   ;end select
   IF (bmi_chart_source_cd=bmi_chart_source_cdc_cd
    ...
    SELECT INTO "nl:"
     ...
     DETAIL
      ...
      concat("^~:!",zscore_source_string," ",uar_get_code_display(extendedbmi_chart_source_cd))
     WITH nocounter
    ;end select
   ENDIF
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE calculatebmizscore(null)
   ...
   IF (ispatientobesecdc=1)
    ...
    DECLARE percentileupperlimit = f8 WITH noconstant(99.9999713348428)
    ...
    IF (bmi_percentile <= percentileupperlimit)
     SET bmi_percentile /= 100
    ELSE
     SET bmi_percentile = (percentileupperlimit/ 100)
    ENDIF
    SET bmi_z_score = ((- ((2.0 * log((1 - bmi_percentile))))** 0.5) - (((((0.010328 * (- ((2.0 * log
    ((1 - bmi_percentile))))** 0.5))+ 0.802853) * (- ((2.0 * log((1 - bmi_percentile))))** 0.5))+
    2.515517)/ ((((((0.001308 * (- ((2.0 * log((1 - bmi_percentile))))** 0.5))+ 0.189269) * (- ((2.0
     * log((1 - bmi_percentile))))** 0.5))+ 1.432788) * (- ((2.0 * log((1 - bmi_percentile))))** 0.5)
    )+ 1.0)))
   ENDIF
 END ;Subroutine
#exit_script
 SET log_misc1 = concat(trim(cnvtstringchk(bmi_z_score,10,6)),"|",trim(cnvtstringchk(
 ...
 ELSE
  SET retval = 100
  SET log_message = "BMI z-score calculation finished."
 ENDIF
END GO
