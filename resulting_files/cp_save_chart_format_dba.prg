CREATE PROGRAM cp_save_chart_format:dba
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "CP_SAVE_CHART_FORMAT"
 ...
 DECLARE insertchartformat(null) = null
 DECLARE insertchartsections(null) = null
 CALL log_message("Starting script: cp_save_chart_format",log_level_debug)
 SET reply->status_data.status = "F"
 SET reqinfo->commit_ind = 0
 CALL buildreplystructure(null)
 SET request->chart_format_id = getnextchartformatid(null)
 CALL insertchartformat(null)
 SET reqinfo->commit_ind = 1
 SET reply->status_data.status = "S"
 SUBROUTINE insertchartformat(null)
   CALL log_message("In InsertChartFormat()",log_level_debug)
   IF (size(request->resubmit_disclaimer) > 0)
    SET resubmit_disclaimer_id = insertresubmitdisclaimer(null)
   ENDIF
   IF (request->alt_text_apply_all)
    CALL deleteadditionalinfo(request->alt_text_apply_all,0.0)
   ENDIF
   CALL insertformat(null)
   CALL insertchartsections(null)
   CALL insertenhancedlayoutxml(null)
 END ;Subroutine
 SUBROUTINE insertchartsections(null)
   CALL log_message("In InsertChartSections()",log_level_debug)
   DECLARE x = i4 WITH noconstant(0), protect
   FOR (x = 1 TO request->chart_section_count)
     IF ((request->chart_section_list[x].section_type_flag != facesheet_section_type))
      CALL insertchartsection(x)
      CALL insertchartformsects(x)
      CALL insertchartsectionfields(x)
     ENDIF
   ENDFOR
   IF (trim(request->program_name) != null)
    CALL add_facesheet_section(null)
   ENDIF
 END ;Subroutine
#exit_script
 CALL log_message("Exiting script: cp_save_chart_format",log_level_debug)
END GO
