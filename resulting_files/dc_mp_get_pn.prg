CREATE PROGRAM dc_mp_get_pn
 ...
 SUBROUTINE iqrversioncontrolstring(category_topic_mean,beg_time)
   ...
   RETURN(version_number)
 END ;Subroutine
 DECLARE ver_num = f8 WITH protect, constant(iqrversioncontrol("PNEUMONIA",cnvtdatetime(curdate,
    curtime3)))
 IF (( $2 != null))
  SET jrec = cnvtjsontorec( $2)
  IF (validate(debug_dc_mp_get_pn_ind,0)=1)
...
#exit_script
 IF (validate(_memory_reply_string)=1)
  SET _memory_reply_string = cnvtrectojson(pnreply)
 ELSE
  CALL echojson(pnreply, $1)
  SELECT INTO "nl:"
   DETAIL
    row + 0
   WITH nocounter
