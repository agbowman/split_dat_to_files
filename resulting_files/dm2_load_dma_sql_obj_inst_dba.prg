CREATE PROGRAM dm2_load_dma_sql_obj_inst:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dld_loop_ind = i2 WITH protect, noconstant(1)
 DECLARE dld_commit_num = i4 WITH protect, noconstant(10000)
 DECLARE dld_for_cnt = i4 WITH protect, noconstant(1)
 DECLARE dld_count = i4 WITH protect, noconstant(1)
 IF (check_logfile("dm2_load_dsoi",".log","DM2_LOAD_DSOI LogFile...") != 1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((dm2_db_options->cursor_commit_cnt != "NOT_SET"))
  SET dld_commit_num = cnvtint(dm2_db_options->cursor_commit_cnt)
 ENDIF
 SET dm_err->eproc = "Populate updt_cl_id  column on DMA_DSOI_STAGE"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dld_loop_ind = 1
 WHILE (dld_loop_ind=1)
   UPDATE  FROM dma_dsoi_stage dds
    SET dds.marked_flag = 1, dds.updt_cl_id =
     (SELECT
      d.dma_sql_obj_inst_id
      FROM dma_sql_obj_inst d
      WHERE d.process_type=dds.process_type
       AND d.object_type=dds.object_type
       AND d.object_owner=dds.object_owner
       AND d.object_name=dds.object_name
       AND d.object_instance=dds.object_instance
       AND d.table_name=dds.table_name
       AND d.updt_cnt < dds.updt_cnt)
    WHERE dds.updt_cl_id = null
     AND dds.marked_flag = null
    WITH nocounter, maxqual(dds,value(dld_commit_num))
   ;end update
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
   ELSE
    COMMIT
   ENDIF
   IF (curqual < dld_commit_num)
    SET dld_loop_ind = 0
   ENDIF
 ENDWHILE
 SET dm_err->eproc = "Populate new_cl_id  column on DMA_DSOI_STAGE"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dld_loop_ind = 1
 WHILE (dld_loop_ind=1)
   UPDATE  FROM dma_dsoi_stage dg
    SET dg.new_cl_id = seq(dm_seq,nextval)
    WHERE  NOT ( EXISTS (
    (SELECT
     "x"
     FROM dma_sql_obj_inst d
     WHERE d.process_type=dg.process_type
      AND d.object_type=dg.object_type
      AND d.object_owner=dg.object_owner
      AND d.object_name=dg.object_name
      AND d.object_instance=dg.object_instance
      AND d.table_name=dg.table_name)))
     AND dg.new_cl_id = null
    WITH nocounter, maxqual(dg,value(dld_commit_num))
   ;end update
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
   ...
   ENDIF
 ENDWHILE
 SET dm_err->eproc = "Delete rows from DMA_DSOI_STAGE that do not have any changes"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dld_loop_ind = 1
 WHILE (dld_loop_ind=1)
   DELETE  FROM dma_dsoi_stage d
    WHERE d.updt_cl_id = null
     AND d.new_cl_id = null
    WITH nocounter, maxqual(d,value(dld_commit_num))
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
   ...
   ENDIF
 ENDWHILE
 SET dm_err->eproc = "Populate DMA_DSOIA_STAGE temp_cl_id to reflect parent table new/updt_cl_id"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dld_loop_ind = 1
 WHILE (dld_loop_ind=1)
   UPDATE  FROM dma_dsoia_stage dds
    SET dds.marked_flag = 1, dds.temp_cl_id =
     (SELECT
      nullval(d.updt_cl_id,d.new_cl_id)
      FROM dma_dsoi_stage d
      WHERE d.dma_sql_obj_inst_id=dds.dma_sql_obj_inst_id)
    WHERE dds.temp_cl_id = null
     AND dds.marked_flag = null
    WITH nocounter, maxqual(dds,value(dld_commit_num))
   ;end update
   IF (check_error(dm_err->eproc)=1)
   ...
   ENDIF
 ENDWHILE
 SET dm_err->eproc = "Delete rows from DMA_DSOIA_STAGE that do not have any changes"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dld_loop_ind = 1
 WHILE (dld_loop_ind=1)
   DELETE  FROM dma_dsoia_stage d
    WHERE d.temp_cl_id = null
    WITH nocounter, maxqual(d,value(dld_commit_num))
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   ENDIF
 ENDWHILE
 SET dm_err->eproc = "Delete from dma_sql_obj_inst and _attr rows that would be updated"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dld_loop_ind = 1
 WHILE (dld_loop_ind=1)
   DELETE  FROM dma_sql_obj_inst_attr da
    WHERE da.dma_sql_obj_inst_id IN (
    (SELECT
     d.updt_cl_id
     FROM dma_dsoi_stage d
     WHERE d.updt_cl_id IS NOT null))
    WITH nocounter, maxqual(da,value(dld_commit_num))
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
 ...
 SET dld_loop_ind = 1
 WHILE (dld_loop_ind=1)
   DELETE  FROM dma_sql_obj_inst di
    WHERE di.dma_sql_obj_inst_id IN (
    (SELECT
     d.updt_cl_id
     FROM dma_dsoi_stage d
     WHERE d.updt_cl_id IS NOT null))
    WITH nocounter, maxqual(di,value(dld_commit_num))
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
   ...
   ENDIF
 ENDWHILE
 SET dm_err->eproc = "Delete child orphan rows"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dld_loop_ind = 1
 WHILE (dld_loop_ind=1)
   DELETE  FROM dma_sql_obj_inst_attr dsoia
    WHERE  NOT ( EXISTS (
    (SELECT
     "x"
     FROM dma_sql_obj_inst dsoi
     WHERE dsoi.dma_sql_obj_inst_id=dsoia.dma_sql_obj_inst_id)))
    WITH nocounter, maxqual(dsoia,value(dld_commit_num))
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
   ...
   ENDIF
 ENDWHILE
 SET dm_err->eproc = "Insert new rows in dma tables"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dld_loop_ind = 1
 WHILE (dld_loop_ind=1)
   SET dld_count = (dld_count+ 1)
   UPDATE  FROM dma_dsoia_stage dds
    SET dds.marked_flag = dld_count
    WHERE dds.marked_flag=1
    WITH nocounter, maxqual(dds,value(dld_commit_num))
   ;end update
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   ELSE
    COMMIT
   ENDIF
   INSERT  FROM dma_sql_obj_inst_attr di
    (di.dma_sql_obj_inst_id, di.dma_sql_obj_inst_attr_id, di.attr_name,
    di.attr_seg_nbr, di.attr_value_char, di.attr_value_num,
    di.attr_value_dt_tm, di.attr_count, di.updt_cnt,
    di.updt_id, di.updt_task, di.updt_dt_tm,
    di.updt_applctx)(SELECT
     d.temp_cl_id, seq(dm_seq,nextval), d.attr_name,
     d.attr_seg_nbr, d.attr_value_char, d.attr_value_num,
     d.attr_value_dt_tm, d.attr_count, d.updt_cnt,
     d.updt_id, d.updt_task, d.updt_dt_tm,
     d.updt_applctx
     FROM dma_dsoia_stage d
     WHERE d.marked_flag=dld_count)
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 WHILE (dld_loop_ind=1)
   ...
   ELSE
    COMMIT
   ENDIF
   INSERT  FROM dma_sql_obj_inst di
    (di.dma_sql_obj_inst_id, di.process_type, di.object_type,
    di.object_owner, di.object_name, di.object_instance,
    di.active_ind, di.updt_cnt, di.table_name,
    di.updt_id, di.updt_task, di.updt_dt_tm,
    di.updt_applctx)(SELECT
     nullcheck(d.updt_cl_id,d.new_cl_id,nullind(d.updt_cl_id)), d.process_type, d.object_type,
     d.object_owner, d.object_name, d.object_instance,
     d.active_ind, d.updt_cnt, d.table_name,
     d.updt_id, d.updt_task, d.updt_dt_tm,
     d.updt_applctx
     FROM dma_dsoi_stage d
     WHERE d.marked_flag=dld_count)
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_load_dma_sql_obj_inst completed"
 ELSE
  ROLLBACK
 ENDIF
 CALL final_disp_msg("dm2_load_dma_sql_obj_inst")
END GO
