CREATE PROGRAM dm2_aload_task
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_task",".log","dm2_aload_task")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_task"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dat_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dat_file = vc WITH protect, noconstant( $2)
 DECLARE dat_row_cnt = i4 WITH protect, noconstant( $3)
 DECLARE dat_batch = i4 WITH protect, noconstant(0)
 DECLARE dat_rows_returned = i4 WITH protect, noconstant(10000)
 DECLARE dat_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(dat_pkg,dat_file_loc_ret,dat_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dat_batch=0)
  SET dat_batch = 5000
 ENDIF
 SET dm_err->eproc = "Clean existing data from DM_OCD_TASK for package."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 WHILE ((dm_err->err_ind=0)
  AND dat_rows_returned=10000)
   DELETE  FROM dm_ocd_task d
    WHERE d.alpha_feature_nbr=dat_pkg
   ...
   ELSE
    COMMIT
   ENDIF
   SET dat_rows_returned = curqual
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("Rows deleted:",dat_rows_returned))
   ENDIF
 ENDWHILE
 IF (dat_row_cnt > 0)
  EXECUTE dm2_dbimport concat(dat_file_loc_ret,dat_file), "dm2_aload_atr_task", dat_batch
  CALL echo(concat(dat_file_loc_ret,dat_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_task completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_task")
END GO
