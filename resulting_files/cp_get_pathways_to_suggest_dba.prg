CREATE PROGRAM cp_get_pathways_to_suggest:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Pathway Name" = "",
  "Rule Name" = "",
  "Triggering Text" = ""
  WITH outdev, pathway_name, rule_name,
  triggering_text
 ...
 DECLARE quick_visit_pw_type_cd = f8 WITH constant(uar_get_code_by("MEANING",4003197,"QUICKVISIT")),
 protect
 ...
 DECLARE condition_json = vc WITH constant(trim( $1)), protect
 ...
 DECLARE rule_type_mean = vc WITH constant("SUGGESTED_BY_RULE"), protect
 DECLARE activepathwayind = i2 WITH noconstant(0), protect
 DECLARE pathwayname = vc WITH constant(trim( $PATHWAY_NAME,3)), protect
 DECLARE rulename = vc WITH constant( $RULE_NAME), protect
 DECLARE triggeringtext = vc WITH constant( $TRIGGERING_TEXT), protect
 DECLARE main(null) = null WITH protect
 DECLARE getpathwaysbynomenclatureid(null) = null WITH protect
 DECLARE getpathwaysbyontologycondition(null) = null WITH protect
 FREE RECORD conditionstocheck
 RECORD conditionstocheck(
   1 ont_concept_cnt = i4
   1 ont_concept_qual[*]
     2 condition_display = vc
     2 nomenclature_id = f8
     2 do_context_name = vc
     2 do_concept_name = vc
     2 do_context_version_nbr = i4
   1 nomen_cnt = i4
   1 nomen_qual[*]
     2 condition_display = vc
     2 nomenclature_id = f8
 ...
 FREE RECORD pathwaystosuggest
 RECORD pathwaystosuggest(
   1 cnt = i4
   1 qual[*]
     2 cp_pathway_id = f8
     2 pathway_name = vc
     2 criteria_type_mean = vc
     2 trigger_display = vc
     2 trigger_parent_entity_id = f8
     2 trigger_parent_entity_name = vc
     2 active_pathway_exists_ind = i2
 ) WITH protect
 SUBROUTINE getpathwaysbynomenclatureid(null)
   DECLARE start_tm = dq8 WITH constant(curtime3), private
   DECLARE ncntr = i4 WITH noconstant(0), protect
   DECLARE searchcntr = i4 WITH noconstant(0), protect
   DECLARE nindx = i4 WITH noconstant(0), protect
   CALL log_message("start getPathwaysByNomenclatureId()",log_level_debug)
   SELECT INTO "nl:"
    FROM cp_triggering_criteria ctc,
     cp_pathway cp
    PLAN (ctc
     WHERE expand(ncntr,1,conditionstocheck->nomen_cnt,ctc.triggering_entity_id,conditionstocheck->
      nomen_qual[ncntr].nomenclature_id)
      AND ctc.triggering_entity_name="NOMENCLATURE")
     JOIN (cp
     WHERE cp.cp_pathway_id=ctc.cp_pathway_id
      AND cp.pathway_status_cd=active_pw_status_cd
      AND cp.pathway_type_cd != quick_visit_pw_type_cd
      AND cp.active_ind=1)
    ORDER BY cp.cp_pathway_id
    HEAD cp.cp_pathway_id
     nindx = locateval(searchcntr,1,conditionstocheck->nomen_cnt,ctc.triggering_entity_id,
      conditionstocheck->nomen_qual[searchcntr].nomenclature_id),
     CALL addpathwaytosuggest("NOMENCLATURE",conditionstocheck->nomen_qual[nindx].nomenclature_id,
     conditionstocheck->nomen_qual[nindx].condition_display)
    FOOT REPORT
     stat = alterlist(pathwaystosuggest->qual,pathwaystosuggest->cnt)
    WITH nocounter, expand = 1
   ;end select
   CALL log_message(build("exit getPathwaysByNomenclatureId, elapsed time in seconds:",((curtime3 -
     start_tm)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getpathwaysbynomenconceptcki(null)
   DECLARE start_tm = dq8 WITH constant(curtime3), private
   DECLARE ncntr = i4 WITH noconstant(0), protect
   DECLARE searchcntr = i4 WITH noconstant(0), protect
   DECLARE nindx = i4 WITH noconstant(0), protect
   CALL log_message("start getPathwaysByNomenConceptCki()",log_level_debug)
   SELECT INTO "nl:"
    FROM cmt_concept_explode cce,
     cp_triggering_criteria ctc,
     cp_pathway cp
    PLAN (cce
     WHERE expand(ncntr,1,conditionstocheck->nomen_cnt,cce.child_concept_cki,conditionstocheck->
      nomen_qual[ncntr].concept_cki))
     JOIN (ctc
     WHERE ctc.triggering_criteria_ident=cce.parent_concept_cki)
     JOIN (cp
     WHERE cp.cp_pathway_id=ctc.cp_pathway_id
      AND cp.pathway_status_cd=active_pw_status_cd
      AND cp.pathway_type_cd != quick_visit_pw_type_cd
    ...
    ORDER BY cp.cp_pathway_id
    HEAD cp.cp_pathway_id
     nindx = locateval(searchcntr,1,conditionstocheck->nomen_cnt,ctc.triggering_criteria_ident,
      conditionstocheck->nomen_qual[searchcntr].concept_cki),
     CALL addpathwaytosuggest("NOMENCLATURE",conditionstocheck->nomen_qual[nindx].nomenclature_id,
     conditionstocheck->nomen_qual[nindx].condition_display)
    FOOT REPORT
     stat = alterlist(pathwaystosuggest->qual,pathwaystosuggest->cnt)
   ...
   SELECT INTO "nl:"
    FROM cp_triggering_criteria ctc,
     cp_pathway cp
    PLAN (ctc
     WHERE expand(ncntr,1,conditionstocheck->nomen_cnt,ctc.triggering_criteria_ident,
      conditionstocheck->nomen_qual[ncntr].concept_cki))
     JOIN (cp
     WHERE cp.cp_pathway_id=ctc.cp_pathway_id
      AND cp.pathway_status_cd=active_pw_status_cd
      AND cp.pathway_type_cd != quick_visit_pw_type_cd
    ...
    WITH nocounter, expand = 1
   ;end select
   CALL log_message(build("exit getPathwaysByNomenConceptCki, elapsed time in seconds:",((curtime3 -
     start_tm)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getpathwaysbyontologycondition(null)
   DECLARE start_tm = dq8 WITH constant(curtime3), private
   DECLARE ncntr = i4 WITH noconstant(0), protect
   DECLARE searchcntr = i4 WITH noconstant(0), protect
   DECLARE nindx = i4 WITH noconstant(0), protect
   CALL log_message("start getPathwaysByOntologyCondition()",log_level_debug)
   SELECT INTO "nl:"
    FROM cp_triggering_criteria ctc,
     cp_pathway cp
    PLAN (ctc
     WHERE expand(ncntr,1,conditionstocheck->ont_concept_cnt,ctc.triggering_criteria_ident,
      conditionstocheck->ont_concept_qual[ncntr].do_concept_name,
      ctc.triggering_criteria_group_mean,conditionstocheck->ont_concept_qual[ncntr].do_context_name))
     JOIN (cp
     WHERE cp.cp_pathway_id=ctc.cp_pathway_id
      AND cp.pathway_status_cd=active_pw_status_cd
      AND cp.pathway_type_cd != quick_visit_pw_type_cd
    ...
    ORDER BY cp.cp_pathway_id
    HEAD cp.cp_pathway_id
     nindx = locateval(searchcntr,1,conditionstocheck->ont_concept_cnt,ctc.triggering_criteria_ident,
      conditionstocheck->ont_concept_qual[searchcntr].do_concept_name,
      ctc.triggering_criteria_group_mean,conditionstocheck->ont_concept_qual[searchcntr].
      do_context_name),
     CALL addpathwaytosuggest("NOMENCLATURE",conditionstocheck->ont_concept_qual[nindx].
     nomenclature_id,conditionstocheck->ont_concept_qual[nindx].condition_display)
    FOOT REPORT
     stat = alterlist(pathwaystosuggest->qual,pathwaystosuggest->cnt)
    WITH nocounter, expand = 1
   ;end select
   CALL log_message(build("exit getPathwaysByOntologyCondition, elapsed time in seconds:",((curtime3
      - start_tm)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE (addpathwaytosuggest(trigger_parent_entity_name=vc,trigger_parent_entity_id=f8,
  trigger_display=vc) =null WITH protect)
   DECLARE start_tm = dq8 WITH constant(curtime3), private
   DECLARE search_cntr = i4 WITH noconstant(0), protect
   CALL log_message("start addPathwayToSuggest()",log_level_debug)
   IF (locateval(search_cntr,1,pathwaystosuggest->cnt,cp.cp_pathway_id,pathwaystosuggest->qual[
    search_cntr].cp_pathway_id)=0)
    SET pathwaystosuggest->cnt += 1
    IF ((pathwaystosuggest->cnt > size(pathwaystosuggest->qual,5)))
     SET stat = alterlist(pathwaystosuggest->qual,(pathwaystosuggest->cnt+ 10))
    ENDIF
    SET pathwaystosuggest->qual[pathwaystosuggest->cnt].cp_pathway_id = cp.cp_pathway_id
    SET pathwaystosuggest->qual[pathwaystosuggest->cnt].pathway_name = cp.pathway_name
    SET pathwaystosuggest->qual[pathwaystosuggest->cnt].criteria_type_mean = ctc.criteria_type_mean
    SET pathwaystosuggest->qual[pathwaystosuggest->cnt].trigger_display = trigger_display
    SET pathwaystosuggest->qual[pathwaystosuggest->cnt].trigger_parent_entity_name =
    trigger_parent_entity_name
    SET pathwaystosuggest->qual[pathwaystosuggest->cnt].trigger_parent_entity_id =
    trigger_parent_entity_id
   ENDIF
   CALL log_message(build("exit addPathwayToSuggest, elapsed time in seconds:",((curtime3 - start_tm)
 ...
 SUBROUTINE main(null)
   ...
   IF (textlen(pathwayname) > 0)
    SELECT INTO "nl:"
     FROM cp_pathway c
     WHERE c.pathway_name=pathwayname
      AND c.pathway_type_cd != quick_visit_pw_type_cd
     DETAIL
      pathwayid = c.cp_pathway_id
     WITH nocounter
    ;end select
    IF (curqual > 0)
     SET stat = alterlist(pathwaystosuggest->qual,1)
     SET pathwaystosuggest->cnt = 1
     SET pathwaystosuggest->qual[pathwaystosuggest->cnt].cp_pathway_id = pathwayid
     SET pathwaystosuggest->qual[pathwaystosuggest->cnt].pathway_name = pathwayname
     SET pathwaystosuggest->qual[pathwaystosuggest->cnt].criteria_type_mean = rule_type_mean
     IF (triggeringtext > " ")
      SET pathwaystosuggest->qual[pathwaystosuggest->cnt].trigger_display = triggeringtext
     ELSE
      SET pathwaystosuggest->qual[pathwaystosuggest->cnt].trigger_display = rulename
     ENDIF
     CALL log_message(build2("Found ",trim(cnvtstring(pathwaystosuggest->cnt),3),
       " pathways to suggest."),log_level_debug)
     SET log_misc1 = cnvtrectojson(pathwaystosuggest)
     SET retval = 100
    ELSE
     CALL log_message(build("Found no qualifying pathways to suggest."),log_level_debug)
     SET log_misc1 = ""
     SET retval = 0
    ENDIF
   ELSEIF (textlen(condition_json) > 0)
    SET stat = cnvtjsontorec(condition_json)
    CALL echorecord(conditionstocheck)
    CALL getpathwaysbynomenconceptcki(null)
    CALL getpathwaysbynomenclatureid(null)
    CALL getpathwaysbyontologycondition(null)
    IF ((pathwaystosuggest->cnt > 0))
     CALL log_message(build2("Found ",trim(cnvtstring(pathwaystosuggest->cnt),3),
       " pathways to suggest."),log_level_debug)
     SET log_misc1 = cnvtrectojson(pathwaystosuggest)
     SET retval = 100
...
#exit_script
 ...
 CALL log_message(build("Total time in seconds:",((curtime3 - script_start_tm)/ 100.0)),
  log_level_debug)
END GO
