CREATE PROGRAM cp_add_pathway_action:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Request JSON" = ""
  WITH outdev, request_json
 ...
 RECORD mpagereply(
   1 actions[*]
     ...
     2 action_dt_tm = dq8
     2 details[*]
       3 cp_pathway_action_detail_id = f8
       3 action_detail_entity_name = vc
       3 action_detail_entity_id = f8
       3 action_detail_text = vc
       3 cp_action_detail_type_cd = f8
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
 ...
 SUBROUTINE main(null)
   ...
   IF (size(actionreq->actions,5) > 0)
    CALL addpathwayaction(null)
   ELSE
    CALL reporterrorandexit("validate","actionReq is empty","0")
   ENDIF
   CALL displaylogtimermessage("end main()",start_tm)
 END ;Subroutine
 SUBROUTINE displaylogtimermessage(message,start_tm)
   CALL log_message(build(message," Elapsed time in seconds:",((curtime3 - start_tm)/ 100.0)),
    log_level_debug)
 END ;Subroutine
 SUBROUTINE reporterrorandexit(operationname,targetobjectname,targetobjectvalue)
   CALL log_message("start reportErrorAndExit()",log_level_debug)
   DECLARE start_tm = dq8 WITH constant(curtime3), private
   SET mpagereply->status_data.subeventstatus[1].operationname = operationname
   SET mpagereply->status_data.subeventstatus[1].operationstatus = "F"
   SET mpagereply->status_data.subeventstatus[1].targetobjectname = targetobjectname
   SET mpagereply->status_data.subeventstatus[1].targetobjectvalue = targetobjectvalue
   CALL displaylogtimermessage("end reportErrorAndExit()",start_tm)
   GO TO exit_script
 END ;Subroutine
 SUBROUTINE getidbysequence(seq_type,refrecord)
   CALL log_message("start GetIdBySequence()",log_level_debug)
   DECLARE start_tm = dq8 WITH constant(curtime3), private
   DECLARE new_id = f8 WITH noconstant(0.0), protect
   ...
   ENDIF
   IF (new_id=0.0)
    CALL reporterrorandexit("PERFORM","INSERT",build(" Unable to generate ",seq_type,"."))
   ENDIF
   CALL displaylogtimermessage("end GetIdBySequence()",start_tm)
   RETURN(new_id)
 END ;Subroutine
 SUBROUTINE addpathwayaction(null)
   CALL log_message("start addPathwayAction()",log_level_debug)
   DECLARE start_tm = dq8 WITH constant(curtime3), private
   DECLARE active_cd = f8 WITH constant(uar_get_code_by("MEANING",48,"ACTIVE")), protect
   DECLARE action_type_cd = f8 WITH noconstant(0), protect
   DECLARE action_detail_type_cd = f8 WITH noconstant(0), protect
   DECLARE new_pathway_action_id = f8 WITH noconstant(0), protect
   DECLARE new_pathway_action_detail_id = f8 WITH noconstant(0), protect
   ...
   FOR (x = 1 TO size(actionreq->actions,5))
     SELECT INTO "nl:"
      FROM cp_pathway_activity cpa
      PLAN (cpa
       WHERE (cpa.pathway_instance_id=actionreq->actions[x].pathway_instance_id))
      WITH nocounter
     ;end select
     CALL error_and_zero_check_rec(curqual,"CP_PATHWAY_ACTIVITY","UPDATE",1,1,
      mpagereply)
     SET action_type_cd = uar_get_code_by("MEANING",4003135,cnvtupper(actionreq->actions[x].
       action_type_mean))
     IF (action_type_cd <= 0)
      CALL reporterrorandexit("validate","invalid action type code","0")
     ENDIF
     IF ((actionreq->actions[x].action_dt_tm=0))
      SET actionreq->actions[x].action_dt_tm = cnvtdatetime(sysdate)
     ENDIF
     SET new_pathway_action_id = getidbysequence("MPAGES_SEQ",mpagereply)
     INSERT  FROM cp_pathway_action cpa
      SET cpa.action_dt_tm = cnvtdatetime(actionreq->actions[x].action_dt_tm), cpa.action_type_cd =
       action_type_cd, cpa.encntr_id = actionreq->actions[x].encntr_id,
       cpa.cp_node_id = actionreq->actions[x].cp_node_id, cpa.cp_pathway_action_id =
       new_pathway_action_id, cpa.pathway_instance_id = actionreq->actions[x].pathway_instance_id,
       cpa.prsnl_id = actionreq->actions[x].prsnl_id, cpa.cp_component_id = actionreq->actions[x].
       cp_component_id, cpa.treatment_line_cd = actionreq->actions[x].treatment_line_cd,
       cpa.updt_applctx = reqinfo->updt_applctx, cpa.updt_cnt = 0, cpa.updt_dt_tm = cnvtdatetime(
        sysdate),
       cpa.updt_id = reqinfo->updt_id, cpa.updt_task = reqinfo->updt_task
      WITH nocounter
     ;end insert
     CALL error_and_zero_check_rec(curqual,"CP_PATHWAY_ACTION","INSERT",1,1,
      mpagereply)
     SET stat = alterlist(mpagereply->actions,x)
     SET mpagereply->actions[x].cp_pathway_action_id = new_pathway_action_id
     SET mpagereply->actions[x].action_dt_tm = actionreq->actions[x].action_dt_tm
     SET mpagereply->actions[x].encntr_id = actionreq->actions[x].encntr_id
     SET mpagereply->actions[x].action_type_cd = action_type_cd
     SET mpagereply->actions[x].cp_node_id = actionreq->actions[x].cp_node_id
     SET mpagereply->actions[x].cp_component_id = actionreq->actions[x].cp_component_id
     SET mpagereply->actions[x].pathway_instance_id = actionreq->actions[x].pathway_instance_id
     SET mpagereply->actions[x].prsnl_id = actionreq->actions[x].prsnl_id
     SET mpagereply->actions[x].treatment_line_cd = actionreq->actions[x].treatment_line_cd
     FOR (y = 1 TO size(actionreq->actions[x].details,5))
       SET action_detail_type_cd = uar_get_code_by("MEANING",4003199,cnvtupper(actionreq->actions[x].
         details[y].cp_action_detail_type_mean))
       IF (action_detail_type_cd <= 0)
        CALL reporterrorandexit("validate","invalid action detail type code","0")
       ENDIF
       SET new_pathway_action_detail_id = getidbysequence("MPAGES_SEQ",mpagereply)
       IF ((actionreq->actions[x].details[y].action_detail_entity_name="LONG_TEXT"))
        SET long_text_id = getidbysequence("LONG_DATA_SEQ",mpagereply)
        INSERT  FROM long_text lt
         SET lt.active_ind = 1, lt.active_status_cd = active_cd, lt.active_status_dt_tm =
          cnvtdatetime(sysdate),
          lt.active_status_prsnl_id = actionreq->actions[x].prsnl_id, lt.long_text_id = long_text_id,
          lt.long_text = actionreq->actions[x].details[y].action_detail_entity_text,
          lt.parent_entity_name = "CP_PATHWAY_ACTION_DETAIL", lt.parent_entity_id =
          new_pathway_action_detail_id, lt.updt_applctx = reqinfo->updt_applctx,
          lt.updt_cnt = 0, lt.updt_dt_tm = cnvtdatetime(sysdate), lt.updt_id = reqinfo->updt_id,
          lt.updt_task = reqinfo->updt_task
         WITH nocounter
        ;end insert
        CALL error_and_zero_check_rec(curqual,"long text","unable to add new long text",1,1,
         mpagereply)
        SET actionreq->actions[x].details[y].action_detail_entity_id = long_text_id
       ENDIF
       INSERT  FROM cp_pathway_action_detail cpad
        SET cpad.action_detail_entity_name = actionreq->actions[x].details[y].
         action_detail_entity_name, cpad.action_detail_entity_id = actionreq->actions[x].details[y].
         action_detail_entity_id, cpad.action_detail_text = actionreq->actions[x].details[y].
         action_detail_text,
         cpad.cp_action_detail_type_cd = action_detail_type_cd, cpad.cp_pathway_action_id =
         new_pathway_action_id, cpad.cp_pathway_action_detail_id = new_pathway_action_detail_id,
         cpad.updt_applctx = reqinfo->updt_applctx, cpad.updt_cnt = 0, cpad.updt_dt_tm = cnvtdatetime
         (sysdate),
         cpad.updt_id = reqinfo->updt_id, cpad.updt_task = reqinfo->updt_task
        WITH nocounter
       ;end insert
       CALL error_and_zero_check_rec(curqual,"CP_PATHWAY_ACTION_DETAIL","INSERT",1,1,
        mpagereply)
       SET stat = alterlist(mpagereply->actions[x].details,y)
       SET mpagereply->actions[x].details[y].cp_action_detail_type_cd = action_detail_type_cd
       SET mpagereply->actions[x].details[y].action_detail_entity_id = actionreq->actions[x].details[
       y].action_detail_entity_id
       SET mpagereply->actions[x].details[y].action_detail_entity_name = actionreq->actions[x].
       details[y].action_detail_entity_name
       SET mpagereply->actions[x].details[y].cp_pathway_action_detail_id =
       new_pathway_action_detail_id
     ENDFOR
   ENDFOR
   CALL displaylogtimermessage("end addPathwayAction()",start_tm)
 END ;Subroutine
...
#exit_script
 ...
 ELSE
  COMMIT
 ENDIF
 CALL displaylogtimermessage(concat("End script: ",log_program_name),script_start_tm)
 IF (( $OUTDEV != "NOFORMS"))
