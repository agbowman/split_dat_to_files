CREATE PROGRAM dcp_rpt_missingmed_org:dba
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   ...
   MACRO (select_printer)
    IF (cnvtupper(facility)="BMC")
     ...
     "NNURD"))
      IF (curtime BETWEEN 0800 AND 1815)
       med_output_device = "bmchs4rxpo", iv_output_device = "bmchs4rxtpn"
      ELSE
       med_output_device = "bmchs4rxpo", iv_output_device = "bmchs4rxtpn"
      ENDIF
     ELSE
    ...
    ELSEIF (cnvtupper(facility) IN ("BWH"))
     med_output_device = "wmhgrrxpo", iv_output_device = "wmhgrrxtpn"
    ELSE
     med_output_device = "mlhstgrxpo", iv_output_device = "mlhstgrxtpn"
    ENDIF
