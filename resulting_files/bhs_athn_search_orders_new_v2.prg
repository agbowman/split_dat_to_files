CREATE PROGRAM bhs_athn_search_orders_new_v2
 FREE RECORD result
 RECORD result(
   1 search_phrase = vc
   1 birth_dt_tm = dq8
 ...
 WHILE (size(clincatcdparam) > 0)
   ...
   IF (startpos < endpos)
    ...
    SET clincatcdcnt = (clincatcdcnt+ 1)
...
#exit_script
   ...
   SELECT INTO value(moutputdevice)
    ...
    HEAD REPORT
     ...
     FOR (idx = 1 TO size(result->orderables,5))
       IF ((result->orderables[idx].oe_format_id > 0))
        ...
        row + 1, v2 = build("<Mnemonic>",trim(replace(replace(replace(replace(replace(result->
           ...
           "&quot;",0),3),"</Mnemonic>"), col + 1,
        v2, row + 1, v3 = build("<CatalogCD>",cnvtint(result->orderables[idx].catalog_cd),
         "</CatalogCD>"),
        col + 1, v3, row + 1,
        v4 = build("<CatalogTypeCD>",cnvtint(result->orderables[idx].catalog_type_cd),
         "</CatalogTypeCD>"), col + 1, v4,
        row + 1, v7 = build("<RefTextMask>",
         IF ((result->orderables[idx].text_type_cnt > 0)) "RefTextAvailable"
         ELSE "RefTextNotAvailable"
         ENDIF
         ,"</RefTextMask>"), col + 1,
        v7, row + 1, v8 = build("<OrderFormatId>",cnvtstring(result->orderables[idx].oe_format_id),
         "</OrderFormatId>"),
        col + 1, v8, row + 1,
        col + 1, "<Catalog>", row + 1,
        v9 = build("<Value>",cnvtstring(result->orderables[idx].catalog_cd),"</Value>"), col + 1, v9,
        row + 1, v10 = build("<Meaning>",trim(replace(replace(replace(replace(replace(
               uar_get_code_meaning(result->orderables[idx].catalog_cd),"&","&amp;",0),"<","&lt;",0),
             ">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),"</Meaning>"), col + 1,
        v10, row + 1, v11 = build("<Display>",trim(replace(replace(replace(replace(replace(replace(
                uar_get_code_display(result->orderables[idx].catalog_cd),"Â ","",0),"&","&amp;",0),
              "<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),"</Display>"),
        col + 1, v11, row + 1,
        col + 1, "</Catalog>", row + 1,
        col + 1, "<CatalogType>", row + 1,
        v12 = build("<Value>",cnvtstring(result->orderables[idx].catalog_type_cd),"</Value>"), col +
        1, v12,
        row + 1, v13 = build("<Meaning>",trim(replace(replace(replace(replace(replace(
               uar_get_code_meaning(result->orderables[idx].catalog_type_cd),"&","&amp;",0),"<",
              "&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),"</Meaning>"), col + 1,
        v13, row + 1, v14 = build("<Display>",trim(replace(replace(replace(replace(replace(replace(
                uar_get_code_display(result->orderables[idx].catalog_type_cd),"Â ","",0),"&",
               "&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),"</Display>"
         ),
        col + 1, v14, row + 1,
        col + 1, "</CatalogType>", row + 1,
        col + 1, "<ClinicalCategory>", row + 1,
        v15 = build("<Value>",cnvtstring(result->orderables[idx].dcp_clin_cat_cd),"</Value>"), col +
        1, v15,
        row + 1, v16 = build("<Meaning>",trim(replace(replace(replace(replace(replace(
               uar_get_code_meaning(result->orderables[idx].dcp_clin_cat_cd),"&","&amp;",0),"<",
              "&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),"</Meaning>"), col + 1,
        v16, row + 1, v17 = build("<Display>",trim(replace(replace(replace(replace(replace(replace(
                uar_get_code_display(result->orderables[idx].dcp_clin_cat_cd),"Â ","",0),"&",
               "&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),"</Display>"
         ),
        col + 1, v17, row + 1,
        col + 1, "</ClinicalCategory>", row + 1,
        col + 1, "<ActivityType>", row + 1,
        v18 = build("<Value>",cnvtstring(result->orderables[idx].activity_type_cd),"</Value>"), col
         + 1, v18,
        row + 1, v19 = build("<Meaning>",trim(replace(replace(replace(replace(replace(
               uar_get_code_meaning(result->orderables[idx].activity_type_cd),"&","&amp;",0),"<",
              "&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),"</Meaning>"), col + 1,
        v19, row + 1, v20 = build("<Display>",trim(replace(replace(replace(replace(replace(replace(
                uar_get_code_display(result->orderables[idx].activity_type_cd),"Â ","",0),"&",
               "&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),"</Display>"
         ),
        col + 1, v20, row + 1,
        col + 1, "</ActivityType>", row + 1,
        col + 1, "<ActivitySubTypeCD>", row + 1,
        v21 = build("<Value>",cnvtstring(result->orderables[idx].activity_subtype_cd),"</Value>"),
        col + 1, v21,
        row + 1, v22 = build("<Meaning>",trim(replace(replace(replace(replace(replace(
               uar_get_code_meaning(result->orderables[idx].activity_subtype_cd),"&","&amp;",0),"<",
              "&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),"</Meaning>"), col + 1,
        v22, row + 1, v23 = build("<Display>",trim(replace(replace(replace(replace(replace(replace(
 ...
 SUBROUTINE callgetsearch(null)
   ...
   IF (( $9=0)
    ...
    SET req680220->pharmacy_type_inclusions[1].sliding_scale_ind = 1
    SET req680220->pharmacy_type_inclusions[1].tapered_dose_ind = 1
