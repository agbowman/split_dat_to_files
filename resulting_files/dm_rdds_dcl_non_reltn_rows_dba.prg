CREATE PROGRAM dm_rdds_dcl_non_reltn_rows:dba
 ...
 IF (v_err_code2 != 0)
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"CHILDENVERR",
   "Error obtaining list of child environment IDs: %1","s",nullterm(v_errmsg2))
  GO TO exit_main
 ENDIF
 IF ((d_envs->cnt=0))
 ...
 IF (v_archi_version >= 2.0)
  SET v_parser_stmt = sbr_getrowidnotexists(
   "expand(v_idx2,1,d_envs->cnt,dcl.target_env_id,d_envs->qual[v_idx2].env_id)","dcl")
 ELSE
  SET v_parser_stmt = "expand(v_idx2,1,d_envs->cnt,dcl.target_env_id,d_envs->qual[v_idx2].env_id)"
 ENDIF
 SELECT INTO "nl:"
  dcl.rowid
  FROM dm_chg_log dcl
  ...
  WITH nocounter, maxqual(dcl,value(request->max_rows))
 ;end select
 SET v_err_code2 = error(v_errmsg2,1)
 IF (v_err_code2 != 0)
  SET reply->status_data.status = "F"
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"COLLECTERR",
   "Error collectiong purgeable rows: %1","s",nullterm(v_errmsg2))
  GO TO exit_main
 ENDIF
 SET reply->status_data.status = "S"
 SET reply->err_code = 0
