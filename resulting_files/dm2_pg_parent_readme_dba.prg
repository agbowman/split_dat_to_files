CREATE PROGRAM dm2_pg_parent_readme:dba
 ...
 DECLARE errmsg = vc WITH protect, noconstant("")
 DECLARE mf_updt_id_tag = f8 WITH protect, noconstant(1.1)
 DECLARE ml_child_size = i4 WITH protect, noconstant(0)
 DECLARE ml_maxqual_size = i4 WITH protect, noconstant(5000)
 DECLARE ml_curqual = i4 WITH protect, noconstant(0)
 DECLARE ml_invalid_row_count = i4 WITH protect, noconstant(0)
 DECLARE errmsg = vc WITH protect, noconstant("")
 SET ms_info_domain_nm = "UPDATE EPR'S ENCNTR_TYPE_CD"
 SET ms_range_prefix = "ENCNTR_PRSNL_RELTN_ID RANGE EVALUATED"
 SET mn_num_children = 3
 EXECUTE dm_drop_obsolete_objects "EPR_VIEW_TRIGGER_1", "TRIGGER", 1
 IF (error(errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to drop trigger EPR_VIEW_TRIGGER_1",errmsg)
  GO TO exit_program
 ENDIF
 EXECUTE dm_drop_obsolete_objects "EPR_VIEW_TRIGGER_2", "TRIGGER", 1
 IF (error(errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to drop trigger EPR_VIEW_TRIGGER_2",errmsg)
  GO TO exit_program
 ENDIF
 IF (currdb="ORACLE")
  EXECUTE dm_readme_include_sql "cer_install:dm2_provider_group_trig_tmp.sql"
  EXECUTE dm_readme_include_sql_chk "TEMP_README_TRIG1", "trigger"
  SET readme_data->status = dm_sql_reply->status
  IF ((readme_data->status="F"))
   SET readme_data->message = concat("Failed to build trigger TEMP_README_TRIG1: ",dm_sql_reply->msg)
   GO TO exit_program
  ENDIF
  EXECUTE dm_readme_include_sql_chk "TEMP_README_TRIG2", "trigger"
  SET readme_data->status = dm_sql_reply->status
  IF ((readme_data->status="F"))
   SET readme_data->message = concat("Failed to build trigger TEMP_README_TRIG2: ",dm_sql_reply->msg)
   GO TO exit_program
  ENDIF
 ELSE
  SET readme_data->status = "S"
  SET readme_data->message = "Auto-Success: Readme will only execute in an Oracle Database"
  GO TO exit_program
 ENDIF
 SET mf_min_range_id = 1.0
 SELECT INTO "nl:"
  themax = max(encntr_prsnl_reltn_id)
  FROM encntr_prsnl_reltn
  WHERE encntr_prsnl_reltn_id > 0
  DETAIL
   mf_max_range_id = themax
  WITH nocounter
 ;end select
 IF (error(errmsg,0) != 0)
  SET readme_data->status = "F"
 ...
 IF (sbr_run_parent(mf_max_range_id,mf_min_range_id,ms_range_prefix,ms_info_domain_nm,mn_num_children
  ...
  GO TO exit_program
 ENDIF
 SET ml_curqual = ml_maxqual_size
 WHILE (ml_curqual=ml_maxqual_size)
   UPDATE  FROM encntr_prsnl_reltn epr
    SET epr.updt_id = mf_updt_id_tag
    WHERE epr.updt_id != mf_updt_id_tag
     AND  NOT ( EXISTS (
    (SELECT
     e.encntr_id
     FROM encounter e
     WHERE epr.encntr_id=e.encntr_id)))
    WITH maxqual(epr,value(ml_maxqual_size))
   ;end update
   IF (error(errmsg,0) != 0)
    ROLLBACK
    SET readme_data->status = "F"
    SET readme_data->message = concat("Failed to update orphaned rows on the EPR table:",errmsg)
    GO TO exit_program
   ENDIF
   COMMIT
   SET ml_invalid_row_count += curqual
   SET ml_curqual = curqual
 ENDWHILE
 CALL echo(build("There Are....",ml_invalid_row_count,
   "....Orphaned Rows on the ENCNTR_PRSNL_RELTN table!"))
 SET readme_data->status = "S"
