CREATE PROGRAM bhs_req_adt2
 EXECUTE bhs_sys_stand_subroutine
 CALL echo("**************************************************")
 CALL echo("begin script bhs_req_adt2")
 CALL echo("**************************************************")
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   ...
   IF (e.loc_facility_cd IN (bmc_psych_cd, bmc_cd))
    bmc_flag = 1
   ENDIF
 ...
 IF ((orders->spoolout_ind=1))
  CALL echo(build2("name: ",orders->name))
  CALL echo(build2("orders: ",size(orders->qual,5)))
  ...
  IF (bmc_flag=1)
   ...
   ;end select
   SET subject = concat(s_primarymnemonic," - ",orders->name)
   SET dclcom = concat('mail -s "',subject,'" joe.echols@bhs.org < ',bmcreqfile)
