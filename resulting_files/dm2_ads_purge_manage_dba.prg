CREATE PROGRAM dm2_ads_purge_manage:dba
 ...
 SUBROUTINE ds_scramble(null)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_ads_purge_mng",".log","dm2_ads_purge_manage")=0)
  GO TO exit_program
 ENDIF
 FREE RECORD dapmip_reply
 RECORD dapmip_reply(
   1 status = c1
   1 status_msg = vc
 )
 DECLARE dapm_config_status = vc WITH protect, noconstant("")
 DECLARE dapm_config_name = vc WITH protect, noconstant("")
 DECLARE dapm_config_updt_dt_tm = f8 WITH protect, noconstant(0)
 DECLARE dapm_config_exists = i2 WITH protect, noconstant(0)
 DECLARE dapm_option = i2 WITH protect, noconstant(0)
 DECLARE dapm_continue = i2 WITH protect, noconstant(1)
 DECLARE dapm_csv_dir_name = vc WITH protect, noconstant("")
 DECLARE dapm_csv_file_name = vc WITH protect, noconstant("")
 DECLARE dapm_dac_id = f8 WITH protect, noconstant(0)
 DECLARE dapm_name_in_use = i2 WITH protect, noconstant(0)
 DECLARE dapm_header(prof_stat=vc,prof_msg=vc) = i2
 DECLARE dapm_ads_pid_rpt(null) = i2
 SET dapm_continue = 1
 WHILE (dapm_continue=1)
   SET width = 132
   SET message = window
   SET dm_err->eproc = "Retrieving ADS Purge configuration information"
   SELECT INTO "nl:"
    FROM dm_ads_config d
    WHERE d.sample_method=dpl_purge
    DETAIL
     dapm_config_status = d.config_status, dapm_config_name = d.config_name, dapm_config_updt_dt_tm
      = d.updt_dt_tm,
     dads_list->config_name = trim(d.config_name), dads_list->config_pct = d.sample_percent_nbr,
     dads_list->config_method = trim(d.sample_method),
     dads_list->config_status = d.config_status, dads_list->config_updt_dt_tm = cnvtdatetime(d
      .updt_dt_tm), dads_list->pct_override = 0,
     dads_list->method_override = "", dads_list->config_id = d.dm_ads_config_id
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual > 0)
    SET dapm_config_exists = 1
   ELSE
    SET dapm_config_exists = 0
   ENDIF
   IF (dapm_header(dapm_config_status,"")=0)
    GO TO exit_program
   ENDIF
   IF (dapm_config_exists=1)
    IF (dam_driverkey_menu(null)=0)
     GO TO exit_program
    ENDIF
   ELSE
    IF (dapm_name_in_use=1)
     CALL text(12,5,"Configuration name in use, please choose another name")
    ELSE
     CALL text(12,5,"No current ADS Purge configuration found ")
    ENDIF
    CALL text(14,5,"1. Create NEW configuration")
    CALL text(18,5,"Your Selection (0 to Exit)? ")
    CALL accept(18,33,"9",0
     WHERE curaccept IN (0, 1))
    SET dapm_option = curaccept
    CASE (dapm_option)
     OF 1:
      ...
      ENDIF
      CALL clear(12,5,60)
      CALL accept(8,18,"P(15);CU")
      SET dapm_config_name = curaccept
      SET dm_err->eproc = "Determining if an ADS Purge configuration exists"
      SELECT INTO "nl:"
       FROM dm_ads_config d
       WHERE d.config_name=dapm_config_name
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      ...
      ENDIF
      IF (curqual > 0)
       SET dapm_name_in_use = 1
      ELSE
       SET dapm_config_status = "NEEDSBUILD"
       SET dapm_config_exists = 1
       SELECT INTO "nl:"
        x = seq(dm_clinical_seq,nextval)
        FROM dual
        DETAIL
         dapm_dac_id = cnvtreal(x)
        WITH nocounter
       ;end select
       SET dm_err->eproc = "Creating new ADS Purge configuration"
       INSERT  FROM dm_ads_config d
        SET d.dm_ads_config_id = dapm_dac_id, d.config_name = dapm_config_name, d.config_status =
         dapm_config_status,
         d.sample_method = dpl_purge, d.updt_id = reqinfo->updt_id, d.updt_dt_tm = cnvtdatetime(
          curdate,curtime3),
       ...
       ENDIF
       COMMIT
       SET dads_list->config_id = dapm_dac_id
       SET dm_err->eproc = "Retrieving ADS Purge configuration information"
       SELECT INTO "nl:"
        FROM dm_ads_config dac
        WHERE (dac.dm_ads_config_id=dads_list->config_id)
        DETAIL
         dads_list->config_name = trim(dac.config_name), dads_list->config_pct = dac
         .sample_percent_nbr, dads_list->config_method = trim(dac.sample_method),
         dads_list->config_status = dac.config_status, dads_list->config_updt_dt_tm = cnvtdatetime(
          dac.updt_dt_tm), dads_list->pct_override = 0,
         dads_list->method_override = ""
        WITH nocounter
       ;end select
       IF (check_error(dm_err->eproc)=1)
 ...
 SUBROUTINE dam_driverkey_menu(null)
   DECLARE dapm_continue_loop = i2 WITH protect, noconstant(1)
   DECLARE dapm_continue_loop2 = i2 WITH protect, noconstant(0)
   DECLARE dapm_csv_dir_name = vc WITH protect, noconstant("")
   DECLARE dapm_csv_file_name = vc WITH protect, noconstant("")
   DECLARE dapm_dkey_build_ind = i2 WITH protect, noconstant(0)
   SET dm_err->eproc = "Driver Table Key Generation Menu"
   WHILE (dapm_continue_loop=1)
     SET message = window
     SELECT INTO "nl:"
      FROM dm_ads_config d
      WHERE d.sample_method=dpl_purge
      DETAIL
       dapm_config_status = d.config_status
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     IF (dapm_header(dapm_config_status,"")=0)
      GO TO exit_program
     ENDIF
     SET dm_err->eproc = "Determining if ADS Purge driverkeys have been built"
     SELECT INTO "nl:"
      FROM dm_ads_config_extract d
      WHERE (d.dm_ads_config_id=dads_list->config_id)
      DETAIL
       dapm_dkey_build_ind = 1
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_program
     ENDIF
     CALL text(13,5,build("1. Import Driver Table Keys via CSV File ",evaluate(dapm_config_status,
        "REPLICATE_RUNNING","  **Unavailable**","")))
     CALL text(14,5,"2. Export Driver Table Keys to CSV File ")
     CALL text(19,5,"Your Selection (0 to Exit)? ")
     IF (dapm_dkey_build_ind=1)
      CALL text(15,5,"3. View PERSON_IDs to Purge Report ")
      CALL text(16,5,build("4. Confirm Current List of PERSON_IDs and proceed to Replicate step",
        evaluate(dapm_config_status,"REPLICATE_RUNNING","  **Unavailable**","")))
      CALL accept(19,34,"9;",0
     ...
     CASE (curaccept)
      OF 0:
       SET dapm_continue_loop = 0
       SET dapm_continue = 0
       IF (dapm_dkey_build_ind=1)
        SET dm_err->eproc = "Updating current ADS Purge configuration to NEEDSBUILD"
        UPDATE  FROM dm_ads_config dac
         SET dac.config_status = "NEEDSBUILD"
         WHERE (dac.dm_ads_config_id=dads_list->config_id)
         WITH nocounter
        ;end update
        IF (check_error(dm_err->eproc)=1)
       ...
       ENDIF
      OF 1:
       IF (dapm_config_status != "REPLICATE_RUNNING")
        SET dapm_imp_continue = 1
        WHILE (dapm_imp_continue=1)
          IF (dapm_header(dapm_config_status,"")=0)
           GO TO exit_program
          ENDIF
          CALL text(13,5,"CSV File format:  TABLE_NAME, DRIVER_KEYCOL_NAME, DRIVER_KEY_ID")
          CALL text(15,5,"Please provide an absolute directory path (i.e. without OS logicals)")
          CALL text(16,5,"Directory:")
          CALL text(17,5,"A file with extension *.csv is required for CSV Importing")
          CALL text(18,5,"File Name:")
          CALL accept(16,17,"P(80);C",dapm_csv_dir_name
           WHERE curaccept > " ")
          SET dapm_csv_dir_name = curaccept
          CALL accept(18,17,"P(30);C",dapm_csv_file_name
           WHERE curaccept > " ")
          SET dapm_csv_file_name = curaccept
          IF (findstring(".csv",dapm_csv_file_name)=0)
           SET dapm_csv_file_name = concat(dapm_csv_file_name,".csv")
          ENDIF
          CALL text(20,5,"Is this information correct?  (Y)es, (N)o, e(X)it ")
          CALL accept(20,56,"A;CU","X"
           ...
           OF "Y":
            IF (dapm_dkey_build_ind=1)
             ...
             IF ((xxcclseclogin->loggedin=1))
              SET dm_err->eproc = "Reactivating PERSON_IDs using DM2_ADS_PURGE_ACT_INACT_PERSON"
              EXECUTE dm2_ads_purge_act_inact_person dads_list->config_id, 1 WITH replace("REPLY",
               "DAPMIP_REPLY")
              IF (check_error(dm_err->eproc)=1)
               CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
               ROLLBACK
               GO TO exit_program
              ENDIF
              IF ((dapmip_reply->status="F"))
               SET dm_err->err_ind = 1
               CALL disp_msg(dapmip_reply->status_msg,dm_err->logfile,1)
               ROLLBACK
               GO TO exit_program
              ENDIF
             ENDIF
            ENDIF
            IF (dam_import_csv(dapm_csv_dir_name,dapm_csv_file_name)=0)
             RETURN(0)
            ENDIF
            DELETE  FROM dm_ads_config_driver dac
             WHERE (dac.dm_ads_config_id=dads_list->config_id)
              AND  EXISTS (
             (SELECT
              "x"
              FROM person p
              WHERE p.name_last_key IN ("SYSTEM", "SYSTEMOE", "CERNER")
               AND p.person_id=dac.driver_key_id))
             WITH nocounter
            ;end delete
            SET dads_list->preserve_ind = 0
            SET dm_err->eproc = "Executing DM2_ADS_DRIVERKEY_GEN"
            ...
            ENDIF
            COMMIT
            SET dapm_imp_continue = 0
           OF "N":
            SET dapm_continue_loop = 0
           OF "X":
            SET dapm_imp_continue = 0
            SET dapm_csv_file_name = ""
            SET dapm_csv_dir_name = ""
          ENDCASE
        ENDWHILE
       ENDIF
      OF 2:
       SET dapm_exp_continue = 1
       WHILE (dapm_exp_continue=1)
         IF (dapm_header(dapm_config_status,"")=0)
          GO TO exit_program
         ENDIF
         CALL text(13,5,"CSV File format:  TABLE_NAME, DRIVER_KEYCOL_NAME, DRIVER_KEY_ID")
         ...
         CASE (curaccept)
          OF "Y":
           IF (dam_export_csv(dapm_csv_dir_name,dapm_csv_file_name)=0)
            RETURN(0)
           ENDIF
           SET dapm_exp_continue = 0
          OF "N":
           SET dapm_continue_loop = 0
          OF "X":
           SET dapm_exp_continue = 0
       ...
       ENDWHILE
      OF 3:
       IF (dapm_ads_pid_rpt(null)=0)
        GO TO exit_program
       ENDIF
      OF 4:
       IF (dapm_config_status != "REPLICATE_RUNNING")
        IF (dapm_dkey_build_ind=1)
         CALL text(20,5,"This will inactivate all PERSON_ID values in the current ADS Purge list")
         CALL text(21,5,"Continue ?  (Y)es, (N)o")
         CALL accept(21,30,"A;CU","N"
          WHERE curaccept IN ("Y", "N"))
         CASE (curaccept)
          OF "Y":
           IF ((xxcclseclogin->loggedin=0))
           ...
           ENDIF
           IF ((xxcclseclogin->loggedin=1))
            SET dm_err->eproc = "Inactivating current PERSON_ID list for ADS Purge"
            EXECUTE dm2_ads_purge_act_inact_person dads_list->config_id, 0
            IF (check_error(dm_err->eproc)=1)
             CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
             ROLLBACK
             GO TO exit_program
            ENDIF
            SET dm_err->eproc = "Updating current ADS Purge configuration as COMPLETE"
            UPDATE  FROM dm_ads_config dac
             SET dac.config_status = "COMPLETE"
             WHERE (dac.dm_ads_config_id=dads_list->config_id)
             WITH nocounter
            ;end update
            IF (check_error(dm_err->eproc)=1)
 ...
 SUBROUTINE dam_export_csv(dec_dir_name,dec_file_name)
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     ...
     dec_driverkeys->qual[dec_driverkeys->cnt].driver_key_id = build(cnvtint(dacd.driver_key_id))
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
 ...
 SUBROUTINE dam_import_csv(dic_dir_name,dic_file_name)
   ...
   IF (dm2_findfile(dic_file)=0)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Storing away ADS Purge csv information"
   UPDATE  FROM dm_info d
    SET d.info_name = dic_file
    WHERE d.info_domain="ADS DELETE CSV NAME"
    WITH nocounter
   ;end update
   IF (curqual=0)
    INSERT  FROM dm_info d
     SET d.info_domain = "ADS DELETE CSV NAME", d.info_name = dic_file
     WITH nocounter
    ;end insert
   ENDIF
   IF (check_error(dm_err->eproc)=1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dapm_ads_pid_rpt(null)
   SET message = nowindow
   SET dm_err->eproc = "Retrieving PERON_IDs for ADS Purge report"
   SELECT INTO mine
    FROM dm_ads_config_driver d
    WHERE (d.dm_ads_config_id=dads_list->config_id)
    HEAD REPORT
     CALL print(fillstring(250,"-")), row + 1, col 0,
     CALL print("ACTIVITY DATA SAMPLE PURGE [PURGE ENTITY DETAIL REPORT]"), col 80,
     CALL print(build("REPORT DATE/TIME:",format(cnvtdatetime(curdate,curtime3),
       "DD-MMM-YYYY HH:MM:SS;;D"))),
     row + 1,
     CALL print(fillstring(250,"-")), row + 1,
     col 0, "SAMPLE NAME: ", col 15,
     CALL print(trim(dapm_config_name)), row + 1,
     CALL print(fillstring(250,"-")),
     row + 1, row + 1, col 0,
     "TABLE NAME", col 40, "| VALUE ",
     col 60, row + 1,
     CALL print(fillstring(250,"-")),
     row + 1
    DETAIL
     col 0, "PERSON", col 40,
     "| ", d.driver_key_id, row + 1
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dapm_header(prof_stat,prof_msg)
   CALL clear(1,1)
   CALL box(1,1,3,132)
   CALL box(1,1,24,132)
   CALL text(2,2,"ACTIVITY DATA SAMPLE (ADS) PURGE - MANAGE SAMPLE")
   IF (prof_stat < "")
    SET dm_err->eproc = "Retrieving current ADS Purge config status"
    SELECT INTO "nl:"
     FROM dm_ads_config d
     WHERE (d.dm_ads_config_id=dads_list->config_id)
     DETAIL
      prof_stat = d.config_status
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
   ENDIF
   IF (dapm_config_exists=0)
    SET prof_stat = "PROFILE_SETUP"
   ENDIF
   IF (((prof_stat="PROFILE_SETUP") OR (prof_stat="NEEDSBUILD")) )
    CALL video(r)
    CALL text(5,3,"*")
    CALL text(5,15,"*")
    CALL text(5,4,"   SETUP   ")
   ...
   CALL box(4,2,6,16)
   CALL video(n)
   IF (((prof_stat="REPLICATE_READY") OR (prof_stat="COMPLETE")) )
    CALL video(r)
    CALL text(5,19,"*")
    CALL text(5,31,"*")
    CALL text(5,20," REPLICATE ")
   ELSE
    CALL text(5,19,"  REPLICATE  ")
   ENDIF
   IF (prof_stat="REPLICATE_RUNNING")
    CALL video(r)
    CALL text(5,19,"*")
    CALL text(5,31,"*")
    CALL text(5,20," REPLICATE ")
   ...
   CALL box(4,18,6,32)
   CALL video(n)
   CALL text(5,35,"    PURGE    ")
   CALL box(4,34,6,48)
   CALL video(n)
   CALL text(9,2,"Sample Status: ")
   CALL text(8,2,"Sample Name: ")
   CALL text(8,18,dapm_config_name)
   CALL text(9,18,evaluate(prof_stat,"COMPLETE","REPLICATE READY",prof_stat))
   RETURN(1)
 END ;Subroutine
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "LEAVING dm2_ads_purge_manage"
 CALL final_disp_msg("dm2_ads_purge_mng")
END GO
