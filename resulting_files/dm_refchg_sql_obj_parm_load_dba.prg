CREATE PROGRAM dm_refchg_sql_obj_parm_load:dba
 ...
 SUBROUTINE check_open_event(cur_env_id,paired_env_id)
   ...
   RETURN(coe_event_flg)
 END ;Subroutine
 DECLARE dsop_file_name = vc WITH protect, noconstant("")
 DECLARE dsop_cur_id = f8 WITH protect, noconstant(0.0)
 DECLARE dsop_oe_ind = i4 WITH protect, noconstant(0)
 DECLARE dsop_parse_str = vc WITH protect, noconstant(" ")
 DECLARE dsop_cnt = i4 WITH protect, noconstant(0)
 SET readme_data->status = "F"
 SET readme_data->message = "Starting dm_refchg_sql_obj_parm_load..."
 IF (check_logfile("dm_sql_parm_load",".log","dm_sql_parm_load LOG FILE...") != 1)
  SET dm_err->err_ind = 1
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dsop_oe_ind = check_open_event(dsop_cur_id,0.0)
 IF (check_error(dm_err->eproc)=1)
  SET readme_data->status = "F"
 ...
 IF (validate(dwtsl_object_name,"ABC") != "ABC"
  ...
  IF (dwtsl_object_name > " ")
   SET dsop_parse_str = " requestin->list_0[d.seq].object_name = patstring(dwtsl_object_name)"
   SET dsop_cnt = 1
  ELSE
   SET dsop_parse_str = " 1 = 1"
  ENDIF
 ELSE
  SET dsop_parse_str = " 1 = 1"
 ENDIF
 IF (((dsop_oe_ind=0) OR (dsop_cnt=1)) )
  SET dm_err->eproc = "Starting dm_refchg_sql_obj_parm_load..."
  CALL disp_msg("",dm_err->logfile,0)
  SET dsop_file_name = "cer_install:dm_refchg_sql_obj_parm.csv"
  INSERT  FROM dm_refchg_sql_obj_parm dsop,
    (dummyt d  WITH seq = size(requestin->list_0,5))
   SET dsop.dm_refchg_sql_obj_parm_id = seq(dm_clinical_seq,nextval), dsop.object_name = requestin->
    list_0[d.seq].object_name, dsop.column_name = requestin->list_0[d.seq].column_name,
    dsop.parm_nbr = cnvtint(requestin->list_0[d.seq].parm_nbr), dsop.active_ind = cnvtint(requestin->
     list_0[d.seq].active_ind), dsop.updt_id = reqinfo->updt_id,
    dsop.updt_dt_tm = cnvtdatetime(curdate,curtime3), dsop.updt_applctx = reqinfo->updt_applctx, dsop
    .updt_cnt = 0,
    dsop.updt_task = reqinfo->updt_task
   PLAN (d
    WHERE parser(dsop_parse_str))
    JOIN (dsop)
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
   SET readme_data->status = "F"
   SET readme_data->message = concat("Error inserting into dm_refchg_sql_obj_parm:",dm_err->emsg)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ELSE
   COMMIT
  ENDIF
  SELECT INTO "nl:"
   FROM dm_refchg_sql_obj_parm dsop,
    (dummyt d  WITH seq = size(requestin->list_0,5))
   PLAN (d
    WHERE parser(dsop_parse_str))
    JOIN (dsop
    WHERE (dsop.object_name=requestin->list_0[d.seq].object_name))
   WITH outerjoin = d, dontexist, nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   SET readme_data->status = "F"
   SET readme_data->message = concat("Error validating data on dm_refchg_sql_obj_parm:",dm_err->emsg)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  IF (curqual=0)
   SET readme_data->status = "S"
   SET readme_data->message = concat("Dm_refchg_sql_obj_parm data successfully loaded.")
  ELSE
   SET readme_data->status = "F"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SET readme_data->status = "S"
  SET readme_data->message = "DM_REFCHG_SQL_OBJ* Tables successfully loaded."
 ELSE
  SET readme_data->status = "S"
