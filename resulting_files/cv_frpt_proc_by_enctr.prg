CREATE PROGRAM cv_frpt_proc_by_enctr
 PROMPT
 ...
 IF (validate(i18nuar_def,999)=999)
  ...
  persist
 ENDIF
 EXECUTE cv_frpt_proc_by_enctr_drv "NL:",  $START_DATE,  $END_DATE,
  $ORG_ID
 ...
 DECLARE headenctrdispsection(ncalc=i2) = f8 WITH protect
 DECLARE headenctrdispsectionabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE detailsection(ncalc=i2) = f8 WITH protect
 DECLARE detailsectionabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE footenctrdispsection(ncalc=i2) = f8 WITH protect
 DECLARE footenctrdispsectionabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 ...
 SUBROUTINE get_reply_obj(dummy)
  ...
  ENDIF
  SELECT
   plist_proc_catalog_disp = substring(1,60,reply_obj->plist[d1.seq].proc_catalog_disp),
   plist_proc_encntr = substring(1,60,reply_obj->plist[d1.seq].proc_encntr), plist_proc_encntr_cd =
   reply_obj->plist[d1.seq].proc_encntr_cd,
   plist_proc_birthday = reply_obj->plist[d1.seq].proc_birthday, plist_proc_age = reply_obj->plist[d1
   .seq].proc_age, plist_proc_reg_dt_tm = reply_obj->plist[d1.seq].proc_reg_dt_tm
   FROM (dummyt d1  WITH seq = value(size(reply_obj->plist,5)))
   PLAN (d1)
   ORDER BY plist_proc_catalog_disp, plist_proc_encntr
   HEAD REPORT
    _d0 = plist_proc_catalog_disp, _d1 = plist_proc_encntr, _d2 = plist_proc_encntr_cd,
    ...
    ped_cnt = 0, ped_total = 0, adult_cnt = 0,
    adult_total = 0, tgraph = 0, max_tgraph = 0,
    graph_max = 0, p_curr = 0, p_max = 0,
   ...
   HEAD plist_proc_catalog_disp
    _fdrawheight = headcatalogdispsection(rpt_calcheight)
    IF (((_yoffset+ _fdrawheight) > _fenddetail))
     BREAK
    ENDIF
    dummy_val = headcatalogdispsection(rpt_render)
   HEAD plist_proc_encntr
    _fdrawheight = headenctrdispsection(rpt_calcheight)
    IF (((_yoffset+ _fdrawheight) > _fenddetail))
     BREAK
    ENDIF
    dummy_val = headenctrdispsection(rpt_render)
   DETAIL
    IF (18 <= cnvtint(substring(1,3,plist_proc_age)))
   ...
   FOOT  plist_proc_encntr
    ped_total = (ped_cnt+ ped_total), adult_total = (adult_cnt+ adult_total), _fdrawheight =
    footenctrdispsection(rpt_calcheight)
    IF (((_yoffset+ _fdrawheight) > _fenddetail))
     BREAK
    ENDIF
    dummy_val = footenctrdispsection(rpt_render), ped_cnt = 0, adult_cnt = 0
   FOOT  plist_proc_catalog_disp
    tgraph = count(plist_proc_catalog_disp), p_curr = count(plist_proc_catalog_disp)
    IF (p_curr >= p_max)
     p_max = p_curr
    ENDIF
    ...
    IF (((_yoffset+ _fdrawheight) > _fenddetail))
     BREAK
    ENDIF
    dummy_val = footcatalogdispsection(rpt_render), ped_total = 0, adult_total = 0,
    stat = alterlist(rptgraphrec->m_labels,_nfieldscnt), rptgraphrec->m_labels[_nfieldscnt].label =
    build2(plist_proc_catalog_disp,char(0)), stat = alterlist(rptgraphrec->m_series[1].y_values,
     _nfieldscnt),
 ...
 SUBROUTINE headreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.188),(offsety+ 1.380),(offsetx+ 7.001),(offsety+
     1.380))
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build2(uar_i18ngetmessage(_hi18nhandle,
        "PROCEDURESBYENCOUNTERTYPE","Procedures by Encounter Type")),char(0)))
 ...
 SUBROUTINE headcatalogdispsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(plist_proc_catalog_disp,char(0)))
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "PEDIATRICS","Pediatrics"),char(0)))
    SET rptsd->m_y = (offsety+ 0.094)
    SET rptsd->m_x = (offsetx+ 5.250)
    SET rptsd->m_width = 0.990
    SET rptsd->m_height = 0.260
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "ADULTS","Adults"),char(0)))
    SET rptsd->m_y = (offsety+ 0.094)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE headenctrdispsection(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headenctrdispsectionabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE headenctrdispsectionabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.060000), private
   IF ( NOT (((( $U_VIEW=1)) OR (( $U_VIEW=3))) ))
    RETURN(0.0)
   ENDIF
 ...
 SUBROUTINE detailsectionabs(ncalc,offsetx,offsety)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE footenctrdispsection(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = footenctrdispsectionabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE footenctrdispsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
      IF (plist_proc_encntr_cd=0.0) "Unknown"
      ELSE plist_proc_encntr
      ENDIF
      ,char(0)))
    SET rptsd->m_flags = 64
    SET rptsd->m_y = (offsety+ 0.000)
    ...
    SET rptsd->m_height = 0.188
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(cnvtstring(ped_cnt)),char(0)))
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 5.000)
    SET rptsd->m_width = 0.563
    SET rptsd->m_height = 0.188
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(cnvtstring(adult_cnt)),char(0)))
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 6.125)
    SET rptsd->m_width = 0.625
    SET rptsd->m_height = 0.188
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(cnvtstring((ped_cnt+ adult_cnt))),
      char(0)))
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
 ...
 SUBROUTINE footcatalogdispsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_height = 0.188
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(cnvtstring(ped_total)),char(0)))
    SET rptsd->m_y = (offsety+ 0.063)
    SET rptsd->m_x = (offsetx+ 4.813)
    SET rptsd->m_width = 0.750
    SET rptsd->m_height = 0.188
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(cnvtstring(adult_total)),char(0))
     )
    SET rptsd->m_y = (offsety+ 0.063)
    SET rptsd->m_x = (offsetx+ 6.000)
    SET rptsd->m_width = 0.750
    SET rptsd->m_height = 0.188
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(cnvtstring((adult_total+
        ped_total))),char(0)))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 3.750),(offsety+ 0.032),(offsetx+ 7.000),(offsety+
     0.032))
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
 ...
 SUBROUTINE footreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF ( NOT (size(reply_obj->plist,5) > 0))
    RETURN(0.0)
   ENDIF
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 16
    ...
    SET rptsd->m_height = 0.260
    SET _dummyfont = uar_rptsetfont(_hreport,_times10b255)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,"NOTE",
       "***Under 18 at the time of procedure or at requested time if the procedure has not yet been performed"
       ),char(0)))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
   ENDIF
 ...
 SUBROUTINE graphsectionabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(3.380000), private
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CV_FRPT_PROC_BY_ENCTR"
