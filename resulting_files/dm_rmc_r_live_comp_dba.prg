CREATE PROGRAM dm_rmc_r_live_comp:dba
 ...
 SUBROUTINE drrm_check_freeze(dcf_env_id)
   ...
   RETURN(dcf_ret_val)
 END ;Subroutine
 DECLARE drlc_loop = i4 WITH protect, noconstant(0)
 DECLARE drlc_ret_ind = i2 WITH protect, noconstant(0)
 DECLARE drlc_pos = i4 WITH protect, noconstant(0)
 DECLARE drlc_src = vc WITH protect, noconstant("")
 DECLARE drlc_tgt = vc WITH protect, noconstant("")
 DECLARE drlc_file_short = vc WITH protect, noconstant("")
 DECLARE drlc_file_full = vc WITH protect, noconstant("")
 DECLARE drlc_varray_ind = i2 WITH protect, noconstant(0)
 DECLARE drlc_diff_ind = i2 WITH protect, noconstant(0)
 DECLARE drcc_batch_size = f8 WITH protect, noconstant(0.0)
 ...
 ENDIF
 IF (check_logfile("dm_rmc_r_live_comp",".log","DM_RMC_R_LIVE_COMP LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_comp
 ENDIF
 IF (validate(drcc_reply->status,"G")="G"
  AND validate(drcc_reply->status,"D")="D")
 ...
 ENDIF
 IF (validate(drcc_request->table_name,"XYZ")="XYZ"
  AND validate(drcc_request->table_name,"ABC")="ABC")
  FREE RECORD drcc_request
  RECORD drcc_request(
    1 table_name = vc
    1 context_name = vc
    1 file_name = vc
  )
  IF (((reflect(parameter(1,0)) != "C*") OR (((reflect(parameter(2,0)) != "C*") OR (reflect(parameter
   (3,0)) != "C*")) )) )
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Expected syntax: dm_rmc_r_live_comp <Context name>, <Table Name>, <File Name>"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_comp
  ELSE
   SET drcc_request->context_name =  $1
   SET drcc_request->table_name =  $2
   SET drcc_request->file_name =  $3
  ENDIF
 ENDIF
 FREE RECORD drlc_tabs
 RECORD drlc_tabs(
   1 cnt = i4
   1 qual[*]
     2 table_name = vc
 ...
 IF (drlc_debug_flag > 0)
  SELECT INTO "dm_rmc_r_live_debug.txt"
   FROM (dummyt d  WITH seq = 1)
   DETAIL
    "Start of Audit run", row + 1
   WITH nocounter, maxrow = 1, maxcol = 4000,
    format = variable, formfeed = none
  ;end select
 ENDIF
 IF (cnvtlower(substring((size(drcc_request->file_name) - 3),4,drcc_request->file_name))=".xml")
  SET drlc_file_full = drcc_request->file_name
  SET drlc_file_short = substring(1,(size(drcc_request->file_name) - 4),drcc_request->file_name)
 ELSE
  SET drlc_file_short = drcc_request->file_name
  SET drlc_file_full = concat(drcc_request->file_name,".xml")
 ENDIF
 SELECT INTO "NL:"
  FROM user_objects u
  WHERE u.object_name IN ("COLUMN_DIFF_DATA", "COLUMN_DIFF_VARRAY")
   AND u.object_type="TYPE"
  DETAIL
   IF (u.object_name="COLUMN_DIFF_DATA")
    drlc_diff_ind = 1
   ELSE
    drlc_varray_ind = 1
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error("Check required oracle objects")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_comp
 ENDIF
 IF (((drlc_diff_ind=0) OR (drlc_varray_ind=0)) )
  IF (drlc_varray_ind=1)
   CALL parser("rdb drop type column_diff_varray go",1)
  ENDIF
  IF (drlc_diff_ind=1)
   CALL parser("rdb drop type column_diff_data go",1)
  ENDIF
  CALL parser(concat(
    "rdb create type column_diff_data as object(tname varchar2(30), cname varchar2(30), ",
    "value varchar2(4000), r_rowid varchar2(18), r_ptam_hash number) go"),1)
  CALL parser("rdb create type column_diff_varray as varray(1000) of column_diff_data go",1)
  IF (check_error("Error creating objects")=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_comp
  ENDIF
 ENDIF
 SELECT INTO "NL:"
  FROM dm_info d
  WHERE d.info_domain="RDDS AUDIT BATCH SIZE"
   AND d.info_name="COMP BATCH SIZE"
  DETAIL
   drcc_batch_size = d.info_number
  WITH nocounter
 ;end select
 IF (check_error("Error creating objects")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_comp
 ENDIF
 IF (((curqual=0) OR (drcc_batch_size <= 0.0)) )
  SET drcc_batch_size = 10000.0
 ENDIF
 ...
 SELECT INTO "NL:"
  FROM dm_refchg_attribute d,
   dm_refchg_attribute d2
  WHERE d.attribute_name="MEANINGFUL_IND"
   AND d2.attribute_name="MEANINGFUL_POS"
   AND d2.table_name=d.table_name
   AND d2.column_name=d.column_name
  ORDER BY d.table_name, d2.attribute_value
  HEAD d.table_name
   drlc_meaningful->cnt = (drlc_meaningful->cnt+ 1), stat = alterlist(drlc_meaningful->qual,
    drlc_meaningful->cnt), drlc_meaningful->qual[drlc_meaningful->cnt].table_name = d.table_name
  DETAIL
   drlc_meaningful->qual[drlc_meaningful->cnt].col_cnt = (drlc_meaningful->qual[drlc_meaningful->cnt]
   .col_cnt+ 1), stat = alterlist(drlc_meaningful->qual[drlc_meaningful->cnt].col_qual,
    drlc_meaningful->qual[drlc_meaningful->cnt].col_cnt), drlc_meaningful->qual[drlc_meaningful->cnt]
   .col_qual[drlc_meaningful->qual[drlc_meaningful->cnt].col_cnt].column_name = d.column_name
  WITH nocounter
 ;end select
 ...
 IF (check_error("Gather pattern non defining column list")=1)
  ...
  GO TO exit_comp
 ENDIF
 SET dm_err->eproc = "Gathering list of valid tables to work on"
 SELECT INTO "NL:"
  FROM dm_info d,
   user_tables u
  WHERE d.info_domain="RDDS COMP TAB LIST"
   AND u.table_name=d.info_name
  ORDER BY d.info_name
  DETAIL
   drlc_tabs->cnt = (drlc_tabs->cnt+ 1), stat = alterlist(drlc_tabs->qual,drlc_tabs->cnt), drlc_tabs
   ->qual[drlc_tabs->cnt].table_name = d.info_name
  WITH nocounter
 ;end select
 IF (check_error("Gather valid table list")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_comp
 ENDIF
 SELECT INTO "NL:"
  ...
  WHERE d.info_domain="DATA MANAGEMENT"
   AND d.info_name="DM_ENV_ID"
   AND e.environment_id=d.info_number
  DETAIL
   drlc_tgt = concat(trim(e.environment_name)," (",trim(cnvtstring(e.environment_id)),")")
  WITH nocounter
 ;end select
 IF (check_error("Gather environment information")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_comp
 ENDIF
 SET drcc_reply->text_cnt = 7
 SET stat = alterlist(drcc_reply->text_qual,7)
 SET drcc_reply->text_qual[1].str = concat('<?xml-stylesheet href="',drlc_file_short,
  '.xsl" type="text/xsl"?>')
 SET drcc_reply->text_qual[2].str = "<REPORT_DATA>"
 SET drcc_reply->text_qual[3].str = concat("<REPORT_DATE>",format(cnvtdatetime(curdate,curtime3),
   "DD-MON-YYYY HH:MM:SS;;D"),"</REPORT_DATE>")
 SET drcc_reply->text_qual[4].str = "<FILE_NAME>dctest_file.xml</FILE_NAME>"
 SET drcc_reply->text_qual[5].str = concat("<TGT_DATA>",drlc_tgt,"</TGT_DATA>")
 SET drcc_reply->text_qual[6].str = concat("<TABLE_PARM>",drcc_request->table_name,"</TABLE_PARM>")
 SET drcc_reply->text_qual[7].str = concat("<CONTEXT_PARM>",drcc_request->context_name,
  "</CONTEXT_PARM>")
 SET drlc_ret_ind = drcc_create_report(drlc_file_full,drcc_reply,1)
 IF (drlc_ret_ind=1)
  GO TO exit_comp
 ENDIF
 IF ((drcc_request->table_name=char(42)))
  FOR (drlc_loop = 1 TO drlc_tabs->cnt)
    SET dm_err->eproc = concat("Calling child script for ",drlc_tabs->qual[drlc_loop].table_name)
    CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
    SET drcc_reply->text_cnt = 0
    SET stat = alterlist(drcc_reply->text_qual,0)
    SET drcc_request->table_name = drlc_tabs->qual[drlc_loop].table_name
    EXECUTE dm_rmc_comp_child  WITH replace("REQUEST","DRCC_REQUEST"), replace("REPLY","DRCC_REPLY")
    IF (((drlc_src="") OR (drlc_src="Not Found")) )
     SET drlc_pos = locateval(drlc_pos,1,drc_tab_info->cnt,drlc_tabs->qual[drlc_loop].table_name,
      drc_tab_info->qual[drlc_pos].table_name)
     IF (drlc_pos > 0
      AND (drc_tab_info->qual[drlc_pos].r_exist_ind=1))
      SELECT INTO "NL:"
       FROM (parser(drc_tab_info->qual[drlc_pos].r_table_name) r),
        dm_environment e
       WHERE r.rdds_status_flag < 9000
        AND e.environment_id=r.rdds_source_env_id
       DETAIL
        drlc_src = concat(trim(e.environment_name)," (",trim(cnvtstring(e.environment_id)),")")
       WITH maxqual(r,1)
      ;end select
      IF (check_error("Gather environment information")=1)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       SET drlc_src = "Not Found"
       SET dm_err->err_ind = 0
      ENDIF
     ENDIF
    ENDIF
    IF ((drcc_reply->status="F"))
     SET drlc_ret_ind = drcc_log_error_info(drcc_request->table_name,dm_err->eproc,dm_err->emsg,
      drlc_file_full)
     IF (drlc_ret_ind=1)
      GO TO exit_comp
     ENDIF
     SET dm_err->err_ind = 0
    ELSEIF ((drcc_reply->status="Z"))
     SET drlc_tabs->cnt = (drlc_tabs->cnt+ 1)
     SET stat = alterlist(drlc_tabs->qual,drlc_tabs->cnt)
     SET drlc_tabs->qual[drlc_tabs->cnt].table_name = drlc_tabs->qual[drlc_loop].table_name
    ELSE
     IF ((drcc_reply->text_cnt > 2))
      SET dm_err->eproc = "Creating report with comparison data"
      SET drlc_ret_ind = drcc_create_report(drlc_file_full,drcc_reply,0)
      IF (drlc_ret_ind=1)
       GO TO exit_comp
      ENDIF
     ENDIF
  ...
  ENDFOR
 ELSE
  IF (locateval(drlc_loop,1,drlc_tabs->cnt,drcc_request->table_name,drlc_tabs->qual[drlc_loop].
   table_name)=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("The ",drcc_request->table_name,
    " table is not currently supported by this report.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_comp
  ELSE
   SET dm_err->eproc = concat("Calling child script for ",drcc_request->table_name)
   SET drcc_reply->status = "Z"
   WHILE ((drcc_reply->status="Z"))
     SET drcc_reply->text_cnt = 0
     SET stat = alterlist(drcc_reply->text_qual,0)
     EXECUTE dm_rmc_comp_child  WITH replace("REQUEST","DRCC_REQUEST"), replace("REPLY","DRCC_REPLY")
   ENDWHILE
   IF (((drlc_src="") OR (drlc_src="Not Found")) )
    SET drlc_pos = locateval(drlc_pos,1,drc_tab_info->cnt,drcc_request->table_name,drc_tab_info->
     qual[drlc_pos].table_name)
    IF ((drc_tab_info->qual[drlc_pos].r_exist_ind=1))
     SELECT INTO "NL:"
      FROM (parser(drc_tab_info->qual[drlc_pos].r_table_name) r),
       dm_environment e
      WHERE r.rdds_status_flag < 9000
  ...
  ENDIF
 ENDIF
 SET drcc_reply->text_cnt = 1
 SET stat = alterlist(drcc_reply->text_qual,1)
 SET drcc_reply->text_qual[1].str = concat("<SRC_DATA>",drlc_src,"</SRC_DATA></REPORT_DATA>")
 SET drlc_ret_ind = drcc_create_report(drlc_file_full,drcc_reply,0)
 IF (drlc_ret_ind=1)
  GO TO exit_comp
 ENDIF
 SET dm_err->eproc = concat(curprog,": Copying the xsl file (",drlc_file_short,".xsl)")
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 SET drlc_ret_ind = copy_xsl("dm_rmc_r_live_comp.xsl",concat(drlc_file_short,".xsl"))
 IF ((drlc_ret_ind=- (1)))
  GO TO exit_comp
 ENDIF
#exit_comp
 IF ((dm_err->err_ind=0))
  ...
  CALL echo("---------------------------")
  CALL echo(drlc_file_full)
  CALL echo(concat(drlc_file_short,".xsl"))
  CALL echo("---------------------------")
  CALL echo("***********************************************************************************")
 ENDIF
END GO
