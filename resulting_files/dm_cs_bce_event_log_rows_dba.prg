CREATE PROGRAM dm_cs_bce_event_log_rows:dba
 DECLARE dm_cs_bce_event_log_rows = vc WITH private, noconstant("206564.FT.002")
 ...
 SET reply->table_name = "BCE_EVENT_LOG"
 ...
 ELSE
  ...
  WHILE (curmindate <= maxdate
   AND rowsleft > 0)
    SELECT INTO "nl:"
     FROM bce_event_log bce
     WHERE bce.updt_dt_tm >= cnvtdatetime(curmindate)
      AND bce.updt_dt_tm <= cnvtdatetime(curmaxdate)
      AND parser(sbr_getrowidnotexists("bce.bce_event_log_id+0 > 0","bce"))
     DETAIL
      v_rows = (v_rows+ 1)
      IF (mod(v_rows,10)=1)
       stat = alterlist(reply->rows,(v_rows+ 9))
      ENDIF
      reply->rows[v_rows].row_id = bce.rowid
     WITH nocounter, maxqual(bce,value(rowsleft))
    ;end select
 ...
 SUBROUTINE sbr_get_min_date(null)
   ...
   DECLARE mindate = dq8
   SELECT INTO "nl:"
    seqval = min(bce.updt_dt_tm)
    FROM bce_event_log bce
    WHERE bce.bce_event_log_id=0.0
    DETAIL
     zerorowdate = seqval
    WITH nocounter
   ;end select
   ...
   IF (v_err_code2 != 0)
    ...
    GO TO exit_script
   ENDIF
   UPDATE  FROM bce_event_log bce
    SET bce.updt_dt_tm = cnvtdatetime(curdate,curtime3)
    WHERE bce.bce_event_log_id=0.0
    WITH nocounter
   ;end update
   SET v_err_code2 = error(v_errmsg2,0)
   IF (v_err_code2 != 0)
    ...
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"UPDATEERROR1",
     "Failed to set bce.updt_dt_tm to curdate:%1","s",nullterm(v_errmsg2))
    ROLLBACK
    GO TO exit_script
   ELSE
    COMMIT
   ...
   ENDIF
   UPDATE  FROM bce_event_log bce
    SET bce.updt_dt_tm = cnvtdatetime(zerorowdate)
    WHERE bce.bce_event_log_id=0.0
    WITH nocounter
   ;end update
   SET v_err_code2 = error(v_errmsg2,0)
   IF (v_err_code2 != 0)
    ...
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"UPDATEERROR2",
     "Failed to set bce.updt_dt_tm to original date:%1","s",nullterm(v_errmsg2))
    ROLLBACK
    GO TO exit_script
   ELSE
    COMMIT
