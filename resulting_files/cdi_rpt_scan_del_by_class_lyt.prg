CREATE PROGRAM cdi_rpt_scan_del_by_class_lyt
 PROMPT
 ...
 EXECUTE cdi_rpt_scan_del_by_class_drvr  $BEGINDATE,  $ENDDATE,  $BATCHCLASS
 ...
 SUBROUTINE headreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(concat(uar_i18ngetmessage(_hi18nhandle,
        ...
        "HeadReportSection_title1","Pages Scanned and Deleted for Batch Class")),char(0)))
   ENDIF
   SET rptsd->m_flags = 16
   SET rptsd->m_y = (offsety+ 0.69)
 ...
 SUBROUTINE footbatch_lyt_batch_details_usernamesectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(total_scanned_per_user),
       char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_y = (offsety+ 0.00)
 ...
 SUBROUTINE footreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(grand_total_scanned),char(
        0)))
    ENDIF
   ENDIF
   SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CDI_RPT_SCAN_DEL_BY_CLASS_LYT"
