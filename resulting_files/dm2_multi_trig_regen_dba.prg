CREATE PROGRAM dm2_multi_trig_regen:dba
 ...
 SUBROUTINE drtq_view_task_process(i_process_name)
   ...
   SET message = nowindow
 END ;Subroutine
 DECLARE dmtr_task_ind = i4 WITH protect, noconstant(0)
 DECLARE dmtr_ret_status = c1 WITH protect, noconstant("")
 SET readme_data->status = "F"
 IF (check_logfile("dm2_regen_multi_",".log","dm2_regen_multi LOG FILE...") != 1)
  SET dm_err->emsg = "Unable to Create Log File"
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_multi_trig_regen..."
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Updating any dead tasks back to QUEUED status."
 UPDATE  FROM dm_refchg_task_queue d
  SET d.task_status = "QUEUED", d.rdbhandle_value = - (1), d.log_file = null,
   d.begin_dt_tm = null, d.end_dt_tm = null, d.updt_id = reqinfo->updt_id,
   d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d.updt_applctx = reqinfo->updt_applctx, d.updt_task
    = reqinfo->updt_task,
   d.updt_cnt = (d.updt_cnt+ 1)
  WHERE d.process_name="REFRESH TRIGGER PROCESS"
   AND d.task_status="RUNNING"
   AND  NOT ( EXISTS (
  (SELECT
   "x"
   FROM gv$session gv
 ...
 SELECT INTO "NL:"
  ...
  (SELECT
   "x"
   FROM dm_refchg_task_queue q
   WHERE q.process_name=d.process_name
    AND q.task_status="RUNNING"
    AND q.task_level <= d.task_level
    AND  EXISTS (
   (SELECT
    "x"
    FROM gv$session gv
    WHERE gv.audsid=q.rdbhandle_value)))))
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) > 0)
  GO TO exit_script
 ELSEIF (curqual > 0)
  SET dmtr_ret_status = drtq_reset_task_process("REFRESH TRIGGER PROCESS")
  IF (dmtr_ret_status="F")
   GO TO exit_script
  ENDIF
 ENDIF
 SELECT INTO "NL:"
  ...
  FROM dm_refchg_task_queue d
  WHERE d.process_name="REFRESH TRIGGER PROCESS"
   AND ((d.task_status IN ("READY", "RUNNING")) OR (d.task_status="QUEUED"
   AND d.task_level IN (
  (SELECT
   min(r.task_level)
  ...
  (SELECT
   ...
   FROM dm_refchg_task_queue q
   WHERE q.process_name=d.process_name
    AND q.task_status IN ("ERROR", "READY", "RUNNING")
    AND q.task_level <= d.task_level)))))
  DETAIL
   dmtr_task_ind = dmtr_task_cnt
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) > 0)
  GO TO exit_script
 ENDIF
 WHILE (dmtr_task_ind > 0)
   EXECUTE dm_rmc_task_wrapper "REFRESH TRIGGER PROCESS"
   IF (check_error(dm_err->eproc) > 0)
    SET dmtr_task_ind = - (1)
    GO TO exit_script
   ENDIF
   SELECT INTO "NL:"
    ...
    WHERE d.process_name="REFRESH TRIGGER PROCESS"
     AND d.task_status="ERROR"
    DETAIL
     readme_data->message = concat("LOGFILE:",d.log_file," ",d.error_text), dmtr_task_ind = - (1)
    WITH nocounter
   ;end select
   IF ((dmtr_task_ind != - (1)))
    SELECT INTO "NL:"
 ...
 ENDWHILE
 IF ((dmtr_task_ind != - (1)))
  SET readme_data->message = "Trigger regeneration complete."
