CREATE PROGRAM cdi_rpt_prod_by_date_lyt
 PROMPT
 ...
 EXECUTE cdi_rpt_prod_by_date_drvr  $BEGINDATE
 ...
 SUBROUTINE batch_query(dummy)
   ...
   SELECT INTO "NL:"
    ...
    batch_lyt_batch_details_documentsdeleted = batch_lyt->batch_details[dtrs1.seq].documentsdeleted,
    ...
    ORDER BY batch_lyt_batch_details_username
    HEAD REPORT
     _d0 = batch_lyt_batch_details_username, _d1 = batch_lyt_batch_details_documentscreated, _d2 =
     batch_lyt_batch_details_documentsdeleted,
     _d3 = batch_lyt_batch_details_pagesscanned, _d4 = batch_lyt_batch_details_pagesdeleted,
     _fenddetail = ((rptreport->m_pageheight - rptreport->m_marginbottom) - footpagesection(
      rpt_calcheight)),
     dummy_val = headreportsection(rpt_render), total_docs_created = 0, total_pages_scanned = 0,
     total_docs_deleted = 0, total_pages_deleted = 0
    HEAD PAGE
     IF (curpage > 1)
    ...
    DETAIL
     IF (((_yoffset+ detailsection(rpt_calcheight)) > _fenddetail))
      BREAK
     ENDIF
     dummy_val = detailsection(rpt_render), total_docs_created = (total_docs_created+
     batch_lyt_batch_details_documentscreated), total_pages_scanned = (total_pages_scanned+
     batch_lyt_batch_details_pagesscanned),
     total_docs_deleted = (total_docs_deleted+ batch_lyt_batch_details_documentsdeleted),
     total_pages_deleted = (total_pages_deleted+ batch_lyt_batch_details_pagesdeleted)
    FOOT PAGE
 ...
 SUBROUTINE headreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(concat(uar_i18ngetmessage(_hi18nhandle,
        ...
        "HeadReportSection_title1","Productivity Report")),char(0)))
   ENDIF
 ...
 SUBROUTINE headpagesectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=0)
    IF (size(batch_lyt->batch_details,5) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
        "HeadPageSection_FieldName0","User Name"),char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_y = (offsety+ 0.00)
   SET rptsd->m_x = (offsetx+ 3.25)
   SET rptsd->m_width = 1.25
   SET rptsd->m_height = 0.26
   IF (ncalc=0)
    IF (size(batch_lyt->batch_details,5) > 0)
   ...
   IF (ncalc=0)
    IF (size(batch_lyt->batch_details,5) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
        "HeadPageSection_FieldName1","Documents Deleted"),char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_y = (offsety+ 0.00)
   SET rptsd->m_x = (offsetx+ 6.25)
   SET rptsd->m_width = 1.25
   SET rptsd->m_height = 0.25
   IF (ncalc=0)
    IF (size(batch_lyt->batch_details,5) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
        "HeadPageSection_FieldName2","Pages Deleted"),char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_flags = 16
   SET rptsd->m_y = (offsety+ 0.00)
 ...
 SUBROUTINE detailsectionabs(ncalc,offsetx,offsety)
   ...
   SET rptsd->m_y = (offsety+ 0.00)
   SET rptsd->m_x = (offsetx+ 4.75)
   SET rptsd->m_width = 1.27
   SET rptsd->m_height = 0.25
   IF (ncalc=0)
    IF (size(batch_lyt->batch_details,5) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(
        batch_lyt_batch_details_documentsdeleted),char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_y = (offsety+ 0.00)
   SET rptsd->m_x = (offsetx+ 6.25)
   ...
   IF (ncalc=0)
    IF (size(batch_lyt->batch_details,5) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(
        batch_lyt_batch_details_pagesdeleted),char(0)))
    ENDIF
   ENDIF
   SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
   IF (ncalc=0)
 ...
 SUBROUTINE footreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=0)
    IF (size(batch_lyt->batch_details,5) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(total_docs_created),char(0
        )))
    ENDIF
   ENDIF
   SET rptsd->m_y = (offsety+ 0.00)
   ...
   IF (ncalc=0)
    IF (size(batch_lyt->batch_details,5) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(total_pages_scanned),char(
        0)))
    ENDIF
   ENDIF
   SET rptsd->m_y = (offsety+ 0.00)
   ...
   IF (ncalc=0)
    IF (size(batch_lyt->batch_details,5) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(total_docs_deleted),char(0
        )))
    ENDIF
   ENDIF
   SET rptsd->m_y = (offsety+ 0.00)
   ...
   IF (ncalc=0)
    IF (size(batch_lyt->batch_details,5) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(total_pages_deleted),char(
        0)))
    ENDIF
   ENDIF
   SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CDI_RPT_PROD_BY_DATE_LYT"
