CREATE PROGRAM cp_get_concept_group
 ...
 IF (validate(reply) != 1)
  RECORD reply(
    1 cg_list[*]
      ...
      2 rule_ind = i2
      2 cg_dtl_list[*]
        3 concept_entity_id = f8
        3 concept_entity_name = vc
        3 concept_entity_ident = vc
        3 concept_entity_text = vc
        3 cp_concept_group_dtl_id = f8
    1 status_data
      2 status = c1
      2 subeventstatus[1]
        3 operationname = c25
 ...
 SUBROUTINE getconcepts(null)
   ...
   SELECT INTO "nl:"
    ...
    PLAN (ccg
     WHERE expand(x,1,conceptcdlistsize,ccg.concept_cd,request->concept_cds[x].concept_cd))
     JOIN (ccgd
    ...
    HEAD ccg.concept_cd
     dcnt = 0, ccnt += 1
     IF (ccnt > size(reply->cg_list,5))
      stat = alterlist(reply->cg_list,(ccnt+ 5))
     ENDIF
     reply->cg_list[ccnt].concept_cd = ccg.concept_cd, reply->cg_list[ccnt].concept_group_cd = ccg
     .concept_group_cd, reply->cg_list[ccnt].cp_concept_group_id = ccg.cp_concept_group_id,
     reply->cg_list[ccnt].intention_cd = ccg.intention_cd, reply->cg_list[ccnt].rule_ind = ccg
     .rule_ind
    DETAIL
     dcnt += 1
     IF (dcnt > size(reply->cg_list[ccnt].cg_dtl_list,5))
      stat = alterlist(reply->cg_list[ccnt].cg_dtl_list,(dcnt+ 5))
     ENDIF
     reply->cg_list[ccnt].cg_dtl_list[dcnt].cp_concept_group_dtl_id = ccgd.cp_concept_group_dtl_id,
     reply->cg_list[ccnt].cg_dtl_list[dcnt].concept_entity_id = ccgd.concept_entity_id, reply->
     cg_list[ccnt].cg_dtl_list[dcnt].concept_entity_ident = ccgd.concept_entity_ident,
     reply->cg_list[ccnt].cg_dtl_list[dcnt].concept_entity_name = ccgd.concept_entity_name, reply->
     cg_list[ccnt].cg_dtl_list[dcnt].concept_entity_text = ccgd.concept_entity_text
    FOOT  ccg.concept_cd
     stat = alterlist(reply->cg_list[ccnt].cg_dtl_list,dcnt)
    FOOT REPORT
     stat = alterlist(reply->cg_list,ccnt)
    WITH nocounter
   ;end select
