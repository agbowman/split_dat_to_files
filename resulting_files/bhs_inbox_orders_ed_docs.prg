CREATE PROGRAM bhs_inbox_orders_ed_docs
 ...
 SELECT INTO "inboxorders"
  ...
  HEAD REPORT
   ...
   row + 1
  DETAIL
   phyname = concat(trim(p.name_last)," ",trim(p.name_first)), ordcnt = temp->qual[d.seq].ocnt,
   position = uar_get_code_display(p.position_cd),
   line = build(',"',phyname,'","',ordcnt,'","',
    position,'",'), col 0, line,
   row + 1
 ...
 CALL emailfile("inboxorders.dat","inboxorders.csv","farid.faruqui@bhs.org","Inbox Orders",1)
#exit_prog
END GO
