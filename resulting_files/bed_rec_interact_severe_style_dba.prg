CREATE PROGRAM bed_rec_interact_severe_style:dba
 ...
 SET detail_mode = validate(reply->rowlist)
 ...
 SELECT INTO "nl:"
  FROM br_rec b,
   br_long_text bl
  PLAN (b
   WHERE b.rec_mean="ORDINTCHECKSEVERITYSTYLE2")
   JOIN (bl
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ap
   ...
   WHERE nvp.parent_entity_name="APP_PREFS"
    ...
    AND nvp.pvc_name="MUL_INTERPRETEDSEVERITY"
 ...
 IF (detail_mode=1)
  IF (((powerchart_found=0) OR (powerchart_pref_row_exists=0)) )
   ...
   SET tempreply->rowlist[row_tot_cnt].celllist[2].string_value = "MUL_INTERPRETEDSEVERITY"
   SET tempreply->rowlist[row_tot_cnt].celllist[3].string_value = "1"
 ...
 SELECT INTO "nl:"
  ...
  HEAD ap.application_number
   ...
   IF (priv_exists_ind=1
    ...
    IF (nvp.name_value_prefs_id > 0
     ...
     row_tot_cnt].celllist[2].string_value = "MUL_INTERPRETEDSEVERITY", tempreply->rowlist[
  ...
  FOOT  p.position_cd
   ...
   IF (priv_exists_ind=1
    ...
    stat = alterlist(tempreply->rowlist[row_tot_cnt].celllist,col_cnt),
    tempreply->rowlist[row_tot_cnt].celllist[1].string_value = short_desc, tempreply->rowlist[
    row_tot_cnt].celllist[2].string_value = "MUL_INTERPRETEDSEVERITY", tempreply->rowlist[row_tot_cnt
