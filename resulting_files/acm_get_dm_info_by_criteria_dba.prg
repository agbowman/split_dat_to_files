CREATE PROGRAM acm_get_dm_info_by_criteria:dba
 IF (validate(reply->info_domains)=0)
  RECORD reply(
    1 status_data
      2 status = c1
      2 subeventstatus[1]
 ...
 DECLARE domain_cnt = i4 WITH protect, constant(size(request->info_domains,5))
 DECLARE name_cnt = i4 WITH protect, noconstant(size(request->info_names,5))
 DECLARE info_index = i4 WITH protect, noconstant(0)
 DECLARE domain_rep_index = i4 WITH protect, noconstant(0)
 DECLARE name_rep_index = i4 WITH protect, noconstant(0)
 SET failed = false
 SET reply->status_data.status = "F"
 IF ((request->info_domain_id < 0))
  SET failed = none_found
  GO TO exit_script
 ENDIF
 IF (domain_cnt=0)
  SET failed = none_found
  SET table_name = build("request->info_domains list is empty")
  GO TO exit_script
 ELSE
  FOR (i = 1 TO domain_cnt)
    IF ((request->info_domains[i].info_domain=""))
     SET failed = none_found
     SET table_name = build("request->info_domains[",i,"].info_domain empty string")
     GO TO exit_script
    ENDIF
  ENDFOR
 ENDIF
 IF (name_cnt=0)
  SET failed = none_found
  SET table_name = build("request->info_names list is empty")
  GO TO exit_script
 ELSE
  FOR (i = 1 TO name_cnt)
    IF ((request->info_domains[i].info_domain=""))
     SET failed = none_found
     SET table_name = build("request->info_names[",i,"].info_name empty string")
     GO TO exit_script
    ENDIF
  ENDFOR
 ENDIF
 ...
 FOR (i = 1 TO domain_cnt)
   FOR (j = 1 TO name_cnt)
     SET t_record->dm_info_cnt = (t_record->dm_info_cnt+ 1)
     IF (mod(t_record->dm_info_cnt,10)=1)
      CALL alterlist(t_record->dm_info,(t_record->dm_info_cnt+ 9))
     ENDIF
     SET t_record->dm_info[t_record->dm_info_cnt].info_domain = request->info_domains[i].info_domain
     SET t_record->dm_info[t_record->dm_info_cnt].info_name = request->info_names[j].info_name
   ENDFOR
 ENDFOR
 CALL alterlist(t_record->dm_info,t_record->dm_info_cnt)
 SELECT INTO "nl:"
  FROM dm_info m
  WHERE expand(info_index,1,t_record->dm_info_cnt,m.info_domain,t_record->dm_info[info_index].
   info_domain,
   m.info_name,t_record->dm_info[info_index].info_name)
   AND (m.info_domain_id=request->info_domain_id)
  ORDER BY m.info_domain, m.info_name
  HEAD REPORT
   domain_rep_index = 0
  HEAD m.info_domain
   name_rep_index = 0, domain_rep_index = (domain_rep_index+ 1)
   IF (mod(domain_rep_index,10)=1)
    stat = alterlist(reply->info_domains,(domain_rep_index+ 9))
   ENDIF
   reply->info_domains[domain_rep_index].info_domain = m.info_domain
  DETAIL
   name_rep_index = (name_rep_index+ 1)
   IF (mod(name_rep_index,10)=1)
    stat = alterlist(reply->info_domains[domain_rep_index].info_names,(name_rep_index+ 9))
   ENDIF
   reply->info_domains[domain_rep_index].info_names[name_rep_index].info_char = m.info_char, reply->
   info_domains[domain_rep_index].info_names[name_rep_index].info_date = m.info_date, reply->
   info_domains[domain_rep_index].info_names[name_rep_index].info_number = m.info_number,
   reply->info_domains[domain_rep_index].info_names[name_rep_index].info_long_id = m.info_long_id,
   reply->info_domains[domain_rep_index].info_names[name_rep_index].info_name = m.info_name
  FOOT  m.info_domain
   IF (mod(name_rep_index,10) != 0)
    stat = alterlist(reply->info_domains[domain_rep_index].info_names,name_rep_index)
   ENDIF
  FOOT REPORT
   IF (mod(domain_rep_index,10) != 0)
    stat = alterlist(reply->info_domains,domain_rep_index)
   ENDIF
  WITH nocounter
 ;end select
#exit_script
