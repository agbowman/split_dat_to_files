CREATE PROGRAM dm2_ads_driverkey_build:dba
 ...
 SUBROUTINE ds_scramble(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dadb_config_extract_id = f8 WITH protect, noconstant(0.0)
 DECLARE dadb_config_id = f8 WITH protect, noconstant(0.0)
 DECLARE dadb_extract_id = f8 WITH protect, noconstant(0.0)
 DECLARE dadb_dpe_id = f8 WITH protect, noconstant(0.0)
 DECLARE dadb_dcsd_id = f8 WITH protect, noconstant(0.0)
 DECLARE dadb_cnt = i4 WITH protect, noconstant(0)
 DECLARE dadb_nostats_ind = i2 WITH protect, noconstant(0)
 DECLARE dadb_row_sample = i4 WITH protect, noconstant(0)
 DECLARE dadb_err_emsg = vc WITH protect, noconstant("")
 DECLARE dadb_err_eproc = vc WITH protect, noconstant("")
 DECLARE dadb_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dadb_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dadb_query_file = vc WITH protect, noconstant("")
 DECLARE dadb_table_name = vc WITH protect, noconstant("")
 IF (check_logfile("dm2_ads_drv_bld",".log","dm2_ads_driverkey_build LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_ads_driverkey_build."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dadb_err_ind = dm_err->err_ind
 SET dadb_err_emsg = dm_err->emsg
 SET dadb_err_eproc = dm_err->eproc
 SET dm_err->eproc = ""
 SET dm_err->emsg = ""
 SET dm_err->err_ind = 0
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dadb_config_extract_id = cnvtreal(abs( $1))
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg = concat("Parameter usage: dm2_ads_driverkey_build '<config_extract_id>'")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validating <config_extract_id> input parameter."
 SELECT INTO "nl:"
  FROM dm_ads_config_extract dace
  WHERE dace.dm_ads_config_extract_id=dadb_config_extract_id
  DETAIL
   dadb_config_id = dace.dm_ads_config_id, dadb_extract_id = dace.dm_ads_extract_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (((curqual=0) OR (((dadb_config_id=0) OR (dadb_extract_id=0)) )) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = build2("No Information found for Sample Configuration Extract: ",
   dadb_config_extract_id)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((validate(dm2_only_preview_op,- (1))=- (1))
  AND (validate(dm2_only_preview_op,- (2))=- (2)))
  SET dm_err->eproc = build2("Setting status of current config extract row with id: ",
   dadb_config_extract_id)
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  UPDATE  FROM dm_ads_config_extract dace
   SET dace.status_txt = dpl_executing, dace.begin_dt_tm = cnvtdatetime(curdate,curtime3), dace
  ...
  CALL dm2_process_log_add_detail_number(dpl_config_extract_id,dadb_config_extract_id)
  CALL dm2_process_log_row(dpl_sample,dpl_execution,dpl_no_prev_id,1)
  SET dadb_dpe_id = dm2_process_event_rs->dm_process_event_id
 ENDIF
 IF (dar_get_extract_details(dadb_config_extract_id)=0)
  GO TO exit_program
 ENDIF
 SET dm2scramble->mode_ind = 0
 SET dadb_table_name = dar_extract_details->table_name
 SET dm2scramble->in_text = dar_extract_details->table_name
 IF (ds_scramble(null)=0)
  GO TO exit_program
 ENDIF
 SET dar_extract_details->table_name = build(dm2scramble->out_text)
 SET dm2scramble->out_text = ""
 SET dm2scramble->in_text = dar_extract_details->parent_table_name
 IF (ds_scramble(null)=0)
  GO TO exit_program
 ENDIF
 SET dar_extract_details->parent_table_name = build(dm2scramble->out_text)
 SET dm2scramble->out_text = ""
 SET dm2scramble->in_text = dar_extract_details->driver_keycol_name
 IF (ds_scramble(null)=0)
  GO TO exit_program
 ENDIF
 SET dar_extract_details->driver_keycol_name = build(dm2scramble->out_text)
 SET dm2scramble->out_text = ""
 SET dm2scramble->in_text = dar_extract_details->driver_rankcol_name
 IF (ds_scramble(null)=0)
  GO TO exit_program
 ENDIF
 SET dar_extract_details->driver_rankcol_name = build(dm2scramble->out_text)
 SET dm2scramble->out_text = ""
 SET dm_err->eproc = build2("Removing old config driver rows from dm_ads_config_driver for: ",
  dar_extract_details->full_key)
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 DELETE  FROM dm_ads_config_driver dacd
  WHERE dacd.dm_ads_config_id=dadb_config_id
   AND dacd.dm_ads_extract_id IN (
  (SELECT
   dae.dm_ads_extract_id
   FROM dm_ads_extract dae
   WHERE (dae.owner_name=dar_extract_details->owner_name)
    AND dae.table_name=dadb_table_name
    AND (dae.table_extract_nbr=dar_extract_details->table_extract_nbr)))
  WITH nocounter
 ;end delete
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 1
 ...
 ELSE
  COMMIT
 ENDIF
 SET dadb_nostats_ind = 0
 SET dm_err->eproc = build2("Gathering latest statistics info for: ",dar_extract_details->table_name)
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  analyze_ind = nullind(dut.last_analyzed)
  FROM dm_user_tables_actual_stats dut
  WHERE (dut.table_name=dar_extract_details->table_name)
  DETAIL
   IF (analyze_ind=1)
    dadb_nostats_ind = 1
   ELSE
    dar_extract_details->src_last_analyzed_dt_tm = dut.last_analyzed, dar_extract_details->
    src_num_rows = dut.num_rows, dar_extract_details->src_avg_row_length = dut.avg_row_len,
    dar_extract_details->row_sample = 0
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 IF ((dar_extract_details->tgt_sample_method=dpl_recent))
  IF (dadb_nostats_ind=1)
   SET dar_extract_details->message = dpl_invalid_stats
   SET dar_extract_details->status = dpl_failed
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("Invalid / Missing Table Statistics. Sampling method ",dpl_recent,
    " requires table statistics.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 SET dar_extract_details->tgt_rank_val = dm2ceil(((dar_extract_details->src_num_rows *
  dar_extract_details->tgt_sample_pct)/ 100))
 SET dar_extract_details->tgt_mod_val = dm2floor((100/ dar_extract_details->tgt_sample_pct))
 IF ((dar_extract_details->tgt_mod_val < cnvtint(dar_extract_details->src_num_rows)))
  SET dar_extract_details->rand_val = mod(rand(0),dar_extract_details->tgt_mod_val)
 ELSE
  SET dar_extract_details->rand_val = mod(rand(0),cnvtint(dar_extract_details->src_num_rows))
 ENDIF
 IF ((dar_extract_details->tgt_sample_method=dpl_interval))
  IF ((validate(dm2_only_preview_op,- (1))=- (1))
   AND (validate(dm2_only_preview_op,- (2))=- (2)))
   SET dm_err->eproc = build2("Inserting driver key rows into dm_ads_config_driver using ",
    dpl_interval," Sample Method for table: ",dar_extract_details->table_name)
   CALL disp_msg("",dm_err->logfile,0)
   CALL dm2_push_cmd(concat("rdb asis(^INSERT INTO DM_ADS_CONFIG_DRIVER DACD ",
     "(DACD.DM_ADS_CONFIG_DRIVER_ID,DACD.DM_ADS_CONFIG_ID,",
     "DACD.DM_ADS_EXTRACT_ID,DACD.DRIVER_KEY_ID,DACD.CUSTOM_IND,",
     "DACD.UPDT_ID,DACD.UPDT_TASK,DACD.UPDT_APPLCTX,DACD.UPDT_CNT,DACD.UPDT_DT_TM)","^)"),0)
   CALL dm2_push_cmd(concat("asis(^SELECT DM_CLINICAL_SEQ.NEXTVAL,",build(dar_extract_details->
      config_id),",",build(dar_extract_details->extract_id),",",
     dar_extract_details->driver_keycol_name,",0,0,0,0,0,SYSDATE ^)"),0)
   CALL dm2_push_cmd(concat("asis(^FROM (^)"),0)
   CALL dm2_push_cmd(concat("asis(^SELECT ROWNUM AS ROW_COUNTER,",dar_extract_details->
     driver_keycol_name," FROM ",dar_extract_details->table_name," WHERE ",
     dar_extract_details->driver_keycol_name," > 0.0)^)"),0)
   CALL dm2_push_cmd(concat("asis(^WHERE MOD(ROW_COUNTER,",build(dar_extract_details->tgt_mod_val),
     ") = ",build(dar_extract_details->rand_val),"^)"),0)
   ...
   IF (curqual > 0)
    COMMIT
    SET dadb_row_sample = curqual
   ENDIF
  ELSE
   SET dm_err->eproc = build2("Create driver key build query preview file using ",dpl_interval,
    " Sample Method for table: ",dar_extract_details->table_name)
   CALL disp_msg("",dm_err->logfile,0)
   IF (get_unique_file(build("dadb_",cnvtstring(dadb_config_extract_id)),".dat")=0)
    GO TO exit_program
   ELSE
    SET dadb_query_file = dm_err->unique_fname
   ENDIF
   SET dm_err->eproc = "Creating file for Driver Key Build query preview"
   SELECT INTO value(dadb_query_file)
    FROM dummyt d
    DETAIL
     col 0,
     CALL print("rdb INSERT INTO DM_ADS_CONFIG_DRIVER DACD "), row + 1,
     col 0,
     CALL print(concat("(DACD.DM_ADS_CONFIG_DRIVER_ID,DACD.DM_ADS_CONFIG_ID,",
      "DACD.DM_ADS_EXTRACT_ID,DACD.DRIVER_KEY_ID,DACD.CUSTOM_IND)")), row + 1,
     col 0,
     CALL print(concat("SELECT  DM_CLINICAL_SEQ.NEXTVAL,",build(dar_extract_details->config_id),",",
      build(dar_extract_details->extract_id),",",
      dar_extract_details->driver_keycol_name,",0")), row + 1,
     col 0,
     CALL print("FROM ("), row + 1,
     col 0,
     CALL print(concat("SELECT ROWNUM AS ROW_COUNTER,",dar_extract_details->driver_keycol_name,
      " FROM ",dar_extract_details->table_name," WHERE ",
      dar_extract_details->driver_keycol_name," > 0.0)")), row + 1,
     col 0,
     CALL print(concat("WHERE MOD(ROW_COUNTER,",build(dar_extract_details->tgt_mod_val),") = ",build(
       dar_extract_details->rand_val))), row + 1,
     col 0,
     CALL print("go"), row + 1
   ...
   IF (check_error(dm_err->eproc))
    ...
    GO TO exit_program
   ENDIF
   SET dm_err->eproc = "Displaying Driver Key Build Query File"
   IF (dm2_disp_file(dadb_query_file,build("Driver Key Build Query for: ",dadb_config_extract_id))=0)
    GO TO exit_program
   ENDIF
  ENDIF
 ELSEIF ((dar_extract_details->tgt_sample_method=dpl_recent))
  IF ((validate(dm2_only_preview_op,- (1))=- (1))
   AND (validate(dm2_only_preview_op,- (2))=- (2)))
   SET dm_err->eproc = build2("Inserting driver row into dm_ads_config_driver using ",dpl_recent,
    " Sample Metho for table: ",dar_extract_details->table_name)
   CALL disp_msg("",dm_err->logfile,0)
   CALL dm2_push_cmd(concat("rdb asis(^INSERT INTO DM_ADS_CONFIG_DRIVER DACD",
     "(DACD.DM_ADS_CONFIG_DRIVER_ID,DACD.DM_ADS_CONFIG_ID,",
     "DACD.DM_ADS_EXTRACT_ID,DACD.DRIVER_KEY_ID,DACD.CUSTOM_IND,",
   ...
   CALL dm2_push_cmd(concat("asis(^SELECT DM_CLINICAL_SEQ.NEXTVAL,",build(dar_extract_details->
     ...
     dar_extract_details->driver_keycol_name,",0,0,0,0,0,SYSDATE ^)"),0)
   CALL dm2_push_cmd(concat("asis(^FROM (^)"),0)
   CALL dm2_push_cmd(concat("asis(^SELECT ",dar_extract_details->driver_keycol_name,
     ", ROWNUM AS COL_RANK","^)"),0)
   CALL dm2_push_cmd(concat("asis(^ FROM ",dar_extract_details->table_name," WHERE ",
     dar_extract_details->driver_keycol_name," > 0.0",
     " ORDER BY ",dar_extract_details->driver_rankcol_name," DESC)^)"),0)
   CALL dm2_push_cmd(concat("asis(^WHERE COL_RANK <= ",build(dar_extract_details->tgt_rank_val),"^)"),
    0)
   CALL dm2_push_cmd(asis("go"),1)
   IF (check_error(dm_err->eproc) != 0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("Error inserting driver row for table ",dar_extract_details->table_name
     )
    ROLLBACK
    GO TO exit_program
   ENDIF
   ...
   ENDIF
  ELSE
   SET dm_err->eproc = build2("Create driver key build query preview file using ",dpl_recent,
    " Sample Method for table: ",dar_extract_details->table_name)
   CALL disp_msg("",dm_err->logfile,0)
   IF (get_unique_file(build("dadb_",cnvtstring(dadb_config_extract_id)),".dat")=0)
    GO TO exit_program
   ...
   SELECT INTO value(dadb_query_file)
    ...
    DETAIL
     ...
     CALL print("FROM ("), row + 1,
     col 0,
     CALL print(concat("SELECT ",dar_extract_details->driver_keycol_name,", ROWNUM AS COL_RANK ")),
     row + 1,
     col 0,
     CALL print(concat("FROM ",dar_extract_details->table_name," WHERE ",dar_extract_details->
      driver_keycol_name," > 0.0",
      " ORDER BY ",dar_extract_details->driver_rankcol_name," DESC)")), row + 1,
     col 0,
     CALL print(concat("WHERE COL_RANK <= ",build(dar_extract_details->tgt_rank_val))), row + 1,
     col 0,
     CALL print("go"), row + 1
    WITH nocounter, maxcol = 500, formfeed = none
   ;end select
  ...
  ENDIF
 ELSE
  DECLARE dadb_file = vc
  IF ((dads_list->config_method=dpl_custom))
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain="ADS DPL_CUSTOM CSV NAME"
    DETAIL
     dadb_file = d.info_name
    WITH nocounter
   ;end select
   EXECUTE dm2_dbimport dadb_file, "dm2_ads_driverkey_imp", 1000
   IF (check_error(dm_err->eproc))
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
  ELSEIF ((dads_list->config_method=dpl_purge))
   SELECT INTO "nl:"
    FROM dm_info d
  ...
  ELSE
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("Driver Keys can only be built for the following extract methods: ",
    dpl_interval,"/",dpl_recent,". Current method is: ",
    dar_extract_details->tgt_sample_method)
   GO TO exit_program
  ENDIF
 ENDIF
 GO TO exit_program
#exit_program
 SET dm_err->eproc = build2("Updating status of driver key build in dm_ads_config_extract for id: ",
  dadb_config_extract_id)
 CALL disp_msg("",dm_err->logfile,0)
 IF ((validate(dm2_only_preview_op,- (1))=- (1))
  AND (validate(dm2_only_preview_op,- (2))=- (2)))
  UPDATE  FROM dm_ads_config_extract dace
   SET dace.status_txt = evaluate(dm_err->err_ind,0,dpl_complete,dpl_failed), dace.message_txt =
    evaluate(dm_err->err_ind,1,dm_err->emsg,""), dace.row_sample_nbr = dadb_row_sample,
    dace.end_dt_tm = cnvtdatetime(curdate,curtime3), dace.updt_dt_tm = cnvtdatetime(curdate,curtime3)
   WHERE dace.dm_ads_config_extract_id=dadb_config_extract_id
   WITH nocounter
  ;end update
  COMMIT
  CALL dm2_process_log_row(dpl_sample,dpl_execution,dadb_dpe_id,1)
 ENDIF
 IF ((dm_err->err_ind > 0))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ENDIF
 SET dm_err->err_ind = dadb_err_ind
 SET dm_err->emsg = dadb_err_emsg
 SET dm_err->eproc = dadb_err_eproc
 SET dm_err->eproc = "Ending dm2_ads_driverkey_build."
 CALL final_disp_msg("dm2_ads_drv_bld")
END GO
