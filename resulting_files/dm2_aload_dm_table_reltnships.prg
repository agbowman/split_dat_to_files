CREATE PROGRAM dm2_aload_dm_table_reltnships
 ...
 ENDIF
 IF (check_logfile("dm2_aload_dtr",".log","dm2_aload_dm_table_reltnships")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_dm_table_reltnships"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dadtr_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dadtr_file = vc WITH protect, noconstant( $2)
 DECLARE dadtr_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE dadtr_batch = i4 WITH protect, noconstant(0)
 DECLARE dadtr_file_loc_ret = vc WITH protect, noconstant("")
 DECLARE dadtr_tbl = vc WITH protect, noconstant("")
 DECLARE dadtr_tmp_tbl = vc WITH protect, noconstant("")
 DECLARE dadtr_cnt = i4 WITH protect, noconstant(0)
 DECLARE dadtr_tbl_exists = i2 WITH protect, noconstant(0)
 DECLARE dadtr_tmp_tbl_exists = i2 WITH protect, noconstant(0)
 SET dadtr_tbl = "DM_TABLE_RELATIONSHIPS"
 SET dadtr_tmp_tbl = "DM_TABLE_RELATIONSHIPS_GTTP"
 SET dm_err->eproc = concat("Check if table ",dadtr_tbl," exists.")
 IF (dm2_table_and_ccldef_exists(dadtr_tbl,dadtr_tbl_exists)=0)
  GO TO exit_program
 ENDIF
 IF (dadtr_tbl_exists=0)
  SET dm_err->eproc = concat("Table ",dadtr_tbl," does not exist.")
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Check if table ",dadtr_tmp_tbl," exists.")
 IF (dm2_table_and_ccldef_exists(dadtr_tmp_tbl,dadtr_tmp_tbl_exists)=0)
  GO TO exit_program
 ENDIF
 IF (dadtr_tmp_tbl_exists=0)
  SET dm_err->eproc = concat("Table ",dadtr_tmp_tbl," does not exist.")
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ELSE
  SET dm_err->eproc = concat("Truncate temp table",dadtr_tmp_tbl)
  CALL disp_msg("",dm_err->logfile,0)
  IF (dm2_push_cmd(concat("rdb truncate table ",dadtr_tmp_tbl," go"),1)=0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (dac_prelim(dadtr_pkg,dadtr_file_loc_ret,dadtr_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dadtr_batch=0)
  SET dadtr_batch = 5000
 ENDIF
 IF (dadtr_expected_rows > 0)
  EXECUTE dm2_dbimport concat(dadtr_file_loc_ret,dadtr_file), "dm2_aload_dtr_gttp", dadtr_batch
  CALL echo(concat(dadtr_file_loc_ret,dadtr_file))
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  EXECUTE dm2_aload_dtr
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_dm_table_reltnships completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_dm_table_reltnships")
END GO
