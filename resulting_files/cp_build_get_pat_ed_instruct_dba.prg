CREATE PROGRAM cp_build_get_pat_ed_instruct:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Input Search Text" = "",
  "Domain Cd" = 0.0
  WITH outdev, sinputsearchtext, dinputdomaincd
 ...
 DECLARE dpat_ed_domain_cd_exitcareer = f8 WITH protect, constant(uar_get_code_by("MEANING",24849,
   "EXITCAREER"))
 DECLARE dlanguage_id = f8 WITH protect, constant(0.0)
 DECLARE dparent_reltn_id = f8 WITH protect, constant(0.0)
 DECLARE lsearch_type = i2 WITH protect, constant(1)
 DECLARE dsearchdomaincd = f8 WITH noconstant( $DINPUTDOMAINCD), protect
 ...
 DECLARE pcntr = i4 WITH noconstant(0), protect
 DECLARE pindx = i4 WITH noconstant(0), protect
 DECLARE psize = i4 WITH noconstant(0), protect
 IF (( $DINPUTDOMAINCD=0.0))
  SET dsearchdomaincd = dpat_ed_domain_cd_exitcareer
 ENDIF
 EXECUTE mp_get_pat_edu_instructions "NOFORMS", dsearchdomaincd,  $SINPUTSEARCHTEXT,
 lsearch_type, dlanguage_id, dparent_reltn_id,
 0
 SET psize = size(reply->qual,5)
 SELECT INTO "nl:"
  FROM pat_ed_reltn per
  PLAN (per
   WHERE expand(pcntr,1,psize,per.pat_ed_reltn_id,reply->qual[pcntr].relation_id)
    AND per.custom_ind=1
    AND trim(per.key_doc_ident) IN (null, ""))
  ORDER BY per.pat_ed_reltn_id
  HEAD per.pat_ed_reltn_id
   pindx = locateval(searchcntr,1,psize,per.pat_ed_reltn_id,reply->qual[searchcntr].relation_id),
   reply->qual[pindx].key_doc_ident = per.pat_ed_reltn_desc_key
  WITH nocounter
 ;end select
#exit_script
 IF (validate(debug_ind,0)=1)
