CREATE PROGRAM cdi_rpt_module_access_lyt
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Date" = "CURDATE"
  WITH outdev, begindate
 EXECUTE reportrtl
 RECORD batch_lyt(
   1 batch_details[*]
     2 username = vc
 ...
 EXECUTE cdi_rpt_module_access_drvr  $BEGINDATE
 ...
 SUBROUTINE batch_query(dummy)
   ...
   SELECT INTO "NL:"
    ...
    batch_lyt_batch_details_accesscount = batch_lyt->batch_details[dtrs1.seq].accesscount,
    batch_lyt_batch_details_totaltime = batch_lyt->batch_details[dtrs1.seq].totaltime
    FROM (dummyt dtrs1  WITH seq = value(size(batch_lyt->batch_details,5)))
    ORDER BY batch_lyt_batch_details_username, batch_lyt_batch_details_modulename
    HEAD REPORT
     _d0 = batch_lyt_batch_details_username, _d1 = batch_lyt_batch_details_modulename, _d2 =
     batch_lyt_batch_details_accesscount,
     _d3 = batch_lyt_batch_details_totaltime, _fenddetail = ((rptreport->m_pageheight - rptreport->
     m_marginbottom) - footpagesection(rpt_calcheight)), dummy_val = headreportsection(rpt_render),
     total_time_for_user = 0
    HEAD PAGE
     IF (curpage > 1)
    ...
    HEAD batch_lyt_batch_details_username
     IF (((_yoffset+ headbatch_lyt_batch_details_usernamesection(rpt_calcheight)) > _fenddetail))
      BREAK
     ENDIF
     dummy_val = headbatch_lyt_batch_details_usernamesection(rpt_render), total_time_for_user = 0
    DETAIL
     IF (((_yoffset+ detailsection(rpt_calcheight)) > _fenddetail))
      BREAK
     ENDIF
     dummy_val = detailsection(rpt_render), total_time_for_user = (total_time_for_user+
     batch_lyt_batch_details_totaltime)
 ...
 SUBROUTINE headreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(concat(uar_i18ngetmessage(_hi18nhandle,
        "CPDI_title","ProVision Document Imaging"),_crlf,uar_i18ngetmessage(_hi18nhandle,
        "HeadReportSection_title2","Ascent Module Access Report")),char(0)))
   ENDIF
   ...
   IF (ncalc=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(format(cnvtdatetime(cnvtdate2(
          $BEGINDATE,"MM/DD/YY"),0),"@SHORTDATE"),char(0)))
   ENDIF
 ...
 SUBROUTINE headpagesectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
        "HeadPageSection_FieldName01","Total Time (hh:mm:ss)"),char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_flags = 20
   SET rptsd->m_y = (offsety+ 0.00)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
        "HeadPageSection_FieldName02","Accessed"),char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_y = (offsety+ 0.00)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
        "HeadPageSection_userNameHeader","UserName"),char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_flags = 16
   SET rptsd->m_y = (offsety+ 0.00)
 ...
 SUBROUTINE detailsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(
        batch_lyt_batch_details_accesscount),char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_y = (offsety+ 0.00)
   SET rptsd->m_x = (offsetx+ 5.50)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(floor((
        batch_lyt_batch_details_totaltime/ 3600)),":",trim(cnvtstring(((mod(
          batch_lyt_batch_details_totaltime,3600) - mod(batch_lyt_batch_details_totaltime,60))/ 60),2,
         0,r)),":",trim(cnvtstring(mod(batch_lyt_batch_details_totaltime,60),2,0,r)),
       char(0)))
    ENDIF
   ENDIF
   SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
 ...
 SUBROUTINE footbatch_lyt_batch_details_usernamesectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
        "Footbatch_lyt_batch_details_usernameSection_FieldName0","Total Time:"),char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_flags = 20
   SET rptsd->m_y = (offsety+ 0.00)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(floor((total_time_for_user/ 3600)),
       ":",trim(cnvtstring(((mod(total_time_for_user,3600) - mod(total_time_for_user,60))/ 60),2,0,r)
        ),":",trim(cnvtstring(mod(total_time_for_user,60),2,0,r)),
       char(0)))
    ENDIF
   ENDIF
   SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CDI_RPT_MODULE_ACCESS_LYT"
