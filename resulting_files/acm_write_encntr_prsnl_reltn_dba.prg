CREATE PROGRAM acm_write_encntr_prsnl_reltn:dba
 ...
 DECLARE ubound = i4 WITH protect, noconstant(reply->encntr_prsnl_reltn_qual_cnt)
 SET stat = alterlist(reply->encntr_prsnl_reltn_qual,reply->encntr_prsnl_reltn_qual_cnt)
 ...
 FOR (index = lbound TO ubound)
   CASE (acm_request->encntr_prsnl_reltn_qual[index].action_flag)
    OF add_action:
    ...
    ELSE
     SET reply->encntr_prsnl_reltn_qual[index].encntr_prsnl_reltn_id = acm_request->
     encntr_prsnl_reltn_qual[index].encntr_prsnl_reltn_id
     SET reply->encntr_prsnl_reltn_qual[index].status = 1
     SET reply->status_data.status = "S"
   ENDCASE
 ENDFOR
 IF ((xref->del_cnt > 0))
  EXECUTE acm_del_encntr_prsnl_reltn
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->act_cnt > 0))
  EXECUTE acm_act_encntr_prsnl_reltn
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->chg_cnt > 0))
  EXECUTE acm_chg_encntr_prsnl_reltn
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->add_cnt > 0))
  EXECUTE acm_add_encntr_prsnl_reltn
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
