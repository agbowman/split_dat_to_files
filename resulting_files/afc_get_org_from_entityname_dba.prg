CREATE PROGRAM afc_get_org_from_entityname:dba
 ...
 SELECT INTO "nl:"
  FROM cs_org_reltn cs,
   organization o
  PLAN (cs
   WHERE (cs.cs_org_reltn_type_cd=request->cs_org_reltn_type_cd)
    AND (cs.key1_id=request->key1_id)
    AND ((cs.active_ind+ 0)=true))
   JOIN (o
   WHERE o.organization_id=cs.organization_id
    AND ((o.active_ind+ 0)=true))
  DETAIL
   cnt = (cnt+ 1), stat = alterlist(reply->sched_list,cnt), reply->sched_list[cnt].cs_org_reltn_id =
   cs.cs_org_reltn_id,
   reply->sched_list[cnt].organization_id = cs.organization_id, reply->sched_list[cnt].org_name = o
   .org_name, reply->sched_list[cnt].beg_effective_dt_tm = cs.beg_effective_dt_tm,
   reply->sched_list[cnt].end_effective_dt_tm = cs.end_effective_dt_tm
  WITH nocounter
 ;end select
 IF (curqual=0)
  CALL exitservicenodata("No organization relationship found for the input",true)
 ENDIF
...
#exit_script
 ...
 ENDIF
 SET stat = alterlist(reply->sched_list,cnt)
END GO
