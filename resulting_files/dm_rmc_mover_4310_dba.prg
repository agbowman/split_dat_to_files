CREATE PROGRAM dm_rmc_mover_4310:dba
 ...
 DECLARE cust_root_name = vc
 DECLARE cust_lt_id = vc
 DECLARE cust_lb_id = vc
 DECLARE cust_lt_t_id = vc
 DECLARE cust_lb_t_id = vc
 DECLARE cust_lb_col_pos = i4
 DECLARE cust_lt_col_pos = i4
  ...
  IF (cust_lb_id=cust_lt_id)
   ...
   IF (cust_lb_t_id != cust_lt_t_id)
    ...
    UPDATE  FROM dm_merge_translate
     ...
     WITH nocounter
    ;end update
    SET temp_$r_tbl = cutover_tab_name("LONG_BLOB_REFERENCE","")
    SELECT INTO "NL:"
     FROM (parser(temp_$r_tbl))
     WHERE long_blob_id=cnvtreal(cust_lt_t_id)
     WITH nocounter
    ;end select
    IF (curqual > 0)
     DELETE  FROM (parser(temp_$r_tbl))
      WHERE long_blob_id=cnvtreal(cust_lb_t_id)
     ;end delete
    ELSE
     UPDATE  FROM (parser(temp_$r_tbl))
