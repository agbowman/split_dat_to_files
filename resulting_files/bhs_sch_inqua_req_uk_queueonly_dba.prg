CREATE PROGRAM bhs_sch_inqua_req_uk_queueonly:dba
 ...
 RECORD reply(
   ...
   1 query_qual[*]
     ...
     2 referral_type = vc
     2 wl_status = vc
     2 wl_status_end_dt_tm = dq8
     2 wl_status_updt_dt_tm = dq8
     2 wl_status_reason = vc
     2 availability = vc
 ...
 DECLARE dcabcd = f8 WITH constant(loadcodevalue(23010,"CAB",1))
 DECLARE dcab_pref_val = f8 WITH noconstant(0.0)
 DECLARE dcab_event_alias_cd = f8 WITH constant(loadcodevalue(26881,"CABEVENT",1))
 IF (dcabcd > 0.0)
  SELECT INTO "nl:"
   a.pref_id
   FROM sch_pref a
   PLAN (a
    WHERE a.pref_type_cd=dcabcd
     AND a.parent_table="SYSTEM"
     AND a.parent_id=0
     AND a.version_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00"))
   DETAIL
    dcab_pref_val = a.pref_value
   WITH nocounter
  ;end select
 ENDIF
 SET reply->attr_qual_cnt = 54
 ...
 SET reply->attr_qual[t_index].attr_label = uar_i18ngetmessage(i18nhandle,"Referral Type",
  "Referral Type")
 SET reply->attr_qual[t_index].attr_type = "vc"
 SET t_index = (t_index+ 1)
 SET reply->attr_qual[t_index].attr_name = "wl_status"
 SET reply->attr_qual[t_index].attr_label = uar_i18ngetmessage(i18nhandle,"WL Status","WL Status")
 SET reply->attr_qual[t_index].attr_type = "vc"
 SET reply->attr_qual[t_index].attr_def_seq = 6
 SET t_index = (t_index+ 1)
 SET reply->attr_qual[t_index].attr_name = "wl_status_end_dt_tm"
 SET reply->attr_qual[t_index].attr_label = uar_i18ngetmessage(i18nhandle,"WL Status End Date",
  "WL Status End Date")
 SET reply->attr_qual[t_index].attr_type = "dq8"
 SET t_index = (t_index+ 1)
 SET reply->attr_qual[t_index].attr_name = "wl_status_updt_dt_tm"
 SET reply->attr_qual[t_index].attr_label = uar_i18ngetmessage(i18nhandle,"WL Status Update Date",
  "WL Status Update Date")
 SET reply->attr_qual[t_index].attr_type = "dq8"
 SET t_index = (t_index+ 1)
 SET reply->attr_qual[t_index].attr_name = "wl_status_reason"
 SET reply->attr_qual[t_index].attr_label = uar_i18ngetmessage(i18nhandle,"WL Status Reason",
  "WL Status Reason")
 SET reply->attr_qual[t_index].attr_type = "vc"
 SET t_index = (t_index+ 1)
 ...
 SET reply->attr_qual[t_index].attr_type = "vc"
 SET t_index = (t_index+ 1)
 SET reply->attr_qual[t_index].attr_name = "ordering_provider"
 SET reply->attr_qual[t_index].attr_label = uar_i18ngetmessage(i18nhandle,"Ordering Provider",
  "Ordering Provider")
 SET reply->attr_qual[t_index].attr_type = "vc"
 SET t_index = (t_index+ 1)
 SET reply->attr_qual[t_index].attr_name = "order_date"
 SET reply->attr_qual[t_index].attr_label = uar_i18ngetmessage(i18nhandle,"Order Date","Order Date")
 SET reply->attr_qual[t_index].attr_type = "dq8"
 SET t_index = (t_index+ 1)
 SET reply->attr_qual[t_index].attr_name = "order_loc"
 SET reply->attr_qual[t_index].attr_label = uar_i18ngetmessage(i18nhandle,"Order Location",
  "Order Location")
 SET reply->attr_qual[t_index].attr_type = "vc"
 IF (dcab_pref_val > 0.0)
  SET t_index = (t_index+ 1)
  SET reply->attr_qual[t_index].attr_name = "ubrn"
  SET reply->attr_qual[t_index].attr_label = uar_i18ngetmessage(i18nhandle,"UBRN","UBRN")
  SET reply->attr_qual[t_index].attr_type = "vc"
 ENDIF
 ...
 SELECT
  IF ((t_record->consultant_id > 0.0))
   ...
   PLAN (a
    ...
    JOIN (sea
    WHERE sea.sch_event_id=outerjoin(e.sch_event_id)
     AND sea.active_ind=outerjoin(1)
     AND sea.event_alias_type_cd=outerjoin(dcab_event_alias_cd)
     AND sea.version_dt_tm=outerjoin(cnvtdatetime("31-DEC-2100 00:00:00.00")))
   ORDER BY a.sch_action_id
  ELSE
   epr_exists = evaluate(nullind(epr.encntr_prsnl_reltn_id),1,0,1), a.queue_id
   FROM sch_entry a,
    sch_event_action ea,
    sch_event e,
    person p,
  ...
  HEAD a.sch_action_id
   ...
   ENDIF
   reply->query_qual[reply->query_qual_cnt].hide#schentryid = a.sch_entry_id, reply->query_qual[reply
   ->query_qual_cnt].hide#scheventid = a.sch_event_id, reply->query_qual[reply->query_qual_cnt].
   hide#scheduleid = a.schedule_id,
   reply->query_qual[reply->query_qual_cnt].hide#scheduleseq = e.schedule_seq, reply->query_qual[
   reply->query_qual_cnt].hide#reqactionid = a.sch_action_id, reply->query_qual[reply->query_qual_cnt
   ].hide#actionid = ea.req_action_id,
   reply->query_qual[reply->query_qual_cnt].hide#schapptid = a.sch_appt_id, reply->query_qual[reply->
   query_qual_cnt].hide#statemeaning = e.sch_meaning, reply->query_qual[reply->query_qual_cnt].
   hide#earliestdttm = cnvtdatetime(a.earliest_dt_tm),
   reply->query_qual[reply->query_qual_cnt].hide#latestdttm = cnvtdatetime(a.latest_dt_tm), reply->
   query_qual[reply->query_qual_cnt].hide#reqmadedttm = cnvtdatetime(a.request_made_dt_tm), reply->
   query_qual[reply->query_qual_cnt].hide#entrystatemeaning = a.entry_state_meaning,
   reply->query_qual[reply->query_qual_cnt].hide#reqactionmeaning = a.req_action_meaning, reply->
   query_qual[reply->query_qual_cnt].hide#encounterid = a.encntr_id, reply->query_qual[reply->
   query_qual_cnt].hide#personid = a.person_id,
   ...
   ELSE
    reply->query_qual[reply->query_qual_cnt].earliest_dt_tm = 0
   ENDIF
   reply->query_qual[reply->query_qual_cnt].req_action_display = uar_get_code_display(ea
   ...
   ENDIF
   IF (sea.sch_event_id > 0.0)
    reply->query_qual[reply->query_qual_cnt].ubrn = cnvtalias(sea.alias,sea.alias_pool_cd)
   ELSE
    reply->query_qual[reply->query_qual_cnt].ubrn = ""
   ENDIF
  FOOT REPORT
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   reply->query_qual[d.seq].referral_type = uar_get_code_display(wl.referral_type_cd), reply->
   query_qual[d.seq].wl_status = uar_get_code_display(wl.status_cd), reply->query_qual[d.seq].
   wl_status_end_dt_tm = wl.status_end_dt_tm,
   reply->query_qual[d.seq].wl_status_updt_dt_tm = wl.status_dt_tm, reply->query_qual[d.seq].
   wl_status_reason = uar_get_code_display(wl.reason_for_change_cd), reply->query_qual[d.seq].
   availability = uar_get_code_display(wl.stand_by_cd),
   reply->query_qual[d.seq].guaranteed_by_dt_tm = wl.admit_guaranteed_dt_tm, reply->query_qual[d.seq]
   .status_cd = wl.status_cd, reply->query_qual[d.seq].adj_waiting_start_dt_tm = wl
   .adj_waiting_start_dt_tm,
   reply->query_qual[d.seq].waiting_end_dt_tm = wl.waiting_end_dt_tm, reply->query_qual[d.seq].
   suspended_days = wl.suspended_days, reply->query_qual[d.seq].status_dt_tm = wl.status_dt_tm,
   reply->query_qual[d.seq].waiting_start_dt_tm = wl.waiting_start_dt_tm
  WITH nocounter
 ;end select
 ...
 ;end select
 SELECT INTO "nl:"
  oapr_exists = evaluate(nullind(oapr.alias_pool_cd),1,0,1), al.sch_action_id
  FROM (dummyt d  WITH seq = value(reply->query_qual_cnt)),
   sch_action_loc al,
   location l,
   org_alias_pool_reltn oapr,
   person_alias pa
  PLAN (d
   WHERE (reply->query_qual[d.seq].hide#reqactionid > 0))
   JOIN (al
   WHERE (al.sch_action_id=reply->query_qual[d.seq].hide#reqactionid))
   JOIN (l
   WHERE l.location_cd=al.location_cd)
   JOIN (oapr
   WHERE oapr.organization_id=outerjoin(l.organization_id)
    AND oapr.alias_entity_name=outerjoin("PERSON_ALIAS")
    AND oapr.alias_entity_alias_type_cd=outerjoin(mrn_cd)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   ...
   JOIN (a
   WHERE (a.sch_event_id=reply->query_qual[d.seq].hide#scheventid)
    AND a.sch_action_id IN (reply->query_qual[d.seq].hide#reqactionid, reply->query_qual[d.seq].
   hide#actionid)
 ...
 IF ((t_record->max_order_cnt > 0))
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d)
    ...
    JOIN (od2
    WHERE od2.order_id=outerjoin(oa.order_id)
     AND od2.action_sequence=outerjoin(oa.action_sequence)
     AND od2.oe_field_meaning=outerjoin("ORDERLOC"))
   ORDER BY d.seq, d2.seq, o.order_id,
    od.oe_field_id, od.action_sequence DESC, od2.action_sequence
   HEAD d.seq
    t_index = 0
   HEAD d2.seq
    t_index = 0
   HEAD o.order_id
    reply->query_qual[d.seq].order_date = o.orig_order_dt_tm, reply->query_qual[d.seq].
    ordering_provider = substring(1,100,p.name_full_formatted)
   ...
   DETAIL
    IF (flag=1
     AND od_exists=1
     AND od.oe_field_value=stat_cd)
     reply->query_qual[d.seq].stat = uar_i18ngetmessage(i18nhandle,"Yes","Yes")
    ENDIF
    IF (size(reply->query_qual[d.seq].order_loc)=0)
     reply->query_qual[d.seq].order_loc = od2.oe_field_display_value
    ENDIF
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
