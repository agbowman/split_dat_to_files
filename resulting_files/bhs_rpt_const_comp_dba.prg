CREATE PROGRAM bhs_rpt_const_comp:dba
 PROMPT
 ...
 SET echo_ind = 1
 SET rpt_name = "Patients with Constant Companion orders"
 SET prg_name = cnvtlower(curprog)
 DECLARE mf_constantcompanion_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",200,
   "CONSTANTCOMPANION"))
 DECLARE mrn_cd = f8
 SET mrn_cd = 0.0
 ...
 SELECT INTO  $OUTDEV
  ...
  PLAN (ed
   ...
   WHERE o.encntr_id=e.encntr_id
    AND o.catalog_cd=mf_constantcompanion_cd
  ...
  DETAIL
   ...
   mnem = trim(o.ordered_as_mnemonic), col 005, "{b/6}Order: ",
   mnem, col 061, "{b/18}Date/Time Ordered: ",
   o.orig_order_dt_tm"mm/dd/yy hh:mm;;q", ln_feed, init_wrap
