CREATE PROGRAM ams_email_file:dba
 PAINT
 ...
 END ;Subroutine
 SET last_mod = "010"
 DECLARE promptuserforfile(null) = null WITH protect
 DECLARE setemailaddress(null) = null WITH protect
 DECLARE title_line = c75 WITH protect, constant(
  "                          AMS Email File Utility                           ")
 DECLARE detail_line = c75 WITH protect, constant(
  "                   Email a file from a backend directory                   ")
 ...
 IF (loadprefs(script_name)=0)
  SET prefs->list[1].value = "NO"
  CALL setprefs(script_name,0)
  CALL loadprefs(script_name)
 ENDIF
#main_menu
 CALL drawmenu(title_line,detail_line,"")
 CALL text((soffrow+ 4),(soffcol+ 27),"1 Send a file")
 CALL text((soffrow+ 5),(soffcol+ 27),"2 Set default email address")
 CALL text((soffrow+ 6),(soffcol+ 27),"3 View/Set Preferences")
 CALL text((soffrow+ 7),(soffcol+ 27),"4 Exit")
 CALL accept(quesrow,(soffcol+ 18),"9;",4
  WHERE curaccept IN (1, 2, 3, 4))
 CASE (curaccept)
  OF 1:
   CALL promptuserforfile(null)
  OF 2:
   CALL setemailaddress(null)
  OF 3:
  ...
  OF 4:
   GO TO exit_script
 ENDCASE
 SUBROUTINE promptuserforfile(null)
   DECLARE done = i2 WITH protect, noconstant(0)
   DECLARE fileind = i2 WITH protect, noconstant(0)
   WHILE (done=0)
     CALL clearscreen(null)
     WHILE (fileind=0)
       CALL text(soffrow,soffcol,
        "Enter case sensitive filename (if not in CCLUSERDIR include logical):")
       CALL accept((soffrow+ 1),(soffcol+ 1),"P(74);C")
       IF (cnvtupper(curaccept)="QUIT")
        GO TO main_menu
       ELSEIF (findfile(curaccept))
        SET email_request->filenamestr = curaccept
        SET fileind = 1
       ELSE
        CALL text((soffrow+ 2),soffcol,"File not found.")
        CALL text((soffrow+ 3),soffcol,
         "Make sure file is in CCLUSERDIR or include logical (CER_PRINT:TEST.CSV)")
       ENDIF
     ENDWHILE
     ...
     CALL text(quesrow,soffcol,"Correct? (Y)es (N)o (M)ain Menu:")
     CALL accept(quesrow,(soffcol+ 33),"A;CU","Y"
      WHERE curaccept IN ("Y", "N", "M"))
     IF (curaccept="Y")
      CALL clearscreen(null)
      EXECUTE ams_run_email_file  WITH replace("REQUEST",email_request), replace("REPLY",email_reply)
      CALL text(soffrow,soffcol,substring(1,70,email_reply->errorstr))
      CALL text(quesrow,soffcol,"Continue? (Y)es:")
      CALL accept(quesrow,(soffcol+ 17),"A;CU","Y"
       WHERE curaccept IN ("Y"))
      SET done = 1
     ELSEIF (curaccept="M")
      GO TO main_menu
   ...
   GO TO main_menu
 END ;Subroutine
 SUBROUTINE setemailaddress(null)
   CALL clearscreen(null)
   CALL text(soffrow,soffcol,"Enter default email address (sets email for user in HNAUser.exe):")
   CALL accept((soffrow+ 1),(soffcol+ 1),"P(74);C")
   SELECT INTO "nl:"
    p.email
    FROM prsnl p
   ...
   ;end select
   UPDATE  FROM prsnl p
    SET p.email = curaccept
    WHERE (p.person_id=reqinfo->updt_id)
    WITH nocounter
   ;end update
   IF (curqual=1)
    CALL text(quesrow,soffcol,"Correct? (Y)es (N)o:")
    CALL accept(quesrow,(soffcol+ 21),"A;CU","Y"
     WHERE curaccept IN ("Y", "N"))
    IF (curaccept="Y")
     COMMIT
     CALL text((soffrow+ 2),soffcol,"Update completed.")
    ELSE
     ROLLBACK
     CALL text((soffrow+ 2),soffcol,"Update aborted.")
    ENDIF
   ELSE
    ROLLBACK
    CALL text((soffrow+ 2),soffcol,"Error updating email. Changes rolled back")
   ENDIF
   CALL clear(quesrow,soffcol,numcols)
   CALL text(quesrow,soffcol,"Continue? (Y)es:")
   CALL accept(quesrow,(soffcol+ 17),"A;CU","Y"
