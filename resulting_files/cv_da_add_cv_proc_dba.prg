CREATE PROGRAM cv_da_add_cv_proc:dba
 IF (validate(stat)=0)
 ...
 SUBROUTINE (cv_log_msg_post(script_vrsn=vc) =null)
  ...
  ENDIF
 END ;Subroutine
 DECLARE cv_da_add_cv_proc_vrsn = vc WITH private, constant("20091117")
 ...
 DECLARE activitysubtype_us_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",5801,
   "ULTRASOUND"))
 ...
 WHILE (i <= size(request->objarray,5))
   SET did = 0.0
   IF (validate(request->objarray[i].cv_proc_id,- (0.00001)) <= 0.0)
    SET did = id_list->item[i].id
    SET request->objarray[i].cv_proc_id = did
   ELSE
    SET did = request->objarray[i].cv_proc_id
   ENDIF
   IF (validate(reply->insert_ids)=1)
    SET reply->insert_ids[i].id = did
   ENDIF
   INSERT  FROM cv_proc c
    SET c.cv_proc_id = did, c.order_id =
     ...
     ,
     c.accession =
     IF (validate(request->objarray[i].accession,char(128)) != char(128)) validate(request->objarray[
       i].accession,char(128))
     ELSE ""
     ENDIF
     , c.catalog_cd =
     ...
     ENDIF
     , c.group_event_id =
     IF ((validate(request->objarray[i].group_event_id,- (0.00001)) != - (0.00001))) validate(request
       ->objarray[i].group_event_id,- (0.00001))
     ELSE 0.0
     ENDIF
     , c.sequence =
     ...
     IF (validate(request->objarray[i].activity_subtype_cd,activitysubtype_us_cd) !=
     activitysubtype_us_cd) validate(request->objarray[i].activity_subtype_cd,activitysubtype_us_cd)
     ELSE activitysubtype_us_cd
     ENDIF
     , c.ed_review_ind =
     IF ((validate(request->objarray[i].ed_review_ind,- (1)) != - (1))) validate(request->objarray[i]
       .ed_review_ind,- (1))
     ELSE 0
     ENDIF
     , c.orig_order_dt_tm =
     IF (validate(request->objarray[i].orig_order_dt_tm,0.0) > 0.0) cnvtdatetime(validate(request->
        objarray[i].orig_order_dt_tm,0.0))
     ELSE null
     ENDIF
     ,
 ...
 END ;Subroutine
#end_program
 CALL cv_log_msg_post("004 05/24/16 VJ043510")
END GO
