CREATE PROGRAM bhs_diabetes_pat_status:dba
 PROMPT
 ...
 ;end select
 IF (( $TYPE=1))
  SELECT INTO "problem_patient_status.xls"
   name = t_record->pat_qual[d.seq].name, id = t_record->pat_qual[d.seq].pid
   FROM (dummyt d  WITH seq = t_record->pat_cnt)
   PLAN (d)
   ORDER BY name, id
   HEAD REPORT
    t_line = concat( $PROBLEM," Patient Status Report for ",t_record->name), col 0, t_line,
 ...
 ELSEIF (( $TYPE=2))
  ...
  ;end select
 ENDIF
 IF (findfile("problem_patient_status.xls")=1)
  SET email_list =  $EMAIL
  SET subject_line = concat( $PROBLEM," Patient Status")
  CALL emailfile("problem_patient_status.xls","problem_patient_status.xls",email_list,subject_line,1)
