CREATE PROGRAM bhs_rpt_pmr_orders:dba
 PROMPT
 ...
 DECLARE ms_month = vc WITH protect
 ...
 IF (( $FTPFILES=1))
  ...
  SET ms_month = trim(cnvtlower(format(sysdate,"MMM;;D")),3)
  ...
  IF (cnvtupper(trim( $S_RANGE,3))="MONTHLY")
   SET ms_year = trim(substring(1,4,build(year(cnvtdatetime(ms_start_date)))),3)
   SET ms_month = trim(cnvtlower(format(cnvtdatetime(ms_start_date),"MMM;;D")),3)
  ENDIF
  ...
  ELSEIF (( $OUTPUT_TYPE != "CSV"))
   ...
   ENDIF
   IF (cnvtupper(trim( $S_RANGE,3))="MONTHLY")
    SET ms_var_outpmr = build(ms_files_loc,ms_name_fac,"_",ms_fileprefix,"_monthly_",
     ms_month,"_",ms_year,"_",ms_time,
     ".pdf")
   ELSE
    SET ms_var_outpmr = build(ms_files_loc,ms_name_fac,"_",ms_fileprefix,"_",
     ms_month,"_",ms_day,"_",ms_time,
 ...
 SELECT INTO "nl:"
  ...
  HEAD o.order_id
   ...
   pmr_consult->prsnl[pmr_consult->cnt_p].pmr_order[mi_ord].before_4pm = "N", pmr_consult->prsnl[
   pmr_consult->cnt_p].pmr_order[mi_ord].before_12pm = "N"
 ...
 IF (( $OUTPUT_TYPE="CSV"))
  SELECT
   ...
   order_id = pmr_consult->prsnl[d1.seq].pmr_order[d2.seq].order_id, ordered_date_time = format(
   ...
   due_date_12pm_order = format(pmr_consult->prsnl[d1.seq].pmr_order[d2.seq].due_date1,
    "mm/dd/yyyy hh:mm;;d"), due_date_4pm_order = format(pmr_consult->prsnl[d1.seq].pmr_order[d2.seq].
    due_date2,"mm/dd/yyyy hh:mm;;d"), discharge_date_time = format(pmr_consult->prsnl[d1.seq].
   ...
   totnot_on_time1 = pmr_consult->prsnl[d1.seq].totnot_on_time1, totnot_on_time2 = pmr_consult->
   prsnl[d1.seq].totnot_on_time2, percent_on_time1 = substring(1,30,pmr_consult->per_on_time1),
   percent_on_time2 = substring(1,30,pmr_consult->per_on_time2), personnel_on_time1 = substring(1,30,
