CREATE PROGRAM bbt_rpt_xm_modify:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 SET captions->crossmatch_mod_reasons = uar_i18ngetmessage(i18nhandle,"crossmatch_mod_reasons",
  "CROSSMATCH MODIFICATION REASONS")
 ...
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_xm_modify", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  ...
  PLAN (cv
   WHERE cv.code_set=14073)
   JOIN (cd
   WHERE cd.code_set=outerjoin(14073)
  ...
  HEAD PAGE
   ...
   curdate"@DATECONDENSED;;d", col 54, captions->database_audit,
   col 108, captions->page_no, col 120,
   curpage"##", row + 1, col 7,
   captions->rpt_time, col 14, curtime"@TIMENOSECONDS;;M",
   col 51, captions->reason_tool, row + 1,
   col 46, captions->crossmatch_mod_reasons, row + 1,
