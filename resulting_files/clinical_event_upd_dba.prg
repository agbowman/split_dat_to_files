CREATE PROGRAM clinical_event_upd:dba
 RECORD reply(
   1 array_size = i4
   1 num_inserted = i4
   1 error_code = i4
 ...
 UPDATE  FROM clinical_event t,
  ...
  SET t.clinical_event_id = evaluate2(
   ...
   t.normal_ref_range_txt = evaluate2(
    IF (validate(request->lst[d.seq].normal_ref_range_txt,t.normal_ref_range_txt)=" ") null
    ELSE validate(request->lst[d.seq].normal_ref_range_txt,t.normal_ref_range_txt)
    ENDIF
    ), t.ce_grouping_id = evaluate2(
    IF (validate(request->lst[d.seq].ce_grouping_id,t.ce_grouping_id)=0) null
    ELSE validate(request->lst[d.seq].ce_grouping_id,t.ce_grouping_id)
    ENDIF
    ), t.subtable_bit_map2 = evaluate2(
    IF (validate(request->lst[d.seq].subtable_bit_map2,t.subtable_bit_map2)=0) null
    ELSE validate(request->lst[d.seq].subtable_bit_map2,t.subtable_bit_map2)
    ENDIF
    )
  PLAN (d)
   JOIN (t
