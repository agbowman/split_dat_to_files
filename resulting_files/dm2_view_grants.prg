CREATE PROGRAM dm2_view_grants
 ...
 SUBROUTINE dpc_get_opt_code(in_mode)
   ...
   RETURN(out_code)
 END ;Subroutine
 DECLARE dvg_logfile_prefix = vc WITH protect, constant("dm2_view_grants")
 DECLARE dvg_env = vc WITH protect, noconstant("")
 DECLARE dvg_node = vc WITH protect, noconstant("")
 DECLARE dvg_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dvg_cont = i2 WITH protect, noconstant(1)
 DECLARE dvg_os_type = vc WITH protect, noconstant("")
 DECLARE dvg_input = vc WITH protect, noconstant("")
 DECLARE dvg_script = vc WITH protect, noconstant("")
 DECLARE dvg_mode = vc WITH protect, noconstant("RULEONLY")
 DECLARE dvg_display_report(ddr_script=vc,ddr_mode=vc) = i2
 SET width = 132
 IF (check_logfile(dvg_logfile_prefix,".log","dm2_view_grants")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_view_grants"
 CALL disp_msg("",dm_err->logfile,0)
 SET dvg_env = trim(logical("environment"))
 SET dvg_node = trim(curnode)
 SET dvg_os_type = trim(cursys2)
 SET dm_err->eproc = "Getting environment_id for current node/env"
 SELECT INTO "nl:"
  FROM dm_environment de
  WHERE de.environment_name=cnvtupper(dvg_env)
  DETAIL
   dvg_env_id = de.environment_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Current Environment: ",dvg_env))
  CALL echo(concat("Current Environment Id: ",cnvtstring(dvg_env_id)))
  CALL echo(concat("Current Node: ",cnvtupper(dvg_node)))
  CALL echo(concat("Current OS Type: ",dvg_os_type))
 ENDIF
 SET dm_err->eproc = "Validating input parameters"
 SET dvg_input = cnvtupper( $1)
 IF ( NOT (dvg_input IN ("PROMPT", "MINE")))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Invalid input parameter - should be 'MINE' or 'PROMPT'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (dvg_input="MINE")
  SET dvg_script = cnvtupper( $2)
  SET dvg_mode = cnvtupper( $3)
  IF ( NOT (dvg_mode IN ("ALL", "RULEONLY")))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Invalid input parameter - should be 'ALL' or 'RULEONLY'"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  IF (dvg_mode="ALL"
   AND trim(dvg_script,3)=char(42))
   SET dm_err->err_ind = 1
   SET dm_err->emsg =
   "Script name (*) and mode (ALL) is not allowed due to too much generated output."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  IF (dvg_display_report(dvg_script,dvg_mode)=0)
   GO TO exit_program
  ENDIF
 ELSE
  WHILE (dvg_cont=1)
    ...
    CALL box(1,1,3,132)
    CALL box(1,1,24,132)
    CALL text(2,2,"VIEW CCL GRANTS [REPORT]")
    CALL text(2,70,"DATE/TIME: ")
    CALL text(2,80,format(cnvtdatetime(curdate,curtime3),"DD-MMM-YYYY HH:MM:SS;;D"))
    CALL text(4,2,concat("Current Environment:  ",cnvtupper(dvg_env)," (",trim(cnvtstring(dvg_env_id)
       ),")"))
    CALL text(4,70,concat("Current Application Node:  ",cnvtupper(dvg_node)))
    CALL text(9,5,"SCRIPT NAME (Wildcards accepted): ")
    CALL text(11,5,"OPTIMIZER CRITERIA (RULEONLY/ALL): ")
    CALL text(16,2,"(M)odify Input, (V)iew Grants, (B)ack: ")
    CALL text(20,2,"** Enter the Script Name (or pattern) for which you would like to view grants.")
    SET accept = nopatcheck
    CALL accept(9,45,"P(30);CU",dvg_script
     WHERE curaccept > " ")
    SET dvg_script = curaccept
    CALL clear(20,2,100)
    CALL text(20,2,'** Enter the Optimizer Mode Criteria. Valid values are "RULEONLY" and "ALL".')
    CALL accept(11,45,"P(8);CU",dvg_mode
     WHERE curaccept IN ("RULEONLY", "ALL"))
    SET dvg_mode = curaccept
    CALL clear(20,2,100)
    CALL accept(16,43,"A;CU"," "
     WHERE curaccept IN ("M", "V", "B"))
    SET message = nowindow
    CASE (curaccept)
     OF "M":
      SET dvg_cont = 1
 ...
 ENDIF
 GO TO exit_program
 SUBROUTINE dvg_display_report(ddr_script,ddr_mode)
   ...
   IF ((dm_err->debug_flag > 0))
    CALL echo(concat("SCRIPT: ",ddr_script))
    CALL echo(concat("MODE: ",ddr_mode))
   ENDIF
   IF (dpc_get_curr_cclgrants(ddr_script)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Generating CCL Grants Summary Report"
   CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
   IF (get_unique_file("dm2_grant_rpt",".dat")=0)
    GO TO exit_program
   ELSE
   ...
   SELECT INTO value(ddr_rpt_file)
    ...
    HEAD REPORT
     ...
     CALL print("CCL SCRIPT GRANTS [SUMMARY REPORT]"), col 80,
    ...
    DETAIL
     col 2, "SCRIPT CRITERIA:  ", col 30,
     CALL print(ddr_script), row + 1, col 2,
     "OPTIMIZER CRITERIA:  ", col 30,
     CALL print(ddr_mode),
     row + 1,
     CALL print(fillstring(250,"-")), row + 2,
     ddr_cnt = 0
     FOR (ddr_cnt = 1 TO dpc_data->grant_cnt)
       IF ((((dpc_data->grants[ddr_cnt].rule_grant_ind=1)) OR (ddr_mode="ALL"
        AND (dpc_data->grants[ddr_cnt].rule_grant_ind=0))) )
        col 2,
        CALL print(build(dpc_data->grants[ddr_cnt].script_group)), row + 1,
        col 10, "QUERY NUMBER ", col 40,
        "OPTIMIZER MODE", row + 1, col 10,
        CALL print(fillstring(20,"-")), col 40,
        CALL print(fillstring(20,"-")),
        row + 1
        IF ((((dpc_data->grants[ddr_cnt].dyn_query_rdbopt=1)) OR (ddr_mode="ALL"
         AND (dpc_data->grants[ddr_cnt].dyn_query_rdbopt != 1))) )
         col 10, "0 (Dynamic Queries)", col 40,
         CALL print(build(dpc_get_opt_mode(dpc_data->grants[ddr_cnt].dyn_query_rdbopt))), row + 1
        ENDIF
        IF ((dpc_data->grants[ddr_cnt].all_equal_ind=1))
         IF ((((dpc_data->grants[ddr_cnt].all_grant_value=1)) OR (ddr_mode="ALL"
          AND (dpc_data->grants[ddr_cnt].all_grant_value != 1))) )
          col 10, "1 - 50", col 40,
          CALL print(build(dpc_get_opt_mode(dpc_data->grants[ddr_cnt].all_grant_value))), row + 1
         ENDIF
        ELSE
         FOR (ddr_cnt2 = 1 TO 50)
           IF ((((dpc_data->grants[ddr_cnt].query_grant_value[ddr_cnt2]=1)) OR (ddr_mode="ALL"
            AND (dpc_data->grants[ddr_cnt].query_grant_value[ddr_cnt2] != 1))) )
            col 10,
            CALL print(build(ddr_cnt2)), col 40,
            CALL print(build(dpc_get_opt_mode(dpc_data->grants[ddr_cnt].query_grant_value[ddr_cnt2]))
            ), row + 1
           ENDIF
         ENDFOR
        ENDIF
        IF ((((dpc_data->grants[ddr_cnt].rdboptdefault=1)) OR (ddr_mode="ALL"
         AND (dpc_data->grants[ddr_cnt].rdboptdefault != 1))) )
         col 10, "> 50", col 40,
         CALL print(build(dpc_get_opt_mode(dpc_data->grants[ddr_cnt].rdboptdefault))), row + 1
        ENDIF
        row + 1
       ENDIF
     ENDFOR
   ...
   IF (check_error(dm_err->eproc))
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Displaying CCL Grants Summary"
   IF (dm2_disp_file(ddr_rpt_file,"CCL Grants Summary Report")=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "Ending dm2_view_grants"
 CALL final_disp_msg(dvg_logfile_prefix)
END GO
