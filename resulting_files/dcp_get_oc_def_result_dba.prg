CREATE PROGRAM dcp_get_oc_def_result:dba
 RECORD reply(
   1 task_assay_cd = f8
...
#resize_reply
 ...
 SELECT INTO "nl:"
  o.oe_field_meaning, o.action_sequence, o.oe_field_value,
  oef.field_type_flag
  FROM order_detail o,
  ...
  PLAN (o
   WHERE (o.order_id=request->order_id)
    AND (o.oe_field_meaning=request->dta_mnemonic))
   JOIN (d1
   WHERE d1.seq=1)
   JOIN (oef
   WHERE oef.oe_field_id=o.oe_field_id)
  ORDER BY o.action_sequence DESC
  HEAD o.oe_field_meaning
   reply->default_result = o.oe_field_display_value, reply->oe_field_value = o.oe_field_value, reply
   ->oe_field_dt_tm_value = o.oe_field_dt_tm_value,
   reply->oe_field_code_set = oef.codeset, reply->oe_field_type_flag = oef.field_type_flag
  DETAIL
   row + 0
  WITH nocounter
 ;end select
 ...
 CALL echo(build("field type:",reply->oe_field_type_flag))
 CALL echo(build("def rslt:",reply->default_result))
 CALL echo(build("field value:",reply->oe_field_value))
 CALL echo(build("code set:",reply->oe_field_code_set))
 CALL echo(build("dt tm value:",reply->oe_field_dt_tm_value))
 SET reply->call_event_server_ind = 0
 SET reply->status_data.status = "S"
