CREATE PROGRAM dcp_rdm_add_phx_ver_mrp:dba
 ...
 SET readme_data->message = "Readme Failed:  Script dcp_rdm_add_phx_ver_mrp "
 DECLARE phx_section_request = i4 WITH constant(1349805), protect
 DECLARE section_display = vc WITH constant("Pregnancy History Revisions"), protect
 ...
 IF (error_code != 0)
  SET readme_data->message = concat(
   "FAILURE: dcp_rdm_add_phx_ver_mrp failed on existence check. Request number: ",trim(build(
 ...
 IF (error_code != 0)
  SET readme_data->message = concat(
   "FAILURE: dcp_rdm_add_phx_ver_mrp failed to insert section. Request number: ",trim(build(
     phx_section_request)),". Error Message: ",error_msg)
  ROLLBACK
  SET failure_ind = 1
 ELSE
...
#exit_script
 IF (failure_ind=0)
  ...
  IF (phxrequestexists=1)
   ...
   "Success - Pregnancy History Versions already exists in CHART_DISCERN_REQUEST."
  ELSE
   SET readme_data->message =
   "Success - Pregnancy History Versions added to CHART_DISCERN_REQUEST successfully."
  ENDIF
 ENDIF
