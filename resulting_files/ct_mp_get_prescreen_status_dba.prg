CREATE PROGRAM ct_mp_get_prescreen_status:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Code Value:" = 0.0
  WITH outdev, codevalue
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "ct_mp_get_prescreen_status"
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE getprescreenstatus(null) = null WITH protect
 DECLARE applicationcd = f8 WITH protect, noconstant(0.0)
 DECLARE codevalue = f8 WITH protect, constant( $CODEVALUE)
 SET stat = uar_get_meaning_by_codeset(17914,"MANAGER",1,applicationcd)
 SET public::"reply"->status_data.status = "F"
 IF (( $CODEVALUE=0))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_prescreen_status","No Code Value",
   PUBLIC::"reply")
  GO TO exit_script
 ENDIF
 IF (applicationcd > 0)
  CALL getprescreenstatus(null)
 ELSE
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_prescreen_status","No Application Code",
   PUBLIC::"reply")
 ENDIF
 SET public::"reply"->status_data.status = "S"
 SUBROUTINE getprescreenstatus(null)
   CALL log_message("In GetPrescreenStatus()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   DECLARE status_mean = vc WITH protect
   SET status_mean = uar_get_code_meaning(codevalue)
   SELECT
    FROM code_value cv,
     code_value_group cvg
    PLAN (cvg
     WHERE cvg.parent_code_value=applicationcd)
     JOIN (cv
    ...
    DETAIL
     skip = 0
     IF (status_mean != "PENDING"
      AND cv.cdf_meaning="PENDING")
      skip = 1
     ENDIF
     IF (status_mean != "REFERRED"
      AND cv.cdf_meaning="REFERRED")
      skip = 1
     ENDIF
     IF (((status_mean="DECLINED") OR (status_mean="DECLINEDPT"))
      AND cv.cdf_meaning="NOTQUAL")
      skip = 1
     ENDIF
     IF (skip=0)
      index += 1, stat = alterlist(public::"reply"->"codes",index), public::"reply"->codes[index].
      "code" = cv.code_value,
      public::"reply"->codes[index]."display" = cv.display, public::"reply"->codes[index]."meaning"
       = cv.cdf_meaning
     ENDIF
    WITH nocounter
   ;end select
   CALL log_message(build("Exit GetPrescreenStatus(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
#exit_script
 IF (g_debug_ind=1)
