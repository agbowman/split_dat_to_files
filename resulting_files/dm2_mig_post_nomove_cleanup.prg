CREATE PROGRAM dm2_mig_post_nomove_cleanup
 ...
 SUBROUTINE dm2_get_rdbms_version(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dpnc_cmd = vc WITH protect, noconstant("")
 DECLARE dpnc_dir = vc WITH protect, noconstant("")
 DECLARE dpnc_cnt = i4 WITH protect, noconstant(0)
 FREE RECORD dpnc_clean
 RECORD dpnc_clean(
   1 nmuser_cnt = i4
   1 user_available_for_cleanup = i2
   1 nmuser[*]
     2 user = vc
     2 user_exists = i2
   1 nmtbl_cnt = i4
   1 table_available_for_cleanup = i2
   1 nmtbl[*]
     2 tbl = vc
     2 tbl_owner = vc
     2 tbl_exists = i2
   1 syn_cnt = i4
   1 syn_available_for_cleanup = i2
   1 syn[*]
     2 owner = vc
     2 syn = vc
     2 tbl = vc
 ...
 IF (check_logfile("dm2_mignmvclean",".log","dm2_mig_post_nomove_cleanup")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_mig_post_nomove_cleanup"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dmr_prompt_retrieve_mig_data(0,1,1,0)=0)
  GO TO exit_program
 ENDIF
 IF (dmr_issue_summary_screen(1,1,concat(
   "Continuing this process will drop NOMOVE objects from the TARGET database."))=0)
  GO TO exit_program
 ENDIF
 IF (dmr_prompt_connect_data("SOURCE","V500","CO")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Load NOMOVE tables and users."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain IN ("DM2_MIG_EXCL", "DM2_MIG_DT_USER")
   AND d.info_char="0"
   AND  NOT (d.info_number IN (16, 17))
   AND  NOT (d.info_name IN ("V500.DM2_DDL_OPS1", "V500.DM2_DDL_OPS_LOG1"))
  DETAIL
   IF (d.info_domain="DM2_MIG_DT_USER")
    IF (d.info_name="V500")
     dm_err->emsg = "V500 is not a valid NO MOVE user.", dm_err->err_ind = 1,
     CALL cancel(1)
    ENDIF
    dpnc_clean->nmuser_cnt = (dpnc_clean->nmuser_cnt+ 1), stat = alterlist(dpnc_clean->nmuser,
     dpnc_clean->nmuser_cnt), dpnc_clean->nmuser[dpnc_clean->nmuser_cnt].user = d.info_name
   ELSE
    dpnc_clean->nmtbl_cnt = (dpnc_clean->nmtbl_cnt+ 1), stat = alterlist(dpnc_clean->nmtbl,dpnc_clean
     ->nmtbl_cnt), dpnc_clean->nmtbl[dpnc_clean->nmtbl_cnt].tbl_owner = substring(1,(findstring(".",d
      .info_name) - 1),d.info_name),
    dpnc_clean->nmtbl[dpnc_clean->nmtbl_cnt].tbl = substring((findstring(".",d.info_name)+ 1),size(d
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Load synonyms that are against a NOMOVE users tables or NOMOVE tables."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dba_synonyms d
  WHERE  NOT (d.table_owner IN (
  (SELECT
   info_name
   FROM dm_info
   ...
   AND d.db_link = null
  DETAIL
   IF (locateval(dpnc_cnt,1,dpnc_clean->nmuser_cnt,d.table_owner,dpnc_clean->nmuser[dpnc_cnt].user)
    > 0)
    dpnc_clean->syn_cnt = (dpnc_clean->syn_cnt+ 1), stat = alterlist(dpnc_clean->syn,dpnc_clean->
     syn_cnt), dpnc_clean->syn[dpnc_clean->syn_cnt].owner = d.owner,
    dpnc_clean->syn[dpnc_clean->syn_cnt].syn = d.synonym_name, dpnc_clean->syn[dpnc_clean->syn_cnt].
    tbl = d.table_name, dpnc_clean->syn[dpnc_clean->syn_cnt].tbl_owner = d.table_owner
   ELSEIF (locateval(dpnc_cnt,1,dpnc_clean->nmtbl_cnt,d.table_owner,dpnc_clean->nmtbl[dpnc_cnt].
    tbl_owner,
    d.table_name,dpnc_clean->nmtbl[dpnc_cnt].tbl) > 0)
    dpnc_clean->syn_cnt = (dpnc_clean->syn_cnt+ 1), stat = alterlist(dpnc_clean->syn,dpnc_clean->
     syn_cnt), dpnc_clean->syn[dpnc_clean->syn_cnt].owner = d.owner,
    dpnc_clean->syn[dpnc_clean->syn_cnt].syn = d.synonym_name, dpnc_clean->syn[dpnc_clean->syn_cnt].
    tbl = d.table_name, dpnc_clean->syn[dpnc_clean->syn_cnt].tbl_owner = d.table_owner
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 IF ((dmr_mig_data->tgt_ora_level1 > 10))
  SET dm_err->eproc = "Load PLAN_TABLES for users being migrated."
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
   FROM dba_tables d
   WHERE d.table_name="PLAN_TABLE"
    AND d.owner IN (
   (SELECT
    info_name
    FROM dm_info d
   ...
   DETAIL
    dpnc_clean->nmtbl_cnt = (dpnc_clean->nmtbl_cnt+ 1), stat = alterlist(dpnc_clean->nmtbl,dpnc_clean
     ->nmtbl_cnt), dpnc_clean->nmtbl[dpnc_clean->nmtbl_cnt].tbl_owner = d.owner,
    dpnc_clean->nmtbl[dpnc_clean->nmtbl_cnt].tbl = d.table_name
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 IF ((dpnc_clean->nmtbl_cnt=0)
  AND (dpnc_clean->nmuser_cnt=0))
  SET dm_err->emsg = "No tables or users were designated as NO MOVE."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ...
 ENDIF
 IF ((dpnc_clean->nmtbl_cnt > 0))
  SET dm_err->eproc = "Check which tables exist."
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
   FROM dba_tables dt,
    (dummyt d  WITH seq = value(dpnc_clean->nmtbl_cnt))
   PLAN (d)
    JOIN (dt
    WHERE (dpnc_clean->nmtbl[d.seq].tbl_owner=dt.owner)
     AND (dpnc_clean->nmtbl[d.seq].tbl=dt.table_name))
   HEAD REPORT
    dpnc_clean->table_available_for_cleanup = 1
   DETAIL
    dpnc_clean->nmtbl[d.seq].tbl_exists = 1
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 IF ((dpnc_clean->syn_cnt > 0))
  SET dm_err->eproc = "Check which synonyms exist."
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
   FROM dba_synonyms dt,
    (dummyt d  WITH seq = value(dpnc_clean->syn_cnt))
   PLAN (d)
    JOIN (dt
    WHERE (dpnc_clean->syn[d.seq].owner=dt.owner)
     AND (dpnc_clean->syn[d.seq].syn=dt.synonym_name))
   HEAD REPORT
    dpnc_clean->syn_available_for_cleanup = 1
   DETAIL
    dpnc_clean->syn[d.seq].syn_exists = 1
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 IF ((dpnc_clean->nmuser_cnt > 0))
  SET dm_err->eproc = "Check which users exist."
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
   FROM dba_users dt,
    (dummyt d  WITH seq = value(dpnc_clean->nmuser_cnt))
   PLAN (d)
    JOIN (dt
    WHERE (dpnc_clean->nmuser[d.seq].user=dt.username))
   HEAD REPORT
    dpnc_clean->user_available_for_cleanup = 1
   DETAIL
    dpnc_clean->nmuser[d.seq].user_exists = 1
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 IF ((dpnc_clean->user_available_for_cleanup=0)
  AND (dpnc_clean->table_available_for_cleanup=0)
  AND (dpnc_clean->syn_available_for_cleanup=0))
  SET dm_err->emsg = "No tables, Synonyms or users are available to cleanup."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (get_unique_file("dm2mignmrpt",".rpt")=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dpnc_clean)
 ENDIF
 SET dm_err->eproc = "Generate report"
 SELECT INTO value(cnvtlower(dm_err->unique_fname))
  FROM (dummyt d  WITH seq = 1)
  HEAD REPORT
   col 0,
   "**************************************************************************************************************",
   row + 1,
   col 0, "Cerner Database Post Migration No Move Cleanup Report", row + 1,
   col 0,
   "**************************************************************************************************************",
   row + 2
  DETAIL
   IF ((dpnc_clean->table_available_for_cleanup=1))
    col 0, "The following table(s) are designated as NO MOVE and will be DROPPED", row + 1,
    col 0, "--------------------------------------------------------------------", row + 1
    FOR (dpnc_cnt = 1 TO dpnc_clean->nmtbl_cnt)
      col 2,
      CALL print(concat(dpnc_clean->nmtbl[dpnc_cnt].tbl_owner,".",dpnc_clean->nmtbl[dpnc_cnt].tbl)),
      row + 1
    ENDFOR
   ENDIF
   row + 2
   IF ((dpnc_clean->user_available_for_cleanup=1))
    col 0, "The following user(s) are designated as NO MOVE and will be DROPPED", row + 1,
    col 0, "-------------------------------------------------------------------", row + 1
    FOR (dpnc_cnt = 1 TO dpnc_clean->nmuser_cnt)
      col 2, dpnc_clean->nmuser[dpnc_cnt].user, row + 1
    ENDFOR
   ENDIF
   row + 2
   IF ((dpnc_clean->syn_available_for_cleanup=1))
    col 0, "The following Synonyms(s) associated to table(s) will be DROPPED", row + 1,
    col 0, "-------------------------------------------------------------------", row + 1
    FOR (dpnc_cnt = 1 TO dpnc_clean->syn_cnt)
      col 0,
      CALL print(concat(dpnc_clean->syn[dpnc_cnt].owner,".",dpnc_clean->syn[dpnc_cnt].syn," on ",
       dpnc_clean->syn[dpnc_cnt].tbl_owner,
       ".",dpnc_clean->syn[dpnc_cnt].tbl)), row + 1
    ENDFOR
   ENDIF
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 IF (dm2_disp_file(dm_err->unique_fname,"Database Migration Post Migration No Move Cleanup Report")=0
 )
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Obtain verification from user to continue cleanup"
 SET message = window
 CALL clear(1,1)
 CALL box(1,1,6,131)
 CALL text(2,5,"Please enter 'C' if you reviewed the report and would like to proceed with cleanup.")
 CALL text(3,5,"Please enter 'Q' to quit process and abort cleanup.")
 CALL text(5,5,"Enter 'C' to continue or 'Q' to quit (C or Q): ")
 CALL accept(5,60,"P;CU"," "
  WHERE curaccept IN ("C", "Q"))
 IF (curaccept="Q")
  SET message = nowindow
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "User choose to Quit No Move cleanup process."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSE
  CALL clear(1,1)
  CALL box(1,1,4,131)
  CALL text(2,5,
   "Are you sure you would like to proceed with cleanup? Tables,Synonyms and users from report will be dropped ('Y'es or 'N'o):"
   )
  CALL accept(2,128,"P;CU"," "
   WHERE curaccept IN ("Y", "N"))
  IF (curaccept="N")
   SET message = nowindow
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "User choose to Quit No Move cleanup process at second prompt."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 ...
 IF ((dpnc_clean->table_available_for_cleanup=1))
  FOR (dpnc_cnt = 1 TO dpnc_clean->nmtbl_cnt)
    IF ((dpnc_clean->nmtbl[dpnc_cnt].tbl_exists=1))
     SET dm_err->eproc = concat("Dropping NO-MOVE Table:",dpnc_clean->nmtbl[dpnc_cnt].tbl_owner,".",
      dpnc_clean->nmtbl[dpnc_cnt].tbl,". ",
      build(dpnc_cnt)," of ",build(dpnc_clean->nmtbl_cnt))
     CALL disp_msg("",dm_err->logfile,0)
     SET dpnc_cmd = concat("rdb drop table ",dpnc_clean->nmtbl[dpnc_cnt].tbl_owner,".",dpnc_clean->
      nmtbl[dpnc_cnt].tbl," cascade constraints purge go")
     IF (dm2_push_cmd(dpnc_cmd,1)=0)
      GO TO exit_program
     ENDIF
    ENDIF
  ENDFOR
 ...
 IF ((dpnc_clean->user_available_for_cleanup=1))
  FOR (dpnc_cnt = 1 TO dpnc_clean->nmuser_cnt)
    IF ((dpnc_clean->nmuser[dpnc_cnt].user_exists=1))
     SET dm_err->eproc = concat("Dropping NO-MOVE User:",dpnc_clean->nmuser[dpnc_cnt].user,". ",build
      (dpnc_cnt)," of ",
      build(dpnc_clean->nmuser_cnt))
     CALL disp_msg("",dm_err->logfile,0)
     EXECUTE dm2_drop_user dpnc_clean->nmuser[dpnc_cnt].user
     IF ((dm_err->err_ind > 0))
      GO TO exit_program
     ENDIF
    ENDIF
 ...
 IF ((dpnc_clean->syn_available_for_cleanup=1))
  FOR (dpnc_cnt = 1 TO dpnc_clean->syn_cnt)
    IF ((dpnc_clean->syn[dpnc_cnt].syn_exists=1))
     SET dm_err->eproc = concat("Dropping Synonym(s):",dpnc_clean->syn[dpnc_cnt].owner,".",dpnc_clean
      ->syn[dpnc_cnt].syn,". ",
      build(dpnc_cnt)," of ",build(dpnc_clean->syn_cnt))
     CALL disp_msg("",dm_err->logfile,0)
     IF ((dpnc_clean->syn[dpnc_cnt].owner="PUBLIC"))
      SET dpnc_cmd = concat("rdb drop public synonym ",dpnc_clean->syn[dpnc_cnt].syn," go")
     ELSE
      SET dpnc_cmd = concat("rdb drop synonym ",dpnc_clean->syn[dpnc_cnt].owner,".",dpnc_clean->syn[
       dpnc_cnt].syn," go")
     ENDIF
     IF (dm2_push_cmd(dpnc_cmd,1)=0)
      GO TO exit_program
 ...
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "dm2_mig_post_nomove_cleanup completed"
 CALL final_disp_msg("dm2_mignmvclean")
END GO
