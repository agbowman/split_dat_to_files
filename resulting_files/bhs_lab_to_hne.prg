CREATE PROGRAM bhs_lab_to_hne
 ...
 SELECT INTO "nl:"
  ...
  WITH noformat
 ;end select
 DECLARE mf_loinc_cd = f8 WITH protect, noconstant(uar_get_code_by("DISPLAYKEY",400,"LOINC"))
 DECLARE mf_hneffnonbhphmoppo_cd = f8 WITH protect, noconstant(uar_get_code_by("DISPLAYKEY",354,
   "HNEFFNONBHPHMOPPO"))
 DECLARE mf_healthnewengland_cd = f8 WITH protect, noconstant(uar_get_code_by("DISPLAYKEY",354,
   "HEALTHNEWENGLAND"))
 SELECT INTO value(printer)
  eventloinc = substring(1,10,n1.source_identifier), event2loinc = substring(1,10,n2
   .source_identifier), pat_name = substring(1,28,p.name_full_formatted),
  admit = substring(1,8,format(e.reg_dt_tm,"mm/dd/yy;;d")), etype = substring(1,2,cva2.alias), pnbr
   = substring(1,20,ep.member_nbr),
  dob = substring(1,10,format(cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p.birth_tz),1),
    "mm/dd/yyyy;;d")), catalog = substring(1,16,uar_get_code_display(ce.catalog_cd)), event =
  substring(1,15,uar_get_code_display(ce.event_cd)),
  event_dt = substring(1,10,format(ce.event_end_dt_tm,"mm/dd/yyyy hh:mm;;q")), facility = trim(
   uar_get_code_display(e.loc_facility_cd)), fin_class = uar_get_code_display(e.financial_class_cd),
  event2 = substring(1,15,uar_get_code_display(ce2.event_cd)), result = substring(1,30,concat(trim(
     ce2.result_val))), units = substring(1,10,uar_get_code_display(ce2.result_units_cd)),
  proc = substring(1,5,cva.alias), ssn = substring(1,9,pa.alias)"#########;rp0", acct = substring(1,
   10,ea.alias),
  ce.event_id, ce.parent_event_id, ce.encntr_id,
  ce.event_cd, e.encntr_type_cd
  FROM encounter e,
   ...
   encntr_alias ea,
   ref_cd_map_header rh1,
   ref_cd_map_header rh2,
   ref_cd_map_detail rd1,
   ref_cd_map_detail rd2,
   nomenclature n1,
   nomenclature n2
  PLAN (e
   ...
   59174347, 59174348, 348704876.0, 234271927.0, mf_hneffnonbhphmoppo_cd,
   mf_healthnewengland_cd)))
   JOIN (ep
   ...
   JOIN (rh1
   WHERE (rh1.event_id= Outerjoin(ce.event_id)) )
   JOIN (rd1
   WHERE (rd1.ref_cd_map_header_id= Outerjoin(rh1.ref_cd_map_header_id)) )
   JOIN (n1
   WHERE (n1.nomenclature_id= Outerjoin(rd1.nomenclature_id))
    AND (n1.source_vocabulary_cd= Outerjoin(mf_loinc_cd)) )
   JOIN (rh2
   WHERE (rh2.event_id= Outerjoin(ce2.event_id)) )
   JOIN (rd2
   WHERE (rd2.ref_cd_map_header_id= Outerjoin(rh2.ref_cd_map_header_id)) )
   JOIN (n2
   WHERE (n2.nomenclature_id= Outerjoin(rd2.nomenclature_id))
    AND (n2.source_vocabulary_cd= Outerjoin(mf_loinc_cd)) )
  ...
  DETAIL
   ...
   ELSE
    col 121, "           "
   ENDIF
   col 133, delim, col 134
   IF (textlen(eventloinc) > 0)
    eventloinc
   ELSE
    "          "
   ENDIF
   col 145, delim, col 146,
   event2loinc, row + 1
  WITH maxcol = 170
 ;end select
 FREE SET dclcom
