CREATE PROGRAM bhs_phys_missing_npi:dba
 PROMPT
  "Output to File/Printer/MINE" = "Jane.Cullinan@bhs.org"
  WITH outdev
 IF (findstring("@", $1) > 0)
 ...
 SELECT INTO value(output_dest)
  ...
  HEAD REPORT
   ...
   "ORG ID", ",", "Ext ID",
   ",", "UPIN", ",",
   "License", row + 1, display_line = build(md_alias->qual[d.seq].name)
   FOR (y = 1 TO size(md_alias->qual[d.seq],5))
     IF ((md_alias->qual[y].npi_nbr=null))
      xperson_id = format(md_alias->qual[y].personid,"#########"), output_string = build(',"',
       ...
       '","',md_alias->qual[y].doc_upin,'","',md_alias->qual[y].license_nbr,'",'), col 1,
      output_string
      IF ( NOT (curendreport))
       row + 1
      ENDIF
 ...
 IF (email_ind=1)
  ...
  SET filename_out = concat(format(curdate,"YYYY-MM-DD;;D"),"_PHYS_missing_NPI.csv")
  DECLARE subject_line = vc
  SET subject_line = concat(curprog,"-V1.x - PHYS missing NPI ",curnode)
