CREATE PROGRAM dm_ocd_install_runner:dba
 PAINT
 SET width = 132
 ...
 SUBROUTINE drr_alert_killed_appl(daka_load_ind,daka_fmt_appl_id,daka_kill_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE doir_toolset_usage = i2
 SET doir_toolset_usage = dm2_toolset_usage(null)
 CASE (doir_toolset_usage)
  OF 0:
   SET message = nowindow
   CALL echo("*")
   CALL echo("*")
  ...
  OF 1:
   SET message = nowindow
   EXECUTE dm2_install_runner
   GO TO 9999_exit_program
 ENDCASE
 EXECUTE FROM 1000_initialize TO 1999_initialize_exit
 EXECUTE FROM 2000_loop TO 2999_loop_exit
 GO TO 9999_exit_program
 SUBROUTINE ir_status(s_text)
   CALL echo(s_text)
 END ;Subroutine
#1000_initialize
 FREE RECORD ir_data
...
#2000_loop
 ...
 CALL clear(1,1)
 CALL box(1,1,3,132)
 CALL text(2,3,"O C D   I N S T A L L   R U N N E R")
 CALL text(5,3,
  "This program periodically scans for OCD components to install.  If components are found, control is  "
  )
 CALL text(6,3,
  "is given to CCL and installation occurs until no more components are available to install (which     "
  )
 CALL text(7,3,
  "may take several minutes).  When no more components exist, control returns to this program and       "
  )
 CALL text(8,3,
  "it may be exited.                                                                                    "
  )
 CALL text(10,3,
  "It is recommended that one or more instances of this program remain running during any OCD          "
  )
 CALL text(11,3,
  "installation to reduce overall OCD installation time.                                               "
  )
 SET accept = time(20)
 CALL text(13,3,
