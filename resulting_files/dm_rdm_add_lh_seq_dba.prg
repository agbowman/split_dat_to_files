CREATE PROGRAM dm_rdm_add_lh_seq:dba
 ...
 SET readme_data->message = "Readme failed: starting script dm_rdm_add_lh_seq..."
 DECLARE errmsg = vc WITH protect, noconstant("")
 DECLARE seqexists = i2 WITH protect, noconstant(0)
 DECLARE sequencename = vc WITH protect, noconstant("LH_SEQ")
 SELECT INTO "nl:"
 ...
 ;end select
 IF (seqexists=1)
  CALL createsynonym(sequencename)
...
#exit_script
 ...
 SUBROUTINE createsynonym(name)
   CALL parser(concat("rdb create or replace public synonym ",name))
   CALL parser(concat("for ",name))
   CALL parser("go")
   IF (error(errmsg,0) != 0)
    SET readme_data->status = "F"
    SET readme_data->message = concat("Failed to create public synonym ",name,": ",errmsg)
    GO TO exit_script
   ENDIF
 END ;Subroutine
END GO
