CREATE PROGRAM dm_eem_batch_rows:dba
 ...
 SET reply->table_name = "EEM_BATCH"
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    minval = min(eb.eem_batch_id)
    FROM eem_batch eb
    WHERE eb.eem_batch_id > 0
    DETAIL
  ...
  WHILE (v_cur_min_id <= v_max_id
   AND v_rows_left > 0)
    SELECT INTO "nl:"
     eb.rowid
     FROM eem_batch eb
     WHERE parser(sbr_getrowidnotexists("eb.updt_dt_tm < cnvtdatetime(v_days_back)","eb"))
      AND eb.eem_batch_id BETWEEN v_cur_min_id AND v_cur_max_id
     DETAIL
      rows = (rows+ 1)
      IF (mod(rows,100)=1)
       stat = alterlist(reply->rows,(rows+ 99))
      ENDIF
      reply->rows[rows].row_id = eb.rowid
     WITH nocounter, maxqual(eb,value(v_rows_left))
    ;end select
