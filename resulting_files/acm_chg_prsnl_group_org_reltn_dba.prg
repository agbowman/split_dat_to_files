CREATE PROGRAM acm_chg_prsnl_group_org_reltn:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->prsnl_group_org_reltn_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
   prsnl_group_org_reltn p
  SET p.prsnl_group_org_reltn_id = acm_request->prsnl_group_org_reltn_qual[xref->chg[d.seq].idx].
   prsnl_group_org_reltn_id, p.prsnl_group_id =
   IF ((acm_request->prsnl_group_org_reltn_qual[xref->chg[d.seq].idx].prsnl_group_id > 0))
    acm_request->prsnl_group_org_reltn_qual[xref->chg[d.seq].idx].prsnl_group_id
   ELSE reply->prsnl_group_qual[acm_request->prsnl_group_org_reltn_qual[xref->chg[d.seq].idx].
    prsnl_group_idx].prsnl_group_id
   ENDIF
   , p.organization_id = acm_request->prsnl_group_org_reltn_qual[xref->chg[d.seq].idx].
   organization_id,
   p.reltn_type_cd = acm_request->prsnl_group_org_reltn_qual[xref->chg[d.seq].idx].reltn_type_cd, p
   .updt_cnt = (p.updt_cnt+ 1), p.updt_dt_tm = cnvtdatetime(curdate,curtime3),
   p.updt_id = reqinfo->updt_id, p.updt_applctx = reqinfo->updt_applctx, p.updt_task = reqinfo->
   updt_task
  PLAN (d)
   JOIN (p
   WHERE (p.prsnl_group_org_reltn_id=acm_request->prsnl_group_org_reltn_qual[xref->chg[d.seq].idx].
   prsnl_group_org_reltn_id))
  WITH nocounter, status(reply->prsnl_group_org_reltn_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->prsnl_group_org_reltn_qual[xref->chg[index].idx].status != 1))
    SET failed = update_error
    SET table_name = "PRSNL_GROUP_ORG_RELTN"
    GO TO exit_script
   ENDIF
 ENDFOR
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    ...
    WHERE expand(t1,f_val,t_val,p.prsnl_group_org_reltn_id,acm_request->prsnl_group_org_reltn_qual[
     xref->chg[t1].idx].prsnl_group_org_reltn_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.prsnl_group_org_reltn_id,acm_request->
      prsnl_group_org_reltn_qual[xref->chg[t1].idx].prsnl_group_org_reltn_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((p.updt_cnt=acm_request->prsnl_group_org_reltn_qual[idx].updt_cnt)) OR ((acm_request->
     force_updt_ind=1))) )
      reply->prsnl_group_org_reltn_qual[idx].status = - (1), reply->prsnl_group_org_reltn_qual[idx].
      prsnl_group_org_reltn_id = acm_request->prsnl_group_org_reltn_qual[idx].
      prsnl_group_org_reltn_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->prsnl_group_org_reltn_qual[idx].chg_str
     IF (findstring("PRSNL_GROUP_ID,",chg_str)=0)
      acm_request->prsnl_group_org_reltn_qual[idx].prsnl_group_id = p.prsnl_group_id
     ENDIF
     IF (findstring("ORGANIZATION_ID,",chg_str)=0)
      acm_request->prsnl_group_org_reltn_qual[idx].organization_id = p.organization_id
     ENDIF
     IF (findstring("RELTN_TYPE_CD,",chg_str)=0)
      acm_request->prsnl_group_org_reltn_qual[idx].reltn_type_cd = p.reltn_type_cd
     ENDIF
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->prsnl_group_org_reltn_qual[xref->chg[index].idx].status=0))
