CREATE PROGRAM ec_profiler_m97:dba
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   icnt = 0
  DETAIL
   startpos = findstring('layout="comparison-graph"',ltr.long_text)
   IF (startpos > 0)
    icnt = (icnt+ 1), stat = alterlist(section_hold->qual,icnt), section_hold->qual[icnt].section_id
     = crs.section_id
   ELSE
    startpos = findstring('layout="ref-range-graph"',ltr.long_text)
    IF (startpos > 0)
     icnt = (icnt+ 1), stat = alterlist(section_hold->qual,icnt), section_hold->qual[icnt].section_id
      = crs.section_id
    ENDIF
