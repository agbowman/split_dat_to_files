CREATE PROGRAM aps_prt_worklist_summary:dba
 ...
 SET captions->contin = uar_i18ngetmessage(i18nhandle,"key666","(cont.)")
 SET captions->rpt = uar_i18ngetmessage(i18nhandle,"key1","REPORT: APS_PRT_WORKLIST_SUMMARY.PRG")
 SET captions->ana = uar_i18ngetmessage(i18nhandle,"key2","Anatomic Pathology")
 SET captions->dt = uar_i18ngetmessage(i18nhandle,"key3","DATE:")
 SET captions->tm = uar_i18ngetmessage(i18nhandle,"key4","TIME:")
 SET captions->dir = uar_i18ngetmessage(i18nhandle,"key5","DIRECTORY:")
 SET captions->pro = uar_i18ngetmessage(i18nhandle,"key6","PROCESSING TASK WORKLIST - SUMMARY")
 SET captions->bye = uar_i18ngetmessage(i18nhandle,"key7","BY:")
 SET captions->pg = uar_i18ngetmessage(i18nhandle,"key8","PAGE:")
 SET captions->ser = uar_i18ngetmessage(i18nhandle,"key9","Service Resource:")
 SET captions->task = uar_i18ngetmessage(i18nhandle,"key10","            Task:")
 SET captions->all_task = uar_i18ngetmessage(i18nhandle,"key11","All Tasks")
 SET captions->cse = uar_i18ngetmessage(i18nhandle,"key12","            Case:")
 SET captions->batch_no = uar_i18ngetmessage(i18nhandle,"key13","    Batch Number:")
 SET captions->none_spec = uar_i18ngetmessage(i18nhandle,"key14","None specified")
 SET captions->task_stat = uar_i18ngetmessage(i18nhandle,"key15","     Task Status:")
 SET captions->cs = uar_i18ngetmessage(i18nhandle,"key16","CASE")
 SET captions->nm_id = uar_i18ngetmessage(i18nhandle,"key17","NAME/ID")
 SET captions->assign = uar_i18ngetmessage(i18nhandle,"key18","ASSIGNED")
 SET captions->blk_id = uar_i18ngetmessage(i18nhandle,"key19","BLOCKS/IDS")
 SET captions->no = uar_i18ngetmessage(i18nhandle,"key20","#")
 SET captions->fix = uar_i18ngetmessage(i18nhandle,"key21","FIXATIVE")
 SET captions->slide_id = uar_i18ngetmessage(i18nhandle,"key22","SLIDES/IDS")
 SET captions->spec = uar_i18ngetmessage(i18nhandle,"key23","SPECIMEN")
 SET captions->none = uar_i18ngetmessage(i18nhandle,"key24","NONE")
 SET captions->summ = uar_i18ngetmessage(i18nhandle,"key25","SUMMARY:")
 SET captions->comm = uar_i18ngetmessage(i18nhandle,"key26","COMMENTS:")
 SET captions->title = uar_i18ngetmessage(i18nhandle,"key27",
  "REPORT: PROCESSING TASK WORKLIST - SUMMARY")
 SET captions->no_cse = uar_i18ngetmessage(i18nhandle,"key28",
  "No cases found meeting established criteria.")
 SET captions->unable = uar_i18ngetmessage(i18nhandle,"key29",
  "* unable to show # / fixative information.")
 SET captions->cont = uar_i18ngetmessage(i18nhandle,"key30","CONTINUED...")
 SET captions->all_cse = uar_i18ngetmessage(i18nhandle,"key31","All Cases")
#script
 RECORD tmptext(
   1 qual[*]
     2 text = vc
 ...
 DECLARE staskstatuswhere = vc WITH protect, noconstant(" ")
 ...
 DECLARE first_blob = vc WITH protect, noconstant(" ")
 DECLARE last_blob = vc WITH protect, noconstant(" ")
 ...
 IF (nworklistnbrind=1)
  IF ((request->stask_status="V"))
   SET staskstatuswhere = "pt.status_cd+0 in (verified_code, processing_code)"
   SET task_status = "Verified tasks only"
  ELSEIF ((request->stask_status="U"))
   SET staskstatuswhere = "ordered_code = pt.status_cd+0"
   SET task_status = "Unverified tasks only"
  ELSEIF ((request->stask_status="B"))
   SET staskstatuswhere = "pt.status_cd+0 in (verified_code, ordered_code, processing_code)"
 ...
 ELSE
  ...
  ELSE
   IF ((request->stask_status="V"))
    SET staskstatuswhere = "pt.status_cd in (verified_code, processing_code)"
    SET task_status = "Verified tasks only"
   ELSEIF ((request->stask_status="U"))
    SET staskstatuswhere = "ordered_code = pt.status_cd"
    SET task_status = "Unverified tasks only"
   ELSEIF ((request->stask_status="B"))
    SET staskstatuswhere = "pt.status_cd in (verified_code, ordered_code, processing_code)"
    SET task_status = "Both Verified and Unverified tasks"
   ENDIF
  ENDIF
 ENDIF
  ...
  IF ((request->beg_acc > " ")
   ...
   PLAN (pc
    ...
    WHERE pt.case_id=pc.case_id
     AND (tempsrs->qual[d.seq].service_resource_cd=pt.service_resource_cd)
     AND parser(staskstatuswhere)
 ...
 SELECT INTO "nl:"
  ...
  HEAD pc.prefix_id
   tag_ctr = 0
  DETAIL
   temp1->qual[d.seq].accession_nbr = pc.accession_nbr, temp1->qual[d.seq].encntr_id = pc.encntr_id,
   temp1->qual[d.seq].res_person_id = pc.responsible_resident_id,
   temp1->qual[d.seq].path_person_id = pc.responsible_pathologist_id, temp1->qual[d.seq].prefix_cd =
   pc.prefix_id, temp1->qual[d.seq].person_name = pr.name_full_formatted
   IF (tag_ctr < 2)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   JOIN (d2)
   JOIN (ea
   WHERE (temp1->qual[d.seq].encntr_id=ea.encntr_id)
    AND ea.encntr_alias_type_cd=mrn_alias_type_cd
    AND ea.active_ind=1
    AND ea.beg_effective_dt_tm <= cnvtdatetime(sysdate)
    AND ea.end_effective_dt_tm >= cnvtdatetime(sysdate))
  DETAIL
   IF ((temp1->qual[d.seq].encntr_id=ea.encntr_id))
    temp1->qual[d.seq].patient_alias = frmt_mrn
   ELSE
    temp1->qual[d.seq].patient_alias = "Unknown"
   ENDIF
  WITH nocounter, outerjoin = d2
 ;end select
 IF (max_spec_ctr != 0)
 ...
 ENDIF
#report_section
 EXECUTE cpm_create_file_name_logical "aps_worklist_sum", "dat", "x"
 ...
 SELECT INTO reply->print_status_data.print_filename
  x = 0, temp1->qual[d.seq].accession_nbr
  FROM (dummyt d  WITH seq = value(size(temp1->qual,5)))
  PLAN (d
   WHERE (temp1->qual[d.seq].accession_nbr > " "))
  ORDER BY temp1->qual[d.seq].accession_nbr
  HEAD REPORT
   first_cass_tag = "       ", first_cass_tag_len = 0, last_cass_tag = "       ",
   cass_tag = "       ", line1 = fillstring(125,"-"), blob = fillstring(25," "),
   line2 = fillstring(116,"-"), cass_pieces = "   ", found_cases = 0,
   alias_printed = 0, bno_specimen = 0, bbreak = 0,
   scuraccession = fillstring(22," "), scurname = fillstring(23," "), bprintingsummary = 0,
   bmoretoprint = 0, lnumofresources = 0
  HEAD PAGE
   ...
   CALL center(captions->pro,row,132), col 112, captions->bye,
   col 117, request->scuruser"##############", row + 1,
   col 110, captions->pg, col 117,
   curpage"###", row + 1, col 0,
   captions->ser, lnumofresources = cnvtint(size(tempsrs->qual,5)), col 18
   FOR (loop = 1 TO lnumofresources)
    tempsrs->qual[loop].service_resource_disp,
    IF (lnumofresources > 1
     AND loop < lnumofresources)
     ",", col + 1
     IF (col >= 91)
      row + 1, col 18
     ENDIF
   ...
   IF ((request->beg_acc > " "))
    ...
    beg_acc, " through ", end_acc
   ELSE
    col 18, captions->all_cse
   ENDIF
   row + 1, col 0, captions->batch_no
   IF ((request->worklist_nbr > 0))
    col 18, request->worklist_nbr
   ELSE
    col 18, captions->none_spec
   ENDIF
   row + 1, col 0, captions->task_stat,
   col 18, task_status, row + 2,
   col 0, captions->cs, col 20,
   captions->nm_id, col 43, captions->assign,
   col 53, captions->blk_id, col 74,
   captions->no, col 78, captions->fix,
   col 89, captions->slide_id, col 113,
   captions->spec, row + 1, col 0,
   "-------------------", col 20, "--------------------",
   col 43, "--------", col 53,
   "--------------------", col 74, "---",
   col 78, "----------", col 89,
   ...
   row + 1
   IF (bbreak=1)
    col 0, scuraccession, col 20,
    captions->contin
    IF (bprintingsummary=1)
     row + 1
    ENDIF
    bbreak = 0, name_printed_row = row, alias_printed_row = (name_printed_row+ 1)
   ENDIF
  DETAIL
   IF (((row+ 6) > maxrow))
    bbreak = 1, BREAK
   ENDIF
   scuraccession = uar_fmt_accession(temp1->qual[d.seq].accession_nbr,size(trim(temp1->qual[d.seq].
   ...
   FOR (spec = 1 TO cnvtint(temp1->qual[d.seq].spec_ctr))
     ...
     IF (alias_printed_row=row)
      col 20, temp1->qual[d.seq].patient_alias"######################"
     ENDIF
     s_c_ctr = cnvtint(temp1->qual[d.seq].spec_qual[spec].s_c_ctr)
     IF (s_c_ctr > 0)
      col 53, "    ", col 53,
      s_c_ctr"##", ",", first_cass_tag = trim(temp1->qual[d.seq].spec_qual[spec].cass_qual[1].
       cass_tag),
      first_cass_tag_len = size(trim(temp1->qual[d.seq].spec_qual[spec].cass_qual[1].cass_tag)),
      last_cass_tag = trim(temp1->qual[d.seq].spec_qual[spec].cass_qual[s_c_ctr].cass_tag)
      IF (last_cass_tag > " "
       AND last_cass_tag != first_cass_tag)
       print_cass_string = concat(substring(1,first_cass_tag_len,first_cass_tag)," - ",last_cass_tag),
       col + 1, print_cass_string
      ELSE
       col + 1, first_cass_tag
      ENDIF
      bno_specimen = 0
     ELSE
      col 53, captions->none, bno_specimen = 1
     ENDIF
     IF (substring(12,79,temp1->qual[d.seq].spec_qual[spec].spec_descr) > " ")
      spec_descr = build(substring(1,12,temp1->qual[d.seq].spec_qual[spec].spec_descr),"...")
     ELSE
      spec_descr = temp1->qual[d.seq].spec_qual[spec].spec_descr
     ENDIF
     col 113, spec_descr, spec_tag = temp1->qual[d.seq].spec_qual[spec].spec_tag,
     first_blob = build(spec_tag)
     IF ((temp1->qual[d.seq].spec_qual[spec].s_slide_ctr > 0))
      first_blob = concat(build(spec_tag,sep1)," ",build(sep2,temp1->qual[d.seq].spec_qual[spec].
        slide_qual[1].sl_tag))
     ENDIF
     IF (first_blob > " ")
      last_slide_cntr = cnvtint(temp1->qual[d.seq].spec_qual[spec].s_slide_ctr)
      IF ((temp1->qual[d.seq].spec_qual[spec].s_slide_ctr > 0))
       last_blob = concat(build(spec_tag,sep1)," ",build(sep2,temp1->qual[d.seq].spec_qual[spec].
         slide_qual[last_slide_cntr].sl_tag))
      ELSE
       last_blob = ""
      ENDIF
      IF (last_blob > " ")
       IF (substring(21,5,first_blob) > " ")
        first_blob = concat(substring(1,17,first_blob),"...")
       ENDIF
       col 89, first_blob, nextcol = ((89+ textlen(first_blob))+ 1)
       IF (last_blob > " "
        AND last_blob != first_blob)
        col nextcol, "-", col + 1,
        last_blob
       ENDIF
      ENDIF
     ...
     FOR (cass1 = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].s_c_ctr))
       cass_tag = temp1->qual[d.seq].spec_qual[spec].cass_qual[cass1].cass_tag
       IF (substring(10,28,temp1->qual[d.seq].spec_qual[spec].cass_qual[cass1].cass_fixative_disp) >
       " ")
        cass_fixative_disp = build(substring(1,8,temp1->qual[d.seq].spec_qual[spec].cass_qual[cass1].
          cass_fixative_disp),"...")
       ELSE
        cass_fixative_disp = temp1->qual[d.seq].spec_qual[spec].cass_qual[cass1].cass_fixative_disp
       ENDIF
       IF (cnvtint(temp1->qual[d.seq].spec_qual[spec].cass_qual[cass1].s_c_slide_ctr) > 0)
        col 74, temp1->qual[d.seq].spec_qual[spec].cass_qual[cass1].cass_pieces"###", col 78,
        cass_fixative_disp
       ELSE
        IF (bprinted=0)
         bprinted = 1, col 74, "*",
         col 78, "*", bmoretoprint = 1,
         row + 1
        ENDIF
       ENDIF
       IF ((temp1->qual[d.seq].spec_qual[spec].cass_qual[cass1].s_c_slide_ctr > 0))
        first_blob = build(spec_tag,sep1,cass_tag,sep2,temp1->qual[d.seq].spec_qual[spec].cass_qual[
         cass1].slide_qual[1].s_tag)
        IF (first_blob > " ")
         last_slide_cntr = temp1->qual[d.seq].spec_qual[spec].cass_qual[cass1].s_c_slide_ctr
         IF (last_slide_cntr > 0)
          last_blob = build(spec_tag,sep1,cass_tag,sep2,temp1->qual[d.seq].spec_qual[spec].cass_qual[
           cass1].slide_qual[last_slide_cntr].s_tag)
         ELSE
          last_blob = ""
         ENDIF
         IF (substring(21,5,first_blob) > " ")
   ...
   IF (alias_printed_row=row)
    col 20, temp1->qual[d.seq].patient_alias"######################"
   ENDIF
   bprint = 0, bprintingsummary = 0
   FOR (spec = 1 TO cnvtint(temp1->qual[d.seq].spec_ctr))
     spec_tag = temp1->qual[d.seq].spec_qual[spec].spec_tag
     FOR (var2 = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].s_slide_ctr))
      ...
      FOR (var3 = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].slide_qual[var2].s_s_t_ctr))
        blob = concat(build(spec_tag,sep1)," ",build(sep2,temp1->qual[d.seq].spec_qual[spec].
          slide_qual[var2].sl_tag)), blob = concat(build(blob)," ",build(slide1_mod))
        IF (slide1_mod > " ")
         IF (bprint=0)
          row + 2, col 20, captions->summ,
          row + 1, bprint = 1, bprintingsummary = 1
         ENDIF
         col 25, blob, row + 1
        ENDIF
        IF (((row+ 6) > maxrow))
     ...
     FOR (cass1 = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].s_c_ctr))
       ...
       ->qual[d.seq].spec_qual[spec].cass_qual[cass1].cass_origin_modifier
       FOR (task1 = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].cass_qual[cass1].s_c_t_ctr))
         blob = build(spec_tag,sep1,cass_tag), blob = concat(build(blob)," ",build(cass_modifier))
         IF (cass_modifier > " ")
          IF (bprint=0)
           row + 2, col 20, captions->summ,
           row + 1, bprint = 1, bprintingsummary = 1
       ...
       FOR (slide2 = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].cass_qual[cass1].s_c_slide_ctr))
         ...
         s_origin_modifier
         FOR (task3 = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].cass_qual[cass1].slide_qual[
          slide2].s_c_s_t_ctr))
           blob = build(spec_tag,sep1,cass_tag,sep2,slide_tag)
           IF (textlen(trim(slide_modifier)) > 0
            AND textlen(trim(cass_modifier)) > 0)
            blob = concat(build(blob)," ",build(cass_modifier),"; ",build(slide_modifier))
           ELSEIF (textlen(trim(cass_modifier)) > 0)
            blob = concat(build(blob)," ",build(cass_modifier))
           ELSE
            blob = concat(build(blob)," ",build(slide_modifier))
           ENDIF
           IF (((cass_modifier > " ") OR (slide_modifier > " ")) )
            IF (bprint=0)
             row + 2, col 20, captions->summ,
             row + 1, bprint = 1, bprintingsummary = 1
            ENDIF
     ...
     ENDFOR
   ENDFOR
   bprintingsummary = 0, comments_printed = "N"
   FOR (spec = 1 TO cnvtint(temp1->qual[d.seq].spec_ctr))
     FOR (inner1 = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].s_t_ctr))
       IF ((temp1->qual[d.seq].spec_qual[spec].t_qual[inner1].t_comment_cnt > 0))
        IF (comments_printed="N")
         row + 1, col 20, captions->comm,
         row + 1
        ENDIF
        comments_printed = "Y", col 20, temp1->qual[d.seq].spec_qual[spec].t_qual[inner1].t_mnemonic
        "###############:"
        FOR (com_cnt = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].t_qual[inner1].t_comment_cnt))
          col 37, temp1->qual[d.seq].spec_qual[spec].t_qual[inner1].t_comment[com_cnt].text
          IF (((row+ 6) > maxrow))
           bbreak = 1, BREAK
          ENDIF
          row + 1
       ...
       ENDIF
     ENDFOR
     FOR (inner1 = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].s_slide_ctr))
       FOR (inner2 = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].slide_qual[inner1].s_s_t_ctr))
         IF ((temp1->qual[d.seq].spec_qual[spec].slide_qual[inner1].t_qual[inner2].t_comment_cnt > 0)
         )
          IF (comments_printed="N")
           row + 1, col 20, captions->comm,
           row + 1
          ENDIF
          comments_printed = "Y", col 20, temp1->qual[d.seq].spec_qual[spec].slide_qual[inner1].
          t_qual[inner2].t_mnemonic"###############:"
          FOR (com_cnt = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].slide_qual[inner1].t_qual[
           inner2].t_comment_cnt))
            col 37, temp1->qual[d.seq].spec_qual[spec].slide_qual[inner1].t_qual[inner2].t_comment[
            com_cnt].text
            IF (((row+ 6) > maxrow))
             bbreak = 1, BREAK
            ENDIF
       ...
       ENDFOR
     ENDFOR
     FOR (inner1 = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].s_c_ctr))
       FOR (inner2 = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].cass_qual[inner1].s_c_t_ctr))
         IF ((temp1->qual[d.seq].spec_qual[spec].cass_qual[inner1].t_qual[inner2].t_comment_cnt > 0))
          IF (comments_printed="N")
           row + 1, col 20, captions->comm,
           row + 1
          ENDIF
          comments_printed = "Y", col 20, temp1->qual[d.seq].spec_qual[spec].cass_qual[inner1].
          t_qual[inner2].t_mnemonic"###############:"
          FOR (com_cnt = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].cass_qual[inner1].t_qual[
           inner2].t_comment_cnt))
            col 37, temp1->qual[d.seq].spec_qual[spec].cass_qual[inner1].t_qual[inner2].t_comment[
            com_cnt].text
            IF (((row+ 6) > maxrow))
             bbreak = 1, BREAK
            ENDIF
     ...
     ENDFOR
     FOR (inner1 = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].s_c_ctr))
       FOR (inner2 = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].cass_qual[inner1].s_c_slide_ctr)
        )
         FOR (inner3 = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].cass_qual[inner1].slide_qual[
          inner2].s_c_s_t_ctr))
           IF ((temp1->qual[d.seq].spec_qual[spec].cass_qual[inner1].slide_qual[inner2].t_qual[inner3
           ].t_comment_cnt > 0))
            IF (comments_printed="N")
             row + 1, col 20, captions->comm,
             row + 1
            ENDIF
            comments_printed = "Y", col 20, temp1->qual[d.seq].spec_qual[spec].cass_qual[inner1].
            slide_qual[inner2].t_qual[inner3].t_mnemonic"###############:"
            FOR (com_cnt = 1 TO cnvtint(temp1->qual[d.seq].spec_qual[spec].cass_qual[inner1].
             slide_qual[inner2].t_qual[inner3].t_comment_cnt))
              col 37, temp1->qual[d.seq].spec_qual[spec].cass_qual[inner1].slide_qual[inner2].t_qual[
              inner3].t_comment[com_cnt].text
              IF (((row+ 6) > maxrow))
               bbreak = 1, BREAK
              ENDIF
              row + 1
