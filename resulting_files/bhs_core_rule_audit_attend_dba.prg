CREATE PROGRAM bhs_core_rule_audit_attend:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Starting Registration Date" = curdate,
  "Ending Discharge Date" = curdate,
  "Type in email address or leave default for report preview:" = "report preview"
 ...
 SELECT DISTINCT INTO "NL:"
  ...
  PLAN (ema
   ...
   WHERE e.encntr_id=emad.encntr_id
    AND ((e.reg_dt_tm+ 0) BETWEEN cnvtdatetime(start_disch_date) AND cnvtdatetime(end_disch_date)))
   JOIN (epr
 ...
 IF (email_ind=0)
  SELECT INTO value(var_output)
   ...
   HEAD REPORT
    ...
    today = format(curdate,"MMM-DD-YYYY;;D"), st_dt2 = build("{B}Audit From...",format(cnvtdate(
        $BEG_DT),"mm/dd/yy;;d"),"  To...    ",format(cnvtdate( $END_DT),"mm/dd/yy;;d")), col + 1,
    st_dt2, row + 2
   HEAD PAGE
    col 0, "{B}PAGE:", col + 1,
    curpage, row + 1, row + 1,
   ...
   HEAD doctor
    ...
    CALL print(trim(cnvtstring(cnvtreal(temp->qual[d1.seq].tot_admt_res)))),
    call reportmove('COL',(32+ 15),0),
    CALL print(trim(cnvtstring(cnvtreal(temp->qual[d1.seq].tot_admt_rule)))), call reportmove('COL',(
    42+ 15),0),
    ...
    CALL print(trim(cnvtstring(cnvtreal(((cnvtreal(temp->qual[d1.seq].tot_disch_res)/ cnvtreal(temp->
        qual[d1.seq].tot_disch_rule)) * 100))))), rpt_tot_adm_res = (temp->qual[d1.seq].tot_admt_res
    + rpt_tot_adm_res),
    ...
    rpt_tot_comp18 = (temp->qual[d1.seq].tot_comp_18+ rpt_tot_comp18), rpt_tot_comp12 = (temp->qual[
    d1.seq].tot_comp_12+ rpt_tot_comp12)
   FOOT REPORT
    row + 1, col 0, "{REPEAT/118/_/}",
 ...
 ELSEIF (email_ind=1)
  SELECT INTO value(var_output)
   ...
   tran_per_done = cnvtreal(((temp->qual[d1.seq].tot_tran_res/ temp->qual[d1.seq].tot_tran_rule) *
    100)), tran_comp12 = temp->qual[d1.seq].tot_comp_12, disch_comp = temp->qual[d1.seq].
   tot_disch_res,
