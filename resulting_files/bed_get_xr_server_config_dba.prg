CREATE PROGRAM bed_get_xr_server_config:dba
 ...
 SUBROUTINE check_log_level(arg_log_level)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "BED_GET_XR_SERVER_CONFIG"
 RECORD reply(
   1 environmententry[*]
     2 type_flag = i2
     2 type_value_txt = vc
 ...
 CALL getserverconfigurationentries(null)
 SUBROUTINE getserverconfigurationentries(null)
   CALL log_message("In GetServerConfigurationEntries()",log_level_debug)
   SELECT INTO "nl:"
    sc.cr_server_value_txt, sc.type_flag
    FROM cr_server_configuration sc
    WHERE sc.type_flag > 0
    HEAD REPORT
     enventrycnt = 0
    DETAIL
     enventrycnt = (enventrycnt+ 1)
     IF (mod(enventrycnt,5)=1)
      stat = alterlist(reply->environmententry,(enventrycnt+ 4))
     ENDIF
     reply->environmententry[enventrycnt].type_flag = sc.type_flag, reply->environmententry[
     enventrycnt].type_value_txt = sc.cr_server_value_txt
    FOOT REPORT
     stat = alterlist(reply->environmententry,enventrycnt)
    WITH nocounter
   ;end select
   IF (curqual > 0)
   ...
   ELSE
    SET reply->status_data.status = "Z"
   ENDIF
   CALL log_message("Exit GetServerConfigurationEntries()",log_level_debug)
 END ;Subroutine
#exit_script
 IF (validate(debug_ind,0))
  CALL echorecord(reply)
 ENDIF
 CALL log_message("End of script: bed_get_xr_server_config.",log_level_debug)
END GO
