CREATE PROGRAM ct_mp_chg_prescreen_status:dba
 PROMPT
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "ct_mp_chg_prescreen_status"
 ...
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_chg_prescreen_status",
   "Invalid/Partial Input Data",PUBLIC::"reply")
  GO TO exit_script
 ENDIF
 SET stat = cnvtjsontorec( $JSONREQUEST)
 IF ((request->pt_prot_prescreen_id > 0))
  CALL updateprotprescreenrecord(cnvtreal(request->pt_prot_prescreen_id),cnvtreal(request->status_cd),
   request->status_comment_text)
 ELSE
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_chg_prescreen_status","No Prescreen ID",
   PUBLIC::"reply")
  GO TO exit_script
 ENDIF
 FREE RECORD request
 SET public::"reply"->status_data.status = "S"
 SUBROUTINE (updateprotprescreenrecord(pt_prot_prescreen_id=f8,status_cd=f8,status_comment_text=vc) =
  null WITH protect)
   CALL log_message("In UpdateProtPrescreenRecord()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   ...
   IF (curqual > 0)
    ...
    IF (size(status_comment_text,1) > 0)
    ...
    ;end update
    IF (curqual=0)
     CALL populate_subeventstatus_rec("UPDATE","F","ct_mp_chg_prescreen_status",
      "Failed to update row in pt_prot_prescreen table.",PUBLIC::"reply")
     GO TO exit_script
    ENDIF
   ELSE
    CALL populate_subeventstatus_rec("LOCK","F","ct_mp_chg_prescreen_status",
     "Failed to lock row in pt_prot_prescreen table.",PUBLIC::"reply")
    GO TO exit_script
   ENDIF
   CALL log_message(build("Exit UpdateProtPrescreenRecord(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
#exit_script
 IF ((public::"reply"->status_data.status="F"))
