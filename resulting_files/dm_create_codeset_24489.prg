CREATE PROGRAM dm_create_codeset_24489
 RECORD pers(
   1 pers_cnt = i4
   1 qual[*]
     2 table_name = vc
 ...
 SELECT INTO "nl:"
  ...
  WHERE uc.constraint_name=ucc.constraint_name
   ...
   AND uc.r_constraint_name="XPKPERSON"
  ...
  HEAD uc.table_name
   ...
   request->qual[request->qual_cnt].description = concat(uc.table_name," is a child of person."),
   request->qual[request->qual_cnt].definition = concat(uc.table_name," is a child of person."),
 ...
 FOR (i = 1 TO pers->pers_cnt)
   FOR (j = 1 TO pers->qual[i].column_cnt)
    INSERT  FROM code_value_extension cve
     SET cve.code_value = pers->qual[i].code_value, cve.code_set = 24489, cve.field_name = pers->
 ...
 FOR (i = 1 TO pers->pers_cnt)
  INSERT  FROM code_value_extension cve
   SET cve.code_value = pers->qual[i].code_value, cve.code_set = 24489, cve.field_name = pers->qual[i
    ].pk_column_name,
