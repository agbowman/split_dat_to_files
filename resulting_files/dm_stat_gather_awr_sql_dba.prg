CREATE PROGRAM dm_stat_gather_awr_sql:dba
 ...
 DECLARE adsf_db_id = f8 WITH protect, noconstant(0.0)
 DECLARE adsf_version = i4 WITH protect, noconstant(0)
 SET dm_err->eproc = "Beginning DM_STAT_GATHER_AWR_SQL"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl"
  v.version
  FROM v$instance v
  DETAIL
   adsf_version = cnvtint(substring(1,(findstring(".",v.version,1,0) - 1),v.version))
  WITH nocounter
 ;end select
 IF (adsf_version >= 10)
  SET stat = alterlist(dsr->qual,1)
  SET dsr->qual[1].snapshot_type = "AWR_SQL"
  ...
  ;end select
  SELECT INTO "nl"
   sqt.buffer_gets, sqt.total_executions, sqt.disk_reads,
   sqt.rows_processed, sqt.cpu_time, sqt.elapsed_time,
   sqt.parse_calls, sqt.sql_id, sqt.username,
   sqt.optimizer_mode, sqt.sharable_memory, st.sql_text
   FROM (
    (
    (SELECT
     dsql.sql_id, au.username, dsql.optimizer_mode,
     buffer_gets = sum(dsql.buffer_gets_delta), disk_reads = sum(dsql.disk_reads_delta),
     total_executions = sum(dsql.executions_delta),
     rows_processed = sum(dsql.rows_processed_delta), cpu_time = sum(dsql.cpu_time_delta),
     elapsed_time = sum(dsql.elapsed_time_delta),
     parse_calls = sum(dsql.parse_calls_delta), sharable_memory = max(dsql.sharable_mem)
     FROM dba_hist_snapshot dh,
      dba_hist_sqlstat dsql,
      all_users au
     WHERE dh.begin_interval_time BETWEEN cnvtdatetimeutc(cnvtdatetime((curdate - 1),0),2) AND
     cnvtdatetimeutc(cnvtdatetime((curdate - 1),235959),2)
      AND dh.snap_id=dsql.snap_id
      AND dsql.dbid=adsf_db_id
      AND dsql.parsing_schema_id=au.user_id
      AND au.username != "SYS*"
     GROUP BY dsql.sql_id, au.username, dsql.optimizer_mode
     WITH sqltype("vc","vc","vc","f8","f8",
       "f8","f8","f8","f8","f8",
       "f8")))
    sqt),
    dba_hist_sqltext st
   WHERE st.sql_id=outerjoin(sqt.sql_id)
    AND st.dbid=outerjoin(adsf_db_id)
   ORDER BY sqt.sql_id
   HEAD REPORT
    cnt = 0
   HEAD sqt.sql_id
    stat_seq = 0
   DETAIL
    stat_seq = (stat_seq+ 1), cnt = (cnt+ 1)
    IF (mod(cnt,100)=1)
     stat = alterlist(dsr->qual[ml_dsr_cnt].qual,(cnt+ 99))
    ENDIF
    dsr->qual[1].qual[cnt].stat_seq = stat_seq, dsr->qual[1].qual[cnt].stat_name = sqt.sql_id, dsr->
    qual[1].qual[cnt].stat_str_val = substring(1,255,build(sqt.optimizer_mode,"||",sqt.username,"||",
      sqt.buffer_gets,
      "||",sqt.disk_reads,"||",sqt.total_executions,"||",
      sqt.rows_processed,"||",sqt.cpu_time,"||",sqt.sharable_memory,
      "||",sqt.parse_calls,"||",sqt.elapsed_time)),
    dsr->qual[1].qual[cnt].stat_clob_val = substring(1,4000,st.sql_text)
   FOOT REPORT
    IF (cnt=0)
  ...
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->err_ind = 1
   SET dm_err->eproc = "Error: Gathering Daily SQL Statistics"
   GO TO exit_program
  ENDIF
 ELSE
