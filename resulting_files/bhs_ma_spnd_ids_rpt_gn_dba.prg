CREATE PROGRAM bhs_ma_spnd_ids_rpt_gn:dba
 PROMPT
 ...
 SELECT DISTINCT INTO value(output_dest)
  ...
  position = uar_get_code_display(pr.position_cd), updt_dt_tm = format(pr.updt_dt_tm,
   "YYYY/MM/DD hh:mm;;D")
  FROM prsnl pr
  PLAN (pr
  ...
  HEAD pr.name_last
   ...
   position = trim(uar_get_code_display(pr.position_cd)), output_string = build(',"',pr.name_last,
    ...
    '","',updt_dt_tm,'","',pr.active_status_cd,'",'), col 1,
   output_string
   IF ( NOT (curendreport))
    row + 1
   ENDIF
 ...
 IF (email_ind=1)
  ...
  SET filename_out = concat(format(curdate,"YYYY-MM-DD;;D"),"_GN_SPND.csv")
  DECLARE subject_line = vc
  SET subject_line = concat(curprog," V1.x - List Suspended IDs")
