CREATE PROGRAM bed_get_child_code_values:dba
 ...
 IF (qual_cnt > 0)
  ...
  FOR (qualidx = 1 TO qual_cnt)
    SET reply->parent_code_value = request->qual[qualidx].parent_code_value
  ENDFOR
 ...
 SUBROUTINE getcodevaluegroups(dummy)
   ...
   SELECT INTO "nl:"
    FROM code_value_group cvg,
     code_value cvc
    PLAN (cvg
     WHERE (cvg.parent_code_value=reply->parent_code_value))
     JOIN (cvc
    ...
    HEAD REPORT
     childcnt = 0
    HEAD cvc.code_value
     childcnt = (childcnt+ 1), stat = alterlist(reply->qual,childcnt), reply->qual[childcnt].
     child_code_value = cvc.code_value,
     reply->qual[childcnt].display = cvc.display, reply->qual[childcnt].code_set = cvc.code_set
    WITH nocounter
   ;end select
   RETURN(true)
 END ;Subroutine
