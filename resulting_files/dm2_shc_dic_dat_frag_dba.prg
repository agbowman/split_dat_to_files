CREATE PROGRAM dm2_shc_dic_dat_frag:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsd_err_status = c1 WITH protect, noconstant(" ")
 DECLARE dsd_file = vc WITH protect, noconstant(" ")
 DECLARE dsd_cmd1 = vc WITH protect, noconstant(" ")
 DECLARE dsd_cmd2 = vc WITH protect, noconstant(" ")
 DECLARE dsd_cnt = i4 WITH protect, noconstant(0)
 DECLARE dsd_len = i4 WITH protect, noconstant(0)
 DECLARE dsd_pos = i4 WITH protect, noconstant(0)
 DECLARE dsd_beg_pos = i4 WITH protect, noconstant(0)
 DECLARE dsd_end_pos = i4 WITH protect, noconstant(0)
 DECLARE dsd_temp = vc WITH protect, noconstant(" ")
 DECLARE dsd_gbc = i4 WITH protect, noconstant(0)
 DECLARE dsd_acp = i4 WITH protect, noconstant(0)
 DECLARE dsd_get_final_cnt(null) = null
 IF (check_logfile("DM2_SHC_DIC",".log","DM2_SHC_DIC_DAT_FRAG LOG FILE")=0)
  SET dsc_err_status = "F"
  GO TO exit_script
 ENDIF
 IF (cursys="AIX")
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Health Check working on DIC.DAT Fragmentation..."
 CALL disp_msg("",dm_err->logfile,0)
 ...
 ELSE
  ...
  ENDIF
 ENDIF
 IF (dm2_push_dcl("dir ccldir:dic.dat /full")=0)
  SET dsc_err_status = "E"
 ELSE
  FREE DEFINE rtl2
  FREE SET file_loc
  SET logical file_loc value(dm_err->errfile)
  ...
  SELECT INTO "nl:"
   ...
   FROM rtl2t r
   DETAIL
    dsd_pos = findstring("Global buffer count: ",r.line)
    IF (dsd_pos > 0)
     dsd_len = textlen(trim(r.line)), dsd_beg_pos = (dsd_pos+ 21), dsd_temp = substring(dsd_beg_pos,
      dsd_len,r.line),
     dsd_end_pos = findstring(",",dsd_temp), dsd_gbc = cnvtint(substring(1,(dsd_end_pos - 1),dsd_temp
       ))
     IF ((dm_err->debug_flag >= 1))
      CALL echo(build("global_buffer_cnt =",dsd_gbc))
     ENDIF
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(concat("Parse out global_buffer_cnt.")) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dsd_err_status = "E"
  ENDIF
  IF (dsd_err_status != "E")
   IF (dm2_push_dcl("mcr sysgen show acp_window")=0)
    SET dsd_err_status = "E"
   ELSE
    FREE DEFINE rtl2
    FREE SET file_loc
    ...
    SELECT INTO "nl:"
     ...
     FROM rtl2t r
     DETAIL
      dsd_pos = findstring("ACP_WINDOW",r.line)
      IF (dsd_pos > 0)
       dsd_len = textlen(trim(r.line)), dsd_beg_pos = (dsd_pos+ 10), dsd_temp = substring(dsd_beg_pos,
        dsd_len,r.line)
       WHILE (substring(1,1,dsd_temp)=" ")
         dsd_temp = replace(dsd_temp," ","",1)
       ENDWHILE
       dsd_end_pos = findstring(" ",dsd_temp), dsd_acp = cnvtint(substring(1,(dsd_end_pos - 1),
         dsd_temp))
       IF ((dm_err->debug_flag >= 1))
        CALL echo(build("acp_win_val=",dsd_acp))
       ENDIF
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(concat("Parse out acp_win_val.")) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET dsd_err_status = "E"
    ENDIF
    IF (dsd_err_status != "E")
     IF (get_unique_file("dm2_dshc_output",".dat"))
      SET dsd_file = dm_err->unique_fname
      SET dsd_cmd1 = concat("backup/ignore=interlock ccldir:dic.dat ccluserdir:",dsd_file)
      SET dm_err->disp_dcl_err_ind = 0
      IF (dm2_push_dcl(dsd_cmd1)=0)
       IF ((dm_err->err_ind=1))
        SET dsd_err_status = "E"
       ELSEIF (findstring("is open for write by another user",dm_err->emsg)=0)
        SET dm_err->err_ind = 1
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        SET dsd_err_status = "E"
       ELSE
        CALL dsd_get_final_cnt(null)
       ENDIF
      ELSE
       CALL dsd_get_final_cnt(null)
      ENDIF
      FREE DEFINE rtl2
      FREE SET file_loc
      SET dsrs_del_cmd = concat("delete ccluserdir:",dsd_file,";*")
      IF (dm2_push_dcl(dsrs_del_cmd)=0)
       SET dsrs_err_status = "E"
      ENDIF
     ELSE
      SET dsd_err_status = "E"
     ENDIF
 ...
 IF (dsd_err_status != "E")
  IF (dsd_gbc > 0
   AND dsd_acp > dsd_cnt)
   SET dsd_err_status = "G"
  ELSE
   SET dsd_err_status = "R"
  ENDIF
 ENDIF
 IF (dshc_add_check_to_rs("DIC.DAT FRAGMENTATION","DIC.DAT Fragmentation",0,dsd_err_status,
  dshc_ctf_millennium)=0)
  SET dsd_err_status = "F"
  GO TO exit_script
 ENDIF
 IF (dsd_err_status IN ("E", "G"))
  GO TO exit_script
 ENDIF
 SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
 SET cnt = dshc->check[dshc->chk_cnt].desc_dtl_cnt
 SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,cnt)
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "DIC.DAT Fragmented"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "DIC.DAT Fragmented:"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "NOT_SET"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 2
 IF (dsd_acp > dsd_cnt)
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].str_value = "No"
 ELSE
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].str_value = "Yes"
 ENDIF
 SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
 SET cnt = dshc->check[dshc->chk_cnt].desc_dtl_cnt
 SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,cnt)
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Global Buffering Enabled"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "Global Buffering Enabled:"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "NOT_SET"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 2
 IF (dsd_gbc > 0)
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].str_value = "Yes"
 ...
 SET cnt = dshc->check[dshc->chk_cnt].desc_dtl_cnt
 SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,cnt)
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Global Buffer Count"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "Global Buffer Count:"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "NOT_SET"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 1
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].nbr_value = dsd_gbc
 SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
  "If fragmentation is found the CCL dictionary should be optimized to remove this fragmentation.",
  "  This optimization will decrease disk I/O when accessing the CCL dictionary.",
  "<SPACE>Global buffering should also be enabled on the file to further decrease disk I/O ",
  "when accessing the CCL dictionary.<SPACE>Instructions for optimizing the CCL dictionary ",
  "and setting up global buffering can be found in the Cerner Millennium Support Guide. ",
  "<SPACE>The CCL dictionary should be checked for fragmentation periodically.")
#exit_script
 ...
 IF (dsd_err_status="F")
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "EXITING DM2_SHC_DIC_DAT_FRAG WITH ERRORS"
 ELSEIF ((dm_err->err_ind=1))
  SET dm_err->err_ind = 0
  SET dm_err->eproc = "EXITING DM2_SHC_DIC_DAT_FRAG WITH ERRORS"
 ELSE
  SET dm_err->eproc = "DM2_SHC_DIC_DAT_FRAG COMPLETED SUCCESSFULLY"
 ENDIF
 CALL final_disp_msg("DM2_SHC_DIC")
 SUBROUTINE dsd_get_final_cnt(null)
  SET dsd_cmd2 = concat("pipe dump/header/block=count:0 ccluserdir:",dsd_file,
   " | sea sys$input count | sea sys$input /match=nor global /stat")
  IF (dm2_push_dcl(dsd_cmd2)=0)
   SET dsd_err_status = "E"
  ELSE
   FREE DEFINE rtl2
   FREE SET file_loc
   ...
   SELECT INTO "nl:"
    ...
    FROM rtl2t r
    DETAIL
     IF (findstring("Count:",r.line) > 0)
      dsd_cnt += 1
     ENDIF
    WITH nocounter
   ;end select
   IF ((dm_err->debug_flag >= 1))
    CALL echo(build("final_cnt=",dsd_cnt))
   ENDIF
   IF (check_error(concat("Find final count.")) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dsd_err_status = "E"
   ENDIF
  ENDIF
