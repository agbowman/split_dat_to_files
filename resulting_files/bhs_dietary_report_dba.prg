CREATE PROGRAM bhs_dietary_report:dba
 ...
 DECLARE mf_cs12020_other_allergy_type_cd = f8 WITH protect, constant(uar_get_code_by_cki(
   "CKI.CODEVALUE!492431"))
 ...
 DECLARE mf_cs106_nicu_formula = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",106,
   "NICUINFANTFORMULAS"))
 DECLARE mf_cs106_isolation = f8 WITH constant(uar_get_code_by("DISPLAYKEY",106,"ISOLATION")),
 protect
 ...
 DECLARE ms_error_head = vc WITH protect, noconstant("Dietary Report")
 ...
 IF (ml_cnt <= 0)
  ...
  GO TO exit_script
 ENDIF
 CALL echo("option 1, 3 ")
 SELECT INTO "nl:"
  FROM orders o,
   (dummyt d  WITH seq = value(diet->cnt))
  PLAN (d
   WHERE ( $ML_TPN_OPTION IN (1, 3)))
   JOIN (o
   ...
   mf_nut_services_consult_cd, mf_supplements_cd, mf_diets_cd, mf_diet_spec_inst,
   mf_cs106_nicu_formula))
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 CALL echo("Option 1, 2")
 SELECT INTO "nl:"
  FROM orders o,
   order_ingredient oi,
   (dummyt d  WITH seq = value(diet->cnt))
  PLAN (d
   WHERE ( $ML_TPN_OPTION IN (1, 2)))
   JOIN (o
   WHERE (o.encntr_id=diet->qual[d.seq].encntr_id)
    AND o.template_order_flag != 2
    AND o.active_ind=1
  ...
  WITH nocounter
 ;end select
 CALL echo("option 3,4,5 ")
 SELECT INTO "nl:"
  FROM orders o,
   (dummyt d  WITH seq = value(diet->cnt))
  PLAN (d
   WHERE ( $ML_TPN_OPTION IN (3, 4, 5)))
   JOIN (o
   WHERE (o.encntr_id=diet->qual[d.seq].encntr_id)
    AND o.template_order_flag != 2
    AND o.active_ind=1
    AND parser(ms_order_p)
    AND o.activity_type_cd IN (mf_cs106_isolation))
  ORDER BY o.encntr_id, o.order_id
  HEAD o.order_id
   diet->qual[d.seq].order_cnt += 1, ml_ord_cnt = diet->qual[d.seq].order_cnt,
   CALL alterlist(diet->qual[d.seq].orders,ml_ord_cnt),
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 CALL echo("Print to the screen or to the file")
 SELECT INTO  $OUTDEV
  ...
  PLAN (d
   WHERE ((( $ML_TPN_OPTION=1)
    ...
    AND (diet->qual[d.seq].has_w_assist=1))) )) )) )) )) )
   ...
   MACRO (line_wrap)
    ...
    WHILE (ms_temp > " "
     AND limit < 1000)
      ...
      WHILE (pos=0)
       ii += 1,
       IF (substring((maxlen - ii),1,ms_temp) IN (" ", ","))
        pos = (maxlen - ii)
       ELSEIF (ii=maxlen)
        pos = maxlen
       ENDIF
      ...
      IF (lfloc=0
       ...
       ycol += 12, ms_temp = substring((pos+ 1),9999,ms_temp)
      ELSE
       IF (((crloc < lfloc
        AND crloc > 0) OR (lfloc=0)) )
        printstring = substring(1,(crloc - 1),printstring),
        CALL print(calcpos(xcol,ycol)), printstring,
        row + 1, ycol += 12, ms_temp = substring((crloc+ 2),9999,ms_temp)
       ELSEIF (((lfloc < crloc
        AND crloc > 0) OR (crloc=0)) )
        printstring = substring(1,(lfloc - 1),printstring),
        CALL print(calcpos(xcol,ycol)), printstring,
        row + 1, ycol += 12, ms_temp = substring((lfloc+ 2),9999,ms_temp)
       ENDIF
       WHILE (substring(1,1,ms_temp) IN (" ", cr, lf))
         ms_temp = substring(2,9999,ms_temp)
       ENDWHILE
      ENDIF
    ENDWHILE
   ENDMACRO
   ...
   IF ((diet->qual[d.seq].order_cnt > 0))
    FOR (i = 1 TO diet->qual[d.seq].order_cnt)
      IF (ycol > 700)
       BREAK
      ENDIF
      ms_temp = substring(1,500,trim(concat(trim(diet->qual[d.seq].orders[i].order_mnemonic)," ",trim
         (diet->qual[d.seq].orders[i].clinical_display_line)," (",trim(uar_get_code_display(diet->
      ...
      FOR (k = 1 TO size(diet->qual[d.seq].orders[i].comments,5))
        IF ((diet->qual[d.seq].orders[i].comments[k].order_comment > " "))
         ...
         ms_temp = replace(replace(diet->qual[d.seq].orders[i].comments[k].order_comment,char(10),""),
          char(13)," "), xcol = 150, wrapcol = 80,
