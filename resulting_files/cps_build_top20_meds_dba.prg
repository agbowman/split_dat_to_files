CREATE PROGRAM cps_build_top20_meds:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ap
   ...
   WHERE nvp.parent_entity_id=ap.app_prefs_id
    ...
    AND nvp.pvc_name="TOP20_MEDICATION_COUNT")
  HEAD REPORT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (o
   WHERE o.updt_dt_tm >= cnvtdatetime(anchor->dt_tm)
    ...
    AND ((o.orig_ord_as_flag+ 0)=5)
    AND ((o.active_ind+ 0)=true))
  GROUP BY o.last_update_provider_id, o.synonym_id, o.orig_ord_as_flag
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   IF (p.username != null
    AND p.username > " ")
    hold->qual[d1.seq].long_description_key_cap = concat(trim(cnvtupper(p.username)),"_TOP20_MEDS")
   ENDIF
  WITH nocounter
 ;end select
 SET ierrcode = error(serrmsg,1)
   ...
   IF (curqual < 1)
    ...
    INSERT  FROM alt_sel_cat ac
     SET ac.alt_sel_category_id = hold->qual[i].alt_sel_category_id, ac.short_description =
      "System Tracked Medication Favorites", ac.long_description = hold->qual[i].
