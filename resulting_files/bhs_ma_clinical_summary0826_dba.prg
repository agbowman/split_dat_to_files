CREATE PROGRAM bhs_ma_clinical_summary0826:dba
  ...
  SELECT DISTINCT INTO "nl:"
   ...
   DETAIL
    ...
    dlrec->seq[dd.seq].section4_events[cnt].event_dt_tm = substring(1,14,format(c.event_end_dt_tm,
      "@SHORTDATETIME;;Q"))
    IF (cdr.event_id > 0.0)
     dlrec->seq[dd.seq].section4_events[cnt].result = substring(1,14,format(cdr.result_dt_tm,
       "@SHORTDATETIME;;Q"))
    ELSE
     dlrec->seq[dd.seq].section4_events[cnt].result = concat(substring(1,14,format(c.event_end_dt_tm,
        "@SHORTDATETIME;;Q"))," ",trim(c.result_val,3)," ",uar_get_code_display(c.result_units_cd))
    ENDIF
   FOOT  c.encntr_id
  ...
  SELECT INTO request->output_device
   ...
   DETAIL
    FOR (i = 1 TO dlrec->encntr_total)
      ...
      CALL print(calcpos(xcol1,yrow1)), "UNDER CONSTRUCTION", rowplusone,
