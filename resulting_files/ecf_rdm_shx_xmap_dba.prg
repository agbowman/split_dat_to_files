CREATE PROGRAM ecf_rdm_shx_xmap:dba
 ...
 SUBROUTINE kia_script_inc(pkg,ccl_obj)
   ...
   CALL parser(obj_prs)
 END ;Subroutine
 SET kia_short_desc = "SHX XMAP"
 SET kia_tbl_desc = "SHX XMAP"
 SET kia_rdm_nbr = 9824
 SET kia_rdm_vocab_mean = "CERNER"
 SET readme_data->status = "F"
 SET readme_data->message = concat("FAILED - ",trim(kia_tbl_desc))
 SET script_status = " "
 SET csv_stat = 1
 SET kia_rdm_log_name = "kia_rdm_shx_xmap.log"
 ...
 CALL log_sub(concat("ECF_RDM_SHX_XMAP Begin> ",format(cnvtdatetime(curdate,curtime3),
 ...
 IF (kia_obj_suc_ind=0)
  EXECUTE kia_load_xmap kia_rdm_vocab_mean, "SHX"
