CREATE PROGRAM dm_rdm_delete_saved_docs_p:dba
 ...
 SELECT INTO "n1:"
  ...
  DETAIL
   mf_min_range_id = cnvtreal(min_id), mf_max_range_id = cnvtreal(max_id)
  WITH nocounter
 ;end select
 ...
 IF (sbr_run_parent(mf_max_range_id,mf_min_range_id,ms_range_prefix,ms_info_domain_nm,mn_num_children
  ...
  GO TO exit_program
 ENDIF
 CALL sbr_add_sql_stmt("TABLE","TEMP_TASK_ACTIVITY",concat(
   "create global temporary table TEMP_TASK_ACTIVITY(task_id number)","on commit preserve rows"),
  "Creating temporary table")
 CALL sbr_add_sql_stmt("INDEX","XIETEMP_TASK_ACTIVITY",
  "create index XIETEMP_TASK_ACTIVITY ON TEMP_TASK_ACTIVITY(task_id)","Creating index")
