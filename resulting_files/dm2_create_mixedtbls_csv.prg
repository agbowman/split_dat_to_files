CREATE PROGRAM dm2_create_mixedtbls_csv
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_mixed_csv",".log","dm2_create_mixedtbls_csv")=0)
  GO TO exit_program
 ENDIF
 DECLARE did = vc WITH protect, noconstant("")
 DECLARE din = vc WITH protect, noconstant("")
 DECLARE dic = vc WITH protect, noconstant("")
 DECLARE dinbr = vc WITH protect, noconstant("")
 DECLARE duc = vc WITH protect, noconstant("")
 DECLARE dcm_cnt = i4 WITH protect, noconstant(0)
 SET dm_err->eproc = "Checking if return characters in INFO_CHAR column"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE ((d.info_domain="DM2_MIXED_TABLE*") OR (d.info_domain="DM2_MIXTBL_FLIP"))
  DETAIL
   FOR (dcm_cnt = 1 TO textlen(d.info_char))
     IF (((ichar(substring(dcm_cnt,1,d.info_char))=13) OR (ichar(substring(dcm_cnt,1,d.info_char))=10
     )) )
      CALL echo(concat(d.info_char," : ",cnvtstring(ichar(substring(dcm_cnt,1,d.info_char))))),
      CALL echo("ERROR: Column contains return characters or line feeds."), dm_err->emsg =
      "ERROR: Column contains return characters or line feeds."
     ENDIF
   ENDFOR
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Generating dm2_mixed_tables.csv into CCLUSERDIR"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "dm2_mixed_tables.csv"
  FROM dm_info d
  WHERE d.info_domain="DM2_MIXED_TABLE*"
  DETAIL
   did = trim(d.info_domain), din = trim(d.info_name), dic = trim(d.info_char),
   dinbr = trim(cnvtstring(d.info_number)), duc = trim(cnvtstring(d.updt_cnt)), col 0,
   "^", did, "^,^",
   din, "^,^", dic,
   "^,^", dinbr, "^,^",
   duc, "^", row + 1
 ...
 ENDIF
#exit_program
 CALL final_disp_msg("dm2_mixed_csv")
END GO
