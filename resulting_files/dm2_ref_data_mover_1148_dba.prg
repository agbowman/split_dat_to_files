CREATE PROGRAM dm2_ref_data_mover_1148:dba
 ...
 DECLARE cust_p_from_id = vc
 DECLARE cust_p_to_id = vc
 DECLARE cust_c_from_id = vc
 DECLARE cust_c_to_id = vc
 DECLARE cust_val = f8
 ...
 DECLARE cust_pe_pos = i2
 DECLARE cust_ce_pos = i2
 DECLARE cust_dmt_cnt = i4
 DECLARE cust_from_vc = vc
 DECLARE cust_ext_child_cd = f8
 DECLARE cust_nomen_ind = i2
 DECLARE cust_ext_parent_cd = f8
 DECLARE cust_child_cdf_mean = vc
 DECLARE cust_parent_cdf_mean = vc
 DECLARE cust_ce_name = vc
 DECLARE cust_drdm_dmt_tab = vc
 DECLARE cust_idcd_ind = i2
 DECLARE cust_ext_owner_cd = f8
 DECLARE cust_owner_cdf_mean = vc
 DECLARE cust_seq_new = f8
 DECLARE cust_rpt_missing = vc
 DECLARE cust_par_col_name = vc
 DECLARE cust_child_col_name = vc
 DECLARE cust_par_pos = i4
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_nomen_ind = 0
 SET cust_dmt_cnt = 0
 SET cust_ext_child_cd = cnvtreal(get_value(cust_tab_name,"EXT_CHILD_CONTRIBUTOR_CD","TO"))
 SET cust_ext_parent_cd = cnvtreal(get_value(cust_tab_name,"EXT_PARENT_CONTRIBUTOR_CD","TO"))
 SET cust_ext_owner_cd = cnvtreal(get_value(cust_tab_name,"EXT_OWNER_CD","TO"))
 SET cust_pe_name = get_value(cust_tab_name,"EXT_PARENT_ENTITY_NAME","FROM")
 SET cust_pe_pos = get_col_pos(cust_tab_name,"EXT_PARENT_REFERENCE_ID")
 SET cust_ce_pos = get_col_pos(cust_tab_name,"EXT_CHILD_REFERENCE_ID")
 SET cust_ce_name = get_value(cust_tab_name,"EXT_CHILD_ENTITY_NAME","FROM")
 SET cust_p_from_id = get_value(cust_tab_name,"EXT_PARENT_REFERENCE_ID","FROM")
 SET cust_c_from_id = get_value(cust_tab_name,"EXT_CHILD_REFERENCE_ID","FROM")
 SET cust_bill_item_id = get_value(cust_tab_name,"BILL_ITEM_ID","FROM")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_1148
 ENDIF
 SET stat = alterlist(rdds_exception->qual,0)
 IF (is_translated(cust_tab_name,"EXT_CHILD_CONTRIBUTOR_CD")=0)
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg =
  "BILL_ITEM: The EXT_CHILD_CONTRIBUTOR_CD column was not translated"
  GO TO exit_1148
 ELSE
  IF (cust_ext_child_cd > 0)
   SELECT INTO "NL:"
    FROM code_value c
    WHERE c.code_value=cust_ext_child_cd
    DETAIL
     cust_child_cdf_mean = c.cdf_meaning
    WITH nocounter
   ;end select
   IF (check_error("Obtaining CDF meaning")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_1148
   ENDIF
  ENDIF
 ENDIF
 IF (is_translated(cust_tab_name,"EXT_PARENT_CONTRIBUTOR_CD")=0)
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg =
  "BILL_ITEM: The EXT_PARENT_CONTRIBUTOR_CD column was not translated"
  GO TO exit_1148
 ELSE
  IF (cust_ext_parent_cd > 0)
   SELECT INTO "NL:"
    FROM code_value c
    WHERE c.code_value=cust_ext_parent_cd
    DETAIL
     cust_parent_cdf_mean = c.cdf_meaning
    WITH nocounter
   ;end select
   IF (check_error("Obtaining CDF meaning")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 IF (is_translated(cust_tab_name,"EXT_OWNER_CD")=0)
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "BILL_ITEM: The cust_ext_owner_cd column was not translated"
  GO TO exit_1148
 ELSE
  IF (cust_ext_owner_cd > 0)
   SELECT INTO "NL:"
    FROM code_value c
    WHERE c.code_value=cust_ext_owner_cd
    DETAIL
     cust_owner_cdf_mean = c.cdf_meaning
    WITH nocounter
   ;end select
   IF (check_error("Obtaining CDF Meaning")=1)
    CALL disp_mesg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_1148
   ENDIF
  ENDIF
 ENDIF
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_pe_pos].translated = 0
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_ce_pos].translated = 0
 IF (cust_pe_name="")
  SET cust_pe_name = "CODE_VALUE"
  SET stat = alterlist(rdds_exception->qual,1)
  SET rdds_exception->qual[1].tab_col_name = "EXT_PARENT_REFERENCE_ID"
  SET rdds_exception->qual[1].tru_tab_name = "CODE_VALUE"
  SET rdds_exception->qual[1].tru_col_name = "CODE_VALUE"
 ELSE
  SET stat = alterlist(rdds_exception->qual,1)
  CASE (cust_parent_cdf_mean)
   OF "TASKCAT":
    SET cust_pe_name = "ORDER_TASK"
    SET rdds_exception->qual[1].tab_col_name = "EXT_PARENT_REFERENCE_ID"
    SET rdds_exception->qual[1].tru_tab_name = "ORDER_TASK"
    SET rdds_exception->qual[1].tru_col_name = "REFERENCE_TASK_ID"
   OF "ITEM MASTER":
    SET cust_pe_name = "ITEM_DEFINITION"
    SET rdds_exception->qual[1].tab_col_name = "EXT_PARENT_REFERENCE_ID"
    SET rdds_exception->qual[1].tru_tab_name = "ITEM_DEFINITION"
    SET rdds_exception->qual[1].tru_col_name = "ITEM_ID"
   OF "ALPHA RESP":
    SET cust_pe_name = "NOMENCLATURE"
    SET rdds_exception->qual[1].tab_col_name = "EXT_PARENT_REFERENCE_ID"
    SET rdds_exception->qual[1].tru_tab_name = "NOMENCLATURE"
    SET rdds_exception->qual[1].tru_col_name = "NOMENCLATURE_ID"
   OF "MANF ITEM":
    SET cust_pe_name = "ITEM_DEFINITION"
    SET rdds_exception->qual[1].tab_col_name = "EXT_PARENT_REFERENCE_ID"
    SET rdds_exception->qual[1].tru_tab_name = "ITEM_DEFINITION"
    SET rdds_exception->qual[1].tru_col_name = "ITEM_ID"
   OF "INTERNAL":
    IF (cust_owner_cdf_mean="AFC ADD GEN")
     IF (cust_bill_item_id=cust_p_from_id)
      SET cust_p_to_id = select_merge_translate(cust_p_from_id,"BILL_ITEM")
      IF (cust_p_to_id="No Trans")
       SET cust_seq_new = get_seq(cust_tab_name,"BILL_ITEM_ID")
       CALL put_value(cust_tab_name,"BILL_ITEM_ID",cnvtstring(cust_seq_new))
       CALL put_value(cust_tab_name,"EXT_PARENT_REFERENCE_ID",cnvtstring(cust_seq_new))
       SET stat = alterlist(dm2_ref_data_reply->qual,1)
       SET dm2_ref_data_reply->qual[1].table_name = cust_tab_name
       SET from_id = get_value(cust_tab_name,"BILL_ITEM_ID","FROM")
       IF ((dm2_ref_data_reply->error_ind=1))
        GO TO exit_1148
       ELSE
        SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(from_id)
       ENDIF
       SET dm2_ref_data_reply->qual[1].to_value = cust_seq_new
      ELSE
       CALL put_value(cust_tab_name,"EXT_PARENT_REFERENCE_ID",cust_p_to_id)
      ENDIF
      SET cust_pe_name = "BILL_ITEM"
      SET rdds_exception->qual[1].tab_col_name = "EXT_PARENT_REFERENCE_ID"
      SET rdds_exception->qual[1].tru_tab_name = "BILL_ITEM"
      SET rdds_exception->qual[1].tru_col_name = "BILL_ITEM_ID"
     ENDIF
    ELSE
     SET cust_pe_name = "CODE_VALUE"
     SET rdds_exception->qual[1].tab_col_name = "EXT_PARENT_REFERENCE_ID"
    ...
    ENDIF
   OF "BBPOOL":
    SET cust_pe_name = "BB_MOD_OPTION"
    SET rdds_exception->qual[1].tab_col_name = "EXT_PARENT_REFERENCE_ID"
    SET rdds_exception->qual[1].tru_tab_name = "BB_MOD_OPTION"
    SET rdds_exception->qual[1].tru_col_name = "OPTION_ID"
   OF "BBMODIFY":
    SET cust_pe_name = "BB_MOD_OPTION"
    SET rdds_exception->qual[1].tab_col_name = "EXT_PARENT_REFERENCE_ID"
    SET rdds_exception->qual[1].tru_tab_name = "BB_MOD_OPTION"
 ...
 ENDIF
 IF (cust_ce_name="")
  SET cust_ce_name = "CODE_VALUE"
  SET stat = alterlist(rdds_exception->qual,2)
  SET rdds_exception->qual[2].tab_col_name = "EXT_CHILD_REFERENCE_ID"
  SET rdds_exception->qual[2].tru_tab_name = "CODE_VALUE"
  SET rdds_exception->qual[2].tru_col_name = "CODE_VALUE"
 ELSE
  SET stat = alterlist(rdds_exception->qual,2)
  CASE (cust_child_cdf_mean)
   OF "TASKCAT":
    SET cust_ce_name = "ORDER_TASK"
    SET rdds_exception->qual[2].tab_col_name = "EXT_CHILD_REFERENCE_ID"
    SET rdds_exception->qual[2].tru_tab_name = "ORDER_TASK"
    SET rdds_exception->qual[2].tru_col_name = "REFERENCE_TASK_ID"
   OF "ITEM MASTER":
    SET cust_ce_name = "ITEM_DEFINITION"
    SET rdds_exception->qual[2].tab_col_name = "EXT_CHILD_REFERENCE_ID"
    SET rdds_exception->qual[2].tru_tab_name = "ITEM_DEFINITION"
    SET rdds_exception->qual[2].tru_col_name = "ITEM_ID"
   OF "ALPHA RESP":
    SET cust_ce_name = "NOMENCLATURE"
    SET rdds_exception->qual[2].tab_col_name = "EXT_CHILD_REFERENCE_ID"
    SET rdds_exception->qual[2].tru_tab_name = "NOMENCLATURE"
    SET rdds_exception->qual[2].tru_col_name = "NOMENCLATURE_ID"
   OF "MANF ITEM":
    SET cust_ce_name = "ITEM_DEFINITION"
    SET rdds_exception->qual[2].tab_col_name = "EXT_CHILD_REFERENCE_ID"
    SET rdds_exception->qual[2].tru_tab_name = "ITEM_DEFINITION"
   ...
   OF "INTERNAL":
    IF (cust_owner_cdf_mean="AFC ADD GEN")
     SET cust_ce_name = "BILL_ITEM"
     SET rdds_exception->qual[2].tab_col_name = "EXT_CHILD_REFERENCE_ID"
     SET rdds_exception->qual[2].tru_tab_name = "BILL_ITEM"
     SET rdds_exception->qual[2].tru_col_name = "BILL_ITEM_ID"
    ELSE
     SET cust_ce_name = "CODE_VALUE"
     SET rdds_exception->qual[2].tab_col_name = "EXT_CHILD_REFERENCE_ID"
     SET rdds_exception->qual[2].tru_tab_name = "CODE_VALUE"
    ...
    ENDIF
   OF "BBPOOL":
    SET cust_ce_name = "BB_MOD_OPTION"
    SET rdds_exception->qual[2].tab_col_name = "EXT_CHILD_REFERENCE_ID"
    SET rdds_exception->qual[1].tru_tab_name = "BB_MOD_OPTION"
    SET rdds_exception->qual[2].tru_col_name = "OPTION_ID"
   OF "BBMODIFY":
    SET cust_ce_name = "BB_MOD_OPTION"
    SET rdds_exception->qual[2].tab_col_name = "EXT_CHILD_REFERENCE_ID"
    SET rdds_exception->qual[1].tru_tab_name = "BB_MOD_OPTION"
  ...
  ENDCASE
 ENDIF
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_pe_pos].translated=0))
  IF (cnvtreal(cust_p_from_id) > 0)
   SET cust_p_to_id = select_merge_translate(cust_p_from_id,cust_pe_name)
   IF (cust_p_to_id="No Trans")
    IF ((rdds_exception->qual[1].tru_col_name=""))
     SET cust_par_pos = locateval(cust_loop,1,dm2_ref_data_doc->tbl_cnt,cust_pe_name,dm2_ref_data_doc
      ->tbl_qual[cust_loop].table_name)
     FOR (cust_loop = 1 TO dm2_ref_data_doc->tbl_qual[cust_par_pos].col_cnt)
       IF ((dm2_ref_data_doc->tbl_qual[cust_par_pos].col_qual[cust_loop].pk_ind=1)
        AND (dm2_ref_data_doc->tbl_qual[cust_par_pos].col_qual[cust_loop].root_entity_name=
       cust_pe_name))
        SET cust_par_col_name = dm2_ref_data_doc->tbl_qual[cust_par_pos].col_qual[cust_loop].
        column_name
       ENDIF
     ENDFOR
    ELSE
     SET cust_par_col_name = rdds_exception->qual[1].tru_col_name
    ENDIF
    SET cust_rpt_missing = report_missing(cnvtupper(cust_pe_name),cust_par_col_name,cnvtreal(
      cust_p_from_id))
   ...
   ENDIF
  ELSE
   CALL put_value(cust_tab_name,"EXT_PARENT_REFERENCE_ID",cust_p_from_id)
  ENDIF
 ENDIF
 IF (cnvtreal(cust_c_from_id) > 0)
  SET cust_c_to_id = select_merge_translate(cust_c_from_id,cust_ce_name)
  IF (cust_c_to_id="No Trans")
   IF ((rdds_exception->qual[2].tru_col_name=""))
    SET cust_par_pos = locateval(cust_loop,1,dm2_ref_data_doc->tbl_cnt,cust_ce_name,dm2_ref_data_doc
     ->tbl_qual[cust_loop].table_name)
    FOR (cust_loop = 1 TO dm2_ref_data_doc->tbl_qual[cust_par_pos].col_cnt)
      IF ((dm2_ref_data_doc->tbl_qual[cust_par_pos].col_qual[cust_loop].pk_ind=1)
       AND (dm2_ref_data_doc->tbl_qual[cust_par_pos].col_qual[cust_loop].root_entity_name=
      cust_ce_name))
       SET cust_child_col_name = dm2_ref_data_doc->tbl_qual[cust_par_pos].col_qual[cust_loop].
       column_name
      ENDIF
    ENDFOR
   ELSE
    SET cust_child_col_name = rdds_exception->qual[2].tru_col_name
   ENDIF
   SET cust_rpt_missing = report_missing(cnvtupper(cust_ce_name),cust_child_col_name,cnvtreal(
     cust_c_from_id))
   CALL evaluate_rpt_missing(cust_rpt_missing)
  ELSE
   CALL put_value(cust_tab_name,"EXT_CHILD_REFERENCE_ID",cust_c_to_id)
  ENDIF
 ELSE
  CALL put_value(cust_tab_name,"EXT_CHILD_REFERENCE_ID",cust_c_from_id)
 ENDIF
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_1148
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"BILL_ITEM_ID")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_1148
 ENDIF
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"BILL_ITEM_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"BILL_ITEM_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  BILL_ITEM_ID")
     ELSE
      GO TO exit_1148
     ENDIF
    ELSE
   ...
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(from_id)
   ENDIF
   SET dm2_ref_data_reply->qual[1].to_value = cust_val
...
#exit_1148
 IF (cust_idcd_ind=0)
  CALL is_translated(cust_tab_name,"ALL")
 ENDIF
 IF ((dm2_ref_data_reply->error_ind=1)
  AND (global_mover_rec->one_pass_ind=0)
