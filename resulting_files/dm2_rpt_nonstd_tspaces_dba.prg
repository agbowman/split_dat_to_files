CREATE PROGRAM dm2_rpt_nonstd_tspaces:dba
 ...
 SUBROUTINE dtr_load_clin_tspaces(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE drnt_rpt_file = vc WITH protect, noconstant("")
 IF (check_logfile("dm2_rpt_nonstdtsp",".log","DM2_RPT_NONSTD_TSPACES LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM2_RPT_NONSTD_TSPACES"
 CALL disp_msg("",dm_err->logfile,0)
 IF (dtr_load_tspaces("REPORT")=0)
  GO TO exit_script
 ENDIF
 IF (get_unique_file("dm2_nonstd_tspaces",".rpt")=0)
  GO TO exit_script
 ELSE
  SET drnt_rpt_file = dm_err->unique_fname
 ENDIF
 IF (dtr_rpt_nonstd_tspace(drnt_rpt_file,1)=0)
  GO TO exit_script
 ENDIF
 IF (dm2_disp_file(drnt_rpt_file,concat("Report File may be found in CCLUSERDIR:",drnt_rpt_file))=0)
  GO TO exit_script
 ENDIF
#exit_script
 CALL final_disp_msg("dm2_rpt_nonstdtsp")
END GO
