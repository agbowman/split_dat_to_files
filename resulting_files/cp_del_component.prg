CREATE PROGRAM cp_del_component
 ...
 SUBROUTINE verifyinputs(null)
   ...
   DECLARE comp_type_cd_codeset = i4 WITH protect, constant(4003130)
   IF (complistsize <= 0)
    CALL reporterrorandexit("PERFORM","Validate","Component list must be nonzero in size.")
   ENDIF
   FOR (x = 1 TO complistsize)
     CALL verifynonzerocodevalue(comp_del_request->qual[x].comp_type_cd,comp_type_cd_codeset,
      "Component Type")
     SELECT INTO "NL:"
      FROM cp_node cn
      WHERE (cn.cp_node_id=comp_del_request->qual[x].cp_node_id)
      WITH nocounter
     ;end select
     IF (curqual=0)
 ...
 SUBROUTINE deleterows(null)
   ...
   FOR (x = 1 TO size(comp_del_request->qual,5))
     ...
     SELECT INTO "nl:"
      ...
      WHERE (cc.cp_node_id=comp_del_request->qual[x].cp_node_id)
     ...
     ELSE
      ...
      cp_component_id
      SET comp_del_reply->component_list[x].cp_node_id = comp_del_request->qual[x].cp_node_id
     ENDIF
   ENDFOR
   COMMIT
