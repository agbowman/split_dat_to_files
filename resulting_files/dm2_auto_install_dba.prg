CREATE PROGRAM dm2_auto_install:dba
 ...
 DECLARE dai_install_mode = vc WITH protect, noconstant(" ")
 DECLARE dai_pkg_str_tmp = vc WITH protect, noconstant("")
 DECLARE dai_logfile_prefix = vc WITH noconstant("NOT_SET")
 SET reply->status_data.status = "F"
 SET reply->execution_ind = 0
 SET dai_pkg_str_tmp = build("b",cnvtint(request->plan_id))
 SET dai_logfile_prefix = concat("dm2o",dai_pkg_str_tmp,"_")
 IF (check_logfile(dai_logfile_prefix,".log","dm2_auto_install")=0)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_auto_install"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Execute dm2_auto_install_verify"
 CALL disp_msg(" ",dm_err->logfile,0)
 EXECUTE dm2_auto_install_verify  WITH replace("REPLY","DAI_REPLY")
 IF ((dm_err->err_ind > 0))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET reply->status_data.subeventstatus[1].operationname = "dm2_auto_install_verify"
  SET reply->status_data.subeventstatus[1].operationstatus = "F"
  SET reply->status_data.subeventstatus[1].targetobjectname = "dm2_auto_install_verify"
  SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
  GO TO exit_program
 ENDIF
 IF ((dai_reply->auto_install_ind=0))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Execute dm2_auto_install_verify"
  ...
  GO TO exit_program
 ENDIF
 SET dai_install_mode = concat(trim(request->install_mode),"ABG")
 SET dm_err->eproc = "Submit a package install job to background"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dir_submit_jobs(request->plan_id,dai_install_mode,currdbuser,dm2_install_schema->v500_p_word,
  dm2_install_schema->v500_connect_str,
  " ",1)=0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET reply->status_data.subeventstatus[1].operationname = "Package install Job"
  SET reply->status_data.subeventstatus[1].operationstatus = "F"
  SET reply->status_data.subeventstatus[1].targetobjectname = "dir_submit_jobs"
  SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
  GO TO exit_program
 ENDIF
 GO TO exit_program
#exit_program
 IF ((dm_err->err_ind=0))
  ...
  SET reply->execution_ind = 1
  SET dm_err->eproc = "dm2_auto_install completed"
 ENDIF
 IF ((dm_err->debug_flag >= 2))
  CALL echorecord(reply)
  CALL echorecord(dai_reply)
 ENDIF
 CALL final_disp_msg("dm2_auto_install")
END GO
