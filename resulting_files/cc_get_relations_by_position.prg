CREATE PROGRAM cc_get_relations_by_position
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Position cd" = 0.0,
  "Prsnl id" = 0.0
  WITH outdev, positioncd, prsnlid
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "CC_GET_RELATIONS_BY_POSITION"
 ...
 SUBROUTINE (validatesrvreply(nsrvstat=i4,recorddata=vc(ref),zeroforceexit=i2) =i4 WITH protect)
   ...
   ENDIF
 END ;Subroutine
 DECLARE request_number = i4 WITH protect, constant(3208642)
 DECLARE srv_request = vc WITH protect, constant("QueryRelationshipsByPosition")
 SET reply_data->status_data.status = "F"
 CALL initializerequest(reply_data,request_number)
 SET nsrvstat = uar_srvsetdouble(hreq,"position_cd", $POSITIONCD)
 ...
 SUBROUTINE (getdata(hreply=i4(ref)) =null WITH protect)
   ...
   SET putrequest->source_dir =  $OUTDEV
   SET putrequest->isblob = "1"
   SET pos = uar_srvgetasissize(hreply,"relationships_json")
   SET putrequest->document = substring(1,pos,uar_srvgetasisptr(hreply,"relationships_json"))
   SET putrequest->document_size = size(putrequest->document)
   CALL exit_servicerequest(happ,htask,hstep)
