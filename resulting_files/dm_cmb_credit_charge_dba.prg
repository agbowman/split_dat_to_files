CREATE PROGRAM dm_cmb_credit_charge:dba
 ...
 FOR (x = 1 TO cc_col_count)
  ...
  CASE (trim(rcolumns->col[x].col_name))
   ...
   OF "CREDITED_DT_TM":
    CALL parser(build("cnvtdatetime(curdate, curtime)",v_comma_str))
   OF "ITEM_PRICE":
    CALL parser(build("(rRecList->from_rec[pcc2_request->sbr_loopcount]->item_price * -1)",
      v_comma_str))
   OF "ITEM_EXTENDED_PRICE":
    CALL parser(build("(rRecList->from_rec[pcc2_request->sbr_loopcount]->item_extended_price * -1)",
      v_comma_str))
   OF "CHARGE_TYPE_CD":
    IF ((rreclist->from_rec[pcc2_request->sbr_loopcount].charge_type_cd=credit_cd))
     CALL parser(build("DEBIT_CD",v_comma_str))
    ELSE
     CALL parser(build("CREDIT_CD",v_comma_str))
    ENDIF
 ...
 CALL parser(build("where FRM.charge_item_id = ",rreclist->from_rec[pcc2_request->sbr_loopcount].
   from_id,")"))
 CALL parser("with nocounter go")
 CALL error_logger(curqual,"Couldn't insert charge on the from person",insert_error)
 IF ((pcc2_request->sbr_cc_process_type="*C"))
  SET icombinedet += 1
  SET stat = alterlist(request->xxx_combine_det,icombinedet)
  SET request->xxx_combine_det[icombinedet].combine_action_cd = add
  SET request->xxx_combine_det[icombinedet].entity_id = cc_new_nbr
  SET request->xxx_combine_det[icombinedet].entity_name = "CHARGE"
  IF ((pcc2_request->sbr_cc_process_type="PC"))
   SET request->xxx_combine_det[icombinedet].attribute_name = "PERSON_ID"
  ELSE
   SET request->xxx_combine_det[icombinedet].attribute_name = "ENCNTR_ID"
  ENDIF
  CALL copy_charge_mod(rreclist->from_rec[pcc2_request->sbr_loopcount].from_id,cc_new_nbr,
   pcc2_request->sbr_cc_process_type)
  SET tmp_offset_charge_item_id = cc_new_nbr
 ENDIF
 SET dcgea_request->alias_type_cd = 319_mrn
 SET dcgea_request->alias_dt_tm = rreclist->from_rec[pcc2_request->sbr_loopcount].beg_effective_dt_tm
 SET dcgea_request->encntr_id = rreclist->from_rec[pcc2_request->sbr_loopcount].encntr_id
 EXECUTE dm_cmb_get_enc_alias
 IF ((dcgea_reply->status="F"))
  SET failed = select_error
  SET ucb_failed = select_error
 ...
 ENDIF
 IF ((dcgea_reply->encntr_alias_id > 0))
  CALL insert_fin_mrn_charge_mod(cc_new_nbr,dcgea_reply->alias,13019_mrn_alias,pcc2_request->
   sbr_cc_process_type)
 ENDIF
 SET dcgea_request->alias_type_cd = 319_fin_nbr
 EXECUTE dm_cmb_get_enc_alias
 ...
 ENDIF
 IF ((dcgea_reply->encntr_alias_id > 0))
  CALL insert_fin_mrn_charge_mod(cc_new_nbr,dcgea_reply->alias,13019_finnbr_alias,pcc2_request->
   sbr_cc_process_type)
 ENDIF
 CALL echo("credit_charge end")
#end_of_program
END GO
