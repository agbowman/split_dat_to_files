CREATE PROGRAM ecf_rdm_us_tribal_entity:dba
 ...
 DECLARE kia_concept_csv_filename = vc WITH public, constant("US_tribal_ent_concept.csv")
 DECLARE kia_reltn_csv_filename = vc WITH public, constant("US_tribal_ent_relation.csv")
 DECLARE kia_nomen_csv_filename = vc WITH public, constant("US_tribal_ent_nomen.csv")
 SET kia_short_desc = "TRIBALENT_US Load"
 SET kia_tbl_desc = "TRIBALENT_US Content Load"
 SET kia_rdm_log_name = "ecf_rdm_us_tribal_entity.log"
 SET readme_data->status = "F"
 SET readme_data->message = concat("FAILED - ",kia_tbl_desc)
 SET script_status = " "
 SET csv_stat = 1
 SET kia_rdm_vocab_mean = "TRIBALENT_US"
 SET kia_rdm_nbr = 11979
 CALL kia_chk_error(1)
 COMMIT
 CALL kia_db2_check(1)
 CALL kia_inhouse_chk(1)
 EXECUTE kia_dm_dbimport "cer_install:US_tribal_ent_version.csv", "kia_get_rdm_vars", 100,
 0
 CALL kia_chk_script_failure("KIA_GET_RDM_VARS")
 SET start_time_var = cnvtdatetime(curdate,curtime3)
 CALL log_sub(concat("ecf_rdm_us_tribal_entity Begin> ",format(cnvtdatetime(curdate,curtime3),
    "dd-mmm-yyyy hh:mm:ss;;d")))
 CALL kia_chk_status_info(kia_tbl_desc,"table")
 SET kia_vocab_cd = kia_rdm_get_cd(kia_rdm_vocab_mean)
 CALL kia_get_parse_info(kia_pkg_nbr,kia_rdm_nbr,"kia_dm_dbimport",1000)
 CALL kia_chk_status_info("LOAD_CONCEPT_BASELINE","check")
 IF (kia_obj_suc_ind=0)
  CALL echo("DELETING CMT_CONCEPT_LOAD_NS ROWS...")
  DELETE  FROM cmt_concept_load_ns c
   WHERE ((c.concept_cki="TRIBALENT_US*") OR (c.concept_cki="SNOMED*"))
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
  CALL kia_load_file(kia_concept_csv_filename,"kia_imp_concept_load_ns","kia_imp_concept_load_ns.log"
   )
  CALL kia_set_status_success("LOAD_CONCEPT_BASELINE")
 ENDIF
 CALL kia_chk_status_info("kia_load_cmt_concept_by_ld_2","check")
 IF (kia_obj_suc_ind=0)
  EXECUTE kia_load_cmt_concept_by_ld_2 "TRIBALENT_US"
  CALL kia_chk_script_failure("kia_load_cmt_concept_by_ld_2")
  CALL kia_set_status_success("kia_load_cmt_concept_by_ld_2")
 ENDIF
 CALL kia_chk_status_info("LOAD_RELTN_BASELINE_DATA","check")
 IF (kia_obj_suc_ind=0)
  CALL echo("DELETING CMT_CONCEPT_RELTN_LD_NS ROWS...")
  DELETE  FROM cmt_concept_reltn_ld_ns c
   WHERE c.concept_cki1="TRIBALENT_US*"
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
  CALL kia_load_file(kia_reltn_csv_filename,"kia_imp_reltn_load_ns","kia_imp_reltn_load_ns.log")
  CALL kia_set_status_success("LOAD_RELTN_BASELINE_DATA")
 ENDIF
 CALL kia_chk_status_info("kia_load_concept_reltn_2","check")
 IF (kia_obj_suc_ind=0)
  EXECUTE kia_load_concept_reltn_2 "TRIBALENT_US"
  CALL kia_chk_script_failure("kia_load_concept_reltn_2")
  CALL kia_set_status_success("kia_load_concept_reltn_2")
 ENDIF
 CALL kia_chk_status_info("LOAD_NOMEN_BASELINE","check")
 IF (kia_obj_suc_ind=0)
  CALL echo("DELETING EXISTING CONTENT FROM NOMECLATURE_LOAD_NS TABLE...")
  DELETE  FROM nomenclature_load_ns n
   WHERE n.source_vocabulary_mean IN ("SNMCT", kia_rdm_vocab_mean)
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
  CALL kia_load_file(kia_nomen_csv_filename,"kia_imp_nomen_load_ns","kia_imp_nomen_load_ns.log")
  CALL kia_set_status_success("LOAD_NOMEN_BASELINE")
 ENDIF
...
#exit_script
 ...
 CALL log_sub(concat("Script Elapsed Time (in seconds): ",cnvtstring(datetimediff(stop_time_var,
     start_time_var,5))))
 IF ((readme_data->status="F"))
  CALL log_sub(readme_data->message)
