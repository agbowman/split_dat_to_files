CREATE PROGRAM ct_sub_rpt_di_billing_rep_ra
 PROMPT
 ...
 SUBROUTINE layout_query(dummy)
   SELECT DISTINCT
    ...
    HEAD REPORT
     ...
     _d3 = ppr.prot_accession_nbr, _d4 = o_catalog_type_disp, _d5 = o.order_id,
     _d6 = pa.description, _d7 = o.orig_order_dt_tm, _d8 = oa.action_dt_tm,
    ...
    HEAD o.order_id
     encounter_type_display = e_encntr_type_disp
 ...
 SUBROUTINE (headreportsectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   DECLARE __fieldname3 = vc WITH noconstant(build2(label->rpt_title,char(0))), protect
   DECLARE __fieldname4 = vc WITH noconstant(build2(label->rpt_exec_mode,char(0))), protect
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_width = 8.511
    SET rptsd->m_height = 0.428
    ...
    SET rptsd->m_x = (offsetx+ - (0.020))
    SET rptsd->m_width = 8.521
 ...
 SUBROUTINE (headpm_primary_mnemonicsectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   DECLARE __fieldname8 = vc WITH noconstant(build2(label->prot_mnemonic_header,char(0))), protect
   DECLARE __fieldname9 = vc WITH noconstant(build2(label->prot_status,char(0))), protect
   DECLARE __fieldname10 = vc WITH noconstant(build2(label->principal_investigator,char(0))), protect
   DECLARE __fieldname13 = vc WITH noconstant(build2(label->prot_alias,char(0))), protect
   DECLARE __fieldname14 = vc WITH noconstant(build2(label->prot_sponsor,char(0))), protect
   DECLARE __fieldname3 = vc WITH noconstant(build2(label->res_acc,char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 0.063),(offsetx+ 8.501),(offsety+
     0.063))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 1.250),(offsetx+ 8.491),(offsety+
     1.250))
 ...
 SUBROUTINE (headp_person_idsectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   DECLARE __fieldname6 = vc WITH noconstant(build2(label->enroll_id,char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
 ...
 SUBROUTINE (heado_order_mnemonicsectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   DECLARE __fieldname0 = vc WITH noconstant(build2(label->encntr_type,char(0))), protect
   DECLARE __fieldname2 = vc WITH noconstant(build2(label->order_type,char(0))), protect
   DECLARE __fieldname4 = vc WITH noconstant(build2(label->powerplan_name,char(0))), protect
   ...
   DECLARE __fieldname12 = vc WITH noconstant(build2(label->order_placed_date,char(0))), protect
   DECLARE __fieldname13 = vc WITH noconstant(build2(label->order_status,char(0))), protect
   DECLARE __fieldname1 = vc WITH noconstant(build2(label->order_name,char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
 ...
 SUBROUTINE initializereport(dummy)
   IF (_bsubreport=0)
    ...
    SET rptreport->m_reportname = "CT_SUB_RPT_DI_BILLING_REP_RA"
