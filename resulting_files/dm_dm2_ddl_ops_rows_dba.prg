CREATE PROGRAM dm_dm2_ddl_ops_rows:dba
 ...
 DECLARE v_tablename = vc WITH noconstant("DM2_DDL_OPS1")
 ...
 IF (check_logfile("DM_DM2_DDL_OPS",".log","DM_DM2_DDL_OPS log file")=0)
  SET reply->status_data.status = "F"
  ...
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Begin DM_DM2_DDL_OPS_ROWS"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  SET reply->status_data.status = "F"
  SET reply->err_code = dm_err->ecode
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"ORATBLCHKERR",
   "Error checking for table existence: %1","s",nullterm(dm_err->emsg))
  GO TO end_program
 ENDIF
 ...
 SET v_rows = 0
 SET v_parser_stmt = concat("not exists (select ","ddol.run_id ","from dm2_ddl_ops_log1 ddol ",
  "where ddol.run_id = ddo.run_id)")
 IF (v_archi_version >= 2.0)
  SET v_parser_stmt = sbr_getrowidnotexists(v_parser_stmt,"ddo")
 ENDIF
 SET dm_err->eproc = "Find rows to purge from DM2_DDL_OPS1 table."
 SELECT INTO "nl:"
  ddo.rowid
  ...
  DETAIL
   ...
   ENDIF
   reply->rows[v_rows].row_id = ddo.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,v_rows)
  WITH nocounter, maxqual(ddo,value(request->max_rows))
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  SET reply->status_data.status = "F"
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"COLLECTERR",
   "Error collecting rows for purging: %1","s",nullterm(dm_err->emsg))
  GO TO end_program
 ENDIF
 SET reply->status_data.status = "S"
 SET reply->err_code = 0
 ...
 ENDIF
 SET dm_err->eproc = "Ending DM_DM2_DDL_OPS_ROWS"
 CALL final_disp_msg("DM_DM2_DDL_OPS")
END GO
