CREATE PROGRAM dm2_reset_mixedtbls_ops
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE drmo_tbl_name = vc WITH protect, noconstant(" ")
 DECLARE drmo_ndx = i4 WITH protect, noconstant(0)
 DECLARE drmo_dbase_name = vc WITH protect, noconstant("")
 DECLARE drmo_env_name = vc WITH protect, noconstant("")
 DECLARE drmo_str = vc WITH protect, noconstant("")
 IF (check_logfile("dm2_resetmixed_ops",".log","dm2_reset_mixedtbls_ops")=0)
  GO TO exit_program
 ENDIF
 SET drmo_tbl_name = cnvtupper( $1)
 IF (dir_load_mixed_table_data(0)=0)
  GO TO exit_program
 ENDIF
 IF (drmo_tbl_name != "\*")
  SET dm_err->eproc = "Check if Table passed in is a Mixed Table."
  SET drmo_ndx = locateval(drmo_ndx,1,value(dir_mixed_tables_data->cnt),drmo_tbl_name,
   dir_mixed_tables_data->tbl[drmo_ndx].table_name)
  IF (drmo_ndx=0)
   SET dm_err->err_ind = 1
   CALL disp_msg(concat(drmo_tbl_name," IS NOT A MIXED TABLE"),dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 SET dm_err->eproc = "Get the SCHEMA DATE and RUN_ID from DM2_DDL_OPS_LOG for latest CLIN COPY."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm2_ddl_ops d
  WHERE d.run_id IN (
  ...
  DETAIL
   dm2_install_schema->run_id = d.run_id, dir_clin_copy_data->schema_date = d.schema_date
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual=0)
  SET dm_err->emsg =
  "No CLIN COPY operation has completed in this domain, unable to obtain schema date."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echo("RUN_ID and SCHEMA_DATE")
  CALL echo(dm2_install_schema->run_id)
  CALL echo(dir_clin_copy_data->schema_date)
 ENDIF
 SET dm_err->eproc = "Get the database name for current run."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM v$database d
  DETAIL
   drmo_dbase_name = d.name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm2_install_schema->dbase_name = drmo_dbase_name
 ...
 ENDIF
 SET dm_err->eproc = "Get the database name."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM v$database d
  DETAIL
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Prompt User for Connection Correctness"
 IF ((dm_err->debug_flag < 100))
  SET message = window
  CALL clear(1,1)
 ENDIF
 SET drmo_str = concat("Removing operations for ",evaluate(drmo_tbl_name,"*","All Tables",
   drmo_tbl_name)," for Schema Date Capture: ",format(dir_clin_copy_data->schema_date,
   "DD-MMM-YYYY;;D"))
 CALL text(4,2,drmo_str)
 CALL text(6,2,"Current Connection  (Should be same as Target Connect Information)")
 CALL text(9,2,concat("Database Name = ",drmo_dbase_name))
 CALL text(10,2,concat("Database Connect String = ",currdblink))
 CALL text(13,2,concat("Captured Schema Date = ",format(dir_clin_copy_data->schema_date,
    "DD-MMM-YYYY;;D")))
 CALL text(15,2,"Target Connect Information")
 CALL text(17,2,concat("Database User Name = ",dm2_install_schema->target_dbase_name))
 CALL text(18,2,concat("Database Password = ",dm2_install_schema->v500_p_word))
 CALL text(19,2,concat("Database Connect String = ",dm2_install_schema->v500_connect_str))
 CALL text(22,2,"Continue (C), Quit (Q):")
 CALL accept(22,27,"p;cu","C"
  WHERE curaccept IN ("C", "Q"))
 IF (curaccept="Q")
  SET message = nowindow
  CALL clear(1,1)
  SET dm_err->emsg = "User chose to quit from Confirmation Screen."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 CALL text(24,1,"Are you sure the above information is correct? (Y,N)")
 CALL accept(24,54,"p;cu","Y"
  WHERE curaccept IN ("Y", "N"))
 IF (curaccept="N")
  CALL clear(1,1)
  SET message = nowindow
  SET dm_err->emsg = "User chose to quit from second prompt at Confirmation Screen."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag < 100))
  ...
  CALL clear(1,1)
 ENDIF
 SET dm_err->eproc = "RESETTING OPERATIONS."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (drmo_tbl_name="\*")
  DELETE  FROM dm2_ddl_ops_log d
   WHERE (d.run_id=dm2_install_schema->run_id)
    AND d.op_type IN ("EXPORT", "IMPORT", "TRUNCATE")
   WITH nocounter
  ;end delete
 ELSE
  DELETE  FROM dm2_ddl_ops_log d
   WHERE (d.run_id=dm2_install_schema->run_id)
    AND d.op_type IN ("EXPORT", "IMPORT", "TRUNCATE")
    AND d.table_name=drmo_tbl_name
   WITH nocounter
  ;end delete
 ENDIF
 IF (check_error(dm_err->eproc)=1)
 ...
 COMMIT
#exit_program
 SET dm_err->eproc = "Operations have been reset."
 CALL final_disp_msg("dm2_resetmixed_ops")
END GO
