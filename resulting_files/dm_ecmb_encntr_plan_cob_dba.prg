CREATE PROGRAM dm_ecmb_encntr_plan_cob:dba
 ...
 DECLARE v_to_count = i4
 DECLARE v_at_hist_ind = i1
 DECLARE dhistid = f8
 SET v_cust_count1 = 0
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "ENCNTR_PLAN_COB"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_ECMB_ENCNTR_PLAN_COB"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count1].from_id = frm.encntr_plan_cob_id, rreclist->from_rec[
 ...
 ;end select
 SELECT INTO "nl:"
  FROM encntr_plan_cob tu
  WHERE (tu.encntr_id=request->xxx_combine[icombine].to_xxx_id)
   AND tu.active_ind=1
  HEAD REPORT
   v_to_count = 0
  FOOT REPORT
   v_to_count = count(tu.encntr_plan_cob_id)
 ...
 SUBROUTINE del_from(s_df_pk_id,s_df_to_fk_id,s_df_prev_act_ind,s_df_prev_act_status)
   UPDATE  FROM encntr_plan_cob frm
    ...
    WHERE frm.encntr_plan_cob_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "ENCNTR_PLAN_COB"
 ...
 SUBROUTINE upt_from(s_uf_pk_id,s_uf_to_fk_id)
   UPDATE  FROM encntr_plan_cob frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     s_uf_to_fk_id
    WHERE frm.encntr_plan_cob_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
 ...
 SUBROUTINE add_hist(s_at_to_parent_pk_id)
   ...
   SET dm_cmb_cust_cols2->add_col_val[1].col_name = "ENCNTR_PLAN_COB_HIST_ID"
   ...
   SET dm_cmb_cust_cols2->where_col_val[1].col_name = "ENCNTR_PLAN_COB_ID"
   SET dm_cmb_cust_cols2->where_col_val[1].col_value = build(s_at_to_parent_pk_id)
   IF (size(dm_cmb_cust_cols2->col,5)=0)
    SET dm_cmb_cust_cols2->tbl_name = "ENCNTR_PLAN_COB_HIST"
    SET dm_cmb_cust_cols2->sub_select_from_tbl = "ENCNTR_PLAN_COB"
 ...
 SUBROUTINE initialize_hist_id(null)
  IF (v_at_hist_ind=0)
   ...
   SET dcipht_request->transaction_reason_txt = "ENCNTR_CMB_ENCNTR_PLAN_COB"
