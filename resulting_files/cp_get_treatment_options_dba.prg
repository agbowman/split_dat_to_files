CREATE PROGRAM cp_get_treatment_options:dba
 PROMPT
  ...
  prsnl_id, ppr_cd, concept_cd,
  concept_group_cd, intention_cd, alt_sel_cat_ids,
  oc_id, venue_type, viewable_encntrs,
  cp_node_id
 ...
 DECLARE getflexorders(null) = null
 DECLARE buildflatorderlist(null) = null
 DECLARE loadordereditems(null) = null
 DECLARE retrieveordersentenceinfo(null) = null
 DECLARE retrievefreetextvalues(null) = null
 ...
 SUBROUTINE main(null)
  CALL getflexorders(null)
  IF ((treatment_reply->status_data.status="S")
   AND ( $VIEWABLE_ENCNTRS != 0.0))
   CALL getparametervalues(12,viewable_encntr_list)
   SET viewable_encntr_list->cnt += 1
   SET stat = alterlist(viewable_encntr_list->qual,viewable_encntr_list->cnt)
   CALL buildflatorderlist(null)
 ...
 SUBROUTINE getflexorders(null)
   ...
   CALL log_message("Begin getFlexOrders()",log_level_debug)
   EXECUTE cpm_get_orders_flex "NOFORMS",  $PERSON_ID,  $ENCNTR_ID,
    $PRSNL_ID,  $CONCEPT_CD,  $CONCEPT_GROUP_CD,
    $INTENTION_CD,  $ALT_SEL_CAT_IDS,  $OC_ID,
    $VENUE_TYPE WITH replace("MPAGEREPLY","ORDER_FLEX_REPLY")
   SET treatment_reply->oc_id = order_flex_reply->oc_id
   SET treatment_reply->cbt_mean = order_flex_reply->cbt_mean
   SET treatment_reply->person_id = order_flex_reply->person_id
   SET treatment_reply->encntr_id = order_flex_reply->encntr_id
   SET treatment_reply->intention_mean = order_flex_reply->intention_mean
   SET stat = moverec(order_flex_reply->parent,treatment_reply->parent)
   SET stat = moverec(order_flex_reply->status_data,treatment_reply->status_data)
   SET stat = moverec(order_flex_reply->venue_type_list,treatment_reply->venue_type_list)
   CALL log_message(build("Exit getFlexOrders(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE buildflatorderlist(null)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   ...
   FOR (parent_cnt = 1 TO size(treatment_reply->parent,5))
     FOR (child_cnt = 1 TO size(treatment_reply->parent[parent_cnt].child,5))
       IF ((treatment_reply->parent[parent_cnt].child[child_cnt].synonym_id > 0))
        SET flat_order_list->synonym_cnt += 1
        SET stat = alterlist(flat_order_list->synonyms,flat_order_list->synonym_cnt)
        SET flat_order_list->synonyms[flat_order_list->synonym_cnt].id = treatment_reply->parent[
        parent_cnt].child[child_cnt].synonym_id
        SET flat_order_list->synonyms[flat_order_list->synonym_cnt].display = treatment_reply->
        parent[parent_cnt].child[child_cnt].synonym
        IF ((treatment_reply->parent[parent_cnt].child[child_cnt].synonym_id > 0))
         SET flat_order_list->sentence_cnt += 1
         SET stat = alterlist(flat_order_list->sentences,flat_order_list->sentence_cnt)
         SET flat_order_list->sentences[flat_order_list->sentence_cnt].id = treatment_reply->parent[
         parent_cnt].child[child_cnt].sentence_id
         SET flat_order_list->sentences[flat_order_list->sentence_cnt].display = treatment_reply->
         parent[parent_cnt].child[child_cnt].synonym
        ENDIF
       ELSEIF ((treatment_reply->parent[parent_cnt].child[child_cnt].path_cat_id > 0))
        SET flat_order_list->plan_cnt += 1
        SET stat = alterlist(flat_order_list->plans,flat_order_list->plan_cnt)
        SET flat_order_list->plans[flat_order_list->plan_cnt].id = treatment_reply->parent[parent_cnt
        ].child[child_cnt].path_cat_id
        SET flat_order_list->plans[flat_order_list->plan_cnt].display = treatment_reply->parent[
        parent_cnt].child[child_cnt].plan_description
       ELSEIF ((treatment_reply->parent[parent_cnt].child[child_cnt].reg_cat_id > 0))
        SET flat_order_list->regimen_cnt += 1
        SET stat = alterlist(flat_order_list->regimens,flat_order_list->regimen_cnt)
        SET flat_order_list->regimens[flat_order_list->regimen_cnt].id = treatment_reply->parent[
        parent_cnt].child[child_cnt].reg_cat_id
        SET flat_order_list->regimens[flat_order_list->regimen_cnt].display = treatment_reply->
        parent[parent_cnt].child[child_cnt].reg_cat_syn_display
       ENDIF
     ENDFOR
   ENDFOR
 ...
 SUBROUTINE loadordereditems(null)
   ...
   EXECUTE cp_get_ordered_synonyms  $OUTDEV,  $PERSON_ID,  $ENCNTR_ID,
    $PRSNL_ID,  $PPR_CD, 2,
   5
    ...
    HEAD os.order_sentence_id
     ...
     FOR (pcnt = 1 TO size(treatment_reply->parent,5))
      sentence_idx = locateval(idx,1,size(treatment_reply->parent[pcnt].child,5),os.order_sentence_id,
       ...
       "",treatment_reply->parent[pcnt].child[idx].sentence),
      IF (sentence_idx > 0)
 ...
 SUBROUTINE retrievefreetextvalues(null)
   ...
   SELECT
    ...
    PLAN (cnb
     WHERE cnb.cp_node_id=cnvtreal( $CP_NODE_ID)
    ...
    HEAD ltr.long_text_id
     ...
     treatment_reply->freetext_suggestions.qual[cnt].response_ident = cnb.response_ident
    FOOT REPORT
