CREATE PROGRAM act_get_entry_keys_by_space_id:dba
 RECORD reply(
   1 qual[*]
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   cnt = (cnt+ 1), stat = alterlist(reply->qual,cnt), reply->qual[cnt].object_entry_name = e
   .object_entry_name,
   reply->qual[cnt].object_space_id = s.object_space_id, reply->qual[cnt].long_blob_id = e
   .long_blob_id, reply->qual[cnt].object_space_entry_id = e.object_space_entry_id,
   reply->qual[cnt].object_space_expiration = s.object_space_expiration, reply->qual[cnt].
