CREATE PROGRAM cmt_q2_2003_log:dba
 PAINT
 FREE SET all
#accept_script
 CALL clear(1,1)
 CALL box(1,1,22,80)
 CALL text(2,4,"CMT Q2 2003 Log Menu")
 CALL text(3,4,"Choose from the following:")
 CALL line(4,1,80,xhor)
 CALL text(6,4,"1)  apc obsolete terms log")
 CALL text(7,4,"2)  apc new nomenclature import log")
 CALL text(8,4,"3)  apc changed terms import log")
 CALL text(9,4,"4)  apc extension log")
 CALL text(10,4,"5)  apc related codes log")
 CALL text(12,4,"6)  hcpcs obsolete nomen log")
 CALL text(13,4,"7)  hcpcs obsolete terms log")
 CALL text(14,4,"8)  hcpcs changed terms log")
 CALL text(15,4,"9)  hcpcs new nomenclature import log")
 ...
 CASE (curaccept)
  OF 1:
   CALL display_logfile("kia_obs_apc_pvterm.log",0)
  OF 2:
   CALL display_logfile("kia_imp_ins_apc_nomen.log",1)
  OF 3:
   CALL display_logfile("kia_imp_ins_apc_nomen.log",2)
  OF 4:
   CALL display_logfile("kia_import_apc_extension.log",0)
  OF 5:
   CALL display_logfile("cps_imp_apc_related_code.log",0)
  OF 6:
   CALL display_logfile("kia_obs_imp_hcpcs_nomen.log",0)
  OF 7:
   CALL display_logfile("kia_obs_hcpcs_pvterm.log",0)
  OF 8:
   CALL display_logfile("kia_imp_ins_hcpcs_nomen.log",1)
  OF 9:
   CALL display_logfile("kia_imp_ins_hcpcs_nomen.log",2)
  OF 50:
   CALL get_counts("APC")
  OF 51:
   CALL obs_report("APC")
  OF 52:
   CALL new_nomen_report("APC")
  OF 53:
   CALL ext_report("APC")
  OF 54:
   CALL rel_report("APC")
  OF 55:
   CALL get_counts("HCPCS")
  OF 56:
   CALL obs_report("HCPCS")
  OF 57:
   CALL new_nomen_report("HCPCS")
  OF 99:
   GO TO end_program
 ENDCASE
 IF (curscroll=1)
 ...
 SUBROUTINE kia_get_version_ft(mean)
   ...
   SELECT INTO "nl:"
    ...
    WHERE cv.code_set=400
     ...
     AND sysdate < cv.end_effective_dt_tm
 ...
 SUBROUTINE kia_insert_cmt_version(beg_date,ver_ft)
   ...
   UPDATE  FROM cmt_content_version c
    SET c.ver_end_effective_dt_tm = datetimeadd(cnvtdatetime(concat(trim(beg_date)," 23:59:59")),- (1
     ...
     c.updt_dt_tm = sysdate, c.updt_id = 0, c.updt_task = 0
    WHERE c.source_vocabulary_cd=new_ver_cd
     AND c.ver_beg_effective_dt_tm <= sysdate
     AND sysdate <= c.ver_end_effective_dt_tm
     AND c.content_type_flag=kia_rdm_cont_type
     AND c.parent_entity_id=par_ent_id
     AND c.parent_entity_name=kia_rdm_pent_name
    WITH nocounter
   ...
   INSERT  FROM cmt_content_version c
    SET c.active_ind = 1, c.beg_effective_dt_tm = cnvtdatetime(beg_date), c.cmt_content_version_id =
     ...
     c.content_type_flag = kia_rdm_cont_type, c.end_effective_dt_tm = cnvtdatetime(
      "31-DEC-2100 23:59:59"), c.parent_entity_id =
     ...
     ELSE " "
     ENDIF
     , c.source_vocabulary_cd = new_ver_cd, c.version_ft = ver_ft,
     c.version_number = kia_rdm_cur_ver, c.ver_beg_effective_dt_tm = cnvtdatetime(beg_date), c
     .ver_end_effective_dt_tm = cnvtdatetime("31-DEC-2100 23:59:59"),
     c.updt_applctx = 0, c.updt_cnt = 0, c.updt_id = 0,
     c.updt_task = 0, c.updt_dt_tm = sysdate
    WITH nocounter
   ;end insert
   COMMIT
 END ;Subroutine
 ...
 SUBROUTINE kia_set_status_success(suc_obj)
   ...
   INSERT  FROM cmt_import_log c
    SET c.cmt_import_log_id = new_cmt_id, c.end_dt_tm = sysdate, c.input_filename = trim(cnvtupper(
       suc_obj)),
   ...
   IF (((curqual < 1) OR (((trim(kia_parse_script) <= "") OR (kia_parse_blocks < 1)) )) )
    ...
    INSERT  FROM cmt_import_log c
     SET c.block_size = kia_parse_blocks, c.cmt_import_log_id = new_cmt_id, c.input_filename = trim(
      ...
      c.script_name = kia_parse_script, c.start_dt_tm = sysdate, c.start_record = 0,
      c.status_flag = 0, c.updt_id = readme_number, c.updt_dt_tm = sysdate
     WITH nocounter
    ;end insert
    COMMIT
   ENDIF
 ...
 SUBROUTINE kia_inhouse_chk(var)
   IF ( NOT (cnvtupper(curuser) IN ("P30INS", "DD011127", "JL2501", "CERLWS", "KW4926",
   "CERFTS", "RS011138", "RK010957", "BG2117")))
    SELECT INTO "nl:"
     FROM dm_info d
   ...
   ELSE
    ...
    INSERT  FROM cmt_import_log c
     SET c.block_size = 500, c.cmt_import_log_id = new_cmt_id, c.input_filename = trim(cnvtupper(
      ...
      c.script_name = "kia_dm_dbimport", c.start_dt_tm = sysdate, c.start_record = 0,
      c.status_flag = 0, c.updt_id = kia_rdm_nbr, c.updt_dt_tm = sysdate
     WITH nocounter
    ;end insert
   ENDIF
   COMMIT
