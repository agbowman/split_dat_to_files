CREATE PROGRAM dm2_undo_default_rows_c:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dadrc_logfile_name = vc WITH protect, constant("dm2_undodrc_")
 DECLARE dadr_text = vc WITH protect, noconstant(" ")
 DECLARE dadr_insert_sql = vc WITH protect, noconstant(" ")
 DECLARE dadr_delete_sql = vc WITH protect, noconstant(" ")
 DECLARE dadr_ccl_datefmt = vc WITH protect, noconstant(" ")
 DECLARE dadr_when_text = vc WITH protect, noconstant(" ")
 DECLARE dadr_of_text = vc WITH protect, noconstant(" ")
 DECLARE dadr_valid_table = i2 WITH protect, noconstant(1)
 DECLARE dadr_invalid_table_cnt = i2 WITH protect, noconstant(0)
 DECLARE dadr_mon = c3 WITH protect, noconstant(" ")
 DECLARE dadr_error_found = i2 WITH protect, noconstant(0)
 DECLARE dadr_ins_default_row(tbl_idx=i4) = i2
 DECLARE dadr_create_db2_triggers(sub_idx=i4) = i2
 DECLARE dadr_create_db2_del_trig(sub_trig_name=vc,sub_idx=i4) = i2
 DECLARE dadr_create_db2_upd_trig(sub_trig_name=vc,sub_idx=i4) = i2
 DECLARE dadr_create_ora_trigger(sub_trig_name=vc,sub_idx=i4) = i2
 IF (check_logfile(dadrc_logfile_name,".log","dm2_undo_default_ROWS_C.PRG ...") != 1)
  GO TO exit_program
 ENDIF
 IF (validate(dadr_tname," ")=" ")
  SET dm_err->eproc = concat(
   " dm2_undo_default_ROWS_C Error. This a sub-program for dm2_undo_default_ROWS and cannot",
   " run by itself.  Please execute dm2_undo_default_ROWS instead.")
  CALL disp_msg(" ",dm_err->logfile,1)
  SET dm_err->err_ind = 1
  GO TO exit_program
 ENDIF
 SET dadr_error_found = 0
 IF (dadr_valid_table=1
  AND dadr_error_found=0)
  CASE (currdb)
   OF "ORACLE":
    IF (dadr_create_ora_trigger(ix1) != 1)
     IF (dadr_wildcard_found=0)
      GO TO exit_program
     ENDIF
    ENDIF
   OF "DB2UDB":
    IF (dadr_create_db2_triggers(ix1)=1)
     COMMIT
    ELSE
     ROLLBACK
     IF (dadr_wildcard_found=0)
   ...
   OF "SQLSRV":
    IF ((dm_err->debug_flag > 0))
     CALL echo(concat("SQLSRV: Bypassing Default Row Triggers for table: ",dadr_tbl->tbl[ix1].tname))
    ENDIF
  ENDCASE
 ENDIF
 IF (dadr_ins_default_row(ix1) != 1)
  ROLLBACK
  IF (dadr_wildcard_found=0)
   GO TO exit_program
  ENDIF
 ENDIF
 GO TO exit_program
 SUBROUTINE dadr_ins_default_row(sub_idx)
   DECLARE didr_error_code = vc WITH protect, noconstant(" ")
   DECLARE didr_date_default = vc WITH protect, noconstant(" ")
   SET dadr_valid_table = 1
   SET dadr_text = concat(' SELECT INTO "NL:" FROM ',dadr_tbl->tbl[ix1].tname," WHERE ")
   SET dadr_delete_sql = concat(" DELETE FROM ",dadr_tbl->tbl[ix1].tname," WHERE ")
   SET dadr_insert_sql = concat(" INSERT INTO ",dadr_tbl->tbl[ix1].tname," SET ")
   SET dadr_when_text = "WHEN ("
   SET dadr_of_text = "OF"
   IF ((dm_err->debug_flag > 0))
    CALL echo(concat("**** $$$ Checking TABLE: ",trim(dadr_tbl->tbl[sub_idx].tname)))
   ENDIF
   FOR (idx2 = 1 TO dadr_tbl->tbl[sub_idx].col_cnt)
     IF ((dadr_tbl->tbl[sub_idx].col[idx2].pk_ind=1)
      AND (dadr_tbl->tbl[sub_idx].col[idx2].data_type IN ("TIMESTAMP", "DATE", "DATETIME")))
      CASE (currdb)
       OF "ORACLE":
        SET didr_date_default = cnvtupper(cnvtalphanum(dadr_tbl->tbl[sub_idx].col[idx2].data_default)
         )
        IF (substring(1,6,didr_date_default)="TODATE"
         AND substring(21,16,didr_date_default)="MMDDYYYYHH24MISS")
         CASE (substring(7,2,didr_date_default))
          OF "01":
           SET dadr_mon = "JAN"
          OF "02":
           SET dadr_mon = "FEB"
          ...
          OF "12":
           SET dadr_mon = "DEC"
         ENDCASE
         SET dadr_ccl_datefmt = concat(" CNVTDATETIME('",substring(9,2,didr_date_default),"-",
          dadr_mon,"-",
          substring(11,4,didr_date_default)," ",substring(15,2,didr_date_default),":",substring(17,2,
           didr_date_default),
          ":",substring(19,2,didr_date_default),"')")
        ELSE
         SET dadr_ccl_datefmt = "CNVTDATETIME('31-DEC-2100 00:00:00')"
         SET dadr_tbl->tbl[sub_idx].col[idx2].data_default =
         "TO_DATE('12/31/2100 00:00:00','MM/DD/YYYY HH24:MI:SS')"
        ENDIF
       OF "DB2UDB":
        IF (substring(2,6,dadr_tbl->tbl[sub_idx].col[idx2].data_default)="SYSIBM")
         IF (substring(11,9,dadr_tbl->tbl[sub_idx].col[idx2].data_default)="TIMESTAMP")
          CASE (substring(28,2,dadr_tbl->tbl[sub_idx].col[idx2].data_default))
           OF "01":
            SET dadr_mon = "JAN"
           OF "02":
            SET dadr_mon = "FEB"
           ...
           OF "12":
            SET dadr_mon = "DEC"
          ENDCASE
          SET dadr_ccl_datefmt = concat(" CNVTDATETIME('",substring(31,2,dadr_tbl->tbl[sub_idx].col[
            idx2].data_default),"-",dadr_mon,"-",
           substring(23,4,dadr_tbl->tbl[sub_idx].col[idx2].data_default)," ",substring(34,2,dadr_tbl
            ->tbl[sub_idx].col[idx2].data_default),":",substring(37,2,dadr_tbl->tbl[sub_idx].col[idx2
            ].data_default),
           ":",substring(40,2,dadr_tbl->tbl[sub_idx].col[idx2].data_default),"')")
          SET dadr_tbl->tbl[sub_idx].col[idx2].data_default = concat("'",substring(23,4,dadr_tbl->
            tbl[sub_idx].col[idx2].data_default),"-",substring(28,2,dadr_tbl->tbl[sub_idx].col[idx2].
            data_default),"-",
           substring(31,2,dadr_tbl->tbl[sub_idx].col[idx2].data_default),"-",substring(34,2,dadr_tbl
            ->tbl[sub_idx].col[idx2].data_default),".",substring(37,2,dadr_tbl->tbl[sub_idx].col[idx2
            ].data_default),
           ".",substring(40,2,dadr_tbl->tbl[sub_idx].col[idx2].data_default),".000000'")
         ELSE
          SET dadr_ccl_datefmt = "CNVTDATETIME('31-DEC-2100 00:00:00')"
          SET dadr_tbl->tbl[sub_idx].col[idx2].data_default = concat('"SYSIBM"."TIMESTAMP"',
           "('2100-12-31-00.00.00.000000')")
         ENDIF
        ELSE
         SET dadr_ccl_datefmt = "CNVTDATETIME('31-DEC-2100 00:00:00')"
         SET dadr_tbl->tbl[sub_idx].col[idx2].data_default = concat('"SYSIBM"."TIMESTAMP"',
        ...
        ENDIF
       OF "SQLSRV":
        IF (substring(1,18,dadr_tbl->tbl[sub_idx].col[idx2].data_default)="convert(datetime,'")
         CASE (substring(24,2,dadr_tbl->tbl[sub_idx].col[idx2].data_default))
          OF "01":
           SET dadr_mon = "JAN"
          OF "02":
           SET dadr_mon = "FEB"
          ...
          OF "12":
           SET dadr_mon = "DEC"
         ENDCASE
         SET dadr_ccl_datefmt = concat("CNVTDATETIME('",substring(27,2,dadr_tbl->tbl[sub_idx].col[
           idx2].data_default),"-",dadr_mon,"-",
          substring(19,4,dadr_tbl->tbl[sub_idx].col[idx2].data_default)," ",substring(30,2,dadr_tbl->
           tbl[sub_idx].col[idx2].data_default),":",substring(33,2,dadr_tbl->tbl[sub_idx].col[idx2].
           data_default),
          ":",substring(36,2,dadr_tbl->tbl[sub_idx].col[idx2].data_default),"')")
        ELSE
         SET dadr_ccl_datefmt = "CNVTDATETIME('31-DEC-2100 00:00:00')"
         SET dadr_tbl->tbl[sub_idx].col[idx2].data_default =
         "CONVERT(DATETIME, '2100-12-31 00:00:00')"
        ENDIF
      ENDCASE
      SET dadr_tbl->tbl[sub_idx].col[idx2].data_default_null_ind = 0
     ENDIF
     IF ((dadr_tbl->tbl[sub_idx].col[idx2].nullable="N")
      AND (dadr_tbl->tbl[sub_idx].col[idx2].data_default_null_ind=1))
      IF ((dm_err->debug_flag > 0))
       CALL echo(concat("**** $$$ Checking COLUMN: ",trim(dadr_tbl->tbl[sub_idx].col[idx2].col_name))
        )
      ENDIF
      CASE (trim(dadr_tbl->tbl[sub_idx].col[idx2].data_type))
       OF "INTEGER":
        SET dadr_tbl->tbl[sub_idx].col[idx2].data_default = "0"
       OF "BIGINT":
        SET dadr_tbl->tbl[sub_idx].col[idx2].data_default = "0"
       OF "SMALLINT":
        SET dadr_tbl->tbl[sub_idx].col[idx2].data_default = "0"
       ...
       OF "LONG":
        SET dadr_tbl->tbl[sub_idx].col[idx2].data_default = "' '"
       OF "LONG RAW":
        SET dadr_tbl->tbl[sub_idx].col[idx2].data_default = "sqlpassthru(^rawtohex('0')^,1)"
       OF "DATE":
        SET dadr_ccl_datefmt = "CNVTDATETIME(CURDATE, CURTIME3)"
       OF "TIMESTAMP":
        SET dadr_ccl_datefmt = "CNVTDATETIME(CURDATE, CURTIME3)"
       OF "DATETIME":
        SET dadr_ccl_datefmt = "CNVTDATETIME(CURDATE, CURTIME3)"
       ELSE
        SET dadr_valid_table = 0
        SET dm_err->eproc = concat("*** Warning: A not null column was found with a ",
         "null default value, but was a data_type (",trim(dadr_tbl->tbl[sub_idx].col[idx2].data_type),
         ") that this program is not designed to provide a default value for.","  Table/column is: (",
         trim(dadr_tbl->tbl[sub_idx].tname),"/",trim(dadr_tbl->tbl[sub_idx].col[idx2].col_name,3),
         ").  This table is bypassed.  No default row or triggers were created for this table. ")
        CALL disp_msg("",dm_err->logfile,0)
        SET idx2 = dadr_tbl->tbl[sub_idx].col_cnt
        SET dadr_warning_cnt = (dadr_warning_cnt+ 1)
      ENDCASE
     ENDIF
     IF (dadr_valid_table=1)
      IF ((dadr_tbl->tbl[sub_idx].col[idx2].pk_ind=1))
       SET dadr_of_text = concat(dadr_of_text," ",trim(dadr_tbl->tbl[sub_idx].col[idx2].col_name,3),
        ",")
       IF (trim(dadr_tbl->tbl[sub_idx].col[idx2].data_type,3) IN ("DATE", "TIMESTAMP", "DATETIME"))
        SET dadr_text = concat(dadr_text," ",trim(dadr_tbl->tbl[sub_idx].col[idx2].col_name,3)," = ",
         dadr_ccl_datefmt,
         " AND ")
        SET dadr_delete_sql = concat(dadr_delete_sql," ",trim(dadr_tbl->tbl[sub_idx].col[idx2].
          col_name,3)," = ",dadr_ccl_datefmt,
         " AND ")
        CASE (currdb)
         OF "ORACLE":
          SET dadr_when_text = concat(dadr_when_text," OLD.",trim(dadr_tbl->tbl[sub_idx].col[idx2].
            col_name,3)," = ",trim(dadr_tbl->tbl[sub_idx].col[idx2].data_default,3),
           " AND ")
         OF "DB2UDB":
          SET dadr_when_text = concat(dadr_when_text," OLD.",trim(dadr_tbl->tbl[sub_idx].col[idx2].
            col_name,3)," = ","CAST(",
           trim(dadr_tbl->tbl[sub_idx].col[idx2].data_default,3)," AS TIMESTAMP) AND ")
        ENDCASE
       ELSE
        SET dadr_text = concat(dadr_text," ",trim(dadr_tbl->tbl[sub_idx].col[idx2].col_name,3)," = ",
         trim(dadr_tbl->tbl[sub_idx].col[idx2].data_default,3),
         " AND ")
        SET dadr_delete_sql = concat(dadr_delete_sql," ",trim(dadr_tbl->tbl[sub_idx].col[idx2].
          col_name,3)," = ",trim(dadr_tbl->tbl[sub_idx].col[idx2].data_default,3),
         " AND ")
      ...
      ENDIF
      IF (trim(dadr_tbl->tbl[sub_idx].col[idx2].data_type,3) IN ("DATE", "TIMESTAMP", "DATETIME"))
       SET dadr_insert_sql = concat(dadr_insert_sql," ",trim(dadr_tbl->tbl[sub_idx].col[idx2].
         col_name,3)," = ",dadr_ccl_datefmt,
        ",")
      ELSE
       SET dadr_insert_sql = concat(dadr_insert_sql," ",trim(dadr_tbl->tbl[sub_idx].col[idx2].
     ...
     ENDIF
   ENDFOR
   SET dadr_text = replace(dadr_text,"AND"," GO",2)
   SET dadr_delete_sql = replace(dadr_delete_sql,"AND"," GO",2)
   SET dadr_insert_sql = replace(dadr_insert_sql,","," WITH NOCOUNTER GO ",2)
   SET dadr_when_text = replace(dadr_when_text,"AND"," )",2)
   SET dadr_of_text = replace(dadr_of_text,","," ",2)
   IF (dadr_valid_table=1)
    SET dm_err->eproc = "dm2_undo_default_ROWS"
    IF (dm2_push_cmd(dadr_text,1) != 1)
     ROLLBACK
     SET dadr_error_found = 1
     SET dadr_error_cnt = (dadr_error_cnt+ 1)
     SET dm_err->err_ind = 0
     RETURN(1)
    ENDIF
    IF (curqual=1)
     IF ((dm_err->debug_flag > 0))
      CALL echo(concat("Call parsing DELETE of default row, command is: ",dadr_delete_sql))
      SET dm_err->eproc = concat("Executing default row delete: ",dadr_delete_sql)
      CALL disp_msg("",dm_err->logfile,0)
     ENDIF
     IF (dm2_push_cmd(dadr_delete_sql,1) != 1)
      ROLLBACK
      SET dadr_error_found = 1
      SET dadr_error_cnt = (dadr_error_cnt+ 1)
      SET dm_err->err_ind = 0
     ...
     IF ((dm_err->ecode > 0))
      ROLLBACK
      SET dadr_valid_table = 0
      SET dm_err->eproc = concat("*** Warning : Unable to DELETE the default row for table: ",trim(
        dadr_tbl->tbl[sub_idx].tname),"  Reason code: ",dm_err->emsg)
      CALL disp_msg("",dm_err->logfile,0)
      SET dadr_warning_cnt = (dadr_warning_cnt+ 1)
     ELSE
      COMMIT
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dadr_create_db2_triggers(sub_idx)
   DECLARE dadr_del_trig_name = vc WITH protect, noconstant(" ")
   DECLARE dadr_del_drop_ind = i2 WITH protect, noconstant(0)
   DECLARE dadr_upd_trig_name = vc WITH protect, noconstant(" ")
   DECLARE dadr_upd_drop_ind = i2 WITH protect, noconstant(0)
   IF ((dadr_tbl->tbl[sub_idx].trig_del_c_exists=1)
    AND (dadr_tbl->tbl[sub_idx].trig_del_reg_exists=1))
    SET dadr_text = concat("RDB ASIS(^ DROP TRIGGER ",trim(dadr_tbl->tbl[sub_idx].trig_del_c_name),
     " ^) GO")
    IF (dm2_push_cmd(dadr_text,1) != 1)
     SET dadr_error_found = 1
     SET dadr_error_cnt = (dadr_error_cnt+ 1)
     ...
     ROLLBACK
    ENDIF
    SET dadr_del_trig_name = dadr_tbl->tbl[sub_idx].trig_del_reg_name
    SET dadr_del_drop_ind = 1
   ELSEIF ((dadr_tbl->tbl[sub_idx].trig_del_c_exists=1))
    SET dadr_del_trig_name = dadr_tbl->tbl[sub_idx].trig_del_c_name
    SET dadr_del_drop_ind = 1
   ELSE
    SET dadr_del_trig_name = dadr_tbl->tbl[sub_idx].trig_del_reg_name
    IF ((dadr_tbl->tbl[sub_idx].trig_del_reg_exists=1))
     SET dadr_del_drop_ind = 1
    ENDIF
   ENDIF
   IF (dadr_del_drop_ind=1
    AND dadr_error_found=0)
    SET dadr_text = concat("RDB ASIS(^ DROP TRIGGER ",trim(dadr_del_trig_name)," ^) GO")
    IF (dm2_push_cmd(dadr_text,1) != 1)
     ROLLBACK
     SET dadr_error_found = 1
     SET dadr_error_cnt = (dadr_error_cnt+ 1)
    ...
    ENDIF
   ENDIF
   IF ((dadr_tbl->tbl[sub_idx].trig_upd_c_exists=1)
    AND (dadr_tbl->tbl[sub_idx].trig_upd_reg_exists=1)
    AND dadr_error_found=0)
    SET dadr_text = concat("RDB ASIS(^ DROP TRIGGER ",trim(dadr_tbl->tbl[sub_idx].trig_upd_c_name),
     " ^) GO")
    IF (dm2_push_cmd(dadr_text,1) != 1)
     ROLLBACK
     SET dadr_error_found = 1
     ...
     RETURN(1)
    ENDIF
    SET dadr_upd_trig_name = dadr_tbl->tbl[sub_idx].trig_upd_reg_name
    SET dadr_upd_drop_ind = 1
   ELSEIF ((dadr_tbl->tbl[sub_idx].trig_upd_c_exists=1))
    SET dadr_upd_trig_name = dadr_tbl->tbl[sub_idx].trig_upd_c_name
    SET dadr_upd_drop_ind = 1
   ELSE
    SET dadr_upd_trig_name = dadr_tbl->tbl[sub_idx].trig_upd_reg_name
    IF ((dadr_tbl->tbl[sub_idx].trig_upd_reg_exists=1))
     SET dadr_upd_drop_ind = 1
    ENDIF
   ENDIF
   IF (dadr_upd_drop_ind=1
    AND dadr_error_found=0)
    SET dadr_text = concat("RDB ASIS(^ DROP TRIGGER ",trim(dadr_upd_trig_name)," ^) GO")
    IF (dm2_push_cmd(dadr_text,1) != 1)
     ROLLBACK
     SET dadr_error_found = 1
     SET dadr_error_cnt = (dadr_error_cnt+ 1)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE dadr_create_db2_del_trig(sub_trig_name,sub_idx)
   SET dadr_text = concat(" CREATE TRIGGER ",trim(sub_trig_name)," NO CASCADE BEFORE DELETE ON ",trim
    (dadr_tbl->tbl[sub_idx].tname)," REFERENCING OLD AS OLD ",
    " FOR EACH ROW MODE DB2SQL ")
   IF (trim(dadr_tbl->tbl[sub_idx].tname)="CODE_VALUE_0619")
    SET dadr_text = concat(dadr_text," WHEN (OLD.CODE_VALUE = 0")
   ELSE
    SET dadr_text = concat(dadr_text," ",dadr_when_text)
   ENDIF
   SET dadr_text = concat(dadr_text," BEGIN ATOMIC "," SIGNAL SQLSTATE '75000' ",
    " SET MESSAGE_TEXT = 'Default row on ",trim(dadr_tbl->tbl[sub_idx].tname),
    " cannot be deleted'; "," END")
   SET dadr_text = concat("RDB ASIS(^ ",dadr_text," ^) GO")
   IF (dm2_push_cmd(dadr_text,1) != 1)
    ROLLBACK
    SET dadr_error_found = 1
    SET dadr_error_cnt = (dadr_error_cnt+ 1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dadr_create_db2_upd_trig(sub_trig_name,sub_idx)
   SET dadr_text = concat(" CREATE TRIGGER ",trim(sub_trig_name)," NO CASCADE BEFORE UPDATE ")
   IF (trim(dadr_tbl->tbl[sub_idx].tname) != "CODE_VALUE_0619")
    SET dadr_text = concat(dadr_text," ",dadr_of_text)
   ENDIF
   SET dadr_text = concat(dadr_text," ON ",trim(dadr_tbl->tbl[sub_idx].tname),
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dadr_create_ora_trigger(sub_idx)
   DECLARE dcot_del_trig_name = vc WITH protect, noconstant(" ")
   IF ((dadr_tbl->tbl[sub_idx].trig_del_c_exists=1)
    AND (dadr_tbl->tbl[sub_idx].trig_del_reg_exists=1))
    SET dadr_text = concat("RDB ASIS(^ DROP TRIGGER ",trim(dadr_tbl->tbl[sub_idx].trig_del_c_name),
     " ^) GO")
    IF (dm2_push_cmd(dadr_text,1) != 1)
     ...
     RETURN(1)
    ENDIF
    SET dcot_del_trig_name = dadr_tbl->tbl[sub_idx].trig_del_reg_name
   ELSEIF ((dadr_tbl->tbl[sub_idx].trig_del_c_exists=1))
    SET dcot_del_trig_name = dadr_tbl->tbl[sub_idx].trig_del_c_name
   ELSE
    SET dcot_del_trig_name = dadr_tbl->tbl[sub_idx].trig_del_reg_name
   ENDIF
   IF ((dadr_tbl->tbl[sub_idx].trig_del_c_exists=0)
    AND (dadr_tbl->tbl[sub_idx].trig_del_reg_exists=0))
    SET dcot_del_trig_name = concat("TRG_",dadr_tbl->tbl[sub_idx].short_tname,"_DR_UPDT_DEL")
   ENDIF
   SELECT INTO "NL:"
    FROM dm2_user_triggers
    WHERE trigger_name=dcot_del_trig_name
    WITH nocounter
   ;end select
   IF (check_error(concat("SELECTING FROM DM2_USER_TRIGGER FOR ORACLE TRIGGER.")) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   SET dadr_text = concat("RDB DROP TRIGGER ",dcot_del_trig_name," GO")
   IF (curqual=1)
    SET dm_err->eproc = concat("Executing trigger drop: ",dadr_text)
    CALL disp_msg("",dm_err->logfile,0)
    IF (dm2_push_cmd(dadr_text,1) != 1)
     ROLLBACK
     SET dadr_error_found = 1
