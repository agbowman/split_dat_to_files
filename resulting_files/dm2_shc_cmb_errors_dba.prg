CREATE PROGRAM dm2_shc_cmb_errors:dba
 ...
 DECLARE dsce_pop_errfile(sbr_str) = null
 FREE RECORD dsce_errfile
 RECORD dsce_errfile(
   1 file_cnt = i4
   1 file[*]
     2 file_name = vc
 )
 IF (check_logfile("DM2_SHC_CMB_ERRORS",".log","DM2_SHC_CMB_ERRORS LOG FILE")=0)
  SET dsce_err_status = "F"
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "System Health Check working on Error Files in CMB..."
 CALL disp_msg("",dm_err->logfile,0)
 ...
 ENDIF
 IF (cursys="AIX")
  IF (dm2_push_dcl('find $cmb_temp -name "\*.err" -exec basename {} \;')=0)
   SET dsce_err_status = "E"
  ELSE
   CALL dsce_pop_errfile("cmb")
  ENDIF
 ELSEIF (cursys="AXP")
  SET dm_err->disp_dcl_err_ind = 0
  IF (dm2_push_dcl("dir /column=1 cmb_temp:\*.err")=0)
   IF ((dm_err->err_ind=1))
    SET dsce_err_status = "E"
   ELSEIF (findstring("no files found",dm_err->emsg) > 0)
 ...
 ENDIF
 IF (dsce_err_status=" ")
  IF ((dsce_errfile->file_cnt=0))
   SET dsce_err_status = "G"
  ELSE
   SET dsce_err_status = "R"
  ENDIF
 ENDIF
 IF (dshc_add_check_to_rs("ERROR FILES IN CMB","Error Files (.err) in CMB",0,dsce_err_status,
 ...
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(dsce_errfile->file_cnt))
  DETAIL
   ...
   dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Error Files in CMB", dshc->check[dshc->chk_cnt
   ].desc_detail[cnt].value_prefix = "NOT_SET", dshc->check[dshc->chk_cnt].desc_detail[cnt].
   dtl_desc_text = "NOT_SET",
   dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 2, dshc->check[dshc->chk_cnt].desc_detail[cnt].
   str_value = dsce_errfile->file[d.seq].file_name
  WITH nocounter
 ;end select
 IF (check_error(concat("Write out details for cmb error files.")) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dsce_err_status = "F"
 ENDIF
 SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
  "The CMB_TEMP directory should be checked periodically for .ERR files.  ",
  "Application servers that are generating .ERR files need to be checked that they ",
  "are functioning properly. <SPACE>The following CMB*.ERR files were found.")
#exit_script
 ...
 IF (dsce_err_status="F")
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "EXITING DM2_SHC_CMB_ERRORS WITH ERRORS"
 ELSEIF ((dm_err->err_ind=1))
  SET dm_err->err_ind = 0
  SET dm_err->eproc = "EXITING DM2_SHC_CMB_ERRORS WITH ERRORS"
 ELSE
  SET dm_err->eproc = "DM2_SHC_CMB_ERRORS COMPLETED SUCCESSFULLY"
 ENDIF
 CALL final_disp_msg("DM2_SHC_CMB_ERRORS")
 SUBROUTINE dsce_pop_errfile(sbr_str)
   FREE DEFINE rtl2
   FREE SET file_loc
   SET logical file_loc value(dm_err->errfile)
   DEFINE rtl2 "file_loc"
   SELECT INTO "nl:"
    ...
    HEAD REPORT
     beg_pos = 0, len = 0
    DETAIL
     beg_pos = 0, len = 0, beg_pos = findstring(sbr_str,r.line)
     IF (beg_pos > 0)
      len = textlen(trim(r.line)), dsce_errfile->file_cnt += 1, stat = alterlist(dsce_errfile->file,
       dsce_errfile->file_cnt),
      dsce_errfile->file[dsce_errfile->file_cnt].file_name = substring(beg_pos,len,r.line)
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(concat("Parse out cmb error file names")) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dsce_err_status = "E"
   ENDIF
 END ;Subroutine
 FREE RECORD dsce_pop_errfile
END GO
