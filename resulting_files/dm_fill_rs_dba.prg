CREATE PROGRAM dm_fill_rs:dba
 ...
 DECLARE dm2_rdds_col_add(sbr_tbl_idx=i4,sbr_col_idx=i4,r_ind=i2) = i2
 DECLARE dm2_rdds_col_extend(sbr_tbl_idx=i4,sbr_col_idx=i4,r_ind=i2) = i2
 DECLARE dm2_rdds_col_compare(srb_tbl_idx=i4,r_ind=i2) = i2
 DECLARE fill_ccl_data_info(cdi_tbl_cnt=i4) = i2
 DECLARE dm2_rdds_record_plsql_metadata(metadata=vc(ref),rs_pos=i4) = null
 DECLARE check_null_string(string=vc) = vc
 IF (validate(reply->env_source_id,- (1)) < 0)
  RECORD reply(
    1 pre_link_name = vc
    1 post_link_name = vc
    1 mock_target_id = f8
 ...
 DECLARE dfr_type = vc
 DECLARE dfr_info = vc
 DECLARE dfr_query_ret = i2
 DECLARE dfr_r_tab_flg = i4
 DECLARE dfr_l_tab_flg = i4
 DECLARE t_cnt = i4
 DECLARE dfr_rollback_ind = i2
 DECLARE parm_qual_cnt = i4 WITH protect, noconstant(0)
 DECLARE parm_col_idx = i4 WITH protect, noconstant(0)
 DECLARE parm_loop = i4 WITH protect, noconstant(0)
 DECLARE parm_reply_pos = i4 WITH protect, noconstant(0)
 DECLARE parm_reply_idx = i4 WITH protect, noconstant(0)
 DECLARE parm_pk_loop = i4 WITH protect, noconstant(0)
 DECLARE parm_pk_parm_pos = i4 WITH protect, noconstant(0)
 DECLARE parm_pk_idx = i4 WITH protect, noconstant(0)
 DECLARE dfr_long_cnt = i4 WITH protect, noconstant(0)
 DECLARE dfr_col_cnt = i4 WITH protect, noconstant(0)
 DECLARE break_pt_pos = i4 WITH protect, noconstant(0)
 DECLARE break_pt_pos2 = i4 WITH protect, noconstant(0)
 DECLARE dfr_idx7 = i4 WITH protect, noconstant(0)
 DECLARE circ_cnt = i4 WITH protect, noconstant(0)
 DECLARE dfr_lock_domain = vc WITH protect, noconstant(" ")
 DECLARE dfr_pause_ind = i2 WITH protect, noconstant(0)
 DECLARE dfr_run_backfill = i2 WITH protect, noconstant(0)
 DECLARE dfr_backfill_ret = vc WITH protect, noconstant(" ")
 SET dfr_rollback_ind = 0
 SET fr_di_appl_id = "NOT SET"
 SET fr_cur_appl_id = "NOT SET"
 SET dfr_r_tab_flg = 0
 SET dfr_l_tab_flg = 0
 SET dfr_type = request->type
 SET dfr_info = request->info
 FREE RECORD drm_table_list
 RECORD drm_table_list(
   1 qual[*]
     2 table_name = vc
 )
 FREE RECORD dfr_circ
 RECORD dfr_circ(
   1 circular_cnt = i4
   1 circ_qual[*]
     2 circ_table_name = vc
     2 circ_id_col_name = vc
     2 circ_pe_name_col = vc
     2 circular_type = i4
     2 pe_name_col = vc
     2 id_col = vc
 )
 FREE RECORD dfr_attributes
 RECORD dfr_attributes(
   1 att_cnt = i4
   1 att_qual[*]
     2 column_name = vc
     2 attribute_name = vc
 ...
 RECORD dfr_rec(
   ...
   1 env_qual[*]
     ...
     2 done2_name = vc
     2 done2_value = f8
     2 done2_exist_ind = i2
 )
 SELECT INTO "nl:"
  FROM dm_rdds_refmrg_tables rt
  DETAIL
   t_cnt = (t_cnt+ 1), stat = alterlist(drm_table_list->qual,t_cnt), drm_table_list->qual[t_cnt].
   table_name = rt.table_name
  WITH nocounter
 ;end select
 IF (dfr_type="TABLE")
  SET dfr_query_ret = dm2_rdds_get_tbl_col_info(dfr_info)
  IF (dfr_query_ret=0)
   IF ((reply->tbl_qual[temp_tbl_cnt].mergeable_ind=1))
    SELECT INTO "nl:"
     FROM user_tables ut
     WHERE (ut.table_name=reply->tbl_qual[temp_tbl_cnt].r_table_name)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET dm_err->err_ind = 1
     SET error_reply->error_flag = 1
     GO TO exit_fill
    ENDIF
    IF (curqual=0)
     ROLLBACK
     SET dfr_rollback_ind = 1
    ENDIF
    IF (create_$r_tab(dfr_info,1)="F")
     SET dm_err->eproc = "Attempting to create $R tables and triggers"
     SET dm_err->emsg = "Error attempting to create $R tables and triggers"
     SET stat = alterlist(reply->tbl_qual,(temp_tbl_cnt - 1))
     SET temp_tbl_cnt = (temp_tbl_cnt - 1)
     SET perm_tbl_cnt = (perm_tbl_cnt - 1)
     SET reply->tbl_cnt = (reply->tbl_cnt - 1)
     SET error_reply->error_flag = 1
     SET drdm_error_out_ind = 1
     GO TO exit_fill
    ENDIF
   ...
   IF (fr_di_appl_id=fr_cur_appl_id
    ...
    CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
    SET dm_err->err_ind = 0
    CALL check_concurrent_snapshot("D",currdbhandle)
   ENDIF
   SET dm_err->err_ind = 1
   SET drdm_error_out_ind = 1
   ...
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET error_reply->error_flag = 0
   GO TO exit_fill
  ELSEIF (dfr_query_ret=2)
   ROLLBACK
   SET stat = alterlist(reply->tbl_qual,(temp_tbl_cnt - 1))
   SET temp_tbl_cnt = (temp_tbl_cnt - 1)
   SET perm_tbl_cnt = (perm_tbl_cnt - 1)
   SET reply->tbl_cnt = (reply->tbl_cnt - 1)
   SET error_reply->error_flag = 1
   SET src_tab_name = dm2_get_rdds_tname("DM_CHG_LOG")
   UPDATE  FROM (parser(src_tab_name) d)
    SET log_type = "NOMV15"
    WHERE (target_env_id=reply->env_target_id)
     AND table_name=dfr_info
     AND log_type IN ("NORDDS", "REFCHG", "PROCES")
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  SELECT INTO "nl:"
   ...
   FROM dm_info di
   WHERE di.info_domain="DATA MANAGEMENT"
    AND di.info_name=concat("CUTOVER SCRIPT:",reply->tbl_qual[temp_tbl_cnt].table_name)
   DETAIL
    reply->tbl_qual[temp_tbl_cnt].custom_script = di.info_char
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  SELECT INTO "nl:"
   FROM code_value cv
   WHERE (cv.display=reply->tbl_qual[temp_tbl_cnt].table_name)
    AND cv.code_set=4000220
    AND cv.active_ind=1
   DETAIL
    drm_ioru_only = cv.code_value, drm_ioru_meaning = cv.cdf_meaning
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  IF (drm_ioru_only > 0)
   IF (drm_ioru_meaning="NONE")
    SET reply->tbl_qual[temp_tbl_cnt].insert_only_ind = 0
    SET reply->tbl_qual[temp_tbl_cnt].update_only_ind = 0
   ELSEIF (drm_ioru_meaning="INSERT_ONLY")
    SET reply->tbl_qual[temp_tbl_cnt].insert_only_ind = 1
   ELSEIF (drm_ioru_meaning="UPDATE_ONLY")
    SET reply->tbl_qual[temp_tbl_cnt].update_only_ind = 1
   ENDIF
  ELSE
   SET reply->tbl_qual[temp_tbl_cnt].insert_only_ind = 0
   SET reply->tbl_qual[temp_tbl_cnt].update_only_ind = 0
  ...
  ENDIF
  IF (curqual=1)
   SET reply->tbl_qual[temp_tbl_cnt].skip_seqmatch_ind = 1
  ELSE
   SET reply->tbl_qual[temp_tbl_cnt].skip_seqmatch_ind = 0
  ENDIF
  SET alg_code = 0
  SELECT INTO "nl:"
   FROM code_value cv
   WHERE (cv.display=reply->tbl_qual[temp_tbl_cnt].table_name)
    ...
    AND cv.active_ind=1
   DETAIL
    alg_code = cv.code_value, reply->tbl_qual[temp_tbl_cnt].version_type = cv.cdf_meaning
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->err_ind = 1
  ...
  ENDIF
  IF (alg_code > 0)
   IF ((reply->tbl_qual[temp_tbl_cnt].version_type="NONE"))
    SET reply->tbl_qual[temp_tbl_cnt].version_ind = 0
   ELSE
    SET reply->tbl_qual[temp_tbl_cnt].version_ind = 1
   ENDIF
  ELSEIF ((alg_code=- (1)))
   SET reply->tbl_qual[temp_tbl_cnt].version_ind = 0
  ENDIF
  SET dm_err->eproc = "Querying for MD Pre-scan information"
  IF ((reply->tbl_qual[temp_tbl_cnt].merge_delete_ind=1))
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain="RDDS MD PRE-SCAN TABLE"
     AND (d.info_name=reply->tbl_qual[temp_tbl_cnt].table_name)
    DETAIL
     reply->tbl_qual[temp_tbl_cnt].pre_scan_ind = 1
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  SELECT INTO "NL:"
   FROM (dummyt d  WITH seq = value(reply->tbl_qual[temp_tbl_cnt].col_cnt)),
    dm_info i
   PLAN (d
    WHERE (reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].idcd_ind=0))
    JOIN (i
    WHERE i.info_domain=concat("RDDS TRANS COLUMN:",reply->tbl_qual[temp_tbl_cnt].table_name)
     AND (i.info_name=reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name))
   DETAIL
    reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].idcd_ind = 1
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   GO TO exit_fill
  ENDIF
  SET tbl_loop = locateval(tbl_loop,1,dguc_reply->rs_tbl_cnt,dfr_info,dguc_reply->dtd_hold[tbl_loop].
   tbl_name)
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = size(reply->tbl_qual[temp_tbl_cnt].col_qual,5)),
    (dummyt d2  WITH seq = dguc_reply->dtd_hold[tbl_loop].pk_cnt)
   PLAN (d)
    JOIN (d2
    WHERE (dguc_reply->dtd_hold[tbl_loop].pk_hold[d2.seq].pk_name=reply->tbl_qual[temp_tbl_cnt].
    col_qual[d.seq].column_name))
   DETAIL
    reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].pk_ind = 1
   WITH nocounter
  ;end select
  SELECT INTO "NL:"
   FROM (dummyt d  WITH seq = value(reply->tbl_qual[temp_tbl_cnt].col_cnt)),
   ...
   PLAN (d)
    JOIN (di
    WHERE di.info_domain=concat("RDDS 7 CIRCULAR:",reply->tbl_qual[temp_tbl_cnt].table_name,":",reply
     ->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name))
   DETAIL
    break_pt_pos = findstring(":",di.info_name,1,0), break_pt_pos2 = findstring(":",di.info_name,(
     break_pt_pos+ 1),0), reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].7circ_cnt = (reply->tbl_qual[
    temp_tbl_cnt].col_qual[d.seq].7circ_cnt+ 1),
    circ_cnt = (reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].7circ_cnt+ 1), stat = alterlist(reply->
     tbl_qual[temp_tbl_cnt].col_qual[d.seq].7circ_qual,circ_cnt), reply->tbl_qual[temp_tbl_cnt].
    col_qual[d.seq].7circ_qual[circ_cnt].other_tab_name = substring(1,(break_pt_pos - 1),di.info_name
     )
    IF (break_pt_pos2=0)
     reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].7circ_qual[circ_cnt].other_id_col = substring((
      break_pt_pos+ 1),30,di.info_name), reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].7circ_qual[
     circ_cnt].other_name_col = ""
    ELSE
     reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].7circ_qual[circ_cnt].other_id_col = substring((
      break_pt_pos+ 1),((break_pt_pos2 - break_pt_pos) - 1),di.info_name), reply->tbl_qual[
     temp_tbl_cnt].col_qual[d.seq].7circ_qual[circ_cnt].other_name_col = substring((break_pt_pos2+ 1),
      30,di.info_name)
    ENDIF
    reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].7circ_qual[circ_cnt].7id_col = reply->tbl_qual[
    temp_tbl_cnt].col_qual[d.seq].column_name, reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].
    7circ_qual[circ_cnt].7name_col = di.info_char
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  SELECT INTO "NL:"
   ...
   WHERE di.info_domain="RDDS 7 CIRCULAR:*"
    AND di.info_name=patstring(concat(reply->tbl_qual[temp_tbl_cnt].table_name,":*"))
   DETAIL
    break_pt_pos = (findstring(":",di.info_name,1,0)+ 1), break_pt_pos2 = findstring(":",di.info_name,
     break_pt_pos,0)
    IF (break_pt_pos2=0)
     break_pt_pos2 = 30
    ELSE
     break_pt_pos2 = (break_pt_pos2 - break_pt_pos)
    ENDIF
    IF (locateval(dfr_idx7,1,reply->tbl_qual[temp_tbl_cnt].col_cnt,trim(substring(break_pt_pos,
       break_pt_pos2,di.info_name)),reply->tbl_qual[temp_tbl_cnt].col_qual[dfr_idx7].column_name) > 0
    )
     reply->tbl_qual[temp_tbl_cnt].other_circ_cnt = (reply->tbl_qual[temp_tbl_cnt].other_circ_cnt+ 1),
     circ_cnt = reply->tbl_qual[temp_tbl_cnt].other_circ_cnt, stat = alterlist(reply->tbl_qual[
      temp_tbl_cnt].other_circ_qual,circ_cnt),
     break_pt_pos = findstring(":",di.info_name,1,0), break_pt_pos2 = findstring(":",di.info_name,(
      break_pt_pos+ 1),0), reply->tbl_qual[temp_tbl_cnt].other_circ_qual[circ_cnt].other_id_col =
     substring(break_pt_pos,break_pt_pos2,di.info_name)
     IF (break_pt_pos2 > 0)
      reply->tbl_qual[temp_tbl_cnt].other_circ_qual[circ_cnt].other_name_col = substring((
       break_pt_pos2+ 1),30,di.info_name)
     ENDIF
     break_pt_pos = findstring(":",di.info_domain,1,0), break_pt_pos2 = findstring(":",di.info_domain,
      (break_pt_pos+ 1),0), reply->tbl_qual[temp_tbl_cnt].other_circ_qual[circ_cnt].7name_col = di
     .info_char,
     reply->tbl_qual[temp_tbl_cnt].other_circ_qual[circ_cnt].7tab_name = substring((break_pt_pos+ 1),
      ((break_pt_pos2 - break_pt_pos) - 1),di.info_domain), reply->tbl_qual[temp_tbl_cnt].
     other_circ_qual[circ_cnt].7id_col = substring((break_pt_pos2+ 1),30,di.info_domain)
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
  ...
  SELECT INTO "NL:"
   ...
   PLAN (d)
   DETAIL
    IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name="ACTIVE_IND"))
     reply->tbl_qual[temp_tbl_cnt].active_ind_ind = 1
    ENDIF
    IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name IN ("BEGIN_EFFECTIVE_DT_TM",
    "BEGIN_EFF_DT_TM", "BEG_EFFECTIVE_DT_TM", "BEG_EFFECTIVE_UTC_DT_TM", "BEG_EFF_DT_TM",
    "CNTRCT_BEG_EFF_DT_TM", "PRSNL_BEG_EFF_DT_TM")))
     reply->tbl_qual[temp_tbl_cnt].beg_col_name = reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].
     column_name
    ENDIF
    IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name IN ("END_EFFECTIVE_DT_TM",
    "PRSNL_END_EFFECTIVE_DT_TM", "END_EFFECTIVE_UTC_DT_TM", "END_EFF_DT_TM", "CNTRCT_EFF_DT_TM")))
     reply->tbl_qual[temp_tbl_cnt].end_col_name = reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].
     column_name
    ENDIF
    IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].db_data_type="*LOB"))
     IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].db_data_type_tgt="*LOB"))
      reply->tbl_qual[temp_tbl_cnt].lob_process_type = "LOB_LOB"
     ELSE
      reply->tbl_qual[temp_tbl_cnt].lob_process_type = "LOB_LONG"
     ENDIF
    ENDIF
   WITH nocounter
  ;end select
  IF (daf_is_not_blank(reply->tbl_qual[temp_tbl_cnt].beg_col_name)
   AND daf_is_not_blank(reply->tbl_qual[temp_tbl_cnt].end_col_name))
   SET reply->tbl_qual[temp_tbl_cnt].effective_col_ind = 1
  ENDIF
  SET dfr_circ->circular_cnt = 0
  SET stat = alterlist(dfr_circ->circ_qual,0)
  SET src_tab_name = dm2_get_rdds_tname("USER_TAB_COLUMNS")
  IF (daf_is_not_blank(reply->tbl_qual[temp_tbl_cnt].root_col_name))
   SELECT DISTINCT INTO "NL:"
    d.info_name
    FROM dm_info d
    WHERE d.info_domain=patstring(concat("RDDS CIRCULAR:",reply->tbl_qual[temp_tbl_cnt].table_name,
      ":*"))
    ORDER BY d.info_domain, d.info_name
    DETAIL
     dfr_circ->circular_cnt = (dfr_circ->circular_cnt+ 1), stat = alterlist(dfr_circ->circ_qual,
      dfr_circ->circular_cnt), dfr_circ->circ_qual[dfr_circ->circular_cnt].circ_table_name =
     substring(1,(findstring(":",d.info_name,1,1) - 1),d.info_name),
     dfr_circ->circ_qual[dfr_circ->circular_cnt].circ_id_col_name = substring((findstring(":",d
       .info_name,1,1)+ 1),30,d.info_name), dfr_circ->circ_qual[dfr_circ->circular_cnt].
     circ_pe_name_col = d.info_char, dfr_circ->circ_qual[dfr_circ->circular_cnt].circular_type = d
     .info_number
     IF ((dfr_circ->circ_qual[dfr_circ->circular_cnt].circular_type=2))
      dfr_circ->circ_qual[dfr_circ->circular_cnt].pe_name_col = substring((findstring(":",d
        .info_domain,15,1)+ 1),30,d.info_domain), parm_loop = locateval(parm_loop,1,reply->tbl_qual[
       temp_tbl_cnt].col_cnt,dfr_circ->circ_qual[dfr_circ->circular_cnt].pe_name_col,reply->tbl_qual[
       temp_tbl_cnt].col_qual[parm_loop].parent_entity_col)
      IF (parm_loop > 0)
       dfr_circ->circ_qual[dfr_circ->circular_cnt].id_col = reply->tbl_qual[temp_tbl_cnt].col_qual[
       parm_loop].column_name
      ELSE
       dm_err->err_ind = 1, error_reply->error_flag = 1, dm_err->emsg = concat(
        "No ID column found with PARENT_ENTITY_COL of ",dfr_circ->circ_qual[dfr_circ->circular_cnt].
        pe_name_col)
      ENDIF
     ELSE
      dfr_circ->circ_qual[dfr_circ->circular_cnt].id_col = substring((findstring(":",d.info_domain,15,
        1)+ 1),30,d.info_domain)
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   ENDIF
  ENDIF
  IF ((dfr_circ->circular_cnt > 0))
   SELECT INTO "NL:"
    FROM (parser(src_tab_name) u),
     (dummyt d  WITH seq = dfr_circ->circular_cnt)
    PLAN (d)
     JOIN (u
     WHERE (u.table_name=dfr_circ->circ_qual[d.seq].circ_table_name)
      AND (u.column_name=dfr_circ->circ_qual[d.seq].circ_id_col_name))
    DETAIL
     reply->tbl_qual[temp_tbl_cnt].circular_cnt = (reply->tbl_qual[temp_tbl_cnt].circular_cnt+ 1),
     stat = alterlist(reply->tbl_qual[temp_tbl_cnt].circ_qual,reply->tbl_qual[temp_tbl_cnt].
      circular_cnt), reply->tbl_qual[temp_tbl_cnt].circ_qual[reply->tbl_qual[temp_tbl_cnt].
     circular_cnt].circ_table_name = dfr_circ->circ_qual[d.seq].circ_table_name,
     reply->tbl_qual[temp_tbl_cnt].circ_qual[reply->tbl_qual[temp_tbl_cnt].circular_cnt].
     circ_id_col_name = dfr_circ->circ_qual[d.seq].circ_id_col_name, reply->tbl_qual[temp_tbl_cnt].
     circ_qual[reply->tbl_qual[temp_tbl_cnt].circular_cnt].circ_pe_name_col = dfr_circ->circ_qual[d
     .seq].circ_pe_name_col, reply->tbl_qual[temp_tbl_cnt].circ_qual[reply->tbl_qual[temp_tbl_cnt].
     circular_cnt].circular_type = dfr_circ->circ_qual[d.seq].circular_type,
     reply->tbl_qual[temp_tbl_cnt].circ_qual[reply->tbl_qual[temp_tbl_cnt].circular_cnt].pe_name_col
      = dfr_circ->circ_qual[d.seq].pe_name_col, reply->tbl_qual[temp_tbl_cnt].circ_qual[reply->
     tbl_qual[temp_tbl_cnt].circular_cnt].id_col = dfr_circ->circ_qual[d.seq].id_col
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
  ENDIF
  FOR (dfr_col_cnt = 1 TO reply->tbl_qual[temp_tbl_cnt].col_cnt)
    IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[dfr_col_cnt].binary_long_ind=1))
     SET dfr_long_cnt = 0
     SELECT INTO "NL:"
      FROM dm_info di
      WHERE di.info_domain IN ("RDDS LONG XML CHECK", "RDDS LONG JSON CHECK")
       AND di.info_name=patstring(concat(reply->tbl_qual[temp_tbl_cnt].table_name,":",reply->
        tbl_qual[temp_tbl_cnt].col_qual[dfr_col_cnt].column_name,":*"))
      DETAIL
       reply->tbl_qual[temp_tbl_cnt].col_qual[dfr_col_cnt].idcd_ind = 1, dfr_long_cnt = (dfr_long_cnt
       + 1)
       IF (mod(dfr_long_cnt,10)=1)
        stat = alterlist(reply->tbl_qual[temp_tbl_cnt].col_qual[dfr_col_cnt].long_encoded_qual,(
         dfr_long_cnt+ 9))
       ENDIF
       reply->tbl_qual[temp_tbl_cnt].col_qual[dfr_col_cnt].long_encoded_qual[dfr_long_cnt].check_str
        = di.info_char
       IF (di.info_domain="RDDS LONG XML CHECK")
        reply->tbl_qual[temp_tbl_cnt].col_qual[dfr_col_cnt].long_encoded_qual[dfr_long_cnt].
        encoded_type = "XML"
       ELSE
        reply->tbl_qual[temp_tbl_cnt].col_qual[dfr_col_cnt].long_encoded_qual[dfr_long_cnt].
        encoded_type = "JSON"
      ...
      FOOT REPORT
       stat = alterlist(reply->tbl_qual[temp_tbl_cnt].col_qual[dfr_col_cnt].long_encoded_qual,
        dfr_long_cnt), reply->tbl_qual[temp_tbl_cnt].col_qual[dfr_col_cnt].long_encoded_cnt =
       dfr_long_cnt
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
    ...
    ENDIF
  ENDFOR
  FOR (fr_loop = 1 TO reply->tbl_qual[temp_tbl_cnt].col_cnt)
    IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[fr_loop].exception_flg=9))
     SELECT INTO "NL:"
      FROM dm_refchg_version_r d
      WHERE (child_table=reply->tbl_qual[temp_tbl_cnt].table_name)
       AND (child_vers_col=reply->tbl_qual[temp_tbl_cnt].col_qual[fr_loop].column_name)
      DETAIL
       reply->tbl_qual[temp_tbl_cnt].col_qual[fr_loop].version_nbr_child_ind = 1, reply->tbl_qual[
       temp_tbl_cnt].col_qual[fr_loop].parent_table = d.parent_table, reply->tbl_qual[temp_tbl_cnt].
       col_qual[fr_loop].parent_pk_col = d.parent_id_col,
       reply->tbl_qual[temp_tbl_cnt].col_qual[fr_loop].parent_vers_col = d.parent_vers_col, reply->
       tbl_qual[temp_tbl_cnt].col_qual[fr_loop].child_fk_col = d.child_id_col
      WITH nocounter
     ;end select
     IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[fr_loop].version_nbr_child_ind=0)
      AND (reply->tbl_qual[temp_tbl_cnt].version_ind=0))
      SET dm_err->err_ind = 1
      SET drdm_error_out_ind = 1
      SET dm_err->eproc = "Retrieving schema information"
  ...
  SELECT INTO "nl:"
   ...
   WHERE (d.table_name=reply->tbl_qual[temp_tbl_cnt].table_name)
    AND d.active_ind=1
    AND d.mover_string != ""
    AND d.mover_string IS NOT null
   DETAIL
    reply->tbl_qual[temp_tbl_cnt].filter_string = d.mover_string
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  IF ((reply->tbl_qual[temp_tbl_cnt].merge_delete_ind=1))
   SELECT INTO "nl:"
    FROM code_value c
    WHERE c.code_set=4002213
     AND (c.display=reply->tbl_qual[temp_tbl_cnt].table_name)
     AND c.cdf_meaning="MDTRG"
     AND c.active_ind=1
    DETAIL
     reply->tbl_qual[temp_tbl_cnt].check_dual_build_ind = 1
    WITH nocounter
   ;end select
  ENDIF
  SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].child_flag = 1
  SET dm_err->eproc = "Querying for current state information."
  SELECT INTO "nl:"
   d.info_name, d.info_char
   FROM dm_info d
   WHERE d.info_domain="RDDS CURRENT STATE TABLES"
    AND d.info_name=patstring(concat(reply->tbl_qual[temp_tbl_cnt].table_name,":*"))
   DETAIL
    IF (daf_is_not_blank(d.info_char))
     reply->tbl_qual[temp_tbl_cnt].cur_state_flag = 2
    ELSE
     reply->tbl_qual[temp_tbl_cnt].cur_state_flag = 1
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_fill
  ENDIF
  SET dm_err->eproc = "Querying for pk_where and ptam_match hash values"
  SELECT INTO "nl:"
   d.pk_where_hash, d.ptam_match_hash
   FROM dm_pk_where d
   WHERE (d.table_name=reply->tbl_qual[temp_tbl_cnt].table_name)
   DETAIL
    reply->tbl_qual[temp_tbl_cnt].ptam_match_hash = d.ptam_match_hash
    IF (d.delete_ind=0)
     reply->tbl_qual[temp_tbl_cnt].pk_where_hash = d.pk_where_hash
    ELSE
     reply->tbl_qual[temp_tbl_cnt].del_pk_where_hash = d.pk_where_hash
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
  ...
  SELECT INTO "nl:"
   FROM dm_refchg_sql_obj drso,
    (dummyt d  WITH seq = value(size(reply->tbl_qual[temp_tbl_cnt].col_qual,5)))
   PLAN (d)
    JOIN (drso
    WHERE (drso.table_name=reply->tbl_qual[temp_tbl_cnt].table_name)
     AND (drso.column_name=reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name)
     AND drso.active_ind=1)
   DETAIL
    reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].execution_flag = drso.execution_flag, reply->
    tbl_qual[temp_tbl_cnt].col_qual[d.seq].object_name = replace(drso.object_name,
     "::SOURCE TARGET MAPPING::",cnvtstring(global_mover_rec->xlat_funct_nbr,20,0),1), reply->
    tbl_qual[temp_tbl_cnt].col_qual[d.seq].masked_object_name = drso.object_name
    IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].data_type != "VC"))
     reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].ccl_data_type = reply->tbl_qual[temp_tbl_cnt].
     col_qual[d.seq].data_type
    ELSE
     reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].ccl_data_type = build("C",reply->tbl_qual[
      temp_tbl_cnt].col_qual[d.seq].data_length)
    ENDIF
    reply->tbl_qual[temp_tbl_cnt].col_qual[d.seq].idcd_ind = 1
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_fill
  ENDIF
  FOR (parm_col_idx = 1 TO reply->tbl_qual[temp_tbl_cnt].col_cnt)
    IF (daf_is_not_blank(reply->tbl_qual[temp_tbl_cnt].col_qual[parm_col_idx].object_name))
     SELECT INTO "nl:"
      FROM dm_refchg_sql_obj_parm rsop
      WHERE (rsop.object_name=reply->tbl_qual[temp_tbl_cnt].col_qual[parm_col_idx].masked_object_name
      )
      ORDER BY rsop.parm_nbr
      HEAD REPORT
       parm_qual_cnt = 0
      DETAIL
       parm_qual_cnt = (parm_qual_cnt+ 1), stat = alterlist(reply->tbl_qual[temp_tbl_cnt].col_qual[
        parm_col_idx].parm_list,parm_qual_cnt), reply->tbl_qual[temp_tbl_cnt].col_qual[parm_col_idx].
       parm_list[parm_qual_cnt].column_name = rsop.column_name
      FOOT REPORT
       reply->tbl_qual[temp_tbl_cnt].col_qual[parm_col_idx].parm_cnt = parm_qual_cnt
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      ...
      GO TO exit_fill
     ENDIF
     IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[parm_col_idx].pk_ind=1)
      AND (reply->tbl_qual[temp_tbl_cnt].col_qual[parm_col_idx].execution_flag=3)
      AND (reply->tbl_qual[temp_tbl_cnt].table_name=reply->tbl_qual[temp_tbl_cnt].col_qual[
     parm_col_idx].root_entity_name)
      AND (reply->tbl_qual[temp_tbl_cnt].col_qual[parm_col_idx].column_name=reply->tbl_qual[
     temp_tbl_cnt].col_qual[parm_col_idx].root_entity_attr))
      FOR (parm_pk_loop = 1 TO reply->tbl_qual[temp_tbl_cnt].col_qual[parm_col_idx].parm_cnt)
       SET parm_pk_parm_pos = locateval(parm_pk_idx,1,reply->tbl_qual[temp_tbl_cnt].col_cnt,reply->
        tbl_qual[temp_tbl_cnt].col_qual[parm_col_idx].parm_list[parm_pk_loop].column_name,reply->
        tbl_qual[temp_tbl_cnt].col_qual[parm_pk_idx].column_name)
       IF (parm_pk_parm_pos > 0)
        IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[parm_pk_parm_pos].column_name != reply->tbl_qual[
        temp_tbl_cnt].col_qual[parm_col_idx].column_name))
         SET reply->tbl_qual[temp_tbl_cnt].col_qual[parm_pk_parm_pos].pk_required_ind = 1
        ENDIF
       ELSE
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        SET dm_err->err_ind = 1
      ...
      ENDFOR
     ENDIF
     IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[parm_col_idx].in_tgt_flag=0))
      EXECUTE dm_rmc_create_sql_object reply->tbl_qual[temp_tbl_cnt].col_qual[parm_col_idx].
      masked_object_name, reply->env_source_id
     ELSE
      FOR (parm_loop = 1 TO parm_qual_cnt)
       SET parm_reply_pos = locateval(parm_reply_idx,1,reply->tbl_qual[temp_tbl_cnt].col_cnt,reply->
        tbl_qual[temp_tbl_cnt].col_qual[parm_col_idx].parm_list[parm_loop].column_name,reply->
        tbl_qual[temp_tbl_cnt].col_qual[parm_reply_idx].column_name)
       IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[parm_reply_pos].in_tgt_flag=0))
        EXECUTE dm_rmc_create_sql_object reply->tbl_qual[temp_tbl_cnt].col_qual[parm_col_idx].
        masked_object_name, reply->env_source_id
        SET parm_loop = parm_qual_cnt
       ENDIF
      ENDFOR
     ENDIF
    ENDIF
  ...
  SELECT INTO "NL:"
   ...
   WHERE (d.table_name=reply->tbl_qual[temp_tbl_cnt].table_name)
   DETAIL
    dfr_attributes->att_cnt = (dfr_attributes->att_cnt+ 1), stat = alterlist(dfr_attributes->att_qual,
     dfr_attributes->att_cnt), dfr_attributes->att_qual[dfr_attributes->att_cnt].column_name = d
    .column_name,
    dfr_attributes->att_qual[dfr_attributes->att_cnt].attribute_name = d.attribute_name,
    dfr_attributes->att_qual[dfr_attributes->att_cnt].attribute_value = d.attribute_value,
    dfr_attributes->att_qual[dfr_attributes->att_cnt].attribute_char = d.attribute_char,
    dfr_attributes->att_qual[dfr_attributes->att_cnt].attribute_dt_tm = d.attribute_dt_tm
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   GO TO exit_fill
  ENDIF
  FOR (parm_col_idx = 1 TO dfr_attributes->att_cnt)
    IF (daf_is_not_blank(dfr_attributes->att_qual[parm_col_idx].column_name))
     IF ((dfr_attributes->att_qual[parm_col_idx].attribute_name="BEG_EFFECTIVE COLUMN_NAME_IND")
      AND (dfr_attributes->att_qual[parm_col_idx].attribute_value=1))
      SET reply->tbl_qual[temp_tbl_cnt].beg_col_name = dfr_attributes->att_qual[parm_col_idx].
      column_name
     ELSEIF ((dfr_attributes->att_qual[parm_col_idx].attribute_name="END_EFFECTIVE COLUMN_NAME_IND")
      AND (dfr_attributes->att_qual[parm_col_idx].attribute_value=1))
      SET reply->tbl_qual[temp_tbl_cnt].end_col_name = dfr_attributes->att_qual[parm_col_idx].
      column_name
     ELSE
      SET parm_loop = locateval(parm_loop,1,reply->tbl_qual[temp_tbl_cnt].col_cnt,dfr_attributes->
       att_qual[parm_col_idx].column_name,reply->tbl_qual[temp_tbl_cnt].col_qual[parm_loop].
       column_name)
      IF (parm_loop > 0)
       IF ((dfr_attributes->att_qual[parm_col_idx].attribute_name="BASE62 NUMERIC VALUE"))
        SET reply->tbl_qual[temp_tbl_cnt].col_qual[parm_loop].base62_re_name = dfr_attributes->
        att_qual[parm_col_idx].attribute_char
       ENDIF
      ENDIF
     ENDIF
    ELSE
     IF ((dfr_attributes->att_qual[parm_col_idx].attribute_name="USER GENERATED DATE INDICATOR")
      AND (dfr_attributes->att_qual[parm_col_idx].attribute_value=1))
      SET reply->tbl_qual[temp_tbl_cnt].user_gen_date_ind = 1
     ENDIF
    ENDIF
  ENDFOR
  IF (daf_is_not_blank(reply->tbl_qual[temp_tbl_cnt].beg_col_name)
   ...
   SET reply->tbl_qual[temp_tbl_cnt].effective_col_ind = 1
  ENDIF
  CALL dm2_rdds_record_plsql_metadata(reply,temp_tbl_cnt)
 ELSE
  SET dm_err->eproc = "Filling code set info into RS"
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT INTO "NL:"
   FROM code_value_set cvs
   WHERE cvs.code_set=cnvtint(dfr_info)
   DETAIL
    perm_cs_cnt = (perm_cs_cnt+ 1), temp_cs_cnt = perm_cs_cnt, stat = alterlist(reply->cs_qual,
     temp_cs_cnt),
    reply->cs_qual[temp_cs_cnt].code_set = cvs.code_set, reply->cs_qual[temp_cs_cnt].
    cdf_meaning_dup_ind = cvs.cdf_meaning_dup_ind, reply->cs_qual[temp_cs_cnt].display_dup_ind = cvs
    .display_dup_ind,
    reply->cs_qual[temp_cs_cnt].display_key_dup_ind = cvs.display_key_dup_ind, reply->cs_qual[
    temp_cs_cnt].active_ind_dup_ind = cvs.active_ind_dup_ind, reply->cs_qual[temp_cs_cnt].
    definition_dup_ind = cvs.definition_dup_ind
   FOOT REPORT
    reply->cs_cnt = temp_cs_cnt
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  SELECT INTO "NL:"
   ...
   WHERE code_set=cnvtint(dfr_info)
   DETAIL
    reply->cs_qual[temp_cs_cnt].merge_ui_query = dcs.merge_ui_query
    IF (dcs.merge_ui_query=null)
     reply->cs_qual[temp_cs_cnt].merge_ui_query_ni = 1
    ELSE
     reply->cs_qual[temp_cs_cnt].merge_ui_query_ni = 0
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
  ...
  ENDIF
  IF (curqual=0)
   SET reply->cs_qual[temp_cs_cnt].merge_ui_query_ni = 1
  ENDIF
  IF (cnvtint(dfr_info)=93)
   SELECT INTO "nl"
    FROM dm_info di
    WHERE di.info_domain=concat("MERGE",trim(cnvtstring(reply->env_source_id,20)),trim(cnvtstring(
       reply->mock_target_id,20)),"SEQMATCHDONE2")
     AND di.info_name="CODESET93"
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   IF (curqual=0)
    SELECT INTO "nl"
     FROM dm_info di
     WHERE di.info_domain=concat("MERGE",trim(cnvtstring(reply->env_source_id,20)),trim(cnvtstring(
        reply->mock_target_id,20)),"SEQMATCH")
      AND di.info_name="CODESET93"
     DETAIL
      dfr_run_backfill = 1
     WITH nocounter
    ...
    IF (check_error(dm_err->eproc)=1)
     ...
     GO TO exit_fill
    ENDIF
    IF (dfr_run_backfill=0)
     SET dfr_backfill_ret = cs93_create_seqmatch(dfr_rec)
     IF (dfr_backfill_ret="F")
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET dm_err->err_ind = 1
      SET error_reply->error_flag = 1
      GO TO exit_fill
     ENDIF
     SET dfr_backfill_ret = " "
     SELECT INTO "nl"
      FROM dm_info di
      WHERE di.info_domain=concat("MERGE",trim(cnvtstring(reply->env_source_id,20)),trim(cnvtstring(
         reply->mock_target_id,20)),"SEQMATCH")
     ...
     ENDIF
    ENDIF
    IF (dfr_run_backfill=1)
     SET dfr_lock_domain = concat("XLAT BACKFILL",trim(cnvtstring(reply->env_source_id,20)),trim(
       cnvtstring(reply->mock_target_id,20)))
     WHILE (dfr_pause_ind=1)
       SET dfr_pause_ind = 0
       SET dfr_run_backfill = 0
       SET drl_reply->status = ""
       SET drl_reply->status_msg = ""
       CALL get_lock(dfr_lock_domain,"CODESET93",0,drl_reply)
       IF ((drl_reply->status="F"))
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        SET dm_err->err_ind = 1
        SET error_reply->error_flag = 1
       ...
       IF (dfr_pause_ind=1)
        ...
        SELECT INTO "nl"
         FROM dm_info di
         WHERE di.info_domain=concat("MERGE",trim(cnvtstring(reply->env_source_id,20)),trim(
           cnvtstring(reply->mock_target_id,20)),"SEQMATCHDONE2")
          AND di.info_name="CODESET93"
         DETAIL
          dfr_pause_ind = 0
         WITH nocounter
     ...
     ENDWHILE
     IF (dfr_run_backfill=1)
      SET dfr_backfill_ret = cs93_xlat_backfill(reply->env_source_id)
      SET drl_reply->status = ""
      SET drl_reply->status_msg = ""
      CALL remove_lock(cb_info_domain,cb_seq_name,currdbhandle,drl_reply)
      COMMIT
 ...
 SUBROUTINE dm2_rdds_get_tbl_col_info(sbr_gtci_tname)
   ...
   DECLARE rrt_pos = i4
   DECLARE rrt_num = i4
   DECLARE drgtci_retry_cnt = i2 WITH protect, noconstant(0)
   DECLARE drgtci_idx = i2 WITH protect, noconstant(0)
   DECLARE drgtci_r_col_cnt = i2 WITH protect, noconstant(0)
   ...
   SET src_tab_name = dm2_get_rdds_tname("USER_TAB_COLS")
   SELECT INTO "NL:"
    FROM dm_tables_doc_local drt
    WHERE drt.table_name=sbr_gtci_tname
    DETAIL
     temp_tbl_cnt = (perm_tbl_cnt+ 1), perm_tbl_cnt = (perm_tbl_cnt+ 1), stat = alterlist(reply->
      tbl_qual,value((size(reply->tbl_qual,5)+ 1))),
     reply->tbl_qual[temp_tbl_cnt].r_table_name = cutover_tab_name(drt.table_name,drt.table_suffix),
     reply->tbl_qual[temp_tbl_cnt].table_name = drt.table_name, rrt_pos = locateval(rrt_num,1,size(
       drm_table_list->qual,5),reply->tbl_qual[temp_tbl_cnt].table_name,drm_table_list->qual[rrt_num]
      .table_name)
     IF (rrt_pos > 0)
      reply->tbl_qual[temp_tbl_cnt].mergeable_ind = drt.mergeable_ind, reply->tbl_qual[temp_tbl_cnt].
      reference_ind = 1
     ELSE
      reply->tbl_qual[temp_tbl_cnt].mergeable_ind = drt.mergeable_ind, reply->tbl_qual[temp_tbl_cnt].
      reference_ind = drt.reference_ind
     ENDIF
     reply->tbl_qual[temp_tbl_cnt].merge_delete_ind = drt.merge_delete_ind, reply->tbl_qual[
     temp_tbl_cnt].suffix = drt.table_suffix, reply->tbl_qual[temp_tbl_cnt].merge_ui_query = drt
     .merge_ui_query
     IF (drt.merge_ui_query=null)
      reply->tbl_qual[temp_tbl_cnt].merge_ui_query_ni = 1
     ELSE
      reply->tbl_qual[temp_tbl_cnt].merge_ui_query_ni = 0
     ENDIF
    FOOT REPORT
     stat = alterlist(reply->tbl_qual,temp_tbl_cnt), reply->tbl_cnt = temp_tbl_cnt
    WITH nocounter
   ;end select
   IF (curqual=0)
    IF (check_error(dm_err->eproc)=1)
   ...
   SELECT INTO "nl:"
    FROM dm_info
    WHERE info_name=sbr_gtci_tname
     AND info_domain="RDDS OVERRULE NOMV06"
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual=0)
    SET reply->tbl_qual[temp_tbl_cnt].overrule_nomv06_ind = 0
   ELSE
    SET reply->tbl_qual[temp_tbl_cnt].overrule_nomv06_ind = 1
   ENDIF
   ...
   SELECT INTO "NL:"
    ...
    ORDER BY drt.column_name
    DETAIL
     col_qual_cnt = (col_qual_cnt+ 1)
     IF (mod(col_qual_cnt,10)=1)
      stat = alterlist(reply->tbl_qual[temp_tbl_cnt].col_qual,(col_qual_cnt+ 9))
     ENDIF
     reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].column_name = drt.column_name, reply->
     tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].unique_ident_ind = drt.unique_ident_ind, reply->
     tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].exception_flg = drt.exception_flg,
     reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].constant_value = drt.constant_value, reply
     ->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].parent_entity_col = cnvtupper(drt
      .parent_entity_col), reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].sequence_name = drt
     .sequence_name,
     reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].root_entity_name = cnvtupper(drt
      .root_entity_name), reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].root_entity_attr =
     cnvtupper(drt.root_entity_attr), reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].code_set
      = drt.code_set,
     reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].merge_delete_ind = drt.merge_delete_ind,
     reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].defining_attribute_ind = drt
     .defining_attribute_ind
     IF (drt.column_name IN ("*_ID", "*_CD", "CODE_VALUE")
      AND utc.data_type IN ("NUMBER", "FLOAT"))
      reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].idcd_ind = 1
     ELSE
      reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].idcd_ind = 0
     ENDIF
     IF (drt.table_name=drt.root_entity_name
      AND drt.column_name=drt.root_entity_attr)
      reply->tbl_qual[temp_tbl_cnt].root_col_name = drt.column_name
     ENDIF
     reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].db_data_type = utc.data_type, reply->
     tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].db_data_length = utc.data_length, reply->tbl_qual[
     temp_tbl_cnt].col_qual[col_qual_cnt].nullable = utc.nullable,
     reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].check_null = 0, reply->tbl_qual[
     temp_tbl_cnt].col_qual[col_qual_cnt].translated = 0, reply->tbl_qual[temp_tbl_cnt].col_qual[
     col_qual_cnt].data_default = utc.data_default,
     reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].data_default_ni = udd
     IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].data_default_ni=0))
      IF (((cnvtupper(utc.data_default)="NULL") OR (((daf_is_blank(utc.data_default)) OR (((utc
      .data_default="''") OR (utc.data_default='""')) )) )) )
       reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].data_default_ni = 1
      ENDIF
     ENDIF
     IF (utc.data_type IN ("BLOB", "LONG RAW", "CLOB", "LONG"))
      reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].binary_long_ind = 1, reply->tbl_qual[
      temp_tbl_cnt].long_ind = 1
     ELSE
      reply->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].binary_long_ind = 0
     ENDIF
    FOOT REPORT
     stat = alterlist(reply->tbl_qual[temp_tbl_cnt].col_qual,col_qual_cnt), reply->tbl_qual[
     temp_tbl_cnt].col_cnt = col_qual_cnt
    WITH nocounter
   ;end select
   IF (curqual=0)
    IF (check_error(dm_err->eproc)=1)
   ...
   SELECT INTO "nl:"
    ...
    WHERE d.table_name=sbr_gtci_tname
    DETAIL
     rrt_pos = locateval(rrt_num,1,reply->tbl_qual[temp_tbl_cnt].col_cnt,d.column_name,reply->
      tbl_qual[temp_tbl_cnt].col_qual[rrt_num].column_name)
     IF (rrt_pos > 0)
      reply->tbl_qual[temp_tbl_cnt].col_qual[rrt_pos].nullable = "N"
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   SELECT INTO "nl:"
    FROM user_tables ut
    WHERE ut.table_name=sbr_gtci_tname
    WITH nocounter
   ;end select
   IF (curqual=0)
    RETURN(2)
   ENDIF
   IF (dm2_rdds_col_compare(temp_tbl_cnt,0)=0)
    RETURN(0)
   ENDIF
   IF ((dm2_rdds_curdb_schema->same_count != reply->tbl_qual[temp_tbl_cnt].col_cnt))
    ...
    ENDIF
    IF ((inhouse_misc->inhouse_domain=1)
     AND sbr_gtci_tname != "RDDS_ALG5_TAB")
     SET dm_err->emsg = concat(
     ...
     RETURN(0)
    ENDIF
    IF (check_concurrent_snapshot("I",currdbhandle)=0)
     RETURN(0)
    ENDIF
    IF (dm2_rdds_col_compare(temp_tbl_cnt,0)=0)
     RETURN(0)
    ...
    FOR (rcc_lp = 1 TO reply->tbl_qual[temp_tbl_cnt].col_cnt)
      IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].in_tgt_flag != 1))
       SET dm_err->eproc = concat("Schema diff found for ",sbr_gtci_tname,".",reply->tbl_qual[
        temp_tbl_cnt].col_qual[rcc_lp].column_name)
       CALL disp_msg("",dm_err->logfile,0)
       IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].db_data_type IN ("LONG", "LONG RAW",
       "CLOB", "BLOB")))
       ...
       IF ((dm2_rdds_curdb_schema->ddl_exist_flag="F")
        AND (reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].in_tgt_flag=0))
        ...
        SELECT INTO "nl:"
         ...
         WHERE (d.table_name=reply->tbl_qual[temp_tbl_cnt].table_name)
          AND (d.obj_name=reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].column_name)
        ...
        IF ((dm2_rdds_curdb_schema->appl_id != "NOT SET"))
         IF (dm2_get_appl_status(dm2_rdds_curdb_schema->appl_id) != "I")
          IF ((dm_err->err_ind=0))
           SET dm_err->eproc = concat("Another process is getting ready to add column ",reply->
            tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].column_name," to ",reply->tbl_qual[temp_tbl_cnt].
            table_name)
        ...
        ENDIF
       ENDIF
       IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].in_tgt_flag=0))
        ROLLBACK
        SET dfr_rollback_ind = 1
        IF (dm2_rdds_col_add(temp_tbl_cnt,rcc_lp,0)=0)
         RETURN(0)
        ENDIF
        IF (drm_log_sch_chg("RDDS SCHEMA MAINTENANCE:ADD COLUMN",build(reply->tbl_qual[temp_tbl_cnt].
          table_name,".",reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].column_name),concat("TYPE= ",
          reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].db_data_type,"; DEFAULT=",substring(1,200,
           reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].data_default)))=0)
         RETURN(0)
        ENDIF
       ELSEIF ((reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].in_tgt_flag=2))
        ROLLBACK
        SET dfr_rollback_ind = 1
        IF (dm2_rdds_col_extend(temp_tbl_cnt,rcc_lp,0)=0)
         RETURN(0)
        ENDIF
        IF (drm_log_sch_chg(build("RDDS SCHEMA MAINTENANCE:EXTEND DATA LENGTH (",reply->tbl_qual[
          temp_tbl_cnt].col_qual[rcc_lp].db_data_length,")"),build(reply->tbl_qual[temp_tbl_cnt].
          table_name,".",reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].column_name),concat("TYPE= ",
          reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].db_data_type))=0)
         RETURN(0)
        ENDIF
       ENDIF
      ENDIF
    ...
    EXECUTE oragen3 reply->tbl_qual[temp_tbl_cnt].table_name
    IF (dfr_l_tab_flg=1)
     EXECUTE dm_create_rmc_triggers reply->tbl_qual[temp_tbl_cnt].table_name
    ENDIF
    IF ((dm_err->err_ind=1))
     RETURN(0)
    ENDIF
    ...
    IF ((dm2_rdds_curdb_schema->same_count != reply->tbl_qual[temp_tbl_cnt].col_cnt))
     ...
     RETURN(0)
    ENDIF
    IF (check_concurrent_snapshot("D",currdbhandle)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF (fill_ccl_data_info(temp_tbl_cnt)=0)
    RETURN(0)
   ENDIF
   IF ((dm2_rdds_curdb_schema->ccl_same_cnt != reply->tbl_qual[temp_tbl_cnt].col_cnt))
    SET oragen3_ignore_dm_columns_doc = 1
    EXECUTE oragen3 reply->tbl_qual[temp_tbl_cnt].table_name
    IF (dfr_l_tab_flg=1)
     EXECUTE dm_create_rmc_triggers reply->tbl_qual[temp_tbl_cnt].table_name
    ...
    IF ((dm2_rdds_curdb_schema->ccl_same_cnt != reply->tbl_qual[temp_tbl_cnt].col_cnt))
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("Cannot find all columns for table ",reply->tbl_qual[temp_tbl_cnt].
      table_name," in CCL dictionary after performing oragen3")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
   ...
   IF (curqual=0)
    RETURN(1)
   ENDIF
   IF (dm2_rdds_col_compare(temp_tbl_cnt,1)=0)
    RETURN(0)
   ENDIF
   IF ((dm2_rdds_curdb_schema->same_count != reply->tbl_qual[temp_tbl_cnt].col_cnt))
    SET dm_err->eproc = "Schema differences exist between target and source domains"
    ...
    SET dm_err->eproc = "Getting ready to make the schema changes"
    FOR (rcc_lp = 1 TO reply->tbl_qual[temp_tbl_cnt].col_cnt)
      IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].r_tgt_flag != 1))
       SET dm_err->eproc = concat("Schema diff found for ",reply->tbl_qual[temp_tbl_cnt].r_table_name,
        ".",reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].column_name)
       CALL disp_msg("",dm_err->logfile,0)
       IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].db_data_type IN ("LONG", "LONG RAW",
       "CLOB", "BLOB")))
        SET dm_err->emsg = "The missing column has LONG, LONG RAW, CLOB, or BLOB data type"
       ...
       ENDIF
       IF ((dm2_rdds_curdb_schema->ddl_exist_flag="F")
        AND (reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].r_tgt_flag=0))
        SET dm_err->eproc = "Checking if another process is getting ready to make the same change"
        CALL disp_msg("",dm_err->logfile,0)
        SET dm2_rdds_curdb_schema->appl_id = "NOT SET"
        SELECT INTO "nl:"
         d.*
         FROM dm2_ddl_ops_log d
         WHERE (d.table_name=reply->tbl_qual[temp_tbl_cnt].r_table_name)
          AND (d.obj_name=reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].column_name)
          AND d.op_type="ADD COLUMN"
         DETAIL
          dm2_rdds_curdb_schema->appl_id = d.appl_id
        ...
        ENDIF
       ENDIF
       IF ((reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].r_tgt_flag=0))
        ROLLBACK
        SET dfr_rollback_ind = 1
        IF (dm2_rdds_col_add(temp_tbl_cnt,rcc_lp,1)=0)
         RETURN(0)
        ENDIF
        IF (drm_log_sch_chg("RDDS SCHEMA MAINTENANCE:ADD COLUMN",build(reply->tbl_qual[temp_tbl_cnt].
          r_table_name,".",reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].column_name),concat(
          "TYPE= ",reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].db_data_type,"; DEFAULT=",substring
          (1,200,reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].data_default)))=0)
         RETURN(0)
        ENDIF
       ELSEIF ((reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].r_tgt_flag=2))
        ROLLBACK
        SET dfr_rollback_ind = 1
        IF (dm2_rdds_col_extend(temp_tbl_cnt,rcc_lp,1)=0)
         RETURN(0)
        ENDIF
        IF (drm_log_sch_chg(build("RDDS SCHEMA MAINTENANCE:EXTEND DATA LENGTH (",reply->tbl_qual[
          temp_tbl_cnt].col_qual[rcc_lp].db_data_length,")"),build(reply->tbl_qual[temp_tbl_cnt].
          r_table_name,".",reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].column_name),concat(
          "TYPE= ",reply->tbl_qual[temp_tbl_cnt].col_qual[rcc_lp].db_data_type))=0)
         RETURN(0)
        ENDIF
       ENDIF
      ENDIF
    ENDFOR
    FOR (drgtci_retry_cnt = 1 TO 3)
      SET drl_reply->status = ""
      SET drl_reply->status_msg = ""
      CALL get_lock("RDDS $R CREATION",reply->tbl_qual[temp_tbl_cnt].r_table_name,0,drl_reply)
      IF ((drl_reply->status="F"))
       CALL disp_msg(drl_reply->status_msg,dm_err->logfile,1)
       SET dm_err->err_ind = 1
       RETURN(0)
      ELSEIF ((drl_reply->status="S"))
       SET oragen3_ignore_dm_columns_doc = 1
       EXECUTE oragen3 reply->tbl_qual[temp_tbl_cnt].r_table_name
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg("",dm_err->logfile,1)
        RETURN(0)
       ENDIF
       SET drl_reply->status = ""
       SET drl_reply->status_msg = ""
       CALL remove_lock("RDDS $R CREATION",reply->tbl_qual[temp_tbl_cnt].r_table_name,currdbhandle,
        drl_reply)
      ELSE
       CALL pause(10)
       SELECT INTO "nl:"
        FROM dtableattr a,
         dtableattrl l
        WHERE (a.table_name=reply->tbl_qual[temp_tbl_cnt].r_table_name)
         AND l.structtype="F"
        DETAIL
         drgtci_idx = 0, drgtci_idx = locateval(drgtci_idx,1,reply->tbl_qual[temp_tbl_cnt].col_cnt,l
          .attr_name,reply->tbl_qual[temp_tbl_cnt].col_qual[drgtci_idx].column_name)
         IF (drgtci_idx > 0)
          drgtci_r_col_cnt = (drgtci_r_col_cnt+ 1)
         ENDIF
        WITH nocounter
       ;end select
       IF ((drgtci_r_col_cnt=reply->tbl_qual[temp_tbl_cnt].col_cnt))
        SET drgtci_retry_cnt = 3
       ELSE
        IF (drgtci_retry_cnt=3)
         CALL disp_msg(drl_reply->status_msg,dm_err->logfile,1)
         SET dm_err->emsg = drl_reply->status_msg
         SET dm_err->err_ind = 1
         RETURN(0)
    ...
    ENDFOR
    IF (dfr_r_tab_flg=1)
     EXECUTE dm_create_rmc_triggers reply->tbl_qual[temp_tbl_cnt].r_table_name
    ENDIF
    IF ((dm_err->err_ind=1))
     RETURN(0)
    ENDIF
   ...
   IF ((dm2_rdds_curdb_schema->ccl_same_cnt != reply->tbl_qual[temp_tbl_cnt].col_cnt))
    ...
    IF ((dm2_rdds_curdb_schema->ccl_same_cnt != reply->tbl_qual[temp_tbl_cnt].col_cnt))
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("Cannot find all columns for table ",reply->tbl_qual[temp_tbl_cnt].
      r_table_name," in CCL dictionary after performing oragen3")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dm2_rdds_col_extend(sbr_tbl_idx,sbr_col_idx,r_ind)
  IF (r_ind=0)
   SET dm_err->eproc = concat("Extend length for ",reply->tbl_qual[sbr_tbl_idx].table_name,".",reply
    ->tbl_qual[sbr_tbl_idx].col_qual[sbr_col_idx].column_name," to ",
    cnvtstring(reply->tbl_qual[sbr_tbl_idx].col_qual[sbr_col_idx].db_data_length))
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dm2_push_cmd(concat("rdb alter table ",reply->tbl_qual[sbr_tbl_idx].table_name," modify (",
     reply->tbl_qual[sbr_tbl_idx].col_qual[sbr_col_idx].column_name," ",
     reply->tbl_qual[sbr_tbl_idx].col_qual[sbr_col_idx].db_data_type,"(",cnvtstring(reply->tbl_qual[
      sbr_tbl_idx].col_qual[sbr_col_idx].db_data_length),")) go"),1)=0)
    RETURN(0)
   ENDIF
  ELSE
   SET dm_err->eproc = concat("Extend length for ",reply->tbl_qual[sbr_tbl_idx].r_table_name,".",
    reply->tbl_qual[sbr_tbl_idx].col_qual[sbr_col_idx].column_name," to ",
    cnvtstring(reply->tbl_qual[sbr_tbl_idx].col_qual[sbr_col_idx].db_data_length))
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dm2_push_cmd(concat("rdb alter table ",reply->tbl_qual[sbr_tbl_idx].r_table_name," modify (",
     reply->tbl_qual[sbr_tbl_idx].col_qual[sbr_col_idx].column_name," ",
     reply->tbl_qual[sbr_tbl_idx].col_qual[sbr_col_idx].db_data_type,"(",cnvtstring(reply->tbl_qual[
      sbr_tbl_idx].col_qual[sbr_col_idx].db_data_length),")) go"),1)=0)
    RETURN(0)
  ...
  RETURN(1)
 END ;Subroutine
 SUBROUTINE dm2_rdds_col_add(sbr_tbl_idx,sbr_col_idx,r_ind)
   DECLARE drca_ddl_str = vc WITH protect, noconstant(" ")
   DECLARE drca_tab_name = vc WITH protect, noconstant(" ")
   DECLARE drca_db_type = vc WITH protect, noconstant(" ")
   DECLARE drca_comment_tag = vc WITH protect, noconstant(" ")
   DECLARE drm_bb_trg_cnt = i4
   SET dm_err->eproc = concat("Add column ",reply->tbl_qual[sbr_tbl_idx].col_qual[sbr_col_idx].
    column_name," for table ",reply->tbl_qual[sbr_tbl_idx].table_name)
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dm2_get_rdbms_version(null)=0)
    RETURN(0)
   ENDIF
   IF (r_ind=0)
    SET drca_tab_name = reply->tbl_qual[sbr_tbl_idx].table_name
   ELSE
    SET drca_tab_name = reply->tbl_qual[sbr_tbl_idx].r_table_name
   ENDIF
   IF ((reply->tbl_qual[sbr_tbl_idx].col_qual[sbr_col_idx].db_data_type IN ("CHAR", "VARCHAR",
   "VARCHAR2", "CHARACTER", "RAW")))
    SET drca_db_type = concat(reply->tbl_qual[sbr_tbl_idx].col_qual[sbr_col_idx].db_data_type,"(",
     cnvtstring(reply->tbl_qual[sbr_tbl_idx].col_qual[sbr_col_idx].db_data_length),")")
   ELSE
    SET drca_db_type = reply->tbl_qual[sbr_tbl_idx].col_qual[sbr_col_idx].db_data_type
   ENDIF
   IF ((dm2_rdbms_version->level1 >= 9))
    SET drca_comment_tag = "/*DM2MIG RDDS SOURCE DDL ONLY*/"
   ENDIF
   SET drca_ddl_str = concat("rdb asis (^alter table ",drca_tab_name," add ",reply->tbl_qual[
    sbr_tbl_idx].col_qual[sbr_col_idx].column_name," ",
    drca_db_type," ",trim(drca_comment_tag),"^) go")
   IF (dm2_push_cmd(drca_ddl_str,1)=0)
    RETURN(0)
   ELSE
    IF (r_ind=0)
     SET dfr_l_tab_flg = 1
    ...
    ENDIF
   ENDIF
   IF ((reply->tbl_qual[sbr_tbl_idx].table_name IN ("BED", "BILL_ITEM")))
    SET drm_bb_trg_cnt = 0
    SELECT DISTINCT INTO "nl:"
     u.table_name
     FROM user_triggers u
     WHERE (u.table_name=reply->tbl_qual[sbr_tbl_idx].table_name)
    ...
    ;end select
    IF (drm_bb_trg_cnt > 0)
     SET dguc_request->what_tables = reply->tbl_qual[sbr_tbl_idx].table_name
     EXECUTE dm2_add_chg_log_triggers reply->tbl_qual[sbr_tbl_idx].table_name, "REFCHG"
     IF (check_error("Failed to generate RDDS trigger for table BED and BILL_ITEM") != 0)
      RETURN(0)
     ENDIF
    ENDIF
   ENDIF
   IF ((reply->tbl_qual[sbr_tbl_idx].col_qual[sbr_col_idx].data_default_ni=0))
    IF (r_ind=0)
     IF (dm2_push_cmd(concat("rdb alter table ",reply->tbl_qual[sbr_tbl_idx].table_name," modify (",
       reply->tbl_qual[sbr_tbl_idx].col_qual[sbr_col_idx].column_name," ",
       " default ",reply->tbl_qual[sbr_tbl_idx].col_qual[sbr_col_idx].data_default,") go"),1)=0)
      RETURN(0)
     ENDIF
    ELSE
     IF (dm2_push_cmd(concat("rdb alter table ",reply->tbl_qual[sbr_tbl_idx].r_table_name," modify (",
 ...
 END ;Subroutine
 SUBROUTINE dm2_rdds_col_compare(sbr_tbl_idx,r_ind)
   DECLARE rcc_tbl_cnt = i4 WITH noconstant(0)
   DECLARE rcc_table = vc
   IF (r_ind=0)
    SET rcc_table = reply->tbl_qual[sbr_tbl_idx].table_name
   ELSE
    SET rcc_table = reply->tbl_qual[sbr_tbl_idx].r_table_name
   ENDIF
   SET dm_err->eproc = concat("Compare the schema for table ",rcc_table)
   CALL dm2_rdds_init_drcs_rec(null)
   SELECT INTO "nl:"
    FROM user_tab_cols utc
    WHERE utc.table_name=rcc_table
     AND utc.hidden_column="NO"
     AND utc.virtual_column="NO"
     AND  NOT ( EXISTS (
    (SELECT
    ...
    DETAIL
     IF (r_ind=0)
      ...
      col[rcc_tbl_cnt].data_type = utc.data_type, dm2_rdds_curdb_schema->col[rcc_tbl_cnt].data_length
       = utc.data_length
     ELSE
      IF (utc.column_name != "RDDS_*")
       rcc_tbl_cnt = (rcc_tbl_cnt+ 1)
       IF (mod(rcc_tbl_cnt,50)=1)
        stat = alterlist(dm2_rdds_curdb_schema->col,(rcc_tbl_cnt+ 49))
       ENDIF
       dm2_rdds_curdb_schema->col[rcc_tbl_cnt].column_name = utc.column_name, dm2_rdds_curdb_schema->
       col[rcc_tbl_cnt].data_type = utc.data_type, dm2_rdds_curdb_schema->col[rcc_tbl_cnt].
   ...
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = value(dm2_rdds_curdb_schema->col_cnt)),
     (dummyt dt  WITH seq = value(reply->tbl_qual[sbr_tbl_idx].col_cnt))
    PLAN (dt)
     JOIN (d
     WHERE (reply->tbl_qual[sbr_tbl_idx].col_qual[dt.seq].column_name=dm2_rdds_curdb_schema->col[d
     .seq].column_name))
    DETAIL
     IF (r_ind=0)
      reply->tbl_qual[sbr_tbl_idx].col_qual[dt.seq].in_tgt_flag = 1
     ELSE
      reply->tbl_qual[sbr_tbl_idx].col_qual[dt.seq].r_tgt_flag = 1
     ENDIF
     IF ((reply->tbl_qual[sbr_tbl_idx].col_qual[dt.seq].db_data_type IN ("CHAR", "VARCHAR",
     "VARCHAR2", "RAW", "CHARACTER"))
      AND (reply->tbl_qual[sbr_tbl_idx].col_qual[dt.seq].db_data_length > dm2_rdds_curdb_schema->col[
     d.seq].data_length))
      IF (r_ind=0)
       reply->tbl_qual[sbr_tbl_idx].col_qual[dt.seq].in_tgt_flag = 2
      ELSE
       reply->tbl_qual[sbr_tbl_idx].col_qual[dt.seq].r_tgt_flag = 2
      ENDIF
     ELSE
      dm2_rdds_curdb_schema->same_count = (dm2_rdds_curdb_schema->same_count+ 1)
     ENDIF
     reply->tbl_qual[sbr_tbl_idx].col_qual[dt.seq].db_data_type_tgt = dm2_rdds_curdb_schema->col[d
     .seq].data_type
    WITH nocounter
   ;end select
   IF ((dm_err->debug_flag > 0))
    SET dm_err->eproc = build("dm2_rdds_curdb_schema->same_count=",dm2_rdds_curdb_schema->same_count,
     "; source col_cnt=",reply->tbl_qual[sbr_tbl_idx].col_cnt,"; currdb col_cnt=",
     dm2_rdds_curdb_schema->col_cnt)
    CALL disp_msg("",dm_err->logfile,0)
    SET dm_err->eproc = concat("Table name:",rcc_table,"; column changes needed:")
    FOR (df_lp = 1 TO reply->tbl_qual[sbr_tbl_idx].col_cnt)
      IF ((((reply->tbl_qual[sbr_tbl_idx].col_qual[df_lp].in_tgt_flag=0)) OR ((reply->tbl_qual[
      sbr_tbl_idx].col_qual[df_lp].r_tgt_flag=0))) )
       SET dm_err->eproc = concat(dm_err->eproc,"Add column:",reply->tbl_qual[sbr_tbl_idx].col_qual[
        df_lp].column_name,";")
      ELSEIF ((((reply->tbl_qual[sbr_tbl_idx].col_qual[df_lp].in_tgt_flag=2)) OR ((reply->tbl_qual[
      sbr_tbl_idx].col_qual[df_lp].r_tgt_flag=2))) )
       SET dm_err->eproc = concat(dm_err->eproc,"Extend column:",reply->tbl_qual[sbr_tbl_idx].
 ...
 SUBROUTINE fill_ccl_data_info(cdi_tbl_cnt)
   ...
   SELECT INTO "NL:"
    ...
    FROM dtableattr a,
     ...
     user_tab_cols utc
    PLAN (a
     WHERE (a.table_name=reply->tbl_qual[cdi_tbl_cnt].table_name))
     JOIN (l
     ...
     (SELECT
      ...
      WHERE di.info_domain="RDDS IGNORE COL LIST:*"
       AND sqlpassthru(" utc.column_name like di.info_name and utc.table_name like di.info_char")))))
    DETAIL
     sbr_cdi_idx = 0, sbr_cdi_idx = locateval(sbr_cdi_idx,1,reply->tbl_qual[cdi_tbl_cnt].col_cnt,l
      .attr_name,reply->tbl_qual[cdi_tbl_cnt].col_qual[sbr_cdi_idx].column_name)
     IF (sbr_cdi_idx > 0)
      reply->tbl_qual[cdi_tbl_cnt].col_qual[sbr_cdi_idx].data_length = cnvtstring(l.len)
      IF (l.type="F")
       reply->tbl_qual[cdi_tbl_cnt].col_qual[sbr_cdi_idx].data_type = "F8"
      ELSEIF (l.type="I")
       reply->tbl_qual[cdi_tbl_cnt].col_qual[sbr_cdi_idx].data_type = "I4"
      ELSEIF (l.type="C")
       IF (utc.data_type="CHAR")
        reply->tbl_qual[cdi_tbl_cnt].col_qual[sbr_cdi_idx].data_type = build(l.type,l.len)
       ELSE
        reply->tbl_qual[cdi_tbl_cnt].col_qual[sbr_cdi_idx].data_type = "VC"
       ENDIF
      ELSEIF (l.type="Q")
       reply->tbl_qual[cdi_tbl_cnt].col_qual[sbr_cdi_idx].data_type = "DQ8"
      ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dm2_rdds_record_plsql_metadata(metadata,rs_pos)
   DECLARE asis_call = vc
   DECLARE col_loop = i4
   SET asis_call = "begin rdds_meta_data.put_table_info("
   SET asis_call = concat(asis_call,check_null_string(metadata->tbl_qual[rs_pos].table_name),",")
   SET asis_call = concat(asis_call,check_null_string(metadata->tbl_qual[rs_pos].r_table_name),",")
   SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].mergeable_ind)),",")
   SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].reference_ind)),",")
   SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].version_ind)),",")
   SET asis_call = concat(asis_call,check_null_string(metadata->tbl_qual[rs_pos].version_type),",")
   SET asis_call = concat(asis_call,check_null_string(metadata->tbl_qual[rs_pos].merge_ui_query),",")
   SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].merge_ui_query_ni)),
    ",")
   SET asis_call = concat(asis_call,check_null_string(metadata->tbl_qual[rs_pos].suffix),",")
   SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].merge_delete_ind)),","
    )
   SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].skip_seqmatch_ind)),
    ",")
   SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].insert_only_ind)),",")
   SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].update_only_ind)),",")
   SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].active_ind_ind)),",")
   SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].effective_col_ind)),
    ",")
   SET asis_call = concat(asis_call,check_null_string(metadata->tbl_qual[rs_pos].beg_col_name),",")
   SET asis_call = concat(asis_call,check_null_string(metadata->tbl_qual[rs_pos].end_col_name),",")
   SET asis_call = concat(asis_call,check_null_string(metadata->tbl_qual[rs_pos].lob_process_type),
    ",")
   SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].check_dual_build_ind)),
    ",")
   SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].cur_state_flag)),",")
   SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].pk_where_hash,20)),
    ".0",",")
   SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].del_pk_where_hash,20)),
    ".0",",")
   SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].ptam_match_hash,20)),
    ".0",",")
   SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].overrule_nomv06_ind)),
    ",")
   SET asis_call = concat(asis_call,check_null_string(metadata->tbl_qual[rs_pos].root_col_name))
   SET asis_call = concat(asis_call,"); end;")
   CALL parser(concat("RDB ASIS(^",asis_call,"^) go"),1)
   IF (check_error("Adding table metadata to rdds_meta_data oracle package")=1)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Adding table metadata to rdds_meta_data oracle package"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET error_reply->error_flag = 1
    GO TO exit_fill
   ENDIF
   FOR (col_loop = 1 TO metadata->tbl_qual[rs_pos].col_cnt)
     SET asis_call = "begin rdds_meta_data.put_column_info("
     SET asis_call = concat(asis_call,check_null_string(metadata->tbl_qual[rs_pos].table_name),",")
     SET asis_call = concat(asis_call,check_null_string(metadata->tbl_qual[rs_pos].col_qual[col_loop]
       .column_name),",")
     SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].col_qual[col_loop].
        unique_ident_ind)),",")
     SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].col_qual[col_loop].
        exception_flg)),",")
     SET asis_call = concat(asis_call,check_null_string(metadata->tbl_qual[rs_pos].col_qual[col_loop]
     ...
     SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].col_qual[col_loop].
        merge_delete_ind)),",")
     SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].col_qual[col_loop].
        internal_col_id,20,1)),",")
     SET asis_call = concat(asis_call,check_null_string(metadata->tbl_qual[rs_pos].col_qual[col_loop]
       .db_data_type_tgt),",")
     SET asis_call = concat(asis_call,"NULL",",")
     SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].col_qual[col_loop].
        binary_long_ind)),",")
     SET asis_call = concat(asis_call,trim(cnvtstring(metadata->tbl_qual[rs_pos].col_qual[col_loop].
        pk_ind)),",")
     ...
     SET asis_call = concat(asis_call,"); end;")
     CALL parser(concat("RDB ASIS(^",asis_call,"^) go"),1)
     IF (check_error("Adding column metadata to rdds_meta_data oracle package")=1)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "Adding column metadata to rdds_meta_data oracle package"
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET error_reply->error_flag = 1
      GO TO exit_fill
     ENDIF
   ENDFOR
 END ;Subroutine
 SUBROUTINE check_null_string(string)
   IF (daf_is_not_blank(string))
 ...
 END ;Subroutine
#exit_fill
 IF (dfr_rollback_ind=1
  AND drdm_error_out_ind != 1)
  SET error_reply->error_flag = - (2)
 ENDIF
END GO
