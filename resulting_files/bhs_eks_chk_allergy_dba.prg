CREATE PROGRAM bhs_eks_chk_allergy:dba
 DECLARE patname = vc
 ...
 IF (curqual > 0)
  ...
  IF (( $1="flu"))
   SET msg = build2("Egg or Vaccine Allergy: Patient ",patname,
    " has a documented egg allergy and the Influenza vaccine is contraindicated.",
  ...
  IF (( $1="Pneumo"))
   SET msg = build2("Egg or Vaccine Allergy: Patient ",patname,
    " has a documented egg allergy and the Pneumococcal vaccine is contraindicated.",
    "  Therefore an order for the vaccine was not created.",
    "  Please modify the immunization contraindicated section on the admit assessment.")
   SET retval = 100
   SET log_message = build(log_message,"_",msg)
  ...
  SUBROUTINE srvreply(taskhandle,reqno)
    ...
    IF (crmstatus=ecrmok)
     ...
     ELSE
      ...
      SET ndx1 = uar_srvgetitemcount(hinboxreply,"ASSIGN_PRSNL_LIST")
      SET stat = alterlist(inboxreply->assign_prsnl_list,ndx1)
      FOR (ndx1 = 1 TO ndx1)
      ...
      ELSE
       ...
       SET ndx2 = uar_srvgetitemcount(hstatus_data,"SUBEVENTSTATUS")
       SET stat = alterlist(inboxreply->status_data.subeventstatus,ndx2)
       FOR (ndx2 = 1 TO ndx2)
  ...
  SET inboxrequest->msg_subject = "Vaccine Allergy Alert"
