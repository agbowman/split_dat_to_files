CREATE PROGRAM ct_rpt_pt_inq_dx_lo:dba
 PROMPT
 ...
 SUBROUTINE (finalizereport(ssendreport=vc) =null WITH protect)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE (pending_job_head(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = pending_job_headabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (pending_job_headabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =f8
  WITH protect)
   ...
   IF (_rempending_job_label > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_rempending_job_label,((size(
     ...
     SET _rempending_job_label += rptsd->m_drawlength
    ELSE
     SET _rempending_job_label = 0
    ENDIF
    SET growsum += _rempending_job_label
   ENDIF
   SET rptsd->m_flags = 4
   IF (bcontinue)
    SET rptsd->m_y = offsety
 ...
 END ;Subroutine
 SUBROUTINE (pending_job_protocol(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = pending_job_protocolabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (pending_job_protocolabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =
  f8 WITH protect)
   ...
   IF (_rempending_job_protocol > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_rempending_job_protocol,((size(
     ...
     SET _rempending_job_protocol += rptsd->m_drawlength
    ELSE
     SET _rempending_job_protocol = 0
    ENDIF
    SET growsum += _rempending_job_protocol
   ENDIF
   SET rptsd->m_flags = 4
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (pending_job_foot(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = pending_job_footabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (pending_job_footabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =f8
  WITH protect)
   DECLARE sectionheight = f8 WITH noconstant(0.250000), private
   DECLARE growsum = i4 WITH noconstant(0), private
   DECLARE drawheight_pending_job_label = f8 WITH noconstant(0.0), private
  ...
  WITH protect)
   ...
   IF (_remno_patient_eval > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remno_patient_eval,((size(
        __no_patient_eval) - _remno_patient_eval)+ 1),__no_patient_eval)))))
     SET _remno_patient_eval += rptsd->m_drawlength
    ELSE
     SET _remno_patient_eval = 0
    ENDIF
    SET growsum += _remno_patient_eval
   ENDIF
   SET rptsd->m_flags = 4
   IF (bcontinue)
    SET rptsd->m_y = offsety
 ...
 SUBROUTINE initializereport(dummy)
   IF (_bsubreport=0)
    ...
    SET rptreport->m_reportname = "CT_RPT_PT_INQ_DX_LO"
 ...
 DECLARE patientinqmode = vc WITH public, constant("PATIENTINQ")
 EXECUTE ct_trial_prescreen_dx:dba  $OUTDEV, patientinqmode,  $STARTDATE,
  $ENDDATE,  $ENCNTRTYPECD,  $FACILITYCD,
  $TRIGGERNAME,  $ORDERBY,  $GENDER,
  $QUALIFIER,  $AGE1,  $AGE2,
  $RACE,  $ETHNICITY,  $TERMINOLOGY,
 ...
 IF (((eksctrequest->opsind) OR ((eksctrequest->execmodeflag=- (1)))) )
  SET _bsubreport = 1
  EXECUTE ct_sub_rpt_prescreen_dx_pt:dba  $OUTDEV, patientinqmode,  $STARTDATE,
   $ENDDATE,  $ENCNTRTYPECD,  $FACILITYCD,
   $TRIGGERNAME,  $ORDERBY,  $GENDER,
   $QUALIFIER,  $AGE1,  $AGE2,
   $RACE,  $ETHNICITY,  $TERMINOLOGY,
  ...
  IF ( NOT (eksctrequest->opsind)
   ...
   IF ((eksctreply->ctfndind=1))
    IF (size(eksctreply->qual,5) > 0)
     ...
     IF (( $ORDERBY=0))
      SET _bsubreport = 1
      EXECUTE ct_sub_rpt_prescreen_dx_prot:dba  $OUTDEV, patientinqmode,  $STARTDATE,
       $ENDDATE,  $ENCNTRTYPECD,  $FACILITYCD,
       $TRIGGERNAME,  $ORDERBY,  $GENDER,
       $QUALIFIER,  $AGE1,  $AGE2,
       $RACE,  $ETHNICITY,  $TERMINOLOGY,
     ...
     IF (( $ORDERBY=1))
      SET _bsubreport = 1
      EXECUTE ct_sub_rpt_prescreen_dx_prsn:dba  $OUTDEV, patientinqmode,  $STARTDATE,
       $ENDDATE,  $ENCNTRTYPECD,  $FACILITYCD,
       $TRIGGERNAME,  $ORDERBY,  $GENDER,
       $QUALIFIER,  $AGE1,  $AGE2,
       $RACE,  $ETHNICITY,  $TERMINOLOGY,
   ...
   ELSE
    ...
    WHILE (((_bcontdetailsection=1) OR (bfirsttime=1)) )
      SET _bholdcontinue = _bcontdetailsection
      SET _fdrawheight = detailsection(rpt_calcheight,(_fenddetail - _yoffset),_bholdcontinue)
      IF (((_yoffset+ _fdrawheight) > _fenddetail))
       CALL pagebreak(0)
      ELSEIF (_bholdcontinue=1
       ...
       CALL pagebreak(0)
      ENDIF
      SET dummy_val = detailsection(rpt_render,(_fenddetail - _yoffset),_bcontdetailsection)
      SET bfirsttime = 0
    ENDWHILE
   ENDIF
  ENDIF
