CREATE PROGRAM bed_get_vocabularies
 ...
 SUBROUTINE validatesrvreply(nsrvstat,recorddata,zeroforceexit)
   ...
   ENDIF
 END ;Subroutine
 DECLARE populatesearchlist(p1=i4) = null WITH protect
 DECLARE requestnumber = i4 WITH protect, noconstant(0)
 DECLARE srv_request = vc WITH protect, noconstant("")
 SUBROUTINE populatesearchlist(requestnumber)
   IF (requestnumber=4174071)
    SET srv_request = "ProceduresList"
   ELSEIF (requestnumber=4174073)
    SET srv_request = "ProblemsList"
   ENDIF
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(curdate,curtime3)), private
   DECLARE sreturnvalue = vc WITH protect, noconstant("")
   ...
   DECLARE crossmapcnt = i4 WITH protect, noconstant(0)
   ...
   IF (listtransactioncheck != 0)
     ...
     IF (resultcnts > 0)
      FOR (i = 0 TO (resultcnts - 1))
        ...
        FOR (a = 0 TO (termcnt - 1))
          ...
          SET record_data->available_searches[(i+ 1)].terminologies[(a+ 1)].codified_value =
          uar_get_code_by("MEANING",400,record_data->available_searches[(i+ 1)].terminologies[(a+ 1)]
           .identifier)
        ...
        IF (corrcnt)
         ...
         FOR (d = 0 TO (corrcnt - 1))
           ...
           IF (msubsetscnt)
            SET stat = alterlist(record_data->available_searches[(i+ 1)].cross_mapped_terminologies[(
             d+ 1)].subsets,msubsetscnt)
            FOR (e = 0 TO (msubsetscnt - 1))
             SET hsubsets = uar_srvgetitem(curs,"subsets",e)
             SET record_data->available_searches[(i+ 1)].cross_mapped_terminologies[(d+ 1)].subsets[(
             e+ 1)].identifier = uar_srvgetstringptr(hsubsets,"identifier")
            ENDFOR
           ENDIF
           ...
           IF (maxescnt)
            ...
            FOR (f = 0 TO maxescnt)
             ...
             SET record_data->available_searches[(i+ 1)].cross_mapped_terminologies[(d+ 1)].axes[(f+
             1)].identifier = uar_srvgetstringptr(haxess,"identifier")
            ENDFOR
           ENDIF
         ENDFOR
        ENDIF
      ENDFOR
      SET hstatuss = uar_srvgetstruct(hrep,"status_data",1)
     ENDIF
     IF (hreq != 0)
    ...
    ENDIF
   ELSE
    SET record_data->status.success_ind = 0
   ENDIF
 ...
 DECLARE term_count = i4 WITH protect, noconstant(0)
 SELECT INTO "nl:"
  FROM code_value cv
  PLAN (cv
  ...
  HEAD REPORT
   cnt = 0
  DETAIL
   cnt = (size(temp_filter_reply->custom_filter_items,5)+ 1), stat = alterlist(temp_filter_reply->
    custom_filter_items,cnt), temp_filter_reply->custom_filter_items[cnt].parent_entity_id = cv
   .code_value,
   temp_filter_reply->custom_filter_items[cnt].parent_entity_name = "code_value", temp_filter_reply->
   custom_filter_items[cnt].display = cv.display, temp_filter_reply->custom_filter_items[cnt].
   description = cv.description,
   temp_filter_reply->custom_filter_items[cnt].status_ind = 1, temp_filter_reply->
   custom_filter_items[cnt].cdf_meaning = cv.cdf_meaning
  WITH nocounter
 ;end select
 CALL bederrorcheck("Error[1]: Failure in getting vocabularies from code_value table")
 SELECT INTO "nl:"
  FROM cmt_subset cs
  PLAN (cs
   WHERE cs.subset_id > 0)
  DETAIL
   cnt = (size(temp_filter_reply->custom_filter_items,5)+ 1), stat = alterlist(temp_filter_reply->
    custom_filter_items,cnt), temp_filter_reply->custom_filter_items[cnt].parent_entity_id = cs
   .subset_id,
   temp_filter_reply->custom_filter_items[cnt].parent_entity_name = "cmt_subset", temp_filter_reply->
   custom_filter_items[cnt].display = cs.subset_name, temp_filter_reply->custom_filter_items[cnt].
   description = "",
   temp_filter_reply->custom_filter_items[cnt].status_ind = 1, temp_filter_reply->
   custom_filter_items[cnt].cdf_meaning = cs.subset_mean
  WITH nocounter
 ;end select
 CALL bederrorcheck("Error[2]: Failure in getting vocabularies from cmt_subset table")
 IF ((request->filter_type_ind=1))
  CALL populatesearchlist(4174073)
 ELSEIF ((request->filter_type_ind=2))
  CALL populatesearchlist(4174071)
 ENDIF
 SET voc_count = size(record_data->available_searches,5)
 IF (voc_count > 0)
  FOR (x = 1 TO voc_count)
   SET term_count = size(record_data->available_searches[x].terminologies,5)
   FOR (y = 1 TO term_count)
     SET num = 0
     SET pos = locateval(num,start,size(temp_filter_reply->custom_filter_items,5),record_data->
      available_searches[x].terminologies[y].identifier,temp_filter_reply->custom_filter_items[num].
      cdf_meaning)
     IF (pos > 0)
      SET rep_size = size(reply->custom_filter_items,5)
      SET stat = alterlist(reply->custom_filter_items,rep_size)
      SET temp_filter_reply->custom_filter_items[pos].display = build(temp_filter_reply->
       custom_filter_items[pos].display,"(",record_data->available_searches[x].identifier,")")
      SET stat = movereclist(temp_filter_reply->custom_filter_items,reply->custom_filter_items,pos,
       rep_size,1,
       true)
     ENDIF
   ENDFOR
  ENDFOR
 ELSE
  SET stat = moverec(temp_filter_reply->custom_filter_items,reply->custom_filter_items)
 ENDIF
#exit_script
