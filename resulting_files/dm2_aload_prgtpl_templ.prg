CREATE PROGRAM dm2_aload_prgtpl_templ
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dapt_req_size = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2_aload_prg_templ",".log","dm2_aload_prgtpl_templ")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_prgtpl_templ"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(requestin)
 ENDIF
 SET dapt_req_size = size(requestin->list_0,5)
 IF (dac_pop_coldic_rec("DM_ADM_PURGE_TEMPLATE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Remove purge template rows."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 DELETE  FROM dm_adm_purge_template tpl,
   (dummyt d  WITH seq = value(dapt_req_size))
  SET tpl.seq = 0
  PLAN (d)
   JOIN (tpl
   WHERE tpl.ocd=cnvtint(requestin->list_0[d.seq].ocd)
    AND tpl.template_nbr=cnvtint(requestin->list_0[d.seq].template_nbr))
  WITH nocounter
 ;end delete
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
   " dcv ,(dummyt d with seq = value(dapt_req_size) )"),0)
 CALL dm2_push_cmd("set",0)
 IF (dac_build_col_list(null)=0)
  ROLLBACK
  GO TO exit_program
 ENDIF
 IF (dm2_push_cmd("plan d join dcv with nocounter go",1)=0)
  ROLLBACK
  GO TO exit_program
 ENDIF
 IF (curqual != dapt_req_size)
  SET dm_err->err_ind = 1
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = "Remove child purge token rows."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 DELETE  FROM dm_adm_purge_token tkn,
   (dummyt d  WITH seq = value(dapt_req_size))
  SET tkn.seq = 0
  PLAN (d)
   JOIN (tkn
   WHERE tkn.ocd=cnvtint(requestin->list_0[d.seq].ocd)
    AND tkn.template_nbr=cnvtint(requestin->list_0[d.seq].template_nbr))
  WITH nocounter
 ;end delete
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 ELSE
  COMMIT
 ENDIF
 SET dm_err->eproc = "Remove child purge table rows."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 DELETE  FROM dm_adm_purge_table tkn,
   (dummyt d  WITH seq = value(dapt_req_size))
  SET tkn.seq = 0
  PLAN (d)
   JOIN (tkn
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_prgtpl_templ completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_prgtpl_templ")
END GO
