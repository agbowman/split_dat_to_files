CREATE PROGRAM dcp_get_missing_required_ind:dba
 ...
 DECLARE debug = i2 WITH protect, constant(query_missing_required_request->debug)
 DECLARE i_stat = i2 WITH protect, noconstant(0)
 DECLARE l_start = i4 WITH protect, noconstant(1)
 ...
 DECLARE lrequestcount = i4 WITH protect, noconstant(0)
 DECLARE lrequestsize = i4 WITH protect, noconstant(0)
 DECLARE lrequestbatchsize = i4 WITH protect, noconstant(20)
 ...
 DECLARE dtimeinseconds = f8 WITH protect, noconstant(0.0)
 DECLARE dtotaltimeinseconds = f8 WITH protect, noconstant(0.0)
 DECLARE starttime = q8 WITH protect, noconstant(cnvtdatetime(curdate,curtime3))
 DECLARE stoptime = q8
 DECLARE starttimer(dummy=i4) = null
 DECLARE stoptimer(sdisplay=vc) = null
 IF (debug > 0)
  CALL starttimer(0)
 ENDIF
 SET l_start = 1
 SET lbatchsize = lrequestbatchsize
 SET lrequestcount = size(query_missing_required_request->list,5)
 SET lloopcount = ceil((cnvtreal(lrequestcount)/ lbatchsize))
 SET lrequestsize = (lloopcount * lbatchsize)
 SET i_stat = alterlist(query_missing_required_request->list,lrequestsize)
 IF (lrequestcount > 0
  AND lrequestcount < lrequestsize)
  FOR (idx = (lrequestcount+ 1) TO lrequestsize)
    SET query_missing_required_request->list[idx].pathway_comp_id = query_missing_required_request->
    list[lrequestcount].pathway_comp_id
  ENDFOR
 ENDIF
 CALL catcherrors("DCP_GET_MISSING_REQUIRED_IND 01 - Resize request")
 IF (debug > 0)
  CALL stoptimer("DCP_GET_MISSING_REQUIRED_IND 01 - Resize request")
 ENDIF
 IF (lrequestcount < 1)
  GO TO exit_script
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  WHERE cv.code_set=catalog_type_cs
   AND cv.cdf_meaning IN ("PHARMACY")
   AND cv.active_ind=1
  DETAIL
   CASE (cv.cdf_meaning)
    OF "PHARMACY":
  ...
  WITH nocounter
 ;end select
 CALL catcherrors("DCP_GET_MISSING_REQUIRED_IND 02 - Get code values")
 IF (debug > 0)
  CALL stoptimer("DCP_GET_MISSING_REQUIRED_IND 02 - Get code values")
  CALL echo(concat("CATALOG_TYPE_CD_PHARMACY	= ",build(catalog_type_cd_pharmacy)))
 ENDIF
 IF (debug > 0)
  CALL starttimer(0)
 ENDIF
 ...
 IF (debug > 0)
  CALL stoptimer("DCP_GET_MISSING_REQUIRED_IND 02 - Get code values")
  CALL echo(concat("ACTION_TYPE_CD_ORDER		= ",build(action_type_cd_order)))
  CALL echo(concat("ACTION_TYPE_CD_DISORDER	= ",build(action_type_cd_disorder)))
 ENDIF
 IF (debug > 0)
  CALL starttimer(0)
 ENDIF
 ...
 IF (debug > 0)
  CALL stoptimer("DCP_GET_MISSING_REQUIRED_IND 02 - Get code values")
  CALL echo(concat("COMP_TYPE_CD_ORDER		= ",build(comp_type_cd_order)))
  CALL echo(concat("COMP_TYPE_CD_PRESCRIPTION	= ",build(comp_type_cd_prescription)))
 ENDIF
 IF (debug > 0)
  CALL starttimer(0)
 ENDIF
 SELECT INTO "nl:"
  ...
  PLAN (d1
   WHERE initarray(l_start,evaluate(d1.seq,1,1,(l_start+ lbatchsize))))
   JOIN (pwc
   WHERE expand(idx,l_start,((l_start+ lbatchsize) - 1),pwc.pathway_comp_id,
    query_missing_required_request->list[idx].pathway_comp_id))
   JOIN (pcor
  ...
  ORDER BY oefp.oe_format_id, pwc.comp_type_cd, pcor.order_sentence_id,
   pwc.pathway_comp_id
  HEAD REPORT
   loeformatcount = 0, loeformatsize = 0, lqueryoeformatcount = 0,
   lqueryoeformatsize = 0, lordersentencecount = 0, lordersentencesize = 0,
   livsetcount = 0, livsetsize = 0, baddedivsetdata = 0
  ...
  FOOT REPORT
   ...
   IF (ltimedprioritycount > 0
    ...
    FOR (idx = (ltimedprioritycount+ 1) TO ltimedprioritysize)
      data->timed_priority_list[idx].oe_format_id = data->timed_priority_list[ltimedprioritycount].
      oe_format_id
    ENDFOR
   ENDIF
   IF (lqueryoeformatcount > 0
    AND lqueryoeformatcount < lqueryoeformatsize)
    FOR (idx = (lqueryoeformatcount+ 1) TO lqueryoeformatsize)
     data->query_oe_format_list[idx].oe_format_id = data->query_oe_format_list[lqueryoeformatcount].
     oe_format_id,data->query_oe_format_list[idx].action_type_cd = data->query_oe_format_list[
     lqueryoeformatcount].action_type_cd
    ENDFOR
   ENDIF
   IF (lordersentencecount > 0
    AND lordersentencecount < lordersentencesize)
    FOR (idx = (lordersentencecount+ 1) TO lordersentencesize)
      data->order_sentence_list[idx].order_sentence_id = data->order_sentence_list[
      lordersentencecount].order_sentence_id
    ENDFOR
   ENDIF
   IF (livsetcount > 0
    AND livsetcount < livsetsize)
    FOR (idx = (livsetcount+ 1) TO livsetsize)
      data->iv_set_list[idx].synonym_id = data->iv_set_list[livsetcount].synonym_id
    ENDFOR
   ENDIF
  WITH nocounter
 ;end select
 CALL catcherrors("DCP_GET_MISSING_REQUIRED_IND 03 - Get component order sentence information")
 IF (debug > 0)
  CALL stoptimer("DCP_GET_MISSING_REQUIRED_IND 03 - Initial select")
 ENDIF
 ...
 ENDIF
 SET l_start = 1
 SET lbatchsize = lqueryoeformatbatchsize
 SET lloopcount = ceil((cnvtreal(lqueryoeformatcount)/ lbatchsize))
 SELECT INTO "nl:"
  ...
  PLAN (d1
   WHERE initarray(l_start,evaluate(d1.seq,1,1,(l_start+ lbatchsize))))
   JOIN (oeff
   WHERE expand(idx,l_start,((l_start+ lbatchsize) - 1),oeff.oe_format_id,data->query_oe_format_list[
    idx].oe_format_id,
    oeff.action_type_cd,data->query_oe_format_list[idx].action_type_cd))
   JOIN (oef
  ...
  WITH nocounter
 ;end select
 CALL catcherrors("DCP_GET_MISSING_REQUIRED_IND 05 - Get order format fields")
 IF (debug > 0)
  CALL stoptimer("DCP_GET_MISSING_REQUIRED_IND 05 - Get order format fields")
 ENDIF
 IF (debug > 0)
  CALL starttimer(0)
 ENDIF
 SET l_start = 1
 SET lbatchsize = ltimedprioritybatchsize
 SET lloopcount = ceil((cnvtreal(ltimedprioritysize)/ lbatchsize))
 IF (lloopcount > 0)
  SELECT INTO "nl:"
   ...
   PLAN (d1
    ...
    WHERE opf.order_priority_flexing_id > 0.0
     AND expand(idx,l_start,((l_start+ lbatchsize) - 1),opf.oe_format_id,data->timed_priority_list[
     idx].oe_format_id)
   ...
   WITH nocounter
  ;end select
  CALL catcherrors("DCP_GET_MISSING_REQUIRED_IND 06 - Get timed priority from format")
 ENDIF
 IF (debug > 0)
  CALL stoptimer("DCP_GET_MISSING_REQUIRED_IND 06 - Get timed priority from format")
 ENDIF
 IF (debug > 0)
  CALL starttimer(0)
 ENDIF
 SET l_start = 1
 SET lbatchsize = livsetbatchsize
 SET lloopcount = ceil((cnvtreal(livsetsize)/ lbatchsize))
 IF (lloopcount > 0)
  SELECT INTO "nl:"
   ...
   PLAN (d1
    WHERE initarray(l_start,evaluate(d1.seq,1,1,(l_start+ lbatchsize))))
    JOIN (ocs
    WHERE expand(idx,l_start,((l_start+ lbatchsize) - 1),ocs.synonym_id,data->iv_set_list[idx].
     synonym_id))
    JOIN (csc
   ...
   WITH nocounter
  ;end select
  CALL catcherrors("DCP_GET_MISSING_REQUIRED_IND 07 - Get IV Sets")
 ENDIF
 IF (debug > 0)
  CALL stoptimer("DCP_GET_MISSING_REQUIRED_IND 07 - Get IV Sets")
 ENDIF
 IF (debug > 0)
  CALL starttimer(0)
 ENDIF
 SET l_start = 1
 SET lbatchsize = lordersentencebatchsize
 SET lloopcount = ceil((cnvtreal(lordersentencesize)/ lbatchsize))
 SELECT INTO "nl:"
  ...
  PLAN (d1
   WHERE initarray(l_start,evaluate(d1.seq,1,1,(l_start+ lbatchsize))))
   JOIN (osd
   WHERE expand(idx,l_start,((l_start+ lbatchsize) - 1),osd.order_sentence_id,data->
    order_sentence_list[idx].order_sentence_id)
  ...
  FOOT REPORT
   IF (ltimedcollectionprioritycount > 0
    ...
    FOR (idx = (ltimedcollectionprioritycount+ 1) TO ltimedcollectionprioritysize)
      data->timed_collection_priority_list[idx].collection_priority_cd = data->
      timed_collection_priority_list[ltimedcollectionprioritycount].collection_priority_cd
    ENDFOR
   ENDIF
  WITH nocounter
 ;end select
 CALL catcherrors("DCP_GET_MISSING_REQUIRED_IND 08 - Get order sentence fields")
 IF (lordersentencecount > 0
  AND lordersentencecount < lordersentencesize)
  SET i_stat = alterlist(data->order_sentence_list,lordersentencecount)
 ENDIF
 IF (debug > 0)
  CALL stoptimer("DCP_GET_MISSING_REQUIRED_IND 08 - Get order sentence fields")
 ENDIF
 IF (ltimedcollectionprioritycount > 0)
  IF (debug > 0)
   CALL starttimer(0)
  ENDIF
  SET l_start = 1
  SET lbatchsize = ltimedcollectionprioritybatchsize
  SET lloopcount = ceil((cnvtreal(ltimedcollectionprioritysize)/ lbatchsize))
  SELECT INTO "nl:"
   ...
   PLAN (d1
    WHERE initarray(l_start,evaluate(d1.seq,1,1,(l_start+ lbatchsize))))
    JOIN (cp
    WHERE expand(idx,l_start,((l_start+ lbatchsize) - 1),cp.collection_priority_cd,data->
     timed_collection_priority_list[idx].collection_priority_cd)
   ...
   WITH nocounter
  ;end select
  CALL catcherrors("DCP_GET_MISSING_REQUIRED_IND 09 - Get timed collection priority codes")
  IF (debug > 0)
   CALL stoptimer("DCP_GET_MISSING_REQUIRED_IND 09 - Get timed collection priority codes")
  ENDIF
 ENDIF
 IF (debug > 0)
  CALL starttimer(0)
 ...
 ENDIF
 CALL catcherrors("DCP_GET_MISSING_REQUIRED_IND 10 - Get missing required field indicators")
 IF (debug > 0)
  CALL stoptimer("DCP_GET_MISSING_REQUIRED_IND 10 - Get missing required field indicators")
 ENDIF
...
#exit_script
 IF (debug > 1)
  CALL echorecord(query_missing_required_request)
  CALL echorecord(query_missing_required_reply)
 ENDIF
 IF (debug > 2)
  CALL echorecord(data)
 ENDIF
 FREE RECORD data
 DECLARE dcp_get_missing_required_ind_last_mod = c3 WITH protect, constant("000")
END GO
