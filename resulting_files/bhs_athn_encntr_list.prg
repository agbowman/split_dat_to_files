CREATE PROGRAM bhs_athn_encntr_list
 FREE RECORD result
 RECORD result(
   1 person_id = f8
   1 prsnl_id = f8
 ...
 SUBROUTINE (addencntrtosecurityreq(personid=f8(val),encntrid=f8(val)) =null)
 ...
 IF ((request->person_id <= 0.0))
  ...
  GO TO exit_script
 ELSEIF ((request->prsnl_id <= 0.0))
  CALL echo("INVALID PERSONNEL ID PARAMETER...EXITING")
  GO TO exit_script
 ENDIF
 SET result->person_id = request->person_id
 SET result->prsnl_id = request->prsnl_id
...
#exit_script
 ...
 FOR (idx = 1 TO size(cve_reply->encntrs,5))
   IF ((cve_reply->encntrs[idx].secure_ind=0))
    SET jdx += 1
    SET stat = alterlist(reply->qual,jdx)
    SET reply->qual[jdx].encntr_id = cve_reply->encntrs[idx].encntr_id
   ENDIF
 ENDFOR
 ...
 SUBROUTINE getencounters(null)
   SELECT INTO "NL:"
    ...
    HEAD e.encntr_id
     ecnt += 1, stat = alterlist(result->list,ecnt), result->list[ecnt].encntr_id = e.encntr_id
