CREATE PROGRAM dm2_aload_comm_data_fnd
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_cdf",".log","dm2_aload_comm_data_fnd")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_comm_data_fnd"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dacdf_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dacdf_file = vc WITH protect, noconstant( $2)
 DECLARE dacdf_row_cnt = i4 WITH protect, noconstant( $3)
 DECLARE dacdf_batch = i4 WITH protect, noconstant(0)
 DECLARE dacdf_rows_returned = i4 WITH protect, noconstant(10000)
 DECLARE dacdf_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(dacdf_pkg,dacdf_file_loc_ret,dacdf_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dacdf_batch=0)
  SET dacdf_batch = 5000
 ENDIF
 SET dm_err->eproc = "Clean existing data from DM_AFD_COMMON_DATA_FOUNDATION for package."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 WHILE ((dm_err->err_ind=0)
  AND dacdf_rows_returned=10000)
   DELETE  FROM dm_afd_common_data_foundation d
    WHERE d.alpha_feature_nbr=dacdf_pkg
   ...
   ELSE
    COMMIT
   ENDIF
   SET dacdf_rows_returned = curqual
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("Rows deleted:",dacdf_rows_returned))
   ENDIF
 ENDWHILE
 IF (dacdf_row_cnt > 0)
  EXECUTE dm2_dbimport concat(dacdf_file_loc_ret,dacdf_file), "dm2_aload_cs_cdf", dacdf_batch
  CALL echo(concat(dacdf_file_loc_ret,dacdf_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_aload_comm_data_fnd completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_comm_data_fnd")
END GO
