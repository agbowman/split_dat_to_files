CREATE PROGRAM dcp_get_pl_service:dba
 RECORD orgs(
   1 qual[*]
     2 org_id = f8
     2 confid_level = i4
 ...
 DECLARE service_cd = f8 WITH noconstant(0.0)
 ...
 FOR (counter = 1 TO arg_nbr)
   CASE (request->arguments[counter].argument_name)
    OF "medical_service_cd":
     SET service_cd = request->arguments[counter].parent_entity_id
    OF "lag_minutes":
 ...
 SELECT INTO "nl:"
  FROM encntr_domain ed,
   ...
   person p
  PLAN (ed
   WHERE ed.med_service_cd=service_cd
