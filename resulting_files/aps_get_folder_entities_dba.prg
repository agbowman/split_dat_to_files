CREATE PROGRAM aps_get_folder_entities:dba
 ...
 SET cnt = 0
 DECLARE uar_fmt_accession(x1,x2) = c25
 CALL initresourcesecurity(1)
 SELECT INTO mine
  afe.folder_id, afe.entity_id, afe.display,
  pc_exists = decode(pc.seq,"Y","N")
  FROM ap_folder_entity afe,
   long_text lt,
   ap_prefix ap,
   pathology_case pc
  PLAN (afe
   WHERE (afe.folder_id=request->folder_id)
    AND afe.entity_id != 0)
   JOIN (lt
   WHERE afe.comment_id=lt.long_text_id)
   JOIN (pc
   WHERE (pc.accession_nbr= Outerjoin(afe.accession_nbr)) )
   JOIN (ap
  ...
  DETAIL
   ...
   IF (isresourceviewable(service_resource_cd)=true)
    cnt += 1
    IF (mod(cnt,10)=1)
     stat = alterlist(reply->folder_entity_qual,(cnt+ 9))
    ENDIF
    reply->folder_entity_qual[cnt].entity_id = afe.entity_id, reply->folder_entity_qual[cnt].
    create_prsnl_id = afe.create_prsnl_id, reply->folder_entity_qual[cnt].parent_entity_id = afe
    .parent_entity_id,
    reply->folder_entity_qual[cnt].parent_entity_name = afe.parent_entity_name, reply->
    folder_entity_qual[cnt].entity_type_flag = afe.entity_type_flag, reply->folder_entity_qual[cnt].
    display = afe.display
    IF (lt.long_text_id != 0.0)
     reply->folder_entity_qual[cnt].comment = lt.long_text
    ENDIF
    reply->folder_entity_qual[cnt].accession_nbr = afe.accession_nbr, afe_accession_display =
    uar_fmt_accession(afe.accession_nbr,size(afe.accession_nbr)), reply->folder_entity_qual[cnt].
    accession_display = afe_accession_display,
    reply->folder_entity_qual[cnt].updt_cnt = afe.updt_cnt
   ENDIF
  FOOT REPORT
   stat = alterlist(reply->folder_entity_qual,cnt)
  WITH nocounter
 ;end select
 IF (curqual=0)
