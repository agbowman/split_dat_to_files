CREATE PROGRAM bed_get_med_order_sets:dba
 ...
 SELECT INTO "nl:"
  ...
  HEAD md.item_id
   cnt = (cnt+ 1), stat = alterlist(reply->med_order_sets,cnt), reply->med_order_sets[cnt].item_id =
   md.item_id,
   reply->med_order_sets[cnt].description = mi1.value, reply->med_order_sets[cnt].short_description
    = mi2.value
  WITH nocounter
 ;end select
 IF (cnt > max_cnt)
  SET stat = alterlist(reply->med_order_sets,0)
