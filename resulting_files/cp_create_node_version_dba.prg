CREATE PROGRAM cp_create_node_version:dba
 PROMPT
 ...
 SUBROUTINE loadnodedetails(null)
   ...
   EXECUTE cp_get_nodes_config  WITH replace("REPLY","NODE_REPLY")
 ...
 SUBROUTINE versionnode(null)
   ...
   SET create_node_req->node_list[1].publish_ind = 1
   SET create_node_req->node_list[1].version_node_id =  $DNODE_ID
   EXECUTE cp_create_node  WITH replace("REQUEST","CREATE_NODE_REQ"), replace("REPLY",
    "CREATE_NODE_REPLY")
   ...
   FOR (vx = 1 TO size(node_reply->node_list[1].component_list,5))
     ...
     SET add_comp_req->component_list[1].cp_node_id = dnewnodeid
     SET add_comp_req->component_list[1].comp_type_cd = node_reply->node_list[1].component_list[vx].
     comp_type_cd
     SET add_comp_req->component_list[1].concept_group_cd = node_reply->node_list[1].component_list[
     vx].concept_group_cd
     SET add_comp_req->component_list[1].component_seq_txt = node_reply->node_list[1].component_list[
     vx].comp_seq_txt
     EXECUTE cp_add_component  WITH replace("REQUEST","ADD_COMP_REQ"), replace("REPLY","ADD_COMP_REP"
      )
   ...
   UPDATE  FROM cp_node cn
    SET cn.beg_effective_dt_tm = cnvtdatetime(sysdate), publish_ind = 0
    WHERE (cn.cp_node_id= $DNODE_ID)
    WITH nocounter
   ;end update
