CREATE PROGRAM dm_readme_log:dba
 ...
 ENDIF
 SET width = 132
 IF (check_logfile("dm_readme_log",".log","dm_readme_log")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting DM_README_LOG."
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE drl_validate_input(null) = i2
 DECLARE drl_menu(dm_menu_selection=i2(ref)) = i2
 DECLARE drl_report(dr_group_flag=i2) = i2
 DECLARE drl_standalone(null) = i2
 DECLARE drl_ocd(do_group_ret=i2(ref)) = i2
 DECLARE drl_standalone_mode = i2 WITH protect, constant(0)
 DECLARE drl_ocd_mode = i2 WITH protect, constant(1)
 DECLARE drl_group_flag_ret = i2 WITH protect, noconstant(0)
 DECLARE drl_menu_return = i2 WITH protect, noconstant(0)
 DECLARE drl_env_id_hold = f8 WITH protect, noconstant(0.0)
 DECLARE drl_get_out_return = i2 WITH protect, noconstant(0)
 DECLARE drl_projtype_qual = vc WITH protect, noconstant("README*")
 FREE SET work
 RECORD work(
   1 low_status = vc
   1 high_status = vc
 )
 FREE SET drl_readmes
 RECORD drl_readmes(
   1 readme[*]
     2 readme_id = f8
     2 description = vc
 ...
 IF ((dm2_rr_misc->env_id=0.0))
  IF (dm2_get_env_data(1,drl_env_id_hold)=0)
   GO TO exit_program
  ENDIF
  SET dm2_rr_misc->env_id = drl_env_id_hold
 ENDIF
 IF ( NOT (validate(mode,0)))
  SET mode = 0
 ENDIF
 IF (drl_validate_input(null)=0)
  GO TO exit_program
 ENDIF
 WHILE (true)
   SET stat = alterlist(drl_readmes->readme,0)
   SET drl_projtype_qual = "README*"
   IF (drl_menu(drl_menu_return)=0)
    GO TO exit_program
   ENDIF
   IF (drl_menu_return=0)
    GO TO exit_program
   ELSE
   ...
   IF (dm2_rr_clean_stranded_readmes(dm2_rr_misc->env_id)=0)
    GO TO exit_program
   ENDIF
   IF (drl_menu_return=6)
    EXECUTE dm_parallel_reports
    IF ((dm_err->err_ind=1))
     GO TO exit_program
    ENDIF
   ELSE
    CASE (mode)
     OF drl_standalone_mode:
      IF (drl_standalone(null)=0)
       GO TO exit_program
      ENDIF
     OF drl_ocd_mode:
      IF (drl_ocd(drl_group_flag_ret)=0)
       GO TO exit_program
      ENDIF
    ENDCASE
    IF (size(drl_readmes->readme,5)=0)
     CALL clear(24,1)
     CALL text(24,2,"No readmes found.  Press RETURN to continue...")
     CALL accept(24,50,"P"," ")
    ELSE
     IF (drl_report(drl_group_flag_ret)=0)
      GO TO exit_program
     ENDIF
    ENDIF
   ENDIF
 ENDWHILE
 GO TO exit_program
 SUBROUTINE drl_validate_input(null)
   DECLARE di_failure = vc WITH protect, noconstant("")
   IF (mode=drl_ocd_mode)
    IF ( NOT (ocd))
     SET di_failure = "Please provide a valid OCD number.  Press RETURN to exit..."
    ENDIF
   ENDIF
   IF (validate(ocd,0))
    SET dm_err->eproc = build("Check DM_OCD_REAMDE for READMES on ocd:",ocd)
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    SELECT INTO "nl:"
    ...
    ENDIF
    IF (curqual=0)
     SET di_failure = "This OCD contains no readmes.  Press RETURN to exit..."
    ENDIF
   ENDIF
   IF (size(trim(validate(di_failure,""),3)))
    CALL text(1,1,di_failure)
    CALL accept(1,(size(di_failure)+ 2),"P"," ")
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drl_menu(dm_menu_selection)
   SET dm_err->eproc = "Get the menu selection."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SET dm_menu_selection = 0
   CALL clear(1,1)
   CALL box(1,1,3,132)
   IF (validate(ocd,0))
    CALL text(2,3,build("R E A D M E   L O G   V I E W E R  (Plan ID: ",abs(ocd),")"))
   ELSE
    CALL text(2,3,"R E A D M E   L O G   V I E W E R")
   ENDIF
   CALL text(5,3,"1) View log of all readmes.")
   CALL text(6,3,"2) View log of successful readmes.")
   CALL text(7,3,"3) View log of running readmes.")
   CALL text(8,3,"4) View log of failed readmes.")
   CALL text(9,3,"5) View log of rolled back / uninstalled readmes.")
   CALL text(10,3,"6) View log of parallel readmes.")
   CALL text(12,3,">>> Enter selection ('0' to exit): ")
   CALL accept(12,38,"9",1
    WHERE curaccept IN (0, 1, 2, 3, 4,
    5, 6))
   SET dm_menu_selection = curaccept
   CASE (dm_menu_selection)
    OF 1:
     SET work->low_status = ""
     SET work->high_status = "ZZZZZZZ"
    OF 2:
     SET work->low_status = dm2_rr_done
     SET work->high_status = dm2_rr_done
    OF 3:
     SET work->low_status = dm2_rr_running
     SET work->high_status = dm2_rr_running
    OF 4:
     SET work->low_status = dm2_rr_failed
     SET work->high_status = dm2_rr_failed
    OF 5:
     SET work->low_status = ""
     SET work->high_status = "ZZZZZZZ"
     SET drl_projtype_qual = dm2_rr_readme_rback
   ENDCASE
   SET dm_err->eproc = build("User chose:",dm_menu_selection)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drl_standalone(null)
   DECLARE ds_readme_count = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Standalone readme info gather."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    ...
    PLAN (l
     WHERE (l.environment_id=dm2_rr_misc->env_id)
      AND l.project_type=patstring(drl_projtype_qual)
      AND l.project_name > " "
      AND l.status BETWEEN work->low_status AND work->high_status
      AND l.active_ind=1)
     JOIN (r
     WHERE r.readme_id=cnvtint(l.project_name)
      ...
      AND r.active_ind=1)
    DETAIL
     ds_readme_count = (ds_readme_count+ 1), stat = alterlist(drl_readmes->readme,ds_readme_count),
     drl_readmes->readme[ds_readme_count].readme_id = r.readme_id,
     drl_readmes->readme[ds_readme_count].description = trim(r.description,3), drl_readmes->readme[
     ds_readme_count].execution = r.execution, drl_readmes->readme[ds_readme_count].status = l.status,
     drl_readmes->readme[ds_readme_count].message = trim(l.message,3), drl_readmes->readme[
     ds_readme_count].start_dt_tm = l.start_dt_tm, drl_readmes->readme[ds_readme_count].end_dt_tm = l
     .end_dt_tm,
     drl_readmes->readme[ds_readme_count].estimated_time = l.estimated_time, drl_readmes->readme[
     ds_readme_count].sort = format(r.readme_id,"######;P0"), drl_readmes->readme[ds_readme_count].
     project_type = l.project_type,
     drl_readmes->readme[ds_readme_count].updt_dt_tm = l.updt_dt_tm, drl_readmes->readme[
     ds_readme_count].project_instance = l.project_instance, drl_readmes->readme[ds_readme_count].
     script_name = r.script,
     drl_readmes->readme[ds_readme_count].parent_readme = r.parent_readme_id
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drl_ocd(do_group_ret)
   DECLARE do_readme_count = i2 WITH protect, noconstant(0)
   DECLARE do_pkg_count = i2 WITH protect, noconstant(0)
   DECLARE do_num = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "OCD readme info gather."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SET do_group_ret = 1
   SELECT INTO "nl:"
    ...
    PLAN (l
     ...
     WHERE r.readme_id=cnvtint(l.project_name)
      ...
      AND r.active_ind=1)
    DETAIL
     do_readme_count = (do_readme_count+ 1), stat = alterlist(drl_readmes->readme,do_readme_count),
     drl_readmes->readme[do_readme_count].readme_id = r.readme_id,
     drl_readmes->readme[do_readme_count].description = trim(r.description,3), drl_readmes->readme[
     do_readme_count].execution = r.execution, drl_readmes->readme[do_readme_count].status = l.status,
     drl_readmes->readme[do_readme_count].message = trim(l.message,3), drl_readmes->readme[
     do_readme_count].start_dt_tm = l.start_dt_tm, drl_readmes->readme[do_readme_count].end_dt_tm = l
     .end_dt_tm,
     drl_readmes->readme[do_readme_count].estimated_time = l.estimated_time, drl_readmes->readme[
     do_readme_count].project_type = l.project_type, drl_readmes->readme[do_readme_count].updt_dt_tm
      = l.updt_dt_tm,
     drl_readmes->readme[do_readme_count].project_instance = l.project_instance, drl_readmes->readme[
     do_readme_count].script_name = r.script, drl_readmes->readme[do_readme_count].parent_readme = r
     .parent_readme_id
     IF (findstring("PRE",r.execution))
      drl_readmes->readme[do_readme_count].grouping = "PRE", drl_readmes->readme[do_readme_count].
      sort = "1|"
     ELSE
      drl_readmes->readme[do_readme_count].grouping = "POST", drl_readmes->readme[do_readme_count].
      sort = "2|"
     ENDIF
     CASE (l.status)
      OF dm2_rr_failed:
       drl_readmes->readme[do_readme_count].sort = concat(drl_readmes->readme[do_readme_count].sort,
        "1")
      OF dm2_rr_running:
       drl_readmes->readme[do_readme_count].sort = concat(drl_readmes->readme[do_readme_count].sort,
        "2")
   ...
   ENDIF
   IF (ocd < 0)
    SET dm_err->eproc = "Retrieving package numbers associated with readme ids"
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    SELECT INTO "nl:"
     ...
     (SELECT
      x.package_number
      FROM dm_install_plan x
      WHERE (x.install_plan_id=- (ocd))))
      AND l.status BETWEEN "" AND "ZZZZZZZ"
      AND l.active_ind=1
      AND r.readme_id=cnvtint(l.project_name)
      AND r.instance=l.project_instance
      AND r.active_ind=1
     ORDER BY r.readme_id, l.ocd DESC
     HEAD r.readme_id
      do_pkg_count = 0, do_num = locateval(do_num,1,size(drl_readmes->readme,5),r.readme_id,
       drl_readmes->readme[do_num].readme_id)
     DETAIL
      IF (do_num > 0)
       do_pkg_count = (do_pkg_count+ 1), stat = alterlist(drl_readmes->readme[do_num].package_nbrs,
        do_pkg_count), drl_readmes->readme[do_num].package_nbrs[do_pkg_count].package_nbr = l.ocd
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
   ...
   ENDIF
   IF ((dm_err->debug_flag > 1))
    CALL echorecord(drl_readmes)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drl_report(dr_group_flag)
   DECLARE dr_total_time = f8 WITH protect, noconstant(0.0)
   DECLARE dr_first_readme = i2 WITH protect, noconstant(1)
   DECLARE dr_package_nbrs = vc WITH protect, noconstant("")
   DECLARE dr_count = i4 WITH protect, noconstant(0)
   SELECT
    grouping = drl_readmes->readme[d.seq].grouping
    FROM (dummyt d  WITH seq = value(size(drl_readmes->readme,5)))
    PLAN (d)
    ORDER BY drl_readmes->readme[d.seq].sort
    HEAD REPORT
     IF ( NOT (dr_group_flag))
      CALL print("****************************************************************"), row + 1,
      CALL print("  README LOG                                                    "),
      row + 1,
      CALL print("****************************************************************"), row + 1
     ENDIF
    HEAD grouping
     IF (dr_group_flag)
      CALL print("****************************************************************"), row + 1,
      CALL print(concat(" ",trim(grouping,3)," READMES")),
      row + 1,
      CALL print("****************************************************************"), row + 1
     ENDIF
     dr_total_time = 0.0, dr_first_readme = 1
    DETAIL
     IF (dr_first_readme)
      dr_first_readme = 0
     ELSE
     ...
     ENDIF
     row + 1
     IF ((drl_readmes->readme[d.seq].project_type=dm2_rr_readme_rback))
      CALL print(concat(" Readme ID [",trim(cnvtstring(drl_readmes->readme[d.seq].readme_id),3),
       "]  {Rolled back/Uninstalled}"))
     ELSE
      CALL print(concat(" Readme ID [",trim(cnvtstring(drl_readmes->readme[d.seq].readme_id),3),"]"))
     ENDIF
     row + 2,
     CALL print(concat("     Description: '",drl_readmes->readme[d.seq].description,"'")), row + 1,
     CALL print(build("        Instance: '",drl_readmes->readme[d.seq].project_instance,"'")), row +
     2,
     CALL print(concat("          Status: '",drl_readmes->readme[d.seq].status,"'")),
     row + 1,
     CALL print(concat("          Script: '",drl_readmes->readme[d.seq].script_name,"'")), row + 1,
     CALL print(concat("          Parent Readme: '",evaluate(drl_readmes->readme[d.seq].parent_readme,
       0,"n/a",trim(cnvtstring(drl_readmes->readme[d.seq].parent_readme),3)),"'")), row + 1
     IF (size(drl_readmes->readme[d.seq].package_nbrs,5) > 0)
      FOR (dr_count = 1 TO size(drl_readmes->readme[d.seq].package_nbrs,5))
       dr_package_nbrs = concat(dr_package_nbrs,trim(cnvtstring(drl_readmes->readme[d.seq].
          package_nbrs[dr_count].package_nbr),3),","),
       IF (dr_count=10)
        dr_count = size(drl_readmes->readme[d.seq].package_nbrs,5)
       ENDIF
      ENDFOR
      IF (size(drl_readmes->readme[d.seq].package_nbrs,5) > 10)
       dr_package_nbrs = replace(dr_package_nbrs,",",",...",2)
      ELSE
       dr_package_nbrs = replace(dr_package_nbrs,",","",2)
      ENDIF
      CALL print(concat("          Package Number: '",trim(dr_package_nbrs,3),"'")), row + 1,
      dr_package_nbrs = ""
     ENDIF
     CALL print(concat("         Message: '",drl_readmes->readme[d.seq].message,"'")), row + 2,
     CALL print(concat("         Started: ",format(drl_readmes->readme[d.seq].start_dt_tm,";;q"))),
     row + 1,
     CALL print(concat("        Finished: ",format(drl_readmes->readme[d.seq].end_dt_tm,";;q"))), row
      + 1
     IF ((drl_readmes->readme[d.seq].project_type=dm2_rr_readme_rback))
      CALL print(concat("  Rollback Dt/Tm: ",format(drl_readmes->readme[d.seq].updt_dt_tm,";;q"))),
      row + 1
     ENDIF
     CALL print(concat("  Estimated Time: ",format((drl_readmes->readme[d.seq].estimated_time/ ((24.0
        * 60.0) * 60.0)),"DD.HH:MM:SS;3;z"))), row + 1,
     CALL print(concat("     Actual Time: ",format(datetimediff(drl_readmes->readme[d.seq].end_dt_tm,
        drl_readmes->readme[d.seq].start_dt_tm),"DD.HH:MM:SS;3;z"))),
     row + 2
     IF ((drl_readmes->readme[d.seq].status=dm2_rr_done))
      dr_total_time = (dr_total_time+ datetimediff(drl_readmes->readme[d.seq].end_dt_tm,drl_readmes->
       readme[d.seq].start_dt_tm))
     ENDIF
    FOOT  grouping
     IF (dr_group_flag)
      CALL print(concat(" >>> ",trim(grouping,3)," READMES - Total Time: ",format(dr_total_time,
        "DD.HH:MM:SS;3;z"))), row + 2
     ENDIF
    FOOT REPORT
     row + 1,
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Ending DM_README_LOG"
 ENDIF
 CALL final_disp_msg("dm_readme_log")
 IF ((dm_err->debug_flag=0))
  SET message = nowindow
  CALL clear(1,1)
 ENDIF
