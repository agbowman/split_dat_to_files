CREATE PROGRAM dm_si_haad_file_poll_rows:dba
 ...
 SET reply->rows_between_commit = 200
 SET i18nhandle = 0
  ...
  AND v_rows_left > 0)
   SELECT DISTINCT INTO "nl:"
    ...
    FROM si_haad_file_poll fp,
     ...
     si_haad_transaction ht
    PLAN (fp
     WHERE fp.si_haad_file_poll_id > 0
      AND fp.si_haad_file_poll_id BETWEEN v_cur_min_id AND v_cur_max_id)
     JOIN (fd
     WHERE fd.si_haad_file_poll_id=fp.si_haad_file_poll_id)
     JOIN (ht
     WHERE ht.si_haad_file_download_id=fd.si_haad_file_download_id
      AND parser(sbr_getrowidnotexists(
       "ht.completed_dt_tm < cnvtdatetime(curdate - v_days_to_keep,curtime3)","ht")))
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
