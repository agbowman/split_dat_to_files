CREATE PROGRAM dm2_verify_data_genviews
 ...
 SUBROUTINE dvdr_get_verify2(null)
  ...
  RETURN(1)
 END ;Subroutine
 DECLARE dgvv_cnt = i4 WITH protect, noconstant(0)
 DECLARE dgvv_full_tname = vc WITH protect, noconstant("")
 DECLARE dgvv_cur_vname = vc WITH protect, noconstant("")
 DECLARE dgvv_object_id_str = vc WITH protect, noconstant("")
 IF (dm2_prg_maint("BEGIN")=0)
  GO TO exit_program
 ENDIF
 ...
 IF ((dmvd->use_verify2_ind=1))
  EXECUTE dm2_verify_data_genviews2  $1
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Validating inputs for dm2_verify_data_genviews"
 CALL disp_msg(dm_err->eproc,dm_err->logfile,10)
 SET dgvv_cnt =  $1
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->emsg = "Parameter usage: dm2_verify_data_genviews <index in dmvd->tbls[]>"
  GO TO exit_script
 ENDIF
 IF (((dgvv_cnt < 1) OR ((dgvv_cnt > dmvd->tbl_cnt))) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Input parameter (index to dmvd->tbls) is not in the valid range"
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Generating verify data views for ",dmvd->tbls[dgvv_cnt].owner,".",dmvd->
  tbls[dgvv_cnt].table_name)
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 SET dgvv_object_id_str = cnvtstring(dmvd->tbls[dgvv_cnt].src_object_id)
 SET dgvv_full_tname = concat(dmvd->tbls[dgvv_cnt].owner,".",dmvd->tbls[dgvv_cnt].table_name)
 SET dm_err->user_action = concat("The above ORA error can be ignored. The table ",dgvv_full_tname,
  " has been excluded.")
 SET dgvv_cur_vname = concat("dm2vdat",dgvv_object_id_str,"1")
 SET dm_err->eproc = concat("Creating ",dgvv_cur_vname)
 CALL dm2_push_cmd(concat("rdb asis(^CREATE OR REPLACE VIEW ",dgvv_cur_vname," AS ^)"),0)
 IF ((dmvd->tbls[dgvv_cnt].scui_ind IN (1, 2)))
  CALL dm2_push_cmd(concat("asis(^SELECT v.",dmvd->tbls[dgvv_cnt].keyid_col_name," AS UNIQUE_ID ^)"),
   0)
 ELSEIF ((dmvd->tbls[dgvv_cnt].scui_ind=3))
  CALL dm2_push_cmd(concat("asis(^SELECT count(v.",dmvd->tbls[dgvv_cnt].keyid_col_name,
    ") AS UNIQUE_ID ^)"),0)
 ELSE
  CALL dm2_push_cmd(concat("asis(^SELECT count(*) AS UNIQUE_ID ^)"),0)
 ENDIF
 CALL dm2_push_cmd(asis(" , 1  AS COMP_TYPE "),0)
 CALL dm2_push_cmd(concat("asis(^FROM (^)"),0)
 CALL dm2_push_cmd(concat("asis(^SELECT ",dmvd->tbls[dgvv_cnt].col_list," FROM ",dgvv_full_tname,"@",
   dmvd->db_link,"^)"),0)
 CALL dm2_push_cmd("asis(^MINUS^)",0)
 CALL dm2_push_cmd(concat("asis(^SELECT ",dmvd->tbls[dgvv_cnt].col_list," FROM ",dgvv_full_tname,"^)"
   ),0)
 CALL dm2_push_cmd(asis(")v"),0)
 IF ((dmvd->tgt_to_src=1))
  CALL dm2_push_cmd("asis(^UNION^)",0)
  IF ((dmvd->tbls[dgvv_cnt].scui_ind IN (1, 2)))
   CALL dm2_push_cmd(concat("asis(^SELECT z.",dmvd->tbls[dgvv_cnt].keyid_col_name," AS UNIQUE_ID ^)"),
    0)
  ELSEIF ((dmvd->tbls[dgvv_cnt].scui_ind=3))
   CALL dm2_push_cmd(concat("asis(^SELECT count(z.",dmvd->tbls[dgvv_cnt].keyid_col_name,
     ") AS UNIQUE_ID ^)"),0)
  ELSE
   CALL dm2_push_cmd(concat("asis(^SELECT count(*) AS UNIQUE_ID ^)"),0)
  ENDIF
  CALL dm2_push_cmd(asis(" , 2  AS COMP_TYPE "),0)
  CALL dm2_push_cmd(concat("asis(^FROM (^)"),0)
  CALL dm2_push_cmd(concat("asis(^SELECT ",dmvd->tbls[dgvv_cnt].col_list," FROM ",dgvv_full_tname,
    "^)"),0)
  CALL dm2_push_cmd("asis(^MINUS^)",0)
  CALL dm2_push_cmd(concat("asis(^SELECT ",dmvd->tbls[dgvv_cnt].col_list," FROM ",dgvv_full_tname,"@",
    dmvd->db_link,"^)"),0)
  CALL dm2_push_cmd(asis(")z"),0)
 ENDIF
 CALL dm2_push_cmd(asis("go"),1)
 IF (check_error(dm_err->eproc) != 0)
  IF ((dmvd->tbls[dgvv_cnt].excl_flag != 50))
   SET dmvd->tbls[dgvv_cnt].excl_flag = 80
   SET dmvd->tbls[dgvv_cnt].excl_reason = concat("View Generation Error -- ",dm_err->emsg)
  ENDIF
  SET dm_err->err_ind = 0
  SET dm_err->user_action = concat("The above ORA error can be ignored. The table ",dgvv_full_tname,
   " has been excluded.")
 ENDIF
 SET dgvv_cur_vname = concat("dm2vdats",dgvv_object_id_str)
 SET dm_err->eproc = concat("Creating ",dgvv_cur_vname)
 CALL dm2_push_cmd(concat("rdb asis(^CREATE OR REPLACE VIEW ",dgvv_cur_vname," AS ^)"),0)
 CALL dm2_push_cmd(concat("asis(^ SELECT 'SRC' AS DB,",dmvd->tbls[dgvv_cnt].full_col_list," FROM ",
   dgvv_full_tname,"@",
   dmvd->db_link,"^) go"),1)
 IF (check_error(dm_err->eproc) != 0)
  IF ((dmvd->tbls[dgvv_cnt].excl_flag != 50))
  ...
  SET dm_err->user_action = concat("The above ORA error can be ignored. The table ",dgvv_full_tname,
   " has been excluded")
 ENDIF
 SET dgvv_cur_vname = concat("dm2vdatt",dgvv_object_id_str)
 SET dm_err->eproc = concat("Creating ",dgvv_cur_vname)
 CALL dm2_push_cmd(concat("rdb asis(^CREATE OR REPLACE VIEW ",dgvv_cur_vname," AS ^)"),0)
 CALL dm2_push_cmd(concat("asis(^ SELECT 'TGT' AS DB,",dmvd->tbls[dgvv_cnt].full_col_list," FROM ",
   dgvv_full_tname,"^) go"),1)
 IF (check_error(dm_err->eproc) != 0)
  IF ((dmvd->tbls[dgvv_cnt].excl_flag != 50))
   SET dmvd->tbls[dgvv_cnt].excl_flag = 80
   SET dmvd->tbls[dgvv_cnt].excl_reason = concat("View Generation Error -- ",dm_err->emsg)
  ...
  SET dm_err->user_action = concat("The above ORA error can be ignored. The table ",dgvv_full_tname,
   " has been excluded")
 ENDIF
 SET dgvv_cur_vname = concat("dm2vdatu",dgvv_object_id_str)
 SET dm_err->eproc = concat("Creating ",dgvv_cur_vname)
 CALL dm2_push_cmd(concat("rdb asis(^CREATE OR REPLACE VIEW ",dgvv_cur_vname," AS ^)"),0)
 CALL dm2_push_cmd(concat("asis(^ SELECT 'SRC' AS SRC, A.* FROM ",concat("dm2vdats",
    dgvv_object_id_str)," A"," UNION ALL SELECT 'TGT' AS TGT, B.* FROM ",concat("dm2vdatt",
    dgvv_object_id_str),
   " B^) go"),1)
 IF (check_error(dm_err->eproc) != 0)
  IF ((dmvd->tbls[dgvv_cnt].excl_flag != 50))
...
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_verify_data_genviews completed succesfully."
 ELSE
  CALL disp_msg(dm_err->emsg,dm_err->logfile,10)
 ENDIF
 SET dm_err->user_action = "NONE"
