CREATE PROGRAM aps_get_groups_prefixes:dba
...
#script
 ...
 SELECT INTO "nl:"
  pg.group_id, prefix_cd = decode(ap.seq,ap.prefix_id,0.0), ap.prefix_id
  ...
  ORDER BY pg.group_id, prefix_cd
  HEAD REPORT
   group_cnt = 0, pre_cnt = 0
  HEAD pg.group_id
   pre_cnt = 0, group_cnt += 1, stat = alterlist(reply->group_qual,group_cnt),
   reply->group_qual[group_cnt].site_cd = pg.site_cd, reply->group_qual[group_cnt].group_cd = pg
   .group_id, reply->group_qual[group_cnt].group_name = pg.group_name,
   reply->group_qual[group_cnt].group_desc = pg.group_desc, reply->group_qual[group_cnt].active_ind
    = pg.active_ind
  HEAD prefix_cd
   IF (prefix_cd != 0.0)
    service_resource_cd = ap.service_resource_cd
    IF (isresourceviewable(service_resource_cd)=true)
     pre_tag_cnt = 0, pre_cnt += 1, stat = alterlist(reply->group_qual[group_cnt].prefix_qual,pre_cnt
      ),
     reply->group_qual[group_cnt].prefix_qual[pre_cnt].site_cd = ap.site_cd, reply->group_qual[
     group_cnt].prefix_qual[pre_cnt].prefix_cd = ap.prefix_id, reply->group_qual[group_cnt].
     prefix_qual[pre_cnt].prefix_desc = ap.prefix_desc,
     reply->group_qual[group_cnt].prefix_qual[pre_cnt].prefix_name = ap.prefix_name, reply->
     group_qual[group_cnt].prefix_qual[pre_cnt].active_ind = ap.active_ind, reply->group_qual[
     group_cnt].prefix_qual[pre_cnt].case_type_cd = ap.case_type_cd,
     reply->group_qual[group_cnt].pre_cnt = pre_cnt
    ENDIF
   ENDIF
  WITH nocounter
 ;end select
 IF (curqual=0)
  ...
  SET reply->status_data.subeventstatus[1].targetobjectvalue = "GROUPS"
  SET failed = "T"
