CREATE PROGRAM ct_credit_profit_charge:dba
 SET ct_credit_profit_charge_version = "646960.FT.008"
 CALL echo("executing ct_credit_profit_charge")
 ...
 FREE SET temprequestinterfaced
 RECORD temprequestinterfaced(
   1 interface_charge[*]
     2 charge_item_id = f8
 )
 FREE SET tempreply
 ...
 SET cdf_meaning = "CR"
 SET cnt = 1
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,cnt,credittypecd)
 DECLARE chargeitemid = f8
 SET chargeitemid = cnvtreal( $1)
 CALL echo(build("CT_CREDIT_PROFIT_CHARGE::getting current record for charge_item_id: ",chargeitemid)
  )
 SELECT INTO "nl:"
  FROM charge c
  ...
  DETAIL
   ...
   tmp_charge_struct->process_flg = c.process_flg, tmp_charge_struct->service_dt_tm = cnvtdatetime(c
    .service_dt_tm), tmp_charge_struct->activity_dt_tm = cnvtdatetime(c.activity_dt_tm),
   tmp_charge_struct->updt_cnt = c.updt_cnt, tmp_charge_struct->updt_dt_tm = cnvtdatetime(c
    .updt_dt_tm), tmp_charge_struct->updt_id = c.updt_id,
   tmp_charge_struct->updt_task = c.updt_task, tmp_charge_struct->updt_applctx = c.updt_applctx,
   tmp_charge_struct->active_ind = c.active_ind,
   tmp_charge_struct->active_status_cd = c.active_status_cd, tmp_charge_struct->active_status_dt_tm
    = cnvtdatetime(c.active_status_dt_tm), tmp_charge_struct->active_status_prsnl_id = c
   .active_status_prsnl_id,
   tmp_charge_struct->beg_effective_dt_tm = cnvtdatetime(c.beg_effective_dt_tm), tmp_charge_struct->
   end_effective_dt_tm = cnvtdatetime(c.end_effective_dt_tm), tmp_charge_struct->credited_dt_tm =
   cnvtdatetime(c.credited_dt_tm),
   tmp_charge_struct->adjusted_dt_tm = cnvtdatetime(c.adjusted_dt_tm), tmp_charge_struct->
   interface_file_id = c.interface_file_id, tmp_charge_struct->tier_group_cd = c.tier_group_cd,
   tmp_charge_struct->def_bill_item_id = c.def_bill_item_id, tmp_charge_struct->verify_phys_id = c
   .verify_phys_id, tmp_charge_struct->gross_price = c.gross_price,
   tmp_charge_struct->discount_amount = c.discount_amount, tmp_charge_struct->manual_ind = c
   .manual_ind, tmp_charge_struct->combine_ind = c.combine_ind,
   tmp_charge_struct->activity_type_cd = c.activity_type_cd, tmp_charge_struct->activity_sub_type_cd
    = c.activity_sub_type_cd, tmp_charge_struct->provider_specialty_cd = c.provider_specialty_cd,
   tmp_charge_struct->admit_type_cd = c.admit_type_cd, tmp_charge_struct->bundle_id = c.bundle_id,
   tmp_charge_struct->department_cd = c.department_cd,
   tmp_charge_struct->institution_cd = c.institution_cd, tmp_charge_struct->section_cd = c.section_cd,
   tmp_charge_struct->subsection_cd = c.subsection_cd,
   tmp_charge_struct->level5_cd = c.level5_cd, tmp_charge_struct->med_service_cd = c.med_service_cd,
   tmp_charge_struct->abn_status_cd = c.abn_status_cd,
   tmp_charge_struct->cost_center_cd = c.cost_center_cd, tmp_charge_struct->inst_fin_nbr = c
   .inst_fin_nbr, tmp_charge_struct->fin_class_cd = c.fin_class_cd,
   tmp_charge_struct->health_plan_id = c.health_plan_id, tmp_charge_struct->item_interval_id = c
   .item_interval_id, tmp_charge_struct->item_list_price = c.item_list_price,
   tmp_charge_struct->item_reimbursement = c.item_reimbursement, tmp_charge_struct->
   list_price_sched_id = c.list_price_sched_id, tmp_charge_struct->payor_type_cd = c.payor_type_cd,
   tmp_charge_struct->epsdt_ind = c.epsdt_ind, tmp_charge_struct->ref_phys_id = c.ref_phys_id,
   tmp_charge_struct->start_dt_tm = cnvtdatetime(c.start_dt_tm),
   tmp_charge_struct->stop_dt_tm = cnvtdatetime(c.stop_dt_tm), tmp_charge_struct->alpha_nomen_id = c
   .alpha_nomen_id, tmp_charge_struct->offset_charge_item_id = c.offset_charge_item_id,
   tmp_charge_struct->item_deductible_amt = c.item_deductible_amt, tmp_charge_struct->
   patient_responsibility_flag = c.patient_responsibility_flag, tmp_charge_struct->original_org_id =
   c.original_org_id
  WITH nocounter
 ;end select
 CALL echo(build("CT_CREDIT_PROFIT_CHARGE::getting current mod record for charge_item_id: ",
   chargeitemid))
 SET count1 = 0
 SELECT INTO "nl:"
  FROM charge_mod cm
  WHERE cm.charge_item_id=chargeitemid
  DETAIL
 ...
 CALL echo(build("CT_CREDIT_PROFIT_CHARGE::adding new record from charge_item_id: ",chargeitemid))
 SET new_nbr = 0.0
 SELECT INTO "nl:"
 ...
 INSERT  FROM charge c
  SET c.charge_item_id = new_nbr, c.parent_charge_item_id = chargeitemid, c.charge_event_act_id =
   tmp_charge_struct->charge_event_act_id,
   ...
   c.item_quantity = tmp_charge_struct->item_quantity, c.item_price = (- (1) * tmp_charge_struct->
   item_price), c.item_extended_price = ((- (1) * tmp_charge_struct->item_price) * tmp_charge_struct
   ->item_quantity),
   c.item_allowable = tmp_charge_struct->item_allowable, c.item_copay = tmp_charge_struct->item_copay,
   c.charge_type_cd = credittypecd,
   ...
   .end_effective_dt_tm = cnvtdatetime(tmp_charge_struct->end_effective_dt_tm), c.credited_dt_tm =
   ...
   c.offset_charge_item_id = chargeitemid, c.original_org_id = tmp_charge_struct->original_org_id, c
   .original_encntr_id =
   IF (validate(chargeitemid,0)=0) tmp_charge_struct->encntr_id
   ELSE
    (SELECT
 ...
 ;end insert
 IF (curqual > 0)
  CALL echo(build("CT_CREDIT_PROFIT_CHARGE::new charge created: ",new_nbr))
  SET reply_credit->new_charge_item_id = new_nbr
  UPDATE  FROM charge c
   SET c.updt_cnt = (c.updt_cnt+ 1), c.updt_dt_tm = cnvtdatetime(sysdate), c.updt_id = reqinfo->
    updt_id,
    c.updt_applctx = reqinfo->updt_applctx, c.updt_task = reqinfo->updt_task, c.offset_charge_item_id
     = new_nbr
   WHERE c.charge_item_id=chargeitemid
   WITH nocounter
  ;end update
  ...
  ENDIF
 ENDIF
 CALL echo(build("CT_CREDIT_PROFIT_CHARGE::adding new mod record for new charge_item_id: ",new_nbr))
 FOR (cm_count = 1 TO tmp_charge_mod_struct->charge_mod_qual)
   ...
   INSERT  FROM charge_mod cm
    SET cm.charge_mod_id = new_mod_nbr, cm.charge_item_id = new_nbr, cm.charge_mod_type_cd =
     ...
     cm.field4_id = chargemodruleid_for_credits, cm.field5_id = tmp_charge_mod_struct->charge_mod[
     cm_count].field5_id
    WITH nocounter
   ;end insert
   IF (curqual > 0)
    SET reply_credit->charge_mod_qual = tmp_charge_mod_struct->charge_mod_qual
    SET stat = alterlist(reply_credit->charge_mods,cm_count)
    SET reply_credit->charge_mods[cm_count].charge_mod_id = new_mod_nbr
   ENDIF
 ENDFOR
 IF ((tmp_charge_struct->process_flg=100))
  SET stat = alterlist(temprequest->charges,1)
  SET temprequest->charges[1].charge_item_id = new_nbr
  CALL echo(build("CT_CREDIT_PROFIT_CHARGE::notifying Profit of newly created charge: ",new_nbr))
  EXECUTE pft_nt_chrg_billing  WITH replace("REQUEST",temprequest), replace("REPLY",tempreply)
  CALL echo("CT_CREDIT_PROFIT_CHARGE::back from call to Profit")
 ELSE
  SET nrealtimeind = 0
  SELECT INTO "nl:"
   FROM charge c,
  ...
  ;end select
  IF (nrealtimeind=1)
   SET stat = alterlist(temprequestinterfaced->interface_charge,1)
   SET temprequestinterfaced->interface_charge[1].charge_item_id = new_nbr
   EXECUTE afc_post_interface_charge  WITH replace("REQUEST",temprequestinterfaced)
   CALL echo("CT_CREDIT_PROFIT_CHARGE::back from call to interface charge")
  ENDIF
 ENDIF
END GO
