CREATE PROGRAM bbd_chg_contact_type:dba
 RECORD reply(
   1 qual[1]
     2 code_value = f8
     2 disp_key = c40
 ...
 SET failed = "F"
 SELECT INTO "nl:"
  cvs.display_key_dup_ind, cvs.cdf_meaning_dup_ind, cvs.primary_ind_dup_ind,
  cvs.active_ind_dup_ind, cvs.display_dup_ind, cvs.chg_access_ind
  ...
  DETAIL
   ...
   primary_ind_dup_ind = cvs.primary_ind_dup_ind,
   active_ind_dup_ind = cvs.active_ind_dup_ind, display_dup_ind = cvs.display_dup_ind, chg_access_ind
    = cvs.chg_access_ind
  WITH nocounter
 ;end select
#start_loop
 FOR (x = x TO number_to_change)
   ...
   IF (curqual > 0)
    ...
    IF ((new_code_value != request->qual[x].code_value))
     ...
     SET reply->qual[x].disp_key = request->qual[x].display_key
     SET reply->qual[x].recstatus = "X"
     GO TO next_code
    ENDIF
   ENDIF
   SELECT INTO "nl:"
   ...
   UPDATE  FROM code_value c
    SET c.display = request->qual[x].display, c.display_key = trim(cnvtupper(cnvtalphanum(request->
     ...
     c.definition = request->qual[x].definition, c.updt_id = reqinfo->updt_id, c.updt_applctx =
