CREATE PROGRAM bbt_rpt_abo_only:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 SET captions->abo_rh = uar_i18ngetmessage(i18nhandle,"abo_rh","ABO/Rh BUILDING TOOL")
 SET captions->abo_groups = uar_i18ngetmessage(i18nhandle,"abo_groups","ABO GROUPS ONLY")
 ...
 SET select_ok_ind = 0
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_abo_only", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  ...
  PLAN (cv
   WHERE cv.code_set=1641)
   JOIN (cve
   WHERE cv.code_set=cve.code_set
  ...
  HEAD PAGE
   ...
   col 52, captions->abo_groups, row + 2,
