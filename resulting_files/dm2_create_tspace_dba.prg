CREATE PROGRAM dm2_create_tspace:dba
 DECLARE dm2_mod = vc WITH private, constant("000")
 DECLARE cdb2_db = vc WITH protect, constant("DB2UDB")
 DECLARE coracle_db = vc WITH public, constant("ORACLE")
 DECLARE csqlsrv_db = vc WITH public, constant("SQLSRV")
 DECLARE caix_os = vc WITH public, constant("AIX")
 DECLARE cvms_os = vc WITH public, constant("AXP")
 DECLARE chpx_os = vc WITH public, constant("HPX")
 DECLARE cdevice_name_prefix = vc WITH public
 DECLARE cpushcmd = vc WITH public, constant("PUSHCMD")
 DECLARE cpushdcl = vc WITH public, constant("PUSHDCL")
 DECLARE tspacex = i4 WITH protect, noconstant(0)
 DECLARE filename_str = vc WITH private, noconstant(" ")
 DECLARE tspacex_max = i4 WITH protect, noconstant(0)
 DECLARE bkoutx = i4 WITH protect, noconstant(0)
 DECLARE physical_device_name = vc WITH protect, noconstant(" ")
 DECLARE logical_device_name = vc WITH protect, noconstant(" ")
 DECLARE contx = i4 WITH protect, noconstant(0)
 DECLARE ee_cont_ndx_in = i4 WITH protect, noconstant(0)
 DECLARE ddl_cmd_cnt = i4 WITH protect, noconstant(0)
 DECLARE ddl_os_cmd_cnt = i4 WITH protect, noconstant(0)
 DECLARE cmd_ndx_hold = i4 WITH protect, noconstant(0)
 DECLARE dct_ts_size_exists = i2 WITH protect, noconstant(0)
 DECLARE ct_update_tspace_size = i2 WITH protect, noconstant(0)
 DECLARE dct_from_backout = i2 WITH protect, noconstant(0)
 DECLARE dct_obj_size_exists = i2 WITH protect, noconstant(0)
 DECLARE dct_gen_id = f8 WITH protect, noconstant(0.0)
 DECLARE dct_ksh_file = vc WITH protect, noconstant("")
 DECLARE dct_signal_failure = i2 WITH protect, noconstant(0)
 DECLARE dct_cmd_str = vc WITH protect, noconstant("")
 DECLARE dct_sub_amount = i4 WITH protect, noconstant(1)
 DECLARE dct_default_ts = vc WITH protect, noconstant("MISC")
 DECLARE dct_dm_info_exists = i2 WITH protect, noconstant(0)
 SET cmd_ndx_hold = 0
 DECLARE existing_tablespace_work(null) = i2
 DECLARE dct_parse_to_logfile(null) = i2
 DECLARE create_new_tablespace() = i2
 DECLARE create_new_device(l_device_name=vc,p_device_name=vc,volume_name=vc,disk_name=vc,partitions=
  f8,
  cnd_mwc_in=i2) = i2
 DECLARE resize_device(rd_l_device_name=vc,rd_p_device_name=vc,rd_volume_name=vc,rd_disk_name=vc,
  rd_partitions=f8) = i2
 DECLARE create_tablespace(tspace_name=vc,container_statement=vc) = i2
 DECLARE dct_create_asm_tspace(dcat_ndx=i4) = i2
 DECLARE format_device_name(dbname=vc,tspace_name=vc,container=i4,name_in=vc) = i2
 DECLARE execute_command(command=vc,type=vc,add_to_list=i2,ec_lv_chk=i2,ec_lv_file=vc) = i2
 DECLARE convert_tspace_name(tspace_name=vc) = vc
 DECLARE format_backout_command(backout_cmd=vc,cmd_type=vc) = null
 DECLARE backout_commands() = null
 DECLARE update_tmp_tablespace(tablespace_tbl_name=vc,tablespace_name=vc,tablespace_status=vc,
  tablespace_emsg=vc) = i2
 DECLARE add_sql_file_group() = i2
 DECLARE verify_filename(vf_filename_str=vc,vf_tspace_name=vc,vf_container_ndx=i4) = i2
 DECLARE dct_create_ddl_file(null) = i2
 FREE RECORD bkout
 RECORD bkout(
   1 commands[*]
     2 cmd = vc
 ...
 SUBROUTINE dtr_load_clin_tspaces(null)
   ...
   RETURN(1)
 END ;Subroutine
 CALL check_logfile("dm2_create_tspace","log","dm2_create_tspace")
 SET dm_err->eproc = "Starting dm2_create_tspace"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 IF ((rtspace->mode="CREATE_DDL_ONLY"))
  IF ( NOT (get_unique_file("dm2_tspace_ddl_",".rpt")))
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO end_program
  ELSE
   SET rtspace->ddl_report_fname = dm_err->unique_fname
  ENDIF
 ENDIF
 IF ((dm2_sys_misc->cur_db_os=caix_os))
  SET cdevice_name_prefix = "/dev/r"
 ELSEIF ((dm2_sys_misc->cur_db_os=cvms_os))
  SET cdevice_name_prefix = concat("[",trim(currdbuser),".DB_",trim(rtspace->dbname),"]")
 ENDIF
 IF ((rtspace->install_type="MANUAL")
  AND (rtspace->install_type_value="DM2NOTSET"))
  IF (d2tr_get_man_inst_type_val(null)=0)
   GO TO end_program
  ENDIF
 ENDIF
 IF ((dir_storage_misc->tgt_storage_type != "ASM"))
  SET dm_err->eproc = "Check DM2_TSPACE_SIZE table existence"
  IF (dm2_table_and_ccldef_exists("DM2_TSPACE_SIZE1",dct_ts_size_exists)=0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO end_program
  ELSEIF (dct_ts_size_exists=0)
   SET dm_err->emsg = "DM2_TSPACE_SIZE table does not exist."
   SET dm_err->user_action = "No action needed."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  ENDIF
 ENDIF
 SET dm_err->eproc = "Check DM2_TSPACE_OBJ_SIZE table existance"
 IF (dm2_table_and_ccldef_exists("DM2_TSPACE_OBJ_SIZE1",dct_obj_size_exists)=0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 IF (dct_obj_size_exists=1
  AND (rtspace->install_type IN ("PACKAGE ", "SCHEMA_DATE")))
  SET dm_err->eproc = concat("Determine max gen_id from dm2_tspace_obj_size table for ",rtspace->
   install_type," ",rtspace->install_type_value)
  SELECT INTO "nl:"
   max_gen_id = max(s.gen_id)
   FROM dm2_tspace_obj_size s
   WHERE s.install_type=cnvtupper(rtspace->install_type)
    AND s.install_type_value=cnvtupper(rtspace->install_type_value)
   FOOT REPORT
    dct_gen_id = max_gen_id
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 IF (currdb=csqlsrv_db)
  SET tspacex = 1
  SET tspacex_max = size(rtspace->qual,5)
  FOR (tspacex = 1 TO tspacex_max)
    IF ((rtspace->qual[tspacex].new_ind=1))
     IF (add_sql_file_group(null)=false)
      IF (bkoutx > 0)
       SET dm_err->err_ind = 0
       CALL backout_commands(null)
       SET dm_err->err_ind = 1
      ENDIF
     ELSE
      SET bkoutx = 0
      SET stat = alterlist(bkout->commands,bkoutx)
     ENDIF
    ENDIF
  ENDFOR
 ELSEIF (currdb=cdb2_db)
  SET tspacex = 1
  SET tspacex_max = size(rtspace->qual,5)
  WHILE (tspacex <= tspacex_max)
   IF ((rtspace->qual[tspacex].new_ind=1))
    IF (create_new_tablespace(null)=false)
     SET dm_err->err_ind = 0
     CALL update_tmp_tablespace(rtspace->tmp_table_name,rtspace->qual[tspacex].tspace_name,"ERROR",
      dm_err->emsg)
     SET rtspace->qual[tspacex].ct_err_msg = concat(rtspace->qual[tspacex].ct_err_msg,dm_err->emsg)
     SET rtspace->qual[tspacex].ct_err_ind = 1
     IF (bkoutx > 0)
      CALL backout_commands(null)
     ENDIF
     SET dm_err->err_ind = 1
    ...
    ENDIF
   ELSE
    CALL echo("Extend Existing Tablespace is Future Functionality")
   ENDIF
   SET tspacex = (tspacex+ 1)
  ENDWHILE
 ELSEIF (currdb=coracle_db)
  SET tspacex = 1
  SET tspacex_max = size(rtspace->qual,5)
  WHILE (tspacex <= tspacex_max)
    SET stat = alterlist(rtspace->qual[tspacex].commands,0)
    IF ((rtspace->qual[tspacex].cont_complete_ind=1))
     IF ((rtspace->qual[tspacex].ct_err_ind=1))
      SET dm_err->eproc = concat("Working on: ",rtspace->qual[tspacex].tspace_name)
      CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
      IF ((dir_storage_misc->tgt_storage_type="ASM"))
       IF (dct_create_asm_tspace(tspacex)=0)
        SET dm_err->err_ind = 0
        CALL update_tmp_tablespace(rtspace->tmp_table_name,rtspace->qual[tspacex].tspace_name,"ERROR",
         dm_err->emsg)
        SET rtspace->qual[tspacex].ct_err_msg = concat(rtspace->qual[tspacex].ct_err_msg,dm_err->emsg
      ...
      ELSE
       IF ((rtspace->qual[tspacex].new_ind=1))
        IF (create_new_tablespace(null)=false)
         SET dm_err->err_ind = 0
         CALL update_tmp_tablespace(rtspace->tmp_table_name,rtspace->qual[tspacex].tspace_name,
          "ERROR",dm_err->emsg)
         SET rtspace->qual[tspacex].ct_err_msg = concat(rtspace->qual[tspacex].ct_err_msg,dm_err->
          emsg)
         SET rtspace->qual[tspacex].ct_err_ind = 1
         IF (bkoutx > 0)
          CALL backout_commands(null)
        ...
        ENDIF
       ELSE
        IF (existing_tablespace_work(null)=false)
         SET dm_err->err_ind = 0
         CALL update_tmp_tablespace(rtspace->tmp_table_name,rtspace->qual[tspacex].tspace_name,
          "ERROR",dm_err->emsg)
         SET rtspace->qual[tspacex].ct_err_msg = concat(rtspace->qual[tspacex].ct_err_msg,dm_err->
     ...
     ENDIF
    ENDIF
    SET dm_err->eproc = concat("Finished: ",rtspace->qual[tspacex].tspace_name)
    CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
    SET tspacex = (tspacex+ 1)
    SET dm_err->err_ind = 0
  ENDWHILE
  IF ((rtspace->database_remote=1)
   ...
   IF (get_unique_file("dm2_create_lvs_",".ksh")=0)
    GO TO exit_program
   ENDIF
   SET dct_ksh_file = dm_err->unique_fname
   SET tspacex = 0
   SET ddl_cmd_cnt = 0
   SELECT INTO value(dct_ksh_file)
    ...
    HEAD REPORT
     IF ((dm2_sys_misc->cur_db_os="HPX"))
      ...
      col 0, "  then", row + 1,
      col 0, '    echo " "', row + 1,
      col 0, '    echo "ERROR: Logon as root and run again."', row + 1,
      col 0, '    echo " "', row + 1,
      col 0, "    exit 1", row + 1,
      col 0, "fi", row + 1
     ENDIF
    DETAIL
     FOR (tspacex = 1 TO size(rtspace->qual,5))
       FOR (ddl_cmd_cnt = 1 TO size(rtspace->qual[tspacex].commands,5))
         IF ((rtspace->qual[tspacex].commands[ddl_cmd_cnt].cmd_type=cpushdcl))
          IF ((rtspace->qual[tspacex].commands[ddl_cmd_cnt].lv_exist_chk=1))
           dct_cmd_str = concat("if [[ -c ",rtspace->qual[tspacex].commands[ddl_cmd_cnt].lv_file,
            " ]]"), col 0, dct_cmd_str,
           row + 1, col 0, "then",
           row + 1, col 0, 'echo "logical volume already exists"',
           row + 1, col 0, "else",
           row + 1, col 0, rtspace->qual[tspacex].commands[ddl_cmd_cnt].cmd,
           row + 1, col 0, "if [[ $? -ne 0 ]]",
           row + 1, col 0, "then",
           row + 1, dct_cmd_str = concat('echo "  ERROR executing:',rtspace->qual[tspacex].commands[
            ddl_cmd_cnt].cmd,'"'), col 0,
           dct_cmd_str, row + 1, col 0,
           "exit 1", row + 1, col 0,
           "fi", row + 1, col 0,
           "fi", row + 1
          ELSE
           col 0, rtspace->qual[tspacex].commands[ddl_cmd_cnt].cmd, row + 1,
           col 0, "if [[ $? -ne 0 ]]", row + 1,
           col 0, "then", row + 1,
           dct_cmd_str = concat('echo "  ERROR executing:',rtspace->qual[tspacex].commands[
            ddl_cmd_cnt].cmd,'"'), col 0, dct_cmd_str,
           row + 1, col 0, "    exit 1",
           row + 1, col 0, "fi",
           row + 1
          ENDIF
   ...
   SET message = window
   CALL clear(1,1)
   CALL text(2,1,"Take the following KSH file located in CCLUSERDIR and copy to database node:")
   CALL text(3,4,dct_ksh_file)
   CALL text(5,1,
    "Logged in as ROOT, open permission for the following KSH file that was moved to the database node as follows:"
    )
   CALL text(6,4,concat("chmod 777 ",dct_ksh_file))
   CALL text(8,1,
    "Logged in as ROOT, execute the following KSH file that was moved to the database node as follows:"
    )
   CALL text(9,4,concat("$. ",dct_ksh_file," "))
   CALL text(11,1,
    "If KSH successfully executed and all logical volumes created, continue process to create tablespaces."
    )
   SET dm_err->eproc = "Prompt user for completion of steps"
   CALL text(15,1,"Enter 'C' to continue with create tablespaces or 'Q' to quit:")
   CALL accept(15,64,"p;cu","C"
    WHERE curaccept IN ("C", "Q"))
   SET message = nowindow
   CALL clear(1,1)
   IF (curaccept="Q")
    SET dm_err->emsg = "User chose to quit from remote node lv create steps. Cannot continue"
    SET dct_signal_failure = 1
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ENDIF
   FOR (tspacex = 1 TO size(rtspace->qual,5))
     FOR (ddl_cmd_cnt = 1 TO size(rtspace->qual[tspacex].commands,5))
       IF ((rtspace->qual[tspacex].commands[ddl_cmd_cnt].cmd_type=cpushcmd))
        IF ((dm_err->debug_flag > 0))
         CALL echo(rtspace->qual[tspacex].commands[ddl_cmd_cnt].cmd)
        ENDIF
        IF (dct_signal_failure=0)
         CALL parser(rtspace->qual[tspacex].commands[ddl_cmd_cnt].cmd,1)
         SET dm_err->ecode = error(dm_err->emsg,1)
         IF ((dm_err->ecode != 0))
          SET dm_err->err_ind = 1
          CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         ENDIF
        ENDIF
        IF ((((dm_err->err_ind > 0)) OR (dct_signal_failure=1)) )
         SET rtspace->qual[tspacex].ct_err_msg = dm_err->emsg
         SET rtspace->qual[tspacex].ct_err_ind = 1
         SET dm_err->err_ind = 0
        ELSE
         SET rtspace->qual[tspacex].ct_err_ind = 0
        ENDIF
       ENDIF
     ENDFOR
     IF ((rtspace->qual[tspacex].ct_err_ind=0))
      IF (update_tmp_tablespace(rtspace->tmp_table_name,rtspace->qual[tspacex].tspace_name,"SUCCESS",
       "")=false)
       RETURN(false)
      ENDIF
     ELSE
 ...
 ENDIF
 GO TO end_program
 SUBROUTINE dct_create_asm_tspace(dcat_ndx)
   DECLARE dcat_str = vc WITH protect, noconstant("")
   DECLARE dcat_cnt = i4 WITH protect, noconstant(0)
   DECLARE dcat_df_to_add = i4 WITH protect, noconstant(0)
   DECLARE dcat_size_str = vc WITH protect, noconstant("")
   SET dcat_df_to_add = dm2ceil((rtspace->qual[dcat_ndx].bytes_needed/ (((32.0 * 1024.0) * 1024.0) *
    1024.0)))
   IF ((rtspace->qual[dcat_ndx].user_tspace_ind=1))
    IF ((rtspace->qual[dcat_ndx].temp_ind=1))
     SET dcat_str = concat("rdb create temporary tablespace ",rtspace->qual[dcat_ndx].tspace_name,
      " TEMPFILE ")
     SET dcat_size_str = build(trim(cnvtstring(convert_bytes(rtspace->qual[dcat_ndx].bytes_needed,"b",
         "m"))),"M")
    ELSE
     SET dcat_str = concat("rdb create tablespace ",rtspace->qual[dcat_ndx].tspace_name," DATAFILE ")
     SET dcat_size_str = build(trim(cnvtstring(convert_bytes(rtspace->qual[dcat_ndx].bytes_needed,"b",
         "m"))),"M")
    ENDIF
   ELSEIF ((rtspace->qual[dcat_ndx].temp_ind=1))
    SET dcat_str = concat("rdb create temporary tablespace ",rtspace->qual[dcat_ndx].tspace_name,
     " TEMPFILE ")
    SET dcat_size_str = build(trim(cnvtstring(convert_bytes(rtspace->qual[dcat_ndx].bytes_needed,"b",
        "m"))),"M")
   ELSE
    SET dcat_str = concat("rdb create tablespace ",rtspace->qual[dcat_ndx].tspace_name," DATAFILE ")
    SET dcat_size_str = "1M"
   ENDIF
   FOR (dcat_cnt = 1 TO dcat_df_to_add)
    IF (dcat_cnt=1)
     SET dcat_str = concat(dcat_str," '+",rtspace->qual[dcat_ndx].asm_disk_group,"' ")
    ELSE
     SET dcat_str = concat(dcat_str,",'+",rtspace->qual[dcat_ndx].asm_disk_group,"' ")
    ENDIF
    SET dcat_str = concat(dcat_str," size ",dcat_size_str," autoextend on next 64M ")
   ENDFOR
   SET dcat_str = concat(dcat_str," go")
   IF ((dm_err->debug_flag > 10))
    CALL echo(dcat_str)
   ENDIF
   IF (execute_command(dcat_str,cpushcmd,1,0,"")=0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET rtspace->qual[tspacex].ct_err_msg = concat(rtspace->qual[tspacex].ct_err_msg,dm_err->emsg)
    SET rtspace->qual[tspacex].ct_err_ind = 1
    RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE create_new_tablespace(null)
   DECLARE container_str = vc WITH private, noconstant("")
   SET curalias tspace rtspace->qual[tspacex]
   SET curalias contain rtspace->qual[tspacex].cont[contx]
   SET contx = 1
   WHILE (contx <= size(rtspace->qual[tspacex].cont,5))
     SET logical_device_name = ""
     SET physical_device_name = ""
     IF (format_device_name(rtspace->dbname,tspace->tspace_name,contx,contain->lv_file)=0)
      RETURN(0)
     ENDIF
     IF ((dm2_sys_misc->cur_db_os IN (chpx_os, caix_os)))
      IF (create_new_device(logical_device_name,physical_device_name,contain->vg_name,contain->
       disk_name,contain->pps_to_add,
       contain->mwc_flag)=false)
       RETURN(false)
      ENDIF
     ENDIF
     IF (currdb=coracle_db)
      IF (contx > 1)
       SET container_str = concat(container_str,",  '")
      ELSE
       SET container_str = "datafile '"
      ENDIF
     ELSE
      IF (contx > 1)
       SET container_str = concat(container_str,", device '")
      ELSE
       SET container_str = "device '"
      ENDIF
     ENDIF
     IF ((dm2_sys_misc->cur_db_os IN (chpx_os, caix_os)))
      SET container_str = concat(container_str,physical_device_name,"' size ",trim(cnvtstring((
         contain->cont_size_mb - dct_sub_amount))),"M")
     ELSEIF ((dm2_sys_misc->cur_db_os=cvms_os))
      SET container_str = concat(container_str,contain->volume_label,":",physical_device_name,
       " ' size ",
       trim(cnvtstring(contain->cont_size_mb)),"M")
     ENDIF
     SET contx = (contx+ 1)
   ENDWHILE
   SET container_str = concat(container_str)
   IF (create_tablespace(tspace->tspace_name,container_str,tspace->init_ext)=false)
    RETURN(false)
   ENDIF
   IF ((((rtspace->database_remote=0)) OR ((dm2_sys_misc->cur_db_os="AXP"))) )
    IF (update_tmp_tablespace(rtspace->tmp_table_name,tspace->tspace_name,"SUCCESS","")=false)
     RETURN(false)
    ENDIF
   ENDIF
   ...
   RETURN(true)
 END ;Subroutine
 SUBROUTINE existing_tablespace_work(null)
   DECLARE ee_contx = i4 WITH private, noconstant(0)
   DECLARE ee_container_str = vc WITH private, noconstant("")
   DECLARE ee_cont_ndx = i4 WITH private, noconstant(0)
   DECLARE ea_container_str = vc WITH private, noconstant("")
   DECLARE ea_add_datafile = i2 WITH private, noconstant(0)
   DECLARE et_cmd = vc WITH private, noconstant("")
   DECLARE et_pps_add_total = f8 WITH protect, noconstant(0.0)
   SET contx = 0
   FOR (dct_cont_ndx = 1 TO size(rtspace->qual[tspacex].cont,5))
     IF ((rtspace->qual[tspacex].cont[dct_cont_ndx].space_to_add > 0))
      SET ee_cont_ndx_in = dct_cont_ndx
      IF ((rtspace->qual[tspacex].cont[dct_cont_ndx].new_ind=0))
       SET logical_device_name = ""
       SET physical_device_name = ""
       SET logical_device_name = rtspace->qual[tspacex].cont[ee_cont_ndx_in].lv_file
       IF ((dm2_sys_misc->cur_db_os IN (cvms_os, caix_os)))
        SET physical_device_name = concat(cdevice_name_prefix,logical_device_name)
       ELSE
        SET physical_device_name = concat(rtspace->qual[tspacex].cont[dct_cont_ndx].vg_name,"/r",
         logical_device_name)
       ENDIF
       IF ((dm2_sys_misc->cur_db_os IN (chpx_os, caix_os)))
        IF ((dm2_sys_misc->cur_db_os=chpx_os))
         SET et_pps_add_total = (ceil((rtspace->qual[tspacex].cont[ee_cont_ndx_in].cont_size_mb/
          rtspace->qual[tspacex].cont[ee_cont_ndx_in].pp_size_mb))+ rtspace->qual[tspacex].cont[
         ee_cont_ndx_in].pps_to_add)
        ELSE
         SET et_pps_add_total = rtspace->qual[tspacex].cont[ee_cont_ndx_in].pps_to_add
        ENDIF
        IF (resize_device(logical_device_name,physical_device_name,rtspace->qual[tspacex].cont[
         ee_cont_ndx_in].vg_name,rtspace->qual[tspacex].cont[ee_cont_ndx_in].disk_name,
         et_pps_add_total)=false)
         RETURN(false)
        ENDIF
       ENDIF
       IF ((dm2_sys_misc->cur_db_os IN (chpx_os, caix_os)))
        SET ee_container_str = concat("datafile '",physical_device_name,"'")
       ELSEIF ((dm2_sys_misc->cur_db_os=cvms_os))
        SET ee_container_str = concat("datafile '",rtspace->qual[tspacex].cont[ee_cont_ndx_in].
         volume_label,":",physical_device_name,".DBS '  ")
       ENDIF
       IF (currdb=coracle_db)
        IF ((dm2_sys_misc->cur_db_os IN (chpx_os, caix_os)))
         SET et_cmd = concat("rdb alter database ",rtspace->dbname," ",ee_container_str," resize ",
          trim(cnvtstring((rtspace->qual[tspacex].cont[ee_cont_ndx_in].cont_size_mb - dct_sub_amount)
            )),"M"," GO")
        ELSEIF ((dm2_sys_misc->cur_db_os=cvms_os))
         SET et_cmd = concat("rdb alter database ",rtspace->dbname," ",ee_container_str," resize ",
          trim(cnvtstring(rtspace->qual[tspacex].cont[ee_cont_ndx_in].cont_size_mb)),"M"," GO")
        ENDIF
       ENDIF
       SET dm_err->eproc = concat("Extend Tablespace:",rtspace->qual[tspacex].tspace_name)
       CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
       IF (execute_command(et_cmd,cpushcmd,1,0,"")=false)
        SET dm_err->eproc = "Extend_Tspace"
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        SET rtspace->qual[tspacex].ct_err_msg = concat(rtspace->qual[tspacex].ct_err_msg,dm_err->emsg
         )
        SET rtspace->qual[tspacex].ct_err_ind = 1
       ...
       ENDIF
       IF ((((rtspace->database_remote=0)) OR ((dm2_sys_misc->cur_db_os="AXP"))) )
        IF (update_tmp_tablespace(rtspace->tmp_table_name,rtspace->qual[tspacex].tspace_name,
         "SUCCESS","")=false)
         RETURN(false)
        ENDIF
       ENDIF
      ELSE
       ...
       SET logical_device_name = ""
       SET physical_device_name = ""
       IF (format_device_name(rtspace->dbname,rtspace->qual[tspacex].tspace_name,ee_cont_ndx_in,
        rtspace->qual[tspacex].cont[ee_cont_ndx_in].lv_file)=0)
        RETURN(0)
       ENDIF
       IF ((dm2_sys_misc->cur_db_os IN (chpx_os, caix_os)))
        IF (create_new_device(logical_device_name,physical_device_name,rtspace->qual[tspacex].cont[
         ee_cont_ndx_in].vg_name,rtspace->qual[tspacex].cont[ee_cont_ndx_in].disk_name,rtspace->qual[
         tspacex].cont[ee_cont_ndx_in].pps_to_add,
         rtspace->qual[tspacex].cont[ee_cont_ndx_in].mwc_flag)=false)
         RETURN(false)
        ENDIF
       ENDIF
       IF (currdb=coracle_db)
        IF (contx > 1)
         SET ea_container_str = concat(ea_container_str,",  '")
        ELSE
         SET ea_container_str = "datafile '"
        ENDIF
       ELSE
        IF (contx > 1)
         SET ea_container_str = concat(ea_container_str,", device '")
        ELSE
         SET ea_container_str = "device '"
        ENDIF
       ENDIF
       IF ((dm2_sys_misc->cur_db_os IN (chpx_os, caix_os)))
        SET ea_container_str = concat(ea_container_str,physical_device_name,"' size ",trim(cnvtstring
          ((rtspace->qual[tspacex].cont[ee_cont_ndx_in].cont_size_mb - dct_sub_amount))),"M")
       ELSEIF ((dm2_sys_misc->cur_db_os=cvms_os))
        SET ea_container_str = concat(ea_container_str,rtspace->qual[tspacex].cont[ee_cont_ndx_in].
         volume_label,":",physical_device_name," ' size ",
         trim(cnvtstring(rtspace->qual[tspacex].cont[ee_cont_ndx_in].cont_size_mb)),"M")
       ENDIF
       SET ea_add_datafile = 1
      ENDIF
     ELSE
      IF ((rtspace->mode != "CREATE_DDL_ONLY"))
       SET rtspace->qual[tspacex].ct_err_ind = 0
     ...
     ENDIF
   ENDFOR
   IF (ea_add_datafile=1)
    SET et_cmd = ""
    SET et_cmd = "rdb alter tablespace"
    SET et_cmd = concat(et_cmd," ",rtspace->qual[tspacex].tspace_name," add ",ea_container_str)
    SET et_cmd = concat(et_cmd," GO")
    IF (execute_command(et_cmd,cpushcmd,1,0,"")=false)
     SET dm_err->eproc = "Create_Tablespace"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET rtspace->qual[tspacex].ct_err_msg = concat(rtspace->qual[tspacex].ct_err_msg,dm_err->emsg)
     SET rtspace->qual[tspacex].ct_err_ind = 1
     RETURN(false)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE resize_device(rd_l_device_name,rd_p_device_name,rd_volume_name,rd_disk_name,rd_partitions
  )
   DECLARE rd_cmd = vc WITH private, noconstant(" ")
   SET dm_err->eproc = concat("Resize Device:",rd_l_device_name," on disk:",rd_disk_name,
    " with size of:",
    trim(cnvtstring(rd_partitions))," partitions")
   CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
   IF ((dm2_sys_misc->cur_db_os=caix_os))
    SET rd_cmd = concat("extendlv ",rd_l_device_name," ",trim(cnvtstring(rd_partitions))," ",
     rd_disk_name)
   ELSE
    SET rd_cmd = concat("lvextend -l  ",trim(cnvtstring(rd_partitions))," ",rd_volume_name,"/",
     rd_l_device_name)
   ENDIF
   IF (execute_command(rd_cmd,cpushdcl,1,0,"")=false)
    SET dm_err->eproc = "resize_Device - resize logical device"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET rtspace->qual[tspacex].ct_err_msg = concat(rtspace->qual[tspacex].ct_err_msg,dm_err->emsg)
    SET rtspace->qual[tspacex].ct_err_ind = 1
    RETURN(false)
   ...
   IF (currdb=coracle_db)
    SET rd_cmd = ""
    SET rd_cmd = concat("chown oracle:dba ",rd_p_device_name)
    IF (execute_command(rd_cmd,cpushdcl,1,0,"")=false)
     SET dm_err->eproc = "Create_New_Device - Change Owner"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(false)
    ENDIF
    SET rd_cmd = ""
    SET rd_cmd = concat("chmod 600 ",rd_p_device_name)
    IF (execute_command(rd_cmd,cpushdcl,1,0,"")=false)
     SET dm_err->eproc = "Create_New_Device - Change Mode"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(false)
    ENDIF
   ENDIF
   RETURN(true)
 END ;Subroutine
 SUBROUTINE create_new_device(l_device_name,p_device_name,volume_name,disk_name,partitions,cnd_mwc_in
  )
   DECLARE cnd_mwc_flag = vc WITH protect, noconstant("")
   DECLARE cnd_offset_str = vc WITH protect, noconstant(" ")
   DECLARE cmd = vc WITH private, noconstant(" ")
   SET cnd_mwc_flag = trim(cnvtstring(cnd_mwc_in))
   IF ((dm2_rdbms_version->level1 >= 10)
    AND (dm2_sys_misc->cur_db_os="AIX"))
    SET cnd_offset_str = "-T O"
   ENDIF
   IF ((dm2_sys_misc->cur_db_os=caix_os))
    IF (cnd_mwc_flag="1")
     SET cmd = concat("mklv -y '",l_device_name,"' -w y -t 'raw'"," ",cnd_offset_str,
      " ",volume_name," ",trim(cnvtstring(partitions))," ",
      disk_name)
    ELSE
     SET cmd = concat("mklv -y '",l_device_name,"' -w n -t 'raw'"," ",cnd_offset_str,
      " ",volume_name," ",trim(cnvtstring(partitions))," ",
      disk_name)
    ENDIF
   ELSE
    SET cmd = concat("lvcreate -l ",trim(cnvtstring(partitions))," -n ",l_device_name," ",
     volume_name)
   ENDIF
   IF ((dm2_sys_misc->cur_db_os=caix_os))
    SET dm_err->eproc = concat("Create_New_Device - Make Logical Device:",l_device_name," on Volume:",
     volume_name," on disk:",
     disk_name," with size of:",trim(cnvtstring(partitions))," partitions")
   ELSE
    SET dm_err->eproc = concat("Create_New_Device - Make Logical Device:",l_device_name," on Volume:",
     volume_name," with size of:",
     trim(cnvtstring(partitions))," partitions")
   ENDIF
   CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
   IF (execute_command(cmd,cpushdcl,1,1,p_device_name)=false)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(false)
   ELSE
    IF ((dm2_sys_misc->cur_db_os=caix_os))
     CALL format_backout_command(concat("rmlv -f ",l_device_name),cpushdcl)
    ELSE
     CALL format_backout_command(concat("lvremove -f ",p_device_name),cpushdcl)
    ENDIF
   ENDIF
   IF (currdb=cdb2_db)
    SET cmd = ""
    SET cmd = concat("chown ",trim(currdblink),".db2adm ",p_device_name)
    IF (execute_command(cmd,cpushdcl,1,0,"")=false)
     SET dm_err->eproc = "Create_New_Device - Change Owner"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET rtspace->qual[tspacex].ct_err_msg = concat(rtspace->qual[tspacex].ct_err_msg,dm_err->emsg)
     SET rtspace->qual[tspacex].ct_err_ind = 1
    ...
    ENDIF
    SET cmd = ""
    SET cmd = concat("chmod 600 ",p_device_name)
    IF (execute_command(cmd,cpushdcl,1,0,"")=false)
     SET dm_err->eproc = "Create_New_Device - Change Mode"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET rtspace->qual[tspacex].ct_err_msg = concat(rtspace->qual[tspacex].ct_err_msg,dm_err->emsg)
   ...
   ELSEIF (currdb=coracle_db)
    SET cmd = ""
    SET cmd = concat("chown oracle:dba ",p_device_name)
    IF (execute_command(cmd,cpushdcl,1,0,"")=false)
     SET dm_err->eproc = "Create_New_Device - Change Owner"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET rtspace->qual[tspacex].ct_err_msg = concat(rtspace->qual[tspacex].ct_err_msg,dm_err->emsg)
   ...
   RETURN(true)
 END ;Subroutine
 SUBROUTINE create_tablespace(tspace_name,container_statement,init_ext_size)
   DECLARE cmd = vc WITH private, noconstant(" ")
   IF (currdb=cdb2_db)
    IF (substring(1,2,tspace_name)="L_")
     SET cmd = "rdb create long tablespace"
    ELSE
     SET cmd = "rdb create tablespace"
    ENDIF
    SET cmd = concat(cmd," ",tspace_name," pagesize 16k managed by database using ",
     container_statement)
    IF (init_ext_size > 0)
     IF (currdb=cdb2_db)
      SET cmd = concat(cmd," extentsize ",trim(cnvtstring(init_ext_size)),"K ")
     ELSE
      SET cmd = concat(cmd," extentsize ",trim(cnvtstring(init_ext_size)),"K ")
     ENDIF
    ENDIF
    SET cmd = concat(cmd," bufferpool bp16k ")
    SET cmd = concat(cmd," GO")
   ELSEIF (currdb=coracle_db)
    SET cmd = "rdb create tablespace"
    SET cmd = concat(cmd," ",tspace_name," ",container_statement)
    CASE (rtspace->qual[tspacex].ext_mgmt)
     OF "L":
      SET cmd = concat(cmd," extent management local autoallocate segment space management auto")
     OF "D":
      SET cmd = concat(cmd," default storage(initial ",trim(cnvtstring(dmt_min_ext_size))," next ",
       trim(cnvtstring(dmt_min_ext_size)),
       " pctincrease 0) extent management dictionary  ")
     ELSE
      SET dm_err->err_ind = 1
      SET dm_err->eproc = "No Extent Management specification has been recorded."
      SET dm_err->emsg = "No Extent Management specification has been recorded."
      SET dm_err->user_action = "Please Specify Locally or Dictionary managed"
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET rtspace->qual[tspacex].ct_err_msg = concat(rtspace->qual[tspacex].ct_err_msg,dm_err->emsg)
      SET rtspace->qual[tspacex].ct_err_ind = 1
      RETURN(false)
    ...
    SET cmd = concat(cmd," GO")
   ENDIF
   SET dm_err->eproc = concat("Create Tablespace:",tspace_name)
   CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
   IF (execute_command(cmd,cpushcmd,1,0,"")=false)
    SET dm_err->eproc = "Create_Tablespace"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET rtspace->qual[tspacex].ct_err_msg = concat(rtspace->qual[tspacex].ct_err_msg,dm_err->emsg)
    SET rtspace->qual[tspacex].ct_err_ind = 1
   ...
   RETURN(true)
 END ;Subroutine
 SUBROUTINE format_device_name(dbname,tspace_name,container,name_in)
   DECLARE table_type = c1 WITH private, noconstant(" ")
   DECLARE table_abbr = vc WITH private, noconstant(convert_tspace_name(tspace_name))
   IF ((dm2_sys_misc->cur_db_os=chpx_os))
    SET logical_device_name = name_in
    SET physical_device_name = concat(rtspace->qual[tspacex].cont[container].vg_name,"/r",
     logical_device_name)
   ELSE
    SET logical_device_name = name_in
    SET physical_device_name = concat(cdevice_name_prefix,logical_device_name)
   ...
   SET seqx = 0
   SET seqx = (seqx+ 1)
   SET filename_exists_ind = 1
   IF ((dm_err->debug_flag > 0))
    CALL echo(physical_device_name)
    CALL echo(logical_device_name)
   ENDIF
   WHILE (filename_exists_ind=1)
     IF (currdb=coracle_db)
      IF ((dm2_sys_misc->cur_db_os=cvms_os))
       IF (seqx=1)
        SET filename_str = cnvtupper(build(name_in,".dbs"))
       ELSE
        SET filename_str = ""
        SET filename_str = cnvtupper(build(tspace_name,"_",format(seqx,"###;P0"),".dbs"))
       ENDIF
      ELSEIF ((dm2_sys_misc->cur_db_os IN (chpx_os, caix_os)))
       IF (seqx=1)
        SET filename_str = name_in
       ELSE
        SET filename_str = build(cnvtlower(substring(1,1,tspace_name)),cnvtlower(rtspace->dbname),"_",
         format(seqx,"#######;P0"))
       ENDIF
      ENDIF
     ELSEIF (currdb=cdb2_db)
     ...
     ENDIF
     IF ((dm_err->debug_flag > 0))
      CALL echo(filename_str)
     ENDIF
     CASE (verify_filename(filename_str,tspace_name,container))
      OF 0:
       RETURN(0)
      OF 1:
       SET filename_exists_ind = 0
       SET logical_device_name = filename_str
       IF ((dm2_sys_misc->cur_db_os=chpx_os))
        SET physical_device_name = concat(rtspace->qual[tspacex].cont[container].vg_name,"/r",
         filename_str)
       ELSE
        SET physical_device_name = concat(cdevice_name_prefix,filename_str)
       ENDIF
      OF 2:
       SET seqx = (seqx+ 1)
     ENDCASE
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE add_sql_file_group(null)
   DECLARE diskx = i4 WITH private, noconstant(0)
   DECLARE seqx = i4 WITH private, noconstant(0)
   DECLARE diskx_max = i4 WITH private, noconstant(0)
   DECLARE filegroup_file_str = vc WITH private, noconstant(" ")
   DECLARE cmd = vc WITH private, noconstant(" ")
   DECLARE filename_exists_ind = i2 WITH private, noconstant(0)
   IF (dm2_set_autocommit(1)=0)
    GO TO end_program
   ENDIF
   SET cmd = concat("ALTER DATABASE ",trim(currdbname)," ADD FILEGROUP ",trim(rtspace->qual[tspacex].
     tspace_name))
   SET cmd = concat('RDB ASIS ("',cmd,'") GO')
   IF (execute_command(cmd,cpushcmd,1,0,"")=false)
    SET dm_err->eproc = "Add_Sql_File_Group"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(false)
   ELSE
    SET cmd = ""
    SET cmd = concat("ALTER DATABASE ",trim(currdbname)," REMOVE FILEGROUP ",trim(rtspace->qual[
      tspacex].tspace_name))
    SET cmd = concat('RDB ASIS ("',cmd,'") GO')
    CALL format_backout_command(cmd,cpushcmd)
   ENDIF
   SET cmd = ""
   SET cmd = concat("ALTER DATABASE ",trim(currdbname)," ADD FILE ")
   SET diskx_max = size(rdisk->qual,5)
   SET seqx = 0
   FOR (diskx = 1 TO diskx_max)
     SET seqx = (seqx+ 1)
     SET filename_exists_ind = 1
     WHILE (filename_exists_ind=1)
      SET filename_str = concat(dm2_install_schema->frmt_servername,"_",trim(currdbname),"_",trim(
        rtspace->qual[tspacex].tspace_name),
       trim(cnvtstring(seqx)),".NDF")
      CASE (verify_filename(concat(trim(rdisk->qual[diskx].disk_name),trim(filename_str)),rtspace->
       qual[tspacex].tspace_name,diskx))
       OF 0:
        RETURN(0)
       OF 1:
        SET filename_exists_ind = 0
     ...
     ENDWHILE
     IF (diskx > 1)
      SET filegroup_file_str = concat(filegroup_file_str,", ")
     ENDIF
     SET filegroup_file_str = concat(filegroup_file_str,"(NAME = ",trim(rtspace->qual[tspacex].
       tspace_name),trim(cnvtstring(diskx)),", ")
     SET filegroup_file_str = concat(filegroup_file_str," FILENAME = "," '",trim(rdisk->qual[diskx].
       disk_name),trim(filename_str),
      "'",", ")
     SET filegroup_file_str = concat(filegroup_file_str," SIZE = ",trim(cnvtstring(rtspace->
        sql_size_mb)),", ")
     SET filegroup_file_str = concat(filegroup_file_str," FILEGROWTH = ",trim(cnvtstring(rtspace->
        sql_filegrowth_mb)),") ")
   ENDFOR
   SET cmd = concat(cmd," ",filegroup_file_str," TO FILEGROUP ",trim(rtspace->qual[tspacex].
     tspace_name))
   SET cmd = concat('RDB ASIS ("',cmd,'") GO')
   IF (execute_command(cmd,cpushcmd,1,0,"")=false)
    SET dm_err->eproc = "Add_Sql_File_Group"
   ...
   RETURN(true)
 END ;Subroutine
 SUBROUTINE verify_filename(vf_filename_str,vf_tspace_name,vf_container_ndx)
   DECLARE vf_start_pt = i4 WITH noconstant(0)
   DECLARE vf_end_pt = i4 WITH noconstant(0)
   DECLARE vf_exists_ind = i2 WITH noconstant(0)
   DECLARE tspace_fnd_ind = i2 WITH protect, noconstant(0)
   DECLARE vf_cmd_txt = vc WITH protect, noconstant("")
   DECLARE vf_file_path = vc WITH protect, noconstant("")
   IF ((rtspace->database_remote=1))
    RETURN(1)
   ENDIF
   IF (currdb="SQLSRV")
    SELECT INTO "nl:"
     df.file_name, df.tablespace_name
     FROM dm2_dba_data_files df
     WHERE cnvtupper(df.file_name)=trim(cnvtupper(vf_filename_str))
     WITH nocounter
    ;end select
    IF (check_error(concat("verify_filename")) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    IF (curqual > 0)
   ...
   ELSEIF (currdb="ORACLE")
    ...
    SELECT INTO "nl:"
     df.file_name, df.tablespace_name
     FROM dm2_dba_data_files df
     WHERE cnvtupper(df.tablespace_name)=trim(cnvtupper(vf_tspace_name))
     DETAIL
      IF ((dm2_sys_misc->cur_db_os=cvms_os))
       vf_start_pt = (findstring("]",df.file_name,1,1)+ 1), vf_end_pt = textlen(df.file_name)
       IF (trim(cnvtupper(vf_filename_str))=substring(vf_start_pt,((vf_end_pt - vf_start_pt)+ 1),df
        .file_name))
        vf_exists_ind = 1
       ENDIF
      ELSE
       vf_start_pt = (findstring("/r",df.file_name,1,1)+ 2), vf_end_pt = textlen(df.file_name)
       IF (trim(cnvtupper(vf_filename_str))=substring(vf_start_pt,((vf_end_pt - vf_start_pt)+ 1),df
        .file_name))
        vf_exists_ind = 1
       ENDIF
    ...
    IF ((dm2_sys_misc->cur_db_os IN (chpx_os, caix_os)))
     IF ((dm2_sys_misc->cur_db_os=caix_os))
      SET vf_file_path = concat(cdevice_name_prefix,vf_filename_str)
     ELSE
      SET vf_file_path = concat(rtspace->qual[tspacex].cont[vf_container_ndx].vg_name,"/r",
       vf_filename_str)
     ENDIF
     IF (findfile(vf_file_path)=0)
      SET dm_err->emsg = concat("File",vf_file_path," not found.")
      CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
      SET vf_exists_ind = 0
     ELSE
      SET vf_exists_ind = 1
     ENDIF
    ELSEIF ((dm2_sys_misc->cur_db_os=cvms_os))
     SET vf_file_path = concat(rtspace->qual[tspacex].cont[vf_container_ndx].volume_label,":",
      cdevice_name_prefix,vf_filename_str)
     IF ((dm_err->errfile="NONE"))
      IF (get_unique_file("dm2_",".err")=0)
       RETURN(0)
      ELSE
      ...
      ENDIF
     ENDIF
     SET vf_cmd_txt = concat('pipe @cer_install:dm2_findfile_os.com "',vf_file_path,
      '  " > ccluserdir:',dm_err->errfile)
     IF ((dm_err->debug_flag > 0))
      CALL echo("***********")
      CALL echo(vf_cmd_txt)
      CALL echo("***********")
     ENDIF
     CALL dcl(vf_cmd_txt,textlen(vf_cmd_txt),stat)
     SET dm_err->errtext = " "
     FREE DEFINE rtl2
     FREE SET file_loc
     SET logical file_loc value(dm_err->errfile)
     ...
     ELSEIF ((dm_err->errtext="FOUND"))
      SET vf_exists_ind = 1
     ELSE
      CALL disp_msg("Error Occurred in Verify Filename",dm_err->logfile,1)
      RETURN(0)
     ENDIF
    ENDIF
    IF (vf_exists_ind > 0)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE execute_command(command,type,add_to_list,ec_lv_chk,ec_lv_file)
   IF ((((rtspace->mode != "CREATE_DDL_ONLY")
    AND (rtspace->database_remote != 1)) OR ((rtspace->mode != "CREATE_DDL_ONLY")
    AND (rtspace->database_remote=1)
    AND (dm2_sys_misc->cur_db_os="AXP"))) )
    IF (type=cpushcmd)
     IF ((dm_err->debug_flag=1))
      CALL echo(concat("DB Command: ",command))
     ENDIF
     CALL parser(command,1)
     SET dm_err->ecode = error(dm_err->emsg,1)
     IF ((dm_err->ecode != 0))
      SET dm_err->err_ind = 1
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ENDIF
    ELSEIF (type=cpushdcl)
     IF ((dm_err->debug_flag=1))
      CALL echo(concat("OS Command: ",command))
     ENDIF
     CALL dcl(command,textlen(command),dm_err->ecode)
     IF ((dm_err->ecode=0))
      SET dm_err->err_ind = 1
      SET dm_err->emsg = concat("ERROR Executing ",command)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ENDIF
    ENDIF
   ENDIF
   IF (tspacex != cmd_ndx_hold)
    SET ddl_cmd_cnt = 0
    SET ddl_os_cmd_cnt = 0
    SET cmd_ndx_hold = tspacex
   ENDIF
   IF (add_to_list=1)
    SET ddl_cmd_cnt = (ddl_cmd_cnt+ 1)
    SET stat = alterlist(rtspace->qual[tspacex].commands,ddl_cmd_cnt)
    SET rtspace->qual[tspacex].commands[ddl_cmd_cnt].cmd = command
    SET rtspace->qual[tspacex].commands[ddl_cmd_cnt].cmd_type = type
    SET rtspace->qual[tspacex].commands[ddl_cmd_cnt].lv_exist_chk = ec_lv_chk
    SET rtspace->qual[tspacex].commands[ddl_cmd_cnt].lv_file = ec_lv_file
    SET rtspace->commands_written_ind = 1
   ENDIF
   IF ((dm_err->err_ind > 0))
    SET rtspace->qual[tspacex].ct_err_msg = concat(dm_err->emsg,rtspace->qual[tspacex].ct_err_msg)
    SET rtspace->qual[tspacex].ct_err_ind = 1
    IF (type=cpushcmd)
     IF (currdb=cdb2_db)
      CALL echo("ROLLBACK")
   ...
   RETURN(true)
 END ;Subroutine
 SUBROUTINE update_tmp_tablespace(tablespace_tbl_name,tablespace_name,tablespace_status,
  tablespace_emsg)
  DECLARE utt_rt_ndx = i4 WITH protect, noconstant(0)
  IF (dct_ts_size_exists=1
   AND (rtspace->mode != "CREATE_DDL_ONLY"))
   IF ((dm_err->debug_flag >= 1))
    CALL echo(concat("Table Name : ",value(trim(tablespace_tbl_name))))
    CALL echo(concat("Tspace Name: ",tablespace_name))
    CALL echo(concat("Status     : ",tablespace_status))
    CALL echo(concat("Error Msg  : ",tablespace_emsg))
   ENDIF
   SET dm_err->eproc = "Check for row in DM2_TSPACE_SIZE"
   SELECT INTO "nl:"
    FROM dm2_tspace_size d
    WHERE d.tspace_name=tablespace_name
   ...
   CASE (curqual)
    OF 1:
     IF (dct_obj_size_exists=1)
      IF ((rtspace->install_type IN ("PACKAGE", "SCHEMA_DATE")))
       SET dm_err->eproc = "Update row in DM2_TSPACE_OBJ_SIZE"
       UPDATE  FROM dm2_tspace_obj_size o
        SET o.install_status = tablespace_status, o.updt_dt_time = cnvtdatetime(curdate,curtime3)
        WHERE o.tspace_name=tablespace_name
         AND (o.install_type=rtspace->install_type)
         AND (o.install_type_value=rtspace->install_type_value)
         AND o.gen_id=dct_gen_id
        WITH nocounter
       ;end update
       IF (check_error(dm_err->eproc) != 0)
        ROLLBACK
      ...
      ENDIF
     ENDIF
     SET dm_err->eproc = "Update row in DM2_TSPACE_SIZE"
     UPDATE  FROM dm2_tspace_size d
      SET d.install_status = tablespace_status, d.err_msg = tablespace_emsg, d.updt_dt_tm =
       cnvtdatetime(curdate,curtime3)
      WHERE d.tspace_name=tablespace_name
       AND (d.install_type=rtspace->install_type)
       AND (d.install_type_value=rtspace->install_type_value)
     ...
     ;end update
    OF 0:
     SET dm_err->eproc = "Insert row into DM2_TSPACE_SIZE"
     SET utt_rt_ndx = locateval(utt_rt_ndx,1,size(rtspace->qual,5),tablespace_name,rtspace->qual[
      utt_rt_ndx].tspace_name)
     INSERT  FROM dm2_tspace_size d
      SET d.tspace_name = tablespace_name, d.install_type = rtspace->install_type, d
       .install_type_value = rtspace->install_type_value,
       d.bytes_needed = rtspace->qual[utt_rt_ndx].bytes_needed, d.extent_size_bytes = rtspace->qual[
       utt_rt_ndx].init_ext, d.chunk_size = rtspace->qual[utt_rt_ndx].chunk_size,
       d.install_status = tablespace_status, d.new_ind = rtspace->qual[utt_rt_ndx].new_ind, d
       .extent_management = rtspace->qual[utt_rt_ndx].ext_mgmt,
       d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d.err_msg = substring(1,500,rtspace->qual[
        utt_rt_ndx].ct_err_msg)
      WITH nocounter
     ;end insert
   ENDCASE
   IF (check_error(dm_err->eproc) != 0)
  ...
  ENDIF
 END ;Subroutine
 SUBROUTINE convert_tspace_name(tspace_name)
   DECLARE tspace_num = i4 WITH private, noconstant(0)
   DECLARE tspace_name_cnvt = vc WITH private, noconstant(cnvtlower(tspace_name))
   DECLARE tspace_length_cnt = i4 WITH private, noconstant(0)
   SET tspace_num = 0
   FOR (tspace_length_cnt = 1 TO size(tspace_name_cnvt,1))
     SET tspace_num = (tspace_num+ ((ichar(substring(tspace_length_cnt,1,tspace_name_cnvt)) *
     tspace_length_cnt) * tspace_length_cnt))
   ENDFOR
   RETURN(cnvtstring(tspace_num,6,0,"L"))
 END ;Subroutine
 SUBROUTINE format_backout_command(backout_cmd,cmd_type)
   SET bkoutx = (bkoutx+ 1)
   SET stat = alterlist(bkout->commands,bkoutx)
   SET bkout->commands[bkoutx].cmd = backout_cmd
   SET bkout->commands[bkoutx].cmd_type = cmd_type
 END ;Subroutine
 SUBROUTINE backout_commands(null)
   SET dm_err->eproc = "************* Backing Out Commands ***************"
   CALL disp_msg("",dm_err->logfile,0)
   IF ((dm_err->debug_flag=1))
    CALL echorecord(bkout)
   ENDIF
   WHILE (bkoutx > 0)
     SET dct_from_backout = 1
     CALL execute_command(bkout->commands[bkoutx].cmd,bkout->commands[bkoutx].cmd_type,0,0,"")
     SET bkoutx = (bkoutx - 1)
     SET dct_from_backout = 0
   ENDWHILE
 END ;Subroutine
 SUBROUTINE dct_parse_to_logfile(null)
   DECLARE start_pt = i4 WITH public, noconstant(0)
   DECLARE end_pt = i4 WITH public, noconstant(0)
   DECLARE dct_string = vc WITH public, noconstant("")
   DECLARE dct_looking = i2 WITH public, noconstant(0)
   DECLARE ddl_cnt = i4 WITH public, noconstant(0)
   DECLARE dct_date = vc WITH public, noconstant("")
   DECLARE check_cnt = i4 WITH public, noconstant(0)
   DECLARE xfactor = i4 WITH public, noconstant(0)
   IF ((rtspace->mode != "CREATE_DDL_ONLY"))
    SET dm_err->eproc = concat("Creating Report:",dm_err->logfile)
   ELSE
    SET dm_err->eproc = concat("Creating Report:",rtspace->ddl_report_fname)
   ENDIF
   SET dct_date = format(curdate,"mm/dd/yyyy;;d")
   SET xfactor = 0
   SET check_cnt = 0
   SELECT
    IF ((rtspace->mode != "CREATE_DDL_ONLY"))INTO dm_err->logfile
    ELSE INTO rtspace->ddl_report_fname
    ENDIF
    FROM (dummyt d  WITH seq = size(rtspace->qual,5))
    HEAD REPORT
     CALL center("Tablespace Operations",1,80), row + 1,
     CALL center(concat("Date:",dct_date),1,80),
     row + 1,
     MACRO (dct_macro)
      check_cnt = 0, end_pt = 0, start_pt = 0,
      dct_looking = 0, check_cnt = 0
      WHILE (dct_looking=0)
        IF ((rtspace->qual[d.seq].commands[ddl_cnt].cmd_type="PUSHCMD"))
         start_pt = end_pt, end_pt = findstring("datafile",rtspace->qual[d.seq].commands[ddl_cnt].cmd,
          start_pt)
         IF (end_pt=0)
          end_pt = findstring(",",rtspace->qual[d.seq].commands[ddl_cnt].cmd,start_pt)
          IF (end_pt=0)
           end_pt = findstring(" size ",rtspace->qual[d.seq].commands[ddl_cnt].cmd,start_pt)
          ENDIF
          xfactor = 1
         ELSE
          xfactor = 8
         ENDIF
         IF (((end_pt=0) OR (((end_pt=start_pt) OR (check_cnt > 100)) )) )
          dct_looking = 1, dct_string = substring(start_pt,((textlen(rtspace->qual[d.seq].commands[
            ddl_cnt].cmd) - start_pt)+ xfactor),rtspace->qual[d.seq].commands[ddl_cnt].cmd)
         ELSE
          dct_string = substring(start_pt,((end_pt - start_pt)+ xfactor),rtspace->qual[d.seq].
           commands[ddl_cnt].cmd)
         ENDIF
         dct_string = replace(dct_string,"rdb ","",1), dct_string = replace(dct_string," GO","",1),
         col 1,
         dct_string, end_pt = (end_pt+ xfactor), row + 1,
         check_cnt = (check_cnt+ 1)
        ELSE
         col 1, rtspace->qual[d.seq].commands[ddl_cnt].cmd, row + 1,
         dct_looking = 1
        ENDIF
      ENDWHILE
     ENDMACRO
    DETAIL
     col 1, rtspace->qual[d.seq].tspace_name, row + 1,
     col 1, "----------------------------------", row + 1
     IF ((rtspace->mode != "CREATE_DDL_ONLY"))
      IF ((rtspace->qual[d.seq].ct_err_ind=1))
       col 1, "Failed Operations"
      ELSE
       col 1, "Successful operations"
      ENDIF
      row + 1
     ENDIF
     ddl_cnt = 0
     IF (size(rtspace->qual[d.seq].commands,5) > 0)
      FOR (ddl_cnt = 1 TO size(rtspace->qual[d.seq].commands,5))
        dct_macro, row + 1
        IF ((rtspace->qual[d.seq].ct_err_ind=1)
         AND (rtspace->mode != "CREATE_DDL_ONLY")
         AND ddl_cnt=size(rtspace->qual[d.seq].commands,5))
         dct_string = "", dct_string = concat("Error Message: ",substring(1,500,rtspace->qual[d.seq].
           ct_err_msg)), col 1,
         dct_string, row + 2, col 1,
         "Command in Error: ", row + 1, dct_macro,
         row + 1
        ENDIF
      ENDFOR
     ELSE
      col 1, "No disk information provided", row + 1
     ENDIF
     row + 2
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE dct_create_ddl_file(null)
   DECLARE dcdf_tspace_cnt = i4 WITH protect, noconstant(0)
   DECLARE dcdf_new_ind = i2 WITH protect, noconstant(0)
   DECLARE dcdf_ext_mgmt = vc WITH protect, noconstant(" ")
   DECLARE dcdf_lmt_new = i2 WITH protect, noconstant(0)
   DECLARE dcdf_lmt_old = i2 WITH protect, noconstant(0)
   DECLARE dcdf_dmt_new = i2 WITH protect, noconstant(0)
   DECLARE dcdf_dmt_old = i2 WITH protect, noconstant(0)
   DECLARE dcdf_text = vc WITH protect, noconstant(" ")
   SET dm_err->eproc = "Writing DDL Report."
   SELECT INTO rtspace->ddl_report_fname
    FROM (dummyt d  WITH seq = 1)
    HEAD REPORT
     col 1, "Tablespace DDL Report", row + 1,
     dcdf_text = concat("Date:",format(cnvtdatetime(curdate,curtime3),cclfmt->shortdatetime)), col 1,
     dcdf_text,
     row + 1,
     MACRO (dcdf_macro)
      FOR (dcdf_tspace_cnt = 1 TO size(rtspace->qual,5))
        IF ((rtspace->qual[dcdf_tspace_cnt].new_ind=dcdf_new_ind)
         AND (rtspace->qual[dcdf_tspace_cnt].ext_mgmt=dcdf_ext_mgmt))
         IF (((dcdf_ext_mgmt="D"
          AND dcdf_new_ind=0
          AND dcdf_dmt_old=0) OR (dcdf_ext_mgmt="L"
          AND dcdf_new_ind=0
          AND dcdf_lmt_old=0)) )
          row + 2, dcdf_text = "Sample Alter Tablespace Statement to add new datafile:", col 1,
          dcdf_text, row + 2, dcdf_text =
          "sql> alter tablespace <tablespace name> add <file name> size <size in bytes> ;",
          col 1, dcdf_text, row + 2,
          dcdf_text = "Sample Alter Database Statement to extend existing datafile:", col 1,
          dcdf_text,
          row + 1, dcdf_text = concat("sql> alter database ",rtspace->dbname,
           " datafile <file name> resize <size in bytes>  ;"), col 1,
          dcdf_text, row + 2, dcdf_text =
          "The following tablespaces need to be altered. Either add new datafile(s) or extend existing datafile(s):",
          col 1, dcdf_text, row + 1,
          dcdf_dmt_old = 1, dcdf_lmt_old = 1
         ELSEIF (dcdf_ext_mgmt="D"
          AND dcdf_new_ind=1
          AND dcdf_dmt_new=0)
          row + 2, dcdf_text = "Sample Create Tablespace Statement (Dictionary Managed):", col 1,
          dcdf_text, row + 2, dcdf_text = concat(
           "sql> create tablespace <tablespace name> datafile [<file name> size <size in bytes> , ...]",
           " default storage (initial ",trim(cnvtstring(dmt_min_ext_size))),
          col 1, dcdf_text, row + 1,
          dcdf_text = concat("next ",trim(cnvtstring(dmt_min_ext_size)),
           " pctincrease 0) extent management dictionary ;"), col 1, dcdf_text,
          row + 2, dcdf_text = "The following tablespaces need to be created:", col 1,
          dcdf_text, row + 1, dcdf_dmt_new = 1
         ELSEIF (dcdf_ext_mgmt="L"
          AND dcdf_new_ind=1
          AND dcdf_lmt_new=0)
          row + 2, dcdf_text = "Sample Create Tablespace Statement (Locally Managed):", col 1,
          dcdf_text, row + 2, dcdf_text = concat(
           "sql> create tablespace <tablespace name> datafile [<file name>",
           " size <size in bytes> , ...] extent management local"),
          col 1, dcdf_text, row + 1,
          dcdf_text = "autoallocate segment space management auto ;", col 1, dcdf_text,
          row + 2, dcdf_text = "The following tablespaces need to be created:", col 1,
          dcdf_text, row + 1, dcdf_lmt_new = 1
         ENDIF
         dcdf_text = concat("  ",rtspace->qual[dcdf_tspace_cnt].tspace_name," needs ",trim(cnvtstring
           (rtspace->qual[dcdf_tspace_cnt].bytes_needed))," bytes "), col 1, dcdf_text,
         row + 1
        ENDIF
      ENDFOR
     ENDMACRO
    DETAIL
     dcdf_ext_mgmt = "D", dcdf_new_ind = 1, dcdf_macro,
     dcdf_ext_mgmt = "L", dcdf_new_ind = 1, dcdf_macro,
     dcdf_ext_mgmt = "D", dcdf_new_ind = 0, dcdf_macro,
     dcdf_ext_mgmt = "L", dcdf_new_ind = 0, dcdf_macro
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF ((rtspace->mode != "CREATE_DDL_ONLY"))
  FOR (tspacex = 1 TO size(rtspace->qual,5))
    IF ((rtspace->qual[tspacex].ct_err_ind=1))
     SET bkoutx = 1
     SET tspacex = size(rtspace->qual,5)
    ENDIF
  ENDFOR
  IF (bkoutx=1)
   EXECUTE dm2_tspace_error_rpt rtspace->install_type, rtspace->install_type_value
  ENDIF
 ENDIF
 IF ((rtspace->commands_written_ind=0)
  AND (rtspace->mode="CREATE_DDL_ONLY"))
  IF (dct_create_ddl_file(null)=0)
   SET dm_err->eproc = concat("Error in creating DDL REPORT.")
  ELSE
   SET dm_err->eproc = concat("DDL Report created successfully. Report File: ",rtspace->
    ddl_report_fname)
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
 ELSEIF ((rtspace->commands_written_ind=1)
  AND (rtspace->mode="CREATE_DDL_ONLY"))
  IF (dct_parse_to_logfile(null)=0)
   SET dm_err->eproc = concat("Error in creating DDL REPORT.")
  ELSE
   SET dm_err->eproc = concat("DDL Report created successfully. Report File: ",rtspace->
    ddl_report_fname)
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
 ELSEIF ((rtspace->mode != "CREATE_DDL_ONLY"))
  IF (dct_parse_to_logfile(null)=0)
   SET dm_err->eproc = concat("Error in creating DDL REPORT.")
  ELSE
   SET dm_err->eproc = concat("DDL Report created successfully. Report File: ",dm_err->logfile)
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
 ENDIF
 SET dm_err->eproc = "Ending dm2_create_tspace"
 CALL final_disp_msg(" ")
 IF ((rtspace->mode != "CREATE_DDL_ONLY"))
  IF (dct_signal_failure=0)
   SET dm_err->err_ind = 0
