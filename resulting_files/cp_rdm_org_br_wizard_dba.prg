CREATE PROGRAM cp_rdm_org_br_wizard:dba
 ...
 ENDIF
 DECLARE setupcporglevelsettings(null) = null
 DECLARE fetchbedrockreportscpcomponents(null) = null
 DECLARE copynewfilterstoexistingcomponents(null) = null
 DECLARE addind = i2 WITH protect, constant(1)
 DECLARE zeroflag = i2 WITH protect, constant(0)
 DECLARE treatmentoptionsreportid = f8 WITH protect, noconstant(0)
 DECLARE statuscompind = i2 WITH protect, constant(2)
 DECLARE idx = i2 WITH noconstant(0), protect
 DECLARE idx2 = i2 WITH noconstant(0), protect
 DECLARE reportcnt = i2 WITH noconstant(0), protect
 DECLARE ccnt = i2 WITH protect, noconstant(0)
 DECLARE treatmentoptionmean = vc WITH constant("PW_TREATMENT_OPTION"), protect
 SET readme_data->status = "F"
 SET readme_data->message = "Readme failure: Starting Script cp_rdm_org_br_wizard."
 CALL fetchbedrockreportscpcomponents(null)
 CALL setupcporglevelsettings(null)
 SUBROUTINE setupcporglevelsettings(null)
   SET stat = initrec(bed_ens_req)
   SET stat = initrec(bed_ens_rep)
   SELECT INTO "nl:"
    FROM br_datamart_category bdc
    WHERE bdc.category_mean=patstring("CP_ORG_SETTINGS*")
    WITH nocounter
   ;end select
   IF (curqual=0)
    SET bed_ens_req->action_flag = addind
    SET bed_ens_req->id = zeroflag
    SET bed_ens_req->display = concat("Care Pathway Organization Level Settings")
    SET bed_ens_req->identifier = concat("CP_ORG_SETTINGS","_",cnvtstring(cnvtdatetime(curdate,
       curtime)))
    SET bed_ens_req->layout_flag = zeroflag
    SET stat = alterlist(bed_ens_req->components,1)
    SET bed_ens_req->components[ccnt].action_flag = addind
    SET bed_ens_req->components[ccnt].id = treatmentoptionsreportid
   ...
   ENDIF
 END ;Subroutine
 CALL copynewfilterstoexistingcomponents(null)
 SUBROUTINE fetchbedrockreportscpcomponents(null)
   ...
   SET bed_get_datamart_cat_req->category_type_flag = 7
   EXECUTE bed_get_datamart_cat_reports  WITH replace(request,bed_get_datamart_cat_req), replace(
    reply,bed_get_datamart_cat_cp)
   SET reportcnt = size(bed_get_datamart_cat_cp->category[1].reports,5)
   SET idx2 = locateval(idx,1,reportcnt,"PW_TREATMENT_OPTION",bed_get_datamart_cat_cp->category[1].
    reports[idx].report_mean)
   SET treatmentoptionsreportid = bed_get_datamart_cat_cp->category[1].reports[idx2].
 ...
 END ;Subroutine
 SUBROUTINE copynewfilterstoexistingcomponents(null)
   SET stat = initrec(temp_filters)
   DECLARE i = i2 WITH noconstant(1), protect
   DECLARE j = i2 WITH noconstant(1), protect
   DECLARE k = i2 WITH noconstant(1), protect
   DECLARE l = i2 WITH noconstant(1), protect
   SELECT INTO "nl:"
    FROM br_datamart_filter bdf,
     br_datamart_category bdc
    PLAN (bdc
     WHERE bdc.category_mean="MP_PATHWAYS_STAND")
     JOIN (bdf
     WHERE bdf.br_datamart_category_id=bdc.br_datamart_category_id
      AND bdf.filter_mean IN ("PW_TO_LATEST_RECO_ORDER", "PW_TO_POWERPLAN_FAV_DISP",
     "PW_TREATMENT_OPTION_RECO_SEQ", "PW_TO_SHOW_ALL_TO_CHECKBOX", "PW_TO_SHOW_OFF_PATHWAY_BTN"))
    DETAIL
     stat = alterlist(temp_filters->filters,i), temp_filters->filters[i].filter_id = bdf
     .br_datamart_filter_id, temp_filters->filters[i].filter_mean = bdf.filter_mean,
     i += 1
    WITH nocounter
   ;end select
   IF (validate(debug_ind,0)=1)
    CALL echorecord(temp_filters)
   ENDIF
   SELECT INTO "nl:"
    FROM br_datamart_report bdr,
     br_datamart_category bdc
    PLAN (bdr
     WHERE bdr.report_mean=treatmentoptionmean)
     JOIN (bdc
     WHERE bdc.br_datamart_category_id=bdr.br_datamart_category_id)
    DETAIL
     IF (((bdc.category_mean="MP_PATHWAYS_STAND") OR (bdc.category_name=
     "Care Pathway Organization Level Settings")) )
      stat = 0
     ELSE
      stat = alterlist(temp_components->components,j), temp_components->components[j].component_id =
      bdr.br_datamart_report_id, temp_components->components[j].component_mean = treatmentoptionmean,
      updaterequest->category_id = bdc.br_datamart_category_id, j += 1
     ENDIF
    WITH nocounter
   ;end select
   IF (validate(debug_ind,0)=1)
    CALL echorecord(temp_components)
   ENDIF
   FOR (k = 1 TO size(temp_components->components,5))
     FOR (l = 1 TO size(temp_filters->filters,5))
       SET stat = initrec(updaterequest)
       SET updaterequest->components[1].component_id = temp_components->components[k].component_id
       SET updaterequest->components[1].component_mean = temp_components->components[k].
       component_mean
       SET stat = alterlist(updaterequest->components[k].filters,l)
       SET updaterequest->components[1].filters[l].filter_id = temp_filters->filters[l].filter_id
       SET updaterequest->components[1].filters[l].filter_mean = temp_filters->filters[l].filter_mean
       SET stat = alterlist(updaterequest->components[k].filters[l].flags,1)
       SET updaterequest->components[1].filters[l].flags[1].flag_type = 6
       EXECUTE cp_bed_upd_custom_mpage  WITH replace(request,updaterequest)
       IF (validate(debug_ind,0)=1)
