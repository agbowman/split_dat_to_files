CREATE PROGRAM dm2_ref_data_mover_1173:dba
 ...
 SUBROUTINE dm2_rdds_get_tgt_id(s_gmti_tgt_rs)
   ...
   RETURN(drgmti_return_val)
 END ;Subroutine
 DECLARE find_parent(cur_position=i4,parent_name=vc) = i4
 DECLARE alg_type = vc
 DECLARE merge_id = f8
 ...
 SET cd_value = uar_get_code_by("DISPLAY",255350,nullterm(cust_display))
 IF (cd_value < 0)
  SET spec_pvc_name = get_dyn_name(cust_display)
  CALL echo(spec_pvc_name)
  IF (spec_pvc_name != "")
   SET cd_value = uar_get_code_by("DISPLAY",255350,nullterm(spec_pvc_name))
   IF (cd_value < 0)
    SELECT INTO "NL:"
     FROM code_value c
    ...
    IF (curqual=0)
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "NOMV05"
     ROLLBACK
     SET drdm_no_trans_ind = 1
     GO TO exit_1173
    ELSE
     ...
     CALL merge_audit("FAILREASON","The code cache is out of sync for code_set 255350",2)
     CALL echo(dm2_ref_data_reply->error_msg)
     SET drdm_error_out_ind = 1
     GO TO exit_1173
    ENDIF
 ...
 IF ((rs_1173->from_values.parent_entity_name="DCP_INPUT_REF"))
  ...
  ENDIF
 ENDIF
 SET alg_type = uar_get_code_meaning(cd_value)
 CASE (alg_type)
  OF "ALG0":
  ...
  ELSE
   ...
   GO TO exit_1173
 ENDCASE
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[pvc_value_pos].translated=0)
  AND get_err_val(null) > 0)
  ROLLBACK
  SET drdm_chg->log[drdm_log_loop].next_to_process = find_parent(drdm_log_loop,rs_1173->from_values.
   parent_entity_name)
  GO TO exit_1173
 ENDIF
 ...
 END ;Subroutine
 SUBROUTINE find_parent(cur_position,parent_name)
   ...
   IF (parent_name IN ("DETAIL_PREFS", "VIEW_PREFS", "VIEW_COMP_PREFS"))
    WHILE ((((drdm_chg->log[return_pos].table_name != parent_name)) OR ((drdm_chg->log[return_pos].
    process_flg != 4))) )
      SET return_pos = drdm_chg->log[return_pos].next_to_process
    ENDWHILE
   ELSE
    WHILE ((drdm_chg->log[return_pos].table_name != parent_name))
      SET return_pos = drdm_chg->log[return_pos].next_to_process
    ENDWHILE
   ENDIF
   RETURN(return_pos)
