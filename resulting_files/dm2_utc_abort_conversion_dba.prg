CREATE PROGRAM dm2_utc_abort_conversion:dba
 ...
 SUBROUTINE dm2_get_dbase_name(dgdn_name_out)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE duac_backout_downtime(null) = i2
 DECLARE duac_backout_uptime(null) = i2
 DECLARE duac_backout_setup(null) = i2
 DECLARE duac_logfile_prefix = vc WITH protect, constant("dm2_utc_abort")
 DECLARE duac_backout_run = vc WITH protect, noconstant("not set")
 DECLARE duac_schema_changed = i2 WITH protect, noconstant(0)
 DECLARE duac_connect_str = vc WITH protect, noconstant("not set")
 DECLARE duac_dbase_name = vc WITH protect, noconstant(" ")
 IF (check_logfile(duac_logfile_prefix,".log","Starting DM2_UTC_ABORT_CONVERSION")=0)
  GO TO exit_program
 ENDIF
 IF (dm2_get_dbase_name(duac_dbase_name)=0)
  GO TO exit_program
 ENDIF
 SET dm2_install_schema->target_dbase_name = cnvtupper(duac_dbase_name)
 SET dm_err->eproc = "Checking for UTC CONVERSION status row on ADMIN DM_INFO."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm2_admin_dm_info di
  ...
  DETAIL
   ...
   status = di.info_name,
   dum_utc_data->in_process = 1
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ELSEIF ((dum_utc_data->in_process=0))
  SET dm_err->emsg = "The UTC CONVERSION process has not been initiated.  Exiting."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ELSEIF ((dum_utc_data->status IN ("SWITCH_START", "COMPLETE")))
  SET dm_err->emsg =
  "The UTC CONVERSION process is either complete or being finalized.  Cannot abort UTC Conversion."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 ...
 IF (dum_stop_conversion_runner("ALL")=0)
  GO TO exit_program
 ENDIF
 IF ((dum_utc_data->status IN ("SETUP_START", "SETUP_COMPLETE", "RESTART", "HOLD", "UPTIME",
 "DOWNTIME")))
  SET dm_err->eproc = "Getting the uptime and downtime run IDs."
  CALL disp_msg(" ",dm_err->logfile,0)
  IF ((dm_err->debug_flag > 1))
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_program
  ELSEIF (curqual=0)
   SET dm_err->emsg = "No run IDs found for uptime or downtime UTC conversion."
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Looking for downtime DDL that has been executed."
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM dm2_ddl_ops_log ddol
   WHERE (ddol.run_id=dum_utc_data->downtime_run_id)
  ...
  IF (curqual > 0)
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log ddol
    WHERE (ddol.run_id=dum_utc_data->downtime_run_id)
     AND  NOT (ddol.status IN ("COMPLETE", "ABORT_COMPLETE"))
    WITH nocounter, maxqual(ddol,1)
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   IF (curqual > 0)
    SET dm_err->emsg =
    "This process can only be initiated when all downtime ddl has been executed successfully."
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
   ELSE
    SET duac_schema_changed = 0
    IF (dum_check_for_new_run_id(dum_utc_data->downtime_run_id,duac_schema_changed,duac_dbase_name)=0
    )
     GO TO exit_program
    ENDIF
    SET dum_utc_data->schema_changed = duac_schema_changed
    IF ((dum_utc_data->schema_changed=1))
     SET dm_err->emsg =
     "Schema has changed since downtime schema changes were generated.  Cannot abort UTC Conversion."
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     GO TO exit_program
    ELSEIF ((dum_utc_data->schema_changed=0))
     SET duac_backout_run = "DOWNTIME"
    ENDIF
   ENDIF
  ELSE
   SET dm_err->eproc =
   "No downtime DDL has been completed.  Looking for uptime DDL that has been executed."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log ddol
   ...
   ENDIF
   IF (curqual > 0)
    SET duac_backout_run = "UPTIME"
   ELSEIF (curqual=0)
    SET duac_backout_run = "SETUP"
   ENDIF
  ENDIF
 ENDIF
 CASE (duac_backout_run)
  OF "DOWNTIME":
   IF (duac_backout_downtime(null)=0)
    GO TO exit_program
   ENDIF
   IF (duac_backout_uptime(null)=0)
    GO TO exit_program
   ENDIF
   IF (duac_backout_setup(null)=0)
    GO TO exit_program
   ENDIF
  OF "UPTIME":
   IF (duac_backout_uptime(null)=0)
 ...
 ENDCASE
 GO TO exit_program
 SUBROUTINE duac_backout_downtime(null)
   DECLARE dbd_tmp_cnt = i4 WITH protect, noconstant(0)
   DECLARE dbd_tmp_drop_str = vc WITH protect, noconstant("not set")
   FREE RECORD dbd_ops
   RECORD dbd_ops(
     1 op_cnt = i4
     1 op_qual[*]
   ...
   SET dbd_ops->op_cnt = 0
   SET dm_err->eproc = "Cleaning up downtime steps."
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dm_err->eproc = "Getting a list of foreign key constraints to drop."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    ddol.op_id, ddol.operation, ddol.table_name
    FROM dm2_ddl_ops_log ddol
    WHERE (ddol.run_id=dum_utc_data->downtime_run_id)
     AND ddol.op_type="DROP FK CONSTRAINT"
     AND ddol.priority=1300
     AND ddol.status="COMPLETE"
    DETAIL
     dbd_ops->op_cnt = (dbd_ops->op_cnt+ 1)
     IF (mod(dbd_ops->op_cnt,1000)=1)
      stat = alterlist(dbd_ops->op_qual,(dbd_ops->op_cnt+ 999))
     ENDIF
     dbd_ops->op_qual[dbd_ops->op_cnt].op_id = ddol.op_id, dbd_ops->op_qual[dbd_ops->op_cnt].
     operation = ddol.operation
    FOOT REPORT
     stat = alterlist(dbd_ops->op_qual,dbd_ops->op_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   ENDIF
   IF ((dbd_ops->op_cnt > 0))
    FOR (dbd_tmp_cnt = 1 TO dbd_ops->op_cnt)
      IF (dm2_push_cmd(dbd_ops->op_qual[dbd_tmp_cnt].operation,1)=0)
       RETURN(0)
      ENDIF
      UPDATE  FROM dm2_ddl_ops_log ddol
       SET ddol.status = "ABORT_COMPLETE"
       WHERE (ddol.op_id=dbd_ops->op_qual[dbd_tmp_cnt].op_id)
       WITH nocounter
      ;end update
      IF (check_error("Setting DDL row status to ABORT_COMPLETE")=1)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
       RETURN(0)
      ELSE
       COMMIT
   ...
   ENDIF
   SET dbd_ops->op_cnt = 0
   SET stat = alterlist(dbd_ops->op_qual,dbd_ops->op_cnt)
   SET dm_err->eproc = "Getting a list of primary key constraints to drop."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    ddol.op_id, ddol.operation, ddol.table_name
    FROM dm2_ddl_ops_log ddol
    WHERE (ddol.run_id=dum_utc_data->downtime_run_id)
     AND ddol.op_type="DROP PK CONSTRAINT"
     AND ddol.priority=1310
     AND ddol.status="COMPLETE"
    DETAIL
     dbd_ops->op_cnt = (dbd_ops->op_cnt+ 1)
     IF (mod(dbd_ops->op_cnt,1000)=1)
   ...
   SET dbd_ops->op_cnt = 0
   SET stat = alterlist(dbd_ops->op_qual,dbd_ops->op_cnt)
   SET dm_err->eproc = "Getting a list of indexes to rename."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    ddol.op_id, ddol.operation, ddol.table_name
    FROM dm2_ddl_ops_log ddol
    WHERE (ddol.run_id=dum_utc_data->downtime_run_id)
     AND ddol.op_type="RENAME INDEX"
     AND ddol.priority=1320
     AND ddol.status="COMPLETE"
    DETAIL
     dbd_ops->op_cnt = (dbd_ops->op_cnt+ 1)
     IF (mod(dbd_ops->op_cnt,1000)=1)
     ...
     ENDIF
     dbd_ops->op_qual[dbd_ops->op_cnt].op_id = ddol.op_id, dbd_ops->op_qual[dbd_ops->op_cnt].
     operation = replace(ddol.operation,"ZTC_TMP2","ZTC_TMP1",1)
    FOOT REPORT
     stat = alterlist(dbd_ops->op_qual,dbd_ops->op_cnt)
    WITH nocounter
   ;end select
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     ...
     ENDIF
     dbd_ops->op_qual[dbd_ops->op_cnt].op_id = ddol.op_id, dbd_ops->op_qual[dbd_ops->op_cnt].
     operation = replace(ddol.operation,"ZTC_TMP1","ZTC_TMP2",1)
    FOOT REPORT
     stat = alterlist(dbd_ops->op_qual,dbd_ops->op_cnt)
    WITH nocounter
   ;end select
   ...
   SET dbd_ops->op_cnt = 0
   SET stat = alterlist(dbd_ops->op_qual,dbd_ops->op_cnt)
   SET dm_err->eproc = "Getting a list of columns to rename."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log ddol
    WHERE (ddol.run_id=dum_utc_data->downtime_run_id)
     AND ddol.op_type="RENAME COLUMN"
     AND ddol.priority=2010
     AND ddol.status="COMPLETE"
    DETAIL
     dbd_ops->op_cnt = (dbd_ops->op_cnt+ 1)
     IF (mod(dbd_ops->op_cnt,1000)=1)
     ...
     ENDIF
     dbd_ops->op_qual[dbd_ops->op_cnt].op_id = ddol.op_id, dbd_ops->op_qual[dbd_ops->op_cnt].
     operation = replace(ddol.operation,"UTC_TMP2","UTC_TMP1",1)
    FOOT REPORT
     stat = alterlist(dbd_ops->op_qual,dbd_ops->op_cnt)
    WITH nocounter
   ;end select
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     ...
     ENDIF
     dbd_ops->op_qual[dbd_ops->op_cnt].op_id = ddol.op_id, dbd_ops->op_qual[dbd_ops->op_cnt].
     operation = replace(ddol.operation,"UTC_TMP1","UTC_TMP2",1)
    FOOT REPORT
     stat = alterlist(dbd_ops->op_qual,dbd_ops->op_cnt)
    WITH nocounter
   ;end select
   ...
   SET dbd_ops->op_cnt = 0
   SET stat = alterlist(dbd_ops->op_qual,dbd_ops->op_cnt)
   SET dm_err->eproc = "Getting a list of primary key constraints to create."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log ddol
    WHERE (ddol.run_id=dum_utc_data->downtime_run_id)
     AND ddol.op_type="CREATE PK CONSTRAINT"
     AND ddol.priority=2410
     AND ddol.status="COMPLETE"
    DETAIL
     dbd_ops->op_cnt = (dbd_ops->op_cnt+ 1)
     IF (mod(dbd_ops->op_cnt,1000)=1)
   ...
   SET dbd_ops->op_cnt = 0
   SET stat = alterlist(dbd_ops->op_qual,dbd_ops->op_cnt)
   SET dm_err->eproc = "Getting a list of foreign key constraints to create."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log ddol
    WHERE (ddol.run_id=dum_utc_data->downtime_run_id)
     AND ddol.op_type="CREATE FK CONSTRAINT"
     AND ddol.priority=2420
     AND ddol.status="COMPLETE"
    DETAIL
     dbd_ops->op_cnt = (dbd_ops->op_cnt+ 1)
     IF (mod(dbd_ops->op_cnt,1000)=1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE duac_backout_uptime(null)
   DECLARE dbu_tmp_cnt = i4 WITH protect, noconstant(0)
   DECLARE dbu_tmp2_cnt = i4 WITH protect, noconstant(0)
   DECLARE dbu_tmp_str = vc WITH protect, noconstant("not set")
   SET dm_err->eproc = "Cleaning up uptime steps."
   CALL disp_msg(" ",dm_err->logfile,0)
   FREE RECORD dbu_idx
   RECORD dbu_idx(
     1 idx_cnt = i4
   ...
   SET dbu_idx->idx_cnt = 0
   FREE RECORD dbu_cols
   RECORD dbu_cols(
     1 tab_cnt = i4
     1 tab_qual[*]
       2 table_name = vc
   ...
   SET dbu_trg->trg_cnt = 0
   SET dm_err->eproc = "Getting a list of temporary downtime triggers to drop."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    ut.trigger_name
    FROM user_triggers ut
    WHERE ((ut.trigger_name="UTC_CONVERT*") OR (ut.trigger_name="UTC_REVERSE*"))
    DETAIL
     dbu_trg->trg_cnt = (dbu_trg->trg_cnt+ 1)
     IF (mod(dbu_trg->trg_cnt,1000)=1)
      stat = alterlist(dbu_trg->trg_qual,(dbu_trg->trg_cnt+ 999))
     ENDIF
     dbu_trg->trg_qual[dbu_trg->trg_cnt].trg_name = ut.trigger_name
    FOOT REPORT
     stat = alterlist(dbu_trg->trg_qual,dbu_trg->trg_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF ((dbu_trg->trg_cnt > 0))
    FOR (dbd_tmp_cnt = 1 TO dbu_trg->trg_cnt)
     SET dbu_tmp_str = concat("rdb drop trigger ",dbu_trg->trg_qual[dbd_tmp_cnt].trg_name," go")
     IF (dm2_push_cmd(dbu_tmp_str,1)=0)
      RETURN(0)
     ENDIF
    ENDFOR
   ENDIF
   SET dm_err->eproc = "Getting a list of temporary indexes to drop."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    ui.index_name
    FROM dm2_user_indexes ui
    WHERE ui.index_name="ZTC_TMP1*"
    DETAIL
     dbu_idx->idx_cnt = (dbu_idx->idx_cnt+ 1)
     IF (mod(dbu_idx->idx_cnt,1000)=1)
      stat = alterlist(dbu_idx->idx_qual,(dbu_idx->idx_cnt+ 999))
     ENDIF
     dbu_idx->idx_qual[dbu_idx->idx_cnt].idx_name = ui.index_name
    FOOT REPORT
     stat = alterlist(dbu_idx->idx_qual,dbu_idx->idx_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF ((dbu_idx->idx_cnt > 0))
    FOR (dbu_tmp_cnt = 1 TO dbu_idx->idx_cnt)
     SET dbu_tmp_str = concat("rdb asis (^drop index ",dbu_idx->idx_qual[dbu_tmp_cnt].idx_name,
      " ^) go")
     IF (dm2_push_cmd(dbu_tmp_str,1)=0)
      RETURN(0)
     ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE duac_backout_setup(null)
   DECLARE dbs_convert_function_exists = i2 WITH protect, noconstant(0)
   DECLARE dbs_reverse_function_exists = i2 WITH protect, noconstant(0)
   SET dm_err->eproc = "Cleaning up setup steps."
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dm_err->eproc = "Deleting 'UTC DATA' and 'UTC CONVERSION' status ADMIN dm_info rows..."
   ...
   DELETE  FROM dm2_ddl_ops_log d
    WHERE d.run_id IN (dum_utc_data->uptime_run_id, dum_utc_data->downtime_run_id)
    WITH nocounter
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     IF (uo.object_name="DM2_UTC_CONVERT_PKG")
      dbs_convert_function_exists = 1
     ELSEIF (uo.object_name="DM2_UTC_REVERSE_PKG")
      dbs_reverse_function_exists = 1
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   IF (dbs_convert_function_exists=1)
    ...
    ENDIF
   ENDIF
   IF (dbs_reverse_function_exists=1)
 ...
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Finished DM2_UTC_ABORT_CONVERSION."
 CALL final_disp_msg(duac_logfile_prefix)
END GO
