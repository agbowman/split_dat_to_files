CREATE PROGRAM dm2_ref_data_mover_7997:dba
 ...
 SET cust_action_id = cnvtreal(get_value(cust_tab_name,"SA_REF_ACTION_ITEM_ID","FROM"))
 SET cust_par_t_name = dm2_get_rdds_tname("SA_REF_ACTION_ITEM")
 SELECT INTO "NL:"
  FROM (parser(cust_par_t_name) sa)
  WHERE sa.sa_ref_action_item_id=cust_action_id
  DETAIL
   cust_type_flag = sa.value_type_flag
  WITH nocounter
 ;end select
 IF (cust_type_flag=4)
  SET cust_action_value = get_value(cust_tab_name,"ACTION_VALUE","FROM")
  SET cust_action_val_pos = get_col_pos(cust_tab_name,"ACTION_VALUE")
  SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_action_val_pos].translated = 0
  SET cust_to_action_val = select_merge_translate(cust_action_value,"PRSNL")
  IF (cust_to_action_val != "No Trans")
   CALL put_value(cust_tab_name,"ACTION_VALUE",cust_to_action_val)
  ELSE
   SET cust_rpt_missing = report_missing("PRSNL","PERSON_ID",cnvtreal(cust_action_value))
   CALL evaluate_rpt_missing(cust_rpt_missing)
  ENDIF
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"SA_REF_MACRO_ACTION_ITEM_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"SA_REF_MACRO_ACTION_ITEM_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"SA_REF_MACRO_ACTION_ITEM_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,
       "  SA_REF_MACRO_ACTION_ITEM_ID")
     ELSE
      GO TO exit_7997
     ENDIF
    ELSE
   ...
   SET cust_from_cd = get_value(cust_tab_name,"SA_REF_MACRO_ACTION_ITEM_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_7997
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
   ENDIF
   SET cust_new_cd = get_value(cust_tab_name,"SA_REF_MACRO_ACTION_ITEM_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_7997
   ELSE
 ...
 IF (cust_idcd_check=1)
  SET cust_idcd_check = is_translated(cust_tab_name,"ACTION_VALUE")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "This row wasn't translated because of the ACTION_VALUE"
