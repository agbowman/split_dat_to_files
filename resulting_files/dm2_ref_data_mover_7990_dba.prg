CREATE PROGRAM dm2_ref_data_mover_7990:dba
 ...
 DECLARE cust_set_name = vc
 DECLARE cust_new_set_name = vc
 DECLARE cust_type_cd = vc
 DECLARE cust_cdf_meaning = vc
 DECLARE cust_set_name_pos = i4
 DECLARE cust_type_cd_pos = i4
 DECLARE cust_loop = i4
 DECLARE cust_err_val = i4
 DECLARE cust_pe_tab = vc
 DECLARE cust_pe_col = vc
 DECLARE cust_rpt_val = vc
 DECLARE cust_value_cdf = vc
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_set_name_pos = get_col_pos(cust_tab_name,"PREFERENCE_SET_NAME")
 SET cust_type_cd_pos = get_col_pos(cust_tab_name,"PREFERENCE_TYPE_CD")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_type_cd_pos].translated=1))
  SET cust_type_cd = get_value(cust_tab_name,"PREFERENCE_TYPE_CD","TO")
  SELECT INTO "nl:"
   FROM code_value cv
   WHERE cv.code_value=cnvtreal(cust_type_cd)
   DETAIL
    cust_value_cdf = cv.cdf_meaning
   WITH nocounter
  ;end select
  ...
  CASE (cust_value_cdf)
   OF "CASETRACKING":
    SET cust_pe_tab = "VIEW_COMP_PREFS"
    SET cust_pe_col = "VIEW_COMP_PREFS_ID"
   OF "CTMILESTONE":
   OF "DVLIST":
    SET cust_pe_tab = "CODE_VALUE"
    SET cust_pe_col = "CODE_VALUE"
  ENDCASE
  IF (cust_pe_tab != "")
   SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_set_name_pos].translated = 0
   SET cust_set_name = get_value(cust_tab_name,"PREFERENCE_SET_NAME","FROM")
   SET cust_set_new_name = select_merge_translate(cust_set_name,cust_pe_tab)
   IF (cust_set_new_name != "No Trans")
    CALL put_value(cust_tab_name,"PREFERENCE_SET_NAME",cust_set_new_name)
   ELSE
    SET cust_rpt_val = report_missing(cust_pe_tab,cust_pe_col,cnvtreal(cust_set_name))
 ...
 ELSE
  ...
  GO TO exit_7990
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"SA_PREFERENCE_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"SA_PREFERENCE_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"SA_PREFERENCE_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"SA_PREFERENCE_ID")
     ELSE
      GO TO exit_7990
     ENDIF
    ELSE
   ...
   SET cust_from_cd = get_value(cust_tab_name,"SA_PREFERENCE_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_7990
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
   ENDIF
   SET cust_new_cd = get_value(cust_tab_name,"SA_PREFERENCE_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_7990
   ELSE
 ...
 IF (cust_idcd_check=1)
  SET cust_idcd_check = is_translated(cust_tab_name,"PREFERENCE_SET_NAME")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    ...
    "This row wasn't translated because of the PREFERENCE_SET_NAME"
