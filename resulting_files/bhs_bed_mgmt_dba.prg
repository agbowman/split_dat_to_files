CREATE PROGRAM bhs_bed_mgmt:dba
 PROMPT
 ...
 DECLARE dischobv = f8 WITH constant(uar_get_code_by("DISPLAYKEY",69,"DISCHOBV")), protect
 DECLARE dischip = f8 WITH constant(uar_get_code_by("DISPLAYKEY",69,"DISCHIP")), protect
 ...
 ELSEIF (datetimediff(end_date_qual,beg_date_qual) > 5)
  CALL echo("Date range > 5")
  SELECT INTO  $OUTDEV
   FROM dummyt
   HEAD REPORT
    msg1 = "Your date range is larger than 5 days.", msg2 = "  Please retry.", col 0,
 ...
 SELECT
  IF (any_loc_ind="C")
   PLAN (e
    WHERE e.disch_dt_tm BETWEEN cnvtdatetime(beg_date_qual) AND cnvtdatetime(end_date_qual)
     AND ((e.encntr_type_class_cd+ 0) IN (obs, inpatient, dischobv, dischip)))
    JOIN (ehx
    WHERE ehx.encntr_id=e.encntr_id
     AND ((ehx.end_effective_dt_tm+ 0) >= e.disch_dt_tm)
     AND ((ehx.loc_nurse_unit_cd+ 0) != 0)
     AND ((ehx.loc_facility_cd+ 0)= $FNAME))
    JOIN (o
    WHERE o.encntr_id=e.encntr_id
     AND ((o.catalog_cd+ 0)=dischargepatient_var))
  ELSEIF (( $5 > 0))
   PLAN (ehx
    WHERE (ehx.loc_nurse_unit_cd= $NUNIT)
     AND ehx.end_effective_dt_tm BETWEEN cnvtdatetime(beg_date_qual) AND cnvtdatetime(end_date_qual)
     AND ehx.loc_nurse_unit_cd != 0)
    JOIN (e
    WHERE e.encntr_id=ehx.encntr_id
     AND e.disch_dt_tm BETWEEN cnvtdatetime(beg_date_qual) AND cnvtdatetime(end_date_qual)
     AND ((e.encntr_type_class_cd+ 0) IN (obs, inpatient, dischobv, dischip)))
    JOIN (o
    WHERE o.encntr_id=e.encntr_id
     AND ((o.catalog_cd+ 0)=dischargepatient_var))
  ELSE
   PLAN (ehx
    WHERE ((ehx.loc_facility_cd+ 0)= $FNAME)
     AND ehx.end_effective_dt_tm BETWEEN cnvtdatetime(beg_date_qual) AND cnvtdatetime(end_date_qual)
     AND ehx.loc_nurse_unit_cd != 0
     AND ehx.loc_facility_cd != 0)
    JOIN (e
    WHERE e.encntr_id=ehx.encntr_id
     AND e.disch_dt_tm BETWEEN cnvtdatetime(beg_date_qual) AND cnvtdatetime(end_date_qual)
     AND ((e.encntr_type_class_cd+ 0) IN (obs, inpatient, dischobv, dischip)))
