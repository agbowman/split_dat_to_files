CREATE PROGRAM bhs_rad_audit_chgrpt
 PROMPT
  "Output to File/Printer/MINE" = "MINE"
  WITH outdev
 DECLARE mf_rad_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",106,"RADIOLOGY"))
 DECLARE mf_bo_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",13019,"BILLCODE"))
 SELECT DISTINCT INTO  $OUTDEV
  procedure_description = b.ext_description, cdm_cpt = bi.key6, charge_description = bi.key7,
  bill_item_type = uar_get_code_display(bi.bill_item_type_cd), active_status_cd =
  uar_get_code_display(bi.active_status_cd)
  FROM bill_item b,
