CREATE PROGRAM acm_chg_person_name:dba
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
   person_name p
  SET p.person_name_id = acm_request->person_name_qual[xref->chg[d.seq].idx].person_name_id, p
   .beg_effective_dt_tm = cnvtdatetime(acm_request->person_name_qual[xref->chg[d.seq].idx].
    beg_effective_dt_tm), p.contributor_system_cd =
   IF ((acm_request->person_name_qual[xref->chg[d.seq].idx].contributor_system_cd > 0.0)) acm_request
    ->person_name_qual[xref->chg[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   p.end_effective_dt_tm = cnvtdatetime(acm_request->person_name_qual[xref->chg[d.seq].idx].
    end_effective_dt_tm), p.name_degree = acm_request->person_name_qual[xref->chg[d.seq].idx].
   name_degree, p.name_first = acm_request->person_name_qual[xref->chg[d.seq].idx].name_first,
   p.name_first_key = trim(cnvtupper(cnvtalphanum(acm_request->person_name_qual[xref->chg[d.seq].idx]
      .name_first)),3), p.name_format_cd = acm_request->person_name_qual[xref->chg[d.seq].idx].
   name_format_cd, p.name_full = acm_request->person_name_qual[xref->chg[d.seq].idx].name_full,
   p.name_initials = acm_request->person_name_qual[xref->chg[d.seq].idx].name_initials, p.name_last
    = acm_request->person_name_qual[xref->chg[d.seq].idx].name_last, p.name_last_key = trim(cnvtupper
    (cnvtalphanum(acm_request->person_name_qual[xref->chg[d.seq].idx].name_last)),3),
   p.name_middle = acm_request->person_name_qual[xref->chg[d.seq].idx].name_middle, p.name_middle_key
    = trim(cnvtupper(cnvtalphanum(acm_request->person_name_qual[xref->chg[d.seq].idx].name_middle)),3
    ), p.name_prefix = acm_request->person_name_qual[xref->chg[d.seq].idx].name_prefix,
   p.name_suffix = acm_request->person_name_qual[xref->chg[d.seq].idx].name_suffix, p.name_title =
   acm_request->person_name_qual[xref->chg[d.seq].idx].name_title, p.name_type_cd = acm_request->
   person_name_qual[xref->chg[d.seq].idx].name_type_cd,
   p.person_id =
   IF ((acm_request->person_name_qual[xref->chg[d.seq].idx].person_id > 0)) acm_request->
    person_name_qual[xref->chg[d.seq].idx].person_id
   ELSE reply->person_qual[acm_request->person_name_qual[xref->chg[d.seq].idx].person_idx].person_id
   ENDIF
   , p.source_identifier = acm_request->person_name_qual[xref->chg[d.seq].idx].source_identifier, p
   .name_type_seq = acm_request->person_name_qual[xref->chg[d.seq].idx].name_type_seq,
   p.active_ind = acm_request->person_name_qual[xref->chg[d.seq].idx].active_ind, p.active_status_cd
    = acm_request->person_name_qual[xref->chg[d.seq].idx].active_status_cd, p.active_status_prsnl_id
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.person_name_id=acm_request->person_name_qual[xref->chg[d.seq].idx].person_name_id))
  WITH nocounter, status(reply->person_name_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->person_name_qual[xref->chg[index].idx].status != 1))
 ...
 ENDFOR
 IF (acm_hist_ind=1)
  EXECUTE acm_chg_person_name_hist
 ...
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    FROM person_name p
    WHERE expand(t1,f_val,t_val,p.person_name_id,acm_request->person_name_qual[xref->chg[t1].idx].
     person_name_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.person_name_id,acm_request->person_name_qual[xref->chg[t1].idx].
      person_name_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((p.updt_cnt=acm_request->person_name_qual[idx].updt_cnt)) OR ((acm_request->force_updt_ind
     =1))) )
      reply->person_name_qual[idx].status = - (1), reply->person_name_qual[idx].person_name_id =
      acm_request->person_name_qual[idx].person_name_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->person_name_qual[idx].chg_str
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->person_name_qual[idx].beg_effective_dt_tm = p.beg_effective_dt_tm
     ENDIF
     IF (findstring("CONTRIBUTOR_SYSTEM_CD,",chg_str)=0)
      acm_request->person_name_qual[idx].contributor_system_cd = p.contributor_system_cd
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->person_name_qual[idx].end_effective_dt_tm = p.end_effective_dt_tm
     ENDIF
     IF (findstring("NAME_DEGREE,",chg_str)=0)
      acm_request->person_name_qual[idx].name_degree = p.name_degree
     ENDIF
     IF (findstring("NAME_FIRST,",chg_str)=0)
      acm_request->person_name_qual[idx].name_first = p.name_first
     ENDIF
     IF (findstring("NAME_FORMAT_CD,",chg_str)=0)
      acm_request->person_name_qual[idx].name_format_cd = p.name_format_cd
     ENDIF
     IF (findstring("NAME_FULL,",chg_str)=0)
      acm_request->person_name_qual[idx].name_full = p.name_full
     ENDIF
     IF (findstring("NAME_INITIALS,",chg_str)=0)
      acm_request->person_name_qual[idx].name_initials = p.name_initials
     ENDIF
     IF (findstring("NAME_LAST,",chg_str)=0)
      acm_request->person_name_qual[idx].name_last = p.name_last
     ENDIF
     IF (findstring("NAME_MIDDLE,",chg_str)=0)
      acm_request->person_name_qual[idx].name_middle = p.name_middle
     ENDIF
     IF (findstring("NAME_PREFIX,",chg_str)=0)
      acm_request->person_name_qual[idx].name_prefix = p.name_prefix
     ENDIF
     IF (findstring("NAME_SUFFIX,",chg_str)=0)
      acm_request->person_name_qual[idx].name_suffix = p.name_suffix
     ENDIF
     IF (findstring("NAME_TITLE,",chg_str)=0)
      acm_request->person_name_qual[idx].name_title = p.name_title
     ENDIF
     IF (findstring("NAME_TYPE_CD,",chg_str)=0)
      acm_request->person_name_qual[idx].name_type_cd = p.name_type_cd
     ENDIF
     IF (findstring("PERSON_ID,",chg_str)=0)
      acm_request->person_name_qual[idx].person_id = p.person_id
     ENDIF
     IF (findstring("SOURCE_IDENTIFIER,",chg_str)=0)
      acm_request->person_name_qual[idx].source_identifier = p.source_identifier
     ENDIF
     IF (findstring("NAME_TYPE_SEQ,",chg_str)=0)
      acm_request->person_name_qual[idx].name_type_seq = p.name_type_seq
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->person_name_qual[idx].active_ind = p.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->person_name_qual[idx].active_status_cd = p.active_status_cd
     ENDIF
