CREATE PROGRAM dm_async_exec_monthly:dba
 ...
 SUBROUTINE async_exec_script(appid,taskid,reqid,scriptname)
   ...
   ENDIF
 END ;Subroutine
 SET stat = alterlist(async_exec->qual,0)
 FOR (itr = 1 TO size(async_exec->qual,5))
   CALL echo(build("Executing Async Monthly script: ",async_exec->qual[itr].scriptname))
