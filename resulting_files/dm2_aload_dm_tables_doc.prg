CREATE PROGRAM dm2_aload_dm_tables_doc
 ...
 ENDIF
 IF (check_logfile("dm2_aload_dtd",".log","dm2_aload_dm_tables_doc")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_dm_tables_doc"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dadtd_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dadtd_file = vc WITH protect, noconstant( $2)
 DECLARE dadtd_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE dadtd_batch = i4 WITH protect, noconstant(0)
 DECLARE dadtd_file_loc_ret = vc WITH protect, noconstant("")
 DECLARE dadtd_curqual = i4 WITH noconstant(0)
 IF (dac_prelim(dadtd_pkg,dadtd_file_loc_ret,dadtd_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dadtd_batch=0)
  SET dadtd_batch = 5000
 ENDIF
 IF (dadtd_expected_rows > 0)
  EXECUTE dm2_dbimport concat(dadtd_file_loc_ret,dadtd_file), "dm2_aload_adoc_dtd", dadtd_batch
  CALL echo(concat(dadtd_file_loc_ret,dadtd_file))
 ENDIF
...
#exit_program
 ...
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_dm_tables_doc completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_dm_tables_doc")
END GO
