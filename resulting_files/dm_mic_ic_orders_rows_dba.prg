CREATE PROGRAM dm_mic_ic_orders_rows:dba
 ...
 SET reply->table_name = "MIC_IC_ORDERS"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(mo.order_id)
   FROM mic_ic_orders mo
   WHERE mo.order_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    mo.rowid
    FROM mic_ic_orders mo
    WHERE parser(sbr_getrowidnotexists("mo.order_id between curMinID and curMaxID","mo"))
     AND mo.complete_dt_tm <= cnvtdatetime(purge_dt_tm)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
      stat = alterlist(reply->rows,(rows+ 49))
     ENDIF
     reply->rows[rows].row_id = mo.rowid
    WITH nocounter, maxqual(mo,value(rowsleft))
   ;end select
