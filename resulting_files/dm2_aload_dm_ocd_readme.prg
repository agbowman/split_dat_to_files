CREATE PROGRAM dm2_aload_dm_ocd_readme
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_ocdreadme",".log","dm2_aload_dm_ocd_readme")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_dm_ocd_readme"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dador_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dador_file = vc WITH protect, noconstant( $2)
 DECLARE dador_row_cnt = i4 WITH protect, noconstant( $3)
 DECLARE dador_batch = i4 WITH protect, noconstant(0)
 DECLARE dador_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(dador_pkg,dador_file_loc_ret,dador_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dador_batch=0)
  SET dador_batch = 5000
 ENDIF
 SET dm_err->eproc = build(
  "Remove existing readmes from dm_ocd_readme for package since no readmes on package:",dador_pkg)
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 DELETE  FROM dm_ocd_readme r
  WHERE r.ocd=dador_pkg
   AND  EXISTS (
  (SELECT
   "x"
   FROM dm_readme dr
   WHERE dr.owner=currdbuser
    AND dr.readme_id=r.readme_id
    AND dr.instance=r.instance))
  WITH nocounter
 ;end delete
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 COMMIT
 IF (dador_row_cnt > 0)
  EXECUTE dm2_dbimport concat(dador_file_loc_ret,dador_file), "dm2_aload_rdm_dor", dador_batch
  CALL echo(concat(dador_file_loc_ret,dador_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_dm_ocd_readme completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_dm_ocd_readme")
END GO
