CREATE PROGRAM dm_rmc_execute_task:dba
 ...
 SUBROUTINE daf_is_not_blank(dinb_str)
  ...
  RETURN(false)
 END ;Subroutine
 DECLARE dret_task = vc WITH protect, noconstant("")
 DECLARE dret_request = vc WITH protect, noconstant("")
 DECLARE dret_reply = vc WITH protect, noconstant("")
 DECLARE dret_task_id = f8 WITH protect, noconstant(0.0)
 DECLARE dret_status = c1 WITH protect, noconstant("F")
 DECLARE dret_idx = i4 WITH protect, noconstant(0)
 DECLARE dret_error_item = vc WITH protect, noconstant("")
 DECLARE dret_error_ind = i2 WITH protect, noconstant(0)
 DECLARE dret_result_vc = vc WITH protect, noconstant("")
 DECLARE dret_result_f8 = f8 WITH protect, noconstant(0.0)
 DECLARE dret_result_i4 = i4 WITH protect, noconstant(0)
 SET dm_err->eproc = "Starting dm_rmc_execute_task"
 IF (check_logfile("dm_rmc_execute_task",".log","DM_RMC_EXECUTE_TASK LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (((reflect(parameter(1,0)) != "C*") OR (((reflect(parameter(2,0)) != "C*") OR (((reflect(
  parameter(3,0)) != "C*") OR (reflect(parameter(4,0)) != "I*"
  AND reflect(parameter(4,0)) != "F*")) )) )) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Expected syntax:  dm_rmc_execute_task <task>,<task_request>",
   ", <task_reply>, <task_queue_id>")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dret_task =  $1
 ...
 SET dret_reply =  $3
 SET dret_task_id =  $4
 SET stat = initrec(parsed_task_data)
 IF (((daf_is_not_blank(dret_request)) OR (daf_is_not_blank(dret_reply))) )
  SET dret_status = drtq_parse_task_data(dret_request,dret_reply,parsed_task_data)
  IF (dret_status="F")
   CALL disp_msg("Call to drtq_parse_task_data returned an error",dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  IF (daf_is_not_blank(parsed_task_data->request_definition))
   SET dm_err->eproc = "Declaring task request structure."
   CALL parser(parsed_task_data->request_definition,1)
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
   ENDIF
  ENDIF
  SET dm_err->eproc = "Declaring task variables."
  FOR (dret_idx = 1 TO parsed_task_data->var_cnt)
   CALL parser(parsed_task_data->var_qual[dret_idx].variable_declaration,1)
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
   ENDIF
  ENDFOR
  SET dm_err->eproc = "Populating required fields for task."
  FOR (dret_idx = 1 TO parsed_task_data->set_cnt)
   CALL parser(parsed_task_data->set_qual[dret_idx].set_command,1)
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
   ENDIF
  ENDFOR
  IF (daf_is_not_blank(parsed_task_data->reply_definition))
   SET dm_err->eproc = "Declaring task reply structure."
   CALL parser(parsed_task_data->reply_definition,1)
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
   ENDIF
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Executing task."
 CALL parser(dret_task,1)
 IF (daf_is_not_blank(dret_reply))
  IF (cnvtupper(parsed_task_data->error_type) IN ("VC", "C*"))
   CALL parser(concat("set dret_result_vc = ",parsed_task_data->error_ind_item," go"),1)
   IF ((dret_result_vc != parsed_task_data->no_err_result))
    SET dret_error_ind = 1
   ENDIF
  ELSEIF (cnvtupper(parsed_task_data->error_type)="I4")
   CALL parser(concat("set dret_result_i4 = ",parsed_task_data->error_ind_item," go"),1)
   IF (dret_result_i4 != cnvtint(parsed_task_data->no_err_result))
    SET dret_error_ind = 1
   ENDIF
  ELSEIF (cnvtupper(parsed_task_data->error_type)="F8")
   CALL parser(concat("set dret_result_f8 = ",parsed_task_data->error_ind_item," go"),1)
   IF (dret_result_f8 != cnvtreal(parsed_task_data->no_err_result))
    SET dret_error_ind = 1
   ENDIF
  ENDIF
  IF (dret_error_ind=1)
   IF (daf_is_not_blank(parsed_task_data->error_msg_item))
    SET dm_err->err_ind = 0
    CALL parser(concat(
      "set dret_status = drtq_update_task_process(dret_task_id,dm_err->logfile,'ERROR',",
      parsed_task_data->error_msg_item,") go"),1)
   ELSE
    CALL check_error(dm_err->eproc)
    SET dm_err->err_ind = 0
    SET dret_status = drtq_update_task_process(dret_task_id,dm_err->logfile,"ERROR",dm_err->emsg)
   ENDIF
   IF (dret_status="F")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
 ...
 ENDIF
 IF (dret_error_ind=0)
  SET dm_err->eproc = "Marking task as finished."
  SET dret_status = drtq_update_task_process(dret_task_id,dm_err->logfile,"FINISHED","")
  IF (dret_status="F")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ENDIF
 ENDIF
#exit_program
 CALL final_disp_msg("dm_rmc_execute_task")
END GO
