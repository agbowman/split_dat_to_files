CREATE PROGRAM dc_mp_upd_sticky_note:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "update_flag" = 0,
  "sticky_note_id" = 0.0,
  "sticky_note_type_mean" = "",
  "sticky_note_type_cd" = 0.0,
  "parent_entity_id" = 0.0,
  "parent_entity_name" = "PERSON",
  "sticky_note_text" = "",
  "sticky_note_status_mean" = "",
  "sticky_note_status_cd" = 0.0,
  "public_ind" = 0,
  "valid_until_date" = "",
  "persist_reply_flag" = 0
  WITH outdev, uflag, snid,
  sncdf, scd, pid,
  pn, snt, statcdf,
  statcd, pub, vdt,
  pstrep
 ...
 DECLARE getpasseddata(null) = i4
 DECLARE addstickynote(null) = i4
 DECLARE getnewsnid(null) = f8
 DECLARE getnewltid(null) = f8
 DECLARE sscript_name = vc WITH protect, constant("DC_MP_UPD_STICKY_NOTE")
 DECLARE ssntext = vc WITH protect, constant(formathtmlcharactercodes( $SNT))
 DECLARE dpid = f8 WITH protect, constant(cnvtreal( $PID))
 DECLARE dactive = f8 WITH protect, constant(uar_get_code_by("MEANING",48,"ACTIVE"))
 DECLARE nuflag = i2 WITH protect, constant( $UFLAG)
 DECLARE npubind = i2 WITH protect, constant( $PUB)
 DECLARE dqvuntil_dt = dq8 WITH protect
 DECLARE ndelete = i2 WITH protect, constant(0)
 DECLARE nadd = i2 WITH protect, constant(1)
 DECLARE nmodify = i2 WITH protect, constant(2)
 DECLARE dsnid = f8 WITH protect, noconstant(cnvtreal( $SNID))
 DECLARE dnewlongtextid = f8 WITH protect, noconstant(0.0)
 DECLARE dsncd = f8 WITH protect, noconstant(0.0)
 DECLARE dstatcd = f8 WITH protect, noconstant(0.0)
 DECLARE sname = vc WITH protect, noconstant(" ")
 DECLARE sbegdttm = vc WITH protect, noconstant(" ")
 DECLARE senddttm = vc WITH protect, noconstant(" ")
 DECLARE lstat = i4 WITH protect, noconstant(0)
 SET reply->status_data.status = "F"
 CASE (nuflag)
  OF ndelete:
   SET reply->commit_type = "DELETE"
   SET lstat = delstickynote(dsnid)
  OF nadd:
   SET lstat = getpasseddata(0)
   SET reply->commit_type = "ADD"
   SET lstat = addstickynote(0)
  OF nmodify:
   SET lstat = getpasseddata(0)
   SET reply->commit_type = "MODIFY"
   SET lstat = updstickynote(dsnid)
  ELSE
   GO TO exit_script
 ENDCASE
 SET reply->sticky_note_id = dsnid
 SET reply->sticky_note_text = ssntext
 SET reply->sticky_note_type_cd =  $SCD
 SET reply->updt_id = reqinfo->updt_id
 SET reply->updt_dt_tm = cnvtdatetime(sysdate)
 SET reply->supdt_dt_tm = format(reply->updt_dt_tm,"@SHORTDATETIME")
 SELECT INTO "nl:"
  FROM prsnl pnl
  WHERE (pnl.person_id=reply->updt_id)
  DETAIL
   reply->updt_name = trim(pnl.name_full_formatted)
  WITH nocounter
 ;end select
 CALL errorhandler(build("SCRIPT DETAILS - PRSNL(",reqinfo->updt_id,")"),"F",sscript_name,reply)
 IF (lstat=1)
  SET reply->status_data.status = "S"
 ENDIF
 GO TO set_return
 SUBROUTINE getpasseddata(null)
   IF (( $SCD=0.0))
    SET dsncd = uar_get_code_by("MEANING",14122,nullterm( $SNCDF))
   ELSE
    SET dsncd =  $SCD
   ENDIF
   CALL echo(build2("dSnCd  --> ",dsncd))
   IF (dsncd <= 0)
    CALL echo("calling error handler..")
    CALL errorhandler(build("GetPassedData() - CDF passed in: [",trim( $SNCDF),"]",
      "uar_get_code_by(MEANING) returned: [",trim(cnvtstring(dsncd)),
      "]"),"F",sscript_name,reply)
   ENDIF
   IF (( $STATCD=0.0)
    AND size(trim( $STATCDF),1) > 0)
    SET dstatcd = uar_get_code_by("MEANING",14122,nullterm( $STATCDF))
   ELSE
    SET dstatcd =  $STATCD
   ENDIF
   CALL echo(build2("dStatCd  --> ",dstatcd))
   IF (dstatcd < 0)
    CALL echo("calling error handler..")
    CALL errorhandler(build("GetPassedData() - CDF passed in: [",trim( $STATCDF),"]",
      "uar_get_code_by(MEANING) returned: [",trim(cnvtstring(dstatcd)),
      "]"),"F",sscript_name,reply)
   ENDIF
   CALL echo(build("$PN passed: *", $PN,"*"))
   IF (size(trim( $PN),1)=0)
    SET sname = "PERSON"
   ELSE
    SET sname = trim(cnvtupper( $PN))
   ENDIF
   CALL echo(build("sName *",sname,"*"))
   IF (size(trim( $VDT),1) > 0)
    SET dqvuntil_dt = cnvtdatetime(concat( $VDT," 23:59"))
   ELSE
    SET dqvuntil_dt = cnvtdatetime("31-DEC-2100 23:59")
   ENDIF
   CALL echo(build("dqVUntilDt--->",format(dqvuntil_dt,";;q")))
   RETURN(1)
 END ;Subroutine
 SUBROUTINE addstickynote(null)
   CALL echo(build("subroutine AddStickyNote(null)..."))
   SET dsnid = getnewsnid(0)
   IF (dsnid <= 0)
    CALL errorhandler(build("AddStickyNote() ","1)AddStickyNote()","dSNID[",dsnid,"]"),"F",
     sscript_name,reply)
   ENDIF
   IF (textlen(ssntext) > 255)
    SET dnewlongtextid = getnewltid(0)
    IF (dnewlongtextid <= 0)
     CALL errorhandler(build("AddStickyNote() ","2)AddStickyNote()","dNewLongTextId[",dnewlongtextid,
       "]"),"F",sscript_name,reply)
    ENDIF
   ENDIF
   INSERT  FROM sticky_note sn
    SET sn.sticky_note_id = dsnid, sn.sticky_note_type_cd = dsncd, sn.parent_entity_name =  $PN,
     sn.parent_entity_id = dpid, sn.sticky_note_text = trim(substring(1,255,ssntext)), sn
     .sticky_note_status_cd = dstatcd,
     sn.public_ind = npubind, sn.beg_effective_dt_tm = cnvtdatetime(sysdate), sn.end_effective_dt_tm
      = cnvtdatetime(dqvuntil_dt),
     sn.updt_dt_tm = cnvtdatetime(sysdate), sn.updt_id = reqinfo->updt_id, sn.updt_task = reqinfo->
     updt_task,
     sn.updt_applctx = reqinfo->updt_applctx, sn.updt_cnt = 0, sn.long_text_id = dnewlongtextid
    WITH nocounter
   ;end insert
   CALL errorhandler(build("AddStickyNote() ","3)AddStickyNote()"),"F",sscript_name,reply)
   IF (textlen(ssntext) > 255)
    INSERT  FROM long_text lt
     SET lt.active_ind = 1, lt.active_status_cd = dactive, lt.active_status_dt_tm = cnvtdatetime(
       sysdate),
      lt.active_status_prsnl_id = reqinfo->updt_id, lt.long_text = ssntext, lt.long_text_id =
      dnewlongtextid,
      lt.parent_entity_id = dsnid, lt.parent_entity_name = "STICKY_NOTE", lt.updt_applctx = reqinfo->
      updt_applctx,
      lt.updt_cnt = 0, lt.updt_dt_tm = cnvtdatetime(sysdate), lt.updt_id = reqinfo->updt_id,
      lt.updt_task = reqinfo->updt_task
     WITH nocounter
    ;end insert
    CALL errorhandler(build("AddStickyNote() ","4)AddStickyNote()"),"F",sscript_name,reply)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (delstickynote(sid=f8) =i4)
   CALL echo(build("subroutine DelStickyNote(",sid,")..."))
   DECLARE lid = f8 WITH protect, noconstant(0.0)
   IF (sid > 0)
    SELECT INTO "nl:"
     FROM sticky_note sn
     WHERE sn.sticky_note_id=sid
     DETAIL
      IF (sn.long_text_id > 0.0)
       lid = sn.long_text_id
      ENDIF
     WITH nocounter
    ;end select
    CALL errorhandler(build("DelStickyNote(sid)","1)DelStickyNote(sid=",sid,")"),"F",sscript_name,
     reply)
    IF (lid > 0.0)
     DELETE  FROM long_text lt
      WHERE lt.long_text_id=lid
      WITH nocounter
     ;end delete
     CALL errorhandler(build("DelStickyNote(sid)","2)DelStickyNote(lid=",lid,")"),"F",sscript_name,
      reply)
    ENDIF
    DELETE  FROM sticky_note sn
     ...
     WITH nocounter
    ;end delete
    CALL errorhandler(build("DelStickyNote(sid)","3)DelStickyNote(sid=",sid,")"),"F",sscript_name,
     reply)
   ELSE
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (updstickynote(sid=f8) =i4)
   CALL echo(build("subroutine UpdStickyNote(",sid,")..."))
   DECLARE dholdlt = f8 WITH protect, noconstant(0.0)
   SELECT INTO "nl:"
    FROM sticky_note sn
    WHERE sn.sticky_note_id=sid
    DETAIL
     dholdlt = sn.long_text_id
    WITH nocounter
   ;end select
   CALL errorhandler(build("UpdStickyNote(sid)","1)UpdStickyNote(sid=",sid,")"),"F",sscript_name,
    reply)
   IF (textlen(ssntext) > 255)
    IF (dholdlt=0.0)
     SET dholdlt = getnewltid(0)
     IF (dholdlt <= 0)
      CALL errorhandler(build("UpdStickyNote(sid)","2)UpdStickyNote(sid=",sid,")","dNewLongTextId[",
        dholdlt,"]"),"F",sscript_name,reply)
     ELSE
      INSERT  FROM long_text lt
       SET lt.active_ind = 1, lt.active_status_cd = dactive, lt.active_status_dt_tm = cnvtdatetime(
         sysdate),
        lt.active_status_prsnl_id = reqinfo->updt_id, lt.long_text = ssntext, lt.long_text_id =
        dholdlt,
        lt.parent_entity_id = sid, lt.parent_entity_name = "STICKY_NOTE", lt.updt_applctx = reqinfo->
        updt_applctx,
        lt.updt_cnt = 0, lt.updt_dt_tm = cnvtdatetime(sysdate), lt.updt_id = reqinfo->updt_id,
        lt.updt_task = reqinfo->updt_task
       WITH nocounter
      ;end insert
      CALL errorhandler(build("UpdStickyNote(sid)","3)UpdStickyNote(sid=",sid,")"),"F",sscript_name,
       reply)
     ENDIF
    ELSE
     CALL echo("****In Else****")
     UPDATE  FROM long_text lt
      SET lt.long_text = ssntext, lt.parent_entity_id = sid, lt.parent_entity_name = "STICKY_NOTE",
      ...
      WHERE lt.long_text_id=dholdlt
      WITH nocounter
     ;end update
     CALL errorhandler(build("UpdStickyNote(sid)","4)UpdStickyNote(sid=",sid,")"),"F",sscript_name,
      reply)
    ENDIF
   ELSE
    IF (dholdlt > 0.0)
     DELETE  FROM long_text lt
      ...
      WITH nocounter
     ;end delete
     CALL errorhandler(build("UpdStickyNote(sid)","5)UpdStickyNote(sid=",sid,")"),"F",sscript_name,
      reply)
     SET dholdlt = 0.0
    ENDIF
   ENDIF
   UPDATE  FROM sticky_note sn
    SET sn.sticky_note_type_cd = dsncd, sn.parent_entity_name =  $PN, sn.parent_entity_id = dpid,
     sn.sticky_note_text = trim(substring(1,255,ssntext)), sn.sticky_note_status_cd = dstatcd, sn
     .public_ind = npubind,
     sn.beg_effective_dt_tm = cnvtdatetime(sysdate), sn.end_effective_dt_tm = cnvtdatetime(
      dqvuntil_dt), sn.updt_dt_tm = cnvtdatetime(sysdate),
     sn.updt_id = reqinfo->updt_id, sn.updt_task = reqinfo->updt_task, sn.updt_applctx = reqinfo->
     updt_applctx,
     sn.updt_cnt = (sn.updt_cnt+ 1), sn.long_text_id = dholdlt
    WHERE sn.sticky_note_id=sid
    WITH nocounter
   ;end update
   CALL errorhandler(build("UpdStickyNote(sid)","6)UpdStickyNote(sid=",sid,")"),"F",sscript_name,
    reply)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE getnewsnid(null)
   DECLARE new_id = f8 WITH protect, noconstant(0.0)
   SELECT INTO "nl:"
    nextseqnum = seq(reference_seq,nextval)"#################;rp0"
    FROM dual
    ...
    WITH format, nocounter
   ;end select
   CALL errorhandler("GetNewSNID()","F",sscript_name,reply)
   RETURN(new_id)
 END ;Subroutine
 SUBROUTINE getnewltid(null)
   DECLARE new_id = f8 WITH protect, noconstant(0.0)
   SELECT INTO "nl:"
    nextseqnum = seq(long_data_seq,nextval)"#################;rp0"
    FROM dual
    ...
    WITH format, nocounter
   ;end select
   CALL errorhandler("GetNewLTID()","F",sscript_name,reply)
   RETURN(new_id)
 END ;Subroutine
 SUBROUTINE (ajaxreply(jsonstr=vc) =null)
   IF (trim(jsonstr) != "")
...
#set_return
 ...
 CALL ajaxreply(trim(cnvtrectojson(reply)))
#exit_script
