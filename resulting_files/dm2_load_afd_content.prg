CREATE PROGRAM dm2_load_afd_content
 ...
 IF (check_logfile("dm2_load_afd_content",".log","dm2_load_afd_content")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_load_afd_content..."
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dlac_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dlac_file_loc = vc WITH protect, noconstant( $2)
 DECLARE dlac_file_prefix = vc WITH protect, noconstant( $3)
 DECLARE dlac_batch = i4 WITH protect, noconstant(5000)
 DECLARE dlac_cnt = i4 WITH protect, noconstant(0)
 DECLARE dlac_rows_returned = i4 WITH protect, noconstant(10000)
 DECLARE dlac_full_file_name = vc WITH protect, noconstant(" ")
 DECLARE dlac_row_count = f8 WITH protect, noconstant(0.0)
 DECLARE dlac_csv_rows = i4 WITH protect, noconstant(0)
 DECLARE dlac_load_table = vc WITH protect, noconstant(" ")
 DECLARE dlac_current_date = f8 WITH protect, noconstant(0.0)
 SET dlac_current_date = cnvtdatetime(curdate,curtime3)
 IF (dlac_pkg >= 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Input package number invalid"
  SET dm_err->eproc = "Input package number Validation"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (cnvtupper(dlac_file_loc) != "CER_INSTALL")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Input file location invalid"
  SET dm_err->eproc = "Input file location Validation"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dlac_file_loc = dm2_install_schema->cer_install
 IF (cnvtupper(dlac_file_prefix)="")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Input file prefilx invalid"
  SET dm_err->eproc = "Input file prefix Validation"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 ...
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2_LOAD_AFD_CONTENT"
   AND d.info_name="BATCH_SIZE"
  DETAIL
   dlac_batch = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(dlac_batch)
 ENDIF
 FOR (dlac_cnt = 1 TO dcfr_csv_file->file_cnt)
   IF ((dcfr_csv_file->qual[dlac_cnt].target_table > ""))
    SET dlac_rows_returned = 10000
    WHILE ((dm_err->err_ind=0)
     AND dlac_rows_returned=10000)
      SET dm_err->eproc = concat("Delete ",trim(dcfr_csv_file->qual[dlac_cnt].target_table)," Rows")
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
      DELETE  FROM (value(dcfr_csv_file->qual[dlac_cnt].target_table) d)
       WHERE d.alpha_feature_nbr=dlac_pkg
      ...
      ELSE
       COMMIT
      ENDIF
      SET dlac_rows_returned = curqual
      IF ((dm_err->debug_flag > 0))
       CALL echo(build("Rows deleted:",dlac_rows_returned))
      ENDIF
    ENDWHILE
   ENDIF
 ENDFOR
 SET dlac_cnt = dcfr_csv_file->file_cnt
 WHILE (dlac_cnt > 0)
  IF ((dcfr_csv_file->qual[dlac_cnt].target_table > ""))
   SET dlac_load_table = dcfr_csv_file->qual[dlac_cnt].target_table
   SET dm_err->eproc = concat("Load ",dcfr_csv_file->qual[dlac_cnt].file_desc," data into ",
    dlac_load_table)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SET dlac_full_file_name = build(dlac_file_loc,dlac_file_prefix,dcfr_csv_file->qual[dlac_cnt].
    file_suffix,".csv")
   EXECUTE dm2_dbimport dlac_full_file_name, "dm2_load_afd_contentc", dlac_batch
   IF ((dm_err->err_ind=1))
    GO TO exit_program
   ENDIF
   SET dm_err->eproc = concat("Determine if all ",dcfr_csv_file->qual[dlac_cnt].file_desc,
    " data loaded into ",dlac_load_table," successfully...")
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SET dlac_row_count = 0
   SET dlac_csv_rows = 0
   SELECT INTO "nl:"
    tmpcnt = count(*)
    FROM (value(dlac_load_table) t)
    WHERE t.alpha_feature_nbr=dlac_pkg
     AND t.owner=currdbuser
    DETAIL
     dlac_row_count = tmpcnt
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   IF (dcfr_get_csv_row_cnt(dlac_full_file_name,dlac_csv_rows)=0)
    GO TO exit_program
   ENDIF
   IF (dlac_row_count != dlac_csv_rows)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("CSV count (",trim(cnvtstring(dlac_csv_rows)),
     ") does not match table count (",trim(cnvtstring(dlac_row_count)),")")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
   ELSE
    IF ((dm_err->debug_flag > 0))
     SET dm_err->eproc = concat("Rows successfully inserted into ",dlac_load_table,": ",trim(
       cnvtstring(dlac_row_count)))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
   ENDIF
  ENDIF
  SET dlac_cnt = (dlac_cnt - 1)
 ENDWHILE
 GO TO exit_program
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_load_afd_content completed."
 ENDIF
 CALL final_disp_msg("dm2_load_afd_content")
END GO
