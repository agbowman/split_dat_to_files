CREATE PROGRAM dm_ecmb_encntr_inptnt_rehab:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_ECMB_ENCNTR_INPTNT_REHAB"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count_from].from_id = frm.encntr_inptnt_rehab_id, rreclist->from_rec[
 ...
 SUBROUTINE (del_from(s_df_pk_id=f8,s_df_to_fk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) =
  i4)
   UPDATE  FROM encntr_inptnt_rehab frm
    ...
    WHERE frm.encntr_inptnt_rehab_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "ENCNTR_INPTNT_REHAB"
   ...
   IF (add_hist(s_df_pk_id)=0)
    ...
    SET request->error_message = substring(1,132,build(
      "Could not insert history for encntr_inptnt_rehab with pk=",s_df_pk_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i4)
   UPDATE  FROM encntr_inptnt_rehab frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     s_uf_to_fk_id
    WHERE frm.encntr_inptnt_rehab_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
 ...
 SUBROUTINE initialize_hist_id(null)
   IF (v_hist_init_ind=0)
    ...
    SET dcipht_request->transaction_reason_txt = "DM_ECMB_ENCNTR_INPTNT_REHAB"
