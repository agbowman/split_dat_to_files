CREATE PROGRAM dm_txtfnd_conversation_rule
 ...
 SUBROUTINE dst_add_log_det(dald_log_id,dald_status,dald_msg,dald_start_dt_tm,dald_end_dt_tm)
   ...
   RETURN(0)
 END ;Subroutine
 DECLARE dtcr_idx = i4 WITH protect, noconstant(0)
 DECLARE dtcr_pfp_rule_ind = i2 WITH protect, noconstant(0)
 DECLARE dtcr_pfp_info_ind = i2 WITH protect, noconstant(0)
 DECLARE dtcr_pfp_option_ind = i2 WITH protect, noconstant(0)
 DECLARE dtcr_pfc_copy_ind = i2 WITH protect, noconstant(0)
 DECLARE dtcr_pfc_epsd_ind = i2 WITH protect, noconstant(0)
 DECLARE dtcr_pfc_info_ind = i2 WITH protect, noconstant(0)
 DECLARE dtcr_pfc_option_ind = i2 WITH protect, noconstant(0)
 DECLARE dtcr_pfc_reltn_ind = i2 WITH protect, noconstant(0)
 DECLARE dtcr_pfc_rule_ind = i2 WITH protect, noconstant(0)
 DECLARE dtcr_prev_id = f8 WITH protect, noconstant(0.0)
 FREE RECORD dtcr_attrib
 RECORD dtcr_attrib(
   1 pfc_cnt = i4
   1 pfc_qual[*]
     2 pfc_column_name = vc
     2 pfc_column_label = vc
     2 pfc_column_flg = i4
   1 pfp_cnt = i4
   1 pfp_qual[*]
     2 pfp_column_name = vc
     2 pfp_column_label = vc
     2 pfp_column_flg = i4
 )
 IF (check_logfile("dm_tf_conv",".log","DM_TXTFND_CONVERSATION_RULE LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM_TXTFND_CONVERSATION_RULE..."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF (dtla_detail_id=0.0)
  ...
  SELECT INTO "nl:"
   ...
   WHERE d.detail_type_flag=2
    AND d.active_ind=1
    AND d.detail_script_name="DM_TXTFND_CONVERSATION_RULE"
   DETAIL
  ...
  IF (curqual=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg =
   "No active data found in DM_TEXT_FIND_DETAIL for script DM_TXTFND_CONVERSATION_RULE."
   IF ((script_rec->dm_text_find_log_id > 0.0))
    CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",dm_err->emsg,0.0,0.0)
   ENDIF
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SET dm_err->eproc = "Gathering list of LONG* foreign keys"
 SELECT INTO "NL:"
  FROM user_tab_columns u
  WHERE u.table_name="PM_FLX_CONVERSATION"
   AND u.column_name IN ("COPY_FIELDS_LONG_TEXT_ID", "EPSD_COLUMNS_LONG_TEXT_ID", "INFO_LONG_TEXT_ID",
  "OPTIONS_LONG_TEXT_REF_ID", "RELTN_DEFAULT_LONG_TEXT_ID",
  "RULE_LONG_TEXT_ID")
  DETAIL
   IF (u.column_name="COPY_FIELDS_LONG_TEXT_ID")
    dtcr_pfc_copy_ind = 1
   ELSEIF (u.column_name="EPSD_COLUMNS_LONG_TEXT_ID")
    dtcr_pfc_epsd_ind = 1
   ELSEIF (u.column_name="INFO_LONG_TEXT_ID")
    dtcr_pfc_info_ind = 1
   ELSEIF (u.column_name="OPTIONS_LONG_TEXT_REF_ID")
    dtcr_pfc_option_ind = 1
   ELSEIF (u.column_name="RELTN_DEFAULT_LONG_TEXT_ID")
    dtcr_pfc_reltn_ind = 1
   ELSEIF (u.column_name="RULE_LONG_TEXT_ID")
    dtcr_pfc_rule_ind = 1
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying for FK list:",dm_err
    ->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 SELECT INTO "NL:"
  FROM user_tab_columns u
  WHERE u.table_name="PM_FLX_PROMPT"
   AND u.column_name IN ("EXT_OPTIONS_LONG_TEXT_REF_ID", "INFO_LONG_TEXT_ID", "RULE_LONG_TEXT_ID")
  DETAIL
   IF (u.column_name="EXT_OPTIONS_LONG_TEXT_REF_ID")
    dtcr_pfp_option_ind = 1
   ELSEIF (u.column_name="INFO_LONG_TEXT_ID")
    dtcr_pfp_info_ind = 1
   ELSEIF (u.column_name="RULE_LONG_TEXT_ID")
    dtcr_pfp_rule_ind = 1
 ...
 IF (check_error(dm_err->eproc) != 0)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Gathering attribute list from dm_info"
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM_TXTFND_ATTRIBUTE:PM_FLX_CONVERSATION"
   AND  EXISTS (
  (SELECT
   "x"
   FROM user_tab_columns utc
   WHERE utc.table_name="PM_FLX_CONVERSATION"
    AND utc.column_name=di.info_name))
  ORDER BY di.info_name
  DETAIL
   dtcr_attrib->pfc_cnt = (dtcr_attrib->pfc_cnt+ 1), stat = alterlist(dtcr_attrib->pfc_qual,
    dtcr_attrib->pfc_cnt), dtcr_attrib->pfc_qual[dtcr_attrib->pfc_cnt].pfc_column_name = di.info_name,
   dtcr_attrib->pfc_qual[dtcr_attrib->pfc_cnt].pfc_column_label = di.info_char, dtcr_attrib->
   pfc_qual[dtcr_attrib->pfc_cnt].pfc_column_flg = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying for Attribute list:",
    dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ELSEIF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No attribute data found for PM_FLX_CONVERSATION."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying for Attribute list:",
    dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ...
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM_TXTFND_ATTRIBUTE:PM_FLX_PROMPT"
   AND  EXISTS (
  (SELECT
   "x"
   FROM user_tab_columns utc
   WHERE utc.table_name="PM_FLX_PROMPT"
    AND utc.column_name=di.info_name))
  ORDER BY di.info_name
  DETAIL
   dtcr_attrib->pfp_cnt = (dtcr_attrib->pfp_cnt+ 1), stat = alterlist(dtcr_attrib->pfp_qual,
    dtcr_attrib->pfp_cnt), dtcr_attrib->pfp_qual[dtcr_attrib->pfp_cnt].pfp_column_name = di.info_name,
   dtcr_attrib->pfp_qual[dtcr_attrib->pfp_cnt].pfp_column_label = di.info_char, dtcr_attrib->
   pfp_qual[dtcr_attrib->pfp_cnt].pfp_column_flg = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSEIF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No attribute data found for PM_FLX_PROMPT."
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying for Attribute list:",
    dm_err->emsg),0.0,0.0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ...
 SET script_rec->dm_text_find_detail_id = dtla_detail_id
 SET dm_err->eproc = "Gathering PM Conversation rules.  "
 CALL parser("select into 'nl:'",0)
 CALL parser("  from pm_flx_prompt p, pm_flx_conversation c",0)
 CALL parser(
  "plan p where p.description = 'Diagnosis*' and p.parent_entity_name = 'PM_FLX_CONVERSATION'",0)
 CALL parser("join c where c.conversation_id = p.parent_entity_id",0)
 CALL parser(" and c.conversation_id > 0.0",0)
 CALL parser("order by c.conversation_id",0)
 CALL parser("detail",0)
 CALL parser("  if(c.conversation_id != dtcr_prev_id)",0)
 IF (dtcr_pfc_copy_ind=1)
  CALL parser("   if(c.copy_fields_long_text_id > 0 )",0)
  CALL parser("      dtg->cnt = dtg->cnt + 1",0)
  CALL parser("      stat = alterlist(dtg->qual, dtg->cnt)",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_name = 'LONG_TEXT_REFERENCE'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_id = c.copy_fields_long_text_id",0)
  CALL parser("      dtg->qual[dtg->cnt].srch_col = 'LONG_TEXT'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_col = 'LONG_TEXT_ID'",0)
  CALL parser("      dtg->qual[dtg->cnt].desc = 'COPY_FIELDS'",0)
  CALL parser(
   "      dtg->qual[dtg->cnt].data_source = concat('Copy Fields Text:',trim(c.description))",0)
  FOR (dtcr_idx = 1 TO dtcr_attrib->pfc_cnt)
    CALL parser("      dtg->qual[dtg->cnt].att_cnt = dtg->qual[dtg->cnt].att_cnt + 1",0)
    CALL parser("      stat = alterlist(dtg->qual[dtg->cnt].att_qual, dtg->qual[dtg->cnt].att_cnt)",0
     )
    CALL parser(concat(
      "      dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].tab_name = 'PM_FLX_CONVERSATION'"
      ),0)
    CALL parser(concat(
      "      dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_name = dtcr_attrib->pfc_qual[",
      trim(cnvtstring(dtcr_idx)),"].pfc_column_name"),0)
    CALL parser(concat(
      "      dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_txt = dtcr_attrib->pfc_qual[",
      trim(cnvtstring(dtcr_idx)),"].pfc_column_label"),0)
    IF ((dtcr_attrib->pfc_qual[dtcr_idx].pfc_column_flg=3))
     CALL parser(concat(
       "   dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_val = format(c.",dtcr_attrib
       ->pfc_qual[dtcr_idx].pfc_column_name,",';;q')"),0)
    ELSEIF ((dtcr_attrib->pfc_qual[dtcr_idx].pfc_column_flg=2))
     CALL parser(concat(
       "   dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_val = cnvtstring(c.",
       dtcr_attrib->pfc_qual[dtcr_idx].pfc_column_name,",20)"),0)
    ELSE
     CALL parser(concat("   dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_val = c.",
       dtcr_attrib->pfc_qual[dtcr_idx].pfc_column_name),0)
    ENDIF
  ENDFOR
  CALL parser("   endif",0)
 ENDIF
 IF (dtcr_pfc_epsd_ind=1)
  CALL parser("   if(c.epsd_columns_long_text_id > 0)",0)
  CALL parser("      dtg->cnt = dtg->cnt + 1",0)
  CALL parser("      stat = alterlist(dtg->qual, dtg->cnt)",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_name = 'LONG_TEXT_REFERENCE'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_id = c.epsd_columns_long_text_id",0)
  CALL parser("      dtg->qual[dtg->cnt].srch_col = 'LONG_TEXT'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_col = 'LONG_TEXT_ID'",0)
  CALL parser("      dtg->qual[dtg->cnt].desc = 'EPSD_COLUMNS'",0)
  CALL parser("      dtg->qual[dtg->cnt].data_source = concat('Episode Text:',trim(c.description))",0
   )
  FOR (dtcr_idx = 1 TO dtcr_attrib->pfc_cnt)
    CALL parser("      dtg->qual[dtg->cnt].att_cnt = dtg->qual[dtg->cnt].att_cnt + 1",0)
    CALL parser("      stat = alterlist(dtg->qual[dtg->cnt].att_qual, dtg->qual[dtg->cnt].att_cnt)",0
  ...
  CALL parser("   endif   ",0)
 ENDIF
 IF (dtcr_pfc_info_ind=1)
  CALL parser("   if(c.info_long_text_id > 0)",0)
  CALL parser("      dtg->cnt = dtg->cnt + 1",0)
  CALL parser("      stat = alterlist(dtg->qual, dtg->cnt)",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_name = 'LONG_TEXT_REFERENCE'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_id = c.info_long_text_id",0)
  CALL parser("      dtg->qual[dtg->cnt].srch_col = 'LONG_TEXT'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_col = 'LONG_TEXT_ID'",0)
  CALL parser("      dtg->qual[dtg->cnt].desc = 'INFO'",0)
  CALL parser(
   "      dtg->qual[dtg->cnt].data_source = concat('Information Text:',trim(c.description))",0)
  FOR (dtcr_idx = 1 TO dtcr_attrib->pfc_cnt)
    CALL parser("      dtg->qual[dtg->cnt].att_cnt = dtg->qual[dtg->cnt].att_cnt + 1",0)
    CALL parser("      stat = alterlist(dtg->qual[dtg->cnt].att_qual, dtg->qual[dtg->cnt].att_cnt)",0
     )
  ...
  CALL parser("   endif",0)
 ENDIF
 IF (dtcr_pfc_option_ind=1)
  CALL parser("   if(c.options_long_text_ref_id > 0)",0)
  CALL parser("      dtg->cnt = dtg->cnt + 1",0)
  CALL parser("      stat = alterlist(dtg->qual, dtg->cnt)",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_name = 'LONG_TEXT_REFERENCE'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_id = c.options_long_text_ref_id",0)
  CALL parser("      dtg->qual[dtg->cnt].srch_col = 'LONG_TEXT'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_col = 'LONG_TEXT_ID'",0)
  CALL parser("      dtg->qual[dtg->cnt].desc = 'OPTIONS'",0)
  CALL parser("      dtg->qual[dtg->cnt].data_source = concat('Options Text:',trim(c.description))",0
   )
  FOR (dtcr_idx = 1 TO dtcr_attrib->pfc_cnt)
    CALL parser("      dtg->qual[dtg->cnt].att_cnt = dtg->qual[dtg->cnt].att_cnt + 1",0)
    CALL parser("      stat = alterlist(dtg->qual[dtg->cnt].att_qual, dtg->qual[dtg->cnt].att_cnt)",0
  ...
  CALL parser("   endif",0)
 ENDIF
 IF (dtcr_pfc_reltn_ind=1)
  CALL parser("   if(c.reltn_default_long_text_id > 0)",0)
  CALL parser("      dtg->cnt = dtg->cnt + 1",0)
  CALL parser("      stat = alterlist(dtg->qual, dtg->cnt)",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_name = 'LONG_TEXT_REFERENCE'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_id = c.reltn_default_long_text_id",0)
  CALL parser("      dtg->qual[dtg->cnt].srch_col = 'LONG_TEXT'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_col = 'LONG_TEXT_ID'",0)
  CALL parser("      dtg->qual[dtg->cnt].desc = 'RELTN_DEFAULT'",0)
  CALL parser(
   "      dtg->qual[dtg->cnt].data_source = concat('Relationship Text:',trim(c.description))",0)
  FOR (dtcr_idx = 1 TO dtcr_attrib->pfc_cnt)
    CALL parser("      dtg->qual[dtg->cnt].att_cnt = dtg->qual[dtg->cnt].att_cnt + 1",0)
    CALL parser("      stat = alterlist(dtg->qual[dtg->cnt].att_qual, dtg->qual[dtg->cnt].att_cnt)",0
     )
  ...
  CALL parser("   endif",0)
 ENDIF
 IF (dtcr_pfc_rule_ind=1)
  CALL parser("   if(c.rule_long_text_id > 0)",0)
  CALL parser("      dtg->cnt = dtg->cnt + 1",0)
  CALL parser("      stat = alterlist(dtg->qual, dtg->cnt)",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_name = 'LONG_TEXT_REFERENCE'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_id = c.rule_long_text_id",0)
  CALL parser("      dtg->qual[dtg->cnt].srch_col = 'LONG_TEXT'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_col = 'LONG_TEXT_ID'",0)
  CALL parser("      dtg->qual[dtg->cnt].desc = 'RULE'",0)
  CALL parser("      dtg->qual[dtg->cnt].data_source = concat('Rule Text:',trim(c.description))",0)
  FOR (dtcr_idx = 1 TO dtcr_attrib->pfc_cnt)
    CALL parser("      dtg->qual[dtg->cnt].att_cnt = dtg->qual[dtg->cnt].att_cnt + 1",0)
    CALL parser("      stat = alterlist(dtg->qual[dtg->cnt].att_qual, dtg->qual[dtg->cnt].att_cnt)",0
     )
  ...
  CALL parser("   endif   ",0)
 ENDIF
 CALL parser("  dtcr_prev_id = c.conversation_id",0)
 CALL parser("  endif",0)
 IF (dtcr_pfp_option_ind=1)
  CALL parser("   if(p.ext_options_long_text_ref_id > 0)",0)
  CALL parser("      dtg->cnt = dtg->cnt + 1",0)
  CALL parser("      stat = alterlist(dtg->qual, dtg->cnt)",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_name = 'LONG_TEXT_REFERENCE'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_id = p.ext_options_long_text_ref_id",0)
  CALL parser("      dtg->qual[dtg->cnt].srch_col = 'LONG_TEXT'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_col = 'LONG_TEXT_ID'",0)
  CALL parser("      dtg->qual[dtg->cnt].desc = 'PROMPT EXT_OPTIONS'",0)
  CALL parser(concat("      dtg->qual[dtg->cnt].data_source = concat('Extended Options Text:',",
    "trim(p.description),':',trim(c.description))"),0)
  FOR (dtcr_idx = 1 TO dtcr_attrib->pfc_cnt)
    CALL parser("      dtg->qual[dtg->cnt].att_cnt = dtg->qual[dtg->cnt].att_cnt + 1",0)
    CALL parser("      stat = alterlist(dtg->qual[dtg->cnt].att_qual, dtg->qual[dtg->cnt].att_cnt)",0
     )
    ...
    ENDIF
  ENDFOR
  FOR (dtcr_idx = 1 TO dtcr_attrib->pfp_cnt)
    CALL parser("      dtg->qual[dtg->cnt].att_cnt = dtg->qual[dtg->cnt].att_cnt + 1",0)
    CALL parser("      stat = alterlist(dtg->qual[dtg->cnt].att_qual, dtg->qual[dtg->cnt].att_cnt)",0
     )
    CALL parser(concat(
      "      dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].tab_name = 'PM_FLX_PROMPT'"),0
     )
    CALL parser(concat(
      "      dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_name = dtcr_attrib->pfp_qual[",
      trim(cnvtstring(dtcr_idx)),"].pfp_column_name"),0)
    CALL parser(concat(
      "      dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_txt = dtcr_attrib->pfp_qual[",
      trim(cnvtstring(dtcr_idx)),"].pfp_column_label"),0)
    IF ((dtcr_attrib->pfp_qual[dtcr_idx].pfp_column_flg=3))
     CALL parser(concat(
       "   dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_val = format(p.",dtcr_attrib
       ->pfp_qual[dtcr_idx].pfp_column_name,",';;q')"),0)
    ELSEIF ((dtcr_attrib->pfp_qual[dtcr_idx].pfp_column_flg=2))
     CALL parser(concat(
       "   dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_val = cnvtstring(p.",
       dtcr_attrib->pfp_qual[dtcr_idx].pfp_column_name,",20)"),0)
    ELSE
     CALL parser(concat("   dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_val = p.",
       dtcr_attrib->pfp_qual[dtcr_idx].pfp_column_name),0)
    ENDIF
  ENDFOR
  CALL parser("   endif      ",0)
 ENDIF
 IF (dtcr_pfp_info_ind=1)
  CALL parser("   if(p.info_long_text_id > 0)",0)
  CALL parser("      dtg->cnt = dtg->cnt + 1",0)
  CALL parser("      stat = alterlist(dtg->qual, dtg->cnt)",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_name = 'LONG_TEXT'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_id = p.info_long_text_id",0)
  CALL parser("      dtg->qual[dtg->cnt].srch_col = 'LONG_TEXT'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_col = 'LONG_TEXT_ID'",0)
  CALL parser("      dtg->qual[dtg->cnt].desc = 'PROMPT INFO'",0)
  CALL parser(concat("      dtg->qual[dtg->cnt].data_source = concat('Prompt Information Text:',",
    "trim(p.description),':',trim(c.description))"),0)
  FOR (dtcr_idx = 1 TO dtcr_attrib->pfc_cnt)
    CALL parser("      dtg->qual[dtg->cnt].att_cnt = dtg->qual[dtg->cnt].att_cnt + 1",0)
    CALL parser("      stat = alterlist(dtg->qual[dtg->cnt].att_qual, dtg->qual[dtg->cnt].att_cnt)",0
  ...
  CALL parser("   endif         ",0)
 ENDIF
 IF (dtcr_pfp_rule_ind=1)
  CALL parser("   if(p.rule_long_text_id > 0)",0)
  CALL parser("      dtg->cnt = dtg->cnt + 1",0)
  CALL parser("      stat = alterlist(dtg->qual, dtg->cnt)",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_name = 'LONG_TEXT_REFERENCE'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_id = p.rule_long_text_id",0)
  CALL parser("      dtg->qual[dtg->cnt].srch_col = 'LONG_TEXT'",0)
  CALL parser("      dtg->qual[dtg->cnt].pe_col = 'LONG_TEXT_ID'",0)
  CALL parser("      dtg->qual[dtg->cnt].desc = 'PROMPT RULE'",0)
  CALL parser(concat("      dtg->qual[dtg->cnt].data_source = concat('Prompt Rule Text:',",
    "trim(p.description),':',trim(c.description))"),0)
  FOR (dtcr_idx = 1 TO dtcr_attrib->pfc_cnt)
    CALL parser("      dtg->qual[dtg->cnt].att_cnt = dtg->qual[dtg->cnt].att_cnt + 1",0)
    CALL parser("      stat = alterlist(dtg->qual[dtg->cnt].att_qual, dtg->qual[dtg->cnt].att_cnt)",0
 ...
 IF (check_error(dm_err->eproc) != 0)
  ...
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
    "Querying for Conversation and Prompt data:",dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 IF ((dtg->cnt > 0))
  ...
  IF (check_error(dm_err->eproc) != 0)
   ...
   SET dm_err->err_ind = 0
  ENDIF
  CALL dst_insert_all(script_rec)
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ENDIF
#exit_script
 SET dm_err->eproc = "...Ending DM_TXTFND_CONVERSATION_RULE"
 CALL final_disp_msg("dm_tf_conv")
END GO
