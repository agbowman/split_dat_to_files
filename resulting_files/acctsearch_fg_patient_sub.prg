CREATE PROGRAM acctsearch_fg_patient_sub
 SELECT INTO "nl:"
  FROM code_value c
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   patient_code = c.code_value
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.code_set=18936
   AND c.cdf_meaning="GUARANTOR"
  DETAIL
   guarantor_code = c.code_value
  WITH nocounter
 ;end select
 SELECT
  IF ((request->billing_entity_id=- (1))
   ...
   PLAN (a
    ...
    WHERE a.acct_id=aor.acct_id
     AND aor.role_type_cd=client_owner_cd)
    JOIN (o
  ...
  ELSEIF ((request->billing_entity_id=- (1)))
   FROM account a,
    ...
    acct_org_reltn aor
   PLAN (o
    WHERE adr.parent_entity_id=o.organization_id)
    JOIN (aor
