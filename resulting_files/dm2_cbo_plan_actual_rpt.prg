CREATE PROGRAM dm2_cbo_plan_actual_rpt
 ...
 SUBROUTINE dcr_get_instance_info(dgii_instance_name,dgii_instance_nbr)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_plnactrpt",".log","Dm2_cbo_plan_actual_rpt")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validating input parameters."
 DECLARE dcpar_sqlid = vc WITH protect, noconstant("")
 DECLARE dcpar_fname = vc WITH protect, noconstant( $1)
 DECLARE dcpar_search = vc WITH protect, noconstant( $2)
 DECLARE dcpar_search_text = vc WITH protect, noconstant( $3)
 DECLARE dcpar_child = vc WITH protect, noconstant(cnvtupper( $4))
 DECLARE dcpar_mode = vc WITH protect, noconstant( $5)
 DECLARE dcpar_qnum = vc WITH protect, noconstant( $6)
 DECLARE dcpar_sort_order = vc WITH protect, noconstant(cnvtupper( $7))
 DECLARE dcpar_sortby = vc WITH protect, noconstant("")
 DECLARE dcpar_sortby2 = vc WITH protect, noconstant("")
 DECLARE dcpar_user_name = vc WITH protect, noconstant(trim(cnvtupper(currdbuser)))
 DECLARE dcpar_database_name = vc WITH protect, noconstant(trim(cnvtupper(currdbname)))
 DECLARE dcpar_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcpar_cnt2 = i4 WITH protect, noconstant(0)
 DECLARE dcpar_cnt3 = i4 WITH protect, noconstant(0)
 DECLARE dcpar_instance_name = vc WITH protect, noconstant("")
 DECLARE dcpar_instance_nbr = i4 WITH protect, noconstant(0)
 DECLARE dcpar_qnum_int = i4 WITH protect, noconstant(0)
 IF (dcpar_search="S")
  SET dcpar_sqlid = dcpar_search_text
 ENDIF
 SET dcpar_qnum_int = cnvtint(dcpar_qnum)
 SET dcpar_sortby2 = "v.sql_id"
 CASE (dcpar_sort_order)
  OF "0":
   SET dcpar_sortby = "v.sql_id"
   SET dcpar_sortby2 = "v.child_number"
  OF "1":
   SET dcpar_sortby = "v.executions"
  OF "2":
   SET dcpar_sortby = "v.elapsed_time"
  OF "3":
   SET dcpar_sortby = "v.buffer_gets"
  OF "4":
   SET dcpar_sortby = "v.disk_reads"
  OF "5":
   SET dcpar_sortby = "v.cpu_time"
  OF "2R":
   SET dcpar_sortby = "e_ratio"
  OF "3R":
   SET dcpar_sortby = "b_ratio"
  OF "4R":
   SET dcpar_sortby = "d_ratio"
  OF "5R":
   SET dcpar_sortby = "c_ratio"
 ENDCASE
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting Dm2_cbo_plan_actual_rpt"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dcr_init_sqlplan(null)=0)
  GO TO exit_program
 ENDIF
 SET dcr_sqlplan->mode = dcpar_mode
 SET dm_err->eproc = concat("Obtain Data for:",dcpar_search_text)
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dcpar_cnt2 = 0
 SELECT
  IF (dcpar_search="S")
   FROM v$sql v
   WHERE v.sql_id=dcpar_sqlid
    AND parser(evaluate(dcpar_child,"ALL","1=1",build("v.child_number = ",cnvtint(dcpar_child))))
    AND v.executions > 0
  ELSE
   FROM v$sql v
   WHERE v.sql_text=patstring(dcpar_search_text)
  ...
  ORDER BY parser(dcpar_sortby) DESC, parser(dcpar_sortby2)
  HEAD REPORT
   row + 0
  HEAD v.sql_id
   IF (dcpar_qnum_int > dcpar_cnt2)
    dcr_sqlplan->sql_cnt = (dcr_sqlplan->sql_cnt+ 1), stat = alterlist(dcr_sqlplan->sql,dcr_sqlplan->
     sql_cnt), dcr_sqlplan->sql[dcr_sqlplan->sql_cnt].sql_id = v.sql_id,
    dcpar_cnt = 0
   ENDIF
  HEAD v.plan_hash_value
   IF (dcpar_qnum_int > dcpar_cnt2)
    dcpar_cnt = (dcpar_cnt+ 1), dcpar_cnt2 = (dcpar_cnt2+ 1), stat = alterlist(dcr_sqlplan->sql[
     dcr_sqlplan->sql_cnt].child,dcpar_cnt),
    dcr_sqlplan->sql[dcr_sqlplan->sql_cnt].child[dcpar_cnt].child_number = v.child_number,
    dcr_sqlplan->sql[dcr_sqlplan->sql_cnt].child[dcpar_cnt].plan_hash = v.plan_hash_value,
    dcr_sqlplan->sql[dcr_sqlplan->sql_cnt].child_cnt = dcpar_cnt
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 SET dcpar_cnt = 0
 SET dcpar_cnt2 = 0
 IF ((dcr_sqlplan->sql_cnt > 0))
  FOR (dcpar_cnt = 1 TO dcr_sqlplan->sql_cnt)
    FOR (dcpar_cnt2 = 1 TO dcr_sqlplan->sql[dcpar_cnt].child_cnt)
      IF (dcr_get_plan(dcpar_cnt,dcpar_cnt2)=0)
       GO TO exit_program
      ENDIF
    ENDFOR
  ENDFOR
 ...
 ENDIF
 IF (dcr_get_instance_info(dcpar_instance_name,dcpar_instance_nbr)=0)
  GO TO exit_program
 ENDIF
 SELECT INTO value(dcpar_fname)
  FROM (dummyt d  WITH seq = 1)
  DETAIL
   CALL print("SQL PLAN ACTUAL  REPORT"),
   CALL center(concat("Report Date/Time: ",format(cnvtdatetime(curdate,curtime3),
     "DD-MMM-YY HH:MM:SS;;D")),1,132), row + 1,
   row + 1,
   CALL print(concat("User Name        : ",dcpar_user_name)), row + 1,
   CALL print(concat("Database Name    : ",dcpar_database_name)), row + 1,
   CALL print(concat("Instance Name    : ",dcpar_instance_name)),
   row + 1,
   CALL print(concat("Instance Number  : ",cnvtstring(dcpar_instance_nbr))), row + 1,
   col 0,
   ...
   FOR (dcpar_cnt = 1 TO dcr_sqlplan->sql_cnt)
     FOR (dcpar_cnt2 = 1 TO dcr_sqlplan->sql[dcpar_cnt].child_cnt)
       CALL print(concat("Plan Details. SQL_ID:",dcr_sqlplan->sql[dcpar_cnt].sql_id," Child Number: ",
        build(dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2].child_number))), row + 1,
       CALL print(fillstring(132,"-")),
       row + 1
       FOR (dcpar_cnt3 = 1 TO dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2].plan_line_cnt)
         col 0, dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2].exec_plan[dcpar_cnt3].plan_line, row +
         1
       ENDFOR
       CALL print(concat("Plan Statistics. SQL_ID:",dcr_sqlplan->sql[dcpar_cnt].sql_id,
        " Child Number: ",build(dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2].child_number))), row +
       1,
       CALL print(fillstring(132,"-")),
       row + 1,
       CALL print(concat("Optimizer Mode      : ",dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2].
        optimizer_mode)), row + 1,
       CALL print(concat("Optimizer Cost      : ",build(dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2]
         .optimizer_cost))), row + 1,
       CALL print(concat("First Load Time     : ",dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2].
        first_time)),
       row + 1,
       CALL print(concat("Executions          : ",build(dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2]
         .exec))), row + 1,
       CALL print(concat("Fetches             : ",build(dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2]
         .fetches))), row + 1,
       CALL print(concat("Rows                : ",build(dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2]
         .rows_processed))),
       row + 1,
       CALL print(concat("Row Ratio           : ",build(dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2]
         .actual_rows_processed))), row + 1,
       CALL print(concat("Buffer Gets         : ",build(dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2]
         .buff))), row + 1,
       CALL print(concat("Buffer Ratio        : ",build(dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2]
         .rat))),
       row + 1,
       CALL print(concat("Disk Reads          : ",build(dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2]
         .disk))), row + 1,
       CALL print(concat("Disk Ratio          : ",build(dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2]
         .drat))), row + 1,
       CALL print(concat("Elapsed Time(Sec)   : ",build((dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2
         ].ela_time/ 1000000)))),
       row + 1,
       CALL print(concat("Elapsed Ratio       : ",build(dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2]
         .erat))), row + 1
       IF ((dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2].cpu_time > - (1)))
        CALL print(concat("CPU Time(sec)       : ",build((dcr_sqlplan->sql[dcpar_cnt].child[
          dcpar_cnt2].cpu_time/ 1000000)))), row + 1
       ENDIF
       CALL print(concat("CPU Ratio           : ",build(dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2]
         .crat))), row + 1,
       CALL print(concat("Bind Aware          : ",dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2].
        bind_aware)),
       row + 1,
       CALL print(concat("Bind Sensitive      : ",dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2].
        bind_sensitive)), row + 1,
       CALL print(concat("Parse Calls         : ",build(dcr_sqlplan->sql[dcpar_cnt].child[dcpar_cnt2]
         .parse_calls))), row + 1, row + 1
     ENDFOR
   ENDFOR
 ...
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "Dm2_cbo_plan_actual_rpt completed"
 CALL final_disp_msg("dm2_plnactrpt")
END GO
