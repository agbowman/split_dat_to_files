CREATE PROGRAM dm2_exec_table_utilities:dba
 SET trace progcachesize 255
 DECLARE etu_logfile_prefix = vc WITH protect, constant("dm2_ex_tbl_utilities")
 DECLARE etu_tname = vc WITH protect, noconstant(" ")
 DECLARE etu_schema = c1 WITH protect, noconstant(" ")
 DECLARE etu_standalone = i2 WITH protect, noconstant(0)
 DECLARE detu_bypass_node_check = i4 WITH protect, noconstant(0)
 ...
 IF ((dm2_install_schema->curprog="NONE"))
  SET etu_standalone = 1
 ELSE
  SET etu_standalone = 0
 ENDIF
 IF (check_logfile(etu_logfile_prefix,".log","DM2_EXEC_TABLE_UTILITIES LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET reqinfo->updt_task = dm2_install_schema->dm2_updt_task_value
 IF ((dm_err->debug_flag > 0))
  CALL echo(dm_err->asterisk_line)
  CALL echo(build("etu::reqinfo->updt_task=",reqinfo->updt_task))
  CALL echo(dm_err->asterisk_line)
 ENDIF
 SET dm_err->eproc = "Beginning Execute Table Utilities Process"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm2_etu->mode = cnvtupper( $2)
 IF ( NOT ((dm2_etu->mode IN ("TABLE", "PACKAGE"))))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Input mode is invalid, may only be TABLE or PACKAGE."
  SET dm_err->eproc = "Input Mode Validation"
  SET dm_err->user_action = "Please enter TABLE or PACKAGE for mode."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ELSEIF ((dm2_etu->mode="PACKAGE")
  AND currdbuser="CDBA")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "PACKAGE Input mode is invalid when CURRDBUSER is CDBA."
  SET dm_err->eproc = "Input Mode Validation"
  SET dm_err->user_action =
  "Please log in as non-CDBA to run DM2_EXEC_TABLE_UTILITIES in PACKAGE mode."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET etu_tname = build(cnvtupper(trim( $1,3)))
 IF ((dm2_etu->mode="PACKAGE"))
  IF (isnumeric( $1) > 0)
   SET dm2_etu->tbl_name = " "
   SET dm2_etu->pkg_num = cnvtint( $1)
  ELSE
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Invalid Package Number."
   SET dm_err->eproc = "Input Package Number Validation"
   SET dm_err->user_action = "Please enter a numeric Package Number."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ENDIF
 ELSE
  IF (etu_tname=" ")
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Input table name to execute table utilities is invalid."
   SET dm_err->eproc = "Input Table Name Validation"
   SET dm_err->user_action = "Please enter a table name to execute table utilities."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ENDIF
  SET dm2_etu->tbl_name = etu_tname
  SET dm2_etu->pkg_num = 0
 ENDIF
  ...
  IF (etu_standalone=1)
   ...
   IF (curqual=0)
    ...
    SET dm_err->emsg = concat(
     "You must be logged into the database as valid user to run this process. ",
     "Current database user is: ",currdbuser)
    SET dm_err->eproc = "Determining Database Schema Based on Current User"
    SET dm_err->user_action = "Please login as valid user to execute table utilities."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
   ENDIF
  ENDIF
 ...
 IF (etu_schema="C")
  IF (currdb IN ("ORACLE"))
   EXECUTE dm2_compile_objects "TABLE", value(etu_tname)
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
  ...
  IF (cnvtupper(currdbuser)="V500")
   EXECUTE dm2_create_views value(etu_tname)
   IF ((dm_err->err_ind=1))
    GO TO exit_program
   ENDIF
  ENDIF
 ENDIF
 IF ((((dm2_etu->mode="TABLE")) OR (currdbuser != "V500")) )
  EXECUTE oragen3 value(etu_tname)
 ELSEIF ((dm2_etu->mode="PACKAGE"))
  IF (dan_bypass_node_check(detu_bypass_node_check)=0)
   GO TO exit_program
  ENDIF
  IF (detu_bypass_node_check=1)
   SET dm_err->eproc = "execution of oragen3 if bypass is enabled"
   CALL disp_msg(" ",dm_err->logfile,0)
   EXECUTE oragen3 value(etu_tname)
  ELSE
   SET dm_err->eproc = "execution of oragen3 if mutli app nodes exists"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF ((dan_nodes_list->cnt=0))
    IF (dan_get_app_nodes(null)=0)
     GO TO exit_program
 ...
 ENDIF
 IF (etu_schema="C")
  EXECUTE dm2_add_default_rows value(etu_tname)
  IF ((dm_err->err_ind=1))
   GO TO exit_program
  ENDIF
  IF (currdb IN ("DB2UDB", "ORACLE"))
   EXECUTE dm2_create_actind_triggers value(etu_tname)
   IF ((dm_err->err_ind=1))
    GO TO exit_program
   ENDIF
  ENDIF
 ENDIF
#exit_program
 SET dm_err->eproc = "Ending Execute Table Utilities Process.  Process Completed Successfully."
 CALL final_disp_msg(etu_logfile_prefix)
END GO
