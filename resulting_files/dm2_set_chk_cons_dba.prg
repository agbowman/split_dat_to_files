CREATE PROGRAM dm2_set_chk_cons:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dscc_logfile_prefix = vc WITH protect, constant("dm2_set_chk_cons")
 DECLARE dscc_table_name = vc WITH protect, noconstant(" ")
 DECLARE dscc_column_name = vc WITH protect, noconstant(" ")
 DECLARE dscc_state = vc WITH protect, noconstant(" ")
 DECLARE dscc_sc_str = vc WITH protect, noconstant(" ")
 DECLARE dscc_cons_found = i2 WITH protect, noconstant(0)
 DECLARE dscc_cons_name = vc WITH protect, noconstant(" ")
 DECLARE dscc_cons_status = vc WITH protect, noconstant(" ")
 DECLARE dscc_cons_val = vc WITH protect, noconstant(" ")
 DECLARE dscc_push_cmd = vc WITH protect, noconstant(" ")
 IF (check_logfile(dscc_logfile_prefix,".log","DM2_SET_CHK_CONS LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_set_chk_cons process..."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dscc_table_name = cnvtupper(trim( $1,3))
 IF (dscc_table_name=" ")
  SET dm_err->err_ind = 1
  ...
  GO TO exit_program
 ENDIF
 SET dscc_column_name = cnvtupper(trim( $2,3))
 IF (dscc_column_name=" ")
  SET dm_err->err_ind = 1
  ...
  GO TO exit_program
 ENDIF
 SET dscc_state = cnvtupper(trim( $3,3))
 IF ( NOT (dscc_state IN ("NOVALIDATE", "ENABLE")))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Constraint state is invalid."
  SET dm_err->eproc = "Constraint State Validation"
  SET dm_err->user_action = "Please specify a valid constraint state:  NOVALIDATE or ENABLE."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Retrieve check constraint information"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_constraints dc,
   dba_cons_columns dcc
  PLAN (dc
   WHERE dc.table_name=dscc_table_name
    AND dc.owner=currdbuser
    AND dc.constraint_type="C")
   JOIN (dcc
   WHERE dcc.constraint_name=dc.constraint_name
    AND dcc.owner=dc.owner
    AND dcc.table_name=dscc_table_name
    AND dcc.column_name=dscc_column_name)
  DETAIL
   dscc_sc_str = cnvtupper(trim(trim(dc.search_condition,3),4))
   IF (findstring("ISNOTNULL",dscc_sc_str) > 0)
    dscc_cons_found = 1, dscc_cons_name = dc.constraint_name, dscc_cons_status = dc.status,
    dscc_cons_val = dc.validated
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 2))
  CALL echo(concat("Check constraint info for table ",trim(dscc_table_name,3)," column ",trim(
     dscc_column_name,3),":"))
  CALL echo(concat("      constraint name = ",dscc_cons_name))
  CALL echo(concat("    constraint status = ",dscc_cons_status))
  CALL echo(concat(" constraint validated = ",dscc_cons_val))
 ENDIF
 IF (dscc_state="NOVALIDATE")
  IF (dscc_cons_found=1)
   IF (((dscc_cons_status != "ENABLED") OR (dscc_cons_val != "NOT VALIDATED")) )
    SET dm_err->eproc = concat("Modifying check constraint to enable, novalidate for table ",trim(
      dscc_table_name,3)," column ",trim(dscc_column_name,3))
    CALL disp_msg(" ",dm_err->logfile,0)
    SET dscc_push_cmd = concat("RDB ASIS (^ ALTER TABLE ",trim(dscc_table_name,3),
     " MODIFY CONSTRAINT ",trim(dscc_cons_name,3)," ENABLE NOVALIDATE ^) GO")
    IF (dm2_push_cmd(dscc_push_cmd,1)=0)
     ROLLBACK
     GO TO exit_program
    ELSE
     COMMIT
   ...
   ENDIF
  ELSE
   SET dm_err->eproc = concat("Creating an enable, novalidate check constraint on table ",trim(
     dscc_table_name,3)," column ",trim(dscc_column_name,3))
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dscc_push_cmd = concat("RDB ASIS (^ ALTER TABLE ",trim(dscc_table_name,3)," MODIFY (",trim(
     dscc_column_name,3)," NOT NULL ENABLE NOVALIDATE) ^) GO")
   IF (dm2_push_cmd(dscc_push_cmd,1)=0)
    ROLLBACK
    GO TO exit_program
   ELSE
 ...
 ELSE
  IF (dscc_cons_found=1)
   SET dm_err->eproc = concat("Modifying check constraint on table ",trim(dscc_table_name,3),
    " column ",trim(dscc_column_name,3)," to enabled")
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dscc_push_cmd = concat("RDB ASIS (^ ALTER TABLE ",trim(dscc_table_name,3),
    " MODIFY CONSTRAINT ",trim(dscc_cons_name,3)," ENABLE ^) GO")
   IF (dm2_push_cmd(dscc_push_cmd,1)=0)
    ROLLBACK
    GO TO exit_program
   ELSE
  ...
  ELSE
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Not null check constraint not found"
   SET dm_err->eproc = concat("Retrieving check constraint information for table ",trim(
     dscc_table_name,3)," column ",trim(dscc_column_name,3))
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ENDIF
 ...
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "DM2_SET_CHK_CONS COMPLETED"
 CALL final_disp_msg(dscc_logfile_prefix)
END GO
