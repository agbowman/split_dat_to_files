CREATE PROGRAM dm_rmc_load_audit_info:dba
 ...
 DECLARE nondel_row_cnt = i4
 DECLARE del_row_cnt = i4
 ...
 SET dim_filename = "cer_install:dm_rmc_audit_info.csv"
 SET readme_data->status = "F"
 SET readme_data->message = "Starting Readme..."
 IF (check_logfile("dm_auditinfo_",".log","dm_rmc_load_audit_info LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm_rmc_load_audit_info..."
 CALL disp_msg("",dm_err->logfile,0)
 IF (findfile(dim_filename)=0)
  SET readme_data->status = "F"
  SET readme_data->message = "Cannot find dm_rmc_audit_info.csv in CER_INSTALL."
  SET dm_err->emsg = "Cannot find dm_rmc_audit_info.csv in CER_INSTALL."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 SELECT INTO "NL:"
  ...
  DETAIL
   rs_exist->qual[d.seq].delete_ind = cnvtint(requestin->list_0[d.seq].delete_ind)
   IF ((rs_exist->qual[d.seq].delete_ind=0))
    nondel_row_cnt = (nondel_row_cnt+ 1)
   ELSE
    del_row_cnt = (del_row_cnt+ 1)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error("Querying rs_exist for delete_ind") != 0)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET readme_data->status = "F"
  SET readme_data->message = "Error querying rs_exist for delete_ind."
  GO TO exit_script
 ENDIF
 IF (merge_check_cnt > 0)
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = merge_check_cnt),
    dm_info dc
   PLAN (d
    WHERE cnvtint(requestin->list_0[d.seq].delete_ind)=0)
    JOIN (dc
   ...
   WITH nocounter
  ;end select
  IF (check_error("Querying DM_INFO for csv data") != 0)
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET readme_data->status = "F"
   SET readme_data->message = "Error querying dm_info with info in dm_rmc_audit_info.csv."
   GO TO exit_script
  ENDIF
  INSERT  FROM dm_info dc,
    (dummyt d  WITH seq = value(size(rs_exist->qual,5)))
   SET dc.info_name = requestin->list_0[d.seq].info_name, dc.info_domain = requestin->list_0[d.seq].
    info_domain, dc.info_number = cnvtreal(requestin->list_0[d.seq].info_number)
   PLAN (d
    WHERE (rs_exist->qual[d.seq].is_there=0)
     AND (rs_exist->qual[d.seq].delete_ind=0))
    JOIN (dc
    WHERE (dc.info_name=requestin->list_0[d.seq].info_name)
     AND (dc.info_domain=requestin->list_0[d.seq].info_domain))
   WITH nocounter
  ;end insert
  IF (check_error("Inserting csv data into DM_INFO") != 0)
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET readme_data->status = "F"
   SET readme_data->message = "Error inserting dm_info with info in dm_rmc_audit_info.csv."
   GO TO exit_script
  ENDIF
  UPDATE  FROM dm_info dc,
    (dummyt d  WITH seq = value(size(rs_exist->qual,5)))
   SET dc.info_number = cnvtreal(requestin->list_0[d.seq].info_number)
   PLAN (d
   ...
   WITH nocounter
  ;end update
  IF (check_error("Updating csv data into DM_INFO") != 0)
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET readme_data->status = "F"
   SET readme_data->message = "Error updating dm_info with info in dm_rmc_audit_info.csv."
   GO TO exit_script
  ENDIF
  SET dim_cnt = 0
  SELECT INTO "nl:"
   ...
   WITH nocounter
  ;end select
  IF (check_error("Querying DM_INFO for non delete rows") != 0)
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET readme_data->status = "F"
   SET readme_data->message = "Error querying DM_INFO for non delete rows."
   GO TO exit_script
  ENDIF
  IF (nondel_row_cnt=dim_cnt)
  ...
  ELSE
   ...
   GO TO exit_script
  ENDIF
  DELETE  FROM dm_info dc,
    (dummyt d  WITH seq = value(size(rs_exist->qual,5)))
   SET dc.info_name = requestin->list_0[d.seq].info_name, dc.info_domain = requestin->list_0[d.seq].
    info_domain
   PLAN (d
    WHERE (rs_exist->qual[d.seq].delete_ind > 0))
    JOIN (dc
    WHERE (dc.info_name=requestin->list_0[d.seq].info_name)
     AND (dc.info_domain=requestin->list_0[d.seq].info_domain))
   WITH nocounter
  ;end delete
  IF (check_error("Deleting from DM_INFO where csv has DELETE_IND set") != 0)
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET readme_data->status = "F"
   SET readme_data->message = "Error deleting dm_info with info in dm_rmc_audit_info.csv."
   GO TO exit_script
  ENDIF
  SET dim_cnt = 0
  SELECT INTO "nl:"
   ...
   WITH nocounter
  ;end select
  IF (check_error("Querying DM_INFO for delete rows") != 0)
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET readme_data->status = "F"
   SET readme_data->message = "Error querying DM_INFO for delete rows."
   GO TO exit_script
  ENDIF
  IF (dim_cnt=0)
   SET readme_data->status = "S"
  ELSE
   SET readme_data->status = "F"
   SET readme_data->message = "Check failed. Not all rows with DELETE_IND > 0 were deleted."
   GO TO exit_script
  ENDIF
 ELSE
  SET readme_data->status = "F"
