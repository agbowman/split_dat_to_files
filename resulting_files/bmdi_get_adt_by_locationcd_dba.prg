CREATE PROGRAM bmdi_get_adt_by_locationcd:dba
 RECORD reply(
   1 list[*]
     2 device_cd = f8
     2 location_cd = f8
 ...
 SELECT INTO "nl:"
  ...
  WHERE (((badt.location_cd=request->location_cd)
   AND badt.active_ind=1) OR ((badt.location_cd=request->location_cd)
 ...
 IF (curqual < 1)
  ...
  SET reply->status_data.subeventstatus[1].targetobjectname = "bmdi_get_adt_by_locationcd"
  IF (ierrcode > 0)
   SET reply->status_data.subeventstatus[2].operationname = "SELECT"
   SET reply->status_data.subeventstatus[2].operationstatus = "F"
   SET reply->status_data.subeventstatus[2].targetobjectname = "bmdi_get_adt_by_locationcd"
