CREATE PROGRAM dm_scratchpad_session_rows:dba
 ...
 SET reply->table_name = "SCRATCHPAD_SESSION"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(ss.scratchpad_session_id)
   FROM scratchpad_session ss
   WHERE ss.scratchpad_session_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(ss.scratchpad_session_id)
  FROM scratchpad_session ss
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    ss.rowid
    FROM scratchpad_session ss
    WHERE ss.scratchpad_session_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists(
      "ss.expire_dt_tm < cnvtdatetime(curdate - v_days_to_keep, curtime3)","ss"))
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = ss.rowid
    WITH nocounter, maxqual(ss,value(v_rows_left))
   ;end select
