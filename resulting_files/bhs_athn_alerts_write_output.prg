CREATE PROGRAM bhs_athn_alerts_write_output
   ...
   SELECT INTO value(moutputdevice)
    ...
    DETAIL
     ...
     v5 = build("<Title>",trim(replace(replace(replace(replace(replace(result->alerts[d.seq].title,
            "&","&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),"</Title>"),
     col + 1, v5,
     row + 1, v6 = build("<Text>",trim(replace(replace(replace(replace(replace(replace(replace(
              replace(replace(result->alerts[d.seq].text,"&","&amp;",0),"<","&lt;",0),">","&gt;",0),
             "’","&apos;",0),"‘","&apos;",0),"'","&apos;",0),'"',"&quot;",0),"â€˜","&quotl;",
         0),"â€™","&quotr;",0),3),"</Text>"), col + 1,
