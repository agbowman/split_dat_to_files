CREATE PROGRAM dm2_install_plan_menu_notify:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dbmn_row = i4 WITH protect, noconstant(0)
 DECLARE dbmn_idx = i4 WITH protect, noconstant(0)
 DECLARE dbmn_continue = i4 WITH protect, noconstant(0)
 DECLARE dbmn_iter = i4 WITH protect, noconstant(0)
 DECLARE dbmn_index = i4 WITH protect, noconstant(0)
 DECLARE dbmn_remove_location = i4 WITH protect, noconstant(0)
 DECLARE dbmn_mod_ind = i2 WITH protect, noconstant(0)
 DECLARE dbmn_pre_status = i2 WITH protect, noconstant(0)
 DECLARE dbmn_pre_email_list = vc WITH protect, noconstant("")
 DECLARE dbmn_test_location = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2_install_notify",".log","dm2_install_plan_menu_notify LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_install_plan_menu_notify"
 CALL disp_msg("",dm_err->logfile,0)
 IF (dn_get_notify_settings(null)=0)
  RETURN(0)
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dnotify)
 ENDIF
 SET dbmn_pre_status = dnotify->status
 SET dbmn_pre_email_list = dnotify->email_address_list
 SET message = window
 SET dbmn_continue = 1
 WHILE (dbmn_continue)
   CALL clear(1,1)
   CALL box(1,1,24,131)
   CALL text(2,2,"Install Plan Menu [Automated Notification Options]")
   CALL text(4,2,concat("Notification Status : ",evaluate(dnotify->status,1,"ON","OFF")))
   IF ((dnotify->status=1))
    CALL text(5,2,concat("DDL retry alert Status : ",evaluate(dnotify->suppression_flag,0,"ON","OFF")
      ))
   ENDIF
   SET dbmn_row = 6
   FOR (dbmn_idx = 1 TO least(15,dnotify->email_cnt))
     SET dbmn_row = (dbmn_row+ 1)
     CALL text(dbmn_row,4,build("Email Address",dbmn_idx,":  "))
     CALL text(dbmn_row,24,trim(dnotify->email[dbmn_idx].address))
   ENDFOR
   IF ((dnotify->email_cnt=0))
    CALL text(23,2,"(M)odify status, (A)dd email address, (S)ave, (Q)uit")
    CALL accept(23,100,"P;CU","M"
     WHERE curaccept IN ("M", "A", "S", "Q"))
   ELSEIF ((dnotify->email_cnt < 15))
    CALL text(23,2,
     "(M)odify status, (A)dd email address, (R)emove email address, (T)est Email, (S)ave,  (Q)uit")
    CALL accept(23,100,"P;CU","M"
     WHERE curaccept IN ("M", "A", "R", "S", "Q",
     "T"))
   ELSE
    CALL text(23,2,"(M)odify status, (R)emove email address, (S)ave,  (Q)uit")
    CALL accept(23,100,"P;CU","M"
     WHERE curaccept IN ("M", "R", "S", "Q"))
   ENDIF
   CASE (curaccept)
    OF "M":
     CALL clear(4,2,120)
     IF ((dm_err->debug_flag=722))
      SET message = nowindow
     ENDIF
     CALL drr_cleanup_dm_info_runners(null)
     ...
     SELECT INTO "nl:"
      ...
      WHERE dp.dm_process_id=dpe.dm_process_id
       ...
       AND dp.action_type=dpl_execution
       AND dpe.dm_process_event_id=dped.dm_process_event_id
       AND dpe.event_status IN (dpl_executing, dpl_paused)
       AND dped.detail_type=dpl_unattended
       AND dped.detail_number=1
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      SET message = nowindow
      SET dm_err->emsg = "Failed to check for Unattended installs status."
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_script
     ENDIF
     IF ((dm_err->debug_flag=722))
     ...
     ENDIF
     IF (curqual > 0
      AND (dnotify->status=1))
      CALL text(4,2,
       "Notification Status : ON [Notification cannot be turned OFF while Unattended Installs are executing.]"
       )
      CALL clear(23,2,129)
      CALL text(23,2,"(R)eturn to the main menu")
      CALL accept(23,30,"P;CU","R"
       WHERE curaccept IN ("R"))
     ELSEIF (curqual > 0)
      CALL text(4,2,"Notification Status [ON]: ")
      SET help = "ON"
      CALL accept(4,30,"P(3);CU",evaluate(dnotify->status,1,"ON",0,"OFF")
       WHERE curaccept IN ("ON"))
     ELSE
      CALL text(4,2,"Notification Status [ON | OFF]: ")
      SET help = "ON, OFF"
      CALL accept(4,35,"P(3);CU",evaluate(dnotify->status,1,"ON",0,"OFF")
       WHERE curaccept IN ("ON", "OFF"))
     ENDIF
     IF (curaccept="ON")
      SET dnotify->status = 1
     ELSEIF (curaccept="OFF")
      SET dnotify->status = 0
     ENDIF
     IF ((dnotify->status > 0))
      CALL text(5,2,"Suppress notifications related to DDL retry attempts? [YES/NO] : ")
      SET help = "YES, NO "
      CALL accept(5,65,"P(3);CU",evaluate(dnotify->suppression_flag,1,"YES",0,"NO")
       WHERE curaccept IN ("YES", "NO"))
      IF (curaccept="YES")
       SET dnotify->suppression_flag = 1
      ELSEIF (curaccept="NO")
       SET dnotify->suppression_flag = 0
      ENDIF
     ELSE
     ...
     ENDIF
     SET help = off
     IF ((dbmn_pre_status != dnotify->status))
      SET dbmn_mod_ind = 1
     ENDIF
    OF "A":
     CALL text(21,2,"Email Address to Add:")
     CALL accept(21,24,"P(40);CHU","NONE"
     ...
     IF (curaccept != "NONE"
      AND locateval(dbmn_index,1,dnotify->email_cnt,curaccept,dnotify->email[dbmn_index].address)=0)
      SET dnotify->email_cnt = (dnotify->email_cnt+ 1)
      SET stat = alterlist(dnotify->email,dnotify->email_cnt)
      SET dnotify->email[dnotify->email_cnt].address = curaccept
      IF (findstring(trim(curaccept),dbmn_pre_email_list,1,0)=0)
       SET dnotify->email[dnotify->email_cnt].new_ind = 1
       SET dbmn_mod_ind = 1
      ENDIF
     ENDIF
     CALL clear(21,2,129)
    OF "R":
     ...
     CALL accept(21,38,"99;H",0
      WHERE curaccept BETWEEN 0 AND dnotify->email_cnt)
     SET dbmn_remove_location = curaccept
     IF (dbmn_remove_location > 0)
      FOR (dbmn_iter = dbmn_remove_location TO (dnotify->email_cnt - 1))
        SET dnotify->email[dbmn_iter].address = dnotify->email[(dbmn_iter+ 1)].address
      ENDFOR
      SET stat = alterlist(dnotify->email,(dnotify->email_cnt - 1))
      SET dnotify->email_cnt = (dnotify->email_cnt - 1)
      SET dbmn_mod_ind = 1
     ENDIF
     CALL clear(21,2,129)
    OF "T":
     CALL text(21,2,"Email Address to Test, by number:")
     CALL accept(21,38,"99;H",0
      WHERE curaccept BETWEEN 0 AND dnotify->email_cnt)
     SET dbmn_test_location = curaccept
     IF (dbmn_test_location > 0)
      SET dnotify->process = "TEST NOTIFICATION"
      SET dnotify->install_status = "N/A"
      SET dnotify->msgtype = "N/A"
      SET dnotify->email_address_list = dnotify->email[dbmn_test_location].address
      SET dnotify->test_single_ind = 1
      SET dnotify->file_name = dn_test_file_name
      CALL dn_add_body_text("This is a test email.",1)
      SET dm2_process_event_rs->install_plan_id = 0
      ...
      SET dm2_process_event_rs->begin_dt_tm = cnvtdatetime(curdate,curtime3)
      SET dm2_process_event_rs->end_dt_tm = cnvtdatetime(curdate,curtime3)
      CALL dm2_process_log_add_detail_text(dpl_audit_name,"EMAIL:TEST SINGLE")
      CALL dm2_process_log_add_detail_text(dpl_audit_type,dpl_progress)
      IF (dn_notify(null)=0)
       IF ((dm_err->debug_flag=722))
        SET message = window
       ENDIF
       CALL clear(22,2,129)
       CALL text(22,2,concat("Failed to test email address : ",dnotify->email_address_list))
       CALL clear(23,2,129)
       CALL text(23,2,"(R)eturn to the main menu")
       CALL accept(23,30,"P;CU","R"
        WHERE curaccept IN ("R"))
    ...
    OF "S":
     ...
     ENDIF
     IF ((dnotify->status=1)
      AND (dnotify->email_cnt=0))
      CALL clear(22,2,129)
      CALL text(22,2,concat("Notification Status is ON.  Please enter at least one email address."))
      CALL clear(23,2,129)
      CALL text(23,2,"(R)eturn to the main menu")
      CALL accept(23,30,"P;CU","R"
       WHERE curaccept IN ("R"))
     ELSE
      IF (dn_save_notify_settings(null)=0)
       SET dbmn_continue = 0
       GO TO exit_script
      ELSE
       IF ((dm_err->debug_flag=722))
        SET message = window
       ENDIF
       IF ((dnotify->test_email_failed=1))
        CALL clear(22,2,129)
        CALL text(22,2,concat("Failed to test email address : ",dnotify->email_address_list))
        CALL clear(23,2,129)
        CALL text(23,2,"(R)eturn to the main menu")
    ...
    OF "Q":
     IF (dbmn_mod_ind=1)
      CALL clear(23,2,130)
      CALL text(23,2,
       "Unsaved changes will be lost, please confirm Quit selection : (Q)uit without saving, (R)eturn"
       )
      CALL accept(23,100,"P;CU","R"
...
#exit_script
 ...
 IF ((dm_err->err_ind=1))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ELSE
  SET dm_err->eproc = "dm2_install_plan_menu_notify completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_install_notify")
END GO
