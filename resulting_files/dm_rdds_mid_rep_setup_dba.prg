CREATE PROGRAM dm_rdds_mid_rep_setup:dba
 ...
 FREE RECORD drmrs_invalid
 RECORD drmrs_invalid(
   1 data[*]
     2 name = vc
 ) WITH protect
 DECLARE drmrs_src_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drmrs_tgt_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drmrs_type_flag = i4 WITH protect, noconstant(0)
 DECLARE drmrs_i = i4 WITH protect, noconstant(0)
 DECLARE drmrs_return = i4 WITH protect, noconstant(0)
 DECLARE drmrs_trig_cnt = i4 WITH protect, noconstant(0)
 DECLARE drmrs_reltn_val_ret = vc WITH protect, noconstant("")
 DECLARE drmrs_check_xlat_ret = vc WITH protect, noconstant("")
 DECLARE drmrs_dual_build_reject_ind = i2 WITH protect, noconstant(- (1))
 DECLARE drmrs_cur_db_reltn = vc WITH protect, noconstant("")
 SET dm_err->eproc = "Starting dm_rdds_mid_rep_setup"
 IF (check_logfile("dm_rdds_mid_rep",".log","DM_RDDS_MID_REP LOG")=0)
  SET dm_err->err_ind = 1
 ...
 IF ((((validate(drr_rdds_maint_rs->src_env_id,- (1.23))=- (1.23))) OR ((((validate(drr_rdds_maint_rs
  ...
  GO TO exit_program
 ENDIF
 SET drmrs_src_env_id = drr_rdds_maint_rs->src_env_id
 SET drmrs_tgt_env_id = drr_rdds_maint_rs->tgt_env_id
 SET drmrs_type_flag = drr_rdds_maint_rs->replicate_type
 SET drmrs_reltn_val_ret = dm2_rdds_chk_rel_val_rows(drmrs_src_env_id,drmrs_tgt_env_id,
  "@REF_DATA_LINK")
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (drmrs_reltn_val_ret="MISMATCH_FOUND")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "A new environment_id is not being used in the replicate target environment."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSEIF (drmrs_reltn_val_ret IN ("SRC_MISSING", "TGT_MISSING", "NONE_FOUND"))
  IF (drmrs_type_flag=1)
   SET drmrs_check_xlat_ret = dm2_check_xlat_seqmatch(drmrs_src_env_id,drmrs_tgt_env_id,
    "@REF_DATA_LINK")
  ELSE
   SET drmrs_check_xlat_ret = dm2_check_dmt_xlat(drmrs_src_env_id,drmrs_tgt_env_id,"@REF_DATA_LINK")
  ENDIF
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  IF (drmrs_check_xlat_ret="")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ELSE
   EXECUTE dm2_rdds_write_rel_val_rows drmrs_src_env_id, drmrs_tgt_env_id, "@REF_DATA_LINK"
   IF ((dm_err->err_ind=1))
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
  ENDIF
 ELSEIF (drmrs_reltn_val_ret="")
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET drmrs_return = dm_rdds_replicate(drmrs_type_flag,2,drmrs_src_env_id,drmrs_tgt_env_id)
 IF (drmrs_return < 0)
  ROLLBACK
  GO TO exit_program
 ENDIF
 COMMIT
 SET dm_err->eproc = "Querying to see if REFCHG* triggers exist."
 SELECT INTO "nl:"
  ut.trigger_name
  FROM user_triggers ut
  WHERE ut.trigger_name="REFCHG*"
  WITH nocounter, maxqual(ut,1)
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ...
 IF (curqual > 0)
  ...
  SELECT INTO "NL:"
   ...
   HEAD REPORT
    drmrs_trig_cnt = 0
   DETAIL
    drmrs_trig_cnt = (drmrs_trig_cnt+ 1)
    IF (mod(drmrs_trig_cnt,10)=1)
     stat = alterlist(drmrs_invalid->data,(drmrs_trig_cnt+ 9))
    ENDIF
    drmrs_invalid->data[drmrs_trig_cnt].name = d1.object_name
   FOOT REPORT
    stat = alterlist(drmrs_invalid->data,drmrs_trig_cnt)
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  FOR (drmrs_i = 1 TO size(drmrs_invalid->data,5))
    CALL parser(concat("RDB ASIS(^alter trigger ",drmrs_invalid->data[drmrs_i].name," compile^) go"))
  ENDFOR
  SELECT INTO "NL:"
   d1.object_name
   FROM dm2_user_objects d1
  ...
  ENDIF
  SET dm_err->eproc = "Gather trigger information for event."
  SET stat = alterlist(auto_ver_request->qual,1)
  SET auto_ver_request->qual[1].rdds_event = "Add Environment Triggers"
  SET auto_ver_request->qual[1].cur_environment_id = drmrs_tgt_env_id
  SET auto_ver_request->qual[1].paired_environment_id = drmrs_src_env_id
  SET auto_ver_request->qual[1].event_reason = "Automated Replicate Setup"
  SELECT INTO "nl:"
   ...
   DETAIL
    drmrs_trig_cnt = (drmrs_trig_cnt+ 1)
    IF (mod(drmrs_trig_cnt,10)=1)
     stat = alterlist(auto_ver_request->qual[1].detail_qual,(drmrs_trig_cnt+ 9))
    ENDIF
    auto_ver_request->qual[1].detail_qual[drmrs_trig_cnt].event_detail1_txt = ut.table_name,
    auto_ver_request->qual[1].detail_qual[drmrs_trig_cnt].event_value = cnt
   FOOT REPORT
    stat = alterlist(auto_ver_request->qual[1].detail_qual,drmrs_trig_cnt)
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "nl:"
  FROM user_source us
  WHERE us.name="DM_REFCHG_DUAL_BUILD_REJECT"
  WITH nocounter
 ;end select
 IF (curqual > 0)
  DECLARE dm_refchg_dual_build_reject() = i2
  SELECT INTO "nl:"
   reject_ind = dm_refchg_dual_build_reject()
   FROM dual
   DETAIL
    drmrs_dual_build_reject_ind = reject_ind
   WITH nocounter
  ;end select
  SET stat = alterlist(dera_request->env_list,1)
  SET dera_request->env_list[1].parent_env_id = drmrs_src_env_id
  SET dera_request->env_list[1].child_env_id = drmrs_tgt_env_id
  IF (drmrs_dual_build_reject_ind=1)
  ...
  ENDIF
  SET dm_err->eproc = "Checking dual build configuration to dm_env_reltn."
  CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM dm_env_reltn der
   WHERE der.parent_env_id=drmrs_src_env_id
    AND der.child_env_id=drmrs_tgt_env_id
    AND der.relationship_type IN ("BLOCK DUAL BUILD", "ALLOW DUAL BUILD")
   DETAIL
    drmrs_cur_db_reltn = der.relationship_type
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF (curqual=0)
   SET dm_err->eproc = "Adding dual build configuration to dm_env_reltn."
   ...
   IF ((dera_reply->err_num > 0))
    ...
    GO TO exit_program
   ENDIF
  ELSEIF (drmrs_cur_db_reltn="BLOCK DUAL BUILD"
   AND drmrs_dual_build_reject_ind=0)
   UPDATE  FROM dm_env_reltn der
    SET der.relationship_type = "ALLOW DUAL BUILD"
    WHERE der.parent_env_id=drmrs_src_env_id
     AND der.child_env_id=drmrs_tgt_env_id
     AND der.relationship_type="BLOCK DUAL BUILD"
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc) != 0)
    ROLLBACK
   ...
   ELSE
    COMMIT
   ENDIF
  ELSEIF (drmrs_cur_db_reltn="ALLOW DUAL BUILD"
   AND drmrs_dual_build_reject_ind=1)
   UPDATE  FROM dm_env_reltn der
    SET der.relationship_type = "BLOCK DUAL BUILD"
    WHERE der.parent_env_id=drmrs_src_env_id
     AND der.child_env_id=drmrs_tgt_env_id
     AND der.relationship_type="ALLOW DUAL BUILD"
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc) != 0)
    ROLLBACK
