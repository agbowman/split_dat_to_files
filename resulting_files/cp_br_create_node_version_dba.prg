CREATE PROGRAM cp_br_create_node_version:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "CP Node ID" = 0.0,
  "Version Text" = ""
  WITH outdev, dnode_id, sversion_txt
 ...
 DECLARE checkpublishind(null) = null WITH protect
 DECLARE loadnodedetails(null) = null WITH protect
 DECLARE versionnode(null) = null WITH protect
 DECLARE dcppathwayid = f8 WITH noconstant(0), protect
 CALL log_message(build2("Starting Program:",log_program_name),log_level_debug)
 DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
 SET mpagereply->status_data.status = "F"
 CALL checkpublishind(null)
 CALL loadnodedetails(null)
 CALL versionnode(null)
 SET mpagereply->status_data.status = "S"
 SUBROUTINE checkpublishind(null)
   CALL log_message("Begin CheckPublishInd()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SELECT INTO "NL:"
    FROM cp_node cn
    WHERE (cn.cp_node_id= $DNODE_ID)
     AND cn.publish_ind=1
    DETAIL
     dcppathwayid = cn.cp_pathway_id
    WITH nocounter
   ;end select
   CALL error_and_zero_check_rec(curqual,"CheckPublishInd","Ensure Publish is 1",1,1,
    mpagereply)
   CALL log_message(build("Exit CheckPublishInd(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE loadnodedetails(null)
   CALL log_message("Begin LoadNodeDetails()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   FREE RECORD request
   RECORD request(
     1 node_ids[*]
   ...
   SET stat = alterlist(request->node_ids,1)
   SET request->node_ids[1].cp_node_id =  $DNODE_ID
   EXECUTE cp_br_get_nodes_config  WITH replace("REPLY","NODE_REPLY")
   IF (validate(debug_ind,0))
    CALL echorecord(node_reply)
   ENDIF
   CALL log_message(build("Exit LoadNodeDetails(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE (updatecurrentnode(nodeid=f8) =vc WITH protect)
   CALL log_message("Begin updateCurrentNode()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE updateind = vc WITH protect, noconstant("F")
   UPDATE  FROM cp_node cn
    SET cn.end_effective_dt_tm = cnvtdatetime(sysdate), cn.active_ind = 1, cn.updt_cnt = (cn.updt_cnt
     + 1),
     cn.updt_dt_tm = cnvtdatetime(sysdate), cn.updt_applctx = reqinfo->updt_applctx, cn.updt_task =
     reqinfo->updt_task
   ...
   IF (curqual != 0)
    SET updateind = "S"
   ENDIF
   CALL log_message(build("Exit updateCurrentNode(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
   RETURN(updateind)
 END ;Subroutine
 SUBROUTINE versionnode(null)
   CALL log_message("Begin VersionNode()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE vx = i4 WITH noconstant(0), protect
   DECLARE vy = i4 WITH noconstant(0), protect
   DECLARE dnewnodeid = f8 WITH noconstant(0), protect
   DECLARE inactnodeind = vc WITH protect, noconstant("")
   SET inactnodeind = updatecurrentnode( $DNODE_ID)
   IF (inactnodeind="F")
    SET mpagereply->status_data.status = "F"
    SET mpagereply->status_data.subeventstatus[1].operationname = "updateCurrentNode"
    SET mpagereply->status_data.subeventstatus[1].operationstatus = "F"
    SET mpagereply->status_data.subeventstatus[1].targetobjectname = "CP_NODE"
    SET mpagereply->status_data.subeventstatus[1].targetobjectvalue =
    "ACTIVE_IND, BEG_EFFECTIVE_DT_TM"
    GO TO exit_script
   ENDIF
   ...
   SET create_node_req->cp_pathway_id = dcppathwayid
   SET stat = alterlist(create_node_req->node_list,1)
   SET create_node_req->node_list[1].concept_cd = node_reply->node_list[1].concept_cd
   SET create_node_req->node_list[1].intention_cd = node_reply->node_list[1].intention_cd
   SET create_node_req->node_list[1].node_name = node_reply->node_list[1].node_name
   SET create_node_req->node_list[1].publish_ind = 0
   SET create_node_req->node_list[1].category_mean = node_reply->node_list[1].category_mean
   SET create_node_req->node_list[1].version_node_id =  $DNODE_ID
   CALL echorecord(create_node_req)
   ...
   IF ((create_node_reply->status_data.status != "S"))
    ...
    GO TO exit_script
   ENDIF
   SET dnewnodeid = create_node_reply->node_list[1].cp_node_id
   SET mpagereply->version_node_id = dnewnodeid
   SET mpagereply->cp_node_id =  $DNODE_ID
   RECORD add_comp_req(
     1 cp_node_id = f8
   ...
   FOR (vx = 1 TO size(node_reply->node_list[1].component_list,5))
     SET stat = initrec(add_comp_req)
     SET stat = alterlist(add_comp_req->component_list,1)
     SET add_comp_req->bedrock_wizard_ind = 1
     SET add_comp_req->cp_node_id = dnewnodeid
     SET add_comp_req->component_list[1].comp_type_cd = node_reply->node_list[1].component_list[vx].
     comp_type_cd
     SET add_comp_req->component_list[1].component_seq_txt = node_reply->node_list[1].component_list[
     vx].comp_seq_txt
     EXECUTE cp_br_add_component  WITH replace("REQUEST","ADD_COMP_REQ"), replace("REPLY",
      "ADD_COMP_REP")
     ...
     IF ((add_comp_rep->status_data.status != "S"))
      ...
      GO TO exit_script
     ENDIF
     SET stat = initrec(add_comp_detail_req)
     FOR (vy = 1 TO size(node_reply->node_list[1].component_list[vx].comp_detail_list,5))
       SET stat = alterlist(add_comp_detail_req->comp_detail_list,size(node_reply->node_list[1].
         component_list[vx].comp_detail_list,5))
       SET add_comp_detail_req->comp_detail_list[vy].cp_component_id = add_comp_rep->component_list[1
       ].cp_component_id
       SET add_comp_detail_req->comp_detail_list[vy].comp_detail_reltn_cd = node_reply->node_list[1].
       component_list[vx].comp_detail_list[vy].detail_reltn_cd
       SET add_comp_detail_req->comp_detail_list[vy].component_entity_id = node_reply->node_list[1].
       component_list[vx].comp_detail_list[vy].entity_id
       SET add_comp_detail_req->comp_detail_list[vy].component_entity_name = node_reply->node_list[1]
       .component_list[vx].comp_detail_list[vy].entity_name
       SET add_comp_detail_req->comp_detail_list[vy].component_ident = node_reply->node_list[1].
       component_list[vx].comp_detail_list[vy].entity_ident
       SET add_comp_detail_req->comp_detail_list[vy].component_text = node_reply->node_list[1].
       component_list[vx].comp_detail_list[vy].entity_text
     ENDFOR
     CALL echorecord(add_comp_detail_req)
     IF (size(add_comp_detail_req->comp_detail_list,5) > 0)
      EXECUTE cp_add_component_detail  WITH replace("REQUEST","ADD_COMP_DETAIL_REQ"), replace("REPLY",
       "ADD_COMP_DETAIL_REP")
      IF ((add_comp_detail_rep->status_data.status != "S"))
       SET mpagereply->status_data.status = "F"
       GO TO exit_script
      ENDIF
      SET stat = initrec(add_comp_detail_rep)
     ENDIF
   ENDFOR
   UPDATE  FROM cp_node cn
    SET cn.beg_effective_dt_tm = cnvtdatetime(sysdate), active_ind = 0
    WHERE (cn.cp_node_id= $DNODE_ID)
    WITH nocounter
   ;end update
   CALL error_and_zero_check_rec(curqual,"Update New Node","Set Pub",1,1,
    mpagereply)
   CALL log_message(build("Exit VersionNode(), Elapsed time in seconds:",((curtime3 - begin_curtime3)
