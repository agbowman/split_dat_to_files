CREATE PROGRAM djh_ma_phys_90day_nonmedstf
 PROMPT
 ...
 SELECT DISTINCT INTO value(output_dest)
  ...
  create_dt_tm = format(pr.create_dt_tm,"YYYY/MM/DD;;D")
  FROM prsnl pr,
   omf_app_ctx_day_st oa
  PLAN (pr
   WHERE pr.active_ind=1
    AND pr.active_status_cd=188.00
    AND pr.position_cd IN (1646210)
 ...
 IF (email_ind=1)
  ...
  SET filename_out = concat(format(curdate,"YYYY-MM-DD;;D"),"-Activity.csv")
  DECLARE subject_line = vc
  SET subject_line = concat(curprog," V1 - Baystate Health CIS PHYS Acnts Inactive 1 days")
