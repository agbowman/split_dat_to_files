CREATE PROGRAM dm2_add_old_dcl_triggers:dba
   ...
   FOR (type_ndx = 1 TO 2)
     ...
     IF (type_ndx=1
      ...
      IF (i_local_ind=0)
       ...
       ;end insert
       IF (check_error(dm_err->eproc)=1)
        CALL disp_mst(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
        GO TO exit_program
       ENDIF
      ENDIF
      SET stat = alterlist(rs_dtd_reply->dtd_hold[i_dtd_ndx].pk_where_parm,s_parm_cnt)
 ...
 CALL disp_msg("",dm_err->logfile,0)
 DECLARE create_filter_function_ora(i_dtd_ndx=i4,io_drop_cnt=i4(ref),io_stmt_cnt=i4(ref)) = null
 DECLARE create_triggers_ora(i_dtd_ndx=i4,io_stmt_cnt=i4(ref)) = null
 ...
 DECLARE refchg_xlat_gather(null) = null
 DECLARE ac_log_type = vc
 DECLARE ac_table_name = vc
 DECLARE dtd_ndx = i4
 DECLARE upto_value = i4
 DECLARE envid_cnt = i4
 DECLARE cl_id_cnt = i4
 DECLARE cl_temp_envid = i4
 DECLARE dtd_find = i4
 DECLARE is_asterisk = i4
 DECLARE skip_flag = i4
 DECLARE add_trig_date = f8
 DECLARE total_trigger_cnt = i4
 DECLARE env_csv = vc
 DECLARE cl_drop_cnt = i4 WITH public, noconstant(0)
 DECLARE cio_pseudo_column = vc WITH protect, noconstant("")
 DECLARE daclt_inhouse_flag = i2
 DECLARE daclt_tab_flag = i4
 DECLARE rs_ndx = i4
 DECLARE child_cnt = i4
 DECLARE daclt_proc_name = vc
 DECLARE daclt_proc_name_new = vc
 DECLARE daclt_trig_add = vc
 DECLARE daclt_trig_del = vc
 DECLARE daclt_trig_upd = vc
 DECLARE loop_ndx_parent = i4
 DECLARE loop_ndx_child2 = i4
 DECLARE v_single_tab_ind = i2
 DECLARE refchg_tab_r_find = i2
 DECLARE child_loop = i2
 DECLARE parent_loop = i2
 DECLARE single_commit_table_name = vc
 DECLARE v_drop_cnt = i4
 DECLARE v_stmt_cnt = i4
 DECLARE v_parm_list = vc
 DECLARE v_trigger_name = vc
 DECLARE v_log_type = vc
 DECLARE updt_ndx = i2
 DECLARE loop_cnt = i2
 DECLARE trig_drop_cnt = i4
 DECLARE create_xlat_trig_flag = i2
 DECLARE trig_drop_stmt = vc
 DECLARE daclt_tab_cnt = i4
 DECLARE daclt_tab_found = i4
 DECLARE daclt_src_id = f8
 DECLARE daclt_temp_tab = vc
 DECLARE daclt_num = i4
 DECLARE daclt_ev_reason = vc
 DECLARE daclt_r_event = vc
 DECLARE daclt_dcl_curqual = i2
 DECLARE daclt_r_tab_name = vc
 DECLARE sc_curqual = i4
 DECLARE daclt_sc_ndx = i4
 DECLARE sc_rmv_ndx = i4
 ...
 FREE RECORD daclt_sc_remove
 RECORD daclt_sc_remove(
   1 trig_cnt = i4
   1 qual[*]
     2 tbl_name = vc
     2 trig_name = vc
 ...
 ENDIF
 SET dm_err->eproc = "Checking to see if single commit is on or off."
 CALL disp_msg(" ",dm_err->logfile,dm_err->err_ind)
 SELECT INTO "NL:"
  FROM dm_refchg_tab_r drtr
  WHERE substring(1,1,drtr.parent_table) >= "A"
   AND substring(1,1,drtr.parent_table) <= "Z"
  WITH nocounter
 ;end select
 SET sc_curqual = curqual
 IF (check_error(dm_err->eproc)=1)
  CALL disp_mst(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (sc_curqual=0)
  SET dm_err->eproc = "Looking for single commit child table triggers to drop."
  CALL disp_msg(" ",dm_err->logfile,dm_err->err_ind)
  SELECT INTO "NL:"
   ut.table_name, ut.trigger_name
   FROM user_triggers ut
   WHERE ((ut.trigger_name="REFCHG????") OR (ut.trigger_name="REFCHG????$C"))
   DETAIL
    daclt_sc_remove->trig_cnt = (daclt_sc_remove->trig_cnt+ 1), stat = alterlist(daclt_sc_remove->
     qual,daclt_sc_remove->trig_cnt), daclt_sc_remove->qual[daclt_sc_remove->trig_cnt].tbl_name = ut
    .table_name,
    daclt_sc_remove->qual[daclt_sc_remove->trig_cnt].trig_name = ut.trigger_name
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_mst(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 ENDIF
 SET dm_err->eproc = "Get list of tables that do no need REFCHG triggers"
 SELECT INTO "NL:"
  ...
  DETAIL
   t_count = (t_count+ 1)
   IF (mod(t_count,10)=1)
    stat = alterlist(daclt_nordds_rs->dnr_hold,(t_count+ 9))
   ENDIF
   daclt_nordds_rs->dnr_hold[t_count].table_name = cnvtupper(cv.display)
  FOOT REPORT
   stat = alterlist(daclt_nordds_rs->dnr_hold,t_count)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 SELECT INTO "NL:"
  dr.child_table, dr.parent_table, dr.parent_id_col,
  dr.parent_tab_col, dr.pk_where, dr.md_value_str,
  dr.md_var_str
  FROM dm_refchg_tab_r dr
  WHERE process_type != "MOVER ONLY"
  ORDER BY cnvtupper(dr.child_table)
  HEAD REPORT
   child_cnt = 0
  HEAD dr.child_table
   child_cnt = (child_cnt+ 1), stat = alterlist(refchg_tab_r->child,child_cnt), par_cnt = 0
  DETAIL
   par_cnt = (par_cnt+ 1), stat = alterlist(refchg_tab_r->child[child_cnt].parent,par_cnt),
   refchg_tab_r->child[child_cnt].child_table = cnvtupper(dr.child_table),
   refchg_tab_r->child[child_cnt].parent[par_cnt].parent_table = cnvtupper(dr.parent_table),
   refchg_tab_r->child[child_cnt].parent[par_cnt].parent_id_col = cnvtupper(dr.parent_id_col),
   refchg_tab_r->child[child_cnt].parent[par_cnt].parent_tab_col = cnvtupper(dr.parent_tab_col),
   refchg_tab_r->child[child_cnt].parent[par_cnt].pk_where = dr.pk_where, refchg_tab_r->child[
   child_cnt].parent[par_cnt].md_value_str = dr.md_value_str, refchg_tab_r->child[child_cnt].parent[
   par_cnt].md_var_str = dr.md_var_str,
   refchg_tab_r->child[child_cnt].regen_ind = 0
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (size(dguc_reply->dtd_hold,5)=0)
  SET is_asterisk = findstring("*",ac_table_name)
  FOR (child_loop = 1 TO size(refchg_tab_r->child,5))
    IF ((((refchg_tab_r->child[child_loop].child_table=ac_table_name)) OR ((refchg_tab_r->child[
    child_loop].child_table=patstring(ac_table_name)))) )
     SET child_loop = size(refchg_tab_r->child,5)
     SET refchg_tab_r_find = 1
    ELSE
     FOR (parent_loop = 1 TO size(refchg_tab_r->child[child_loop].parent,5))
       IF ((((refchg_tab_r->child[child_loop].parent[parent_loop].parent_table=ac_table_name)) OR ((
       refchg_tab_r->child[child_loop].parent[parent_loop].parent_table=patstring(ac_table_name)))) )
        SET parent_loop = size(refchg_tab_r->child[child_loop].parent,5)
        SET child_loop = size(refchg_tab_r->child,5)
        SET refchg_tab_r_find = 1
       ENDIF
     ENDFOR
    ENDIF
  ENDFOR
  IF (refchg_tab_r_find=1)
   SET dguc_request->what_tables = "*"
  ELSE
   SET dguc_request->what_tables = ac_table_name
  ENDIF
  IF (ac_log_type="REFCHG")
   SET dguc_request->is_ref_ind = 1
   SET dguc_request->is_mrg_ind = 0
  ELSE
   SET dguc_request->is_ref_ind = 0
   SET dguc_request->is_mrg_ind = 0
  ENDIF
  SET dguc_request->only_special_ind = 0
  IF ((dguc_request->what_tables="*"))
   SET stat = alterlist(dguc_request->req_special,10)
  ...
  ENDIF
  SET dm_err->eproc = "Executing DM_GET_UNIQUE_COLUMNS from DM2_ADD_CHG_LOG_TRIGGERS"
  CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
  ...
  IF ((dguc_reply->dguc_err_ind=1))
   ...
   GO TO exit_program
  ENDIF
  IF (((size(dguc_reply->dtd_hold,5) > 0) OR (refchg_tab_r_find=1)) )
   CALL cl_move_over(dguc_reply,ac_table_name,ac_log_type,refchg_tab_r_find,0)
   IF (findstring("*",ac_table_name)=0)
    SET v_single_tab_ind = 1
    FOR (child_loop = 1 TO size(rs_dtd_reply->dtd_hold,5))
      IF ((((rs_dtd_reply->dtd_hold[child_loop].tbl_name=ac_table_name)) OR ((rs_dtd_reply->dtd_hold[
      child_loop].tbl_name=patstring(ac_table_name)))) )
       SET rs_dtd_reply->dtd_hold[child_loop].process_ind = 1
      ENDIF
    ENDFOR
    FOR (child_loop = 1 TO size(refchg_tab_r->child,5))
      SET loop_ndx_child2 = 0
      SET loop_ndx_child2 = locateval(loop_ndx_child2,1,size(rs_dtd_reply->dtd_hold,5),refchg_tab_r->
       child[child_loop].child_table,rs_dtd_reply->dtd_hold[loop_ndx_child2].tbl_name)
      IF (loop_ndx_child2 > 0)
       SET rs_dtd_reply->dtd_hold[loop_ndx_child2].process_ind = 1
      ENDIF
      FOR (parent_loop = 1 TO size(refchg_tab_r->child[child_loop].parent,5))
        SET loop_ndx_child2 = 0
        SET loop_ndx_child2 = locateval(loop_ndx_child2,1,size(rs_dtd_reply->dtd_hold,5),refchg_tab_r
         ->child[child_loop].parent[parent_loop].parent_table,rs_dtd_reply->dtd_hold[loop_ndx_child2]
         .tbl_name)
        IF (loop_ndx_child2 > 0)
         SET rs_dtd_reply->dtd_hold[loop_ndx_child2].process_ind = 1
        ENDIF
    ...
    ENDFOR
   ENDIF
   SET upto_value = rs_dtd_reply->rs_tbl_cnt
   SET dtd_find = 1
  ELSE
   SET dm_err->err_ind = 1
   SET dm_err->emsg =
 ...
 ELSE
  ...
  IF (is_asterisk > 0)
   ...
   SET upto_value = rs_dtd_reply->rs_tbl_cnt
  ELSE
   SET dtd_find = locateval(dtd_find,1,value(size(rs_dtd_reply->dtd_hold,5)),dguc_request->
    what_tables,rs_dtd_reply->dtd_hold[dtd_find].tbl_name)
   SET upto_value = dtd_find
  ENDIF
  IF (dtd_find < 1)
   SET dm_err->err_ind = 1
   SET dm_err->emsg =
 ...
 ENDIF
 SET cl_id_cnt = 0
 SET dm_err->eproc = concat("Getting environment_id.")
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_name="DM_ENV_ID"
 ...
 IF (size(derg_reply->child_env_list,5)=0)
  SET derg_request->env_id = cl_temp_envid
  SET derg_request->relationship_type = "REFERENCE MERGE"
  EXECUTE dm_get_env_reltn
 ENDIF
 SET envid_cnt = size(derg_reply->child_env_list,5)
 IF (envid_cnt > 0)
  SELECT INTO "NL;"
   FROM dm_env_reltn der,
    (dummyt d  WITH seq = size(derg_reply->child_env_list,5))
   PLAN (d)
    JOIN (der
    WHERE der.parent_env_id=cl_temp_envid
     AND (der.child_env_id=derg_reply->child_env_list[d.seq].env_id)
     AND der.relationship_type="RDDS MOVER CHANGES NOT LOGGED")
   DETAIL
    derg_reply->child_env_list[d.seq].no_log_ind = 1
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Building DM_REFCHG_ENVID PROCEDURE."
 CALL disp_msg("",dm_err->logfile,0)
 CALL stmt_push(v_stmt_cnt,concat('RDB ASIS("create or replace procedure dm_refchg_envid ',
   '(io_envid_tbl OUT dbms_sql.number_table,io_log_tbl OUT dbms_sql.number_table) as")'))
 CALL stmt_push(v_stmt_cnt,concat('ASIS("begin")'))
 IF (envid_cnt=0)
  CALL stmt_push(v_stmt_cnt,'ASIS(" io_envid_tbl(1) := -1;")')
  CALL stmt_push(v_stmt_cnt,'ASIS(" io_log_tbl(1) :=  -1;")')
 ELSE
  FOR (cl_id_cnt = 1 TO envid_cnt)
   CALL stmt_push(v_stmt_cnt,concat('ASIS(" io_envid_tbl(',trim(cnvtstring(cl_id_cnt)),") :=  ",trim(
      cnvtstring(derg_reply->child_env_list[cl_id_cnt].env_id)),';")'))
   CALL stmt_push(v_stmt_cnt,concat('ASIS(" io_log_tbl(',trim(cnvtstring(cl_id_cnt)),") :=  ",trim(
      cnvtstring(derg_reply->child_env_list[cl_id_cnt].no_log_ind)),';")'))
  ENDFOR
 ENDIF
 CALL stmt_push(v_stmt_cnt,concat('ASIS("end;")'))
 CALL stmt_push(v_stmt_cnt,"end go")
 SET stat = alterlist(stmt_buffer->data,v_stmt_cnt)
 EXECUTE dm2_stmt_exe "PROCEDURE", "dm_refchg_envid" WITH replace("REQUEST","STMT_BUFFER"), replace(
  "REPLY","SE_REPLY")
 SET v_stmt_cnt = 0
 IF ((se_reply->err_ind=1))
  SET dm_err->emsg = "Error occurred in dm2_stmt_exe."
  ...
  GO TO exit_program
 ENDIF
 IF (daclt_dcl_curqual=0)
  SET dm_err->eproc = "Writing 'End Single Commit Backfill' event row."
  CALL disp_msg(" ",dm_err->logfile,dm_err->err_ind)
  SET stat = alterlist(auto_ver_request->qual,1)
  SET auto_ver_request->qual[1].rdds_event = "End Single Commit Backfill"
  SET auto_ver_request->qual[1].cur_environment_id = daclt_src_id
  SET auto_ver_request->qual[1].event_reason = ""
  EXECUTE dm_rmc_auto_verify_setup
  IF ((auto_ver_reply->status="F"))
   SET dm_err->err_ind = 1
  ...
  ENDIF
 ENDIF
 FOR (daclt_loop_cnt = 1 TO size(refchg_tab_r->child,5))
   SET daclt_tab_flag = locateval(daclt_tab_flag,1,rs_dtd_reply->rs_tbl_cnt,refchg_tab_r->child[
    daclt_loop_cnt].child_table,rs_dtd_reply->dtd_hold[daclt_tab_flag].tbl_name)
   SET refchg_tab_r->child[daclt_loop_cnt].child_tab_sfx = rs_dtd_reply->dtd_hold[daclt_tab_flag].
   tbl_suffix
   SET daclt_proc_name = build("PROC_REFCHG",refchg_tab_r->child[daclt_loop_cnt].child_tab_sfx,"*")
   SET refchg_tab_r->child[daclt_loop_cnt].proc_name = build("PROC_REFCHG",refchg_tab_r->child[
    daclt_loop_cnt].child_tab_sfx,"_1")
   SET refchg_tab_r->child[daclt_loop_cnt].old_proc_name = build("PROC_REFCHG",refchg_tab_r->child[
    daclt_loop_cnt].child_tab_sfx,"_2")
   SELECT INTO "NL:"
    uo.object_name
    FROM user_objects uo
    WHERE uo.object_name=patstring(daclt_proc_name)
    DETAIL
     rs_ndx = locateval(rs_ndx,1,rs_dtd_reply->rs_tbl_cnt,refchg_tab_r->child[daclt_loop_cnt].
      child_table,rs_dtd_reply->dtd_hold[rs_ndx].tbl_name)
     IF (rs_ndx > 0)
      refchg_tab_r->child[daclt_loop_cnt].regen_ind = 1, daclt_proc_name_new = build("PROC_REFCHG",
       refchg_tab_r->child[daclt_loop_cnt].child_tab_sfx,"_1")
      IF (uo.object_name=daclt_proc_name_new)
       refchg_tab_r->child[daclt_loop_cnt].proc_name = build("PROC_REFCHG",refchg_tab_r->child[
        daclt_loop_cnt].child_tab_sfx,"_2"), refchg_tab_r->child[daclt_loop_cnt].old_proc_name =
       build("PROC_REFCHG",refchg_tab_r->child[daclt_loop_cnt].child_tab_sfx,"_1")
      ENDIF
     ELSE
      refchg_tab_r->child[daclt_loop_cnt].regen_ind = 0, refchg_tab_r->child[daclt_loop_cnt].
      proc_name = uo.object_name
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
 ...
 IF (currdb="ORACLE")
  ...
  ENDIF
  IF (curqual != 0)
   CALL refchg_xlat_gather(null)
   SET create_xlat_trig_flag = 1
  ENDIF
  FOR (dtd_ndx = dtd_find TO upto_value)
    IF (((v_single_tab_ind=0) OR ((rs_dtd_reply->dtd_hold[dtd_ndx].process_ind=1))) )
     SET skip_flag = 0
     SELECT INTO "NL:"
      di.info_name
      FROM dm_info di
      WHERE di.info_domain="APPREG"
       AND di.info_name="ATR SOURCE DOMAIN"
      DETAIL
       daclt_inhouse_flag = 1
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      GO TO exit_program
     ENDIF
     IF (daclt_inhouse_flag=1
      AND (rs_dtd_reply->dtd_hold[dtd_ndx].tbl_name IN ("APPLICATION", "APPLICATION_TASK",
     "APPLICATION_TASK_R", "REQUEST", "TASK_REQUEST_R")))
      SET skip_flag = 1
     ELSE
      SET skip_flag = 0
      SET loop_ndx_child2 = 0
      SET loop_ndx_child2 = locateval(loop_ndx_child2,1,size(refchg_tab_r->child,5),rs_dtd_reply->
       dtd_hold[dtd_ndx].tbl_name,refchg_tab_r->child[loop_ndx_child2].child_table)
      IF (loop_ndx_child2 > 0)
       SET skip_flag = 0
      ELSE
       IF (ac_log_type="REFCHG")
        IF (skip_flag=0)
         SET cl_drop_cnt = 0
         SET skip_flag = locateval(skip_flag,1,size(daclt_nordds_rs->dnr_hold,5),rs_dtd_reply->
          dtd_hold[dtd_ndx].tbl_name,daclt_nordds_rs->dnr_hold[skip_flag].table_name)
         IF (skip_flag > 0)
          SET dm_err->eproc = concat("Checking for RDDS triggers on ",rs_dtd_reply->dtd_hold[dtd_ndx]
           .tbl_name)
          CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
          SET skip_flag = 1
          SELECT INTO "NL:"
           FROM user_triggers u
           WHERE (u.table_name=rs_dtd_reply->dtd_hold[dtd_ndx].tbl_name)
            AND ((u.trigger_name="REFCHG*") OR (u.trigger_name="TRG_DM_CHG_LOG_*"))
           DETAIL
            cl_drop_cnt = (cl_drop_cnt+ 1), stat = alterlist(cl_drop_stmt->qual,cl_drop_cnt),
            cl_drop_stmt->qual[cl_drop_cnt].stmt = concat("rdb drop trigger ",u.trigger_name," go")
           WITH nocounter
          ;end select
          IF (curqual > 0
           AND size(cl_drop_stmt->qual,5) > 0)
           SET dm_err->eproc = concat("Dropping RDDS triggers on ",rs_dtd_reply->dtd_hold[dtd_ndx].
            tbl_name)
           CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
           SET cl_drop_cnt = 0
      ...
      ENDIF
      IF (skip_flag=0)
       CALL echo(concat("About to call create_filter_function for ",rs_dtd_reply->dtd_hold[dtd_ndx].
         tbl_name))
       CALL create_filter_function_ora(dtd_ndx,v_drop_cnt,v_stmt_cnt)
       CALL echo(concat("About to call create_pk_where for ",rs_dtd_reply->dtd_hold[dtd_ndx].tbl_name
         ))
       CALL create_pk_where_function_ora(dtd_ndx,v_drop_cnt,v_stmt_cnt,0)
       SET dm_err->eproc = "Calling UPD_PKW_VERS_TAB"
       CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
       IF (upd_pkw_vers_tab(rs_dtd_reply->dtd_hold[dtd_ndx].tbl_name,rs_dtd_reply->dtd_hold[dtd_ndx].
        tbl_suffix)="F")
        SET dm_err->err_ind = 1
        CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
        GO TO exit_program
       ENDIF
       SET dm_err->eproc = "Selecting PK_WHERE_VERSION_ID from DM_REFCHG_PKW_VERS"
       CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
       SET rs_dtd_reply->dtd_hold[dtd_ndx].pkw_vers_id = 0
       SELECT INTO "NL:"
        drpv.dm_refchg_pkw_vers_id
        FROM dm_refchg_pkw_vers drpv
        WHERE (drpv.table_name=rs_dtd_reply->dtd_hold[dtd_ndx].tbl_name)
         AND drpv.active_ind=1
        DETAIL
         rs_dtd_reply->dtd_hold[dtd_ndx].pkw_vers_id = drpv.dm_refchg_pkw_vers_id
        WITH nocounter
       ;end select
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
       ...
       SET found_ndx = 0
       IF (ac_log_type="REFCHG")
        SET found_ndx = binsearch_refchg(rs_dtd_reply->dtd_hold[dtd_ndx].tbl_name)
       ELSE
        SET found_ndx = 0
       ENDIF
       IF (found_ndx > 0)
        CALL create_sc_proc_ora(dtd_ndx,found_ndx,v_stmt_cnt)
       ELSE
        CALL create_triggers_ora(dtd_ndx,v_stmt_cnt)
       ENDIF
       IF (create_xlat_trig_flag=1)
        CALL create_xlat_triggers(dtd_ndx,v_stmt_cnt)
       ENDIF
      ENDIF
     ENDIF
    ENDIF
  ENDFOR
  CALL create_sc_triggers_ora(dtd_find,upto_value,v_single_tab_ind,v_stmt_cnt)
  CALL drop_old_functions(v_drop_cnt)
  FOR (loop_ndx = 1 TO size(refchg_tab_r->child,5))
   SET rs_ndx = locateval(rs_ndx,1,rs_dtd_reply->rs_tbl_cnt,refchg_tab_r->child[loop_ndx].child_table,
    rs_dtd_reply->dtd_hold[rs_ndx].tbl_name)
   IF (rs_ndx > 0)
    ...
    IF (curqual > 0
     AND size(cl_drop_stmt->qual,5) > 0)
     SET dm_err->eproc = concat("Dropping RDDS triggers on ",rs_dtd_reply->dtd_hold[rs_ndx].tbl_name)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
     SET cl_drop_cnt = 0
     FOR (cl_drop_cnt = 1 TO size(cl_drop_stmt->qual,5))
       CALL dm2_push_cmd(cl_drop_stmt->qual[cl_drop_cnt].stmt,1)
 ...
 SELECT INTO "NL:"
  ...
  WHERE ut.trigger_name="REFCHG*"
   AND ut.table_name=dtd.table_name
   AND dtd.reference_ind=0
   AND  NOT (ut.table_name IN ("ACCESSION", "ADDRESS", "PHONE", "PERSON", "PERSON_NAME",
  ...
  DETAIL
   find_trigger_name_ndx = 0, find_trigger_name_ndx = locateval(find_trigger_name_ndx,1,size(
     refchg_xlat_drop,5),ut.trigger_name,dm2_cl_trg_updt_cols->qual[find_trigger_name_ndx].tname)
 ...
 ENDFOR
 SUBROUTINE create_filter_function_ora(i_dtd_ndx,io_drop_cnt,io_stmt_cnt)
   DECLARE s_or_str = vc
   DECLARE s_mode = vc
   DECLARE s_if_str = vc
   DECLARE s_filter_ind = i2 WITH noconstant(0)
   DECLARE s_filter_type = vc
   DECLARE s_statement_ind = i2
   DECLARE s_start_pos = i4
   DECLARE s_line_length = i4
   DECLARE s_continue_ind = i4
   DECLARE s_stmt_str = vc
   DECLARE s_func_name = vc
   DECLARE s_drop_func_name = vc
   DECLARE s_temp_str = vc
   DECLARE s_omf_prsnl_group_flg = i2
   SET s_omf_prsnl_group_flg = 1
   SELECT INTO "NL:"
    drf.filter_type, drf.statement_ind
    FROM dm_refchg_filter drf
    WHERE (drf.table_name=rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name)
     AND drf.active_ind=1
     AND  NOT ( EXISTS (
    (SELECT
     "X"
     FROM dm_refchg_filter_parm p
     WHERE (p.table_name=rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name)
      AND  NOT ( EXISTS (
     (SELECT
      "X"
      FROM user_tab_columns utc
    ...
    WITH nocounter
   ;end select
   IF ((rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name IN ("OMF_PVI_ATTRIB", "OMF_PVI_ATT_VALUE",
   ...
   IF (s_omf_prsnl_group_flg=1)
    SET s_func_name = concat("REFCHG_FILTER_",rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_suffix,"_1")
    SET s_drop_func_name = ""
    SELECT INTO "NL:"
     uo.object_name
     FROM user_objects uo
     WHERE uo.object_name=patstring(concat("REFCHG_FILTER_",rs_dtd_reply->dtd_hold[i_dtd_ndx].
       tbl_suffix,"*"))
      AND uo.object_type="FUNCTION"
     DETAIL
      IF (uo.object_name=concat("REFCHG_FILTER_",rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_suffix,"_1"))
       s_drop_func_name = uo.object_name, s_func_name = concat("REFCHG_FILTER_",rs_dtd_reply->
        dtd_hold[i_dtd_ndx].tbl_suffix,"_2")
      ELSE
       s_drop_func_name = uo.object_name, s_func_name = concat("REFCHG_FILTER_",rs_dtd_reply->
        dtd_hold[i_dtd_ndx].tbl_suffix,"_1")
     ...
     WITH nocounter
    ;end select
    SET rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_function = s_func_name
    IF (s_drop_func_name > " ")
     SET io_drop_cnt = (io_drop_cnt+ 1)
     IF (mod(io_drop_cnt,50)=1)
      SET stat = alterlist(drop_func->data,(io_drop_cnt+ 49))
    ...
    ENDIF
    IF (s_filter_ind=0)
     SET rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_function = ""
    ELSE
     SELECT INTO "nl:"
      drfp.column_name
      FROM dm_refchg_filter_parm drfp
      WHERE (drfp.table_name=rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name)
       AND drfp.active_ind=1
      ORDER BY drfp.parm_nbr, drfp.column_name
      HEAD REPORT
       parm_cnt = 0
      DETAIL
       parm_cnt = (parm_cnt+ 1), stat = alterlist(rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_parm,
        parm_cnt), rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_parm[parm_cnt].column_name = cnvtupper(
        drfp.column_name)
      WITH nocounter
     ;end select
     SELECT INTO "nl:"
      utc.data_type
      FROM user_tab_columns utc,
       (dummyt d  WITH seq = size(rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_parm,5))
      PLAN (d)
       JOIN (utc
       WHERE (utc.table_name=rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name)
        AND (utc.column_name=rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_parm[d.seq].column_name))
      DETAIL
       rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_parm[d.seq].data_type = utc.data_type
      WITH nocounter
     ;end select
     SELECT INTO "nl:"
      drft.test_str, drft.statement_ind
      FROM dm_refchg_filter_test drft
      WHERE (drft.table_name=rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name)
       AND drft.active_ind=1
      ORDER BY drft.statement_ind
      HEAD REPORT
       test_cnt = 0
      DETAIL
       test_cnt = (test_cnt+ 1), stat = alterlist(rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str,
        test_cnt), rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str[test_cnt].add_str = drft.test_str,
       rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str[test_cnt].upd_str = drft.test_str, rs_dtd_reply->
       dtd_hold[i_dtd_ndx].filter_str[test_cnt].del_str = drft.test_str, rs_dtd_reply->dtd_hold[
       i_dtd_ndx].filter_str[test_cnt].statement_ind = drft.statement_ind
       FOR (parm_ndx = 1 TO size(rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_parm,5))
         s_temp_str = concat("::",rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_parm[parm_ndx].column_name,
          "::"), rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str[test_cnt].add_str = replace(
          rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str[test_cnt].add_str,s_temp_str,concat("i_",
           rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_parm[parm_ndx].column_name,"_new"),0),
         rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str[test_cnt].del_str = replace(rs_dtd_reply->
          dtd_hold[i_dtd_ndx].filter_str[test_cnt].del_str,s_temp_str,concat("i_",rs_dtd_reply->
           dtd_hold[i_dtd_ndx].filter_parm[parm_ndx].column_name,"_old"),0)
       ENDFOR
       rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str[test_cnt].upd_str = rs_dtd_reply->dtd_hold[
       i_dtd_ndx].filter_str[test_cnt].add_str
      WITH nocounter
     ;end select
     CALL stmt_push(io_stmt_cnt,concat('RDB ASIS("create or replace function ',rs_dtd_reply->
       dtd_hold[i_dtd_ndx].filter_function,'")'))
     CALL stmt_push(io_stmt_cnt,'ASIS("(i_mode varchar2")')
     FOR (fp_ndx = 1 TO size(rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_parm,5))
      CALL stmt_push(io_stmt_cnt,concat('ASIS(",i_',rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_parm[
        fp_ndx].column_name,"_new ",rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_parm[fp_ndx].data_type,
        '")'))
      CALL stmt_push(io_stmt_cnt,concat('ASIS(",i_',rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_parm[
        fp_ndx].column_name,"_old ",rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_parm[fp_ndx].data_type,
        '")'))
     ENDFOR
     ...
     FOR (mode_ndx = 1 TO 3)
       ...
       IF (s_filter_type IN ("INCLUDE", "EXCLUDE"))
        ...
        ENDIF
        SET s_or_str = " ("
        FOR (fs_ndx = 1 TO size(rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str,5))
          IF ((rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str[fs_ndx].statement_ind=0))
           IF (mode_ndx=1)
            SET s_if_str = rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str[fs_ndx].add_str
           ELSEIF (mode_ndx=2)
            SET s_if_str = rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str[fs_ndx].upd_str
           ELSE
            SET s_if_str = rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str[fs_ndx].del_str
           ENDIF
       ...
       IF (s_statement_ind=1)
        FOR (fs_ndx = 1 TO size(rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str,5))
          IF ((rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str[fs_ndx].statement_ind=1))
           IF (((s_filter_type IN ("INCLUDE", "EXCLUDE")) OR (fs_ndx > 1)) )
            CALL stmt_push(io_stmt_cnt,'ASIS("if v_return_val = 1 then ")')
           ENDIF
           IF (mode_ndx=1)
            SET s_stmt_str = rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str[fs_ndx].add_str
           ELSEIF (mode_ndx=2)
            SET s_stmt_str = rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str[fs_ndx].upd_str
           ELSE
            SET s_stmt_str = rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_str[fs_ndx].del_str
           ENDIF
           SET s_line_length = 250
           SET s_start_pos = 0
           SET s_continue_ind = 1
           WHILE (s_continue_ind=1)
             IF (((s_start_pos+ s_line_length) > size(s_stmt_str)))
              CALL stmt_push(io_stmt_cnt,concat('ASIS("',trim(substring(s_start_pos,s_line_length,
                  s_stmt_str),3),'")'))
              SET s_continue_ind = 0
             ELSE
              SET s_space_pos = findstring(" ",substring(s_start_pos,(s_line_length+ 1),s_stmt_str),1,
               1)
              IF (s_space_pos=0)
               SET dm_err->emsg = "No space in the string, length was set to the s_line_length."
               CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
               GO TO exit_program
              ENDIF
              CALL stmt_push(io_stmt_cnt,concat('ASIS("',trim(substring(s_start_pos,s_space_pos,
                  s_stmt_str),3),'")'))
              SET s_start_pos = (s_start_pos+ s_space_pos)
             ENDIF
           ENDWHILE
           IF (((s_filter_type IN ("INCLUDE", "EXCLUDE")) OR (fs_ndx > 1)) )
            CALL stmt_push(io_stmt_cnt,'ASIS("  end if;")')
     ...
     CALL stmt_push(io_stmt_cnt,"end go")
     SET stat = alterlist(stmt_buffer->data,v_stmt_cnt)
     EXECUTE dm2_stmt_exe "FUNCTION", rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_function WITH replace(
      "REQUEST","STMT_BUFFER"), replace("REPLY","SE_REPLY")
     SET v_stmt_cnt = 0
     IF ((se_reply->err_ind=1))
      SET dm_err->emsg = "Error occurred in dm2_stmt_exe."
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE create_triggers_ora(i_dtd_ndx,io_stmt_cnt)
   DECLARE s_type = c3
   DECLARE s_event = c6
   DECLARE s_old_new = c5
   DECLARE s_del_ind_str = c1
   DECLARE s_trigger_name = vc
   DECLARE s_log_type = vc
   DECLARE s_pkw_type = vc
   DECLARE ic_idx = i4
   DECLARE ic_idx2 = i4
   DECLARE ic_string = vc
   DECLARE cto_lob_ind = i2
   DECLARE cto_del_ndx = i2
   DECLARE s_unm_chg = i2
   SET csa_tbl_cnt = 0
   SET csa_col_cnt = 0
   IF ((rs_dtd_reply->dtd_hold[i_dtd_ndx].mrg_ind=0)
    AND (rs_dtd_reply->dtd_hold[i_dtd_ndx].ref_ind=1)
    AND (rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name != "ACCOUNT"))
   ...
   FOR (type_ndx = 1 TO 3)
     ...
     ELSE
      ...
      SET s_del_ind_str = "1"
     ENDIF
     SET s_trigger_name = concat("REFCHG",rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_suffix,s_type)
     SELECT INTO "nl:"
      FROM user_triggers ut
      WHERE ut.trigger_name=concat(s_trigger_name,"$C")
       AND (ut.table_name=rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name)
      DETAIL
     ...
     SELECT INTO "nl:"
      ...
      WHERE di.info_domain="UNMAPPED CHANGES EXCLUDE"
       AND (di.info_name=rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name)
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     ...
     SET cto_lob_ind = 0
     SET dm_err->eproc = "Checking for *LOB,LONG and *RAW data type columns for current table."
     CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
     SELECT INTO "NL:"
      FROM user_tab_columns utc
      WHERE (utc.table_name=rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name)
     ...
     IF (s_event="UPDATE"
      ...
      SELECT INTO "NL:"
       ...
       (SELECT
        ...
        WHERE (drtc.table_name=rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name)
         AND drtc.active_ind=1)))
        AND  NOT (utc.column_name IN (
       (SELECT
        utc2.column_name
        FROM user_tab_columns utc2
        WHERE (utc2.table_name=rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name)
         AND ((utc2.column_name IN ("UPDT_ID", "UPDT_APPLCTX", "UPDT_CNT", "UPDT_TASK")
         AND ((utc2.data_type="NUMBER") OR (utc2.data_type="FLOAT")) ) OR (utc2.column_name=
        "UPDT_DT_TM"
         AND utc2.data_type="DATE")) )))) OR (utc.column_name IN (
       (SELECT
        di.info_name
        FROM dm_info di
        WHERE di.info_domain=concat("RDDS UPDATE OF OVERRIDE::",rs_dtd_reply->dtd_hold[i_dtd_ndx].
         tbl_name)))))
       HEAD REPORT
        ic_string = "if (NVL(SYS_CONTEXT('CERNER','FIRE_REFCHG_TRG_MVR'),'YES')!='YES' or "
       DETAIL
        ic_string = concat(ic_string," :new.",trim(utc.column_name)," != :old.",trim(utc.column_name),
         " or ")
       FOOT REPORT
        ic_string = replace(ic_string,"or"," ",2), ic_string = concat(ic_string," ) then")
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     ...
     CALL stmt_push(io_stmt_cnt,concat('ASIS(" ',rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name,
       ' for each row")'))
     IF (ac_log_type="REFCHG")
     ...
     ENDIF
     IF ((rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_function > " "))
      CALL stmt_push(io_stmt_cnt,concat('ASIS(" if ',
        " NVL(SYS_CONTEXT('CERNER','FIRE_REFCHG_TRG_MVR'),'YES')!='YES' "," OR ",rs_dtd_reply->
        dtd_hold[i_dtd_ndx].filter_function,' (")'))
      FOR (fp_ndx = 1 TO size(rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_parm,5))
       IF (fp_ndx=1)
        CALL stmt_push(io_stmt_cnt,concat(^ASIS(" '^,s_type,^' ")^))
       ENDIF
       CALL stmt_push(io_stmt_cnt,concat('ASIS(",:new.',rs_dtd_reply->dtd_hold[i_dtd_ndx].
         filter_parm[fp_ndx].column_name,",:old.",rs_dtd_reply->dtd_hold[i_dtd_ndx].filter_parm[
         fp_ndx].column_name,'")'))
      ENDFOR
     ...
     ENDIF
     CALL stmt_push(io_stmt_cnt,concat('ASIS(" v_pk_where := ',rs_dtd_reply->dtd_hold[i_dtd_ndx].
       pkw_function,"('",s_pkw_type,^' ")^))
     FOR (pk_ndx = 1 TO size(rs_dtd_reply->dtd_hold[i_dtd_ndx].pk_where_parm,5))
       CALL stmt_push(io_stmt_cnt,concat('ASIS(",',s_old_new,rs_dtd_reply->dtd_hold[i_dtd_ndx].
     ...
     SET updt_ndx = locateval(updt_ndx,1,dm2_cl_trg_updt_cols->tbl_cnt,rs_dtd_reply->dtd_hold[
      i_dtd_ndx].tbl_name,dm2_cl_trg_updt_cols->qual[updt_ndx].tname)
     CALL stmt_push(io_stmt_cnt,concat(^ASIS(" proc_refchg_ins_log('^,rs_dtd_reply->dtd_hold[
       i_dtd_ndx].tbl_name,"',v_pk_where,dbms_utility.get_hash_value(v_pk_where, 0, 1073741824),'",
       s_log_type,"',",
       s_del_ind_str,",",evaluate(s_pkw_type,"DELETE","0",evaluate(dm2_cl_trg_updt_cols->qual[
        ...
        dm2_cl_trg_updt_cols->qual[updt_ndx].updt_task_exist_ind,1,build(s_old_new,"updt_task"),0,"0"
        ),
       ",",evaluate(dm2_cl_trg_updt_cols->qual[updt_ndx].updt_applctx_exist_ind,1,build(s_old_new,
         "updt_applctx"),0,"0"),",NULL",",NULL,",trim(cnvtstring(rs_dtd_reply->dtd_hold[i_dtd_ndx].
         pkw_vers_id)),
       '.0);")'))
     IF (s_type="UPD")
      SET s_del_ind_str = "1"
      CALL stmt_push(io_stmt_cnt,concat('ASIS(" if ")'))
      FOR (pk_loop_cnt = 1 TO rs_dtd_reply->dtd_hold[dtd_ndx].pk_cnt)
       SET cio_pseudo_column = rs_dtd_reply->dtd_hold[dtd_ndx].pk_hold[pk_loop_cnt].pk_name
       IF ((pk_loop_cnt != rs_dtd_reply->dtd_hold[dtd_ndx].pk_cnt))
        CALL stmt_push(io_stmt_cnt,concat('ASIS(" (:new.',cio_pseudo_column," != :old.",
          cio_pseudo_column,") or ",
          '")'))
       ELSE
        CALL stmt_push(io_stmt_cnt,concat('ASIS(" (:new.',cio_pseudo_column," != :old.",
          cio_pseudo_column,' ) then ")'))
       ENDIF
      ENDFOR
      CALL stmt_push(io_stmt_cnt,concat('ASIS(" v_pk_where := ',rs_dtd_reply->dtd_hold[i_dtd_ndx].
        pkw_function,^('DELETE'")^))
      FOR (pk_ndx = 1 TO size(rs_dtd_reply->dtd_hold[i_dtd_ndx].pk_where_parm,5))
        CALL stmt_push(io_stmt_cnt,concat('ASIS(", :old.',rs_dtd_reply->dtd_hold[i_dtd_ndx].
          pk_where_parm[pk_ndx].column_name,' ")'))
      ENDFOR
      CALL stmt_push(io_stmt_cnt,'ASIS(" ); ")')
      SET updt_ndx = 0
      ...
      CALL stmt_push(io_stmt_cnt,concat(^ASIS(" proc_refchg_ins_log('^,rs_dtd_reply->dtd_hold[
        ...
        ",",evaluate(dm2_cl_trg_updt_cols->qual[updt_ndx].updt_applctx_exist_ind,1,
         ":new.updt_applctx",0,"0"),",NULL",",NULL,",trim(cnvtstring(rs_dtd_reply->dtd_hold[i_dtd_ndx
          ].pkw_vers_id)),
        '.0);")'))
      CALL stmt_push(io_stmt_cnt,'ASIS(" end if; ")')
      SET s_del_ind_str = "0"
     ENDIF
     IF ((rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name="PRSNL")
      AND s_type="ADD")
      ...
      SET updt_ndx = locateval(updt_ndx,1,dm2_cl_trg_updt_cols->tbl_cnt,rs_dtd_reply->dtd_hold[
       i_dtd_ndx].tbl_name,dm2_cl_trg_updt_cols->qual[updt_ndx].tname)
      CALL stmt_push(io_stmt_cnt,concat(^ASIS(" proc_refchg_ins_log('PERSON',v_pk_where,^,
        ...
        ",NULL,",trim(cnvtstring(rs_dtd_reply->dtd_hold[i_dtd_ndx].pkw_vers_id)),'.0);")'))
     ENDIF
     IF ((daclt_ptam_env_list->ptam_on_ind=1))
      SET daclt_r_tab_name = cutover_tab_name(rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name,rs_dtd_reply
       ->dtd_hold[i_dtd_ndx].tbl_suffix)
      CALL stmt_push(io_stmt_cnt,
      ...
      IF (size(rs_dtd_reply->dtd_hold[i_dtd_ndx].del_hold,5) > 0)
       ...
       FOR (cto_del_ndx = 1 TO size(rs_dtd_reply->dtd_hold[i_dtd_ndx].pk_hold,5))
         IF (s_event IN ("INSERT", "UPDATE"))
          IF (cto_del_ndx=1)
           CALL stmt_push(io_stmt_cnt,concat('ASIS("and (( ',rs_dtd_reply->dtd_hold[i_dtd_ndx].
             pk_hold[cto_del_ndx].pk_name," ) = ('||:new.",rs_dtd_reply->dtd_hold[i_dtd_ndx].pk_hold[
             cto_del_ndx].pk_name,^||' ) ")^))
          ELSE
           CALL stmt_push(io_stmt_cnt,concat('ASIS("and ( ',rs_dtd_reply->dtd_hold[i_dtd_ndx].
             pk_hold[cto_del_ndx].pk_name," ) = ('||:new.",rs_dtd_reply->dtd_hold[i_dtd_ndx].pk_hold[
         ...
         ELSE
          IF (cto_del_ndx=1)
           CALL stmt_push(io_stmt_cnt,concat('ASIS("and (( ',rs_dtd_reply->dtd_hold[i_dtd_ndx].
             pk_hold[cto_del_ndx].pk_name," ) = ('||:old.",rs_dtd_reply->dtd_hold[i_dtd_ndx].pk_hold[
             cto_del_ndx].pk_name,^||' ) ")^))
          ELSE
           CALL stmt_push(io_stmt_cnt,concat('ASIS("and ( ',rs_dtd_reply->dtd_hold[i_dtd_ndx].
             pk_hold[cto_del_ndx].pk_name," ) = ('||:old.",rs_dtd_reply->dtd_hold[i_dtd_ndx].pk_hold[
       ...
       ENDFOR
       CALL stmt_push(io_stmt_cnt,^ASIS(")' ;")^)
       CALL stmt_push(io_stmt_cnt,'ASIS(" Execute immediate v_md_vers_update; ")')
      ENDIF
      IF (s_event="INSERT")
       CALL stmt_push(io_stmt_cnt,'ASIS(" v_rep_pk_where := dm_ccl_to_sql_str(v_pk_where); ")')
      ELSE
       CALL stmt_push(io_stmt_cnt,concat('ASIS(" v_del_pk_where := ',rs_dtd_reply->dtd_hold[i_dtd_ndx
         ].pkw_function,^('INS/UPD' ")^))
       FOR (pk_ndx = 1 TO size(rs_dtd_reply->dtd_hold[i_dtd_ndx].pk_where_parm,5))
         CALL stmt_push(io_stmt_cnt,concat('ASIS(",',s_old_new,rs_dtd_reply->dtd_hold[i_dtd_ndx].
           pk_where_parm[pk_ndx].column_name,' ")'))
       ENDFOR
       CALL stmt_push(io_stmt_cnt,'ASIS(" ); ")')
       CALL stmt_push(io_stmt_cnt,'ASIS(" v_rep_pk_where := dm_ccl_to_sql_str(v_del_pk_where); ")')
      ENDIF
      IF ((rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name="PRSNL"))
       CALL stmt_push(io_stmt_cnt,
        ^ASIS(" v_rep_pk_where := replace(v_rep_pk_where, 't4859','t0386'); ")^)
      ENDIF
      CALL stmt_push(io_stmt_cnt,concat(^ASIS(" v_sql_stmt := 'UPDATE ^,daclt_r_tab_name," T",
        rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_suffix," set rdds_status_flag = 9003 ' ||",
        " v_rep_pk_where ","|| ' and rdds_status_flag = 0 and rdds_source_env_id in( ",
        daclt_ptam_env_list->env_ids,^ )'; ")^))
      CALL stmt_push(io_stmt_cnt,'ASIS(" Execute immediate v_sql_stmt; ")')
      CALL stmt_push(io_stmt_cnt,'ASIS(" end if; ")')
      CALL stmt_push(io_stmt_cnt,'ASIS(" end if; ")')
     ENDIF
     IF (s_unm_chg=0)
      ...
      CALL stmt_push(io_stmt_cnt,concat(
        ^ASIS("  RAISE_APPLICATION_ERROR(-20203 ,'RDDS: User is not allowed to change RDDS^,
     ...
     IF ((se_reply->err_ind=1))
      ...
      GO TO exit_program
     ENDIF
     SET daclt_temp_tab = rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name
     SET daclt_tab_cnt = size(daclt_trig_info->qual,5)
     SET daclt_tab_found = locateval(daclt_num,1,daclt_tab_cnt,daclt_temp_tab,daclt_trig_info->qual[
      daclt_num].table_name)
     ...
     ENDIF
   ENDFOR
   SET daclt_sc_ndx = locateval(sc_rmv_ndx,1,daclt_sc_remove->trig_cnt,rs_dtd_reply->dtd_hold[
    i_dtd_ndx].tbl_name,daclt_sc_remove->qual[sc_rmv_ndx].tbl_name)
   IF (daclt_sc_ndx > 0)
    CALL parser(concat("RDB ASIS(^drop trigger ",daclt_sc_remove->qual[sc_rmv_ndx].trig_name," ^) go"
      ))
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     GO TO exit_program
   ...
   FOR (s_dtd_ndx = i_dtd_find TO i_upto_value)
     IF (((i_single_tab_ind=0) OR ((rs_dtd_reply->dtd_hold[s_dtd_ndx].process_ind=1))) )
      ...
      IF (s_found_ndx > 0)
       ...
       CALL stmt_push(io_stmt_cnt,concat(
         ^asis("when (NVL(SYS_CONTEXT('CERNER','FIRE_REFCHG_TRG'),'DM2NULLVAL')!='NO')")^))
 ...
 SUBROUTINE drop_old_functions(io_drop_cnt)
   FOR (drop_ndx = 1 TO io_drop_cnt)
     ...
     CALL dm2_push_cmd(concat("RDB ",drop_func->data[drop_ndx].stmt," go"),1)
     IF (check_error(concat("Build: Error in dropping FUNCTION ",drop_func->data[drop_ndx].stmt)))
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "Error occurred in drop_old_functions."
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_program
     ENDIF
   ENDFOR
 ...
 SUBROUTINE create_xlat_triggers(i_dtd_ndx,io_stmt_cnt)
   ...
   SET cxl_find_tbl_ndx = locateval(cxl_find_tbl_ndx,1,size(refchg_xlat_create->tbl,5),rs_dtd_reply->
    dtd_hold[i_dtd_ndx].tbl_name,refchg_xlat_create->tbl[cxl_find_tbl_ndx].tbl_name)
   SET cxl_table_name = rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_name
   SET cxl_trig_ins = concat("REFCHGXLAT",rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_suffix,"INS")
   SET cxl_trig_del = concat("REFCHGXLAT",rs_dtd_reply->dtd_hold[i_dtd_ndx].tbl_suffix,"DEL")
   IF (cxl_find_tbl_ndx > 0)
    IF ((rs_dtd_reply->dtd_hold[i_dtd_ndx].mrg_del_ind != 2))
     SET cxl_create_flg = 1
     SELECT INTO "NL:"
      FROM user_triggers ut
      WHERE ut.table_name=cxl_table_name
       AND ut.trigger_name IN (concat(cxl_trig_ins,"$C"), concat(cxl_trig_del,"$C"), cxl_trig_ins,
      cxl_trig_del)
      DETAIL
     ...
     IF (check_error(dm_err->eproc)=1)
      ...
      GO TO exit_program
     ENDIF
     IF (cxl_trigger_name_ins > ""
      AND cxl_trigger_name_ins_c > "")
      SET cxl_trig_ins = cxl_trigger_name_ins
      SET cxl_drop->trig_cnt = (cxl_drop->trig_cnt+ 1)
      SET stat = alterlist(cxl_drop->trig_qual,cxl_drop->trig_cnt)
      SET cxl_drop->trig_qual[cxl_drop->trig_cnt].drop_stmt = concat("rdb drop trigger ",
       cxl_trigger_name_ins_c," go")
     ELSEIF (cxl_trigger_name_ins > "")
      SET cxl_trig_ins = cxl_trigger_name_ins
     ELSEIF (cxl_trigger_name_ins_c > "")
      SET cxl_trig_ins = cxl_trigger_name_ins_c
     ENDIF
     IF (cxl_trigger_name_del > ""
      AND cxl_trigger_name_del_c > "")
      SET cxl_trig_del = cxl_trigger_name_del
      SET cxl_drop->trig_cnt = (cxl_drop->trig_cnt+ 1)
      SET stat = alterlist(cxl_drop->trig_qual,cxl_drop->trig_cnt)
      SET cxl_drop->trig_qual[cxl_drop->trig_cnt].drop_stmt = concat("rdb drop trigger ",
       cxl_trigger_name_del_c," go")
     ELSEIF (cxl_trigger_name_del > "")
      SET cxl_trig_del = cxl_trigger_name_del
     ELSEIF (cxl_trigger_name_del_c > "")
      SET cxl_trig_del = cxl_trigger_name_del_c
     ENDIF
     CALL stmt_push(io_stmt_cnt,concat('RDB asis("create or replace trigger ',cxl_trig_ins,' ")'))
     CALL stmt_push(io_stmt_cnt,'asis("after insert on")')
     CALL stmt_push(io_stmt_cnt,concat('asis(" ',refchg_xlat_create->tbl[cxl_find_tbl_ndx].tbl_name,
       ' for each row ")'))
     IF ((refchg_xlat_create->tbl[cxl_find_tbl_ndx].tbl_name="CODE_VALUE"))
      CALL stmt_push(io_stmt_cnt,'asis("when ( new.code_set != 93 )")')
     ENDIF
     CALL stmt_push(io_stmt_cnt,'asis("begin delete from dm_refchg_invalid_xlat")')
     CALL stmt_push(io_stmt_cnt,concat('asis("where parent_entity_id = :new.',refchg_xlat_create->
       tbl[cxl_find_tbl_ndx].column_name,' ")'))
     CALL stmt_push(io_stmt_cnt,concat(^asis("and parent_entity_name = '^,refchg_xlat_create->tbl[
       cxl_find_tbl_ndx].tbl_name,^' ;")^))
     CALL stmt_push(io_stmt_cnt,'asis("end;")')
     CALL stmt_push(io_stmt_cnt,"end go")
     CALL echo(concat("Creating trigger....",cxl_trigger_name_ins))
     SET stat = alterlist(stmt_buffer->data,io_stmt_cnt)
     EXECUTE dm2_stmt_exe "TRIGGER", cxl_trigger_name_ins WITH replace("REQUEST","STMT_BUFFER"),
     replace("REPLY","SE_REPLY")
     SET io_stmt_cnt = 0
     SET v_stmt_cnt = 0
     IF ((se_reply->err_ind=1))
      SET dm_err->emsg = "Error occurred in dm2_stmt_exe."
      ...
      GO TO exit_program
     ENDIF
     CALL stmt_push(io_stmt_cnt,concat('RDB asis("create or replace trigger ',cxl_trig_del,' ") '))
     CALL stmt_push(io_stmt_cnt,'asis("after delete on")')
     CALL stmt_push(io_stmt_cnt,concat('asis(" ',refchg_xlat_create->tbl[cxl_find_tbl_ndx].tbl_name,
       ' for each row")'))
     IF ((refchg_xlat_create->tbl[cxl_find_tbl_ndx].tbl_name="CODE_VALUE"))
      CALL stmt_push(io_stmt_cnt,'asis("when ( old.code_set != 93 )")')
     ENDIF
     CALL stmt_push(io_stmt_cnt,'asis("begin insert into dm_refchg_invalid_xlat")')
     ...
     CALL stmt_push(io_stmt_cnt,concat('asis("values(:old.',refchg_xlat_create->tbl[cxl_find_tbl_ndx]
       .column_name,",'",refchg_xlat_create->tbl[cxl_find_tbl_ndx].tbl_name,^',0,0,0,sysdate,0);")^))
     CALL stmt_push(io_stmt_cnt,'asis(" exception when DUP_VAL_ON_INDEX then null; ")')
     CALL stmt_push(io_stmt_cnt,'asis("end;")')
     CALL stmt_push(io_stmt_cnt,"end go")
     CALL echo(concat("Creating trigger....",cxl_trigger_name_del))
     SET stat = alterlist(stmt_buffer->data,io_stmt_cnt)
     EXECUTE dm2_stmt_exe "TRIGGER", cxl_trigger_name_del WITH replace("REQUEST","STMT_BUFFER"),
     replace("REPLY","SE_REPLY")
     SET io_stmt_cnt = 0
     SET v_stmt_cnt = 0
     IF ((se_reply->err_ind=1))
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE refchg_xlat_gather(null)
   ...
   SELECT INTO "NL:"
    ...
    WHERE drcd.root_entity_name=drcd.table_name
...
#exit_program
 ...
 FOR (cl_id_cnt = 1 TO envid_cnt)
   IF (cl_id_cnt=1)
    SET env_csv = cnvtstring(derg_reply->child_env_list[cl_id_cnt].env_id)
   ELSE
    SET env_csv = concat(env_csv,",",cnvtstring(derg_reply->child_env_list[cl_id_cnt].env_id))
   ENDIF
 ENDFOR
 IF (size(dguc_request->req_special,5) > 0)
  IF (ac_log_type="REFCHG")
   SET total_trigger_cnt = (total_trigger_cnt+ (size(dguc_request->req_special,5) * 3))
  ENDIF
 ENDIF
 SET dm_err->eproc = concat("Logging Trigger History,.")
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 INSERT  FROM dm_chg_log d
  SET d.log_id = seq(dm_clinical_seq,nextval), d.target_env_id = cl_temp_envid, d.log_type = "ADDTRG",
   d.table_name = ac_log_type, d.pk_where = env_csv, d.chg_dt_tm = cnvtdatetime(add_trig_date),
   d.updt_task = rs_dtd_reply->rs_tbl_cnt, d.updt_cnt = total_trigger_cnt
  WITH nocounter
 ;end insert
 ...
 FOR (i = 1 TO size(derg_reply->child_env_list,5))
   ...
   FOR (j = 1 TO size(daclt_trig_info->qual,5))
    ...
    table_name
    SET auto_ver_request->qual[i].detail_qual[j].event_value = daclt_trig_info->qual[j].trigger_count
   ENDFOR
 ENDFOR
 EXECUTE dm_rmc_auto_verify_setup
 IF ((auto_ver_reply->status="F"))
