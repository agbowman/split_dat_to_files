CREATE PROGRAM bed_get_mltm_synonyms:dba
 FREE SET reply
 RECORD reply(
   1 orderables[*]
     2 code_value = f8
 ...
 IF (tot_cnt > 0)
  ...
  UPDATE  FROM order_catalog_synonym ocs,
   ...
   SET ocs.cki = temp_ords->ords[d.seq].mul_cki, ocs.concept_cki = temp_ords->ords[d.seq].
    mul_concept_cki, ocs.ref_text_mask = 64,
   ...
   PLAN (d
    WHERE (temp_ords->ords[d.seq].synonym_id > 0))
    JOIN (ocs
    WHERE (ocs.synonym_id=temp_ords->ords[d.seq].synonym_id))
   WITH nocounter
  ;end update
  SET ierrcode = error(serrmsg,1)
  IF (ierrcode > 0)
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   cnt = 0, tot_cnt = 0, stat = alterlist(temp_reply->orderables,200),
   match_ind = 1
  HEAD m.catalog_cki
   ...
   ENDIF
   temp_reply->orderables[cnt].cki = m.catalog_cki, temp_reply->orderables[cnt].concept_cki = m
   .catalog_concept_cki, temp_reply->orderables[cnt].description = m.description,
   stat = alterlist(temp_reply->orderables[cnt].synonyms,200), match_ind = 0
  DETAIL
