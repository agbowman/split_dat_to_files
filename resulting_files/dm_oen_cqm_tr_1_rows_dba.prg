CREATE PROGRAM dm_oen_cqm_tr_1_rows:dba
 ...
 END ;Subroutine
 DECLARE days_to_keep = i4 WITH protect, noconstant(360.0)
 FREE RECORD temp_oen_reply
 RECORD temp_oen_reply(
   1 status = c1
   1 errcode = i4
   1 errmsg = vc
 )
 IF ((request->purge_flag IN (c_del_high_log, c_del_dtl_log)))
  EXECUTE oen_tx_stat_by_hour
  IF ((temp_oen_reply->status="F"))
   SET reply->err_code = temp_oen_reply->errcode
   SET reply->err_msg = temp_oen_reply->errmsg
   SET reply->status_data.status = temp_oen_reply->status
   GO TO the_end
  ENDIF
 ENDIF
 ...
 SET days_to_keep = - (1)
 DECLARE hours_to_keep = i4 WITH protect, noconstant(- (1))
 DECLARE hours_old = i4 WITH protect, noconstant(0)
 ...
 FOR (tok_ndx = 1 TO token_size)
   ...
   ELSEIF ((request->tokens[tok_ndx].token_str="HRSTOKEEP"))
    SET hours_to_keep = ceil(cnvtreal(request->tokens[tok_ndx].value))
   ENDIF
 ENDFOR
 IF (days_to_keep < 0)
 ...
 SET hours_old = (((hours_to_keep * 60) * 60) * 100)
  ...
  AND rowsleft > 0)
   SELECT INTO "nl:"
    ...
    WHERE parser(sbr_getrowidnotexists("t.trigger_id between curMinID and curMaxID","t"))
     AND t.queue_id > 0
     AND  NOT (t.process_status_flag IN (10, 1100, 1170))
     AND t.create_dt_tm < cnvtdatetime((curdate - days_to_keep),(curtime3 - hours_old))
    DETAIL
     rows += 1
     IF (mod(rows,10)=1)
   ...
   IF ((reply->err_code > 0))
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s3","Failed in collection of rows: %1","s",
