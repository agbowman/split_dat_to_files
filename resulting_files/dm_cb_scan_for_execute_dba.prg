CREATE PROGRAM dm_cb_scan_for_execute:dba
 DECLARE scan_ques_in = i4
 DECLARE scan_call_prog = c30
 DECLARE action_hold = c132
 DECLARE scan_ques_cnt = i4
 DECLARE scan_ques_ndx = i4
 DECLARE exe_ans_hold = i4
 DECLARE exe_status_hold = c20
 DECLARE exe_hist_insert(null) = null WITH public
 DECLARE inhouse_ind2 = i2
 DECLARE dcse_ebookings_ind = i2
 DECLARE dcse_rhio_ind = i2
 SET dcse_ebookings_ind = 0
 SET inhouse_ind2 = 0
 SET dcse_rhio_ind = 0
 SELECT INTO "nl:"
  FROM dm_info
 ...
 ;end select
 IF (inhouse_ind2=1)
  CALL echo("Not executed since we are in inhouse domain.")
  GO TO exit_script
 ENDIF
 SET scan_ques_in =  $1
 SET scan_call_prog =  $2
 SET scan_ques_cnt = 0
 SET scan_ques_ndx = 0
 ...
 IF (dcse_rhio_ind=0
  AND scan_ques_in=9)
  SET hold_success_status = 1
  IF ( NOT (validate(execute_successful,- (1)) < 0))
   SET execute_successful = hold_success_status
  ENDIF
  CALL echo("Not executed since not RHIO Domain.")
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info
  ...
  DETAIL
   dcse_ebookings_ind = 1
  WITH nocounter
 ;end select
 IF (dcse_ebookings_ind=0
  AND scan_ques_in=8)
  SET hold_success_status = 1
  IF ( NOT (validate(execute_successful,- (1)) < 0))
   SET execute_successful = hold_success_status
  ENDIF
  CALL echo("Not executed since not Ebookings Domain.")
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  FROM dm_cb_questions q,
   dm_cb_answers a
  PLAN (q
   WHERE q.question_nbr=scan_ques_in
    AND q.active_ind=1)
   JOIN (a
   WHERE a.question_nbr=q.question_nbr
    AND a.answer_status="SELECTED"
    AND a.active_ind=1)
  DETAIL
   action_hold = a.action
  WITH nocounter
 ;end select
 CALL dcm_get_questions(cnvtstring(scan_ques_in))
 CALL dcm_get_answers(cnvtstring(scan_ques_in))
 CALL dcm_get_objects(cnvtstring(scan_ques_in))
 CALL dcm_get_requests(cnvtstring(scan_ques_in))
 SET action_hold = trim(action_hold)
 CALL execute_actions(action_hold)
 FOR (scan_ques_cnt = 1 TO size(dccs_master->question,5))
   IF ((dccs_master->question[scan_ques_cnt].question_nbr=scan_ques_in))
    SET scan_ques_ndx = scan_ques_cnt
    SET scan_ques_cnt = size(dccs_master->question,5)
   ENDIF
 ENDFOR
 FOR (ans_cnt = 1 TO size(dccs_master->question[scan_ques_ndx].answer,5))
   IF ((dccs_master->question[scan_ques_ndx].answer[ans_cnt].answer_status_db="SELECTED"))
    SET exe_ans_hold = dccs_master->question[scan_ques_ndx].answer[ans_cnt].answer_nbr
    SET exe_status_hold = dccs_master->question[scan_ques_ndx].answer[ans_cnt].action_status
   ENDIF
 ENDFOR
 IF (findstring("BUILD",action_hold)=0)
  CALL dcm_process_operations(scan_ques_ndx,"Y")
  CALL dcm_check_op_success(0,scan_ques_ndx)
  IF (dcm_error_operation=0)
   UPDATE  FROM dm_cb_answers a
    SET a.action_status = "COMPLETE", a.updt_applctx = 999, a.updt_cnt = (a.updt_cnt+ 1),
     a.updt_dt_tm = cnvtdatetime(curdate,curtime3), a.updt_task = 9
    WHERE a.question_nbr=scan_ques_in
     AND a.answer_status="SELECTED"
     AND a.active_ind=1
    WITH nocounter
   ;end update
   CALL exe_hist_insert(null)
   CALL dcm_postop_updates(scan_ques_ndx,"Y")
   SET hold_success_status = 1
  ELSE
   CALL dcm_create_error_log("DM_CB_SCAN_FOR_EXECUTE",1)
   CALL echo("*")
   CALL echo("*")
   CALL echo("*")
   CALL echo("**********************************************************")
   CALL echo("**********************************************************")
   CALL echo(concat("Error occurred performing actions for DM_CB_QUESTIONS.QUESTION_NBR:",trim(
      cnvtstring(scan_ques_in)),". View log_file:",dccs_master->dccs_logfile))
   CALL echo("**********************************************************")
   CALL echo("**********************************************************")
   CALL echo("*")
   CALL echo("*")
   CALL echo("*")
   SET hold_success_status = 0
  ENDIF
 ELSE
  CALL dcm_postop_updates(scan_ques_ndx,"Y")
  UPDATE  FROM dm_cb_answers a
   SET a.action_status = "COMPLETE", a.updt_applctx = 998, a.updt_cnt = (a.updt_cnt+ 1),
    a.updt_dt_tm = cnvtdatetime(curdate,curtime3), a.updt_task = 9
   WHERE a.question_nbr=scan_ques_in
    AND a.answer_status="SELECTED"
    AND a.active_ind=1
  ...
  SET hold_success_status = 1
 ENDIF
 SUBROUTINE exe_hist_insert(null)
   INSERT  FROM dm_cb_history
    SET history_id = seq(dm_clinical_seq,nextval), table_name = "DM_CB_ANSWERS", column_name =
     "ACTION_STATUS",
     change_dt_tm = cnvtdatetime(curdate,curtime3), old_value_txt = exe_status_hold, new_value_txt =
     "COMPLETE",
     change_reason = "CSV LOAD", change_process = "DM_DBIMPORT", question_nbr = scan_ques_in,
     answer_nbr = exe_ans_hold
    WITH nocounter
   ;end insert
 END ;Subroutine
 IF ( NOT (validate(execute_successful,- (1)) < 0))
  SET execute_successful = hold_success_status
 ENDIF
 IF ( NOT (validate(error_logfile,"x")="x"))
  SET error_logfile = dccs_master->dccs_logfile
 ENDIF
#exit_script
END GO
