CREATE PROGRAM cs_test
 ...
 SET startdate =  $ST_DT
 SET enddate =  $END_DT
 SET st_date = concat(startdate," 00:00:00.00")
 SET end_date = concat(enddate," 23:59:59.99")
 SET prt_st_date = concat(startdate," 00:00")
 SET prt_end_date = concat(enddate," 23:59")
 SET echo_ind = 1
 ...
 IF (echo_ind)
  CALL echo(build("startdate =",startdate))
  CALL echo(build("enddate =",enddate))
  CALL echo(build("st_dt =", $ST_DT))
  CALL echo(build("end_dt =", $END_DT))
  CALL echo(build("act_type_cd =",act_type_cd))
 ...
 SELECT INTO  $OUTDEV
  ...
  cat_disp = uar_get_code_display(o.catalog_cd), p.name_full_formatted, e.loc_facility_cd,
  e_loc_facility_cd = uar_get_code_description(e.loc_facility_cd), e.loc_nurse_unit_cd,
  e_loc_nurse_unit_cd = uar_get_code_display(e.loc_nurse_unit_cd),
  e.loc_room_cd, e_loc_room_cd = uar_get_code_display(e.loc_room_cd), e.loc_bed_cd,
  e_loc_bed_cd = uar_get_code_display(e.loc_bed_cd), rm_bd = concat(trim(uar_get_code_display(e
     .loc_room_cd)),"/",trim(uar_get_code_display(e.loc_bed_cd))), fin = substring(1,10,ea.alias),
  mrn = substring(1,10,ea1.alias)
  FROM orders o,
   person p,
   encounter e,
   encntr_alias ea,
  ...
  PLAN (o
   WHERE o.active_ind=1
    ...
    AND o.orig_order_dt_tm BETWEEN cnvtdatetime(st_date) AND cnvtdatetime(end_date))
   JOIN (p
  ...
  HEAD PAGE
   ...
   font1, row + 1, col 01,
   CALL print(calcpos(197,20)), "{b/23}VITALIZE HEALTH SYSTEMS", font3,
   row + 1, col 01,
   ...
   CALL print(calcpos(ct_xcol,y_col)), "Summary From ", prt_st_date,
   " To ", prt_end_date, row + 1,
   CALL print(calcpos(rt_xcol,y_col)), "{b/5}Page: ", curpage"###",
   row + 1, r_cnt = 16
  ...
  HEAD rm_bd
   IF (rm_bd=last_rm_bd)
    ln_feed, ln_feed, pt_name = substring(1,40,last_pt_name),
    col 005, "{b/8}Patient: ", col 023,
    "Test, Patient1", col 060, "{b/4}MRN: 99999999",
    col 080, "{b/4}FIN: 999999999", rm_prtd = 1,
    ln_feed, col 005, "{b/9}Room/Bed: ",
    col 023, last_rm_bd
   ELSE
    IF (r_cnt >= 50)
    ...
    col 005, "{b/8}Patient: ", col 023,
    "Test, Patient1", col 060, "{b/4}MRN: 99999999",
    col 080, "{b/4}FIN: 99999999", ln_feed,
    col 005, "{b/9}Room/Bed: ", col 023,
    rm_bd
   ENDIF
  DETAIL
   nur_rests = (nur_rests+ 1), fac_rests = (fac_rests+ 1), org_rests = (org_rests+ 1),
   ln_feed, ln_feed, mnem = trim(o.ordered_as_mnemonic),
   col 005, "{b/18}Restraint Ordered: ", mnem,
   col 061, "{b/18}Date/Time Ordered: ", o.orig_order_dt_tm"mm/dd/yy hh:mm;;q",
   init_wrap
  ...
  FOOT  e.loc_facility_cd
   ln_feed, row r_cnt, fac_line = concat("{b}","****  End of Facility - Vitalize  ****","{endb}"),
  ...
  FOOT REPORT
   ...
   "Grand Total For Vitalize Health System = ", org_rests"###"
  WITH check, dio = postscript, maxrow = 70,
