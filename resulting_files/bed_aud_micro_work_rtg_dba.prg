CREATE PROGRAM bed_aud_micro_work_rtg:dba
 ...
 DECLARE micro_cd = f8 WITH protect, noconstant(0.0)
  ...
  SELECT INTO "nl:"
   ...
   PLAN (oc
    WHERE oc.catalog_type_cd=genlab
     AND oc.activity_type_cd=micro_cd
     AND oc.active_ind=1)
   DETAIL
 ...
 SELECT INTO "nl:"
  ...
  PLAN (o
   WHERE o.catalog_type_cd=genlab
    AND o.activity_type_cd=micro_cd
    AND o.active_ind=1
 ...
 SET reply->collist[3].header_text = "Label Description"
 ...
 FOR (x = 1 TO temp->o_cnt)
   IF ((temp->olist[x].r_cnt=0))
    ...
    SET reply->rowlist[row_nbr].celllist[3].string_value = temp->olist[x].dept_display_name
    ...
    IF ((temp->olist[x].resource_route_lvl=2))
     SET reply->rowlist[row_nbr].celllist[4].string_value = "Assay Level"
    ENDIF
   ELSE
    FOR (y = 1 TO temp->olist[x].r_cnt)
      ...
      SET reply->rowlist[row_nbr].celllist[7].string_value = temp->olist[x].activity_subtype_disp
      SET reply->rowlist[row_nbr].celllist[8].double_value = temp->olist[x].catalog_cd
      SET reply->rowlist[row_nbr].celllist[4].string_value = temp->olist[x].rlist[y].
      service_resource_disp
      SET reply->rowlist[row_nbr].celllist[6].string_value = cnvtstring(temp->olist[x].rlist[y].
       sequence)
...
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("micro_work_rtg_audit.csv")
 ENDIF
