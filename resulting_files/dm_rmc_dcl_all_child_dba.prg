CREATE PROGRAM dm_rmc_dcl_all_child:dba
 ...
 SUBROUTINE drdc_file_success(dfs_name,dfs_file_name,dfs_error_ind)
   ...
   RETURN(null)
 END ;Subroutine
 DECLARE dclma_loop = i4 WITH protect, noconstant(0)
 DECLARE dclma_idx = i4 WITH protect, noconstant(0)
 DECLARE dclma_lt = vc WITH protect, noconstant("")
 IF ((validate(dclma_menu_ind,- (1))=- (1)))
  DECLARE dclma_menu_ind = i2 WITH protect, noconstant(0)
 ENDIF
 FREE RECORD dclma_parse
 RECORD dclma_parse(
   1 stmt[*]
     2 str = vc
 )
 SET dm_err->eproc = "Starting dm_rmc_dcl_all_child"
 IF (check_logfile("dm_rmc_dcl_mon",".log","DM_RMC_DCL_MON LOG")=0)
  ...
  GO TO exit_mon_all
 ENDIF
 SET stat = alterlist(dclma_parse->stmt,31)
 IF ((dclm_all->cur_id=0.0))
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    dclm_all->cur_id = di.info_number
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF ((dclm_all->src_id=0.0))
  ...
  SELECT INTO "NL:"
   ...
   (SELECT
    ...
    WHERE (d1.cur_environment_id=dclm_all->cur_id)
     AND d1.rdds_event_key="ENDREFERENCEDATASYNC")))
   DETAIL
    dclm_all->src_id = d.paired_environment_id, dclm_all->oe_name = concat(trim(d.event_reason),"(",
  ...
  IF (curqual=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("No open event detected for target environment ",dclm_all->cur_name,
    ". The monitoring report will not run.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_mon_all
  ENDIF
 ENDIF
 IF ((((dclm_all->src_name <= " ")) OR ((dclm_all->cur_name <= " "))) )
  SET dm_err->eproc = "Getting getting source environment name."
  SELECT INTO "NL:"
   FROM dm_environment d
   WHERE d.environment_id IN (dclm_all->src_id, dclm_all->cur_id)
   DETAIL
    IF ((d.environment_id=dclm_all->cur_id))
     dclm_all->cur_name = d.environment_name
    ELSE
     dclm_all->src_name = d.environment_name
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
 ...
 IF ((dclm_all->db_link <= " "))
  ...
  SELECT INTO "NL:"
   FROM dm_env_reltn d
   WHERE (d.child_env_id=dclm_all->cur_id)
    AND (d.parent_env_id=dclm_all->src_id)
    AND d.relationship_type="REFERENCE MERGE"
    AND d.post_link_name > " "
    AND d.post_link_name IS NOT null
   DETAIL
    dclm_all->db_link = d.post_link_name
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  IF (curqual=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("No database link found to the source environment ",dclm_all->src_name,
    ". The monitoring report will not run.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_mon_all
  ENDIF
 ENDIF
 IF ((dclm_all->audit_cnt=0))
  ...
  SELECT INTO "NL:"
   ...
   WHERE d.info_domain="RDDS CONFIGURATION:AUDITABLE NOMV"
    AND d.info_number=1
   DETAIL
    dclm_all->audit_cnt = (dclm_all->audit_cnt+ 1), stat = alterlist(dclm_all->audit_qual,dclm_all->
     audit_cnt), dclm_all->audit_qual[dclm_all->audit_cnt].log_type = d.info_name
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 IF ((dclm_all->max_lines=0))
  SET dclm_all->max_lines = 15
 ENDIF
 SET dclm_all->cnt = 0
 SET stat = alterlist(dclm_all->qual,0)
 SET dclm_all->no_qual_msg = ""
 SET dclm_all->refresh_time = concat("As of ",format(cnvtdatetime(curdate,curtime3),
 ...
 ENDIF
 SET dclma_parse->stmt[1].str = concat(
  "select into 'nl:' r.log_type, r.table_name, amount = count(*), ",
  " ctxt = nullval(r.context_name,'<NULL>')")
 IF ((dclm_all->db_link="NONE"))
  SET dclma_parse->stmt[2].str = "from DM_CHG_LOG r"
 ELSE
  SET dclma_parse->stmt[2].str = concat("from DM_CHG_LOG",dclm_all->db_link," r")
 ENDIF
 SET dclma_parse->stmt[3].str = concat("where r.target_env_id = ",trim(cnvtstring(dclm_all->cur_id,20,
    1)))
 SET dclma_parse->stmt[4].str =
 " group by r.log_type,r.table_name,r.context_name order by r.log_type,r.context_name, r.table_Name"
 SET dclma_parse->stmt[5].str = "head r.log_type "
 SET dclma_parse->stmt[6].str = " if (r.log_type = 'NOMV*')"
 SET dclma_parse->stmt[7].str = concat(
  " if (locateval(dclma_loop, 1, dclm_all->audit_cnt, r.log_type, ",
  "dclm_all->audit_qual[dclma_loop].log_type) > 0)")
 SET dclma_parse->stmt[8].str = " dclma_lt = 'INV NOMV*'"
 SET dclma_parse->stmt[9].str = " else"
 SET dclma_parse->stmt[10].str = " dclma_lt = 'NOMV*'"
 SET dclma_parse->stmt[11].str = " endif"
 SET dclma_parse->stmt[12].str = " elseif (r.log_type = 'CHLD*')"
 SET dclma_parse->stmt[13].str = " dclma_lt = 'CHLD*'"
 SET dclma_parse->stmt[14].str =
 " elseif (r.log_type in ('ORPHAN', 'OLDVER','REFCHG','MERGED','NORDDS','PROCES'))"
 SET dclma_parse->stmt[15].str = " dclma_lt = r.log_Type"
 SET dclma_parse->stmt[16].str = " else"
 SET dclma_parse->stmt[17].str = " dclma_lt = 'OTHER'"
 SET dclma_parse->stmt[18].str = " endif"
 SET dclma_parse->stmt[19].str = concat(
  " dclma_idx = locateval(dclma_loop,1,dclm_all->type_cnt, dclma_lt, ",
  "dclm_all->lt_qual[dclma_loop].log_type)")
 SET dclma_parse->stmt[20].str = concat(
  "if (dclma_idx = 0) dclma_idx = dclm_all->type_cnt + 1 dclm_all->type_cnt = ",
  " dclm_all->type_cnt + 1 stat = alterlist(dclm_all->lt_qual, dclma_idx)")
 SET dclma_parse->stmt[21].str = " dclm_all->lt_qual[dclma_idx].log_Type = dclma_lt"
 SET dclma_parse->stmt[22].str = " endif"
 SET dclma_parse->stmt[23].str = " detail dclm_all->cnt = dclm_all->cnt + 1 "
 SET dclma_parse->stmt[24].str =
 " if(mod(dclm_all->cnt, 10) = 1) stat = alterlist(dclm_all->qual, dclm_all->cnt + 9)"
 SET dclma_parse->stmt[25].str = " endif dclm_all->qual[dclm_all->cnt]->table_name = r.table_name"
 SET dclma_parse->stmt[26].str = " dclm_all->qual[dclm_all->cnt]->log_type  = r.log_type"
 SET dclma_parse->stmt[27].str = " dclm_all->qual[dclm_all->cnt]->row_cnt = amount"
 SET dclma_parse->stmt[28].str =
 " dclm_all->qual[dclm_all->cnt]->context_name = substring(1,25,ctxt)"
 SET dclma_parse->stmt[29].str =
 "if (dclma_lt = 'INV NOMV*') dclm_all->qual[dclm_all->cnt].nomv_ind = 1 endif"
 SET dclma_parse->stmt[30].str = "foot r.log_type dclm_all->lt_qual[dclma_idx].lt_cnt = sum(amount)"
 SET dclma_parse->stmt[31].str = " with nocounter go"
 EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DCLMA_PARSE")
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_mon_all
 ENDIF
 SET stat = alterlist(dclm_all->qual,dclm_all->cnt)
 IF ((dclm_all->cnt=0))
  SET dclm_all->no_qual_msg =
  "There are no rows in the source change log for the current target environment."
 ENDIF
#exit_mon_all
END GO
