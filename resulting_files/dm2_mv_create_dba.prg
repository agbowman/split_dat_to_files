CREATE PROGRAM dm2_mv_create:dba
 ...
 SUBROUTINE dmvr_drop_mlog(ddm_bt_name)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmc_mv_name = vc WITH protect, noconstant(" ")
 DECLARE dmc_mv_instance = i2 WITH protect, noconstant(- (1))
 DECLARE dmc_exists_ind = i2 WITH protect, noconstant(0)
 DECLARE dmc_syn_exists_ind = i2 WITH protect, noconstant(0)
 DECLARE dmc_syn_ref_name = vc WITH protect, noconstant(" ")
 DECLARE dmc_iter = i2 WITH protect, noconstant(0)
 DECLARE dmc_build_allowed = i2 WITH protect, noconstant(1)
 DECLARE dmc_mv_status = vc WITH protect, noconstant(" ")
 DECLARE dmc_di_exists_ind = i2 WITH protect, noconstant(1)
 IF (check_logfile("dm2_mv_create",".log","dm2_mv_create LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validating input parameter data"
 CALL disp_msg("",dm_err->logfile,dm_err->err_ind)
 SET dmc_mv_name = trim(cnvtupper( $1))
 SET dmc_instance = cnvtint( $2)
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->emsg = "Parameter usage: dm2_mv_create '<Materialzied View Name>, <Instance>'"
  GO TO exit_program
 ENDIF
 IF (dm2_table_and_ccldef_exists("DM_INFO",dmc_di_exists_ind)=0)
  GO TO exit_program
 ENDIF
 IF (dmc_di_exists_ind=1)
  SET dm_err->eproc = "Load MVIEW Active Swith - DB Level"
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM dm_info di
   ...
   DETAIL
    dmc_build_allowed = di.info_number
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (dmc_build_allowed=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Materialized Views disabled at database level.  Creation request denied."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Retrieving Admin Metadata for MVIEW ",dmc_mv_name)
 CALL disp_msg("",dm_err->logfile,dm_err->err_ind)
 IF (dmvr_get_mv_metadata(dmc_mv_name,dmc_instance)=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(mv)
 ...
 IF (trim(mv->name)="")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Metadata not found to build this object. Materialized View not created."
  SET dm_err->user_action = "Please resolve missing metadata and restart the process."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF ((dm2_rdbms_version->level1 < 9))
  IF (dmvr_create_mv_cview(null)=0)
   GO TO exit_program
  ENDIF
  IF (dmvr_create_mv_syn(mv->syn_name,mv->cview_name)=0)
   GO TO exit_program
  ENDIF
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Checking MVIEW status",dmc_mv_name)
 CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
 SELECT INTO "nl:"
  FROM dm_info a
  WHERE a.info_domain="DM2_MVIEW_INSTANCE"
   AND a.info_name=cnvtupper(mv->name)
   AND a.info_char="DISABLED"
  DETAIL
   dmc_build_allowed = 0, dmc_mv_status = a.info_char
  WITH nocounter
 ;end select
 IF (check_error(build("Checking if MVIEW (Re)Creation is allowed"))=1)
  GO TO exit_program
 ELSEIF (dmc_build_allowed=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("MVIEW in unbuildable status (",dmc_mv_status,
   ") - bypassing Materialized View creation.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (dmvr_updt_mv_inst("IU",mv->name,1,"MVIEWBUILD")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Checking existence for MVIEW ",mv->name)
 CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
 IF (dmvr_get_mv(mv->name,dmc_exists_ind)=0)
  GO TO exit_program
 ENDIF
 SET mv->exists_ind = dmc_exists_ind
 IF (dmvr_create_mv_cview(null)=0)
  GO TO exit_program
 ENDIF
 IF (dmvr_create_mv_syn(mv->syn_name,mv->cview_name)=0)
 ...
 IF ((mv->mlog_cnt < 1))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Base tables not found for MVIEW (",dmc_mv_name,")")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Creating MVIEW Logs if needed for MVIEW: ",dmc_mv_name)
 CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
 FOR (dmc_iter = 1 TO mv->mlog_cnt)
   IF (dmvr_mv_log_maint("CREATE",mv->mlog[dmc_iter].table_name)=0)
    GO TO exit_program
   ENDIF
 ENDFOR
 SET dm_err->eproc = concat("Creating MVIEW on prebuilt table for MVIEW: ",dmc_mv_name)
 CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
 IF (dmvr_create_mview(null)=0)
  GO TO exit_program
 ENDIF
 IF (dmvr_refresh_mview(mv->name,"c")=0)
  GO TO exit_program
 ENDIF
 IF (dmvr_create_mv_syn(mv->syn_name,mv->name)=0)
  GO TO exit_program
 ENDIF
 IF (dmvr_updt_mv_inst("IU",mv->name,1,"MVIEW")=0)
  GO TO exit_program
 ENDIF
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "DM2_MV_CREATE completed"
 CALL final_disp_msg("dm2_mv_create")
END GO
