CREATE PROGRAM dcp_rpt_pvpatlist:dba
 ...
 DECLARE strmrntext = vc WITH noconstant(fillstring(50," "))
 DECLARE fieldsmatch = i4 WITH noconstant(0)
 DECLARE strunittext = vc WITH noconstant(fillstring(50," "))
 DECLARE strroomtext = vc WITH noconstant(fillstring(50," "))
 DECLARE strbedtext = vc WITH noconstant(fillstring(50," "))
 DECLARE strnametext = vc WITH noconstant(fillstring(50," "))
 DECLARE stradmittext = vc WITH noconstant(fillstring(50," "))
 DECLARE strdischtext = vc WITH noconstant(fillstring(50," "))
 DECLARE intdos = i2 WITH noconstant(0)
 DECLARE attendphyspref = i2 WITH noconstant(0)
 SET attendphyspref = request->print_pref
 SUBROUTINE (word_wrap(comment_text=vc,ll=i4) =i4)
   SET wrap_cnt = 0
   SET break_pos = 0
 ...
 IF ((request->visit_cnt > 0))
  SELECT
   IF (attendphyspref=1)
    HEAD REPORT
     cnt = 0, gender = " ", dos = 0,
     stat = alterlist(temp->qual,cnt), stat = alterlist(visit_persons->qual,cnt)
    DETAIL
     cnt = (cnt+ 1)
     IF (mod(cnt,10)=1)
      stat = alterlist(temp->qual,(cnt+ 9)), stat = alterlist(visit_persons->qual,(cnt+ 9))
     ENDIF
     pos = locateval(pos,1,size(trep->encntrs,5),e.encntr_id,trep->encntrs[pos].encntr_id),
     visit_persons->qual[cnt].person_id = e.person_id, temp->qual[cnt].name = substring(1,30,p
      .name_full_formatted),
     temp->qual[cnt].age = cnvtage(p.birth_dt_tm), temp->qual[cnt].dob = datetimezoneformat(p
      .birth_dt_tm,p.birth_tz,"@SHORTDATE"), temp->qual[cnt].mrn = cnvtalias(ea.alias,ea
      .alias_pool_cd),
     ...
     ].room = substring(1,18,uar_get_code_display(e.loc_room_cd)), temp->qual[cnt].bed = substring(1,
      10,uar_get_code_display(e.loc_bed_cd)),
     temp->qual[cnt].admit = datetimezoneformat(e.reg_dt_tm,trep->encntrs[pos].time_zone_indx,
      "@SHORTDATE")
     IF (disch_null_ind=0
      AND e.disch_dt_tm <= cnvtdatetime(sysdate))
   ...
   HEAD e.encntr_id
    ...
    visit_persons->qual[cnt].person_id = e.person_id, temp->qual[cnt].name = substring(1,30,p
     .name_full_formatted),
    temp->qual[cnt].age = cnvtage(p.birth_dt_tm), temp->qual[cnt].dob = datetimezoneformat(p
     .birth_dt_tm,p.birth_tz,"@SHORTDATE"), temp->qual[cnt].mrn = cnvtalias(ea.alias,ea.alias_pool_cd
     ),
    gender = cnvtupper(substring(1,1,uar_get_code_display(p.sex_cd))), temp->qual[cnt].gender =
    gender, temp->qual[cnt].admitdoc = substring(1,30,pl.name_full_formatted),
    temp->qual[cnt].unit = substring(1,20,uar_get_code_display(e.loc_nurse_unit_cd)), temp->qual[cnt]
    .room = substring(1,18,uar_get_code_display(e.loc_room_cd)), temp->qual[cnt].bed = substring(1,10,
     uar_get_code_display(e.loc_bed_cd)),
    temp->qual[cnt].admit = datetimezoneformat(e.reg_dt_tm,trep->encntrs[pos].time_zone_indx,
     "@SHORTDATE")
    IF (disch_null_ind=0
  ...
  IF ((request->person_cnt > 0))
   ...
   SELECT INTO "nl:"
    ...
    HEAD REPORT
     gender = " "
    HEAD p.person_id
     cnt += 1, stat = alterlist(temp->qual,cnt), temp->qual[cnt].name = substring(1,30,p
      .name_full_formatted),
     temp->qual[cnt].age = cnvtage(p.birth_dt_tm), temp->qual[cnt].dob = datetimezoneformat(p
      .birth_dt_tm,p.birth_tz,"@SHORTDATE"), temp->qual[cnt].mrn = cnvtalias(pa.alias,pa
      .alias_pool_cd),
  ...
  EXECUTE cpm_create_file_name "dcp", "dat"
  SET tempfile1a = cpm_cfn_info->file_name_path
  SELECT INTO value(tempfile1a)
   ...
   DETAIL
    ...
    FOR (x = start_cnt TO cnt)
      ...
      linebump = 0
      IF (attendphyspref=1)
       IF ((temp->qual[x].mrn=strmrntext)
        AND (temp->qual[x].unit=strunittext)
        AND (temp->qual[x].room=strroomtext)
        AND (temp->qual[x].bed=strbedtext)
        AND (temp->qual[x].name=strnametext)
        AND (temp->qual[x].admit=stradmittext)
        AND (temp->qual[x].disch=strdischtext)
        AND (temp->qual[x].dos=intdos))
        fieldsmatch = 1
       ENDIF
      ENDIF
      IF (fieldsmatch=1)
       fieldsmatch = 0, ycol -= 21, strwraptext1 = "",
       ...
       IF (size(strwraptext2,1) > 0)
        CALL print(calcpos(490,(ycol+ 7))), strwraptext2, linebump += 7
       ENDIF
       IF (size(strwraptext3,1) > 0)
        CALL print(calcpos(490,(ycol+ 14))), strwraptext3, linebump += 7
       ENDIF
      ELSE
       strmrntext = temp->qual[x].mrn, strunittext = temp->qual[x].unit, strroomtext = temp->qual[x].
       room,
       strbedtext = temp->qual[x].bed, strnametext = temp->qual[x].name, stradmittext = temp->qual[x]
       .admit,
       strdischtext = temp->qual[x].disch, intdos = temp->qual[x].dos, fieldsmatch = 0
       IF ((temp->qual[x].unit > " "))
        ...
        ELSE
         xxx = concat(trim(temp->qual[x].unit)," ",trim(temp->qual[x].room),"-",trim(temp->qual[x].
           bed))
        ENDIF
        CALL word_wrap(xxx,18)
        IF ((wrap->wrap_cnt > 0))
       ...
       IF (size(strwraptext2,1) > 0)
        CALL print(calcpos(30,(ycol+ 7))), strwraptext2, linebump += 7
       ENDIF
       IF (size(strwraptext3,1) > 0)
        CALL print(calcpos(30,(ycol+ 14))), strwraptext3, linebump += 7
       ENDIF
       ...
       IF (size(strwraptext2,1) > 0)
        CALL print(calcpos(180,(ycol+ 7))), strwraptext2, linebump += 7
       ENDIF
       IF (size(strwraptext3,1) > 0)
        CALL print(calcpos(180,(ycol+ 14))), strwraptext3, linebump += 7
       ENDIF
      ...
      IF (ycol > 680
       AND x < cnt)
       start_cnt = (x+ 1), strmrntext = "", strunittext = "",
       strroomtext = "", strbedtext = "", strnametext = "",
       stradmittext = "", strdischtext = "", intdos = 0,
       BREAK
      ENDIF
    ENDFOR
   FOOT PAGE
  ...
  SELECT INTO "nl:"
   ...
   HEAD p.person_id
    cnt += 1, stat = alterlist(temp->qual,cnt), temp->qual[cnt].name = substring(1,30,p
     .name_full_formatted),
    temp->qual[cnt].age = cnvtage(p.birth_dt_tm), temp->qual[cnt].dob = datetimezoneformat(p
     .birth_dt_tm,p.birth_tz,"@SHORTDATE"), temp->qual[cnt].mrn = cnvtalias(pa.alias,pa.alias_pool_cd
     ),
    gender = cnvtupper(substring(1,1,uar_get_code_display(p.sex_cd))), temp->qual[cnt].gender =
    gender
   WITH nocounter, outerjoin = d1, dontcare = pa
  ...
  SELECT INTO value(tempfile1a)
   ...
   DETAIL
    ...
    FOR (x = start_cnt TO cnt)
      ...
      IF (size(strwraptext2,1) > 0)
       CALL print(calcpos(120,(ycol+ 7))), strwraptext2, linebump += 7
      ENDIF
      IF (size(strwraptext3,1) > 0)
       CALL print(calcpos(120,(ycol+ 14))), strwraptext3, linebump += 7
      ENDIF
