CREATE PROGRAM ec_profiler_m27:dba
 ...
 SELECT INTO "nl"
  ...
  ORDER BY cv.display, tcp.sub_comp_name
  HEAD REPORT
  ...
  HEAD tcp.sub_comp_name
   ...
   reply->facilities[1].positions[1].details[detailcnt].detail_name = cv.display, reply->facilities[1
   ].positions[1].details[detailcnt].detail_value_txt = trim(tcp.sub_comp_name)
  WITH nocounter
 ;end select
