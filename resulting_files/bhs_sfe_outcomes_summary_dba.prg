CREATE PROGRAM bhs_sfe_outcomes_summary:dba
 PROMPT
 ...
 SET permission = 0
 IF (person_id IN (589853, 20497040, 943042, 19821227))
  SET permission = 1
 ELSE
  SELECT INTO "nl:"
   FROM prsnl p
 ...
 ENDIF
 IF (permission=0)
  SET usrerr = "You do not have permission to execute this program."
  GO TO exit_program
 ENDIF
 IF (( $LOCATION=7)
  AND ( $AUNIT <= 0))
 ...
 DECLARE any_loc_ind = c1 WITH noconstant(substring(1,1,reflect(parameter(4,0)))), public
 ...
 DECLARE smokingcessationmaterialgiven = f8 WITH constant(uar_get_code_by("DISPLAYKEY",72,
   "SMOKINGCESSATIONMATERIALGIVEN")), protect
 ...
 DECLARE ordcomplete = f8 WITH constant(uar_get_code_by("DISPLAYKEY",6004,"COMPLETED")), protect
 ...
 DECLARE tempbegeffdttm = q8
 DECLARE tempendeffdttm = q8
 DECLARE nrtgiven = i2
 ...
 IF (((datetimediff(end_date_qual,beg_date_qual) > 6
  ...
  SELECT INTO  $OUTDEV
   ...
   HEAD REPORT
    msg1 =
    IF (( $AUNIT=0)) "For facilities your date range can not be larger than 5 days."
    ELSE "For nurse units your date range can not be larger than 31 days"
    ENDIF
 ...
 IF (( $LOCATION > 1))
  ...
  SELECT INTO "NL:"
   ...
   PLAN (d
    ...
    WHERE expand(num,expand_start,expand_stop,e.encntr_id,formactlisttemp->qual[num].encntr_id)
     ...
     AND ( $AUNIT > 0)
     AND ( $LOCATION IN (4, 5, 6))) OR (((( $LOCATION=7)
     AND e.loc_facility_cd IN ( $AUNIT)) OR (( $AUNIT <= 0)
     AND ((( $LOCATION=2)
     ...
     AND e.loc_facility_cd IN (bfmc, bfmcinptpsych))) )) )) )) )) )) )
   ORDER BY e.encntr_id
   HEAD e.encntr_id
    pos = 0, locnum = 0, pos = locateval(locnum,1,size(formactlisttemp->qual,5),e.encntr_id,
     formactlisttemp->qual[locnum].encntr_id)
    IF (pos > 0)
     formactlisttemp->qual[pos].encntrqual = 1, formactlisttemp->qual[pos].encntr_beg_dt_tm =
     cnvtdatetime(e.beg_effective_dt_tm), formactlisttemp->qual[pos].encntr_end_dt_tm = cnvtdatetime(
  ...
  SET expand_stop = expandcnt
  SET num = 0
  SET tempbegeffdttm = 0
  SET tempendeffdttm = 0
  SELECT INTO "NL:"
   e.encntr_id
   FROM encounter e,
    encntr_loc_hist el,
   ...
   PLAN (d
    ...
    WHERE el.encntr_id=e.encntr_id
     ...
     AND ( $AUNIT > 0)
     AND ( $LOCATION IN (4, 5, 6))) OR (((( $LOCATION=7)
     AND el.loc_facility_cd IN ( $AUNIT)) OR (( $AUNIT=0)
     AND ((( $LOCATION=2)
     ...
     AND el.loc_facility_cd IN (bfmc, bfmcinptpsych))) )) )) )) )) )) )
   ORDER BY e.encntr_id
   HEAD e.encntr_id
    pos = 0, locnum = 0, pos = locateval(locnum,1,size(formactlisttemp->qual,5),e.encntr_id,
     formactlisttemp->qual[locnum].encntr_id)
    IF (pos > 0)
     formactlisttemp->qual[pos].encntr_beg_dt_tm = cnvtdatetime(el.beg_effective_dt_tm),
     formactlisttemp->qual[pos].encntr_end_dt_tm = cnvtdatetime(el.end_effective_dt_tm)
    ENDIF
   HEAD el.encntr_loc_hist_id
    IF (pos > 0)
     formactlisttemp->qual[pos].encntrqual = 1
     IF (datetimediff(cnvtdatetime(formactlisttemp->qual[pos].encntr_beg_dt_tm),cnvtdatetime(el
       .beg_effective_dt_tm),5) > 0)
      formactlisttemp->qual[pos].encntr_beg_dt_tm = cnvtdatetime(el.beg_effective_dt_tm)
     ENDIF
     IF (datetimediff(cnvtdatetime(el.end_effective_dt_tm),cnvtdatetime(formactlisttemp->qual[pos].
       encntr_end_dt_tm),5) > 0)
      formactlisttemp->qual[pos].encntr_end_dt_tm = cnvtdatetime(el.end_effective_dt_tm)
     ENDIF
    ENDIF
   WITH counter
  ...
  FOR (x = 1 TO actual_size)
    IF ((formactlisttemp->qual[x].encntrqual=1))
     ...
     dcp_forms_activity_idvc
     SET formactlist->qual[count].encntr_beg_dt_tm = cnvtdatetime(formactlisttemp->qual[x].
      encntr_beg_dt_tm)
     SET formactlist->qual[count].encntr_end_dt_tm = cnvtdatetime(formactlisttemp->qual[x].
      encntr_end_dt_tm)
    ENDIF
  ENDFOR
 ...
 IF (size(formactlist->qual,5) <= 0)
  ...
  GO TO exit_program
 ENDIF
 CALL echo(build("# of Encounters found for form: ",size(formactlist->qual,5)))
 ...
 SELECT INTO "NL:"
  ...
  PLAN (d
   ...
   WHERE expand(num,expand_start,expand_stop,ce.encntr_id,formactlist->qual[num].encntr_id)
    AND ((((ce.event_cd+ 0) IN (smokingcessation, assessmentstage, pharmacotherapypastuse,
   healthrisks, yearssmoking,
   ...
   smokinghistoryandmanagementform, smokingcessationresourcesinformation))
    AND ((ce.event_end_dt_tm+ 0) BETWEEN cnvtdatetime(beg_date_qual) AND cnvtdatetime(end_date_qual))
   ) OR (((ce.event_cd+ 0) IN (smokingcessationmaterialgiven))
    AND ((ce.event_end_dt_tm+ 0) > cnvtdatetime(beg_date_qual))
    AND ((ce.result_status_cd+ 0) IN (altered, modified, auth))
    AND ((ce.event_class_cd+ 0) != 654645.00)
    AND ((ce.authentic_flag+ 0)=1)
    AND ce.valid_until_dt_tm >= cnvtdatetime(beg_date_qual))) )
  ...
  HEAD ce.event_id
   ...
   CALL echo(format(cnvtdatetime(formactlist->qual[pos].encntr_beg_dt_tm),";;q")),
   CALL echo(format(cnvtdatetime(formactlist->qual[pos].encntr_end_dt_tm),";;q"))
   IF (pos > 0
   ...
   cnvtdatetime(formactlist->qual[pos].encntr_end_dt_tm))
    ...
    ELSEIF (ce.event_cd=smokingreferraloutcome)
     sfeinfo->detailqual[count].referralorderids = concat(sfeinfo->detailqual[count].referralorderids,
      ",",trim(cnvtstring(ce.order_id),3)),
     CALL echo(build("*************SMOKINGREFERRALOUTCOME**************",ce.encntr_id))
     IF (cnvtupper(ce.result_val) IN ("AGREED TO COUNSELING"))
      sfeinfo->patagreetocounseling = (sfeinfo->patagreetocounseling+ 1)
     ELSEIF (cnvtupper(ce.result_val) IN ("NOT READY TO QUIT*"))
      sfeinfo->notreadytoquit = (sfeinfo->notreadytoquit+ 1)
     ELSEIF (cnvtupper(ce.result_val) IN ("REFUSED COUNSELING*"))
      sfeinfo->refusedcounseling = (sfeinfo->refusedcounseling+ 1)
     ELSEIF (cnvtupper(ce.result_val) IN ("CLINICAL CONDITION PRECLUDES*"))
      sfeinfo->clinicalcondition = (sfeinfo->clinicalcondition+ 1)
     ENDIF
    ELSEIF (ce.event_cd=patientwantsreferraltoquitsmoking)
     sfeinfo->detailqual[count].referraldttm = cnvtdatetime(ce.clinsig_updt_dt_tm)
    ...
    ELSEIF (ce.event_cd=smokingcessationresourcesinformation)
     ...
     ENDIF
    ELSEIF (ce.event_cd=smokingcessationmaterialgiven)
     IF (cnvtupper(ce.event_tag) IN ("DONE*"))
      CALL echo(build("val>>>>>",ce.result_val,"@",ce.event_tag)), sfeinfo->pattaskresourceinfogiven
       = (sfeinfo->pattaskresourceinfogiven+ 1)
     ENDIF
 ...
 SELECT INTO "NL:"
  ...
  FROM orders o,
   (dummyt d  WITH seq = size(sfeinfo->detailqual,5))
  PLAN (d)
   JOIN (o
   WHERE (o.person_id=sfeinfo->detailqual[d.seq].person_id)
    AND (o.encntr_id=sfeinfo->detailqual[d.seq].encntr_id)
  ...
  HEAD o.catalog_cd
   ...
   IF (pos > 0)
    IF (o.orig_order_dt_tm >= cnvtdatetime(sfeinfo->detailqual[d.seq].referraldttm))
     IF (o.catalog_cd=patientwantsreferralquitsmokingtask)
      CALL echo(build("ORDER>>>>>>>>>>>>>>>>>>",o.order_id)), sfeinfo->mlhfmcpatientwantsrefer = (
      sfeinfo->mlhfmcpatientwantsrefer+ 1)
      IF (o.order_status_cd=ordcomplete
       AND findstring(trim(cnvtstring(o.order_id)),sfeinfo->detailqual[d.seq].referralorderids) > 0)
       sfeinfo->mlhfmcpatientwantsrefercompl = (sfeinfo->mlhfmcpatientwantsrefercompl+ 1),
       CALL echo(build("*************SMOKINGREFERRALOUTCOME**task****",o.order_id))
      ENDIF
     ELSEIF (o.catalog_cd=patientwantsreferraltoquitsmokingtask)
      CALL echo(build("ORDER>>>>>>>>>>>>>>>>>>",o.order_id)), sfeinfo->bmcpatientwantsrefer = (
      sfeinfo->bmcpatientwantsrefer+ 1)
      IF (o.order_status_cd=ordcomplete
       AND findstring(trim(cnvtstring(o.order_id)),sfeinfo->detailqual[d.seq].referralorderids) > 0)
       CALL echo(build("*************SMOKINGREFERRALOUTCOME**task****",o.order_id)), sfeinfo->
       bmcpatientwantsrefercompl = (sfeinfo->bmcpatientwantsrefercompl+ 1)
      ENDIF
     ENDIF
    ENDIF
    IF (o.catalog_cd IN (cpgnicotinereplacementtherapy, nicotine, bupropion, varenicline))
