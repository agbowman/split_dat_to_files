CREATE PROGRAM afc_da_add_charge_batch_dtl_cd:dba
 SET afc_da_add_charge_batch_dtl_cd_vrsn = 20090219
 ...
 DECLARE add_charge_batch_detail_code(nstart=i4,nstop=i4) = null WITH protect
 ...
 CALL add_charge_batch_detail_code(mndastart,mndastop)
 SUBROUTINE add_charge_batch_detail_code(nstart,nstop)
   ...
   FOR (i = 1 TO size(request->objarray,5))
     IF (validate(request->objarray[i].charge_batch_detail_code_id,- (0.00001)) <= 0.0)
      SELECT INTO "nl:"
       sdapk = seq(bce_event_log_seq,nextval)"##################;rp0"
       FROM dual
       DETAIL
        mrsstatus->objarray[i].daid = cnvtreal(sdapk)
       WITH format, counter
      ;end select
      IF (validate(request->objarray[i].charge_batch_detail_code_id))
       SET request->objarray[i].charge_batch_detail_code_id = mrsstatus->objarray[i].daid
      ENDIF
      ...
      ENDIF
     ELSE
      SET mrsstatus->objarray[i].daid = request->objarray[i].charge_batch_detail_code_id
     ENDIF
   ENDFOR
   INSERT  FROM charge_batch_detail_code c,
     (dummyt dt  WITH seq = value(size(request->objarray,5)))
    SET c.charge_batch_detail_code_id = mrsstatus->objarray[dt.seq].daid, c.charge_batch_detail_id =
