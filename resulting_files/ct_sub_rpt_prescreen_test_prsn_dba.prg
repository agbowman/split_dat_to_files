CREATE PROGRAM ct_sub_rpt_prescreen_test_prsn:dba
 PROMPT
 ...
 WITH protect
 DECLARE footd1_seqsection_line(ncalc=i2) = f8 WITH protect
 DECLARE footd1_seqsection_lineabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
   ...
   SELECT
    ...
    HEAD d1.seq
     IF (size(protlist->protqual,5) > 0)
      prot_count = (prot_count+ 1), _bcontheadd1_seqsection = 0, bfirsttime = 1
     ...
     WHILE (((_bcontfootd1_seqsection=1) OR (bfirsttime=1)) )
       ...
       IF (((_bholdcontinue=1) OR (_fdrawheight > 0)) )
        IF ((_fenddetail >= (_yoffset+ _fdrawheight)))
         _fdrawheight = (_fdrawheight+ footd1_seqsection_line(rpt_calcheight))
        ENDIF
       ENDIF
       IF (((_yoffset+ _fdrawheight) > _fenddetail))
        BREAK
       ...
       bfirsttime = 0
     ENDWHILE
     _fdrawheight = footd1_seqsection_line(rpt_calcheight)
     IF (((_yoffset+ _fdrawheight) > _fenddetail))
      BREAK
     ENDIF
     dummy_val = footd1_seqsection_line(rpt_render), match_count = 0
    FOOT PAGE
 ...
 SUBROUTINE headpagesectionabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   ...
   DECLARE __label_rpt_title = vc WITH noconstant(build2(uar_i18ngetmessage(i18nhandle,
      "TEST_SCREENING_PRESCREEN_RPT","Test Pre-Screening"),char(0))), protect
 ...
 SUBROUTINE headpagesection1abs(ncalc,offsetx,offsety,maxheight,bcontinue)
   ...
   DECLARE __label_prot_view = vc WITH noconstant(build2(uar_i18ngetmessage(i18nhandle,"PROT_VIEW",
      "Protocol View"),char(0))), protect
 ...
 SUBROUTINE headpagesection2abs(ncalc,offsetx,offsety,maxheight,bcontinue)
   ...
   DECLARE __label_prot_mnemonic = vc WITH noconstant(build2(uar_i18ngetmessage(i18nhandle,
      "PROT_MNEMONIC","Protocol Mnemonic"),char(0))), protect
   DECLARE __label_init_srv = vc WITH noconstant(build2(uar_i18ngetmessage(i18nhandle,"INIT_SERVICE",
      "Initiating Service"),char(0))), protect
   IF (bcontinue=0)
 ...
 SUBROUTINE footd1_seqsectionabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   ...
   DECLARE __total_patient = vc WITH noconstant(build2(uar_i18nbuildmessage(i18nhandle,
      "TOTAL_PATIENTS_PRESCREEN_RPT","Total Potential Patients: %1","i",size(protlist->protqual[d1
       .seq].personqual,5)),char(0))), protect
   IF (bcontinue=0)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE footd1_seqsection_line(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = footd1_seqsection_lineabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE footd1_seqsection_lineabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.130000), private
   IF (ncalc=rpt_render)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s3c0)
 ...
 SUBROUTINE initializereport(dummy)
   IF (_bsubreport=0)
    ...
    SET rptreport->m_reportname = "CT_SUB_RPT_PRESCREEN_TEST_PRSN"
