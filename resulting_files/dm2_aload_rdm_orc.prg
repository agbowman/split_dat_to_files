CREATE PROGRAM dm2_aload_rdm_orc
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_rdm_orc",".log","dm2_aload_rdm_orc")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_rdm_orc"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE darorc_cnt = i4 WITH protect, noconstant(0)
 DECLARE darorc_str = vc WITH protect, noconstant("")
 DECLARE darorc_req_size = i4 WITH protect, noconstant(0)
 DECLARE darorc_ndx = i4 WITH protect, noconstant(0)
 FREE RECORD darorc_maint
 RECORD darorc_maint(
   1 qual[*]
     2 maint_type = c1
   1 num_of_inserts = i4
 ...
 SET darorc_maint->num_of_inserts = 0
 SET darorc_maint->num_of_none = 0
 SET darorc_req_size = size(requestin->list_0,5)
 SET stat = alterlist(darorc_maint->qual,darorc_req_size)
 IF (dac_pop_coldic_rec("OCD_README_COMPONENT")=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  SET dm_err->eproc = "Determining maintenance type for OCD_README_COMPONENT."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(darorc_req_size))
  DETAIL
   darorc_maint->qual[d.seq].maint_type = "I", darorc_maint->num_of_inserts = (darorc_maint->
   num_of_inserts+ 1)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 SELECT INTO "nl:"
  FROM ocd_readme_component dr,
   (dummyt d  WITH seq = value(darorc_req_size))
  PLAN (d)
   JOIN (dr
   WHERE cnvtupper(dr.end_state)=cnvtupper(requestin->list_0[d.seq].end_state)
    AND dr.component_type="SCRIPT")
  DETAIL
   darorc_maint->qual[d.seq].maint_type = "N", darorc_maint->num_of_none = (darorc_maint->num_of_none
   + 1), darorc_maint->num_of_inserts = (darorc_maint->num_of_inserts - 1)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(darorc_maint)
  CALL echorecord(requestin)
 ENDIF
 IF ((darorc_maint->num_of_inserts > 0))
  ...
  CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
    " dcv ,(dummyt d with seq = value(darorc_req_size) )"),0)
  CALL dm2_push_cmd("set",0)
  IF (dac_build_col_list(null)=0)
   ROLLBACK
   GO TO exit_program
  ENDIF
  CALL dm2_push_cmd("plan d where darorc_maint->qual[d.seq].maint_type = 'I' ",0)
  IF (dm2_push_cmd(" join dcv with nocounter go ",1)=0)
   ROLLBACK
   GO TO exit_program
  ENDIF
  IF ((curqual != darorc_maint->num_of_inserts))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = build("Rows Inserted:",curqual,". Does not match expected rows inserted:",
    darorc_maint->num_of_inserts)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ROLLBACK
   GO TO exit_program
  ENDIF
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_rdm_orc completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_rdm_orc")
END GO
