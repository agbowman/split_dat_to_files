CREATE PROGRAM ecf_rdm_occ_industry:dba
 ...
 DECLARE kia_concept_csv_filename = vc WITH public, constant("occ_industry_concept.csv")
 DECLARE kia_reltn_csv_filename = vc WITH public, constant("occ_industry_relation.csv")
 DECLARE kia_nomen_csv_filename = vc WITH public, constant("occ_industry_nomen.csv")
 SET kia_short_desc = "OCC_INDUSTRY Load"
 SET kia_tbl_desc = "OCC_INDUSTRY Content Load"
 SET kia_rdm_log_name = "ecf_rdm_occ_industry.log"
 SET readme_data->status = "F"
 SET readme_data->message = concat("FAILED - ",kia_tbl_desc)
 SET script_status = " "
 SET csv_stat = 1
 SET kia_rdm_vocab_mean = "OCC_INDUSTRY"
 SET kia_rdm_nbr = 11987
 CALL kia_chk_error(1)
 COMMIT
 CALL kia_db2_check(1)
 CALL kia_inhouse_chk(1)
 EXECUTE kia_dm_dbimport "cer_install:occ_industry_version.csv", "kia_get_rdm_vars", 100,
 0
 CALL kia_chk_script_failure("KIA_GET_RDM_VARS")
 SET start_time_var = cnvtdatetime(curdate,curtime3)
 CALL log_sub(concat("ecf_rdm_occ_industry Begin> ",format(cnvtdatetime(curdate,curtime3),
 ...
 IF (kia_obj_suc_ind=0)
  ...
  DELETE  FROM cmt_concept_load_ns c
   WHERE ((c.concept_cki="OCC_INDUSTRY*") OR (c.concept_cki="SNOMED*"))
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
 ...
 IF (kia_obj_suc_ind=0)
  EXECUTE kia_load_cmt_concept_by_ld_2 "OCC_INDUSTRY"
 ...
 IF (kia_obj_suc_ind=0)
  ...
  DELETE  FROM cmt_concept_reltn_ld_ns c
   WHERE c.concept_cki1="OCC_INDUSTRY*"
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
 ...
 IF (kia_obj_suc_ind=0)
  EXECUTE kia_load_concept_reltn_2 "OCC_INDUSTRY"
