CREATE PROGRAM acm_ina_proc_prsnl_rel:dba
 ...
 IF (validate(schuar_def,999)=999)
  ...
  persist
 ENDIF
 IF (validate(ina_proc_prsnl_rel_request,0))
 ...
 IF (called_by_script_server=true)
  ...
  SET stat = alterlist(ina_proc_prsnl_rel_request->qual,nbr1)
  SET ina_proc_prsnl_rel_request->call_echo_ind = request->call_echo_ind
  FOR (i = 1 TO nbr1)
    SET ina_proc_prsnl_rel_request->qual[i].proc_prsnl_reltn_id = request->qual[i].
    proc_prsnl_reltn_id
    SET ina_proc_prsnl_rel_request->qual[i].updt_cnt = request->qual[i].updt_cnt
    SET ina_proc_prsnl_rel_request->qual[i].active_status_cd = request->qual[i].active_status_cd
    SET ina_proc_prsnl_rel_request->qual[i].allow_partial_ind = request->qual[i].allow_partial_ind
    SET ina_proc_prsnl_rel_request->qual[i].version_ind = request->qual[i].version_ind
    SET ina_proc_prsnl_rel_request->qual[i].force_updt_ind = request->qual[i].force_updt_ind
  ENDFOR
  RECORD reply(
    1 qual_cnt = i4
    1 qual[*]
 ...
 SET table_name = "PROC_PRSNL_RELTN"
 SET ina_proc_prsnl_rel_reply->qual_cnt = size(ina_proc_prsnl_rel_request->qual,5)
 SET stat = alterlist(ina_proc_prsnl_rel_reply->qual,ina_proc_prsnl_rel_reply->qual_cnt)
 IF ((ina_proc_prsnl_rel_reply->qual_cnt=0))
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  t.updt_cnt, d.seq
  FROM proc_prsnl_reltn t,
   (dummyt d  WITH seq = value(ina_proc_prsnl_rel_reply->qual_cnt))
  PLAN (d)
   JOIN (t
   WHERE (t.proc_prsnl_reltn_id=ina_proc_prsnl_rel_request->qual[d.seq].proc_prsnl_reltn_id))
  HEAD REPORT
   i_version = 0
  DETAIL
   IF ((((t.updt_cnt=ina_proc_prsnl_rel_request->qual[d.seq].updt_cnt)) OR ((
   ina_proc_prsnl_rel_request->qual[d.seq].force_updt_ind=1))) )
    ina_proc_prsnl_rel_reply->qual[d.seq].status = 1
   ELSE
    ina_proc_prsnl_rel_reply->qual[d.seq].status = update_cnt_error
   ENDIF
  WITH nocounter, forupdate(t)
 ;end select
 SET nbr_correct = 0
 FOR (i = 1 TO ina_proc_prsnl_rel_reply->qual_cnt)
   IF ((ina_proc_prsnl_rel_reply->qual[i].status=1))
    SET nbr_correct = (nbr_correct+ 1)
   ELSE
    IF ((ina_proc_prsnl_rel_request->qual[i].allow_partial_ind != 1))
 ...
 IF (nbr_correct=0)
  GO TO exit_script
 ENDIF
 UPDATE  FROM proc_prsnl_reltn t,
   (dummyt d  WITH seq = value(ina_proc_prsnl_rel_reply->qual_cnt))
  SET t.updt_cnt = (t.updt_cnt+ 1), t.updt_dt_tm = cnvtdatetime(curdate,curtime3), t.updt_id =
   reqinfo->updt_id,
   t.updt_task = reqinfo->updt_task, t.updt_applctx = reqinfo->updt_applctx, t.active_ind = false,
   t.active_status_cd =
   IF ((ina_proc_prsnl_rel_request->qual[d.seq].active_status_cd > 0)) ina_proc_prsnl_rel_request->
    qual[d.seq].active_status_cd
   ELSE reqdata->inactive_status_cd
   ENDIF
   , t.active_status_dt_tm = cnvtdatetime(curdate,curtime3), t.active_status_prsnl_id = reqinfo->
   updt_id
  PLAN (d
   WHERE (ina_proc_prsnl_rel_reply->qual[d.seq].status=1))
   JOIN (t
   WHERE (t.proc_prsnl_reltn_id=ina_proc_prsnl_rel_request->qual[d.seq].proc_prsnl_reltn_id))
  WITH nocounter, status(ina_proc_prsnl_rel_reply->qual[d.seq].status)
 ;end update
 SET nbr_correct = 0
 FOR (i = 1 TO ina_proc_prsnl_rel_reply->qual_cnt)
   IF ((ina_proc_prsnl_rel_reply->qual[i].status=1))
    SET nbr_correct = (nbr_correct+ 1)
   ELSE
    IF ((ina_proc_prsnl_rel_reply->qual[i].status=0))
     SET ina_proc_prsnl_rel_reply->qual[i].status = inactivate_error
     IF ((ina_proc_prsnl_rel_request->qual[i].allow_partial_ind != 1))
...
#exit_script
 IF (called_by_script_server=true)
  SET reply->qual_cnt = ina_proc_prsnl_rel_reply->qual_cnt
  SET stat = alterlist(reply->qual,reply->qual_cnt)
  FOR (i = 1 TO reply->qual_cnt)
    SET reply->qual[i].status = ina_proc_prsnl_rel_reply->qual[i].status
  ENDFOR
 ENDIF
#check_failed
 IF (failed=false)
  CASE (nbr_correct)
   ...
   OF ina_proc_prsnl_rel_reply->qual_cnt:
  ...
  ENDCASE
 ELSE
  CALL echorecord(ina_proc_prsnl_rel_request)
  CALL echorecord(ina_proc_prsnl_rel_reply)
  SET reqinfo->commit_ind = false
  IF (failed != true
   AND failed != false)
