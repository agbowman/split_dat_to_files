CREATE PROGRAM afc_modify_charge:dba
 DECLARE afc_modify_charge = vc WITH constant("CHARGSRV-15575.003")
 ...
 IF ( NOT (validate(cs18649_payment)))
  DECLARE cs18649_payment = f8 WITH noconstant(getcodevaluelc(18649,"PAYMENT",0))
 ENDIF
 IF ( NOT (validate(cs18649_adjust)))
  DECLARE cs18649_adjust = f8 WITH noconstant(getcodevaluelc(18649,"ADJUST",0))
 ENDIF
 IF ( NOT (validate(cs18937_reversal)))
  DECLARE cs18937_reversal = f8 WITH noconstant(getcodevaluelc(18937,"REVERSAL",0))
 ENDIF
 IF ( NOT (validate(cs25753_reversal)))
  DECLARE cs25753_reversal = f8 WITH noconstant(getcodevaluelc(25753,"REVERSAL",0))
 ENDIF
 IF ( NOT (validate(cs13028_cr)))
  DECLARE cs13028_cr = f8 WITH noconstant(getcodevaluelc(13028,"CR",0))
 ENDIF
 IF (validate(processlatechargebalance,char(128))=char(128))
  SUBROUTINE (processlatechargebalance(pbenefitorderid=f8) =i2)
    CALL logmessage("processLateChargeBalance","Entering",log_debug)
    DECLARE isalllatechrgsworked = i2 WITH protect, noconstant(false)
    DECLARE pmtexistsind = i2 WITH protect, noconstant(false)
    DECLARE currentbalanceid = f8 WITH protect, noconstant(0.0)
    DECLARE paycnt = i4 WITH protect, noconstant(0)
    DECLARE nidx = i4 WITH protect, noconstant(0)
    DECLARE pos = i4 WITH protect, noconstant(0)
    DECLARE claimlevelpayment = i2 WITH protect, noconstant(false)
    DECLARE chargelevelpayment = i2 WITH protect, noconstant(false)
    DECLARE claimid = f8 WITH protect, noconstant(0.0)
    DECLARE totallatechgamount = f8 WITH protect, noconstant(0.0)
    IF ( NOT (validate(cs24451_generated)))
     DECLARE cs24451_generated = f8 WITH protect, noconstant(getcodevaluelc(24451,"GENERATED",0))
    ENDIF
    IF ( NOT (validate(cs29904_technical)))
     DECLARE cs29904_technical = f8 WITH protect, noconstant(getcodevaluelc(29904,"TECHNICAL",0))
    ENDIF
    FREE RECORD compinsbalrequest
    RECORD compinsbalrequest(
      1 balanceid = f8
    )
    FREE RECORD compinsbalreply
    RECORD compinsbalreply(
      1 statusmessages[*]
        2 messageflag = i4
        2 message = vc
      1 status_data
    ...
    FREE RECORD paytrans
    RECORD paytrans(
      1 objarray[*]
        2 activityid = f8
    )
    CALL getlatechargeamount(pbenefitorderid,totallatechgamount)
    IF (isequal(totallatechgamount,0.0))
     SET isalllatechrgsworked = true
    ENDIF
    IF (isalllatechrgsworked)
     SELECT INTO "nl:"
      FROM bo_hp_reltn bh
      WHERE bh.benefit_order_id=pbenefitorderid
       AND bh.bo_hp_status_cd=cs24451_generated
       AND bh.priority_seq > 0
       AND bh.active_ind=true
      ORDER BY bh.priority_seq DESC
      DETAIL
       currentbalanceid = bh.bo_hp_reltn_id
      WITH nocounter
     ;end select
     IF (currentbalanceid <= 0.0)
      RETURN(true)
     ENDIF
     SELECT INTO "nl:"
      FROM pft_trans_reltn ptr,
       trans_log tl
      PLAN (ptr
       WHERE ptr.benefit_order_id=currentbalanceid
       ...
       WHERE tl.activity_id=ptr.activity_id
        ...
        AND tl.trans_reason_cd != cs18937_reversal
        AND (tl.total_trans_amount !=
       (SELECT
        nullval(sum(ttr2.amount),0)
        FROM trans_trans_reltn ttr2
        WHERE ttr2.parent_activity_id=tl.activity_id
         AND ttr2.trans_reltn_reason_cd=cs25753_reversal))
        AND  NOT ( EXISTS (
       (SELECT
        brn.parent_entity_id
        FROM bill_reltn brn,
         bill_rec br,
         denial d
        WHERE brn.parent_entity_id=ptr.benefit_order_id
         AND brn.parent_entity_name="BO_HP_RELTN"
         AND br.corsp_activity_id=brn.corsp_activity_id
         AND br.bill_vrsn_nbr=brn.bill_vrsn_nbr
         AND d.corsp_activity_id=br.corsp_activity_id
         AND d.active_ind=true
         AND d.denial_type_cd=cs29904_technical))))
      DETAIL
       paycnt += 1, stat = alterlist(paytrans->objarray,paycnt), paytrans->objarray[paycnt].
       activityid = ptr.activity_id
      WITH nocounter
     ;end select
     IF (curqual > 0)
      SET pmtexistsind = true
     ENDIF
     IF (pmtexistsind)
      SELECT INTO "nl:"
       FROM (dummyt d  WITH seq = size(paytrans->objarray,5)),
        pft_trans_reltn ptr
       PLAN (d)
        JOIN (ptr
        WHERE (ptr.activity_id=paytrans->objarray[d.seq].activityid)
         AND ptr.parent_entity_name="BILL"
         AND  NOT ( EXISTS (
        (SELECT
         ptr1.activity_id
         FROM pft_trans_reltn ptr1
         WHERE ptr1.parent_entity_name="PFTCHARGE"
          AND ptr1.activity_id=ptr.activity_id))))
       WITH nocounter
      ;end select
      IF (curqual > 0)
       SET claimlevelpayment = 1
      ENDIF
      SELECT INTO "nl:"
       FROM (dummyt d  WITH seq = size(paytrans->objarray,5)),
        pft_trans_reltn ptr,
       ...
       PLAN (d)
        ...
        JOIN (ptr1
        WHERE (ptr1.activity_id=paytrans->objarray[d.seq].activityid)
         AND ptr1.parent_entity_name="BILL")
       DETAIL
        claimid = ptr1.parent_entity_id
       WITH nocounter
      ;end select
      IF (curqual > 0)
       SET chargelevelpayment = 1
      ENDIF
      IF (currentbalanceid > 0.0)
       IF (claimlevelpayment=false
        AND chargelevelpayment=true)
        IF ( NOT (checkchargelevelpayments(claimid)))
         CALL logmessage("processLateChargeBalance",
          "Not Rolling the balance as payment not done on all the charges.",log_error)
         RETURN(true)
        ENDIF
       ENDIF
       SET compinsbalrequest->balanceid = currentbalanceid
       EXECUTE pft_complete_ins_balance  WITH replace("REQUEST",compinsbalrequest), replace("REPLY",
        compinsbalreply)
       IF ((compinsbalreply->status_data.status != "S"))
        CALL logmessage("processLateChargeBalance","Script PFT_COMPLETE_INS_BALANCE failed.",
         log_error)
        RETURN(false)
       ENDIF
      ENDIF
     ENDIF
    ENDIF
    CALL logmessage("processLateChargeBalance","Exiting",log_debug)
    RETURN(true)
  END ;Subroutine
 ENDIF
 IF (validate(getlatechargeamount,char(128))=char(128))
  SUBROUTINE (getlatechargeamount(pbenefitorderid=f8,pramount=f8(ref)) =i2)
    CALL logmessage("getLateChargeAmount","Entering",log_debug)
    SET pramount = 0.0
    IF ( NOT (validate(cs21749_hcfa_1450)))
     DECLARE cs21749_hcfa_1450 = f8 WITH protect, noconstant(getcodevaluelc(21749,"HCFA_1450",0))
    ENDIF
    IF ( NOT (validate(cs21749_hcfa_1500)))
     DECLARE cs21749_hcfa_1500 = f8 WITH protect, noconstant(getcodevaluelc(21749,"HCFA_1500",0))
    ENDIF
    DECLARE latebalance = f8 WITH protect, noconstant(0.0)
    DECLARE adjustments = f8 WITH protect, noconstant(0.0)
    DECLARE chrgcnt = i4 WITH protect, noconstant(0.0)
    DECLARE chrgidx = i4 WITH protect, noconstant(0.0)
    RECORD chargelist(
      1 objarray[*]
        2 chargeitemid = f8
    ) WITH protect
    SELECT INTO "nl:"
     FROM pft_charge_bo_reltn pcbr,
      ...
      pft_trans_reltn ptr
     PLAN (pcbr
      WHERE pcbr.benefit_order_id=pbenefitorderid
       AND pcbr.active_ind=true)
      JOIN (pc
      WHERE pc.pft_charge_id=pcbr.pft_charge_id
       ...
       AND  NOT ( EXISTS (
      (SELECT
       r.pft_line_item_chrg_reltn_id
       FROM pft_line_item_chrg_reltn r,
        pft_line_item l
       WHERE r.pft_charge_id=pc.pft_charge_id
        AND l.pft_line_item_id=r.pft_line_item_id
        AND l.bill_type_cd IN (cs21749_hcfa_1450, cs21749_hcfa_1500)))))
      JOIN (c
      WHERE c.charge_item_id=pc.charge_item_id
       AND c.active_ind=true)
      JOIN (ptr
      WHERE (ptr.parent_entity_id= Outerjoin(pc.pft_charge_id))
       AND (ptr.parent_entity_name= Outerjoin("PFTCHARGE"))
       AND (ptr.trans_type_cd= Outerjoin(cs18649_adjust)) )
     ORDER BY c.charge_item_id
     HEAD c.charge_item_id
      adjustments = 0.0
     DETAIL
      adjustments += evaluate(ptr.dr_cr_flag,2,(ptr.amount * - (1)),ptr.amount)
     FOOT  c.charge_item_id
      IF (pc.late_chrg_flag=1)
       chrgcnt += 1, stat = alterlist(chargelist->objarray,chrgcnt), chargelist->objarray[chrgcnt].
       chargeitemid = c.charge_item_id
      ENDIF
      IF ( NOT (isequal((c.item_extended_price+ adjustments),0.0)))
       latebalance += c.item_extended_price
      ENDIF
     WITH nocounter
    ;end select
    IF (size(chargelist->objarray,5) > 0)
     SELECT INTO "nl:"
      FROM charge c,
       pft_charge pc
      PLAN (c
       WHERE expand(chrgidx,1,size(chargelist->objarray,5),c.parent_charge_item_id,chargelist->
        objarray[chrgidx].chargeitemid)
      ...
      ORDER BY c.charge_item_id
      DETAIL
       IF (pc.late_chrg_flag=0
        AND  NOT (isequal(c.item_extended_price,0.0)))
        latebalance += c.item_extended_price
       ENDIF
      WITH nocounter
     ;end select
    ENDIF
    SET pramount = latebalance
    CALL logmessage("getLateChargeAmount","Exiting",log_debug)
    RETURN(true)
  END ;Subroutine
 ENDIF
 IF (validate(checkchargelevelpayments,char(128))=char(128))
  DECLARE nchrgcnt = i4 WITH protect, noconstant(0)
  SUBROUTINE (checkchargelevelpayments(pclaimid=f8) =i2)
    CALL logmessage("checkChargeLevelPayments","Entering",log_debug)
    FREE RECORD tempbillchrg
    RECORD tempbillchrg(
      1 objarray[*]
        2 pft_line_item_id = f8
        2 payment_ind = i2
    ...
    SELECT INTO "nl:"
     FROM pft_line_item pli,
      pft_line_item_chrg_reltn plir,
      pft_charge pc,
      charge c
     PLAN (pli
      WHERE pli.corsp_activity_id=pclaimid)
      JOIN (plir
      WHERE plir.pft_line_item_id=pli.pft_line_item_id)
      JOIN (pc
      WHERE pc.pft_charge_id=plir.pft_charge_id)
      JOIN (c
      WHERE c.charge_item_id=pc.charge_item_id)
     HEAD REPORT
      nchrgcnt = 0
     DETAIL
      nchrgcnt += 1
      IF (mod(nchrgcnt,10)=1)
       stat = alterlist(tempbillchrg->objarray,(nchrgcnt+ 9))
      ENDIF
      tempbillchrg->objarray[nchrgcnt].pft_line_item_id = pli.pft_line_item_id, tempbillchrg->
      objarray[nchrgcnt].payment_ind = 0
     FOOT REPORT
      stat = alterlist(tempbillchrg->objarray,nchrgcnt)
     WITH nocounter
    ;end select
    SELECT INTO "nl:"
     FROM (dummyt d  WITH seq = size(tempbillchrg->objarray,5)),
      pft_trans_reltn ptr,
      trans_log tl
     PLAN (d)
      JOIN (ptr
      WHERE (ptr.parent_entity_id=tempbillchrg->objarray[d.seq].pft_line_item_id)
      ...
      (SELECT
       ...
       WHERE ttr2.parent_activity_id=tl.activity_id
        AND ttr2.trans_reltn_reason_cd=cs25753_reversal)))
     DETAIL
      tempbillchrg->objarray[d.seq].payment_ind = 1
     WITH nocounter
    ;end select
    SET pos = locateval(nidx,1,size(tempbillchrg->objarray,5),0,tempbillchrg->objarray[nidx].
     payment_ind)
    IF (pos > 0)
     RETURN(false)
    ENDIF
    RETURN(true)
    CALL logmessage("checkChargeLevelPayments","Exiting",log_debug)
  END ;Subroutine
 ENDIF
 IF (validate(getcodevaluelc,char(128))=char(128))
  SUBROUTINE (getcodevaluelc(code_set=i4,cdf_meaning=vc,option_flag=i2) =f8)
    SET s_cdf_meaning = cdf_meaning
    SET s_code_value = 0.0
    SET stat = uar_get_meaning_by_codeset(code_set,s_cdf_meaning,1,s_code_value)
    IF (((stat != 0) OR (s_code_value <= 0.0)) )
     ...
     CASE (option_flag)
      OF 0:
       SET table_name = build("ERROR-->GetCodeValueLC (",code_set,",",'"',s_cdf_meaning,
        '"',",",option_flag,") not found, CURPROG [",curprog,
        "]")
       ...
       GO TO exit_script
      OF 1:
       SET table_name = build("INFO-->GetCodeValueLC (",code_set,",",'"',s_cdf_meaning,
        '"',",",option_flag,") not found, CURPROG [",curprog,
        "]")
       CALL echo(table_name)
      OF 2:
     ...
     ENDCASE
    ELSE
     CALL echo(build("SUCCESS-->GetCodeValueLC (",code_set,",",'"',s_cdf_meaning,
       '"',",",option_flag,") CODE_VALUE [",s_code_value,
       "]"))
    ENDIF
    RETURN(s_code_value)
  END ;Subroutine
 ENDIF
 IF (validate(chargehasbeenbilled,char(128))=char(128))
  SUBROUTINE (chargehasbeenbilled(pftchrgid=f8) =i2)
    SELECT INTO "NL:"
     FROM pft_line_item_chrg_reltn lir
     WHERE lir.pft_charge_id=pftchrgid
     WITH nocounter
    ;end select
    IF (curqual > 0)
     RETURN(true)
  ...
  END ;Subroutine
 ENDIF
 IF (validate(chargehasbeencredited,char(128))=char(128))
  SUBROUTINE (chargehasbeencredited(chrgid=f8,latechargeslist=vc(ref)) =i2)
    SELECT INTO "NL:"
     FROM (dummyt d  WITH seq = value(size(latechargeslist->latechargeclaim[1].charge_list,5)))
     PLAN (d
      WHERE (latechargeslist->latechargeclaim[1].charge_list[d.seq].parentchrgid=chrgid)
       AND (latechargeslist->latechargeclaim[1].charge_list[d.seq].chrgtypecd=cs13028_cr))
     WITH nocounter
    ;end select
    IF (curqual > 0)
     RETURN(true)
  ...
  END ;Subroutine
 ENDIF
 IF (validate(chargehasbeenadjustedorpaid,char(128))=char(128))
  SUBROUTINE (chargehasbeenadjustedorpaid(pftchrgid=f8) =i2)
    DECLARE totalreversalamt = f8 WITH noconstant(0.0)
    DECLARE transisfullreversal = i2 WITH noconstant(false)
    SELECT INTO "NL:"
     FROM pft_trans_reltn ptr,
      trans_log tl,
      trans_trans_reltn ttr
     PLAN (ptr
      WHERE ptr.parent_entity_id=pftchrgid
       AND ptr.parent_entity_name="PFTCHARGE"
       AND ptr.active_ind=true)
      JOIN (tl
      WHERE tl.activity_id=ptr.activity_id
       AND tl.active_ind=true
       AND tl.trans_type_cd IN (cs18649_adjust, cs18649_payment))
      JOIN (ttr
      WHERE (ttr.parent_activity_id= Outerjoin(tl.activity_id))
       AND (ttr.active_ind= Outerjoin(true)) )
     DETAIL
      IF ( NOT (ttr.trans_trans_reltn_id IN (0.0, null)))
       IF (ttr.trans_reltn_reason_cd=cs25753_reversal)
        totalreversalamt += ttr.amount
        IF (totalreversalamt=tl.total_trans_amount)
         transisfullreversal = true
        ENDIF
       ENDIF
      ENDIF
     WITH nocounter
  ...
  END ;Subroutine
 ENDIF
 CALL echo("Begin including AFC_MODIFY_CHARGE.INC, version [CHARGSRV-15575.005]")
 ...
 END ;Subroutine
 RECORD pendchgmodrec(
   1 charge_mods[*]
     2 charge_mod_type_cd = f8
     2 field1_id = f8
     2 field2_id = f8
     2 field6 = vc
     2 find_reply_index = i4
     2 added = i2
 ) WITH protect
 RECORD newdebitchargerec(
   1 objarray[*]
     2 charge_item_id = f8
     2 parent_charge_item_id = f8
     2 charge_event_act_id = f8
 ...
 RECORD chargemodifiersdetail(
   1 modifierslist[*]
     2 field6 = vc
     2 field2_id = f8
     2 charge_mod_id = f8
 ) WITH protect
 IF ( NOT (validate(reprocesssettings->no_reprocess)))
  RECORD reprocesssettings(
    1 no_reprocess = i2
  ) WITH protect
  SET reprocesssettings->no_reprocess = 1
 ELSE
  SET reprocesssettings->no_reprocess = 1
 ENDIF
 DECLARE inactivate_parent_event = vc WITH protect, constant(
  "INACTIVATE PARENT EVENT ICD AND MODIFIERS")
 DECLARE max_quantity_string = vc WITH protect, constant("MAX QUANTITY")
 DECLARE min_quantity_string = vc WITH protect, constant("MIN QUANTITY")
 DECLARE icd_princ_diag_type = vc WITH protect, constant("ICD PRINCIPAL DIAGNOSIS TYPE")
 DECLARE icd_second_diag_type = vc WITH protect, constant("SECONDARY ICD PRINCIPAL DIAGNOSIS TYPE")
 DECLARE icd_princ_proc_type = vc WITH protect, constant("ICD PRINCIPAL PROCEDURE TYPE")
 DECLARE icd_second_proc_type = vc WITH protect, constant("SECONDARY ICD PRINCIPAL PROCEDURE TYPE")
 DECLARE cvs14002_afc_schedule_type = f8 WITH protect, constant(14002.0)
 DECLARE perf_loc_codeset = i4 WITH protect, constant(220)
 DECLARE abn_codeset = i4 WITH protect, constant(17969)
 DECLARE suspense_reason_codeset = i4 WITH protect, constant(1309)
 DECLARE cpt_modifier_codeset = i4 WITH protect, constant(17769)
 DECLARE revenue_code_codeset = i4 WITH protect, constant(20769)
 DECLARE flex_field_codeset = i4 WITH protect, constant(4002352)
 DECLARE charge_mod_source_cd_codeset = i4 WITH protect, constant(4518006)
 DECLARE unit_of_measure_codeset = i4 WITH protect, constant(54)
 DECLARE service_date_beg_of_time = dq8 WITH protect, constant(cnvtdatetime("01-JAN-1990 00:00:00"))
 DECLARE service_date_end_of_time = dq8 WITH protect, constant(cnvtdatetime("31-DEC-2100 23:59:59"))
 DECLARE max_quantity = f8 WITH protect, noconstant(0.0)
 DECLARE min_quantity = f8 WITH protect, noconstant(0.0)
 DECLARE newrendphys = f8 WITH noconstant(0.0)
 DECLARE newordphys = f8 WITH noconstant(0.0)
 DECLARE newservicedttm = f8 WITH noconstant(cnvtdatetime(0.0))
 DECLARE newperfloccd = f8 WITH noconstant(0.0)
 DECLARE newchargedesc = c200 WITH noconstant("")
 DECLARE newitemquantity = f8 WITH noconstant(0.0)
 DECLARE newitemprice = f8 WITH noconstant(0.0)
 DECLARE newitemextendedprice = f8 WITH noconstant(0.0)
 DECLARE newpatientrespflag = i2 WITH noconstant(0)
 DECLARE newabnstatuscd = f8 WITH noconstant(0.0)
 DECLARE neworiginalorgid = f8 WITH noconstant(0.0)
 DECLARE newsuspreasoncd = f8 WITH noconstant(0.0)
 DECLARE newreasoncomment = c200 WITH noconstant("")
 ...
 IF ( NOT (validate(cs13028_credit_cd)))
  DECLARE cs13028_credit_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",13028,"CR"))
 ENDIF
 IF ( NOT (validate(cs13029_ordphys_cd)))
  DECLARE cs13029_ordphys_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",13029,"ORDERED"))
 ENDIF
 IF ( NOT (validate(cs13029_verphys_cd)))
  DECLARE cs13029_verphys_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",13029,"VERIFIED"))
 ENDIF
 IF ( NOT (validate(cs23549_chargeicd9_cd)))
  DECLARE cs23549_chargeicd9_cd = f8 WITH protect, constant(getcodevalue(23549,"CHARGEICD9",0))
 ENDIF
 IF ( NOT (validate(cs4001989_modify_cd)))
  DECLARE cs4001989_modify_cd = f8 WITH protect, constant(getcodevalue(4001989,"MODIFY",0))
 ENDIF
 IF ( NOT (validate(cs400_cpt_cd)))
  DECLARE cs400_cpt_cd = f8 WITH protect, constant(getcodevalue(400,"CPT4",0))
 ENDIF
 ...
 ENDIF
 IF ( NOT (validate(primary_cs400_icd_diag_cd)))
  DECLARE primary_cs400_icd_diag_cd = f8 WITH protect, noconstant(0.0)
 ENDIF
 IF ( NOT (validate(secondary_cs400_icd_diag_cd)))
  DECLARE secondary_cs400_icd_diag_cd = f8 WITH protect, noconstant(0.0)
 ENDIF
 IF ( NOT (validate(primary_cs400_icd_proc_cd)))
  DECLARE primary_cs400_icd_proc_cd = f8 WITH protect, noconstant(0.0)
 ENDIF
 IF ( NOT (validate(secondary_cs400_icd_proc_cd)))
  DECLARE secondary_cs400_icd_proc_cd = f8 WITH protect, noconstant(0.0)
 ENDIF
 IF ( NOT (validate(inactivate_parent_event_cd)))
  DECLARE inactivate_parent_event_cd = vc WITH protect, noconstant("")
 ENDIF
 DECLARE pending_flg = i4 WITH protect, constant(0)
 DECLARE suspended_flg = i4 WITH protect, constant(1)
 DECLARE review_flg = i4 WITH protect, constant(2)
 DECLARE held_flg = i4 WITH protect, constant(3)
 DECLARE manual_flg = i4 WITH protect, constant(4)
 DECLARE abnrequired_flg = i4 WITH protect, constant(8)
 DECLARE posted_flg = i4 WITH protect, constant(100)
 DECLARE interfaced_flg = i4 WITH protect, constant(999)
 DECLARE reprocesschargeind = i2 WITH protect, noconstant(false)
 SUBROUTINE (initializedata(dummyvar=i2) =null)
   CALL logmsg(curprog,"Entering initializeData",log_debug)
   SET stat = initrec(valchgmodrequest)
   SET stat = initrec(valchgmodrep)
   CALL logmsg(curprog,"Exiting initializeData",log_debug)
 END ;Subroutine
 SUBROUTINE (getsitepreferences(logicaldomainid=f8) =i2)
   CALL logmsg(curprog,"Entering getSitePreferences",log_debug)
   DECLARE prefcount = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain="CHARGE SERVICES"
     AND di.info_name=inactivate_parent_event
     AND di.info_domain_id=logicaldomainid
    DETAIL
     inactivate_parent_event_cd = di.info_char
    WITH nocounter
   ;end select
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain="CHARGE SERVICES"
     AND di.info_name IN (max_quantity_string, min_quantity_string, icd_princ_diag_type,
    icd_second_diag_type, icd_princ_proc_type,
    icd_second_proc_type)
     AND di.info_domain_id=logicaldomainid
    DETAIL
     prefcount += 1
     IF (di.info_name=max_quantity_string)
      max_quantity = di.info_number
     ELSEIF (di.info_name=min_quantity_string)
      min_quantity = di.info_number
     ELSEIF (di.info_name=icd_princ_diag_type)
      primary_cs400_icd_diag_cd = di.info_long_id
     ELSEIF (di.info_name=icd_second_diag_type)
      secondary_cs400_icd_diag_cd = di.info_long_id
     ELSEIF (di.info_name=icd_princ_proc_type)
      primary_cs400_icd_proc_cd = di.info_long_id
     ELSEIF (di.info_name=icd_second_proc_type)
      secondary_cs400_icd_proc_cd = di.info_long_id
     ENDIF
    WITH nocounter
   ;end select
   IF (((prefcount != 6) OR (((max_quantity=0) OR (((primary_cs400_icd_diag_cd=0) OR (
   primary_cs400_icd_proc_cd=0)) )) )) )
    CALL logmsg(curprog,"Exiting getSitePreferences, failed to obtain preferences",log_debug)
    RETURN(false)
   ENDIF
   CALL logmsg(curprog,"Exiting getSitePreferences",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (modifypendingcharge(idx0=i4,chargefindreply=vc(ref)) =i2)
   CALL logmsg(curprog,"Entering modifyPendingCharge",log_debug)
   DECLARE idx1 = i4 WITH protect, noconstant(0)
   DECLARE chrgcnt = i4 WITH noconstant(0)
   DECLARE idx3 = i4 WITH noconstant(0)
   SET reprocesschargeind = false
   CALL initializedata(null)
   IF (validate(request->charge[idx0].no_upt_for_charge,0)=0)
    IF ( NOT (updatecharge(idx0,chargefindreply)))
     CALL logmsg(curprog,"Exiting modifyPendingCharge, failed to update the charge",log_debug)
     CALL writeerrortoreply(idx0,"modifyPendingCharge","updateCharge returned False",true)
     RETURN(false)
    ENDIF
   ELSE
    SET chrgcnt = (size(reply->charge[idx0].new_charges,5)+ 1)
    SET stat = alterlist(reply->charge[idx0].new_charges,chrgcnt)
    SET reply->charge[idx0].new_charges[chrgcnt].new_charge_item_id = request->charge[idx0].
    charge_item_id
    SET reply->charge[idx0].new_charges[chrgcnt].charge_type_cd = cs13028_debit_cd
    SET reply->charge[idx0].new_charges[chrgcnt].action_type = "NOP"
   ENDIF
   IF (size(request->charge[idx0].charge_mod,5) > 0)
    CALL checkforchargeeventmodid(idx0)
    FOR (idx1 = 1 TO size(request->charge[idx0].charge_mod,5))
      CASE (request->charge[idx0].charge_mod[idx1].action_type)
       OF "ADD":
        CALL addchargemodrec(idx0,idx1,request->charge[idx0].charge_item_id)
       OF "UPT":
        CALL adduptchrgmodrec(idx0,idx1,request->charge[idx0].charge_item_id)
       OF "DEL":
        CALL adddelchrgmodrec(idx0,idx1)
      ENDCASE
    ENDFOR
   ENDIF
   IF (validate(request->charge[idx0].no_upt_for_charge,0) != 0)
    IF (((newsuspreasoncd > 0) OR (trim(newreasoncomment) != ""))
     AND cs4001989_modify_cd > 0)
     CALL addreasoncommentmod(request->charge[idx0].charge_item_id)
    ENDIF
   ENDIF
   IF (size(valchgmodrequest->objarray,5) > 0)
    IF (validate(debug,- (1)) > 0)
     CALL echo("valChargeModReq before adjust Priorities")
     CALL echorecord(valchgmodrequest)
    ENDIF
    CALL adjustpriorities(true)
    IF (validate(debug,- (1)) > 0)
     CALL echo("valChargeModReq after adjust Priorities")
     CALL echorecord(valchgmodrequest)
    ENDIF
    IF ( NOT (updatenomenentityrelation(chargefindreply,valchgmodrequest,request->charge[idx0].
     charge_item_id,true)))
     CALL logmsg(curprog,"Exiting modifyPendingCharge, failed to update nomen_entity_reltn",log_debug
      )
     RETURN(false)
    ENDIF
    IF ( NOT (modifychargemods(null)))
     CALL logmsg(curprog,"Exiting modifyPendingCharge, failed to modify charge mods",log_debug)
     CALL writeerrortoreply(idx0,"modifyPendingCharge","modifyChargeMods returned false",true)
     RETURN(false)
    ENDIF
   ENDIF
   IF ( NOT (updatechargeeventmod(idx0,chargefindreply->charge_items[1].charge_event_id)))
    CALL logmsg(curprog,
     "Exiting modifyPendingCharge, failed to update new charge's charge event mods",log_debug)
    CALL writeerrortoreply(idx0,"modifyPendingCharge","updateChargeEventMod returned false",true)
    RETURN(false)
   ENDIF
   IF ( NOT (updatephysandabnstatusresearchaccount(idx0)))
    CALL logmsg(curprog,"Exiting modifyPendingCharge, failed to update physicians",log_debug)
    CALL writeerrortoreply(idx0,"modifyPendingCharge",
     "updatePhysandABNStatusResearchAccount returned false",true)
    RETURN(false)
   ENDIF
   SET reprocesschargeind = evaluatechargeforreprocess(request,idx0,chargefindreply)
   IF (reprocesschargeind=true)
    IF (validate(debug,- (1)) > 0)
     CALL echo("Reprocess charge set to true")
    ENDIF
    IF ( NOT (creditrelatedaddonsbeforereprocessing(request->charge[idx0].charge_item_id,
     newreasoncomment,newsuspreasoncd)))
     CALL logmsg(curprog,"Exiting modifyPendingCharge, failed to credit the charges related addons",
      log_debug)
    ENDIF
    FOR (idx3 = 1 TO size(afcaddcreditreply->charge,5))
      IF (validate(debug,- (1)) > 0)
       CALL echo("related credit charge item id")
       CALL echo(afcaddcreditreply->charge[idx3].charge_item_id)
      ENDIF
      SET newcharges->numberofcharges += 1
      SET stat = alterlist(newcharges->charges,newcharges->numberofcharges)
      SET newcharges->charges[newcharges->numberofcharges].charge_item_id = afcaddcreditreply->
      charge[idx3].charge_item_id
      SET newcharges->charges[newcharges->numberofcharges].process_flg = 0
      SET chrgcnt = (size(reply->charge[idx0].new_charges,5)+ 1)
      SET stat = alterlist(reply->charge[idx0].new_charges,chrgcnt)
      SET reply->charge[idx0].new_charges[chrgcnt].new_charge_item_id = afcaddcreditreply->charge[
      idx3].charge_item_id
      SET reply->charge[idx0].new_charges[chrgcnt].charge_type_cd = cs13028_credit_cd
      SET reply->charge[idx0].new_charges[chrgcnt].action_type = "ADD"
      IF ((request->contributor_system_cd > 0))
       CALL addcontributorsystemmod(afcaddcreditreply->charge[idx3].charge_item_id)
      ENDIF
    ENDFOR
    IF ( NOT (performchargereprocess(idx0,request->charge[idx0].charge_item_id,chargefindreply->
     charge_items[1].charge_event_id)))
     CALL logmsg(curprog,"Exiting modifyPendingCharge, failed to reprocess",log_debug)
     CALL writeerrortoreply(idx0,"modifyPendingCharge","performChargeReprocess returned false",true)
     RETURN(false)
    ENDIF
   ENDIF
   IF ((request->contributor_system_cd > 0))
    CALL addcontributorsystemmod(request->charge[idx0].charge_item_id)
   ENDIF
   CALL logmsg(curprog,"Exiting modifyPendingCharge, returning success",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (modifypostedcharge(idx0=i4,chargefindreply=vc(ref),newcreditid=f8(ref),newdebitid=f8(ref
   )) =i2)
   CALL logmsg(curprog,"Entering modifyPostedCharge",log_debug)
   SET reprocesschargeind = false
   DECLARE newdebitchargeid = f8 WITH protect, noconstant(0.0)
   DECLARE newcreditchargeid = f8 WITH protect, noconstant(0.0)
   CALL initializedata(null)
   SET reprocesschargeind = evaluatechargeforreprocess(request,idx0,chargefindreply)
   IF ( NOT (createcreditcharge(idx0,reprocesschargeind,newcreditchargeid)))
    CALL logmsg(curprog,"Exiting modifyPostedCharge, failed to credit charge",log_debug)
    CALL writeerrortoreply(idx0,"modifyPostedCharge","createCreditCharge returned false",true)
    RETURN(false)
   ENDIF
   IF ( NOT (createdebitcharge(idx0,chargefindreply,newdebitchargeid)))
    CALL logmsg(curprog,"Exiting modifyPostedCharge, failed to add new debit charge",log_debug)
    CALL writeerrortoreply(idx0,"modifyPostedCharge","createDebitCharge returned false",true)
    RETURN(false)
   ENDIF
   IF ( NOT (updatephysandabnstatusresearchaccount(idx0)))
    CALL logmsg(curprog,"Exiting modifyPostedCharge, failed to update physicians",log_debug)
    CALL writeerrortoreply(idx0,"modifyPostedCharge",
     "updatePhysandABNStatusResearchAccount returned false",true)
    RETURN(false)
   ENDIF
   FOR (idx1 = 1 TO size(request->charge[idx0].charge_mod,5))
     IF ((request->charge[idx0].charge_mod[idx1].action_type != "DEL")
      AND (request->charge[idx0].charge_mod[idx1].action_type != "EXI")
      AND (request->charge[idx0].charge_mod[idx1].charge_mod_type_cd != cs13019_mod_rsn_cd))
      CALL addchargemodrec(idx0,idx1,newdebitchargeid)
     ENDIF
   ENDFOR
   FOR (idx1 = 1 TO size(chargefindreply->charge_items[1].charge_mods,5))
     IF ( NOT ((chargefindreply->charge_items[1].charge_mods[idx1].charge_mod_type_cd IN (
     cs13019_mod_rsn_cd, cs13019_suspense_cd)))
      AND (chargefindreply->charge_items[1].charge_mods[idx1].active_ind=1))
      CALL addoriginalchargemod(idx0,idx1,newdebitchargeid,chargefindreply)
     ENDIF
   ENDFOR
   CALL checkforchargeeventmodid(idx0)
   IF (((newsuspreasoncd > 0) OR (trim(newreasoncomment) != ""))
    AND cs4001989_modify_cd > 0)
    CALL copyexistingreasoncommentmods(chargefindreply,newdebitchargeid)
    CALL addreasoncommentmod(newdebitchargeid)
   ENDIF
   IF (size(valchgmodrequest->objarray,5) > 0)
    IF (validate(debug,- (1)) > 0)
     CALL echo("valChargeModReq before adjust Priorities")
     CALL echorecord(valchgmodrequest)
    ENDIF
    CALL adjustpriorities(false)
    IF (validate(debug,- (1)) > 0)
     CALL echo("valChargeModReq after adjust Priorities")
     CALL echorecord(valchgmodrequest)
    ENDIF
    IF ( NOT (modifychargemods(null)))
     CALL logmsg(curprog,"Exiting modifyPostedCharge, failed to add charge mods",log_debug)
     CALL writeerrortoreply(idx0,"modifyPostedCharge","modifyChargeMods returned false",true)
     RETURN(false)
    ENDIF
   ENDIF
   IF ( NOT (updatechargeeventmod(idx0,chargefindreply->charge_items[1].charge_event_id)))
    CALL logmsg(curprog,"Exiting modifyPostedCharge, failed to update new charge's charge event mods",
     log_debug)
    CALL writeerrortoreply(idx0,"modifyPostedCharge","updateChargeEventMod returned false",true)
    RETURN(false)
   ENDIF
   IF (newdebitchargeid > 0.0
    AND reprocesschargeind)
    IF (validate(debug,- (1)) > 0)
     CALL echo("Reprocess charge set to true")
    ENDIF
    IF ( NOT (performchargereprocess(idx0,newdebitchargeid,chargefindreply->charge_items[1].
     charge_event_id)))
     IF (validate(debug,- (1)) > 0)
      CALL echo("Charge reprocess failed")
     ENDIF
     CALL logmsg(curprog,"Exiting modifyPostedCharge, failed to reprocess charge",log_debug)
     CALL writeerrortoreply(idx0,"modifyPostedCharge","performChargeReprocess returned false",true)
     RETURN(false)
    ENDIF
   ELSEIF (newdebitchargeid > 0)
    IF ((request->contributor_system_cd > 0))
     CALL addcontributorsystemmod(newdebitchargeid)
    ENDIF
   ENDIF
   IF ( NOT (updatenomenentityrelation(chargefindreply,valchgmodrequest,newdebitchargeid,false)))
    CALL logmsg(curprog,"Exiting modifyPostedCharge, failed to update nomen_entity_reltn",log_debug)
    CALL writeerrortoreply(idx0,"modifyPostedCharge","updateNomenEntityRelation returned false",true)
    RETURN(false)
   ENDIF
   CALL logmsg(curprog,"Calling into saveChangeLogDetails",log_debug)
   CALL savechangelogdetails(request->charge[idx0].charge_item_id,newdebitchargeid)
   SET newcreditid = newcreditchargeid
   SET newdebitid = newdebitchargeid
   CALL logmsg(curprog,"Exiting modifyPostedCharge, returning success",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (modifysuspendedcharge(idx0=i4,chargefindreply=vc(ref)) =i2)
   CALL logmsg(curprog,"Entering modifySuspendedCharge",log_debug)
   IF (validate(debug,- (1)) > 0)
    CALL echo("Updating Suspended charge")
   ENDIF
   CALL initializedata(null)
   IF ( NOT (updatecharge(idx0,chargefindreply)))
    CALL logmsg(curprog,"Exiting modifySuspendedCharge, failed to update the charge",log_debug)
    CALL writeerrortoreply(idx0,"modifySuspendedCharge","updateCharge returned false",true)
    RETURN(false)
   ENDIF
   IF (validate(debug,- (1)) > 0)
    CALL echo("Releasing Suspended charge")
   ENDIF
   IF ( NOT (performchargereprocess(idx0,request->charge[idx0].charge_item_id,chargefindreply->
    charge_items[1].charge_event_id)))
    IF (validate(debug,- (1)) > 0)
     CALL echo("Charge reprocess failed")
    ENDIF
    CALL logmsg(curprog,"Exiting modifySuspendedCharge, failed to reprocess charge",log_debug)
    RETURN(false)
   ENDIF
   IF ((request->contributor_system_cd > 0))
    CALL addcontributorsystemmod(request->charge[idx0].charge_item_id)
   ...
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (modifyresearchacct(idx0=i4,chargefindreply=vc(ref)) =i2)
   CALL logmsg(curprog,"Entering modifyResearchAcct",log_debug)
   DECLARE necessarymodificationfound = i2 WITH protect, noconstant(false)
   DECLARE oldresearchacct = f8 WITH protect, noconstant(0.0)
   DECLARE newresearchacct = f8 WITH protect, noconstant(0.0)
   IF (validate(debug,- (1)) > 0)
    CALL echo("Updating Research Account on charge")
   ENDIF
   CALL initializedata(null)
   IF ((request->charge[idx0].ord_phys_id=0)
    AND (request->charge[idx0].verify_phys_id=0)
    AND (request->charge[idx0].perf_loc_cd=0)
    AND (request->charge[idx0].abn_status_cd=0)
    AND (request->charge[idx0].item_quantity=0)
    AND (request->charge[idx0].item_price=0)
    AND (request->charge[idx0].patient_responsibility_flag=0)
    AND (request->charge[idx0].no_upt_for_charge=0)
    AND size(request->charge[idx0].charge_mod,5)=0
    AND (request->charge[idx0].service_dt_tm=0)
    AND (request->charge[idx0].charge_description=""))
    IF ((request->charge[idx0].research_acct_id > 0)
     AND (request->charge[idx0].research_acct_id != chargefindreply->charge_items[1].research_acct_id
    ))
     IF (isvalidresearchaccount(request->charge[idx0].research_acct_id))
      SET newresearchacct = request->charge[idx0].research_acct_id
     ELSE
      CALL logmsg(curprog,"Exiting modifyResearchAcct, invalid research_acct_id provided",log_debug)
      CALL writeerrortoreply(idx0,"modifyResearchAcct","isValidResearchAccount returned false",true)
      RETURN(false)
     ENDIF
    ELSEIF ((request->charge[idx0].research_acct_id > 0)
     AND (request->charge[idx0].research_acct_id=chargefindreply->charge_items[1].research_acct_id))
     CALL logmsg(curprog,"Exiting modifyResearchAcct, existing research_acct_id provided",log_debug)
     CALL writeerrortoreply(idx0,"modifyResearchAcct",
      "requested research account is the same as existing",true)
     RETURN(false)
    ELSEIF ((request->charge[idx0].research_acct_id=- (1))
     AND (chargefindreply->charge_items[1].research_acct_id > 0))
     SET newresearchacct = 0.0
    ENDIF
   ELSE
    CALL logmsg(curprog,
     "Exiting modifyResearchAcct, Research Account cannot be modified along with other attributes",
     log_debug)
    CALL writeerrortoreply(idx0,"modifyResearchAcct",
     "Research Account cannot be modified along with other attributes",true)
    RETURN(false)
   ENDIF
   SELECT INTO "nl:"
    FROM charge_event ce
    WHERE (ce.charge_event_id=chargefindreply->charge_items[1].charge_event_id)
    DETAIL
     oldresearchacct = ce.research_account_id
    WITH nocounter
   ;end select
   IF ( NOT (updatephysandabnstatusresearchaccount(idx0)))
    CALL logmsg(curprog,
     "Exiting modifyResearchAcct, failed to update research account on charge_event",log_debug)
    CALL writeerrortoreply(idx0,"modifyResearchAcct",
     "updatePhysandABNStatusResearchAccount returned False",true)
    RETURN(false)
   ENDIF
   IF ((request->nocommitind=0))
    IF (validate(debug,- (1)) > 0)
     CALL echo("modifyResearchAcct - committing before calling to server")
    ENDIF
    COMMIT
    SET changescommitted = true
    IF (reprocessresearchacct(chargefindreply->charge_items[1].charge_event_act_id,chargefindreply->
     charge_items[1].charge_event_id,idx0))
     IF (validate(debug,- (1)) > 0)
      CALL echo("modifyResearchAcct - Server call Success.")
     ENDIF
    ELSE
     IF (validate(debug,- (1)) > 0)
      CALL echo("modifyResearchAcct - Server call failed. Rolling back changes")
     ENDIF
     ROLLBACK
     UPDATE  FROM charge_event ce
      SET ce.research_account_id = oldresearchacct, ce.updt_cnt = (ce.updt_cnt+ 1), ce.updt_dt_tm =
     ...
     ;end update
     IF (validate(debug,- (1)) > 0)
      CALL echo("modifyResearchAcct - committing after resetting research account on charge event")
     ENDIF
     COMMIT
     SET changescommitted = true
     CALL logmsg(curprog,"modifyResearchAcct - Failed to modify research account.",log_debug)
     CALL writeerrortoreply(idx0,"modifyResearchAcct","reprocessResearchAcct returned False",true)
     RETURN(false)
    ENDIF
   ELSE
    CALL logmsg(curprog,"NoCommitInd != 0. Skipping call to server in modifyResearchAcct.",log_debug)
   ENDIF
   CALL logmsg(curprog,"modifyResearchAcct - Successfully modified research account.",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (reprocessresearchacct(ceaid=f8,ceid=f8,idx0=i4) =i2)
   CALL logmsg(curprog,"reprocessResearchAcct - Entering reprocessResearchAcct.",log_debug)
   ...
   DECLARE ichargemodcnt = i4 WITH protect, noconstant(0)
   ...
   IF (stat != 0)
    ...
    CALL writeerrortoreply(idx0,"reprocessResearchAcct",
     "Failed to find CDFMeaning REPROCESS on codeset 13029",true)
    RETURN(false)
   ENDIF
   ...
   SELECT INTO "nl:"
    ...
    PLAN (ce
     ...
     WHERE c.charge_event_id=ce.charge_event_id
      ...
      AND c.active_ind=1)
    DETAIL
     existingchrgcnt += 1, stat = alterlist(existingcharges->charge,existingchrgcnt), existingcharges
     ->charge[existingchrgcnt].charge_item_id = c.charge_item_id
    WITH nocounter
   ;end select
   CALL impersonatepersonnelinfo(1)
   SET iret = uar_crmbeginapp(ireprocessappid,happreprocess)
   IF (iret != 0)
    CALL logmsg(curprog,"reprocessResearchAcct - Failed to begin App.",log_debug)
    RETURN(false)
   ENDIF
   SET iret = uar_crmbegintask(happreprocess,ireprocesstaskid,htaskreprocess)
   IF (iret != 0)
    CALL logmsg(curprog,"reprocessResearchAcct - Failed to begin Task.",log_debug)
    CALL uar_crmendapp(happreprocess)
    RETURN(false)
   ENDIF
   SET iret = uar_crmbeginreq(htaskreprocess,"",ireprocessreqid,hstepreprocess)
   IF (iret != 0)
    CALL logmsg(curprog,"reprocessResearchAcct - Failed to begin Step.",log_debug)
   ...
   IF (hreq=0)
    CALL logmsg(curprog,"reprocessResearchAcct - Failed to get request handle",log_debug)
   ...
   IF (hlist=0)
    CALL logmsg(curprog,"reprocessResearchAcct - Failed to get process_event handle",log_debug)
    CALL uar_crmendreq(hstepreprocess)
    CALL uar_crmendtask(htaskreprocess)
    CALL uar_crmendapp(happreprocess)
    RETURN(false)
   ENDIF
   SET stat = uar_srvsetdouble(hlist,"charge_event_id",ceid)
   SET hlist2 = uar_srvadditem(hlist,"charge_acts")
   IF (hlist2=0)
    CALL logmsg(curprog,"reprocessResearchAcct - Failed to get charge_acts handle",log_debug)
    CALL uar_crmendreq(hstepreprocess)
    CALL uar_crmendtask(htaskreprocess)
    CALL uar_crmendapp(happreprocess)
    RETURN(false)
   ENDIF
   SET stat = uar_srvsetdouble(hlist2,"charge_event_act_id",ceaid)
   SET iret = uar_crmperform(hstepreprocess)
   IF (iret != 0)
    CALL logmsg(curprog,build("reprocessResearchAcct - Server returned Failure. CRMSTAT:",iret),
     log_debug)
    CALL uar_crmendreq(hstepreprocess)
    CALL uar_crmendtask(htaskreprocess)
    CALL uar_crmendapp(happreprocess)
    CALL writeerrortoreply(idx0,"reprocessResearchAcct",build(
      "reprocessResearchAcct - Server returned Failure. CRMSTAT:",iret),true)
    RETURN(false)
   ENDIF
   SET hreply = uar_crmgetreply(hstepreprocess)
   IF (hreply=0)
    CALL logmsg(curprog,"reprocessResearchAcct - Failed to get handle to Reply",log_debug)
    CALL uar_crmendreq(hstepreprocess)
    CALL uar_crmendtask(htaskreprocess)
    CALL uar_crmendapp(happreprocess)
    RETURN(false)
   ...
   IF (ichargecount=0)
    CALL logmsg(curprog,"reprocessResearchAcct - Reply has no charges in it",log_debug)
    CALL uar_crmendreq(hstepreprocess)
    CALL uar_crmendtask(htaskreprocess)
    CALL uar_crmendapp(happreprocess)
    CALL writeerrortoreply(idx0,"reprocessResearchAcct",
     "Server Reply to request 951359 contains no charges",true)
    RETURN(false)
   ENDIF
   ...
   FOR (ichargeloop = 1 TO ichargecount)
     ...
     IF (hlist=0)
      CALL logmsg(curprog,"reprocessResearchAcct - Failed to get handle to reply's charge",log_debug)
      CALL uar_crmendreq(hstepreprocess)
      CALL uar_crmendtask(htaskreprocess)
      CALL uar_crmendapp(happreprocess)
      RETURN(false)
     ENDIF
     SET reprocessreply->charge[ichargeloop].charge_item_id = uar_srvgetdouble(hlist,"charge_item_id"
      )
     ...
     SET reprocessreply->charge[ichargeloop].charge_type_cd = uar_srvgetdouble(hlist,"charge_type_cd"
      )
     ...
     ELSE
      ...
      SET chrgcnt = (size(reply->charge[idx0].new_charges,5)+ 1)
      SET stat = alterlist(reply->charge[idx0].new_charges,chrgcnt)
      SET reply->charge[idx0].new_charges[chrgcnt].new_charge_item_id = reprocessreply->charge[
      ichargeloop].charge_item_id
      SET reply->charge[idx0].new_charges[chrgcnt].charge_type_cd = reprocessreply->charge[
      ichargeloop].charge_type_cd
      SET reply->charge[idx0].new_charges[chrgcnt].action_type = "ADD"
      IF ((request->contributor_system_cd > 0))
       SET ichargemodcnt += 1
       SET stat = alterlist(addchargemodreqrc->charge_mod,ichargemodcnt)
       SET addchargemodreqrc->charge_mod_qual = ichargemodcnt
       SET addchargemodreqrc->charge_mod[ichargemodcnt].charge_item_id = reprocessreply->charge[
       ichargeloop].charge_item_id
       SET addchargemodreqrc->charge_mod[ichargemodcnt].charge_mod_type_cd = cs13019_other_cd
       SET addchargemodreqrc->charge_mod[ichargemodcnt].field6 = "Contributor System"
       SET addchargemodreqrc->charge_mod[ichargemodcnt].field7 = uar_get_code_display(request->
        contributor_system_cd)
       SET addchargemodreqrc->charge_mod[ichargemodcnt].code1_cd = request->contributor_system_cd
       SET addchargemodreqrc->charge_mod[ichargemodcnt].action_type = "ADD"
      ENDIF
     ENDIF
     SET ichargemodcnt += 1
     SET stat = alterlist(addchargemodreqrc->charge_mod,ichargemodcnt)
     ...
     SET addchargemodreqrc->charge_mod[ichargemodcnt].charge_item_id = reprocessreply->charge[
     ichargeloop].charge_item_id
     SET addchargemodreqrc->charge_mod[ichargemodcnt].charge_mod_type_cd = cs13019_mod_rsn_cd
     SET addchargemodreqrc->charge_mod[ichargemodcnt].field6 = "The research account was modified"
     SET addchargemodreqrc->charge_mod[ichargemodcnt].field7 = request->charge[idx0].reason_comment
     SET addchargemodreqrc->charge_mod[ichargemodcnt].field1_id = cs4001989_modify_cd
     SET addchargemodreqrc->charge_mod[ichargemodcnt].field2_id = request->charge[idx0].
     suspense_rsn_cd
     SET addchargemodreqrc->charge_mod[ichargemodcnt].action_type = "ADD"
   ENDFOR
   ...
   IF ((addchargemodreprc->status_data.status="F"))
    CALL logmsg(curprog,"reprocessResearchAcct - Call to afc_add_charge_mod failed",log_debug)
    CALL uar_crmendreq(hstepreprocess)
    CALL uar_crmendtask(htaskreprocess)
    CALL uar_crmendapp(happreprocess)
    CALL writeerrortoreply(idx0,"reprocessResearchAcct","afc_add_charge_mod returned failure",true)
    RETURN(false)
   ENDIF
   IF (validate(debug,- (1)) > 0)
    CALL logmsg(curprog,"reprocessResearchAcct - Reprocess Research Account reply",log_debug)
   ...
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (isencountervalid(encntrid=f8) =i2)
   CALL logmsg(curprog,"Entering isEncounterValid",log_debug)
   SELECT INTO "nl:"
    FROM encounter e
    PLAN (e
     WHERE e.encntr_id=encntrid
   ...
   ;end select
   IF (curqual=1)
    CALL logmsg(curprog,"Exiting isEncounterValid, encounter is valid",log_debug)
    RETURN(true)
   ENDIF
   CALL logmsg(curprog,"Exiting isEncounterValid, encounter is not valid",log_debug)
   RETURN(false)
 END ;Subroutine
 SUBROUTINE (ispersonvalid(personid=f8) =i2)
   CALL logmsg(curprog,"Entering isPersonValid",log_debug)
   SELECT INTO "nl:"
    FROM person p
    PLAN (p
   ...
   ;end select
   IF (curqual=1)
    CALL logmsg(curprog,"Exiting isPersonValid, person is valid",log_debug)
    RETURN(true)
   ENDIF
   CALL logmsg(curprog,"Exiting isPersonValid, person is not valid",log_debug)
   RETURN(false)
 END ;Subroutine
 SUBROUTINE (isvalidpendingcharge(chargefindreply=vc(ref)) =i2)
   CALL logmsg(curprog,"Entering isValidPendingCharge",log_debug)
   DECLARE charge_process_flg = i4 WITH protect, noconstant(- (1))
   SET charge_process_flg = chargefindreply->charge_items[1].process_flg
   IF ((chargefindreply->charge_items[1].charge_type_cd=cs13028_debit_cd)
    AND (chargefindreply->charge_items[1].offset_charge_item_id=0)
    AND (chargefindreply->charge_items[1].active_ind=1))
    IF (charge_process_flg=pending_flg
     AND (chargefindreply->charge_items[1].interface_file_id > 0))
     SELECT INTO "nl:"
      FROM interface_file i
      PLAN (i
       WHERE (i.interface_file_id=chargefindreply->charge_items[1].interface_file_id)
        AND i.realtime_ind=0
        AND i.profit_type_cd=0)
     ;end select
     IF (curqual=1)
      CALL logmsg(curprog,"Exiting isValidPendingCharge, charge is a valid pending charge",log_debug)
      RETURN(true)
     ELSE
      CALL logmsg(curprog,"Exiting isValidPendingCharge, charge is not valid pending charge",
       log_debug)
      RETURN(false)
     ENDIF
    ELSEIF (((charge_process_flg=review_flg) OR (((charge_process_flg=held_flg) OR (((
    charge_process_flg=manual_flg) OR (charge_process_flg=abnrequired_flg)) )) )) )
     CALL logmsg(curprog,"Exiting isValidPendingCharge, charge is a valid pending charge",log_debug)
     RETURN(true)
    ELSE
     CALL logmsg(curprog,"Exiting isValidPendingCharge, charge is not a valid pending charge",
      log_debug)
     RETURN(false)
    ENDIF
   ELSE
    CALL logmsg(curprog,"Exiting isValidPendingCharge, charge is not a valid debit",log_debug)
    RETURN(false)
   ENDIF
   CALL logmsg(curprog,"Exiting isValidPendingCharge, charge is not a valid Pending Charge",log_debug
    )
   RETURN(false)
 END ;Subroutine
 SUBROUTINE (isvalidpostedcharge(chargefindreply=vc(ref)) =i2)
   CALL logmsg(curprog,"Entering isValidPostedCharge",log_debug)
   DECLARE charge_process_flg = i4 WITH protect, noconstant(chargefindreply->charge_items[1].
    process_flg)
   IF ((chargefindreply->charge_items[1].charge_type_cd=cs13028_debit_cd)
    AND (chargefindreply->charge_items[1].offset_charge_item_id=0)
    AND (chargefindreply->charge_items[1].active_ind=1))
    IF (((charge_process_flg=posted_flg) OR (charge_process_flg=interfaced_flg)) )
     CALL logmsg(curprog,"Exiting isValidPostedCharge, charge is a valid posted/interfaced charge",
      log_debug)
     RETURN(true)
    ENDIF
   ENDIF
   CALL logmsg(curprog,"Exiting isValidPostedCharge, charge is not a valid posted/interfaced charge",
    log_debug)
   RETURN(false)
 END ;Subroutine
 SUBROUTINE (isvalidsuspendedcharge(chargefindreply=vc(ref)) =i2)
   CALL logmsg(curprog,"Entering isValidSuspendedCharge",log_debug)
   DECLARE charge_process_flg = i4 WITH protect, noconstant(chargefindreply->charge_items[1].
    process_flg)
   IF ((chargefindreply->charge_items[1].charge_type_cd=cs13028_debit_cd)
    AND (chargefindreply->charge_items[1].offset_charge_item_id=0)
    AND (chargefindreply->charge_items[1].active_ind=1))
    IF (charge_process_flg=suspended_flg)
     CALL logmsg(curprog,"Exiting isValidSuspendedCharge, charge is a valid suspended charge",
      log_debug)
     RETURN(true)
    ENDIF
   ENDIF
   CALL logmsg(curprog,"Exiting isValidSuspendedCharge, charge is not a valid suspended charge",
    log_debug)
   RETURN(false)
 END ;Subroutine
 SUBROUTINE (isvalidmodification(idx0=i4,chargefindreply=vc(ref)) =i2)
   CALL logmsg(curprog,"Entering isValidModification",log_debug)
   DECLARE necessarymodificationfound = i2 WITH protect, noconstant(false)
   IF ((((request->charge[idx0].ord_phys_id < - (1))) OR ((((request->charge[idx0].verify_phys_id <
   - (1))) OR ((((request->charge[idx0].service_dt_tm < 0)) OR ((((request->charge[idx0].perf_loc_cd
    < - (1))) OR ((((request->charge[idx0].abn_status_cd < - (1))) OR ((((request->charge[idx0].
   research_acct_id < - (1))) OR ((((request->charge[idx0].item_quantity < 0)) OR ((((request->
   charge[idx0].patient_responsibility_flag < - (1))) OR ((request->charge[idx0].item_price < - (1))
   )) )) )) )) )) )) )) )) )
    CALL logmsg(curprog,"Exiting isValidModification, invalid modification request value",log_debug)
    CALL writeerrortoreply(idx0,"isValidModification","Invalid charge attribute in request",true)
    RETURN(false)
   ENDIF
   IF ((chargefindreply->charge_items[1].process_flg=suspended_flg))
    IF ((request->charge[idx0].ord_phys_id=0)
     AND (request->charge[idx0].verify_phys_id=0)
     AND (request->charge[idx0].patient_responsibility_flag=0)
     AND (request->charge[idx0].perf_loc_cd=0)
     AND (request->charge[idx0].abn_status_cd=0)
     AND (request->charge[idx0].research_acct_id=0)
     AND (request->charge[idx0].item_quantity=0)
     AND (request->charge[idx0].item_price=0)
     AND (request->charge[idx0].charge_description="")
     AND (request->charge[idx0].no_upt_for_charge=0)
     AND size(request->charge[idx0].charge_mod,5)=0
     AND (request->charge[idx0].service_dt_tm >= service_date_beg_of_time)
     AND (request->charge[idx0].service_dt_tm <= service_date_end_of_time)
     AND (request->charge[idx0].service_dt_tm != chargefindreply->charge_items[1].service_dt_tm))
     CALL logmsg(curprog,"Exiting isValidModification, Valid Suspended Charge Modification",log_debug
      )
     SET newservicedttm = request->charge[idx0].service_dt_tm
     SET necessarymodificationfound = true
     IF ((((request->charge[idx0].suspense_rsn_cd > 0)) OR (trim(request->charge[idx0].reason_comment
      ) != ""
      AND cs4001989_modify_cd > 0)) )
      IF (isvalidcodevalue(request->charge[idx0].suspense_rsn_cd,suspense_reason_codeset))
       SET newsuspreasoncd = request->charge[idx0].suspense_rsn_cd
      ELSE
       IF (validate(debug,- (1)) > 0)
        CALL echo("Invalid Suspense Reason Code Provided")
       ENDIF
       CALL logmsg(curprog,"Invalid Suspense Reason Code Provided",log_debug)
       SET newsuspreasoncd = 0.0
      ENDIF
      SET newreasoncomment = substring(1,200,trim(request->charge[idx0].reason_comment))
     ELSE
      SET newsuspreasoncd = 0.0
      SET newreasoncomment = ""
     ENDIF
     RETURN(true)
    ELSE
     CALL logmsg(curprog,"Exiting isValidModification, Invalid Suspended Charge Modification",
      log_debug)
     CALL writeerrortoreply(idx0,"isValidModification","Invalid Suspended Charge Modification",true)
     RETURN(false)
    ENDIF
   ENDIF
   IF ((request->charge[idx0].ord_phys_id > 0)
    AND (request->charge[idx0].ord_phys_id != chargefindreply->charge_items[1].ord_phys_id))
    IF (isvalidphysician(request->charge[idx0].ord_phys_id))
     SET necessarymodificationfound = true
     SET newordphys = request->charge[idx0].ord_phys_id
    ELSE
     CALL logmsg(curprog,"Exiting isValidModification, invalid ord_phys_id provided",log_debug)
     CALL writeerrortoreply(idx0,"isValidModification","invalid ord_phys_id provided",true)
     RETURN(false)
    ENDIF
   ELSEIF ((request->charge[idx0].ord_phys_id=- (1))
    AND (chargefindreply->charge_items[1].ord_phys_id > 0))
    SET necessarymodificationfound = true
    SET newordphys = 0.0
   ELSE
    SET newordphys = chargefindreply->charge_items[1].ord_phys_id
   ENDIF
   IF ((request->charge[idx0].verify_phys_id > 0)
    AND (request->charge[idx0].verify_phys_id != chargefindreply->charge_items[1].verify_phys_id))
    IF (isvalidphysician(request->charge[idx0].verify_phys_id))
     SET necessarymodificationfound = true
     SET newrendphys = request->charge[idx0].verify_phys_id
    ELSE
     CALL logmsg(curprog,"Exiting isValidModification, invalid verify_phys_id provided",log_debug)
     CALL writeerrortoreply(idx0,"isValidModification","invalid verify_phys_id provided",true)
     RETURN(false)
    ENDIF
   ELSEIF ((request->charge[idx0].verify_phys_id=- (1))
    AND (chargefindreply->charge_items[1].verify_phys_id > 0))
    SET necessarymodificationfound = true
    SET newrendphys = 0.0
   ELSE
    SET newrendphys = chargefindreply->charge_items[1].verify_phys_id
   ENDIF
   IF ((request->charge[idx0].service_dt_tm > 0)
    AND (request->charge[idx0].service_dt_tm != chargefindreply->charge_items[1].service_dt_tm))
    IF ((request->charge[idx0].service_dt_tm >= service_date_beg_of_time)
     AND (request->charge[idx0].service_dt_tm <= service_date_end_of_time))
     SET necessarymodificationfound = true
     SET newservicedttm = request->charge[idx0].service_dt_tm
     SET neworiginalorgid = determineoriginalorgforcharge(chargefindreply->charge_items[1].encntr_id,
      cnvtdatetime(newservicedttm))
    ELSE
     CALL logmsg(curprog,"Exiting isValidModification, invalid service_dt_tm provided",log_debug)
     CALL writeerrortoreply(idx0,"isValidModification","invalid service_dt_tm provided",true)
     RETURN(false)
    ENDIF
   ELSE
    SET newservicedttm = chargefindreply->charge_items[1].service_dt_tm
    SET neworiginalorgid = chargefindreply->charge_items[1].original_org_id
   ENDIF
   IF ((request->charge[idx0].perf_loc_cd > 0)
    AND (request->charge[idx0].perf_loc_cd != chargefindreply->charge_items[1].perf_loc_cd))
    IF (isvalidcodevalue(request->charge[idx0].perf_loc_cd,perf_loc_codeset))
     SET necessarymodificationfound = true
     SET newperfloccd = request->charge[idx0].perf_loc_cd
    ELSE
     CALL logmsg(curprog,"Exiting isValidModification, invalid perf_loc_cd provided",log_debug)
     CALL writeerrortoreply(idx0,"isValidModification","invalid perf_loc_cd provided",true)
     RETURN(false)
    ENDIF
   ELSEIF ((request->charge[idx0].perf_loc_cd=- (1))
    AND (chargefindreply->charge_items[1].perf_loc_cd > 0))
    SET necessarymodificationfound = true
    SET newperfloccd = 0.0
   ELSE
    SET newperfloccd = chargefindreply->charge_items[1].perf_loc_cd
   ENDIF
   IF ((request->charge[idx0].abn_status_cd > 0)
    AND (request->charge[idx0].abn_status_cd != chargefindreply->charge_items[1].abn_status_cd))
    IF (isvalidcodevalue(request->charge[idx0].abn_status_cd,abn_codeset))
     SET necessarymodificationfound = true
     SET newabnstatuscd = request->charge[idx0].abn_status_cd
    ELSE
     CALL logmsg(curprog,"Exiting isValidModification, invalid abn_status_cd provided",log_debug)
     CALL writeerrortoreply(idx0,"isValidModification","invalid abn_status_cd provided",true)
     RETURN(false)
    ENDIF
   ELSE
    SET newabnstatuscd = chargefindreply->charge_items[1].abn_status_cd
   ENDIF
   IF ( NOT (trim(request->charge[idx0].charge_description) IN ("", null))
    AND (request->charge[idx0].charge_description != chargefindreply->charge_items[1].
   charge_description))
    SET necessarymodificationfound = true
    SET newchargedesc = substring(1,200,request->charge[idx0].charge_description)
   ELSE
    SET newchargedesc = substring(1,200,chargefindreply->charge_items[1].charge_description)
   ENDIF
   IF ((request->charge[idx0].item_quantity > 0)
    AND (request->charge[idx0].item_quantity != chargefindreply->charge_items[1].item_quantity))
    IF ((chargefindreply->charge_items[1].item_interval_id > 0))
     CALL logmsg(curprog,
      "Exiting isValidModification, quanity cannot be changed on an Interval charge",log_debug)
     CALL writeerrortoreply(idx0,"isValidModification",
      "Quantity cannot be changed on an Interval charge with this logic. Use Charge Viewer instead.",
      true)
     RETURN(false)
    ELSEIF ((request->charge[idx0].item_quantity >= min_quantity)
     AND (request->charge[idx0].item_quantity <= max_quantity))
     SET necessarymodificationfound = true
     SET newitemquantity = request->charge[idx0].item_quantity
    ELSE
     CALL logmsg(curprog,"Exiting isValidModification, invalid item_quantity provided",log_debug)
     CALL writeerrortoreply(idx0,"isValidModification","invalid item_quantity provided",true)
     RETURN(false)
    ENDIF
   ELSE
    SET newitemquantity = chargefindreply->charge_items[1].item_quantity
   ENDIF
   IF ((request->charge[idx0].item_price > 0)
    AND (request->charge[idx0].item_price != chargefindreply->charge_items[1].item_price))
    SET necessarymodificationfound = true
    SET newitemprice = request->charge[idx0].item_price
   ELSEIF ((request->charge[idx0].item_price=- (1))
    AND (chargefindreply->charge_items[1].item_price > 0))
    SET necessarymodificationfound = true
    SET newitemprice = 0.0
   ELSE
    SET newitemprice = chargefindreply->charge_items[1].item_price
   ENDIF
   IF ((request->charge[idx0].patient_responsibility_flag > 0)
    AND (request->charge[idx0].patient_responsibility_flag <= 2)
    AND (request->charge[idx0].patient_responsibility_flag != chargefindreply->charge_items[1].
   patient_responsibility_flag))
    SET necessarymodificationfound = true
    SET newpatientrespflag = request->charge[idx0].patient_responsibility_flag
   ELSEIF ( NOT ((request->charge[idx0].patient_responsibility_flag IN (- (1), 0, 1, 2))))
    CALL logmsg(curprog,"Exiting isValidModification, invalid patient_responsibilty_flag provided",
     log_debug)
    CALL writeerrortoreply(idx0,"isValidModification","invalid patient_responsibilty_flag provided",
     true)
    RETURN(false)
   ELSEIF ((request->charge[idx0].patient_responsibility_flag=- (1)))
    SET necessarymodificationfound = true
    SET newpatientrespflag = 0
   ELSE
    SET newpatientrespflag = chargefindreply->charge_items[1].patient_responsibility_flag
   ENDIF
   IF ((((request->charge[idx0].suspense_rsn_cd > 0)) OR (trim(request->charge[idx0].reason_comment)
    != ""
    AND cs4001989_modify_cd > 0)) )
   ...
   ELSE
    ...
    SET newreasoncomment = ""
   ENDIF
   IF ((((newitemquantity != chargefindreply->charge_items[1].item_quantity)) OR ((newitemprice !=
   chargefindreply->charge_items[1].item_price))) )
    SET newitemextendedprice = (newitemquantity * newitemprice)
   ELSE
    SET newitemextendedprice = chargefindreply->charge_items[1].item_extended_price
   ENDIF
   IF (size(request->charge[idx0].charge_mod,5) < 1)
    CALL logmsg(curprog,"Skipping isValidChargeModModification, No Charge Mods passed in",log_debug)
   ELSE
    IF ( NOT (isvalidchargemodmodification(idx0,chargefindreply,necessarymodificationfound)))
     CALL logmsg(curprog,
      "Exiting isValidModification, Charge mod modification requested is not valid",log_debug)
     CALL writeerrortoreply(idx0,"isValidModification","isValidModification returned false",true)
     RETURN(false)
    ENDIF
   ENDIF
   IF (necessarymodificationfound=true)
    CALL logmsg(curprog,"Exiting isValidModification, modification requested is valid",log_debug)
    RETURN(true)
   ENDIF
   CALL logmsg(curprog,
    "Exiting isValidModification, modification requested is not going to change charge details",
    log_debug)
   RETURN(false)
 END ;Subroutine
 SUBROUTINE (isvalidchargemodmodification(idx0=i4,chargefindreply=vc(ref),necessarymodificationfound=
  i2(ref)) =i2)
   DECLARE codeset14002meaning = c12 WITH protect, noconstant("")
   DECLARE cve_field_value = vc WITH protect, noconstant("")
   DECLARE valid_code_value = i2 WITH protect, noconstant(0)
   DECLARE tempcodeset = i4 WITH protect, noconstant(0)
   DECLARE tempsourcecd = f8 WITH protect, noconstant(0)
   DECLARE tempfield3id = f8 WITH protect, noconstant(0)
   DECLARE billcode = vc WITH protect, noconstant("")
   DECLARE billcodedesc = c200 WITH protect, noconstant(fillstring(200," "))
   DECLARE idx = i4 WITH protect, noconstant(0)
   DECLARE searchidx = i4 WITH protect, noconstant(0)
   DECLARE pos = i4 WITH protect, noconstant(0)
   DECLARE pos2 = i4 WITH protect, noconstant(0)
   DECLARE chargemoddeleted = i2 WITH protect, noconstant(true)
   FOR (i = 1 TO size(request->charge[idx0].charge_mod,5))
     SET codeset14002meaning = ""
     SET tempcodeset = 0.0
     SET pos = 0
     SET idx = 0
     SET searchidx = 1
     SET pos2 = 0
     SET chargemoddeleted = true
     SET codeset14002meaning = uar_get_code_meaning(request->charge[idx0].charge_mod[i].field1_id)
     IF ( NOT (codeset14002meaning IN ("ASA", "CDM_SCHED", "CPT4", "HCPCS", "ICD9",
     "MODIFIER", "NDC", "PROCCODE", "REVENUE"))
      AND (request->charge[idx0].charge_mod[i].charge_mod_type_cd != cs13019_flex_cd))
      IF ((request->charge[idx0].charge_mod[i].action_type != "DEL"))
       CALL logmsg(curprog,
        "Exiting isValidChargeModModification, invalid bill code schedule provided",log_debug)
       CALL writeerrortoreply(idx0,"isValidChargeModModification",
        "Invalid charge_mod Field1_id or charge_mod_type_cd",true)
       RETURN(false)
      ENDIF
     ENDIF
     IF ((request->charge[idx0].charge_mod[i].action_type="DEL"))
      IF ((request->charge[idx0].charge_mod[i].charge_mod_type_cd != cs13019_bill_code_cd)
       AND (request->charge[idx0].charge_mod[i].charge_mod_type_cd != cs13019_flex_cd))
       CALL logmsg(curprog,
        "Exiting isValidChargeModModification, Charge Mod Type Code isn't Bill Code or Flex field",
        log_debug)
       CALL writeerrortoreply(idx0,"isValidChargeModModification",
        "Charge_mod_type_cd is not Bill Code or Flex Field for a delete action",true)
       RETURN(false)
      ENDIF
      IF ((request->charge[idx0].charge_mod[i].charge_mod_id <= 0))
       CALL logmsg(curprog,"Exiting isValidChargeModModification, no charge mod to delete provided",
        log_debug)
       CALL writeerrortoreply(idx0,"isValidChargeModModification",
        "Charge_mod_id not provided for a delete action",true)
       RETURN(false)
      ELSEIF ( NOT (ischargemodvalidandactive(request->charge[idx0].charge_mod[i].charge_mod_id)))
       CALL logmsg(curprog,
        "Exiting isChargeModValidAndActive, charge mod provided isn't valid to delete",log_debug)
       CALL writeerrortoreply(idx0,"isValidChargeModModification",
        "Charge_mod_id is not valid for a delete action",true)
       RETURN(false)
      ELSE
       SET necessarymodificationfound = true
      ENDIF
     ELSEIF ((((request->charge[idx0].charge_mod[i].action_type="ADD")) OR ((request->charge[idx0].
     charge_mod[i].action_type="UPT"))) )
      IF ((request->charge[idx0].charge_mod[i].charge_mod_type_cd=cs13019_bill_code_cd))
       IF ((request->charge[idx0].charge_mod[i].field1_id <= 0))
        CALL logmsg(curprog,"Exiting isValidChargeModModification, no bill code schedule provided",
         log_debug)
        CALL writeerrortoreply(idx0,"isValidChargeModModification",
         "Field1_id is not populated for an add/update action",true)
        RETURN(false)
       ENDIF
       IF ((request->charge[idx0].charge_mod[i].field2_id <= 0))
        CALL logmsg(curprog,"Exiting isValidChargeModModification, no bill code priority provided",
         log_debug)
        CALL writeerrortoreply(idx0,"isValidChargeModModification",
         "Field2_id is not populated for an add/update action",true)
        RETURN(false)
       ENDIF
       IF ((request->charge[idx0].charge_mod[i].charge_mod_source_cd <= 0))
        CALL logmsg(curprog,"Exiting isValidChargeModModification, no charge_mod_source_cd provided",
         log_debug)
        CALL writeerrortoreply(idx0,"isValidChargeModModification",
         "Charge_mod_source_cd is not populated for an add/upt action",true)
        RETURN(false)
       ELSEIF ( NOT (isvalidcodevalue(request->charge[idx0].charge_mod[i].charge_mod_source_cd,
        charge_mod_source_cd_codeset)))
        CALL logmsg(curprog,
         "Exiting isValidChargeModModification, invalid charge_mod_source_cd provided",log_debug)
        CALL writeerrortoreply(idx0,"isValidChargeModModification",
         "Charge_mod_source_cd is not valid for an add/upt action",true)
        RETURN(false)
       ENDIF
       IF ((request->charge[idx0].charge_mod[i].action_type="UPT")
        AND (((request->charge[idx0].charge_mod[i].charge_mod_id <= 0)) OR ( NOT (
       ischargemodvalidandactive(request->charge[idx0].charge_mod[i].charge_mod_id)))) )
        CALL logmsg(curprog,
         "Exiting isValidChargeModModification, charge mod provided isn't valid to update",log_debug)
        CALL writeerrortoreply(idx0,"isValidChargeModModification",
         "Charge_mod_id is not valid/populated for an update action",true)
        RETURN(false)
       ELSEIF ((request->charge[idx0].charge_mod[i].action_type="UPT")
        AND (request->charge[idx0].charge_mod[i].charge_mod_id > 0))
        SET pos = locateval(idx,1,size(request->charge[idx0].charge_mod,5),request->charge[idx0].
         charge_mod[i].charge_mod_id,request->charge[idx0].charge_mod[idx].charge_mod_id,
         "DEL",request->charge[idx0].charge_mod[idx].action_type)
        IF (pos > 0)
         CALL logmsg(curprog,
          "Exiting isValidChargeModModification, charge mod provided to update is also being deleted",
          log_debug)
         CALL writeerrortoreply(idx0,"isValidChargeModModification",
          "Charge_mod_id provided to update is also being deleted",true)
         RETURN(false)
        ENDIF
       ENDIF
       IF ((request->charge[idx0].charge_mod[i].action_type="ADD")
        AND (request->charge[idx0].charge_mod[i].charge_mod_id > 0))
        CALL logmsg(curprog,
         "Exiting isValidChargeModModification, charge_mod_id provided on an add request",log_debug)
        CALL writeerrortoreply(idx0,"isValidChargeModModification",
         "Charge_mod_id is populated for an add action",true)
        RETURN(false)
       ENDIF
       IF (codeset14002meaning IN ("MODIFIER", "REVENUE"))
        IF ((request->charge[idx0].charge_mod[i].field3_id <= 0))
         CALL logmsg(curprog,"Exiting isValidChargeModModification, no bill code code value provided",
          log_debug)
         CALL writeerrortoreply(idx0,"isValidChargeModModification",
          "field3_id is not populated for a modifier/revenue Code",true)
         RETURN(false)
        ENDIF
        IF (codeset14002meaning="MODIFIER")
         SET tempcodeset = cpt_modifier_codeset
        ELSE
         SET tempcodeset = revenue_code_codeset
        ENDIF
        IF ( NOT (isvalidcodevalue(request->charge[idx0].charge_mod[i].field3_id,tempcodeset)))
         CALL logmsg(curprog,"Exiting isValidChargeModModification, invalid code value provided",
          log_debug)
         CALL writeerrortoreply(idx0,"isValidChargeModModification",
          "field3_id is invalid for a modifier/revenue Code",true)
         RETURN(false)
        ELSE
         IF ((request->charge[idx0].charge_mod[i].action_type="ADD"))
          SET chargemoddeleted = true
          SET searchidx = 1
          WHILE (chargemoddeleted)
           SET pos = locateval(idx,searchidx,size(chargefindreply->charge_items[1].charge_mods,5),
            request->charge[idx0].charge_mod[i].field3_id,chargefindreply->charge_items[1].
            charge_mods[idx].field3_id,
            request->charge[idx0].charge_mod[i].field1_id,chargefindreply->charge_items[1].
            charge_mods[idx].field1_id,true,chargefindreply->charge_items[1].charge_mods[idx].
            active_ind)
           IF (pos > 0)
            SET pos2 = locateval(idx,1,size(request->charge[idx0].charge_mod,5),chargefindreply->
             charge_items[1].charge_mods[pos].charge_mod_id,request->charge[idx0].charge_mod[idx].
             charge_mod_id,
             "DEL",request->charge[idx0].charge_mod[idx].action_type)
            IF (pos2 > 0)
             SET searchidx = (pos+ 1)
          ...
          ENDWHILE
         ELSEIF ((request->charge[idx0].charge_mod[i].action_type="UPT"))
          SET pos = locateval(idx,1,size(chargefindreply->charge_items[1].charge_mods,5),request->
           charge[idx0].charge_mod[i].field3_id,chargefindreply->charge_items[1].charge_mods[idx].
           field3_id,
           request->charge[idx0].charge_mod[i].field2_id,chargefindreply->charge_items[1].
           charge_mods[idx].field2_id,request->charge[idx0].charge_mod[i].field1_id,chargefindreply->
           charge_items[1].charge_mods[idx].field1_id,true,
           chargefindreply->charge_items[1].charge_mods[idx].active_ind)
         ENDIF
         IF (pos > 0)
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, requested charge mod exists (code_value)",log_debug
           )
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
           "requested code already exists on charge",true)
          RETURN(false)
         ENDIF
         SET necessarymodificationfound = true
         SET request->charge[idx0].charge_mod[i].field6 = uar_get_code_display(request->charge[idx0].
          charge_mod[i].field3_id)
         SET request->charge[idx0].charge_mod[i].field7 = uar_get_code_description(request->charge[
          idx0].charge_mod[i].field3_id)
        ENDIF
       ELSEIF (codeset14002meaning IN ("CPT4", "HCPCS", "PROCCODE", "ICD9", "ASA"))
        IF ((request->charge[idx0].charge_mod[i].nomen_id <= 0))
         CALL logmsg(curprog,
          "Exiting isValidChargeModModification, no bill code nomenclature provided",log_debug)
         CALL writeerrortoreply(idx0,"isValidChargeModModification","nomen_id is not populated",true)
         RETURN(false)
        ENDIF
        IF (codeset14002meaning="CPT4")
         SET tempsourcecd = cs400_cpt_cd
        ELSEIF (codeset14002meaning="HCPCS")
         SET tempsourcecd = cs400_hcpcs_cd
         IF ((request->charge[idx0].charge_mod[i].cm1_nbr <= 0))
          CALL logmsg(curprog,"Exiting isValidChargeModModification, invalid QCF value provided",
           log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
           "cm1_nbr is not populated for a HCPCS",true)
          RETURN(false)
         ENDIF
        ELSEIF (codeset14002meaning="ASA")
         SET tempsourcecd = cs400_asa_cd
         IF (((trim(request->charge[idx0].charge_mod[i].field1)="") OR (((trim(request->charge[idx0].
          charge_mod[i].field2)="") OR (((trim(request->charge[idx0].charge_mod[i].field3)="") OR (((
         trim(request->charge[idx0].charge_mod[i].field4)="") OR (trim(request->charge[idx0].
          charge_mod[i].field5)="")) )) )) )) )
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, invalid ASA field values provided",log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
           "Field1,2,3,4 or 5 is not populated for an ASA",true)
          RETURN(false)
         ENDIF
        ELSEIF (codeset14002meaning="ICD9")
         SET tempsourcecd = primary_cs400_icd_diag_cd
        ELSE
         SET tempsourcecd = primary_cs400_icd_proc_cd
        ENDIF
        IF ( NOT (isvalidnomenclature(request->charge[idx0].charge_mod[i].nomen_id,tempsourcecd,
         chargefindreply->charge_items[1].service_dt_tm,billcode,billcodedesc)))
         IF ( NOT (codeset14002meaning IN ("ICD9", "PROCCODE")))
          CALL logmsg(curprog,"Exiting isValidChargeModModification, invalid nomenclature provided",
           log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification","Nomen_id is not valid",true)
          RETURN(false)
         ELSEIF (codeset14002meaning="ICD9"
          AND ((secondary_cs400_icd_diag_cd=0.0) OR (codeset14002meaning="PROCCODE"
          AND secondary_cs400_icd_proc_cd=0.0)) )
          CALL logmsg(curprog,"Exiting isValidChargeModModification, invalid nomenclature provided",
           log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification","Nomen_id is not valid",true)
          RETURN(false)
         ELSEIF (codeset14002meaning="ICD9"
          AND secondary_cs400_icd_diag_cd > 0.0
          AND  NOT (isvalidnomenclature(request->charge[idx0].charge_mod[i].nomen_id,
          secondary_cs400_icd_diag_cd,chargefindreply->charge_items[1].service_dt_tm,billcode,
          billcodedesc)))
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, invalid nomenclature (ICD Diagnosis) provided",
           log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification","Nomen_id is not valid",true)
          RETURN(false)
         ELSEIF (codeset14002meaning="PROCCODE"
          AND secondary_cs400_icd_proc_cd > 0.0
          AND  NOT (isvalidnomenclature(request->charge[idx0].charge_mod[i].nomen_id,
          secondary_cs400_icd_proc_cd,chargefindreply->charge_items[1].service_dt_tm,billcode,
          billcodedesc)))
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, invalid nomenclature (ICD Procedure) provided",
           log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification","Nomen_id is not valid",true)
          RETURN(false)
         ENDIF
        ELSE
         IF (codeset14002meaning="HCPCS")
          IF ((request->charge[idx0].charge_mod[i].action_type="ADD"))
           SET chargemoddeleted = true
           SET searchidx = 1
           WHILE (chargemoddeleted)
            SET pos = locateval(idx,searchidx,size(chargefindreply->charge_items[1].charge_mods,5),
             request->charge[idx0].charge_mod[i].nomen_id,chargefindreply->charge_items[1].
             charge_mods[idx].nomen_id,
             request->charge[idx0].charge_mod[i].field1_id,chargefindreply->charge_items[1].
             charge_mods[idx].field1_id,true,chargefindreply->charge_items[1].charge_mods[idx].
             active_ind)
            IF (pos > 0)
          ...
          ELSEIF ((request->charge[idx0].charge_mod[i].action_type="UPT"))
           SET pos = locateval(idx,1,size(chargefindreply->charge_items[1].charge_mods,5),request->
            charge[idx0].charge_mod[i].nomen_id,chargefindreply->charge_items[1].charge_mods[idx].
            nomen_id,
            request->charge[idx0].charge_mod[i].field2_id,chargefindreply->charge_items[1].
            charge_mods[idx].field2_id,request->charge[idx0].charge_mod[i].field1_id,chargefindreply
            ->charge_items[1].charge_mods[idx].field1_id,request->charge[idx0].charge_mod[i].cm1_nbr,
            chargefindreply->charge_items[1].charge_mods[idx].cm1_nbr,true,chargefindreply->
            charge_items[1].charge_mods[idx].active_ind)
          ENDIF
         ELSE
          IF ((request->charge[idx0].charge_mod[i].action_type="ADD"))
           SET chargemoddeleted = true
          ...
          ELSEIF ((request->charge[idx0].charge_mod[i].action_type="UPT"))
           SET pos = locateval(idx,1,size(chargefindreply->charge_items[1].charge_mods,5),request->
            ...
            nomen_id,
            request->charge[idx0].charge_mod[i].field1_id,chargefindreply->charge_items[1].
            charge_mods[idx].field1_id,request->charge[idx0].charge_mod[i].field2_id,chargefindreply
            ->charge_items[1].charge_mods[idx].field2_id,true,
            chargefindreply->charge_items[1].charge_mods[idx].active_ind)
          ENDIF
         ENDIF
         IF (pos > 0)
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, requested charge mod exists (nomenclature)",
           log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
           "HCPCS bill code already exists on charge",true)
          RETURN(false)
         ENDIF
         SET necessarymodificationfound = true
         SET request->charge[idx0].charge_mod[i].field6 = billcode
         SET request->charge[idx0].charge_mod[i].field7 = billcodedesc
        ENDIF
       ELSEIF (codeset14002meaning="CDM_SCHED")
        SET tempfield3id = request->charge[idx0].charge_mod[i].field3_id
        IF ( NOT (isvalidcdm(tempfield3id,trim(request->charge[idx0].charge_mod[i].field6))))
         CALL logmsg(curprog,"Exiting isValidCDM, invalid CDM code provided",log_debug)
         CALL writeerrortoreply(idx0,"isValidCDM",
          "CDM field3_id is not valid for the corresponding field6",true)
         RETURN(false)
        ENDIF
        SET request->charge[idx0].charge_mod[i].field3_id = tempfield3id
        IF (trim(request->charge[idx0].charge_mod[i].field6) IN ("", null))
         CALL logmsg(curprog,"Exiting isValidChargeModModification, invalid CDM code provided",
          log_debug)
         CALL writeerrortoreply(idx0,"isValidChargeModModification","CDM field6 is not populated",
          true)
         RETURN(false)
        ELSE
         IF ((request->charge[idx0].charge_mod[i].action_type="ADD"))
          ...
          WHILE (chargemoddeleted)
           SET pos = locateval(idx,searchidx,size(chargefindreply->charge_items[1].charge_mods,5),
            request->charge[idx0].charge_mod[i].field6,chargefindreply->charge_items[1].charge_mods[
            idx].field6,
            request->charge[idx0].charge_mod[i].field1_id,chargefindreply->charge_items[1].
            charge_mods[idx].field1_id,true,chargefindreply->charge_items[1].charge_mods[idx].
            active_ind)
         ...
         ELSEIF ((request->charge[idx0].charge_mod[i].action_type="UPT"))
          SET pos = locateval(idx,1,size(chargefindreply->charge_items[1].charge_mods,5),request->
           charge[idx0].charge_mod[i].field6,chargefindreply->charge_items[1].charge_mods[idx].field6,
           request->charge[idx0].charge_mod[i].field7,chargefindreply->charge_items[1].charge_mods[
           idx].field7,request->charge[idx0].charge_mod[i].field1_id,chargefindreply->charge_items[1]
           .charge_mods[idx].field1_id,request->charge[idx0].charge_mod[i].field2_id,
           chargefindreply->charge_items[1].charge_mods[idx].field2_id,true,chargefindreply->
           charge_items[1].charge_mods[idx].active_ind)
         ENDIF
         IF (pos > 0)
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, requested charge mod exists (CDM)",log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
           "CDM bill code already exists on charge",true)
          RETURN(false)
         ENDIF
         SET necessarymodificationfound = true
        ENDIF
       ELSEIF (codeset14002meaning="NDC")
        DECLARE ndc_index = i4 WITH protect, noconstant(0)
        FOR (j = 1 TO size(chargefindreply->charge_items[1].charge_mods,5))
         SET pos2 = locateval(idx,1,size(request->charge[idx0].charge_mod,5),chargefindreply->
          charge_items[1].charge_mods[j].charge_mod_id,request->charge[idx0].charge_mod[idx].
          charge_mod_id,
          "DEL",request->charge[idx0].charge_mod[idx].action_type)
         IF (uar_get_code_meaning(chargefindreply->charge_items[1].charge_mods[j].field1_id)="NDC"
          AND (chargefindreply->charge_items[1].charge_mods[j].active_ind=1)
          AND pos2=0)
          SET ndc_index = j
         ENDIF
        ENDFOR
        IF ((request->charge[idx0].charge_mod[i].action_type="ADD")
         AND ndc_index > 0)
         CALL logmsg(curprog,
          "Exiting isValidChargeModModification, and NDC already exists on this charge",log_debug)
         CALL writeerrortoreply(idx0,"isValidChargeModModification",
          "A NDC bill code already exists on charge (only 1 is allowed)",true)
         RETURN(false)
        ENDIF
        IF ((request->charge[idx0].charge_mod[i].action_type="UPT")
         AND ndc_index > 0)
         IF ((request->charge[idx0].charge_mod[i].charge_mod_id != chargefindreply->charge_items[1].
         charge_mods[ndc_index].charge_mod_id))
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, Request does not update existing NDC row",log_debug
           )
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
           "Request does not update existing NDC row",true)
          RETURN(false)
         ENDIF
        ENDIF
        IF ((((request->charge[idx0].charge_mod[i].field4_id <= 0)) OR ( NOT (isvalidcodevalue(
         request->charge[idx0].charge_mod[i].field4_id,unit_of_measure_codeset)))) )
         CALL logmsg(curprog,
          "Exiting isValidChargeModModification, invalid NDC unit of measure provided",log_debug)
         CALL writeerrortoreply(idx0,"isValidChargeModModification",
          "NDC Field4_id is not populated or valid",true)
         RETURN(false)
        ELSEIF ((request->charge[idx0].charge_mod[i].field3_id <= 0))
         CALL logmsg(curprog,"Exiting isValidChargeModModification, invalid NDC QCF factor provided",
          log_debug)
         CALL writeerrortoreply(idx0,"isValidChargeModModification",
          "NDC QCF(field3_id) is not populated",true)
         RETURN(false)
        ELSEIF (trim(request->charge[idx0].charge_mod[i].field6) IN ("", null))
         CALL logmsg(curprog,"Exiting isValidChargeModModification, invalid NDC provided",log_debug)
         CALL writeerrortoreply(idx0,"isValidChargeModModification","NDC field6 is not populated",
          true)
         RETURN(false)
        ELSE
         SET pos = locateval(idx,1,size(chargefindreply->charge_items[1].charge_mods,5),request->
          charge[idx0].charge_mod[i].field4_id,chargefindreply->charge_items[1].charge_mods[idx].
          field4_id,
          request->charge[idx0].charge_mod[i].field3_id,chargefindreply->charge_items[1].charge_mods[
          idx].field3_id,request->charge[idx0].charge_mod[i].field6,chargefindreply->charge_items[1].
          charge_mods[idx].field6,true,
          chargefindreply->charge_items[1].charge_mods[idx].active_ind)
         IF (pos > 0)
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, requested charge mod exists (NDC)",log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
           "NDC bill code already exists on charge",true)
          RETURN(false)
         ENDIF
         SET necessarymodificationfound = true
        ENDIF
       ENDIF
      ELSEIF ((request->charge[idx0].charge_mod[i].charge_mod_type_cd=cs13019_flex_cd))
       IF ((request->charge[idx0].charge_mod[i].action_type="UPT"))
        IF ((request->charge[idx0].charge_mod[i].charge_mod_id=0))
         CALL logmsg(curprog,
          "Exiting isValidChargeModModification, charge_mod_id is not populated for a flex field",
          log_debug)
         CALL writeerrortoreply(idx0,"isValidChargeModModification",
          "charge_mod_id is not populated for a flex field upt action",true)
         RETURN(false)
        ELSEIF ( NOT (ischargemodvalidandactive(request->charge[idx0].charge_mod[i].charge_mod_id)))
         CALL logmsg(curprog,
          "Exiting isValidChargeModModification, charge_mod_id is not valid for a flex field",
          log_debug)
         CALL writeerrortoreply(idx0,"isValidChargeModModification",
          "charge_mod_id is not valid for a flex field upt action",true)
         RETURN(false)
        ENDIF
       ELSEIF ((request->charge[idx0].charge_mod[i].action_type="ADD")
        AND (request->charge[idx0].charge_mod[i].charge_mod_id > 0))
        CALL logmsg(curprog,
         "Exiting isValidChargeModModification, charge_mod_id is populated on an ADD action",
         log_debug)
        CALL writeerrortoreply(idx0,"isValidChargeModModification",
         "charge_mod_id is populated for a flex field add action",true)
        RETURN(false)
       ENDIF
       IF ((request->charge[idx0].charge_mod[i].field1_id <= 0))
        CALL logmsg(curprog,
         "Exiting isValidChargeModModification, field1_id is not populated for a flex field",
         log_debug)
        CALL writeerrortoreply(idx0,"isValidChargeModModification",
         "field1_id is not populated for a flex field add/upt action",true)
        RETURN(false)
       ELSEIF ( NOT (isvalidcodevalue(request->charge[idx0].charge_mod[i].field1_id,
        flex_field_codeset)))
        CALL logmsg(curprog,
         "Exiting isValidChargeModModification, field1_id is not populated for a flex field",
         log_debug)
        ...
        RETURN(false)
       ENDIF
       IF (trim(request->charge[idx0].charge_mod[i].field7) != trim(uar_get_code_display(request->
         charge[idx0].charge_mod[i].field1_id)))
        CALL logmsg(curprog,
         "Exiting isValidChargeModModification, field7 is not populated correctly for a flex field",
         log_debug)
        CALL writeerrortoreply(idx0,"isValidChargeModModification",
         "field7 is not populated correctly for a flex field add/upt action",true)
        RETURN(false)
       ENDIF
       SELECT INTO "nl:"
        FROM code_value_extension cve
        PLAN (cve
         WHERE (cve.code_value=request->charge[idx0].charge_mod[i].field1_id)
          AND cve.field_type=2)
        DETAIL
         cve_field_value = cve.field_value
        WITH nocounter
       ;end select
       IF (cve_field_value="")
        CALL logmsg(curprog,
         "Exiting isValidChargeModModification, code_value_extension is not populated correctly for a flex field",
         log_debug)
        CALL writeerrortoreply(idx0,"isValidChargeModModification",
         "code_value_extension is not populated correctly for a flex field add/upt action",true)
        RETURN(false)
       ENDIF
       IF (trim(request->charge[idx0].charge_mod[i].field2) != trim(cve_field_value))
        CALL logmsg(curprog,
         "Exiting isValidChargeModModification, field2 is not populated correctly for a flex field",
         log_debug)
        CALL writeerrortoreply(idx0,"isValidChargeModModification",
         "field2 is not populated correctly for a flex field add/upt action",true)
        RETURN(false)
       ENDIF
       CASE (cve_field_value)
        OF "STRING":
         IF (textlen(trim(request->charge[idx0].charge_mod[i].field1))=0)
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, field1 is not populated for a flex field type of STRING",
           log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
           "field1 is not populated for a flex field type of STRING",true)
          RETURN(false)
         ENDIF
         SET pos = locateval(idx,1,size(chargefindreply->charge_items[1].charge_mods,5),request->
          charge[idx0].charge_mod[i].field1_id,chargefindreply->charge_items[1].charge_mods[idx].
          field1_id,
          request->charge[idx0].charge_mod[i].field1,chargefindreply->charge_items[1].charge_mods[idx
          ].field1,true,chargefindreply->charge_items[1].charge_mods[idx].active_ind)
         IF (pos > 0)
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, flex field already exists with value",log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
           "flex field already exists with value",true)
          RETURN(false)
         ELSE
          SET necessarymodificationfound = true
         ENDIF
        OF value("NUMBER","CURRENCY"):
         IF (validate(debug,- (1)) > 0)
          CALL echo(build2("field2/flex_field_type is NUMBER or CURRENCY. cm1_nbr =",request->charge[
            idx0].charge_mod[i].cm1_nbr))
         ENDIF
         SET pos = locateval(idx,1,size(chargefindreply->charge_items[1].charge_mods,5),request->
          charge[idx0].charge_mod[i].field1_id,chargefindreply->charge_items[1].charge_mods[idx].
          field1_id,
          request->charge[idx0].charge_mod[i].cm1_nbr,chargefindreply->charge_items[1].charge_mods[
          idx].cm1_nbr,true,chargefindreply->charge_items[1].charge_mods[idx].active_ind)
         IF (pos > 0)
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, flex field already exists with value",log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
        ...
        OF "PROVLOOKUP":
         IF ((request->charge[idx0].charge_mod[i].field3_id=0))
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, field3_id is not populated for a flex field type of PROVLOOKUP",
           log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
           "field3_id is not curprog for a flex field type of PROVLOOKUP",true)
          RETURN(false)
         ELSEIF ( NOT (isvalidphysician(request->charge[idx0].charge_mod[i].field3_id)))
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, field3_id is not a valid physician for a flex field type of PROVLOOKUP",
           log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
           "field3_id is not a valid physician for a flex field type of PROVLOOKUP",true)
          RETURN(false)
         ENDIF
         SET pos = locateval(idx,1,size(chargefindreply->charge_items[1].charge_mods,5),request->
          charge[idx0].charge_mod[i].field1_id,chargefindreply->charge_items[1].charge_mods[idx].
          field1_id,
          request->charge[idx0].charge_mod[i].field3_id,chargefindreply->charge_items[1].charge_mods[
          idx].field3_id,true,chargefindreply->charge_items[1].charge_mods[idx].active_ind)
         IF (pos > 0)
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, flex field already exists with value",log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
         ...
         ENDIF
        OF "CODE":
         IF ((request->charge[idx0].charge_mod[i].code1_cd=0))
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, code1_cd is not populated for a flex field type of CODE",
           log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
           "code1_cd is not populated for a flex field type of CODE",true)
          RETURN(false)
         ELSE
          SELECT INTO "nl:"
           FROM code_value cv
           WHERE (cv.code_value=request->charge[idx0].charge_mod[i].code1_cd)
            AND cv.active_ind=1
           DETAIL
            valid_code_value = 1
           WITH nocounter
          ;end select
          IF (valid_code_value != 1)
           CALL logmsg(curprog,
            "Exiting isValidChargeModModification, code1_cd is not valid for a flex field type of CODE",
            log_debug)
           CALL writeerrortoreply(idx0,"isValidChargeModModification",
            "code1_cd is not valid for a flex field type of CODE",true)
           RETURN(false)
          ENDIF
         ENDIF
         SET pos = locateval(idx,1,size(chargefindreply->charge_items[1].charge_mods,5),request->
          charge[idx0].charge_mod[i].field1_id,chargefindreply->charge_items[1].charge_mods[idx].
          field1_id,
          request->charge[idx0].charge_mod[i].code1_cd,chargefindreply->charge_items[1].charge_mods[
          idx].code1_cd,true,chargefindreply->charge_items[1].charge_mods[idx].active_ind)
         IF (pos > 0)
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, flex field already exists with value",log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
         ...
         ENDIF
        OF "DATE":
         IF ((request->charge[idx0].charge_mod[i].activity_dt_tm=0))
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, activity_dt_tm is not populated for a flex field type of DATE",
           log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
           "activity_dt_tm is not populated for a flex field type of DATE",true)
          RETURN(false)
         ENDIF
         SET pos = locateval(idx,1,size(chargefindreply->charge_items[1].charge_mods,5),request->
          charge[idx0].charge_mod[i].field1_id,chargefindreply->charge_items[1].charge_mods[idx].
          field1_id,
          request->charge[idx0].charge_mod[i].activity_dt_tm,chargefindreply->charge_items[1].
          charge_mods[idx].activity_dt_tm,true,chargefindreply->charge_items[1].charge_mods[idx].
          active_ind)
         IF (pos > 0)
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, flex field already exists with value",log_debug)
        ...
        OF "INDICATOR":
         SET pos = locateval(idx,1,size(chargefindreply->charge_items[1].charge_mods,5),request->
          charge[idx0].charge_mod[i].field1_id,chargefindreply->charge_items[1].charge_mods[idx].
          field1_id,
          request->charge[idx0].charge_mod[i].field2_id,chargefindreply->charge_items[1].charge_mods[
          idx].field2_id,true,chargefindreply->charge_items[1].charge_mods[idx].active_ind)
         IF (pos > 0)
          CALL logmsg(curprog,
           "Exiting isValidChargeModModification, flex field already exists with value",log_debug)
          CALL writeerrortoreply(idx0,"isValidChargeModModification",
        ...
        ELSE
         CALL logmsg(curprog,
          "Exiting isValidChargeModModification, flex field type (field2) is not valid",log_debug)
         CALL writeerrortoreply(idx0,"isValidChargeModModification",build2(
           "flex field type (field2) is not valid, only supports ",
           "STRING, NUMBER, CURRENCY, PROVLOOKUP, CODE, DATE, and INDICATOR"),true)
         RETURN(false)
       ENDCASE
      ELSE
       CALL logmsg(curprog,
        ...
        log_debug)
       CALL writeerrortoreply(idx0,"isValidChargeModModification",
        "Charge_mod_type_cd is not Bill Code or Flex Field for an update or add action",true)
       RETURN(false)
      ENDIF
     ELSEIF ((request->charge[idx0].charge_mod[i].action_type="EXI"))
      IF ((request->charge[idx0].charge_mod[i].charge_mod_id <= 0))
       CALL logmsg(curprog,"Exiting isValidChargeModModification, no charge mod to retain provided",
        log_debug)
       CALL writeerrortoreply(idx0,"isValidChargeModModification",
        "charge_mod_id is not populated for an Existing action",true)
       RETURN(false)
      ELSEIF ( NOT (ischargemodvalidandactive(request->charge[idx0].charge_mod[i].charge_mod_id)))
       CALL logmsg(curprog,
        "Exiting isChargeModValidAndActive, charge mod provided isn't valid to retain",log_debug)
       CALL writeerrortoreply(idx0,"isValidChargeModModification",
        "charge_mod_id is not valid for an Existing action",true)
       RETURN(false)
      ENDIF
     ELSE
      CALL logmsg(curprog,"Exiting isValidChargeModModification, invalid action_type provided",
       log_debug)
      CALL writeerrortoreply(idx0,"isValidChargeModModification","Invalid Charge Mod Action type",
       true)
      RETURN(false)
     ENDIF
   ENDFOR
   CALL logmsg(curprog,"Exiting isValidChargeModModification, Valid charge Mod",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (ischargemodvalidandactive(chargemodid=f8) =i2)
   CALL logmsg(curprog,"Entering isChargeModValidAndActive",log_debug)
   DECLARE idx = i4 WITH protect, noconstant(0)
   DECLARE pos = i4 WITH protect, noconstant(0)
   SET pos = locateval(idx,1,size(chargefindreply->charge_items[1].charge_mods,5),chargemodid,
    chargefindreply->charge_items[1].charge_mods[idx].charge_mod_id)
   IF (pos > 0)
    IF ((chargefindreply->charge_items[1].charge_mods[pos].active_ind=1))
     CALL logmsg(curprog,"Exiting isChargeModValidAndActive, charge mod is active and valid",
      log_debug)
     RETURN(true)
    ELSE
     CALL logmsg(curprog,"Exiting isChargeModValidAndActive, charge mod is not active",log_debug)
     RETURN(false)
    ENDIF
   ENDIF
   CALL logmsg(curprog,"Exiting isChargeModValidAndActive, charge mod is not valid",log_debug)
   RETURN(false)
 END ;Subroutine
 SUBROUTINE (islogicaldomainvalid(chargeitemid=f8,curlogicaldomainid=f8) =i2)
   CALL logmsg(curprog,"Entering isLogicalDomainValid",log_debug)
   SELECT INTO "nl:"
    FROM charge c,
     encounter e,
     organization o
   ...
   ;end select
   IF (curqual > 0)
    CALL logmsg(curprog,"Exiting isLogicalDomainValid, logical domain is valid",log_debug)
    RETURN(true)
   ENDIF
   CALL logmsg(curprog,
    "Exiting isLogicalDomainValid, logical domains don't match - do not allow modifications",
    log_debug)
   RETURN(false)
 END ;Subroutine
 SUBROUTINE (isvalidphysician(physid=f8) =i2)
   CALL logmsg(curprog,"Entering isValidPhysician",log_debug)
   SELECT INTO "nl:"
    FROM prsnl p
    WHERE p.person_id=physid
     AND p.active_ind=1
     AND p.physician_ind=1
    WITH nocounter
   ;end select
   IF (curqual > 0)
    CALL logmsg(curprog,"Exiting isValidPhysician, requested physician is valid",log_debug)
    RETURN(true)
   ENDIF
   CALL logmsg(curprog,"Exiting isValidPhysician, requested physician is not valid",log_debug)
   RETURN(false)
 END ;Subroutine
 SUBROUTINE (isvalidcodevalue(codevalue=f8,codeset=i4) =i2)
   CALL logmsg(curprog,"Entering isValidCodeValue",log_debug)
   SELECT INTO "nl:"
    FROM code_value c
    WHERE c.code_value=codevalue
     AND c.code_set=codeset
     AND c.active_ind=1
    WITH nocounter
   ;end select
   IF (curqual > 0)
    CALL logmsg(curprog,"Exiting isValidCodeValue, requested codevalue is valid",log_debug)
    RETURN(true)
   ENDIF
   CALL logmsg(curprog,"Exiting isValidCodeValue, requested codevalue is not valid",log_debug)
   RETURN(false)
 END ;Subroutine
 SUBROUTINE (isvalidresearchaccount(researchacctid=f8) =i2)
   CALL logmsg(curprog,"Entering isValidResearchAccount",log_debug)
   SELECT INTO "nl:"
    FROM research_account c
    WHERE c.research_account_id=researchacctid
     AND c.active_ind=1
     AND c.organization_id > 0
    WITH nocounter
   ;end select
   IF (curqual > 0)
    CALL logmsg(curprog,"Exiting isValidResearchAccount, requested codevalue is valid",log_debug)
    RETURN(true)
   ENDIF
   CALL logmsg(curprog,"Exiting isValidResearchAccount, requested codevalue is not valid",log_debug)
   RETURN(false)
 END ;Subroutine
 SUBROUTINE (isvalidnomenclature(nomenid=f8,sourcevocabcd=f8,servicedate=f8,billcode=vc(ref),
  billcodedesc=c200(ref)) =i2)
   CALL logmsg(curprog,"Entering isValidNomenclature",log_debug)
   SELECT INTO "nl:"
    FROM nomenclature n
    PLAN (n
     WHERE n.nomenclature_id=nomenid
      ...
      AND n.beg_effective_dt_tm <= cnvtdatetime(servicedate)
      AND n.end_effective_dt_tm >= cnvtdatetime(servicedate))
    DETAIL
     billcode = n.source_identifier, billcodedesc = substring(1,200,n.source_string)
    WITH nocounter
   ;end select
   IF (curqual != 1)
    CALL logmsg(curprog,"Exiting isValidNomenclature, requested nomenclature is not valid",log_debug)
    RETURN(false)
   ENDIF
   CALL logmsg(curprog,"Entering isValidNomenclature",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (isvalidcdm(tempfield3id=f8(ref),field6=vc) =i2)
   CALL logmsg(curprog,"Entering isValidCDM",log_debug)
   DECLARE charge_desc_master_id = f8 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM charge_desc_master cdm
    PLAN (cdm
     WHERE cdm.cdm_code_txt=field6
      AND cdm.active_ind=1
      AND cdm.logical_domain_id=curlogicaldomainid)
    DETAIL
     charge_desc_master_id = cdm.charge_desc_master_id
    WITH nocounter
   ;end select
   IF (curqual=0)
    RETURN(true)
    CALL logmsg(curprog,"Exiting isValidCDM, requested CDM is not not found",log_debug)
   ELSEIF (curqual != 1)
    CALL logmsg(curprog,"Exiting isValidCDM, requested CDM is not valid",log_debug)
   ENDIF
   IF (tempfield3id=0)
    SET tempfield3id = charge_desc_master_id
   ELSEIF (tempfield3id != charge_desc_master_id)
    CALL logmsg(curprog,"Exiting isValidCDM, CDM field3_id is not valid for the corresponding field6",
     log_debug)
    RETURN(false)
   ENDIF
   CALL logmsg(curprog,"Exiting isValidCDM",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (updatecharge(idx0=i4,chargefindreply=vc(ref)) =null)
   CALL logmsg(curprog,"Entering updateCharge",log_debug)
   DECLARE chrgcnt = i4 WITH noconstant(0)
   SET stat = initrec(uptchrgrequest)
   SET stat = initrec(uptchrgreply)
   SET stat = alterlist(uptchrgrequest->charge,1)
   SET uptchrgrequest->charge_qual = 1
   SET uptchrgrequest->charge[1].charge_item_id = request->charge[idx0].charge_item_id
   CALL getoldchargeitemdetails(request->charge[idx0].charge_item_id)
   IF (newperfloccd=0.0)
    SET uptchrgrequest->charge[1].perf_loc_cd = - (1.0)
   ELSE
    SET uptchrgrequest->charge[1].perf_loc_cd = newperfloccd
   ENDIF
   IF (newordphys=0.0)
    SET uptchrgrequest->charge[1].ord_phys_id = - (1.0)
   ELSE
    SET uptchrgrequest->charge[1].ord_phys_id = newordphys
   ENDIF
   IF (newrendphys=0.0)
    SET uptchrgrequest->charge[1].verify_phys_id = - (1.0)
   ELSE
    SET uptchrgrequest->charge[1].verify_phys_id = newrendphys
   ENDIF
   IF (newitemprice=0.0)
    SET uptchrgrequest->charge[1].item_price = - (1.0)
   ELSE
    SET uptchrgrequest->charge[1].item_price = newitemprice
   ENDIF
   IF (newabnstatuscd=0.0)
    SET uptchrgrequest->charge[1].abn_status_cd = - (1.0)
   ELSE
    SET uptchrgrequest->charge[1].abn_status_cd = newabnstatuscd
   ENDIF
   SET uptchrgrequest->charge[1].suspense_rsn_cd = newsuspreasoncd
   SET uptchrgrequest->charge[1].reason_comment = newreasoncomment
   SET uptchrgrequest->charge[1].service_dt_tm = newservicedttm
   SET uptchrgrequest->charge[1].original_org_id = neworiginalorgid
   SET uptchrgrequest->charge[1].active_status_cd = chargefindreply->charge_items[1].active_status_cd
   SET uptchrgrequest->charge[1].updt_cnt = chargefindreply->charge_items[1].updt_cnt
   SET uptchrgrequest->charge[1].charge_description = newchargedesc
   SET uptchrgrequest->charge[1].item_quantity = newitemquantity
   SET uptchrgrequest->charge[1].item_extended_price = newitemextendedprice
   SET uptchrgrequest->charge[1].patient_responsibility_flag = newpatientrespflag
   IF (validate(debug,- (1)) > 0)
    CALL echo("Updating a charge, request structure is:")
    CALL echorecord(uptchrgrequest)
   ENDIF
   EXECUTE afc_upt_charge  WITH replace("REQUEST",uptchrgrequest), replace("REPLY",uptchrgreply)
   IF ((uptchrgreply->status_data.status != "S"))
    CALL logmsg(curprog,"Exiting updateCharge, failed to update charge",log_debug)
    IF (validate(debug,- (1)) > 0)
     CALL echorecord(uptchrgrequest)
     CALL echorecord(uptchrgreply)
    ENDIF
    CALL writeerrortoreply(idx0,"updateCharge","afc_upt_charge returned failure",true)
    RETURN(false)
   ELSE
    SET chrgcnt = (size(reply->charge[idx0].new_charges,5)+ 1)
    SET stat = alterlist(reply->charge[idx0].new_charges,chrgcnt)
    ...
    charge_item_id
    SET reply->charge[idx0].new_charges[chrgcnt].charge_type_cd = cs13028_debit_cd
    SET reply->charge[idx0].new_charges[chrgcnt].action_type = "UPT"
   ENDIF
   CALL logmsg(curprog,"updateCharge: saveChargeChangeLogAttributes",log_debug)
   CALL savechargechangelogattributes(uptchrgrequest)
   CALL logmsg(curprog,"Exiting updateCharge, successfully updated the charge",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (createcreditcharge(idx0=i4,reprocesschargeind=i2,newcreditchargeid=f8(ref)) =i2)
   DECLARE chrgcnt = i4 WITH noconstant(0)
   CALL logmsg(curprog,"Entering createCreditCharge",log_debug)
   SET stat = initrec(creditchargerequest)
   SET stat = initrec(creditchargereply)
   SET stat = alterlist(creditchargerequest->charge,1)
   SET creditchargerequest->charge_qual = 1
   SET creditchargerequest->charge[1].charge_item_id = request->charge[idx0].charge_item_id
   SET creditchargerequest->charge[1].suspense_rsn_cd = newsuspreasoncd
   SET creditchargerequest->charge[1].reason_comment = substring(1,200,newreasoncomment)
   IF (validate(debug,- (1)) > 0)
    CALL echo("createCreditCharge creditChargeRequest")
    CALL echorecord(creditchargerequest)
   ENDIF
   IF (reprocesschargeind)
    IF ( NOT (creditrelatedaddonsbeforereprocessing(request->charge[idx0].charge_item_id,
     newreasoncomment,newsuspreasoncd)))
     CALL logmsg(curprog,"Exiting createCreditCharge, failed to credit related addons",log_debug)
     CALL writeerrortoreply(idx0,"createCreditCharge",
      "creditRelatedAddonsBeforeReprocessing returned false",true)
     RETURN(false)
    ENDIF
   ENDIF
   FOR (idx3 = 1 TO size(afcaddcreditreply->charge,5))
     ...
     SET newcharges->numberofcharges += 1
     SET stat = alterlist(newcharges->charges,newcharges->numberofcharges)
     SET newcharges->charges[newcharges->numberofcharges].charge_item_id = afcaddcreditreply->charge[
     idx3].charge_item_id
     SET newcharges->charges[newcharges->numberofcharges].process_flg = 0
     SET chrgcnt = (size(reply->charge[idx0].new_charges,5)+ 1)
     SET stat = alterlist(reply->charge[idx0].new_charges,chrgcnt)
     SET reply->charge[idx0].new_charges[chrgcnt].new_charge_item_id = afcaddcreditreply->charge[idx3
     ].charge_item_id
     SET reply->charge[idx0].new_charges[chrgcnt].charge_type_cd = cs13028_credit_cd
     SET reply->charge[idx0].new_charges[chrgcnt].action_type = "ADD"
     IF ((request->contributor_system_cd > 0))
   ...
   EXECUTE afc_add_credit  WITH replace("REQUEST",creditchargerequest), replace("REPLY",
    creditchargereply)
   IF (validate(debug,- (1)) > 0)
    CALL echo("createCreditCharge creditChargeReply")
    CALL echorecord(creditchargereply)
   ENDIF
   IF ((creditchargereply->status_data.status="F"))
    CALL logmsg(curprog,"Exiting createCreditCharge, failed to credit charge",log_debug)
    CALL writeerrortoreply(idx0,"createCreditCharge","afc_add_credit returned failure",true)
    RETURN(false)
   ENDIF
   SET newcreditchargeid = creditchargereply->charge[1].charge_item_id
   SET chrgcnt = (size(reply->charge[idx0].new_charges,5)+ 1)
   SET stat = alterlist(reply->charge[idx0].new_charges,chrgcnt)
   SET reply->charge[idx0].new_charges[chrgcnt].new_charge_item_id = creditchargereply->charge[1].
   charge_item_id
   SET reply->charge[idx0].new_charges[chrgcnt].charge_type_cd = cs13028_credit_cd
   SET reply->charge[idx0].new_charges[chrgcnt].action_type = "ADD"
   IF ((request->contributor_system_cd > 0))
    CALL addcontributorsystemmod(creditchargereply->charge[1].charge_item_id)
   ENDIF
   CALL logmsg(curprog,"Exiting createCreditCharge",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (createdebitcharge(idx0=i4,chargefindreply=vc(ref),newcid=f8(ref)) =i2)
   DECLARE chrgcnt = i4 WITH noconstant(0)
   CALL logmsg(curprog,"Entering createDebitCharge",log_debug)
   SET stat = initrec(newdebitchargerec)
   SET stat = initrec(afcaddchargereply)
   SET stat = alterlist(newdebitchargerec->objarray,1)
   SET newdebitchargerec->objarray[1].parent_charge_item_id = request->charge[idx0].charge_item_id
   SET newdebitchargerec->objarray[1].charge_event_act_id = chargefindreply->charge_items[1].
   charge_event_act_id
   SET newdebitchargerec->objarray[1].charge_event_id = chargefindreply->charge_items[1].
   charge_event_id
   SET newdebitchargerec->objarray[1].bill_item_id = chargefindreply->charge_items[1].bill_item_id
   SET newdebitchargerec->objarray[1].order_id = chargefindreply->charge_items[1].order_id
   SET newdebitchargerec->objarray[1].encntr_id = chargefindreply->charge_items[1].encntr_id
   SET newdebitchargerec->objarray[1].person_id = chargefindreply->charge_items[1].person_id
   SET newdebitchargerec->objarray[1].payor_id = chargefindreply->charge_items[1].payor_id
   SET newdebitchargerec->objarray[1].ord_loc_cd = chargefindreply->charge_items[1].ord_loc_cd
   SET newdebitchargerec->objarray[1].perf_loc_cd = newperfloccd
   SET newdebitchargerec->objarray[1].ord_phys_id = newordphys
   SET newdebitchargerec->objarray[1].perf_phys_id = chargefindreply->charge_items[1].perf_phys_id
   SET newdebitchargerec->objarray[1].charge_description = newchargedesc
   SET newdebitchargerec->objarray[1].price_sched_id = chargefindreply->charge_items[1].
   price_sched_id
   SET newdebitchargerec->objarray[1].item_quantity = newitemquantity
   SET newdebitchargerec->objarray[1].item_price = newitemprice
   SET newdebitchargerec->objarray[1].item_extended_price = newitemextendedprice
   SET newdebitchargerec->objarray[1].item_allowable = chargefindreply->charge_items[1].
   item_allowable
   SET newdebitchargerec->objarray[1].item_copay = chargefindreply->charge_items[1].item_copay
   SET newdebitchargerec->objarray[1].charge_type_cd = chargefindreply->charge_items[1].
   charge_type_cd
   SET newdebitchargerec->objarray[1].research_acct_id = chargefindreply->charge_items[1].
   research_acct_id
   SET newdebitchargerec->objarray[1].suspense_rsn_cd = newsuspreasoncd
   SET newdebitchargerec->objarray[1].reason_comment = newreasoncomment
   SET newdebitchargerec->objarray[1].posted_cd = chargefindreply->charge_items[1].posted_cd
   SET newdebitchargerec->objarray[1].posted_dt_tm = cnvtdatetime(chargefindreply->charge_items[1].
    posted_dt_tm)
   SET newdebitchargerec->objarray[1].process_flg = 0
   SET newdebitchargerec->objarray[1].service_dt_tm = newservicedttm
   SET newdebitchargerec->objarray[1].activity_dt_tm = cnvtdatetime(chargefindreply->charge_items[1].
    activity_dt_tm)
   SET newdebitchargerec->objarray[1].updt_cnt = 0
   SET newdebitchargerec->objarray[1].updt_dt_tm = cnvtdatetime(sysdate)
   SET newdebitchargerec->objarray[1].updt_id = reqinfo->updt_id
   SET newdebitchargerec->objarray[1].updt_task = reqinfo->updt_task
   SET newdebitchargerec->objarray[1].updt_applctx = reqinfo->updt_applctx
   SET newdebitchargerec->objarray[1].active_ind = chargefindreply->charge_items[1].active_ind
   SET newdebitchargerec->objarray[1].active_status_cd = chargefindreply->charge_items[1].
   active_status_cd
   SET newdebitchargerec->objarray[1].active_status_dt_tm = cnvtdatetime(sysdate)
   SET newdebitchargerec->objarray[1].active_status_prsnl_id = reqinfo->updt_id
   SET newdebitchargerec->objarray[1].beg_effective_dt_tm = cnvtdatetime(chargefindreply->
    charge_items[1].beg_effective_dt_tm)
   SET newdebitchargerec->objarray[1].end_effective_dt_tm = cnvtdatetime(chargefindreply->
    charge_items[1].end_effective_dt_tm)
   SET newdebitchargerec->objarray[1].adjusted_dt_tm = cnvtdatetime(chargefindreply->charge_items[1].
    adjusted_dt_tm)
   SET newdebitchargerec->objarray[1].interface_file_id = chargefindreply->charge_items[1].
   interface_file_id
   SET newdebitchargerec->objarray[1].tier_group_cd = chargefindreply->charge_items[1].tier_group_cd
   SET newdebitchargerec->objarray[1].def_bill_item_id = chargefindreply->charge_items[1].
   def_bill_item_id
   SET newdebitchargerec->objarray[1].verify_phys_id = newrendphys
   SET newdebitchargerec->objarray[1].gross_price = chargefindreply->charge_items[1].gross_price
   SET newdebitchargerec->objarray[1].discount_amount = chargefindreply->charge_items[1].
   discount_amount
   SET newdebitchargerec->objarray[1].item_price_adj_amt = chargefindreply->charge_items[1].
   item_price_adj_amt
   SET newdebitchargerec->objarray[1].manual_ind = chargefindreply->charge_items[1].manual_ind
   SET newdebitchargerec->objarray[1].combine_ind = chargefindreply->charge_items[1].combine_ind
   SET newdebitchargerec->objarray[1].activity_type_cd = chargefindreply->charge_items[1].
   activity_type_cd
   SET newdebitchargerec->objarray[1].activity_sub_type_cd = chargefindreply->charge_items[1].
   activity_sub_type_cd
   SET newdebitchargerec->objarray[1].provider_specialty_cd = chargefindreply->charge_items[1].
   provider_specialty_cd
   SET newdebitchargerec->objarray[1].alpha_nomen_id = chargefindreply->charge_items[1].
   alpha_nomen_id
   SET newdebitchargerec->objarray[1].admit_type_cd = chargefindreply->charge_items[1].admit_type_cd
   SET newdebitchargerec->objarray[1].bundle_id = chargefindreply->charge_items[1].bundle_id
   SET newdebitchargerec->objarray[1].department_cd = chargefindreply->charge_items[1].department_cd
   SET newdebitchargerec->objarray[1].institution_cd = chargefindreply->charge_items[1].
   institution_cd
   SET newdebitchargerec->objarray[1].level5_cd = chargefindreply->charge_items[1].level5_cd
   SET newdebitchargerec->objarray[1].med_service_cd = chargefindreply->charge_items[1].
   med_service_cd
   SET newdebitchargerec->objarray[1].section_cd = chargefindreply->charge_items[1].section_cd
   SET newdebitchargerec->objarray[1].subsection_cd = chargefindreply->charge_items[1].subsection_cd
   SET newdebitchargerec->objarray[1].abn_status_cd = newabnstatuscd
   SET newdebitchargerec->objarray[1].cost_center_cd = chargefindreply->charge_items[1].
   cost_center_cd
   SET newdebitchargerec->objarray[1].inst_fin_nbr = chargefindreply->charge_items[1].inst_fin_nbr
   SET newdebitchargerec->objarray[1].fin_class_cd = chargefindreply->charge_items[1].fin_class_cd
   SET newdebitchargerec->objarray[1].health_plan_id = chargefindreply->charge_items[1].
   health_plan_id
   SET newdebitchargerec->objarray[1].item_interval_id = chargefindreply->charge_items[1].
   item_interval_id
   SET newdebitchargerec->objarray[1].item_list_price = chargefindreply->charge_items[1].
   item_list_price
   SET newdebitchargerec->objarray[1].item_reimbursement = chargefindreply->charge_items[1].
   item_reimbursement
   SET newdebitchargerec->objarray[1].list_price_sched_id = chargefindreply->charge_items[1].
   list_price_sched_id
   SET newdebitchargerec->objarray[1].payor_type_cd = chargefindreply->charge_items[1].payor_type_cd
   SET newdebitchargerec->objarray[1].epsdt_ind = chargefindreply->charge_items[1].epsdt_ind
   SET newdebitchargerec->objarray[1].ref_phys_id = chargefindreply->charge_items[1].ref_phys_id
   SET newdebitchargerec->objarray[1].start_dt_tm = cnvtdatetime(chargefindreply->charge_items[1].
    start_dt_tm)
   SET newdebitchargerec->objarray[1].stop_dt_tm = cnvtdatetime(chargefindreply->charge_items[1].
    stop_dt_tm)
   SET newdebitchargerec->objarray[1].server_process_flag = chargefindreply->charge_items[1].
   server_process_flag
   SET newdebitchargerec->objarray[1].offset_charge_item_id = 0
   SET newdebitchargerec->objarray[1].item_deductible_amt = chargefindreply->charge_items[1].
   item_deductible_amt
   SET newdebitchargerec->objarray[1].patient_responsibility_flag = newpatientrespflag
   SET newdebitchargerec->objarray[1].original_org_id = neworiginalorgid
   IF (validate(debug,- (1)) > 0)
    CALL echo("createDebitCharge NewDebitChargeRec")
    CALL echorecord(newdebitchargerec)
   ENDIF
   EXECUTE afc_add_charge  WITH replace("REQUEST",newdebitchargerec), replace("REPLY",
    afcaddchargereply)
   IF (validate(debug,- (1)) > 0)
    CALL echo("createDebitCharge afcAddChargeReply")
    CALL echorecord(afcaddchargereply)
   ENDIF
   IF ((afcaddchargereply->status_data.status != "S"))
    CALL logmsg(curprog,"Exiting createDebitCharge, failed to insert new debit",log_debug)
    CALL writeerrortoreply(idx0,"createDebitCharge","afc_add_charge returned failure",true)
    RETURN(false)
   ENDIF
   SET newcid = afcaddchargereply->charges[1].charge_item_id
   SET chrgcnt = (size(reply->charge[idx0].new_charges,5)+ 1)
   SET stat = alterlist(reply->charge[idx0].new_charges,chrgcnt)
   SET reply->charge[idx0].new_charges[chrgcnt].new_charge_item_id = newcid
   SET reply->charge[idx0].new_charges[chrgcnt].charge_type_cd = cs13028_debit_cd
   SET reply->charge[idx0].new_charges[chrgcnt].action_type = "ADD"
   IF ((request->contributor_system_cd > 0))
    CALL addcontributorsystemmod(newcid)
   ENDIF
   CALL logmsg(curprog,"Exiting createDebitCharge",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (updatephysandabnstatusresearchaccount(idx0=i4) =i2)
   CALL logmsg(curprog,"Entering updatePhysandABNStatusResearchAccount",log_debug)
   DECLARE temp_charge_act_id = f8 WITH protect, noconstant(0.0)
   DECLARE temp_charge_event_id = f8 WITH protect, noconstant(0.0)
   DECLARE temp_abn_status_cd = f8 WITH protect, noconstant(0.0)
   DECLARE temp_res_acct_id = f8 WITH protect, noconstant(0.0)
   DECLARE temp_pat_resp_flg = i2 WITH protect, noconstant(0)
   DECLARE temp_phys_id = f8 WITH protect, noconstant(0.0)
   SET temp_charge_act_id = chargefindreply->charge_items[1].charge_event_act_id
   SET temp_charge_event_id = chargefindreply->charge_items[1].charge_event_id
   IF ((request->charge[idx0].ord_phys_id != 0))
    IF ((request->charge[idx0].ord_phys_id=- (1)))
     SELECT INTO "nl:"
      FROM charge_event_act_prsnl ceap
      WHERE ceap.charge_event_act_id=temp_charge_act_id
       AND ceap.prsnl_type_cd=cs13029_ordphys_cd
      WITH nocounter
     ;end select
     IF (curqual > 0)
      ...
      ;end update
      IF (curqual < 1)
       CALL logmsg(curprog,"Exiting updatePhysandABNStatusResearchAccount, failed to delete ord phys",
        log_debug)
       CALL writeerrortoreply(idx0,"updatePhysandABNStatusResearchAccount",
        "Failed to inactivate Ord Phys on charge_event_act_prsnl",true)
       RETURN(false)
      ENDIF
     ENDIF
    ELSE
     ...
     IF (temp_phys_id=0)
      INSERT  FROM charge_event_act_prsnl ceap
       SET ceap.prsnl_id = request->charge[idx0].ord_phys_id, ceap.prsnl_type_cd = cs13029_ordphys_cd,
        ceap.charge_event_act_id = temp_charge_act_id,
        ceap.active_ind = true, ceap.updt_cnt = 0, ceap.updt_dt_tm = cnvtdatetime(sysdate),
        ceap.updt_id = reqinfo->updt_id, ceap.updt_task = reqinfo->updt_task, ceap.updt_applctx =
        reqinfo->updt_applctx
       WITH nocounter
      ;end insert
      IF (curqual < 1)
       CALL logmsg(curprog,"Exiting updatePhysandABNStatusResearchAccount, failed to add ord phys",
        log_debug)
       CALL writeerrortoreply(idx0,"updatePhysandABNStatusResearchAccount",
        "Failed to add Ord Phys on charge_event_act_prsnl",true)
       RETURN(false)
      ENDIF
     ELSEIF ((temp_phys_id != request->charge[idx0].ord_phys_id))
      UPDATE  FROM charge_event_act_prsnl ceap
       SET ceap.prsnl_id = request->charge[idx0].ord_phys_id, ceap.updt_cnt = (ceap.updt_cnt+ 1),
      ...
      ;end update
      IF (curqual < 1)
       CALL logmsg(curprog,"Exiting updatePhysandABNStatusResearchAccount, failed to update ord phys",
        log_debug)
       CALL writeerrortoreply(idx0,"updatePhysandABNStatusResearchAccount",
        "Failed to update Ord Phys on charge_event_act_prsnl",true)
       RETURN(false)
      ENDIF
     ENDIF
    ENDIF
   ENDIF
   IF ((request->charge[idx0].verify_phys_id != 0))
    IF ((request->charge[idx0].verify_phys_id=- (1)))
     SELECT INTO "nl:"
      FROM charge_event_act_prsnl ceap
      WHERE ceap.charge_event_act_id=temp_charge_act_id
       AND ceap.prsnl_type_cd=cs13029_verphys_cd
      WITH nocounter
     ;end select
     IF (curqual > 0)
      UPDATE  FROM charge_event_act_prsnl ceap
      ...
      ;end update
      IF (curqual < 1)
       CALL logmsg(curprog,"Exiting updatePhysandABNStatusResearchAccount, failed to delete ver phys",
        log_debug)
       CALL writeerrortoreply(idx0,"updatePhysandABNStatusResearchAccount",
        "Failed to inactivate Verifying Phys on charge_event_act_prsnl",true)
       RETURN(false)
      ENDIF
     ENDIF
    ELSE
     ...
     IF (temp_phys_id=0)
      INSERT  FROM charge_event_act_prsnl ceap
       SET ceap.prsnl_id = request->charge[idx0].verify_phys_id, ceap.prsnl_type_cd =
        cs13029_verphys_cd, ceap.charge_event_act_id = temp_charge_act_id,
        ceap.active_ind = true, ceap.updt_cnt = 0, ceap.updt_dt_tm = cnvtdatetime(sysdate),
        ceap.updt_id = reqinfo->updt_id, ceap.updt_task = reqinfo->updt_task, ceap.updt_applctx =
        reqinfo->updt_applctx
       WITH nocounter
      ;end insert
      IF (curqual < 1)
       CALL logmsg(curprog,"Exiting updatePhysandABNStatusResearchAccount, failed to add ver phys",
        log_debug)
       CALL writeerrortoreply(idx0,"updatePhysandABNStatusResearchAccount",
        "Failed to insert Verifying Phys on charge_event_act_prsnl",true)
       RETURN(false)
      ENDIF
     ELSEIF ((temp_phys_id != request->charge[idx0].verify_phys_id))
      UPDATE  FROM charge_event_act_prsnl ceap
       SET ceap.prsnl_id = request->charge[idx0].verify_phys_id, ceap.updt_cnt = (ceap.updt_cnt+ 1),
        ceap.updt_dt_tm = cnvtdatetime(sysdate),
        ceap.updt_id = reqinfo->updt_id, ceap.updt_task = reqinfo->updt_task, ceap.updt_applctx =
        reqinfo->updt_applctx
       WHERE ceap.charge_event_act_id=temp_charge_act_id
      ...
      ;end update
      IF (curqual < 1)
       CALL logmsg(curprog,"Exiting updatePhysandABNStatusResearchAccount, failed to update ver phys",
        log_debug)
       CALL writeerrortoreply(idx0,"updatePhysandABNStatusResearchAccount",
        "Failed to update Verifying Phys on charge_event_act_prsnl",true)
       RETURN(false)
      ENDIF
     ENDIF
    ENDIF
   ...
   SELECT INTO "nl:"
    FROM charge_event ce
    PLAN (ce
     WHERE ce.charge_event_id=temp_charge_event_id)
    DETAIL
     temp_abn_status_cd = ce.abn_status_cd, temp_res_acct_id = ce.research_account_id
    WITH nocounter
   ;end select
   IF ((request->charge[idx0].abn_status_cd > 0)
    AND (temp_abn_status_cd != request->charge[idx0].abn_status_cd))
    UPDATE  FROM charge_event ce
     SET ce.abn_status_cd = request->charge[idx0].abn_status_cd, ce.updt_cnt = (ce.updt_cnt+ 1), ce
    ...
    IF (curqual < 1)
     CALL logmsg(curprog,
      "Exiting updatePhysandABNStatusResearchAccount, failed to update ABN on charge_event",log_debug
      )
     CALL writeerrortoreply(idx0,"updatePhysandABNStatusResearchAccount",
      "Failed to update ABN on charge_event",true)
     RETURN(false)
    ENDIF
   ENDIF
   IF ((request->charge[idx0].research_acct_id > 0)
    AND (temp_res_acct_id != request->charge[idx0].research_acct_id))
    UPDATE  FROM charge_event ce
     SET ce.research_account_id = request->charge[idx0].research_acct_id, ce.updt_cnt = (ce.updt_cnt
      + 1), ce.updt_dt_tm = cnvtdatetime(sysdate),
      ce.updt_id = reqinfo->updt_id, ce.updt_task = reqinfo->updt_task, ce.updt_applctx = reqinfo->
      updt_applctx
     WHERE ce.charge_event_id=temp_charge_event_id
     WITH nocounter
    ...
    IF (curqual < 1)
     ...
     SET msg =
     "Exiting updatePhysandABNStatusResearchAccount, failed to update research acct on charge_event"
     CALL logmsg(curprog,msg,log_debug)
     CALL writeerrortoreply(idx0,"updatePhysandABNStatusResearchAccount",
      "Failed to update research_acct_id on charge_event",true)
     RETURN(false)
    ENDIF
   ELSEIF ((request->charge[idx0].research_acct_id=- (1))
    AND temp_res_acct_id != 0.0)
    UPDATE  FROM charge_event ce
     SET ce.research_account_id = 0.0, ce.updt_cnt = (ce.updt_cnt+ 1), ce.updt_dt_tm = cnvtdatetime(
       sysdate),
      ce.updt_id = reqinfo->updt_id, ce.updt_task = reqinfo->updt_task, ce.updt_applctx = reqinfo->
      updt_applctx
     WHERE ce.charge_event_id=temp_charge_event_id
    ...
    IF (curqual < 1)
     DECLARE msg = vc WITH protect, noconstant("")
     SET msg =
     "Exiting updatePhysandABNStatusResearchAccount, failed to remove research acct on charge_event"
     CALL logmsg(curprog,msg,log_debug)
     CALL writeerrortoreply(idx0,"updatePhysandABNStatusResearchAccount",
      "Failed to update research_acct_id to 0.0 on charge_event",true)
     RETURN(false)
    ENDIF
   ENDIF
   SELECT INTO "nl:"
    FROM charge_event_act cea
    WHERE cea.charge_event_act_id=temp_charge_act_id
    DETAIL
     temp_pat_resp_flg = cea.patient_responsibility_flag
    WITH nocounter
   ;end select
   IF ((request->charge[idx0].patient_responsibility_flag IN (1, 2))
    AND (temp_pat_resp_flg != request->charge[idx0].patient_responsibility_flag))
    UPDATE  FROM charge_event_act cea
     SET cea.patient_responsibility_flag = request->charge[idx0].patient_responsibility_flag, cea
      .updt_cnt = (cea.updt_cnt+ 1), cea.updt_dt_tm = cnvtdatetime(sysdate),
      cea.updt_id = reqinfo->updt_id, cea.updt_task = reqinfo->updt_task, cea.updt_applctx = reqinfo
      ->updt_applctx
     WHERE cea.charge_event_act_id=temp_charge_act_id
     WITH nocounter
    ;end update
    IF (curqual < 1)
     DECLARE msg = vc WITH protect, noconstant("")
     SET msg =
     "Exiting updatePhysandABNStatusResearchAccount, failed to update pat resp flag on charge_event_act"
     CALL logmsg(curprog,msg,log_debug)
     CALL writeerrortoreply(idx0,"updatePhysandABNStatusResearchAccount",
      "Failed to update pat resp flag on charge_event_act",true)
     RETURN(false)
    ENDIF
   ELSEIF ((request->charge[idx0].patient_responsibility_flag=- (1))
    AND temp_pat_resp_flg != 0)
    UPDATE  FROM charge_event_act cea
     SET cea.patient_responsibility_flag = 0, cea.updt_cnt = (cea.updt_cnt+ 1), cea.updt_dt_tm =
      cnvtdatetime(sysdate),
      cea.updt_id = reqinfo->updt_id, cea.updt_task = reqinfo->updt_task, cea.updt_applctx = reqinfo
      ->updt_applctx
     WHERE cea.charge_event_act_id=temp_charge_act_id
    ...
    IF (curqual < 1)
     ...
     CALL logmsg(curprog,msg,log_debug)
     CALL writeerrortoreply(idx0,"updatePhysandABNStatusResearchAccount",
      "Failed to update pat resp flag to 0 on charge_event_act",true)
     RETURN(false)
    ENDIF
   ENDIF
   CALL logmsg(curprog,"Exiting updatePhysandABNStatusResearchAccount",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (checkforchargeeventmodid(idx0=i4) =null)
   CALL logmsg(curprog,"Entering checkForChargeEventModId",log_debug)
   DECLARE i = i4 WITH protect, noconstant(0)
   FOR (i = 1 TO size(request->charge[idx0].charge_mod,5))
    IF (validate(debug,- (1)) > 0)
     CALL echo(build2("Index:",i))
    ENDIF
    IF ((request->charge[idx0].charge_mod[i].charge_event_mod_id=0)
     AND (request->charge[idx0].charge_mod[i].action_type IN ("UPT", "DEL")))
     IF (validate(debug,- (1)) > 0)
      CALL echo("charge qualified")
     ENDIF
     SELECT INTO "nl:"
      FROM charge_mod cm,
       charge c,
      ...
      PLAN (cm
       WHERE (cm.charge_mod_id=request->charge[idx0].charge_mod[i].charge_mod_id))
       JOIN (c
       WHERE c.charge_item_id=cm.charge_item_id)
       JOIN (cem
       WHERE cem.charge_event_id=c.charge_event_id
        AND cem.field6=cm.field6
        AND cem.active_ind=1
        AND cem.field1_id=cm.field1_id
        AND cem.field2_id=cm.field2_id)
      DETAIL
       request->charge[idx0].charge_mod[i].charge_event_mod_id = cem.charge_event_mod_id
      WITH nocounter
     ;end select
    ENDIF
   ENDFOR
   CALL logmsg(curprog,"Exiting checkForChargeEventModId",log_debug)
 END ;Subroutine
 SUBROUTINE (updatenomenentityrelation(chargefindreply=vc(ref),valchgmodrequest=vc(ref),chargeid=f8,
  pendingcharge=i2) =i2)
   CALL logmsg(curprog,"Entering updateNomenEntityRelation",log_debug)
   DECLARE insertperformed = i2 WITH protect, noconstant(false)
   DECLARE codeset14002meaning = c12 WITH protect, noconstant("")
   DECLARE new_n_id = f8 WITH protect, noconstant(0.0)
   DECLARE ner_id = f8 WITH protect, noconstant(0.0)
   DECLARE old_nomen_id = f8 WITH protect, noconstant(0.0)
   FOR (k = 1 TO valchgmodrequest->charge_mod_qual)
     IF (pendingcharge=true
      AND (valchgmodrequest->objarray[k].action_type="UPT"))
      SET ner_id = 0
      SELECT INTO "nl:"
       FROM nomen_entity_reltn ner,
        charge_mod cm
       PLAN (cm
        WHERE cm.charge_item_id=chargeid
         AND (cm.charge_mod_id=valchgmodrequest->objarray[k].charge_mod_id))
        JOIN (ner
        WHERE ner.parent_entity_id=chargeid
         AND ner.parent_entity_name="CHARGE"
         AND ner.nomenclature_id=cm.nomen_id
         AND ner.active_ind=1
         AND ner.child_entity_name="NOMENCLATURE"
         AND ner.child_entity_id=cm.nomen_id)
       DETAIL
        ner_id = ner.nomen_entity_reltn_id, old_nomen_id = ner.nomenclature_id
       WITH nocounter
      ;end select
      IF (ner_id > 0
       AND (old_nomen_id != valchgmodrequest->objarray[k].nomen_id))
       UPDATE  FROM nomen_entity_reltn ner
        SET ner.nomenclature_id = valchgmodrequest->objarray[k].nomen_id, ner.child_entity_id =
         valchgmodrequest->objarray[k].nomen_id, ner.updt_cnt = (ner.updt_cnt+ 1),
         ner.updt_dt_tm = cnvtdatetime(sysdate), ner.updt_id = reqinfo->updt_id, ner.updt_task =
         reqinfo->updt_task,
         ner.updt_applctx = reqinfo->updt_applctx
        WHERE ner.nomen_entity_reltn_id=ner_id
        WITH nocounter
       ;end update
       IF (curqual < 1)
        CALL logmsg(curprog,"Exiting updateNomenEntityRelation, failed to update nomen_entity_reltn",
         log_debug)
        CALL writeerrortoreply(idx0,"updateNomenEntityRelation","Failed to update Nomen_entity_reltn",
         true)
        RETURN(false)
       ENDIF
      ENDIF
     ELSEIF (pendingcharge=true
      AND (valchgmodrequest->objarray[k].action_type="DEL"))
      SET ner_id = 0
      SELECT INTO "nl:"
       FROM nomen_entity_reltn ner,
        charge_mod cm
      ...
      IF (ner_id > 0
       AND (old_nomen_id != valchgmodrequest->objarray[k].nomen_id))
       UPDATE  FROM nomen_entity_reltn ner
        SET ner.active_ind = 0, ner.updt_cnt = (ner.updt_cnt+ 1), ner.updt_dt_tm = cnvtdatetime(
          sysdate),
         ner.updt_id = reqinfo->updt_id, ner.updt_task = reqinfo->updt_task, ner.updt_applctx =
         reqinfo->updt_applctx
        WHERE ner.nomen_entity_reltn_id=ner_id
        WITH nocounter
       ;end update
       IF (curqual < 1)
        CALL logmsg(curprog,"Exiting updateNomenEntityRelation, failed to update nomen_entity_reltn",
         log_debug)
        CALL writeerrortoreply(idx0,"updateNomenEntityRelation",
         "Failed to inactivate Nomen_entity_reltn",true)
        RETURN(false)
       ENDIF
      ENDIF
     ELSEIF (((pendingcharge=false) OR (((pendingcharge=true
      AND (valchgmodrequest->objarray[k].action_type="ADD")) OR (pendingcharge=true
      AND (valchgmodrequest->objarray[k].action_type="UPT")
      AND ner_id=0)) )) )
      SET codeset14002meaning = uar_get_code_meaning(valchgmodrequest->objarray[k].field1_id)
      IF (((codeset14002meaning="ICD9") OR (((codeset14002meaning="PROCCODE") OR (codeset14002meaning
      ="CPT4")) )) )
       SET new_n_id = 0.0
       SELECT INTO "nl:"
        dd = seq(entity_reltn_seq,nextval)"##################;rp0"
        FROM dual
        DETAIL
         new_n_id = cnvtreal(dd)
        WITH format, counter
       ;end select
       INSERT  FROM nomen_entity_reltn n
        SET n.nomen_entity_reltn_id = new_n_id, n.nomenclature_id = valchgmodrequest->objarray[k].
         nomen_id, n.parent_entity_id = chargeid,
         n.parent_entity_name = "CHARGE", n.child_entity_id = valchgmodrequest->objarray[k].nomen_id,
         n.child_entity_name = "NOMENCLATURE",
         n.reltn_type_cd =
         IF (codeset14002meaning="CPT4") 0.0
         ELSE cs23549_chargeicd9_cd
         ENDIF
         ...
         reqinfo->updt_task,
         n.updt_cnt = 0, n.updt_applctx = reqinfo->updt_applctx, n.person_id = chargefindreply->
         charge_items[1].person_id,
         n.encntr_id = chargefindreply->charge_items[1].encntr_id, n.active_ind = 1
        WITH nocounter
       ;end insert
       IF (curqual < 1)
        CALL logmsg(curprog,"Exiting updateNomenEntityRelation, failed to add nomen_entity_reltn",
         log_debug)
        CALL writeerrortoreply(idx0,"updateNomenEntityRelation","Failed to insert Nomen_entity_reltn",
         true)
        RETURN(false)
       ELSE
        SET insertperformed = true
       ENDIF
      ENDIF
     ENDIF
   ENDFOR
   IF (insertperformed)
    UPDATE  FROM charge c
     SET c.adjusted_dt_tm = cnvtdatetime(sysdate), c.updt_cnt = (c.updt_cnt+ 1), c.updt_dt_tm =
      cnvtdatetime(sysdate),
    ...
    ;end update
    IF (curqual < 1)
     CALL logmsg(curprog,"Exiting updateNomenEntityRelation, failed to update charge",log_debug)
     CALL writeerrortoreply(idx0,"updateNomenEntityRelation","Failed to update charge fields",true)
     RETURN(false)
    ENDIF
   ENDIF
   CALL logmsg(curprog,"Exiting updateNomenEntityRelation",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (performchargereprocess(idx0=i4,chargeitemid=f8,chargeeventid=f8) =i2)
   CALL logmsg(curprog,"Entering performChargeReprocess",log_debug)
   DECLARE hreq = i4 WITH protect, noconstant(0)
   ...
   DECLARE iret = i4 WITH protect, noconstant(0)
   DECLARE hprocess = i4 WITH protect, noconstant(0)
   DECLARE hcharge = i4 WITH protect, noconstant(0)
   DECLARE hchargeeventmod = i4 WITH protect, noconstant(0)
   DECLARE hchargemod = i4 WITH protect, noconstant(0)
   ...
   DECLARE idx = i4 WITH protect, noconstant(0)
   DECLARE idx1 = i4 WITH protect, noconstant(0)
   DECLARE chargeeventmodcount = i4 WITH protect, noconstant(0)
   ...
   DECLARE ichargeloop = i4 WITH protect, noconstant(0)
   DECLARE chrgcnt = i4 WITH noconstant(0)
   DECLARE replypos = i4 WITH noconstant(0)
   DECLARE num = i4 WITH noconstant(0)
   DECLARE cmparser = vc WITH noconstant("1=1")
   RECORD releasereply(
     1 charge[*]
       2 charge_item_id = f8
       2 charge_type_cd = f8
   ...
   IF (size(request->charge[idx0].charge_mod,5) > 0)
    SELECT INTO "nl:"
     FROM charge_event_mod cem,
      code_value cv
     PLAN (cem
      WHERE cem.charge_event_id=chargeeventid
       AND cem.active_ind=1
       AND cem.charge_event_mod_type_cd=cs13019_bill_code_cd
       AND  NOT (expand(idx,1,size(valchgmodrequest->objarray,5),cem.field6,valchgmodrequest->
       objarray[idx].field6,
       "ADD",valchgmodrequest->objarray[idx].action_type,"MODIFIER",uar_get_code_meaning(
        valchgmodrequest->objarray[idx].field1_id)))
       AND  NOT (expand(idx,1,size(valchgmodrequest->objarray,5),cem.field6,valchgmodrequest->
       objarray[idx].field6,
       "UPT",valchgmodrequest->objarray[idx].action_type,"MODIFIER",uar_get_code_meaning(
        valchgmodrequest->objarray[idx].field1_id)))
       AND  NOT (expand(idx,1,size(pendchgmodrec->charge_mods,5),cem.field6,pendchgmodrec->
       charge_mods[idx].field6,
       "MODIFIER",uar_get_code_meaning(pendchgmodrec->charge_mods[idx].field1_id))))
      JOIN (cv
      WHERE cv.code_value=cem.field1_id
       ...
       AND cv.active_ind=1)
     DETAIL
      chargeeventmodcount += 1, stat = alterlist(ignore_mods->ignore_event_mod,chargeeventmodcount),
      ignore_mods->ignore_event_mod[chargeeventmodcount].field6 = cem.field6
     WITH nocounter
    ;end select
   ENDIF
   IF ((request->charge[idx0].charge_item_id=chargeitemid))
    SET cmparser = "1=1"
   ELSE
    SET cmparser = "cm.active_ind = TRUE"
   ENDIF
   SELECT INTO "nl:"
    FROM charge_mod cm,
     code_value cv
    PLAN (cm
     WHERE (cm.charge_item_id=request->charge[idx0].charge_item_id)
      ...
      AND  NOT (expand(idx,1,size(valchgmodrequest->objarray,5),cm.field6,valchgmodrequest->objarray[
      idx].field6,
      "ADD",valchgmodrequest->objarray[idx].action_type,"MODIFIER",uar_get_code_meaning(
       valchgmodrequest->objarray[idx].field1_id)))
      AND  NOT (expand(idx,1,size(valchgmodrequest->objarray,5),cm.field6,valchgmodrequest->objarray[
      ...
      "UPT",valchgmodrequest->objarray[idx].action_type,"MODIFIER",uar_get_code_meaning(
       valchgmodrequest->objarray[idx].field1_id)))
      AND  NOT (expand(idx,1,size(pendchgmodrec->charge_mods,5),cm.field6,pendchgmodrec->charge_mods[
      idx].field6,
      "MODIFIER",uar_get_code_meaning(pendchgmodrec->charge_mods[idx].field1_id))))
     JOIN (cv
     WHERE cv.code_value=cm.field1_id
      ...
      AND cv.active_ind=1)
    DETAIL
     chargemodcount += 1, stat = alterlist(ignore_mods->ignore_charge_mod,chargemodcount),
     ignore_mods->ignore_charge_mod[chargemodcount].field6 = cm.field6
    WITH nocounter
   ;end select
   IF (validate(debug,- (1)) > 0)
    CALL echo("performChargeReprocess - ignore_mods")
    CALL echorecord(ignore_mods)
   ENDIF
   IF ((request->nocommitind=0))
    CALL impersonatepersonnelinfo(1)
    ...
    IF (iret=0)
     ...
     IF (iret=0)
      ...
      IF (iret=0)
       ...
       SET srvstat = uar_srvsetdouble(hprocess,"charge_event_id",chargeeventid)
       SET hcharge = uar_srvadditem(hprocess,"charge_item")
       SET srvstat = uar_srvsetdouble(hcharge,"charge_item_id",chargeitemid)
       SET srvstat = uar_srvsetshort(hprocess,"ignored_event_mod_qual",chargeeventmodcount)
       FOR (nreprocessloop = 1 TO size(ignore_mods->ignore_event_mod,5))
        SET hchargeeventmod = uar_srvadditem(hprocess,"ignored_event_mod")
        SET srvstat = uar_srvsetstring(hchargeeventmod,"field6",nullterm(ignore_mods->
          ignore_event_mod[nreprocessloop].field6))
       ENDFOR
       SET srvstat = uar_srvsetshort(hprocess,"ignored_charge_mod_qual",chargemodcount)
       FOR (nreprocessloop2 = 1 TO size(ignore_mods->ignore_charge_mod,5))
        SET hchargemod = uar_srvadditem(hprocess,"ignored_charge_mod")
        SET srvstat = uar_srvsetstring(hchargemod,"field6",nullterm(ignore_mods->ignore_charge_mod[
          nreprocessloop2].field6))
       ENDFOR
       COMMIT
       SET changescommitted = true
       SET iret = uar_crmperform(hsteprelease)
       IF (iret != 0)
        CALL logmsg(curprog,"Exiting performChargeReprocess, CRM perform failed",log_debug)
        CALL writeerrortoreply(idx0,"performChargeReprocess",
         "Server call 951359 did not return success",true)
       ...
       ELSE
        ...
        IF (hreply=0)
         CALL logmsg(curprog,"performChargeReprocess - Failed to get handle to Reply",log_debug)
         CALL uar_crmendreq(hsteprelease)
         CALL uar_crmendtask(htaskrelease)
         CALL uar_crmendapp(happrelease)
         RETURN(false)
        ...
        IF (ichargecount=0)
         ...
         CALL uar_crmendapp(happrelease)
         CALL writeerrortoreply(idx0,"performChargeReprocess",
          "Server reply 951359 does not contain charges",true)
         CALL uar_crmendreq(hsteprelease)
         CALL uar_crmendtask(htaskrelease)
         CALL uar_crmendapp(happrelease)
         RETURN(false)
        ENDIF
        SET stat = alterlist(releasereply->charge,ichargecount)
        FOR (ichargeloop = 1 TO ichargecount)
          SET hlist = uar_srvgetitem(hreply,"charges",(ichargeloop - 1))
          SET releasereply->charge[ichargeloop].charge_item_id = uar_srvgetdouble(hlist,
           "charge_item_id")
          SET releasereply->charge[ichargeloop].charge_type_cd = uar_srvgetdouble(hlist,
           "charge_type_cd")
          SET releasereply->charge[ichargeloop].process_flg = uar_srvgetshort(hlist,"process_flg")
          SET newcharges->numberofcharges += 1
          SET stat = alterlist(newcharges->charges,newcharges->numberofcharges)
          SET newcharges->charges[newcharges->numberofcharges].charge_item_id = releasereply->charge[
          ichargeloop].charge_item_id
          SET newcharges->charges[newcharges->numberofcharges].process_flg = releasereply->charge[
          ichargeloop].process_flg
          SET replypos = locateval(num,1,size(reply->charge[idx0].new_charges,5),releasereply->
           charge[ichargeloop].charge_item_id,reply->charge[idx0].new_charges[num].new_charge_item_id
           )
          IF (replypos=0)
           IF ((request->contributor_system_cd > 0))
            CALL addcontributorsystemmod(releasereply->charge[ichargeloop].charge_item_id)
           ENDIF
           SET chrgcnt = (size(reply->charge[idx0].new_charges,5)+ 1)
           SET stat = alterlist(reply->charge[idx0].new_charges,chrgcnt)
           SET reply->charge[idx0].new_charges[chrgcnt].new_charge_item_id = releasereply->charge[
           ichargeloop].charge_item_id
           SET reply->charge[idx0].new_charges[chrgcnt].charge_type_cd = releasereply->charge[
           ichargeloop].charge_type_cd
           SET reply->charge[idx0].new_charges[chrgcnt].action_type = "ADD"
          ENDIF
        ENDFOR
       ...
       CALL uar_crmendreq(hsteprelease)
      ELSE
       CALL logmsg(curprog,"Exiting performChargeReprocess, Begin request unsuccessful",log_debug)
       CALL uar_crmendtask(htaskrelease)
       CALL uar_crmendapp(happrelease)
       RETURN(false)
      ENDIF
      CALL uar_crmendtask(htaskrelease)
     ELSE
      CALL logmsg(curprog,"Exiting performChargeReprocess, Unsuccessful begin task",log_debug)
      CALL uar_crmendapp(happrelease)
      RETURN(false)
     ENDIF
     CALL uar_crmendapp(happrelease)
    ELSE
     CALL logmsg(curprog,"Exiting performChargeReprocess, Begin app failed with code",log_debug)
     RETURN(false)
    ENDIF
   ELSE
    CALL logmsg(curprog,"NoCommitInd != 0. Skipping call to server in performChargeReprocess.",
     log_debug)
   ENDIF
   CALL logmsg(curprog,"Exiting performChargeReprocess",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (modifychargemods(dummyvar=i2) =i2)
   RECORD chg_mod_updates(
     1 charge_mod_qual = i4
     1 charge_mod[*]
       2 action_type = c3
       2 charge_mod_id = f8
   ...
   IF (validate(debug,- (1)) > 0)
    CALL echo("Adding a charge mod, request structure is:")
    CALL echorecord(valchgmodrequest)
   ENDIF
   SET stat = initrec(changelogrecord)
   IF (size(valchgmodrequest->objarray,5) > 0
    AND (valchgmodrequest->objarray[1].charge_item_id=request->charge[idx0].charge_item_id))
    SET stat = movereclist(valchgmodrequest->objarray,chg_mod_updates->charge_mod,1,0,size(
      valchgmodrequest->objarray,5),
     1)
    SET chg_mod_updates->charge_mod_qual = size(chg_mod_updates->charge_mod,5)
    IF (validate(debug,- (1)) > 0)
     CALL echorecord(chg_mod_updates)
    ENDIF
    CALL logmsg(curprog,"modifyChargeMods: saveChargeModChangeLogAttributes",log_debug)
    CALL savechargemodchangelogattributes(chg_mod_updates)
   ENDIF
   EXECUTE afc_val_charge_mod  WITH replace("REQUEST",valchgmodrequest), replace("REPLY",valchgmodrep
    )
   IF ((valchgmodrep->status_data.status != "S"))
    CALL logmsg(curprog,"Exiting modifyChargeMods, failed to add charge mod",log_debug)
    CALL writeerrortoreply(idx0,"modifyChargeMods","afc_val_charge_mod did not return success",true)
    RETURN(false)
   ENDIF
   CALL logmsg(curprog,"Exiting modifyChargeMods",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (addoriginalchargemod(idx0=i4,idx1=i4,chargeid=f8,chargefindreply=vc(ref)) =null)
   CALL logmsg(curprog,"Entering addOriginalChargeMod",log_debug)
   DECLARE pos = i4 WITH protect, noconstant(0)
   DECLARE idx = i4 WITH protect, noconstant(0)
   SET pos = locateval(idx,1,size(request->charge[idx0].charge_mod,5),chargefindreply->charge_items[1
    ].charge_mods[idx1].charge_mod_id,request->charge[idx0].charge_mod[idx].charge_mod_id)
   IF ((((request->charge[idx0].charge_mod[pos].action_type="EXI")) OR (pos <= 0)) )
    DECLARE chargemodcount = i4 WITH noconstant((valchgmodrequest->charge_mod_qual+ 1))
    SET stat = alterlist(valchgmodrequest->objarray,chargemodcount)
    SET valchgmodrequest->charge_mod_qual = chargemodcount
    SET valchgmodrequest->objarray[chargemodcount].action_type = "ADD"
    SET valchgmodrequest->objarray[chargemodcount].charge_mod_id = 0.0
    SET valchgmodrequest->objarray[chargemodcount].charge_item_id = chargeid
    SET valchgmodrequest->objarray[chargemodcount].charge_mod_type_cd = chargefindreply->
    charge_items[1].charge_mods[idx1].charge_mod_type_cd
    SET valchgmodrequest->objarray[chargemodcount].field1 = chargefindreply->charge_items[1].
    charge_mods[idx1].field1
    SET valchgmodrequest->objarray[chargemodcount].field2 = chargefindreply->charge_items[1].
    charge_mods[idx1].field2
    SET valchgmodrequest->objarray[chargemodcount].field3 = chargefindreply->charge_items[1].
    charge_mods[idx1].field3
    SET valchgmodrequest->objarray[chargemodcount].field4 = chargefindreply->charge_items[1].
    charge_mods[idx1].field4
    SET valchgmodrequest->objarray[chargemodcount].field5 = chargefindreply->charge_items[1].
    charge_mods[idx1].field5
    SET valchgmodrequest->objarray[chargemodcount].field6 = chargefindreply->charge_items[1].
    charge_mods[idx1].field6
    SET valchgmodrequest->objarray[chargemodcount].field7 = chargefindreply->charge_items[1].
    charge_mods[idx1].field7
    SET valchgmodrequest->objarray[chargemodcount].field8 = chargefindreply->charge_items[1].
    charge_mods[idx1].field8
    SET valchgmodrequest->objarray[chargemodcount].field9 = chargefindreply->charge_items[1].
    charge_mods[idx1].field9
    SET valchgmodrequest->objarray[chargemodcount].field10 = chargefindreply->charge_items[1].
    charge_mods[idx1].field10
    SET valchgmodrequest->objarray[chargemodcount].activity_dt_tm = chargefindreply->charge_items[1].
    charge_mods[idx1].activity_dt_tm
    SET valchgmodrequest->objarray[chargemodcount].active_ind = chargefindreply->charge_items[1].
    charge_mods[idx1].active_ind
    SET valchgmodrequest->objarray[chargemodcount].active_status_cd = chargefindreply->charge_items[1
    ].charge_mods[idx1].active_status_cd
    SET valchgmodrequest->objarray[chargemodcount].active_status_dt_tm = chargefindreply->
    charge_items[1].charge_mods[idx1].active_status_dt_tm
    SET valchgmodrequest->objarray[chargemodcount].active_status_prsnl_id = chargefindreply->
    charge_items[1].charge_mods[idx1].active_status_prsnl_id
    SET valchgmodrequest->objarray[chargemodcount].beg_effective_dt_tm = chargefindreply->
    charge_items[1].charge_mods[idx1].beg_effective_dt_tm
    SET valchgmodrequest->objarray[chargemodcount].end_effective_dt_tm = chargefindreply->
    charge_items[1].charge_mods[idx1].end_effective_dt_tm
    SET valchgmodrequest->objarray[chargemodcount].field1_id = chargefindreply->charge_items[1].
    charge_mods[idx1].field1_id
    SET valchgmodrequest->objarray[chargemodcount].field2_id = chargefindreply->charge_items[1].
    charge_mods[idx1].field2_id
    SET valchgmodrequest->objarray[chargemodcount].field3_id = chargefindreply->charge_items[1].
    charge_mods[idx1].field3_id
    SET valchgmodrequest->objarray[chargemodcount].field4_id = chargefindreply->charge_items[1].
    charge_mods[idx1].field4_id
    SET valchgmodrequest->objarray[chargemodcount].field5_id = chargefindreply->charge_items[1].
    charge_mods[idx1].field5_id
    SET valchgmodrequest->objarray[chargemodcount].cm1_nbr = chargefindreply->charge_items[1].
    charge_mods[idx1].cm1_nbr
    SET valchgmodrequest->objarray[chargemodcount].nomen_id = chargefindreply->charge_items[1].
    charge_mods[idx1].nomen_id
    SET valchgmodrequest->objarray[chargemodcount].updt_cnt = chargefindreply->charge_items[1].
    charge_mods[idx1].updt_cnt
    SET valchgmodrequest->objarray[chargemodcount].code1_cd = chargefindreply->charge_items[1].
    charge_mods[idx1].code1_cd
    SET valchgmodrequest->objarray[chargemodcount].charge_mod_source_cd = chargefindreply->
    charge_items[1].charge_mods[idx1].charge_mod_source_cd
   ENDIF
   CALL logmsg(curprog,"Exiting addOriginalChargeMod",log_debug)
 END ;Subroutine
 SUBROUTINE (addchargemodrec(idx0=i4,idx1=i4,chargeid=f8) =null)
   CALL logmsg(curprog,"Entering addChargeModRec",log_debug)
   DECLARE chargemodcount = i4 WITH noconstant((valchgmodrequest->charge_mod_qual+ 1))
   SET stat = alterlist(valchgmodrequest->objarray,chargemodcount)
   SET valchgmodrequest->charge_mod_qual = chargemodcount
   SET valchgmodrequest->objarray[chargemodcount].action_type = "ADD"
   SET valchgmodrequest->objarray[chargemodcount].charge_mod_id = 0.0
   SET valchgmodrequest->objarray[chargemodcount].charge_item_id = chargeid
   SET valchgmodrequest->objarray[chargemodcount].charge_mod_type_cd = request->charge[idx0].
   charge_mod[idx1].charge_mod_type_cd
   SET valchgmodrequest->objarray[chargemodcount].field1 = request->charge[idx0].charge_mod[idx1].
   field1
   SET valchgmodrequest->objarray[chargemodcount].field2 = request->charge[idx0].charge_mod[idx1].
   field2
   SET valchgmodrequest->objarray[chargemodcount].field3 = request->charge[idx0].charge_mod[idx1].
   field3
   SET valchgmodrequest->objarray[chargemodcount].field4 = request->charge[idx0].charge_mod[idx1].
   field4
   SET valchgmodrequest->objarray[chargemodcount].field5 = request->charge[idx0].charge_mod[idx1].
   field5
   SET valchgmodrequest->objarray[chargemodcount].field6 = request->charge[idx0].charge_mod[idx1].
   field6
   SET valchgmodrequest->objarray[chargemodcount].field7 = request->charge[idx0].charge_mod[idx1].
   field7
   SET valchgmodrequest->objarray[chargemodcount].field8 = request->charge[idx0].charge_mod[idx1].
   field8
   SET valchgmodrequest->objarray[chargemodcount].field9 = request->charge[idx0].charge_mod[idx1].
   field9
   SET valchgmodrequest->objarray[chargemodcount].field10 = request->charge[idx0].charge_mod[idx1].
   field10
   SET valchgmodrequest->objarray[chargemodcount].active_ind = true
   SET valchgmodrequest->objarray[chargemodcount].field1_id = request->charge[idx0].charge_mod[idx1].
   field1_id
   SET valchgmodrequest->objarray[chargemodcount].field2_id = request->charge[idx0].charge_mod[idx1].
   field2_id
   SET valchgmodrequest->objarray[chargemodcount].field3_id = request->charge[idx0].charge_mod[idx1].
   field3_id
   SET valchgmodrequest->objarray[chargemodcount].field4_id = request->charge[idx0].charge_mod[idx1].
   field4_id
   SET valchgmodrequest->objarray[chargemodcount].field5_id = request->charge[idx0].charge_mod[idx1].
   field5_id
   SET valchgmodrequest->objarray[chargemodcount].cm1_nbr = request->charge[idx0].charge_mod[idx1].
   cm1_nbr
   SET valchgmodrequest->objarray[chargemodcount].nomen_id = request->charge[idx0].charge_mod[idx1].
   nomen_id
   SET valchgmodrequest->objarray[chargemodcount].code1_cd = request->charge[idx0].charge_mod[idx1].
   code1_cd
   SET valchgmodrequest->objarray[chargemodcount].activity_dt_tm = request->charge[idx0].charge_mod[
   idx1].activity_dt_tm
   SET valchgmodrequest->objarray[chargemodcount].updt_cnt = 0
   SET valchgmodrequest->objarray[chargemodcount].charge_mod_source_cd = request->charge[idx0].
   charge_mod[idx1].charge_mod_source_cd
   CALL logmsg(curprog,"Exiting addChargeModRec",log_debug)
   IF (validate(debug,- (1)) > 0)
    CALL echo("addChargeModRec: populated ValChgModRequest")
    CALL echorecord(valchgmodrequest)
   ENDIF
 END ;Subroutine
 SUBROUTINE (adduptchrgmodrec(idx0=i4,idx1=i4,chargeid=f8) =null)
   CALL logmsg(curprog,"Entering addUptChrgModRec",log_debug)
   DECLARE chargemodcount = i4 WITH noconstant((valchgmodrequest->charge_mod_qual+ 1))
   DECLARE findreplyindex = i4 WITH noconstant(0)
   DECLARE num = i4 WITH noconstant(0)
   SET findreplyindex = locateval(num,1,size(chargefindreply->charge_items[1].charge_mods,5),request
    ->charge[idx0].charge_mod[idx1].charge_mod_id,chargefindreply->charge_items[1].charge_mods[num].
    charge_mod_id)
   SET stat = alterlist(valchgmodrequest->objarray,chargemodcount)
   SET valchgmodrequest->charge_mod_qual = chargemodcount
   SET valchgmodrequest->objarray[chargemodcount].action_type = "UPT"
   SET valchgmodrequest->objarray[chargemodcount].charge_mod_id = request->charge[idx0].charge_mod[
   idx1].charge_mod_id
   SET valchgmodrequest->objarray[chargemodcount].charge_item_id = chargeid
   SET valchgmodrequest->objarray[chargemodcount].charge_mod_type_cd = request->charge[idx0].
   charge_mod[idx1].charge_mod_type_cd
   ...
   charge_mod[idx1].charge_mod_source_cd
   IF (findreplyindex > 0)
    IF ((valchgmodrequest->objarray[chargemodcount].activity_dt_tm=0))
     SET valchgmodrequest->objarray[chargemodcount].activity_dt_tm = chargefindreply->charge_items[1]
     .charge_mods[findreplyindex].activity_dt_tm
    ENDIF
    SET valchgmodrequest->objarray[chargemodcount].active_status_cd = chargefindreply->charge_items[1
    ].charge_mods[findreplyindex].active_status_cd
    SET valchgmodrequest->objarray[chargemodcount].beg_effective_dt_tm = chargefindreply->
    charge_items[1].charge_mods[findreplyindex].beg_effective_dt_tm
    SET valchgmodrequest->objarray[chargemodcount].end_effective_dt_tm = chargefindreply->
    charge_items[1].charge_mods[findreplyindex].end_effective_dt_tm
    SET valchgmodrequest->objarray[chargemodcount].updt_cnt = chargefindreply->charge_items[1].
    charge_mods[findreplyindex].updt_cnt
   ENDIF
   CALL logmsg(curprog,"Exiting addUptChrgModRec",log_debug)
   IF (validate(debug,- (1)) > 0)
    CALL echo("addUptChrgModRec: populated ValChgModRequest")
    CALL echorecord(valchgmodrequest)
   ENDIF
 END ;Subroutine
 SUBROUTINE (adddelchrgmodrec(idx0=i4,idx1=i4) =null)
   CALL logmsg(curprog,"Entering addDelChrgModRec",log_debug)
   DECLARE chargemodcount = i4 WITH noconstant((valchgmodrequest->charge_mod_qual+ 1))
   DECLARE findreplyindex = i4 WITH noconstant(0)
   DECLARE num = i4 WITH noconstant(0)
   SET findreplyindex = locateval(num,1,size(chargefindreply->charge_items[1].charge_mods,5),request
   ...
   SET stat = alterlist(valchgmodrequest->objarray,chargemodcount)
   SET valchgmodrequest->charge_mod_qual = chargemodcount
   SET valchgmodrequest->objarray[chargemodcount].action_type = "DEL"
   SET valchgmodrequest->objarray[chargemodcount].charge_item_id = request->charge[idx0].
   charge_item_id
   SET valchgmodrequest->objarray[chargemodcount].charge_mod_id = request->charge[idx0].charge_mod[
   idx1].charge_mod_id
   SET valchgmodrequest->objarray[chargemodcount].active_status_cd = cs48_inactive_cd
   IF (findreplyindex > 0)
    SET valchgmodrequest->objarray[chargemodcount].beg_effective_dt_tm = chargefindreply->
    charge_items[1].charge_mods[findreplyindex].beg_effective_dt_tm
    SET valchgmodrequest->objarray[chargemodcount].end_effective_dt_tm = chargefindreply->
   ...
   ENDIF
   IF (validate(debug,- (1)) > 0)
    CALL echo("addDelChrgModRec: populated ValChgModRequest")
    CALL echorecord(valchgmodrequest)
   ENDIF
   CALL logmsg(curprog,"Exiting addDelChrgModRec",log_debug)
 END ;Subroutine
 SUBROUTINE (adjustpriorities(pendingcharge=i2) =null)
   CALL logmsg(curprog,"Entering adjustPriorities",log_debug)
   DECLARE existingindex1 = i4 WITH noconstant(0)
   DECLARE existingindex2 = i4 WITH noconstant(0)
   DECLARE num = i4 WITH noconstant(0)
   DECLARE cmidx = i4 WITH protect, noconstant(0)
   DECLARE findreplyidx = i4 WITH protect, noconstant(0)
   DECLARE chgmodcnt = i4 WITH protect, noconstant(0)
   DECLARE cmidaddedalready = i4 WITH protect, noconstant(0)
   DECLARE sched_cdf_meaning = vc WITH protect, noconstant("")
   SET stat = copyrec(valchgmodrequest,temprec,0)
   SELECT INTO "nl:"
    sortval = evaluate(uar_get_code_meaning(valchgmodrequest->objarray[d1.seq].charge_mod_source_cd),
     "COPYFROMCEM",1,"MANUALLY_ADD",2,
     "REF_DATA",3,4), sched_meaning = evaluate(valchgmodrequest->objarray[d1.seq].charge_mod_type_cd,
     cs13019_bill_code_cd,uar_get_code_meaning(valchgmodrequest->objarray[d1.seq].field1_id),""),
    priority = valchgmodrequest->objarray[d1.seq].field2_id
    FROM (dummyt d1  WITH seq = value(size(valchgmodrequest->objarray,5)))
    PLAN (d1)
    ORDER BY sched_meaning, sortval, priority
    HEAD REPORT
     stat = alterlist(temprec->objarray,size(valchgmodrequest->objarray,5)), tempmodcount = 0
    DETAIL
     tempmodcount += 1, stat = assign(validate(temprec->objarray[tempmodcount].action_type),validate(
       valchgmodrequest->objarray[d1.seq].action_type,0.0)), stat = assign(validate(temprec->
       objarray[tempmodcount].charge_mod_id),validate(valchgmodrequest->objarray[d1.seq].
       charge_mod_id,0.0)),
     stat = assign(validate(temprec->objarray[tempmodcount].charge_item_id),validate(valchgmodrequest
       ->objarray[d1.seq].charge_item_id,0.0)), stat = assign(validate(temprec->objarray[tempmodcount
       ].charge_mod_type_cd),validate(valchgmodrequest->objarray[d1.seq].charge_mod_type_cd,0.0)),
     stat = assign(validate(temprec->objarray[tempmodcount].field1),validate(valchgmodrequest->
       objarray[d1.seq].field1,"")),
     stat = assign(validate(temprec->objarray[tempmodcount].field2),validate(valchgmodrequest->
       objarray[d1.seq].field2,"")), stat = assign(validate(temprec->objarray[tempmodcount].field3),
      validate(valchgmodrequest->objarray[d1.seq].field3,"")), stat = assign(validate(temprec->
       objarray[tempmodcount].field4),validate(valchgmodrequest->objarray[d1.seq].field4,"")),
     stat = assign(validate(temprec->objarray[tempmodcount].field5),validate(valchgmodrequest->
       objarray[d1.seq].field5,"")), stat = assign(validate(temprec->objarray[tempmodcount].field6),
      validate(valchgmodrequest->objarray[d1.seq].field6,"")), stat = assign(validate(temprec->
       objarray[tempmodcount].field7),validate(valchgmodrequest->objarray[d1.seq].field7,"")),
     stat = assign(validate(temprec->objarray[tempmodcount].field8),validate(valchgmodrequest->
       objarray[d1.seq].field8,"")), stat = assign(validate(temprec->objarray[tempmodcount].field9),
      validate(valchgmodrequest->objarray[d1.seq].field9,"")), stat = assign(validate(temprec->
       objarray[tempmodcount].field10),validate(valchgmodrequest->objarray[d1.seq].field10,"")),
     stat = assign(validate(temprec->objarray[tempmodcount].updt_cnt),validate(valchgmodrequest->
       objarray[d1.seq].updt_cnt,0)), stat = assign(validate(temprec->objarray[tempmodcount].
       active_ind),validate(valchgmodrequest->objarray[d1.seq].active_ind,0)), stat = assign(validate
      (temprec->objarray[tempmodcount].active_status_cd),validate(valchgmodrequest->objarray[d1.seq].
       active_status_cd,0.0)),
     stat = assign(validate(temprec->objarray[tempmodcount].active_status_dt_tm),validate(
       valchgmodrequest->objarray[d1.seq].active_status_dt_tm,0)), stat = assign(validate(temprec->
       objarray[tempmodcount].active_status_prsnl_id),validate(valchgmodrequest->objarray[d1.seq].
       active_status_prsnl_id,0.0)), stat = assign(validate(temprec->objarray[tempmodcount].
       beg_effective_dt_tm),validate(valchgmodrequest->objarray[d1.seq].beg_effective_dt_tm,0)),
     stat = assign(validate(temprec->objarray[tempmodcount].end_effective_dt_tm),validate(
       valchgmodrequest->objarray[d1.seq].end_effective_dt_tm,0)), stat = assign(validate(temprec->
       objarray[tempmodcount].code1_cd),validate(valchgmodrequest->objarray[d1.seq].code1_cd,0.0)),
     stat = assign(validate(temprec->objarray[tempmodcount].nomen_id),validate(valchgmodrequest->
       objarray[d1.seq].nomen_id,0.0)),
     stat = assign(validate(temprec->objarray[tempmodcount].field1_id),validate(valchgmodrequest->
       objarray[d1.seq].field1_id,0.0)), stat = assign(validate(temprec->objarray[tempmodcount].
       field2_id),validate(valchgmodrequest->objarray[d1.seq].field2_id,0.0)), stat = assign(validate
      (temprec->objarray[tempmodcount].field3_id),validate(valchgmodrequest->objarray[d1.seq].
       field3_id,0.0)),
     stat = assign(validate(temprec->objarray[tempmodcount].field4_id),validate(valchgmodrequest->
       objarray[d1.seq].field4_id,0.0)), stat = assign(validate(temprec->objarray[tempmodcount].
       field5_id),validate(valchgmodrequest->objarray[d1.seq].field5_id,0.0)), stat = assign(validate
      (temprec->objarray[tempmodcount].cm1_nbr),validate(valchgmodrequest->objarray[d1.seq].cm1_nbr,
       0.0)),
     stat = assign(validate(temprec->objarray[tempmodcount].activity_dt_tm),validate(valchgmodrequest
       ->objarray[d1.seq].activity_dt_tm,0)), stat = assign(validate(temprec->objarray[tempmodcount].
       charge_mod_source_cd),validate(valchgmodrequest->objarray[d1.seq].charge_mod_source_cd,0.0))
    WITH nocounter
   ;end select
   IF (size(temprec->objarray,5) > 0)
    SET stat = movereclist(temprec->objarray,valchgmodrequest->objarray,1,1,size(temprec->objarray,5),
     0)
    IF (validate(debug,- (1)) > 0)
     CALL echo("ValChgModRequest After Sorting")
     CALL echorecord(valchgmodrequest)
    ENDIF
   ENDIF
   FREE RECORD temprec
   IF (pendingcharge)
    IF (validate(debug,- (1)) > 0)
     CALL logmsg(curprog,"adjustPriorities - Pending Charge",log_debug)
    ENDIF
    FOR (cmidx = 1 TO size(chargefindreply->charge_items[1].charge_mods,5))
      SET cmidaddedalready = 0
      SET cmidaddedalready = locateval(num,1,size(valchgmodrequest->objarray,5),chargefindreply->
       charge_items[1].charge_mods[cmidx].charge_mod_id,valchgmodrequest->objarray[num].charge_mod_id
       )
      IF ((chargefindreply->charge_items[1].charge_mods[cmidx].active_ind=1)
       AND (chargefindreply->charge_items[1].charge_mods[cmidx].charge_mod_type_cd=
      cs13019_bill_code_cd)
       AND cmidaddedalready=0)
       IF (validate(debug,- (1)) > 0)
        CALL echo("Adding charge_mod_id:")
        CALL echo(chargefindreply->charge_items[1].charge_mods[cmidx].charge_mod_id)
       ENDIF
       SET chgmodcnt += 1
       SET stat = alterlist(pendchgmodrec->charge_mods,chgmodcnt)
       SET pendchgmodrec->charge_mods[chgmodcnt].field1_id = chargefindreply->charge_items[1].
       charge_mods[cmidx].field1_id
       SET pendchgmodrec->charge_mods[chgmodcnt].field2_id = chargefindreply->charge_items[1].
       charge_mods[cmidx].field2_id
       SET pendchgmodrec->charge_mods[chgmodcnt].field6 = chargefindreply->charge_items[1].
       charge_mods[cmidx].field6
       SET pendchgmodrec->charge_mods[chgmodcnt].charge_mod_type_cd = chargefindreply->charge_items[1
       ].charge_mods[cmidx].charge_mod_type_cd
       SET pendchgmodrec->charge_mods[chgmodcnt].find_reply_index = cmidx
       SET pendchgmodrec->charge_mods[chgmodcnt].added = 0
      ENDIF
    ENDFOR
    IF (validate(debug,- (1)) > 0)
     CALL echo("pendChgModRec:")
     CALL echorecord(pendchgmodrec)
    ENDIF
   ENDIF
   SET cmidx = 0
   WHILE (cmidx < size(valchgmodrequest->objarray,5))
     SET cmidx += 1
     IF (validate(debug,- (1)) > 0)
      CALL logmsg(curprog,"While Loop",log_debug)
      CALL echo(build(cmidx,"_of_",size(valchgmodrequest->objarray,5)))
     ENDIF
     IF ((valchgmodrequest->objarray[cmidx].charge_mod_type_cd=cs13019_bill_code_cd))
      SET sched_cdf_meaning = uar_get_code_meaning(valchgmodrequest->objarray[cmidx].field1_id)
      IF (validate(debug,- (1)) > 0)
       CALL echo(build("sched_cdf_meaning: ",sched_cdf_meaning))
      ENDIF
      SET existingindex1 = size(valchgmodrequest->objarray,5)
      WHILE (existingindex1 > 0)
        SET existingindex1 = locateval(num,1,(cmidx - 1),valchgmodrequest->objarray[cmidx].
         charge_mod_type_cd,valchgmodrequest->objarray[num].charge_mod_type_cd,
         sched_cdf_meaning,uar_get_code_meaning(valchgmodrequest->objarray[num].field1_id),
         valchgmodrequest->objarray[cmidx].field2_id,valchgmodrequest->objarray[num].field2_id)
        IF (existingindex1 > 0)
         IF (validate(debug,- (1)) > 0)
          CALL echo("incrementing priority")
         ENDIF
         SET valchgmodrequest->objarray[cmidx].field2_id += 1
        ENDIF
        IF (pendingcharge)
         SET existingindex2 = locateval(num,1,size(pendchgmodrec->charge_mods,5),valchgmodrequest->
          objarray[cmidx].charge_mod_type_cd,pendchgmodrec->charge_mods[num].charge_mod_type_cd,
          sched_cdf_meaning,uar_get_code_meaning(pendchgmodrec->charge_mods[num].field1_id),
          valchgmodrequest->objarray[cmidx].field2_id,pendchgmodrec->charge_mods[num].field2_id,0,
          pendchgmodrec->charge_mods[num].added)
         IF (existingindex2 > 0)
          IF (validate(debug,- (1)) > 0)
           CALL echo("found existing value, adding it to ValChgModRequest")
          ENDIF
          SET findreplyidx = pendchgmodrec->charge_mods[existingindex2].find_reply_index
          SET pendchgmodrec->charge_mods[existingindex2].added = 1
          SET valchgmodrequest->charge_mod_qual += 1
          SET chgmodcnt = valchgmodrequest->charge_mod_qual
          SET stat = alterlist(valchgmodrequest->objarray,valchgmodrequest->charge_mod_qual)
          SET valchgmodrequest->objarray[chgmodcnt].action_type = "UPT"
          SET valchgmodrequest->objarray[chgmodcnt].charge_mod_id = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].charge_mod_id
          SET valchgmodrequest->objarray[chgmodcnt].charge_item_id = chargefindreply->charge_items[1]
          .charge_item_id
          SET valchgmodrequest->objarray[chgmodcnt].charge_mod_type_cd = chargefindreply->
          charge_items[1].charge_mods[findreplyidx].charge_mod_type_cd
          SET valchgmodrequest->objarray[chgmodcnt].field1 = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].field1
          SET valchgmodrequest->objarray[chgmodcnt].field2 = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].field2
          SET valchgmodrequest->objarray[chgmodcnt].field3 = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].field3
          SET valchgmodrequest->objarray[chgmodcnt].field4 = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].field4
          SET valchgmodrequest->objarray[chgmodcnt].field5 = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].field5
          SET valchgmodrequest->objarray[chgmodcnt].field6 = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].field6
          SET valchgmodrequest->objarray[chgmodcnt].field7 = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].field7
          SET valchgmodrequest->objarray[chgmodcnt].field8 = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].field8
          SET valchgmodrequest->objarray[chgmodcnt].field9 = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].field9
          SET valchgmodrequest->objarray[chgmodcnt].field10 = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].field10
          SET valchgmodrequest->objarray[chgmodcnt].field1_id = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].field1_id
          SET valchgmodrequest->objarray[chgmodcnt].field2_id = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].field2_id
          SET valchgmodrequest->objarray[chgmodcnt].field3_id = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].field3_id
          SET valchgmodrequest->objarray[chgmodcnt].field4_id = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].field4_id
          SET valchgmodrequest->objarray[chgmodcnt].field5_id = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].field5_id
          SET valchgmodrequest->objarray[chgmodcnt].nomen_id = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].nomen_id
          SET valchgmodrequest->objarray[chgmodcnt].activity_dt_tm = chargefindreply->charge_items[1]
          .charge_mods[findreplyidx].activity_dt_tm
          SET valchgmodrequest->objarray[chgmodcnt].active_ind = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].active_ind
          SET valchgmodrequest->objarray[chgmodcnt].active_status_cd = chargefindreply->charge_items[
          1].charge_mods[findreplyidx].active_status_cd
          SET valchgmodrequest->objarray[chgmodcnt].beg_effective_dt_tm = chargefindreply->
          charge_items[1].charge_mods[findreplyidx].beg_effective_dt_tm
          SET valchgmodrequest->objarray[chgmodcnt].end_effective_dt_tm = chargefindreply->
          charge_items[1].charge_mods[findreplyidx].end_effective_dt_tm
          SET valchgmodrequest->objarray[chgmodcnt].updt_cnt = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].updt_cnt
          SET valchgmodrequest->objarray[chgmodcnt].cm1_nbr = chargefindreply->charge_items[1].
          charge_mods[findreplyidx].cm1_nbr
         ENDIF
        ENDIF
      ENDWHILE
     ENDIF
   ENDWHILE
   IF (validate(debug,- (1)) > 0)
    CALL logmsg(curprog,"After adjusting Priorities",log_debug)
    CALL echorecord(valchgmodrequest)
   ENDIF
   CALL logmsg(curprog,"Exiting adjustPriorities",log_debug)
 END ;Subroutine
 SUBROUTINE (copyexistingreasoncommentmods(chargefindreply=vc(ref),newdebitchargeid=f8) =null)
   CALL logmsg(curprog,"Entering copyExistingReasonCommentMods",log_debug)
   DECLARE chargemodcount = i4 WITH noconstant(valchgmodrequest->charge_mod_qual)
   FOR (idx1 = 1 TO size(chargefindreply->charge_items[1].charge_mods,5))
     IF ((chargefindreply->charge_items[1].charge_mods[idx1].charge_mod_type_cd=cs13019_mod_rsn_cd))
      SET chargemodcount += 1
      SET stat = alterlist(valchgmodrequest->objarray,chargemodcount)
      SET valchgmodrequest->charge_mod_qual = chargemodcount
      SET valchgmodrequest->objarray[valchgmodrequest->charge_mod_qual].action_type = "ADD"
      SET valchgmodrequest->objarray[chargemodcount].charge_mod_id = 0.0
      SET valchgmodrequest->objarray[chargemodcount].charge_item_id = newdebitchargeid
      SET valchgmodrequest->objarray[chargemodcount].charge_mod_type_cd = cs13019_mod_rsn_cd
      SET valchgmodrequest->objarray[chargemodcount].field1 = chargefindreply->charge_items[1].
      charge_mods[idx1].field1
      SET valchgmodrequest->objarray[chargemodcount].field2 = chargefindreply->charge_items[1].
      charge_mods[idx1].field2
      ...
      SET valchgmodrequest->objarray[chargemodcount].active_ind = chargefindreply->charge_items[1].
      charge_mods[idx1].active_ind
      SET valchgmodrequest->objarray[chargemodcount].active_status_cd = chargefindreply->
      charge_items[1].charge_mods[idx1].active_status_cd
      SET valchgmodrequest->objarray[chargemodcount].active_status_dt_tm = chargefindreply->
      charge_items[1].charge_mods[idx1].active_status_dt_tm
      SET valchgmodrequest->objarray[chargemodcount].active_status_prsnl_id = chargefindreply->
      charge_items[1].charge_mods[idx1].active_status_prsnl_id
     ...
     ENDIF
   ENDFOR
   CALL logmsg(curprog,"Exiting copyExistingReasonCommentMods",log_debug)
 END ;Subroutine
 SUBROUTINE (addreasoncommentmod(newdebitchargeid=f8) =null)
   CALL logmsg(curprog,"Entering addReasonCommentMod",log_debug)
   DECLARE chargemodcount = i4 WITH noconstant((valchgmodrequest->charge_mod_qual+ 1))
   SET stat = alterlist(valchgmodrequest->objarray,chargemodcount)
   SET valchgmodrequest->charge_mod_qual = chargemodcount
   SET valchgmodrequest->objarray[chargemodcount].action_type = "ADD"
   SET valchgmodrequest->objarray[chargemodcount].charge_item_id = newdebitchargeid
   SET valchgmodrequest->objarray[chargemodcount].charge_mod_type_cd = cs13019_mod_rsn_cd
   SET valchgmodrequest->objarray[chargemodcount].field6 = "The charge was modified"
   SET valchgmodrequest->objarray[chargemodcount].field7 = newreasoncomment
   SET valchgmodrequest->objarray[chargemodcount].activity_dt_tm = cnvtdatetime(sysdate)
   SET valchgmodrequest->objarray[chargemodcount].active_ind = true
   SET valchgmodrequest->objarray[chargemodcount].active_status_cd = cs48_active_cd
   SET valchgmodrequest->objarray[chargemodcount].active_status_dt_tm = cnvtdatetime(sysdate)
   SET valchgmodrequest->objarray[chargemodcount].active_status_prsnl_id = reqinfo->updt_id
   SET valchgmodrequest->objarray[chargemodcount].beg_effective_dt_tm = cnvtdatetime(sysdate)
   SET valchgmodrequest->objarray[chargemodcount].end_effective_dt_tm = cnvtdatetime(
    "31-DEC-2100 23:59:59.00")
   SET valchgmodrequest->objarray[chargemodcount].field1_id = cs4001989_modify_cd
   SET valchgmodrequest->objarray[chargemodcount].field2_id = newsuspreasoncd
   SET valchgmodrequest->objarray[chargemodcount].updt_cnt = 0
   CALL logmsg(curprog,"Exiting addReasonCommentMod",log_debug)
 END ;Subroutine
 SUBROUTINE (addcontributorsystemmod(chargeid=f8) =null)
   CALL logmsg(curprog,"Entering addContributorSystemMod",log_debug)
   DECLARE cmpos = i4 WITH protect, noconstant(0)
   DECLARE cmidx = i4 WITH protect, noconstant(0)
   DECLARE chargemodcount = i4 WITH noconstant(contsystchgmodrec->charge_mod_qual)
   SET cmidx = locateval(cmpos,1,size(contsystchgmodrec->objarray,5),chargeid,contsystchgmodrec->
    objarray[cmpos].charge_item_id)
   IF (cmidx=0)
    SET chargemodcount += 1
    SET stat = alterlist(contsystchgmodrec->objarray,chargemodcount)
    SET contsystchgmodrec->charge_mod_qual = chargemodcount
    SET contsystchgmodrec->objarray[chargemodcount].action_type = "ADD"
    SET contsystchgmodrec->objarray[chargemodcount].charge_item_id = chargeid
    SET contsystchgmodrec->objarray[chargemodcount].charge_mod_type_cd = cs13019_other_cd
    SET contsystchgmodrec->objarray[chargemodcount].field6 = "Contributor System"
    SET contsystchgmodrec->objarray[chargemodcount].field7 = uar_get_code_display(request->
     contributor_system_cd)
    SET contsystchgmodrec->objarray[chargemodcount].activity_dt_tm = cnvtdatetime(sysdate)
    SET contsystchgmodrec->objarray[chargemodcount].active_ind = true
    SET contsystchgmodrec->objarray[chargemodcount].active_status_cd = cs48_active_cd
    SET contsystchgmodrec->objarray[chargemodcount].active_status_dt_tm = cnvtdatetime(sysdate)
    SET contsystchgmodrec->objarray[chargemodcount].active_status_prsnl_id = reqinfo->updt_id
    SET contsystchgmodrec->objarray[chargemodcount].beg_effective_dt_tm = cnvtdatetime(sysdate)
    SET contsystchgmodrec->objarray[chargemodcount].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 23:59:59.00")
    SET contsystchgmodrec->objarray[chargemodcount].code1_cd = request->contributor_system_cd
    SET contsystchgmodrec->objarray[chargemodcount].updt_cnt = 0
   ELSE
    CALL logmsg(curprog,"Contributor System already added to charge",log_debug)
    CALL echo(build2("charge_item_id: ",chargeid," Index: ",cmidx))
   ENDIF
   CALL logmsg(curprog,"Exiting addContributorSystemMod",log_debug)
 END ;Subroutine
 SUBROUTINE (savecontributorsystemchgmods(idx0=i4) =i2)
   DECLARE idx = i4 WITH noconstant(0)
   DECLARE chargemodcount = i4 WITH noconstant(0)
   CALL logmsg(curprog,"Entering saveContributorSystemChgMods",log_debug)
   IF (validate(debug,- (1)) > 0)
    CALL logmsg(curprog,"contributor system charge mod record",log_debug)
    CALL echorecord(contsystchgmodrec)
   ENDIF
   IF (size(contsystchgmodrec->objarray,5) > 0)
    SELECT INTO "nl:"
     FROM charge_mod cm
     WHERE expand(idx,1,size(contsystchgmodrec->objarray,5),cm.charge_item_id,contsystchgmodrec->
      objarray[idx].charge_item_id)
      AND cm.charge_mod_type_cd=cs13019_other_cd
      AND cm.active_ind=1
      AND cm.field6="Contributor System"
     HEAD REPORT
      chargemodcount = contsystchgmodrec->charge_mod_qual
     DETAIL
      chargemodcount += 1, stat = alterlist(contsystchgmodrec->objarray,chargemodcount),
      contsystchgmodrec->objarray[chargemodcount].action_type = "DEL",
      contsystchgmodrec->objarray[chargemodcount].charge_item_id = cm.charge_item_id,
      contsystchgmodrec->objarray[chargemodcount].charge_mod_id = cm.charge_mod_id, contsystchgmodrec
      ->objarray[chargemodcount].active_status_cd = cs48_inactive_cd,
      contsystchgmodrec->objarray[chargemodcount].active_ind = false
     FOOT REPORT
      contsystchgmodrec->charge_mod_qual = chargemodcount
     WITH nocounter
    ;end select
    EXECUTE afc_val_charge_mod  WITH replace("REQUEST",contsystchgmodrec), replace("REPLY",
     contsystchgmodrep)
    IF ((contsystchgmodrep->status_data.status != "S"))
     CALL logmsg(curprog,"Exiting saveContributorSystemChgMods, failed to add charge mod",log_debug)
     CALL writeerrortoreply(idx0,"saveContributorSystemChgMods",
      "afc_val_charge_mod did not return success",true)
     RETURN(false)
    ENDIF
   ELSE
    CALL logmsg(curprog,
     "contributor system charge mod not populated, skipping call to afc_val_charge_mod",log_debug)
   ENDIF
   RETURN(true)
   CALL logmsg(curprog,"Exiting saveContributorSystemChgMods",log_debug)
 END ;Subroutine
 SUBROUTINE (updatechargeeventmod(idx0=i4,chargeeventid=f8) =i2)
   CALL logmsg(curprog,"Entering updateChargeEventMod",log_debug)
   SET stat = initrec(valchgevntmodreq)
   SET stat = initrec(valchgevntmodrep)
   DECLARE codeset13019meaning = c12 WITH protect, noconstant("")
   DECLARE codeset14002meaning = c12 WITH protect, noconstant("")
   DECLARE new_cem_id = f8 WITH protect, noconstant(0.0)
   IF (size(request->charge[idx0].charge_mod,5) > 0)
    DECLARE billcdcnt = i4 WITH protect, noconstant(0)
    DECLARE k = i4 WITH protect, noconstant(1)
    FOR (k = 1 TO size(request->charge[idx0].charge_mod,5))
      SET codeset14002meaning = uar_get_code_meaning(request->charge[idx0].charge_mod[k].field1_id)
      SET codeset13019meaning = uar_get_code_meaning(request->charge[idx0].charge_mod[k].
       charge_mod_type_cd)
      IF (((codeset14002meaning="ICD9") OR (((codeset14002meaning="MODIFIER"
       AND validate(request->charge[idx0].charge_mod[k].charge_mod_source_cd,0.0) !=
      cs4518006_manually_add) OR (((codeset13019meaning="PROMPT") OR (((codeset13019meaning=
      "USER DEF") OR (((codeset13019meaning="FLEX") OR (codeset14002meaning="NDC")) )) )) )) )) )
       IF ((((request->charge[idx0].charge_mod[k].action_type="ADD")) OR ((request->charge[idx0].
       charge_mod[k].action_type="UPT")
        AND (request->charge[idx0].charge_mod[k].charge_event_mod_id=0.0))) )
        SELECT INTO "nl:"
         ...
         WITH format, counter
        ;end select
        SET request->charge[idx0].charge_mod[k].charge_event_mod_id = new_cem_id
        SET billcdcnt += 1
        SET stat = alterlist(valchgevntmodreq->objarray,billcdcnt)
        SET valchgevntmodreq->objarray[billcdcnt].action_type = "ADD"
        SET valchgevntmodreq->objarray[billcdcnt].charge_event_mod_id = new_cem_id
        SET valchgevntmodreq->objarray[billcdcnt].charge_event_id = chargeeventid
        SET valchgevntmodreq->objarray[billcdcnt].charge_event_mod_type_cd = request->charge[idx0].
        charge_mod[k].charge_mod_type_cd
        SET valchgevntmodreq->objarray[billcdcnt].field6 = request->charge[idx0].charge_mod[k].field6
        SET valchgevntmodreq->objarray[billcdcnt].field7 = request->charge[idx0].charge_mod[k].field7
        SET valchgevntmodreq->objarray[billcdcnt].nomen_id = request->charge[idx0].charge_mod[k].
        nomen_id
        SET valchgevntmodreq->objarray[billcdcnt].field1_id = request->charge[idx0].charge_mod[k].
        field1_id
        SET valchgevntmodreq->objarray[billcdcnt].field2_id = request->charge[idx0].charge_mod[k].
        field2_id
        SET valchgevntmodreq->objarray[billcdcnt].field3_id = request->charge[idx0].charge_mod[k].
        field3_id
        SET valchgevntmodreq->objarray[billcdcnt].field4_id = request->charge[idx0].charge_mod[k].
        field4_id
        SET valchgevntmodreq->objarray[billcdcnt].field5_id = request->charge[idx0].charge_mod[k].
        field5_id
        SET valchgevntmodreq->objarray[billcdcnt].updt_cnt = 0
        SET valchgevntmodreq->objarray[billcdcnt].active_ind = 1
        SET valchgevntmodreq->objarray[billcdcnt].active_status_dt_tm = cnvtdatetime(sysdate)
        SET valchgevntmodreq->objarray[billcdcnt].beg_effective_dt_tm = cnvtdatetime(sysdate)
        SET valchgevntmodreq->objarray[billcdcnt].end_effective_dt_tm = cnvtdatetime(
         "31-DEC-2100 23:59:59")
        SET valchgevntmodreq->objarray[billcdcnt].field1 = request->charge[idx0].charge_mod[k].field1
        SET valchgevntmodreq->objarray[billcdcnt].field2 = request->charge[idx0].charge_mod[k].field2
        SET valchgevntmodreq->objarray[billcdcnt].cm1_nbr = request->charge[idx0].charge_mod[k].
        cm1_nbr
       ELSEIF ((request->charge[idx0].charge_mod[k].action_type="UPT"))
        SET billcdcnt += 1
        SET stat = alterlist(valchgevntmodreq->objarray,billcdcnt)
        SELECT INTO "nl:"
         FROM charge_event_mod cem
         WHERE (cem.charge_event_mod_id=request->charge[idx0].charge_mod[k].charge_event_mod_id)
         DETAIL
          valchgevntmodreq->objarray[billcdcnt].action_type = "UPT", valchgevntmodreq->objarray[
          billcdcnt].charge_event_mod_id = request->charge[idx0].charge_mod[k].charge_event_mod_id,
          valchgevntmodreq->objarray[billcdcnt].charge_event_id = cem.charge_event_id,
          valchgevntmodreq->objarray[billcdcnt].charge_event_mod_type_cd = request->charge[idx0].
          charge_mod[k].charge_mod_type_cd, valchgevntmodreq->objarray[billcdcnt].field1 = cem.field1,
          valchgevntmodreq->objarray[billcdcnt].field2 = cem.field2,
          valchgevntmodreq->objarray[billcdcnt].field3 = cem.field3, valchgevntmodreq->objarray[
          billcdcnt].field4 = cem.field4, valchgevntmodreq->objarray[billcdcnt].field5 = cem.field5,
          valchgevntmodreq->objarray[billcdcnt].field6 = request->charge[idx0].charge_mod[k].field6,
          valchgevntmodreq->objarray[billcdcnt].field7 = request->charge[idx0].charge_mod[k].field7,
          valchgevntmodreq->objarray[billcdcnt].field8 = cem.field8,
          valchgevntmodreq->objarray[billcdcnt].field9 = cem.field9, valchgevntmodreq->objarray[
          billcdcnt].field10 = cem.field10, valchgevntmodreq->objarray[billcdcnt].nomen_id = request
          ->charge[idx0].charge_mod[k].nomen_id,
          valchgevntmodreq->objarray[billcdcnt].field1_id = request->charge[idx0].charge_mod[k].
          field1_id, valchgevntmodreq->objarray[billcdcnt].field2_id = request->charge[idx0].
          charge_mod[k].field2_id, valchgevntmodreq->objarray[billcdcnt].field3_id = request->charge[
          idx0].charge_mod[k].field3_id,
          valchgevntmodreq->objarray[billcdcnt].field4_id = request->charge[idx0].charge_mod[k].
          field4_id, valchgevntmodreq->objarray[billcdcnt].field5_id = request->charge[idx0].
          charge_mod[k].field5_id, valchgevntmodreq->objarray[billcdcnt].updt_cnt = cem.updt_cnt,
          valchgevntmodreq->objarray[billcdcnt].active_ind = 1, valchgevntmodreq->objarray[billcdcnt]
          .active_status_cd = cem.active_status_cd, valchgevntmodreq->objarray[billcdcnt].
          active_status_prsnl_id = cem.active_status_prsnl_id,
          valchgevntmodreq->objarray[billcdcnt].active_status_dt_tm = cem.active_status_dt_tm,
          valchgevntmodreq->objarray[billcdcnt].beg_effective_dt_tm = cem.beg_effective_dt_tm,
          valchgevntmodreq->objarray[billcdcnt].end_effective_dt_tm = cem.end_effective_dt_tm,
          valchgevntmodreq->objarray[billcdcnt].activity_dt_tm = cem.activity_dt_tm, valchgevntmodreq
          ->objarray[billcdcnt].cm1_nbr = cem.cm1_nbr, valchgevntmodreq->objarray[billcdcnt].code1_cd
           = cem.code1_cd
         WITH nocounter
        ;end select
       ELSEIF ((request->charge[idx0].charge_mod[k].action_type="DEL")
        AND (request->charge[idx0].charge_mod[k].charge_event_mod_id != 0))
        SET billcdcnt += 1
        SET stat = alterlist(valchgevntmodreq->objarray,billcdcnt)
        SELECT INTO "nl:"
         FROM charge_event_mod cem
         WHERE (cem.charge_event_mod_id=request->charge[idx0].charge_mod[k].charge_event_mod_id)
          AND cem.active_ind=1
         DETAIL
          valchgevntmodreq->objarray[billcdcnt].action_type = "DEL", valchgevntmodreq->objarray[
          billcdcnt].charge_event_mod_id = cem.charge_event_mod_id, valchgevntmodreq->objarray[
          billcdcnt].charge_event_id = cem.charge_event_id,
          valchgevntmodreq->objarray[billcdcnt].active_status_cd = cs48_inactive_cd, valchgevntmodreq
          ->objarray[billcdcnt].active_ind = false, valchgevntmodreq->objarray[billcdcnt].updt_cnt =
          cem.updt_cnt
         WITH nocounter
        ;end select
       ENDIF
      ...
      cs4518006_manually_add
       AND ((validate(request->charge[idx0].charge_mod[k].action_type,"")="UPT") OR (validate(request
       ->charge[idx0].charge_mod[k].action_type,"")="ADD"))
       AND validate(request->charge[idx0].charge_mod[k].charge_event_mod_id,0.0) > 0.0)
       SET billcdcnt += 1
       SET stat = alterlist(valchgevntmodreq->objarray,billcdcnt)
       SELECT INTO "nl:"
        FROM charge_event_mod cem
      ...
      ENDIF
    ENDFOR
    IF (size(valchgevntmodreq->objarray,5) > 0)
     EXECUTE afc_val_charge_event_mod  WITH replace("REQUEST",valchgevntmodreq), replace("REPLY",
      valchgevntmodrep)
     IF ((valchgevntmodrep->status_data.status != "S"))
      IF (validate(debug,- (1)) > 0)
       CALL echorecord(valchgevntmodreq)
       CALL echorecord(valchgevntmodrep)
      ENDIF
      CALL logmsg(curprog,
       "Exiting updateChargeEventMod, afc_val_charge_event_mod did not return success",log_debug)
      CALL writeerrortoreply(idx0,"updateChargeEventMod",
       "afc_val_charge_event_mod did not return success",true)
      RETURN(false)
     ENDIF
    ENDIF
   ENDIF
   CALL logmsg(curprog,"Exiting updateChargeEventMod",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (postorinterfacecharges(chargestosend=vc(ref)) =i2)
   CALL logmsg(curprog,"Entering postOrInterfaceCharges",log_debug)
   DECLARE idx = i4 WITH protect, noconstant(0)
   DECLARE profitcnt = i4 WITH protect, noconstant(0)
   DECLARE realtimecnt = i4 WITH protect, noconstant(0)
   ...
   RECORD realtimecharges(
     1 interface_charge[*]
       2 charge_item_id = f8
   ) WITH protect
   RECORD realtimechargesreply(
     1 workflow_task_name = vc
     1 interface_charge[*]
       2 abn_status_cd = f8
       2 active_ind = i2
       2 active_status_cd = f8
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     ...
     ELSEIF (i.realtime_ind=1)
      realtimecnt += 1, stat = alterlist(realtimecharges->interface_charge,realtimecnt),
      realtimecharges->interface_charge[realtimecnt].charge_item_id = c.charge_item_id
     ENDIF
    WITH nocounter, expand = 2
   ;end select
   IF (invalidinterfacefilefound)
    CALL logmsg(curprog,"Exiting postOrInterfaceCharges, Invalid Interface file found",log_debug)
    RETURN(false)
   ENDIF
   IF (validate(debug,- (1)) > 0)
    CALL echo("postOrInterfaceCharges - profitCharges")
    CALL echorecord(profitcharges)
    CALL echo("postOrInterfaceCharges - realtimeCharges")
    CALL echorecord(realtimecharges)
   ENDIF
   IF ((request->nocommitind=0))
    IF (size(profitcharges->charges,5) > 0)
     ...
     IF ((profitchargesreply->status_data.status != "S"))
      CALL logmsg(curprog,"Exiting postOrInterfaceCharges,failed to post the new charges",log_debug)
      RETURN(false)
     ELSE
      CALL logmsg(curprog,"pft_nt_post_charges_async, returned success",log_debug)
     ENDIF
    ELSE
     CALL logmsg(curprog,"No charges qualified to be sent to pft_nt_post_charges_async",log_debug)
    ENDIF
    IF (size(realtimecharges->interface_charge,5) > 0)
     EXECUTE afc_post_interface_charge  WITH replace("REQUEST",realtimecharges), replace("REPLY",
      realtimechargesreply)
     IF (validate(debug,- (1)) > 0)
      CALL echorecord(realtimechargesreply)
     ENDIF
     IF ((realtimechargesreply->status_data.status != "S"))
      CALL logmsg(curprog,"Exiting postOrInterfaceCharges, failed to interface the new charges",
       log_debug)
      RETURN(false)
     ELSE
      CALL logmsg(curprog,"afc_post_interface_charge, returned success",log_debug)
     ENDIF
    ELSE
     CALL logmsg(curprog,"No charges qualified to be sent to afc_post_interface_charge",log_debug)
    ENDIF
   ELSE
    CALL logmsg(curprog,"NoCommitInd != 0. Skipping Call to postOrInterfaceCharges.",log_debug)
   ENDIF
   CALL logmsg(curprog,"Exiting postOrInterfaceCharges, Successfully posted or interfaced charges",
    log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (writeerrortoreply(idx=i4,errorsub=vc,errormsg=vc,appendmode=i2) =null)
   IF ((reply->charge[idx].status != "F"))
    SET reply->charge[idx].status = "F"
    SET reply->charge[idx].error_sub = errorsub
    SET reply->charge[idx].error_msg = errormsg
   ELSEIF ((reply->charge[idx].status="F")
    AND appendmode)
    SET reply->charge[idx].error_sub = concat(errorsub,"->",reply->charge[idx].error_sub)
   ENDIF
 END ;Subroutine
 SUBROUTINE (writestatusdatafailure(operationname=vc,targetobjectname=vc,targetobjectvalue=vc) =null)
   SET reply->status_data.status = "F"
 ...
 END ;Subroutine
 SUBROUTINE (loaddiagnosisdetail(chrgidx=i4) =i2)
   CALL logmsg(curprog,"Entering loadDiagnosisDetail...",log_debug)
   DECLARE dxreccnt = i4 WITH noconstant(0)
   SET stat = initrec(chargediagnosisdetail)
   SELECT INTO "nl:"
    FROM charge_mod cm,
     code_value cv
    PLAN (cm
     WHERE (cm.charge_item_id=request->charge[chrgidx].charge_item_id)
      AND cm.active_ind=true)
     JOIN (cv
     WHERE cv.code_value=cm.field1_id
      AND cv.code_set=cvs14002_afc_schedule_type
    ...
    DETAIL
     ...
     chargediagnosisdetail->diagnosislist[dxreccnt].field2_id = cm.field2_id, chargediagnosisdetail->
     diagnosislist[dxreccnt].charge_mod_id = cm.charge_mod_id
    WITH nocounter
   ;end select
   IF (validate(debug,- (1)) > 0)
   ...
   ENDIF
   IF (dxreccnt=0)
    CALL logmsg(curprog,"Exiting loadDiagnosisDetail...",log_debug)
    RETURN(false)
   ENDIF
   CALL logmsg(curprog,"Exiting loadDiagnosisDetail...",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (loadmodifiersdetail(chrgidx=i4) =i2)
   CALL logmsg(curprog,"Entering loadModifiersDetail...",log_debug)
   DECLARE modreccnt = i4 WITH noconstant(0)
   SET stat = initrec(chargemodifiersdetail)
   SELECT INTO "nl:"
    FROM charge_mod cm,
     code_value cv
    PLAN (cm
     ...
     WHERE cv.code_value=cm.field1_id
      ...
      AND cv.active_ind=true)
    DETAIL
     modreccnt += 1, stat = alterlist(chargemodifiersdetail->modifierslist,modreccnt),
     chargemodifiersdetail->modifierslist[modreccnt].field6 = cm.field6,
     chargemodifiersdetail->modifierslist[modreccnt].field2_id = cm.field2_id, chargemodifiersdetail
     ->modifierslist[modreccnt].charge_mod_id = cm.charge_mod_id
    WITH nocounter
   ;end select
   IF (validate(debug,- (1)) > 0)
    CALL echorecord(chargemodifiersdetail)
   ENDIF
   IF (modreccnt=0)
    CALL logmsg(curprog,"Exiting loadModifiersDetail...",log_debug)
    RETURN(false)
   ENDIF
   CALL logmsg(curprog,"Exiting loadModifiersDetail...",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (inactivedxinchargeeventmod(chrgidx=i4) =i2)
   CALL logmsg(curprog,"Entering inactiveDXInChargeEventMod...",log_debug)
   DECLARE dxidxcnt = i4 WITH protect, noconstant(0)
   DECLARE isdxdeletable = i2 WITH protect, noconstant(false)
   DECLARE chargeeventmodid = f8 WITH protect, noconstant(0.0)
   DECLARE icdcdfmeaning = vc WITH protect, constant("ICD9")
   FOR (dxidxcnt = 1 TO size(chargediagnosisdetail->diagnosislist,5))
     SET isdxdeletable = false
     SELECT INTO "nl:"
      FROM (dummyt d1  WITH seq = size(request->charge[chrgidx].charge_mod,5))
      PLAN (d1
       WHERE (request->charge[chrgidx].charge_mod[d1.seq].action_type="DEL")
        AND (request->charge[chrgidx].charge_mod[d1.seq].charge_mod_id=chargediagnosisdetail->
       diagnosislist[dxidxcnt].charge_mod_id)
        AND uar_get_code_meaning(request->charge[chrgidx].charge_mod[d1.seq].field1_id)=icdcdfmeaning
       )
      DETAIL
       isdxdeletable = true
      WITH nocounter
     ...
     IF (isdxdeletable=true)
      IF (getchargeeventmodid(request->charge[chrgidx].charge_item_id,chargediagnosisdetail->
       diagnosislist[dxidxcnt].field6,icdcdfmeaning,chargeeventmodid))
       IF (uptchargeeventmodactiveind(chargeeventmodid,chargediagnosisdetail->diagnosislist[dxidxcnt]
        .field6))
        CALL logmsg(curprog,build("Successfully updated active ind for charge_event_mod_id: ",
          chargeeventmodid),log_debug)
       ELSE
        CALL logmsg(curprog,"uptChargeEventModActiveInd failed to update charge_event_mod table",
         log_debug)
        CALL logmsg(curprog,"Exiting inactiveDXInChargeEventMod...",log_debug)
        RETURN(false)
       ENDIF
      ELSE
       CALL logmsg(curprog,"No charge_event_mod_id present in charge_event_mod table",log_debug)
      ENDIF
      IF (cnvtupper(inactivate_parent_event_cd)="Y")
       IF (inactivateparentcodes(request->charge[chrgidx].charge_item_id,chargediagnosisdetail->
        diagnosislist[dxidxcnt].field6,icdcdfmeaning))
        CALL logmsg(curprog,"Successfully removed parent Diagnosis codes.",log_debug)
       ELSE
        CALL logmsg(curprog,"inactivateParentCodes failed",log_debug)
        CALL logmsg(curprog,"Exiting inactiveDXInChargeEventMod...",log_debug)
        RETURN(false)
       ENDIF
      ENDIF
   ...
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (inactivemodinchargeeventmod(chrgidx=i4) =i2)
   CALL logmsg(curprog,"Entering inactiveModInChargeEventMod...",log_debug)
   DECLARE modidxcnt = i4 WITH protect, noconstant(0)
   DECLARE ismodifierdeletable = i2 WITH protect, noconstant(false)
   DECLARE chargeeventmodid = f8 WITH protect, noconstant(0.0)
   DECLARE modcdfmeaning = vc WITH protect, constant("MODIFIER")
   FOR (modidxcnt = 1 TO size(chargemodifiersdetail->modifierslist,5))
     SET ismodifierdeletable = false
     SELECT INTO "nl:"
      FROM (dummyt d1  WITH seq = size(request->charge[chrgidx].charge_mod,5))
      PLAN (d1
       WHERE (request->charge[chrgidx].charge_mod[d1.seq].action_type="DEL")
        AND (request->charge[chrgidx].charge_mod[d1.seq].charge_mod_id=chargemodifiersdetail->
       modifierslist[modidxcnt].charge_mod_id)
        AND uar_get_code_meaning(request->charge[chrgidx].charge_mod[d1.seq].field1_id)=modcdfmeaning
       )
      DETAIL
       ismodifierdeletable = true
      WITH nocounter
     ;end select
     SET chargeeventmodid = 0.0
     IF (ismodifierdeletable=true)
      IF (getchargeeventmodid(request->charge[chrgidx].charge_item_id,chargemodifiersdetail->
       modifierslist[modidxcnt].field6,modcdfmeaning,chargeeventmodid))
       IF (uptchargeeventmodactiveind(chargeeventmodid,chargemodifiersdetail->modifierslist[modidxcnt
        ].field6))
        CALL logmsg(curprog,build("Successfully updated active ind for charge_event_mod_id: ",
          chargeeventmodid),log_debug)
       ELSE
        CALL logmsg(curprog,"uptChargeEventModActiveInd failed to update charge_event_mod table",
         log_debug)
        CALL logmsg(curprog,"Exiting inactiveModInChargeEventMod...",log_debug)
        RETURN(false)
       ENDIF
      ELSE
       CALL logmsg(curprog,"No charge_event_mod_id present in charge_event_mod table",log_debug)
      ENDIF
      IF (cnvtupper(inactivate_parent_event_cd)="Y")
       IF (inactivateparentcodes(request->charge[chrgidx].charge_item_id,chargemodifiersdetail->
        modifierslist[modidxcnt].field6,modcdfmeaning))
        CALL logmsg(curprog,"Successfully removed parent Diagnosis codes.",log_debug)
       ELSE
        CALL logmsg(curprog,"inactivateParentCodes failed",log_debug)
        CALL logmsg(curprog,"Exiting inactiveDXInChargeEventMod...",log_debug)
   ...
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (inactivateparentcodes(chargeitemid=f8,field6=vc,meaning=vc) =i2)
   CALL logmsg(curprog,"Entering inactivateParentCodes...",log_debug)
     ...
     SELECT INTO "nl:"
      ...
      PLAN (ce
       ...
       WHERE (cem.charge_event_id= Outerjoin(ce.charge_event_id))
        ...
        AND (cem.charge_event_mod_type_cd= Outerjoin(cs13019_bill_code_cd)) )
   ...
   FOR (cemcnt = 1 TO parentchargeeventmods->cnt)
     ...
     ELSE
      CALL logmsg(curprog,"uptChargeEventModActiveInd failed to update parent charge_event_mod table",
       log_debug)
     ENDIF
   ENDFOR
   CALL logmsg(curprog,"Exiting inactivateParentCodes...",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (getchargeeventmodid(prchargeitemid=f8,prfield6=vc,prcdfmeaning=vc,prchargeeventmodid=f8(
   ref)) =i2)
   CALL logmsg(curprog,"Entering getChargeEventModId...",log_debug)
   ...
   IF (prchargeeventmodid <= 0.0)
    CALL logmsg(curprog,"Exiting getChargeEventModId...",log_debug)
    RETURN(false)
   ENDIF
   CALL logmsg(curprog,"Exiting getChargeEventModId...",log_debug)
   RETURN(true)
 ...
 SUBROUTINE (uptchargeeventmodactiveind(prchargeeventmodid=f8,prfield6=vc) =i2)
   CALL logmsg(curprog,"Entering uptChargeEventModActiveInd...",log_debug)
   DECLARE billcdcnt = i4 WITH protect, noconstant(0)
   SET billcdcnt += 1
   SET stat = alterlist(valchgevntmodreq->objarray,billcdcnt)
   SELECT INTO "nl:"
    ...
    WHERE cem.charge_event_mod_id=prchargeeventmodid
     ...
     AND cem.active_ind=true
    DETAIL
     valchgevntmodreq->objarray[billcdcnt].action_type = "DEL", valchgevntmodreq->objarray[billcdcnt]
     .charge_event_id = cem.charge_event_id, valchgevntmodreq->objarray[billcdcnt].
     charge_event_mod_id = prchargeeventmodid,
     valchgevntmodreq->objarray[billcdcnt].updt_cnt = cem.updt_cnt, valchgevntmodreq->objarray[
     billcdcnt].active_ind = false
    WITH nocounter
   ;end select
   IF (size(valchgevntmodreq->objarray,5) <= 0)
   ...
   ELSE
    EXECUTE afc_val_charge_event_mod  WITH replace("REQUEST",valchgevntmodreq), replace("REPLY",
     valchgevntmodrep)
    IF ((valchgevntmodrep->status_data.status != "S"))
     CALL logmsg(curprog,"afc_val_charge_event_mod did not return success",log_debug)
     IF (validate(debug,- (1)) > 0)
      CALL echorecord(valchgevntmodreq)
      CALL echorecord(valchgevntmodrep)
     ENDIF
     SET stat = alterlist(valchgevntmodreq->objarray,0)
     CALL logmsg(curprog,"Exiting uptChargeEventModActiveInd...",log_debug)
     RETURN(false)
    ENDIF
   ENDIF
   SET stat = alterlist(valchgevntmodreq->objarray,0)
   ...
   RETURN(true)
 END ;Subroutine
 CALL echo("End including AFC_MODIFY_CHARGE.INC")
 ...
 SET reprocesssettings->no_reprocess = 1
 RECORD lockchargerequest(
   1 charge_item_id = f8
   1 updt_cnt = i4
   1 updt_cnt_ind = i2
 ) WITH protect
 RECORD lockchargereply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
 ...
 DECLARE chargeisfound = i2 WITH protect, noconstant(false)
 DECLARE chargeislocked = i2 WITH protect, noconstant(false)
 DECLARE newcreditid = f8 WITH protect, noconstant(0.0)
 DECLARE newdebitid = f8 WITH protect, noconstant(0.0)
 DECLARE curlogicaldomainid = f8 WITH protect, noconstant(0.0)
 DECLARE contributor_system_codeset = i4 WITH protect, constant(89)
 DECLARE changescommitted = i2 WITH protect, noconstant(false)
 SET reply->charge_qual = 0
 SET reply->success_cnt = 0
 SET reply->fail_cnt = 0
 SET stat = alterlist(reply->charge,0)
 ...
 IF ((reqinfo->updt_id=0))
  CALL logmsg(curprog,"User is not securely logged into CCL",log_debug)
  CALL writestatusdatafailure("CCLLOG","NoUser","No user to process request against, securely login")
  GO TO end_program
 ENDIF
 IF ( NOT (getlogicaldomain(ld_concept_organization,curlogicaldomainid)))
  CALL writestatusdatafailure("LOG_DOM","LOGICAL_DOMAIN","Failed to obtain logical domain")
  GO TO end_program
 ENDIF
 IF (validate(debug,- (1)) > 0)
  CALL echo(build2("curLogicalDomainID = ",curlogicaldomainid))
 ENDIF
 IF ( NOT (getsitepreferences(curlogicaldomainid)))
  CALL writestatusdatafailure("PREF","SITE_PREF","Failed to obtain site preferences")
  GO TO end_program
 ENDIF
 IF (validate(debug,- (1)) > 0)
 ...
 ENDIF
 IF ((request->contributor_system_cd > 0))
  CALL logmsg(curprog,"Checking Contributor System",log_debug)
  IF ( NOT (isvalidcodevalue(request->contributor_system_cd,contributor_system_codeset)))
   CALL logmsg(curprog,"Contributor System is not a valid code value",log_debug)
   CALL echo(request->contributor_system_cd)
   CALL writestatusdatafailure("CONTSYST","CONTSYSTEM","Contributor System is not a valid code value"
    )
   GO TO end_program
  ELSE
   CALL logmsg(curprog,"Contributor System is valid",log_debug)
  ENDIF
 ENDIF
 IF ((request->charge_qual=size(request->charge,5)))
  FOR (chrgidx = 1 TO size(request->charge,5))
    CALL logmsg(curprog,"Starting Charge Loop",log_debug)
    IF (validate(debug,- (1)) > 0)
     CALL echo(build2("charge:",chrgidx," of:",size(request->charge,5)))
     CALL echo(build2("Current charge_item_id being modified: ",request->charge[chrgidx].
       charge_item_id))
    ENDIF
    SET stat = alterlist(reply->charge,chrgidx)
    SET reply->charge_qual = chrgidx
    SET reply->charge[chrgidx].request_charge_item_id = request->charge[chrgidx].charge_item_id
    SET reply->charge[chrgidx].status = "S"
    SET chargeisfound = false
    SET chargeislocked = false
    SET changescommitted = false
    SET stat = initrec(chargefindrequest)
    SET stat = initrec(chargefindreply)
    SET stat = initrec(lockchargerequest)
    SET stat = initrec(lockchargereply)
    SET stat = initrec(pendchgmodrec)
    SET stat = initrec(contsystchgmodrec)
    SET stat = initrec(changelogrecord)
    SET newcreditid = 0.0
    SET newdebitid = 0.0
    SET failed = false
    IF ((request->charge[chrgidx].charge_mod_qual=size(request->charge[chrgidx].charge_mod,5)))
     IF ((request->charge[chrgidx].charge_item_id > 0))
      SET chargefindrequest->charge_item_id = request->charge[chrgidx].charge_item_id
      CALL logmsg(curprog,"Executing afc_charge_find",log_debug)
      EXECUTE afc_charge_find  WITH replace("REQUEST",chargefindrequest), replace("REPLY",
       chargefindreply)
      IF (validate(debug,- (1)) > 0)
       CALL echo("Executed afc_charge_find")
       CALL echorecord(chargefindrequest)
       CALL echorecord(chargefindreply)
      ENDIF
      IF ((((chargefindreply->status_data.status="F")) OR (size(chargefindreply->charge_items,5) != 1
      )) )
       CALL logmsg(curprog,"Failed to find the original charge in afc_charge_find",log_debug)
       SET chargeisfound = false
       CALL writeerrortoreply(chrgidx,"afc_charge_find",
        "afc_charge_find did returned Failure or did not find charge",false)
      ELSE
       CALL logmsg(curprog,"Successfully found the original charge in afc_charge_find",log_debug)
       SET chargeisfound = true
      ENDIF
      SET lockchargerequest->charge_item_id = request->charge[chrgidx].charge_item_id
      SET lockchargerequest->updt_cnt = validate(request->charge[chrgidx].updt_cnt,0)
      SET lockchargerequest->updt_cnt_ind = validate(request->charge[chrgidx].updt_cnt_ind,0)
      CALL logmsg(curprog,"Executing AFC_LOCK_CHARGE_FOR_UPDATE",log_debug)
      EXECUTE afc_lock_charge_for_update  WITH replace("REQUEST",lockchargerequest), replace("REPLY",
       lockchargereply)
      IF (validate(debug,- (1)) > 0)
       CALL echo("Executed afc_lock_charge_for_update")
       CALL echorecord(lockchargerequest)
       CALL echorecord(lockchargereply)
      ENDIF
      IF ((lockchargereply->status_data.status != "S"))
       IF (validate(debug,- (1)) > 0)
        CALL echo(build("afc_lock_charge_for_update Reply Status is ",lockchargereply->status_data.
          status," adding reason to reply"))
        CALL echorecord(lockchargereply)
       ENDIF
       CALL writeerrortoreply(chrgidx,"afc_lock_charge_for_update",
        "afc_lock_charge_for_update did not return success",false)
       SET chargeislocked = false
      ELSE
       IF (validate(debug,- (1)) > 0)
        CALL echo("Successfully locked the charge record")
       ENDIF
       CALL logmsg(curprog,"Successfully locked the charge record",log_debug)
       SET chargeislocked = true
      ENDIF
      IF (chargeislocked=true
       AND chargeisfound=true)
       IF (ispersonvalid(chargefindreply->charge_items[1].person_id))
        IF (isencountervalid(chargefindreply->charge_items[1].encntr_id))
         IF (islogicaldomainvalid(request->charge[chrgidx].charge_item_id,curlogicaldomainid))
          IF (isvalidpendingcharge(chargefindreply))
           IF ((request->charge[chrgidx].research_acct_id != 0))
            IF (modifyresearchacct(chrgidx,chargefindreply))
             SET reply->success_cnt += 1
             IF (validate(debug,- (1)) > 0)
              CALL logmsg(curprog,"modifyResearchAcct returned success",log_debug)
             ENDIF
            ELSE
             SET reply->fail_cnt += 1
             CALL writeerrortoreply(chrgidx,"modifyResearchAcct",
              "Pending Charge: modifyResearchAcct returned False",false)
             IF (validate(debug,- (1)) > 0)
              CALL echo("modifyResearchAcct returned a failure")
             ENDIF
            ENDIF
           ELSEIF (isvalidmodification(chrgidx,chargefindreply))
            IF ( NOT (loaddiagnosisdetail(chrgidx)))
             CALL logmsg(curprog,"No Diagnosis details present",log_debug)
            ENDIF
            IF ( NOT (loadmodifiersdetail(chrgidx)))
             CALL logmsg(curprog,"No Modifiers details present",log_debug)
            ENDIF
            IF ( NOT (inactivedxinchargeeventmod(chrgidx)))
             CALL logmsg("inactiveDXInChargeEventMod",
              "Failed to inactivate active_ind in charge_event_mod table",log_debug)
            ENDIF
            IF ( NOT (inactivemodinchargeeventmod(chrgidx)))
             CALL logmsg("inactiveModInChargeEventMod",
              "Failed to inactivate active_ind in charge_event_mod table",log_debug)
            ENDIF
            IF (modifypendingcharge(chrgidx,chargefindreply))
             IF (savecontributorsystemchgmods(chrgidx))
              SET reply->success_cnt += 1
              IF (validate(debug,- (1)) > 0)
               CALL echo("saveContributorSystemChgMods returned success")
              ENDIF
             ELSE
              IF (validate(debug,- (1)) > 0)
               CALL echo("saveContributorSystemChgMods returned a failure")
              ENDIF
              SET reply->fail_cnt += 1
              CALL writeerrortoreply(chrgidx,"saveContributorSystemChgMods",
               "Pending Charge: saveContributorSystemChgMods returned false",false)
             ENDIF
            ELSE
             SET reply->fail_cnt += 1
             CALL writeerrortoreply(chrgidx,"modifyPendingCharge",
              "Pending Charge: modifyPendingCharge returned false",false)
            ENDIF
           ELSE
            SET reply->fail_cnt += 1
            CALL writeerrortoreply(chrgidx,"isValidModification",
             "Pending Charge: isValidModification returned false",false)
           ENDIF
          ELSEIF (isvalidpostedcharge(chargefindreply))
           IF ((request->charge[chrgidx].research_acct_id != 0))
            IF (modifyresearchacct(chrgidx,chargefindreply))
             SET reply->success_cnt += 1
             IF (validate(debug,- (1)) > 0)
            ...
            ELSE
             IF (validate(debug,- (1)) > 0)
              CALL logmsg(curprog,"modifyResearchAcct returned failure.",log_debug)
             ENDIF
             SET reply->fail_cnt += 1
             CALL writeerrortoreply(chrgidx,"modifyResearchAcct",
              "Posted Charge: modifyResearchAcct returned false",false)
            ENDIF
           ELSEIF (isvalidmodification(chrgidx,chargefindreply))
            IF ( NOT (loaddiagnosisdetail(chrgidx)))
             CALL logmsg(curprog,"No Diagnosis details present",log_debug)
            ...
            IF ( NOT (inactivemodinchargeeventmod(chrgidx)))
             CALL logmsg("inactiveModInChargeEventMod",
              "Failed to inactivate active_ind in charge_event_mod table",log_debug)
            ENDIF
            IF (modifypostedcharge(chrgidx,chargefindreply,newcreditid,newdebitid))
             IF (savecontributorsystemchgmods(chrgidx))
              SET reply->success_cnt += 1
              IF (validate(debug,- (1)) > 0)
               CALL echo("saveContributorSystemChgMods returned success")
              ENDIF
              SET newcharges->numberofcharges += 2
              SET stat = alterlist(newcharges->charges,newcharges->numberofcharges)
              SET newcharges->charges[(newcharges->numberofcharges - 1)].charge_item_id = newcreditid
              SET newcharges->charges[(newcharges->numberofcharges - 1)].process_flg = 0
              SET newcharges->charges[newcharges->numberofcharges].charge_item_id = newdebitid
              SET newcharges->charges[newcharges->numberofcharges].process_flg = 0
             ELSE
              IF (validate(debug,- (1)) > 0)
               CALL echo("saveContributorSystemChgMods returned a failure")
              ...
              SET reply->fail_cnt += 1
              CALL writeerrortoreply(chrgidx,"saveContributorSystemChgMods",
               "Posted Charge: saveContributorSystemChgMods returned false",false)
             ENDIF
            ELSE
             SET reply->fail_cnt += 1
             CALL writeerrortoreply(chrgidx,"modifyPostedCharge",
              "Posted Charge: modifyPostedCharge returned false",false)
            ENDIF
           ELSE
            SET reply->fail_cnt += 1
            CALL writeerrortoreply(chrgidx,"isValidModification",
             "Posted Charge: isValidModification returned false",false)
           ENDIF
          ELSEIF (isvalidsuspendedcharge(chargefindreply))
           IF (isvalidmodification(chrgidx,chargefindreply))
            IF (modifysuspendedcharge(chrgidx,chargefindreply))
             IF (validate(debug,- (1)) > 0)
              CALL echo("modifySuspendedCharge returned success")
             ENDIF
             IF (savecontributorsystemchgmods(chrgidx))
              SET reply->success_cnt += 1
              IF (validate(debug,- (1)) > 0)
             ...
             ELSE
              ...
              SET reply->fail_cnt += 1
              CALL writeerrortoreply(chrgidx,"saveContributorSystemChgMods",
               "Suspended Charge: saveContributorSystemChgMods returned false",false)
             ENDIF
            ELSE
             SET reply->fail_cnt += 1
             CALL writeerrortoreply(chrgidx,"modifySuspendedCharge",
              "Suspended Charge: modifySuspendedCharge returned false",false)
            ENDIF
           ELSE
            SET reply->fail_cnt += 1
            CALL writeerrortoreply(chrgidx,"isValidModification",
             "Suspended Charge: isValidModification returned false",false)
           ENDIF
          ELSE
           SET reply->fail_cnt += 1
           CALL writeerrortoreply(chrgidx,"Process Flag/Charge Type/Active Ind/Offset Charge Item",
            "Requested charge is not a valid charge to be modified",false)
          ENDIF
         ELSE
          SET reply->fail_cnt += 1
          CALL writeerrortoreply(chrgidx,"isLogicalDomainValid",
           "The charge's logical domain doesn't match the user's",false)
         ENDIF
        ELSE
         SET reply->fail_cnt += 1
         CALL writeerrortoreply(chrgidx,"isEncounterValid","The Encntr_id is not valid",false)
        ENDIF
       ELSE
        SET reply->fail_cnt += 1
        CALL writeerrortoreply(chrgidx,"isPersonValid","The Person_id is not valid",false)
       ENDIF
      ELSE
       SET reply->fail_cnt += 1
       CALL writeerrortoreply(chrgidx,"Charge lock and found",
        "The charge was not able to be locked and/or found",false)
      ENDIF
     ELSE
      SET reply->fail_cnt += 1
      CALL writeerrortoreply(chrgidx,"charge_item_id",
       "Requested Charge Item ID must be provided and greater than 0",false)
     ENDIF
    ELSE
     SET reply->fail_cnt += 1
     CALL writeerrortoreply(chrgidx,"charge_mod_qual",
      "Request's charge_mod_qual does not match the size of the charge_mod structure",false)
    ENDIF
    IF (changescommitted)
     CALL logmsg(curprog,
      "previously encountered a commit for this charge so committing any further changes.",log_debug)
     COMMIT
    ELSEIF ((reply->charge[chrgidx].status="S")
     AND (request->nocommitind=0))
     CALL logmsg(curprog,"status is S, committing changes.",log_debug)
     COMMIT
    ELSEIF ((reply->charge[chrgidx].status="S"))
     CALL logmsg(curprog,"NoCommitInd != 0. Skipping Commit after success.",log_debug)
    ELSEIF ((reply->charge[chrgidx].status="F")
     AND (request->nocommitind=0))
     CALL logmsg(curprog,"status is F, rolling back changes.",log_debug)
     ROLLBACK
    ELSEIF ((reply->charge[chrgidx].status="F"))
     CALL logmsg(curprog,"NoCommitInd != 0. Skipping rollback after failure.",log_debug)
    ENDIF
  ENDFOR
 ELSE
  CALL writestatusdatafailure("CHGQUAL","ChargeQual",
   "request->charge_qual does not match size of charge list")
  GO TO end_program
 ENDIF
 IF (size(newcharges->charges,5) > 0)
  IF ( NOT (postorinterfacecharges(newcharges)))
   CALL writestatusdatafailure("POST","Post/Interface",
    "Failed to submit charges to be posted and/or interfaced")
   GO TO end_program
  ENDIF
 ENDIF
#exit_script
