CREATE PROGRAM bhs_athn_event_prsnl_act_req
 RECORD orequest(
   1 req[*]
     2 ensure_type = i2
     2 version_dt_tm = dq8
 ...
 WHILE (done=0)
   IF (findstring(",",t_line)=0)
    SET t_record->prsnl_cnt = (t_record->prsnl_cnt+ 1)
 ...
 FOR (i = 1 TO t_record->prsnl_cnt)
   ...
   SET orequest->req[i].event_prsnl.action_dt_tm = cnvtdatetimeutc2(date_line,"YYYY-MM-DD",time_line,
    "HH;mm;ss",4)
   ...
   SET orequest->req[i].event_prsnl.request_dt_tm = cnvtdatetimeutc2(date_line,"YYYY-MM-DD",time_line,
    "HH;mm;ss",4)
   SET orequest->req[i].event_prsnl.action_comment =  $8
 ENDFOR
