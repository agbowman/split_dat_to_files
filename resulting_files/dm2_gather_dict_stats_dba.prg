CREATE PROGRAM dm2_gather_dict_stats:dba
 ...
 SUBROUTINE dm2_get_rdbms_version(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dgds_command = vc WITH protect, noconstant(
  "RDB ASIS('BEGIN SYS.DBMS_STATS.GATHER_DICTIONARY_STATS; END;') go")
 DECLARE dgds_command_gt10 = vc WITH protect, noconstant(
  "RDB ASIS('BEGIN DM_DBMS_STATS.GATHER_ORACLE_STATS; END;') go")
 DECLARE dgds_err_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_gather_dict_stat",".log","dm2_gather_dict_stats LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning DM2_GATHER_DICT_STATS"
 CALL disp_msg("",dm_err->logfile,0)
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_script
 ENDIF
 IF ((dm2_rdbms_version->level1 < 10))
  SET dm_err->eproc = "Ora version is lower than 10, auto-successing."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 SET dm2_process_event_rs->status = dpl_executing
 IF ((dm2_rdbms_version->level1 > 10))
  CALL dm2_process_log_add_detail_text(dpl_cmd,dgds_command_gt10)
  IF (dm2_process_log_row(dpl_statistics,dpl_execution,dpl_no_prev_id,1)=0)
   GO TO exit_script
  ELSE
   SET dr_log_id = dm2_process_event_rs->dm_process_event_id
  ENDIF
  IF (dm2_push_cmd(dgds_command_gt10,1)=0)
   GO TO exit_script
  ENDIF
 ELSE
  CALL dm2_process_log_add_detail_text(dpl_cmd,dgds_command)
  IF (dm2_process_log_row(dpl_statistics,dpl_execution,dpl_no_prev_id,1)=0)
   GO TO exit_script
  ELSE
   SET dr_log_id = dm2_process_event_rs->dm_process_event_id
  ENDIF
  IF (dm2_push_cmd(dgds_command,1)=0)
   GO TO exit_script
  ENDIF
 ENDIF
#exit_script
 IF ((dm2_rdbms_version->level1 > 9))
  ...
  SET dgds_err_ind = dm_err->err_ind
  SET dm_err->err_ind = 0
  CALL dm2_process_log_row(dpl_statistics,dpl_execution,dr_log_id,1)
  SET dm_err->err_ind = dgds_err_ind
 ENDIF
 SET dm_err->eproc = "dm2_gather_dict_stats completed successfully."
 CALL final_disp_msg("dm2_gather_dict_stats")
