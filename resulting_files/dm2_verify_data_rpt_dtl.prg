CREATE PROGRAM dm2_verify_data_rpt_dtl
 ...
 SUBROUTINE dvdr_get_verify2(null)
  ...
  RETURN(1)
 END ;Subroutine
 DECLARE dvdrd_cnt = i4 WITH protect, noconstant(0)
 DECLARE dvdrd_dynsql = vc WITH protect, noconstant("")
 DECLARE dvdrd_tname = vc WITH protect, noconstant("")
 DECLARE dvdrd_vname = vc WITH protect, noconstant("")
 DECLARE dvdrd_col_list = vc WITH protect, noconstant("")
 DECLARE dvdrd_src_object_id = f8 WITH protect, noconstant(0.0)
 DECLARE dvdrd_keyid_col_name = vc WITH protect, noconstant("")
 DECLARE dvdrd_dm_vdata_master_id = f8 WITH protect, noconstant(0.0)
 DECLARE dvdrd_status = vc WITH protect, noconstant("")
 DECLARE dvdrd_quit = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_vdata_rpt_dtl",".log","dm2_vdata_rpt_dtl LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Obtaining verify data reporting information"
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 IF (dvdr_get_verify_data_rpt_info(0,dvdrd_quit)=0)
  GO TO exit_script
 ENDIF
 SET message = nowindow
 SET dm_err->eproc = "Obtaining verify data environment information"
 IF (dvdr_get_vdata_rpt_env_info(null)=0)
  GO TO exit_script
 ENDIF
 WHILE (dvdrd_quit=0)
   SET dvdrd_tname = concat(dvdr_rpt_dtl_info->owner,".",dvdr_rpt_dtl_info->table_name)
   SET dm_err->eproc = concat("Generating data verification report for ",dvdrd_tname)
   CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
   SET dm_err->eproc = "Obtaining reporting data from Master Table."
   SELECT INTO "nl:"
    FROM dm_vdata_master dvm
    WHERE (dvm.owner_name=dvdr_rpt_dtl_info->owner)
     AND (dvm.table_name=dvdr_rpt_dtl_info->table_name)
    DETAIL
     dvdrd_col_list = dvm.column_list, dvdrd_src_object_id = dvm.object_id_src, dvdrd_keyid_col_name
      = dvm.keyid_column_name,
     dvdrd_dm_vdata_master_id = dvm.dm_vdata_master_id, dvdrd_status = cnvtupper(trim(dvm
       .compare_status))
     IF (dvm.updt_applctx IN (3, 0)
      AND dvdrd_status=dvdr_mismatch)
      dvdrd_status = "EXCL_3_MISMATCH"
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    ...
    GO TO exit_script
   ENDIF
   IF ( NOT (dvdrd_src_object_id > 0
    AND dvdrd_dm_vdata_master_id > 0))
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "No valid table was returned."
    GO TO exit_script
   ENDIF
   SET dm_err->eproc = concat("Retrieving mismatch details for ",dvdrd_tname)
   SET dvdrd_vname = concat("DM2VDATU",cnvtstring(dvdrd_src_object_id))
   SET width = 132
   IF ((dm_err->debug_flag != 511))
    SET message = window
   ENDIF
   ...
   CALL text(2,2,
    "--------------------------------------------------------------------------------------")
   CALL text(3,2,"Table Validation Report")
   CALL text(4,2,
    "--------------------------------------------------------------------------------------")
   CALL text(6,2,concat("Row matching was performed for ",dvdrd_tname," between:"))
   CALL text(7,2,concat("  Source Database: ",dvdr_rpt_dtl_info->src_db_name,"@",dvdr_rpt_dtl_info->
     db_link))
   CALL text(8,2,concat("  Target Database: ",dvdr_rpt_dtl_info->tgt_db_name))
   CASE (dvdrd_status)
    OF dvdr_match:
     CALL text(11,2,"************* Table Matched 100% *************")
    OF dvdr_mismatch:
     CALL text(11,2,"************* Table Mismatches Found. Detailed Report Follows. *************")
    OF dvdr_comparing:
     CALL text(11,2,"************* Table Currently Being Compared. *************")
    OF dvdr_ready:
     CALL text(11,2,"************* Table Has Not Yet Been Compared. *************")
    OF dvdr_error:
     CALL text(11,2,"************* An Error Occured While Comparing the Table. *************")
    OF dvdr_exclude:
     CALL text(11,2,
      "************* Table Has Been Excluded. View Summary Report for Table Exclusions. *************"
      )
    OF dvdr_nocomp_size:
     CALL text(11,2,
      "************* Table Has Been Excluded. View Summary Report for Table Exclusions. *************"
    ...
    OF "EXCL_3_MISMATCH":
     CALL text(11,2,
      "************* Table Mismatches Found. No Detailed Report Available. *************")
   ENDCASE
   CALL text(15,2,"Enter C to Continue: ")
   CALL accept(15,24,"A;CU","C"
    WHERE curaccept IN ("C"))
   IF (curaccept="C")
    SET message = nowindow
   ENDIF
   IF (dvdrd_status="MISMATCH")
    SET dvdrd_dynsql = concat("select * from ",dvdrd_vname," where ",dvdrd_keyid_col_name," in ",
     "(select dvm.keyid from dm_vdata_mismatch dvm where dvm.dm_vdata_master_id = ",cnvtstring(
      dvdrd_dm_vdata_master_id),")"," order by ",dvdrd_keyid_col_name,
     ",DB")
    CALL parser(concat("rdb ",dvdrd_dynsql," go"))
    IF (check_error(dm_err->eproc) != 0)
     SET dm_err->err_ind = 1
     GO TO exit_script
    ENDIF
   ENDIF
   IF (dvdr_get_verify_data_rpt_info(1,dvdrd_quit)=0)
    GO TO exit_script
   ENDIF
   SET message = nowindow
 ENDWHILE
#exit_script
 SET message = nowindow
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_verify_data_rpt_dtl completed successfully."
 ELSE
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 CALL final_disp_msg("dm2_verify_data_rpt_dtl")
END GO
