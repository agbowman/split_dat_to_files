CREATE PROGRAM dm_rmc_write_dcle_rows:dba
 ...
 SUBROUTINE drcm_get_pk_str(dgps_rec,dgps_tab_name)
   ...
   RETURN(dgps_ret_str)
 END ;Subroutine
 DECLARE drwdr_dcle_id = f8 WITH protect, noconstant(0.0)
 DECLARE v_temp_qual = i2 WITH protect, noconstant(0)
 DECLARE drwdr_postlink_name = vc WITH protect, constant(concat("@merge",trim(cnvtstring(request->
     source_env_id)),trim(cnvtstring(request->target_env_id))))
 DECLARE drwdr_excptn_tab = vc WITH protect, constant(concat("DM_CHG_LOG_EXCEPTION",
   drwdr_postlink_name))
 DECLARE drwdr_remote_utc = vc WITH protect, constant(concat("USER_TAB_COLUMNS",drwdr_postlink_name))
 DECLARE dhe_cnt = i4 WITH protect, noconstant(0)
 IF (trim(request->log_type)="")
  SET request->log_type = "NOMV99"
 ENDIF
 IF ((request->dclei_ind=1))
  IF ((request->table_name IN ("DCP_SECTION_REF", "DCP_FORMS_REF")))
   SELECT INTO "NL:"
    FROM (parser(drwdr_excptn_tab) d)
    WHERE (d.table_name=request->table_name)
     AND (d.target_env_id=request->target_env_id)
     AND (d.from_value=request->from_value)
    DETAIL
     drwdr_dcle_id = d.dm_chg_log_exception_id, request->col_name = d.column_name
    WITH nocounter
   ;end select
  ELSE
   SELECT INTO "NL:"
    FROM (parser(drwdr_excptn_tab) d)
    WHERE (d.table_name=request->table_name)
     AND (d.column_name=request->col_name)
     AND (d.target_env_id=request->target_env_id)
     AND (d.from_value=request->from_value)
    DETAIL
     drwdr_dcle_id = d.dm_chg_log_exception_id
    WITH nocounter
   ;end select
  ENDIF
 ELSE
  IF ((request->table_name IN ("DCP_SECTION_REF", "DCP_FORMS_REF")))
   SELECT INTO "NL:"
    ...
    WHERE (d.table_name=request->table_name)
     ...
     AND (d.from_value=request->from_value)
    DETAIL
     request->col_name = d.column_name
    WITH nocounter
   ;end select
  ELSE
   SELECT INTO "NL:"
 ...
 IF ((request->dclei_ind=1))
  IF (drwdr_dcle_id=0)
   IF ((request->dcl_excep_id > 0))
    SET drwdr_dcle_id = request->dcl_excep_id
   ELSE
    SELECT INTO "nl:"
     y = seq(rdds_source_clinical_seq,nextval)
     FROM dual
     DETAIL
      drwdr_dcle_id = y
     WITH nocounter
 ...
 IF (v_temp_qual=0)
  IF ((request->dclei_ind=1))
   INSERT  FROM (parser(drwdr_excptn_tab) d)
    SET d.table_name = request->table_name, d.column_name = request->col_name, d.target_env_id =
     request->target_env_id,
     d.from_value = request->from_value, d.log_type = request->log_type, d.dm_chg_log_exception_id =
     drwdr_dcle_id
    WITH nocounter
   ;end insert
  ELSE
   INSERT  FROM (parser(drwdr_excptn_tab) d)
    SET d.table_name = request->table_name, d.column_name = request->col_name, d.target_env_id =
     request->target_env_id,
     d.from_value = request->from_value, d.log_type = request->log_type
    WITH nocounter
   ;end insert
  ENDIF
  IF (check_error(dm_err->eproc)=1)
   ...
   SET reply->error_ind = 1
  ELSE
   SET reply->dcle_id = drwdr_dcle_id
  ENDIF
 ELSE
  IF ((request->dclei_ind=1))
   UPDATE  FROM (parser(drwdr_excptn_tab) d)
    SET d.log_type = request->log_type, d.dm_chg_log_exception_id = drwdr_dcle_id
    WHERE (d.table_name=request->table_name)
     AND (d.column_name=request->col_name)
     AND (d.target_env_id=request->target_env_id)
     AND (d.from_value=request->from_value)
  ...
  ELSE
   UPDATE  FROM (parser(drwdr_excptn_tab) d)
    SET d.log_type = request->log_type
    WHERE (d.table_name=request->table_name)
     AND (d.column_name=request->col_name)
     AND (d.target_env_id=request->target_env_id)
     AND (d.from_value=request->from_value)
 ...
 IF ((reply->error_ind=0))
  SET stat = alterlist(drmm_hold_exception->qual,(drmm_hold_exception->value_cnt+ 1))
  SET dhe_cnt = (drmm_hold_exception->value_cnt+ 1)
  SET drmm_hold_exception->value_cnt = dhe_cnt
  SET drmm_hold_exception->qual[dhe_cnt].table_name = request->table_name
  SET drmm_hold_exception->qual[dhe_cnt].column_name = request->col_name
  SET drmm_hold_exception->qual[dhe_cnt].from_value = request->from_value
  SET drmm_hold_exception->qual[dhe_cnt].log_type = request->log_type
  SET drmm_hold_exception->qual[dhe_cnt].dcl_excep_id = reply->dcle_id
 ENDIF
#end_drwdr
END GO
