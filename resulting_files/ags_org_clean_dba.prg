CREATE PROGRAM ags_org_clean:dba
 PROMPT
  "JOB_ID (0.0) = " = 0
  WITH djob_id
 CALL echo("***")
 CALL echo("***   BEG AGS_ORG_CLEAN")
 CALL echo("***")
 ...
 DECLARE sstatus_file_name = vc WITH public, noconstant(concat("ags_org_clean_",format(cnvtdatetime(
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> ags_org_clean"
 ...
 CALL echo("***")
 CALL echo("***")
 CALL echo("Determine Provider Directory Cut-Over")
 CALL echo("***")
 ...
 IF (icutoverflag < 1)
  ...
  SET log->qual[log->qual_knt].smsgtype = "INFO"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = "Not On Provider Directory Code"
 ELSE
  SET log->qual_knt = (log->qual_knt+ 1)
  SET stat = alterlist(log->qual,log->qual_knt)
  SET log->qual[log->qual_knt].smsgtype = "INFO"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = "Provider Directory Detected"
  EXECUTE ags_pvd_org_clean
  GO TO exit_script
 ENDIF
 CALL echo("***")
  ...
  IF (create_orgs)
   PLAN (o1
    WHERE o1.ags_job_id=the_job_id
     AND ((o1.organization_id+ 0) > 0)
     AND o1.status != "IN ERROR"
     AND o1.run_nbr=working_run_nbr)
    JOIN (o2
    WHERE o2.organization_id=o1.organization_id
     AND ((o2.contributor_system_cd+ 0)=o1.contributor_system_cd)
     AND ((o2.run_nbr+ 0) < o1.run_nbr))
  ELSE
   PLAN (o1
    WHERE o1.ags_job_id=the_job_id
     AND o1.location_cd > 0
     AND o1.status != "IN ERROR"
     AND o1.run_nbr=working_run_nbr)
    JOIN (o2
    WHERE o2.location_cd=o1.location_cd
     AND ((o2.contributor_system_cd+ 0)=o1.contributor_system_cd)
     AND ((o2.run_nbr+ 0) < o1.run_nbr))
  ENDIF
  INTO "nl:"
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> ags_org_clean"
 CALL echo("***")
 ...
 CALL echo("***")
 CALL echo("***")
 CALL echo("***   END AGS_ORG_CLEAN")
 CALL echo("***")
 SET script_ver = "003 10/11/06"
END GO
