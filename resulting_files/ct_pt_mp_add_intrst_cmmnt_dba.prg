CREATE PROGRAM ct_pt_mp_add_intrst_cmmnt:dba
 PROMPT
 ...
 SUBROUTINE (nextsequence(x=i2) =f8)
   ...
   RETURN(nsequence)
 END ;Subroutine
 SET log_program_name = "ct_pt_mp_add_intrst_cmmnt"
 FREE RECORD request
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_pt_mp_add_intrst_cmmnt","Invalid JSON Request",
   "reply")
  GO TO exit_script
 ENDIF
 ...
 DECLARE pt_interest_comment_id = f8 WITH protect, constant(nextsequence(0))
 ...
 SELECT INTO "nl:"
  ...
  WHERE (p.person_id=reqinfo->updt_id)
   AND p.active_ind=1
  DETAIL
   reply->prsnl_full_name = concat(trim(p.name_last),", ",trim(p.name_first))
  WITH nocounter
 ;end select
 INSERT  FROM pt_interest_comment pic
  SET pic.pt_interest_comment_id = pt_interest_comment_id, pic.person_id = request->person_id, pic
   .comment_added_prsnl_id = reqinfo->updt_id,
   pic.comment_added_dt_tm = cnvtdatetime(request->comment_add_dt_tm), pic.comment_txt = request->
   comment_txt, pic.active_ind = 1,
   pic.updt_cnt = 0, pic.updt_dt_tm = cnvtdatetime(sysdate), pic.updt_id = reqinfo->updt_id,
   pic.updt_task = reqinfo->updt_task, pic.updt_applctx = reqinfo->updt_applctx
  WITH nocounter
 ;end insert
 IF (curqual=0)
  SET reply->status_data.subeventstatus[1].targetobjectvalue =
  "Error inserting new comment to pt_interest_comment"
  SET fail_flag = insert_error
  GO TO check_error
 ENDIF
#check_error
 IF (fail_flag=0)
  SET reply->added_dt_tm = request->comment_add_dt_tm
  SET reply->status_data.status = "S"
  SET reply->id = pt_interest_comment_id
  SET reqinfo->commit_ind = 1
  SET reply->status_data.subeventstatus[1].operationname = ""
  SET reply->status_data.subeventstatus[1].operationstatus = "S"
  SET reply->status_data.subeventstatus[1].targetobjectname = ""
  SET reply->status_data.subeventstatus[1].targetobjectvalue = ""
  EXECUTE cclaudit audit_mode, "PS_Interest_Comment_Add", "Add",
