CREATE PROGRAM bhs_athn_search_allorders
 ...
 SET where_params = build("O.mnemonic_key_cap = ","'*",cnvtupper(input_string),"*'"," ")
 IF (( $3="0"))
  SET where_params2 = build("O.catalog_type_cd != 2516 ")
 ELSE
  SET where_params2 = build("O.dcp_clin_cat_cd IN"," ", $3," ")
 ENDIF
 SET where_params1 = build(" OS.ORDER_ENCNTR_GROUP_CD IN (0,", $5,")")
 ...
 SELECT INTO  $1
  ...
  d_min_behind_action_disp = uar_get_code_display(d.min_behind_action_cd), d.outpat_flex_ind,
  d_outpat_exact_hit_action_disp = uar_get_code_display(d.outpat_exact_hit_action_cd),
  d.outpat_min_ahead, d_outpat_min_ahead_action_disp = uar_get_code_display(d
   .outpat_min_ahead_action_cd), d.outpat_min_behind,
  d_outpat_min_behind_action_disp = uar_get_code_display(d.outpat_min_behind_action_cd), oc
  ...
  PLAN (o
   ...
   WHERE (os.parent_entity_id= Outerjoin(o.synonym_id))
    ...
    AND (((os.order_encntr_group_cd= Outerjoin( $5)) ) OR ((os.order_encntr_group_cd= Outerjoin(0))
   )) )
   JOIN (osd
  ...
  ORDER BY o.synonym_id, os.order_sentence_id DESC
  HEAD REPORT
  ...
  HEAD o.synonym_id
   ...
   row + 1, v303 = build("<Display>",trim(replace(replace(replace(replace(replace(replace(
           uar_get_code_display(o.catalog_cd),"Â ","",0),"&","&amp;",0),"<","&lt;",0),">","&gt;",0),
       "'","&apos;",0),'"',"&quot;",0),3),"</Display>"), col + 1,
   v303, row + 1, col + 1,
   "</Catalog>", row + 1, col + 1,
