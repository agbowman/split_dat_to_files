CREATE PROGRAM bed_get_iview_dcp_wv_sect:dba
 FREE SET reply
 RECORD reply(
 ...
 DECLARE dcp_wv_id = f8 WITH protect, noconstant(0.0)
 SET reply->status_data.status = "F"
 SET dcp_wv_id = getcurrentdcpworkingviewid(null)
 CALL populateworkingviewsections(dcp_wv_id)
#exit_script
 ...
 SUBROUTINE getcurrentdcpworkingviewid(dummyvar)
   ...
   RETURN(match_id)
 END ;Subroutine
 SUBROUTINE populateworkingviewsections(dcp_wv_id)
   DECLARE scnt = i4 WITH protect, noconstant(0)
   SET reply->working_view_uid = request->working_view_uid
   SET reply->working_view_id = dcp_wv_id
   SELECT INTO "nl:"
    FROM working_view_section s,
     working_view_item i
    PLAN (s
     WHERE s.working_view_id=dcp_wv_id)
     JOIN (i
    ...
    HEAD s.working_view_section_id
     scnt = (scnt+ 1), stat = alterlist(reply->sections,scnt), reply->sections[scnt].
     working_view_section_id = s.working_view_section_id,
     reply->sections[scnt].event_set_name = s.event_set_name, reply->sections[scnt].display_name = s
     .display_name
   ;end select
 END ;Subroutine
END GO
