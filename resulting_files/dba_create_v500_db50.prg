CREATE PROGRAM dba_create_v500_db50
 SET message = window
 ...
 SUBROUTINE aix_create_script(x)
    ...
    *
    ...
    DETAIL
     ...
     CALL print(oracle_ver),
     row + 1, col 0, "export PATH=$PATH:$ORACLE_HOME/bin",
     row + 1, col 0, "export ORA_LINKS=",
     CALL print(oracle_mtpt), "/oralink/",
     CALL print(cnvtlower(dbname)),
     row + 1, col 0, "export ORA_ADMIN=",
     CALL print(oracle_mtpt), "/oracle/admin/",
     CALL print(cnvtlower(dbname)),
     row + 1, col 0, "export DBA=$ORACLE_ADMIN/pfile",
     row + 1, col 0, "export dba=$CerFsMtPt/w_standard/$WareHouse/aixrs6000",
     row + 1, col 0, "export LIBPATH=/usr/lib:$CerFsMtPt/w_standard/$WareHouse/aixrs6000",
     row + 1
    WITH nocounter, format = stream, noheading,
     formfeed = none, maxrow = 1
   ;end select
    ...
    *
    ...
    DETAIL
     ...
     col 0, row + 1, col 0,
     "alter user system temporary tablespace temp;", row + 1, col 0,
     row + 1, col 0, "connect system/manager",
     row + 1, col 0, row + 1,
     col 0, "@",
     CALL print(scrpt_dir),
     "/create_users2.sql", row + 1, col 0,
     row + 1, col 0, "spool off",
     row + 1, col 0, "set termout off",
     row + 1, col 0, "@$ORACLE_HOME/sqlplus/admin/pupbld.sql",
     row + 1, col 0, row + 1,
     col 0, "set termout on", row + 1,
     ...
     CALL print(cnvtlower(dbname)), "1.ora ", "$ORACLE_HOME/dbs/init",
     CALL print(cnvtlower(dbname)), "1.ora", row + 1,
     col 0,
     CALL print(trim(dba_admin)), "/updatetns.ksh -a ",
     CALL print(alias), " -n ",
     CALL print(node),
     " -p 1521 -s ",
     CALL print(dbsid), " -h ",
