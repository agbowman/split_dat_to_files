CREATE PROGRAM dm2_create_admin_nicknames:dba
 DECLARE can_logfile_prefix = vc WITH protect, constant("dm2_cre_adm_nicknms")
 DECLARE can_admn_tname = vc WITH protect, noconstant(" ")
 DECLARE can_admn_pword = vc WITH protect, noconstant(" ")
 DECLARE can_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcan_link_name = vc WITH protect, noconstant(" ")
 DECLARE dcan_exist = i2 WITH protect, noconstant(0)
 DECLARE dcan_ndx = i4 WITH protect, noconstant(0)
 DECLARE dcan_db_link_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcan_dot_pos = i4 WITH protect, noconstant(0)
 DECLARE get_list_of_admin_tables(null) = i2
 DECLARE chk_for_clin_seq(syn_to_test=vc,cfcs_syn_exist=i2(ref)) = i2
 DECLARE get_list_of_admin_sequences(null) = i2
 FREE RECORD nickname_list
 RECORD nickname_list(
   1 cnt = i4
   1 qual[*]
     2 nickname = vc
     2 seq_ind = i2
 ...
 SUBROUTINE dsl_create_dm_flags_objs(null)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile(can_logfile_prefix,".log","DM2_CREATE_ADMIN_NICKNAMES LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning Create Admin Nicknames Process"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET can_admn_tname = cnvtupper(build(trim( $1,3)))
 IF (can_admn_tname=" ")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Admin Table Specified ",can_admn_tname," is invalid.")
  SET dm_err->eproc = "Admin Table Validation"
  SET dm_err->user_action = concat("Please specify a valid admin table. ")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET can_admn_pword = build(trim( $2,3))
 IF (can_admn_pword=" ")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Admin Password  ",can_admn_pword," is invalid.")
  SET dm_err->eproc = "Admin Password Validation"
  SET dm_err->user_action = concat("Please specify a valid admin password. ")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Getting database link."
 SET dcan_link_name = dm2_get_db_link(null)
 IF (dcan_link_name="DM2_ERROR")
  GO TO exit_program
 ELSEIF (dcan_link_name="DM2_UNKNOWN")
  ...
  ELSE
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     dcan_dot_pos = findstring(".",dl.db_link,1)
     IF (dcan_dot_pos > 0)
      dcan_link_name = substring(1,(dcan_dot_pos - 1),dl.db_link)
     ELSE
      dcan_link_name = dl.db_link
     ENDIF
     IF (cnvtupper(dcan_link_name)=cnvtupper(dm2_server_link->server_name))
      dcan_db_link_cnt = (dcan_db_link_cnt+ 1)
     ENDIF
    WITH nocounter
   ;end select
   IF (dcan_db_link_cnt=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "The connect string entered is invalid."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ELSEIF (dcan_db_link_cnt > 1)
    SET dm_err->err_ind = 1
  ...
  ENDIF
 ELSE
  SET dm2_server_link->server_name = dcan_link_name
 ENDIF
 IF ((dm_err->debug_flag > 0))
 ...
 IF (dm2_set_inhouse_domain(null)=0)
  GO TO exit_program
 ENDIF
 IF (((can_admn_tname=char(42)) OR (can_admn_tname="DM_ENVIRONMENT")) )
 ...
 ELSE
  ...
  ENDIF
 ENDIF
 IF (get_list_of_admin_tables(null)=0)
  GO TO exit_program
 ENDIF
 IF (get_list_of_admin_sequences(null)=0)
  GO TO exit_program
 ENDIF
 IF ((nickname_list->cnt=0))
  SET dm_err->eproc = "Checking for nicknames that need to be created"
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("No Admin tables/sequences matched '",can_admn_tname,
 ...
 IF ((dm_err->debug_flag > 20))
  CALL echorecord(nickname_list)
 ENDIF
 IF ((nickname_list->cnt > 0))
  FOR (can_cnt = 1 TO nickname_list->cnt)
    SET dm2_nickname_info->nickname = nickname_list->qual[can_cnt].nickname
    SET dm2_nickname_info->drop_ind = nickname_list->qual[can_cnt].drop_ind
    SET dm2_nickname_info->create_ind = 1
    SET dm2_nickname_info->local_owner = "NONE"
    SET dm2_nickname_info->server = dm2_server_link->server_name
    SET dm2_nickname_info->remote_table = nickname_list->qual[can_cnt].nickname
    ...
    SET dm2_nickname_info->create_view_ind = 0
    SET dcan_ndx = 0
    SET dcan_ndx = locateval(dcan_ndx,1,dsl_dmtools_adm_tables->cnt,dm2_nickname_info->nickname,
     dsl_dmtools_adm_tables->tbl[dcan_ndx].table_name)
    IF (dcan_ndx > 0)
     SET dm2_nickname_info->create_view_ind = 1
    ENDIF
    IF ((nickname_list->qual[can_cnt].seq_ind=1))
     IF (chk_for_clin_seq(nickname_list->qual[can_cnt].nickname,dcan_exist)=0)
      GO TO exit_program
     ENDIF
     IF ((nickname_list->qual[can_cnt].nickname != "DM_SEQ")
      AND dcan_exist=1)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = concat("The sequence [",nickname_list->qual[can_cnt].nickname,
    ...
    ELSE
     IF (dm2_create_nickname(null)=0)
      ...
      GO TO exit_program
     ENDIF
     EXECUTE oragen3 build("cdba.",nickname_list->qual[can_cnt].nickname,"@",dm2_server_link->
      server_name)
     IF ((dm_err->err_ind=1))
      ROLLBACK
      GO TO exit_program
     ...
     ENDIF
     IF (dcan_ndx > 0)
      SET dsl_dmtools_adm_tables->tbl[dcan_ndx].drop_ind = 0
      SET dm_err->eproc = "Checking whether synonym exists."
      SELECT INTO "nl:"
       FROM dm2_dba_synonyms ds
       WHERE (ds.synonym_name=dsl_dmtools_adm_tables->tbl[dcan_ndx].synonym_name)
        AND ds.owner="PUBLIC"
       DETAIL
        dsl_dmtools_adm_tables->tbl[dcan_ndx].drop_ind = 1
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_program
      ENDIF
      IF (dsl_create_spec_admin_synonyms(dcan_ndx)=0)
       GO TO exit_program
      ENDIF
     ENDIF
    ENDIF
  ...
  IF ((inhouse_misc->inhouse_domain=1))
   SET dcan_ndx = 0
   SET dcan_ndx = locateval(dcan_ndx,1,nickname_list->cnt,"DM_README_DEV",nickname_list->qual[
    dcan_ndx].nickname)
   IF (dcan_ndx > 0)
 ...
 ENDIF
 GO TO exit_program
 SUBROUTINE get_list_of_admin_tables(null)
   ...
   SET dm_err->eproc = "Getting list of ADMIN tables that nicknames need to be created."
   CALL disp_msg(" ",dm_err->logfile,0)
   ...
   SELECT
    IF (gloat_dm2_admin_dm_info_exists=0)
     FROM dm2_admin_tables at,
      dm2_dba_synonyms ds
     PLAN (at
      WHERE at.table_name=patstring(can_admn_tname)
    ...
    ELSE
     ...
     PLAN (adm
      ...
      WHERE at.table_name=adm.info_name
       AND at.table_name=patstring(can_admn_tname)
       AND parser(gloat_dyn_in_clause))
      JOIN (ds
      WHERE outerjoin(at.table_name)=ds.synonym_name
       AND ds.owner=outerjoin("PUBLIC"))
    ...
    HEAD REPORT
     stat = alterlist(nickname_list->qual,10), nickname_list->cnt = 0
    DETAIL
     nickname_list->cnt = (nickname_list->cnt+ 1)
     IF (mod(nickname_list->cnt,10)=1
      AND (nickname_list->cnt != 1))
      stat = alterlist(nickname_list->qual,(nickname_list->cnt+ 9))
     ENDIF
     nickname_list->qual[nickname_list->cnt].nickname = at.table_name, nickname_list->qual[
     nickname_list->cnt].seq_ind = 0, nickname_list->qual[nickname_list->cnt].drop_ind = 0
     IF (ds.synonym_name > " ")
      nickname_list->qual[nickname_list->cnt].drop_ind = 1
     ENDIF
     nickname_list->qual[nickname_list->cnt].col_cnt = 0
    FOOT REPORT
     stat = alterlist(nickname_list->qual,nickname_list->cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE get_list_of_admin_sequences(null)
   ...
   SELECT INTO "nl:"
    FROM dm2_admin_sequences us,
     dm2_dba_synonyms ds
    PLAN (us
     WHERE us.sequence_name=patstring(can_admn_tname)
    ...
    DETAIL
     nickname_list->cnt = (nickname_list->cnt+ 1), stat = alterlist(nickname_list->qual,nickname_list
      ->cnt), nickname_list->qual[nickname_list->cnt].nickname = us.sequence_name,
     nickname_list->qual[nickname_list->cnt].seq_ind = 1, nickname_list->qual[nickname_list->cnt].
     drop_ind = 0
     IF (ds.synonym_name > " ")
      nickname_list->qual[nickname_list->cnt].drop_ind = 1
     ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE chk_for_clin_seq(syn_to_test,cfcs_syn_exist)
   SET dm_err->eproc = "Check to see if clinical sequence exists with same name."
   SELECT INTO "nl:"
    FROM dm2_user_sequences us
    WHERE us.sequence_name=syn_to_test
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->emsg = "Error selecting from dm2_user_sequences"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (curqual > 0)
 ...
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Create Admin Nicknames Process Completed!"
 CALL final_disp_msg(can_logfile_prefix)
END GO
