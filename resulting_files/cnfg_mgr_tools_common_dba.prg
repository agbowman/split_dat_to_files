CREATE PROGRAM cnfg_mgr_tools_common:dba
 DECLARE PUBLIC::getstaticcontentroot(applicationname=vc,relative_path=vc) = vc WITH protect, copy
 DECLARE PUBLIC::validateserviceurl(url_str=vc) = i4 WITH protect, copy
 DECLARE PUBLIC::errorstackcheck(operation=vc) = null WITH protect, copy
 DECLARE PUBLIC::get_i18n_message(i18nkey=vc,fallbacktext=vc) = vc WITH protect, copy
 DECLARE PUBLIC::get_locale_data_sub(locale=vc) = vc WITH protect, copy
 DECLARE PUBLIC::geti18nfilepath(dminfoidentifier=vc) = vc WITH protect, copy
 IF (checkfun("CNFG_MGR_TOOLS_COMMON_MAIN")=7)
  CALL cnfg_mgr_tools_common_main(null)
 ENDIF
 SUBROUTINE PUBLIC::getstaticcontentroot(applicationname,relative_path)
 ...
 SUBROUTINE validateserviceurl(url_str)
   ...
   RETURN(false)
 END ;Subroutine
 SUBROUTINE PUBLIC::errorstackcheck(operation)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE PUBLIC::get_i18n_message(i18nkey,fallbacktext)
 ...
 SUBROUTINE get_locale_data_sub(locale)
   ...
   ENDCASE
 END ;Subroutine
 SUBROUTINE PUBLIC::geti18nfilepath(dminfoidentifier)
   DECLARE i18nfilepath = vc WITH noconstant(trim("")), protect
   IF (cmnisnotblank(dminfoidentifier))
    SELECT
     d.info_char
     FROM dm_info d
