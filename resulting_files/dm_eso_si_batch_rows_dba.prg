CREATE PROGRAM dm_eso_si_batch_rows:dba
 ...
 END ;Subroutine
 CALL echo("<===== DM_ESO_SI_BATCH_ROWS  BEGIN =====>")
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    b.rowid
    FROM si_batch b
    WHERE parser(sbr_getrowidnotexists("b.batch_id > 0","b"))
     AND b.updt_dt_tm < cnvtdatetime((curdate - days_to_keep),0)
     AND  NOT (b.batch_status_flag IN (10, 20, 50))
    DETAIL
     rows += 1
     IF (mod(rows,1000)=1)
...
#exit_script
 CALL echo("<===== DM_ESO_SI_BATCH_ROWS END =====>")
END GO
