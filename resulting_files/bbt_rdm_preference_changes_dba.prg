CREATE PROGRAM bbt_rdm_preference_changes:dba
 ...
 DECLARE question_cd = f8 WITH protected, noconstant(0.0)
 DECLARE verify_cd = f8 WITH protected, noconstant(0.0)
 DECLARE modify_cd = f8 WITH protected, noconstant(0.0)
 DECLARE lblvfy_confs_cd = f8 WITH protected, noconstant(0.0)
 DECLARE inactive_type_cd = f8 WITH protected, noconstant(0.0)
 DECLARE remove_lblvfy_cd = f8 WITH protected, noconstant(0.0)
 SET serrormsg = fillstring(255," ")
 SET nerrorstatus = error(serrormsg,1)
 SET sreadme_name = "bbt_rdm_preference_changes"
 ...
 SELECT INTO "nl:"
  ...
  WHERE c.code_set=1661
   AND c.cdf_meaning IN ("INST NAME", "LBLVERIFDEMO", "MODIFY TAG", "LBLVFY CONFS")
   AND c.code_value > 0.0
  DETAIL
   CASE (c.cdf_meaning)
    OF "INST NAME":
     question_cd = c.code_value
    OF "LBLVERIFDEMO":
     verify_cd = c.code_value
    OF "MODIFY TAG":
     modify_cd = c.code_value
    OF "LBLVFY CONFS":
     lblvfy_confs_cd = c.code_value
   ENDCASE
  WITH nocounter, forupdate(c)
 ;end select
 IF (curqual=0)
  SET status_code = 99
  SET msgstr = "Could not find Institution Name or Display Product Demographics in code_value table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 DELETE  FROM valid_response r
  SET r.seq = 1
  WHERE r.question_cd IN (question_cd, verify_cd, modify_cd, lblvfy_confs_cd)
   AND r.question_cd > 0
  WITH nocounter
 ;end delete
 ...
 IF (nerrorstatus != 0)
  SET status_code = 1
  SET msgstr =
  "Could not delete Institution Name or Display Product Demographics or Modify Tag from valid response table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 DELETE  FROM answer a
  SET a.seq = 1
  WHERE a.question_cd IN (question_cd, verify_cd, modify_cd, lblvfy_confs_cd)
   AND a.question_cd > 0
  WITH nocounter
 ;end delete
 SET serror_check = error(serrormsg,0)
 IF (nerrorstatus != 0)
  SET status_code = 1
  SET msgstr =
  "Could not delete Institution Name or Display Product Demographics or Modify Tag from answer table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 DELETE  FROM question q
  SET q.seq = 1
  WHERE q.question_cd IN (question_cd, verify_cd, modify_cd, lblvfy_confs_cd)
   AND q.question_cd > 0
  WITH nocounter
 ;end delete
 SET serror_check = error(serrormsg,0)
 IF (nerrorstatus != 0)
  SET status_code = 1
  SET msgstr =
  "Could not delete Institution Name or Display Product Demographics or Modify Tag from question table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 UPDATE  FROM code_value c
  SET c.active_ind = 0, c.active_type_cd = inactive_type_cd, c.active_dt_tm = null,
  ...
  WHERE c.code_value IN (question_cd, verify_cd, modify_cd, lblvfy_confs_cd)
   AND c.code_value > 0
  WITH nocounter
 ;end update
 SET serror_check = error(serrormsg,0)
 IF (nerrorstatus != 0)
  SET status_code = 1
  SET msgstr =
  "Could not inactive Institution Name or Display Product Demographics or Modify Tag in code_value table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  ...
  WHERE c.code_set=1662
   AND c.cdf_meaning="LABELVERIF"
   AND c.active_ind=1
  DETAIL
   remove_lblvfy_cd = c.code_value
  WITH nocounter, forupdate(c)
 ;end select
 IF (remove_lblvfy_cd > 0.0)
  UPDATE  FROM code_value c
   SET c.active_ind = 0
   WHERE c.code_value=remove_lblvfy_cd
    AND c.code_set=1662
   WITH nocounter
  ;end update
  SET serror_check = error(serrormsg,0)
  IF (nerrorstatus != 0)
   SET status_code = 1
   SET msgstr = "Could not inactivate Label Verification in code_value table."
   CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
   GO TO exit_script
  ENDIF
 ELSE
  SET status_code = 99
  SET msgstr = "Could not find Label Verification in code_value table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
#exit_script
 ...
 IF (status_code=1)
  ROLLBACK
  SET msgstr = "Error adding or delete preference question."
  SET msgstatus = "F"
 ELSEIF (status_code=0)
  SET msgstr = "Preference questions have been successfully deleted."
  SET msgstatus = "S"
  COMMIT
 ELSEIF (status_code=99)
  SET msgstr = "Preferences already inactivated, nothing to process."
  SET msgstatus = "S"
 ENDIF
 CALL logstatus(msgstr,msgstatus)
 CALL logmsg(msgstr,((e_rdmstatus+ e_dispmsg)+ e_logtofile))
 FREE RECORD temp_valid_resp
