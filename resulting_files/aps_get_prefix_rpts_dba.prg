CREATE PROGRAM aps_get_prefix_rpts:dba
 ...
 ENDIF
 SELECT INTO "nl:"
  prr.prefix_id, sd.catalog_cd, service_resource_exists = evaluate(nullind(orl.service_resource_cd),1,
   0,1),
  system_ordered = evaluate(nullind(apat.prefix_id),1,0,1)
  ...
  PLAN (prr
   ...
   JOIN (sd
   WHERE prr.catalog_cd=sd.catalog_cd)
   JOIN (orl
   WHERE (orl.catalog_cd= Outerjoin(sd.catalog_cd)) )
   JOIN (apat
   WHERE (apat.prefix_id= Outerjoin(request->prefix_cd))
    AND (apat.catalog_cd= Outerjoin(prr.catalog_cd)) )
  ORDER BY sd.catalog_cd
  HEAD REPORT
   stat = alterlist(reply->rpt_info_qual,1), stat = alterlist(reply->rpt_info_qual[1].rpt_qual,10),
   resource_cnt = 0,
   service_resource_cd = 0.0
  HEAD sd.catalog_cd
   resource_cnt = 0, rpt_cnt += 1
   IF (mod(rpt_cnt,10)=1
    AND rpt_cnt != 1)
    stat = alterlist(reply->rpt_info_qual[1].rpt_qual,(rpt_cnt+ 9))
   ENDIF
   reply->rpt_info_qual[1].rpt_qual[rpt_cnt].catalog_cd = prr.catalog_cd, reply->rpt_info_qual[1].
   rpt_qual[rpt_cnt].short_description = sd.short_description, reply->rpt_info_qual[1].rpt_qual[
   rpt_cnt].primary_ind = prr.primary_ind,
   reply->rpt_info_qual[1].rpt_qual[rpt_cnt].mult_allowed_ind = prr.mult_allowed_ind, reply->
   rpt_info_qual[1].rpt_qual[rpt_cnt].system_ordered_ind = system_ordered, stat = alterlist(reply->
    rpt_info_qual[1].rpt_qual[rpt_cnt].resource_qual,10)
  DETAIL
   IF (service_resource_exists=1)
    service_resource_cd = orl.service_resource_cd
    IF (isresourceviewable(service_resource_cd)=true)
     resource_cnt += 1
     IF (mod(resource_cnt,10)=1
      AND resource_cnt != 1)
      stat = alterlist(reply->rpt_info_qual[1].rpt_qual[rpt_cnt].resource_qual,(resource_cnt+ 9))
     ENDIF
     reply->rpt_info_qual[1].rpt_qual[rpt_cnt].resource_qual[resource_cnt].service_resource_cd = orl
     .service_resource_cd
    ENDIF
   ENDIF
  FOOT  sd.catalog_cd
   IF (resource_cnt=0)
    stat = alterlist(reply->rpt_info_qual[1].rpt_qual[rpt_cnt].resource_qual,1)
   ELSE
    stat = alterlist(reply->rpt_info_qual[1].rpt_qual[rpt_cnt].resource_qual,resource_cnt)
   ENDIF
  WITH nocounter
 ;end select
 ...
 ELSE
  SET stat = alterlist(reply->rpt_info_qual[1].rpt_qual,rpt_cnt)
