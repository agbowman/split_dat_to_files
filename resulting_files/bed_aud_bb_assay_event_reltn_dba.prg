CREATE PROGRAM bed_aud_bb_assay_event_reltn:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (cv
   WHERE cv.code_set=106
    ...
    AND cv.active_ind=1)
  DETAIL
   bb = cv.code_value, act_type = cv.display
  WITH nocounter
 ;end select
 SELECT INTO "NL:"
  FROM code_value cv
...
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("bb_assays_to_evntcd_evntset.csv")
 ENDIF
