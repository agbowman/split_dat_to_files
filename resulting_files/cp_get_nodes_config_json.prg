CREATE PROGRAM cp_get_nodes_config_json
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Node IDs:" = 0.0,
  "Node Type CDs:" = 0.00,
  "Resolve Concept Details:" = 0
  WITH outdev, node_ids, node_type_cds,
  resolve_concept_details
 FREE RECORD request
 RECORD request(
   1 node_ids[*]
     2 cp_node_id = f8
 ...
 SUBROUTINE preparerequest(null)
   ...
   CALL getparametervalues(2,tempnodeids)
   CALL getparametervalues(3,tempnodetypecds)
   SET stat = alterlist(request->node_ids,tempnodeids->cnt)
   FOR (x = 1 TO tempnodeids->cnt)
     SET request->node_ids[x].cp_node_id = tempnodeids->qual[x].value
   ENDFOR
   SET stat = alterlist(request->node_type_cds,tempnodetypecds->cnt)
   FOR (x = 1 TO tempnodetypecds->cnt)
     SET request->node_type_cds[x].node_type_cd = tempnodetypecds->qual[x].value
   ENDFOR
   SET request->resolve_concept_details =  $RESOLVE_CONCEPT_DETAILS
   CALL log_message(build("Exit prepareRequest(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE executescript(null)
   CALL log_message("In executeScript()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   EXECUTE cp_get_nodes_config
