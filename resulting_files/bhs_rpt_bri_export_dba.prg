CREATE PROGRAM bhs_rpt_bri_export:dba
 ...
 DECLARE mf_baystateradimaging_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",34,
   "BAYSTATERADIMAGING"))
 DECLARE ms_loc_dir = vc WITH protect, constant(build(logical("bhscust"),"/ftp/bhs_rpt_bri_export/"))
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 SELECT INTO value(build(ms_loc_dir,ms_filename))
