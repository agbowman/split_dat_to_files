CREATE PROGRAM dash_reset_anes_breaks_status:dba
 PROMPT
 ...
 DECLARE err_msg = vc WITH protect
 DECLARE begin_date_time = dq8 WITH private, constant(cnvtdatetime(curdate,curtime3))
 SET log_program_name = "dash_reset_anes_breaks_status"
 ...
 UPDATE  FROM dash_break db
  SET db.status_flag = 1, db.updt_cnt = (db.updt_cnt+ 1), db.status_dt_tm = cnvtdatetime(curdate,
    curtime)
  WHERE db.record_dt_tm=cnvtdatetime(curdate,0)
   AND db.provider_type_flag=1
 ;end update
 IF (error(err_msg,0) > 0)
  SET failed = 1
  GO TO exit_script
