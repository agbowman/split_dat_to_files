CREATE PROGRAM dm_rxa_suspend_act_log_rows:dba
 ...
 SET reply->table_name = "RXA_SUSPEND_ACT_LOG"
 ...
 DECLARE minimum_days = i4 WITH protect, constant(180)
 DECLARE time_limit = i4 WITH protect, constant(235959)
 ...
 FOR (ltokenindex = 1 TO size(request->tokens,5))
   ...
   ENDIF
 ENDFOR
 IF (ldaystokeep < minimum_days)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"INVALID_DAYSTOKEEP",
   "You must keep at least %1 days worth of data.You entered %2 or did not enter any value. ","ii",
   minimum_days,
   ldaystokeep)
  GO TO exit_script
 ENDIF
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minimumvalue = min(sa.rxa_suspend_act_log_id)
   FROM rxa_suspend_act_log sa
   WHERE sa.rxa_suspend_act_log_id > 0.0
   DETAIL
 ...
 ENDIF
 SELECT INTO "n1:"
  maximumvalue = max(sa.rxa_suspend_act_log_id)
  FROM rxa_suspend_act_log sa
  DETAIL
   dmaximumid = maxval(cnvtreal(maximumvalue),1.0)
  WITH nocounter
 ;end select
 SET dbatchupperboundid = (dbatchlowerboundid+ (batch_size - 1))
 DECLARE recordcount = i4 WITH protect, noconstant(0)
 WHILE (dbatchlowerboundid <= dmaximumid
  AND lrowsleft > 0)
   SELECT INTO "nl:"
    sa.rxa_suspend_act_log_id
    FROM rxa_suspend_act_log sa
    WHERE sa.rxa_suspend_act_log_id > 0
     AND parser(sbr_getrowidnotexists(
      "sa.rxa_suspend_act_log_id between dBatchLowerBoundId and dBatchUpperBoundId","sa"))
     AND sa.updt_dt_tm <= cnvtdatetime((curdate - ldaystokeep),time_limit)
    DETAIL
     recordcount += 1
     IF (mod(recordcount,100)=1)
      stat = alterlist(reply->rows,(recordcount+ 99))
     ENDIF
     reply->rows[recordcount].row_id = sa.rowid
    WITH nocounter, maxqual(sa,value(lrowsleft))
   ;end select
