CREATE PROGRAM dm_rmc_gen_comp_query:dba
 ...
 SUBROUTINE daf_is_not_blank(dinb_str)
  ...
  RETURN(false)
 END ;Subroutine
 DECLARE drgc_tab_name = vc WITH protect, noconstant(cnvtupper(drgcq_request->table_name))
 DECLARE drgc_col_idx = i4 WITH protect, noconstant(0)
 DECLARE drgc_col_flag = i4 WITH protect, noconstant(0)
 DECLARE drgc_data_added_ind = i2 WITH protect, noconstant(1)
 DECLARE drgc_comma_ind = i2 WITH protect, noconstant(0)
 DECLARE drgc_cnt = i4 WITH protect, noconstant(0)
 DECLARE drgc_tab_idx = i4 WITH protect, noconstant(0)
 DECLARE drgc_get_child_tab_info(gct_data=vc(ref),gct_idx=i4,gct_col_idx=i4,gct_meta=vc(ref)) = null
 DECLARE drgc_gen_child_data(gcd_data=vc(ref),gcd_idx=i4,gcd_col_idx=i4,gcd_md_tab_idx=i4,
  gcd_md_col_idx=i4,
  gcd_par_tab=vc,gcd_meta=vc(ref)) = i4
 DECLARE drgc_add_inline_col(aic_data=vc(ref),aix_idx=i4,aic_cur_pos=i4,aic_table_name=vc,
  aic_column_name=vc,
  aic_table_alias=vc,aic_column_alias=vc,aic_display_flag=i4,aic_table_idx=i4) = null
 IF (check_logfile("DM_RMC_GEN_QUERY",".log","DM_RMC_GEN_COMP_QUERY LogFile")=0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO end_program
 ENDIF
 IF (validate(drgcq_request)=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No DRGCQ_REQUEST structure defined."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 IF (daf_is_blank(drgcq_request->table_name))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No table name supplied through DRGCQ_REQUEST structure."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 IF ( NOT ((drgcq_request->query_type IN (1, 2))))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "QUERY_TYPE in DRGCQ_REQUEST structure must be 1 or 2"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 IF (daf_is_not_blank(drgcq_request->db_link))
  CALL drm_md_wrp(drgc_tab_name,drgcq_request->db_link)
 ELSE
  CALL tmc_get_md(drgc_tab_name)
 ENDIF
 IF (check_error("Gathering meta-data")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 SET drgc_tab_idx = locateval(drgc_tab_idx,1,tmc_md_info->tbl_cnt,drgc_tab_name,tmc_md_info->
  tbl_qual[drgc_tab_idx].table_name)
 IF (validate(query_data)=0)
  FREE RECORD query_data
  RECORD query_data(
    1 query_cnt = i4
    1 query_qual[*]
      2 col_cnt = i4
 ...
 ENDIF
 SET query_data->query_cnt = 1
 SET stat = alterlist(query_data->query_qual,query_data->query_cnt)
 SET drgc_cnt = query_data->query_cnt
 SET query_data->query_qual[drgc_cnt].table_cnt = 1
 SET stat = alterlist(query_data->query_qual[drgc_cnt].table_qual,1)
 SET query_data->query_qual[drgc_cnt].table_qual[1].table_name = tmc_md_info->tbl_qual[drgc_tab_idx].
 table_name
 SET query_data->query_qual[drgc_cnt].table_qual[1].table_alias = concat("t",tmc_md_info->tbl_qual[
  drgc_tab_idx].suffix)
 SET query_data->query_qual[drgc_cnt].table_qual[1].join_type_flag = 1
 SET query_data->query_qual[drgc_cnt].table_qual[1].join_clause = concat("plan ",query_data->
  query_qual[drgc_cnt].table_qual[1].table_alias)
 FOR (drgc_col_idx = 1 TO tmc_md_info->tbl_qual[drgc_tab_idx].col_cnt)
   IF (((daf_is_not_blank(drgcq_request->db_link)
    AND (tmc_md_info->tbl_qual[drgc_tab_idx].col_qual[drgc_col_idx].in_src_ind=1)) OR (daf_is_blank(
    drgcq_request->db_link)))
    AND (tmc_md_info->tbl_qual[drgc_tab_idx].col_qual[drgc_col_idx].matching_ind=1))
    SET query_data->query_qual[drgc_cnt].col_cnt = (query_data->query_qual[drgc_cnt].col_cnt+ 1)
    SET stat = alterlist(query_data->query_qual[drgc_cnt].col_qual,query_data->query_qual[drgc_cnt].
     col_cnt)
    SET query_data->query_qual[drgc_cnt].col_qual[query_data->query_qual[drgc_cnt].col_cnt].
    table_name = tmc_md_info->tbl_qual[drgc_tab_idx].table_name
    SET query_data->query_qual[drgc_cnt].col_qual[query_data->query_qual[drgc_cnt].col_cnt].
    column_name = tmc_md_info->tbl_qual[drgc_tab_idx].col_qual[drgc_col_idx].column_name
    SET query_data->query_qual[drgc_cnt].col_qual[query_data->query_qual[drgc_cnt].col_cnt].
    table_alias = query_data->query_qual[drgc_cnt].table_qual[1].table_alias
    SET query_data->query_qual[drgc_cnt].col_qual[query_data->query_qual[drgc_cnt].col_cnt].
    column_alias = tmc_md_info->tbl_qual[drgc_tab_idx].col_qual[drgc_col_idx].column_name
    SET query_data->query_qual[drgc_cnt].col_qual[query_data->query_qual[drgc_cnt].col_cnt].
    table_qual_idx = 1
    IF ((tmc_md_info->tbl_qual[drgc_tab_idx].col_qual[drgc_col_idx].idcd_ind=0)
     AND daf_is_blank(tmc_md_info->tbl_qual[drgc_tab_idx].col_qual[drgc_col_idx].base62_re_name))
     SET query_data->query_qual[drgc_cnt].col_qual[query_data->query_qual[drgc_cnt].col_cnt].
     display_flag = 1
    ENDIF
   ENDIF
 ENDFOR
 IF ((drgcq_request->query_type=1))
  FOR (drgc_col_idx = 1 TO tmc_md_info->tbl_qual[drgc_tab_idx].col_cnt)
    IF (((daf_is_not_blank(drgcq_request->db_link)
     AND (tmc_md_info->tbl_qual[drgc_tab_idx].col_qual[drgc_col_idx].in_src_ind=1)) OR (daf_is_blank(
     drgcq_request->db_link)))
     AND (tmc_md_info->tbl_qual[drgc_tab_idx].col_qual[drgc_col_idx].relevant_ind=1))
     SET query_data->query_qual[drgc_cnt].col_cnt = (query_data->query_qual[drgc_cnt].col_cnt+ 1)
     SET stat = alterlist(query_data->query_qual[drgc_cnt].col_qual,query_data->query_qual[drgc_cnt].
      col_cnt)
     SET query_data->query_qual[drgc_cnt].col_qual[query_data->query_qual[drgc_cnt].col_cnt].
  ...
  ENDFOR
 ENDIF
 WHILE ((drgc_cnt <= query_data->query_cnt))
   SET drgc_data_added_ind = 1
   WHILE (drgc_data_added_ind=1)
    SET drgc_data_added_ind = 0
    FOR (drgc_col_idx = 1 TO query_data->query_qual[drgc_cnt].col_cnt)
      IF ((query_data->query_qual[drgc_cnt].col_qual[drgc_col_idx].display_flag=0))
       SET drgc_data_added_ind = 1
       CALL drgc_get_child_tab_info(query_data,drgc_cnt,drgc_col_idx,tmc_md_info)
       IF (check_error("Loading additional column data")=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
        GO TO end_program
       ENDIF
      ENDIF
    ENDFOR
   ENDWHILE
   SET drgc_cnt = (drgc_cnt+ 1)
 ENDWHILE
 IF ((dm_err->debug_flag != 0))
  FOR (drgc_cnt = 1 TO query_data->query_cnt)
    CALL echo("select into 'some_file_name'")
    SET drgc_comma_ind = 0
    FOR (drgc_col_idx = 1 TO query_data->query_qual[drgc_cnt].col_cnt)
      IF ((query_data->query_qual[drgc_cnt].col_qual[drgc_col_idx].display_flag=1))
       IF (drgc_comma_ind=1)
        CALL echo(concat("   ,",query_data->query_qual[drgc_cnt].col_qual[drgc_col_idx].column_alias,
          " = ",query_data->query_qual[drgc_cnt].col_qual[drgc_col_idx].table_alias,".",
          query_data->query_qual[drgc_cnt].col_qual[drgc_col_idx].column_name))
       ELSE
        CALL echo(concat("   ",query_data->query_qual[drgc_cnt].col_qual[drgc_col_idx].column_alias,
          " = ",query_data->query_qual[drgc_cnt].col_qual[drgc_col_idx].table_alias,".",
          query_data->query_qual[drgc_cnt].col_qual[drgc_col_idx].column_name))
       ENDIF
       SET drgc_comma_ind = 1
      ENDIF
    ENDFOR
    FOR (drgc_col_idx = 1 TO query_data->query_qual[drgc_cnt].table_cnt)
      IF ((query_data->query_qual[drgc_cnt].table_qual[drgc_col_idx].join_type_flag=1))
       CALL echo(concat("FROM ",query_data->query_qual[drgc_cnt].table_qual[drgc_col_idx].table_name,
         " ",query_data->query_qual[drgc_cnt].table_qual[drgc_col_idx].table_alias))
      ELSEIF ((query_data->query_qual[drgc_cnt].table_qual[drgc_col_idx].join_type_flag=2))
       CALL echo(concat(",(LEFT JOIN ",query_data->query_qual[drgc_cnt].table_qual[drgc_col_idx].
         table_name," ",query_data->query_qual[drgc_cnt].table_qual[drgc_col_idx].table_alias," ON (",
         query_data->query_qual[drgc_cnt].table_qual[drgc_col_idx].join_on_clause,"))"))
      ELSEIF ((query_data->query_qual[drgc_cnt].table_qual[drgc_col_idx].join_type_flag=3))
       CALL echo(concat(",(RIGHT JOIN ",query_data->query_qual[drgc_cnt].table_qual[drgc_col_idx].
         table_name," ",query_data->query_qual[drgc_cnt].table_qual[drgc_col_idx].table_alias," ON (",
         query_data->query_qual[drgc_cnt].table_qual[drgc_col_idx].join_on_clause,"))"))
      ELSE
       CALL echo(concat(", ",query_data->query_qual[drgc_cnt].table_qual[drgc_col_idx].table_name," ",
         query_data->query_qual[drgc_cnt].table_qual[drgc_col_idx].table_alias))
      ENDIF
    ENDFOR
    FOR (drgc_col_idx = 1 TO query_data->query_qual[drgc_cnt].table_cnt)
      CALL echo(query_data->query_qual[drgc_cnt].table_qual[drgc_col_idx].join_clause)
    ENDFOR
    CALL echo("WITH NOCOUNTER GO")
    CALL echo(
     "---------------------------------------------------------------------------------------------------------"
     )
    CALL echo(
  ...
  ENDFOR
 ENDIF
 SUBROUTINE drgc_get_child_tab_info(gct_data,gct_idx,gct_col_idx,gct_meta)
   DECLARE gct_md_tab_idx = i4 WITH protect, noconstant(0)
   DECLARE gct_md_col_idx = i4 WITH protect, noconstant(0)
   DECLARE gct_add_col_ind = i4 WITH protect, noconstant(0)
   DECLARE gct_pe_col_idx = i4 WITH protect, noconstant(0)
   DECLARE gct_pe_loop = i4 WITH protect, noconstant(0)
   DECLARE gct_new_idx = i4 WITH protect, noconstant(0)
   DECLARE gct_pe_list = vc WITH protect, noconstant(" ")
   SET gct_md_tab_idx = locateval(gct_md_tab_idx,1,gct_meta->tbl_cnt,gct_data->query_qual[gct_idx].
    col_qual[gct_col_idx].table_name,gct_meta->tbl_qual[gct_md_tab_idx].table_name)
   IF (gct_md_tab_idx=0)
    IF (daf_is_not_blank(drgcq_request->db_link))
     CALL drm_md_wrp(gct_data->query_qual[gct_idx].col_qual[gct_col_idx].table_name,drgcq_request->
      db_link)
    ELSE
     CALL tmc_get_md(gct_data->query_qual[gct_idx].col_qual[gct_col_idx].table_name)
    ENDIF
    IF (check_error("Gathering meta-data")=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(null)
    ENDIF
    SET gct_md_tab_idx = gct_meta->tbl_cnt
   ENDIF
   SET gct_md_col_idx = locateval(gct_md_col_idx,1,gct_meta->tbl_qual[gct_md_tab_idx].col_cnt,
    gct_data->query_qual[gct_idx].col_qual[gct_col_idx].column_name,gct_meta->tbl_qual[gct_md_tab_idx
    ].col_qual[gct_md_col_idx].column_name)
   IF (gct_md_col_idx=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("No metadata found for column: ",gct_data->query_qual[gct_idx].
     col_qual[gct_col_idx].column_name)
    RETURN(null)
   ENDIF
   IF (daf_is_not_blank(gct_meta->tbl_qual[gct_md_tab_idx].col_qual[gct_md_col_idx].root_entity_name)
   )
    IF ( NOT ((gct_meta->tbl_qual[gct_md_tab_idx].col_qual[gct_md_col_idx].root_entity_name IN (
    "LONG_TEXT", "LONG_TEXT_REFERENCE", "LONG_BLOB", "LONG_BLOB_REFERENCE"))))
     SET gct_add_col_ind = drgc_gen_child_data(gct_data,gct_idx,gct_col_idx,gct_md_tab_idx,
      gct_md_col_idx,
      gct_meta->tbl_qual[gct_md_tab_idx].col_qual[gct_md_col_idx].root_entity_name,gct_meta)
     IF (check_error("Generating parent table column data")=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      RETURN(null)
     ENDIF
    ELSE
     SET gct_data->query_qual[gct_idx].col_qual[gct_col_idx].display_flag = 1
     SET gct_add_col_ind = 1
    ENDIF
   ELSEIF (daf_is_not_blank(gct_meta->tbl_qual[gct_md_tab_idx].col_qual[gct_md_col_idx].
    parent_entity_col))
    SET gct_pe_col_idx = locateval(gct_pe_col_idx,1,gct_meta->tbl_qual[gct_md_tab_idx].col_cnt,
     gct_meta->tbl_qual[gct_md_tab_idx].col_qual[gct_md_col_idx].parent_entity_col,gct_meta->
     tbl_qual[gct_md_tab_idx].col_qual[gct_pe_col_idx].column_name)
    IF (gct_pe_col_idx > 0)
     FOR (gct_pe_loop = 1 TO gct_meta->tbl_qual[gct_md_tab_idx].col_qual[gct_md_col_idx].
     pe_name_audit_cnt)
       IF ( NOT ((gct_meta->tbl_qual[gct_md_tab_idx].col_qual[gct_md_col_idx].pe_name_audit_qual[
       gct_pe_loop].pe_name_value IN ("LONG_TEXT", "LONG_TEXT_REFERENCE", "LONG_BLOB",
       "LONG_BLOB_REFERENCE"))))
        SET gct_data->query_cnt = (gct_data->query_cnt+ 1)
        SET gct_new_idx = gct_data->query_cnt
        SET stat = alterlist(gct_data->query_qual,gct_data->query_cnt)
        SET stat = movereclist(gct_data->query_qual,gct_data->query_qual,gct_idx,gct_data->query_cnt,
         1,
         0)
        IF (daf_is_not_blank(gct_pe_list))
         SET gct_pe_list = concat(gct_pe_list,", '",gct_meta->tbl_qual[gct_md_tab_idx].col_qual[
          gct_md_col_idx].pe_name_audit_qual[gct_pe_loop].pe_name_value,"'")
        ELSE
         SET gct_pe_list = concat("'",gct_meta->tbl_qual[gct_md_tab_idx].col_qual[gct_md_col_idx].
          pe_name_audit_qual[gct_pe_loop].pe_name_value,"'")
        ENDIF
        SET gct_add_col_ind = drgc_gen_child_data(gct_data,gct_new_idx,gct_col_idx,gct_md_tab_idx,
         gct_md_col_idx,
         gct_meta->tbl_qual[gct_md_tab_idx].col_qual[gct_md_col_idx].pe_name_audit_qual[gct_pe_loop].
         pe_name_value,gct_meta)
        IF (check_error("Generating parent table column data")=1)
         CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
         RETURN(null)
        ELSEIF (gct_add_col_ind=0)
         SET dm_err->err_ind = 1
         SET dm_err->emsg = concat("There were no matching or support columns found for table ",
          gct_meta->tbl_qual[gct_md_tab_idx].col_qual[gct_md_col_idx].pe_name_audit_qual[gct_pe_loop]
          .pe_name_value)
         CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
         RETURN(null)
        ELSEIF ((gct_add_col_ind=- (1)))
         RETURN(null)
        ENDIF
        SET gct_join_idx = gct_data->query_qual[gct_new_idx].col_qual[gct_col_idx].table_qual_idx
        IF (findstring(" WHERE ",cnvtupper(gct_data->query_qual[gct_new_idx].table_qual[gct_join_idx]
          .join_clause),0,1) > 0)
         SET gct_data->query_qual[gct_new_idx].table_qual[gct_join_idx].join_clause = concat(gct_data
          ->query_qual[gct_new_idx].table_qual[gct_join_idx].join_clause," AND ",gct_data->
          query_qual[gct_new_idx].col_qual[gct_col_idx].table_alias,".",gct_meta->tbl_qual[
          gct_md_tab_idx].col_qual[gct_md_col_idx].parent_entity_col,
          " = '",gct_meta->tbl_qual[gct_md_tab_idx].col_qual[gct_md_col_idx].pe_name_audit_qual[
          gct_pe_loop].pe_name_value,"'")
        ELSE
         SET gct_data->query_qual[gct_new_idx].table_qual[gct_join_idx].join_clause = concat(gct_data
          ->query_qual[gct_new_idx].table_qual[gct_join_idx].join_clause," WHERE ",gct_data->
          query_qual[gct_new_idx].col_qual[gct_col_idx].table_alias,".",gct_meta->tbl_qual[
          gct_md_tab_idx].col_qual[gct_md_col_idx].parent_entity_col,
          " = '",gct_meta->tbl_qual[gct_md_tab_idx].col_qual[gct_md_col_idx].pe_name_audit_qual[
          gct_pe_loop].pe_name_value,"'")
        ENDIF
        SET gct_data->query_qual[gct_new_idx].col_qual[gct_col_idx].display_flag = 2
       ENDIF
     ENDFOR
     SET gct_data->query_qual[gct_idx].col_qual[gct_col_idx].display_flag = 1
     SET gct_add_col_ind = 1
     IF (daf_is_not_blank(gct_pe_list))
      SET gct_join_idx = gct_data->query_qual[gct_idx].col_qual[gct_col_idx].table_qual_idx
      IF (findstring(" WHERE ",cnvtupper(gct_data->query_qual[gct_idx].table_qual[gct_join_idx].
        join_clause),0,1) > 0)
       SET gct_data->query_qual[gct_idx].table_qual[gct_join_idx].join_clause = concat(gct_data->
        query_qual[gct_idx].table_qual[gct_join_idx].join_clause," AND ",gct_data->query_qual[gct_idx
        ].col_qual[gct_col_idx].table_alias,".",gct_meta->tbl_qual[gct_md_tab_idx].col_qual[
        gct_md_col_idx].parent_entity_col,
        " not in (",gct_pe_list,")")
      ELSE
       SET gct_data->query_qual[gct_idx].table_qual[gct_join_idx].join_clause = concat(gct_data->
        query_qual[gct_idx].table_qual[gct_join_idx].join_clause," WHERE ",gct_data->query_qual[
        gct_idx].col_qual[gct_col_idx].table_alias,".",gct_meta->tbl_qual[gct_md_tab_idx].col_qual[
        gct_md_col_idx].parent_entity_col,
        " not in (",gct_pe_list,")")
      ENDIF
     ENDIF
   ...
   ELSEIF (daf_is_not_blank(gct_meta->tbl_qual[gct_md_tab_idx].col_qual[gct_md_col_idx].
    base62_re_name))
    IF ( NOT ((gct_meta->tbl_qual[gct_md_tab_idx].col_qual[gct_md_col_idx].base62_re_name IN (
    "LONG_TEXT", "LONG_TEXT_REFERENCE", "LONG_BLOB", "LONG_BLOB_REFERENCE"))))
     SET gct_add_col_ind = drgc_gen_child_data(gct_data,gct_idx,gct_col_idx,gct_md_tab_idx,
      gct_md_col_idx,
      gct_meta->tbl_qual[gct_md_tab_idx].col_qual[gct_md_col_idx].base62_re_name,gct_meta)
     IF (check_error("Generating parent table column data")=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      RETURN(null)
     ENDIF
   ...
   ELSE
    ...
    SET gct_add_col_ind = 1
   ENDIF
   IF (gct_add_col_ind=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("There were no matching or support columns found for table ",gct_meta->
     tbl_qual[gct_md_tab_idx].col_qual[gct_md_col_idx].root_entity_name)
   ELSEIF ((gct_data->query_qual[gct_idx].col_qual[gct_col_idx].display_flag=0))
    SET gct_data->query_qual[gct_idx].col_qual[gct_col_idx].display_flag = 2
   ENDIF
   RETURN(null)
 END ;Subroutine
 SUBROUTINE drgc_gen_child_data(gcd_data,gcd_idx,gcd_col_idx,gcd_md_tab_idx,gcd_md_col_idx,
  gcd_par_tab,gcd_meta)
   DECLARE gcd_par_tab_idx = i4 WITH protect, noconstant(0)
   DECLARE gcd_par_col_idx = i4 WITH protect, noconstant(0)
   DECLARE gcd_table_name = vc WITH protect, noconstant(" ")
   DECLARE gcd_column_name = vc WITH protect, noconstant(" ")
   DECLARE gcd_table_alias = vc WITH protect, noconstant(" ")
   DECLARE gcd_column_alias = vc WITH protect, noconstant(" ")
   DECLARE gcd_display_flag = i4 WITH protect, noconstant(0)
   DECLARE gcd_table_idx = i4 WITH protect, noconstant(0)
   DECLARE gcd_add_col_ind = i2 WITH protect, noconstant(0)
   DECLARE gcd_pk_cnt = i4 WITH protect, noconstant(0)
   DECLARE gcd_pk_idx = i4 WITH protect, noconstant(0)
   DECLARE gcd_sub_par = vc WITH protect, noconstant(" ")
   IF (gcd_par_tab="PERSON")
    SET gcd_sub_par = "PRSNL"
   ELSE
    SET gcd_sub_par = gcd_par_tab
   ENDIF
   SET gcd_par_tab_idx = locateval(gcd_par_tab_idx,1,gcd_meta->tbl_cnt,cnvtupper(gcd_sub_par),
    gcd_meta->tbl_qual[gcd_par_tab_idx].table_name)
   IF (gcd_par_tab_idx=0)
    IF (daf_is_not_blank(drgcq_request->db_link))
     CALL drm_md_wrp(cnvtupper(gcd_sub_par),drgcq_request->db_link)
    ELSE
     CALL tmc_get_md(cnvtupper(gcd_sub_par))
    ENDIF
    IF (check_error("Gathering meta-data")=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(0)
    ENDIF
    SET gcd_par_tab_idx = gcd_meta->tbl_cnt
   ENDIF
   IF (gcd_md_tab_idx=gcd_par_tab_idx)
    FOR (gcd_par_col_idx = 1 TO gcd_meta->tbl_qual[gcd_par_tab_idx].col_cnt)
      IF ((((gcd_meta->tbl_qual[gcd_par_tab_idx].col_qual[gcd_par_col_idx].in_src_ind=1)
       AND daf_is_not_blank(drgcq_request->db_link)) OR (daf_is_blank(drgcq_request->db_link))) )
       IF ((((gcd_meta->tbl_qual[gcd_md_tab_idx].col_qual[gcd_md_col_idx].matching_ind=1)
        AND (gcd_meta->tbl_qual[gcd_par_tab_idx].col_qual[gcd_par_col_idx].matching_ind=1)) OR ((((
       gcd_meta->tbl_qual[gcd_md_tab_idx].col_qual[gcd_md_col_idx].relevant_ind=1)) OR ((gcd_meta->
       tbl_qual[gcd_md_tab_idx].col_qual[gcd_md_col_idx].support_ind=1)))
        AND (gcd_meta->tbl_qual[gcd_par_tab_idx].col_qual[gcd_par_col_idx].support_ind=1))) )
        IF ((((gcd_meta->tbl_qual[gcd_par_tab_idx].col_qual[gcd_par_col_idx].root_entity_name=
        gcd_meta->tbl_qual[gcd_md_tab_idx].table_name)) OR (locateval(gcd_pk_idx,1,gcd_meta->
         tbl_qual[gcd_par_tab_idx].col_qual[gcd_par_col_idx].pe_name_audit_cnt,gcd_meta->tbl_qual[
         gcd_md_tab_idx].table_name,gcd_meta->tbl_qual[gcd_par_tab_idx].col_qual[gcd_par_col_idx].
         pe_name_audit_qual[gcd_pk_idx].pe_name_value) > 0)) )
         IF ((dm_err->debug_flag != 0))
          SET dm_err->err_ind = 1
          SET dm_err->emsg = concat("The ",gcd_meta->tbl_qual[gcd_md_tab_idx].table_name,
           " table causes circular self-references.")
          RETURN(0)
         ELSE
          SET gcd_data->query_qual[gcd_idx].col_qual[gcd_col_idx].display_flag = 1
          RETURN(- (1))
         ENDIF
        ENDIF
       ENDIF
    ...
    ENDFOR
   ENDIF
   IF ((gcd_data->query_qual[gcd_idx].table_cnt=30))
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Maximum number of table joins exceeded, while generating a dynamic query."
    RETURN(0)
   ENDIF
   SET gcd_data->query_qual[gcd_idx].table_cnt = (gcd_data->query_qual[gcd_idx].table_cnt+ 1)
   SET gcd_table_idx = gcd_data->query_qual[gcd_idx].table_cnt
   SET stat = alterlist(gcd_data->query_qual[gcd_idx].table_qual,gcd_table_idx)
   SET gcd_data->query_qual[gcd_idx].table_qual[gcd_table_idx].table_name = gcd_meta->tbl_qual[
   gcd_par_tab_idx].table_name
   SET gcd_data->query_qual[gcd_idx].table_qual[gcd_table_idx].table_alias = concat(gcd_data->
    query_qual[gcd_idx].col_qual[gcd_col_idx].table_alias,"_",trim(cnvtstring(gcd_meta->tbl_qual[
      gcd_md_tab_idx].col_qual[gcd_md_col_idx].internal_column_id,20)))
   SET gcd_data->query_qual[gcd_idx].table_qual[gcd_table_idx].join_type_flag = 2
   IF (daf_is_not_blank(gcd_meta->tbl_qual[gcd_par_tab_idx].root_col_name))
    IF (daf_is_not_blank(gcd_meta->tbl_qual[gcd_md_tab_idx].col_qual[gcd_md_col_idx].base62_re_name))
     SET gcd_data->query_qual[gcd_idx].table_qual[gcd_table_idx].join_on_clause = concat(
      "dm_refchg_b62tob10(",gcd_data->query_qual[gcd_idx].col_qual[gcd_col_idx].table_alias,".",
      gcd_data->query_qual[gcd_idx].col_qual[gcd_col_idx].column_name,") = ",
      gcd_data->query_qual[gcd_idx].table_qual[gcd_table_idx].table_alias,".",gcd_meta->tbl_qual[
      gcd_par_tab_idx].root_col_name)
    ELSE
     SET gcd_data->query_qual[gcd_idx].table_qual[gcd_table_idx].join_on_clause = concat(gcd_data->
      query_qual[gcd_idx].col_qual[gcd_col_idx].table_alias,".",gcd_data->query_qual[gcd_idx].
      col_qual[gcd_col_idx].column_name," = ",gcd_data->query_qual[gcd_idx].table_qual[gcd_table_idx]
      .table_alias,
      ".",gcd_meta->tbl_qual[gcd_par_tab_idx].root_col_name)
    ENDIF
   ELSE
    SET gcd_pk_cnt = 0
    SET gcd_pk_idx = 0
    FOR (gcd_par_col_idx = 1 TO gcd_meta->tbl_qual[gcd_par_tab_idx].col_cnt)
      IF ((gcd_meta->tbl_qual[gcd_par_tab_idx].col_qual[gcd_par_col_idx].pk_ind=1)
       AND (gcd_meta->tbl_qual[gcd_par_tab_idx].col_qual[gcd_par_col_idx].idcd_ind=1))
       SET gcd_pk_cnt = (gcd_pk_cnt+ 1)
       SET gcd_pk_idx = gcd_par_col_idx
      ENDIF
    ENDFOR
    IF (gcd_pk_cnt=1)
     SET gcd_data->query_qual[gcd_idx].table_qual[gcd_table_idx].join_on_clause = concat(gcd_data->
      ...
      col_qual[gcd_col_idx].column_name," = ",gcd_data->query_qual[gcd_idx].table_qual[gcd_table_idx]
      .table_alias,
      ".",gcd_meta->tbl_qual[gcd_par_tab_idx].col_qual[gcd_pk_idx].column_name)
    ELSE
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("No top level could be found to join on for table ",gcd_meta->
      tbl_qual[gcd_par_tab_idx].table_name)
     RETURN(0)
    ENDIF
   ENDIF
   SET gcd_data->query_qual[gcd_idx].table_qual[gcd_table_idx].join_clause = concat("join ",gcd_data
    ->query_qual[gcd_idx].table_qual[gcd_table_idx].table_alias)
   FOR (gcd_par_col_idx = 1 TO gcd_meta->tbl_qual[gcd_par_tab_idx].col_cnt)
     IF ((((gcd_meta->tbl_qual[gcd_par_tab_idx].col_qual[gcd_par_col_idx].in_src_ind=1)
      AND daf_is_not_blank(drgcq_request->db_link)) OR (daf_is_blank(drgcq_request->db_link))) )
      IF ((((gcd_meta->tbl_qual[gcd_md_tab_idx].col_qual[gcd_md_col_idx].matching_ind=1)
      ...
      tbl_qual[gcd_md_tab_idx].col_qual[gcd_md_col_idx].support_ind=1)))
       AND (gcd_meta->tbl_qual[gcd_par_tab_idx].col_qual[gcd_par_col_idx].support_ind=1))) )
       SET gcd_add_col_ind = 1
       SET gcd_table_name = gcd_meta->tbl_qual[gcd_par_tab_idx].table_name
       SET gcd_column_name = gcd_meta->tbl_qual[gcd_par_tab_idx].col_qual[gcd_par_col_idx].
       column_name
       SET gcd_table_alias = gcd_data->query_qual[gcd_idx].table_qual[gcd_table_idx].table_alias
       SET gcd_column_alias = concat(substring(1,(size(gcd_meta->tbl_qual[gcd_md_tab_idx].col_qual[
          gcd_md_col_idx].column_name) - 3),gcd_meta->tbl_qual[gcd_md_tab_idx].col_qual[
         gcd_md_col_idx].column_name),"_",gcd_meta->tbl_qual[gcd_par_tab_idx].col_qual[
        gcd_par_col_idx].column_name)
       IF ((gcd_meta->tbl_qual[gcd_par_tab_idx].col_qual[gcd_par_col_idx].idcd_ind=0))
        SET gcd_display_flag = 1
       ELSE
        SET gcd_display_flag = 0
       ENDIF
       CALL drgc_add_inline_col(gcd_data,gcd_idx,gcd_col_idx,gcd_table_name,gcd_column_name,
        gcd_table_alias,gcd_column_alias,gcd_display_flag,gcd_table_idx)
      ENDIF
     ENDIF
   ENDFOR
   RETURN(gcd_add_col_ind)
 END ;Subroutine
 SUBROUTINE drgc_add_inline_col(aic_data,aic_idx,aic_cur_pos,aic_table_name,aic_column_name,
  aic_table_alias,aic_column_alias,aic_display_flag,aic_table_idx)
   DECLARE aic_loop = i4 WITH protect, noconstant(0)
   SET aic_data->query_qual[aic_idx].col_cnt = (aic_data->query_qual[aic_idx].col_cnt+ 1)
   SET stat = alterlist(aic_data->query_qual[aic_idx].col_qual,aic_data->query_qual[aic_idx].col_cnt)
   SET aic_loop = (aic_data->query_qual[aic_idx].col_cnt - 1)
   WHILE (aic_loop > aic_cur_pos)
     SET aic_data->query_qual[aic_idx].col_qual[(aic_loop+ 1)].table_name = aic_data->query_qual[
     aic_idx].col_qual[aic_loop].table_name
     SET aic_data->query_qual[aic_idx].col_qual[(aic_loop+ 1)].column_name = aic_data->query_qual[
     aic_idx].col_qual[aic_loop].column_name
     SET aic_data->query_qual[aic_idx].col_qual[(aic_loop+ 1)].table_alias = aic_data->query_qual[
     aic_idx].col_qual[aic_loop].table_alias
     SET aic_data->query_qual[aic_idx].col_qual[(aic_loop+ 1)].column_alias = aic_data->query_qual[
     aic_idx].col_qual[aic_loop].column_alias
     SET aic_data->query_qual[aic_idx].col_qual[(aic_loop+ 1)].display_flag = aic_data->query_qual[
     aic_idx].col_qual[aic_loop].display_flag
     SET aic_data->query_qual[aic_idx].col_qual[(aic_loop+ 1)].table_qual_idx = aic_data->query_qual[
     aic_idx].col_qual[aic_loop].table_qual_idx
     SET aic_loop = (aic_loop - 1)
   ENDWHILE
   SET aic_data->query_qual[aic_idx].col_qual[(aic_cur_pos+ 1)].table_name = aic_table_name
   SET aic_data->query_qual[aic_idx].col_qual[(aic_cur_pos+ 1)].column_name = aic_column_name
   SET aic_data->query_qual[aic_idx].col_qual[(aic_cur_pos+ 1)].table_alias = aic_table_alias
   SET aic_data->query_qual[aic_idx].col_qual[(aic_cur_pos+ 1)].column_alias = aic_column_alias
   SET aic_data->query_qual[aic_idx].col_qual[(aic_cur_pos+ 1)].display_flag = aic_display_flag
   SET aic_data->query_qual[aic_idx].col_qual[(aic_cur_pos+ 1)].table_qual_idx = aic_table_idx
   RETURN(null)
 END ;Subroutine
#end_program
END GO
