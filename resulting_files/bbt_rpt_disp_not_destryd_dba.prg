CREATE PROGRAM bbt_rpt_disp_not_destryd:dba
 ...
 SET captions->products_disposed = uar_i18ngetmessage(i18nhandle,"products_disposed",
 ...
 SET captions->total = uar_i18ngetmessage(i18nhandle,"total","Total for Product Type:")
 SET captions->report_id = uar_i18ngetmessage(i18nhandle,"report_id",
  "Report ID: BBT_RPT_DISP_NOT_DESTRYD")
 ...
 SELECT INTO cpm_cfn_info->file_name_logical
  d_flg = decode(bp.seq,"BP",de.seq,"DE","XX"), pe.product_id, bp.supplier_prefix,
  pr.product_nbr"###############", pr.product_sub_nbr"#####", product_display = decode(pr.seq,
   uar_get_code_display(pr.product_cd)," "),
  sub_nbr = pr.product_sub_nbr"#####", product_disp = decode(pr.seq,uar_get_code_display(pr
    .product_cd)," "), abo = decode(bp.seq,uar_get_code_display(bp.cur_abo_cd)," "),
  rh = decode(bp.seq,uar_get_code_display(bp.cur_rh_cd)," "), qty = dp.disposed_qty"####", dt_tm = pe
  .event_dt_tm,
  tech = prs.username
  ...
  PLAN (pe
   WHERE pe.event_dt_tm BETWEEN cnvtdatetime(request->beg_dt_tm) AND cnvtdatetime(request->end_dt_tm)
    AND event_code=pe.event_type_cd
    AND pe.active_ind=1)
   JOIN (prs
   ...
   JOIN (dp
   WHERE pe.product_id=dp.product_id
    AND pe.product_event_id=dp.product_event_id
    AND dp.active_ind=1)
   JOIN (d1
   WHERE d1.seq=1)
   JOIN (((d5
  ...
  ORDER BY product_display, abo, rh,
   pr.product_nbr, pr.product_sub_nbr
  HEAD REPORT
   line = fillstring(125,"_"), first_time = "Y", cnt = 0,
   select_ok_ind = 0, aborh_display = fillstring(15," ")
  HEAD PAGE
   ...
   beg_dt_tm"@DATECONDENSED;;d", col 58, beg_dt_tm"@TIMENOSECONDS;;m",
   col 73, captions->end_date, col 86,
   end_dt_tm"@DATECONDENSED;;d", col 94, end_dt_tm"@TIMENOSECONDS;;m",
   row + 2, col 25, captions->product_type,
   col 39, product_display"#########################", row + 2,
   col 25, captions->aborh,
   CALL center(captions->product_number,44,68),
   col 73, captions->qty, col 82,
   captions->dt_tm, col 101, captions->tech_id,
   row + 1, col 25, "---------------",
   col 44, "-------------------------", col 73,
   "----", col 80, "-------------",
   col 101, "-------", row + 1
  ...
  DETAIL
   ...
   IF (d_flg="BP")
    aborh_display = concat(trim(abo)," ",trim(rh)), col 25, aborh_display"###############"
   ELSEIF (d_flg="DE")
    col 25, "               "
   ENDIF
   prod_nbr_display = concat(trim(bp.supplier_prefix),trim(pr.product_nbr)), col 44, prod_nbr_display
   IF (pr.product_sub_nbr > "     ")
    col 64, pr.product_sub_nbr
   ELSE
    col 64, "     "
   ENDIF
   IF (d_flg="DE")
    col 73, dp.disposed_qty"####;p "
   ELSE
    col 73, "    "
   ENDIF
   col 80, event_dt_tm"@DATECONDENSED;;d", col 88,
   event_dt_tm"@TIMENOSECONDS;;m", col 101, prs.username"######",
   row + 2
   IF (row > 56)
    BREAK
   ENDIF
  ...
  FOOT  product_display
   ...
   IF (row > 54)
    BREAK, row + 3
   ENDIF
   col 25, captions->total, col 49,
   product_display"#########################", row + 2, col 28,
   event_display"###############", col 45, cnt"####;p ",
   cnt = 0
  FOOT PAGE
   row 57, col 1, line,
   row + 1, col 1, captions->report_id,
   col 58, captions->page_no, col 64,
   curpage"###", col 109, captions->printed,
