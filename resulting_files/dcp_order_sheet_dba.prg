CREATE PROGRAM dcp_order_sheet:dba
 ...
 SELECT
  ...
  FROM orders o,
   order_action oa
  PLAN (oa
   WHERE (oa.order_conversation_id=request->conversation_id)
    AND oa.order_convs_seq=1)
   JOIN (o
 ...
 DECLARE 319_mrn = f8 WITH constant(uar_get_code_by("MEANING",319,"MRN"))
 DECLARE 4_cmrn = f8 WITH constant(uar_get_code_by("MEANING",4,"CMRN"))
 DECLARE 212_business = f8 WITH constant(uar_get_code_by("MEANING",212,"BUSINESS"))
 DECLARE o_str = vc
 DECLARE od_str = vc
 DECLARE oc_str = vc
 ...
 SELECT INTO "NL:"
  ...
  p1.sex_cd, pa1.alias, e.encntr_id,
  e.loc_facility_cd, e.loc_building_cd, e.loc_nurse_unit_cd,
  e.loc_room_cd, e.loc_bed_cd, e.reg_dt_tm,
  e.disch_dt_tm, e.encntr_type_cd, ea1.alias,
  ea2.alias, p2.name_full_formatted, p3.name_full_formatted,
  ...
  PLAN (p1
   ...
   WHERE ea2.encntr_id=outerjoin(e.encntr_id)
    AND ea2.encntr_alias_type_cd=outerjoin(319_mrn))
   JOIN (pa1
   WHERE pa1.person_id=outerjoin(p1.person_id)
    AND pa1.person_alias_type_cd=outerjoin(4_cmrn))
   JOIN (ea1
   WHERE ea1.encntr_id=outerjoin(e.encntr_id)
    AND ea1.encntr_alias_type_cd=outerjoin(319_fin_nbr))
   JOIN (org
   WHERE org.organization_id=outerjoin(e.organization_id))
   JOIN (adr
   WHERE adr.parent_entity_id=outerjoin(e.organization_id)
    AND adr.address_type_cd=outerjoin(212_business))
   JOIN (epr
   WHERE epr.encntr_id=outerjoin(e.encntr_id)
    AND epr.encntr_prsnl_r_cd=outerjoin(333_admitdoc))
   JOIN (p2
   WHERE p2.person_id=outerjoin(epr.prsnl_person_id))
   JOIN (p3
   WHERE p3.person_id=outerjoin(request->print_prsnl_id))
  DETAIL
   data->person_id = request->person_id, data->name_full_formatted = p1.name_full_formatted, data->
   birth_dt_tm = p1.birth_dt_tm,
   data->sex_cd = p1.sex_cd, data->mrn = ea2.alias, data->cmrn = pa1.alias,
 ...
 FOR (x = 1 TO size(request->order_qual,5))
   SELECT INTO "NL:"
    ...
    DETAIL
     data->ord_qual[x].ord_id = o.order_id, data->ord_qual[x].ord_mnem = o.order_mnemonic, data->
     ord_qual[x].ord_as_mnem = o.ordered_as_mnemonic,
     data->ord_qual[x].ord_stat_disp = uar_get_code_display(o.order_status_cd), data->ord_qual[x].
     ord_detail = o.clinical_display_line, data->ord_qual[x].ord_comment_ind = o.order_comment_ind,
     data->ord_qual[x].ord_comment = trim(lt.long_text,3), data->ord_qual[x].ord_convs_seq = oa
     .order_convs_seq, data->ord_cnt = x
    WITH nocounter
   ;end select
 ENDFOR
 SELECT INTO "NL:"
  ...
  DETAIL
   ...
   FOR (x = 1 TO size(data->ord_qual,5))
     stat = alterlist(outstr->o_qual,size(data->ord_qual,5)), outstr->o_qual[x].o_str = concat(
      "{CPI/9}{f/9}",trim(data->ord_qual[x].ord_mnem,3))
     IF ((data->ord_qual[x].ord_mnem != data->ord_qual[x].ord_as_mnem))
      outstr->o_qual[x].o_str = concat(outstr->o_qual[x].o_str," (",trim(data->ord_qual[x].
        ord_as_mnem,3),") ")
     ENDIF
     IF (x < size(data->ord_qual,5))
      outstr->o_qual[x].o_str = concat(outstr->o_qual[x].o_str,"{CPI/11}{f/24}","  -  ",
       "{CPI/11}{f/24}",trim(data->ord_qual[x].ord_stat_disp,3),
       " - Id ",trim(cnvtstring(data->ord_qual[x].ord_id))), outstr->o_qual[x].o_tot_ht = (outstr->
      o_qual[x].o_tot_ht+ (o_ht * 1))
     ENDIF
 ...
 SELECT INTO request->printer_name
  ...
  HEAD REPORT
   ...
   encntr_type_disp = substring(1,20,uar_get_code_display(data->encntr_type_cd)), fin = substring(1,
  ...
  WITH counter, dio = 08, maxcol = 12000,
   maxrow = 500
 ;end select
 CALL echo("ADAM LOOK AT THIS")
 CALL echorecord(data)
 CALL echorecord(outstr)
