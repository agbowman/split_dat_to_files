CREATE PROGRAM bhs_athn_order_detail_ls_v6
 ...
 SELECT DISTINCT INTO "NL:"
  ...
  HEAD o.order_id
   ...
   order_list->qual[d1.seq].orderable_type_flag = trim(substring(0,8,
   ...
   witness_flag = os.witness_flag, order_list->qual[d1.seq].pathway_plan = trim(p.display_description,
    3),
   ...
   ].stop_duration_unit = uar_get_code_meaning(oc.stop_duration_unit_cd), order_list->qual[d1.seq].
   catlog_stop_type_cd = cnvtstring(oc.stop_type_cd),
   order_list->qual[d1.seq].catlog_stop_type_mean = uar_get_code_meaning(oc.stop_type_cd), order_list
   ->qual[d1.seq].disable_order_comment_flag = oc.disable_order_comment_ind, order_list->qual[d1.seq]
   .protocol_order_id = cnvtstring(o.protocol_order_id),
   order_list->qual[d1.seq].reftext_mask =
   ...
   stop_datetime = format(od1.oe_field_dt_tm_value,"MM/DD/YYYY HH:MM;;D"),
