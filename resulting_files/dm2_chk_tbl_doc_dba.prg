CREATE PROGRAM dm2_chk_tbl_doc:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE ctd_logfile_prefix = vc WITH protect, constant("dm2_chk_tbl_doc")
 DECLARE ctd_pkg_num = i4 WITH protect
 DECLARE ctd_table_size = i4 WITH protect, noconstant(0)
 DECLARE ctd_dyn_where = vc WITH protect, noconstant(" ")
 DECLARE ctd_cnt = i4 WITH protect, noconstant(0)
 DECLARE ctd_work_cnt = i4 WITH protect, noconstant(0)
 IF ((validate(dctd_dtd->tbl_cnt,- (1))=- (1))
  AND (validate(dctd_dtd->tbl_cnt,- (2))=- (2)))
  FREE SET dctd_dtd
  RECORD dctd_dtd(
    1 tbl_cnt = i4
    1 tbl[*]
      2 table_name = vc
      2 updt_cnt = i4
      2 fix_columns_ind = i2
  )
  SET dctd_dtd->tbl_cnt = 0
 ENDIF
 IF (check_logfile(ctd_logfile_prefix,".log","DM2_CHK_TBL_DOC LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning maintenance check on DM_TABLES_DOC...."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET ctd_pkg_num =  $1
 IF (check_error("Validating input parameter - package number.")=1)
  SET dm_err->emsg = "Invalid package number - must be equal to or greater than zero."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ...
 IF ((dm2_install_schema->process_option="INHOUSE")
  AND currdb="SQLSRV")
  SET ctd_dyn_where = "dtd.full_table_name = validate(inhouse_reply->table_name, '@')"
 ELSE
  SET ctd_dyn_where = "1 = 1"
 ENDIF
 IF (dm2_set_autocommit(1)=0)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM dm_tables_doc dtd
  WHERE parser(ctd_dyn_where)
  HEAD REPORT
   dctd_dtd->tbl_cnt = 0, ctd_work_cnt = 0
  DETAIL
   ctd_work_cnt = (ctd_work_cnt+ 1)
   IF (mod(ctd_work_cnt,50)=1)
    stat = alterlist(dctd_dtd->tbl,(ctd_work_cnt+ 49))
   ENDIF
   dctd_dtd->tbl[ctd_work_cnt].table_name = trim(dtd.table_name), dctd_dtd->tbl[ctd_work_cnt].
   updt_cnt = dtd.updt_cnt
   IF (validate(dtd.table_suffix,"Z")="Z"
    AND validate(dtd.table_suffix,"X")="X")
    dctd_dtd->tbl[ctd_work_cnt].fix_columns_ind = 0
   ELSEIF (trim(validate(dtd.table_suffix," "))="0")
    dctd_dtd->tbl[ctd_work_cnt].fix_columns_ind = 1
   ELSE
    dctd_dtd->tbl[ctd_work_cnt].fix_columns_ind = 0
   ENDIF
  FOOT REPORT
   stat = alterlist(dctd_dtd->tbl,ctd_work_cnt), dctd_dtd->tbl_cnt = ctd_work_cnt
  WITH nocounter
 ;end select
 IF (dm2_set_autocommit(0)=0)
  GO TO exit_program
 ...
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSEIF (validate(inhouse_reply->table_name,"@")="@"
  AND curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("No DM_TABLES_DOC Information Found for table: ",inhouse_reply->
   table_name)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET ctd_table_size = size(tbl_doc->qual,5)
 SET stat = alterlist(tbl_doc_maint->qual,ctd_table_size)
 IF (dm2_set_autocommit(1)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Determining maintenance type on DM_TABLES_DOC."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET tbl_doc_maint->num_of_inserts = 0
 SET tbl_doc_maint->num_of_updates = 0
 SET tbl_doc_maint->num_of_none = 0
 FOR (ctd_cnt = 1 TO ctd_table_size)
  SET tbl_doc_maint->qual[ctd_cnt].maint_type = "I"
  SET tbl_doc_maint->num_of_inserts = (tbl_doc_maint->num_of_inserts+ 1)
 ENDFOR
 IF ((dm_err->debug_flag > 1))
  CALL echo(build("size:",size(tbl_doc->qual,5)))
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt td  WITH seq = value(size(tbl_doc->qual,5))),
   (dummyt dtd  WITH seq = value(dctd_dtd->tbl_cnt))
  PLAN (td)
   JOIN (dtd
   WHERE (tbl_doc->qual[td.seq].table_name=dctd_dtd->tbl[dtd.seq].table_name))
  DETAIL
   IF ((dm_err->debug_flag > 5))
    CALL echo(concat("Tables doc matched for table: ",dctd_dtd->tbl[dtd.seq].table_name))
   ENDIF
   IF ((tbl_doc->qual[td.seq].updt_cnt >= dctd_dtd->tbl[dtd.seq].updt_cnt))
    tbl_doc_maint->qual[td.seq].maint_type = "U", tbl_doc_maint->num_of_updates = (tbl_doc_maint->
    num_of_updates+ 1), tbl_doc_maint->num_of_inserts = (tbl_doc_maint->num_of_inserts - 1)
   ELSEIF ((dctd_dtd->tbl[dtd.seq].fix_columns_ind=1))
    tbl_doc_maint->qual[td.seq].maint_type = "F", tbl_doc_maint->num_of_fixes = (tbl_doc_maint->
    num_of_fixes+ 1), tbl_doc_maint->num_of_inserts = (tbl_doc_maint->num_of_inserts - 1)
   ELSE
    tbl_doc_maint->qual[td.seq].maint_type = "N", tbl_doc_maint->num_of_none = (tbl_doc_maint->
    num_of_none+ 1), tbl_doc_maint->num_of_inserts = (tbl_doc_maint->num_of_inserts - 1)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(
  "Matching documented tables for current install to what is currently in the environment.")=1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error matching documented tables for current install to current environment."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Maintenance to DM_TABLES_DOC includes: ",cnvtstring(tbl_doc_maint->
   num_of_inserts,9,0)," Inserts  ",cnvtstring(tbl_doc_maint->num_of_updates,9,0)," Updates  ",
  cnvtstring(tbl_doc_maint->num_of_fixes,9,0)," Fixes  ",cnvtstring(tbl_doc_maint->num_of_none,9,0),
  " w/ no changes.")
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dm2_set_autocommit(0)=0)
  GO TO exit_program
...
#exit_program
 IF ((dm_err->debug_flag > 4))
  CALL echorecord(tbl_doc)
  CALL echorecord(tbl_doc_maint)
 ENDIF
 FREE SET dctd_dtd
 SET dm_err->eproc = "Maintenance check on DM_TABLES_DOC Complete"
 CALL final_disp_msg(ctd_logfile_prefix)
END GO
