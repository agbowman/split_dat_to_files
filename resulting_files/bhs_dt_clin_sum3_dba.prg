CREATE PROGRAM bhs_dt_clin_sum3:dba
 PROMPT
  "Printer" = "mine"
 ...
 DECLARE code_status_cd = f8 WITH public, constant(uar_get_code_by("displaykey",200,"NORESUSCITATION"
   ))
 SELECT INTO "nl:"
  ...
  FROM encntr_domain ed,
   encounter e
  PLAN (ed
   WHERE ed.encntr_id=21354680.00
  ...
  HEAD REPORT
   cnt = 0
  DETAIL
   cnt = (cnt+ 1), stat = alterlist(act_pat->qual,cnt), act_pat->qual[cnt].eid = ed.encntr_id,
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   ...
   WHERE (o.encntr_id=pat->qual[d1.seq].eid)
    ...
    AND o.activity_type_cd IN (callmd_cd, rntorn_cd, woundcare_cd, orthopedictreatments_cd,
   orthosupply_cd,
 ...
 SELECT INTO "nl:"
  ...
  PLAN (dd)
   ...
   neurodiag_cd, pulmlab_cd)) OR (o.activity_type_cd IN (mdtornconsults_cd, consults_cd))) )
 ...
 SELECT INTO "nl:"
  ...
  PLAN (dd)
   ...
   WHERE (o.encntr_id=pat->qual[dd.seq].eid)
    AND o.order_status_cd IN (os_inc_cd, os_inp_cd, os_ord_cd, os_pen_cd, os_per_cd)
    AND o.template_order_flag IN (0, 1)
    AND o.activity_type_cd IN (rttxprocedures_cd, nsgrespiratorytx_cd, ventilationnoninvasive_cd,
   ventilationinvasive_cd, sleepstudies_cd))
  ORDER BY o.encntr_id, sort_order, cnvtdatetime(o.orig_order_dt_tm),
   o.order_id
  HEAD o.encntr_id
 ...
 SELECT INTO "nl:"
  ...
  PLAN (dd)
   ...
   WHERE (o.encntr_id=pat->qual[dd.seq].eid)
    AND o.order_status_cd IN (os_inc_cd, os_inp_cd, os_ord_cd, os_pen_cd, os_per_cd)
    AND o.template_order_flag IN (0, 1)
    AND o.activity_type_cd IN (invasivelinestubesdrains_cd))
  ORDER BY o.encntr_id, sort_order, cnvtdatetime(o.orig_order_dt_tm),
   o.order_id
  HEAD o.encntr_id
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   JOIN (o
   WHERE (o.encntr_id=pat->qual[d1.seq].eid)
    AND o.catalog_cd=code_status_cd
    AND o.order_status_cd=os_ord_cd)
  DETAIL
   pat->qual[d1.seq].code_status = uar_get_code_display(o.catalog_cd)
  WITH nocounter
 ;end select
 CALL echo("outside: ")
 CALL echo(pat_cnt)
 FOR (x = 1 TO pat_cnt)
   ...
   SELECT INTO "nl:"
    ...
    WHERE o.person_id=personid
     ...
     AND ((o.order_status_cd+ 0) IN (os_fut_cd, os_inp_cd, os_inc_cd, os_ord_cd, os_pen_cd,
    os_per_cd))
   ...
   SELECT INTO  $1
    ...
    HEAD REPORT
     l = 0, d_line = 0, x5 = "     ",
     x30 = "                              ", x10 = "          ", x40 = fillstring(40,""),
     x50 = fillstring(50,""), event_len = 0, date_len = 0,
      ...
      WHILE (tempstring > " "
       AND limit < 32000)
        ...
        ELSE
         rowplusone2
        ENDIF
        tempstring = substring((pos+ 1),31999,tempstring)
      ENDWHILE
     ENDMACRO
     ,
     MACRO (line_wrap2)
    ...
    HEAD PAGE
     ...
     pat->qual[x].religion, row + 1, xcol = 30,
     ycol = (ycol+ 10), visit = substring(1,100,pat->qual[x].visit_reason),
     CALL print(calcpos(xcol,ycol)),
     "Reason for Visit: ", visit, row + 1,
     ad_date = pat->qual[x].admit_dt, xcol = 350,
     CALL print(calcpos(xcol,ycol)),
     "Admit Date: ", ad_date, row + 1,
     ycol = (ycol+ 10), xcol = 30, diag = substring(1,100,pat->qual[x].diagnosis),
     CALL print(calcpos(xcol,ycol)), "Diagnosis: ", diag,
     row + 1, xcol = 350
     IF (trim(pat->qual[x].code_status) > "")
      code_status = substring(1,20,trim(pat->qual[x].code_status))
     ELSE
      code_status = "N/A"
     ENDIF
     CALL print(calcpos(xcol,ycol)), "Code Status: ", code_status,
     row + 1, ycol = (ycol+ 10), xcol = 30
     IF (curpage=1)
    ...
    DETAIL
     ...
     FOR (y = 1 TO 2)
       IF (ycol > 740)
        ...
        "**  Continue on Next Page **", row + 1, BREAK
       ENDIF
       l = (l+ 1), temp1 = build(cnvtstring(y),"-",pat->qual[x].sec[y].sec_disp,":"), d_line = (
       d_line+ 1),
       stat = alterlist(displayline->part1,d_line), displayline->part1[d_line].line = concat(x30,
        "{b}"," ",temp1),
       CALL print(calcpos(xcol,ycol)),
       "{b}", temp1, row + 1,
       ycol = (ycol+ 8)
       FOR (z = 1 TO size(pat->qual[x].sec[y].grpr,5))
         ...
         d_line = (d_line+ 1),
         stat = alterlist(displayline->part1,d_line), displayline->qual[d_line].line = concat(x30,x5,
          x5,grpr_date),
         CALL print(calcpos(xcol,ycol)),
           ...
           IF ((pat->qual[x].sec[y].grpr[z].event[zz].max_result_qty > 0))
            ...
            FOR (yy = 1 TO pat->qual[x].sec[y].grpr[z].event[zz].event_cnt)
              ...
              ELSE
               ...
               d_line = (d_line+ 1), stat = alterlist(displayline->qual,d_line), displayline->qual[
               d_line].line = concat(x50,x40," ",temp1),
               CALL print(calcpos(xcol,ycol)), temp1, row + 1,
     ...
     FOR (m1 = 1 TO size(pat->qual[x].invasive,5))
       ...
       tempstring = concat("{b}",pat->qual[x].invasive[m1].order_mnemonic,"{endb} ",pat->qual[x].
        invasive[m1].clinical_display,pat->qual[x].invasive[m1].verify_str), line_wrap
     ENDFOR
     IF (ycol > 710)
      xcol = 100, ycol = (ycol+ 10),
      CALL print(calcpos(xcol,ycol)),
     ...
     ENDFOR
     ycol = (ycol+ 5)
     FOR (y2 = 3 TO size(pat->qual[x].sec,5))
       IF (ycol > 740)
        xcol = 100, ycol = (ycol+ 10),
        CALL print(calcpos(xcol,ycol)),
        "**  Continue on Next Page **", row + 1, BREAK
         ...
         FOR (zz = 1 TO size(pat->qual[x].sec[y2].grpr[z].event,5))
           ...
           IF ((pat->qual[x].sec[y2].grpr[z].event[zz].max_result_qty > 0))
            ...
            event[zz].max_result_qty))
             pat->qual[x].sec[y2].grpr[z].event[zz].event_cnt = pat->qual[x].sec[y2].grpr[z].event[zz
             ].max_result_qty
            ENDIF
