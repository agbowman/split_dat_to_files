CREATE PROGRAM dm2_code_value_rpt:dba
 ...
 SUBROUTINE drr_chk_active_runners(dcar_dblink,dcar_count_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dcvr_prompt_for_package(dpfp_package_nbr=i4(ref)) = i2
 DECLARE dcvr_package_nbr = i4 WITH protect, noconstant(0)
 DECLARE dcvr_env_id = f8 WITH protect, noconstant(0.00)
 DECLARE dcvr_rpt_file = vc WITH protect, noconstant("NOT_SET")
 DECLARE dcvr_rpt_prefix = vc WITH protect, noconstant("NOT_SET")
 DECLARE dcvr_big_cs_cv_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcvr_no_new_cs_ind = i2 WITH protect, noconstant(0)
 DECLARE dcvr_tmp_cs = f8 WITH protect, noconstant(0.00)
 DECLARE dcvr_tmp_cv = f8 WITH protect, noconstant(0.00)
 DECLARE dcvr_tmp_cdfm = vc WITH protect, noconstant("NOT_SET")
 DECLARE dcvr_tmp_disp = vc WITH protect, noconstant("NOT_SET")
 DECLARE dcvr_tmp_desc = vc WITH protect, noconstant("NOT_SET")
 DECLARE dcvr_tmp_actind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_cv_rpt",".log","dm2_code_value_rpt")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Checking for the INSTALL_TEXT column on the OCD_INSTALL_LOG table."
 CALL disp_msg("",dm_err->logfile,dm_err->err_ind)
 IF (dilc_check_oil_column(null)=0)
  SET message = nowindow
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc =
  "The required schema for code value reports has not yet been installed.  Skipping the report step."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Preparing to generate Code Values Installed report."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  di.info_number
  FROM dm_info di
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dcvr_package_nbr =  $1
 IF (dcvr_package_nbr=0)
  IF ((dm2menureq->prompt_mode="PROMPT"))
   IF (dcvr_prompt_for_package(dcvr_package_nbr)=0)
    GO TO exit_program
   ENDIF
  ELSE
   SET dm_err->eproc = "Validating Package number or Batch ID."
   SET dm_err->emsg = "Invalid Package number or Batch ID."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ENDIF
 ENDIF
 IF ((dm2_install_schema->logfile_prefix != "NONE"))
  SET dcvr_rpt_file = build(dm2_install_schema->logfile_prefix,"cv.rpt")
 ELSE
  IF (dcvr_package_nbr < 0)
   SET dcvr_rpt_prefix = build("dm2ob",format(abs(dcvr_package_nbr),"######;p0"),"_")
  ELSE
   SET dcvr_rpt_prefix = build("dm2os",format(dcvr_package_nbr,"######;p0"),"_")
  ENDIF
  IF (get_unique_file(dcvr_rpt_prefix,"cv.rpt")=0)
   GO TO exit_program
  ENDIF
  SET dcvr_rpt_file = dm_err->unique_fname
 ENDIF
 FREE RECORD cv_rpt
 RECORD cv_rpt(
   1 cs_cnt = i4
 ...
 SET cv_rpt->cs_cnt = 0
 SET dm_err->eproc = "Gathering new code sets for report."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  o.end_state
  FROM ocd_install_log o
  WHERE o.component_type=dilc_code_set
   AND o.ocd=dcvr_package_nbr
  ORDER BY o.end_state
  DETAIL
   cv_rpt->cs_cnt = (cv_rpt->cs_cnt+ 1)
   IF (mod(cv_rpt->cs_cnt,100)=1)
    stat = alterlist(cv_rpt->cs_list,(cv_rpt->cs_cnt+ 99))
   ENDIF
   cv_rpt->cs_list[cv_rpt->cs_cnt].code_set = cnvtreal(o.end_state), cv_rpt->cs_list[cv_rpt->cs_cnt].
   cs_description = o.install_text, cv_rpt->cs_list[cv_rpt->cs_cnt].new_ind = 1
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 ENDIF
 IF (curqual=0)
  SET dcvr_no_new_cs_ind = 1
 ENDIF
 IF ((dm_err->debug_flag >= 2))
  CALL echorecord(cv_rpt)
 ENDIF
 SET dm_err->eproc = "Gathering individual new code values for report."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  o.end_state, cv.code_value
  FROM ocd_install_log o,
   code_value cv,
   code_value_set cs
  PLAN (o
   WHERE o.component_type=dilc_code_value
    AND o.ocd=dcvr_package_nbr)
   JOIN (cv
   WHERE cv.code_value=cnvtreal(o.end_state))
   JOIN (cs
  ...
  HEAD cv.code_set
   ...
   IF (mod(cv_rpt->cs_cnt,100)=1)
    stat = alterlist(cv_rpt->cs_list,(cv_rpt->cs_cnt+ 99))
   ENDIF
   cv_rpt->cs_list[cv_rpt->cs_cnt].code_set = cv.code_set, cv_rpt->cs_list[cv_rpt->cs_cnt].
   cs_description = cs.description
  DETAIL
   cv_rpt->cs_list[cv_rpt->cs_cnt].cv_cnt = (cv_rpt->cs_list[cv_rpt->cs_cnt].cv_cnt+ 1)
   IF (mod(cv_rpt->cs_list[cv_rpt->cs_cnt].cv_cnt,100)=1)
    stat = alterlist(cv_rpt->cs_list[cv_rpt->cs_cnt].cv_list,(cv_rpt->cs_list[cv_rpt->cs_cnt].cv_cnt
     + 99))
   ENDIF
   stat = dilc_get_install_text(dcvr_tmp_cs,dcvr_tmp_cv,dcvr_tmp_desc,dcvr_tmp_cdfm,dcvr_tmp_disp,
    dcvr_tmp_actind,o.install_text), cv_rpt->cs_list[cv_rpt->cs_cnt].cv_list[cv_rpt->cs_list[cv_rpt->
   cs_cnt].cv_cnt].code_value = cv.code_value, cv_rpt->cs_list[cv_rpt->cs_cnt].cv_list[cv_rpt->
   cs_list[cv_rpt->cs_cnt].cv_cnt].cdf_meaning = dcvr_tmp_cdfm,
   cv_rpt->cs_list[cv_rpt->cs_cnt].cv_list[cv_rpt->cs_list[cv_rpt->cs_cnt].cv_cnt].display =
   dcvr_tmp_disp, cv_rpt->cs_list[cv_rpt->cs_cnt].cv_list[cv_rpt->cs_list[cv_rpt->cs_cnt].cv_cnt].
   cv_description = dcvr_tmp_desc, cv_rpt->cs_list[cv_rpt->cs_cnt].cv_list[cv_rpt->cs_list[cv_rpt->
   cs_cnt].cv_cnt].active_ind = dcvr_tmp_actind
  FOOT  cv.code_set
   stat = alterlist(cv_rpt->cs_list[cv_rpt->cs_cnt].cv_list,cv_rpt->cs_list[cv_rpt->cs_cnt].cv_cnt)
   IF ((cv_rpt->cs_list[cv_rpt->cs_cnt].cv_cnt > dcvr_big_cs_cv_cnt))
    dcvr_big_cs_cv_cnt = cv_rpt->cs_list[cv_rpt->cs_cnt].cv_cnt
   ENDIF
  FOOT REPORT
   stat = alterlist(cv_rpt->cs_list,cv_rpt->cs_cnt)
  WITH nocounter
 ;end select
 IF ((dm_err->debug_flag >= 2))
  CALL echorecord(cv_rpt)
 ...
 ENDIF
 IF ((cv_rpt->cs_cnt=0))
  SET dm_err->eproc = "No new code sets or code values to report."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Writing the Code Values Installed report to ",dcvr_rpt_file,".")
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO value(dcvr_rpt_file)
  d.seq
  FROM (dummyt d  WITH seq = cv_rpt->cs_cnt)
  WHERE (cv_rpt->cs_list[d.seq].new_ind=1)
  HEAD REPORT
   rpt_ccluserdir_path = fillstring(12," ")
   IF (cursys="AXP")
    rpt_ccluserdir_path = "CCLUSERDIR:"
   ELSE
    rpt_ccluserdir_path = "$CCLUSERDIR/"
   ENDIF
   row + 1, col 0,
   CALL print(concat("Report generated on ",format(cnvtdatetime(curdate,curtime3),";;q"))),
   row + 1, col 0,
   CALL print(concat("This report can be found at ",trim(rpt_ccluserdir_path,3),dcvr_rpt_file)),
   row + 2, col 0, "The following new Code Sets were installed:",
   row + 2, col 0, "Code Set",
   col 20, "Description", row + 1,
   col 0, "-------------------------------------", row + 1
  DETAIL
   row + 1, col 0, cv_rpt->cs_list[d.seq].code_set,
   col 20, cv_rpt->cs_list[d.seq].cs_description
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 IF (dcvr_no_new_cs_ind=1)
  SELECT INTO value(dcvr_rpt_file)
   d.seq
   FROM (dummyt d  WITH seq = 1)
   DETAIL
    col 0, "*** No new Code Sets were installed. ***", row + 2
   WITH nocounter, nullreport, append,
 ...
 SELECT INTO value(dcvr_rpt_file)
  d.seq
  FROM (dummyt d  WITH seq = cv_rpt->cs_cnt),
   (dummyt dd  WITH seq = dcvr_big_cs_cv_cnt)
  WHERE (cv_rpt->cs_list[d.seq].new_ind=0)
  ORDER BY cv_rpt->cs_list[d.seq].code_set
  HEAD REPORT
   row + 2, col 0, "The following new Code Values were installed:",
   row + 3, col 0, "CODE_SET",
   col 10, "CS DESCRIPTION", col 30,
   "CODE_VALUE", col 50, "CDF_MEANING",
   col 70, "DISPLAY", col 90,
   "CV DESCRIPTION", col 120, "ACTIVE_IND",
   row + 1, col 0,
   CALL print(fillstring(130,"-")),
   row + 1
  DETAIL
   IF ((dd.seq <= cv_rpt->cs_list[d.seq].cv_cnt))
    row + 1, col 0,
    CALL print(trim(format(cv_rpt->cs_list[d.seq].code_set,";;I"),3)),
    col 10,
    CALL print(substring(1,19,cv_rpt->cs_list[d.seq].cs_description)), col 30,
    CALL print(trim(format(cv_rpt->cs_list[d.seq].cv_list[dd.seq].code_value,";;I"),3)), col 50,
    CALL print(substring(1,19,cv_rpt->cs_list[d.seq].cv_list[dd.seq].cdf_meaning)),
    col 70,
    CALL print(substring(1,19,cv_rpt->cs_list[d.seq].cv_list[dd.seq].display)), col 90,
    CALL print(substring(1,38,cv_rpt->cs_list[d.seq].cv_list[dd.seq].cv_description)), col 129,
    CALL print(build(cv_rpt->cs_list[d.seq].cv_list[dd.seq].active_ind))
   ENDIF
  FOOT REPORT
   row + 2, col 0,
 ...
 ENDIF
 IF ((dir_ui_misc->dm_process_event_id > 0))
  IF (drr_assign_file_to_installs("RPT:CODE VALUES INSTALLED",dcvr_rpt_file,dir_ui_misc->
   dm_process_event_id)=0)
   RETURN(0)
  ENDIF
 ENDIF
 IF (dir_silmode_requested_ind=1)
  CALL dir_add_silmode_entry("Code Values Installed Report",dcvr_rpt_file)
  SET dm_err->eproc = concat("Silent Mode:  Code Values Installed Report was",
   " bypassed.  Report exits in CCLUSERDIR under file name: ",dcvr_rpt_file)
  CALL disp_msg("",dm_err->logfile,0)
 ELSE
  IF (dm2_disp_file(dcvr_rpt_file,"Code Values Installed Report")=0)
   RETURN(0)
  ENDIF
 ENDIF
 GO TO exit_program
 SUBROUTINE dcvr_prompt_for_package(dpfp_package_nbr)
   DECLARE dpfp_install_type = vc WITH protect, noconstant("X")
   DECLARE dpfp_space_pos = i4 WITH protect, noconstant(0)
   DECLARE dpfp_tmp_str = vc WITH protect, noconstant("NOT_SET")
   FREE RECORD dpfp_pkg_list
   RECORD dpfp_pkg_list(
     1 pkg_cnt = i4
     1 pkg[*]
       2 pkg_nbr = i4
       2 install_dt_tm = dq8
   ...
   SET dpfp_pkg_list->pkg_cnt = 0
   SET dpfp_pkg_list->batch_cnt = 0
   SET dm_err->eproc = "Gathering packages and batch plans for reporting."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    o.ocd, o.install_dt_tm
    FROM ocd_install_log o
    WHERE o.component_type IN (dilc_code_set, dilc_code_value)
    ORDER BY o.ocd, o.install_dt_tm
    HEAD o.ocd
     IF (o.ocd > 0)
      dpfp_pkg_list->pkg_cnt = (dpfp_pkg_list->pkg_cnt+ 1), stat = alterlist(dpfp_pkg_list->pkg,
       dpfp_pkg_list->pkg_cnt), dpfp_pkg_list->pkg[dpfp_pkg_list->pkg_cnt].pkg_nbr = o.ocd,
      dpfp_pkg_list->pkg[dpfp_pkg_list->pkg_cnt].install_dt_tm = o.install_dt_tm
     ELSEIF (o.ocd < 0)
      dpfp_pkg_list->batch_cnt = (dpfp_pkg_list->batch_cnt+ 1), stat = alterlist(dpfp_pkg_list->batch,
       dpfp_pkg_list->batch_cnt), dpfp_pkg_list->batch[dpfp_pkg_list->batch_cnt].batch_nbr = abs(o
       .ocd),
      dpfp_pkg_list->batch[dpfp_pkg_list->batch_cnt].install_dt_tm = o.install_dt_tm
     ENDIF
    WITH nocounter
   ;end select
   SET width = 132
   ...
   CALL clear(1,1)
   CALL box(1,1,24,131)
   CALL text(2,2,"Code Set Report")
   CALL text(4,10,"1. Single Package report")
   CALL text(6,10,"2. Batch Install report")
   CALL text(8,10,"Select a report from the options above (0 to exit):  ")
   CALL accept(8,62,"9;",0
    WHERE curaccept IN (0, 1, 2))
   CASE (curaccept)
    OF 1:
     IF ((dpfp_pkg_list->pkg_cnt > 0))
      SET help =
      SELECT INTO "nl:"
       package_nbr = dpfp_pkg_list->pkg[d.seq].pkg_nbr, install_dt_tm = format(dpfp_pkg_list->pkg[d
        .seq].install_dt_tm,";;Q")
       FROM (dummyt d  WITH seq = dpfp_pkg_list->pkg_cnt)
       ORDER BY dpfp_pkg_list->pkg[d.seq].pkg_nbr DESC
      ;end select
      CALL text(10,10,"Select a Distribution Package from the list:")
      CALL accept(10,56,"P(45);CF")
      SET dpfp_tmp_str = trim(curaccept,2)
      SET dpfp_space_pos = findstring(" ",dpfp_tmp_str,1,0)
      IF ((dm_err->debug_flag >= 1))
       CALL text(9,10,build(dpfp_space_pos,":",substring(1,(dpfp_space_pos - 1),dpfp_tmp_str)))
      ENDIF
      SET dpfp_package_nbr = cnvtint(substring(1,(dpfp_space_pos - 1),dpfp_tmp_str))
      CALL clear(10,56)
      CALL box(1,1,24,131)
      CALL text(10,56,build(dpfp_package_nbr))
     ELSE
      SET dm_err->eproc = "No code value history to report."
      CALL text(10,10,concat(dm_err->eproc,"  Press ENTER to continue."))
      CALL accept(10,72,"AA;D","OK")
      CALL disp_msg("",dm_err->logfile,0)
      RETURN(0)
     ENDIF
    OF 2:
     IF ((dpfp_pkg_list->batch_cnt > 0))
      SET help =
      SELECT INTO "nl:"
       plan_id = dpfp_pkg_list->batch[d.seq].batch_nbr, install_dt_tm = format(dpfp_pkg_list->batch[d
        .seq].install_dt_tm,";;Q")
       FROM (dummyt d  WITH seq = dpfp_pkg_list->batch_cnt)
       ORDER BY dpfp_pkg_list->batch[d.seq].batch_nbr DESC
      ;end select
      CALL text(10,10,"Select a Batch install plan from the list:")
      CALL accept(10,56,"P(45);CFS")
      SET dpfp_tmp_str = trim(curaccept,2)
      SET dpfp_space_pos = findstring(" ",dpfp_tmp_str,1,0)
      IF ((dm_err->debug_flag >= 1))
       CALL text(9,10,build(dpfp_space_pos,":",substring(1,(dpfp_space_pos - 1),dpfp_tmp_str)))
      ENDIF
      SET dpfp_package_nbr = (cnvtint(substring(1,(dpfp_space_pos - 1),dpfp_tmp_str)) * - (1))
      CALL clear(10,56)
      CALL box(1,1,24,131)
      CALL text(10,56,build(abs(dpfp_package_nbr)))
     ELSE
      SET dm_err->eproc = "No code value history to report."
      CALL text(10,10,concat(dm_err->eproc,"  Press ENTER to continue."))
      CALL accept(10,72,"AA;D","OK")
    ...
    OF 0:
     SET message = nowindow
     SET dm_err->eproc = "Exiting Code Values Installed report."
     CALL disp_msg("",dm_err->logfile,0)
     RETURN(0)
   ENDCASE
   CALL text(12,10,"(D)isplay Report or (X)it?")
   CALL accept(12,48,"X;CU","D"
    WHERE cnvtupper(curaccept) IN ("D", "X"))
 ...
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Finished generating Code Values Installed report."
 CALL final_disp_msg("dm2_cv_rpt")
END GO
