CREATE PROGRAM dm2_ref_data_mover_1285:dba
 ...
 DECLARE null_check = i2
 DECLARE from_name = vc
 DECLARE cust_tab_name = vc
 DECLARE dmt_cnt = i4
 DECLARE cust_gen_object = vc
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET dmt_cnt = 0
 SET null_check = get_nullind(cust_tab_name,"REL_PARENT_ENTITY_NAME")
 SET from_name = get_value(cust_tab_name,"REL_PARENT_ENTITY_NAME","FROM")
 SET from_id = get_value(cust_tab_name,"REL_PARENT_ENTITY_ID","FROM")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_1285
 ENDIF
 IF (cnvtreal(from_id) != 0)
  IF (((null_check=1) OR (size(trim(from_name))=0)) )
   SET stat = alterlist(rdds_exception->qual,(size(rdds_exception->qual,5)+ 1))
   SET rdds_exception->qual[size(rdds_exception->qual,5)].tab_col_name = "REL_PARENT_ENTITY_ID"
   SET rdds_exception->qual[size(rdds_exception->qual,5)].tru_tab_name = "INVALID"
   SET rdds_exception->qual[size(rdds_exception->qual,5)].tru_col_name = "INVALID"
   SET to_id = select_merge_translate(from_id,"CLASS_NODE")
   IF (to_id != "No Trans")
    CALL put_value(cust_tab_name,"REL_PARENT_ENTITY_ID",to_id)
    IF ((dm2_ref_data_reply->error_ind=1))
     GO TO exit_1285
    ENDIF
   ELSE
    SET to_id = select_merge_translate(from_id,"CODE_VALUE")
    IF (to_id != "No Trans")
     CALL put_value(cust_tab_name,"REL_PARENT_ENTITY_ID",to_id)
     IF ((dm2_ref_data_reply->error_ind=1))
      GO TO exit_1285
  ...
  ENDIF
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"OBJECT_IDENTIFIER_INDEX_ID")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_1285
 ENDIF
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (no_query=1)
   ...
   IF (val < 0)
    IF ((val=- (3)))
     SET val = get_seq(cust_tab_name,"OBJECT_IDENTIFIER_INDEX_ID")
     IF (val > 0)
      CALL put_value(cust_tab_name,"OBJECT_IDENTIFIER_INDEX_ID",cnvtstring(val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,
       "  OBJECT_IDENTIFIER_INDEX_ID")
     ELSE
      GO TO exit_1285
     ENDIF
    ELSE
   ...
   SET from_id = get_value(cust_tab_name,"OBJECT_IDENTIFIER_INDEX_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1285
   ELSE
 ...
 ENDIF
 IF (idcd_check=1)
  SET cust_gen_object = get_value(cust_tab_name,"REL_PARENT_ENTITY_ID","TO")
  CALL put_value(cust_tab_name,"GENERIC_OBJECT",cust_gen_object)
