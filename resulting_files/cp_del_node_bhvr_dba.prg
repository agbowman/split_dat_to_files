CREATE PROGRAM cp_del_node_bhvr:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Node Behavior IDs:" = 0.0
  WITH outdev, nb
 FREE RECORD reply
 RECORD reply(
   1 qual[*]
 ...
 DECLARE checkcodevalue(codevalue=f8,codeset=i4) = i4 WITH protect
 ...
 SET reply->status_data.status = "F"
 IF ((nblist->cnt=0))
  CALL getparametervalues(2,nblist)
 ENDIF
 IF (validate(debug_ind,0)=1)
  CALL echorecord(nblist)
 ENDIF
 IF ((nblist->cnt > 0))
 ...
 ELSE
  ...
  SET reply->status_data.subeventstatus.targetobjectvalue = build2(
   "CP_NODE_BEHAVIOR_ID list is empty.")
  GO TO exit_script
 ENDIF
 SUBROUTINE (checknodebhvrid(idx=i4) =i4 WITH protect)
   CALL log_message("In CheckNodeBhvrId()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SELECT INTO "nl:"
    FROM cp_node_behavior cnb
    WHERE (cnb.cp_node_behavior_id=nblist->qual[idx].value)
    HEAD REPORT
     i = size(reply->qual,5)
    DETAIL
     i += 1, lstat = alterlist(reply->qual,i), reply->qual[i].cp_node_behavior_id = cnb
     .cp_node_behavior_id,
     reply->qual[i].cp_node_id = cnb.cp_node_id, reply->qual[i].reaction_entity_id = cnb
     .reaction_entity_id, reply->qual[i].reaction_entity_name = trim(cnb.reaction_entity_name)
     IF (textlen(trim(cnb.long_response_ident,3)) > 0)
      reply->qual[i].response_ident = trim(cnb.long_response_ident)
     ELSE
      reply->qual[i].response_ident = trim(cnb.response_ident)
     ENDIF
    FOOT REPORT
     IF (validate(debug_ind,0)=1)
    ...
    WITH nocounter
   ;end select
   CALL log_message(build("Exit CheckNodeBhvrId(), Elapsed time in seconds:",((curtime3 -
 ...
 SUBROUTINE deleterows(null)
   ...
   DECLARE xsize = i4 WITH protect, constant(size(nblist->qual,5))
   ...
   ENDIF
   FOR (x = 1 TO xsize)
    SET lstat = checknodebhvrid(x)
    IF (lstat > 0)
     IF ((nblist->qual[x].reaction_entity_name="LONG_TEXT_REFERENCE"))
      CALL deletelongtextrefrow(nblist->qual[x].reaction_entity_id)
     ENDIF
     IF (validate(debug_ind,0)=1)
      CALL echo(build("Deleting CP_NODE_BEHAVIOR_ID[",nblist->qual[x].value,"]"))
     ENDIF
     DELETE  FROM cp_node_behavior cnb
      WHERE (cnb.cp_node_behavior_id=nblist->qual[x].value)
      WITH nocounter
     ;end delete
     SET lstat = error_and_zero_check_rec(1,"DELETE","ERROR DELETING FROM CP_NODE_BEHAVIOR",1,1,
      reply)
    ELSE
     SET reply->status_data.status = "F"
     SET reply->status_data.subeventstatus.operationname = "PERFORM"
     ...
     SET reply->status_data.subeventstatus.targetobjectvalue = build2("CP_NODE_BEHAVIOR_ID[",nblist->
      qual[x].value,"] does not exist on the CP_NODE_BEHAVIOR_ID table.")
     GO TO exit_script
    ENDIF
   ENDFOR
 ...
 END ;Subroutine
 SUBROUTINE (deletelongtextrefrow(lt_id=f8) =null WITH protect)
  DELETE  FROM long_text_reference lt
   WHERE lt.long_text_id=lt_id
   WITH nocounter
  ;end delete
  IF (curqual=0)
   CALL error_and_zero_check_rec(curqual,"PERFORM DELETE",
    "Unable to delete row into long_text table.",0,0,
    reply)
   ROLLBACK
   GO TO exit_script
  ELSE
