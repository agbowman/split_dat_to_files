CREATE PROGRAM cp_smrt_tmplt_summary_view:dba
 ...
 DECLARE treatment_disp = vc WITH constant("Treatments"), protect
 DECLARE assessment_disp = vc WITH constant("Assessments"), protect
 DECLARE orders_disp = vc WITH constant("Order"), protect
 DECLARE bold = vc WITH protect, constant(" \b{{BOLD_SIZE}} ")
 DECLARE italic = vc WITH protect, constant(" \i{{ITALIC_SIZE}} ")
 DECLARE endbold = vc WITH protect, constant(" \b0 ")
 DECLARE enditalic = vc WITH protect, constant(" \i0 ")
 DECLARE c_return = vc WITH protect, constant("\par")
 DECLARE ul = vc WITH protect, constant("\ul ")
 DECLARE ul_end = vc WITH protect, constant(" \ulnone ")
 DECLARE font_size = vc WITH protect, constant(" \fs{{FONT_SIZE}} ")
 DECLARE hyphen = vc WITH protect, constant(" \endash ")
 ...
 SUBROUTINE main(null)
   ...
   CALL getpathwayactivity(person_id)
   IF (active_pathway_id != 0
    AND active_pathway_instance_id != 0)
    CALL getpathwaytraildata(active_pathway_id,active_pathway_instance_id,active_pathway_name)
   ENDIF
   CALL buildrtftext(null)
   CALL log_message(build("Exit main(), Time (in secs) : ",((curtime3 - begin_curtime3)/ 100.0)),
    log_level_debug)
 END ;Subroutine
 SUBROUTINE (getpathwayactivity(person_id=f8) =null)
 ...
 END ;Subroutine
 SUBROUTINE (getpathwaytraildata(pathway_id=f8,pathway_instance_id=f8,pathway_name=vc) =null)
   ...
   ENDIF
   SET template_display->pathway_tmplt_title = concat(pathway_name," ")
   SET template_display->pathway_start_dt = cnvtdate(active_pathway_begin_dt_tm)
   SET template_display->pathway_start_dt_string = format(active_pathway_begin_dt_tm,"mm/dd/yy;;d")
   IF ((pathway_trail_data->action_cnt != 0))
    SELECT
     date_string = format(pathway_trail_data->actions[d1.seq].action_dt_tm,"mm/dd/yy;;d"),
     node_display = trim(substring(1,100,pathway_trail_data->actions[d1.seq].node_display),3),
     action_mean = trim(substring(1,100,pathway_trail_data->actions[d1.seq].action_mean),3),
     action_detail_entity_mean = trim(substring(1,100,pathway_trail_data->actions[d1.seq].
       action_detail_entity_mean),3)
     ...
     ORDER BY date_string, node_display, action_mean,
      d1.seq
     HEAD date_string
      dcnt += 1, stat = alterlist(template_display->dates,dcnt), template_display->date_cnt = dcnt,
      template_display->dates[dcnt].date_string = format(pathway_trail_data->actions[d1.seq].
       action_dt_tm,"mm/dd/yy;;d")
     HEAD node_display
      row + 0
     HEAD action_mean
      IF (action_mean IN (commitassess_mean, committreat_mean))
       IF (action_mean=commitassess_mean)
        action_display = assessment_disp
       ELSE
        action_display = treatment_disp
       ENDIF
       CALL addsummaryaction(dcnt,action_display,pathway_trail_data->actions[d1.seq].node_display)
      ENDIF
     HEAD d1.seq
      IF (action_detail_entity_mean=ordselect_mean)
       CALL addsummaryaction(dcnt,orders_disp,pathway_trail_data->actions[d1.seq].
       action_detail_entity_value)
      ELSEIF (action_mean IN (onpathway_mean, offpathway_mean, offclintrial_mean, onclintrial_mean))
       action_display = getactiondisplay(pathway_trail_data->actions[d1.seq].action_mean,
        pathway_trail_data->actions[d1.seq].action_detail_entity_mean),
       CALL addsummaryaction(dcnt,action_display,pathway_trail_data->actions[d1.seq].
       action_detail_entity_value)
      ENDIF
     WITH nocounter
    ;end select
 ...
 END ;Subroutine
 SUBROUTINE (addsummaryaction(recindx=i4,actiondisplay=vc,detaildisplay=vc) =null)
   DECLARE aloc = i4 WITH noconstant(0), protect
   DECLARE dloc = i4 WITH noconstant(0), protect
   DECLARE searchcntr = i4 WITH noconstant(0), protect
   SET aloc = locateval(searchcntr,1,template_display->dates[recindx].action_cnt,actiondisplay,
    template_display->dates[recindx].actions[searchcntr].action_display)
   IF (aloc=0)
    SET template_display->dates[recindx].action_cnt += 1
    SET aloc = template_display->dates[recindx].action_cnt
    SET stat = alterlist(template_display->dates[recindx].actions,template_display->dates[recindx].
     action_cnt)
    SET template_display->dates[recindx].actions[aloc].action_display = actiondisplay
   ENDIF
   IF (locateval(searchcntr,1,template_display->dates[recindx].actions[aloc].action_detail_cnt,
    detaildisplay,template_display->dates[recindx].actions[aloc].action_details[searchcntr].
    detail_display)=0)
    SET template_display->dates[recindx].actions[aloc].action_detail_cnt += 1
    SET dloc = template_display->dates[recindx].actions[aloc].action_detail_cnt
    SET stat = alterlist(template_display->dates[recindx].actions[aloc].action_details,
     template_display->dates[recindx].actions[aloc].action_detail_cnt)
    SET template_display->dates[recindx].actions[aloc].action_details[dloc].detail_display =
    detaildisplay
   ENDIF
 END ;Subroutine
 SUBROUTINE (getactiondisplay(actionmeaning=vc,entitymean=vc) =vc)
   CALL log_message(" In getActionDisplay() ",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE returndisplay = vc WITH noconstant(""), protect
   DECLARE acode = f8 WITH noconstant(uar_get_code_by("MEANING",4003135,actionmeaning))
   DECLARE ecode = f8 WITH noconstant(uar_get_code_by("MEANING",4003199,actionmeaning))
   DECLARE adisp = vc WITH noconstant(uar_get_code_display(acode))
   DECLARE edisp = vc WITH noconstant(uar_get_code_display(ecode))
   CASE (actionmeaning)
    OF commitassess_mean:
     IF (entitymean=ordselect_mean)
      SET returndisplay = edisp
     ELSE
      SET returndisplay = adisp
     ENDIF
    OF committreat_mean:
     IF (entitymean=ordselect_mean)
      SET returndisplay = edisp
 ...
 SUBROUTINE getheadertext(null)
   CALL log_message(" In getHeaderText() ",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE header = vc WITH protect, noconstant("")
   SET header = concat("{\rtf1\ansi\ {\fonttbl{\f0\fswiss\fprq2\fcharset0 ","{{FONT_NAME}}",
    ";}}\b\f0\fs","{{SIZE}} ","{{TITLE}}")
   CALL log_message(build(" Exit getHeaderText(), Time (in secs) : ",((curtime3 - begin_curtime3)/
     100.0)),log_level_debug)
   RETURN(header)
 END ;Subroutine
 SUBROUTINE getbodytext(null)
   CALL log_message(" In getBodyText() ",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE body = vc WITH protect, noconstant("")
   SET body = concat("\ul\b\f0\fs22","{{TITLE}}","\ulnone\b0 ")
   CALL log_message(build(" Exit getBodyText(), Time (in secs) : ",((curtime3 - begin_curtime3)/
     100.0)),log_level_debug)
   RETURN(body)
 END ;Subroutine
 ...
 SUBROUTINE buildrtftext(null)
   CALL log_message(" In buildRtfText() ",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE header_text = vc WITH protect, noconstant("")
   DECLARE body_text = vc WITH protect, noconstant("")
   DECLARE title = vc WITH protect, noconstant("Pathway summary view : ")
   DECLARE assessmenttext = vc WITH protect, noconstant("")
   DECLARE orderstext = vc WITH protect, noconstant("")
   DECLARE diseasetext = vc WITH protect, noconstant("")
   DECLARE treatmenttext = vc WITH protect, noconstant("")
   DECLARE clinicaltext = vc WITH protect, noconstant("")
   DECLARE pathwaytext = vc WITH protect, noconstant("")
   ...
   DECLARE datecount = i4 WITH protect, constant(template_display->date_cnt)
   ...
   DECLARE firstdetail = vc WITH protect, noconstant("")
   ...
   IF (datecount > 0)
    FOR (bcnt = 1 TO datecount)
      SET temptext = getdisplayline("  Date",template_display->dates[bcnt].date_string)
      SET option_1_body = concat(option_1_body,temptext)
      SET actioncount = template_display->dates[bcnt].action_cnt
      FOR (acnt = 1 TO actioncount)
        SET detailcount = template_display->dates[bcnt].actions[acnt].action_detail_cnt
        IF (detailcount > 0)
         SET firstdetail = template_display->dates[bcnt].actions[acnt].action_details[1].
         detail_display
        ELSE
         SET firstdetail = ""
        ENDIF
        SET temptext = getdisplayline(template_display->dates[bcnt].actions[acnt].action_display,
         firstdetail)
        SET option_1_body = concat(option_1_body," \tab ",temptext)
        IF (detailcount > 1)
         FOR (dcnt = 2 TO detailcount)
          SET temptext = trim(template_display->dates[bcnt].actions[acnt].action_details[dcnt].
           detail_display,3)
          SET option_1_body = concat(option_1_body," \tab\tab ",temptext,c_return)
         ENDFOR
        ENDIF
      ENDFOR
    ENDFOR
   ...
   ENDIF
   CALL log_message(build(" Exit buildRtfText(), Time (in secs) : ",((curtime3 - begin_curtime3)/
     100.0)),log_level_debug)
 END ;Subroutine
 SET log_program_name = "cp_smrt_tmplt_summary_view"
