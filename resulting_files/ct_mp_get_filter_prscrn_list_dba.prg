CREATE PROGRAM ct_mp_get_filter_prscrn_list:dba
 PROMPT
  ...
  SELECT INTO "nl:"
   ...
   HEAD prescreen_info.person_id
    ...
    IF (((print_all=1) OR (patient_cnt >= start_rownum
     ...
     prescreenlist[cnt].screening_status_disp = uar_get_code_display(prescreen_info
      .screening_status_cd), reply->prescreenlist[cnt].screening_status_mean = uar_get_code_meaning(
      prescreen_info.screening_status_cd),
     reply->prescreenlist[cnt].referral_dt_tm = prescreen_info.referral_dt_tm, reply->prescreenlist[
     cnt].referral_person_id = prescreen_info.referral_person_id, reply->prescreenlist[cnt].
     comment_text = prescreen_info.comment_text,
     reply->prescreenlist[cnt].reason_text = prescreen_info.reason_text, reply->prescreenlist[cnt].
     open_amendment_id = protocol_info.open_amendment_id, reply->prescreenlist[cnt].added_via_flag =
     prescreen_info.added_via_flag
    ENDIF
   FOOT REPORT
