CREATE PROGRAM bhs_newborn_hearing_rpt:dba
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   cnt2 = (cnt2+ 1), stat = alterlist(temp->qual[cnt].pat,cnt2), temp->qual[cnt].pat[cnt2].acc = trim
   (ea.alias),
   temp->qual[cnt].pat[cnt2].dt = format(dcp.last_activity_dt_tm,"mm/dd/yy hh:mm;;q"), temp->qual[cnt
   ].pat[cnt2].ptname = trim(p.name_full_formatted)
 ...
 FOR (x = 1 TO size(temp->qual,5))
   ...
   SELECT DISTINCT INTO value(printer_name)
