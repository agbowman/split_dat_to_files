CREATE PROGRAM dm_pcmb_bbd_special_interest:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "BBD_SPECIAL_INTEREST"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_PCMB_BBD_SPECIAL_INTEREST"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   .active_status_cd,
   rreclist->from_rec[v_cust_count1].special_interest_id = frm.special_interest_id, rreclist->
   from_rec[v_cust_count1].special_interest_cd = frm.special_interest_cd
  FOOT REPORT
 ...
 IF (v_cust_count1 > 0)
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    rreclist->to_rec[v_cust_count2].special_interest_id = tu.special_interest_id, rreclist->to_rec[
    v_cust_count2].special_interest_cd = tu.special_interest_cd
   FOOT REPORT
  ...
  FOR (v_cust_loopcount = 1 TO v_cust_count1)
   SELECT INTO "nl:"
    ...
    FROM bbd_special_interest tu
    WHERE (tu.person_id=request->xxx_combine[icombine].to_xxx_id)
     AND (tu.special_interest_cd=rreclist->from_rec[v_cust_loopcount].special_interest_cd)
     AND tu.active_ind=1
    WITH forupdate(tu)
   ;end select
   IF (curqual=0)
    CALL upt_from(rreclist->from_rec[v_cust_loopcount].special_interest_id,request->xxx_combine[
     icombine].to_xxx_id)
   ELSE
    CALL del_from(rreclist->from_rec[v_cust_loopcount].special_interest_id,request->xxx_combine[
     icombine].to_xxx_id,rreclist->from_rec[v_cust_loopcount].active_ind,rreclist->from_rec[
     v_cust_loopcount].active_status_cd)
   ENDIF
  ENDFOR
 ENDIF
 GO TO exit_sub
 SUBROUTINE del_from(s_df_pk_id,s_df_to_fk_id,s_df_prev_act_ind,s_df_prev_act_status)
   UPDATE  FROM bbd_special_interest frm
    SET frm.active_ind = false, frm.active_status_cd = combinedaway, frm.updt_cnt = (frm.updt_cnt+ 1),
     ...
     reqinfo->updt_task,
     frm.updt_dt_tm = cnvtdatetime(curdate,curtime3), frm.person_id = s_df_to_fk_id
    WHERE frm.special_interest_id=s_df_pk_id
    WITH nocounter
   ;end update
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "BBD_SPECIAL_INTEREST"
 ...
 SUBROUTINE upt_from(s_uf_pk_id,s_uf_to_fk_id)
   UPDATE  FROM bbd_special_interest frm
    ...
    WHERE frm.special_interest_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet = (icombinedet+ 1)
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
