CREATE PROGRAM dm2_shc_log_prop:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsl_err_status = c1 WITH protect, noconstant(" ")
 DECLARE dsl_cmd1 = vc WITH protect, noconstant(" ")
 DECLARE dsl_cmd2 = vc WITH protect, noconstant(" ")
 DECLARE dsl_cmd3 = vc WITH protect, noconstant(" ")
 DECLARE dsl_cmd4 = vc WITH protect, noconstant(" ")
 DECLARE dsl_temp_str = vc WITH protect, noconstant(" ")
 DECLARE dsl_beg_pos = i4 WITH protect, noconstant(0)
 DECLARE dsl_end_pos = i4 WITH protect, noconstant(0)
 DECLARE dsl_len = i4 WITH protect, noconstant(0)
 DECLARE dsl_srv_name = vc WITH protect, noconstant(" ")
 DECLARE dsl_loglevel = i4 WITH protect, noconstant(0)
 FREE RECORD dsl_srv
 RECORD dsl_srv(
   1 cnt = i4
   1 llevel_cnt = i4
 ...
 IF (check_logfile("DM2_SHC_LOG_PROP",".log","DM2_SHC_LOG_PROP LOG FILE")=0)
  SET dsl_err_status = "F"
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "System Health Check working on Logging Properties..."
 CALL disp_msg("",dm_err->logfile,0)
 ...
 ENDIF
 IF (cursys="AIX")
  SET dsl_cmd1 = concat("$cer_exe/alter_server -domain ",dshc_env_info->domain_name,
   " -verbose -display loglevel")
 ELSEIF (cursys="AXP")
  SET dsl_cmd1 = concat("mcr cer_exe:alter_server.exe -domain ",dshc_env_info->domain_name,
   " -verbose -display loglevel")
 ENDIF
 IF (dm2_push_dcl(dsl_cmd1)=0)
  SET dsl_err_status = "E"
 ELSE
  FREE DEFINE rtl2
  FREE SET file_loc
  SET logical file_loc value(dm_err->errfile)
  ...
  SELECT INTO "nl:"
   ...
   FROM rtl2t r
   DETAIL
    dsl_len = textlen(trim(r.line)), dsl_temp_str = r.line, dsl_beg_pos = findstring("server #",
     dsl_temp_str)
    IF (dsl_beg_pos > 0)
     dsl_temp_str = substring((dsl_beg_pos+ 8),dsl_len,dsl_temp_str), dsl_end_pos = findstring("has",
      dsl_temp_str)
     IF (dsl_end_pos > 0)
      dsl_srv_name = substring(1,(dsl_end_pos - 1),dsl_temp_str)
     ENDIF
    ENDIF
    dsl_beg_pos = findstring("loglevel = ",cnvtlower(dsl_temp_str))
    IF (dsl_beg_pos > 0)
     dsl_temp_str = substring((dsl_beg_pos+ 11),dsl_len,dsl_temp_str), dsl_loglevel = cnvtint(
      dsl_temp_str)
     IF (dsl_loglevel > 2)
      dsl_srv->llevel_cnt += 1, stat = alterlist(dsl_srv->llevel,dsl_srv->llevel_cnt), dsl_srv->
      llevel[dsl_srv->llevel_cnt].srv_name = dsl_srv_name
     ENDIF
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(concat("Parse out server loglevel.")) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dsl_err_status = "E"
  ENDIF
  IF (dsl_err_status=" "
   AND (dsl_srv->llevel_cnt > 0))
   SET dsl_err_status = "R"
  ENDIF
 ENDIF
 IF (dsl_err_status != "E")
  IF (cursys="AIX")
   SET dsl_cmd2 = concat("$cer_exe/alter_server -domain ",dshc_env_info->domain_name,
    " -verbose -display clevel")
  ELSEIF (cursys="AXP")
   SET dsl_cmd2 = concat("mcr cer_exe:alter_server.exe -domain ",dshc_env_info->domain_name,
    " -verbose -display clevel")
  ENDIF
  IF (dm2_push_dcl(dsl_cmd2)=0)
   SET dsl_err_status = "E"
  ELSE
   FREE DEFINE rtl2
   FREE SET file_loc
    ...
    DETAIL
     ...
     IF (dsl_beg_pos > 0)
      ...
      IF (dsl_end_pos > 0)
       dsl_srv_name = substring(1,(dsl_end_pos - 1),dsl_temp_str)
      ENDIF
      dsl_srv->clevel_cnt += 1, stat = alterlist(dsl_srv->clevel,dsl_srv->clevel_cnt), dsl_srv->
      clevel[dsl_srv->clevel_cnt].srv_name = dsl_srv_name
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(concat("Parse out server clevel.")) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dsl_err_status = "E"
   ENDIF
   IF (dsl_err_status=" "
    AND (dsl_srv->clevel_cnt > 0))
    SET dsl_err_status = "R"
   ENDIF
  ENDIF
  IF (dsl_err_status != "E")
   IF (cursys="AIX")
    SET dsl_cmd3 = concat("$cer_exe/alter_server -domain ",dshc_env_info->domain_name,
     ' -verbose -display "DEBUGLOGGING"')
   ELSEIF (cursys="AXP")
    SET dsl_cmd3 = concat("mcr cer_exe:alter_server.exe -domain ",dshc_env_info->domain_name,
     " -verbose -display debuglogging")
   ENDIF
   IF (dm2_push_dcl(dsl_cmd3)=0)
    SET dsl_err_status = "E"
   ELSE
    FREE DEFINE rtl2
    FREE SET file_loc
    ...
    SELECT INTO "nl:"
     ...
     DETAIL
      dsl_len = textlen(trim(r.line)), dsl_temp_str = r.line, dsl_beg_pos = findstring("server #",
       dsl_temp_str)
      IF (dsl_beg_pos > 0)
       dsl_temp_str = substring((dsl_beg_pos+ 8),dsl_len,dsl_temp_str), dsl_end_pos = findstring(
        "has",dsl_temp_str)
       IF (dsl_end_pos > 0)
        dsl_srv_name = substring(1,(dsl_end_pos - 1),dsl_temp_str)
       ENDIF
       dsl_srv->debug_cnt += 1, stat = alterlist(dsl_srv->debug,dsl_srv->debug_cnt), dsl_srv->debug[
       dsl_srv->debug_cnt].srv_name = dsl_srv_name
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(concat("Parse out server debugging property.")) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET dsl_err_status = "E"
    ENDIF
    IF (dsl_err_status=" "
     AND (dsl_srv->debug_cnt > 0))
     SET dsl_err_status = "R"
    ENDIF
   ENDIF
   IF (dsl_err_status != "E")
    IF (cursys="AIX")
     SET dsl_cmd4 = concat("$cer_exe/alter_server -domain ",dshc_env_info->domain_name,
      ' -verbose -display "DUMP"')
    ELSEIF (cursys="AXP")
     SET dsl_cmd4 = concat("mcr cer_exe:alter_server.exe -domain ",dshc_env_info->domain_name,
      " -verbose -display dump")
    ENDIF
    IF (dm2_push_dcl(dsl_cmd4)=0)
     SET dsl_err_status = "E"
    ELSE
     FREE DEFINE rtl2
     FREE SET file_loc
      ...
      DETAIL
       ...
       IF (dsl_beg_pos > 0)
        ...
        IF (dsl_end_pos > 0)
         dsl_srv_name = substring(1,(dsl_end_pos - 1),dsl_temp_str)
        ENDIF
        dsl_srv->dump_cnt += 1, stat = alterlist(dsl_srv->dump,dsl_srv->dump_cnt), dsl_srv->dump[
        dsl_srv->dump_cnt].srv_name = dsl_srv_name
       ENDIF
      WITH nocounter
     ;end select
     IF (check_error(concat("Parse out server dump property.")) != 0)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET dsl_err_status = "E"
     ENDIF
     IF (dsl_err_status=" "
      AND (dsl_srv->dump_cnt > 0))
      SET dsl_err_status = "R"
     ENDIF
    ENDIF
   ENDIF
  ENDIF
 ENDIF
 IF ( NOT (dsl_err_status IN ("E", "R")))
  SET dsl_err_status = "G"
 ENDIF
 IF (dshc_add_check_to_rs("SCP LOGGING PROPERTIES","SCP Logging Properties",0,dsl_err_status,
  dshc_ctf_millennium)=0)
  SET dsl_err_status = "F"
  GO TO exit_script
 ENDIF
 IF (dsl_err_status IN ("E", "G"))
  GO TO exit_script
 ENDIF
 IF ((dsl_srv->llevel_cnt > 0))
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = value(dsl_srv->llevel_cnt))
   DETAIL
    dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1, cnt = dshc->check[dshc->chk_cnt].desc_dtl_cnt, stat
     = alterlist(dshc->check[dshc->chk_cnt].desc_detail,cnt),
    dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Server w/ loglevel property higher then 2",
    dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "NOT_SET"
    IF (d.seq=1)
     dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text =
     "The following servers have logging set higher than 2.  Remove or lower the loglevel property and cycle the Servers."
    ELSE
     dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "NOT_SET"
    ENDIF
    dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 2, dshc->check[dshc->chk_cnt].desc_detail[cnt]
    .str_value = dsl_srv->llevel[d.seq].srv_name
   WITH nocounter
  ;end select
  IF (check_error(concat("Write out description details for loglevel higher than 2.")) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dsl_err_status = "E"
  ENDIF
 ENDIF
 IF ((dsl_srv->clevel_cnt > 0))
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = value(dsl_srv->clevel_cnt))
   DETAIL
    dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1, cnt = dshc->check[dshc->chk_cnt].desc_dtl_cnt, stat
     = alterlist(dshc->check[dshc->chk_cnt].desc_detail,cnt),
    dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Server w/ clevel property set", dshc->check[
    dshc->chk_cnt].desc_detail[cnt].value_prefix = "NOT_SET"
    IF (d.seq=1)
     dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = concat(
      'The following servers have "clevel" properties set.  This stops the "COUT" logging to the CMB_TEMP/*.OUT files.',
      "These properties need to be removed and servers cycled.")
    ELSE
     dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "NOT_SET"
    ENDIF
    dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 2, dshc->check[dshc->chk_cnt].desc_detail[cnt]
    .str_value = dsl_srv->clevel[d.seq].srv_name
   WITH nocounter
  ;end select
  IF (check_error(concat("Write out description details for clevel.")) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dsl_err_status = "E"
  ENDIF
 ENDIF
 IF ((dsl_srv->debug_cnt > 0))
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = value(dsl_srv->debug_cnt))
   DETAIL
    dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1, cnt = dshc->check[dshc->chk_cnt].desc_dtl_cnt, stat
     = alterlist(dshc->check[dshc->chk_cnt].desc_detail,cnt),
    dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Server w/ debuglogging* properties set", dshc
    ->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "NOT_SET"
    IF (d.seq=1)
     dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = concat(
      "The following servers have properties set beginning with 'DEBUGLOGGING'.",
      "This stops the server from dumping request and/or reply handles in the CMB_TEMP/*.out files.",
      "These properties need to be removed and servers cycled.")
    ELSE
     dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "NOT_SET"
    ENDIF
    dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 2, dshc->check[dshc->chk_cnt].desc_detail[cnt]
    .str_value = dsl_srv->debug[d.seq].srv_name
   WITH nocounter
  ;end select
  IF (check_error(concat("Write out description details for debuglogging.")) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dsl_err_status = "E"
  ENDIF
 ENDIF
 IF ((dsl_srv->dump_cnt > 0))
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = value(dsl_srv->dump_cnt))
   DETAIL
    dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1, cnt = dshc->check[dshc->chk_cnt].desc_dtl_cnt, stat
     = alterlist(dshc->check[dshc->chk_cnt].desc_detail,cnt),
    dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Server w/ dump* properties set", dshc->check[
    dshc->chk_cnt].desc_detail[cnt].value_prefix = "NOT_SET"
    IF (d.seq=1)
     dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = concat(
      "The following servers have properties set beginning with 'DUMP'.",
      "This stops the server from dumping request and/or reply handles in the CMB_TEMP/*.out files.",
      "These properties need to be removed and servers cycled.")
    ELSE
     dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "NOT_SET"
    ENDIF
    dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 2, dshc->check[dshc->chk_cnt].desc_detail[cnt]
    .str_value = dsl_srv->dump[d.seq].srv_name
   WITH nocounter
  ;end select
  IF (check_error(concat("Write out description details for dump property.")) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dsl_err_status = "E"
  ENDIF
 ENDIF
 SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
  "In order to maintain optimal performance, it is important that ",
  "application servers don't have unnecessary logging or debugging turned on.")
#exit_script
 IF ((dm_err->debug_flag >= 5))
  CALL echorecord(dshc)
  CALL echorecord(dm_err)
 ENDIF
 IF (dsl_err_status="F")
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "EXITING DM2_SHC_LOG_PROP WITH ERRORS"
 ELSEIF ((dm_err->err_ind=1))
  SET dm_err->err_ind = 0
  SET dm_err->eproc = "EXITING DM2_SHC_LOG_PROP WITH ERRORS"
 ELSE
  SET dm_err->eproc = "DM2_SHC_LOG_PROP COMPLETED SUCCESSFULLY"
 ENDIF
 CALL final_disp_msg("DM2_SHC_LOG_PROP")
 FREE RECORD dsl_srv
END GO
