CREATE PROGRAM bhs_rpt_bri_export2:dba
 DECLARE mf_beg_dt_tm = f8 WITH protect, constant(cnvtdatetime("30-aug-2022 00:00:00"))
 DECLARE mf_end_dt_tm = f8 WITH protect, constant(cnvtdatetime("30-aug-2022 23:59:59"))
 ...
 DECLARE mf_baystateradimaging_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",34,
   "BAYSTATERADIMAGING"))
 DECLARE mf_brimobile_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",34,"BRIMOBILE"))
 DECLARE ms_filename = vc WITH protect, constant(concat("bridemo","_",format(cnvtdatetime((curdate -
 ...
 SELECT INTO "nl:"
  ...
  PLAN (rr
   ...
   WHERE e.encntr_id=orad.encntr_id
    ...
    AND  NOT (e.med_service_cd IN (mf_baystateradimaging_cd, mf_brimobile_cd)))
   JOIN (p
 ...
 SELECT INTO value(ms_filename)
  ...
  WITH nocounter, maxcol = 3000, format,
   noheading, maxrow = 1
 ;end select
 SET stat = bhs_ftp_cmd(ms_ftp_cmd,"transfer.baystatehealth.org",'"bhs\cisftp"',"C!sftp01",ms_loc_dir,
  "ciscore/bri_extract")
 IF (gl_bhs_prod_flag=1)
  SET stat = bhs_ftp_cmd(ms_ftp_cmd,"transfer.baystatehealth.org",'"bhs\cisftp"',"C!sftp01",
   ms_loc_dir,
   "cisraddemop")
 ELSE
  SET stat = bhs_ftp_cmd(ms_ftp_cmd,"transfer.baystatehealth.org",'"bhs\cisftp"',"C!sftp01",
