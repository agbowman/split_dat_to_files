CREATE PROGRAM aps_rdm_move_cs_comments:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "aps_rdm_move_cs_comments"
 ...
 CALL logmsg("Getting ready to begin reading for case_specimen comment.",((e_dispmsg+ e_rdmstatus)+
  e_logtofile))
 SELECT INTO "nl:"
 ...
 WHILE (cs_cntr >= 1)
   ...
   IF (curqual != value(cs_commit_recs_whole))
    ROLLBACK
    CALL logmsg("Failed to update case_specimen table.",((e_dispmsg+ e_logtofile)+ e_rdmstatus))
    SET sprogramfailed = "T"
    GO TO exit_script
   ENDIF
   SET sprogramfailed = "F"
