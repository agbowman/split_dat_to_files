CREATE PROGRAM dm_pcmb_patient_event:dba
 ...
 DECLARE v_cust_count_to = i4 WITH protect, noconstant(0)
 DECLARE v_cust_loopcount_from = i4 WITH protect, noconstant(0)
 DECLARE to_has_active_biometric_ind = i2 WITH protect, noconstant(0)
 DECLARE biometriceventtypecd = f8 WITH protect, constant(uar_get_code_by("MEANING",4002773,
   "BIOENRSTAT"))
 DECLARE from_has_active_biometric_ind = i2 WITH protect, noconstant(0)
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_PCMB_PATIENT_EVENT"
 ...
 SELECT
  ...
  DETAIL
   ...
   rreclist->from_rec[v_cust_count_from].event_type_cd = frm.event_type_cd, rreclist->from_rec[
   v_cust_count_from].encntr_id = frm.encntr_id
   IF (frm.event_type_cd=biometriceventtypecd
    AND frm.active_ind=1)
    from_has_active_biometric_ind = 1
   ENDIF
  FOOT REPORT
 ...
 IF (v_cust_count_from > 0
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    ENDIF
    rreclist->to_rec[v_cust_count_to].to_id = tu.patient_event_id, rreclist->to_rec[v_cust_count_to].
    active_ind = tu.active_ind, rreclist->to_rec[v_cust_count_to].active_status_cd = tu
    .active_status_cd,
    rreclist->to_rec[v_cust_count_to].event_type_cd = tu.event_type_cd
    IF (tu.event_type_cd=biometriceventtypecd
     AND tu.active_ind=1)
     to_has_active_biometric_ind = 1
    ENDIF
   FOOT REPORT
    IF (mod(v_cust_count_to,10) != 0)
     stat = alterlist(rreclist->to_rec,v_cust_count_to)
    ENDIF
 ...
 ELSE
  IF ((rev_cmb_request->reverse_ind=0))
   FOR (v_cust_loopcount_from = 1 TO v_cust_count_from)
     IF ((((rreclist->from_rec[v_cust_loopcount_from].event_type_cd != biometriceventtypecd)) OR ((
     rreclist->from_rec[v_cust_loopcount_from].event_type_cd=biometriceventtypecd)
      AND to_has_active_biometric_ind))
      AND (rreclist->from_rec[v_cust_loopcount_from].active_ind=1)
      AND (rreclist->from_rec[v_cust_loopcount_from].encntr_id=0))
  ...
  ELSE
   FOR (v_cust_loopcount_to = 1 TO v_cust_count_to)
     IF ((((rreclist->to_rec[v_cust_loopcount_to].event_type_cd != biometriceventtypecd)) OR ((
     rreclist->to_rec[v_cust_loopcount_to].event_type_cd=biometriceventtypecd)
      AND from_has_active_biometric_ind))
      AND (rreclist->to_rec[v_cust_loopcount_to].active_ind=1))
      IF (del_to(rreclist->to_rec[v_cust_loopcount_to].to_id,rreclist->to_rec[v_cust_loopcount_to].
       active_ind,rreclist->to_rec[v_cust_loopcount_to].active_status_cd)=0)
       GO TO exit_sub
      ENDIF
     ENDIF
   ENDFOR
  ...
  i4)
   UPDATE  FROM patient_event frm
    SET frm.active_ind = false, frm.active_status_cd = combinedaway, frm.updt_cnt = (frm.updt_cnt+ 1),
     ...
     reqinfo->updt_task,
     frm.updt_dt_tm = cnvtdatetime(sysdate), frm.person_id = s_df_to_fk_id
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (del_to(s_df_pk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) =i4)
   UPDATE  FROM patient_event frm
    SET frm.active_ind = false, frm.active_status_cd = combinedaway, frm.updt_cnt = (frm.updt_cnt+ 1),
     frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo->updt_applctx, frm.updt_task =
     reqinfo->updt_task,
   ...
   SET request->xxx_combine_det[icombinedet].entity_id = s_df_pk_id
   SET request->xxx_combine_det[icombinedet].entity_name = "PATIENT_EVENT"
   SET request->xxx_combine_det[icombinedet].attribute_name = "PATIENT_EVENT_ID"
   SET request->xxx_combine_det[icombinedet].prev_active_ind = s_df_prev_act_ind
   SET request->xxx_combine_det[icombinedet].prev_active_status_cd = s_df_prev_act_status
   IF (curqual=0)
    SET failed = delete_error
 ...
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i4)
   UPDATE  FROM patient_event frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     s_uf_to_fk_id
    WHERE frm.patient_event_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
 ...
 SUBROUTINE initialize_hist_id(null)
  IF (v_hist_init_ind=0)
   ...
   SET dcipht_request->transaction_reason_txt = "DM_PCMB_PATIENT_EVENT"
