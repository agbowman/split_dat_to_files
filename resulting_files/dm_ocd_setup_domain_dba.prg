CREATE PROGRAM dm_ocd_setup_domain:dba
 FREE RECORD radm
 RECORD radm(
   1 qual[*]
     2 tname = vc
 ...
 IF (currdb="ORACLE")
  ...
  SET dm2_server_link->server_name = docd_reply->con_str
  IF (check_dm2tools_nicknames(0)=0)
   SET docd_reply->status = "F"
   SET docd_reply->err_msg = dm_err->emsg
   GO TO end_program
  ENDIF
 ELSEIF (currdb="DB2UDB")
  ...
  ENDIF
  EXECUTE dm2_create_admin_nicknames "*", dm2_install_schema->cdba_p_word
  IF ((dm_err->err_ind=1))
   SET docd_reply->status = "F"
   GO TO end_program
  ENDIF
 ...
 ELSE
  CALL echo("****************************************************************")
  CALL echo("*")
  CALL echo("Domain configuration for OCD installation successful.")
  CALL echo("*")
  CALL echo("****************************************************************")
 ENDIF
END GO
