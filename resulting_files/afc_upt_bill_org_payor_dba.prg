CREATE PROGRAM afc_upt_bill_org_payor:dba
 DECLARE versionnbr = vc
 SET versionnbr = "006"
 CALL echo(build("AFC_UPT_BILL_ORG_PAYOR Version: ",versionnbr))
 ...
 DECLARE codecnt = i4
 SET codecnt = 1
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,codecnt,wl_standard_cd)
 DECLARE code_set = i4
 DECLARE cdf_meaning = c12
 ...
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,cntactive,tiergroup)
 SET tgstring->tg_string = build(tgstring->tg_string,cnvtstring(tiergroup))
 IF (cntactive > 1)
