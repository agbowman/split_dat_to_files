CREATE PROGRAM bhs_rpt_tran_tag_amstest1:dba
 PROMPT
 ...
 IF (( $BDATE="OPSJOB"))
  SET beg_date_qual = cnvtdatetime((curdate - 7),0)
  SET end_date_qual = cnvtdatetime(curdate,235959)
 ELSE
  SET beg_date_qual = cnvtdatetime(cnvtdate2( $BDATE,"DD-MMM-YYYY"),0)
  SET end_date_qual = cnvtdatetime(cnvtdate2( $EDATE,"DD-MMM-YYYY"),235959)
 ENDIF
 ...
 ELSEIF (datetimediff(end_date_qual,beg_date_qual) < 0)
  ...
  GO TO exit_program
 ENDIF
 DECLARE ms_email_list = vc WITH noconstant( $EMAILADD)
 ...
 DECLARE mf_bloodproductcomment = f8 WITH constant(validatecodevalue("DISPLAYKEY",72,
 ...
 DECLARE mf_bloodproducttransfused = f8 WITH constant(validatecodevalue("DISPLAYKEY",72,
 ...
 protect
 DECLARE mf_factorviianovoseven = f8 WITH constant(validatecodevalue("DISPLAYKEY",72,"FACTORVIIA")),
 protect
 DECLARE mf_factorviiihumanhumatepvol = f8 WITH constant(validatecodevalue("DISPLAYKEY",72,
   "FACTORVIIIVOL")), protect
 DECLARE mf_3factorprothrombincomplexconcentrate = f8 WITH constant(validatecodevalue("DISPLAY",72,
   "factor IX complex")), protect
 DECLARE mf_factorixrecombinant = f8 WITH constant(validatecodevalue("DISPLAYKEY",72,"FACTORIXVOL")),
 protect
 ...
 DECLARE mf_rhimmuneglobuliniv = f8 WITH constant(validatecodevalue("DISPLAYKEY",72,
   "RHIMMUNEGLOBULINIV")), protect
 DECLARE mf_rhimmuneglobulinim = f8 WITH constant(validatecodevalue("DISPLAYKEY",72,
 ...
 DECLARE mf_unit_id_lot_num_adtnl_unit_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "UNITIDNUMBERLOTNUMBER2"))
 DECLARE mf_unit_id_lot_num_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "UNITIDNUMBERLOTNUMBER"))
 DECLARE mf_red_cell_bag_lot_num_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "REDCELLBAGLOTNUMBER"))
 ...
 DECLARE bloodproducttransfused = vc WITH noconstant(" ")
 DECLARE albuminvo = vc WITH noconstant(" ")
 DECLARE cryoprecipitate = vc WITH noconstant(" ")
 DECLARE factorviianovoseven = vc WITH noconstant(" ")
 DECLARE factorviiihumanvol = vc WITH noconstant(" ")
 DECLARE 3factorprothrombincomplexconcentrate = vc WITH noconstant(" ")
 DECLARE factorixrecombinant = vc WITH noconstant(" ")
 ...
 DECLARE rhimmuneglobulinim = vc WITH noconstant(" ")
 DECLARE rhimmuneglobuliniv = vc WITH noconstant(" ")
 ...
 DECLARE ms_data_type = vc WITH noconstant(" ")
 DECLARE ms_fac = vc WITH noconstant(" ")
 DECLARE ml_tmp_cnt = i4 WITH noconstant(0)
 SET ms_data_type = reflect(parameter(2,0))
 CALL echo(ms_data_type)
 ...
 SELECT INTO "NL:"
  ...
  PLAN (dfr
   ...
   WHERE el.encntr_id=ea.encntr_id
    ...
    AND parser(ms_fac)) OR (nurseunitcd=99999999
    AND parser(ms_fac))) )) )
   JOIN (e
  ...
  HEAD nurse_unit
   i_patient_cnt = 0, i_nurseunit_cnt = (i_nurseunit_cnt+ 1),
  ...
  FOOT  nurse_unit
   ...
   IF (size(forms->nurseunit[i_nurseunit_cnt].patientslist,5) <= 0)
    i_nurseunit_cnt = (i_nurseunit_cnt - 1),
    CALL alterlist(forms->nurseunit,i_nurseunit_cnt)
   ELSE
    CALL alterlist(forms->nurseunit[i_nurseunit_cnt].patientslist,i_patient_cnt)
   ENDIF
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   ...
   mf_transfusiontimetocompletion))
   JOIN (pr
  ...
  HEAD ce.parent_event_id
   ...
   CALL echo(forms->nurseunit[d.seq].f_nurseunit_cd),
   CALL alterlist(forms->nurseunit[d.seq].patientslist[d1.seq].forminstanceslist[i_instance_cnt],
   i_instance_cnt), forms->nurseunit[d.seq].patientslist[d1.seq].forminstanceslist[i_instance_cnt].
   f_parent_event_id = ce.parent_event_id,
  ...
  HEAD ce2.event_cd
   ...
   ELSEIF (ce2.event_cd=mf_bloodproducttransfused)
    forms->nurseunit[d.seq].patientslist[d1.seq].forminstanceslist[i_instance_cnt].
   ...
   ELSEIF (ce2.event_cd=mf_factorviianovoseven)
    forms->nurseunit[d.seq].patientslist[d1.seq].forminstanceslist[i_instance_cnt].
    s_factorviianovoseven = s_tran_result, i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_factorviiihumanhumatepvol)
    forms->nurseunit[d.seq].patientslist[d1.seq].forminstanceslist[i_instance_cnt].
    s_factorviiihumanhumatepvol = s_tran_result, i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_3factorprothrombincomplexconcentrate)
    forms->nurseunit[d.seq].patientslist[d1.seq].forminstanceslist[i_instance_cnt].
    s_3factorprothrombincomplexconcentrate = s_tran_result, i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_factorixrecombinant)
    forms->nurseunit[d.seq].patientslist[d1.seq].forminstanceslist[i_instance_cnt].
    s_factorixrecombinant = s_tran_result, i_infused_cnt = (i_infused_cnt+ 1)
   ...
   ELSEIF (ce2.event_cd=mf_rhimmuneglobulinim)
    forms->nurseunit[d.seq].patientslist[d1.seq].forminstanceslist[i_instance_cnt].
    s_rhimmuneglobulinim = s_tran_result, i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_rhimmuneglobuliniv)
    forms->nurseunit[d.seq].patientslist[d1.seq].forminstanceslist[i_instance_cnt].
    s_rhimmuneglobuliniv = s_tran_result, i_infused_cnt = (i_infused_cnt+ 1)
   ...
   ELSEIF (ce2.event_cd IN (mf_bloodproductcomment))
    CALL echo("MF_BLOODPRODUCTCOMMENT start"), forms->nurseunit[d.seq].patientslist[d1.seq].
    forminstanceslist[i_instance_cnt].s_bloodproductcomment = ce2.result_val,
   ...
   ELSEIF (ce2.event_cd IN (mf_unit_id_lot_num_adtnl_unit_cd))
    forms->nurseunit[d.seq].patientslist[d1.seq].forminstanceslist[i_instance_cnt].
    s_unit_id_lot_num_adtnl_unit = ce2.result_val
   ELSEIF (ce2.event_cd IN (mf_red_cell_bag_lot_num_cd))
    forms->nurseunit[d.seq].patientslist[d1.seq].forminstanceslist[i_instance_cnt].
    s_red_cell_bag_lot_num = ce2.result_val
