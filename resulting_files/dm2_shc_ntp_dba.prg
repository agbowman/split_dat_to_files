CREATE PROGRAM dm2_shc_ntp:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 IF ((validate(ntp->peercnt,- (1))=- (1))
  AND (validate(ntp->peercnt,- (2))=- (2)))
  RECORD ntp(
    1 peercnt = i4
    1 peers[*]
      2 host = vc
  ...
  SET ntp->peercnt = 0
  SET ntp->no_privilege = 0
  SET ntp->not_synchronized = 0
  SET ntp->timezone = "NOT_SET"
  SET ntp->on_startup = 0
  SET ntp->srvc_running = 0
  SET ntp->multinet_ind = 0
 ENDIF
 DECLARE dsn_fatal_err_ind = i2 WITH private, noconstant(0)
 DECLARE dsn_status = c1 WITH protect, noconstant("X")
 DECLARE dsn_getfile = vc WITH protect, noconstant("NOT_SET")
 DECLARE dsn_outfile = vc WITH protect, noconstant("NOT_SET")
 DECLARE dsn_extension = vc WITH protect, noconstant(".ksh")
 DECLARE dsn_prev_errfile = vc WITH protect, noconstant("NOT_SET")
 DECLARE dsn_exec_cmd = vc WITH protect, noconstant("NOT_SET")
 DECLARE cnt = i4 WITH protect, noconstant(0)
 DECLARE dsn_max_width = i4 WITH protect, noconstant(0)
 DECLARE dsn_pad = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2_shc_ntp",".log","SYSTEM HEALTH CHECK NTP LOG FILE")=0)
  SET dstc_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking NTP (Network Time Protocol) configuration."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 ENDIF
 IF (cursys="AXP")
  SET dsn_extension = ".com"
 ENDIF
 IF (dsn_status != "E")
  IF (get_unique_file("dm2_shc_ntp_",dsn_extension)=0)
   SET dsn_status = "E"
  ENDIF
  SET dsn_getfile = dm_err->unique_fname
  SET dsn_outfile = replace(dsn_getfile,dsn_extension,".dat",0)
 ENDIF
 IF (dsn_status != "E")
  SELECT INTO trim(dsn_getfile)
   DETAIL
    IF (cursys="AXP")
     col 0, "$ set noon", row + 1,
     col 0, "$ set term/width = 132", row + 1,
     col 0, "$ multinet_ind = 0", row + 1,
     col 0, "$ pipe show system | search sys$input MULTINET_SERVER", row + 1,
     col 0, '$ if $STATUS .eqs. "%X10000001"', row + 1,
     col 0, "$ then", row + 1,
     col 0, "$    multinet_ind = 1", row + 1,
     col 0, "$ endif", row + 1,
     col 0, "$ if multinet_ind .eqs. 1", row + 1,
     col 0, "$ then", row + 1,
     col 0, '$    write sys$output "IPSTACK = MULTINET"', row + 1,
     col 0, '$    multinet netcontrol ntp noop  !search for "OK: NTP Server Noop"', row + 1,
     col 0, '$    write sys$output "BEGIN NTPTRACE"', row + 1,
     col 0, "$    ntptrace ", dshc_env_info->node_name,
     row + 1, col 0, '$    write sys$output "END NTPTRACE"',
     row + 1, col 0, "$ else",
     row + 1, col 0, '$    write sys$output "IPSTACK = TCPIP"',
     row + 1, col 0, "$    pipe show system | search sys$input ntp",
     row + 1, col 0, '$    if $STATUS .eqs. "%X10000001"',
     row + 1, col 0, "$    then",
     row + 1, col 0, '$       write sys$output "NTP services RUNNING"',
     row + 1, col 0, "$    else",
     row + 1, col 0, '$       write sys$output "NTP services NOT RUNNING"',
     row + 1, col 0, "$    endif",
     row + 1, col 0, "$    @sys$manager:tcpip$define_commands",
     row + 1, col 0, '$    write sys$output "BEGIN NTPTRACE"',
     row + 1, col 0, "$    ntptrace ",
     dshc_env_info->node_name, row + 1, col 0,
     '$    write sys$output "END NTPTRACE"', row + 1, col 0,
     "$ endif", row + 1, col 0,
     "$ check_time :==$cer_exe:check_time.exe", row + 1, col 0,
     "$ check_time", row + 1, col 0,
     dshc_env_info->domain_user_name, row + 1, col 0,
     dshc_env_info->domain_name, row + 1, col 0,
     dshc_env_info->domain_user_pwd
    ELSE
     col 0, "NTP_SRV=`ps -ef | grep xntpd | grep -v grep | wc -l`", row + 1,
     col 0, 'if [[ "$NTP_SRV" -gt 0 ]]', row + 1,
     col 0, "then", row + 1,
     col 0, '   echo "NTP services RUNNING"', row + 1,
     col 0, '   echo "BEGIN NTPTRACE"', row + 1,
     col 0, "   ntptrace ", dshc_env_info->node_name,
     row + 1, col 0, '   echo "END NTPTRACE"',
     row + 1, col 0, "else",
     row + 1, col 0, '   echo "NTP services NOT RUNNING"',
     row + 1, col 0, "fi",
     row + 1, col 0, 'STARTUP=`grep "start /usr/sbin/xntpd" /etc/rc.tcpip | grep "#" | wc -l`',
     row + 1, col 0, 'if [[ "$STARTUP" -gt 0 ]]',
     row + 1, col 0, "then",
     row + 1, col 0, '   echo "NTP NOT ON STARTUP"',
     row + 1, col 0, "else",
     row + 1, col 0, '   echo "NTP ON STARTUP"',
     row + 1, col 0, "fi",
     row + 1, col 0, "$cer_exe/check_time <<!",
     row + 1, col 0, dshc_env_info->domain_user_name,
     row + 1, col 0, dshc_env_info->domain_name,
     row + 1, col 0, dshc_env_info->domain_user_pwd,
     row + 1, col 0, "!"
    ENDIF
 ...
 ENDIF
 IF (cursys="AXP")
  SET dsn_exec_cmd = concat("@CCLUSERDIR:",dsn_getfile)
 ELSE
  SET dsn_exec_cmd = concat("chmod ugo+x $CCLUSERDIR/",dsn_getfile)
  IF (dm2_push_dcl(dsn_exec_cmd)=0)
   SET dsn_status = "E"
  ENDIF
  SET dsn_exec_cmd = concat("$CCLUSERDIR/",dsn_getfile)
 ENDIF
 SET dsn_prev_errfile = dm_err->errfile
 SET dm_err->errfile = dsn_outfile
 SET dm_err->disp_dcl_err_ind = 0
 IF (dm2_push_dcl(dsn_exec_cmd)=0
 ...
 ENDIF
 IF (dsn_status != "E")
  SET dm_err->eproc = "Loading NTP output."
  CALL disp_msg("",dm_err->logfile,0)
  FREE DEFINE rtl2
  FREE SET file_loc
  SET logical file_loc value(dsn_outfile)
  DEFINE rtl2 "file_loc"
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   SET dsn_status = "E"
 ...
 ENDIF
 IF (dsn_status != "E")
  SET dm_err->eproc = "Parsing NTP output."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "nl:"
   r.line
   FROM rtl2t r
   ...
   DETAIL
    IF (cursys="AXP")
     IF (findstring("MULTINET",r.line) > 0)
      ntp->multinet_ind = 1
     ELSEIF (findstring("IPSTACK = TCPIP",r.line) > 0)
      ntp->multinet_ind = - (1)
     ELSEIF (findstring("BEGIN NTPTRACE",r.line) > 0)
      rpt_beg_ntptrace = 1
     ELSEIF (findstring("END NTPTRACE",r.line) > 0)
      rpt_beg_ntptrace = 0
     ENDIF
     IF (((findstring("NTP services RUNNING",r.line) > 0) OR (findstring("OK: NTP Server Noop",r.line
      ) > 0)) )
      ntp->srvc_running = 1
     ENDIF
     IF (rpt_beg_ntptrace=1)
      IF (((findstring("Not Synchronized",r.line) > 0) OR (findstring("Timeout",r.line) > 0)) )
       ntp->not_synchronized = 1
      ENDIF
      IF (findstring(":",r.line) > 0)
       ntp->peercnt += 1, stat = alterlist(ntp->peers,ntp->peercnt), ntp->peers[ntp->peercnt].host =
       substring(1,(findstring(":",r.line) - 1),r.line)
       IF (size(ntp->peers[ntp->peercnt].host) > dsn_max_width)
        dsn_max_width = size(ntp->peers[ntp->peercnt].host)
       ENDIF
      ENDIF
      IF (findstring("stratum ",r.line) > 0)
       ntp->peers[ntp->peercnt].stratum = substring((findstring("stratum ",r.line)+ 8),(findstring(
         ",",r.line) - (findstring("stratum ",r.line)+ 8)),r.line)
      ENDIF
     ENDIF
     IF (findstring("Time Zone:",r.line) > 0)
      ntp->timezone = substring(10,size(trim(r.line,4)),trim(r.line,4))
     ENDIF
    ELSE
     IF (findstring("NTP services RUNNING",r.line) > 0)
      ntp->srvc_running = 1
     ELSEIF (findstring("NTP ON STARTUP",r.line) > 0)
      ntp->on_startup = 1
     ELSEIF (findstring("BEGIN NTPTRACE",r.line) > 0)
      rpt_beg_ntptrace = 1
     ELSEIF (findstring("END NTPTRACE",r.line) > 0)
     ...
     IF (rpt_beg_ntptrace=1)
      IF (((findstring("Not Synchronized",r.line) > 0) OR (findstring("Timeout",r.line) > 0)) )
       ntp->not_synchronized = 1
      ELSEIF (findstring("Execute permission denied.",r.line) > 0)
       ntp->no_privilege = 1
      ENDIF
      IF (findstring(":",r.line) > 0)
       ntp->peercnt += 1, stat = alterlist(ntp->peers,ntp->peercnt), ntp->peers[ntp->peercnt].host =
      ...
      IF (findstring("stratum ",r.line) > 0)
       ntp->peers[ntp->peercnt].stratum = substring((findstring("stratum ",r.line)+ 8),(findstring(
         ",",r.line) - (findstring("stratum ",r.line)+ 8)),r.line)
      ENDIF
     ELSEIF (findstring("Time Zone:",r.line) > 0)
      ntp->timezone = substring(10,size(trim(r.line,4)),trim(r.line,4))
     ENDIF
    ENDIF
   WITH nocounter
 ...
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(ntp)
 ENDIF
 IF ((((dm_err->err_ind=1)) OR (dsn_status="E")) )
  SET dsn_status = "E"
 ELSE
  SET dsn_status = "R"
 ENDIF
 IF (dshc_add_check_to_rs("NTP Configuration","Network Time Protocol (NTP) Configuration",0,
  dsn_status,dshc_ctf_millennium)=0)
  SET dsn_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 IF (dsn_status="E")
  GO TO exit_script
 ...
 SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
  "As time becomes a more critical piece of data in Cerner Millennium transactions, configuring multinode",
  " environments to synchronize their system clocks will become critical to transactional stability.  Cerner",
  " recommends the use of NTP (Network Time Protocol) to keep the back-end host system time in sync with other",
  " back-end hosts, front-end PCs, PC fileservers, and Citrix servers used in the Cerner Millennium solution.",
  "  Instructions for Configuring NTP can be found on the Technologies tab of the Cerner Millennium",
  " Support Guide, located on CKN.")
 ...
 IF (cursys="AIX")
  ...
  SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].type = 2
  SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].value_prefix =
  "NTP Service Autostarts on Boot:  "
  IF ((ntp->on_startup=1))
   SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].str_value =
   "Yes"
  ELSE
 ...
 SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].value_prefix =
 "Time Zone Setting:  "
 SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].str_value = ntp
 ->timezone
 IF ((ntp->not_synchronized=1))
  SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
  SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,dshc->check[dshc->chk_cnt].desc_dtl_cnt
   )
  SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].dtl_desc_text
   =
  "NTP does not appear to be synchronizing time to another host.  Please review your server configuration."
 ELSEIF ((ntp->no_privilege=1))
  SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
  SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,dshc->check[dshc->chk_cnt].desc_dtl_cnt
   )
  SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].dtl_desc_text
   =
  "Current user does not have privilege to run ntptrace.  Contact your system administrator for details."
 ELSE
  FOR (cnt = 1 TO ntp->peercnt)
    SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
    ...
    SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].dtl_desc_text
     = concat(
     "The following list represents the tree of servers from which this host gets system time,",
     " and the stratum at which each server offers its time.  Please review to verify that",
     " each server is offering time at a low stratum.  By convention, a stratum of 10 or",
     " higher indicates the host cannot guarantee the accuracy of its time.")
    SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].value_prefix
     = "NOT_SET"
    SET dsn_pad = ((dsn_max_width - size(ntp->peers[cnt].host))+ 2)
    SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].str_value =
    concat(ntp->peers[cnt].host,fillstring(value(dsn_pad)," "),ntp->peers[cnt].stratum)
  ENDFOR
 ENDIF
#exit_script
 SET dm_err->eproc = "Finished checking for NTP configuration."
 CALL final_disp_msg("dm2_shc_ntp")
 IF (dsn_fatal_err_ind=0)
  SET dm_err->err_ind = 0
 ENDIF
END GO
