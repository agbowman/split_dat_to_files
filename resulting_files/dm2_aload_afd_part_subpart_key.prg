CREATE PROGRAM dm2_aload_afd_part_subpart_key
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_afd_part_subpart_key",".log","dm2_aload_afd_part_subpart_key")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_afd_part_subpart_key"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daapsk_pkg = i4 WITH protect, noconstant( $1)
 DECLARE daapsk_file = vc WITH protect, noconstant( $2)
 DECLARE daapsk_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE daapsk_batch = i4 WITH protect, noconstant(0)
 DECLARE daapsk_rows_returned = i4 WITH protect, noconstant(1)
 DECLARE daapsk_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(daapsk_pkg,daapsk_file_loc_ret,daapsk_batch)=0)
  GO TO exit_program
 ENDIF
 IF (daapsk_batch=0)
  SET daapsk_batch = 5000
 ENDIF
 IF (daapsk_expected_rows > 0)
  EXECUTE dm2_dbimport concat(daapsk_file_loc_ret,daapsk_file), "dm2_aload_sch_part_subpart_key",
  daapsk_batch
  CALL echo(concat(daapsk_file_loc_ret,daapsk_file))
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Remove extraneous DM_AFD_PART_SUBPART_KEYCOLS rows"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  DELETE  FROM dm_afd_part_subpart_keycols dapsk
   WHERE dapsk.alpha_feature_nbr=daapsk_pkg
    AND dapsk.owner=currdbuser
    AND  NOT ( EXISTS (
   (SELECT
    "x"
    FROM dm_afd_tables dat
    WHERE dat.alpha_feature_nbr=dapsk.alpha_feature_nbr
     AND dat.owner=dapsk.owner
     AND dat.table_name=dapsk.table_name
     AND dat.updt_dt_tm=dapsk.updt_dt_tm)))
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_afd_part_subpart_key completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_afd_part_subpart_key")
END GO
