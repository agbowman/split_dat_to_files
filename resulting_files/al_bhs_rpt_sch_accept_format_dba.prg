CREATE PROGRAM al_bhs_rpt_sch_accept_format:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  HEAD p.person_id
   ...
   patient_qual_cnt].birth_dt_tm = p.birth_dt_tm, t_rec->qual[t_rec->qual_cnt].patient_qual[t_rec->
   qual[t_rec->qual_cnt].patient_qual_cnt].birth_formatted = format(p.birth_dt_tm,"MM/DD/YYYY;;D"),
  ...
  DETAIL
   FOR (i = 1 TO t_rec->qual_cnt)
     ...
     FOR (j = 1 TO t_rec->qual[i].patient_qual_cnt)
       ...
       "{ENDB}",
       CALL print(calcpos(138,y_pos)), t_rec->qual[i].patient_qual[j].home_phone,
       row + 1, col + 0,
