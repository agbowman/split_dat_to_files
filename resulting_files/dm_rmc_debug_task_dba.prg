CREATE PROGRAM dm_rmc_debug_task:dba
 ...
 SUBROUTINE daf_is_not_blank(dinb_str)
  ...
  RETURN(false)
 END ;Subroutine
 IF (check_logfile("dm_rmc_debug_task",".log","DM_RMC_DEBUG_TASK LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 DECLARE drdt_task_queue_id = f8 WITH protect, noconstant
 DECLARE drdt_task_request = vc WITH protect, noconstant
 DECLARE drdt_task_reply = vc WITH protect, noconstant
 DECLARE drdt_task_name = vc WITH protect, noconstant
 DECLARE drdt_loop = i4 WITH protect, noconstant
 FREE RECORD drdt_parsed_task_data
 RECORD drdt_parsed_task_data(
   1 request_definition = vc
   1 var_qual[*]
     2 variable_declaration = vc
   1 var_cnt = i2
 ...
 IF (reflect(parameter(1,0)) != "I*"
  ...
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Expected syntax: dm_rmc_debug_task <dm_refchg_task_queue_id> go"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET drdt_task_queue_id =  $1
 SELECT INTO "NL:"
  FROM dm_refchg_task_queue d
  WHERE dm_refchg_task_queue_id=drdt_task_queue_id
  DETAIL
   drdt_task_request = d.task_request, drdt_task_reply = d.task_reply, drdt_task_name = d.task_name
  WITH nocounter
 ;end select
 IF (check_error("Gathering DM_REFCHG_TASK_QUEUE data") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (curqual=0)
  ...
  SET dm_err->emsg = concat(
   "No data was found in DM_REFCHG_TASK_QUEUE for the DM_REFCHG_TASK_QUEUE_ID: ",trim(cnvtstring(
     drdt_task_queue_id)))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (drtq_parse_task_data(drdt_task_request,drdt_task_reply,parsed_task_data) != "S")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "An error occurred while parsing apart the TASK_REQUEST and/or TASK_REPLY data"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 CALL echo(
 ...
 CALL echo(parsed_task_data->request_definition)
 CALL echo(parsed_task_data->reply_definition)
 FOR (drdt_loop = 1 TO parsed_task_data->var_cnt)
   CALL echo(parsed_task_data->var_qual[drdt_loop].variable_declaration)
 ENDFOR
 FOR (drdt_loop = 1 TO parsed_task_data->set_cnt)
   CALL echo(parsed_task_data->set_qual[drdt_loop].set_command)
 ENDFOR
 CALL echo(drdt_task_name)
 IF (daf_is_not_blank(parsed_task_data->error_ind_item)
  AND daf_is_not_blank(parsed_task_data->no_err_result))
  CALL echo(concat("No error occurred if ",parsed_task_data->error_ind_item," = ",parsed_task_data->
    no_err_result))
 ENDIF
 CALL echo(
  "********************************************************************************************************************"
