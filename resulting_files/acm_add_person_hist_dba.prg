CREATE PROGRAM acm_add_person_hist:dba
 ...
 INSERT  FROM person_flex_hist pfh,
  ...
  SET pfh.person_flex_hist_id = seq(person_seq,nextval), pfh.pm_hist_tracking_id = acm_request->
   person_qual[xref->add[d.seq].idx].pm_hist_tracking_id, pfh.person_id = acm_request->person_qual[
   xref->add[d.seq].idx].person_id,
   pfh.transaction_dt_tm = cnvtdatetime(acm_request->person_qual[xref->add[d.seq].idx].
    transaction_dt_tm), pfh.birth_dt_tm = cnvtdatetime(acm_request->person_qual[xref->add[d.seq].idx]
    .birth_dt_tm), pfh.sex_cd = acm_request->person_qual[xref->add[d.seq].idx].sex_cd,
   pfh.language_cd = acm_request->person_qual[xref->add[d.seq].idx].language_cd, pfh.race_cd =
   acm_request->person_qual[xref->add[d.seq].idx].race_cd, pfh.marital_type_cd = acm_request->
   person_qual[xref->add[d.seq].idx].marital_type_cd,
   pfh.deceased_cd = acm_request->person_qual[xref->add[d.seq].idx].deceased_cd, pfh.deceased_dt_tm
    = cnvtdatetime(acm_request->person_qual[xref->add[d.seq].idx].deceased_dt_tm), pfh
   .deceased_source_cd = acm_request->person_qual[xref->add[d.seq].idx].deceased_source_cd,
   pfh.cause_of_death = acm_request->person_qual[xref->add[d.seq].idx].cause_of_death, pfh
   .cause_of_death_cd = acm_request->person_qual[xref->add[d.seq].idx].cause_of_death_cd, pfh
   .autopsy_cd = acm_request->person_qual[xref->add[d.seq].idx].autopsy_cd,
   pfh.ethnic_grp_cd = acm_request->person_qual[xref->add[d.seq].idx].ethnic_grp_cd, pfh.birth_tz =
   acm_request->person_qual[xref->add[d.seq].idx].birth_tz, pfh.tracking_bit = tracking_bit,
   pfh.change_bit = chgbit, pfh.active_ind = 1, pfh.active_status_cd = reqdata->active_status_cd,
