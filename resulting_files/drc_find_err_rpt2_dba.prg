CREATE PROGRAM drc_find_err_rpt2:dba
 PROMPT
 ...
 DECLARE age_gap = i2 WITH public, constant(1)
 DECLARE age_overlap = i2 WITH public, constant(2)
 DECLARE weight_gap = i2 WITH public, constant(3)
 DECLARE weight_overlap = i2 WITH public, constant(4)
 DECLARE invalid_age_op = i2 WITH public, constant(5)
 DECLARE invalid_wgt_op = i2 WITH public, constant(6)
...
#start_loop
 ...
 FOR (x = x TO grpcnt)
   ...
   ENDIF
   SELECT INTO "nl:"
    route = sorted_ranges->groupers[x].premises[d1.seq].route_key
    FROM (dummyt d1  WITH seq = value(sorted_ranges->groupers[x].premise_cnt))
    PLAN (d1
     WHERE (d1.seq <= sorted_ranges->groupers[x].premise_cnt))
    ORDER BY route, sorted_ranges->groupers[x].premises[d1.seq].from_day, sorted_ranges->groupers[x].
     premises[d1.seq].to_day
    HEAD REPORT
     new_route = true
    HEAD route
     new_route = true
     IF ((sorted_ranges->groupers[x].premises[d1.seq].from_day > 0)
      AND (sorted_ranges->groupers[x].premises[d1.seq].age_rel_op_flag != 1))
      IF (grp_warn_cnt=0)
      ...
      reply->group[warn_cnt].warnings[grp_warn_cnt].type = age_gap, reply->group[warn_cnt].warnings[
      grp_warn_cnt].group_ind = 0,
      CALL store_warn_info(warn_cnt,grp_warn_cnt,x,d1.seq)
     ELSEIF ((sorted_ranges->groupers[x].premises[d1.seq].age_rel_op_flag=1))
      IF (grp_warn_cnt=0)
       warn_cnt = (warn_cnt+ 1), stat = alterlist(reply->group,warn_cnt), reply->group[warn_cnt].
       group_name = sorted_ranges->groupers[x].group_name
      ENDIF
      grp_warn_cnt = (grp_warn_cnt+ 1), reply->group[warn_cnt].grp_warn_cnt = grp_warn_cnt, stat =
      alterlist(reply->group[warn_cnt].warnings,grp_warn_cnt),
      reply->group[warn_cnt].warnings[grp_warn_cnt].type = invalid_age_op, reply->group[warn_cnt].
      warnings[grp_warn_cnt].group_ind = 0,
      CALL store_warn_info(warn_cnt,grp_warn_cnt,x,d1.seq)
     ENDIF
    DETAIL
     IF (new_route=true)
     ...
     ELSE
      IF ((sorted_ranges->groupers[x].premises[d1.seq].age_rel_op_flag=1))
       ...
       warnings[grp_warn_cnt].group_ind = 0,
       CALL store_warn_info(warn_cnt,grp_warn_cnt,x,d1.seq)
      ELSEIF ((sorted_ranges->groupers[x].premises[(d1.seq - 1)].from_day=sorted_ranges->groupers[x].
      premises[d1.seq].from_day)
       AND (sorted_ranges->groupers[x].premises[(d1.seq - 1)].to_day=sorted_ranges->groupers[x].
      premises[d1.seq].to_day)
       AND (sorted_ranges->groupers[x].premises[(d1.seq - 1)].age_rel_op_flag != 1)
       AND (sorted_ranges->groupers[x].premises[d1.seq].age_rel_op_flag != 1))
       IF ((sorted_ranges->groupers[x].premises[(d1.seq - 1)].from_weight=sorted_ranges->groupers[x].
       premises[d1.seq].from_weight))
        IF (grp_warn_cnt=0)
         warn_cnt = (warn_cnt+ 1), stat = alterlist(reply->group,warn_cnt), reply->group[warn_cnt].
         group_name = sorted_ranges->groupers[x].group_name
        ENDIF
        grp_warn_cnt = (grp_warn_cnt+ 2), reply->group[warn_cnt].grp_warn_cnt = grp_warn_cnt, stat =
        alterlist(reply->group[warn_cnt].warnings,grp_warn_cnt),
        reply->group[warn_cnt].warnings[(grp_warn_cnt - 1)].type = age_overlap, reply->group[warn_cnt
        ].warnings[(grp_warn_cnt - 1)].group_ind = 1, reply->group[warn_cnt].warnings[grp_warn_cnt].
        type = age_overlap,
        reply->group[warn_cnt].warnings[grp_warn_cnt].group_ind = 2,
        CALL store_warn_info(warn_cnt,(grp_warn_cnt - 1),x,(d1.seq - 1)),
        CALL store_warn_info(warn_cnt,grp_warn_cnt,x,d1.seq)
       ENDIF
      ELSE
       IF ((sorted_ranges->groupers[x].premises[(d1.seq - 1)].to_day >= sorted_ranges->groupers[x].
       premises[d1.seq].from_day)
        AND (sorted_ranges->groupers[x].premises[(d1.seq - 1)].age_rel_op_flag != 1)
        AND (sorted_ranges->groupers[x].premises[d1.seq].age_rel_op_flag != 1))
        IF (grp_warn_cnt=0)
        ...
        CALL store_warn_info(warn_cnt,(grp_warn_cnt - 1),x,(d1.seq - 1)),
        CALL store_warn_info(warn_cnt,grp_warn_cnt,x,d1.seq)
       ELSEIF ((sorted_ranges->groupers[x].premises[(d1.seq - 1)].to_day < sorted_ranges->groupers[x]
       .premises[d1.seq].from_day)
        AND round((sorted_ranges->groupers[x].premises[(d1.seq - 1)].to_day+ 0.1),1) != round(
        sorted_ranges->groupers[x].premises[d1.seq].from_day,1)
        AND (sorted_ranges->groupers[x].premises[(d1.seq - 1)].age_rel_op_flag != 1)
        AND (sorted_ranges->groupers[x].premises[d1.seq].age_rel_op_flag != 1))
        IF (grp_warn_cnt=0)
         warn_cnt = (warn_cnt+ 1), stat = alterlist(reply->group,warn_cnt), reply->group[warn_cnt].
        ...
        grp_warn_cnt = (grp_warn_cnt+ 2), reply->group[warn_cnt].grp_warn_cnt = grp_warn_cnt, stat =
        alterlist(reply->group[warn_cnt].warnings,grp_warn_cnt),
        reply->group[warn_cnt].warnings[(grp_warn_cnt - 1)].type = age_gap, reply->group[warn_cnt].
        warnings[(grp_warn_cnt - 1)].group_ind = 1, reply->group[warn_cnt].warnings[grp_warn_cnt].
        type = age_gap,
        reply->group[warn_cnt].warnings[grp_warn_cnt].group_ind = 2,
        CALL store_warn_info(warn_cnt,(grp_warn_cnt - 1),x,(d1.seq - 1)),
    ...
    FOOT  route
     ...
     IF ((sorted_ranges->groupers[x].premises[d1.seq].to_day != number_of_days)
      AND (sorted_ranges->groupers[x].premises[d1.seq].age_rel_op_flag != 1))
      IF (grp_warn_cnt=0)
       warn_cnt = (warn_cnt+ 1), stat = alterlist(reply->group,warn_cnt), reply->group[warn_cnt].
       group_name = sorted_ranges->groupers[x].group_name
   ...
   SELECT INTO "nl:"
    ...
    HEAD from_day
     new_day = true
     IF ((sorted_ranges->groupers[x].premises[d1.seq].from_weight > 0)
      AND (((sorted_ranges->groupers[x].premises[d1.seq].to_weight > 0.0)) OR ((sorted_ranges->
     groupers[x].premises[d1.seq].from_weight > 0.0)))
      AND (sorted_ranges->groupers[x].premises[d1.seq].wgt_rel_op_flag != 1))
      IF (grp_warn_cnt=0)
       warn_cnt = (warn_cnt+ 1), stat = alterlist(reply->group,warn_cnt), reply->group[warn_cnt].
       group_name = sorted_ranges->groupers[x].group_name
      ENDIF
      grp_warn_cnt = (grp_warn_cnt+ 1), reply->group[warn_cnt].grp_warn_cnt = grp_warn_cnt, stat =
      alterlist(reply->group[warn_cnt].warnings,grp_warn_cnt),
      reply->group[warn_cnt].warnings[grp_warn_cnt].type = weight_gap, reply->group[warn_cnt].
      warnings[grp_warn_cnt].group_ind = 0,
      CALL store_warn_info(warn_cnt,grp_warn_cnt,x,d1.seq)
     ELSEIF ((sorted_ranges->groupers[x].premises[d1.seq].wgt_rel_op_flag=1))
      IF (grp_warn_cnt=0)
       warn_cnt = (warn_cnt+ 1), stat = alterlist(reply->group,warn_cnt), reply->group[warn_cnt].
       group_name = sorted_ranges->groupers[x].group_name
      ENDIF
      grp_warn_cnt = (grp_warn_cnt+ 1), reply->group[warn_cnt].grp_warn_cnt = grp_warn_cnt, stat =
      alterlist(reply->group[warn_cnt].warnings,grp_warn_cnt),
      reply->group[warn_cnt].warnings[grp_warn_cnt].type = invalid_wgt_op, reply->group[warn_cnt].
      warnings[grp_warn_cnt].group_ind = 0,
      CALL store_warn_info(warn_cnt,grp_warn_cnt,x,d1.seq)
     ENDIF
    DETAIL
     IF (new_day=true)
      new_day = false
     ELSE
      IF ((sorted_ranges->groupers[x].premises[(d1.seq - 1)].to_weight >= sorted_ranges->groupers[x].
      premises[d1.seq].from_weight)
       AND (((sorted_ranges->groupers[x].premises[d1.seq].to_weight > 0.0)) OR ((sorted_ranges->
      groupers[x].premises[d1.seq].from_weight > 0.0)))
       AND (sorted_ranges->groupers[x].premises[(d1.seq - 1)].wgt_rel_op_flag != 1)
       AND (sorted_ranges->groupers[x].premises[d1.seq].wgt_rel_op_flag != 1))
       IF (grp_warn_cnt=0)
        warn_cnt = (warn_cnt+ 1), stat = alterlist(reply->group,warn_cnt), reply->group[warn_cnt].
        group_name = sorted_ranges->groupers[x].group_name
       ...
       grp_warn_cnt = (grp_warn_cnt+ 2), reply->group[warn_cnt].grp_warn_cnt = grp_warn_cnt, stat =
       alterlist(reply->group[warn_cnt].warnings,grp_warn_cnt),
       reply->group[warn_cnt].warnings[(grp_warn_cnt - 1)].type = weight_overlap, reply->group[
       warn_cnt].warnings[(grp_warn_cnt - 1)].group_ind = 1, reply->group[warn_cnt].warnings[
       grp_warn_cnt].type = weight_overlap,
       reply->group[warn_cnt].warnings[grp_warn_cnt].group_ind = 2,
       CALL store_warn_info(warn_cnt,(grp_warn_cnt - 1),x,(d1.seq - 1)),
       CALL store_warn_info(warn_cnt,grp_warn_cnt,x,d1.seq)
      ELSEIF ((sorted_ranges->groupers[x].premises[(d1.seq - 1)].to_weight < sorted_ranges->groupers[
      x].premises[d1.seq].from_weight)
       AND round((sorted_ranges->groupers[x].premises[(d1.seq - 1)].to_weight+ 0.1),1) != round(
       sorted_ranges->groupers[x].premises[d1.seq].from_weight,1)
       AND (((sorted_ranges->groupers[x].premises[d1.seq].to_weight > 0.0)) OR ((sorted_ranges->
      groupers[x].premises[d1.seq].from_weight > 0.0)))
       AND (sorted_ranges->groupers[x].premises[(d1.seq - 1)].wgt_rel_op_flag != 1)
       AND (sorted_ranges->groupers[x].premises[d1.seq].wgt_rel_op_flag != 1))
       ...
       grp_warn_cnt = (grp_warn_cnt+ 2), reply->group[warn_cnt].grp_warn_cnt = grp_warn_cnt, stat =
       alterlist(reply->group[warn_cnt].warnings,grp_warn_cnt),
       reply->group[warn_cnt].warnings[(grp_warn_cnt - 1)].type = weight_gap, reply->group[warn_cnt].
       warnings[(grp_warn_cnt - 1)].group_ind = 1, reply->group[warn_cnt].warnings[grp_warn_cnt].type
        = weight_gap,
       reply->group[warn_cnt].warnings[grp_warn_cnt].group_ind = 2,
       CALL store_warn_info(warn_cnt,(grp_warn_cnt - 1),x,(d1.seq - 1)),
       CALL store_warn_info(warn_cnt,grp_warn_cnt,x,d1.seq)
    ...
    FOOT  from_day
     ...
     IF ((sorted_ranges->groupers[x].premises[d1.seq].to_weight != number_of_kgs)
      AND (((sorted_ranges->groupers[x].premises[d1.seq].to_weight > 0.0)) OR ((sorted_ranges->
     groupers[x].premises[d1.seq].from_weight > 0.0)))
      AND (sorted_ranges->groupers[x].premises[d1.seq].wgt_rel_op_flag != 1))
      IF (grp_warn_cnt=0)
  ...
  *
  ...
  HEAD REPORT
   line = fillstring(130,"-")
  HEAD PAGE
   col 50, "DOSE RANGE PREMISE CHECKING REPORT", row + 1,
  ...
  HEAD d1.seq
   ...
   reply->group[d1.seq].group_name, row + 1
  DETAIL
   IF ((((reply->group[d1.seq].warnings[d2.seq].group_ind=0)) OR ((reply->group[d1.seq].warnings[d2
   .seq].group_ind=1))) )
    row + 1
    CASE (reply->group[d1.seq].warnings[d2.seq].type)
     OF age_gap:
      col 5,"AGE GAP"
     OF age_overlap:
      col 5,"AGE OVERLAP"
     ...
     OF invalid_age_op:
      col 5,"INVALID AGE OPERATOR"
     OF invalid_wgt_op:
      col 5,"INVALID WEIGHT OPERATOR"
     ELSE
      CALL echo(build("Can't recognize the warning type:",reply->group[d1.seq].warnings[d2.seq].type)
      )
    ENDCASE
    row + 1
   ENDIF
   ...
   .seq].group_ind=1))) )
    ...
    "Wgt Unit", col 98, "Route(s)",
    row + 1, col 10, reply->group[d1.seq].warnings[d2.seq].age_rel_op"##########;L;T",
    col 21, reply->group[d1.seq].warnings[d2.seq].from_day_str"##########;L;T", col 32,
    reply->group[d1.seq].warnings[d2.seq].to_day_str"##########;L;T", col 43, reply->group[d1.seq].
    warnings[d2.seq].age_units_disp"##########;L;T",
    col 54, reply->group[d1.seq].warnings[d2.seq].wgt_rel_op"##########;L;T", col 65,
    reply->group[d1.seq].warnings[d2.seq].from_weight_str"##########;L;T", col 76, reply->group[d1
    .seq].warnings[d2.seq].to_weight_str"##########;L;T",
    col 87, reply->group[d1.seq].warnings[d2.seq].wgt_units_disp"##########;L;T", col 98,
    reply->group[d1.seq].warnings[d2.seq].route_disp"##############################;L;T", row + 1
   ELSEIF ((reply->group[d1.seq].warnings[d2.seq].group_ind=2))
