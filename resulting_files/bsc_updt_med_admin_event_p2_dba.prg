CREATE PROGRAM bsc_updt_med_admin_event_p2:dba
 ...
 DECLARE errmsg = vc WITH protect, noconstant("")
 SET ms_info_domain_nm = "UPDATE ADMIN EVENT - EVENT_ID"
 SET ms_range = "CAT 5 EVENT_ID RANGE"
 SET ms_max_name_p = "MAX ADMIN EVENT_ID EVALUATED"
 SET ms_child_script = "bsc_updt_med_admin_event_cc"
 SET mn_num_children = 3
 SELECT INTO "nl:"
  max_val = max(m.med_admin_event_id)
  FROM med_admin_event m
  DETAIL
 ...
 SELECT INTO "nl:"
  FROM dba_objects d
  WHERE d.object_name IN ("MED_ADMIN_EVENT_ORV_PROC_2", "MED_ADMIN_EVENT_ORV_DATE_2",
  "MED_ADMIN_EVENT_ORV_VAL_2")
  DETAIL
   mn_count += 1
  WITH nocounter
 ;end select
 ...
 ENDIF
 IF (mn_count != 3)
  EXECUTE dm_readme_include_sql "cer_install:med_admin_event_orv_proc_2.sql"
  SET readme_data->status = dm_sql_reply->status
  IF ((readme_data->status="F"))
   SET readme_data->message = concat(
    "Failed to create the file cer_install:med_admin_event_orv_proc_2.sql:",dm_sql_reply->msg)
   GO TO exit_program
  ENDIF
  EXECUTE dm_readme_include_sql "cer_install:med_admin_event_orv_date_2.sql"
  SET readme_data->status = dm_sql_reply->status
  IF ((readme_data->status="F"))
   SET readme_data->message = concat(
    "Failed to include the file cer_install:med_admin_event_orv_date_2.sql:",dm_sql_reply->msg)
   GO TO exit_program
  ENDIF
  EXECUTE dm_readme_include_sql "cer_install:med_admin_event_orv_val_2.sql"
  SET readme_data->status = dm_sql_reply->status
  IF ((readme_data->status="F"))
   SET readme_data->message = concat(
    "Failed to create the file cer_install:med_admin_event_orv_val_2.sql:",dm_sql_reply->msg)
   GO TO exit_program
  ENDIF
  CALL sbr_check_procedure("MED_ADMIN_EVENT_ORV_DATE_2","FUNCTION")
  CALL sbr_check_procedure("MED_ADMIN_EVENT_ORV_PROC_2","PROCEDURE")
  CALL sbr_check_procedure("MED_ADMIN_EVENT_ORV_VAL_2","FUNCTION")
 ENDIF
 ...
 ELSE
  ...
  IF (mn_object_count=3)
   CALL parser("rdb drop procedure med_admin_event_orv_proc_2 go")
   CALL parser("rdb drop function med_admin_event_orv_date_2 go")
   CALL parser("rdb drop function med_admin_event_orv_val_2 go")
   IF (error(errmsg,0) != 0)
    SET readme_data->status = "S"
