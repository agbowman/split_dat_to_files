CREATE PROGRAM acm_del_encntr_domain:dba
 ...
 DELETE  FROM (dummyt d  WITH seq = value(xref->del_cnt)),
  ...
  PLAN (d)
   JOIN (e
   WHERE (e.encntr_domain_id=acm_request->encntr_domain_qual[xref->del[d.seq].idx].encntr_domain_id))
  WITH nocounter, status(reply->encntr_domain_qual[xref->del[d.seq].idx].status)
 ;end delete
 FOR (index = 1 TO xref->del_cnt)
   IF ((reply->encntr_domain_qual[xref->del[index].idx].status != 1))
 ...
 SUBROUTINE (getexistingrows(x=i4) =null)
   SELECT INTO "nl:"
    ...
    WHERE expand(t1,f_val,t_val,e.encntr_domain_id,acm_request->encntr_domain_qual[xref->del[t1].idx]
     .encntr_domain_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,e.encntr_domain_id,acm_request->encntr_domain_qual[xref->del[t1].
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->encntr_domain_qual[xref->del[index].idx].status=0))
