CREATE PROGRAM dcp_upd_prob_icd9_snmct_imo_p2:dba
 ...
 SUBROUTINE (sbr_p2_check_for_prev_rdm(readme_id=i4,instance=i4) =i4)
   ...
   ENDIF
 END ;Subroutine
 DECLARE parent_script_name = vc WITH protect, constant("DCP_UPD_PROB_ICD9_SNMCT_IMO_AUDIT")
 DECLARE range_name = vc WITH protect, constant("DCP_UPD_PROB_ICD9_SNMCT_IMO_AUDIT RANGE")
 DECLARE child_c_script_name = vc WITH protect, constant("dcp_upd_prob_icd9_snmct_imo_cc")
 DECLARE max_person_id_eval = vc WITH protect, constant("MAX PERSON_ID EVALUATED")
 DECLARE range_name_p2 = vc WITH protect, constant("DCP_UPD_PROB_ICD9_SNMCT_IMO_AUDIT CAT5 RANGE")
 DECLARE max_num_children = i4 WITH protect, constant(5)
 DECLARE batch_size_name = vc WITH protect, constant("DCP_UPD_PROB_ICD9_SNMCT_IMO_AUDIT BATCH SIZE")
 DECLARE oracle_hint_name = vc WITH protect, constant("DCP_UPD_PROB_ICD9_SNMCT_IMO_AUDIT ORAHINT")
 SET readme_data->status = "F"
 ...
 DECLARE dimo = f8 WITH protect, noconstant(0.0)
 ...
 SELECT INTO "nl:"
  ...
  WHERE cv.code_set=dvocabtypecs
   ...
   AND cv.cdf_meaning IN ("ICD9", "SNMCT", "IMO")
  DETAIL
   CASE (cv.cdf_meaning)
    OF "ICD9":
     dicd9 = cv.code_value
...
#exit_program
 ...
 ELSE
  COMMIT
  CALL sbr_add_dropped_object("INDEX","XIETEMPPROBICD9SNMCTIMO","Dropping index")
  CALL sbr_add_dropped_object("TABLE","TEMP_PROB_ICD9_SNMCT_IMO","Dropping table")
