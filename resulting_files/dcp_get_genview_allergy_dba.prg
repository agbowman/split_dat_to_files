CREATE PROGRAM dcp_get_genview_allergy:dba
 SET rhead =
 ...
 IF ((request->person[1].person_id > 0))
  ...
  FOR (x = 1 TO person_cnt)
    SELECT INTO "nl"
     ...
     ORDER BY a.allergy_id, a.onset_dt_tm
     HEAD REPORT
     ...
     HEAD a.allergy_id
      ...
      IF (((n.source_string > " ") OR (a.substance_ftdesc > " ")) )
       ...
       ELSE
        temp_disp1 = a.substance_ftdesc
       ENDIF
       temp_disp2 = fillstring(27," "), temp_disp3 = "Since: ", temp_disp4 = format(a.onset_dt_tm,
        "mm/dd/yyyy;;d")
       IF (((r.reaction_ftdesc > " ") OR (n2.source_string > " ")) )
        temp_disp2 = r.reaction_ftdesc
        IF (n2.source_string > " ")
         temp_disp2 = n2.source_string
        ENDIF
        temp_disp5 = build("(",temp_disp3,temp_disp4,",",temp_disp5,
         ")")
       ELSE
        temp_disp5 = build("(",temp_disp3,temp_disp4,")")
       ENDIF
       drec->line_qual[lidx].disp_line = concat("         ",trim(temp_disp1),"  ","   ",trim(
         temp_disp5),
        reol), ";;T"
      ENDIF
     FOOT REPORT
