CREATE PROGRAM afc_da_add_charge_event_mod:dba
 SET afc_da_add_charge_event_mod_vrsn = 20171002
 ...
 CALL add_charge_event_mod(mndastart,mndastop)
 SUBROUTINE (add_charge_event_mod(nstart=i4,nstop=i4) =null WITH protect)
   ...
   FOR (i = 1 TO size(request->objarray,5))
     IF (validate(request->objarray[i].charge_event_mod_id,- (0.00001)) <= 0.0)
      SELECT INTO "nl:"
       ...
       WITH format, counter
      ;end select
      IF (validate(request->objarray[i].charge_event_mod_id))
       SET request->objarray[i].charge_event_mod_id = mrsstatus->objarray[i].daid
      ENDIF
      ...
      ENDIF
     ELSE
      SET mrsstatus->objarray[i].daid = request->objarray[i].charge_event_mod_id
     ENDIF
   ENDFOR
   INSERT  FROM charge_event_mod c,
     (dummyt dt  WITH seq = value(size(request->objarray,5)))
    SET c.charge_event_mod_id = mrsstatus->objarray[dt.seq].daid, c.charge_event_id =
