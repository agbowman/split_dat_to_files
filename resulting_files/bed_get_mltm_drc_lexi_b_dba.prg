CREATE PROGRAM bed_get_mltm_drc_lexi_b:dba
 FREE SET reply
 RECORD reply(
   1 groupers[*]
     2 grouper_id = f8
  ...
  SELECT INTO "nl:"
   ...
   PLAN (m
    WHERE m.parent_premise_id=0
     ...
     AND cnvtupper(m.age_unit_disp)="YEAR*") OR (cnvtupper(m.age_unit_disp) != "YEAR*")) ))
     AND  NOT ( EXISTS (
    (SELECT
     1
     FROM cmt_import_log c,
      cmt_import_log_msg cm
     WHERE cm.cmt_import_log_id=c.cmt_import_log_id
      AND c.input_filename="LEXICOMP_IMPORT"
      AND cm.log_message=m.grouper_name))))
    JOIN (der
