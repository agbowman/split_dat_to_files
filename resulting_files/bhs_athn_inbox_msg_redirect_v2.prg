CREATE PROGRAM bhs_athn_inbox_msg_redirect_v2
 FREE RECORD result
 RECORD result(
   1 header_text = gvc
   1 to_prsnl[*]
 ...
 ELSEIF (( $3 <= 0.0))
  ...
  GO TO exit_script
 ENDIF
 SET result->pool_ind = evaluate( $8,0.0,0,1)
 SELECT INTO "NL:"
  FROM person p
  PLAN (p
   WHERE (p.person_id= $3)
 ...
 SELECT INTO "NL:"
  ...
  PLAN (ta
   ...
   WHERE taa.task_id=ta.task_id
    ...
    AND (taa.assign_prsnl_group_id= $8)))
 ...
 SET result->task_uid = concat("urn:cerner:mid:object.task:",trim(cnvtlower(curdomain),3),":taskId=",
  ...
  evaluate(result->pool_ind,1,trim(replace(cnvtstring( $8),".00","",0),3),trim(replace(cnvtstring( $3
      ),".00","",0),3)),",enum=",evaluate(result->task_type_cd,c_reminder_msg_cd,"-11",
   c_consult_msg_cd,"-10",
...
#exit_script
 ...
 SUBROUTINE callredirectreminder(null)
   ...
   FOR (idx = 1 TO size(result->to_prsnl,5))
     ...
     prsnl_id
   ENDFOR
   SET stat = alterlist(req967590->redirect_list[1].assign_pool_list,size(result->to_pool,5))
   FOR (idx = 1 TO size(result->to_pool,5))
     SET req967590->redirect_list[1].assign_pool_list[idx].assign_pool_id = result->to_pool[idx].
     pool_id
   ENDFOR
