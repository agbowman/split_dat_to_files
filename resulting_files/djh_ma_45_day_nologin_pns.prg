CREATE PROGRAM djh_ma_45_day_nologin_pns
 PROMPT
 ...
 SET date_qual = cnvtlookbehind("45,D",cnvtdatetime(curdate,curtime3))
 ...
 SELECT DISTINCT INTO value(output_dest)
  ...
  PLAN (pr
   WHERE pr.active_ind=1
    ...
    AND pr.updt_dt_tm < cnvtdatetime((curdate - 45),0)
    AND ((pr.username="PN*") OR (pr.position_cd=922119))
    AND  NOT (pr.person_id IN (
   (SELECT
    oai.person_id
    FROM omf_app_ctx_day_st oai
    WHERE oai.person_id=pr.person_id
     AND oai.start_day > cnvtdatetime((curdate - 45),000)))))
   JOIN (oa
 ...
 IF (email_ind=1)
  ...
  SET filename_out = concat(format(curdate,"YYYYMMDD ;;D"),"-PNs.csv")
  DECLARE subject_line = vc
  SET subject_line = concat(curprog," - CIS PN Acnts InAct 45 days")
