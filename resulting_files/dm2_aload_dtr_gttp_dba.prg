CREATE PROGRAM dm2_aload_dtr_gttp:dba
 ...
 ENDIF
 IF (check_logfile("dm2_aload_dtr",".log","dm2_aload_dtr_gttp")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_dtr_gttp"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dadg_pop_coldic_rec(dpcr_tab_in=vc) = i2
 DECLARE dadg_tbl = vc WITH protect, noconstant("")
 DECLARE dadg_req_size = i4 WITH protect, noconstant(0)
 DECLARE dadg_col_cnt = i4 WITH protect, noconstant(0)
 SET dadg_tbl = "DM_TABLE_RELATIONSHIPS_GTTP"
 SET dadg_req_size = size(requestin->list_0,5)
 IF (dadg_req_size=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("DBIMPORT failed to load csv data into requestin record structure.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (dadg_pop_coldic_rec(dadg_tbl)=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(columns_1)
  CALL echorecord(dac_col_list)
 ENDIF
 SET dm_err->eproc = concat("Load csv data into temp table ",dadg_tbl)
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
   " dcv ,(dummyt d with seq = value(dadg_req_size))"),0)
 ...
 ENDIF
 COMMIT
 SUBROUTINE dadg_pop_coldic_rec(dpcr_tab_in)
   DECLARE dpcr_cnt = i4 WITH protect, noconstant(0)
   DECLARE dpcr_idx = i4 WITH protect, noconstant(0)
   DECLARE dcpr_col_oradef_ind = i2 WITH protect, noconstant(0)
   DECLARE dcpr_col_ccldef_ind = i2 WITH protect, noconstant(0)
   ...
   FOR (dpcr_idx = 1 TO size(columns_1->list_1,5))
     ...
     SET dcpr_col_ccldef_ind = 0
     SET dpcr_data_type = ""
     IF ((columns_1->list_1[dpcr_idx].field_name != "FILEVERSION"))
      IF (dm2_table_column_exists(value(currdbuser),dac_col_list->tbl,columns_1->list_1[dpcr_idx].
       field_name,1,1,
       2,dcpr_col_oradef_ind,dcpr_col_ccldef_ind,dpcr_data_type)=0)
       RETURN(0)
      ENDIF
      IF (dcpr_col_ccldef_ind=0)
       SET dm_err->err_ind = 1
       SET dm_err->emsg = concat("Column mismatch found between csv and CCLDEF: ",columns_1->list_1[
        dpcr_idx].field_name)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(0)
      ELSEIF (dcpr_col_oradef_ind=0)
       SET dm_err->err_ind = 1
       SET dm_err->emsg = concat("Column mismatch found between csv and Oracle: ",columns_1->list_1[
        dpcr_idx].field_name)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(0)
      ELSE
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_dtr_gttp completed batch load."
 ELSE
  SET dm_err->eproc = "Load error during dm2_aload_dtr_gttp."
 ENDIF
END GO
