CREATE PROGRAM ct_eso_crm:dba
 ...
 CALL echo(concat("*** CT_ESO_CRM START CTRL_ID1 = ",concat("Q:",trim(cnvtstring(get_reqinfo_double(
 ...
 SET reg_id = request->triginfo.reg_id
 SET regid_str = cnvtstring(reg_id)
 CALL echo(build("reg_id is ",reg_id))
 CALL echo(build("the reg id string is ",regid_str))
 SET stat = alterlist(context->csr_group,1)
 SET _person_id = 0.0
 ...
 IF (size(context->csr_group,5) > 0)
  SELECT INTO "nl:"
   ppr.reg_id
   FROM pt_prot_reg ppr
   WHERE ppr.reg_id=reg_id
   DETAIL
    prot_master_id = ppr.prot_master_id, _person_id = ppr.person_id, date->off_study_dt_tm = ppr
    .off_study_dt_tm,
    date->end_dt_tm = ppr.end_effective_dt_tm, context->csr_group[1].csr.registration_dt_tm =
    hl7_format_datetime(ppr.on_study_dt_tm,hl7_dt_tm)
    IF ((date->off_study_dt_tm=cnvtdatetime("31-dec-2100 00:00:00.00")))
     context->csr_group[1].csr.ended_study_dt_tm = hl7_format_datetime(0,hl7_dt_tm),
     CALL echo("off study date is max date")
    ELSE
     context->csr_group[1].csr.ended_study_dt_tm = hl7_format_datetime(date->off_study_dt_tm,
      hl7_dt_tm),
     CALL echo(build("off study date is ",cnvtdatetime(date->off_study_dt_tm)))
    ENDIF
    CALL echo(build("person_Id is ",_person_id)),
    CALL echo(build("protmaster_Id is ",prot_master_id))
   WITH nocounter
  ;end select
 ELSE
  CALL echo("context->csr_group is EMPTY!!!")
 ENDIF
 SET stat = set_esoinfo_double("person_id",_person_id)
 SELECT INTO "nl:"
  primary_mnemonic
  FROM prot_master p
  WHERE p.prot_master_id=prot_master_id
  DETAIL
   context->csr_group[1].csr.sponsor_study_id.entity_id = p.primary_mnemonic
  WITH nocounter
 ;end select
 IF ((date->end_dt_tm != cnvtdatetime("31-dec-2100 00:00:00.00")))
  GO TO exit_script
 ENDIF
 SET counter = 0
 SELECT INTO "nl:"
  ...
  DETAIL
   counter += 1, stat = alterlist(context->csr_group[1].alternate_study_id,counter), context->
   csr_group[1].csr.alternate_study_id[counter].entity_id = pa.prot_alias,
   context->csr_group[1].csr.alternate_study_id[counter].name_id = eso_format_code(pa.alias_pool_cd)
  WITH nocounter
 ;end select
 SET _regstr = concat("REG",regid_str)
 SET context->csr_group[1].csr.registering_institution.text = _regstr
 SET eligible = 0.0
 SET verelig = 0.0
 SET stat = alterlist(context->csr_group[1].csr.registering_person_id,1)
 SET stat = uar_get_meaning_by_codeset(17285,"ELIGIBLE",1,eligible)
 SET stat = uar_get_meaning_by_codeset(17285,"VERELIG",1,verelig)
 SET elig_req_person_id = 0.0
 SELECT INTO "nl:"
  per.reg_id
  FROM pt_reg_elig_reltn per,
   pt_elig_tracking pet
  PLAN (per
   WHERE per.reg_id=reg_id)
   JOIN (pet
   WHERE per.pt_elig_tracking_id=pet.pt_elig_tracking_id)
  DETAIL
   IF (pet.elig_status_cd IN (eligible, verelig))
    CALL echo("csr 7 worked"),
    CALL echo(build("the reg id in csr7 is ",reg_id)), elig_req_person_id = pet
    .elig_request_person_id
   ENDIF
  WITH nocounter
 ;end select
 CALL echo(build("elig request is ",elig_req_person_id))
 CALL echo(build("curqual for csr 7 is ",curqual))
 IF (curqual=1)
  CALL echo("study registration worked")
  SET context->csr_group[1].csr.registering_person_id[1].id_nbr = eso_format_prsnl_id_enctr_ctx(
   "REGISTERING_PERSON","ALL_PRSNL",1,"CSR_REG_PERSON",elig_req_person_id,
   0,"XCN",1)
  CALL echo(build("elig req person id ",elig_req_person_id))
 ELSE
  CALL echo(build("error in person reg id - more than one row"))
 ENDIF
 CALL echo("doing Csr 8 and 9")
 SET stat = alterlist(context->csr_group[1].csr.authorizing_provider_id,1)
 SET consent_id = 0.0
 SET ptconsent_id = 0.0
 SET reason_cd = 0.0
 CALL echo("before uar_get....")
 SET stat = uar_get_meaning_by_codeset(17349,"ENROLLING",1,reason_cd)
 CALL echo(build("reason_cd is ",reason_cd))
 SELECT INTO "nl:"
  prc.reg_id
  FROM pt_reg_consent_reltn prc,
   pt_consent pc
  PLAN (prc
   WHERE prc.reg_id=reg_id)
   JOIN (pc
   WHERE pc.consent_id=prc.consent_id)
  DETAIL
   CALL echo("got here!")
   IF (pc.reason_for_consent_cd=reason_cd
    AND pc.end_effective_dt_tm=cnvtdatetime("31-dec-2100 00:00:00.00"))
    CALL echo("csr 8,9 worked"), consent_id = pc.consenting_person_id, ptconsent_id = pc
    .pt_consent_id,
    date->consent_date = pc.consent_signed_dt_tm
   ELSE
    CALL echo("a row did not qualify")
   ENDIF
  WITH nocounter
 ;end select
 CALL echo(build("curqual for csr 8 and 9 = ",curqual))
 CALL echo(build("pt_consent_id is ",ptconsent_id))
 CALL echo(build("authorize id is ",consent_id))
 SET temp_str = fillstring(20," ")
 SET temp_str = trim(concat("OBR_32"))
 IF (curqual=1)
  CALL echo("auth provider worked")
  SET context->csr_group[1].csr.authorizing_provider_id[1].id_nbr = eso_format_prsnl_id_enctr_ctx(
   "AUTHORIZING_PERSON","ALL_PRSNL",1,"CSR_AUTH_PERSON",consent_id,
   0,"XCN",1)
  IF ((date->consent_date=cnvtdatetime("31-dec-2100 00:00:00.00")))
   SET context->csr_group[1].csr.consent_sign_dt_tm = hl7_format_datetime(0,hl7_dt_tm)
  ELSE
   SET context->csr_group[1].csr.consent_sign_dt_tm = hl7_format_datetime(date->consent_date,
    hl7_dt_tm)
  ENDIF
 ELSE
  CALL echo(build("error in auth provider"))
 ENDIF
#exit_script
 SET reply->status_data.status = "S"
 CALL echo(concat("*** CT_ESO_CRM END  CTRL_ID1 = ",concat("Q:",trim(cnvtstring(get_reqinfo_double(
       "queue_id")))," T:",trim(cnvtstring(get_reqinfo_double("trigger_id"))))))
END GO
