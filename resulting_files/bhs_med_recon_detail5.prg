CREATE PROGRAM bhs_med_recon_detail5
 PROMPT
 ...
 ELSE
  ...
  IF (findstring("BHI_",output_dest,1,0))
   SET bhd_ind = 1
  ENDIF
 ENDIF
 ...
 SELECT
  ...
  HEAD REPORT
   ...
   col 1, strtend, row + 1,
   strtend = build("Phys Name",",","Patient Name",",","Account Num",
 ...
 IF (bhs_ind=1)
  ...
  SET dclcom = concat("$cust_script/bhs_ftp_file.ksh ",output_dest,
   ".csv transfer.baystatehealth.org bhs\\udpxb03 golfs \\\\HBI1\\\\WTFILES_BHS\\\\CPOE")
  CALL echo(dclcom)
  SET status = 0
  SET len = size(trim(dclcom))
  CALL dcl(dclcom,len,status)
