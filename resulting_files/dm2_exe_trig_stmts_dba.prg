CREATE PROGRAM dm2_exe_trig_stmts:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE trig_name_in = vc
 DECLARE trig_exe(null) = null WITH public
 DECLARE bg_x = i4
 SET trig_name_in = cnvtupper( $1)
 SET bg_x = 0
 CALL trig_exe(null)
 SUBROUTINE trig_exe(null)
   FOR (bg_x = 1 TO value(size(cl_hold_buffer->bg_hold,5)))
     IF (findstring("#",cl_hold_buffer->bg_hold[bg_x].bg_buffer))
      SET cl_hold_buffer->bg_hold[bg_x].bg_buffer = replace(cl_hold_buffer->bg_hold[bg_x].bg_buffer,
       "#","^",0)
     ENDIF
     IF (findstring("$",cl_hold_buffer->bg_hold[bg_x].bg_buffer))
      SET cl_hold_buffer->bg_hold[bg_x].bg_buffer = replace(cl_hold_buffer->bg_hold[bg_x].bg_buffer,
     ...
     SET cl_hold_buffer->bg_hold[bg_x].bg_buffer = replace(cl_hold_buffer->bg_hold[bg_x].bg_buffer,
      ";",concat(";",char(10)),0)
     CALL dm2_push_cmd(cl_hold_buffer->bg_hold[bg_x].bg_buffer,1)
     SET cl_hold_buffer->bg_hold[bg_x].bg_buffer = ""
   ENDFOR
   SET cl_parse_cnt = 0
   SET stat = alterlist(cl_hold_buffer->bg_hold,0)
   IF (check_error(concat(" Trigger_Build: Error in building trigger ",trig_name_in)) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->err_ind = 1
    IF (currdb="DB2UDB")
     ROLLBACK
    ENDIF
    SET cl_hold_buffer->bg_err = 1
   ELSE
    IF (currdb="DB2UDB")
     COMMIT
    ENDIF
    SET cl_hold_buffer->bg_err = 0
   ENDIF
 END ;Subroutine
END GO
