CREATE PROGRAM bed_get_syns_for_powerplans:dba
 ...
 DECLARE ocs_parse = vc WITH protect
 SET ocs_parse = concat("ocs.synonym_id = pc.parent_entity_id and ocs.active_ind = 1 ")
 ...
 ENDIF
 SET ocs_parse = concat(ocs_parse," and ocs.orderable_type_flag not in (8,11)")
 SELECT INTO "nl:"
  FROM pathway_comp pc,
   order_catalog_synonym ocs,
   code_value cv
  PLAN (pc
   WHERE pc.comp_type_cd IN (order_comp_cd, prescription_comp_cd)
    AND pc.active_ind=1)
   JOIN (ocs
   WHERE parser(ocs_parse))
   JOIN (cv
   WHERE cv.code_value=ocs.catalog_type_cd)
  ORDER BY ocs.synonym_id, pc.pathway_comp_id
  HEAD ocs.synonym_id
   comp_cnt = 0, syn_cnt = (syn_cnt+ 1), stat = alterlist(reply->synonyms,syn_cnt),
   reply->synonyms[syn_cnt].synonym_id = ocs.synonym_id, reply->synonyms[syn_cnt].mnemonic = ocs
   .mnemonic, reply->synonyms[syn_cnt].catalog_type_cd = ocs.catalog_type_cd,
   reply->synonyms[syn_cnt].catalog_type_meaning = cnvtupper(cv.cdf_meaning), reply->synonyms[syn_cnt
   ].catalog_type_display = cv.display
  HEAD pc.pathway_comp_id
   comp_cnt = (comp_cnt+ 1), stat = alterlist(reply->synonyms[syn_cnt].components,comp_cnt), reply->
   synonyms[syn_cnt].components[comp_cnt].component_id = pc.pathway_comp_id
  WITH nocounter
 ;end select
 CALL bederrorcheck("Failed to retrieve synonyms.")
 IF ((syn_cnt > request->max_reply)
