CREATE PROGRAM dm_ccb_csf_get_cs_by_display:dba
 ...
 ENDIF
 DECLARE csfgcbd_filterstring = vc WITH protect, noconstant(" ")
 DECLARE code_set_count = i4 WITH protect, noconstant(0)
 DECLARE failed = c1 WITH protect, noconstant("F")
 DECLARE errmsg = vc WITH protect, noconstant(" ")
 IF (((validate(request->filterlist)=0) OR (validate(request->code_set)=0)) )
  SET reply->message = "Request does not contain correct fields."
  SET failed = "T"
  GO TO exit_script
 ENDIF
 IF (size(request->filterlist,5) < 1)
  SET reply->message = "No display key search value given."
  SET failed = "T"
  GO TO exit_script
 ENDIF
 IF (daf_is_blank(request->filterlist[1].column_value))
  SET reply->message = "Display key search term is blank."
  SET failed = "T"
  GO TO exit_script
 ENDIF
 SET csfgcbd_filterstring = filterstring_builder(csfgcbd_filterstring,"cvs",request)
 SELECT
  IF ((request->code_set > 0))
   PLAN (cvs
    WHERE parser(csfgcbd_filterstring)
     AND (cvs.code_set=request->code_set))
  ELSE
   PLAN (cvs
    WHERE parser(csfgcbd_filterstring)
     AND cvs.code_set > 0)
  ...
  DETAIL
   ...
   IF (mod(code_set_count,10)=1)
    stat = alterlist(reply->code_set_list,(code_set_count+ 9))
   ENDIF
   reply->code_set_list[code_set_count].code_set = cvs.code_set, reply->code_set_list[code_set_count]
   .definition = cvs.definition, reply->code_set_list[code_set_count].description = cvs.description,
   reply->code_set_list[code_set_count].display = cvs.display
  WITH nocounter
 ;end select
 SET csfgcbd_filterstring = concat("cvs.display_key = patstring('*",cnvtupper(request->filterlist[1].
   column_value),"*')"," and cvs.display_key != patstring('",cnvtupper(request->filterlist[1].
   column_value),
  "*')")
 SELECT
  IF ((request->code_set > 0))
  ...
  WITH nocounter
 ;end select
 SET stat = alterlist(reply->code_set_list,code_set_count)
 IF (error(errmsg,0) != 0)
  SET failed = "T"
