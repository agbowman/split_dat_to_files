CREATE PROGRAM dm_rdm_del_saved_docs_entry_p2:dba
 ...
 DECLARE mf_max_range_id_powerform = f8 WITH protect, noconstant(0.0)
 DECLARE mf_max_range_id_wkfdoccomp = f8 WITH protect, noconstant(0.0)
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 IF (((error(ms_errmsg,0) != 0) OR (power_form_entry_cd <= 0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error loading code value POWERFORMS from codeset 29520: ",
   ms_errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 IF (((error(ms_errmsg,0) != 0) OR (wkfdoccomp_entry_cd <= 0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error loading code value WKFDOCCOMP from codeset 29520: ",
   ms_errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
 ...
 IF (((error(ms_errmsg,0) != 0) OR (task_activity_cd_saved_doc <= 0)) )
  ...
  GO TO exit_program
 ENDIF
 SET ms_info_domain_nm = "DM_RDM_DEL_SAVED_DOCS_ENTRY_MODE_CODE"
 SET ms_range = "FINAL RANGE"
 SET ms_max_name_p = "MAX TASK_ACTIVITY_ID EVALUATED"
 SET ms_child_script = "dm_rdm_del_saved_docs_entry_cc"
 ...
 SELECT INTO "n1:"
  ...
  DETAIL
   mf_max_range_id_powerform = cnvtreal(max_id)
  WITH nocounter
 ;end select
 SELECT INTO "n1:"
  max_id = max(ta.task_id)
  ...
  WHERE ta.active_ind=1
   ...
   AND ce.valid_until_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00")
  DETAIL
   mf_max_range_id_wkfdoccomp = cnvtreal(max_id)
  WITH nocounter
 ;end select
 IF (mf_max_range_id_powerform >= mf_max_range_id_wkfdoccomp)
  SET mf_max_range_id = mf_max_range_id_powerform
 ELSE
  SET mf_max_range_id = mf_max_range_id_wkfdoccomp
 ENDIF
...
#exit_program
 ...
 ELSE
  ...
  CALL sbr_add_dropped_object("TABLE","TEMP_ENTRYMODE_TASK_ACTIVITY","Dropping table")
