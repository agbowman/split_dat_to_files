CREATE PROGRAM bhs_rx_tpn:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (cv
   WHERE cv.code_set=72
    AND cv.display_key IN ("WEIGHT", "HEIGHT")
    AND cv.active_ind=1)
  DETAIL
   CASE (cv.display_key)
    OF "HEIGHT":
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ELSEIF (c.event_cd=weight_cd)
    orders->weight = concat(trim(c.result_val)," ",trim(uar_get_code_display(c.result_units_cd))),
 ...
 SELECT INTO "nl:"
  ...
  HEAD o.order_id
   ...
   IF (o.catalog_type_cd=pharmacy_cd)
    ...
    ENDIF
   ENDIF
   CALL echo(uar_get_code_display(oa.action_type_cd))
   IF (oa.action_type_cd IN (transfercancel_cd, activatestudentorder_cd, renew_cd, order_cd,
   modify_cd,
   discont_cd, void_cd, cancelreorder_cd, canceldc_cd, cancel_cd,
   activate_cd)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   ...
   action_type_cd IN (activatestudentorder_cd, activate_cd, modify_cd, renew_cd))
  ...
  HEAD od.oe_field_id
   ...
   ENDIF
   IF (od.oe_field_meaning="INFUSEOVER")
    orders->qual[d1.seq].infuse_over = od.oe_field_display_value
   ENDIF
   IF (od.oe_field_meaning="INFUSEOVERUNIT")
    orders->qual[d1.seq].infuse_over_unit = od.oe_field_display_value
   ENDIF
   IF (od.oe_field_meaning="RATE")
    orders->qual[d1.seq].rate = od.oe_field_display_value
   ENDIF
   IF (od.oe_field_meaning="RATEUNIT")
    orders->qual[d1.seq].rate_unit = od.oe_field_display_value
   ENDIF
   IF (od.oe_field_meaning="VOLUMEDOSE")
    orders->qual[d1.seq].volume_dose = od.oe_field_display_value
   ENDIF
   IF (od.oe_field_meaning="VOLUMEDOSEUNIT")
    orders->qual[d1.seq].volume_dose_unit = od.oe_field_display_value
   ENDIF
   IF (od.oe_field_meaning="DRUGFORM")
    orders->qual[d1.seq].drug_form = od.oe_field_display_value
   ENDIF
   IF (od.oe_field_meaning="FREQ")
    orders->qual[d1.seq].frequency = od.oe_field_display_value
   ENDIF
   IF (od.oe_field_meaning="RXROUTE")
    orders->qual[d1.seq].rx_route = od.oe_field_display_value
   ENDIF
   IF (od.oe_field_meaning="RXPRIORITY")
    orders->qual[d1.seq].rx_priority = od.oe_field_display_value
   ENDIF
 ...
 FOR (x = 1 TO order_cnt)
   IF ((orders->qual[x].iv_ind=1))
    SELECT INTO "nl:"
     o_mnem = substring(1,30,oi.order_mnemonic), oas_mnem = substring(1,30,oi.ordered_as_mnemonic),
     dtl = trim(oi.order_detail_display_line),
     oi.comp_sequence, oi.action_sequence
     FROM order_ingredient oi
     PLAN (oi
      WHERE (oi.order_id=orders->qual[x].order_id))
     ORDER BY oi.action_sequence, oi.comp_sequence
     HEAD REPORT
      r_cnt = 0, i_cnt = 0, mnemonic_line = fillstring(1000," ")
     HEAD oi.action_sequence
      r_cnt = 0
     DETAIL
      i_cnt = oi.comp_sequence, orders->qual[x].mnem_ln_cnt = i_cnt, stat = alterlist(orders->qual[x]
       .mnem_ln_qual,i_cnt)
      IF (oi.ordered_as_mnemonic > " ")
       mnemonic_line = concat(trim(oi.ordered_as_mnemonic),", ",trim(oi.order_detail_display_line)),
       orders->qual[x].mnem_ln_qual[i_cnt].mnem_line = mnemonic_line
      ELSE
 ...
 IF ((((orders->spoolout_ind=1)) OR ((request->print_prsnl_id > 0))) )
  ...
  SELECT INTO value(request->printer_name)
   ...
   HEAD PAGE
    ...
    xcol = 40, ycol = (ycol+ 12),
    CALL print(calcpos(xcol,ycol)),
    "{cpi/12}Order entered by: ", orders->qual[1].enter_by, row + 1,
    xcol = 40, ycol = (ycol+ 12), first_page = "Y"
   DETAIL
    FOR (vv = 1 TO value(order_cnt))
      IF ((orders->qual[vv].display_ind=1))
       ...
       ELSE
        IF (vv > 1)
         break_variable = (ycol+ ((((orders->qual[vv].d_cnt/ 2)+ orders->qual[vv].com_ln_cnt)+ 9) *
         12))
       ...
       line1, "{endb}", row + 1,
       xcol = 40, ycol = (ycol+ 21),
       CALL print(calcpos(xcol,ycol)),
       "{cpi/14}Ordering MD: ", orders->qual[vv].order_dr, row + 1,
       xcol = 40, ycol = (ycol+ 11),
       CALL print(calcpos(xcol,ycol)),
       "Order Type: ", orders->qual[vv].type, row + 1,
       xcol = 40, ycol = (ycol+ 11),
       CALL print(calcpos(xcol,ycol)),
       "Order Action: ", orders->qual[vv].action, row + 1
       FOR (i = 1 TO orders->qual[vv].mnem_ln_cnt)
         IF (first_time)
          first_time = 0, xcol = 30, ycol = (ycol+ 15),
          CALL print(calcpos(xcol,ycol)), "{cpi/12}{b}Procedure: ", ew = findstring(",",trim(orders->
            qual[vv].mnem_ln_qual[i].mnem_line)),
          ord_as = substring(1,(ew - 1),orders->qual[vv].mnem_ln_qual[i].mnem_line), ord_as, row + 1,
          xcol = 373,
          CALL print(calcpos(xcol,ycol)), "{b}Order ID: ",
          orders->qual[vv].order_id"#########", "{endb}", row + 1
         ELSE
          xcol = 50, ycol = (ycol+ 15),
          CALL print(calcpos(xcol,ycol)),
          "{cpi/12}Ingredient: ", orders->qual[vv].mnem_ln_qual[i].mnem_line, row + 1
         ENDIF
       ENDFOR
       lt_xcol = 60, rt_xcol = 310, jump = 12,
       ycol = (ycol+ (jump * 2)),
       CALL print(calcpos(lt_xcol,ycol)), "Start Date and Time: ",
       orders->qual[vv].req_st_dt, row + 1, ycol = (ycol+ jump),
       CALL print(calcpos(lt_xcol,ycol)), "Infuse Over: ", orders->qual[vv].infuse_over,
       orders->qual[vv].infuse_over_unit, row + 1, ycol = (ycol+ jump),
       CALL print(calcpos(lt_xcol,ycol)), "Rate: ", orders->qual[vv].rate,
       orders->qual[vv].rate_unit, row + 1, ycol = (ycol+ jump),
       CALL print(calcpos(lt_xcol,ycol)), "Volume Dose: ", orders->qual[vv].volume_dose,
       orders->qual[vv].volume_dose_unit, row + 1, ycol = (ycol+ jump),
       CALL print(calcpos(lt_xcol,ycol)), "Drug Form: ", orders->qual[vv].drug_form,
       row + 1, ycol = (ycol+ jump),
       CALL print(calcpos(lt_xcol,ycol)),
       "Route of Administration: ", orders->qual[vv].rx_route, row + 1,
       ycol = (ycol+ jump),
       CALL print(calcpos(lt_xcol,ycol)), "Frequency: ",
       orders->qual[vv].frequency, row + 1, ycol = (ycol+ jump),
       CALL print(calcpos(lt_xcol,ycol)), "Pharmacy Order Priority: ", orders->qual[vv].rx_priority
       IF ((orders->qual[vv].comment_ind=1)
        AND (orders->qual[vv].com_ln_cnt > 0))
        ocnt = orders->qual[vv].com_ln_cnt, xcol = 30, ycol = (ycol+ 15)
   ...
   FOOT PAGE
    ...
    "{cpi/12}{pos/270/676}{b}", "Printed: ", curdate,
    "  ", curtime, "{endb}",
    row + 1, "{cpi/12}{pos/270/687}", "{b/8}Patient: ",
    "{pos/311/687}", orders->name, "{pos/465/687}",
    "{b/10}Location: ", orders->location, row + 1,
    "{pos/270/698}", "{b/6}Acct #: ", orders->fnbr,
    "{pos/465/698}", "{b/5}ADM: ", orders->admit_dt,
    row + 1, "{pos/270/709}", "{b/5}MRN #: ",
    orders->mrn, "{pos/380/709}", "{b/5}Age: ",
    orders->age, "{pos/465/709}", "{b/5}Sex: ",
    orders->sex, row + 1, "{pos/270/720}",
    "{b/7}Weight: ", orders->weight, row + 1,
    "{pos/270/731}", "{b/13}Attending MD: ", orders->attending,
    row + 1, "{pos/465/731}", "Page # ",
    curpage, row + 1, "{cpi/10}{pos/30/693}{b}",
    orders->facility, row + 1, "{cpi/10}{pos/30/708}{b}",
    "TPN Order Requisition", "{font/8}{cpi/15}{lpi/10}{pos/30/720}", "BHS_RX_TPN"
