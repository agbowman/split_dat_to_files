CREATE PROGRAM cp_add_file_storage_param:dba
 ...
 SELECT INTO "nl:"
  cv.code_value
  FROM code_value cv
  WHERE cv.cdf_meaning IN ("PRINT")
   AND cv.code_set=22549
  HEAD REPORT
   do_nothing = 0
  DETAIL
   print_only_cd = cv.code_value
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  cv.code_value
 ...
 SELECT INTO "nl:"
  ...
  HEAD co.charting_operations_id
   ...
   temp_rec->temp_list[count1].highest_seq = 0, temp_rec->temp_list[count1].has_14 = 0
  DETAIL
   do_nothing = 0
   IF ((co.sequence > temp_rec->temp_list[count1].highest_seq))
    temp_rec->temp_list[count1].highest_seq = co.sequence
   ENDIF
   IF (co.param_type_flag=14
    AND co.active_ind=1)
    temp_rec->temp_list[count1].has_14 = 1
   ENDIF
 ...
 SET x = 0
 FOR (x = 1 TO count1)
   IF ((temp_rec->temp_list[x].has_14=0))
 ...
 SUBROUTINE insert_param_in_ops(index)
  INSERT  FROM charting_operations c
   SET c.charting_operations_id = temp_rec->temp_list[index].charting_operations_id, c.sequence = (
    ...
    c.batch_name_key = temp_rec->temp_list[index].batch_name_key, c.param_type_flag = 14, c.param =
    cnvtstring(print_only_cd),
    c.active_ind =
...
#exit_script
 ...
 IF (error_check != 0)
  ROLLBACK
  SET readme_data->message = "Errors in cp_add_file_storage_param - FAILURE"
 ...
 ELSE
  ...
  "Successfully added file-storage to charting_operations table - SUCCESSFUL"
