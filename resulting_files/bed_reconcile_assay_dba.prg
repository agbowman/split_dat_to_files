CREATE PROGRAM bed_reconcile_assay:dba
 ...
 DECLARE assayindex = i4 WITH protect, noconstant(0)
 DECLARE rrindex = i4 WITH protect, noconstant(0)
 DECLARE logfilename = vc WITH protect, noconstant("")
 DECLARE refrangesize = i4 WITH protect, noconstant(0)
 DECLARE removereferencerange(aidx=i4,rrfid=f8) = i2
 DECLARE handlealpharesponses(assayindex=i4,rridx=i4) = i2
 DECLARE removealpharesponse(aidx=i4,ridx=i4,nomenclatureid=f8) = i2
 DECLARE addalpharesponse(aidx=i4,ridx=i4,aruid=vc,rrfuid=vc,actionflag=i2) = i2
 DECLARE updateconceptckiincodeset72(taskassaycd=vc,conceptcki=vc,eventcodecki=vc) = i2
 CALL bedaddlogstologgerfile("#### ENTERING INTO BED_RECONCILE_ASSAY.PRG ####")
 CALL bedaddlogstologgerfile(cnvtrectoxml(request))
 FOR (assayindex = 1 TO size(request->assays,5))
   SET stat = alterlist(ensassayrequest->assay_list,assayindex)
   SET ensassayrequest->assay_list[assayindex].action_flag = update_flag
   SET ensassayrequest->assay_list[assayindex].code_value = request->assays[assayindex].taskassaycd
   SET ensassayrequest->assay_list[assayindex].display = request->assays[assayindex].display
   SET ensassayrequest->assay_list[assayindex].description = request->assays[assayindex].description
   SET ensassayrequest->assay_list[assayindex].general_info.result_type_code_value = request->assays[
   assayindex].resulttypecd
   SET ensassayrequest->assay_list[assayindex].general_info.single_select_ind = request->assays[
   assayindex].singleselectind
   SET ensassayrequest->assay_list[assayindex].general_info.io_flag = request->assays[assayindex].
   ioflag
   SET ensassayrequest->assay_list[assayindex].general_info.default_type_flag = request->assays[
   assayindex].defaulttypeflag
   SET ensassayrequest->assay_list[assayindex].general_info.concept_cki = request->assays[assayindex]
   .conceptcki
   SET refrangesize = size(request->assays[assayindex].refranges,5)
   IF (refrangesize > 0)
    FOR (rridx = 1 TO refrangesize)
      IF ((request->assays[assayindex].refranges[rridx].actionflag=add_flag))
       CALL handlereferencerange(assayindex,request->assays[assayindex].refranges[rridx].rrfuid,0.0,
        add_flag)
      ELSEIF ((request->assays[assayindex].refranges[rridx].actionflag=update_flag))
       CALL handlereferencerange(assayindex,request->assays[assayindex].refranges[rridx].rrfuid,
        request->assays[assayindex].refranges[rridx].rrfid,update_flag)
       CALL handlealpharesponses(assayindex,rridx)
      ELSEIF ((request->assays[assayindex].refranges[rridx].actionflag=remove_flag))
       CALL removereferencerange(assayindex,request->assays[assayindex].refranges[rridx].rrfid)
      ELSE
       SET rrindex = (size(ensassayrequest->assay_list[assayindex].rr_list,5)+ 1)
       SET stat = alterlist(ensassayrequest->assay_list[assayindex].rr_list,rrindex)
       SET ensassayrequest->assay_list[assayindex].rr_list[rrindex].action_flag = 0
       SET ensassayrequest->assay_list[assayindex].rr_list[rrindex].rrf_id = request->assays[
       assayindex].refranges[rridx].rrfid
       CALL handlealpharesponses(assayindex,rrindex)
      ENDIF
    ENDFOR
   ENDIF
   IF ((request->assays[assayindex].datamap.actionflag > 0))
    SET stat = alterlist(ensassayrequest->assay_list[assayindex].data_map,1)
    IF ((request->assays[assayindex].datamap.actionflag=remove_flag))
     SET ensassayrequest->assay_list[assayindex].data_map[1].action_flag = 4
     SET ensassayrequest->assay_list[assayindex].data_map[1].service_resource_code_value = 0.0
    ELSE
     SET ensassayrequest->assay_list[assayindex].data_map[1].action_flag = request->assays[assayindex
     ].datamap.actionflag
     SET ensassayrequest->assay_list[assayindex].data_map[1].service_resource_code_value = 0.0
     SET ensassayrequest->assay_list[assayindex].data_map[1].min_digits = request->assays[assayindex]
     .datamap.min_digits
     SET ensassayrequest->assay_list[assayindex].data_map[1].max_digits = request->assays[assayindex]
     .datamap.max_digits
     SET ensassayrequest->assay_list[assayindex].data_map[1].dec_place = request->assays[assayindex].
     datamap.min_decimal_places
    ENDIF
   ENDIF
   CALL executebedensassay(0)
   CALL updateconceptckiincodeset72(request->assays[assayindex].taskassaycd,request->assays[
    assayindex].conceptcki,request->assays[assayindex].eventcodecki)
   CALL updatewitnessrequiredind(request->assays[assayindex].taskassaycd,request->assays[assayindex].
    witnessrequiredind)
   FOR (offsetminindex = 1 TO size(request->assays[assayindex].offsetminutes,5))
     CALL updateoffsetminutes(request->assays[assayindex].offsetminutes[offsetminindex].actionflag,
      request->assays[assayindex].taskassaycd,request->assays[assayindex].offsetminutes[
      offsetminindex].offsetminnbr,request->assays[assayindex].offsetminutes[offsetminindex].
      offsettypecd)
   ENDFOR
   IF ((request->assays[assayindex].referencetextupdateind=true))
    CALL updatereferencetext(request->assays[assayindex].taskassayuid,request->assays[assayindex].
     taskassaycd)
   ENDIF
   IF (size(request->assays[assayindex].equations,5) > 0)
    FOR (ecnt = 1 TO size(request->assays[assayindex].equations,5))
      IF ((request->assays[assayindex].equations[ecnt].actionflag=add_flag))
       CALL addnewequation(request->assays[assayindex].equations[ecnt].equationuid,request->assays[
        assayindex].taskassaycd)
      ELSEIF ((request->assays[assayindex].equations[ecnt].actionflag=update_flag))
       CALL removeequation(request->assays[assayindex].equations[ecnt].equationid)
       CALL addnewequation(request->assays[assayindex].equations[ecnt].equationuid,request->assays[
        assayindex].taskassaycd)
      ELSEIF ((request->assays[assayindex].equations[ecnt].actionflag=remove_flag))
       CALL removeequation(request->assays[assayindex].equations[ecnt].equationid)
      ENDIF
    ENDFOR
   ENDIF
   IF ((request->assays[assayindex].interpretationsupdateind > 0))
    CALL removeinterp(request->assays[assayindex].taskassaycd)
    SELECT INTO "nl:"
     FROM cnt_dcp_interp2 i
     WHERE (i.task_assay_uid=request->assays[assayindex].taskassayuid)
     WITH nocounter
    ;end select
    IF (curqual > 0)
     CALL bedaddlogstologgerfile(build2("### Inside Main Script Logic and curqual value is >0",
       curqual))
     CALL bedaddlogstologgerfile("### Calling addNewInterp...")
     CALL addnewinterp(request->assays[assayindex].taskassayuid,request->assays[assayindex].
      taskassaycd)
    ENDIF
   ENDIF
 ENDFOR
 IF (validate(request->cclloggingind))
  ...
  SET logfilename = request->powerformoriviewname
  CALL bedaddlogstologgerfile("#### EXITING BED_RECONCILE_ASSAY.PRG ####")
  IF ((request->powerformoriviewind=1))
   CALL writeiviewlogstothefile(logfilename,request->cclloggingind)
  ELSEIF ((request->powerformoriviewind=0))
   CALL writelogstothefile(logfilename,request->cclloggingind)
  ENDIF
 ENDIF
#exit_script
 CALL bedexitscript(1)
 SUBROUTINE handlealpharesponses(assayindex,rridx)
   CALL bedlogmessage("handleAlphaResponses","Entering ...")
   CALL bedaddlogstologgerfile("### Entering handleAlphaResponses...")
   CALL bedaddlogstologgerfile(build2("### Inside handleAlphaResponses - Assay Index value:",
     assayindex," rrIdx value:",rridx))
   IF (size(request->assays[assayindex].refranges[rridx].alpharesponses,5) > 0)
    FOR (arcnt = 1 TO size(request->assays[assayindex].refranges[rridx].alpharesponses,5))
      IF ((request->assays[assayindex].refranges[rridx].alpharesponses[arcnt].actionflag=add_flag))
       CALL addalpharesponse(assayindex,rrindex,request->assays[assayindex].refranges[rridx].
        alpharesponses[arcnt].aruid,request->assays[assayindex].refranges[rridx].rrfuid,add_flag)
      ELSEIF ((request->assays[assayindex].refranges[rridx].alpharesponses[arcnt].actionflag=
      update_flag))
       CALL addalpharesponse(assayindex,rrindex,request->assays[assayindex].refranges[rridx].
        alpharesponses[arcnt].aruid,request->assays[assayindex].refranges[rridx].rrfuid,update_flag)
      ELSEIF ((request->assays[assayindex].refranges[rridx].alpharesponses[arcnt].actionflag=
      remove_flag))
       CALL removealpharesponse(assayindex,rridx,request->assays[assayindex].refranges[rridx].
        alpharesponses[arcnt].nomenclatureid)
      ENDIF
    ENDFOR
   ENDIF
   CALL bedlogmessage("handleAlphaResponses","Exiting ...")
   CALL bedaddlogstologgerfile("### Exiting handleAlphaResponses...")
 END ;Subroutine
 SUBROUTINE removereferencerange(aidx,rrfid)
   CALL bedlogmessage("removeReferenceRange","Entering ...")
   CALL bedaddlogstologgerfile("### Entering removeReferenceRange...")
   CALL bedaddlogstologgerfile(build2("### Inside removeReferenceRange - aIdx value:",aidx,
     " rrfId value:",rrfid))
   DECLARE r = i4 WITH protect, noconstant(0)
   SET r = (size(ensassayrequest->assay_list[aidx].rr_list,5)+ 1)
   SET stat = alterlist(ensassayrequest->assay_list[aidx].rr_list,r)
   SET ensassayrequest->assay_list[aidx].rr_list[r].action_flag = remove_flag
   SET ensassayrequest->assay_list[aidx].rr_list[r].rrf_id = rrfid
   CALL bedlogmessage("handleAlphaResponses","Exiting ...")
   CALL bedaddlogstologgerfile("### Exiting removeReferenceRange...")
 END ;Subroutine
 SUBROUTINE addalpharesponse(aidx,ridx,aruid,rrfuid,actionflag)
   CALL bedlogmessage("addAlphaResponse","Entering ...")
   CALL bedaddlogstologgerfile("### Entering addAlphaResponse...")
   CALL bedaddlogstologgerfile(build2("###Inside addAlphaResponse-aIdx value:",aidx,"rIdx value:",
     ridx,"arUID value:",
     aruid))
   CALL bedaddlogstologgerfile(build2("###Inside addAlphaResponse-rrfUID value:",rrfuid,
     "Action Flag:",actionflag))
   DECLARE a = i4 WITH protect, noconstant(0)
   FREE RECORD alpharec
   RECORD alpharec(
     1 sequence = i4
     1 resultprocesscd = f8
     1 resultprocessuid = vc
     1 defaultind = i2
     1 useunitsind = i2
     1 referenceind = i2
     1 resultvalue = f8
     1 sortorder = i4
     1 nomenclatureid = f8
     1 principletypecd = f8
     1 principletypeuid = vc
     1 sourceidentifier = vc
     1 sourcestring = vc
     1 sourcevocabcd = f8
     1 sourcevocabuid = vc
     1 shortstring = vc
     ...
     1 contributorsystemuid = vc
     1 conceptcki = vc
     1 conceptidentifier = vc
     1 vocabaxiscd = f8
     1 vocabaxisuid = vc
     1 truthstatecd = f8
     1 truthstateuid = vc
   ...
   SELECT INTO "nl:"
    FROM cnt_rrf_ar_r arr,
     ...
     cnt_alpha_response ar
    PLAN (arr
     WHERE arr.ar_uid=aruid
      AND arr.rrf_uid=rrfuid)
     JOIN (ark
     WHERE ark.ar_uid=arr.ar_uid)
     JOIN (ar
     WHERE ar.ar_uid=ark.ar_uid)
    ORDER BY arr.ar_uid
    HEAD arr.ar_uid
     alpharec->sequence = arr.ar_sequence, alpharec->resultprocesscd = arr.result_process_cd,
     alpharec->resultprocessuid = arr.result_process_cduid,
     alpharec->defaultind = arr.default_ind, alpharec->useunitsind = arr.use_units_ind, alpharec->
     referenceind = arr.reference_ind,
     alpharec->resultvalue = arr.result_value, alpharec->sortorder = arr.multi_alpha_sort_order,
     alpharec->nomenclatureid = ark.nomenclature_id,
     alpharec->principletypecd = ark.principle_type_cd, alpharec->principletypeuid = ark
     .principle_type_cduid, alpharec->sourceidentifier = ark.source_identifier,
     alpharec->sourcestring = ark.source_string, alpharec->sourcevocabcd = ark.source_vocabulary_cd,
     alpharec->sourcevocabuid = ark.source_vocabulary_cduid,
     alpharec->shortstring = ar.short_string, alpharec->mnemonic = ar.mnemonic, alpharec->
     contributorsystemcd = ar.contributor_system_cd,
     alpharec->contributorsystemuid = ar.contributor_system_cduid, alpharec->conceptcki = ar
     .concept_cki, alpharec->conceptidentifier = ar.concept_identifier,
     alpharec->vocabaxiscd = ar.vocab_axis_cd, alpharec->vocabaxisuid = ar.vocab_axis_cduid, alpharec
     ->truthstatecd = arr.truth_state_cd,
     alpharec->truthstateuid = arr.truth_state_cduid
    WITH nocounter
   ;end select
   IF ((alpharec->resultprocesscd=0)
    AND size(trim(alpharec->resultprocessuid,7),1) > 0)
    SET alpharec->resultprocesscd = findcodevalueforuid(alpharec->resultprocessuid)
   ENDIF
   IF ((alpharec->principletypecd=0)
    AND size(trim(alpharec->principletypeuid,7),1) > 0)
    SET alpharec->principletypecd = findcodevalueforuid(alpharec->principletypeuid)
   ENDIF
   IF ((alpharec->sourcevocabcd=0)
    AND size(trim(alpharec->sourcevocabuid,7),1) > 0)
    SET alpharec->sourcevocabcd = findcodevalueforuid(alpharec->sourcevocabuid)
   ENDIF
   IF ((alpharec->contributorsystemcd=0)
    AND size(trim(alpharec->contributorsystemuid,7),1) > 0)
    SET alpharec->contributorsystemcd = findcodevalueforuid(alpharec->contributorsystemuid)
   ENDIF
   IF ((alpharec->vocabaxiscd=0))
    SET alpharec->vocabaxiscd = findcodevalueforuid(alpharec->vocabaxisuid)
   ENDIF
   IF ((alpharec->truthstatecd=0))
    SET alpharec->truthstatecd = findcodevalueforuid(alpharec->truthstateuid)
   ENDIF
   IF ((alpharec->nomenclatureid=0))
    SET alpharec->nomenclatureid = getnomenclatureid(actionflag,aruid,alpharec->sourcevocabcd,
     alpharec->sourceidentifier,alpharec->sourcestring,
     alpharec->principletypecd,alpharec->shortstring,alpharec->mnemonic,alpharec->contributorsystemcd,
     alpharec->conceptcki,
     alpharec->conceptidentifier,alpharec->vocabaxiscd)
    CALL updatenomenidoncnt(aruid,alpharec->nomenclatureid)
   ENDIF
   CALL bedaddlogstologgerfile(cnvtrectoxml(alpharec))
   SET a = (size(ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list,5)+ 1)
   SET stat = alterlist(ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list,a)
   SET ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list[a].action_flag = actionflag
   SET ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list[a].nomenclature_id = alpharec->
   nomenclatureid
   SET ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list[a].sequence = alpharec->sequence
   SET ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list[a].default_ind = alpharec->
   defaultind
   SET ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list[a].use_units_ind = alpharec->
   useunitsind
   SET ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list[a].reference_ind = alpharec->
   referenceind
   SET ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list[a].result_process_code_value =
   alpharec->resultprocesscd
   SET ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list[a].result_value = alpharec->
   resultvalue
   SET ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list[a].multi_alpha_sort_order =
   alpharec->sortorder
   SET ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list[a].short_string = alpharec->
   shortstring
   SET ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list[a].truth_state_cd = alpharec->
   truthstatecd
   CALL bedlogmessage("addAlphaResponse","Exiting ...")
   CALL bedaddlogstologgerfile("### Exiting addAlphaResponse...")
 END ;Subroutine
 SUBROUTINE removealpharesponse(aidx,ridx,nomenclatureid)
   CALL bedlogmessage("removeAlphaResponse","Entering ...")
   CALL bedaddlogstologgerfile("### Entering removeAlphaResponse...")
   CALL bedaddlogstologgerfile(build2("### Inside removeAlphaResponse-aIdx value:",aidx,"rIdx value:",
     ridx))
   CALL bedaddlogstologgerfile(build2("### Inside removeAlphaResponse - Nomenclature Id:",
     nomenclatureid))
   DECLARE a = i4 WITH protect, noconstant(0)
   SET a = (size(ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list,5)+ 1)
   SET stat = alterlist(ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list,a)
   SET ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list[a].action_flag = remove_flag
   SET ensassayrequest->assay_list[aidx].rr_list[ridx].alpha_list[a].nomenclature_id = nomenclatureid
   CALL bedlogmessage("removeAlphaResponse","Exiting ...")
   CALL bedaddlogstologgerfile("### Exiting removeAlphaResponse...")
 END ;Subroutine
 SUBROUTINE updateconceptckiincodeset72(taskassaycd,conceptcki,eventcodecki)
   CALL bedlogmessage("updateConceptCkiInCodeSet72","Entering ...")
   CALL bedaddlogstologgerfile("### Entering updateConceptCkiInCodeSet72...")
   CALL bedaddlogstologgerfile(build2("###Inside updateConceptCkiInCodeSet72-TaskAssay Code:",
     taskassaycd,"Concept CKI:",conceptcki))
   CALL bedaddlogstologgerfile(build2("### Inside updateConceptCkiInCodeSet72 - Event Code CKI:",
     eventcodecki))
   DECLARE eventcd = f8 WITH protect, noconstant(0.0)
   DECLARE cvwithsamecki = f8 WITH protect, noconstant(0.0)
   SELECT INTO "nl:"
   ...
   SELECT INTO "nl:"
    FROM code_value c
    WHERE c.cki=eventcodecki
     AND c.active_ind=1
    DETAIL
     cvwithsamecki = c.code_value
    WITH nocounter
   ;end select
   CALL bedaddlogstologgerfile(build2("### Inside updateConceptCkiInCodeSet72-Event Code:",eventcd,
     "cvWithSameCki:",cvwithsamecki))
   IF (cvwithsamecki > 0
    AND cvwithsamecki != eventcd)
    CALL echo(build2("Cannot update CKI:",eventcodecki,". It's a duplicate!"))
    CALL echo(build2("Event cd I'm trying to update:",eventcd))
    CALL echo(build2("Code value with same cki:",cvwithsamecki))
   ELSE
    IF (eventcd > 0)
     UPDATE  FROM code_value cv
      SET cv.concept_cki = conceptcki, cv.cki =
       IF (eventcodecki != "") eventcodecki
       ELSE cv.cki
       ENDIF
       , cv.updt_dt_tm = cnvtdatetime(curdate,curtime3),
       cv.updt_id = reqinfo->updt_id, cv.updt_task = reqinfo->updt_task, cv.updt_cnt = (cv.updt_cnt+
       1),
       cv.updt_applctx = reqinfo->updt_applctx
      PLAN (cv
       WHERE cv.code_value=eventcd)
      WITH nocounter
     ;end update
     CALL bedaddlogstologgerfile(
      "### Inside updateConceptCkiInCodeSet72 - Updated the code_value table...")
    ENDIF
   ENDIF
   CALL bedlogmessage("updateConceptCkiInCodeSet72","Exiting ...")
   CALL bedaddlogstologgerfile("### Exiting updateConceptCkiInCodeSet72...")
 END ;Subroutine
END GO
