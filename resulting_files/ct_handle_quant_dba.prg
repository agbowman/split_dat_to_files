CREATE PROGRAM ct_handle_quant:dba
 ...
 SET failed = false
 SET reply_quant->status_data.status = "F"
 SET dummyv = 1
 SET table_name = "CHARGE"
 CALL add_charge(dummyv)
 IF (failed != false)
  GO TO check_error
 ENDIF
#check_error
 IF (failed=false)
  SET reply_quant->status_data.status = "S"
  SET reqinfo->commit_ind = true
 ELSE
  CASE (failed)
   OF gen_nbr_error:
    SET reply_quant->status_data.subeventstatus[1].operationname = "GEN_NBR"
   OF insert_error:
    SET reply_quant->status_data.subeventstatus[1].operationname = "INSERT"
   OF update_error:
    SET reply_quant->status_data.subeventstatus[1].operationname = "UPDATE"
   OF replace_error:
    SET reply_quant->status_data.subeventstatus[1].operationname = "REPLACE"
   OF delete_error:
    SET reply_quant->status_data.subeventstatus[1].operationname = "DELETE"
   OF undelete_error:
    SET reply_quant->status_data.subeventstatus[1].operationname = "UNDELETE"
   OF remove_error:
    SET reply_quant->status_data.subeventstatus[1].operationname = "REMOVE"
   OF attribute_error:
    SET reply_quant->status_data.subeventstatus[1].operationname = "ATTRIBUTE"
   OF lock_error:
    SET reply_quant->status_data.subeventstatus[1].operationname = "LOCK"
   ELSE
    SET reply_quant->status_data.subeventstatus[1].operationname = "UNKNOWN"
  ENDCASE
  SET reply_quant->status_data.subeventstatus[1].operationstatus = "F"
  SET reply_quant->status_data.subeventstatus[1].targetobjectname = "TABLE"
  SET reply_quant->status_data.subeventstatus[1].targetobjectvalue = table_name
 ...
 SUBROUTINE add_charge(i)
   ...
   SELECT INTO "nl:"
    FROM charge c
    WHERE (c.charge_item_id=request_quant->charge_item_id)
    DETAIL
     temprequest->parent_charge_item_id = request_quant->charge_item_id, temprequest->
     charge_event_act_id = c.charge_event_act_id, temprequest->charge_event_id = c.charge_event_id,
     ...
     temprequest->bundle_id = c.bundle_id, temprequest->institution_cd = c.institution_cd,
     temprequest->department_cd = c.department_cd,
     temprequest->section_cd = c.section_cd, temprequest->subsection_cd = c.subsection_cd,
     temprequest->level5_cd = c.level5_cd,
     ...
     temprequest->fin_class_cd = c.fin_class_cd
    WITH nocounter
   ;end select
   SET count1 = 0
   SELECT INTO "nl:"
    FROM charge_mod cm
    WHERE (cm.charge_item_id=request_quant->charge_item_id)
     AND cm.active_ind=1
    DETAIL
   ...
   FOR (v = 1 TO 2)
     ...
     INSERT  FROM charge c
      SET c.charge_item_id = new_nbr, c.parent_charge_item_id = request_quant->charge_item_id, c
       .charge_event_act_id = temprequest->charge_event_act_id,
       c.charge_event_id = temprequest->charge_event_id, c.bill_item_id = temprequest->bill_item_id,
       c.order_id = temprequest->order_id,
       c.encntr_id = temprequest->encntr_id, c.person_id = temprequest->person_id, c.payor_id =
       temprequest->payor_id,
       c.ord_loc_cd = temprequest->ord_loc_cd, c.perf_loc_cd = temprequest->perf_loc_cd, c
       .ord_phys_id = temprequest->ord_phys_id,
       c.perf_phys_id = temprequest->perf_phys_id, c.charge_description = temprequest->
       charge_description, c.price_sched_id = temprequest->price_sched_id,
       c.item_quantity =
       IF (v=1) 1
       ELSE (temprequest->item_quantity - 1)
       ENDIF
       , c.item_price = temprequest->item_price, c.item_extended_price =
       IF (v=1) temprequest->item_price
       ELSE ((temprequest->item_quantity - 1) * temprequest->item_price)
       ENDIF
       ,
       ...
       .discount_amount = (temprequest->gross_price - temprequest->item_price),
       c.manual_ind = temprequest->manual_ind, c.combine_ind = temprequest->combine_ind, c.bundle_id
        = temprequest->bundle_id,
       c.institution_cd = temprequest->institution_cd, c.department_cd = temprequest->department_cd,
       c.section_cd = temprequest->section_cd,
       ...
       c.abn_status_cd = temprequest->abn_status_cd, c.health_plan_id = temprequest->health_plan_id,
       c.fin_class_cd = temprequest->fin_class_cd,
       c.updt_cnt = 0, c.updt_dt_tm = cnvtdatetime(sysdate), c.updt_id = reqinfo->updt_id,
       c.updt_applctx = reqinfo->updt_applctx, c.updt_task = reqinfo->updt_task, c.original_encntr_id
        =
       IF (validate(request_quant->charge_item_id,0)=0) temprequest->encntr_id
       ELSE
        (SELECT
     ...
     IF (curqual > 0)
      ...
      CALL echo(build("Original charge_item_id: ",request_quant->charge_item_id))
     ENDIF
       ...
       INSERT  FROM charge_mod cm
        SET cm.active_ind = bill_code->bill_code[z].active_ind, cm.active_status_cd = bill_code->
         bill_code[z].active_status_cd, cm.active_status_dt_tm = cnvtdatetime(bill_code->bill_code[z]
          .active_status_dt_tm),
         cm.active_status_prsnl_id = bill_code->bill_code[z].active_status_prsnl_id, cm
         .activity_dt_tm = cnvtdatetime(bill_code->bill_code[z].activity_dt_tm), cm
         .beg_effective_dt_tm = cnvtdatetime(bill_code->bill_code[z].beg_effective_dt_tm),
         cm.charge_item_id = cid_new_nbr, cm.charge_mod_id = new_nbr, cm.charge_mod_type_cd =
         bill_code->bill_code[z].charge_mod_type_cd,
         cm.code1_cd = bill_code->bill_code[z].code1_cd, cm.end_effective_dt_tm = cnvtdatetime(
          bill_code->bill_code[z].end_effective_dt_tm), cm.field1 = bill_code->bill_code[z].field1,
         cm.field10 = bill_code->bill_code[z].field10, cm.field1_id = bill_code->bill_code[z].
         field1_id, cm.field2 = bill_code->bill_code[z].field2,
     ...
     IF (v=1)
      ...
      SELECT INTO "nl:"
       ...
       DETAIL
        count1 += 1, stat = alterlist(reply_quant->charge_mods,count1), reply_quant->charge_mods[
        count1].charge_mod_id = cm.charge_mod_id
       WITH nocounter
      ;end select
     ENDIF
   ENDFOR
   CALL echo("leaving ct_handle_quant")
   IF (curqual=0)
