CREATE PROGRAM bhs_gvw_sn_urine_output:dba
 FREE RECORD m_surg
 RECORD m_surg(
   1 ulst[*]
     2 s_indwell_cath_vol = vc
     2 s_straight_cath_vol = vc
 ) WITH protect
 ...
 DECLARE mf_indwell_cath_vol_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "SNINDWELLINGCATHVOLUME"))
 DECLARE mf_straight_cath_vol_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "SNSTRAIGHTCATHVOLUME"))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (sc
   ...
   WHERE ifr.input_form_cd=sh.input_form_cd
    ...
    AND ifr.display IN ("Devices placed in the OR")
   ...
   WHERE ce2.parent_event_id=ce1.event_id
    ...
    AND ce2.event_cd IN (mf_indwell_cath_vol_cd, mf_straight_cath_vol_cd)
  ...
  HEAD ce2.collating_seq
   ml_uknt += 1, u0 = alterlist(m_surg->ulst,ml_uknt)
  HEAD ce2.event_cd
   CASE (ce2.event_cd)
    OF mf_indwell_cath_vol_cd:
     m_surg->ulst[ml_uknt].s_indwell_cath_vol = replace(ce2.result_val,"#",", ")
    OF mf_straight_cath_vol_cd:
     m_surg->ulst[ml_uknt].s_straight_cath_vol = trim(ce2.result_val)
   ENDCASE
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM dummyt d1
  HEAD REPORT
   ms_text_temp = ms_rhead, ms_text_temp = concat(ms_text_temp,ms_rh2b,"{Urine Output}",ms_reol)
   IF (size(m_surg->ulst,5) > 0)
    FOR (ux = 1 TO size(m_surg->ulst,5))
     ms_text_temp = concat(ms_text_temp,ms_wr,"{Indwelling Cath Volume: ",m_surg->ulst[ux].
      s_indwell_cath_vol,"}",
      ms_reol),ms_text_temp = concat(ms_text_temp,ms_wr,"{Straight Cath Volume: ",m_surg->ulst[ux].
      s_straight_cath_vol,"}",
      ms_reol)
    ENDFOR
   ENDIF
