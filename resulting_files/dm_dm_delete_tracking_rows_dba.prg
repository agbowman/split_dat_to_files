CREATE PROGRAM dm_dm_delete_tracking_rows:dba
 ...
 SET reply->table_name = "DM_DELETE_TRACKING"
 SET reply->rows_between_commit = minval(100,request->max_rows)
 SET i18nhandle = 0
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 DECLARE months_to_keep = f8 WITH protect, noconstant(6.0)
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(d.dm_delete_tracking_id)
   FROM dm_delete_tracking d
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(d.dm_delete_tracking_id)
  FROM dm_delete_tracking d
  WHERE d.last_utc_ts < cnvtlookbehind(strunit,cnvtdatetime(curdate,curtime3))
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    d.rowid
    FROM dm_delete_tracking d
    WHERE d.dm_delete_tracking_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists(
      "d.last_utc_ts < cnvtlookbehind(strUnit,cnvtdatetime(curdate,curtime3))","d"))
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
