CREATE PROGRAM bed_aud_core_chg_serv:dba
 ...
 SET reply->collist[7].header_text = "Missing Charge Processing"
 SET reply->collist[7].data_type = 1
 SET reply->collist[7].hide_ind = 0
 SET reply->collist[8].header_text = "CPT/HCPCS Not Linked"
 SET reply->collist[8].data_type = 1
 SET reply->collist[8].hide_ind = 0
 IF (cdms_used=1)
  SET reply->collist[9].header_text = "Missing CDM"
 ...
 IF (bcnt > 0)
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    missing_chg_proc_ind = temp->bilist[d.seq].missing_chg_proc_ind, temp2->bilist[bidx].
 ...
 IF (bcnt > 0)
  SELECT INTO "nl:"
   ...
   HEAD bi_id
    ...
    IF (cdms_used=1)
     stat = alterlist(reply->rowlist[rcnt].celllist,9)
    ELSE
     stat = alterlist(reply->rowlist[rcnt].celllist,8)
    ENDIF
   ...
   DETAIL
    ...
    IF ((temp2->bilist[d.seq].missing_chg_proc_ind=1))
     missing_chg_proc_cnt = (missing_chg_proc_cnt+ 1), reply->rowlist[rcnt].celllist[7].string_value
      = "X"
    ENDIF
    IF ((temp2->bilist[d.seq].cpthcpcs_not_linked_ind=1))
     cpthcpcs_not_linked_cnt = (cpthcpcs_not_linked_cnt+ 1), reply->rowlist[rcnt].celllist[8].
     string_value = "X"
    ENDIF
    IF (cdms_used=1
     AND (temp2->bilist[d.seq].missing_cdm_ind=1))
     missing_cdm_cnt = (missing_cdm_cnt+ 1), reply->rowlist[rcnt].celllist[9].string_value = "X"
    ENDIF
   WITH nocounter
  ;end select
 ENDIF
 ...
 SET reply->statlist[1].statistic_meaning = "CHGSERVMISSINGPRICE"
 IF (missing_price_cnt > 0)
 ...
 SET reply->statlist[2].qualifying_items = missing_chg_proc_cnt
 SET reply->statlist[2].statistic_meaning = "CHGSERVMISSINGCHGPROC"
 ...
 SET reply->statlist[3].qualifying_items = cpthcpcs_not_linked_cnt
 SET reply->statlist[3].statistic_meaning = "CHGSERVCPTHCPCSNOTLINKED"
 ...
 IF (cdms_used=1)
  ...
  SET reply->statlist[4].qualifying_items = missing_cdm_cnt
  SET reply->statlist[4].statistic_meaning = "CHGSERVMISSINGCDM"
...
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("core_chg_serv_audit.csv")
 ENDIF
