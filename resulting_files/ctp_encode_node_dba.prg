CREATE PROGRAM ctp_encode_node:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Node (to execute on)" = "",
  "Operation (ENCODE / DECODE)" = "",
  "File" = ""
  WITH outdev, node, operation,
  file
 ...
 WITH copy = 0
 SUBROUTINE (loadoutput(node_name=vc,out_file_name=vc,file_content=vc(ref),msg=vc(ref)) =i2 WITH
  protect)
    ...
    IF (DISCERN::eksgetsource.crmgetrequest(0))
     ...
     IF (DISCERN::eksgetsource.crmperformas(node_name,"cpmscript"))
      IF (DISCERN::eksgetsource.crmgetreply(0))
       ...
       ELSE
        CALL DISCERN::eksgetsource.logerror(concat(DISCERN::eksgetsource.getrequestname(0),
   ...
   RETURN(success)
 END ;Subroutine
 SUBROUTINE (executeencode(operation=vc,file_name=vc,out_file_name=vc,node_name=vc,msg=vc(ref)) =i2
  WITH protect)
   DECLARE hreq = i4 WITH protect, noconstant(0)
   DECLARE hrep = i4 WITH protect, noconstant(0)
   ...
   IF (DISCERN::vcclrunprogram.crmbegin(0))
    IF (DISCERN::vcclrunprogram.crmgetrequest(0))
     ...
     SET stat = uar_srvsetstring(hreq,"program_name","CTP_ENCODE")
     SET stat = uar_srvsetstring(hreq,"output_device","NL:")
     SET stat = uar_srvsetstring(hreq,"IsBlob","1")
     SET stat = uar_srvsetstring(hreq,"params",nullterm(build('"',out_file_name,'","',operation,'","',
        file_name,'"')))
     SET hqualreq = uar_srvadditem(hreq,"qual")
     SET stat = uar_srvsetstring(hqualreq,"parameter",nullterm(out_file_name))
     SET hqualreq = uar_srvadditem(hreq,"qual")
     SET stat = uar_srvsetstring(hqualreq,"parameter",nullterm(operation))
     SET hqualreq = uar_srvadditem(hreq,"qual")
     SET stat = uar_srvsetstring(hqualreq,"parameter",nullterm(file_name))
 ...
 IF (executeencode( $OPERATION, $FILE,outdev_file, $NODE,error_message))
  IF (loadoutput( $NODE,outdev_file,contents,error_message))
   SET success = true
  ENDIF
 ENDIF
 IF (success)
