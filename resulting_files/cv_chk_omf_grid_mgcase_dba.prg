CREATE PROGRAM cv_chk_omf_grid_mgcase:dba
 SET request->setup_proc[1].success_ind = 1
 SET request->setup_proc[1].error_msg = fillstring(200," ")
 SET request->setup_proc[1].error_msg = "CV_OMF_GRID_MGCASE Successful.  Count= "
 SET v_count1 = 0
 SET v_expected_count = 7
 SELECT INTO "nl:"
  table_count = count(*)
  FROM omf_grid og,
   code_value cv
 ...
 IF (v_count1 < v_expected_count)
  ...
  SET request->setup_proc[1].error_msg = concat("CV_OMF_GRID_MGCASE: Expected ",trim(cnvtstring(
