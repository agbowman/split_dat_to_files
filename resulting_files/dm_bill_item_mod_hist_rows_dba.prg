CREATE PROGRAM dm_bill_item_mod_hist_rows:dba
 ...
 SET reply->table_name = "BILL_ITEM_MODIFIER_HIST"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(bimh.bill_item_modifier_hist_id)
   FROM bill_item_modifier_hist bimh
   WHERE bimh.bill_item_modifier_hist_id > 0.0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(bimh.bill_item_modifier_hist_id)
  FROM bill_item_modifier_hist bimh
  WHERE bimh.bill_item_modifier_hist_id > 0.0
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    FROM bill_item_modifier_hist bimh
    WHERE bimh.updt_dt_tm <= cnvtdatetime((curdate - v_days_to_keep),curtime3)
     AND bimh.permanent_del_ind=1
     AND bimh.bill_item_modifier_hist_id BETWEEN curminid AND curmaxid
     AND parser(sbr_getrowidnotexists("bimh.bill_item_modifier_hist_id != 0","bimh"))
    DETAIL
     row_cnt = (row_cnt+ 1)
     IF (mod(row_cnt,100)=1)
      stat = alterlist(reply->rows,(row_cnt+ 99))
     ENDIF
     reply->rows[row_cnt].row_id = bimh.rowid
    WITH nocounter, maxqual(bimh,value(rowsleft))
   ;end select
