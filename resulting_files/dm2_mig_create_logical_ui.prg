CREATE PROGRAM dm2_mig_create_logical_ui
 ...
 SUBROUTINE dm2_get_rdbms_version(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmclu_expidx = i4 WITH protect, noconstant(0)
 DECLARE dmclu_ind_tspace_name = vc WITH protect, noconstant("I_DM2MIG")
 DECLARE dmclu_idx = i4 WITH protect, noconstant(0)
 DECLARE dmclu_degree = i4 WITH protect, noconstant(0)
 FREE RECORD dmclu_ind_cmds
 RECORD dmclu_ind_cmds(
   1 cnt = i4
   1 qual[*]
     2 ind_cmd = vc
     2 ind_name = vc
 ...
 IF (check_logfile("dm2_mig_crtlui",".log","dm2_mig_create_logical_ui")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_mig_create_logical_ui"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (size(trim(reflect(parameter(1,0))),1) > 0)
  SET dmclu_degree = cnvtint(value(parameter(1,0)))
 ENDIF
 IF (dmclu_degree=0)
  SET dmclu_degree = 1
 ENDIF
 ...
 IF (dmr_prompt_connect_data("TARGET","V500","CO")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Check for index tspace override"
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2_MIG_LOGICAL_KEY_TSP_OVR"
  DETAIL
   dmclu_ind_tspace_name = d.info_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Check if ",dmclu_ind_tspace_name," exists.")
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  d.tablespace_name
  FROM dba_tablespaces d
  WHERE d.tablespace_name=dmclu_ind_tspace_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat(dmclu_ind_tspace_name," does not exist.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Scan for logical UI index to create."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info d,
   dba_objects do
  WHERE d.info_domain="DM2_MIG_LOGICAL_KEY"
   AND concat(do.owner,".",do.object_name)=d.info_name
   AND do.object_type="TABLE"
  DETAIL
   dmclu_ind_cmds->cnt = (dmclu_ind_cmds->cnt+ 1), stat = alterlist(dmclu_ind_cmds->qual,
    dmclu_ind_cmds->cnt), dmclu_ind_cmds->qual[dmclu_ind_cmds->cnt].ind_name = concat(
    "XIEDM2_MIG_LUI",trim(cnvtstring(do.object_id))),
   dmclu_ind_cmds->qual[dmclu_ind_cmds->cnt].ind_cmd = concat("create unique index ",trim(do.owner),
    ".",dmclu_ind_cmds->qual[dmclu_ind_cmds->cnt].ind_name," on ",
    trim(d.info_name)," ( ",trim(d.info_char)," ) tablespace ",dmclu_ind_tspace_name), dmclu_ind_cmds
   ->qual[dmclu_ind_cmds->cnt].ind_owner = do.owner, dmclu_ind_cmds->qual[dmclu_ind_cmds->cnt].
   tbl_owner = do.owner,
   dmclu_ind_cmds->qual[dmclu_ind_cmds->cnt].tbl_name = do.object_name, dmclu_ind_cmds->qual[
   dmclu_ind_cmds->cnt].exists_ind = 0
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ((dmclu_ind_cmds->cnt=0))
  SET dm_err->eproc = "No index creation required."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Check if index already exists"
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_indexes d
  WHERE expand(dmclu_expidx,1,dmclu_ind_cmds->cnt,d.owner,dmclu_ind_cmds->qual[dmclu_expidx].
   ind_owner,
   d.index_name,dmclu_ind_cmds->qual[dmclu_expidx].ind_name)
  DETAIL
   dmclu_idx = 0, dmclu_idx = locateval(dmclu_idx,1,dmclu_ind_cmds->cnt,d.owner,dmclu_ind_cmds->qual[
    dmclu_idx].ind_owner,
    d.index_name,dmclu_ind_cmds->qual[dmclu_idx].ind_name)
   IF (dmclu_idx > 0)
    dmclu_ind_cmds->qual[dmclu_idx].exists_ind = 1
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dmclu_ind_cmds)
 ENDIF
 IF ((validate(dmclu_report_option,- (1)) != - (1)))
  SET dm_err->eproc = "Generate report listing indexes which will be created."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO mine
   FROM (dummyt d  WITH seq = dmclu_ind_cmds->cnt)
   HEAD REPORT
    CALL print("INDEX CREATION REPORT"), row + 2,
    CALL print(concat("THE FOLLOWING INDEXES WILL BE CREATED IN TARGET DATABASE ",dmr_mig_data->
     tgt_db_name," RESIDING ON NODE:",dmr_mig_data->tgt_nodes[1].node_name)),
    row + 1,
    CALL print(
    "---------------------------------------------------------------------------------------------------------"
    ), row + 2
   DETAIL
    IF ((dmclu_ind_cmds->qual[d.seq].exists_ind=0))
     col 2,
     CALL print(dmclu_ind_cmds->qual[d.seq].ind_cmd), row + 1
    ENDIF
  ...
  SET message = window
  CALL clear(1,1)
  CALL text(3,2,"Would you like to continue with index creation? Y/N:")
  CALL accept(3,57,"A;CU"," "
   WHERE curaccept IN ("Y", "N"))
  SET message = nowindow
  IF (curaccept="N")
   SET dm_err->eproc = "User elected to not create indexes."
   CALL disp_msg("",dm_err->logfile,0)
   GO TO exit_script
  ENDIF
 ENDIF
 IF (dmclu_degree > 1)
  SET dm_err->eproc = "Activate parallel ddl for session"
  CALL disp_msg("",dm_err->logfile,0)
  IF (dm2_push_cmd(concat("rdb alter session force parallel ddl parallel ",cnvtstring(dmclu_degree),
    " go"),1)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 FOR (dmclu_idx = 1 TO dmclu_ind_cmds->cnt)
   IF ((dmclu_ind_cmds->qual[dmclu_idx].exists_ind=0))
    SET dm_err->eproc = concat("Creating index ",dmclu_ind_cmds->qual[dmclu_idx].ind_owner,".",
     dmclu_ind_cmds->qual[dmclu_idx].ind_name," on ",
     dmclu_ind_cmds->qual[dmclu_idx].tbl_owner,".",dmclu_ind_cmds->qual[dmclu_idx].tbl_name)
    CALL disp_msg("",dm_err->logfile,0)
    EXECUTE dm2_exec_child_cmd "CMD", concat("rdb asis(^",dmclu_ind_cmds->qual[dmclu_idx].ind_cmd,
     "^) go")
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
    ENDIF
   ENDIF
   EXECUTE dm2_exec_child_cmd "CMD", concat("rdb asis(^ alter index ",dmclu_ind_cmds->qual[dmclu_idx]
    .ind_owner,".",dmclu_ind_cmds->qual[dmclu_idx].ind_name," noparallel ^)")
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
 ...
 GO TO exit_script
#exit_script
 SET dm_err->eproc = "Inactivate parallel ddl for session"
 CALL dm2_push_cmd(concat("rdb alter session force parallel ddl parallel 1 go"),1)
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_mig_create_logical_ui completed"
 ENDIF
 CALL final_disp_msg("dm2_mig_crtlui")
END GO
