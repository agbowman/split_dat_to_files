CREATE PROGRAM dm_pucb_person_data_not_coll:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_PUCB_PERSON_DATA_NOT_COLL"
 ...
 SUBROUTINE cust_ucb_del(dummy)
   UPDATE  FROM person_data_not_coll
    ...
    WHERE (person_data_not_coll_id=rchildren->qual1[det_cnt].entity_id)
    WITH nocounter
   ;end update
   IF (curqual=0)
 ...
 SUBROUTINE initialize_hist_id(null)
  IF (v_hist_init_ind=0.0)
   ...
   SET dcipht_request->transaction_reason_txt = "DM_PUCB_PERSON_DATA_NOT_COLL"
