CREATE PROGRAM dm2_update_db_option
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_update_db_option",".log","Option_Update")=0)
  GO TO exit_program
 ENDIF
 SET reqinfo->updt_task = dm2_install_schema->dm2_updt_task_value
 SET dm_err->eproc = "Beginning DM2_UPDATE_DB_OPTION...."
 CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 DECLARE dudo_mod_opt_in = vc WITH protect, noconstant(cnvtupper( $1))
 DECLARE dudo_desire_opt_in = vc WITH protect, noconstant(cnvtupper( $2))
 DECLARE dudo_pass_in = vc WITH protect, noconstant(cnvtupper( $3))
 DECLARE dudo_valid_ind = i2 WITH protect, noconstant(0)
 DECLARE dudo_retval = i2 WITH protect, noconstant(0)
 DECLARE dudo_val_hold = vc WITH protect, noconstant("")
 DECLARE dudo_true_opt = vc WITH protect, noconstant("")
 DECLARE dudo_dg_state = vc WITH protect, noconstant("")
 DECLARE dudo_set_option(dso_opt_in=vc,dso_val_in=vc) = i2
 SET dudo_val_hold = dudo_desire_opt_in
 SET dm_err->eproc = "Verifying toolset usage"
 SET dudo_retval = dm2_toolset_usage(null)
 CASE (dudo_retval)
  OF 0:
   SET dm_err->eproc = "Error occurred while checking toolset usage."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  OF 1:
   SET dm_err->eproc = "DM2 Tools are being used."
  OF 2:
   SET dm_err->eproc =
   "This program cannot be used until the latest toolset is installed in this domain. Exiting Program."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
   GO TO exit_program
 ENDCASE
 SET dudo_retval = 0
 SET dm_err->eproc = "Check option validity"
 IF ( NOT (dudo_mod_opt_in IN ("COLBACKFILL_COMMIT_CNT", "DMT_FREELIST_GRP",
 "COLBACKFILL_MAXRANGEGROUPS", "RESOURCE_BUSY_RETRY_MINUTES", "DATABASE_STATISTICS_REPORT",
 "USE_INITPRM_ASSIGN_DG_IND", "ASSIGN_DG_OVERRIDE", "DEGREE_OF_PARALLEL_MAX", "DEGREE_OF_PARALLEL")))
  SET dm_err->emsg = concat("An invalid database option ,",dudo_mod_opt_in,
   ", was specified as the first parameter. ",
   "Please see the Install Tools overview guide on Cerner.com for a description ",
   "of valid database options that can be modified.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Validate entered values for ",dudo_mod_opt_in,". Value entered:",
  dudo_desire_opt_in,".")
 CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 CASE (dudo_mod_opt_in)
  OF "COLBACKFILL_COMMIT_CNT":
   IF (cnvtreal(dudo_desire_opt_in) BETWEEN 1000.0 AND 10000000.0)
    SET dudo_valid_value_ind = 1
    SET dudo_true_opt = "CURSOR_COMMIT_CNT"
   ELSE
    SET dudo_valid_value_ind = 0
   ENDIF
  OF "DMT_FREELIST_GRP":
   IF (cnvtreal(dudo_desire_opt_in) >= 0)
    SET dudo_valid_value_ind = 1
    SET dudo_true_opt = "DMT_FREELIST_GRP"
   ELSE
    SET dudo_valid_value_ind = 0
   ENDIF
  OF "COLBACKFILL_MAXRANGEGROUPS":
   IF (cnvtreal(dudo_desire_opt_in) BETWEEN 1 AND 100)
    SET dudo_valid_value_ind = 1
    SET dudo_true_opt = "CBF_MAXRANGEGROUPS"
   ELSE
    SET dudo_valid_value_ind = 0
   ENDIF
  OF "RESOURCE_BUSY_RETRY_MINUTES":
   IF (cnvtreal(dudo_desire_opt_in) BETWEEN 30 AND 720)
    SET dudo_valid_value_ind = 1
    SET dudo_true_opt = "RESOURCE_BUSY_MAXRETRY"
    SET dudo_desire_opt_in = build((cnvtreal(dudo_desire_opt_in) * 20))
   ELSE
    SET dudo_valid_value_ind = 0
   ENDIF
  OF "DATABASE_STATISTICS_REPORT":
   IF (dudo_desire_opt_in IN ("Y", "N"))
    SET dudo_valid_value_ind = 1
    SET dudo_true_opt = "DBSTATS_CHK_RPT"
   ELSE
    SET dudo_valid_value_ind = 0
   ENDIF
  OF "USE_INITPRM_ASSIGN_DG_IND":
   IF (dudo_desire_opt_in IN ("0", "1"))
    SET dudo_valid_value_ind = 1
    SET dudo_true_opt = "USE_INITPRM_ASSIGN_DG_IND"
   ELSE
    SET dudo_valid_value_ind = 0
   ENDIF
  OF "ASSIGN_DG_OVERRIDE":
   SET dm_err->eproc = "Evaluate diskgroup obtained"
   IF ((dm_err->debug_flag > 2))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    FROM v$asm_diskgroup v
    WHERE v.name=dudo_desire_opt_in
    DETAIL
     dudo_dg_state = cnvtupper(v.state)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   IF (dudo_dg_state IN ("MOUNTED", "CONNECTED"))
    SET dudo_valid_value_ind = 1
    SET dudo_true_opt = "ASSIGN_DG_OVERRIDE"
   ELSE
    SET dudo_valid_value_ind = 0
   ENDIF
  OF "DEGREE_OF_PARALLEL_MAX":
   IF (cnvtint(dudo_desire_opt_in) BETWEEN 0 AND 10)
    SET dudo_valid_value_ind = 1
    SET dudo_true_opt = "DEGREE_OF_PARALLEL_MAX"
   ELSE
    SET dudo_valid_value_ind = 0
   ENDIF
  OF "DEGREE_OF_PARALLEL":
   IF (cnvtint(dudo_desire_opt_in) BETWEEN 0 AND 10)
    SET dudo_valid_value_ind = 1
    SET dudo_true_opt = "DEGREE_OF_PARALLEL"
   ELSE
    SET dudo_valid_value_ind = 0
   ENDIF
 ENDCASE
 IF (dudo_valid_value_ind=0)
  SET dm_err->emsg = concat("An invalid database option value, ",dudo_desire_opt_in,
   " ,was specified in the 2nd parameter. ",
   "Please see the Install Tools overview guide on Cerner.com ",
   "for a description of valid database option values for a particular database option.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSE
  SET dm_err->eproc = concat("Setting option, ",dudo_mod_opt_in," , to ",dudo_val_hold,".")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  SET dudo_retval = dudo_set_option(dudo_true_opt,dudo_desire_opt_in)
  CASE (dudo_retval)
   OF 0:
    SET dm_err->eproc = "Error in setting option value. Option not updated."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   OF 2:
    SET dm_err->eproc = "No rows were updated."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
   OF 1:
    SET dm_err->eproc = "Option updated."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  ENDCASE
 ENDIF
 SUBROUTINE dudo_set_option(dso_opt_in,dso_val_in)
  UPDATE  FROM dm_info
   SET info_char = dso_val_in, updt_task = reqinfo->updt_task, updt_dt_tm = cnvtdatetime(curdate,
     curtime3)
   WHERE info_name=dso_opt_in
    AND info_domain=concat("DM2_",currdb,"_DB_OPTION")
   WITH nocounter
  ;end update
  IF (check_error(dm_err->eproc) > 0)
   ROLLBACK
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Ending DM2_UPDATE_DB_OPTION."
 ENDIF
 CALL final_disp_msg("dm2_update_db_option")
END GO
