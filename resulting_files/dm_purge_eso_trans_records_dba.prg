CREATE PROGRAM dm_purge_eso_trans_records:dba
 CALL echo("<===== dm_purge_eso_trans_records Begin =====>")
 ...
 SET reply->table_name = "ESO_TRANSACTION_RECORD"
 ...
 IF (days_to_keep < 90)
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1",
   "You must keep at least 90 day's worth of data. You entered %1 days or did not enter any value.",
   "i",days_to_keep)
  SET reply->status_data.status = "F"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(etr.eso_transaction_record_id)
   FROM eso_transaction_record etr
   WHERE etr.eso_transaction_record_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(etr.eso_transaction_record_id)
  FROM eso_transaction_record etr
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    etr.rowid
    FROM eso_transaction_record etr
    WHERE etr.eso_transaction_record_id BETWEEN curminid AND curmaxid
     AND parser(sbr_getrowidnotexists("etr.updt_dt_tm < cnvtdatetime((curdate - days_to_keep), 0)",
      "etr"))
    DETAIL
     rows += 1
     IF (mod(rows,500)=1)
     ...
     ENDIF
     reply->rows[rows].row_id = etr.rowid
    WITH nocounter, maxqual(etr,value(rowsleft))
   ;end select
...
#exit_script
 CALL echo("<===== dm_purge_eso_trans_records END =====>")
END GO
