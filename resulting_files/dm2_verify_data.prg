CREATE PROGRAM dm2_verify_data
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_verify_data",".log","dm2_verify_data")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting DM2_VERIFY_DATA"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 DECLARE dvd_new_table_mods_updt_ind = i2 WITH protect, noconstant(0)
 ...
 FOR (dvd_cnt = 1 TO dmvd->tbl_cnt)
   ...
   IF ((dmvd->tbls[dvd_cnt].scui_ind IN (1, 2)))
    ...
    IF ((dm_err->err_ind > 0))
     RETURN(0)
    ENDIF
    SET dvd_new_table_mods_updt_ind = 1
   ENDIF
   ...
   WHILE (dvd_cycle=1)
     ...
     ELSE
      IF ((dmvd->tbls[dvd_cnt].status=dvdr_complete))
       IF (dvd_log_status(dvd_cnt,dvdr_complete)=0)
        GO TO exit_program
       ENDIF
      ELSEIF ((((dmvd->tbls[dvd_cnt].status != dvdr_match)) OR (dvd_new_table_mods_updt_ind=1)) )
     ...
     IF ((dmvd->ranges_per_compare > 0))
      ...
      ENDIF
     ENDIF
     SET dvd_new_table_mods_updt_ind = 0
     IF ((datetimediff(cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),cnvtdatetimeutc(cnvtdatetime(
 ...
 SUBROUTINE dvd_get_numrows(dcs_numrows,dcs_table,dcs_owner)
   ...
   SELECT INTO "nl:"
    ...
    FROM (value(concat("DBA_TABLES@",dmvd->db_link)) d)
 ...
 SUBROUTINE dvd_evaluate_compare(dec_ndx,dec_compare)
   ...
   IF ((dmvd->final_row_ind=0))
    ...
    ENDIF
   ENDIF
   IF ((dmvd->tbls[dec_ndx].rows_to_compare < dmvd->tbls[dec_ndx].num_rows)
    AND (dmvd->tbls[dec_ndx].scui_ind IN (3, 0)))
    SET dm_err->eproc = concat(dmvd->tbls[dec_ndx].owner,".",dmvd->tbls[dec_ndx].table_name,
     " can not be compared. Num Rows:",build(dmvd->tbls[dec_ndx].num_rows))
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg("",dm_err->logfile,0)
    ENDIF
    SET dec_compare = 0
    SET dmvd->tbls[dec_ndx].status = dvdr_nocomp_size
    RETURN(1)
   ENDIF
   IF (dec_compare=0
 ...
 SUBROUTINE dvd_log_status(dls_ndx,dls_mode)
   IF (dls_mode IN (dvdr_match, dvdr_mismatch, dvdr_complete))
    SET dm_err->eproc = "Delete previous MISMATCH data."
    DELETE  FROM dm_vdata_mismatch dv,
      (dummyt d  WITH seq = value(dmvd->tbl_cnt))
     SET dv.seq = 0
     PLAN (d
      WHERE d.seq=dls_ndx)
      JOIN (dv
      WHERE (dv.dm_vdata_master_id=dmvd->tbls[dls_ndx].master_id))
   ...
   CASE (dls_mode)
    OF dvdr_error:
     SET dm_err->eproc = "Update table to ERROR status."
     UPDATE  FROM dm_vdata_master d
    ...
    OF dvdr_comparing:
     ...
     COMMIT
    OF dvdr_mismatch:
     SET dm_err->eproc = "Insert MISMATCH rows."
     INSERT  FROM dm_vdata_mismatch d,
       (dummyt ds  WITH seq = value(dmvd->tbls[dls_ndx].mm_cnt))
      SET d.dm_vdata_mismatch_id = seq(dm_clinical_seq,nextval), d.dm_vdata_master_id = dmvd->tbls[
       dls_ndx].master_id, d.keyid = dmvd->tbls[dls_ndx].mms[ds.seq].keyid
      PLAN (ds)
       JOIN (d
       WHERE (dmvd->tbls[dls_ndx].scui_ind IN (1, 2)))
      WITH nocounter
     ;end insert
     IF (check_error(dm_err->eproc)=1)
      ROLLBACK
     ...
     ENDIF
     COMMIT
     IF ((dmvd->tbls[dls_ndx].mm_beg_keyid=dmvd->tbls[dls_ndx].last_mm_beg_keyid)
      AND (dmvd->tbls[dls_ndx].mm_end_keyid=dmvd->tbls[dls_ndx].last_mm_end_keyid))
      SET dmvd->tbls[dls_ndx].mismatch_event_cnt = (dmvd->tbls[dls_ndx].mismatch_event_cnt+ 1)
     ELSE
      SET dmvd->tbls[dls_ndx].last_mm_cnt_scui = dmvd->tbls[dls_ndx].mm_cnt_scui
     ...
     UPDATE  FROM dm_vdata_master d
      SET d.compare_status = dls_mode, d.range_beg_keyid = dmvd->tbls[dls_ndx].last_range_keyid_min,
       ...
       d.last_match_cnt = dmvd->tbls[dls_ndx].mm_cnt_scui, d.last_compare_dt_tm = cnvtdatetime(dmvd->
        tbls[dls_ndx].beg_dt_tm), d.appl_ident = dmvd->appl_id
      WHERE (d.dm_vdata_master_id=dmvd->tbls[dls_ndx].master_id)
      WITH nocounter
     ;end update
     IF (check_error(dm_err->eproc)=1)
    ...
    OF dvdr_match:
     ...
     SET dmvd->tbls[dls_ndx].mm_cnt_scui = 0
     SET dmvd->tbls[dls_ndx].last_mm_cnt_scui = 0
     ...
     UPDATE  FROM dm_vdata_master d
      SET d.compare_status = evaluate(dmvd->tbls[dls_ndx].status,dvdr_nocomp_size,dmvd->tbls[dls_ndx]
        .status,dls_mode), d.range_beg_keyid = dmvd->tbls[dls_ndx].last_range_keyid_min, d
 ...
 SUBROUTINE dvd_compare_table(dct_ndx)
   ...
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   IF ((dmvd->tbls[dct_ndx].status != dvdr_comparing))
    SET dmvd->tbls[dct_ndx].status = dvdr_comparing
    IF (dvd_log_status(dct_ndx,dvdr_comparing)=0)
     RETURN(0)
    ENDIF
   ENDIF
   ...
   SELECT
    IF ((dmvd->tbls[dct_ndx].scui_ind IN (0, 3)))
     FROM (
      (
      (SELECT
       t.unique_id, t.comp_type
       FROM (value(concat("DM2VDAT",trim(cnvtstring(dmvd->tbls[dct_ndx].src_object_id)),"1")) t)
       WITH sqltype("F8","F8")))
      t1)
    ...
    ELSE
     FROM (
      ...
      (SELECT
       t.unique_id, t.comp_type
       FROM (value(concat("DM2VDAT",trim(cnvtstring(dmvd->tbls[dct_ndx].src_object_id)),"1")) t)
       WHERE t.unique_id BETWEEN dmvd->tbls[dct_ndx].compare_keyid_min AND dmvd->tbls[dct_ndx].
       compare_keyid_max
       WITH sqltype("F8","F8")))
      t1)
     ORDER BY t1.unique_id
    ...
    INTO "nl:"
    HEAD t1.unique_id
     IF ((((dmvd->tbls[dct_ndx].scui_ind IN (3, 0))
      AND t1.unique_id > 0) OR ( NOT ((dmvd->tbls[dct_ndx].scui_ind IN (3, 0))))) )
      dmvd->tbls[dct_ndx].mm_cnt = (dmvd->tbls[dct_ndx].mm_cnt+ 1)
      IF (mod(dmvd->tbls[dct_ndx].mm_cnt,1000)=1)
       stat = alterlist(dmvd->tbls[dct_ndx].mms,(dmvd->tbls[dct_ndx].mm_cnt+ 999))
      ENDIF
      dmvd->tbls[dct_ndx].mms[dmvd->tbls[dct_ndx].mm_cnt].keyid = t1.unique_id
      IF ((((dmvd->tbls[dct_ndx].mms[dmvd->tbls[dct_ndx].mm_cnt].keyid < dmvd->tbls[dct_ndx].
      mm_beg_keyid)) OR ((dmvd->tbls[dct_ndx].mm_beg_keyid=0))) )
       dmvd->tbls[dct_ndx].mm_beg_keyid = dmvd->tbls[dct_ndx].mms[dmvd->tbls[dct_ndx].mm_cnt].keyid
      ENDIF
      IF ((dmvd->tbls[dct_ndx].mms[dmvd->tbls[dct_ndx].mm_cnt].keyid > dmvd->tbls[dct_ndx].
      mm_end_keyid))
       dmvd->tbls[dct_ndx].mm_end_keyid = dmvd->tbls[dct_ndx].mms[dmvd->tbls[dct_ndx].mm_cnt].keyid
      ENDIF
      IF ((dmvd->tbls[dct_ndx].scui_ind IN (3, 0)))
       dmvd->tbls[dct_ndx].mm_cnt_scui = t1.unique_id
      ENDIF
     ENDIF
    DETAIL
     IF ( NOT ((dmvd->tbls[dct_ndx].scui_ind IN (3, 0))))
      dmvd->tbls[dct_ndx].mm_cnt_scui = (dmvd->tbls[dct_ndx].mm_cnt_scui+ 1)
     ENDIF
    FOOT REPORT
     stat = alterlist(dmvd->tbls[dct_ndx].mms,dmvd->tbls[dct_ndx].mm_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ELSE
    ...
    IF (dvd_log_status(dct_ndx,dvdr_match)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
...
#exit_program
 ...
 IF ((dm_err->err_ind > 0)
  AND dvd_cnt > 0
  AND (dmvd->use_verify2_ind=0))
  CALL dvd_log_status(dvd_cnt,dvdr_error)
 ENDIF
 SET dm_err->eproc = "DM2_VERIFY_DATA completed"
 CALL final_disp_msg("DM2_VERIFY_DATA")
END GO
