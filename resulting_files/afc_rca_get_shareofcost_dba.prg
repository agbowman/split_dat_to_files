CREATE PROGRAM afc_rca_get_shareofcost:dba
 ...
 ENDIF
 RECORD encntrplancobs(
   1 encntr[*]
     2 encntr_plan_cob_id = f8
 ) WITH protect
 IF ( NOT (validate(debug)))
  DECLARE debug = i2 WITH protect, noconstant(false)
 ENDIF
 SET reply->status_data.status = "F"
 DECLARE ncdmcnt = i4 WITH noconstant(0)
 DECLARE encntrplancobidcnt = i4 WITH protect, noconstant(0)
 IF (validate(request->encntrid,- (1)) > 0.0)
  SELECT INTO "nl:"
   FROM encntr_plan_cob epc,
    encntr_plan_cob_reltn epcr
   PLAN (epc
    WHERE (epc.encntr_id=request->encntrid)
   ...
   DETAIL
    encntrplancobidcnt = (encntrplancobidcnt+ 1), stat = alterlist(encntrplancobs->encntr,
     encntrplancobidcnt), encntrplancobs->encntr[encntrplancobidcnt].encntr_plan_cob_id = epc
    .encntr_plan_cob_id
   WITH nocounter
  ;end select
  IF (debug=true)
   CALL echorecord(encntrplancobs)
  ENDIF
  IF (size(encntrplancobs->encntr,5) > 0)
   SELECT INTO "nl:"
    FROM encntr_plan_cob epc,
     encntr_plan_cob_reltn epcr,
    ...
    DETAIL
     ncdmcnt = (ncdmcnt+ 1), stat = alterlist(reply->deductamtlist,ncdmcnt), reply->deductamtlist[
     ncdmcnt].deductamount = epr.deduct_amt,
     reply->deductamt = reply->deductamtlist[1].deductamount
    WITH nocounter
   ;end select
  ELSE
   SELECT INTO "nl:"
    ...
    PLAN (epr
     WHERE (epr.encntr_id=request->encntrid)
      AND epr.active_ind=true
      AND epr.priority_seq=1
      AND epr.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime)
      AND epr.end_effective_dt_tm >= cnvtdatetime(curdate,curtime))
    DETAIL
     reply->deductamt = epr.deduct_amt
    WITH nocounter
   ;end select
  ENDIF
  IF (curqual=0)
   CALL exitservicenodata("No Data Available",true)
  ELSE
   SET reply->status_data.status = "S"
  ENDIF
 ELSE
  CALL exitservicefailure("No Value Passed for EncountrID",true)
 ENDIF
#exit_script
