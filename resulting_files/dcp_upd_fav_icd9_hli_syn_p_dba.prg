CREATE PROGRAM dcp_upd_fav_icd9_hli_syn_p:dba
 ...
 IF (sbr_run_parent(mf_max_range_id,mf_min_range_id,ms_range_prefix,ms_info_domain_nm,mn_num_children
  ...
  GO TO exit_program
 ENDIF
 CALL sbr_add_sql_stmt("TABLE","TEMP_FAV_ICD9_HLI_SYN",concat(
   "create global temporary table TEMP_FAV_ICD9_HLI_SYN(nomen_cat_list_id number,",
   "nomenclature_id number) on commit preserve rows"),"Creating temporary table")
 CALL sbr_add_sql_stmt("INDEX","XIETEMPFAVICD9HLISYN",
  "create unique index XIETEMPFAVICD9HLISYN ON TEMP_FAV_ICD9_HLI_SYN(nomen_cat_list_id)",
  "Creating index")
