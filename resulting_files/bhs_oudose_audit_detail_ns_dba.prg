CREATE PROGRAM bhs_oudose_audit_detail_ns:dba
 PROMPT
 ...
 IF ((audit_request->display_ind=2))
  SELECT INTO  $OUT_DEV
   ...
   HEAD PAGE
    ...
    row + 1, sdisplay = concat("Facility: ",trim(uar_get_code_display(audit_request->facility_cd),3)),
    col 0,
    ...
    "Alert", col 15, "Patient",
    col 53, "MedID", col 62,
    "Ordered Med/", col 88, "Ordered Dose/",
    col 114, "User", row + 1,
    col 0, "Date/Time", col 15,
    "name", col 32, "Loc",
    col 40, "FIN", col 53,
    "Method", col 62, "Administered Med",
    col 88, "Administered Dose", col 110,
    "OC", col 114, "name",
   ...
   DETAIL
    ...
    IF (last_row != maa.rowid)
     ...
     ELSE
      soutcome = "ADM"
     ENDIF
     sdisplay = substring(1,14,audit_reply->summary_qual[lidx].date), col 0, sdisplay,
     sdisplay = substring(1,16,audit_reply->summary_qual[lidx].patient), col 15, sdisplay,
     sdisplay = substring(1,16,audit_reply->summary_qual[lidx].location), col 32, sdisplay,
     sdisplay = substring(1,15,audit_reply->summary_qual[lidx].fin), col 40, sdisplay,
     sdisplay = substring(1,6,smed_ident), col 53, sdisplay,
     ...
     ENDWHILE
     lnum = 1, lnum2 = 0
     WHILE (lnum2 < lingr_cnt2)
      lnum2 = (lnum2+ 1),
      IF (lnum <= lingr_cnt)
       WHILE (lnum <= lingr_cnt
        AND (parent_admined->qual[lpos].ingr_qual[lnum].synonym_id < ordered_ingrdnts->qual[lpos2].
       ingr_qual[lnum2].synonym_id))
         sdisplay = substring(1,25,ordered_ingrdnts->qual[lpos2].ingr_qual[lnum2].syn_mne), col 62,
         sdisplay,
         sdisplay = substring(1,20,ordered_ingrdnts->qual[lpos2].ingr_qual[lnum2].ordered_dose), col
         88, sdisplay
         IF (lnum=1
          AND lnum2=1)
          sdisplay = substring(1,3,soutcome), col 110, sdisplay,
          sdisplay = substring(1,16,audit_reply->summary_qual[lidx].user), col 114, sdisplay
         ENDIF
         lnum = (lnum+ 1), row + 1
       ENDWHILE
       IF (lnum <= lingr_cnt)
        IF ((parent_admined->qual[lpos].ingr_qual[lnum].synonym_id=ordered_ingrdnts->qual[lpos2].
        ingr_qual[lnum2].synonym_id))
         IF (textlen(trim(ordered_ingrdnts->qual[lpos2].ingr_qual[lnum2].syn_mne,3)) > 0)
          sdisplay = substring(1,25,ordered_ingrdnts->qual[lpos2].ingr_qual[lnum2].syn_mne)
         ELSEIF (textlen(trim(parent_admined->qual[lpos].ingr_qual[lnum].syn_mne,3)) > 0)
          sdisplay = substring(1,25,parent_admined->qual[lpos].ingr_qual[lnum].syn_mne)
         ELSE
          sdisplay = "Unknown"
         ENDIF
         col 62, sdisplay, sdisplay = substring(1,20,ordered_ingrdnts->qual[lpos2].ingr_qual[lnum2].
          ordered_dose),
         col 88, sdisplay, sdisplay = substring(1,20,parent_admined->qual[lpos].ingr_qual[lnum].
          dose_admin),
         col 88, sdisplay
         IF (lnum=1
          AND lnum2=1)
     ...
     ENDWHILE
     WHILE (lnum <= lingr_cnt)
       sdisplay = substring(1,25,parent_admined->qual[lpos].ingr_qual[lnum].syn_mne), col 62,
       sdisplay,
       sdisplay = substring(1,20,parent_admined->qual[lpos].ingr_qual[lnum].dose_admin), col 88,
       sdisplay
       IF (lnum=1
        AND lnum2=1)
        sdisplay = substring(1,3,soutcome), col 110, sdisplay,
 ...
 ELSEIF ((audit_request->display_ind=0))
  SELECT INTO  $OUT_DEV
   ...
   HEAD PAGE
    ...
    "M = Missing Date, L = Late, E = Early)", row + 1, col 0,
    "User", col 17, "Alert",
    col 32, "Patient", col 82,
    "MedID", col 105, "Ordered",
    col 116, "Dose", row + 1,
    col 0, "name", col 17,
    "Date/Time", col 32, "name",
    col 49, "Loc", col 66,
    "FIN", col 82, "Method",
    col 89, "Med", col 105,
    "Dose", col 116, "ADM",
   ...
   DETAIL
    ...
    ENDIF
    IF (last_row != maa.rowid)
     last_row = maa.rowid, lidx = (lidx+ 1), lidx2 = (idx2+ 1)
     IF (lidx2=10)
      dstat = alterlist(audit_reply->summary_qual,(lidx+ 10)), lidx2 = 0
     ENDIF
     audit_reply->summary_qual[lidx].alert_type = uar_get_code_display(maa.alert_type_cd),
     ...
     ELSE
      soutcome = "ADM"
     ENDIF
     sdisplay = substring(1,16,audit_reply->summary_qual[lidx].user), col 0, sdisplay,
     sdisplay = substring(1,14,audit_reply->summary_qual[lidx].date), col 17, sdisplay,
     sdisplay = substring(1,16,audit_reply->summary_qual[lidx].patient), col 32, sdisplay,
     sdisplay = substring(1,16,audit_reply->summary_qual[lidx].location), col 49, sdisplay,
     sdisplay = substring(1,15,audit_reply->summary_qual[lidx].fin), col 66, sdisplay,
     sdisplay = substring(1,6,smed_ident), col 82, sdisplay,
     lpos = locateval(lnum,1,parent_admined->total_ingr_cnt,mame.med_admin_med_error_id,
      parent_admined->qual[lnum].mame_id), lpos2 = locateval(lnum2,1,parent_order->total_orders_cnt,
      mame.order_id,parent_order->qual[lnum2].order_id), lpos2 = parent_order->qual[lpos2].
     ordered_qual,
     ...
     WHILE (lnum2 < lingr_cnt2)
      ...
      IF (lnum <= lingr_cnt)
       WHILE (lnum <= lingr_cnt
        AND (parent_admined->qual[lpos].ingr_qual[lnum].synonym_id < ordered_ingrdnts->qual[lpos2].
       ingr_qual[lnum2].synonym_id))
         sdisplay = substring(1,15,parent_admined->qual[lpos].ingr_qual[lnum].syn_mne), col 89,
         sdisplay,
         sdisplay = substring(1,10,parent_admined->qual[lpos].ingr_qual[lnum].dose_admin), col 116,
         sdisplay
         IF (lnum=1
          AND lnum2=1)
       ...
       IF (lnum <= lingr_cnt)
        ...
        ingr_qual[lnum2].synonym_id))
         IF (textlen(trim(ordered_ingrdnts->qual[lpos2].ingr_qual[lnum2].syn_mne,3)) > 0)
          sdisplay = substring(1,15,ordered_ingrdnts->qual[lpos2].ingr_qual[lnum2].syn_mne)
         ELSEIF (textlen(trim(parent_admined->qual[lpos].ingr_qual[lnum].syn_mne,3)) > 0)
          sdisplay = substring(1,15,parent_admined->qual[lpos].ingr_qual[lnum].syn_mne)
         ELSE
          sdisplay = "unknown"
         ENDIF
         col 89, sdisplay, sdisplay = substring(1,10,ordered_ingrdnts->qual[lpos2].ingr_qual[lnum2].
          ordered_dose),
         col 105, sdisplay, sdisplay = substring(1,10,parent_admined->qual[lpos].ingr_qual[lnum].
          dose_admin),
         col 116, sdisplay
         IF (lnum=1
          AND lnum2=1)
     ...
     WHILE (lnum2 < lingr_cnt2)
      ...
      IF (lnum <= lingr_cnt)
       WHILE (lnum <= lingr_cnt
        AND (parent_admined->qual[lpos].ingr_qual[lnum].synonym_id < ordered_ingrdnts->qual[lpos2].
       ingr_qual[lnum2].synonym_id))
         sdisplay = substring(1,15,parent_admined->qual[lpos].ingr_qual[lnum].syn_mne), col 72,
         sdisplay,
         sdisplay = substring(1,10,parent_admined->qual[lpos].ingr_qual[lnum].dose_admin), col 99,
         sdisplay
         IF (lnum=1
          AND lnum2=1)
          sdisplay = substring(1,3,soutcome), col 110, sdisplay,
       ...
       IF (lnum <= lingr_cnt)
        ...
        ingr_qual[lnum2].synonym_id))
         ...
         ELSE
          sdisplay = "Unknown"
         ENDIF
         col 72, sdisplay, sdisplay = substring(1,10,ordered_ingrdnts->qual[lpos2].ingr_qual[lnum2].
          ordered_dose),
         col 88, sdisplay, sdisplay = substring(1,10,parent_admined->qual[lpos].ingr_qual[lnum].
          dose_admin),
         col 99, sdisplay
         IF (lnum=1
          AND lnum2=1)
