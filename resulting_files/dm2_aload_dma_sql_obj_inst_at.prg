CREATE PROGRAM dm2_aload_dma_sql_obj_inst_at
 ...
 SUBROUTINE dad_get_admin_conn(i_conn_info)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dadsoia_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dadsoia_file = vc WITH protect, noconstant(trim( $2,3))
 DECLARE dadsoia_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE dadsoia_batch_num = i4 WITH protect, noconstant(10000)
 DECLARE dadsoia_file_path = vc WITH protect, noconstant(" ")
 DECLARE dadsoia_table_name = vc WITH protect, constant("DMA_DSOIA_STAGE")
 DECLARE dadsoia_admin_conn = vc WITH protect, noconstant(" ")
 IF (check_logfile("dm2_aload_dsoia",".log","DM2_ALOAD_DSOIA LogFile...") != 1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_dma_sql_obj_inst_at"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dadsoia_expected_rows > 0)
  IF (dac_prelim(dadsoia_pkg,dadsoia_file_path,dadsoia_batch_num)=0)
   GO TO exit_program
  ENDIF
  IF (dad_get_admin_conn(dadsoia_admin_conn) != 1)
   GO TO exit_program
  ENDIF
  EXECUTE dm2_exe_sqlldr dadsoia_file, dadsoia_file_path, dadsoia_table_name,
  dadsoia_admin_conn
  IF ((dm_err->err_ind=1))
   IF (findstring("Column mismatch found between csv and CCLDEF",dm_err->emsg,0) > 0)
    SET dm_err->err_ind = 0
   ENDIF
   GO TO exit_program
  ENDIF
  EXECUTE dm2_load_dma_sql_obj_inst
  IF ((dm_err->err_ind=1))
   GO TO exit_program
  ENDIF
 ENDIF
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_dma_sql_obj_inst_at completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_dma_sql_obj_inst_at")
END GO
