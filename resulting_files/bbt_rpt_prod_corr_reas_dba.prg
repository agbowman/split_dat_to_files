CREATE PROGRAM bbt_rpt_prod_corr_reas:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 SET captions->product_correction = uar_i18ngetmessage(i18nhandle,"product_correction",
  "PRODUCT CORRECTION REASONS")
 ...
 SET select_ok_ind = 0
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_prd_cor_rsn", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  cv.display, cv.description, cv.active_ind
  FROM code_value cv
  WHERE cv.code_set=14114
  HEAD REPORT
   select_ok_ind = 0
  HEAD PAGE
   ...
   col 48, captions->product_correction, row + 1,
