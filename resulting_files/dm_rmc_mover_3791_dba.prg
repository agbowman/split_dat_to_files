CREATE PROGRAM dm_rmc_mover_3791:dba
 ...
 SUBROUTINE create_r_only(cro_tab_name,cro_r_tab_name,cro_trig_ind,cro_remove_lock_ind)
   ...
   RETURN(cro_return)
 END ;Subroutine
 FREE RECORD cust_tag_names
 RECORD cust_tag_names(
   1 tag_cnt = i4
   1 tag_qual[*]
     2 tag_name = vc
     2 parent_tab_name = vc
     2 parent_tab_col = vc
 )
 FREE RECORD cust_tag_xlats
 RECORD cust_tag_xlats(
   1 xlat_cnt = i4
   1 xlat_qual[*]
     2 xlat_id_value = f8
     2 xlat_parent_tab_name = vc
     2 xlat_parent_tab_col = vc
     2 xlat_text_to_replace = vc
 )
 DECLARE cust_tab_name = vc
 DECLARE cust_pe_id = f8
 DECLARE cust_pe_name = vc
 DECLARE cust_long_text = vc
 DECLARE cust_cnt = i4
 DECLARE cust_tag_flag = i2
 DECLARE cust_tag_beg = i4
 DECLARE cust_tag_end = i4
 DECLARE cust_tag_length = i4
 DECLARE cust_text_to_xlat = vc
 DECLARE cust_idcd_check = i4
 DECLARE cust_long_text_length = i4
 DECLARE cust_beg_string = vc
 DECLARE cust_end_string = vc
 DECLARE cust_trans_value = vc
 DECLARE cust_single_pass_ind = i2
 DECLARE cust_wildcard_tag_flag = i2
 DECLARE cust_wildcard_tag = vc
 DECLARE cust_chk_type_cnt = i4
 DECLARE cust_tag_name = vc
 DECLARE cust_buffer = i4
 DECLARE cust_continue = i2
 DECLARE cust_translated_text = vc
 DECLARE cust_translated_text_length = i4
 DECLARE cust_end_tag_name = vc
 DECLARE cust_text_to_replace = vc
 DECLARE cust_beg_half = vc
 DECLARE cust_end_half = vc
 ...
 SET cust_pe_name = get_value(cust_tab_name,"PARENT_ENTITY_NAME","FROM")
 SET cust_long_text = get_value(cust_tab_name,"LONG_TEXT","FROM")
 SET cust_cnt = 0
 SET cust_tag_flag = 0
 SET cust_tag_length = 0
 SET cust_tag_names->tag_cnt = 0
 SET cust_tag_xlats->xlat_cnt = 0
 SET cust_long_text_length = textlen(cust_long_text)
 SET cust_single_pass_ind = 0
 SET cust_wildcard_tag_flag = 0
 SET cust_chk_type_cnt = 0
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_3791
 ENDIF
 IF (cust_pe_name IN ("PM_PREF", "PM_PUBLISH"))
  SET dm_err->eproc = "Selecting for RDDS PM PREF XMFL TAG's."
  CALL disp_msg(" ",dm_err->logfile,dm_err->err_ind)
  SELECT INTO "NL:"
   di.info_domain
   FROM dm_info di
   WHERE di.info_domain="RDDS PM PREF XML TAG:*"
   DETAIL
    cust_tag_names->tag_cnt = (cust_tag_names->tag_cnt+ 1), stat = alterlist(cust_tag_names->tag_qual,
     cust_tag_names->tag_cnt), cust_tag_length = textlen(di.info_domain),
    cust_tag_names->tag_qual[cust_tag_names->tag_cnt].tag_name = substring(22,(cust_tag_length - 21),
     di.info_domain), cust_tag_names->tag_qual[cust_tag_names->tag_cnt].parent_tab_name = di
    .info_name, cust_tag_names->tag_qual[cust_tag_names->tag_cnt].parent_tab_col = di.info_char
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->eproc = "Error: While selecting for RDDS PM PREF XMFL TAG's"
   GO TO exit_3791
  ENDIF
  FOR (cust_cnt = 1 TO cust_tag_names->tag_cnt)
    SET cust_tag_flag = 0
    SET cust_tag_beg = 1
    SET cust_tag_length = textlen(cust_tag_names->tag_qual[cust_cnt].tag_name)
    IF (findstring("#",cust_tag_names->tag_qual[cust_cnt].tag_name,1,0))
     SET cust_wildcard_tag_flag = 1
     SET cust_tag_length = (cust_tag_length - 1)
     SET cust_tag_name = substring(1,cust_tag_length,cust_tag_names->tag_qual[cust_cnt].tag_name)
    ELSE
     SET cust_wildcard_tag_flag = 0
     SET cust_tag_name = cust_tag_names->tag_qual[cust_cnt].tag_name
    ENDIF
    WHILE (cust_tag_flag=0)
     SET cust_tag_beg = findstring(cust_tag_name,cust_long_text,cust_tag_beg,0)
     IF (cust_tag_beg > 0)
      IF (findstring(concat("<",cust_tag_name),cust_long_text,(cust_tag_beg - 1),0) > 0)
       SET cust_tag_beg = (cust_tag_beg - 1)
       IF (cust_wildcard_tag_flag=1)
        SET cust_tag_end = findstring(">",cust_long_text,cust_tag_beg,0)
        SET cust_wildcard_tag = substring(cust_tag_beg,((cust_tag_end - cust_tag_beg)+ 1),
         cust_long_text)
        IF (cust_tag_name=cnvtalphanum(cust_wildcard_tag,2))
         IF (((findstring(concat("<",cust_tag_name),cust_long_text,(cust_tag_end+ 1),0) -
         cust_tag_end)=1))
          SET cust_continue = 0
         ELSE
          SET cust_continue = 1
          SET cust_buffer = (textlen(cust_wildcard_tag) - cust_tag_length)
          SET cust_end_tag_name = replace(cust_wildcard_tag,"<","</",1)
         ENDIF
        ELSE
         SET cust_num_len = 1
         WHILE (isnumeric(substring(((textlen(cust_wildcard_tag) - 1) - cust_num_len),cust_num_len,
           cust_wildcard_tag)))
           SET cust_num_len = (cust_num_len+ 1)
         ENDWHILE
         IF (cust_num_len=1)
          SET cust_continue = 0
         ELSE
       ...
       ELSE
        SET cust_continue = 1
        SET cust_buffer = 2
        SET cust_end_tag_name = concat("</",cust_tag_name,">")
       ENDIF
       IF (cust_continue=1)
        SET cust_tag_end = findstring(cust_end_tag_name,cust_long_text,cust_tag_beg,0)
        IF (cust_tag_end > 0)
         SET cust_text_to_xlat = substring(((cust_tag_beg+ cust_tag_length)+ cust_buffer),(
          cust_tag_end - ((cust_tag_beg+ cust_tag_length)+ cust_buffer)),cust_long_text)
         SET cust_trans_value = select_merge_translate(cust_text_to_xlat,cust_tag_names->tag_qual[
          cust_cnt].parent_tab_name)
         IF (isnumeric(cust_trans_value))
          SET cust_text_to_replace = substring(cust_tag_beg,((((cust_tag_end+ cust_tag_length)+
           cust_buffer)+ 1) - cust_tag_beg),cust_long_text)
          SET cust_translated_text = cust_text_to_replace
          SET cust_translated_text = replace(cust_translated_text,cust_text_to_xlat,cust_trans_value,
           1)
          SET cust_translated_text_length = textlen(cust_translated_text)
          SET cust_beg_half = substring(1,((cust_long_text_length - (cust_long_text_length -
           cust_tag_beg)) - 1),cust_long_text)
          SET cust_end_half = substring(cust_tag_beg,((cust_long_text_length - cust_tag_beg)+ 1),
           cust_long_text)
          SET cust_end_half = replace(cust_end_half,cust_text_to_replace,cust_translated_text,1)
          SET cust_long_text = concat(cust_beg_half,cust_end_half)
          SET cust_tag_beg = (cust_tag_beg+ cust_translated_text_length)
          SET cust_long_text_length = textlen(cust_long_text)
         ELSE
          SET cust_rpt_missing = report_missing(cust_tag_names->tag_qual[cust_cnt].parent_tab_name,
           cust_tag_names->tag_qual[cust_cnt].parent_tab_col,cnvtreal(cust_text_to_xlat))
          CALL evaluate_rpt_missing(cust_rpt_missing)
          SET cust_single_pass_ind = 1
          IF (cust_wildcard_tag_flag=1)
           SET cust_tag_beg = (cust_tag_beg+ textlen(cust_wildcard_tag))
          ELSE
           SET cust_tag_beg = ((cust_tag_beg+ cust_tag_length)+ cust_buffer)
          ENDIF
         ENDIF
        ELSE
         SET cust_tag_flag = 1
       ...
       ELSE
        ...
        ENDIF
       ENDIF
      ELSEIF (findstring(concat("&lt;",cust_tag_name),cust_long_text,(cust_tag_beg - 4),0) > 0)
       SET cust_tag_beg = (cust_tag_beg - 4)
       IF (cust_wildcard_tag_flag=1)
        SET cust_tag_end = (findstring("&gt;",cust_long_text,cust_tag_beg,0)+ 3)
        SET cust_wildcard_tag = substring(cust_tag_beg,((cust_tag_end - cust_tag_beg)+ 1),
         cust_long_text)
        IF (cust_tag_name=cnvtalphanum(substring((cust_tag_beg+ 4),(((cust_tag_end - 4) - (
          cust_tag_beg+ 4))+ 1),cust_long_text),2))
         IF (((findstring(concat("<",cust_tag_name),cust_long_text,(cust_tag_end+ 1),0) -
         cust_tag_end)=1))
          SET cust_continue = 0
         ELSE
        ...
        ELSE
         SET cust_num_len = 1
         WHILE (isnumeric(substring(((textlen(cust_wildcard_tag) - 4) - cust_num_len),cust_num_len,
           cust_wildcard_tag)))
           SET cust_num_len = (cust_num_len+ 1)
         ENDWHILE
         IF (cust_num_len=1)
       ...
       ENDIF
      ELSE
       SET cust_tag_beg = (cust_tag_beg+ cust_tag_length)
      ENDIF
     ELSE
      SET cust_tag_flag = 1
     ENDIF
  ...
  ENDFOR
 ENDIF
 IF (cust_single_pass_ind=1)
  SET cust_err_val = get_err_val(null)
  IF (cust_err_val=0)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg =
   "This row wasn't translated because of the LONG_TEXT_ID column"
  ENDIF
  GO TO exit_3791
 ELSE
  CALL put_value(cust_tab_name,"LONG_TEXT",cust_long_text)
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"LONG_TEXT_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"LONG_TEXT_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"LONG_TEXT_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  LONG_TEXT_ID")
     ELSE
      GO TO exit_3791
     ENDIF
    ELSEIF ((cust_val=- (4)))
   ...
   SET cust_from_cd = get_value(cust_tab_name,"LONG_TEXT_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_3791
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
   ENDIF
   SET cust_new_cd = get_value(cust_tab_name,"LONG_TEXT_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_3791
   ELSE
