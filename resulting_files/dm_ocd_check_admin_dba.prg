CREATE PROGRAM dm_ocd_check_admin:dba
 SET param1 =  $1
 SET param2 =  $2
 ...
 ;end select
 IF (dm_sys="AXP")
  SET fname = "dm_ocd_check_admin.com"
 ELSEIF (dm_sys="AIX")
  SET fname = "dm_ocd_check_admin.ksh"
 ENDIF
 SET dm_env_id = 0.0
 SELECT INTO "nl:"
  i.info_number
 ...
 IF (dm_env_id=0.0)
  SET reply->status_data.status = "F"
  SET reply->output_dist = "INVALID OR NONEXISTENT ENVIRONMENT_ID IN DM_INFO"
  GO TO end_program
 ENDIF
 IF (dm_sys="AIX")
  SELECT INTO value(fname)
   ...
   DETAIL
    ...
    row + 1, col 0, "ccl <<!",
    row + 1, col 0, "execute dm_ocd_check_admin_int '",
    param1, "','", param2,
    "','", param3, "' go"
 ...
 ELSE
  SELECT INTO value(fname)
   ...
   DETAIL
    ...
    col 0, "$CCL", row + 1,
    col 0, "execute dm_ocd_check_admin_int '", param1,
    "','", param2, "','",
    param3, "' go"
 ...
 IF (dm_sys="AIX")
  ...
  ENDIF
  SET dclcom = "chmod 755 $CCLUSERDIR/dm_ocd_check_admin.ksh"
  SET len = size(trim(dclcom))
  SET status = 0
  CALL dcl(dclcom,len,status)
  IF (status=0)
   SET reply->status_data.status = "F"
   SET reply->ops_event = "Couldn't chmod dm_ocd_check_admin.ksh"
  ENDIF
  SET dclcom = "./dm_ocd_check_admin.ksh"
  SET len = size(trim(dclcom))
  SET status = 0
  CALL dcl(dclcom,len,status)
 ELSE
  ...
  IF (status=0)
   ...
   SET reply->ops_event = "Couldn't remove old version of dm_ocd_chk_adm.dat"
  ENDIF
  SET dclcom = "@dm_ocd_check_admin.com"
  SET len = size(trim(dclcom))
  SET status = 0
  CALL dcl(dclcom,len,status)
  IF (status=0)
   SET reply->status_data.status = "F"
   SET reply->ops_event = "Couldn't execute dm_ocd_check_admin.com"
  ENDIF
 ENDIF
 FREE DEFINE rtl
