CREATE PROGRAM dm_cmb_insert_charge:dba
 ...
 SUBROUTINE (error_logger(sbr_qualify=i4,sbr_err_msg=vc,sbr_err_type=i4) =null)
   ...
   ENDIF
 END ;Subroutine
 FREE RECORD dcccm_request
 RECORD dcccm_request(
   1 sbr_ccm_ciicc = f8
   1 sbr_ccm_innbr = f8
   1 sbr_ccm_process_type = c2
 )
 SET new_nbri = 0.0
 SELECT INTO "nl:"
  y = seq(charge_event_seq,nextval)
 ...
 FOR (x = 1 TO col_count)
  ...
  CASE (trim(rcolumns->col[x].col_name))
   ...
   OF "ENCNTR_ID":
    IF ((rreclist->from_rec[pcc2_request->sbr_loopcount].to_encntr_id=0))
     CALL parser(build("rRecList->from_rec[pcc2_request->sbr_loopcount]->encntr_id",v_comma_str))
    ELSE
     CALL parser(build("rRecList->from_rec[pcc2_request->sbr_loopcount]->to_encntr_id",v_comma_str))
    ENDIF
   OF "PERSON_ID":
    CALL parser(build("rRecList->from_rec[pcc2_request->sbr_loopcount]->to_person_id",v_comma_str))
   OF "COMBINE_IND":
    CALL parser(build("0",v_comma_str))
   OF "PAYOR_ID":
    CALL parser(build("rRecList->from_rec[pcc2_request->sbr_loopcount]->payor_id",v_comma_str))
   OF "PROCESS_FLG":
    CALL parser(build("pcc2_request->sbr_cc_type",v_comma_str))
   OF "POSTED_DT_TM":
    CALL parser(build("cnvtdatetime(rRecList->from_rec[pcc2_request->sbr_loopcount]->posted_dt_tm)",
      v_comma_str))
   OF "CREDITED_DT_TM":
    CALL parser(build("cnvtdatetime(rRecList->from_rec[pcc2_request->sbr_loopcount]->credited_dt_tm)",
      v_comma_str))
   OF "ACTIVE_IND":
    CALL parser(build("1",v_comma_str))
   OF "ACTIVE_STATUS_CD":
   ...
   OF "UPDT_TASK":
    CALL parser(build("ReqInfo->updt_task",v_comma_str))
   OF "PARENT_CHARGE_ITEM_ID":
    CALL parser(build("rRecList->from_rec[pcc2_request->sbr_loopcount]->from_id",v_comma_str))
   OF "OFFSET_CHARGE_ITEM_ID":
    IF (charge_originally_offset=1)
     CALL parser(build("FRM.",trim(rcolumns->col[x].col_name),v_comma_str))
    ELSE
 ...
 ENDFOR
 CALL parser("from charge FRM")
 CALL parser(build("where FRM.charge_item_id = ",rreclist->from_rec[pcc2_request->sbr_loopcount].
   from_id,")"))
 CALL parser("go")
 CALL error_logger(curqual,"Couldn't insert charge on the to person",insert_error)
 SET icombinedet += 1
 SET stat = alterlist(request->xxx_combine_det,icombinedet)
 SET request->xxx_combine_det[icombinedet].combine_action_cd = add
 SET request->xxx_combine_det[icombinedet].combine_desc_cd = debit_desc_cd
 SET request->xxx_combine_det[icombinedet].entity_id = new_nbri
 SET request->xxx_combine_det[icombinedet].entity_name = "CHARGE"
 SET request->xxx_combine_det[icombinedet].attribute_name = "PERSON_ID"
 SET dcccm_request->sbr_ccm_ciicc = rreclist->from_rec[pcc2_request->sbr_loopcount].from_id
 SET dcccm_request->sbr_ccm_innbr = new_nbri
 SET dcccm_request->sbr_ccm_process_type = "PC"
 EXECUTE dm_cmb_copy_charge_mod
 IF ((pcc2_request->sbr_cc_type=1))
 ...
 SUBROUTINE insert_charge_mod(ciiic)
   ...
   INSERT  FROM charge_mod c
    SET c.charge_mod_id = new_nbric, c.charge_item_id = ciiic, c.charge_mod_type_cd = suspense_cd,
     c.field6 = trim(ssuspensedesc), c.field1_id = suspense_reason_cd, c.active_ind = true,
    ...
    WITH nocounter
   ;end insert
   CALL error_logger(curqual,"Couldn't insert charge_mod record-insert_charge_mod",insert_error)
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
   SET request->xxx_combine_det[icombinedet].combine_action_cd = add
   SET request->xxx_combine_det[icombinedet].entity_id = new_nbric
   SET request->xxx_combine_det[icombinedet].entity_name = "CHARGE_MOD"
   SET request->xxx_combine_det[icombinedet].attribute_name = "PERSON_ID"
 END ;Subroutine
#end_of_program
