CREATE PROGRAM dm_pft_claim_status_hist_rows:dba
 ...
 SET reply->table_name = "PFT_CLAIM_STATUS_HIST"
 ...
 DECLARE code_set = i4 WITH protect, noconstant(18935)
 DECLARE cdf_meaning = vc WITH protect, noconstant("CMGMTONLY")
 DECLARE claim_status_cd = f8 WITH protect, noconstant(0.0)
 FOR (v_token_idx = 1 TO size(request->tokens,5))
   ...
   ELSE
    SET v_days_to_keep = (curdate - cnvtdate(cnvtlookbehind("7,Y")))
   ENDIF
 ENDFOR
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(csh.pft_claim_status_hist_id)
   FROM pft_claim_status_hist csh
   WHERE csh.pft_claim_status_hist_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(csh.pft_claim_status_hist_id)
  FROM pft_claim_status_hist csh
  DETAIL
 ...
 SET claim_status_cd = getcodevaluebymeaning(code_set,cdf_meaning)
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    FROM pft_claim_status_hist csh
    WHERE csh.activity_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
     AND csh.claim_status_cd=claim_status_cd
     AND csh.pft_claim_status_hist_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists("csh.pft_claim_status_hist_id != 0","csh"))
     AND (csh.activity_dt_tm !=
    (SELECT
     max(csh2.activity_dt_tm)
     FROM pft_claim_status_hist csh2
     WHERE csh2.corsp_activity_id=csh.corsp_activity_id))
    HEAD REPORT
     v_rows = 0
    DETAIL
     v_rows = (v_rows+ 1)
     ...
     ENDIF
     reply->rows[v_rows].row_id = csh.rowid
    WITH nocounter, maxrec(csh,value(v_rows_left))
   ;end select
