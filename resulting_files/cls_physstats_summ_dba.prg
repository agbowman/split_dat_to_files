CREATE PROGRAM cls_physstats_summ:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   row + 0, cpoe_1_cnt = 0, proto_2_cnt = 0,
   phone_3_cnt = 0, down_4_cnt = 0, writ_5_cnt = 0,
   other_6_cnt = 0, bad_cnt = 0, prcount = 0
  ...
  DETAIL
   ...
   ENDIF
  FOOT  p.person_id
   CALL echo(build2("cpoe:  ",cpoe_1_cnt)),
   CALL echo(build2("proto: ",proto_2_cnt)),
   CALL echo(build2("phone: ",phone_3_cnt)),
   CALL echo(build2("down:  ",down_4_cnt)),
   CALL echo(build2("writ:  ",writ_5_cnt)),
   CALL echo(build2("other: ",other_6_cnt)),
   CALL echo(build2("bad:   ",bad_cnt)), totcount = ((((mdcount+ prcount)+ pvcount)+ sidcount)+
   wrcount)
   IF (totcount > 0)
    cnt = (cnt+ 1)
    IF (mod(cnt,100)=1)
 ...
 SELECT INTO value(output_dest)
  ...
  DETAIL
   ...
   ELSE
    ...
    ELSEIF (loc_facility_display IN ("BWH"))
