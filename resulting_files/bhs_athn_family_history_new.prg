CREATE PROGRAM bhs_athn_family_history_new
 FREE RECORD shx_fhx
 RECORD shx_fhx(
   1 patientid = f8
   1 mrn = vc
 ...
 SELECT DISTINCT INTO "NL:"
  ...
  DETAIL
   ...
   shx_fhx->relation[cnt].relative_gender = uar_get_code_display(p.sex_cd)
   IF (p_relative_name=",")
    shx_fhx->relation[cnt].relative_name = ""
   ELSE
    shx_fhx->relation[cnt].relative_name = p_relative_name
   ENDIF
 ...
 SELECT INTO "NL:"
  ...
  HEAD n.source_string
   ...
   IF (f.fhx_value_flag=0)
    shx_fhx->relation[d1.seq].condition[cnt].pos_neg = "Negative"
   ELSEIF (f.fhx_value_flag=1)
    shx_fhx->relation[d1.seq].condition[cnt].pos_neg = "Positive"
   ELSEIF (f.fhx_value_flag=2)
    shx_fhx->relation[d1.seq].condition[cnt].pos_neg = "Unknown"
   ELSEIF (f.fhx_value_flag=3)
    shx_fhx->relation[d1.seq].condition[cnt].pos_neg = "Unable to Obtain"
   ELSEIF (f.fhx_value_flag=4)
    shx_fhx->relation[d1.seq].condition[cnt].pos_neg = "Patient Adopted"
   ENDIF
   scnt = 0
 ...
 SELECT INTO "nl:"
  ...
  PLAN (f
   WHERE f.person_id=mpersonid
    ...
    AND f.active_ind=1)
  DETAIL
   fcnt = (fcnt+ 1), stat = alterlist(shx_fhx->fm_info,fcnt)
   IF (f.fhx_value_flag=0)
    shx_fhx->fm_info[fcnt].description = "Negative"
   ELSEIF (f.fhx_value_flag=1)
    shx_fhx->fm_info[fcnt].description = "Positive"
   ELSEIF (f.fhx_value_flag=2)
    shx_fhx->fm_info[fcnt].description = "Unknown"
   ELSEIF (f.fhx_value_flag=3)
    shx_fhx->fm_info[fcnt].description = "Unable to Obtain"
   ELSEIF (f.fhx_value_flag=4)
    shx_fhx->fm_info[fcnt].description = "Patient Adopted"
   ENDIF
  WITH nocounter, separator = " ", format,
   time = 30
 ;end select
 CALL echojson(shx_fhx, $1)
 FREE RECORD shx_fhx
END GO
