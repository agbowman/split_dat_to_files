CREATE PROGRAM dm2_arc_schema_change:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF ((validate(dav_request->all_tab_ind,- (1))=- (1)))
  FREE RECORD dav_request
  RECORD dav_request(
    1 all_tab_ind = i2
    1 tabs[*]
      2 table_name = vc
 ...
 ENDIF
 IF (validate(dav_reply->status_data.status,"X")="X")
  FREE RECORD dav_reply
  RECORD dav_reply(
    1 status_data
      2 status = c1
      2 subeventstatus[1]
 ...
 ENDIF
 IF (check_logfile("dm2_arc_schchg",".log","DM2_ARC_SCHEMA_CHANGE LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 DECLARE asc_count = i4 WITH noconstant(0)
 DECLARE domain_len = i4 WITH noconstant(0)
 DECLARE asc_req_size = i4 WITH noconstant(0)
 DECLARE exe_new_cons(null) = i2
 SET dm_err->eproc = "starting DM2_ARC_SCHEMA_CHANGE"
 CALL disp_msg(" ",dm_err->logfile,0)
 FREE RECORD asc_entity
 RECORD asc_entity(
   1 asc_cnt = i4
   1 list[*]
     2 arc_entity_name = vc
     2 cons_prefix = vc
     2 info_domain = vc
     2 exe_flag = i2
 )
 IF (exe_new_cons(null)=1)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM dm_arc_info d
  WHERE d.info_name="ACTIVE ARCHIVE"
   AND d.info_domain="ARCHIVE-*"
   AND sysdate BETWEEN d.beg_effective_dt_tm AND d.end_effective_dt_tm
  WITH nocounter
 ;end select
 IF (curqual > 0)
  SET asc_count = 0
  SELECT INTO "nl:"
   FROM dm_arc_constraints dac,
    (dummyt dt  WITH seq = value(size(request->tabs,5)))
   PLAN (dt)
    JOIN (dac
    WHERE (dac.child_table=request->tabs[dt.seq].table_name)
     AND dac.exclude_ind=0)
   DETAIL
    asc_count = (asc_count+ 1)
    IF (mod(asc_count,10)=1)
     stat = alterlist(dav_request->tabs,(asc_count+ 9))
    ENDIF
    dav_request->tabs[asc_count].table_name = request->tabs[dt.seq].table_name
   FOOT REPORT
    stat = alterlist(dav_request->tabs,asc_count)
   WITH nocounter
  ;end select
  IF (curqual=0)
   IF (check_error("check table against dm_arc_constraints") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ELSE
    SET dm_err->eproc = "Tables in request doesn't exist in dm_arc_constraints"
    CALL disp_msg(" ",dm_err->logfile,0)
    GO TO exit_program
   ENDIF
  ENDIF
  SET asc_req_size = size(dav_request->tabs,5)
  SELECT INTO "nl:"
   FROM user_tab_columns utc,
    (dummyt dt  WITH seq = asc_req_size)
   PLAN (dt)
    JOIN (utc
    WHERE (utc.table_name=dav_request->tabs[dt.seq].table_name)
     AND utc.data_type IN ("LONG", "LONG RAW", "RAW"))
   DETAIL
    dav_request->tabs[dt.seq].long_col_ind = 1
   WITH nocounter
  ;end select
  UPDATE  FROM dm_arc_constraints d,
    (dummyt dt  WITH seq = asc_req_size)
   SET d.seq = 1, d.long_col_ind = dav_request->tabs[dt.seq].long_col_ind
   PLAN (dt)
    JOIN (d
    WHERE (d.child_table=dav_request->tabs[dt.seq].table_name))
   WITH nocounter
  ;end update
  IF (check_error("Update long_col_ind on table dm_arc_constraints") != 0)
   ROLLBACK
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  COMMIT
 ENDIF
 SUBROUTINE exe_new_cons(null)
   SELECT INTO "nl:"
    d.info_name
    FROM dm_info d
    WHERE d.info_domain="ARCHIVE-DOMAIN"
    DETAIL
     asc_count = (asc_count+ 1), stat = alterlist(asc_entity->list,asc_count), asc_entity->list[
     asc_count].arc_entity_name = trim(d.info_name),
     asc_entity->list[asc_count].info_domain = build("ARCHIVE-",asc_entity->list[asc_count].
      arc_entity_name)
    WITH nocounter
   ;end select
   IF (curqual=0)
    IF (check_error("Get all archive_entity_names") != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(1)
    ELSE
     SET dm_err->eproc = "There are no ARCHIVE rows in DM_INFO"
     CALL disp_msg(" ",dm_err->logfile,0)
     RETURN(0)
    ENDIF
   ENDIF
   SELECT INTO "nl:"
    d.info_char
    FROM dm_info d,
     (dummyt dt  WITH seq = asc_count)
    PLAN (dt)
     JOIN (d
     WHERE (d.info_domain=asc_entity->list[dt.seq].info_domain)
      AND d.info_name="CONSTRAINT PREFIX")
    DETAIL
     asc_entity->list[dt.seq].cons_prefix = trim(d.info_char), asc_entity->list[dt.seq].exe_flag = 1
    WITH nocounter
   ;end select
   IF (curqual=0)
    IF (check_error("Get the prefix for those archive_entity_names") != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(1)
    ELSE
     SET dm_err->eproc = "There are no CONSTRAINT PREFIX rows in DM_INFO"
     CALL disp_msg(" ",dm_err->logfile,0)
     RETURN(0)
    ENDIF
   ENDIF
   FOR (asc_lp = 1 TO asc_count)
     IF ((asc_entity->list[asc_lp].exe_flag=1))
      SET danc_request->batch_selection = asc_entity->list[asc_lp].arc_entity_name
      SET danc_request->cons_prefix = asc_entity->list[asc_lp].cons_prefix
      SET dm_err->eproc = "Executing dm2_arc_new_constraints"
      CALL disp_msg(" ",dm_err->logfile,0)
      EXECUTE dm2_arc_new_constraints  WITH replace("REQUEST","DANC_REQUEST"), replace("REPLY",
       "DANC_REPLY")
      IF ((danc_reply->status_data.status="F"))
       SET dm_err->err_ind = 1
       SET dm_err->emsg = danc_reply->status_data.subeventstatus.targetobjectvalue
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(1)
      ENDIF
     ENDIF
...
#exit_program
 ...
 FREE RECORD danc_reply
 FREE RECORD asc_entity
 SET dm_err->eproc = "...Ending dm2_arc_schema_changes"
 CALL final_disp_msg("dm2_arc_schchg")
END GO
