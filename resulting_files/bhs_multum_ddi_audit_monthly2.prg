CREATE PROGRAM bhs_multum_ddi_audit_monthly2
 ...
 SET days = (datetimecmp(end_date_qual,beg_date_qual)+ 1)
 SET s_date = beg_date_qual
 FOR (dt = 1 TO days BY 7)
   SET start_date = format(s_date,"mm/dd/yy hh:mm;;d")
   CALL echo(build(" s_date: ",start_date))
   CALL echo(build("# of days: ",days))
   SET cnt = 0
   FREE RECORD eks
   RECORD eks(
     1 list[*]
   ...
   ENDIF
   SET end_date = format(e_date,"mm/dd/yy hh:mm;;d")
   CALL echo(build("E_DATE: ",end_date))
   SELECT INTO "nl:"
    FROM eks_dlg_event ede,
     ...
     encntr_alias ea
    PLAN (ede
     WHERE ede.updt_dt_tm BETWEEN cnvtdatetime(s_date) AND cnvtdatetime(e_date)
    ...
    HEAD ede.dlg_event_id
     cnt = (cnt+ 1)
     IF (mod(cnt,1000)=1)
      stat = alterlist(eks->list,(cnt+ 1000))
     ENDIF
     ...
     eks->list[cnt].override_rsn_lt_id = ede.long_text_id, eks->list[cnt].prsnl_name = trim(pr
      .name_full_formatted), eks->list[cnt].prsnl_position_cd = pr.position_cd,
     eks->list[cnt].fmrn = trim(cnvtalias(ea.alias,ea.alias_pool_cd)), eks->list[cnt].patient_name =
     trim(p.name_full_formatted), eks->list[cnt].facility_cd = e.loc_facility_cd,
   ...
   IF (size(eks->list,5) > 0)
    ...
    SELECT INTO "nl:"
     ...
     DETAIL
      eks->list[d.seq].trigger.order_dt_tm = o.orig_order_dt_tm, eks->list[d.seq].trigger.
      physician_id = pr.person_id, eks->list[d.seq].trigger.physician_name = trim(pr
       .name_full_formatted),
    ...
    SELECT INTO "nl:"
     ...
     DETAIL
      eks->list[d.seq].interact.order_dt_tm = o.orig_order_dt_tm, eks->list[d.seq].interact.
      physician_id = pr.person_id, eks->list[d.seq].interact.physician_name = pr.name_full_formatted,
    ...
    SELECT INTO "nl:"
     ...
     trigger_order_id = eks->list[d.seq].trigger.order_id, interact_order_id = eks->list[d.seq].
     interact.order_id, dlg_event_id = eks->list[d.seq].dlg_event_id,
    ...
    ;end select
    SET xx = eks2_size
    SET stat = alterlist(eks2->list,(xx+ 100))
    FOR (x = 1 TO size(eks->list,5))
      IF ((eks->list[x].interact.orig_ord_as_flag IN (1, 0))
       AND (eks->list[x].severity >= 3)
       AND (eks->list[x].active_ind=1)
       AND (eks->list[x].trigger.catalog_cd > 0))
       SET xx = (xx+ 1)
       IF (mod(xx,100)=1)
        SET stat = alterlist(eks2->list,(xx+ 99))
       ENDIF
       SET eks2->list[xx].encntrtype = eks->list[x].encntrtype
       SET eks2->list[xx].opind = eks->list[x].opind
       SET eks2->list[xx].dlg_event_id = eks->list[x].dlg_event_id
       SET eks2->list[xx].active_ind = eks->list[x].active_ind
       SET eks2->list[xx].prsnl_id = eks->list[x].prsnl_id
       SET eks2->list[xx].encntr_id = eks->list[x].encntr_id
       SET eks2->list[xx].fmrn = eks->list[x].fmrn
       SET eks2->list[xx].person_id = eks->list[x].person_id
       SET eks2->list[xx].patient_name = eks->list[x].patient_name
       SET eks2->list[xx].prsnl_position_cd = eks->list[x].prsnl_position_cd
       SET eks2->list[xx].severity = eks->list[x].severity
       SET eks2->list[xx].override_rsn_cd = eks->list[x].override_rsn_cd
       SET eks2->list[xx].override_rsn_lt_id = eks->list[x].override_rsn_lt_id
       SET eks2->list[xx].override_rsn_ft = eks->list[x].override_rsn_ft
       SET eks2->list[xx].facility_cd = eks->list[x].facility_cd
       SET eks2->list[xx].trigger.order_id = eks->list[x].trigger.order_id
       SET eks2->list[xx].trigger.catalog_cd = eks->list[x].trigger.catalog_cd
       SET eks2->list[xx].trigger.order_row_exists = eks->list[x].trigger.order_row_exists
       SET eks2->list[xx].trigger.orig_ord_as_flag = eks->list[x].trigger.orig_ord_as_flag
       SET eks2->list[xx].trigger.order_dt_tm = eks->list[x].trigger.order_dt_tm
       SET eks2->list[xx].trigger.drug_class_cd = eks->list[x].trigger.drug_class_cd
       SET eks2->list[xx].trigger.physician_id = eks->list[x].trigger.physician_id
       SET eks2->list[xx].trigger.physician_name = eks->list[x].trigger.physician_name
       SET eks2->list[xx].trigger.physician_position_cd = eks->list[x].trigger.physician_position_cd
       SET eks2->list[xx].interact.order_id = eks->list[x].interact.order_id
       SET eks2->list[xx].interact.catalog_cd = eks->list[x].interact.catalog_cd
       SET eks2->list[xx].interact.orig_ord_as_flag = eks->list[x].interact.orig_ord_as_flag
       SET eks2->list[xx].interact.order_dt_tm = eks->list[x].interact.order_dt_tm
       SET eks2->list[xx].interact.drug_class_cd = eks->list[x].interact.drug_class_cd
       SET eks2->list[xx].interact.physician_id = eks->list[x].interact.physician_id
       SET eks2->list[xx].interact.physician_name = eks->list[x].interact.physician_name
       SET eks2->list[xx].interact.physician_position_cd = eks->list[x].interact.
       physician_position_cd
      ENDIF
    ENDFOR
    SET stat = alterlist(eks2->list,xx)
    FREE RECORD eks
    SET eks2_size = size(eks2->list,5)
    CALL echo("********************************")
    CALL echo(build("end of loop"))
    CALL echo(build(" eks2 :",eks2_size))
   ENDIF
 ENDFOR
 ...
 DECLARE printoption = c1
 FOR (l = 1 TO 4)
   SET output_dest = concat(output_dest,cnvtstring(l))
   SET patienttype = reports->qual[l].patienttype
   SET printoption = reports->qual[l].reportoption
   IF (patienttype="1")
    SET pat_type2 = "Exclude Outpatients"
   ELSEIF (patienttype="2")
    SET pat_type2 = "All Patient Types"
   ENDIF
   IF (((patienttype="2") OR (validate(request->batch_selection))) )
    SELECT INTO "nl:"
     FROM long_text lt,
      (dummyt d  WITH seq = value(size(eks2->list,5)))
     PLAN (d
      WHERE (eks2->list[d.seq].override_rsn_lt_id > 0))
      JOIN (lt
      WHERE (lt.long_text_id=eks2->list[d.seq].override_rsn_lt_id))
     DETAIL
      eks2->list[d.seq].override_rsn_ft = lt.long_text
     WITH nocounter
    ;end select
   ENDIF
   ...
   SELECT INTO value(output_dest)
    trig_mnemonic = uar_get_code_display(eks2->list[d.seq].trigger.catalog_cd), interact_mnemonic =
    uar_get_code_display(eks2->list[d.seq].interact.catalog_cd)
    FROM (dummyt d  WITH seq = value(size(eks2->list,5))),
     orders interact,
     orders trgr,
     dummyt d1,
     dummyt d2
    PLAN (d
     WHERE (((eks2->list[d.seq].encntrtype IN (obs_enc_type_cd, inp_enc_type_cd, day_enc_type_cd,
     ...
     exp_day_enc_type_cd, exp_es_enc_type_cd))
      ...
      AND (eks2->list[d.seq].interact.orig_ord_as_flag IN (1, 0))
      AND (eks2->list[d.seq].severity >= 3)
      AND (eks2->list[d.seq].active_ind=1)
      AND (eks2->list[d.seq].trigger.catalog_cd > 0))
     JOIN (d1)
     JOIN (trgr
     WHERE (trgr.order_id=eks2->list[d.seq].trigger.order_id))
     JOIN (d2)
     JOIN (interact
     WHERE (interact.order_id=eks2->list[d.seq].interact.order_id))
    ...
    DETAIL
     ...
     IF ((eks2->list[d.seq].trigger.order_id > 0)
      AND (eks2->list[d.seq].trigger.order_row_exists=1))
      override_count = (override_count+ 1)
     ENDIF
     IF (report_type="2")
      facility_disp = substring(1,40,uar_get_code_display(eks2->list[d.seq].facility_cd)),
      interact_ord_phys_pos = substring(1,40,uar_get_code_display(eks2->list[d.seq].interact.
        physician_position_cd)), trigger_ord_phys_pos = substring(1,40,uar_get_code_display(eks2->
        list[d.seq].trigger.physician_position_cd)),
      user_position = substring(1,40,uar_get_code_display(eks2->list[d.seq].prsnl_position_cd)),
      override_reason = substring(1,100,uar_get_code_display(eks2->list[d.seq].override_rsn_cd)),
      order_date_disp = format(eks2->list[d.seq].trigger.order_dt_tm,"MM/DD/YYYY HH:MM:SS;;D")
      IF ((eks2->list[d.seq].trigger.order_id > 0)
       AND (eks2->list[d.seq].trigger.order_row_exists=1))
       override_yes_no = "Y"
      ELSE
      ...
      ENDIF
      output_line = build(',"',trim(uar_get_code_display(eks2->list[d.seq].trigger.catalog_cd)),'","',
       trim(uar_get_code_display(eks2->list[d.seq].interact.catalog_cd)),'","',
       eks2->list[d.seq].patient_name,'","',facility_disp,'","',eks2->list[d.seq].trigger.
       physician_name,
       '","',trigger_ord_phys_pos,'","',eks2->list[d.seq].fmrn,'","',
       order_date_disp,'","',user_position,'","',eks2->list[d.seq].trigger.order_id,
       '","',override_reason,'","',eks2->list[d.seq].override_rsn_ft,'","',
       eks2->list[d.seq].severity,'","',override_yes_no,'","',trgr.clinical_display_line,
    ...
    FOOT  interact_mnemonic
     IF (report_type="1")
      ...
      FOR (i = 1 TO size(cat_codes->list,5))
        IF ((cat_codes->list[i].catalog_cd=eks2->list[d.seq].trigger.catalog_cd))
      ...
      alert_percent = 0.00, alert_percent = round(((interact_count * 100)/ tot_cnt),2), output_line
       = build(',"',trim(uar_get_code_display(eks2->list[d.seq].trigger.catalog_cd)),'","',trim(
        uar_get_code_display(eks2->list[d.seq].interact.catalog_cd)),'","',
   ...
   ;end select
   IF (email_ind=1)
    SET filename_in = build(output_dest,".dat")
    SET filename_out = build(output_dest,".csv")
    ...
    SET dclcom = concat("$cust_script/bhs_ftp_file.ksh ",filename_out,
     " 172.17.3.15 egftp egftp /tempfiles/hold/egate/cis_reports ")
    CALL echo(dclcom)
    SET status = 0
    SET len = size(trim(dclcom))
    CALL dcl(dclcom,len,status)
