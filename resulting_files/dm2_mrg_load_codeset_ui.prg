CREATE PROGRAM dm2_mrg_load_codeset_ui
 ...
 SET dim_filename = "cer_install:dm2_mrg_codeset_ui.csv"
 SET readme_data->status = "F"
 SET readme_data->message = "Starting Readme..."
 IF (check_logfile("dm2_mrg_codeset_",".log","dm2_mrg_codeset LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_mrg_codeset..."
 CALL disp_msg("",dm_err->logfile,0)
 IF (findfile(dim_filename)=0)
  SET readme_data->status = "F"
  SET readme_data->message = "Cannot find dm2_mrg_codeset_ui.csv in CER_INSTALL."
  SET dm_err->emsg = "Cannot find dm2_mrg_codeset_ui.csv in CER_INSTALL."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 IF (merge_check_cnt > 0)
  ...
  SELECT INTO "nl:"
   ...
   WITH nocounter
  ;end select
  SET dm_err->eproc = "Updating dm_code_set..."
  CALL disp_msg("",dm_err->logfile,0)
  SET readme_data->status = "F"
  SET readme_data->message = "Updating dm_code_set.."
  UPDATE  FROM dm_code_set dc,
    (dummyt d  WITH seq = merge_check_cnt)
   SET dc.merge_ui_query = requestin->list_0[d.seq].merge_ui_query
   PLAN (d
    WHERE (rs_exist->qual[d.seq].is_there=1))
    JOIN (dc
    WHERE dc.code_set=cnvtint(requestin->list_0[d.seq].code_set)
     AND dc.updt_cnt <= cnvtint(requestin->list_0[d.seq].updt_cnt))
   WITH nocounter
  ;end update
  SET after_cnt = curqual
  INSERT  FROM dm_code_set dc,
    (dummyt d  WITH seq = value(size(rs_exist->qual,5)))
   SET dc.merge_ui_query = requestin->list_0[d.seq].merge_ui_query, dc.code_set = cnvtint(requestin->
     list_0[d.seq].code_set)
   PLAN (d
 ...
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error loading dm_code_set with info in dm2_mrg_codeset.csv."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = "Error loading dm_code_set with info in dm2_mrg_codeset.csv."
  GO TO exit_script
 ELSE
  SET dm_err->eproc = "Finished update of dm_code_set"
  SET readme_data->status = "S"
 ENDIF
 SET dim_cnt = 0
 SELECT INTO "nl:"
  ...
  PLAN (d)
   JOIN (dc
   WHERE dc.code_set=cnvtint(requestin->list_0[d.seq].code_set)
    AND dc.merge_ui_query=trim(requestin->list_0[d.seq].merge_ui_query)
    AND dc.updt_cnt=cnvtint(trim(requestin->list_0[d.seq].updt_cnt)))
  DETAIL
   dim_cnt = (dim_cnt+ 1)
  WITH nocounter
 ;end select
 ...
 ELSE
  SET readme_data->status = "F"
  SET readme_data->message = "Check failed. Rows in request != rows in dm_code_set."
 ENDIF
#exit_script
 IF ((readme_data->status="S"))
  IF (inhouse_ind=1)
   SET readme_data->message = "Not executed since we are in inhouse domain."
  ELSE
   SET readme_data->message = "DM_CODE_SET has been updated successfully."
  ENDIF
 ELSE
  ROLLBACK
 ENDIF
