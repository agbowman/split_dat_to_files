CREATE PROGRAM bhs_ma_error_signing_rpt:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  FROM order_review orv,
   ...
   prsnl pl4
  PLAN (orn
   WHERE orn.caused_by_flag > 0
    AND orn.notification_dt_tm BETWEEN cnvtdatetime((curdate - 1),0) AND cnvtdatetime((curdate - 1),
    235959))
   JOIN (orv
   WHERE orv.order_id=orn.order_id
    AND ((orv.review_type_flag+ 0) > 1)
  ...
  FOOT  pl.name_full_formatted
   col + 0
  FOOT REPORT
   dlrec->ord_total = c, stat = alterlist(dlrec->seq,c), dlrec->seq[c].break_flag = 0
  WITH maxcol = 999
 ;end select
 SELECT INTO "nl:"
 ...
 SELECT INTO  $1
  HEAD REPORT
   ...
   ENDMACRO
   ,
   lastorg = dlrec->seq[c].org_name, pagenum = 0
  HEAD PAGE
   pagenum = (pagenum+ 1),
   CALL print(calcpos(xcol5,yrow1)), "{b}",
   dlrec->seq[c].org_name, "{endb}",
   CALL print(calcpos(xcol1,yrow1)),
   curdate, " ", curtime,
   CALL print(calcpos(xcol12,yrow1)), "PAGE ", pagenum"###;l",
   rowplusone2,
   CALL print(calcpos(xcol2,yrow1)), "{b}ERROR SIGNING REPORT  {endb}",
   rowplusone2,
   CALL print(calcpos(xcol2,yrow1)), "Report Range: ",
   ...
   CALL print(calcpos(xcol4,yrow1)), "{b/5}Order",
   ...
   yrow1 = (yrow1+ 4),
   CALL print(calcpos(xcol1,yrow1)), line1,
   rowplusone2,
   CALL print(calcpos(xcol1,yrow1)), "{B}The Following Were Signed In Error by: ",
   dlrec->seq[c].sign_doctor, "{endb}", rowplusone2,
   rowplusone2
  DETAIL
   FOR (c = 1 TO dlrec->ord_total)
     ...
     IF ((dlrec->seq[c].break_flag=1))
      ...
      pagenum = 0, yrow1 = 5, BREAK
     ENDIF
   ENDFOR
  FOOT PAGE
   row + 1,
   CALL print(calcpos(xcol1,760)), "Page ",
   pagenum"###", row + 1
