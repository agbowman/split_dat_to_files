CREATE PROGRAM cp_br_del_component:dba
 IF (validate(comp_del_request) != 1)
 ...
 RECORD comp_action_list(
   1 cp_node_id = f8
   1 qual[*]
     2 cp_component_id = f8
     2 cp_pathway_id = f8
     2 cp_pathway_action_id = f8
 )
 ...
 DECLARE deleteactionrows(null) = i2 WITH protect
 DECLARE deletedetailrows(null) = i2 WITH protect
 DECLARE deletenodebhvr(null) = null WITH protect
 DECLARE delbedrockcomponent(null) = null WITH protect
 DECLARE pathwaydoccd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003130,"PATHWAY_DOC"))
 DECLARE guidedtrmntcd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003130,"GUIDEDTRMNT"))
 DECLARE verified = i2 WITH noconstant(0), protect
 DECLARE pathwaydocfound = i2 WITH protect, noconstant(0)
 ...
 SET comp_del_reply->status_data.status = "F"
 CALL verifyinputs(null)
 CALL delbedrockcomponent(null)
 CALL callbedrockwizard(null)
 SET comp_del_reply->status_data.status = "S"
 SUBROUTINE verifyinputs(null)
   ...
   DECLARE complistsize = i4 WITH protect, noconstant(size(comp_del_request->qual,5))
   IF (complistsize <= 0)
    CALL reporterrorandexit("PERFORM","Validate","Component list must be nonzero in size.")
   ENDIF
   SELECT INTO "NL:"
    FROM cp_node cn
    WHERE (cn.cp_node_id=comp_del_request->cp_node_id)
    WITH nocounter
   ;end select
   IF (curqual=0)
    CALL reporterrorandexit("PERFORM","Validate","cp_node_id is invalid.")
   ELSE
    SET comp_del_reply->cp_node_id = comp_del_request->cp_node_id
   ENDIF
   CALL log_message(build("Exit verifyInputs(), Elapsed time in seconds:",((curtime3 - begin_curtime3
     )/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE delbedrockcomponent(null)
   CALL log_message("In delBedrockComponent()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE i = i4 WITH protect, noconstant(0)
   DECLARE csize = i4 WITH protect, noconstant(size(comp_del_request->qual,5))
   SET stat = alterlist(br_maintain_req->deleted_components,csize)
   SET br_maintain_req->cp_node_id = comp_del_request->cp_node_id
   FOR (i = 0 TO csize)
     IF ((comp_del_request->qual[i].comp_type_cd IN (guidedtrmntcd, pathwaydoccd)))
      IF (validate(debug_ind,0)=1)
       CALL echo("Found Pathways doc to be deleted")
      ENDIF
      SET pathwaydocfound = 1
     ENDIF
     SET br_maintain_req->deleted_components[i].comp_type_cd = comp_del_request->qual[i].comp_type_cd
     SET br_maintain_req->deleted_components[i].report_mean = comp_del_request->qual[i].report_mean
   ENDFOR
   SET br_maintain_req->deleted_components_cnt = size(br_maintain_req->deleted_components,5)
   IF (validate(debug_ind,0)=1)
    CALL echorecord(br_maintain_req)
   ENDIF
   CALL log_message(build("Exit delBedrockComponent(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE callbedrockwizard(null)
   CALL log_message("In callBedrockWizard()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   EXECUTE cp_maintain_br_wizard "NOFORMS"
   IF ((br_maintain_rep->status_data.status="S"))
    SET comp_del_reply->status_data.status = "S"
    CALL deleteactionrows(null)
    CALL deletedetailrows(null)
    IF (pathwaydocfound=1
     AND (comp_del_request->keep_node_behaviors=0))
     CALL deletenodebhvr(null)
    ENDIF
    CALL deleterows(null)
   ELSE
    SET comp_del_reply->status_data.status = br_maintain_rep->status_data.status
    SET stat = moverec(br_maintain_rep->status_data,comp_del_reply->status_data)
    GO TO exit_script
   ENDIF
   CALL log_message(build("Exit callBedrockWizard(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE deletenodebhvr(null)
   CALL log_message("In deleteNodeBhvr()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE bhvrcnt = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM cp_node_behavior cnb
    PLAN (cnb
     WHERE (cnb.cp_node_id=comp_del_request->cp_node_id))
    HEAD REPORT
     bhvrcnt = 0
    HEAD cnb.cp_node_behavior_id
     bhvrcnt += 1, behaviors_to_delete->cnt = bhvrcnt
     IF (bhvrcnt > size(behaviors_to_delete->qual,5))
      stat = alterlist(behaviors_to_delete->qual,(bhvrcnt+ 5))
     ENDIF
     behaviors_to_delete->qual[bhvrcnt].value = cnb.cp_node_behavior_id, behaviors_to_delete->qual[
     bhvrcnt].reaction_entity_id = cnb.reaction_entity_id, behaviors_to_delete->qual[bhvrcnt].
     reaction_entity_name = cnb.reaction_entity_name
    FOOT REPORT
     stat = alterlist(behaviors_to_delete->qual,bhvrcnt)
    WITH nocounter
   ;end select
   IF (size(behaviors_to_delete->qual,5) > 0)
    IF (validate(debug_ind,0)=1)
     CALL echorecord(behaviors_to_delete)
    ENDIF
    EXECUTE cp_del_node_bhvr "NOFORMS", "" WITH replace("NBLIST","BEHAVIORS_TO_DELETE")
    IF ((reply->status_data.status != "S"))
     CALL reporterrorandexit("PERFORM","DELETE"," CP_NODE_BHVR")
    ENDIF
   ELSE
    CALL echo("Unable to any associated behaviors")
   ENDIF
   CALL log_message(build("Exit deleteNodeBhvr(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE deleterows(null)
   ...
   DECLARE cp_component_id = f8 WITH noconstant(0), protect
   IF (checkdic("CP_COMPONENT","T",0) != 2)
    CALL reporterrorandexit("PERFORM","INSERT",
     " CP_COMPONENT does not exist or permission is not granted to access it.")
   ENDIF
   SET stat = alterlist(comp_del_reply->component_list,size(comp_del_request->qual,5))
   FOR (x = 1 TO size(comp_del_request->qual,5))
     SET cp_component_id = 0.0
     SELECT INTO "nl:"
      FROM cp_component cc
      WHERE (cc.cp_node_id=comp_del_request->cp_node_id)
       AND (cc.comp_type_cd=comp_del_request->qual[x].comp_type_cd)
       AND (cc.cp_component_id=comp_del_request->qual[x].cp_component_id)
      WITH nocounter
     ;end select
     IF (curqual=0)
      CALL reporterror("PERFORM","RETRIEVE"," Unable to find CP_COMPONENT_ID on CP_COMPONENT table.")
      ROLLBACK
      GO TO exit_script
     ENDIF
     DELETE  FROM cp_component cc
      WHERE (cc.cp_node_id=comp_del_request->cp_node_id)
       AND (cc.comp_type_cd=comp_del_request->qual[x].comp_type_cd)
       AND (cc.cp_component_id=comp_del_request->qual[x].cp_component_id)
      WITH nocounter
     ;end delete
     IF (curqual=0)
      CALL reporterror("PERFORM","DELETE","Unable to delete row from CP_COMPONENT table.")
      ROLLBACK
      GO TO exit_script
     ELSE
      SET comp_del_reply->component_list[x].comp_type_cd = comp_del_request->qual[x].comp_type_cd
      SET comp_del_reply->component_list[x].cp_component_id = comp_del_request->qual[x].
      cp_component_id
     ENDIF
   ENDFOR
   COMMIT
   CALL log_message(build("Exit deleteRows(), Elapsed time in seconds:",((curtime3 - begin_curtime3)
     / 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE deletedetailrows(null)
   CALL log_message("In deleteDetailRows()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH private, constant(curtime3)
   DECLARE x = i4 WITH protect, noconstant(0)
   DECLARE cp_component_id = f8 WITH protect, noconstant(0)
   IF (checkdic("CP_COMPONENT_DETAIL","T",0) != 2)
    CALL reporterrorandexit("PERFORM","DELETE",
     " CP_COMPONENT_DETAIL does not exist or permission is not granted to access it.")
   ENDIF
   SELECT INTO "nl:"
    FROM (dummyt d1  WITH seq = size(comp_del_request->qual,5)),
     cp_component_detail ccd
    PLAN (d1
     WHERE d1.seq <= size(comp_del_request->qual,5))
     JOIN (ccd
     WHERE (ccd.cp_component_id=comp_del_request->qual[d1.seq].cp_component_id))
    HEAD REPORT
     i = 0
    DETAIL
     i += 1
     IF (i > size(comp_detail_list->qual,5))
      stat = alterlist(comp_detail_list->qual,(i+ 5))
     ENDIF
     comp_detail_list->qual[i].cp_component_id = ccd.cp_component_id, comp_detail_list->qual[i].
     cp_component_detail_id = ccd.cp_component_detail_id, comp_detail_list->qual[i].
     comp_detail_reltn_cd = ccd.component_detail_reltn_cd,
     comp_detail_list->qual[i].component_entity_id = ccd.component_entity_id, comp_detail_list->qual[
     i].component_entity_name = ccd.component_entity_name, comp_detail_list->qual[i].component_ident
      = ccd.component_ident,
     comp_detail_list->qual[i].component_text = ccd.component_text
    FOOT REPORT
     stat = alterlist(comp_detail_list->qual,i)
    WITH nocounter
   ;end select
   IF (size(comp_detail_list->qual,5) > 0)
    EXECUTE cp_delete_component_detail  WITH replace(request,comp_detail_list)
   ELSE
    CALL echo("Unable to find CP_COMPONENT_DETAIL rows to delete.")
   ENDIF
   CALL log_message(build("Exit deleteDetailRows(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE deleteactionrows(null)
   CALL log_message("In deleteActionRows()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH private, constant(curtime3)
   DECLARE x = i4 WITH protect, noconstant(0)
   DECLARE cp_component_id = f8 WITH protect, noconstant(0)
   IF (checkdic("CP_PATHWAY_ACTION","T",0) != 2)
    CALL reporterrorandexit("PERFORM","DELETE",
     " CP_PATHWAY_ACTION does not exist or permission is not granted to access it.")
   ENDIF
   SELECT INTO "nl:"
    FROM (dummyt d1  WITH seq = size(comp_del_request->qual,5)),
    ...
    PLAN (d1
     WHERE d1.seq <= size(comp_del_request->qual,5))
     JOIN (cpa
     WHERE (cpa.cp_component_id=comp_del_request->qual[d1.seq].cp_component_id))
    HEAD REPORT
     i = 0
    DETAIL
     i += 1
     IF (i > size(comp_action_list->qual,5))
      stat = alterlist(comp_action_list->qual,(i+ 5))
     ENDIF
     comp_action_list->qual[i].cp_component_id = cpa.cp_component_id, comp_action_list->qual[i].
     cp_pathway_action_id = cpa.cp_pathway_action_id
    FOOT REPORT
     stat = alterlist(comp_action_list->qual,i)
    WITH nocounter
   ;end select
   IF (size(comp_action_list->qual,5) > 0)
    EXECUTE cp_del_pathway_action  WITH replace(action_del_request,comp_action_list)
   ELSE
    CALL echo("Unable to find CP_PATHWAY_ACTION rows to delete.")
   ENDIF
   CALL log_message(build("Exit deleteActionRows(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (checkcodevalue(codevalue=f8,codeset=i4) =i2 WITH protect)
   CALL log_message("In checkCodeValue()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SELECT INTO "NL:"
    FROM code_value cv
   ...
   ;end select
   RETURN(curqual)
   CALL log_message(build("Exit checkCodeValue(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 ...
 SUBROUTINE (reporterror(operationname=vc,targetobjectname=vc,targetobjectvalue=vc) =null WITH
  protect)
   ...
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SET comp_del_reply->status_data.status = "F"
   SET comp_del_reply->status_data.subeventstatus.operationname = operationname
   SET comp_del_reply->status_data.subeventstatus.operationstatus = "F"
   SET comp_del_reply->status_data.subeventstatus.targetobjectname = targetobjectname
   SET comp_del_reply->status_data.subeventstatus.targetobjectvalue = targetobjectvalue
   CALL log_message(build("Exit reportError(), Elapsed time in seconds:",((curtime3 - begin_curtime3)
     / 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE (verifynonzerocodevalue(codevalue=f8,codeset=i4,codesetname=vc) =null WITH protect)
   CALL log_message("In verifyNonZeroCodeValue()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH private, constant(curtime3)
   DECLARE codevaluevalid = i2 WITH protect, noconstant(0)
   IF (codevalue > 0)
    SET codevaluevalid = checkcodevalue(codevalue,codeset)
    IF (codevaluevalid != 1)
     CALL reporterrorandexit("PERFORM","Validate",build2(codesetname," code value ",trim(cnvtstring(
         codevalue),3)," does not exist or is inactive."))
    ENDIF
   ELSE
    CALL reporterrorandexit("PERFORM","Validate",build2(codesetname,
      " code value must be greater than 0."))
   ENDIF
   CALL log_message(build("Exit verifyNonZeroCodeValue(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
#exit_script
 IF (validate(debug_ind,0)=1)
