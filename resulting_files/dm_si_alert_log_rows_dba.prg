CREATE PROGRAM dm_si_alert_log_rows:dba
 ...
 DECLARE itokenidx = i4 WITH protect, noconstant(0)
 ...
 SET reply->table_name = "SI_ALERT_EVENT"
 SET reply->rows_between_commit = 1000
 FOR (itokenidx = 1 TO size(request->tokens,5))
   IF ((request->tokens[itokenidx].token_str="DAYSTOKEEP"))
    SET idaystokeep = ceil(cnvtreal(request->tokens[itokenidx].value))
   ENDIF
 ENDFOR
 IF (idaystokeep < 30)
 ...
 SELECT INTO "nl:"
  sae.rowid
  FROM si_alert_event sae
  WHERE parser(sbr_getrowidnotexists("sae.create_dt_tm < cnvtdatetime(v_days_back)","sae"))
   AND ((sae.si_alert_event_id+ 0) > 0.0)
  DETAIL
   icnt += 1
   IF (mod(icnt,100)=1)
    stat = alterlist(reply->rows,(icnt+ 99))
   ENDIF
   reply->rows[icnt].row_id = sae.rowid
  WITH nocounter, maxqual(sae,value(request->max_rows))
 ;end select
