CREATE PROGRAM dm2_shc_ast_limit:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsal_status = c1 WITH protect, noconstant("E")
 DECLARE dsal_fatal_err_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_shc_ast",".log","SYSTEM HEALTH CHECK AST_LIMIT LOG FILE")=0)
  SET dsal_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 IF (cursys != "AXP")
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking OpenVMS AST Limit setting."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 SET dshc_lreg_getp->key_name = concat("\node\",dshc_env_info->node_name,"\domain\",dshc_env_info->
  domain_name,'\prop\"AST Limit"')
 SET dshc_lreg_getp->propname = '"DefaultValue"'
 SET dshc_lreg_getp->value_type = 1
 SET return_val = dshc_lreg_getp(null)
 IF (return_val=0)
  SET dsal_status = "E"
 ELSEIF ((dshc_lreg_getp->propvalue != "")
  AND cnvtint(dshc_lreg_getp->propvalue) >= 2048)
  SET dsal_status = "G"
 ELSE
  SET dsal_status = "R"
 ENDIF
 IF (dshc_add_check_to_rs("AST LIMIT","AST Limit",0,dsal_status,dshc_ctf_millennium)=0)
  SET dsal_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 IF (dsal_status IN ("E", "G"))
  GO TO exit_script
 ENDIF
 SET dshc->check[dshc->chk_cnt].desc_dtl_cnt = 1
 SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,1)
 SET dshc->check[dshc->chk_cnt].desc_detail[1].name = "AST Limit"
 SET dshc->check[dshc->chk_cnt].desc_detail[1].value_prefix = "Current Value: "
 SET dshc->check[dshc->chk_cnt].desc_detail[1].dtl_desc_text = " "
 SET dshc->check[dshc->chk_cnt].desc_detail[1].type = 1
 SET dshc->check[dshc->chk_cnt].desc_detail[1].nbr_value = cnvtint(dshc_lreg_getp->propvalue)
 SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
  "AST Limit should be set to 2048 or higher for all servers.  ",
  "This setting increases the number of threads a process can have ",
  "and eliminates processes hanging with an RWAST status when global ",
  "buffering is enabled for the CCL dictionary.")
#exit_script
 SET dm_err->eproc = "Finished checking the AST Limit."
 CALL final_disp_msg("dm2_shc_ast")
 IF (dsal_fatal_err_ind=0)
  SET dm_err->err_ind = 0
 ENDIF
END GO
