CREATE PROGRAM dm_rmc_audit_dml:dba
...
","    char_0_ind = 1 "),0)
 ...
 SUBROUTINE draq_outer_join(dl_md_loc,dl_info_loc,dl_info,dl_parse1,dl_parse2)
   ...
   RETURN(null)
 END ;Subroutine
 DECLARE drad_tab_name = vc WITH protect, noconstant(" ")
 DECLARE drad_limit = f8 WITH protect, noconstant(0.0)
 DECLARE drad_loop = i4 WITH protect, noconstant(0)
 DECLARE drad_md_loc = i4 WITH protect, noconstant(0)
 DECLARE drad_info_loc = i4 WITH protect, noconstant(0)
 DECLARE drad_tgt_pkw_str = vc WITH protect, noconstant(" ")
 DECLARE drad_src_pkw_str = vc WITH protect, noconstant(" ")
 DECLARE drad_sfltr_str = vc WITH protect, noconstant(" ")
 DECLARE drad_fltr_str = vc WITH protect, noconstant(" ")
 FREE RECORD drad_parse1
 RECORD drad_parse1(
   1 stmt[*]
     2 str = vc
 )
 FREE RECORD drad_parse2
 RECORD drad_parse2(
   1 stmt[*]
     2 str = vc
 )
 FREE RECORD drad_parse3
 RECORD drad_parse3(
   1 stmt[*]
     2 str = vc
 )
 FREE RECORD drad_parse4
 RECORD drad_parse4(
   1 stmt[*]
     2 str = vc
 )
 FREE RECORD drad_parse5
 RECORD drad_parse5(
   1 stmt[*]
     2 str = vc
 )
 IF (check_logfile("dm_rmc_audit_dml",".log","dm_rmc_audit_dml LOG FILE...") != 1)
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm_rmc_audit_dml..."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (reflect(parameter(1,0)) != "C*")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = 'Expected syntax:  dm_rmc_audit_dml "<Table Name>"'
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET drad_tab_name =  $1
 IF ((dm_err->debug_flag > 0))
  CALL echo(drad_tab_name)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info d
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 CALL drm_md_wrp(drad_tab_name,drgi_global->db_link)
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET drad_md_loc = locateval(drad_md_loc,1,tmc_md_info->tbl_cnt,drad_tab_name,tmc_md_info->tbl_qual[
  drad_md_loc].table_name)
 SET table_info->tab_cnt = (table_info->tab_cnt+ 1)
 SET stat = alterlist(table_info->tables,table_info->tab_cnt)
 SET table_info->tables[table_info->tab_cnt].table_name = tmc_md_info->tbl_qual[drad_md_loc].
 table_name
 CALL dra_addl_tab_info(table_info,table_info->tab_cnt,drgi_global,tmc_md_info,drad_md_loc)
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET drad_info_loc = locateval(drad_info_loc,1,table_info->tab_cnt,drad_tab_name,table_info->tables[
  drad_info_loc].table_name)
 CALL draq_fcn_strs(drad_info_loc,table_info)
 IF ((table_info->tables[table_info->tab_cnt].pk_cnt <= 1))
  CALL draq_direct_join(drad_md_loc,drad_info_loc,table_info,drad_parse1,drad_parse2,
   drad_parse3,drad_parse4,drad_parse5)
  CALL echo(fillstring(130,"*"))
  CALL echo(concat("DML for table ",drad_tab_name," on data set smaller than ",trim(cnvtstring(
      drad_limit,20))," rows:"))
  CALL echo(fillstring(130,"*"))
  CALL echo("SMALL DML 1:")
  FOR (drad_loop = 1 TO size(drad_parse1->stmt,5))
    CALL echo(replace(replace(drad_parse1->stmt[drad_loop].str,"ASIS(^"," "),"^)"," "))
  ENDFOR
  CALL echo(fillstring(130,"*"))
  CALL echo("SMALL DML 2:")
  FOR (drad_loop = 1 TO size(drad_parse2->stmt,5))
    CALL echo(replace(replace(drad_parse2->stmt[drad_loop].str,"ASIS(^"," "),"^)"," "))
  ENDFOR
  CALL echo(fillstring(130,"*"))
  CALL echo("SMALL DML 3:")
  FOR (drad_loop = 1 TO size(drad_parse3->stmt,5))
    CALL echo(replace(replace(drad_parse3->stmt[drad_loop].str,"ASIS(^"," "),"^)"," "))
  ENDFOR
  CALL echo(fillstring(130,"*"))
  CALL echo("SMALL DML 4:")
  FOR (drad_loop = 1 TO size(drad_parse4->stmt,5))
    CALL echo(replace(replace(drad_parse4->stmt[drad_loop].str,"ASIS(^"," "),"^)"," "))
  ENDFOR
  CALL echo(fillstring(130,"*"))
  CALL echo("SMALL DML 5:")
  FOR (drad_loop = 1 TO size(drad_parse5->stmt,5))
    CALL echo(replace(replace(drad_parse5->stmt[drad_loop].str,"ASIS(^"," "),"^)"," "))
  ENDFOR
  SET stat = initrec(drad_parse1)
  SET stat = initrec(drad_parse2)
  SET stat = initrec(drad_parse3)
  SET stat = initrec(drad_parse4)
  SET stat = initrec(drad_parse5)
 ENDIF
 CALL draq_outer_join(drad_md_loc,drad_info_loc,table_info,drad_parse1,drad_parse2)
 CALL echo(fillstring(130,"*"))
 CALL echo(fillstring(130,"*"))
 IF ((table_info->tables[table_info->tab_cnt].pk_cnt <= 1))
  CALL echo(concat("DML for table ",drad_tab_name," on data set larger than ",trim(cnvtstring(
      drad_limit,20))," rows:"))
 ELSE
  CALL echo(concat("DML for multi-column pk table ",drad_tab_name,":"))
 ENDIF
 CALL echo(fillstring(130,"*"))
 CALL echo("LARGE DML 1:")
 FOR (drad_loop = 1 TO size(drad_parse1->stmt,5))
   CALL echo(replace(replace(drad_parse1->stmt[drad_loop].str,"ASIS(^"," "),"^)"," "))
 ENDFOR
 CALL echo(fillstring(130,"*"))
 CALL echo("LARGE DML 2:")
 FOR (drad_loop = 1 TO size(drad_parse2->stmt,5))
   CALL echo(replace(replace(drad_parse2->stmt[drad_loop].str,"ASIS(^"," "),"^)"," "))
 ENDFOR
 CALL echo(fillstring(130,"*"))
