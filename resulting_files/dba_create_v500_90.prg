CREATE PROGRAM dba_create_v500_90
 SET message = window
 ...
 SUBROUTINE aix_create_script(x)
    ...
    *
    ...
    DETAIL
     ...
     CALL print(request->oracle_home),
     row + 1, col 0, "     export ORA_RDBMS=$ORACLE_HOME/rdbms/admin",
     row + 1, col 0, "     export ORA_INSTALL=$ORACLE_HOME/orainst",
     row + 1, col 0, "     export TNS_ADMIN=$ORACLE_HOME/network/admin",
     row + 1, col 0, "     export ORA_ADMIN=",
     CALL print(oracle_mtpt), "/oracle/admin/",
     CALL print(cnvtlower(dbname)),
     row + 1, col 0, "     export ORA_ALERT=$ORA_ADMIN/bdump/",
     CALL print(cnvtlower(dbname)), "1", row + 1,
     col 0, "     export ORA_DUMP=$ORA_ADMIN/udump/",
     CALL print(cnvtlower(dbname)),
     "1", row + 1, col 0,
     "     export ORA_ARCH=$ORA_ADMIN/arch", row + 1, col 0,
     "     export ORA_PFILE=$ORA_ADMIN/pfile", row + 1, col 0,
     "     export cer_install=",
     CALL print(request->cer_install), row + 1,
     col 0, "     alias sqlplus=$ORACLE_HOME/bin/sqlplus", row + 1,
     col 0, "     alias exp=$ORACLE_HOME/bin/exp", row + 1,
     col 0, "     alias imp=$ORACLE_HOME/bin/imp", row + 1,
     col 0, "     alias oerr=$ORACLE_HOME/bin/oerr", row + 1,
     col 0, "     alias lsnrctl=$ORACLE_HOME/bin/lsnrctl", row + 1,
     col 0, "     alias orapwd=$ORACLE_HOME/bin/orapwd", row + 1,
     col 0, "     alias dbfsize=$ORACLE_HOME/bin/dbfsize", row + 1,
     col 0, "fi"
    ...
    *
    ...
    DETAIL
     ...
     "export ORACLE_HOME=",
     CALL print(request->oracle_home), row + 1,
     col 0, "export ORACLE_SID=",
     CALL print(cnvtlower(dbname)),
     "1", row + 1, col 0,
     " ", row + 1, col 0,
     "$ORACLE_HOME/bin/sqlplus /nolog <<!", row + 1, col 0,
    ...
    *
    ...
    DETAIL
     ...
     "$ORACLE_HOME/bin/sqlplus /nolog <<!", row + 1, col 0,
     "connect / as sysdba", row + 1, col 0,
     "shutdown immediate", row + 1, col 0,
     "exit", row + 1, col 0,
     "!", row + 1
    WITH nocounter, format = stream, noheading,
     formfeed = none, maxrow = 1
