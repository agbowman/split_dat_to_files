CREATE PROGRAM bhs_gvw_enc_mrn:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (e
   ...
   WHERE ea1.encntr_id=e.encntr_id
    ...
    AND ea1.encntr_alias_type_cd=mf_cs319_mrn_cd)
  ORDER BY e.encntr_id, ea1.beg_effective_dt_tm DESC
  HEAD e.encntr_id
   ms_mrn = trim(ea1.alias,3)
  WITH nocounter
 ;end select
 IF (size(trim(ms_mrn,3)) > 0)
  SET reply->text = concat("{\rtf1\ansi \deff0{\fonttbl{\f0\fswiss Tahoma;}}","\fs18 ",ms_mrn,
   " \par}")
 ENDIF
#exit_script
END GO
