CREATE PROGRAM cv_da_upt_cv_step_prsnl:dba
 IF (validate(stat)=0)
 ...
 SUBROUTINE (cv_log_msg_post(script_vrsn=vc) =null)
  ...
  ENDIF
 END ;Subroutine
 DECLARE cv_da_upt_cv_step_prsnl_vrsn = vc WITH private, constant("20050914")
 ...
 SET msdatablename = "CV_STEP_PRSNL"
 ...
 WHILE (i <= size(request->objarray,5))
   ...
   SELECT INTO "nl:"
    c.*
    FROM cv_step_prsnl c
    WHERE (c.cv_step_prsnl_id=request->objarray[i].cv_step_prsnl_id)
    DETAIL
     IF ((validate(request->objarray[i].cv_step_id,c.cv_step_id) != - (0.00001))
      ...
      alteredind = 1
     ENDIF
     IF ((validate(request->objarray[i].step_prsnl_id,c.step_prsnl_id) != - (0.00001))
      AND validate(request->objarray[i].step_prsnl_id,c.step_prsnl_id) != c.step_prsnl_id)
      alteredind = 1
     ENDIF
     IF ((validate(request->objarray[i].step_relation_cd,c.step_relation_cd) != - (0.00001))
      AND validate(request->objarray[i].step_relation_cd,c.step_relation_cd) != c.step_relation_cd)
      alteredind = 1
     ENDIF
     IF ((validate(request->objarray[i].action_type_cd,c.action_type_cd) != - (0.00001))
      AND validate(request->objarray[i].action_type_cd,c.action_type_cd) != c.action_type_cd)
      alteredind = 1
     ENDIF
     IF (((validate(request->objarray[i].action_dt_tm,c.action_dt_tm) != 0.0
      ...
      alteredind = 1
     ENDIF
     IF ((validate(request->objarray[i].action_sequence,c.action_sequence) != - (1))
      AND validate(request->objarray[i].action_sequence,c.action_sequence) != c.action_sequence)
      alteredind = 1
     ENDIF
   ...
   IF (alteredind > 0)
    UPDATE  FROM cv_step_prsnl c
     SET c.cv_step_id =
      ...
      ENDIF
      , c.step_prsnl_id =
      IF ((validate(request->objarray[i].step_prsnl_id,- (0.00001)) != - (0.00001))) validate(request
        ->objarray[i].step_prsnl_id,- (0.00001))
      ELSE c.step_prsnl_id
      ENDIF
      , c.step_relation_cd =
      IF ((validate(request->objarray[i].step_relation_cd,- (0.00001)) != - (0.00001))) validate(
        request->objarray[i].step_relation_cd,- (0.00001))
      ELSE c.step_relation_cd
      ENDIF
      ,
      c.action_type_cd =
      IF ((validate(request->objarray[i].action_type_cd,- (0.00001)) != - (0.00001))) validate(
        request->objarray[i].action_type_cd,- (0.00001))
      ELSE c.action_type_cd
      ENDIF
      , c.action_dt_tm =
      ...
      ENDIF
      , c.action_sequence =
      IF ((validate(request->objarray[i].action_sequence,- (1)) != - (1))) validate(request->
        objarray[i].action_sequence,- (1))
      ELSE c.action_sequence
      ENDIF
      ,
