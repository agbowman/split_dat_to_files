CREATE PROGRAM co_get_events_by_codes:dba
 RECORD reply(
   1 events[*]
     2 event_cd = f8
     2 event_dt_tm = dq8
 ...
 IF (size(request->event_codes,5) > 0)
  ...
  SET modify_cd = meaning_code(8,"MODIFIED")
  SET alter_cd = meaning_code(8,"ALTERED")
  ...
  ELSE
   ...
   SET validation_qual = build(validation_qual,
    " and ce.publish_flag = 1 and ce.result_status_cd in ( ",auth_cd,", ",modify_cd,
    ", ",alter_cd,")")
  ENDIF
  SELECT INTO "nl:"
