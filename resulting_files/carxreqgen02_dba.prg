CREATE PROGRAM carxreqgen02:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
    ...
    SELECT INTO value(tprint_req->job[i].print_loc)
     ...
     HEAD REPORT
      ...
      MACRO (does_req_fit)
       req_fit = true, temp_row_knt = current_row_knt, temp_row_knt = (temp_row_knt+ ((((((((
       tprint_req->job[i].req[j].med_knt+ tprint_req->job[i].req[j].sig_knt)+ tprint_req->job[i].req[
       j].dispense_knt)+ tprint_req->job[i].req[j].refill_knt)+ tprint_req->job[i].req[j].special_knt
       )+ tprint_req->job[i].req[j].prn_knt)+ tprint_req->job[i].req[j].indic_knt)+ tprint_req->job[i
       ].req[j].comment_knt)+ 1))
       IF (temp_row_knt > max_row_knt)
        req_fit = false
       ENDIF
      ...
      MACRO (print_footer)
       ...
       CALL print(calcpos(x_pos,y_pos)),
       "{b}", signature_line, "{endb}",
       row + 1, x_pos = (c_pos+ 205),
       CALL print(calcpos(x_pos,y_pos)),
       "Date: ", tprint_req->job[i].phys_ord_dt, row + 1,
       x_pos = j_pos,
       CALL print(calcpos(x_pos,y_pos)), "{b}",
