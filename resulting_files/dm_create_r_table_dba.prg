CREATE PROGRAM dm_create_r_table:dba
 ...
 SUBROUTINE create_r_only(cro_tab_name,cro_r_tab_name,cro_trig_ind,cro_remove_lock_ind)
   ...
   RETURN(cro_return)
 END ;Subroutine
 IF (check_logfile("DM_CREATE_R_TABLE",".log","DM_CREATE_R_TABLE LogFile")=0)
  GO TO exit_prg
 ENDIF
 ...
 DECLARE tablename_upper = vc
 DECLARE trigger_ind = i2
 DECLARE tablenamer = vc
 DECLARE rmc_tab_cnt = i4
 DECLARE dm_err_msg = vc
 DECLARE v_$r_return = c1
 SET tablename_upper = ""
 SET trigger_ind = 0
 SET tablenamer = ""
 SET rmc_tab_cnt = 0
 SET v_$r_return = ""
 SET tablename_upper = trim(cnvtupper( $1),3)
 SET trigger_ind =  $2
 SELECT INTO "nl:"
  ut.table_name
  FROM user_tables ut
  WHERE ut.table_name=patstring(tablename_upper)
   AND ut.table_name != "*$R"
  DETAIL
   rmc_tab_cnt = (rmc_tab_cnt+ 1)
   IF (mod(rmc_tab_cnt,10)=1)
    stat = alterlist(rmc_tables->list,(rmc_tab_cnt+ 9))
   ENDIF
   rmc_tables->list[rmc_tab_cnt].table_name = ut.table_name, rmc_tables->list[rmc_tab_cnt].r_ind = 0
  FOOT REPORT
   stat = alterlist(rmc_tables->list,rmc_tab_cnt)
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET dm_err->eproc = "Invalid Table Name!"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_prg
 ENDIF
 IF (check_error("Validating input table_name(s)") != 0)
  ...
  GO TO exit_prg
 ENDIF
 FOR (for_cnt = 1 TO rmc_tab_cnt)
   SET rmc_tables->list[for_cnt].r_tab_name = cutover_tab_name(rmc_tables->list[for_cnt].table_name,
    "")
 ENDFOR
 SELECT INTO "nl:"
  ut.table_name
  FROM user_tables ut,
   (dummyt d  WITH seq = rmc_tab_cnt)
  PLAN (d)
   JOIN (ut
   WHERE (ut.table_name=rmc_tables->list[d.seq].r_tab_name))
  DETAIL
   rmc_tables->list[d.seq].r_ind = 1
  WITH nocounter
 ;end select
 IF (check_error("Verifying $R table version") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_prg
 ENDIF
 FOR (for_cnt2 = 1 TO rmc_tab_cnt)
   IF ((rmc_tables->list[for_cnt2].r_ind=0))
    SET v_$r_return = "F"
    SET v_$r_return = create_$r_tab(rmc_tables->list[for_cnt2].table_name,trigger_ind)
    IF (((error(dm_err_msg,1) != 0) OR (v_$r_return="F")) )
     CALL disp_msg("",dm_err->logfile,1)
     SET rmc_tables->list[for_cnt2].r_ind = 3
    ELSE
     SET rmc_tables->list[for_cnt2].r_ind = 2
    ENDIF
   ENDIF
 ENDFOR
 FOR (for_cnt3 = 1 TO rmc_tab_cnt)
   IF ((rmc_tables->list[for_cnt3].r_ind=1))
    SET dm_err->eproc = concat(rmc_tables->list[for_cnt3].r_tab_name," table already exists!")
    CALL disp_msg(" ",dm_err->logfile,0)
   ELSEIF ((rmc_tables->list[for_cnt3].r_ind=2))
    SET dm_err->eproc = concat(rmc_tables->list[for_cnt3].r_tab_name," table has been created!")
    CALL disp_msg(" ",dm_err->logfile,0)
   ELSEIF ((rmc_tables->list[for_cnt3].r_ind=3))
    SET dm_err->eproc = concat(rmc_tables->list[for_cnt3].r_tab_name," was NOT created due to error!"
     )
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
 ENDFOR
#exit_prg
 FREE RECORD rmc_tables
 SET dm_err->eproc = "...Ending dm_create_r_table"
 CALL final_disp_msg("DM_CREATE_R_TABLE")
END GO
