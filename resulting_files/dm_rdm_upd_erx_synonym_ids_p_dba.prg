CREATE PROGRAM dm_rdm_upd_erx_synonym_ids_p:dba
 ...
 SET ms_range_prefix = "MAX IB_RX_REQ_DRUG_ID RANGE"
 SET mn_num_children = 5
 SELECT INTO "nl:"
  min_id = min(irrd.ib_rx_req_drug_id), max_id = max(irrd.ib_rx_req_drug_id)
 ...
 IF (sbr_run_parent(mf_max_range_id,mf_min_range_id,ms_range_prefix,ms_info_domain_nm,mn_num_children
  ...
  GO TO exit_program
 ENDIF
 CALL sbr_add_sql_stmt("TABLE","TEMP_IB_RX_REQ_DRUG",concat(
   "create global temporary table TEMP_IB_RX_REQ_DRUG(ib_rx_req_drug_id number)",
   "on commit preserve rows"),"Creating temporary table")
 CALL sbr_add_sql_stmt("INDEX","XPKTEMP_IB_RX_REQ_DRUG",
  "create unique index XPKTEMP_IB_RX_REQ_DRUG ON TEMP_IB_RX_REQ_DRUG(ib_rx_req_drug_id)",
  "Creating index")
