CREATE PROGRAM ags_prsnl_clean:dba
 PROMPT
  "JOB_ID (0.0) = " = 0
  WITH djob_id
 CALL echo("***")
 CALL echo("***   BEG AGS_PRSNL_CLEAN")
 CALL echo("***")
 ...
 DECLARE sstatus_file_name = vc WITH public, noconstant(concat("ags_prsnl_clean_",format(cnvtdatetime
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> ags_prsnl_clean"
 ...
 CALL echo("***")
 CALL echo("***")
 CALL echo("Determine Provider Directory Status")
 CALL echo("***")
 ...
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("Validate Provider Directory Status :: Select Error :: ",
   trim(serrmsg))
 ...
 IF (icutoverflag < 1)
  ...
  SET log->qual[log->qual_knt].smsgtype = "INFO"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = "No Provider Directory Cut-Over"
 ELSE
  SET log->qual_knt = (log->qual_knt+ 1)
  SET stat = alterlist(log->qual,log->qual_knt)
  SET log->qual[log->qual_knt].smsgtype = "INFO"
 ...
 SELECT INTO "nl:"
  ...
  PLAN (p1
   ...
   WHERE p2.person_id=p1.person_id
    AND ((p2.contributor_system_cd+ 0)=p1.contributor_system_cd)
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> ags_prsnl_clean"
 CALL echo("***")
