CREATE PROGRAM ch_ops_journal:dba
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 SET print_dt_tm = cnvtdatetime(curdate,curtime3)
 ...
 SELECT DISTINCT INTO value(outfile)
  cr.chart_request_id, cr.request_type, cr.dist_run_dt_tm"yyyy-mmm-dd hh:mm:ss ;;d",
  cr.encntr_id, accession_number = uar_fmt_accession(cr.accession_nbr,size(cr.accession_nbr,1)), cr
  .order_id,
  cr.scope_flag, display_dist = uar_get_code_display(cr.dist_run_type_cd), cf.chart_format_id,
  cf.chart_format_desc, od.output_dest_cd, od.description,
  ...
  PLAN (cr
   ...
   JOIN (cf
   WHERE cr.chart_format_id=cf.chart_format_id)
   JOIN (p
  ...
  HEAD REPORT
   ...
   " ", cur_tz_name, row + 1,
   col 1, "Chart Format:", col + 2,
   CALL print(substring(1,50,cf.chart_format_desc)), col 84, "Print Date/Time:",
   col + 1, print_dt_tm";;q", " ",
  ...
  HEAD PAGE
   ...
   row + 1, line1, row + 1
  DETAIL
   chart_ind = 1, chart_count = (chart_count+ 1), size_patients = size(patients->qual,5),
   already_there = 0
   ...
   ENDFOR
   IF (already_there=0)
    pat_count = (pat_count+ 1), stat = alterlist(patients->qual,pat_count), patients->qual[pat_count]
 ...
 SUBROUTINE update_log(str)
   ...
   UPDATE  FROM chart_dist_log cdl
    SET cdl.log_dt_tm = cnvtdatetime(curdate,curtime3), cdl.batch_selection = request->
     batch_selection, cdl.distribution_id = cnvtreal(dist_id),
   ...
   IF (curqual=0)
    INSERT  FROM chart_dist_log cdl
     SET cdl.chart_log_num = nextseq, cdl.log_dt_tm = cnvtdatetime(curdate,curtime3), cdl
      .batch_selection = request->batch_selection,
