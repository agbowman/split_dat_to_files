CREATE PROGRAM bbt_solcap_disp_currsec_aborh:dba
 RECORD facilitylist(
   1 locs[*]
     2 facilitycd = f8
     2 disp_curraborh = i4
 ...
 SUBROUTINE (bbtgetprodtagverifypreference(facility_code=f8(value)) =vc)
   ...
   ENDIF
 END ;Subroutine
 DECLARE disp_curraborh = i4 WITH noconstant(0)
 DECLARE disp_twoaborh = i4 WITH noconstant(0)
 DECLARE testing_facility_cd = f8 WITH protect, noconstant(0.0)
 DECLARE facility_code = f8 WITH protect, noconstant(0.0)
 DECLARE test_facility_cd = f8 WITH protect, noconstant(0.0)
 DECLARE facility_idx = i4 WITH noconstant(0)
 DECLARE facility_index_found = i4 WITH protect, noconstant(0.0)
 DECLARE fac_list_size = i4 WITH protect, noconstant(0)
 SET stat = alterlist(reply->solcap,2)
 SET reply->solcap[1].identifier = "2013.1.00038.3"
 SET reply->solcap[2].identifier = "2013.1.00038.4"
 SET reply->solcap[1].degree_of_use_str = "YES"
 SET reply->solcap[2].degree_of_use_str = "YES"
 FREE SET curraborh_count
 DECLARE curraborh_count = i4 WITH noconstant(0)
 FREE SET twoaborh_count
 DECLARE twoaborh_count = i4 WITH noconstant(0)
 FREE SET cap_notused
 DECLARE cap_notused = i4 WITH noconstant(0)
 SELECT
  ...
  ORDER BY cv.display
  DETAIL
   facility_code = cv.code_value, test_facility_cd = bbtgetflexspectestingfacility(facility_code),
   facility_index_found = locateval(facility_idx,1,size(facilitylist->locs,5),test_facility_cd,
    facilitylist->locs[facility_idx].facilitycd)
   IF (facility_index_found > 0)
    disp_curraborh = facilitylist->locs[facility_index_found].disp_curraborh, disp_twoaborh =
    facilitylist->locs[facility_index_found].disp_twoaborh
   ELSE
    fac_list_size += 1, stat = alterlist(facilitylist->locs,fac_list_size), facilitylist->locs[
    fac_list_size].facilitycd = test_facility_cd
    IF (bbtgetflexspecenableflexexpiration(test_facility_cd)=1)
     facilitylist->locs[fac_list_size].disp_curraborh = bbtgetdisponcurrentaborh(test_facility_cd),
     facilitylist->locs[fac_list_size].disp_twoaborh = bbtgetdisponsecondaborh(test_facility_cd)
    ELSE
     facilitylist->locs[fac_list_size].disp_curraborh = 0, facilitylist->locs[fac_list_size].
     disp_twoaborh = 0
    ENDIF
    disp_curraborh = facilitylist->locs[fac_list_size].disp_curraborh, disp_twoaborh = facilitylist->
    locs[fac_list_size].disp_twoaborh
   ENDIF
   IF (disp_curraborh != 1
    AND disp_twoaborh != 1)
    curraborh_count += 1
    IF (mod(curraborh_count,10)=1)
     stat = alterlist(reply->solcap[1].facility,(curraborh_count+ 9))
    ENDIF
    reply->solcap[1].facility[curraborh_count].display = cv.display, reply->solcap[1].facility[
    curraborh_count].value_str = "NO", twoaborh_count += 1
    IF (mod(twoaborh_count,10)=1)
     stat = alterlist(reply->solcap[2].facility,(twoaborh_count+ 9))
    ENDIF
    reply->solcap[2].facility[twoaborh_count].display = cv.display, reply->solcap[2].facility[
    twoaborh_count].value_str = "NO"
   ENDIF
   IF (disp_twoaborh=1)
    twoaborh_count += 1
    IF (mod(twoaborh_count,10)=1)
    ...
    ENDIF
    reply->solcap[2].facility[twoaborh_count].display = cv.display, reply->solcap[2].facility[
    twoaborh_count].value_str = "YES"
   ELSEIF (disp_curraborh=1)
    curraborh_count += 1
    IF (mod(curraborh_count,10)=1)
     stat = alterlist(reply->solcap[1].facility,(curraborh_count+ 9))
    ENDIF
    reply->solcap[1].facility[curraborh_count].display = cv.display, reply->solcap[1].facility[
    curraborh_count].value_str = "YES"
   ENDIF
  FOOT REPORT
   stat = alterlist(reply->solcap[1].facility,curraborh_count), stat = alterlist(reply->solcap[2].
    facility,twoaborh_count)
  WITH nocounter
 ;end select
END GO
