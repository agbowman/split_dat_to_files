CREATE PROGRAM dm_pcmb_mp_primed_pop:dba
 ...
 DECLARE nencntrmoveind = i2 WITH noconstant(0), protect
 CALL dm_cmb_get_context(0)
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  SET stat = alterlist(dcem_request->qual,1)
  SET dcem_request->qual[1].parent_entity = "PERSON"
  SET dcem_request->qual[1].child_entity = "MP_PRIMED_VIEW_POP"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_PCMB_MP_PRIMED_POP"
  ...
  GO TO exit_sub
 ENDIF
 IF (checkdic("MP_PRIMED_VIEW_POP","T",0)=0)
  GO TO exit_sub
 ENDIF
 SELECT INTO "NL:"
  FROM mp_primed_view_pop mdp
  WHERE (mdp.person_id=request->xxx_combine[icombine].from_xxx_id)
  HEAD mdp.person_id
   v_cust_count1 = (v_cust_count1+ 1)
   IF (mod(v_cust_count1,10)=1)
    stat = alterlist(rreclist->from_rec,(v_cust_count1+ 9))
   ENDIF
   rreclist->from_rec[v_cust_count1].from_id = mdp.mp_primed_view_pop_id, rreclist->from_rec[
   v_cust_count1].active_status_cd = 0.0
  WITH forupdatewait(mdp)
 ;end select
 CALL echo(build2("request->xxx_combine[iCombine]->ENCNTR_ID: ",request->xxx_combine[icombine].
   encntr_id))
 IF ((request->xxx_combine[icombine].encntr_id > 0))
  SET nencntrmoveind = 1
 ENDIF
 SELECT INTO "NL:"
  FROM mp_primed_view_pop mdp
  WHERE (mdp.person_id=request->xxx_combine[icombine].to_xxx_id)
  HEAD mdp.person_id
   v_cust_count2 = (v_cust_count2+ 1)
   IF (mod(v_cust_count2,10)=1)
    stat = alterlist(rreclist->to_rec,(v_cust_count2+ 9))
   ENDIF
   rreclist->to_rec[v_cust_count2].to_id = mdp.mp_primed_view_pop_id, rreclist->to_rec[v_cust_count2]
   .active_status_cd = 0.0
  WITH forupdatewait(mdp)
 ;end select
 IF (((v_cust_count1 > 0) OR (v_cust_count2 > 0)) )
  FOR (v_cust_loopcount1 = 1 TO v_cust_count1)
    IF (del_from(rreclist->from_rec[v_cust_loopcount1].from_id,0,rreclist->from_rec[v_cust_loopcount1
     ].active_ind,rreclist->from_rec[v_cust_loopcount1].active_status_cd,request->xxx_combine[
  ...
  FOR (v_cust_loopcount2 = 1 TO v_cust_count2)
    IF (del_from(rreclist->to_rec[v_cust_loopcount2].to_id,0,rreclist->to_rec[v_cust_loopcount2].
     active_ind,rreclist->to_rec[v_cust_loopcount2].active_status_cd,request->xxx_combine[icombine].
     to_xxx_id,
     1)=0)
     GO TO exit_sub
    ENDIF
  ENDFOR
 ENDIF
 SUBROUTINE del_from(s_df_pk_id,s_df_to_fk_id,s_df_prev_act_ind,s_df_prev_act_status,personid,toind)
   IF (recombining=0)
    DELETE  FROM mp_primed_view_act mda
     WHERE mda.person_id=personid
     WITH nocounter
    ;end delete
    IF (toind=0)
     IF (nencntrmoveind=0)
      DELETE  FROM mp_primed_view_pop mdp
       WHERE mdp.person_id=personid
       WITH nocounter
      ;end delete
      IF (curqual=0)
       SET failed = delete_error
       SET request->error_message = substring(1,132,build("Could not delete 'from' person_id=",
         personid))
       RETURN(0)
      ENDIF
     ENDIF
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "MP_PRIMED_VIEW_POP"
   SET request->xxx_combine_det[icombinedet].attribute_name = "MP_PRIMED_VIEW_POP_ID"
   ...
   IF (recombining=1
    AND ((toind=1) OR (nencntrmoveind=1)) )
    SELECT INTO "nl:"
     FROM request_processing rp
     WHERE rp.request_number=3091116
      AND rp.active_ind=1
      AND rp.target_request_number=3091115
     WITH nocounter
    ;end select
    IF (curqual=0)
     DELETE  FROM mp_primed_view_pop pop
      WHERE pop.person_id=personid
      WITH nocounter
     ;end delete
    ELSE
     CALL reprime(personid)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE reprime(personid)
   DECLARE request_number = i4 WITH protect, constant(3091116)
   RECORD rep_status(
     1 status_data
       2 status = c1
       2 subeventstatus[1]
   ...
   CALL initializerequest(rep_status,request_number)
   SET nsrvstat = uar_srvsetdouble(hreq,"person_id",personid)
