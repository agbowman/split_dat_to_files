CREATE PROGRAM dm2_open_schema_files:dba
 ...
 END ;Subroutine
#end_sch_files_inc
 DECLARE dosf_file_prefix = vc WITH protect, noconstant(" ")
 IF (check_logfile("dm2_open_files",".log","Opening Schema Files")=0)
  GO TO exit_script
 ENDIF
 SET dosf_file_prefix =  $1
 SET dm2_install_schema->schema_prefix = substring(1,4,dosf_file_prefix)
 SET dm2_install_schema->file_prefix = substring(5,(textlen(dosf_file_prefix) - 4),dosf_file_prefix)
 SET dm2_install_schema->schema_loc =  $2
 IF ((validate(dm2_modify_schema_files,- (1))=- (1)))
  DECLARE dm2_modify_schema_files = i2 WITH protect, constant(0)
 ENDIF
 IF ( NOT (dm2_modify_schema_files IN (0, 1)))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Modify option not set correctly."
  SET dm_err->eproc = "Validating modify option."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((validate(dm2_open_tspace_schema_file,- (1))=- (1)))
  DECLARE dm2_open_tspace_schema_file = i2 WITH protect, constant(0)
 ENDIF
 IF ( NOT (dm2_open_tspace_schema_file IN (0, 1)))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Open tspace sch file option not set correctly."
  SET dm_err->eproc = "Validating tspace sch file option."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (dm2_open_tspace_schema_file=1)
 ...
 SET dm2_sch_file->file_prefix = build(dm2_install_schema->schema_prefix,dm2_install_schema->
  file_prefix)
 IF (open_sch_files(dm2_modify_schema_files)=0)
  GO TO exit_script
 ENDIF
#exit_script
 SET dm_err->eproc = "Opening Schema Files Complete"
 CALL final_disp_msg("dm2_open_files")
END GO
