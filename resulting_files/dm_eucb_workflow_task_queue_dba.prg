CREATE PROGRAM dm_eucb_workflow_task_queue:dba
 ...
 DECLARE isencounteruncombinequeueable = i2 WITH protect, noconstant(true)
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_EUCB_WORKFLOW_TASK_QUEUE"
  ...
  GO TO exit_sub
 ENDIF
 CALL processencntruncombtowtp(0)
 SUBROUTINE (processencntruncombtowtp(dummy=i2) =null)
   IF ((request->parent_table="ENCOUNTER"))
    SET wtptaskrequest->smode = "ENCOUNTER_UNCOMBINE"
    SET wtptaskrequest->cmb_qual[1].from_xxx_id = request->xxx_uncombine[ucb_cnt].from_xxx_id
    SET wtptaskrequest->cmb_qual[1].to_xxx_id = request->xxx_uncombine[ucb_cnt].to_xxx_id
   ENDIF
   ...
   SELECT INTO "nl:"
    ...
    WITH nocounter
   ;end select
   IF ((wtptaskrequest->cmb_qual[1].to_xxx_id != 0))
    SELECT INTO "nl:"
     FROM encntr_combine_det ecd
     PLAN (ecd
      WHERE (ecd.entity_id=wtptaskrequest->cmb_qual[1].to_xxx_id)
       AND ecd.entity_name="WORKFLOW_TASK_QUEUE"
       AND ecd.active_ind=true)
    ;end select
    IF (curqual=0)
     SET isencounteruncombinequeueable = false
    ENDIF
   ENDIF
   IF (usewtpframework)
    IF (isencounteruncombinequeueable)
     IF ( NOT (writecombinesrowtowtp(cnvtrectojson(wtptaskrequest),"PFT_UNCOMBINE_CLINICAL_ENCNTR",
      wtptaskrequest->cmb_qual[1].from_xxx_id,request->parent_table,cnvtdatetime(sysdate),
      "")))
      SET ucb_failed = insert_error
      SET error_table = "WORKFLOW_TASK_QUEUE"
      SET request->error_message = "Failed to write uncombine item to workflow_task_queue table"
     ENDIF
    ENDIF
   ENDIF
 END ;Subroutine
