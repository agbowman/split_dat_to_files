CREATE PROGRAM dm_rmc_mover_5299:dba
 ...
 DECLARE cust_value_type = vc
 DECLARE cust_val1 = vc
 DECLARE cust_val1_a = vc
 DECLARE cust_val2 = vc
 DECLARE cust_val3 = vc
 DECLARE cust_val4 = vc
 DECLARE cust_val1_pos = i4
 DECLARE cust_val2_pos = i4
 DECLARE cust_val3_pos = i4
 DECLARE cust_val4_pos = i4
 DECLARE cust_to_val1 = vc
 DECLARE cust_to_val1_a = vc
 DECLARE cust_to_val2 = vc
 DECLARE cust_to_val3 = vc
 DECLARE cust_to_val4 = vc
 ...
 SET cust_val1 = get_value(cust_tab_name,"VAL1","FROM")
 SET cust_val2 = get_value(cust_tab_name,"VAL2","FROM")
 SET cust_val3 = get_value(cust_tab_name,"VAL3","FROM")
 SET cust_val4 = get_value(cust_tab_name,"VAL4","FROM")
 SET cust_val1_pos = get_col_pos(cust_tab_name,"VAL1")
 SET cust_val2_pos = get_col_pos(cust_tab_name,"VAL2")
 SET cust_val3_pos = get_col_pos(cust_tab_name,"VAL3")
 SET cust_val4_pos = get_col_pos(cust_tab_name,"VAL4")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_5299
 ENDIF
 SET cust_value_type = trim(get_value(cust_tab_name,"VALUE_TYPE","FROM"))
 CASE (cust_value_type)
  OF "LIST_FILTER_VO_ID":
   IF (cnvtreal(cust_val1) > 0)
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_val1_pos].translated = 0
    SET cust_to_val1 = select_merge_translate(cust_val1,"CODE_VALUE")
    IF (isnumeric(cust_to_val1))
     CALL put_value(cust_tab_name,"VAL1",cust_to_val1)
    ELSE
     SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_val1))
     CALL evaluate_rpt_missing(cust_rpt_missing)
    ENDIF
   ELSE
    CALL put_value(cust_tab_name,"VAL1",cust_val1)
   ENDIF
  OF "PAIR_1":
   IF (cnvtreal(cust_val1) > 0)
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_val1_pos].translated = 0
   ...
   ELSE
    CALL put_value(cust_tab_name,"VAL1",cust_val1)
   ENDIF
   IF (cnvtreal(cust_val2) > 0)
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_val2_pos].translated = 0
    SET cust_to_val2 = select_merge_translate(cust_val2,"CODE_VALUE")
    IF (isnumeric(cust_to_val2))
     CALL put_value(cust_tab_name,"VAL2",cust_to_val2)
    ELSE
     SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_val2))
     CALL evaluate_rpt_missing(cust_rpt_missing)
    ENDIF
   ELSE
    CALL put_value(cust_tab_name,"VAL2",cust_val2)
   ENDIF
   IF (cnvtreal(cust_val3) > 0)
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_val3_pos].translated = 0
    SET cust_to_val3 = select_merge_translate(cust_val3,"CODE_VALUE")
    IF (isnumeric(cust_to_val3))
     CALL put_value(cust_tab_name,"VAL3",cust_to_val3)
    ELSE
     SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_val3))
     CALL evaluate_rpt_missing(cust_rpt_missing)
    ENDIF
   ELSE
    CALL put_value(cust_tab_name,"VAL3",cust_val3)
   ENDIF
   IF (cnvtreal(cust_val4) > 0)
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_val4_pos].translated = 0
    SET cust_to_val4 = select_merge_translate(cust_val4,"CODE_VALUE")
    IF (isnumeric(cust_to_val4))
     CALL put_value(cust_tab_name,"VAL4",cust_to_val4)
    ELSE
     SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_val4))
     CALL evaluate_rpt_missing(cust_rpt_missing)
    ENDIF
   ELSE
    CALL put_value(cust_tab_name,"VAL4",cust_val4)
   ENDIF
  OF patstring("CRITERIA_COLUMN_IND*",0):
   IF (cnvtupper(substring(1,1,cust_val1))="K")
    SET cust_val1_a = substring(2,size(cust_val1),cust_val1)
    IF (cnvtreal(cust_val1_a) > 0)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_val1_pos].translated = 0
     SET cust_to_val1 = select_merge_translate(cust_val1_a,"CODE_VALUE")
     SET cust_to_val1_a = concat("K",cust_to_val1)
     IF (isnumeric(cust_to_val1))
      CALL put_value(cust_tab_name,"VAL1",cust_to_val1_a)
     ELSE
      SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_val1_a))
      CALL evaluate_rpt_missing(cust_rpt_missing)
     ENDIF
    ELSE
     SET dm_err->emsg = "Unexpected form of VAL1"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "NOMV53"
     GO TO exit_5299
    ENDIF
   ELSE
    SET dm_err->emsg = "Unexpected form of VAL1"
 ...
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_5299
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"VALUE_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (no_query=1)
   ...
   IF (val < 0)
    IF ((val=- (3)))
     SET val = get_seq(cust_tab_name,"VALUE_ID")
     IF (val > 0)
      CALL put_value(cust_tab_name,"VALUE_ID",cnvtstring(val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  VALUE_ID")
     ELSE
      GO TO exit_5299
     ENDIF
    ELSE
   ...
   SET from_cd = get_value(cust_tab_name,"VALUE_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_5299
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(from_cd)
   ENDIF
   SET new_cd = get_value(cust_tab_name,"VALUE_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_5299
   ELSE
 ...
 IF (idcd_check=1)
  ...
  IF (is_translated(cust_tab_name,"VAL1")=1
   AND is_translated(cust_tab_name,"VAL2")=1
   AND is_translated(cust_tab_name,"VAL3")=1
   AND is_translated(cust_tab_name,"VAL4")=1)
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "This row wasn't translated because of a VAL* column"
