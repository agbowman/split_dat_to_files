CREATE PROGRAM bhs_athn_get_message_details
 ...
 SELECT INTO "nl:"
  ...
  HEAD ta.task_id
   ...
   task_status_disp,
   msg_list->msgs[vcnt].subject = subject, msg_list->msgs[vcnt].touser = to_user, msg_list->msgs[vcnt
   ].tasktypedisplay = task_type_disp,
   msg_list->msgs[vcnt].tasktypecd = ta.task_type_cd, msg_list->msgs[vcnt].duedatetime = due_dt_tm,
   msg_list->msgs[vcnt].updatedatetime = update_dt_tm,
   msg_list->msgs[vcnt].updatecount = ta.updt_cnt, msg_list->msgs[vcnt].confidentialindicator =
   cnvtint(ta.confidential_ind), msg_list->msgs[vcnt].deliveryindicator = cnvtint(ta.delivery_ind),
   msg_list->msgs[vcnt].statind = stat_ind, msg_list->msgs[vcnt].copymessageindicator = copy_message,
   msg_list->msgs[vcnt].eventclassmeaning = event_class_mean,
   msg_list->msgs[vcnt].replyallowedindicator = reply_allowed, acnt = 0
  HEAD task_subact_id
 ...
 SELECT INTO value( $1)
  ...
  HEAD REPORT
   ...
   FOR (i = 1 TO size(msg_list->msgs,5))
     ...
     col + 1, v11, row + 1,
     v12 = build("<receiverName>",msg_list->msgs[i].touser,"</receiverName>"), col + 1, v12,
     row + 1, v14 = build("<dueDateTime>",msg_list->msgs[i].duedatetime,"</dueDateTime>"), col + 1,
     v14, row + 1, v15 = build("<statIndicator>",msg_list->msgs[i].statind,"</statIndicator>"),
     col + 1, v15, row + 1,
     v16 = build("<updateDateTime>",msg_list->msgs[i].updatedatetime,"</updateDateTime>"), col + 1,
     v16,
     row + 1, v17 = build("<eventId>",msg_list->msgs[i].eventid,"</eventId>"), col + 1,
     v17, row + 1, v18 = build("<personId>",msg_list->msgs[i].personid,"</personId>"),
     col + 1, v18, row + 1,
     v19 = build("<encounterId>",msg_list->msgs[i].encounterid,"</encounterId>"), col + 1, v19,
     row + 1, v20 = build("<confidentialIndicator>",cnvtint(msg_list->msgs[i].confidentialindicator),
      "</confidentialIndicator>"), col + 1,
     v20, row + 1, v21 = build("<updateCount>",cnvtint(msg_list->msgs[i].updatecount),
      "</updateCount>"),
     col + 1, v21, row + 1,
     v22 = build("<deliveryIndicator>",cnvtint(msg_list->msgs[i].deliveryindicator),
      "</deliveryIndicator>"), col + 1, v22,
     row + 1, v23 = build("<copyMessageIndicator>",msg_list->msgs[i].copymessageindicator,
      "</copyMessageIndicator>"), col + 1,
     v23, row + 1, v24 = build("<eventClassMeaning>",msg_list->msgs[i].eventclassmeaning,
      "</eventClassMeaning>"),
     col + 1, v24, row + 1,
     v241 = build("<replyAllowedIndicator>",msg_list->msgs[i].replyallowedindicator,
      "</replyAllowedIndicator>"), col + 1, v241,
     row + 1, vlt1 = build("<callerName>",msg_list->msgs[i].callername,"</callerName>"), col + 1,
     vlt1, row + 1, vlt2 = build("<callerPhones>",msg_list->msgs[i].callerphones,"</callerPhones>"),
     col + 1, vlt2, row + 1,
     vt1 = build("<taskTypeCD>",cnvtstring(msg_list->msgs[i].tasktypecd),"</taskTypeCD>"), col + 1,
     vt1,
     row + 1, vt2 = build("<taskTypeMeaning>",msg_list->msgs[i].tasktypedisplay,"</taskTypeMeaning>"),
     col + 1,
     vt2, row + 1, vt3 = build("<taskTypeDisplay>",msg_list->msgs[i].tasktypedisplay,
      "</taskTypeDisplay>"),
     col + 1, vt3, row + 1,
