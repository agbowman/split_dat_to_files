CREATE PROGRAM ct_sub_rpt_grp_enroll_site:dba
 PROMPT
 ...
 SUBROUTINE get_enroll_by_site(dummy)
   SELECT
    ...
    FOOT  enrollments_prot_master_id
     ...
     _fdrawheight = footenrollments_prot_master_idsection(rpt_calcheight)
     IF (_fdrawheight > 0)
      IF ((_fenddetail >= (_yoffset+ _fdrawheight)))
       _fdrawheight += footenrollments_prot_master_idsection1(rpt_calcheight)
      ENDIF
     ENDIF
     IF (((_yoffset+ _fdrawheight) > _fenddetail))
      BREAK
     ENDIF
     dummy_val = footenrollments_prot_master_idsection(rpt_render), on_study_total += on_study_cnt,
     off_study_total += off_study_cnt,
     off_tx_total += off_tx_cnt, enroll_total += total_cnt
     FOR (idx = 2 TO prot_pi_cnt)
       tempstr = trim(countlist->prot_pis[idx].pi_name_full), _fdrawheight =
       footenrollments_prot_master_idsection1(rpt_calcheight)
       IF (((_yoffset+ _fdrawheight) > _fenddetail))
        BREAK
       ENDIF
       dummy_val = footenrollments_prot_master_idsection1(rpt_render)
     ENDFOR
    ...
    FOOT PAGE
     ...
     _yoffset = _yhold
    FOOT REPORT
     tempstr = concat(label->total_sites," ",trim(cnvtstring(site_cnt,3))), _fdrawheight =
     footreportsection(rpt_calcheight)
     IF (((_yoffset+ _fdrawheight) > _fenddetail))
      CALL pagebreak(0)
     ENDIF
 ...
 SUBROUTINE (headenrollments_enroll_org_idsection_lineabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH
  protect)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (footenrollments_prot_master_idsection(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = footenrollments_prot_master_idsectionabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (footenrollments_prot_master_idsectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH
  protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_width = 1.500
    SET rptsd->m_height = 0.156
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(tmp_pi,char(0)))
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 2.990)
    SET rptsd->m_width = 1.510
    SET rptsd->m_height = 0.156
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(tmp_init_svc,char(0)))
    SET rptsd->m_flags = 16
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 4.740)
    SET rptsd->m_width = 1.260
    SET rptsd->m_height = 0.156
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__on_study_date)
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 6.125)
    SET rptsd->m_width = 1.260
    SET rptsd->m_height = 0.156
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__off_treat_date)
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 7.531)
    SET rptsd->m_width = 1.219
    SET rptsd->m_height = 0.156
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__off_study_date)
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 8.917)
    SET rptsd->m_width = 1.583
    SET rptsd->m_height = 0.156
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__total_count)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (footenrollments_prot_master_idsection1(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = footenrollments_prot_master_idsection1abs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (footenrollments_prot_master_idsection1abs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH
  protect)
   DECLARE sectionheight = f8 WITH noconstant(0.150000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
 ...
 SUBROUTINE (footenrollments_enroll_org_idsectionabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,
  bcontinue=i2(ref)) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__prot_count)
   ENDIF
   SET rptsd->m_flags = 16
   SET rptsd->m_y = (offsety+ 0.125)
   SET rptsd->m_x = (offsetx+ 1.375)
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__prin_inv_count)
   ENDIF
   SET rptsd->m_flags = 16
   SET rptsd->m_y = (offsety+ 0.125)
   SET rptsd->m_x = (offsetx+ 2.990)
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__init_serv_count)
   ENDIF
   SET rptsd->m_flags = 16
   SET rptsd->m_y = (offsety+ 0.125)
   SET rptsd->m_x = (offsetx+ 4.740)
 ...
 SUBROUTINE (footpagesectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__label_rpt_page)
    SET rptsd->m_flags = 0
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 0.000)
    SET rptsd->m_width = 10.500
 ...
 SUBROUTINE (footreportsectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__label_end_of_rpt)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE initializereport(dummy)
   IF (_bsubreport=0)
    ...
    SET rptreport->m_reportname = "CT_SUB_RPT_GRP_ENROLL_SITE"
