CREATE PROGRAM dm2_sys_hlth_chk_menu:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 IF ((validate(chk_list->chk_cnt,- (1))=- (1))
  AND (validate(chk_list->chk_cnt,- (2))=- (2)))
  FREE RECORD chk_list
  RECORD chk_list(
    1 chk_cnt = i4
    1 qual[*]
      2 program_name = vc
  )
  SET chk_list->chk_cnt = 0
 ENDIF
 DECLARE dshcm_goodopt_ind = i2 WITH protect, noconstant(0)
 DECLARE dshcm_menu_option = i4 WITH protect, noconstant(0)
 DECLARE dshcm_cnt1 = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2_sys_hlth_chk",".log","SYSTEM HEALTH CHECK LOG FILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Begin dm2_sys_hlth_chk"
 CALL disp_msg(" ",dm_err->logfile,0)
#main_menu
 SET dshc->process_option = "REPORT"
 SET width = 132
 SET message = window
 CALL text(2,2,"System Health Check",wide)
 CALL box(3,3,16,130)
 CALL text(5,5,
  "The following options allow you to analyze the configuration of your Cerner Millennium system:")
 CALL text(7,8,"1. Cerner Millennium Configuration")
 CALL text(8,8,"2. Database Configuration (inactive)")
 CALL text(9,8,"3. TCP/IP Configuration (inactive)")
 CALL text(10,8,"4. Operating System Configuration (inactive)")
 SET dshcm_goodopt_ind = 0
 WHILE (dshcm_goodopt_ind=0)
   CALL text(12,5,"Please select an option from the menu above (99 to exit):")
   CALL accept(12,64,"99")
   SET dshcm_menu_option = curaccept
   IF (dshcm_menu_option=1)
    SET dshcm_goodopt_ind = 1
   ELSEIF (dshcm_menu_option IN (2, 3, 4))
    CALL clear(14,4,120)
    CALL text(14,5,
     "That option will be available in a future release.  Please choose another option.")
   ELSEIF (dshcm_menu_option=99)
    SET message = nowindow
    GO TO exit_script
   ELSE
    CALL clear(14,4,120)
    CALL text(14,5,"That was not a valid option.  Please select a valid option.")
   ENDIF
 ENDWHILE
 SET message = nowindow
 IF (dshcm_menu_option=1)
  SET dm_err->eproc = "Gathering Millennium System Health Checks from database."
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT INTO "nl:"
   di.dm_info
   FROM dm_info di
   WHERE di.info_domain="DM2_SYS_HLTH_CHK_MILL"
   ORDER BY di.info_number
   DETAIL
    chk_list->chk_cnt += 1, stat = alterlist(chk_list->qual,chk_list->chk_cnt), chk_list->qual[
    chk_list->chk_cnt].program_name = di.info_name
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF ((dm_err->debug_flag > 1))
   CALL echorecord(chk_list)
  ENDIF
  IF ((chk_list->chk_cnt > 0))
   FOR (dshcm_cnt1 = 1 TO chk_list->chk_cnt)
    EXECUTE value(chk_list->qual[dshcm_cnt1].program_name)
    IF ((dm_err->err_ind=1))
     GO TO exit_script
    ENDIF
   ENDFOR
 ...
 ENDIF
 IF ((chk_list->chk_cnt > 0))
  EXECUTE dm2_sys_hlth_chk_rpt
  IF ((dm_err->err_ind=1))
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Finished displaying System Health Check report."
 ELSE
  SET dm_err->eproc = "No Millennium System Health Checks to execute."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 GO TO main_menu
#exit_script
 CALL final_disp_msg("dm2_sys_hlth_chk")
END GO
