CREATE PROGRAM dm2_aload_prgtpl_tkn
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_prg_tkn",".log","dm2_aload_prgtpl_tkn")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_prgtpl_tkn"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daptkn_cnt = i4 WITH protect, noconstant(0)
 DECLARE daptkn_str = vc WITH protect, noconstant("")
 DECLARE daptkn_req_size = i4 WITH protect, noconstant(0)
 SET daptkn_req_size = size(requestin->list_0,5)
 IF (dac_pop_coldic_rec("DM_ADM_PURGE_TOKEN")=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
 ...
 CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
   " dcv ,(dummyt d with seq = value(daptkn_req_size) )"),0)
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_prgtpl_tkn completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_prgtpl_tkn")
END GO
