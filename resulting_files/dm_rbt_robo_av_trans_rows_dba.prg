CREATE PROGRAM dm_rbt_robo_av_trans_rows:dba
 ...
 SET reply->table_name = "ROBOTICS_AV_TRANSACTION"
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(rat.robotics_av_transaction_id)
    FROM robotics_av_transaction rat
    WHERE rat.robotics_av_transaction_id > 0
    DETAIL
  ...
  ENDIF
  SELECT INTO "nl:"
   seqval = max(rat.robotics_av_transaction_id)
   FROM robotics_av_transaction rat
   DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     rat.rowid
     FROM robotics_av_transaction rat
     WHERE rat.robotics_av_transaction_id BETWEEN curminid AND curmaxid
      AND parser(sbr_getrowidnotexists(
       "rat.updt_dt_tm < cnvtdatetime(curdate - days_to_keep,curtime3)","rat"))
     DETAIL
      rows = (rows+ 1)
      IF (mod(rows,50)=1)
       stat = alterlist(reply->rows,(rows+ 49))
      ENDIF
      reply->rows[rows].row_id = rat.rowid
     WITH maxqual(rat,value(rowsleft))
    ;end select
    SET reply->err_code = error(reply->err_msg,1)
    IF ((reply->err_code > 0))
     SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"COLLECTERROR",
      "Failed collecting rows: %1","s",nullterm(reply->err_msg))
     SET reply->status_data.status = "F"
     GO TO exit_script
    ENDIF
