CREATE PROGRAM bbt_rpt_state_chg_err:dba
 ...
 SET captions->rpt_title = uar_i18ngetmessage(i18nhandle,"rpt_title","PRODUCT STATUS NOT UPDATED")
 SET captions->rpt_page = uar_i18ngetmessage(i18nhandle,"rpt_page","Page:")
 SET captions->rpt_id = uar_i18ngetmessage(i18nhandle,"rpt_id","Report ID:")
 ...
 SET captions->div = uar_i18ngetmessage(i18nhandle,"div","Div")
 SET captions->prsnl = uar_i18ngetmessage(i18nhandle,"prsl","Personnel")
 ...
 SET captions->identifier = uar_i18ngetmessage(i18nhandle,"identifier","Identifier")
 ...
 SELECT INTO "nl:"
  ...
  WHERE bea.admin_dt_tm BETWEEN cnvtdatetime(request->beg_dt_tm) AND cnvtdatetime(request->end_dt_tm)
   ...
   AND (bea.destination_loc_cd=request->facility_cd)) OR ((request->facility_cd=0.0)))
   AND bea.bb_edn_admin_id > 0
   AND bea.edn_complete_ind=0
   AND ((bea.protocol_nbr=5) OR (bea.protocol_nbr=100))
 ...
 SELECT INTO cpm_cfn_info->file_name_logical
  admin_date = cnvtdate(bea.admin_dt_tm), bb_edn_prod_id = bep.bb_edn_product_id, product_number =
  trim(substring(1,20,bep.edn_product_nbr_ident)),
  product_disp = trim(substring(1,25,uar_get_code_display(bep.product_cd))), product_code = trim(
   substring(1,25,bep.product_type_txt)), div = trim(bep.product_sub_nbr_txt),
  prsnl_name = trim(substring(1,16,pr.name_full_formatted)), person_name = trim(substring(1,16,p
    .name_full_formatted)), person_alias = trim(substring(1,20,cnvtalias(bep.alias,bep.alias_pool_cd)
    )),
  person_dob = cnvtdatetime(p.birth_dt_tm), reason = trim(uar_get_definition(bepb.problem_type_cd)),
  ...
  PLAN (bea
   ...
   JOIN (p
   WHERE p.person_id=bep.person_id)
   JOIN (pr
   WHERE pr.person_id=bep.event_prsnl_id)
  ORDER BY admin_date, product_number, bep.bb_edn_product_id
  HEAD REPORT
   row 0
  HEAD PAGE
   row 0,
   ...
   row + 2, col 0, captions->facility,
   col 12, facility_disp, row + 2,
   col 3, captions->product_number, col 24,
   captions->product_type, col 50, captions->div,
   col 56, captions->prsnl, col 77,
   captions->patient, col 98, captions->patient,
   col 119, captions->patient, row + 1,
   col 3, captions->reason, col 56,
   captions->name, col 77, captions->name,
   col 98, captions->identifier, col 119,
   captions->dob, row + 1, col 0,
   line131, row + 1
  HEAD admin_date
   IF (row > 52)
    BREAK
   ENDIF
   col 0, admin_date"@SHORTDATE;;d", row + 2
  HEAD product_number
   row + 0
  ...
  FOOT PAGE
   ...
   col 11, cpm_cfn_info->file_name, col 113,
   captions->rpt_page, col 120, curpage";l",
   row + 1
  FOOT REPORT
   row 59,
