CREATE PROGRAM dm_rc_f_daily_census_smry_rows:dba
 ...
 SET reply->table_name = "RC_F_DAILY_CENSUS_SMRY"
 ...
 DECLARE activitydate = i4 WITH noconstant(0)
 ...
 IF (v_days_to_keep < 370)
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"DAYSTOKEEP",
   ...
   "i",v_days_to_keep)
 ELSE
  SET activitydate = (curdate - v_days_to_keep)
  SELECT INTO "nl:"
   FROM rc_f_daily_census_smry r
   WHERE r.activity_dt_nbr <= activitydate
    AND ((r.rc_f_daily_census_smry_id+ 0) != 0)
