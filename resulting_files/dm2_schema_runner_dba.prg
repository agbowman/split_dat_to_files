CREATE PROGRAM dm2_schema_runner:dba
 DECLARE dsr_cleanup_timestamp = f8 WITH public
 DECLARE dsr_wait_timestamp = f8 WITH protect, noconstant(0.0)
 DECLARE dsr_updt_cnt = i4 WITH public
 DECLARE dsr_updt_fnd_cnt = i4 WITH public
 DECLARE dsr_updt_stolen_cnt = i4 WITH public
 DECLARE dsr_updt_constolen_cnt = i4 WITH public
 DECLARE dsreidx = i4 WITH public
 DECLARE dsr_requery_threshold_cnt = i4 WITH public, constant(40)
 DECLARE dsr_cmd_retry_cnt = i4 WITH public, noconstant(0)
 DECLARE dsr_cmd_successful = i2 WITH public, noconstant(0)
 DECLARE dsr_cmd_error = i2 WITH public, noconstant(0)
 DECLARE dsr_cmd_rscbsy_error = i2 WITH public, noconstant(0)
 DECLARE dsr_cmd_hrdprs_error = i2 WITH public, noconstant(0)
 DECLARE dsr_cmd_firstexec = i2 WITH public, noconstant(0)
 DECLARE dsr_cmd_rscbsy_retry_cnt = i2 WITH public, noconstant(0)
 DECLARE dsr_cmd_liblock_retry_cnt = i2 WITH public, noconstant(0)
 DECLARE dsr_cmd_hrdprs_retry_cnt = i2 WITH public, noconstant(0)
 DECLARE dsr_run_id = f8 WITH public, noconstant(0.0)
 DECLARE dsr_op_id = f8 WITH public, noconstant(0.0)
 DECLARE dsr_op_type = vc WITH public, noconstant(" ")
 DECLARE dsr_command = vc WITH public, noconstant(" ")
 DECLARE dsr_oe_command = vc WITH public, noconstant(" ")
 DECLARE dsr_begin_dt_tm = f8 WITH public, noconstant(0.0)
 DECLARE dsr_dm_info_exists = i2 WITH public, noconstant(0)
 DECLARE dsr_currdbhandle = vc WITH public, noconstant(" ")
 DECLARE dsr_called_in_background = i2 WITH protect, noconstant(0)
 DECLARE dsr_wait = i2 WITH protect, noconstant(0)
 DECLARE dsr_status = i2 WITH protect, noconstant(0)
 DECLARE dsr_serial_op_types = vc WITH protect, noconstant("('DM2NOTSET')")
 DECLARE dsr_exec_context = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dsr_recompile_op_types = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dsr_compile = i2 WITH protect, noconstant(0)
 DECLARE dsr_table_name = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dsr_obj_name = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dsr_luts_context_ind = i2 WITH protect, noconstant(0)
 DECLARE dsr_event_activation = i2 WITH protect, noconstant(0)
 DECLARE dsr_cmd_liblock_error = i2 WITH protect, noconstant(0)
 DECLARE dsr_available_retry = i4 WITH protect, noconstant(0)
 DECLARE dsr_priority = i4 WITH protect, noconstant(0)
 DECLARE dsr_serial_op_types_out = vc WITH protect, noconstant("")
 DECLARE dsr_loadreadytorun(null) = i4
 DECLARE dsr_check_whether_to_wait(dcwtw_wait=i2(ref)) = i4
 DECLARE dsr_whereclause(dw_info_domain=vc,dw_clause=vc(ref)) = i2
 DECLARE dsr_whereclause_parallel(dwp_serial_op_types_in=vc,dwp_serial_op_types_out=vc(ref)) = i2
 FREE RECORD dsr_execlist
 RECORD dsr_execlist(
   1 exec_cnt = i4
   1 exec[*]
     2 op_id = f8
 ...
 SUBROUTINE ddf_context_lock_maint(dclm_mode,dclm_process,dclm_process_code,dclm_owner,
  dclm_table_name,dclm_object_id,dclm_lock_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_schema_runner",".log","dm2_schema_runner")=0)
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_schema_runner"
 CALL disp_msg("",dm_err->logfile,0)
 SET dsr_currdbhandle = currdbhandle
 IF (dm2_get_rdbms_version(null)=0)
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Determining the execution context"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF ((drr_runner_misc->mode="REPLICATE*"))
  SET dsr_exec_context = "CLIN COPY"
  IF ((drr_runner_misc->mode="REPLICATE_BACKGROUND"))
   SET dsr_called_in_background = 1
  ENDIF
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("DSR_EXEC_CONTEXT=",dsr_exec_context))
 ENDIF
 SET dm_err->eproc = "Determining if dm_info exist."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF (dm2_table_and_ccldef_exists("DM_INFO",dsr_dm_info_exists)=0)
  GO TO end_program
 ENDIF
 IF (dsr_dm_info_exists=1)
  IF ((drr_runner_misc->mode="GENERIC_BACKGROUND"))
   IF (dn_get_notify_settings(null)=0)
    GO TO end_program
   ENDIF
   IF ((dnotify->status=1))
  ...
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE di.info_domain="FIRE_LUTS_TRG_SCHEMA_CONTEXT"
    AND di.info_name="GLOBAL"
   DETAIL
    IF (di.info_char="YES")
     dsr_luts_context_ind = 1
    ELSE
     dsr_luts_context_ind = 0
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO end_program
  ENDIF
  IF (dsr_exec_context="DM2NOTSET")
   IF (dsr_whereclause("DM2_SERIAL_EXEC_OP_TYPE",dsr_serial_op_types)=0)
    GO TO end_program
   ENDIF
   IF (dsr_whereclause_parallel(dsr_serial_op_types,dsr_serial_op_types_out)=0)
    GO TO end_program
   ELSE
    IF (dsr_serial_op_types_out != dsr_serial_op_types)
     CALL echo("input op_types change due to parallel op_type consideration.")
     SET dsr_serial_op_types = dsr_serial_op_types_out
    ENDIF
   ENDIF
   IF ((dm2_db_options->recompile_after_alter_tbl="Y"))
    IF (dsr_whereclause("DM2_RECOMPILE_OP_TYPE",dsr_recompile_op_types)=0)
     GO TO end_program
    ENDIF
    IF ((dm_err->debug_flag > 0))
     CALL echo(build("recompile_op_types =",dsr_recompile_op_types))
    ENDIF
   ENDIF
  ENDIF
  ...
  IF ((dm_err->err_ind > 0))
   GO TO end_program
  ENDIF
  IF (drr_insert_runner_row("DM2_SCHEMA_RUNNER",dsr_currdbhandle,evaluate(dm2_install_schema->
    process_option,"NONE","PARALLEL_RUNNER","MAIN_RUNNER"),1,0.0)=0)
   GO TO end_program
  ENDIF
  IF (dsr_exec_context="DM2NOTSET")
   SET dm_err->eproc = "Determining if schema runner initiated in the background..."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain="DM2_BACKGROUND_RUNNER"
     AND di.info_name=dsr_currdbhandle
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF ((((dm2_rdbms_version->level1 > 11)) OR ((((dm2_rdbms_version->level1=11)
    ...
    AND (dm2_rdbms_version->level3=0)
    AND (dm2_rdbms_version->level4 >= 4))) )) )) )
    SET dm_err->eproc = "Determining if event activation can be skipped..."
    SELECT INTO "nl:"
     dsc_inline_cnt = count(*)
     FROM dm_info di
     WHERE di.info_domain="DM2_SCHEMA_RUNNER_OPTION"
      AND di.info_name="DM2_LIBLOCK_EVENT_ACTIVATION"
     DETAIL
      dsr_event_activation = dsc_inline_cnt
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO end_program
    ENDIF
    IF (dsr_event_activation=1)
     IF (dm2_push_cmd(
      "rdb alter session set events '24439608 trace name context forever, level 1' go",1)=0)
      GO TO end_program
     ENDIF
    ENDIF
   ELSE
    SET dsr_event_activation = 0
   ENDIF
  ENDIF
 ENDIF
 IF (dsr_exec_context="CLIN COPY"
  AND (drr_runner_misc->runner_identifier != "RR_RUNNER_DO*"))
  SET dm_err->eproc = "Determining CLIN COPY serial op types..."
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
   FROM dm_info@ref_data_link d
   WHERE d.info_domain=cnvtupper("DM2_RR_SERIAL_EXEC_OP_TYPE")
   HEAD REPORT
    dsr_serial_op_types = ""
   DETAIL
    IF (dsr_serial_op_types="")
     dsr_serial_op_types = concat("('",trim(d.info_name),"'")
    ELSE
     dsr_serial_op_types = concat(dsr_serial_op_types,",'",trim(d.info_name),"'")
    ENDIF
   FOOT REPORT
    dsr_serial_op_types = concat(dsr_serial_op_types,")")
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF (curqual=0)
   SET dsr_serial_op_types = "('CREATE TABLE AS SELECT')"
  ELSEIF (curqual=1
   AND dsr_serial_op_types="*NO_SERIAL_OP_TYPES*")
   SET dsr_serial_op_types = "('DM2NOTSET')"
  ENDIF
 ENDIF
 IF (dsr_luts_context_ind=1)
 ...
 ENDIF
 SET dsr_cleanup_timestamp = cnvtdatetime(curdate,curtime3)
 SET dsr_wait_timestamp = cnvtdatetime(curdate,curtime3)
 SET dsr_updt_cnt = 0
 SET dsr_updt_fnd_cnt = 0
 SET dsr_updt_stolen_cnt = 0
 SET dsr_updt_constolen_cnt = 0
 SET dsr_wait = 1
 WHILE (dsr_wait=1)
  IF (dsr_loadreadytorun(1) != 1)
   GO TO end_program
  ENDIF
  IF ((dsr_execlist->exec_cnt=0)
   AND dsr_called_in_background=1)
   IF (dsr_check_whether_to_wait(dsr_wait)=0)
    GO TO end_program
   ENDIF
   IF (dsr_wait=1)
    IF (datetimediff(cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),cnvtdatetimeutc(cnvtdatetime(
       dsr_cleanup_timestamp)),4) > 60)
     SET dm_err->eproc = concat("Wait time expired, stopping runner. Wait time:",build(datetimediff(
        cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),cnvtdatetimeutc(cnvtdatetime(
          dsr_cleanup_timestamp)),4)))
     CALL disp_msg("",dm_err->logfile,0)
     SET dsr_wait = 0
    ENDIF
   ENDIF
  ...
  ENDIF
 ENDWHILE
 WHILE ((dsr_execlist->exec_cnt > 0))
   FOR (dsreidx = 1 TO dsr_execlist->exec_cnt)
     SET dsr_cmd_successful = 0
     SET dsr_cmd_error = 0
     SET dsr_cmd_retry_cnt = 0
     SET dsr_cmd_rscbsy_error = 0
     SET dsr_cmd_rscbsy_retry_cnt = 0
     SET dsr_cmd_liblock_error = 0
     SET dsr_cmd_liblock_retry_cnt = 0
     SET dsr_cmd_hrdprs_error = 0
     SET dsr_cmd_hrdprs_retry_cnt = 0
     SET dsr_cmd_firstexec = 1
     WHILE (((dsr_cmd_firstexec=1) OR (dsr_cmd_successful=0
      AND dsr_cmd_error=1
      AND dsr_cmd_retry_cnt < cnvtint(dm2_db_options->resource_busy_maxretry))) )
       IF (dsr_cmd_retry_cnt > 1)
        CALL pause(3)
       ENDIF
       SET dsr_cmd_retry_cnt = (dsr_cmd_retry_cnt+ 1)
       EXECUTE dm2_schema_runner_c
       IF ((dm_err->err_ind=1))
        GO TO end_program
       ENDIF
       SET dsr_cmd_firstexec = 0
     ENDWHILE
     IF (dsr_updt_constolen_cnt > dsr_requery_threshold_cnt)
      SET dsreidx = dsr_execlist->exec_cnt
      SET dsr_updt_constolen_cnt = 0
     ENDIF
   ENDFOR
   SET dsr_wait = 1
 ...
 ENDWHILE
 IF ((dm_err->debug_flag > 0))
  CALL echo(build("Total updt_cnt:",dsr_updt_cnt))
  CALL echo(build("Total updt_fnd_cnt:",dsr_updt_fnd_cnt))
  CALL echo(build("Total updt_stolen_cnt:",dsr_updt_stolen_cnt))
 ENDIF
 IF ((dm2_process_event_rs->ui_allowed_ind=1))
 ...
 ENDIF
 GO TO end_program
 SUBROUTINE dsr_loadreadytorun(null)
   DECLARE dl_resumable_priority_inlist = vc WITH protect, noconstant(
    "ddol.priority in (10,20,60,320,340)")
   DECLARE dl_resumable_timeout = i2 WITH protect, noconstant(28800)
   DECLARE dl_resumable_session_name = vc WITH protect, noconstant(currdbhandle)
   DECLARE dsrlrt_cnt = i4 WITH protect
   SET dsr_compile = 0
   SELECT
    IF (dsr_serial_op_types != "('DM2NOTSET')"
     AND dsr_exec_context="CLIN COPY")
     FROM dm2_ddl_ops_log ddol1
     WHERE ddol1.ready_to_run_ind=1
      AND ddol1.status = null
      AND ddol1.op_type != "*(REMOTE)*"
      AND ((parser(concat("ddol1.op_type in ",dsr_serial_op_types))
      AND  NOT ( EXISTS (
     (SELECT
      "x"
      FROM dm2_ddl_ops_log x
      WHERE parser(concat("x.op_type in ",dsr_serial_op_types))
       AND x.status="RUNNING")))) OR (parser(concat("ddol1.op_type not in ",dsr_serial_op_types))))
     ORDER BY ddol1.est_duration DESC
    ELSEIF (dsr_serial_op_types != "('DM2NOTSET')"
     AND dsr_exec_context != "CLIN COPY")
     FROM dm2_ddl_ops_log ddol1
     WHERE ddol1.ready_to_run_ind=1
      AND ddol1.status = null
      AND ddol1.op_type != "EXECUTE DM2_UPDATE_PKG_TABLEDEF*"
      AND  NOT (ddol1.table_name IN (
     (SELECT
      ddol2.table_name
    ...
    ELSEIF (dsr_serial_op_types="('DM2NOTSET')"
     AND dsr_exec_context="CLIN COPY")
     FROM dm2_ddl_ops_log ddol1
     WHERE ddol1.status = null
      AND ddol1.ready_to_run_ind=1
      AND ddol1.op_type != "*(REMOTE)*"
     ORDER BY ddol1.est_duration DESC
    ELSE
     FROM dm2_ddl_ops_log ddol1
    ...
    INTO "nl:"
    HEAD REPORT
     stat = alterlist(dsr_execlist->exec,10), dsrlrt_cnt = 0
    DETAIL
     dsrlrt_cnt = (dsrlrt_cnt+ 1)
     IF (mod(dsrlrt_cnt,10)=1
      AND dsrlrt_cnt != 1)
      stat = alterlist(dsr_execlist->exec,(dsrlrt_cnt+ 9))
     ENDIF
     dsr_execlist->exec[dsrlrt_cnt].op_id = ddol1.op_id
     IF (dsr_compile=0)
      IF (findstring(trim(ddol1.op_type),dsr_recompile_op_types,1,0) > 0)
       dsr_compile = 1
      ENDIF
     ENDIF
    FOOT REPORT
     dsr_execlist->exec_cnt = dsrlrt_cnt, stat = alterlist(dsr_execlist->exec,dsr_execlist->exec_cnt)
    WITH nocounter
   ;end select
   IF (check_error("Load DDL Operations that are ready to run")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (curqual=0)
    SET dsr_execlist->exec_cnt = 0
    SET stat = alterlist(dsr_execlist->exec,dsr_execlist->exec_cnt)
   ENDIF
   IF ((dm_err->debug_flag > 3))
    CALL echorecord(dsr_execlist)
   ENDIF
   IF ((dm2_rdbms_version->level1 >= 9)
    AND (validate(dm2_no_resumable,- (1))=- (1))
    AND (validate(dm2_no_resumable,- (2))=- (2)))
    IF ((((dsr_execlist->exec_cnt=0)) OR (dsr_exec_context != "CLIN COPY")) )
     IF (dm2_push_cmd("rdb alter session disable resumable go",1)=0)
      RETURN(0)
     ENDIF
    ELSE
     IF (dsr_dm_info_exists=1)
      SELECT INTO "nl:"
       FROM dm_info d
       WHERE d.info_domain="DM2_DBA_RESUMABLE"
        AND d.info_name="ENABLED PRIORITIES"
       DETAIL
        dl_resumable_priority_inlist = concat("ddol.priority in (",d.info_char,")")
       WITH nocounter
      ;end select
      IF (check_error("Load RESUMABLE priorities")=1)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(0)
      ENDIF
     ENDIF
     SELECT INTO "nl:"
      FROM dm2_ddl_ops_log ddol
      WHERE ddol.ready_to_run_ind=1
       AND ddol.status = null
       AND parser(dl_resumable_priority_inlist)
      WITH nocounter
     ;end select
     IF (check_error("Check if RESUMABLE priorities are ready to run.")=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ENDIF
     IF (curqual > 0)
      IF (dm2_push_cmd(concat("rdb alter session enable resumable timeout ",build(
         dl_resumable_timeout)," name 'dm2_schema_runner",dl_resumable_session_name,"' go"),1)=0)
       RETURN(0)
      ENDIF
     ELSE
      IF (dm2_push_cmd("rdb alter session disable resumable go",1)=0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsr_check_whether_to_wait(dcwtw_wait)
   IF (datetimediff(cnvtdatetime(curdate,curtime),cnvtdatetime(dsr_wait_timestamp),4) > 5)
    SET dsr_wait_timestamp = cnvtdatetime(curdate,curtime3)
    IF ((dm_err->debug_flag > 1))
     CALL echo(concat("Current date and time = ",format(cnvtdatetime(curdate,curtime),
        "mm/dd/yyyy hh:mm:ss;;d")))
     CALL echo(concat("Last run date and time = ",format(dsr_wait_timestamp,"mm/dd/yyyy hh:mm:ss;;d")
       ))
     CALL echo(concat("Number of minutes elapsed = ",cnvtstring(datetimediff(cnvtdatetime(curdate,
          curtime),cnvtdatetime(dsr_wait_timestamp),4),4,3)))
    ENDIF
    IF (dm2_cleanup_stranded_appl(null)=0)
     RETURN(0)
    ENDIF
    IF (dsr_exec_context="CLIN COPY")
     SET dsr_status = 0
     IF (drr_rr_check_runner_status(drr_runner_misc->runner_identifier,currdbhandle,dsr_status)=0)
      RETURN(0)
     ENDIF
     IF (dsr_status=0)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "Schema background runner has been marked to stop."
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ENDIF
    ELSEIF (dsr_dm_info_exists=1)
     IF (drr_check_runner_status("DM2_SCHEMA_RUNNER",dsr_currdbhandle,dsr_status)=0)
      RETURN(0)
     ENDIF
     IF (dsr_status=0)
      SET dm_err->err_ind = 1
    ...
    ENDIF
   ENDIF
   SET dm_err->eproc = "Looking for later RUNNING priorities..."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log ddol
   ...
   ELSE
    IF (curqual > 0)
     ...
     CALL echo(concat(format(cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),"mm/dd/yyyy hh:mm:ss;;d"
        ),"  Waiting on other background runners to complete priority before exiting script..."))
     CALL echo("**************************************************")
     CALL pause(5)
    ELSE
     SET dcwtw_wait = 0
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsr_whereclause(dw_info_domain,dw_clause)
   DECLARE dw_work = vc WITH protect, noconstant("")
   SET dm_err->eproc = concat("Build where clause from DM_INFO for OP_TYPEs found on ",dw_info_domain
    )
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain=cnvtupper(dw_info_domain)
    HEAD REPORT
     dw_work = ""
    DETAIL
     IF (dw_work="")
      dw_work = concat("('",trim(d.info_name),"'")
     ELSE
      dw_work = concat(dw_work,",'",trim(d.info_name),"'")
     ENDIF
    FOOT REPORT
     dw_work = concat(dw_work,")")
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual=0)
    SET dw_work = "('DM2NOTSET')"
   ENDIF
   SET dw_clause = dw_work
   IF ((dm_err->debug_flag > 0))
    CALL echo(dw_clause)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsr_whereclause_parallel(dwp_serial_op_types_in,dwp_serial_op_types_out)
   DECLARE dwp_cnt = i2 WITH protect, noconstant(0)
   DECLARE dwp_idx = i4 WITH protect, noconstant(1)
   DECLARE dwp_pos = i4 WITH protect, noconstant(0)
   DECLARE dwp_input_op_type = vc WITH protect, noconstant("")
   DECLARE dwp_str = vc WITH protect, noconstant("")
   SET dwp_serial_op_types_out = dwp_serial_op_types_in
   FREE RECORD dwp_op_type_rec
   RECORD dwp_op_type_rec(
     1 cnt = i4
     1 list[*]
       2 op_type = vc
   )
   SET dwp_op_type_rec->cnt = 0
   IF ((dm2_db_options->degree_of_parallel != "NOT_SET")
    AND cnvtint(dm2_db_options->degree_of_parallel) > 1)
    SET dm_err->eproc = "The query will qualify on the DDL using parallelism."
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg("",dm_err->logfile,0)
    ENDIF
    SELECT INTO "nl:"
     ddol.op_type, ddol.operation
     FROM dm2_ddl_ops_log ddol
     WHERE ddol.ready_to_run_ind=1
      AND ddol.status = null
      AND ddol.op_type IN ("REPLACE INDEX", "CREATE INDEX - FOR TWO PHASED CHANGE (TEMP)",
     "CREATE INDEX")
     ORDER BY ddol.op_type
     HEAD ddol.op_type
      dwp_optype_fnd_ind = 0
     DETAIL
      IF (dwp_optype_fnd_ind=0)
       dwp_idx = 0, dwp_idx = findstring("PARALLEL (DEGREE",cnvtupper(ddol.operation))
       IF (dwp_idx > 0)
        dwp_optype_fnd_ind = 1, dwp_op_type_rec->cnt = (dwp_op_type_rec->cnt+ 1), stat = alterlist(
         dwp_op_type_rec->list,dwp_op_type_rec->cnt),
        dwp_op_type_rec->list[dwp_op_type_rec->cnt].op_type = ddol.op_type
       ENDIF
      ENDIF
     WITH nocounter
    ;end select
    ...
    ENDIF
   ENDIF
   IF ((dwp_op_type_rec->cnt > 0))
    IF (dwp_serial_op_types_in != "('DM2NOTSET')")
     SET dwp_input_op_type = dwp_serial_op_types_in
     SET dwp_input_op_type = replace(dwp_input_op_type,")"," ",1)
     SET dwp_input_op_type = replace(dwp_input_op_type,"("," ",1)
     SET dwp_input_op_type = replace(dwp_input_op_type,"'"," ",0)
     WHILE (dwp_input_op_type != "")
       SET dwp_pos = findstring(",",dwp_input_op_type)
       IF (dwp_pos > 0)
        SET dwp_str = substring(1,(dwp_pos - 1),dwp_input_op_type)
       ELSE
        SET dwp_str = dwp_input_op_type
       ENDIF
       SET dwp_idx = 0
       SET dwp_idx = locateval(dwp_idx,1,dwp_op_type_rec->cnt,trim(dwp_str,3),dwp_op_type_rec->list[
        dwp_idx].op_type)
       IF (dwp_idx=0)
        SET dwp_op_type_rec->cnt = (dwp_op_type_rec->cnt+ 1)
        SET stat = alterlist(dwp_op_type_rec->list,dwp_op_type_rec->cnt)
        SET dwp_op_type_rec->list[dwp_op_type_rec->cnt].op_type = dwp_str
       ENDIF
       SET dwp_input_op_type = replace(dwp_input_op_type,dwp_str,"",1)
       SET dwp_input_op_type = replace(dwp_input_op_type,",","",1)
     ENDWHILE
    ENDIF
    FOR (dwp_cnt = 1 TO dwp_op_type_rec->cnt)
      IF (dwp_cnt=1)
       SET dwp_serial_op_types_out = concat("'",trim(dwp_op_type_rec->list[dwp_cnt].op_type,3),"'")
      ELSE
       SET dwp_serial_op_types_out = concat(dwp_serial_op_types_out,",'",trim(dwp_op_type_rec->list[
         dwp_cnt].op_type,3),"'")
      ENDIF
    ENDFOR
    SET dwp_serial_op_types_out = concat("(",dwp_serial_op_types_out,")")
   ENDIF
   RETURN(1)
 END ;Subroutine
#end_program
 IF (dsr_dm_info_exists=1)
  DELETE  FROM dm_info di
   WHERE di.info_domain="DM2_SCHEMA_RUNNER"
    AND di.info_name=dsr_currdbhandle
   WITH nocounter
  ;end delete
  IF ((dm_err->err_ind=0))
   IF (check_error("Removing DM_INFO runner row for DM2_SCHEMA_RUNNER")=1)
    ROLLBACK
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ELSE
    COMMIT
  ...
  ENDIF
  IF (dsr_event_activation=1)
   CALL dm2_push_cmd("rdb alter session set events '24439608 off' go",1)
  ENDIF
 ENDIF
 IF ((dm2_rdbms_version->level1 >= 9)
  AND (validate(dm2_no_resumable,- (1))=- (1))
  ...
  CALL parser("rdb alter session disable resumable go",1)
 ENDIF
 FREE RECORD dsr_execlist
 CALL dctx_set_context("FIRE_LUTS_TRG","YES")
 SET dm_err->eproc = "Ending dm2_schema_runner"
 CALL final_disp_msg(" ")
END GO
