CREATE PROGRAM dm_cs_cpp_undo_rows:dba
 ...
 END ;Subroutine
 DECLARE dm_cs_cpp_undo_rows_version = vc WITH private, noconstant("224651.FT.001")
 ...
 SET reply->table_name = "CS_CPP_UNDO"
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    minval = min(ccu.cs_cpp_undo_id)
    FROM cs_cpp_undo ccu
    WHERE ccu.cs_cpp_undo_id > 0
    DETAIL
  ...
  WHILE (v_cur_min_id <= v_max_id
   AND v_rows_left > 0)
    SELECT INTO "nl:"
     FROM cs_cpp_undo ccu
     WHERE parser(sbr_getrowidnotexists("ccu.updt_dt_tm < cnvtdatetime(v_days_back)","ccu"))
      AND ccu.cs_cpp_undo_id BETWEEN v_cur_min_id AND v_cur_max_id
     DETAIL
      v_rows = (v_rows+ 1)
      IF (mod(v_rows,100)=1)
       stat = alterlist(reply->rows,(v_rows+ 99))
      ENDIF
      reply->rows[v_rows].row_id = ccu.rowid
     WITH nocounter, maxqual(ccu,value(v_rows_left))
    ;end select
