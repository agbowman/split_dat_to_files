CREATE PROGRAM br_add_ado:dba
 ...
 SET readme_data->message = "Readme Failed: Starting <br_add_ado.prg> script"
 ...
 SELECT INTO "nl:"
  FROM br_step bs
  PLAN (bs
   WHERE bs.step_mean="ADVORD")
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed checking existence of Advisor Orders row: ",errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  INSERT  FROM br_step bs
   SET bs.step_mean = "ADVORD", bs.step_disp = "Advisor Orders", bs.step_type = "IMPMAINT",
 ...
 SELECT INTO "nl:"
  ...
  WHERE bcir.br_client_id=1
   ...
   AND bcir.item_mean="COREA"
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to check for existence of SOLUTION/COREA row: ",errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  INSERT  FROM br_client_item_reltn bcir
   SET bcir.br_client_item_reltn_id = seq(bedrock_seq,nextval), bcir.br_client_id = 1, bcir.item_type
     = "SOLUTION",
    bcir.item_mean = "COREA", bcir.item_display = "Advisor", bcir.solution_seq = 265,
 ...
 SELECT INTO "nl:"
  ...
  WHERE bcir.br_client_id=1
   ...
   AND bcir.item_mean="ADVORD"
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to verify existence of STEP/ADVORD row: ",errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  INSERT  FROM br_client_item_reltn bcir
   SET bcir.br_client_item_reltn_id = seq(bedrock_seq,nextval), bcir.br_client_id = 1, bcir.item_type
     = "STEP",
    bcir.item_mean = "ADVORD", bcir.item_display = "Advisor Orders", bcir.step_cat_mean = "CORE",
 ...
 SELECT INTO "nl:"
  ...
  WHERE bcss.br_client_id=1
   AND bcss.solution_mean="COREA"
   AND bcss.step_mean="ADVORD"
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to check for existence of COREA/ADVORD row: ",errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  INSERT  FROM br_client_sol_step bcss
   SET bcss.br_client_id = 1, bcss.solution_mean = "COREA", bcss.step_mean = "ADVORD",
    bcss.sequence = 360, bcss.updt_cnt = 0, bcss.updt_dt_tm = cnvtdatetime(curdate,curtime3),
 ...
 SET readme_data->message = "Readme Succeeded: <br_add_ado.prg> script"
#exit_script
