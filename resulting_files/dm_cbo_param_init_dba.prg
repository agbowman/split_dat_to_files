CREATE PROGRAM dm_cbo_param_init:dba
 ...
 SUBROUTINE dcr_get_opt_mode_by_name(opt_name)
   RETURN(evaluate(opt_name,"RULE",1,"CHOOSE",2,
    ...
    "ALL_ROWS",8,0))
 END ;Subroutine
 IF (check_logfile("dm_cbo_param_init",".log","dm_cbo_param_init LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET readme_data->status = "F"
 SET readme_data->message = "**** FAILED - Starting script dm_cbo_param_init ****"
 IF (currdb != "ORACLE")
 ...
 IF (dcr_get_optimizer_settings(null)=0)
  GO TO exit_script
 ENDIF
 EXECUTE dm_set_session_param value(evaluate(dcr_optvalues->implementer_opt_mode,"RULE","RBO","CBO"))
 IF ((dm_err->err_ind=1))
  SET readme_data->message = "Failed - Error loading csv rows into dm_info."
  GO TO exit_script
 ENDIF
 EXECUTE dm_create_optimizer_objects
 IF ((dm_err->err_ind=1))
  SET readme_data->message = "Failed - Error creating optimizer objects."
  GO TO exit_script
 ENDIF
 EXECUTE dm_set_system_parameters
 IF ((dm_err->err_ind=1))
  SET readme_data->message = "Failed - Error setting system parameters."
  GO TO exit_script
 ENDIF
#exit_script
 IF ((dm_err->err_ind=1))
  SET readme_data->status = "F"
  SET readme_data->message = build(readme_data->message,":",dm_err->emsg)
 ELSE
  SET readme_data->status = "S"
  SET readme_data->message = "Success - New CBO parameters have been loaded and applied."
 ENDIF
 ...
 ELSE
  ...
  CALL echo("##################################################")
 ENDIF
 SET dm_err->eproc = "Ending dm_cbo_param_init."
 CALL final_disp_msg("dm_cbo_param_init")
END GO
