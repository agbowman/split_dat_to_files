CREATE PROGRAM afc_rpt_charges:dba
 PAINT
 SET width = 140
 SET modify = system
 RECORD request(
 ...
 SET request->to_dt_tm = curdate
 SET request->to_dt_tm = cnvtdatetime(concat(format(curdate,"DD-MMM-YYYY;;D")," 23:59:59.99"))
 SET end_date = concat(format(curdate,"DD-MMM-YYYY;;D")," 23:59:59.59")
 SET all = "Y"
 SET pending = "N"
 SET suspend1 = "N"
 SET suspend2 = "N"
...
#main
 ...
 SELECT INTO "nl:"
  ...
  WHERE c.code_set=14002
   ...
   AND c.active_ind=1
  DETAIL
   count2 += 1, cpt_nums->cpt_occ[count2].cpt_num = c.code_value
  WITH nocounter
 ;end select
 SET cpt_numbers = count2
#menu
 ...
 CASE (curaccept)
  ...
  OF 6:
   IF (failed=false)
    ...
    CALL clear(1,1)
    CALL box(3,1,23,100)
    CALL text(13,33,"Now Processing...")
    CALL get_charges(0)
   ENDIF
...
#main_exit
 ...
 SUBROUTINE get_charges(l_dummy)
   ...
   SELECT INTO "nl:"
    ...
    p.name_full_formatted, cm.charge_item_id, cm.field1_id,
    cm.field2_id, cm.field2, e.encntr_type_cd,
    e.organization_id, cv_etc.display, mrn = cnvtstring(trim(pa.alias)),
    oapr_eapc.alias_pool_cd, fn = cnvtstring(trim(ea.alias)), c.order_id
    FROM charge c,
     person p,
     charge_mod cm,
     encounter e,
    ...
    PLAN (c
     WHERE c.active_ind=1
      AND c.service_dt_tm >= cnvtdatetime(request->from_dt_tm)
      AND c.service_dt_tm <= cnvtdatetime(end_date)
      AND c.process_flg IN (pf->p[1].flg, pf->p[2].flg, pf->p[3].flg, pf->p[4].flg, pf->p[5].flg,
     pf->p[6].flg, pf->p[7].flg, pf->p[8].flg, pf->p[9].flg, pf->p[10].flg)
      AND c.charge_type_cd IN (ct->c[1].flg, ct->c[2].flg, ct->c[3].flg, ct->c[4].flg)
      AND c.active_ind=1)
     JOIN (p
    ...
    DETAIL
     ...
     rpt->data[count2].charge_item_id = c.charge_item_id, rpt->data[count2].dt_tm = c.service_dt_tm,
     rpt->data[count2].encntr_type = format(cv_etc.display,"##"),
     ...
     ENDIF
     rpt->data[count2].desc = format(c.charge_description,"#####################"), rpt->data[count2]
     .qty = format(cnvtstring(c.item_quantity),"######"), rpt->data[count2].ext_price = format(c
      .item_extended_price,"#####.##")
     IF (c.process_flg=0)
     ...
     IF (cm.field2_id != null
      AND cm.field2_id > 0)
      FOR (count1 = 1 TO cpt_numbers)
        IF (cm.field1_id != null
         AND cm.field1_id > 0)
         IF ((cm.field1_id=cpt_nums->cpt_occ[count1].cpt_num))
          rpt->data[count2].cpt_num = format(cm.field2,"########"), count1 = cpt_numbers
         ENDIF
        ENDIF
      ENDFOR
     ENDIF
   ...
   ;end select
   SELECT INTO "nl:"
    aor.accession, aor.order_id
    FROM (dummyt d1  WITH seq = value(count2)),
     accession_order_r aor
    PLAN (d1)
     JOIN (aor
     WHERE (aor.order_id=rpt->data[d1.seq].order_id))
    DETAIL
     rpt->data[d1.seq].accession = uar_fmt_accession(aor.accession,size(aor.accession,1))
    WITH nocounter
   ;end select
   SELECT INTO "nl:"
    cn.drawn_dt_tm
   ...
   SELECT INTO value(filename)
    ...
    HEAD REPORT
     team_name = "Health Systems Management - Account For Care", report_name =
     "Charge Reconciliation Report", line140 = fillstring(132,"-"),
     firsttime = 1, billcodetab = 1
    HEAD PAGE
     ...
     ELSE
      col 18, " FIN #"
     ENDIF
     col 32, "Accession #", col 54,
     " Date     Time", col 71, "  Procedure     ",
     col 91, "CDM #", col 100,
     "CPT-4", col 107, "Qty",
     col 118, "Charge  ", row + 1,
     col 01, "-----------", col 15,
     "--", col 18, "-------------",
     col 32, "---------------------", col 54,
     "--------- -----", col 71, "-------------------",
    ...
    DETAIL
     ...
     ELSE
      ...
      ENDIF
      col 32, rpt->data[d1.seq].accession, col 71,
      rpt->data[d1.seq].desc"###################", col 113, rpt->data[d1.seq].qty,
      col 118, rpt->data[d1.seq].ext_price, col 128,
      rpt->data[d1.seq].status
...
#accept_dates_02
 ...
 CASE (curscroll)
  OF 0:
   ...
   SET end_date = concat(curaccept," 23:59:59.59")
  OF 2:
   CALL text(5,29,format(request->to_dt_tm,"DD-MMM-YYYY HH:MM;3;d"))
...
#display_processes
 ...
 CALL text(10,4,"07 Skipped Charges  : ")
 CALL text(11,4,"08 Combined Charges : ")
 CALL text(12,4,"09 Absorbed Charges : ")
 CALL text(13,4,"10 Offset Charges   : ")
 CALL text(14,4,"11 Posted Charges   : ")
...
#accept_processes_07
 ...
 CASE (curscroll)
  OF 0:
   SET skipped = curaccept
   CALL text(10,29,skipped)
  OF 2:
   CALL text(10,29,skipped)
   GO TO accept_processes_06
  OF 3:
...
#accept_processes_10
 ...
 ENDCASE
#accept_processes_11
 CALL accept(14,29,"p(1);CDUS")
 CASE (curscroll)
  OF 0:
   SET charged = curaccept
   CALL text(14,29,charged)
  OF 2:
   CALL text(14,29,charged)
   GO TO accept_processes_10
  OF 3:
