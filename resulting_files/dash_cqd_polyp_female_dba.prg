CREATE PROGRAM dash_cqd_polyp_female:dba
 PROMPT
 ...
 SET terms->qual_cnt = 1
 SET stat = alterlist(terms->qual,terms->qual_cnt)
 SET terms->qual[1].term_disp = "Polyp"
 SET terms->qual[1].term_id = tidpolyp
 ...
 SELECT INTO "nl:"
  ...
  PLAN (st
   WHERE expand(idx,1,procedures->qual_cnt,st.scd_story_id,procedures->qual[idx].scd_story_id,
    cs57female,procedures->qual[idx].sex_cd)
  ...
  DETAIL
   ...
   procedures->qual[pos].status = terms->qual[1].term_disp, json_data->num_polyp = (json_data->
   num_polyp+ 1)
  FOOT REPORT
   json_data->pct_polyp = cnvtreal(((json_data->num_polyp/ cnvtreal(procedures->female_qual_cnt)) *
    100))
  WITH nocounter, expand = 1
 ;end select
#exit_script
 IF ((procedures->female_qual_cnt > 0))
  SET chart_data->results = build2('[[["',trim(cnvtstring(json_data->pct_polyp,5,1)),'%",',trim(
    cnvtstring(json_data->pct_polyp,5,1)),'],[" ",',
   trim(cnvtstring((100 - json_data->pct_polyp),5,1)),"]]]")
 ENDIF
