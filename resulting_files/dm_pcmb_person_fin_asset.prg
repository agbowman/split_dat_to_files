CREATE PROGRAM dm_pcmb_person_fin_asset
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "PERSON_FIN_ASSET"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_PCMB_PERSON_FIN_ASSET"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count_from].from_id = frm.person_fin_asset_id, rreclist->from_rec[
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   IF (tu.active_ind=1
    ...
    has_active_rows_ind = 1
   ENDIF
   rreclist->to_rec[v_cust_count_to].to_id = tu.person_fin_asset_id, rreclist->to_rec[v_cust_count_to
 ...
 SUBROUTINE (del_to(s_df_pk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) =i4)
   UPDATE  FROM person_fin_asset tu
    SET tu.active_ind = false, tu.active_status_cd = combinedaway, tu.active_status_dt_tm =
     ...
     cnvtdatetime(trans_dt_tm)
    WHERE tu.person_fin_asset_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "PERSON_FIN_ASSET"
   ...
   IF (add_hist(s_df_pk_id)=0)
    ...
    SET request->error_message = substring(1,132,build(
      "Could not insert history for person_fin_asset with pk=",s_df_pk_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (del_from(s_df_pk_id=f8,s_df_to_fk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) =
  i4)
   UPDATE  FROM person_fin_asset frm
    SET frm.active_ind = false, frm.active_status_cd = combinedaway, frm.active_status_dt_tm =
     ...
     cnvtdatetime(trans_dt_tm),
     frm.parent_entity_id = s_df_to_fk_id, frm.parent_entity_name = "PERSON"
    WHERE frm.person_fin_asset_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
 ...
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i4)
   UPDATE  FROM person_fin_asset frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     frm.parent_entity_name = "PERSON"
    WHERE frm.person_fin_asset_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
 ...
 SUBROUTINE (add_hist(s_at_to_parent_pk_id=f8) =i4)
   ...
   SET dm_cmb_cust_cols2->add_col_val[1].col_name = "PERSON_FIN_ASSET_HIST_ID"
   ...
   SET dm_cmb_cust_cols2->where_col_val[1].col_name = "PERSON_FIN_ASSET_ID"
   SET dm_cmb_cust_cols2->where_col_val[1].col_value = build(s_at_to_parent_pk_id)
   IF (size(dm_cmb_cust_cols2->col,5)=0)
    SET dm_cmb_cust_cols2->tbl_name = "PERSON_FIN_ASSET_HIST"
    SET dm_cmb_cust_cols2->sub_select_from_tbl = "PERSON_FIN_ASSET"
 ...
 SUBROUTINE initialize_hist_id(null)
   IF (v_hist_init_ind=0)
    ...
    SET dcipht_request->transaction_reason_txt = "PERSON_CMB_PERSON_FIN_ASSET"
