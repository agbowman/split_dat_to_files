CREATE PROGRAM dm2_db2sqlplan:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE num_scripts = i4 WITH protect, noconstant(0)
 DECLARE script_cnt = i4 WITH protect, noconstant(0)
 ...
 IF (( $2="\*"))
  SELECT
   IF (( $3=2))
    ORDER BY ds.num_executions DESC, ds.rows_read DESC, ds.stmt_sorts
   ELSEIF (( $3=3))
    ORDER BY r_ratio DESC, ds.rows_read DESC, ds.num_executions DESC
   ELSEIF (( $3=4))
    ORDER BY ds.total_exec_time DESC, ds.rows_read DESC, ds.num_executions DESC
   ELSEIF (( $3=5))
    ORDER BY r_score DESC, ds.rows_read DESC, ds.num_executions DESC
   ELSE
   ENDIF
   INTO "nl:"
   r_ratio = (ds.rows_read/ ds.num_executions), r_score = ((ds.rows_read+ (ds.num_executions * 200))
   + (ds.rows_written * 200))
   FROM dm2_sqlarea ds
   WHERE ds.rows_read > 0
    AND ds.num_executions > 0
   ORDER BY ds.rows_read DESC, ds.num_executions DESC, ds.stmt_sorts
   HEAD REPORT
    sql_text->text_cnt = 0, script_cnt = 0
   DETAIL
    IF (cnvtupper(substring(1,6,ds.stmt_text)) IN ("SELECT", "INSERT", "DELETE", "UPDATE", "VALUES"))
     script_cnt = (script_cnt+ 1)
     IF (script_cnt <= num_scripts)
      IF (mod(script_cnt,50)=1)
       stat = alterlist(sql_text->qual,(script_cnt+ 49))
      ENDIF
      sql_text->qual[script_cnt].rows_read = ds.rows_read, sql_text->qual[script_cnt].num_executions
       = ds.num_executions, sql_text->qual[script_cnt].row_ratio = r_ratio,
      sql_text->qual[script_cnt].stmt_sorts = ds.stmt_sorts, sql_text->qual[script_cnt].
      total_exec_time = ds.total_exec_time, sql_text->qual[script_cnt].stmt_text = ds.stmt_text,
      sql_text->qual[script_cnt].score = r_score, sql_text->text_cnt = script_cnt
     ENDIF
    ENDIF
   FOOT REPORT
    stat = alterlist(sql_text->qual,sql_text->text_cnt)
   WITH maxcol = 32000
  ;end select
 ELSE
  IF (substring(size( $2),1, $2)="\*")
   SET script_name = substring(1,(size( $2) - 1), $2)
  ELSE
   SET script_name =  $2
  ENDIF
  SELECT
   ...
   DETAIL
    IF (cnvtupper(substring(1,6,ds.stmt_text)) IN ("SELECT", "INSERT", "DELETE", "UPDATE", "VALUES"))
     IF (findstring(trim(script_name,3),cnvtupper(ds.stmt_text)) > 0)
      script_cnt = (script_cnt+ 1)
      IF (script_cnt <= num_scripts)
       IF (mod(script_cnt,50)=1)
        stat = alterlist(sql_text->qual,(script_cnt+ 49))
  ...
  ;end select
 ENDIF
 SET dm_err->eproc = "gathering DM2_SQLAREA data"
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
#exit_plan
