CREATE PROGRAM bbt_rpt_locked_prods:dba
 ...
 SET captions->rpt_locked_unit = uar_i18ngetmessage(i18nhandle,"rpt_locked_unit",
  "L O C K E D   U N I T S   R E P O R T")
 ...
 SET captions->lock_dt_tm = uar_i18ngetmessage(i18nhandle,"lock_dt_tm","Lock Dt/Tm")
 ...
 SET captions->report_id = uar_i18ngetmessage(i18nhandle,"report_id","Report ID: BBT_RPT_LOCK_PRD")
 ...
 SELECT INTO cpm_cfn_info->file_name_logical
  ...
  prod_type = uar_get_code_display(pr.product_cd), pr.locked_ind, pr.updt_dt_tm,
  pr.updt_applctx, pr.updt_id, pr.serial_number_txt,
  bp.supplier_prefix, pnl.username
  FROM product pr,
   blood_product bp,
   prsnl pnl
  PLAN (pr
   WHERE pr.locked_ind=1
   ...
   JOIN (pnl
   WHERE pnl.person_id=pr.updt_id)
  ORDER BY pr.updt_dt_tm, pr.product_nbr, pr.product_id
  HEAD REPORT
   beg_dt_tm = cnvtdatetime(request->beg_dt_tm), end_dt_tm = cnvtdatetime(request->end_dt_tm), line1
    = fillstring(31,"-"),
   line2 = fillstring(45,"-"), line3 = fillstring(30,"-"), line6 = fillstring(125,"-"),
   new_page = "Y"
  HEAD PAGE
   ...
   beg_dt_tm"@DATETIMECONDENSED;;d", col 69, captions->end_date,
   col 82, end_dt_tm"@DATETIMECONDENSED;;d", row + 2,
   col 33, captions->product_nbr, row + 1,
   col 33, captions->product_type, row + 1,
   CALL center(captions->lock_dt_tm,1,11), col 33, captions->serial_number,
   CALL center(captions->tech,79,109), row + 1, col 1,
   line1, col 33, line2,
   col 79, line3, row + 1
  HEAD pr.product_id
   product_display = concat(trim(bp.supplier_prefix),trim(pr.product_nbr)," ",trim(pr.product_sub_nbr
     )), serial_number = pr.serial_number_txt
  DETAIL
   IF (new_page="N")
    row + 2
    IF (row > 54)
    ...
    ENDIF
   ENDIF
   col 1, pr.updt_dt_tm"@DATECONDENSED;;d", col 33,
   product_display, col 79, pnl.username,
   row + 1
   IF (row > 56)
    BREAK, new_page = "Y"
   ELSE
    new_page = "N"
   ENDIF
   col 1, pr.updt_dt_tm"@TIMENOSECONDS;;M", col 33,
   prod_type
   IF (serial_number != null)
    row + 1, col 33, serial_number
   ENDIF
  FOOT PAGE
   row 57, col 0, line6,
