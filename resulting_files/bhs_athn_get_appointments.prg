CREATE PROGRAM bhs_athn_get_appointments
 ...
 FREE RECORD req_get_appt_person
 RECORD req_get_appt_person(
   1 call_echo_ind = i2
   1 qual[*]
     2 person_id = f8
     2 resource_cd = f8
 ...
 FREE RECORD rep_get_appt_person
 RECORD rep_get_appt_person(
   1 qual_cnt = i4
   1 qual[*]
     2 person_id = f8
     2 resource_cd = f8
 ...
 SET stat = alterlist(req_get_appt_person->qual,1)
 SET req_get_appt_person->qual[1].person_id =  $2
 SET req_get_appt_person->qual[1].resource_cd = 0.0
 SET req_get_appt_person->qual[1].beg_dt_tm = cnvtdatetime( $3)
 SET req_get_appt_person->qual[1].end_dt_tm = cnvtdatetime( $4)
 EXECUTE sch_get_appt_person  WITH replace("REQUEST","REQ_GET_APPT_PERSON"), replace("REPLY",
  "REP_GET_APPT_PERSON")
 CALL echorecord(rep_get_appt_person)
 IF ((rep_get_appt_person->status_data.status="S")
  AND size(rep_get_appt_person->qual,5) > 0
  AND (rep_get_appt_person->qual[1].qual_cnt > 0))
  SET idx = 1
  WHILE ((idx <= rep_get_appt_person->qual[1].qual_cnt)
   AND valid_appt_found_ind=0)
    SET pos = locateval(jdx,1,size(sch_states->list,5),rep_get_appt_person->qual[1].appointment[idx].
 ...
 IF (valid_appt_found_ind=1)
  SET loccnt = 0
  SET stat = alterlist(locations->list,size(rep_get_appt_person->qual[1].appointment,5))
  SELECT INTO "NL:"
   FROM location_group lg,
    location_group lg2,
    location l
   PLAN (lg
    WHERE expand(idx,1,rep_get_appt_person->qual[1].qual_cnt,lg.child_loc_cd,rep_get_appt_person->
   ...
   HEAD lg.child_loc_cd
    locidx = locateval(idx,1,rep_get_appt_person->qual[1].qual_cnt,lg.child_loc_cd,
     rep_get_appt_person->qual[1].appointment[idx].location_cd)
 ...
 ENDIF
 IF (valid_appt_found_ind=1)
  SET stat = alterlist(encounters->list,size(rep_get_appt_person->qual[1].appointment,5))
  FOR (idx = 1 TO size(rep_get_appt_person->qual[1].appointment,5))
   SET pos = locateval(encidx,1,enccnt,rep_get_appt_person->qual[1].appointment[idx].patient[1].
    encntr_id,encounters->list[encidx].encntr_id)
   IF (pos=0)
    SET enccnt += 1
    SET encounters->list[enccnt].encntr_id = rep_get_appt_person->qual[1].appointment[idx].patient[1]
    .encntr_id
    SET encounters->list[enccnt].person_id = rep_get_appt_person->qual[1].appointment[idx].patient[1]
    .person_id
   ENDIF
  ENDFOR
 ...
 ENDIF
 IF (valid_appt_found_ind=1)
  SET stat = alterlist(resources->list,size(rep_get_appt_person->qual[1].appointment,5))
  FOR (idx = 1 TO size(rep_get_appt_person->qual[1].appointment,5))
   SET pos = locateval(residx,1,rescnt,rep_get_appt_person->qual[1].appointment[idx].
    primary_resource_cd,resources->list[residx].scheduled_resource_cd)
   IF (pos=0)
    SET rescnt += 1
    SET resources->list[rescnt].scheduled_resource_cd = rep_get_appt_person->qual[1].appointment[idx]
    .primary_resource_cd
 ...
 ENDIF
 IF (valid_appt_found_ind=1)
  SET stat = alterlist(patients->list,size(rep_get_appt_person->qual[1].appointment,5))
  FOR (idx = 1 TO size(rep_get_appt_person->qual[1].appointment,5))
   SET pos = locateval(patidx,1,patcnt,rep_get_appt_person->qual[1].appointment[idx].patient[1].
    person_id,patients->list[patidx].person_id)
   IF (pos=0)
    SET patcnt += 1
    SET patients->list[patcnt].person_id = rep_get_appt_person->qual[1].appointment[idx].patient[1].
    person_id
   ENDIF
  ENDFOR
 ...
 IF (valid_appt_found_ind=1)
  ...
  SET stat = alterlist(apptsort->list,size(rep_get_appt_person->qual[1].appointment,5))
  FOR (idx = 1 TO size(rep_get_appt_person->qual[1].appointment,5))
   SET pos = locateval(locidx,1,sortcnt,rep_get_appt_person->qual[1].appointment[idx].sch_event_id,
    apptsort->list[locidx].sch_event_id)
   IF (pos=0)
    SET sortcnt += 1
    SET apptsort->list[sortcnt].sch_event_id = rep_get_appt_person->qual[1].appointment[idx].
    sch_event_id
    SET apptsort->list[sortcnt].sch_appt_id = rep_get_appt_person->qual[1].appointment[idx].
    sch_appt_id
    SET apptsort->list[sortcnt].schedule_seq = rep_get_appt_person->qual[1].appointment[idx].
    schedule_seq
   ELSEIF ((rep_get_appt_person->qual[1].appointment[idx].schedule_seq > apptsort->list[pos].
   schedule_seq))
    SET apptsort->list[pos].sch_appt_id = rep_get_appt_person->qual[1].appointment[idx].sch_appt_id
    SET apptsort->list[pos].schedule_seq = rep_get_appt_person->qual[1].appointment[idx].schedule_seq
   ENDIF
  ENDFOR
...
#exit_script
 IF (size(trim(moutputdevice,3)) > 0)
  ...
  DECLARE v181 = vc WITH protect, noconstant("")
  ...
  IF (valid_appt_found_ind=1)
   SELECT INTO value(moutputdevice)
    FROM (dummyt d  WITH seq = value(rep_get_appt_person->qual[1].qual_cnt))
    PLAN (d
     WHERE d.seq > 0)
    HEAD REPORT
     ...
     row + 1, v1 = build("<PersonId>",cnvtint( $2),"</PersonId>"), col + 1,
     v1, row + 1, col + 1,
     "<AppointmentList>", row + 1
    DETAIL
     state_cd_pos = locateval(jdx,1,size(sch_states->list,5),rep_get_appt_person->qual[1].
      appointment[d.seq].sch_state_cd,sch_states->list[jdx].state_cd), apptsort_pos = locateval(jdx,1,
      size(apptsort->list,5),rep_get_appt_person->qual[1].appointment[d.seq].sch_appt_id,apptsort->
      list[jdx].sch_appt_id), sch_evnt_id = rep_get_appt_person->qual[1].appointment[d.seq].
     sch_event_id
     IF (state_cd_pos > 0
      AND apptsort_pos > 0
      AND sch_evnt_id > 0)
      col + 1, "<Appointment>", row + 1,
      v2 = build("<ScheduleEventId>",cnvtint(rep_get_appt_person->qual[1].appointment[d.seq].
        sch_event_id),"</ScheduleEventId>"), col + 1, v2,
      row + 1, v3 = build("<ApptTypeCd>",cnvtint(rep_get_appt_person->qual[1].appointment[d.seq].
        appt_type_cd),"</ApptTypeCd>"), col + 1,
      v3, row + 1, v4 = build("<ApptTypeDesc>",trim(replace(replace(replace(replace(replace(
             rep_get_appt_person->qual[1].appointment[d.seq].appt_type_desc,"&","&amp;",0),"<","&lt;",
            0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),"</ApptTypeDesc>"),
      col + 1, v4, row + 1,
      v5 = build("<ApptSynonymCd>",cnvtint(rep_get_appt_person->qual[1].appointment[d.seq].
        appt_synonym_cd),"</ApptSynonymCd>"), col + 1, v5,
      row + 1, v6 = build("<ApptSynonymFree>",trim(replace(replace(replace(replace(replace(
             rep_get_appt_person->qual[1].appointment[d.seq].appt_synonym_free,"&","&amp;",0),"<",
            "&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),"</ApptSynonymFree>"), col + 1,
      v6, row + 1, v7 = build("<SchStateCd>",cnvtint(rep_get_appt_person->qual[1].appointment[d.seq].
        sch_state_cd),"</SchStateCd>"),
      col + 1, v7, row + 1,
      v8 = build("<SchStateMeaning>",trim(replace(replace(replace(replace(replace(rep_get_appt_person
             ->qual[1].appointment[d.seq].state_meaning,"&","&amp;",0),"<","&lt;",0),">","&gt;",0),
          "'","&apos;",0),'"',"&quot;",0),3),"</SchStateMeaning>"), col + 1, v8,
      row + 1, v9 = build("<PatientId>",cnvtint(rep_get_appt_person->qual[1].appointment[d.seq].
        patient[1].person_id),"</PatientId>"), col + 1,
      v9, row + 1, v10 = build("<EncounterId>",cnvtint(rep_get_appt_person->qual[1].appointment[d.seq
        ].patient[1].encntr_id),"</EncounterId>"),
      col + 1, v10, row + 1,
      encidx = locateval(idx,1,enccnt,rep_get_appt_person->qual[1].appointment[d.seq].patient[1].
      ...
      ELSE
       fin = ""
      ENDIF
      patidx = locateval(idx,1,patcnt,rep_get_appt_person->qual[1].appointment[d.seq].patient[1].
      ...
      v15, row + 1, v16 = build("<PatientMrn>",trim(replace(replace(replace(replace(replace(mrn,"&",
       ...
       "</PatientMrn>"),
      col + 1, v16, row + 1,
      v17 = build("<BegDtTm>",format(rep_get_appt_person->qual[1].appointment[d.seq].beg_dt_tm,
        "MM/DD/YYYY HH:MM;;D"),"</BegDtTm>"), col + 1, v17,
      row + 1, v18 = build("<EndDtTm>",format(rep_get_appt_person->qual[1].appointment[d.seq].
        end_dt_tm,"MM/DD/YYYY HH:MM;;D"),"</EndDtTm>"), col + 1,
      v18, row + 1, v181 = build("<PrsnlId>",cnvtint( $2),"</PrsnlId>"),
      col + 1, v181, row + 1,
      v19 = build("<LocationCd>",cnvtint(rep_get_appt_person->qual[1].appointment[d.seq].location_cd),
       "</LocationCd>"), col + 1, v19,
      row + 1, v20 = build("<LocationDisp>",trim(replace(replace(replace(replace(replace(
             rep_get_appt_person->qual[1].appointment[d.seq].location_freetext,"&","&amp;",0),"<",
            "&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),"</LocationDisp>"), col + 1,
      v20, row + 1, locidx = locateval(idx,1,loccnt,rep_get_appt_person->qual[1].appointment[d.seq].
       location_cd,locations->list[idx].location_cd)
      IF (locidx > 0)
      ...
      v22, row + 1, residx = locateval(idx,1,rescnt,rep_get_appt_person->qual[1].appointment[d.seq].
      ...
      row + 1
      IF (textlen(trim(rep_get_appt_person->qual[1].appointment[d.seq].appt_reason_free,3)) > 0)
       v33 = build("<EncounterReason><VisitReason>",trim(replace(replace(replace(replace(replace(
              rep_get_appt_person->qual[1].appointment[d.seq].appt_reason_free,"&","&amp;",0),"<",
             "&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),
