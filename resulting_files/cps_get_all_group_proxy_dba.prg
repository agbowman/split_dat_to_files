CREATE PROGRAM cps_get_all_group_proxy:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   ...
   WHERE (request->type_list[d.seq].proxy_type_cd=p.proxy_type_cd)
    ...
    AND p.proxy_person_id < 1)
   JOIN (p1
   WHERE p.person_id=p1.person_id)
   JOIN (g
   WHERE p.group_proxy_id=g.prsnl_group_id)
  ...
  DETAIL
   ...
   reply->qual[count1].group_proxy_id = p.group_proxy_id, reply->qual[count1].active_ind = p
   .active_ind, reply->qual[count1].beg_effective_dt_tm = p.beg_effective_dt_tm,
   reply->qual[count1].end_effective_dt_tm = p.end_effective_dt_tm, reply->qual[count1].proxy_type_cd
    = p.proxy_type_cd, reply->qual[count1].group_proxy_type_cd = g.prsnl_group_type_cd
  WITH nocounter
 ;end select
 IF (curqual < 1)
  SET ierrcode = error(serrmsg,1)
