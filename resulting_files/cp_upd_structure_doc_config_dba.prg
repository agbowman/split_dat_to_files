CREATE PROGRAM cp_upd_structure_doc_config:dba
 PROMPT
  ...
  "Version Flag" = 0,
  "Version Text" = ""
  WITH outdev, cpcomponentid, instance_ident,
  versionflag, versiontext
 FREE RECORD scriptreply
 RECORD scriptreply(
   1 layout_long_text_id = f8
   1 document_event_long_text_id = f8
   1 document_decorations_long_text_id = f8
   1 behaviors
     2 newcount = i4
     2 modifycount = i4
     2 deletecount = i4
 ...
 FREE RECORD documentation_data
 RECORD documentation_data(
   1 layout_long_text_id = vc
   1 layout_json = vc
   1 events_long_text_id = vc
   1 events_json = vc
   1 behaviors_json = vc
   1 decorations_long_text_id = vc
   1 decorations_json = vc
 )
 FREE RECORD behaviors_rec
 RECORD behaviors_rec(
   1 cnt = i4
   1 qual[*]
     2 description = vc
 ...
 FREE RECORD behaviors_to_add
 RECORD behaviors_to_add(
   1 cnt = i4
   1 qual[*]
     2 cp_node_id = f8
     2 description = vc
 ...
 FREE RECORD behaviors_to_delete
 RECORD behaviors_to_delete(
   1 cnt = i4
   1 qual[*]
     2 value = f8
     2 reaction_entity_id = f8
 ...
 FREE RECORD behaviors_to_update
 RECORD behaviors_to_update(
   1 cnt = i4
   1 qual[*]
     2 cp_node_behavior_id = f8
     2 cp_node_id = f8
 ...
 SUBROUTINE (setauditinitreplystatusbyprogramreplystatus(reply=vc(ref)) =null WITH protect)
  ...
  ENDIF
 END ;Subroutine
 DECLARE savelayoutdata(null) = null WITH protect
 DECLARE savedocumentevents(null) = null WITH protect
 DECLARE savedocumentdecorations(null) = null WITH protect
 DECLARE getlongtextseqid(null) = f8 WITH protect
 DECLARE updatedefaultdoccontentrows(null) = null WITH protect
 IF (validate(request->blob_in))
  IF ((request->blob_in > " "))
   CALL logblobindata("CP_UPD_STRUCTURE_DOC_CONFIG",request->blob_in, $OUTDEV)
   IF (validate(debug_ind,0)=1)
    CALL echo(request->blob_in)
   ENDIF
   SET jrec = cnvtjsontorec(request->blob_in)
   IF (jrec != 1)
    SET scriptreply->status_data.status = "F"
    SET scriptreply->status_data.subeventstatus.operationname = "Failed to parse blob in data"
    SET scriptreply->status_data.subeventstatus.operationstatus = "F"
    GO TO exit_script
   ENDIF
   IF (validate(debug_ind,0)=1)
    CALL echorecord(documentation_data)
   ENDIF
   FREE RECORD request
  ENDIF
 ENDIF
 DECLARE behaviorsjson = vc WITH protect, constant(safeescape(trim(documentation_data->behaviors_json,
    3)))
 DECLARE decorationsjson = vc WITH protect, constant(safeescape(trim(documentation_data->
    decorations_json,3)))
 DECLARE layoutjson = vc WITH protect, constant(safeescape(trim(documentation_data->layout_json,3)))
 DECLARE documenteventjson = vc WITH protect, constant(safeescape(trim(documentation_data->
    events_json,3)))
 DECLARE cp_component_id = f8 WITH protect, constant( $CPCOMPONENTID)
 DECLARE instance_identifier = vc WITH protect, constant(trim( $INSTANCE_IDENT))
 DECLARE layout_long_text_id = f8 WITH protect, noconstant(cnvtreal(documentation_data->
   layout_long_text_id))
 DECLARE document_event_long_text_id = f8 WITH protect, noconstant(cnvtreal(documentation_data->
   events_long_text_id))
 DECLARE document_decorations_long_text_id = f8 WITH protect, noconstant(cnvtreal(documentation_data
   ->decorations_long_text_id))
 DECLARE latestversionnumber = i4 WITH protect, noconstant(0)
 SUBROUTINE getlongtextseqid(null)
   DECLARE newid = f8 WITH protect, noconstant(0.0)
   SELECT INTO "nl:"
   ...
   RETURN(newid)
 END ;Subroutine
 SUBROUTINE (safeescape(originalstring=vc) =vc WITH protect)
   RETURN(replace(originalstring,'\"','"'))
 END ;Subroutine
 SUBROUTINE (updatebehaviors(behaviors_rec=vc(ref)) =null WITH protect)
   CALL log_message("In updateBehaviors()",log_level_debug)
   DECLARE b_cntr = i4 WITH protect, noconstant(0)
   DECLARE requestjson = vc WITH protect, noconstant("")
   DECLARE nodebhvrlist = vc WITH protect, noconstant("")
   IF (validate(debug_ind,0)=1)
    CALL echorecord(behaviors_rec)
   ENDIF
   FOR (b_cntr = 1 TO behaviors_rec->cnt)
     IF ((behaviors_rec->qual[b_cntr].update_ind=1))
      CASE (behaviors_rec->qual[b_cntr].update_action)
       OF "NEW":
        SET behaviors_to_add->cnt += 1
        SET stat = alterlist(behaviors_to_add->qual,behaviors_to_add->cnt)
        SET behaviors_to_add->qual[behaviors_to_add->cnt].cp_node_id = behaviors_rec->qual[b_cntr].
        cp_node_id
        SET behaviors_to_add->qual[behaviors_to_add->cnt].reaction_entity_id = behaviors_rec->qual[
        b_cntr].reaction_entity_id
        SET behaviors_to_add->qual[behaviors_to_add->cnt].reaction_entity_name = behaviors_rec->qual[
        b_cntr].reaction_entity_name
        SET behaviors_to_add->qual[behaviors_to_add->cnt].reaction_type_mean = behaviors_rec->qual[
        b_cntr].reaction_type_mean
        SET behaviors_to_add->qual[behaviors_to_add->cnt].response_ident = behaviors_rec->qual[b_cntr
        ].response_ident
        SET behaviors_to_add->qual[behaviors_to_add->cnt].description = behaviors_rec->qual[b_cntr].
        description
       OF "MODIFY":
        IF ((behaviors_rec->qual[b_cntr].cp_node_behavior_id > 0))
         SET behaviors_to_update->cnt += 1
         SET stat = alterlist(behaviors_to_update->qual,behaviors_to_update->cnt)
         SET behaviors_to_update->qual[behaviors_to_update->cnt].cp_node_behavior_id = behaviors_rec
         ->qual[b_cntr].cp_node_behavior_id
         SET behaviors_to_update->qual[behaviors_to_update->cnt].cp_node_id = behaviors_rec->qual[
         b_cntr].cp_node_id
         SET behaviors_to_update->qual[behaviors_to_update->cnt].reaction_entity_id = behaviors_rec->
         qual[b_cntr].reaction_entity_id
         SET behaviors_to_update->qual[behaviors_to_update->cnt].reaction_entity_name = behaviors_rec
         ->qual[b_cntr].reaction_entity_name
         SET behaviors_to_update->qual[behaviors_to_update->cnt].reaction_type_mean = behaviors_rec->
         qual[b_cntr].reaction_type_mean
         SET behaviors_to_update->qual[behaviors_to_update->cnt].response_ident = behaviors_rec->
         qual[b_cntr].response_ident
        ENDIF
       OF "DELETE":
        IF ((behaviors_rec->qual[b_cntr].cp_node_behavior_id > 0))
         SET behaviors_to_delete->cnt += 1
         SET stat = alterlist(behaviors_to_delete->qual,behaviors_to_delete->cnt)
         SET behaviors_to_delete->qual[behaviors_to_delete->cnt].value = behaviors_rec->qual[b_cntr].
         cp_node_behavior_id
         SET behaviors_to_delete->qual[behaviors_to_delete->cnt].reaction_entity_id = behaviors_rec->
         qual[b_cntr].reaction_entity_id
         SET behaviors_to_delete->qual[behaviors_to_delete->cnt].reaction_entity_name = behaviors_rec
         ->qual[b_cntr].reaction_entity_name
        ENDIF
      ENDCASE
     ENDIF
   ENDFOR
   IF (validate(debug_ind,0)=1)
    CALL echorecord(behaviors_to_update)
    CALL echorecord(behaviors_to_delete)
    CALL echorecord(behaviors_to_add)
   ENDIF
   IF ((behaviors_to_update->cnt > 0))
    SET requestjson = cnvtrectojson(behaviors_to_update)
    SET requestjson = replace(requestjson,"BEHAVIORS_TO_UPDATE","REQUEST")
    EXECUTE cp_upd_node_bhvr "NOFORMS", value(requestjson)
    IF ((reply->status_data.status != "S"))
     SET stat = moverec(reply->status_data,scriptreply->status_data)
     GO TO exit_script
    ELSE
     SET scriptreply->behaviors.modifycount = behaviors_to_update->cnt
    ENDIF
   ENDIF
   IF ((behaviors_to_delete->cnt > 0))
    EXECUTE cp_del_node_bhvr "NOFORMS", "" WITH replace("NBLIST","BEHAVIORS_TO_DELETE")
    IF ((reply->status_data.status != "S"))
     SET stat = moverec(reply->status_data,scriptreply->status_data)
     GO TO exit_script
    ELSE
     SET scriptreply->behaviors.deletecount = behaviors_to_delete->cnt
    ENDIF
   ENDIF
   IF ((behaviors_to_add->cnt > 0))
    FREE RECORD request
    RECORD request(
      1 blob_in = vc
    )
    SET requestjson = cnvtrectojson(behaviors_to_add)
    SET requestjson = replace(requestjson,"BEHAVIORS_TO_ADD","REQUEST")
    SET request->blob_in = requestjson
    EXECUTE cp_add_node_bhvr "NOFORMS", "", value(instance_identifier) WITH replace("REPLY",
     "ADD_BEHAVIOR_REPLY")
    FREE RECORD request
    IF ((add_behavior_reply->status_data.status != "S"))
     SET stat = moverec(add_behavior_reply->status_data,scriptreply->status_data)
     GO TO exit_script
    ELSE
     SET scriptreply->behaviors.newcount = behaviors_to_add->cnt
    ENDIF
   ENDIF
   CALL log_message("Exit updateBehaviors()",log_level_debug)
 END ;Subroutine
 SUBROUTINE savelayoutdata(null)
   CALL log_message("In saveLayoutData()",log_level_debug)
   IF (layout_long_text_id=0)
    SET layout_long_text_id = insertnewlongtext(cp_component_id,layoutjson)
    CALL addnewcomponentdetail(cp_component_id,"DOCLAYOUT","LONG_TEXT_REFERENCE",layout_long_text_id,
     "",
     "")
   ELSE
    CALL updatelongtext(layout_long_text_id,layoutjson)
   ENDIF
   CALL log_message("Exit saveLayoutData()",log_level_debug)
 END ;Subroutine
 SUBROUTINE savedocumentevents(null)
   CALL log_message("In saveDocumentEvents()",log_level_debug)
   IF (document_event_long_text_id=0)
    SET document_event_long_text_id = insertnewlongtext(cp_component_id,documenteventjson)
    CALL addnewcomponentdetail(cp_component_id,"DOCEVENTS","LONG_TEXT_REFERENCE",
     document_event_long_text_id,"",
     "")
   ELSE
    CALL updatelongtext(document_event_long_text_id,documenteventjson)
   ENDIF
   CALL log_message("Exit saveDocumentEvents()",log_level_debug)
 END ;Subroutine
 SUBROUTINE savedocumentdecorations(null)
   CALL log_message("In saveDocumentDecorations()",log_level_debug)
   IF (document_decorations_long_text_id=0)
    SET document_decorations_long_text_id = insertnewlongtext(cp_component_id,decorationsjson)
    CALL addnewcomponentdetail(cp_component_id,"DOCTERMDEC","LONG_TEXT_REFERENCE",
     document_decorations_long_text_id,"",
     "")
   ELSE
    CALL updatelongtext(document_decorations_long_text_id,decorationsjson)
   ENDIF
   CALL log_message("Exit saveDocumentDecorations()",log_level_debug)
 END ;Subroutine
 SUBROUTINE (insertnewlongtext(component_id=f8,text=vc) =f8 WITH protect)
   DECLARE dactive_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",48,"ACTIVE"))
   DECLARE dnewltextid = f8 WITH protect, noconstant(0.0)
   DECLARE lsetactind = i4 WITH protect, noconstant(1)
   SET dnewltextid = getlongtextseqid("LONG_DATA_SEQ")
   INSERT  FROM long_text_reference lt
    SET lt.active_ind = lsetactind, lt.active_status_cd = dactive_cd, lt.active_status_dt_tm =
     cnvtdatetime(sysdate),
     lt.active_status_prsnl_id = reqinfo->updt_id, lt.long_text = text, lt.long_text_id = dnewltextid,
     lt.parent_entity_id = component_id, lt.parent_entity_name = "CP_COMPONENT", lt.updt_applctx =
   ...
   ;end insert
   IF (curqual=0)
    CALL error_and_zero_check_rec(curqual,"PERFORM INSERT",
     "Unable to insert row into long_text table.",0,0,
     reply)
    ROLLBACK
    GO TO exit_script
   ELSE
   ...
   RETURN(dnewltextid)
 END ;Subroutine
 SUBROUTINE (updatelongtext(det_long_text_id=f8,text=vc) =null WITH protect)
  UPDATE  FROM long_text_reference lt
   SET lt.long_text = text, lt.updt_dt_tm = cnvtdatetime(sysdate)
   WHERE lt.long_text_id=det_long_text_id
  ;end update
  IF (curqual=0)
   CALL error_and_zero_check_rec(curqual,"PERFORM UPDATE",
    "Unable to update row into long_text table.",0,0,
    reply)
   ROLLBACK
   GO TO exit_script
  ELSE
  ...
  ENDIF
 END ;Subroutine
 SUBROUTINE addnewcomponentdetail(component_id,reltn_mean,entity_name,entity_id,component_ident,
  component_text)
   DECLARE comp_detail_reltn_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003134,
     reltn_mean))
   DECLARE version_number = i4 WITH protect, noconstant(0)
   DECLARE doccontenttypecd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003134,
     "DOCCONTENT"))
   IF (validate(debug_ind,0)=1)
    CALL echo(build(" entity_id -- > ",entity_id))
    CALL echo(build(" comp_detail_reltn_cd -- > ",comp_detail_reltn_cd))
   ENDIF
   IF (comp_detail_reltn_cd=doccontenttypecd)
    SELECT INTO "nl:"
     FROM cp_component_detail ccd
     WHERE ccd.component_detail_reltn_cd=comp_detail_reltn_cd
      AND ccd.cp_component_id=component_id
     ORDER BY ccd.version_nbr DESC
     HEAD REPORT
      version_number = ccd.version_nbr
     WITH nocounter
    ;end select
    CALL updateprevversiondefault(comp_detail_reltn_cd,version_number)
   ENDIF
   FREE RECORD detail_request
   ...
   IF (comp_detail_reltn_cd > 0)
    SET stat = alterlist(detail_request->comp_detail_list,1)
    SET detail_request->comp_detail_list[1].cp_component_id = component_id
    SET detail_request->comp_detail_list[1].comp_detail_reltn_cd = comp_detail_reltn_cd
    SET detail_request->comp_detail_list[1].component_entity_name = entity_name
    SET detail_request->comp_detail_list[1].component_entity_id = entity_id
    SET detail_request->comp_detail_list[1].component_ident = component_ident
    SET detail_request->comp_detail_list[1].component_text = component_text
    SET detail_request->comp_detail_list[1].version_nbr = latestversionnumber
    SET detail_request->comp_detail_list[1].default_ind = 1
    EXECUTE cp_add_component_detail  WITH replace("REQUEST","DETAIL_REQUEST"), replace("REPLY",
     "DETAIL_REPLY")
    IF ((detail_reply->status_data.status="S"))
     COMMIT
    ELSE
     SET scriptreply->status_data.status = "F"
     SET scriptreply->status_data.subeventstatus.operationname = "Error adding component detail"
     SET scriptreply->status_data.subeventstatus.operationstatus = "F"
    ENDIF
   ENDIF
 END ;Subroutine
 SUBROUTINE updatedefaultdoccontentrows(null)
   CALL log_message("In updateDefaultDocContentRows()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE newversionnbr = i4 WITH noconstant(0), protect
   DECLARE doccontenttypecd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003134,
     "DOCCONTENT"))
   DECLARE updcompid = f8 WITH protect, noconstant(0)
   DECLARE prevcompid = f8 WITH protect, noconstant(0)
   SET layout_long_text_id = 0
   SET document_event_long_text_id = 0
   SET document_decorations_long_text_id = 0
   SELECT INTO "nl:"
    FROM cp_component_detail ccd,
     cp_component_detail ccd1
    PLAN (ccd1
     WHERE ccd1.cp_component_id=cp_component_id
      AND ccd1.component_detail_reltn_cd=doccontenttypecd
      AND ccd1.default_ind=1)
     JOIN (ccd
     WHERE ccd.cp_component_id=ccd1.cp_component_id
      AND ccd.component_detail_reltn_cd=doccontenttypecd
      AND ccd.version_nbr > ccd1.version_nbr
      AND ccd.default_ind=0)
    ORDER BY ccd.version_nbr DESC
    HEAD REPORT
     updcompid = ccd.cp_component_detail_id, latestversionnumber = ccd.version_nbr, prevcompid = ccd1
     .cp_component_id
    WITH nocounter
   ;end select
   IF (updcompid > 0)
    UPDATE  FROM cp_component_detail ccd
     SET ccd.default_ind = 0
     WHERE ccd.default_ind=1
      AND ccd.cp_component_id=prevcompid
     WITH nocounter
    ;end update
    UPDATE  FROM cp_component_detail ccd
     SET ccd.default_ind = 1, ccd.version_flag =  $VERSIONFLAG, ccd.version_text =  $VERSIONTEXT
     WHERE ccd.cp_component_detail_id=updcompid
      AND ccd.component_detail_reltn_cd=doccontenttypecd
      AND ccd.version_nbr=latestversionnumber
      AND ccd.default_ind=0
     WITH nocounter
    ;end update
   ENDIF
   SET reqinfo->commit_ind = 1
   CALL log_message(build("Exit updateDefaultDocContentRows(), Elapsed time in seconds:",((curtime3
      - begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE (updateprevversiondefault(detailtypecd=f8,versionnbr=i4) =null WITH protect)
   CALL log_message("In updatePrevVersionDefault()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   UPDATE  FROM cp_component_detail ccd
    SET ccd.default_ind = 0
    WHERE ccd.cp_component_id=cp_component_id
     AND ccd.component_detail_reltn_cd=detailtypecd
     AND ccd.default_ind=1
     AND ccd.version_nbr=versionnbr
    WITH nocounter
   ;end update
   SET reqinfo->commit_ind = 1
   CALL log_message(build("Exit updatePrevVersionDefault(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 IF (((cp_component_id=0) OR (documenteventjson=""
  AND layoutjson=""
  AND behaviorsjson=""
  AND decorationsjson="")) )
  SET scriptreply->status_data.status = "F"
  SET scriptreply->status_data.subeventstatus.operationname =
  "No Data to Save or Component Id Provided"
  SET scriptreply->status_data.subeventstatus.operationstatus = "F"
 ELSE
  IF (behaviorsjson > " ")
   SET jrec = cnvtjsontorec(behaviorsjson)
   IF (jrec=1)
    IF (validate(debug_ind,0)=1)
     CALL echorecord(behaviors)
    ENDIF
    CALL updatebehaviors(behaviors)
   ELSE
    SET scriptreply->status_data.status = "F"
    SET scriptreply->status_data.subeventstatus.operationname = "Failed to parse behavior data"
    SET scriptreply->status_data.subeventstatus.operationstatus = "F"
    GO TO exit_script
   ENDIF
  ENDIF
  IF (( $VERSIONFLAG > 0))
   CALL updatedefaultdoccontentrows(null)
  ENDIF
  IF (latestversionnumber=0)
   SET latestversionnumber = 1
  ENDIF
  CALL savelayoutdata(null)
  CALL savedocumentevents(null)
  CALL savedocumentdecorations(null)
  SET scriptreply->document_event_long_text_id = document_event_long_text_id
  SET scriptreply->document_decorations_long_text_id = document_decorations_long_text_id
  SET scriptreply->layout_long_text_id = layout_long_text_id
  SET scriptreply->status_data.status = "S"
 ENDIF
#exit_script
 CALL setauditinitreplystatusbyprogramreplystatus(scriptreply)
 IF (validate(debug_ind,0)=1)
  CALL echorecord(scriptreply)
 ENDIF
 CALL putjsonrecordtofile(scriptreply)
END GO
