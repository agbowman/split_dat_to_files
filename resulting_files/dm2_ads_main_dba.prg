CREATE PROGRAM dm2_ads_main:dba
 ...
 SUBROUTINE dar_continue_prompt(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dam_license_found = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_ads_main",".log","dm2_ads_main")=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag=511))
  SET message = nowindow
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 IF (dam_license_found=0)
  SET message = window
  CALL clear(1,1)
  CALL box(1,1,3,132)
  CALL box(1,1,24,132)
  CALL text(2,2,"ACTIVITY DATA SAMPLE MENU [MAIN]")
  CALL text(2,70,"DATE/TIME: ")
  CALL text(2,80,format(cnvtdatetime(curdate,curtime3),"DD-MMM-YYYY HH:MM:SS;;D"))
  CALL text(5,5,"Activity Data Sampling is not licensed for this database.")
  CALL text(6,5,"Press <Enter> to Continue.")
  CALL accept(6,32,"p;cu"," "
   WHERE curaccept IN (" "))
  SET message = nowindow
 ELSE
  WHILE (true)
    ...
    CALL text(2,70,"DATE/TIME: ")
    CALL text(2,80,format(cnvtdatetime(curdate,curtime3),"DD-MMM-YYYY HH:MM:SS;;D"))
    CALL text(5,10,"1. Manage ADS Configurations")
    CALL text(7,10,"2. Summary of ADS Configurations")
    ...
    CASE (curaccept)
     OF 0:
      GO TO exit_program
     OF 1:
      EXECUTE dm2_ads_manage
      IF ((dm_err->err_ind > 0))
       GO TO exit_program
      ENDIF
     OF 2:
      EXECUTE dm2_ads_rpt_summary
      IF ((dm_err->err_ind > 0))
       GO TO exit_program
      ENDIF
    ENDCASE
...
#exit_program
 ...
 SET message = nowindow
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_ads_main has completed"
 ENDIF
 CALL final_disp_msg("dm2_ads_main")
END GO
