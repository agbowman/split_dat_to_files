CREATE PROGRAM dm_rmc_mover_1174:dba
 ...
 DECLARE cust_pref_type_meaning = vc
 DECLARE cust_predefined_tname = vc
 DECLARE cust_new_value = vc
 DECLARE cust_pref_id = vc
 DECLARE cust_seq_new = f8
 DECLARE cust_new_value = vc
 DECLARE cust_emsg = vc
 DECLARE cust_pe_name = vc
 DECLARE cust_code_value_flag = i2
 DECLARE cust_pred_prefs_flag = i2
 DECLARE cust_rpt_missing = vc
 DECLARE cust_err_val = i4
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_pref_type_meaning = get_value(cust_tab_name,"PREDEFINED_TYPE_MEANING","FROM")
 SET cust_pref_id = get_value(cust_tab_name,"PREDEFINED_PREFS_ID","FROM")
 SET cust_predefined_tname = dm2_get_rdds_tname("PREDEFINED_PREFS")
 SET cust_code_value_tname = dm2_get_rdds_tname("CODE_VALUE")
 SET cust_code_value_flag = 0
 SET cust_pred_prefs_flag = 0
 IF (isnumeric(cust_pref_type_meaning) > 0)
  SELECT INTO "NL:"
   c.code_value
   FROM (parser(cust_code_value_tname) c)
   WHERE c.code_value=cnvtreal(cust_pref_type_meaning)
    AND c.code_set IN (18989, 20323)
   WITH nocounter
  ;end select
  IF (curqual != 0)
   SET cust_code_value_flag = 1
  ENDIF
  SELECT INTO "NL:"
   p.predefined_prefs_id
   FROM (parser(cust_predefined_tname) p)
   WHERE p.predefined_prefs_id=cnvtreal(cust_pref_type_meaning)
   WITH nocounter
  ;end select
  IF (curqual != 0)
   SET cust_pred_prefs_flag = 1
  ENDIF
  IF (cust_code_value_flag=1
   AND cust_pred_prefs_flag=1)
   SET dm2_ref_data_reply->error_ind = 1
   SET cust_emsg =
   "This row wasn't translated because the sequence is on CODE_VALUE and PREDEFINED_PREFS"
   SET dm2_ref_data_reply->error_msg = "NOMV33"
   CALL echo(cust_emsg)
   CALL merge_audit("FAILREASON",cust_emsg,1)
   GO TO exit_1174
  ELSEIF (cust_code_value_flag=1
   AND cust_pred_prefs_flag=0)
   SET cust_pe_name = "CODE_VALUE"
  ELSEIF (cust_code_value_flag=0
   AND cust_pred_prefs_flag=1)
   IF (cnvtreal(cust_pref_id)=cnvtreal(cust_pref_type_meaning))
    SET cust_new_value = select_merge_translate(cust_pref_id,"PREDEFINED_PREFS")
    IF (cust_new_value="No Trans")
     SET cust_seq_new = get_seq(cust_tab_name,"PREDEFINED_PREFS_ID")
     CALL put_value(cust_tab_name,"PREDEFINED_PREFS_ID",cust_seq_new)
     CALL put_value(cust_tab_name,"PREDEFINED_TYPE_MEANING",cust_seq_new)
     SET stat = alterlist(dm2_ref_data_reply->qual,1)
     SET dm2_ref_data_reply->qual[1].table_name = cust_tab_name
     SET cust_from_cd = get_value(cust_tab_name,"PREDEFINED_PREFS_ID","FROM")
     IF ((dm2_ref_data_reply->error_ind=1))
      GO TO exit_1174
     ELSE
      SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
     ENDIF
     SET cust_new_cd = get_value(cust_tab_name,"PREDEFINED_PREFS_ID","TO")
     IF ((dm2_ref_data_reply->error_ind=1))
      GO TO exit_1174
     ELSE
      SET dm2_ref_data_reply->qual[1].to_value = cnvtreal(cust_new_cd)
     ENDIF
    ELSEIF (cust_new_value="NOMV21")
     ...
     GO TO exit_1174
    ELSE
     SET cust_pe_name = "PREDEFINED_PREFS"
    ENDIF
   ELSE
    SET cust_pe_name = "PREDEFINED_PREFS"
   ENDIF
  ELSEIF (cust_code_value_flag=0
   ...
   SET dm2_ref_data_reply->error_ind = 1
   SET cust_emsg =
   "This row wasn't translated as the PREDEFINED_TYPE_MEANING is not a CODE_VALUE/PREDEFINED_PREFS_ID"
   SET dm2_ref_data_reply->error_msg = "NOMV33"
   CALL echo(cust_emsg)
   CALL merge_audit("FAILREASON",cust_emsg,1)
   GO TO exit_1174
  ENDIF
 ELSE
  CALL put_value(cust_tab_name,"PREDEFINED_TYPE_MEANING",cust_pref_type_meaning)
 ENDIF
 IF (cust_pe_name > " ")
  SET cust_new_value = select_merge_translate(cust_pref_type_meaning,cust_pe_name)
  IF (isnumeric(cust_new_value))
   CALL put_value(cust_tab_name,"PREDEFINED_TYPE_MEANING",cust_new_value)
  ELSE
   IF (cust_pe_name="CODE_VALUE")
    SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_pref_type_meaning))
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg =
    "This row wasn't translated because of the PREDEFINED_PREFS_ID column"
   ENDIF
   GO TO exit_1174
  ENDIF
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"PREDEFINED_PREFS_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"PREDEFINED_PREFS_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"PREDEFINED_PREFS_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  PREDEFINED_PREFS_ID")
     ELSE
      GO TO exit_1174
     ENDIF
    ELSE
 ...
 IF (cust_idcd_check=1)
  SET cust_idcd_check = is_translated(cust_tab_name,"PREDEFINED_TYPE_MEANING")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg =
    "This row wasn't translated because of the PREDEFINED_TYPE_MEANING"
