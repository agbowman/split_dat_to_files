CREATE PROGRAM dm2_background_runner:dba
 DECLARE dbr_log_file = vc WITH protect, constant("dm2_backgrnd_runner")
 DECLARE dbr_user = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dbr_pword = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dbr_cnnct_str = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dbr_dm2_ddl_ops_fnd_ind = i2 WITH protect, noconstant(0)
 DECLARE dbr_mode = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE drr_from_replicate = i4 WITH protect, noconstant(1)
 DECLARE dbr_module_name = vc WITH protect, constant("DM_INSTALL_PROCESS")
 DECLARE dbr_action_name = vc WITH protect, constant("BKGRND_RUNNER")
 DECLARE dbr_original_module = vc WITH protect, noconstant("")
 DECLARE dbr_original_action = vc WITH protect, noconstant("")
 ...
 SUBROUTINE dmai_get_cur_mod_act(dgcma_mod_out,dmgca_act_out)
   ...
   RETURN(1)
 END ;Subroutine
 IF (validate(dm2_rrr_log_identifier,"x")="x"
  AND validate(dm2_rrr_log_identifier,"y")="y")
  IF (check_logfile(dbr_log_file,".log","dm2_background_runner")=0)
   GO TO end_program
  ENDIF
 ELSE
  SET dm_err->logfile = build("dm2_rrr_background_",dm2_rrr_log_identifier,".log")
  IF (init_logfile(dm_err->logfile,"dm2_background_runner")=0)
   GO TO end_program
  ENDIF
  IF (dm2_prg_maint("BEGIN")=0)
   GO TO end_program
  ENDIF
 ENDIF
 SET dbr_user = build(trim( $1))
 SET dbr_pword = build(trim( $2))
 SET dbr_cnnct_str = build(trim( $3))
 SET dbr_mode = cnvtupper(build(trim( $4)))
 IF (dbr_user=" ")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("User ",trim(dbr_user)," is invalid.")
 ...
 IF (dbr_pword=" ")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("User Password ",trim(dbr_pword)," is invalid.")
  SET dm_err->eproc = "User Password Validation"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 IF ( NOT (dbr_mode IN ("PACKAGE", "RR_RUNNER*")))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Background runner mode ",trim(dbr_mode)," is invalid.")
  SET dm_err->eproc = "Background runner mode validation"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 IF (dbr_mode="PACKAGE")
  ...
  IF ((dm_err->debug_flag > 1))
   ...
   ;end select
  ENDIF
  SET drr_runner_misc->mode = "GENERIC_BACKGROUND"
 ELSE
  SET drr_runner_misc->mode = "REPLICATE_BACKGROUND"
  SET drr_runner_misc->runner_identifier = dbr_mode
 ENDIF
 SET reqinfo->updt_task = dm2_install_schema->dm2_updt_task_value
 SET dm2_install_schema->v500_p_word = dbr_pword
 SET dm2_install_schema->v500_connect_str = dbr_cnnct_str
 SET dm2_install_schema->appl_id = currdbhandle
 SET dm_err->eproc = concat("Starting (",trim(dbr_mode),") DM2_BACKGROUND_RUNNER...")
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dm2_get_rdbms_version(null)=0)
  GO TO end_program
 ENDIF
 IF ((dm2_rdbms_version->level1 >= 11))
  IF (dmai_get_cur_mod_act(dbr_original_module,dbr_original_action)=0)
   GO TO end_program
  ENDIF
  CALL dmai_set_mod_act(dbr_module_name,dbr_action_name)
 ENDIF
 IF (dbr_mode="PACKAGE")
  IF (drr_insert_runner_row("DM2_BACKGROUND_RUNNER",dm2_install_schema->appl_id,"PARALLEL_RUNNER",1,
   0.0)=0)
   GO TO end_program
  ENDIF
 ELSE
  IF (drr_rr_insert_runner_row(dbr_mode,dm2_install_schema->appl_id)=0)
   GO TO end_program
  ENDIF
 ENDIF
 SET dir_ui_misc->dm_process_event_id = dm2_process_event_rs->dm_process_event_id
 SET dir_ui_misc->parent_script_name = "DM2_BACKGROUND_RUNNER"
 ...
 IF (val_user_privs(1)=0)
  GO TO end_program
 ENDIF
 IF (dm2_table_and_ccldef_exists("DM2_DDL_OPS_LOG",dbr_dm2_ddl_ops_fnd_ind)=0)
  GO TO end_program
 ENDIF
 IF ((dm2_process_event_rs->ui_allowed_ind=1))
...
#end_program
 IF (dbr_mode="PACKAGE")
  CALL drr_remove_runner_row("DM2_BACKGROUND_RUNNER",dm2_install_schema->appl_id)
 ELSEIF (dbr_mode="RR_RUNNER*")
  CALL drr_rr_remove_runner_row(dbr_mode,dm2_install_schema->appl_id)
 ENDIF
 SET dm_err->eproc = "Ending DM2_BACKGROUND_RUNNER"
 IF ((dm2_rdbms_version->level1 >= 11))
  CALL dmai_set_mod_act(dbr_original_module,dbr_original_action)
 ENDIF
 CALL final_disp_msg(dbr_log_file)
END GO
