CREATE PROGRAM bbt_cs_chg_code:dba
 RECORD reply(
   1 qual[1]
     2 code_value = f8
     2 disp_key = c40
 ...
 FOR (x = x TO number_to_change)
   ...
   UPDATE  FROM code_value c
    SET c.cdf_meaning =
     ...
     c.begin_effective_dt_tm = cnvtdatetime(request->qual[x].begin_effective_dt_tm), c
     .end_effective_dt_tm = cnvtdatetime(request->qual[x].end_effective_dt_tm), c.inactive_dt_tm =
