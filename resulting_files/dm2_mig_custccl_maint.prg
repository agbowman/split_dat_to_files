CREATE PROGRAM dm2_mig_custccl_maint
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dmccm_logfile_prefix = vc WITH protect, constant("dm2_mig_cc_maint")
 DECLARE dmccm_master_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccm_master_env = vc WITH protect, noconstant("")
 DECLARE dmccm_master_node = vc WITH protect, noconstant("")
 DECLARE dmccm_master_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccm_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccm_env = vc WITH protect, noconstant("")
 DECLARE dmccm_node = vc WITH protect, noconstant("")
 DECLARE dmccm_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccm_cont = i2 WITH protect, noconstant(1)
 DECLARE dmccm_os_type = vc WITH protect, noconstant("")
 SET width = 132
 IF (check_logfile(dmccm_logfile_prefix,".log","dm2_mig_custccl_maint")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_custccl_maint"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   dmccm_master_id = dena.dma_env_node_id, dmccm_master_node = dne.node_name, dmccm_master_env_id =
   dne.environment_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 SELECT INTO "nl:"
  ...
  WHERE de.environment_id=dmccm_master_id
  DETAIL
   dmccm_master_env = de.environment_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dmccm_env = trim(logical("environment"))
 SET dmccm_node = trim(curnode)
 SET dmccm_os_type = trim(cursys2)
 SET dm_err->eproc = "Getting environment_id for current node/env"
 SELECT INTO "nl:"
  FROM dm_environment de
  WHERE de.environment_name=cnvtupper(dmccm_env)
  DETAIL
   dmccm_env_id = de.environment_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "nl:"
  ...
  WHERE dne.node_name=dmccm_node
   AND dne.environment_id=dmccm_env_id
  DETAIL
   dmccm_id = dne.dma_env_node_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Current Environment: ",dmccm_env))
  CALL echo(concat("Current Environment Id: ",cnvtstring(dmccm_env_id)))
  CALL echo(concat("Current Node: ",dmccm_node))
  CALL echo(concat("Current OS Type: ",dmccm_os_type))
  CALL echo(concat("Current Node/Env ID: ",cnvtstring(dmccm_id)))
  CALL echo(concat("Master Environment: ",dmccm_master_env))
  CALL echo(concat("Master Environment Id: ",cnvtstring(dmccm_master_env_id)))
  CALL echo(concat("Master Node: ",dmccm_master_node))
  CALL echo(concat("Master Node/Env ID: ",cnvtstring(dmccm_master_id)))
 ENDIF
 WHILE (dmccm_cont=1)
   SET message = window
   SET width = 132
   CALL clear(1,1)
   CALL box(1,1,22,131)
   CALL text(1,2,"Custom CCL Migration: Script Maintenance")
   CALL text(1,70,"DATE/TIME: ")
   CALL text(1,80,build(month(curdate),"/",day(curdate),"/",year(curdate),
     " ",hour(curtime),":",minute(curtime)))
   CALL text(3,2,concat("Master Environment:",dmccm_master_env," (",trim(cnvtstring(
       dmccm_master_env_id)),")"))
   CALL text(4,2,concat("Master Node:",dmccm_master_node))
   CALL text(6,2,concat("Current Environment:",dmccm_env," (",trim(cnvtstring(dmccm_env_id)),")"))
   CALL text(7,2,concat("Current Node:",dmccm_node))
   CALL text(9,2,"(V)iew - shows current Custom CCL Script List")
   CALL text(10,2,
    "(E)xport - generates a CSV file listing of the current Custom CCL Scripts - cer_install:dm2_mig_custccl_list.csv"
    )
   CALL text(11,2,
    "(I)import - updates the Custom CCL Script list from cer_install:dm2_mig_custccl_list.csv")
   CALL text(12,2,"(Q)uit")
   CALL text(13,2,"Enter your choice:")
   CALL accept(13,22,"A;cu"," "
    WHERE curaccept IN ("V", "E", "I", "Q"))
   SET message = nowindow
   CASE (curaccept)
    OF "V":
     EXECUTE dm2_mig_custccl_rpt
     IF ((dm_err->err_ind=1))
      GO TO exit_program
     ENDIF
    OF "E":
     ...
     ENDIF
    OF "I":
     EXECUTE dm2_mig_custccl_import
     IF ((dm_err->err_ind=1))
      GO TO exit_program
     ENDIF
    OF "Q":
...
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "Ending dm2_mig_custccl_maint"
 CALL final_disp_msg(dmccm_logfile_prefix)
END GO
