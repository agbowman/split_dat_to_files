CREATE PROGRAM dcp_get_available_pat_lists:dba
 ...
 SELECT INTO "nl:"
  ...
  ORDER BY pl.patient_list_id, pa.argument_id, pe.encntr_filter_id,
   pr.reltn_id
  HEAD REPORT
   arg_ctr = 0
  HEAD pl.patient_list_id
   ...
   ENDIF
   reply->patient_lists[counter].list_access_cd = owner_cd, reply->patient_lists[counter].
   patient_list_id = pl.patient_list_id, reply->patient_lists[counter].name = pl.name,
   reply->patient_lists[counter].description = pl.description, reply->patient_lists[counter].
  ...
  HEAD pa.argument_id
   IF (pa.argument_id != 0)
    ...
    patient_lists[counter].arguments[arg_ctr].parent_entity_name = pa.parent_entity_name,
  ...
  HEAD pe.encntr_filter_id
   IF (pe.encntr_filter_id != 0)
    ...
    IF (mod(encntr_ctr,10)=1)
     stat = alterlist(reply->patient_lists[counter].encntr_type_filters,(encntr_ctr+ 9))
    ENDIF
    reply->patient_lists[counter].encntr_type_filters[encntr_ctr].encntr_type_cd = pe.encntr_type_cd,
    reply->patient_lists[counter].encntr_type_filters[encntr_ctr].encntr_class_cd = pe
    .encntr_class_cd
   ENDIF
   reltn_ctr = 0
  HEAD pr.reltn_id
   IF (pr.reltn_id != 0)
    ...
    IF (mod(reltn_ctr,10)=1)
     stat = alterlist(reply->patient_lists[counter].proxies,(reltn_ctr+ 9))
    ENDIF
    ...
    proxies[reltn_ctr].list_access_cd = pr.list_access_cd,
    reply->patient_lists[counter].proxies[reltn_ctr].beg_effective_dt_tm = cnvtdatetime(pr
     .beg_effective_dt_tm), reply->patient_lists[counter].proxies[reltn_ctr].end_effective_dt_tm =
  ...
  FOOT  pl.patient_list_id
   stat = alterlist(reply->patient_lists[counter].arguments,arg_ctr), stat = alterlist(reply->
    patient_lists[counter].encntr_type_filters,encntr_ctr), stat = alterlist(reply->patient_lists[
    counter].proxies,reltn_ctr)
  FOOT REPORT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (pr
   WHERE (pr.prsnl_id=request->prsnl_id)
    AND pr.dcp_mp_patient_list_id=0
  ...
  HEAD pl.patient_list_id
   counter += 1
   IF (size(reply->patient_lists,5) < counter)
    stat = alterlist(reply->patient_lists,(counter+ 9))
   ENDIF
   reply->patient_lists[counter].list_access_cd = pr.list_access_cd, reply->patient_lists[counter].
   patient_list_id = pl.patient_list_id, reply->patient_lists[counter].name = concat(trim(pl.name),
    "   (",substring(1,1,p.name_first),". ",trim(p.name_last),
    ")"),
   reply->patient_lists[counter].description = pl.description, reply->patient_lists[counter].
   patient_list_type_cd = pl.patient_list_type_cd, reply->patient_lists[counter].owner_id = pl
   .owner_prsnl_id,
  ...
  HEAD pa.argument_id
   IF (pa.argument_id != 0)
    CALL echo(build("PA NOT EQUAL 0",pa.argument_id)), arg_ctr += 1
    IF (mod(arg_ctr,10)=1)
     stat = alterlist(reply->patient_lists[counter].arguments,(arg_ctr+ 9))
    ENDIF
    reply->patient_lists[counter].arguments[arg_ctr].argument_name = pa.argument_name, reply->
