CREATE PROGRAM aps_chg_worklist_nbr:dba
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
...
#script
 ...
 SELECT INTO "nl:"
  ...
  WHERE (request->chg_nbr_to=pt.worklist_nbr)
  DETAIL
   request->chg_nbr_to = request->chg_nbr_to
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET reply->status_data.fail_where = "TONUMBER"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   temp_lock->lock_cntr = cntr, temp_lock->qual[cntr].updt_cnt = pt.updt_cnt, temp_lock->qual[cntr].
   processing_task_id = pt.processing_task_id
  WITH forupdate(pt)
 ;end select
 IF (curqual=0)
 ...
 UPDATE  FROM processing_task pt,
   (dummyt d  WITH seq = value(temp_lock->lock_cntr))
  SET pt.worklist_nbr = request->chg_nbr_to, pt.updt_dt_tm = cnvtdatetime(curdate,curtime), pt
