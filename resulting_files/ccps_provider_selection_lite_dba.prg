CREATE PROGRAM ccps_provider_selection_lite:dba
 ...
 IF (prmpt_search_by=search_by_name)
  SELECT INTO "nl:"
   name_order = build(substring(1,50,name_reply->prsnl[d1.seq].name_last_key)," ",substring(1,50,
     name_reply->prsnl[d1.seq].name_first_key)," ",substring(1,50,cnvtstring(name_reply->prsnl[d1.seq
      ].person_id)))
 ...
 ELSE
  SELECT INTO "nl:"
   alias = substring(1,50,alias_reply->prsnl_alias[d1.seq].alias), id = alias_reply->prsnl_alias[d1
   .seq].prsnl_alias_id
