CREATE PROGRAM aps_solcap_tsk_prfx_assoc:dba
 DECLARE solcap_cnt = i4 WITH protect, noconstant(0)
 DECLARE other_cnt = i4 WITH protect, noconstant(0)
 DECLARE task_prefix_assoc_cnt = i4 WITH protect, noconstant(0)
 DECLARE rep200001_cnt = i4 WITH protect, noconstant(0)
 DECLARE prefix_cnt = i4 WITH protect, noconstant(0)
 DECLARE prefix_idx = i4 WITH protect, noconstant(0)
 RECORD prefix_list(
   1 prefix[*]
     2 prefix_id = f8
     2 prefix_display_name = vc
     2 task_count = i4
 )
 ...
 SET reply->solcap[solcap_cnt].identifier = "2010.2.00084.1"
 ...
 IF ((rep200001->status_data.status != "S"))
  GO TO exit_script
 ENDIF
 SET rep200001_cnt = size(rep200001->prefix_qual,5)
 SET other_cnt += 1
 SET stat = alterlist(reply->solcap[solcap_cnt].other,other_cnt)
 SET reply->solcap[solcap_cnt].other[other_cnt].category_name =
 "Number of tasks associated to a particular prefix"
 IF (rep200001_cnt > 0)
  SET stat = alterlist(prefix_list->prefix,rep200001_cnt)
  SET prefix_cnt = 0
  SELECT INTO "nl:"
   siteprefixcaps = cnvtupper(cnvtalphanum(build(trim(rep200001->prefix_qual[d1.seq].site_disp,5),
      trim(rep200001->prefix_qual[d1.seq].prefix_name,5))))
   FROM (dummyt d1  WITH seq = rep200001_cnt)
   PLAN (d1)
   ORDER BY siteprefixcaps
   DETAIL
    prefix_cnt += 1, prefix_list->prefix[prefix_cnt].prefix_id = rep200001->prefix_qual[d1.seq].
    prefix_cd
    IF (size(trim(rep200001->prefix_qual[d1.seq].prefix_name)) > 0)
     prefix_list->prefix[prefix_cnt].prefix_display_name = build(trim(rep200001->prefix_qual[d1.seq].
       site_disp),trim(rep200001->prefix_qual[d1.seq].prefix_name,5))
    ELSE
     prefix_list->prefix[prefix_cnt].prefix_display_name = build(trim(rep200001->prefix_qual[d1.seq].
       site_disp),trim(rep200001->prefix_qual[d1.seq].prefix_name))
    ENDIF
   WITH nocounter
  ;end select
  FREE SET rep200001
  SELECT INTO "nl:"
   aptr.prefix_id, taskcnt = count(aptr.prefix_id)
   FROM ap_prefix_task_r aptr
   PLAN (aptr
    WHERE aptr.prefix_id > 0.0)
   GROUP BY aptr.prefix_id
   DETAIL
    lvindex = 0, idx = 0, lvindex = locateval(idx,1,prefix_cnt,aptr.prefix_id,prefix_list->prefix[idx
     ].prefix_id)
    IF (lvindex > 0)
     prefix_list->prefix[lvindex].task_count = taskcnt, task_prefix_assoc_cnt += taskcnt
    ENDIF
   WITH nocounter
  ;end select
  SET stat = alterlist(reply->solcap[solcap_cnt].other[other_cnt].value,prefix_cnt)
  FOR (prefix_idx = 1 TO prefix_cnt)
    SET reply->solcap[solcap_cnt].other[other_cnt].value[prefix_idx].display = prefix_list->prefix[
    prefix_idx].prefix_display_name
    SET reply->solcap[solcap_cnt].other[other_cnt].value[prefix_idx].value_num = prefix_list->prefix[
    prefix_idx].task_count
    IF ((prefix_list->prefix[prefix_idx].task_count > 0))
     SET reply->solcap[solcap_cnt].other[other_cnt].value[prefix_idx].value_str = "YES"
    ELSE
     SET reply->solcap[solcap_cnt].other[other_cnt].value[prefix_idx].value_str = "NO"
    ENDIF
  ENDFOR
  IF (task_prefix_assoc_cnt > 0)
   SET reply->solcap[solcap_cnt].degree_of_use_num = task_prefix_assoc_cnt
