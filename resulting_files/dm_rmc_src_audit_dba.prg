CREATE PROGRAM dm_rmc_src_audit:dba
 ...
 SUBROUTINE dgi_get_global_info(dgi_global)
   ...
   RETURN(0)
 END ;Subroutine
 DECLARE drsa_src_id = f8 WITH protect, noconstant(0.0)
 DECLARE drsa_tgt_id = f8 WITH protect, noconstant(0.0)
 DECLARE drsa_tab_pos = i4 WITH protect, noconstant(0)
 DECLARE drsa_pos = i4 WITH protect, noconstant(0)
 DECLARE drsa_start = i4 WITH protect, noconstant(0)
 DECLARE drsa_idx = i4 WITH protect, noconstant(0)
 DECLARE drsa_pk_ind = i2 WITH protect, noconstant(0)
 DECLARE drsa_row_idx = i4 WITH protect, noconstant(0)
 DECLARE drsa_log_type = vc WITH protect, noconstant(" ")
 FREE RECORD drsr_master_rs
 RECORD drsr_master_rs(
   1 dcl_row_cnt = i4
   1 table_name = vc
   1 pk_ind = i2
   1 dcl_row_qual[*]
 ...
 SET dm_err->eproc = "Starting dm_rmc_src_audit..."
 IF (check_logfile("dm_rmc_src_audit",".log","dm_rmc_src_audit LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ( NOT (validate(drsa_request,0)))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Required record structure has not been declared."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (((daf_is_blank(drsa_request->table_name)) OR (daf_is_blank(drsa_request->log_type))) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "Request record structure was not correctly populated. Please ensure table name and log type are provided"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ROLLBACK
 ...
 SET drsa_src_id = drgi_global->source_id
 SET drsa_tgt_id = drgi_global->target_id
 SET drsa_log_type = drsa_request->log_type
 SET drsa_tab_pos = locateval(drsa_pos,1,tmc_md_info->tbl_cnt,drsa_request->table_name,tmc_md_info->
  tbl_qual[drsa_pos].table_name)
 IF (drsa_tab_pos=0)
  CALL drm_md_wrp(drsa_request->table_name,tgsc_req->db_link)
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SET drsa_tab_pos = locateval(drsa_pos,1,tmc_md_info->tbl_cnt,drsa_request->table_name,tmc_md_info->
   tbl_qual[drsa_pos].table_name)
 ENDIF
 IF ((tmc_md_info->tbl_qual[drsa_tab_pos].multi_col_pk_ind=0))
  SET drsa_pk_ind = 1
 ENDIF
 SET tgsc_req->target_env_id = drsa_tgt_id
 SET tgsc_req->log_type = drsa_log_type
 SET tgsc_req->table_name = drsa_request->table_name
 IF ((drsa_request->ctxt_cnt > 0))
  SET tgsc_req->ctxt_cnt = drsa_request->ctxt_cnt
  SET stat = alterlist(tgsc_req->ctxt_qual,tgsc_req->ctxt_cnt)
  FOR (drsa_idx = 1 TO drsa_request->ctxt_cnt)
    SET tgsc_req->ctxt_qual[drsa_idx].ctxt_name = drsa_request->ctxt_qual[drsa_idx].ctxt_name
  ENDFOR
 ENDIF
 FOR (drsa_idx = 1 TO tmc_md_info->tbl_qual[drsa_tab_pos].col_cnt)
   IF ((tmc_md_info->tbl_qual[drsa_tab_pos].col_qual[drsa_idx].pk_ind=1))
    ...
    drsa_tab_pos].col_qual[drsa_idx].column_name
   ENDIF
 ENDFOR
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET drsr_master_rs->table_name = drsa_request->table_name
 SET drsr_master_rs->pk_ind = drsa_pk_ind
 FOR (drsa_idx = 1 TO tgsc_rep->row_cnt)
   IF ((((tgsc_rep->row_qual[drsa_idx].src_cnt > 0)) OR ((tgsc_rep->row_qual[drsa_idx].delete_ind=1)
   )) )
    SET drsr_master_rs->dcl_row_cnt = (drsr_master_rs->dcl_row_cnt+ 1)
    SET stat = alterlist(drsr_master_rs->dcl_row_qual,drsr_master_rs->dcl_row_cnt)
    SET drsr_master_rs->dcl_row_qual[drsr_master_rs->dcl_row_cnt].log_id = tgsc_rep->row_qual[
    drsa_idx].log_id
    SET drsr_master_rs->dcl_row_qual[drsr_master_rs->dcl_row_cnt].updt_id = tgsc_rep->row_qual[
    drsa_idx].updt_id
    SET drsr_master_rs->dcl_row_qual[drsr_master_rs->dcl_row_cnt].blocking_log_id = tgsc_rep->
    row_qual[drsa_idx].blocking_log_id
    SET drsr_master_rs->dcl_row_qual[drsr_master_rs->dcl_row_cnt].context_name = tgsc_rep->row_qual[
    drsa_idx].context_name
    SET drsr_master_rs->dcl_row_qual[drsr_master_rs->dcl_row_cnt].table_row_cnt = tgsc_rep->row_qual[
    drsa_idx].src_cnt
    SET drsr_master_rs->dcl_row_qual[drsr_master_rs->dcl_row_cnt].env_flg = 1
    SET drsr_master_rs->dcl_row_qual[drsr_master_rs->dcl_row_cnt].delete_ind = tgsc_rep->row_qual[
    drsa_idx].delete_ind
    SET drsr_master_rs->dcl_row_qual[drsr_master_rs->dcl_row_cnt].pk_where = tgsc_rep->row_qual[
    drsa_idx].pk_where
    SET stat = moverec(tgsc_rep->row_qual[drsa_idx].qual,drsr_master_rs->dcl_row_qual[drsr_master_rs
     ->dcl_row_cnt].table_row_qual)
   ENDIF
 ENDFOR
 SET stat = initrec(tgsc_rep)
 IF ((drsr_master_rs->dcl_row_cnt > 0))
  EXECUTE dm_rmc_load_row_data  WITH replace("DRLRD_REQUEST","DRSR_MASTER_RS")
 ELSE
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Error: No source rows were found for table ",drsa_request->table_name)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
#exit_program
 SET dm_err->eproc = "...Ending dm_rmc_src_audit"
 CALL final_disp_msg("dm_rmc_src_audit")
END GO
