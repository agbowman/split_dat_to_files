CREATE PROGRAM dm2_ads_driverkey_imp
 ...
 SUBROUTINE ds_scramble(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dadi_logfile_prefix = vc WITH protect, constant("dm2_dk_imp")
 DECLARE dadi_cnt = i4 WITH protect, noconstant(0)
 DECLARE dadi_cnt2 = i4 WITH protect, noconstant(0)
 DECLARE dadi_custom_ind = i2 WITH protect, noconstant(0)
 DECLARE dadi_extract_id = f8 WITH protect, noconstant(0.0)
 DECLARE dadi_schema_instance = i4 WITH protect, noconstant(0)
 DECLARE dadi_found_ind = i2 WITH protect, noconstant(0)
 DECLARE dadi_table_name = vc WITH protect, noconstant("")
 DECLARE dadi_column_name = vc WITH protect, noconstant("")
 DECLARE dadi_dace_id = f8 WITH protect, noconstant(0.0)
 SET width = 132
 IF (dm2_prg_maint("BEGIN")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_ads_driverkey_imp"
 CALL disp_msg("",dm_err->logfile,0)
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(requestin)
 ENDIF
 FOR (dadi_cnt = 1 TO size(requestin->list_0,5))
   SET dm_err->eproc = build("Processing CSV Row <",dadi_cnt,"> of <",size(requestin->list_0,5),">.")
   CALL disp_msg("",dm_err->logfile,0)
   SET requestin->list_0[dadi_cnt].table_name = replace(check(requestin->list_0[dadi_cnt].table_name,
     " ")," ","",0)
   SET requestin->list_0[dadi_cnt].driver_keycol_name = replace(check(requestin->list_0[dadi_cnt].
     driver_keycol_name," ")," ","",0)
   SET dm2scramble->mode_ind = 1
   SET dm2scramble->in_text = cnvtupper(requestin->list_0[dadi_cnt].table_name)
   IF (ds_scramble(null)=0)
    RETURN(0)
   ENDIF
   SET dadi_table_name = dm2scramble->out_text
   SET dm2scramble->out_text = ""
   SET dm2scramble->in_text = cnvtupper(requestin->list_0[dadi_cnt].driver_keycol_name)
   IF (ds_scramble(null)=0)
    RETURN(0)
   ENDIF
   SET dadi_column_name = dm2scramble->out_text
   SET dm2scramble->out_text = ""
   SET dadi_table_name = replace(check(dadi_table_name," ")," ","",0)
   SET dadi_column_name = replace(check(dadi_column_name," ")," ","",0)
   IF (trim(dadi_table_name) != "")
    SET dadi_extract_id = 0.0
    SET dm_err->eproc = "Retrieving highest allowable extract_id among current table's extracts"
    SELECT INTO "nl:"
     FROM dm_ads_extract dae
     WHERE trim(dae.table_name)=trim(dadi_table_name)
      AND trim(dae.driver_keycol_name)=trim(dadi_column_name)
      AND dae.driver_table_ind=1
     ORDER BY dae.table_extract_instance_nbr DESC
     DETAIL
      dadi_extract_id = dae.dm_ads_extract_id
    ...
    IF (curqual=0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("Current CSV Row - (",build(dadi_cnt),") is Invalid. ",
      "No extract information found for table: ",requestin->list_0[dadi_cnt].table_name)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     GO TO exit_program
    ENDIF
    IF (isnumeric(requestin->list_0[dadi_cnt].driver_key_id)=0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("Current CSV Row - (",build(dadi_cnt),") is Invalid. ",
      "Driver Key ID must be numeric. Current value: ",build(requestin->list_0[dadi_cnt].
       driver_key_id))
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     GO TO exit_program
    ENDIF
    SET dm_err->eproc = "Verifying if row already exists in dm_ads_config_driver"
    SELECT INTO "nl:"
     FROM dm_ads_config_driver dacd
     WHERE (dacd.dm_ads_config_id=dads_list->config_id)
      AND dacd.dm_ads_extract_id=dadi_extract_id
      AND dacd.driver_key_id=cnvtreal(requestin->list_0[dadi_cnt].driver_key_id)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc) > 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    ...
    ENDIF
    IF (curqual=0)
     SET dm_err->eproc = concat("Inserting row into dm_ads_config_driver for table: ",requestin->
      list_0[dadi_cnt].table_name)
     CALL disp_msg("",dm_err->logfile,0)
     INSERT  FROM dm_ads_config_driver dacd
      SET dacd.dm_ads_config_driver_id = cnvtreal(seq(dm_clinical_seq,nextval)), dacd
       .dm_ads_config_id = dads_list->config_id, dacd.dm_ads_extract_id = dadi_extract_id,
       dacd.driver_key_id = cnvtreal(requestin->list_0[dadi_cnt].driver_key_id), dacd.custom_ind = 1,
       dacd.updt_dt_tm = cnvtdatetime(curdate,curtime3),
       dacd.updt_id = 0, dacd.updt_task = 0, dacd.updt_applctx = 0,
       dacd.updt_cnt = 0
      WITH nocounter
     ;end insert
     IF (check_error(dm_err->eproc) > 0)
      ...
      GO TO exit_program
     ENDIF
     SET dm_err->eproc = "Verifying if row already exists in dm_ads_config_extract"
     SELECT INTO "nl:"
      FROM dm_ads_config_extract dace
      WHERE (dace.dm_ads_config_id=dads_list->config_id)
       AND dace.dm_ads_extract_id=dadi_extract_id
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc) > 0)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     ...
     IF (curqual=0)
      ...
      SELECT INTO "nl:"
       ...
       FROM dual
       DETAIL
        dadi_dace_id = cnvtreal(x)
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc))
       RETURN(0)
      ENDIF
      SET dm_err->eproc = concat("Inserting row in dm_ads_config_extract for table: ",requestin->
       list_0[dadi_cnt].table_name)
      CALL disp_msg("",dm_err->logfile,0)
      INSERT  FROM dm_ads_config_extract dace
       SET dace.dm_ads_config_extract_id = dadi_dace_id, dace.dm_ads_config_id = dads_list->config_id,
        dace.dm_ads_extract_id = dadi_extract_id,
        dace.status_txt = dpl_complete, dace.target_sample_method = evaluate(dads_list->config_method,
         "CUSTOM-DELETE","CUSTOM-DELETE",dpl_custom)
       WITH nocounter
      ;end insert
      IF (check_error(dm_err->eproc) > 0)
       ROLLBACK
      ...
      ENDIF
     ELSE
      SET dm_err->eproc = concat("Updating row in dm_ads_config_extract for table: ",requestin->
       list_0[dadi_cnt].table_name)
      CALL disp_msg("",dm_err->logfile,0)
      UPDATE  FROM dm_ads_config_extract dace
       SET dace.target_sample_method = evaluate(dads_list->config_method,"CUSTOM-DELETE",
         "CUSTOM-DELETE",dpl_custom), dace.status_txt = dpl_complete
       WHERE (dace.dm_ads_config_id=dads_list->config_id)
        AND dace.dm_ads_extract_id=dadi_extract_id
       WITH nocounter
      ;end update
   ...
   ENDIF
 ENDFOR
 SET dm_err->eproc = concat("Updating row into dm_ads_config for current config: ",build(dads_list->
   config_id))
 CALL disp_msg("",dm_err->logfile,0)
 UPDATE  FROM dm_ads_config dac
  SET dac.sample_method = evaluate(dads_list->config_method,"CUSTOM-DELETE","CUSTOM-DELETE",
    dpl_custom), dac.config_status = dpl_complete
