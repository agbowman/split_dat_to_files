CREATE PROGRAM dm_pha_batch_report_act_dtl_rs:dba
 DECLARE ldaystokeep = i4 WITH protect, noconstant(0)
 DECLARE ltokenidx = i4 WITH protect, noconstant(0)
 DECLARE serrormsg = vc WITH protect, noconstant("")
 DECLARE lerrorcode = i4 WITH protect, noconstant(0)
 DECLARE lrcount = i4 WITH protect, noconstant(0)
 DECLARE sline = vc WITH protect, constant(fillstring(70,"-"))
 DECLARE ltokensize = i4 WITH protect, noconstant(size(request->tokens,5))
 SET reply->status_data.status = "F"
 SET reply->table_name = "PHA_BATCH_REPORT_ACT_DTL"
 ...
 FOR (ltokenidx = 1 TO ltokensize)
   IF ((request->tokens[ltokenidx].token_str="DAYSTOKEEP"))
    SET ldaystokeep = ceil(cnvtreal(request->tokens[ltokenidx].value))
   ENDIF
 ENDFOR
 IF (ldaystokeep < 1)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"key1",
 ...
 SELECT INTO "nl:"
  pbr_detail.rowid
  FROM pha_batch_report_act pbr_act,
   pha_batch_report_act_dtl pbr_detail
  PLAN (pbr_act
   WHERE pbr_act.execution_start_dt_tm <= cnvtdatetime(datetimeadd(cnvtdatetime(curdate,curtime3),- (
     ldaystokeep)))
    AND pbr_act.pha_batch_report_act_id > 0)
   JOIN (pbr_detail
   WHERE parser(sbr_getrowidnotexists(
     "pbr_act.pha_batch_report_act_id = pbr_detail.pha_batch_report_act_id","pbr_detail")))
  HEAD REPORT
   lrcount = 0
  DETAIL
   lrcount = (lrcount+ 1)
   IF (mod(lrcount,100)=1)
    stat = alterlist(reply->rows,(lrcount+ 99))
   ENDIF
   reply->rows[lrcount].row_id = pbr_detail.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,lrcount)
  WITH nocounter, maxqual(pbr_detail,value(request->max_rows))
 ;end select
 SET lerrorcode = error(serrormsg,1)
 IF (lerrorcode != 0)
  SET reply->err_code = lerrorcode
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"key2","Failed while collecting rows: %1","s",
   nullterm(serrormsg))
  SET reply->status_data.status = "F"
  GO TO exit_script
 ELSE
...
#exit_script
 ...
 CALL echo("Mod Date: 03/31/2015")
 CALL echo(sline)
 CALL echo("********** END DM_PHA_BATCH_REPORT_ACT_DTL_RS **********")
 CALL echo(sline)
END GO
