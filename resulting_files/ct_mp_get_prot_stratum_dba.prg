CREATE PROGRAM ct_mp_get_prot_stratum:dba
 PROMPT
  ...
  "RegId:" = 0,
  "PtEligTrackingId:" = 0
  WITH outdev, protamendid, stratumopenonly,
  cohortopenonly, regid, pteligtrackingid
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 SET log_program_name = "CT_MP_GET_PROT_STRATUM"
 ...
 SET cset = 18776
 SET cmean = "DEFAULT"
 SET stat = uar_get_meaning_by_codeset(cset,cmean,1,public::"Reply"->scohorttypedefault)
 SET cset = 18776
 SET cmean = "MULTI"
 SET stat = uar_get_meaning_by_codeset(cset,cmean,1,public::"Reply"->scohorttypemulti)
 SET cset = 18776
 SET cmean = "TYPICAL"
 SET stat = uar_get_meaning_by_codeset(cset,cmean,1,public::"Reply"->scohorttypetypical)
 SET cset = 18778
 SET cmean = "CLOSED"
 SET stat = uar_get_meaning_by_codeset(cset,cmean,1,public::"Reply"->scohortstatusclosed)
 SET cset = 18778
 SET cmean = "OPEN"
 SET stat = uar_get_meaning_by_codeset(cset,cmean,1,public::"Reply"->scohortstatusopen)
 SET cset = 18778
 SET cmean = "SUSPENDED"
 SET stat = uar_get_meaning_by_codeset(cset,cmean,1,public::"Reply"->scohortstatussuspended)
 SET stratum_g_func_status = "F"
 SET public::"Reply"->assignstatus = "F"
 ...
 SELECT INTO "nl:"
  ...
  WHERE (a.prot_amendment_id= $PROTAMENDID)
  DETAIL
   public::"Reply"->enroll_strat_type_cd = a.enroll_stratification_type_cd
  WITH nocounter
 ;end select
 SET stratum_g_func_status = "F"
 CALL getstratumdata(null)
 SET public::"Reply"->status_data.status = stratum_g_func_status
 SUBROUTINE getstratumdata(null)
   SET cval = 0.0
   SET cmean = fillstring(12," ")
   IF (( $STRATUMOPENONLY=false))
    SET whrstratum = " 1=1 "
   ELSE
    SET cset = 18775
    SET cmean = "OPEN"
   ...
   SELECT INTO "nl:"
    ...
    cohortindicator = decode(pr_coh.seq,2,1)
    FROM prot_stratum pr_str,
     ...
     dummyt d2
    PLAN (pr_str
     WHERE (pr_str.prot_amendment_id= $PROTAMENDID)
    ...
    HEAD pr_str.stratum_id
     ...
     IF (mod(stratum_g_func_cnts,10)=1)
      stratum_g_func_new = (stratum_g_func_cnts+ 10), stat = alterlist(public::"Reply"->ss,
       stratum_g_func_new)
     ENDIF
     public::"Reply"->ss[stratum_g_func_cnts].status_chg_reason_cd = pr_str.status_chg_reason_cd,
     public::"Reply"->ss[stratum_g_func_cnts].organization_id = pr_str.organization_id, public::
     "Reply"->ss[stratum_g_func_cnts].prot_amendment_id = pr_str.prot_amendment_id,
     public::"Reply"->ss[stratum_g_func_cnts].stratum_label = pr_str.stratum_label, public::"Reply"->
     ss[stratum_g_func_cnts].stratum_cd = pr_str.stratum_cd, public::"Reply"->ss[stratum_g_func_cnts]
     .stratum_description = pr_str.stratum_description,
     public::"Reply"->ss[stratum_g_func_cnts].stratum_status_cd = pr_str.stratum_status_cd, public::
     "Reply"->ss[stratum_g_func_cnts].stratum_status_mean = uar_get_code_meaning(pr_str
      .stratum_status_cd), public::"Reply"->ss[stratum_g_func_cnts].stratum_cohort_type_cd = pr_str
     .stratum_cohort_type_cd,
     public::"Reply"->ss[stratum_g_func_cnts].stratum_cohort_type_mean = uar_get_code_meaning(pr_str
      .stratum_cohort_type_cd), public::"Reply"->ss[stratum_g_func_cnts].length_evaluation = pr_str
     .length_evaluation, public::"Reply"->ss[stratum_g_func_cnts].length_evaluation_uom_cd = pr_str
     .length_evaluation_uom_cd,
     public::"Reply"->ss[stratum_g_func_cnts].prot_stratum_id = pr_str.prot_stratum_id, public::
     "Reply"->ss[stratum_g_func_cnts].stratum_id = pr_str.stratum_id, public::"Reply"->ss[
     stratum_g_func_cnts].parent_stratum_id = pr_str.parent_stratum_id,
     public::"Reply"->ss[stratum_g_func_cnts].updt_cnt = pr_str.updt_cnt, stratum_g_func_cntcoh = 0
    HEAD pr_coh.prot_cohort_id
     IF (cohortindicator=2)
      stratum_g_func_cntcoh += 1
      IF (mod(stratum_g_func_cntcoh,10)=1)
       stratum_g_func_new = (stratum_g_func_cntcoh+ 10), stat = alterlist(public::"Reply"->ss[
        stratum_g_func_cnts].cs,stratum_g_func_new)
      ENDIF
      public::"Reply"->ss[stratum_g_func_cnts].cs[stratum_g_func_cntcoh].status_chg_reason_cd =
      pr_coh.status_chg_reason_cd, public::"Reply"->ss[stratum_g_func_cnts].cs[stratum_g_func_cntcoh]
      .prot_cohort_id = pr_coh.prot_cohort_id, public::"Reply"->ss[stratum_g_func_cnts].cs[
      stratum_g_func_cntcoh].cohort_id = pr_coh.cohort_id,
      public::"Reply"->ss[stratum_g_func_cnts].cs[stratum_g_func_cntcoh].parent_cohort_id = pr_coh
      .parent_cohort_id, public::"Reply"->ss[stratum_g_func_cnts].cs[stratum_g_func_cntcoh].
      stratum_id = pr_coh.stratum_id, public::"Reply"->ss[stratum_g_func_cnts].cs[
      stratum_g_func_cntcoh].pt_accrual = pr_coh.pt_accrual,
      public::"Reply"->ss[stratum_g_func_cnts].cs[stratum_g_func_cntcoh].cohort_status_cd = pr_coh
      .cohort_status_cd, public::"Reply"->ss[stratum_g_func_cnts].cs[stratum_g_func_cntcoh].
      cohort_status_mean = uar_get_code_meaning(pr_coh.cohort_status_cd), public::"Reply"->ss[
      stratum_g_func_cnts].cs[stratum_g_func_cntcoh].prot_cohort_description = pr_coh
      .prot_cohort_description
      IF (uar_get_code_meaning(pr_str.stratum_cohort_type_cd)="DEFAULT")
       public::"Reply"->ss[stratum_g_func_cnts].cs[stratum_g_func_cntcoh].cohort_label = ""
      ELSE
       public::"Reply"->ss[stratum_g_func_cnts].cs[stratum_g_func_cntcoh].cohort_label = pr_coh
       .cohort_label
      ENDIF
      public::"Reply"->ss[stratum_g_func_cnts].cs[stratum_g_func_cntcoh].valid_from_dt_tm = pr_coh
      .valid_from_dt_tm, public::"Reply"->ss[stratum_g_func_cnts].cs[stratum_g_func_cntcoh].
      valid_to_dt_tm = pr_coh.valid_to_dt_tm, public::"Reply"->ss[stratum_g_func_cnts].cs[
      stratum_g_func_cntcoh].updt_cnt = pr_coh.updt_cnt
     ENDIF
    ...
    DETAIL
     IF (suspindicator=2)
      ...
      IF (mod(stratum_g_func_cntsusp,10)=1)
       stratum_g_func_new = (stratum_g_func_cntsusp+ 10), stat = alterlist(public::"Reply"->ss[
        stratum_g_func_cnts].susps,stratum_g_func_new)
      ENDIF
      public::"Reply"->ss[stratum_g_func_cnts].susps[stratum_g_func_cntsusp].prot_stratum_susp_id =
      susp.prot_stratum_susp_id, public::"Reply"->ss[stratum_g_func_cnts].susps[
      stratum_g_func_cntsusp].susp_id = susp.susp_id, public::"Reply"->ss[stratum_g_func_cnts].susps[
      stratum_g_func_cntsusp].reason_cd = susp.reason_cd,
      public::"Reply"->ss[stratum_g_func_cnts].susps[stratum_g_func_cntsusp].susp_effective_dt_tm =
      susp.susp_effective_dt_tm, public::"Reply"->ss[stratum_g_func_cnts].susps[
      stratum_g_func_cntsusp].susp_end_dt_tm = susp.susp_end_dt_tm, public::"Reply"->ss[
      stratum_g_func_cnts].susps[stratum_g_func_cntsusp].updt_cnt = susp.updt_cnt
     ENDIF
    FOOT  pr_coh.prot_cohort_id
     stat = alterlist(public::"Reply"->ss[stratum_g_func_cnts].susps,stratum_g_func_cntsusp)
    FOOT  pr_str.stratum_id
     stat = alterlist(public::"Reply"->ss[stratum_g_func_cnts].cs,stratum_g_func_cntcoh)
   ...
   IF (( $REGID > 0.0))
    SELECT INTO "nl:"
     ...
     DETAIL
      public::"Reply"->cohort_id = arr.cohort_id, public::"Reply"->parent_cohort_id = pc
      .parent_cohort_id
     WITH nocounter
    ;end select
   ENDIF
   IF (( $REGID=0.0))
    IF (( $PTELIGTRACKINGID > 0.0))
     SELECT INTO "nl:"
      aer.*
      FROM assign_elig_reltn aer,
       prot_cohort pc
      PLAN (aer
       WHERE (aer.pt_elig_tracking_id= $PTELIGTRACKINGID)
        AND aer.end_effective_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00"))
       JOIN (pc
       WHERE pc.cohort_id=aer.cohort_id
        AND pc.end_effective_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00"))
      DETAIL
       public::"Reply"->cohort_id = aer.cohort_id, public::"Reply"->parent_cohort_id = pc
       .parent_cohort_id
      WITH nocounter
     ;end select
    ENDIF
   ENDIF
   IF (( $REGID > 0.0))
    SET public::"Reply"->assignstatus = "S"
   ELSE
    SET public::"Reply"->assignstatus = "Z"
   ENDIF
   SET stat = alterlist(public::"Reply"->ss,stratum_g_func_cnts)
