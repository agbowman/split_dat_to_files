CREATE PROGRAM bhs_wh_ops_stepdetdays_nc:dba
 PROMPT
 ...
 SELECT INTO "NL:"
  ...
  PLAN (o
   ...
   WHERE ot.ops_job_id=oj.ops_job_id
    AND  NOT (oj.name IN ("*Chart*", "*CHART*", "*chart*")))
   JOIN (ocg
 ...
 SELECT INTO  $OUTDEV
  ...
  HEAD REPORT
   col 5, "TOTAL NO. OF OPERATION JOB STEPS RUNNING CONCURRENTLY IN A MINUTE (Excl. Charting)", row
    + 1,
