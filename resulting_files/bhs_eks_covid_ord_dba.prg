CREATE PROGRAM bhs_eks_covid_ord:dba
 ...
 SELECT INTO "nl:"
  FROM encounter e
  PLAN (e
   WHERE e.encntr_id IN (173238264.00))
  HEAD REPORT
   knt = 0, eksopsrequest->expert_trigger = "BHS_EKS_COVID_ORD", knt += 1,
   stat = alterlist(eksopsrequest->qual,knt), eksopsrequest->qual[knt].person_id = e.person_id,
   eksopsrequest->qual[knt].encntr_id = e.encntr_id,
