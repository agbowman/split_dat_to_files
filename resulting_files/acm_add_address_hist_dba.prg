CREATE PROGRAM acm_add_address_hist:dba
 ...
 INSERT  FROM address_hist ah,
   (dummyt d  WITH seq = value(size(xref->add,5)))
  SET ah.address_hist_id = seq(address_seq,nextval), ah.parent_entity_id = acm_request->address_qual[
   xref->add[d.seq].idx].parent_entity_id, ah.address_id = acm_request->address_qual[xref->add[d.seq]
   .idx].address_id,
   ah.transaction_dt_tm = cnvtdatetime(acm_request->address_qual[xref->add[d.seq].idx].
    transaction_dt_tm), ah.pm_hist_tracking_id = acm_request->address_qual[xref->add[d.seq].idx].
   pm_hist_tracking_id, ah.parent_entity_name = acm_request->address_qual[xref->add[d.seq].idx].
   parent_entity_name,
   ah.address_type_cd = acm_request->address_qual[xref->add[d.seq].idx].address_type_cd, ah
   .street_addr = acm_request->address_qual[xref->add[d.seq].idx].street_addr, ah.street_addr2 =
   acm_request->address_qual[xref->add[d.seq].idx].street_addr2,
   ah.street_addr3 = acm_request->address_qual[xref->add[d.seq].idx].street_addr3, ah.street_addr4 =
   acm_request->address_qual[xref->add[d.seq].idx].street_addr4, ah.city = acm_request->address_qual[
   xref->add[d.seq].idx].city,
   ah.state = acm_request->address_qual[xref->add[d.seq].idx].state, ah.state_cd = acm_request->
   address_qual[xref->add[d.seq].idx].state_cd, ah.zipcode = acm_request->address_qual[xref->add[d
   .seq].idx].zipcode,
   ah.county = acm_request->address_qual[xref->add[d.seq].idx].county, ah.county_cd = acm_request->
   address_qual[xref->add[d.seq].idx].county_cd, ah.country = acm_request->address_qual[xref->add[d
   .seq].idx].country,
   ah.country_cd = acm_request->address_qual[xref->add[d.seq].idx].country_cd, ah
   .address_info_status_cd = acm_request->address_qual[xref->add[d.seq].idx].address_info_status_cd,
   ah.residence_cd = acm_request->address_qual[xref->add[d.seq].idx].residence_cd,
   ah.residence_type_cd = acm_request->address_qual[xref->add[d.seq].idx].residence_type_cd, ah
   .mail_stop = acm_request->address_qual[xref->add[d.seq].idx].mail_stop, ah.contact_name =
   acm_request->address_qual[xref->add[d.seq].idx].contact_name,
   ah.operation_hours = acm_request->address_qual[xref->add[d.seq].idx].operation_hours, ah
   .comment_txt = acm_request->address_qual[xref->add[d.seq].idx].comment_txt, ah.postal_identifier
    = acm_request->address_qual[xref->add[d.seq].idx].postal_identifier,
   ah.parent_beg_effective_dt_tm = cnvtdatetime(acm_request->address_qual[xref->add[d.seq].idx].
    beg_effective_dt_tm), ah.parent_end_effective_dt_tm = cnvtdatetime(acm_request->address_qual[xref
    ->add[d.seq].idx].end_effective_dt_tm), ah.tracking_bit = tracking_bit,
   ah.change_bit = chgbit, ah.active_ind = 1, ah.active_status_cd = reqdata->active_status_cd,
