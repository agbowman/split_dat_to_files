CREATE PROGRAM dm2_unlock_ccl_dictionary:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE duld_os_cmd = vc WITH protect, noconstant("")
 DECLARE duld_push_ret = i2 WITH protect, noconstant(0)
 ...
 IF (check_logfile("dm2_unlock_ccl_dict_",".log","DM2_UNLOCK_LIVE_DICT LOG FILE...") != 1)
  SET reply->status_data.subeventstatus[1].targetobjectname = "Check logfile"
  SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
  SET dm_err->err_ind = 1
  GO TO exit_program
 ENDIF
 IF (cursys="AXP")
  SET duld_os_cmd = "@cer_ocdtools:eucswapccl.com UNLOCK"
 ELSE
  SET duld_os_cmd = "$cer_ocdtools/eucswapccl.ksh 'UNLOCK'"
 ENDIF
 SET dm_err->eproc = "Unlocking live CCL dictionary."
 CALL disp_msg("",dm_err->logfile,0)
 SET duld_push_ret = dm2_push_dcl(duld_os_cmd)
 IF (duld_push_ret=0)
  SET dm_err->eproc = "Unlocking live CCL dictionary"
  IF (cursys="AXP")
  ...
  ELSE
   SET dm_err->emsg = concat("Error occurred on call to eucswapccl.ksh 'UNLOCK', ",
    "please review $cer_log/eucswapccl.log for the specific errors that occurred.")
  ENDIF
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
...
#exit_program
 ...
 ENDIF
 SET dm_err->eproc = "...Ending DM2_UNLOCK_CCL_DICTIONARY"
 CALL final_disp_msg("dm2_unlock_ccl_dict_")
END GO
