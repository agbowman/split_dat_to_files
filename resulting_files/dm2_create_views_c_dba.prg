CREATE PROGRAM dm2_create_views_c:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dhd_handle_view_depends(dhdv_mode=vc,dhdv_view_name=vc) = i4
 DECLARE dhd_process_all_objects(dpao_view_name=vc) = i4
 DECLARE dhd_exec_from_file(dwtf_cmd=vc,dwtf_filename=vc,dwtf_exec=i2) = i4
 DECLARE dhd_db2_connect_reset() = null
 DECLARE dhd_perform_rebinds() = i4
 DECLARE dhdix1 = i4 WITH protect, noconstant(0)
 DECLARE dhdix2 = i4 WITH protect, noconstant(0)
 DECLARE dhdix3 = i4 WITH protect, noconstant(0)
 DECLARE dhdix4 = i4 WITH protect, noconstant(0)
 DECLARE dhd_cnt1 = i4 WITH protect, noconstant(0)
 DECLARE dhd_cnt2 = i4 WITH protect, noconstant(0)
 DECLARE dhd_cnt3 = i4 WITH protect, noconstant(0)
 DECLARE dhd_lvlnum = i4 WITH protect, noconstant(0)
 DECLARE dhd_depfnd = i4 WITH protect, noconstant(0)
 DECLARE dhd_pkgfnd = i4 WITH protect, noconstant(0)
 DECLARE dhd_trgfnd = i4 WITH protect, noconstant(0)
 DECLARE dhd_worklvl = i4 WITH protect, noconstant(0)
 DECLARE dhd_loc_value = i4 WITH protect, noconstant(0)
 DECLARE dhd_objname = vc WITH protect, noconstant(" ")
 DECLARE dhd_dynsql = vc WITH protect, noconstant(" ")
 DECLARE dhd_filename = vc WITH protect, noconstant(" ")
 DECLARE dhdstatus = i2 WITH protect, noconstant(0)
 IF ((validate(dhd_obj->cnt,- (1))=- (1)))
  RECORD dhd_obj(
    1 cnt = i4
    1 max_level = i4
    1 master_obj_drop = vc
    1 master_obj_create = vc
    1 qual[*]
      2 level = i4
      2 specific_name = vc
      2 object_name = vc
  ...
  SET dhd_obj->cnt = 0
  SET dhd_obj->max_level = 0
 ENDIF
 IF ((validate(dhd_pkg->cnt,- (1))=- (1)))
  RECORD dhd_pkg(
    1 cnt = i4
    1 qual[*]
      2 pkgname = vc
  ...
  SET dhd_pkg->cnt = 0
 ENDIF
 SUBROUTINE dhd_process_all_objects(dpao_view_name)
   IF (get_unique_file("dm2dhd_",".sql")=0)
    RETURN(0)
   ELSE
    SET dhd_filename = dm_err->unique_fname
   ENDIF
   SET dm_err->eproc = "Connecting to database"
   SET dhd_dynsql = concat("db2 connect to ",currdblink," user v500 using v500")
   IF (dm2_push_dcl(dhd_dynsql)=0)
    CALL dhd_db2_connect_reset(1)
    RETURN(0)
   ENDIF
   IF (dhd_handle_view_dependencies("DROP",dpao_view_name) != 1)
    SET dhd_dynsql = concat("db2 rollback")
    IF (dm2_push_dcl(dhd_dynsql)=0)
     CALL dhd_db2_connect_reset(1)
     RETURN(0)
    ENDIF
    ...
    RETURN(0)
   ENDIF
   SET dhd_dynsql = dhd_obj->master_obj_drop
   IF (dhd_exec_from_file(dhd_dynsql,dhd_filename,1) != 1)
    SET dhd_dynsql = concat("db2 rollback")
    IF (dm2_push_dcl(dhd_dynsql)=0)
     CALL dhd_db2_connect_reset(1)
     RETURN(0)
    ...
    RETURN(0)
   ENDIF
   SET dhd_dynsql = dhd_obj->master_obj_create
   IF (dhd_exec_from_file(dhd_dynsql,dhd_filename,1) != 1)
    SET dhd_dynsql = concat("db2 rollback")
    IF (dm2_push_dcl(dhd_dynsql)=0)
     CALL dhd_db2_connect_reset(1)
    ...
    RETURN(0)
   ENDIF
   IF (dhd_handle_view_dependencies("CREATE",dpao_view_name) != 1)
    SET dhd_dynsql = concat("db2 rollback")
    IF (dm2_push_dcl(dhd_dynsql)=0)
     CALL dhd_db2_connect_reset(1)
     RETURN(0)
   ...
   IF ((dhd_pkg->cnt > 0))
    ...
    ENDIF
   ENDIF
   IF (size(dhd_obj->tqual,5) > 0)
    FOR (dhdix1 = 1 TO size(dhd_obj->tqual,5))
      SET dhd_dynsql = concat("DROP TRIGGER ",trim(dhd_obj->tqual[dhdix1].trigger_name))
      IF ((dm_err->debug_flag > 0))
       CALL echo(dhd_dynsql)
      ENDIF
      IF (dhd_exec_from_file(dhd_dynsql,dhd_filename,1) != 1)
       SET dhd_dynsql = concat("db2 rollback")
       IF (dm2_push_dcl(dhd_dynsql)=0)
       ...
       ENDIF
      ENDIF
      SET dhd_dynsql = dhd_obj->tqual[dhdix1].trigger_text
      IF ((dm_err->debug_flag > 0))
       CALL echo(dhd_dynsql)
      ENDIF
      IF (dhd_exec_from_file(dhd_dynsql,dhd_filename,1) != 1)
    ...
    ENDFOR
   ENDIF
   SET dm_err->eproc = "Commiting all DDL from the connection."
   SET dhd_dynsql = concat("db2 commit")
   IF (dm2_push_dcl(dhd_dynsql)=0)
    CALL dhd_db2_connect_reset(1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Closing the extra connection to database"
   CALL dhd_db2_connect_reset(1)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dhd_handle_view_dependencies(dhvd_mode,dhvd_view_name)
  CASE (dhvd_mode)
   OF "LOAD":
    SELECT INTO "nl:"
     FROM (syscat.routines r),
      (syscat.routinedep rd)
     WHERE r.specificname=rd.routinename
      AND r.routineschema=rd.routineschema
      AND r.routineschema=rd.bschema
      AND r.routineschema=currdbuser
      AND rd.bname=patstring(dhvd_view_name)
      AND rd.btype="V"
     HEAD REPORT
      dhd_cnt1 = 0
     DETAIL
      dhd_cnt1 = (dhd_cnt1+ 1), stat = alterlist(dhd_obj->qual,dhd_cnt1)
      IF ((dm_err->debug_flag > 0))
       CALL echo(build("Level(1) dependent routine found: ",r.routinename))
      ENDIF
      dhd_obj->qual[dhd_cnt1].specific_name = r.specificname, dhd_obj->qual[dhd_cnt1].object_name = r
      .routinename, dhd_obj->qual[dhd_cnt1].level = 1,
      dhd_obj->qual[dhd_cnt1].object_type = "F", dhd_obj->qual[dhd_cnt1].object_source = r.text
     FOOT REPORT
      dhd_obj->cnt = dhd_cnt1
     WITH nocounter
    ;end select
    SET dm_err->eproc = "DM2_HANDLE_DEPENDENCIES.INC- Selecting for view dependent objects."
    IF (check_error(dm_err->eproc) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    SELECT INTO "nl:"
     FROM user_views uv,
      (syscat.tabdep td)
     WHERE uv.view_name=td.tabname
      AND uv.owner=td.tabschema
      AND td.tabschema=currdbuser
      AND td.bname=patstring(dhvd_view_name)
      AND td.btype="V"
     HEAD REPORT
      dhd_cnt1 = dhd_obj->cnt
     DETAIL
      dhd_cnt1 = (dhd_cnt1+ 1), stat = alterlist(dhd_obj->qual,dhd_cnt1)
      IF ((dm_err->debug_flag > 0))
       CALL echo(build("Level (1) dependent view found: ",uv.view_name))
      ENDIF
      dhd_obj->qual[dhd_cnt1].specific_name = uv.view_name, dhd_obj->qual[dhd_cnt1].object_name = uv
      .view_name, dhd_obj->qual[dhd_cnt1].level = 1,
      dhd_obj->qual[dhd_cnt1].object_type = "V", dhd_obj->qual[dhd_cnt1].object_source = uv.text
     FOOT REPORT
      dhd_obj->cnt = dhd_cnt1
     WITH nocounter
    ;end select
    SET dm_err->eproc = "DM2_HANDLE_DEPENDENCIES.INC- Selecting for view dependent views."
    IF (check_error(dm_err->eproc) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    IF ((dhd_obj->cnt > 0))
     SET dhd_obj->max_level = 1
     SET dhd_depfnd = 1
     SET dhd_lvlnum = 1
     WHILE (dhd_depfnd=1)
       SET dhd_depfnd = 0
       SET dhd_lvlnum = (dhd_lvlnum+ 1)
       IF ((dm_err->debug_flag > 0))
        CALL echo(build("Looping into level: ",dhd_lvlnum))
       ENDIF
       FOR (dhdix1 = 1 TO dhd_obj->cnt)
         IF ((dhd_obj->qual[dhdix1].level=(dhd_lvlnum - 1)))
          SELECT INTO "nl:"
           FROM (syscat.routines r),
            (syscat.routinedep rd)
           WHERE r.specificname=rd.routinename
            ...
            AND r.routineschema=rd.bschema
            AND r.routineschema=currdbuser
            AND (rd.bname=dhd_obj->qual[dhdix1].specific_name)
            AND rd.btype IN ("F", "V")
           HEAD REPORT
            dhd_cnt1 = dhd_obj->cnt
           DETAIL
            dhd_loc_value = 0
            FOR (dhdix2 = 1 TO dhd_cnt1)
              IF ((r.routinename=dhd_obj->qual[dhdix2].object_name))
               dhd_loc_value = dhdix2, dhdix2 = dhd_cnt1
              ENDIF
            ENDFOR
            IF (dhd_loc_value=0)
             dhd_cnt1 = (dhd_cnt1+ 1), stat = alterlist(dhd_obj->qual,dhd_cnt1)
             IF ((dm_err->debug_flag > 0))
              CALL echo(build("Level(",dhd_lvlnum,") dependent routine found: ",r.routinename))
             ENDIF
             dhd_obj->qual[dhd_cnt1].specific_name = r.specificname, dhd_obj->qual[dhd_cnt1].
             object_name = r.routinename, dhd_obj->qual[dhd_cnt1].level = dhd_lvlnum,
             dhd_obj->qual[dhd_cnt1].object_type = "F", dhd_obj->qual[dhd_cnt1].object_source = r
             .text
            ELSE
             dhd_obj->qual[dhd_loc_value].level = dhd_lvlnum
            ENDIF
           FOOT REPORT
            dhd_obj->cnt = dhd_cnt1
           WITH nocounter
          ;end select
          SET dm_err->eproc = concat("DM2_HANDLE_DEPENDENCIES.INC- ",
           "Selecting for dependent functions/routines of a dependent object.")
          IF (check_error(dm_err->eproc) != 0)
           CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
           RETURN(0)
          ENDIF
          ...
          SELECT INTO "nl:"
           ...
           WHERE uv.view_name=td.tabname
            AND uv.owner=td.tabschema
            AND td.tabschema=currdbuser
            AND (td.bname=dhd_obj->qual[dhdix1].specific_name)
            AND td.btype IN ("F", "V")
           HEAD REPORT
            dhd_cnt_before = dhd_obj->cnt, dhd_cnt1 = dhd_obj->cnt
           DETAIL
            dhd_loc_value = 0
            FOR (dhdix2 = 1 TO dhd_cnt1)
              IF ((uv.view_name=dhd_obj->qual[dhdix2].object_name))
               dhd_loc_value = dhdix2, dhdix2 = dhd_cnt1
              ENDIF
            ENDFOR
            IF (dhd_loc_value=0)
             dhd_cnt1 = (dhd_cnt1+ 1), stat = alterlist(dhd_obj->qual,dhd_cnt1)
             IF ((dm_err->debug_flag > 0))
              CALL echo(build("Level(",dhd_lvlnum,") dependent view found: ",uv.view_name))
             ENDIF
             dhd_obj->qual[dhd_cnt1].specific_name = uv.view_name, dhd_obj->qual[dhd_cnt1].
             object_name = uv.view_name, dhd_obj->qual[dhd_cnt1].level = dhd_lvlnum,
             dhd_obj->qual[dhd_cnt1].object_type = "V", dhd_obj->qual[dhd_cnt1].object_source = uv
             .text
            ELSE
             dhd_obj->qual[dhd_loc_value].level = dhd_lvlnum
            ENDIF
          ...
          ;end select
          SET dm_err->eproc = concat("DM2_HANDLE_DEPENDENCIES.INC- ",
           "Selecting for dependent views of a dependent object.")
          IF (check_error(dm_err->eproc) != 0)
           CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
           RETURN(0)
          ENDIF
       ...
       ENDFOR
       IF (dhd_depfnd=1)
        SET dhd_obj->max_level = dhd_lvlnum
       ENDIF
     ENDWHILE
    ENDIF
    SELECT DISTINCT INTO "nl:"
     p.pkgname, p.valid, pd.bname,
     pd.btype
     FROM (syscat.packages p),
      (syscat.packagedep pd)
     WHERE p.pkgname=pd.pkgname
      AND p.pkgschema=pd.pkgschema
      AND p.pkgschema=pd.bschema
      AND p.pkgschema=currdbuser
      AND pd.btype IN ("V", "F")
     HEAD REPORT
      dhd_cnt2 = 0, driver_fnd = 0
     DETAIL
      FOR (dhdix1 = 1 TO dhd_obj->cnt)
        IF ((((dhd_obj->qual[dhdix1].specific_name=pd.bname)) OR (dhvd_view_name=pd.bname
         AND driver_fnd=0)) )
         dhd_pkgfnd = 0
         FOR (dhdix2 = 1 TO dhd_cnt2)
           IF ((dhd_pkg->qual[dhdix2].pkgname=p.pkgname))
            IF ((dm_err->debug_flag > 0))
             CALL echo(build("Package dependency already exists for: ",p.pkgname,"  ..bypassing..."))
            ENDIF
            dhd_pkgfnd = 1
           ENDIF
         ENDFOR
         IF (dhd_pkgfnd=0)
          dhd_cnt2 = (dhd_cnt2+ 1), stat = alterlist(dhd_pkg->qual,dhd_cnt2)
          IF ((dm_err->debug_flag > 0))
           CALL echo(build("Package dependency found: ",p.pkgname))
          ENDIF
          dhd_pkg->qual[dhd_cnt2].pkgname = p.pkgname, dhd_pkg->qual[dhd_cnt2].valid = p.valid,
          dhd_pkg->qual[dhd_cnt2].bname = pd.bname,
          dhd_pkg->qual[dhd_cnt2].btype = pd.btype
         ENDIF
         IF (pd.bname=dhvd_view_name)
          driver_fnd = 1
         ENDIF
        ENDIF
      ENDFOR
     FOOT REPORT
      dhd_pkg->cnt = dhd_cnt2
     WITH nocounter
    ;end select
    SET dm_err->eproc = "DHD_HANDLE_DEPENDENCIES.INC- Selecting for dependent procedures/packages."
    IF (check_error(dm_err->eproc) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    SELECT INTO "nl:"
     dut.trigger_name, st.valid, std.bname,
     dut.trigger_body
     FROM (syscat.trigdep std),
      (syscat.triggers st),
      dm2_user_triggers dut
     WHERE std.trigname=dut.trigger_name
      AND st.trigname=std.trigname
      AND st.trigschema=std.trigschema
      AND std.bschema=dut.owner
      AND std.btype IN ("V", "F")
      AND std.bschema=currdbuser
     HEAD REPORT
      dhd_cnt3 = 0, driver_fnd = 0
     DETAIL
      FOR (dhdix3 = 1 TO value(size(dhd_obj->qual,5)))
        IF ((((dhd_obj->qual[dhdix3].specific_name=std.bname)) OR (dhvd_view_name=std.bname
         AND driver_fnd=0)) )
         dhd_trgfnd = 0
         FOR (dhdix4 = 1 TO dhd_cnt3)
           IF ((dhd_obj->tqual[dhdix4].trigger_name=dut.trigger_name))
            IF ((dm_err->debug_flag > 0))
             CALL echo(build("Trigger dependency already exists for: ",dut.trigger_name,
              "  ...bypassing..."))
            ENDIF
            dhd_trgfnd = 1
           ENDIF
         ENDFOR
         IF (dhd_trgfnd=0)
          dhd_cnt3 = (dhd_cnt3+ 1), stat = alterlist(dhd_obj->tqual,dhd_cnt3)
          IF ((dm_err->debug_flag > 0))
           CALL echo(build("Trigger dependency found: ",dut.trigger_name))
          ENDIF
          dhd_obj->tqual[dhd_cnt3].trigger_name = dut.trigger_name, dhd_obj->tqual[dhd_cnt3].
          trigger_text = dut.trigger_body
         ENDIF
         IF (std.bname=dhvd_view_name)
          driver_fnd = 1
         ENDIF
        ENDIF
      ENDFOR
     WITH nocounter
    ;end select
    SET dm_err->eproc = "DHD_HANDLE_DEPENDENCIES.INC- Selecting for dependent triggers."
    IF (check_error(dm_err->eproc) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    IF ((dm_err->debug_flag > 0))
     CALL echorecord(dhd_obj)
     CALL echorecord(dhd_pkg)
    ENDIF
   OF "DROP":
    IF ((dhd_obj->cnt > 0))
     IF ((dm_err->debug_flag > 0))
      CALL echo("DROPPING DEPENDENT OBJECTS")
     ENDIF
     SET dhd_worklvl = dhd_obj->max_level
     WHILE (dhd_worklvl > 0)
      FOR (dhdix1 = 1 TO dhd_obj->cnt)
        IF ((dhd_obj->qual[dhdix1].level=dhd_worklvl))
         SET dhd_dynsql = "DROP "
         CASE (dhd_obj->qual[dhdix1].object_type)
          OF "F":
           SET dhd_dynsql = concat(dhd_dynsql," FUNCTION ")
          OF "V":
           SET dhd_dynsql = concat(dhd_dynsql," VIEW ")
          ELSE
           SET dm_err->eproc = "Setting Object Type in Drop Command."
           SET dm_err->emsg = "Unaccounted for dependent object type."
           CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
           SET dm_err->err_ind = 1
           RETURN(0)
         ENDCASE
         SET dhd_dynsql = concat(dhd_dynsql," ",trim(dhd_obj->qual[dhdix1].object_name))
         IF (dhd_exec_from_file(dhd_dynsql,dhd_filename,1) != 1)
          RETURN(0)
         ENDIF
        ENDIF
      ENDFOR
      SET dhd_worklvl = (dhd_worklvl - 1)
     ENDWHILE
    ENDIF
   OF "CREATE":
    IF ((dm_err->debug_flag > 0))
     CALL echo("CREATING DEPENDENT OBJECTS")
    ENDIF
    IF ((dhd_obj->cnt > 0))
     SET dhd_worklvl = 1
     WHILE ((dhd_worklvl <= dhd_obj->max_level))
      FOR (dhdix1 = 1 TO dhd_obj->cnt)
        IF ((dhd_obj->qual[dhdix1].level=dhd_worklvl))
         SET dhd_dynsql = trim(dhd_obj->qual[dhdix1].object_source)
         IF (dhd_exec_from_file(dhd_dynsql,dhd_filename,1) != 1)
          RETURN(0)
         ENDIF
        ENDIF
    ...
    ENDIF
   ELSE
    SET dm_err->eproc = "Determining the mode for subroutine dhd_handle_view_dependencies. "
    SET dm_err->emsg = "Unaccounted for mode supplied to subroutine dhd_handle_view_dependencies."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->err_ind = 1
    RETURN(0)
  ENDCASE
  RETURN(1)
 END ;Subroutine
 SUBROUTINE dhd_exec_from_file(dwtf_cmd,dwtf_filename,dwtf_exec)
   SELECT INTO value(dwtf_filename)
    FROM (dummyt d  WITH seq = 1)
    DETAIL
     row + 1, col 0, dwtf_cmd
    FOOT REPORT
     row + 1, col 0, "@"
    WITH nocounter, format = variable, formfeed = none,
     maxcol = 32000
   ;end select
   SET dm_err->eproc = "DM2_HANDLE_DEPENDENCIES.INC- Writing dependency command to unique file name."
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (dwtf_exec=1)
    IF ((dm_err->debug_flag > 1))
     CALL echo("EXECUTING command through a file")
     CALL echo("Command in file is: ")
     SET dhd_dynsql = concat("cat ",dwtf_filename)
     SET dhdstatus = 0
     CALL dcl(dhd_dynsql,size(trim(dhd_dynsql)),dhdstatus)
     IF (dhdstatus=0)
      CALL echo(concat("Invalid call dcl command: ",dhd_dynsql))
     ENDIF
    ENDIF
    SET dm_err->eproc = concat(
     "DM2_HANDLE_DEPENDENCIES.INC- Executing dependency command from file: ",dwtf_filename)
    SET dhd_dynsql = concat("db2 +c -td@ -vf $CCLUSERDIR/",dwtf_filename)
    IF (dm2_push_dcl(dhd_dynsql)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dhd_db2_connect_reset(null)
   SET dm_err->eproc = "DM2_HANDLE_DEPENDENCIES.INC- Closing the extra connection to database"
   SET dhd_dynsql = concat("db2 connect reset")
   IF (dm2_push_dcl(dhd_dynsql)=0)
    RETURN(0)
   ENDIF
   RETURN
 END ;Subroutine
 SUBROUTINE dhd_perform_rebinds(null)
  FOR (dhdix1 = 1 TO dhd_pkg->cnt)
    SET dm_err->eproc = concat("DM2_HANDLE_DEPENDENCIES.INC - Performing Rebind for Package :",trim(
      dhd_pkg->qual[dhdix1].pkgname))
    SET dhd_dynsql = concat("rebind ",trim(dhd_pkg->qual[dhdix1].pkgname))
    IF ((dm_err->debug_flag > 1))
     CALL echo(concat("EXECUTING rebind command: ",dhd_dynsql))
    ENDIF
    IF (dhd_exec_from_file(dhd_dynsql,dhd_filename,1) != 1)
     RETURN(0)
    ENDIF
  ...
  RETURN(1)
 END ;Subroutine
 DECLARE cvc_push_str = vc WITH protect, noconstant(" ")
 DECLARE cvc_original_etu_mode = vc WITH protect, noconstant(" ")
 DECLARE chk_drop_syn(null) = null
 DECLARE chk_drop_view(null) = null
 IF (dm2_prg_maint("BEGIN")=0)
  GO TO exit_prog
 ENDIF
 IF (currdb="ORACLE")
  SET dm_err->eproc = concat("Building public synonym for ",dm_objects->obj_cnt[dcv_vcnt].obj_name)
  CALL disp_msg(" ",dm_err->logfile,0)
  IF ((dm_objects->obj_cnt[dcv_vcnt].drop_ind=1))
   SET cvc_push_str = concat("rdb drop public synonym ",dm_objects->obj_cnt[dcv_vcnt].obj_name," go")
   IF (dm2_push_cmd(cvc_push_str,1)=0)
    ROLLBACK
    GO TO exit_prog
   ENDIF
  ENDIF
  SET cvc_push_str = concat("rdb create public synonym ",dm_objects->obj_cnt[dcv_vcnt].obj_name,
   " for ",dm_objects->obj_cnt[dcv_vcnt].obj_name," go")
  IF (dm2_push_cmd(cvc_push_str,1)=0)
   ROLLBACK
   GO TO exit_prog
  ENDIF
  COMMIT
 ELSEIF (currdb IN ("DB2UDB", "SQLSRV"))
  SET dm_err->eproc = concat("Working on view ",dm_objects->obj_cnt[dcv_vcnt].obj_name," for ",
   dm_objects->obj_cnt[dcv_vcnt].suffix)
  CALL disp_msg(" ",dm_err->logfile,0)
  IF ((dm_objects->obj_cnt[dcv_vcnt].drop_ind=1))
   IF (currdb="DB2UDB")
    IF (dhd_handle_view_dependencies("LOAD",dm_objects->obj_cnt[dcv_vcnt].obj_name) != 1)
     ROLLBACK
     GO TO exit_prog
    ENDIF
    IF ((dhd_obj->cnt > 0))
     SET dhd_obj->master_obj_drop = concat("drop view ",dm_objects->obj_cnt[dcv_vcnt].obj_name)
     SET dhd_obj->master_obj_create = concat("create view ",dm_objects->obj_cnt[dcv_vcnt].obj_name,
      " as select * from ",dm_objects->obj_cnt[dcv_vcnt].suffix)
     IF (dhd_process_all_objects(dm_objects->obj_cnt[dcv_vcnt].obj_name) != 1)
      ROLLBACK
      GO TO exit_prog
     ENDIF
    ELSE
     SET dhd_obj->master_obj_drop = ""
     SET dhd_obj->master_obj_create = ""
     SET cvc_push_str = concat("rdb drop view ",dm_objects->obj_cnt[dcv_vcnt].obj_name," go")
     IF (dm2_push_cmd(cvc_push_str,1)=0)
      ROLLBACK
      GO TO exit_prog
     ENDIF
  ...
  ENDIF
  IF (currdb="SQLSRV")
   IF ((dm_objects->obj_cnt[dcv_vcnt].data_model_section="OUTCOMES*"))
    SET cvc_push_str = concat("rdb create view ",dm_objects->obj_cnt[dcv_vcnt].obj_name,
     " as select * from ",dm_objects->obj_cnt[dcv_vcnt].suffix," with(nolock) go")
   ELSE
    SET cvc_push_str = concat("rdb create view ",dm_objects->obj_cnt[dcv_vcnt].obj_name,
     " as select * from ",dm_objects->obj_cnt[dcv_vcnt].suffix," go")
   ENDIF
  ELSE
   SET cvc_push_str = concat("rdb create view ",dm_objects->obj_cnt[dcv_vcnt].obj_name,
    " as select * from ",dm_objects->obj_cnt[dcv_vcnt].suffix," go")
  ...
  ENDIF
  COMMIT
  IF ((dm_objects->obj_cnt[dcv_vcnt].oragen_ind=1))
   IF (dcv_call_by_etu_ind=1)
    SET cvc_original_etu_mode = dm2_etu->mode
    SET dm2_etu->mode = "TABLE"
   ENDIF
   EXECUTE oragen3 value(dm_objects->obj_cnt[dcv_vcnt].obj_name)
   IF ((dm_err->err_ind=1))
    GO TO exit_prog
   ENDIF
   IF (dcv_call_by_etu_ind=1)
    SET dm2_etu->mode = cvc_original_etu_mode
   ENDIF
  ENDIF
  IF (currdb="DB2UDB")
   SET dm_err->eproc = concat("Working on alias V500READ.",dm_objects->obj_cnt[dcv_vcnt].obj_name,
    " for V500.",dm_objects->obj_cnt[dcv_vcnt].obj_name)
   CALL disp_msg(" ",dm_err->logfile,0)
   IF ((dm_objects->obj_cnt[dcv_vcnt].drop_a_ind=1))
    SET cvc_push_str = concat("rdb drop alias V500READ.",dm_objects->obj_cnt[dcv_vcnt].obj_name," go"
     )
    IF (dm2_push_cmd(cvc_push_str,1)=0)
     ROLLBACK
     GO TO exit_prog
    ENDIF
   ENDIF
   SET cvc_push_str = concat("rdb create alias V500READ.",dm_objects->obj_cnt[dcv_vcnt].obj_name,
    " for V500.",dm_objects->obj_cnt[dcv_vcnt].obj_name," go")
   IF (dm2_push_cmd(cvc_push_str,1)=0)
    ROLLBACK
    GO TO exit_prog
   ENDIF
   COMMIT
   SET cvc_push_str = concat("rdb grant select on table ",dm_objects->obj_cnt[dcv_vcnt].obj_name,
    " to user v500read go")
   IF (dm2_push_cmd(cvc_push_str,1)=0)
    ROLLBACK
    GO TO exit_prog
