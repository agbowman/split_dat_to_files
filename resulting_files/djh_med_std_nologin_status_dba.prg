CREATE PROGRAM djh_med_std_nologin_status:dba
 PROMPT
 ...
 SELECT DISTINCT INTO value(output_dest)
  ...
  IF (pr.active_status_cd != 188) format(pr.end_effective_dt_tm,"MM/DD/YYYY;;D")
  ELSE ""
  ENDIF
  , act_stat_disp =
  ...
  PLAN (pr
   WHERE pr.position_cd=777650
    ...
    AND ((pr.active_ind=1
    AND pr.active_status_cd=188.00) OR (pr.active_ind=1
  ...
  HEAD pr.name_full_formatted
   position = trim(uar_get_code_display(pr.position_cd)), last_login = format(oa.start_day,
    "YYYY/MM/DD;;D"), xlapsdays = datetimediff(cnvtdatetime(curdate,0),oa.start_day,1),
   lapsdays = format(xlapsdays,"####"), lapsdaysmsg =
   IF (xlapsdays > 90) "xxx"
   ELSE ""
   ENDIF
