CREATE PROGRAM dm2_resumable_session_menu
 ...
 END ;Subroutine
 IF (check_logfile("dm2_res_sess_menu",".log","dm2_res_sess_menu")=0)
  GO TO exit_program
 ENDIF
 SET width = 132
 IF ((dm_err->debug_flag != 722))
  SET message = window
 ENDIF
 DECLARE drsm_scroll(ds_rec_str=vc) = i2
 DECLARE drsm_screen_header(null) = i2
 DECLARE drsm_driver(null) = i2
 DECLARE drsm_work_menu(dwm_screen_return=vc(ref)) = i2
 DECLARE fill_drsm_rsm_screen(holder=i2) = i2
 DECLARE drsm_load_sessions(null) = i2
 DECLARE drsm_increase(null) = i2
 DECLARE drsm_abort(null) = i2
 DECLARE drsm_leave = i2 WITH protect, noconstant(0)
 DECLARE drsm_drsm_display_line = i2 WITH protect, noconstant(0)
 DECLARE drsm_row = i2 WITH protect, noconstant(0)
 FREE RECORD drsm_screen
 RECORD drsm_screen(
   1 top_line = i4
   1 bottom_line = i4
   1 cur_line = i4
 ...
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_program
 ENDIF
 IF ((dm2_rdbms_version->level1 <= 8))
  SET message = nowindow
  SET dm_err->eproc = "Resumable Menu only Available on Oracle 9 or greater."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF (drsm_screen_header(null)=0)
  GO TO exit_program
 ENDIF
 IF (drsm_load_sessions(null)=0)
  GO TO exit_program
 ENDIF
 IF (drsm_driver(drsm_leave)=0)
  GO TO exit_program
 ENDIF
 IF (drsm_leave=1)
  GO TO exit_program
 ...
 SUBROUTINE drsm_driver(null)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drsm_screen_header(null)
   CALL clear(1,1)
   CALL text(1,52,"*** RESUMABLE SESSIONS ***")
   CALL text(2,50,"Note:Time displayed in MINUTES")
   CALL text(4,1,"SID")
   CALL text(4,23,"Username")
   CALL text(4,45,"Instance Name")
   CALL text(4,67,"Host Name")
   CALL text(4,90,"Status")
   CALL text(4,110,"Timeout")
   CALL text(4,118,"Time Remaining")
   CALL line(5,1,132,horizontal)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drsm_work_menu(dwm_screen_return)
   CALL clear(1,1)
   IF (drsm_screen_header(null)=0)
    RETURN(0)
   ENDIF
   SET drsm_display_line = 6
   IF ((drsm_screen->cur_line <= 1))
    SET drsm_screen->top_line = 1
    SET drsm_screen->cur_line = 1
    SET drsm_screen->bottom_line = 0
    SET drsm_screen->max_scroll = 16
   ELSE
    SET drsm_screen->top_line = drsm_screen->top_line
    SET drsm_screen->cur_line = drsm_screen->cur_line
    SET drsm_screen->bottom_line = drsm_screen->bottom_line
    SET drsm_screen->max_scroll = 16
   ENDIF
   IF (fill_drsm_screen(1)=0)
    RETURN(0)
   ENDIF
   SET dwm_screen_return = ""
   CASE (drsm_scroll("drsm_screen"))
    OF "Q":
     SET dwm_screen_return = "QUIT"
    OF "R":
     SET dwm_screen_return = "REFRESH"
     IF (drsm_load_sessions(null)=0)
      RETURN(0)
     ENDIF
    OF "I":
     IF (size(drsm_res->id,5)=0)
      CALL clear(24,1,130)
      CALL text(24,3,"NO RESUMABLE SESSIONS FOUND.")
      CALL pause(3)
     ELSE
      SET dwm_screen_return = "INCREASE TIMEOUT"
      IF (drsm_increase(null)=0)
       RETURN(0)
      ENDIF
     ENDIF
     IF (drsm_load_sessions(null)=0)
    ...
    OF "A":
     IF (size(drsm_res->id,5)=0)
      ...
      CALL text(24,3,"NO RESUMABLE SESSIONS FOUND.")
     ELSE
      SET dwm_screen_return = "ABORT SESSION"
      IF (drsm_abort(null)=0)
       RETURN(0)
      ENDIF
     ENDIF
     IF (drsm_load_sessions(null)=0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE fill_drsm_screen(holder)
   DECLARE fds_screen_cnt = i4 WITH protect, noconstant(0)
   DECLARE fds_first = i2 WITH protect, noconstant(0)
   DECLARE fds_err_cnt = i2 WITH protect, noconstant(0)
   DECLARE fds_fds_display_loop = i2 WITH protect, noconstant(0)
   DECLARE fds_fds_display_blank = i2 WITH protect, noconstant(0)
   SET drsm_display_line = 6
   SET drsm_screen->max_scroll = 16
   IF (((drsm_screen->top_line+ 16) > drsm_screen->max_value))
    SET drsm_screen->bottom_line = drsm_screen->max_value
   ELSE
    SET drsm_screen->bottom_line = (drsm_screen->top_line+ 15)
   ENDIF
   FOR (fds_display_loop = drsm_screen->top_line TO drsm_screen->bottom_line)
     IF ((fds_display_loop=drsm_screen->cur_line)
      AND (drsm_res->id[fds_display_loop].err_msg_seq=0))
      SET drsm_row = drsm_display_line
      CALL video(r)
     ELSE
      CALL video(n)
     ENDIF
     CALL clear(drsm_display_line,1,132)
     IF ((drsm_res->id[fds_display_loop].err_msg_seq=0))
      CALL text(drsm_display_line,1,cnvtstring(drsm_res->id[fds_display_loop].sid))
      CALL text(drsm_display_line,23,drsm_res->id[fds_display_loop].username)
      CALL text(drsm_display_line,45,drsm_res->id[fds_display_loop].instance)
      CALL text(drsm_display_line,67,drsm_res->id[fds_display_loop].hostname)
      CALL text(drsm_display_line,90,drsm_res->id[fds_display_loop].status)
      CALL text(drsm_display_line,110,trim(cnvtstring(drsm_res->id[fds_display_loop].timeout)))
      CALL text(drsm_display_line,118,trim(cnvtstring(drsm_res->id[fds_display_loop].time_remaining))
       )
     ELSEIF ((drsm_res->id[fds_display_loop].err_msg_seq=1))
      CALL text(drsm_display_line,11,concat("Description: ",drsm_res->id[fds_display_loop].
        description))
     ELSEIF ((drsm_res->id[fds_display_loop].err_msg_seq > 1))
      IF ((drsm_res->id[fds_display_loop].err_msg_seq=2))
       CALL text(drsm_display_line,11,concat("Error: ",drsm_res->id[fds_display_loop].err_msg))
      ELSE
       CALL text(drsm_display_line,11,drsm_res->id[fds_display_loop].err_msg)
      ENDIF
     ENDIF
     SET drsm_display_line = (drsm_display_line+ 1)
   ENDFOR
   IF (drsm_display_line < 21)
    CALL video(n)
    FOR (fds_display_blank = drsm_display_line TO 21)
      CALL clear(fds_display_blank,1,132)
    ENDFOR
   ENDIF
   CALL video(n)
   IF (size(drsm_res->id,5) > 0)
    IF ((drsm_screen->max_value > drsm_screen->bottom_line)
     AND (drsm_screen->top_line=1))
     CALL clear(24,60,72)
     CALL text(24,60,"More available, use <PgDown> key to scroll list")
    ELSEIF ((drsm_screen->max_value > drsm_screen->bottom_line))
     CALL clear(24,60,72)
    ...
    ENDIF
   ELSE
    CALL text(7,10,"NO RESUMABLE SESSIONS FOUND.")
   ENDIF
 END ;Subroutine
 SUBROUTINE drsm_scroll(ds_rec_str)
   ...
   CALL clear(23,1,132)
   CALL text(23,2,"(I)ncrease Timeout, (A)bort Session, (R)efresh Screen, (Q)uit:")
   WHILE (ds_scroll_accept=" ")
     SET ds_valid_value = 0
     SET accept = time(900)
     CALL accept(23,67,"p;cus","R"
      WHERE curaccept IN ("R", "I", "A", "Q"))
     SET accept = notime
     CASE (curscroll)
      OF 0:
      ...
      OF 1:
       WHILE (ds_valid_value=0)
         ...
         IF ((drsm_res->id[drsm_screen->cur_line].err_msg_seq=0))
          SET ds_valid_value = 1
         ENDIF
         IF (ds_valid_value=1)
 ...
 END ;Subroutine
 SUBROUTINE drsm_load_sessions(null)
   DECLARE dls_cnt = i4 WITH protect, noconstant(0)
   DECLARE dls_err_cnt = i4 WITH protect, noconstant(0)
   DECLARE dls_str_cnt = i2 WITH protect, noconstant(0)
   DECLARE dls_str = vc WITH protect, noconstant("")
   DECLARE dls_email = i4 WITH protect, noconstant(0)
   DECLARE dls_new = i2 WITH protect, noconstant(0)
   SET drsm_screen->top_line = 1
   SET drsm_screen->cur_line = 1
   SET drsm_screen->bottom_line = 0
   SET drsm_screen->max_scroll = 16
   SET drsm_display_line = 6
   SET drsm_row = 0
   SET stat = alterlist(drsm_res->id,0)
   CALL clear(24,1,130)
   CALL text(24,3,"RETRIEVING SESSION INFORMATION. PLEASE WAIT..")
   SET dm_err->eproc = "Loading sessions from DBA_RESUMABLE"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    FROM dba_resumable d,
     gv$session u,
     gv$instance i
    PLAN (d
     WHERE  NOT (d.session_id IN (
     (SELECT
      p.sid
      FROM gv$px_session p
      WHERE p.qcsid != p.sid))))
     JOIN (u
     WHERE d.session_id=u.sid
      AND d.instance_id=u.inst_id)
     JOIN (i
     WHERE d.instance_id=i.inst_id)
    HEAD REPORT
     dls_cnt = 0
    DETAIL
     dls_cnt = (dls_cnt+ 1), stat = alterlist(drsm_res->id,dls_cnt), drsm_res->id[dls_cnt].sid = d
     .session_id,
     drsm_res->id[dls_cnt].username = substring(1,20,u.username), drsm_res->id[dls_cnt].instance =
     substring(1,20,i.instance_name), drsm_res->id[dls_cnt].hostname = substring(1,20,i.host_name),
     drsm_res->id[dls_cnt].status = substring(1,19,d.status), drsm_res->id[dls_cnt].timeout = (d
     .timeout/ 60)
     IF (d.status="SUSPENDED")
      drsm_res->id[dls_cnt].time_remaining = ((d.timeout/ 60) - datetimediff(cnvtdatetime(curdate,
        curtime3),cnvtdatetime(concat(format(cnvtdate2(substring(1,findstring(" ",d.suspend_time),d
            .suspend_time),"MM/DD/YY"),"DD-MMM-YYYY;;D")," ",substring((findstring(" ",d.suspend_time
           )+ 1),8,d.suspend_time))),4))
      IF ((drsm_res->id[dls_cnt].time_remaining < 0))
       drsm_res->id[dls_cnt].time_remaining = 0
      ENDIF
     ELSE
      drsm_res->id[dls_cnt].time_remaining = 0
     ENDIF
     dls_err_cnt = 0, dls_err_cnt = (dls_err_cnt+ 1), dls_cnt = (dls_cnt+ 1),
     stat = alterlist(drsm_res->id,dls_cnt), drsm_res->id[dls_cnt].description = d.name, drsm_res->
     id[dls_cnt].err_msg_seq = dls_err_cnt,
     dls_str_cnt = 0
     WHILE (dls_str_cnt < size(trim(d.error_msg)))
       dls_err_cnt = (dls_err_cnt+ 1), dls_cnt = (dls_cnt+ 1), stat = alterlist(drsm_res->id,dls_cnt),
       dls_str = fillstring(110," "), dls_str = substring(dls_str_cnt,110,trim(d.error_msg)),
       drsm_res->id[dls_cnt].err_msg = dls_str,
       drsm_res->id[dls_cnt].err_msg_seq = dls_err_cnt, dls_str_cnt = (dls_str_cnt+ 110)
     ENDWHILE
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET drsm_screen->max_value = size(drsm_res->id,5)
   CALL clear(24,1,130)
   IF ((dm2_install_schema->process_option="CLIN COPY"))
    ...
    ENDIF
    IF (size(drsm_res->id,5) > 0)
     FOR (dls_email = 1 TO size(drsm_res->id,5))
       IF ((drsm_res->id[dls_email].err_msg_seq=0))
        IF (dls_new=1)
         ...
         SET dls_new = 0
        ENDIF
        IF ((drsm_res->id[dls_email].status="SUSPENDED"))
         SET dls_new = 1
         SET drer_email_det->msgtype = "ACTIONREQ"
         SET drer_email_det->status_dt_tm = cnvtdatetime(curdate,curtime3)
         SET drer_email_det->step = "Oracle Suspended Operations Detected"
         ...
         SET drer_email_det->status = "SUSPENDED"
         CALL drer_add_body_text(concat("Username : ",drsm_res->id[dls_email].username),1)
         CALL drer_add_body_text(concat("Instance : ",drsm_res->id[dls_email].instance),0)
         CALL drer_add_body_text(concat("Hostname : ",drsm_res->id[dls_email].hostname),0)
         CALL drer_add_body_text(concat("Timeout : ",trim(cnvtstring(drsm_res->id[dls_email].timeout)
            )),0)
         CALL drer_add_body_text(concat("Time remaining : ",trim(cnvtstring(drsm_res->id[dls_email].
             time_remaining))),0)
        ENDIF
       ELSEIF ((drsm_res->id[dls_email].err_msg_seq=1))
        CALL drer_add_body_text(concat("Description :",drsm_res->id[dls_email].description),0)
       ELSEIF ((drsm_res->id[dls_email].err_msg_seq=2))
        CALL drer_add_body_text(concat("Error :",drsm_res->id[dls_email].err_msg),0)
       ELSE
        CALL drer_add_body_text(drsm_res->id[dls_email].err_msg,0)
       ENDIF
     ENDFOR
     IF (dls_new=1)
      IF (drer_compose_email(null)=1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drsm_increase(null)
   CALL accept(drsm_row,110,"9(6)",drsm_res->id[drsm_screen->cur_line].timeout
    WHERE (curaccept >= drsm_res->id[drsm_screen->cur_line].timeout))
   SET drsm_res->id[drsm_screen->cur_line].timeout = cnvtint(curaccept)
   SET dm_err->eproc = concat("Setting timeout to ",trim(cnvtstring(drsm_res->id[drsm_screen->
      cur_line].timeout))," minutes.")
   CALL disp_msg("",dm_err->logfile,0)
   IF (dm2_push_cmd(build('rdb asis("begin dbms_resumable.set_session_timeout(',drsm_res->id[
     drsm_screen->cur_line].sid,",",(drsm_res->id[drsm_screen->cur_line].timeout * 60),'); end;") go'
     ),1)=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drsm_abort(null)
   DECLARE da_str = vc WITH protect, noconstant("")
   SET dm_err->eproc = concat("Aborting Session ",trim(cnvtstring(drsm_res->id[drsm_screen->cur_line]
      .sid)),".")
   CALL disp_msg("",dm_err->logfile,0)
   IF ((drsm_res->id[drsm_screen->cur_line].status="SUSPENDED"))
    SET da_str = concat("Are you sure you wish to abort session ",trim(cnvtstring(drsm_res->id[
       drsm_screen->cur_line].sid)),"?  (Y)es or (N)o:")
    CALL clear(24,1,130)
    CALL text(24,3,da_str)
    CALL accept(24,(size(da_str)+ 4),"p;cu","N"
     WHERE curaccept IN ("Y", "N"))
    IF (curaccept="Y")
     SET dm_err->eproc = "Checking for sessions in current instance only"
     IF ((dm_err->debug_flag > 0))
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
     SELECT INTO "nl:"
      FROM v$session s
      WHERE (s.sid=drsm_res->id[drsm_screen->cur_line].sid)
       AND trim(cnvtlower(s.machine))=trim(cnvtlower(drsm_res->id[drsm_screen->cur_line].hostname))
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     IF (curqual=0)
      CALL clear(24,1,130)
      CALL text(24,3,"Selected session is running on another instance. CANNOT abort.")
      CALL pause(3)
     ELSE
      CALL clear(24,1,130)
      CALL text(24,3,"ABORTING SESSION. PLEASE WAIT..")
      IF (dm2_push_cmd(build('rdb asis("begin dbms_resumable.abort(',drsm_res->id[drsm_screen->
        cur_line].sid,'); end;") go'),1)=0)
       RETURN(0)
      ENDIF
      CALL pause(3)
      CALL clear(24,1,130)
     ENDIF
    ELSE
     SET dm_err->eproc = concat("User chose to cancel abort operation.")
     CALL disp_msg("",dm_err->logfile,0)
    ENDIF
   ELSE
    SET da_str = "May only abort sessions in a SUSPENDED status."
    CALL clear(24,1,130)
    CALL text(24,3,da_str)
    CALL pause(3)
   ENDIF
...
#exit_program
 SET message = nowindow
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(drsm_screen)
  CALL echorecord(drsm_res)
 ENDIF
 SET dm_err->eproc = "Leaving dm2_resumable_session_menu"
 CALL final_disp_msg("dm2_res_sess_menu")
END GO
