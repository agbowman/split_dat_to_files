CREATE PROGRAM bsc_updt_med_admin_err_p2:dba
 ...
 DECLARE mn_count = i2 WITH protect, noconstant(0)
 DECLARE mn_object_count = i2 WITH protect, noconstant(0)
 DECLARE errmsg = vc WITH protect, noconstant("")
 SET ms_info_domain_nm = "UPDATE ADMIN ERR - ERROR_ID"
 SET ms_range = "CAT 5 ERROR_ID RANGE"
 SET ms_max_name_p = "MAX ADMIN ERROR_ID EVALUATED"
 SET ms_child_script = "bsc_updt_med_admin_err_cc"
 SET mn_num_children = 3
 SELECT INTO "nl:"
  max_val = max(m.med_admin_med_error_id)
  FROM med_admin_med_error m
  DETAIL
 ...
 SELECT INTO "nl:"
  FROM dba_objects d
  WHERE d.object_name IN ("MED_ADMIN_EVENT_ORV_PROC_1", "MED_ADMIN_EVENT_ORV_DATE_1",
  "MED_ADMIN_EVENT_ORV_VAL_1")
  DETAIL
   mn_count += 1
  WITH nocounter
 ;end select
 IF (error(errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error selecting from the DBA_OBJECTS table:",errmsg)
  GO TO exit_program
 ENDIF
 IF (mn_count != 3)
  EXECUTE dm_readme_include_sql "cer_install:med_admin_event_orv_proc_1.sql"
  SET readme_data->status = dm_sql_reply->status
  IF ((readme_data->status="F"))
   SET readme_data->message = concat(
    "Failed to create the file cer_install:med_admin_event_orv_proc_1.sql:",dm_sql_reply->msg)
   GO TO exit_program
  ENDIF
  EXECUTE dm_readme_include_sql "cer_install:med_admin_event_orv_date_1.sql"
  SET readme_data->status = dm_sql_reply->status
  IF ((readme_data->status="F"))
   SET readme_data->message = concat(
    "Failed to include the file cer_install:med_admin_event_orv_date_1.sql:",dm_sql_reply->msg)
   GO TO exit_program
  ENDIF
  EXECUTE dm_readme_include_sql "cer_install:med_admin_event_orv_val_1.sql"
  SET readme_data->status = dm_sql_reply->status
  IF ((readme_data->status="F"))
   SET readme_data->message = concat(
    "Failed to create the file cer_install:med_admin_event_orv_val_1.sql:",dm_sql_reply->msg)
   GO TO exit_program
  ENDIF
  CALL sbr_check_procedure("MED_ADMIN_EVENT_ORV_DATE_1","FUNCTION")
  CALL sbr_check_procedure("MED_ADMIN_EVENT_ORV_PROC_1","PROCEDURE")
  CALL sbr_check_procedure("MED_ADMIN_EVENT_ORV_VAL_1","FUNCTION")
 ENDIF
 ...
 ELSE
  ...
  IF (error(errmsg,0) != 0)
   SET readme_data->status = "S"
   SET readme_data->message = concat("Readme Successful but unable to drop the function/procedures:",
    errmsg)
   GO TO exit_program
  ENDIF
  IF (mn_object_count=3)
   CALL parser("rdb drop procedure med_admin_event_orv_proc_1 go")
   CALL parser("rdb drop function med_admin_event_orv_date_1 go")
   CALL parser("rdb drop function med_admin_event_orv_val_1 go")
   IF (error(errmsg,0) != 0)
    SET readme_data->status = "S"
    SET readme_data->message = concat("Readme Successful but unable to drop the function/procedure:",
     errmsg)
    GO TO exit_program
   ENDIF
  ENDIF
 ...
 ENDIF
 SUBROUTINE (sbr_check_procedure(ps_name=vc,ps_type=vc) =null)
   EXECUTE dm_readme_include_sql_chk value(ps_name), value(ps_type)
   SET readme_data->status = dm_sql_reply->status
   IF ((readme_data->status="F"))
    SET readme_data->message = concat("Failed to create sql function/procedure: ",ps_name," ",
     dm_sql_reply->msg)
    GO TO exit_program
   ENDIF
 END ;Subroutine
