CREATE PROGRAM dm2_utc_execute_schema:dba
 ...
 SUBROUTINE dm2_get_dbase_name(dgdn_name_out)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dues_logfile_prefix = vc WITH protect, noconstant(" ")
 DECLARE dues_run_id = f8 WITH protect, noconstant(0.00)
 DECLARE dues_run_ctx = i2 WITH protect, noconstant(1)
 DECLARE dues_dbase_name = vc WITH protect, noconstant(" ")
 SET dues_logfile_prefix = "dm2_utc_exec_sch_"
 IF ((dm2_install_schema->curprog="DM2_UTC_CONVERT_MAIN"))
  SET dues_run_ctx = 0
 ENDIF
 IF (check_logfile(dues_logfile_prefix,".log","dm2_utc_execute_schema LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_utc_execute_schema process..."
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = concat("Executing utc schema [",trim(currdbhandle)," application id].")
 CALL disp_msg("",dm_err->logfile,0)
 IF (dues_run_ctx=1)
 ...
 IF ((dm_err->debug_flag > 0))
  ...
  CALL echo(dm_err->asterisk_line)
 ENDIF
 IF (dm2_get_dbase_name(dues_dbase_name)=0)
  GO TO exit_program
 ENDIF
 SET dm2_install_schema->target_dbase_name = cnvtupper(dues_dbase_name)
 SET dues_run_id =  $1
 SELECT INTO "nl:"
  di.info_number
  FROM dm2_admin_dm_info di
 ...
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "A UTC CONVERSION is not currently in process.  No UTC CONVERSION row was found in ADMIN DM_INFO."
  SET dm_err->user_action = "Use DM2_UTC_CONVERT_MAIN to begin the process."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
 ...
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "A UTC CONVERSION is not currently in process.  No schema operation was found in DM2_DDL_OPS."
  SET dm_err->user_action = "Use DM2_UTC_CONVERT_MAIN to begin the process."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (dum_cleanup_stranded_appl_id(null)=0)
  GO TO exit_program
 ENDIF
 IF ((dum_utc_data->uptime_run_id > 0.0)
  AND (dum_utc_data->status != "UPTIME"))
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "An Uptime Data Conversion run_id was received, but the UTC Conversion is not in UPTIME mode."
  SET dm_err->user_action = "Use DM2_UTC_CONVERT_MAIN to verify the current step of the process."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF ((dum_utc_data->downtime_run_id > 0.0)
  AND (dum_utc_data->status != "DOWNTIME"))
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "A Downtime Schema run_id was received, but the UTC Conversion is not in DOWNTIME mode."
  SET dm_err->user_action = "Use DM2_UTC_CONVERT_MAIN to verify the current step of the process."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (dm2_get_appl_status(dum_utc_data->appl_id)="A")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "A main session is already in process.  You cannot start another main session."
  SET dm_err->user_action = "Use DM2_UTC_RUNNER to perform UTC Conversion operations in parallel."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ...
 UPDATE  FROM dm2_admin_dm_info di
  ...
  WITH nocounter
 ;end update
 IF (check_error("Updating application ID for main runner.")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  ROLLBACK
  GO TO exit_program
 ELSE
  COMMIT
 ENDIF
 SET dm2_install_schema->run_id = dues_run_id
 UPDATE  FROM dm2_ddl_ops_log ddol
  SET ddol.ready_to_run_ind = 0, ddol.status = null, ddol.appl_id = currdbhandle
 ...
 ELSE
  COMMIT
 ENDIF
 IF (validate(dm2_utc_process_option,"X")="X")
  SET dm2_install_schema->process_option = "UTC"
 ELSE
  SET dm2_install_schema->process_option = dm2_utc_process_option
 ENDIF
 EXECUTE dm2_utc_set_ready_to_run
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 IF (dum_stop_conversion_runner("PARALLEL")=0)
  GO TO exit_program
 ENDIF
 GO TO exit_program
#exit_program
 ...
 IF (dues_run_ctx=1)
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "dm2_utc_execute_schema COMPLETED"
 CALL final_disp_msg(dues_logfile_prefix)
END GO
