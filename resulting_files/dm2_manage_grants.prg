CREATE PROGRAM dm2_manage_grants
 ...
 SUBROUTINE dpc_get_opt_code(in_mode)
   ...
   RETURN(out_code)
 END ;Subroutine
 DECLARE dmg_logfile_prefix = vc WITH protect, constant("dm2_mng_grants")
 DECLARE dmg_env = vc WITH protect, noconstant("")
 DECLARE dmg_node = vc WITH protect, noconstant("")
 DECLARE dmg_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmg_cont = i2 WITH protect, noconstant(1)
 DECLARE dmg_os_type = vc WITH protect, noconstant("")
 DECLARE dmg_grant_method = vc WITH protect, noconstant("BYSCRIPT")
 FREE RECORD dmg_reminders
 RECORD dmg_reminders(
   1 cnt = i4
   1 reminders[*]
     2 text = vc
     2 order_num = i4
 )
 DECLARE dmg_add_grant(null) = i2
 DECLARE dmg_remove_grant(null) = i2
 DECLARE dmg_load_reminders(null) = i2
 SET width = 132
 IF (check_logfile(dmg_logfile_prefix,".log","dm2_manage_grants")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_manage_grants"
 CALL disp_msg("",dm_err->logfile,0)
 SET dmg_env = trim(logical("environment"))
 SET dmg_node = trim(curnode)
 SET dmg_os_type = trim(cursys2)
 SET dm_err->eproc = "Getting environment_id for current node/env"
 SELECT INTO "nl:"
  FROM dm_environment de
  WHERE de.environment_name=cnvtupper(dmg_env)
  DETAIL
   dmg_env_id = de.environment_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Current Environment: ",dmg_env))
  CALL echo(concat("Current Environment Id: ",cnvtstring(dmg_env_id)))
  CALL echo(concat("Current Node: ",cnvtupper(dmg_node)))
  CALL echo(concat("Current OS Type: ",dmg_os_type))
 ENDIF
 SET dm_err->eproc = "Determining if grant method override row exists in dm_info."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE "DM2_MANAGE_GRANTS"=di.info_domain
   AND "GRANT_METHOD"=di.info_name
  DETAIL
   dmg_grant_method = build(di.info_char)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 WHILE (dmg_cont=1)
   ...
   CALL box(1,1,3,132)
   CALL box(1,1,24,132)
   CALL text(2,2,"MANAGE CCL GRANTS [MAIN]")
   CALL text(2,70,"DATE/TIME: ")
   CALL text(2,80,format(cnvtdatetime(curdate,curtime3),"DD-MMM-YYYY HH:MM:SS;;D"))
   CALL text(4,2,concat("Current Environment:  ",cnvtupper(dmg_env)," (",trim(cnvtstring(dmg_env_id)),
     ")"))
   CALL text(4,70,concat("Current Application Node:  ",cnvtupper(dmg_node)))
   CALL text(9,5,"1. VIEW CCL Grants")
   CALL text(11,5,"2. ADD CCL Grants")
   CALL text(13,5,"3. REMOVE CCL Grants")
   CALL text(17,5,"Your Selection (0 to Exit):")
   CALL accept(17,35,"9;C","0"
    WHERE curaccept IN ("0", "1", "2", "3"))
   SET message = nowindow
   CASE (curaccept)
    OF "1":
     EXECUTE dm2_view_grants "prompt", "*", "ALL"
     IF ((dm_err->err_ind=1))
      GO TO exit_program
     ENDIF
    OF "2":
     IF (dmg_add_grant(null)=0)
      GO TO exit_program
     ENDIF
    OF "3":
     IF (dmg_remove_grant(null)=0)
      GO TO exit_program
     ENDIF
    OF "0":
     SET dmg_cont = 0
 ...
 ENDWHILE
 GO TO exit_program
 SUBROUTINE dmg_add_grant(null)
   DECLARE dag_cont = i4 WITH protect, noconstant(1)
   DECLARE dag_cont2 = i4 WITH protect, noconstant(1)
   DECLARE dag_name = vc WITH protect, noconstant(" ")
   DECLARE dag_email = vc WITH protect, noconstant(" ")
   DECLARE dag_script = vc WITH protect, noconstant(" ")
   DECLARE dag_query_nbr = i4 WITH protect, noconstant(- (1))
   DECLARE dag_reason = vc WITH protect, noconstant(" ")
   DECLARE dag_sr_nbr = vc WITH protect, noconstant(" ")
   DECLARE dag_sql_id = vc WITH protect, noconstant(" ")
   DECLARE dag_dpl_id = f8 WITH protect, noconstant(0.0)
   DECLARE dag_num = i4 WITH protect, noconstant(0)
   DECLARE dag_idx = i4 WITH protect, noconstant(0)
   DECLARE dag_pos = i4 WITH protect, noconstant(0)
   DECLARE dag_cmd = vc WITH protect, noconstant("")
   DECLARE dag_error_ind = i2 WITH protect, noconstant(0)
   DECLARE dag_ignore_ind = i2 WITH protect, noconstant(0)
   DECLARE dag_rdbopt = i4 WITH protect, noconstant(0)
   DECLARE dag_group = i4 WITH protect, noconstant(0)
   DECLARE dag_grp = vc WITH protect, noconstant("")
   SET dag_cont = 1
   WHILE (dag_cont=1)
     SET message = window
     SET width = 132
     ...
     CALL box(1,1,3,132)
     CALL box(1,1,24,132)
     CALL text(2,2,"Manage CCL Grants [ADD GRANT]")
     CALL text(2,70,"DATE/TIME: ")
     CALL text(2,80,format(cnvtdatetime(curdate,curtime3),"DD-MMM-YYYY HH:MM:SS;;D"))
     CALL text(4,2,concat("Current Environment:  ",cnvtupper(dmg_env)," (",trim(cnvtstring(dmg_env_id
         )),")"))
     CALL text(4,70,concat("Current Application Node:  ",cnvtupper(dmg_node)))
     CALL text(7,2,"Your Name: ")
     CALL text(8,2,"Your e-mail address: ")
     CALL text(9,2,"Script Name: ")
     CALL text(9,75,"Group: ")
     IF (dmg_grant_method="BYQUERY")
      CALL text(10,2,"Query Number: ")
     ENDIF
     CALL text(11,2,"Reason for Grant: ")
     CALL text(12,2,"Cerner SR#: ")
     CALL text(13,2,"SQL ID: ")
     CALL text(16,2,"(M)odify Input, (A)pply Grant, (B)ack: ")
     CALL text(20,2,"** Required Field. Enter your name for documentation purposes.")
     CALL accept(7,30,"P(30);CU",dag_name
      WHERE curaccept > " ")
     SET dag_name = curaccept
     CALL clear(20,2,100)
     CALL text(20,2,"** Required Field. Enter your email address for documentation purposes.")
     CALL accept(8,30,"P(30);CU",dag_email
      WHERE curaccept > " ")
     SET dag_email = curaccept
     CALL clear(20,2,100)
     CALL text(20,2,"** Required Field. Enter the script name for which the grant will be applied.")
     CALL accept(9,30,"P(30);CU",dag_script
      WHERE curaccept > " ")
     SET dag_script = trim(curaccept)
     CALL clear(20,2,100)
     CALL text(20,2,
      "** Required Field. Enter the group nbr for the script.0 - DBA (Default) ; 1 - Group01.")
     CALL accept(9,82,"9(1)",dag_group
      WHERE curaccept IN (0, 1))
     SET dag_group = curaccept
     SET dag_grp = evaluate(dag_group,0,"DBA","GROUP01")
     CALL clear(20,2,100)
     SET dag_cont2 = 1
     WHILE (dag_cont2=1)
       SET dm_err->eproc = "Verifying script existence in dprotect."
       SELECT INTO "nl:"
        FROM dprotect d
        WHERE d.object="P"
         AND d.group=dag_group
         AND d.object_name=dag_script
        WITH nocounter
       ;end select
       IF (check_error(dm_err->eproc)=1)
        SET dm_err->err_ind = 1
       ...
       IF (curqual=0)
        CALL text(20,2,
         "** The Script does not exist in the dictionary. Please enter a valid script name and group nbr."
         )
        CALL accept(9,30,"P(30);CU",dag_script
         WHERE curaccept > " ")
        SET dag_script = trim(curaccept)
     ...
     ENDWHILE
     IF (dmg_grant_method="BYQUERY")
      CALL text(20,2,"** Required Field. Enter a specific query number within the script (1 - 50).")
      CALL text(21,2,"** Enter 0 to set the optimizer mode for dynamic queries.")
      CALL text(22,2,"** Enter 51 to set the optimizer mode for queries > 50.")
      CALL text(23,2,"** Enter -1 to set the optimizer mode for the ENTIRE script.")
      CALL accept(10,30,"9(2)",dag_query_nbr
       WHERE (curaccept >= - (1))
        AND curaccept < 52)
      SET dag_query_nbr = curaccept
      CALL clear(20,2,100)
      CALL clear(21,2,100)
      CALL clear(22,2,100)
      CALL clear(23,2,100)
     ENDIF
     CALL text(20,2,"** Required Field. Enter a brief explanation for why this grant is needed.")
     CALL accept(11,30,"P(60);CU",dag_reason
      WHERE curaccept > " ")
     SET dag_reason = curaccept
     CALL clear(20,2,100)
     CALL text(20,2,
      "** Optional Field. Enter the Cerner SR number, if available. Press <Enter> to skip this prompt."
      )
     CALL accept(12,30,"P(30);CU",dag_sr_nbr)
     SET dag_sr_nbr = curaccept
     CALL clear(20,2,100)
     CALL text(20,2,
      "** Optional Field. Enter the SQL ID, if known. Press <Enter> to skip this prompt.")
     CALL accept(13,30,"P(30);CU",dag_sql_id)
     SET dag_sql_id = curaccept
     CALL clear(20,2,100)
     CALL accept(16,43,"A;CU"," "
     ...
     CASE (curaccept)
      OF "M":
       SET dag_cont = 1
      OF "A":
       IF (dpc_get_curr_cclgrants(dag_script)=0)
        RETURN(0)
       ENDIF
       IF ((dm_err->debug_flag > 0))
        CALL echorecord(dpc_data)
       ENDIF
       IF ((dpc_data->grant_cnt > 1))
        SET dag_pos = locateval(dag_idx,1,dpc_data->grant_cnt,dag_group,dpc_data->grants[dag_idx].
         group)
       ELSE
        SET dag_pos = dpc_data->grant_cnt
       ENDIF
       IF (((dmg_grant_method="BYSCRIPT") OR (dmg_grant_method="BYQUERY"
        AND (dag_query_nbr=- (1)))) )
        IF ((dpc_data->grants[dag_pos].rule_grant_ind=1)
         AND (dpc_data->grants[dag_pos].all_equal_ind=1))
         SET dag_ignore_ind = 1
        ENDIF
       ENDIF
       IF (dmg_grant_method="BYQUERY"
        AND (dag_query_nbr != - (1)))
        IF (((dag_query_nbr=51
         AND (dpc_data->grants[dag_pos].rdboptdefault=1)) OR (((dag_query_nbr=0
         AND (dpc_data->grants[dag_pos].dyn_query_rdbopt=1)) OR (dag_query_nbr < 50
         AND (dpc_data->grants[dag_pos].query_grant_value[dag_query_nbr]=1))) )) )
         SET dag_ignore_ind = 1
        ENDIF
       ENDIF
       SET message = window
       ...
       CALL box(1,1,3,132)
       CALL box(1,1,24,132)
       CALL text(2,2,"Manage CCL Grants [ADD GRANT - CONFIRMATION]")
       CALL text(2,70,"DATE/TIME: ")
       CALL text(2,80,format(cnvtdatetime(curdate,curtime3),"DD-MMM-YYYY HH:MM:SS;;D"))
       CALL text(4,2,concat("Current Environment:  ",cnvtupper(dmg_env)," (",trim(cnvtstring(
           dmg_env_id)),")"))
       CALL text(4,70,concat("Current Application Node:  ",cnvtupper(dmg_node)))
       CALL text(7,2,concat("GRANT METHOD: ",dmg_grant_method))
       IF (dag_ignore_ind=1)
        IF (dmg_grant_method="BYQUERY"
         AND (dag_query_nbr != - (1)))
         CALL text(9,2,"A RULE Grant already exists for the following Script and Query: ")
        ELSE
         CALL text(9,2,"A RULE Grant already exists for the following Script: ")
        ENDIF
        CALL text(11,5,concat("SCRIPT: ",dag_script,":",dag_grp))
        IF (dmg_grant_method="BYQUERY")
         CALL text(12,5,concat("QUERY NUMBER: ",build(dag_query_nbr),evaluate(dag_query_nbr,- (1),
            " (Entire Script)","")))
        ENDIF
        CALL text(23,2,"Press <Enter> to Continue.")
        CALL accept(23,30,"p;cu"," "
         WHERE curaccept IN (" "))
       ELSE
        IF (dmg_grant_method="BYQUERY")
         CALL text(9,2,"Please confirm that a RULE grant should be applied to: ")
        ELSE
         CALL text(9,2,"Please confirm that the following script should execute in RULE: ")
        ENDIF
        CALL text(11,5,concat("SCRIPT: ",dag_script," [Group: ",dag_grp,"]"))
        IF (dmg_grant_method="BYQUERY")
         CALL text(12,5,concat("QUERY NUMBER: ",build(dag_query_nbr),evaluate(dag_query_nbr,- (1),
            " (Entire Script)","")))
        ENDIF
        IF ((dmg_reminders->cnt=0))
         IF (dmg_load_reminders(null)=0)
          RETURN(0)
         ENDIF
        ENDIF
        IF ((dmg_reminders->cnt > 0))
         CALL text(14,2,"REMINDERS: ")
         FOR (dag_num = 1 TO least(dmg_reminders->cnt,8))
           CALL text((14+ dag_num),5,build(dag_num,". ",dmg_reminders->reminders[dag_num].text))
         ENDFOR
        ENDIF
        CALL text(23,2,"(C)onfirm,(B)ack: ")
        CALL accept(23,21,"A;CU"," "
       ...
       CASE (curaccept)
        OF "B":
         SET dag_cont = 1
        OF "C":
         SET dag_rdbopt = dpc_data->grants[dag_pos].rdboptdefault
         IF (dpc_format_add_rule_grant(dmg_grant_method,concat(dag_script,":",dag_grp),dag_query_nbr,
          dag_rdbopt,dag_cmd)=0)
          RETURN(0)
         ENDIF
         SET dm_err->eproc = concat("Applying grants for: ",dag_script,":",dag_grp)
         IF (dm2_push_cmd(dag_cmd,1)=0)
          RETURN(0)
         ENDIF
         IF (dpc_get_curr_cclgrants(dag_script)=0)
          RETURN(0)
         ...
         ELSE
          SET dag_pos = dpc_data->grant_cnt
         ENDIF
         IF ((((dpc_data->grants[dag_pos].script_name != dag_script)) OR ((dpc_data->grants[dag_pos].
         rule_grant_ind != 1))) )
          SET dag_error_ind = 1
         ENDIF
         IF (dmg_grant_method="BYQUERY")
          IF (((dag_query_nbr=51
           AND (dpc_data->grants[dag_pos].rdboptdefault != 1)) OR (((dag_query_nbr=0
           AND (dpc_data->grants[dag_pos].dyn_query_rdbopt != 1)) OR (dag_query_nbr BETWEEN 1 AND 50
           AND (dpc_data->grants[dag_pos].query_grant_value[dag_query_nbr] != 1))) )) )
           SET dag_error_ind = 1
          ENDIF
         ENDIF
         IF (dag_error_ind=1)
          SET dm_err->err_ind = 1
          SET dm_err->emsg = concat("Error verifying grants applied for: ",dag_script,":",dag_group)
          CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
          RETURN(0)
         ENDIF
         SET dm2_process_rs->process_name = dpl_ccl_grant
         SET dm2_process_event_rs->status = dpl_complete
         CALL dm2_process_log_add_detail_text(dpl_action,"ADD GRANT")
         CALL dm2_process_log_add_detail_text(dpl_grant_method,dmg_grant_method)
         CALL dm2_process_log_add_detail_text(dpl_script,build(dag_script,":",dag_grp))
         CALL dm2_process_log_add_detail_number(dpl_query,cnvtreal(dag_query_nbr))
         CALL dm2_process_log_add_detail_text(dpl_name,dag_name)
         CALL dm2_process_log_add_detail_text(dpl_email,dag_email)
         CALL dm2_process_log_add_detail_text(dpl_reason,dag_reason)
         CALL dm2_process_log_add_detail_text(dpl_sr_nbr,dag_sr_nbr)
         CALL dm2_process_log_add_detail_text(dpl_sql_id,dag_sql_id)
         IF (dm2_process_log_row(dpl_ccl_grant,dpl_auditlog,dpl_no_prev_id,1)=0)
          RETURN(0)
         ENDIF
         SET dag_dpl_id = dm2_process_event_rs->dm_process_event_id
         CALL dm2_process_log_dtl_row(dag_dpl_id,0)
         SET dag_cont = 0
        ELSE
         SET dag_cont = 0
       ENDCASE
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmg_remove_grant(null)
   DECLARE drg_cont = i4 WITH protect, noconstant(1)
   DECLARE drg_cont2 = i4 WITH protect, noconstant(1)
   DECLARE drg_name = vc WITH protect, noconstant(" ")
   DECLARE drg_email = vc WITH protect, noconstant(" ")
   DECLARE drg_script = vc WITH protect, noconstant(" ")
   DECLARE drg_grp = vc WITH protect, noconstant(" ")
   DECLARE drg_group = i4 WITH protect, noconstant(0)
   DECLARE drg_query_nbr = i4 WITH protect, noconstant(0)
   DECLARE drg_reason = vc WITH protect, noconstant(" ")
   DECLARE drg_sr_nbr = vc WITH protect, noconstant(" ")
   DECLARE drg_sql_id = vc WITH protect, noconstant(" ")
   DECLARE drg_dpl_id = f8 WITH protect, noconstant(0.0)
   DECLARE drg_num = i4 WITH protect, noconstant(0)
   DECLARE drg_idx = i4 WITH protect, noconstant(0)
   DECLARE drg_pos = i4 WITH protect, noconstant(0)
   DECLARE drg_cmd = vc WITH protect, noconstant("")
   DECLARE drg_error_ind = i2 WITH protect, noconstant(0)
   SET drg_cont = 1
   WHILE (drg_cont=1)
     SET message = window
     SET width = 132
     ...
     CALL box(1,1,3,132)
     CALL box(1,1,24,132)
     CALL text(2,2,"Manage CCL Grants [REMOVE GRANT]")
     CALL text(2,70,"DATE/TIME: ")
     CALL text(2,80,format(cnvtdatetime(curdate,curtime3),"DD-MMM-YYYY HH:MM:SS;;D"))
     CALL text(4,2,concat("Current Environment:  ",cnvtupper(dmg_env)," (",trim(cnvtstring(dmg_env_id
         )),")"))
     ...
     CALL text(9,2,"Script Name: ")
     CALL text(9,75,"Group: ")
     CALL text(11,2,"Reason for Removal: ")
     CALL text(12,2,"Cerner SR#: ")
     CALL text(16,2,"(M)odify Input, (R)emove Grant, (B)ack: ")
     CALL text(20,2,"** Required Field. Enter your name for documentation purposes.")
     CALL accept(7,30,"P(30);CU",drg_name
      WHERE curaccept > " ")
     SET drg_name = curaccept
     CALL clear(20,2,100)
     CALL text(20,2,"** Required Field. Enter your email address for documentation purposes.")
     CALL accept(8,30,"P(30);CU",drg_email
      WHERE curaccept > " ")
     SET drg_email = curaccept
     CALL clear(20,2,100)
     CALL text(20,2,"** Required Field. Enter the script name for which the grant will be applied.")
     CALL accept(9,30,"P(30);CU",drg_script
      WHERE curaccept > " ")
     SET drg_script = trim(curaccept)
     CALL clear(20,2,100)
     CALL text(20,2,
      "** Required Field. Enter the group nbr for the script.0 - DBA (Default) ; 1 - Group01.")
     CALL accept(9,82,"9(1)",drg_group
      WHERE curaccept IN (0, 1))
     SET drg_group = curaccept
     SET drg_grp = evaluate(drg_group,0,"DBA","GROUP01")
     CALL clear(20,2,100)
     SET drg_cont2 = 1
     WHILE (drg_cont2=1)
       SET dm_err->eproc = "Verifying script existence in dprotect."
       SELECT INTO "nl:"
        FROM dprotect d
        WHERE d.object="P"
         AND d.group=drg_group
         AND d.object_name=drg_script
        WITH nocounter
       ;end select
       IF (check_error(dm_err->eproc)=1)
        SET dm_err->err_ind = 1
     ...
     ENDWHILE
     CALL text(20,2,"** Required Field. Enter a brief explanation for why this grant is needed.")
     CALL accept(11,30,"P(60);CU",drg_reason
      WHERE curaccept > " ")
     SET drg_reason = curaccept
     CALL clear(20,2,100)
     CALL text(20,2,
      "** Optional Field. Enter the Cerner SR number, if available.Press <Enter> to skip this prompt."
      )
     CALL accept(12,30,"P(30);CU",drg_sr_nbr)
     SET drg_sr_nbr = curaccept
     CALL clear(20,2,100)
     CALL accept(16,43,"A;CU"," "
      WHERE curaccept IN ("M", "R", "B"))
     SET message = nowindow
     CASE (curaccept)
      OF "M":
       SET drg_cont = 1
      OF "R":
       ...
       CALL box(1,1,3,132)
       CALL box(1,1,24,132)
       CALL text(2,2,"Manage CCL Grants [REMOVE GRANT - CONFIRMATION]")
       CALL text(2,70,"DATE/TIME: ")
       CALL text(2,80,format(cnvtdatetime(curdate,curtime3),"DD-MMM-YYYY HH:MM:SS;;D"))
       CALL text(4,2,concat("Current Environment:  ",cnvtupper(dmg_env)," (",trim(cnvtstring(
           dmg_env_id)),")"))
       CALL text(4,70,concat("Current Application Node:  ",cnvtupper(dmg_node)))
       CALL text(7,2,
        "Please confirm that all CCL grants for the following script should be removed: ")
       CALL text(9,5,concat("SCRIPT: ",drg_script," [Group: ",drg_grp,"]"))
       IF ((dmg_reminders->cnt=0))
        IF (dmg_load_reminders(null)=0)
         RETURN(0)
        ENDIF
       ...
       IF ((dmg_reminders->cnt > 0))
        CALL text(14,2,"REMINDERS: ")
        FOR (drg_num = 1 TO least(dmg_reminders->cnt,8))
          CALL text((14+ drg_num),5,build(drg_num,". ",dmg_reminders->reminders[drg_num].text))
        ENDFOR
       ENDIF
       CALL text(23,2,"(C)onfirm,(B)ack: ")
       CALL accept(23,21,"A;CU"," "
       ...
       CASE (curaccept)
        OF "B":
         SET drg_cont = 1
        OF "C":
         SET drg_cmd = concat("grant execute on ",drg_script,":",drg_grp," to all go")
         SET dm_err->eproc = concat("Removing grants for: ",drg_script,":",drg_grp)
         IF (dm2_push_cmd(drg_cmd,1)=0)
          RETURN(0)
         ENDIF
         IF (dpc_get_curr_cclgrants(drg_script)=0)
          RETURN(0)
         ENDIF
         IF ((dm_err->debug_flag > 0))
          CALL echorecord(dpc_data)
         ENDIF
         IF ((dpc_data->grant_cnt > 1))
          SET drg_pos = locateval(drg_idx,1,dpc_data->grant_cnt,drg_group,dpc_data->grants[drg_idx].
           group)
         ELSE
          SET drg_pos = dpc_data->grant_cnt
         ENDIF
         IF ((dpc_data->grants[drg_pos].script_name != drg_script))
          SET drg_error_ind = 1
         ENDIF
         IF ((dpc_data->grants[drg_pos].rule_grant_ind=1))
          SET drg_error_ind = 1
         ENDIF
         IF (drg_error_ind=1)
          SET dm_err->err_ind = 1
          SET dm_err->emsg = concat("Error verifying grants removed for: ",drg_script,":",drg_grp)
          CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
          RETURN(0)
         ENDIF
         SET dm_err->eproc = "Removing Grant from DM_INFO current grants listing"
         DELETE  FROM dm_info d
          WHERE info_domain="CCL_GRANTS_CURRENT"
           AND info_name=substring(1,255,concat(drg_script,":",drg_grp,"::",cnvtupper(dmg_node)))
          WITH nocounter
         ;end delete
         IF (check_error(dm_err->eproc)=1)
          ROLLBACK
         ...
         SET dm2_process_rs->process_name = dpl_ccl_grant
         SET dm2_process_event_rs->status = dpl_complete
         CALL dm2_process_log_add_detail_text(dpl_action,"REMOVE GRANT")
         CALL dm2_process_log_add_detail_text(dpl_script,build(drg_script,":",drg_grp))
         CALL dm2_process_log_add_detail_text(dpl_name,drg_name)
         CALL dm2_process_log_add_detail_text(dpl_email,drg_email)
         CALL dm2_process_log_add_detail_text(dpl_reason,drg_reason)
         CALL dm2_process_log_add_detail_text(dpl_sr_nbr,drg_sr_nbr)
         IF (dm2_process_log_row(dpl_ccl_grant,dpl_auditlog,dpl_no_prev_id,1)=0)
          RETURN(0)
         ENDIF
         SET drg_dpl_id = dm2_process_event_rs->dm_process_event_id
         CALL dm2_process_log_dtl_row(drg_dpl_id,0)
         SET drg_cont = 0
       ENDCASE
      OF "B":
       SET drg_cont = 0
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmg_load_reminders(null)
   SET dm_err->eproc = "Load grant reminders from dm_info"
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain="DM2_MANAGE_GRANTS_REMINDERS"
    ORDER BY d.info_number
    HEAD REPORT
     dmg_reminders->cnt = 0, stat = alterlist(dmg_reminders->reminders,dmg_reminders->cnt)
    DETAIL
     dmg_reminders->cnt = (dmg_reminders->cnt+ 1), stat = alterlist(dmg_reminders->reminders,
      dmg_reminders->cnt), dmg_reminders->reminders[dmg_reminders->cnt].text = substring(1,125,trim(d
       .info_char)),
     dmg_reminders->reminders[dmg_reminders->cnt].order_num = d.info_number
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
   ...
   ENDIF
   IF ((dmg_reminders->cnt=0))
    SET dmg_reminders->cnt = (dmg_reminders->cnt+ 2)
    SET stat = alterlist(dmg_reminders->reminders,dmg_reminders->cnt)
    SET dmg_reminders->reminders[1].text =
    "This same action should be performed on all Application nodes"
    SET dmg_reminders->reminders[1].order_num = 1
    SET dmg_reminders->reminders[2].text = concat(
     "Millennium Servers associated with the CCL script should be cycled",
     " to ensure the grant will take effect")
    SET dmg_reminders->reminders[2].order_num = 2
   ENDIF
   RETURN(1)
 END ;Subroutine
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "Ending dm2_manage_grants"
 CALL final_disp_msg(dmg_logfile_prefix)
END GO
