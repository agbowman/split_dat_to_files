CREATE PROGRAM cp_retr_triggering_criteria:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Pathway ID" = 0.0,
  "isCarePathwaysBuildTool" = 0.0
  WITH outdev, pathway_id, is_cp_build_tool
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 DECLARE PUBLIC::verifypathwayid(pathwayid=f8) = null
 SUBROUTINE verifypathwayid(pathwayid)
   DECLARE is_pathway_id_valid = i2 WITH noconstant(0), protect
   DECLARE pathway_active_status_cd = f8 WITH noconstant(0.0), protect
   CALL log_message("In verifyPathwayId()",log_level_debug)
   SET stat = uar_get_meaning_by_codeset(4003198,"ACTIVE",1,pathway_active_status_cd)
   IF (pathwayid > 0.0
    AND pathway_active_status_cd > 0.0)
    SELECT INTO "nl:"
     FROM cp_pathway cp
     PLAN (cp
      WHERE cp.cp_pathway_id=pathwayid
       AND cp.pathway_status_cd=pathway_active_status_cd)
     HEAD cp.cp_pathway_id
      is_pathway_id_valid = 1
     WITH nocounter
    ;end select
   ENDIF
   RETURN(is_pathway_id_valid)
 END ;Subroutine
 DECLARE script_start_curtime3 = dq8 WITH constant(curtime3), private
 DECLARE code_value_mean = vc WITH constant("CODE_VALUE"), protect
 DECLARE reason_for_visit_mean = vc WITH constant("REASON_FOR_VISIT"), protect
 DECLARE this_visit_problem_mean = vc WITH constant("THIS_VISIT_PROBLEM"), protect
 DECLARE cp_pathway_id = f8 WITH noconstant(cnvtreal( $PATHWAY_ID)), protect
 DECLARE is_cp_build_tool = i2 WITH noconstant(cnvtreal( $IS_CP_BUILD_TOOL)), protect
 DECLARE main(null) = null
 DECLARE gettriggeringcriteria(null) = null
 DECLARE gettriggeringcriteriadisp(null) = null
 DECLARE getdisplayusingsourceidentifier(null) = null
 DECLARE getdisplayusingconceptcki(null) = null
 DECLARE getdisplayusingnomenid(null) = null
 DECLARE getterminologyusingconceptcki(null) = null
 DECLARE getterminologyusingnomenid(null) = null
 DECLARE gettriggeringcriteriadisplayforontology = null
 DECLARE gettriggeringcriteriadisplayforordersentence = null
 DECLARE gettriggeringcriteriadisplayforcodevalue = null
 DECLARE getnomenclatureidforthisvisitproblem(null) = null
 IF ( NOT (validate(triggering_criteria_reply)))
 ...
 SUBROUTINE main(null)
   DECLARE sub_start_curtime3 = dq8 WITH private, constant(curtime3)
   CALL log_message("In main()",log_level_debug)
   DECLARE table_access = i4 WITH noconstant(checkdic("CP_TRIGGERING_CRITERIA","T",0)), protect
   DECLARE valid_pathway_id = i2 WITH noconstant(verifypathwayid(cp_pathway_id)), protect
   IF (table_access=2)
    IF (valid_pathway_id=1)
     CALL gettriggeringcriteria(null)
     CALL gettriggeringcriteriadisp(null)
     CALL gettriggeringcriteriaterminology(null)
     IF (is_cp_build_tool=1)
      CALL getnomenclatureidforthisvisitproblem(null)
     ENDIF
     CALL log_message(build("Exit main(), Elapsed time in seconds:",((curtime3 - sub_start_curtime3)
       / 100.0)),log_level_debug)
    ELSE
     CALL log_message("ERROR: Invalid Pathway ID",log_level_debug)
     GO TO exit_script
    ENDIF
   ELSE
    IF (table_access=0)
     CALL log_message("ERROR: CP_TRIGGERING_CRITERIA table does not exist.",log_level_debug)
     GO TO exit_script
    ELSEIF (table_access=1)
     CALL log_message("ERROR: user does not have access to CP_TRIGGERING_CRITERIA table.",
      log_level_debug)
     GO TO exit_script
    ENDIF
   ENDIF
 END ;Subroutine
 SUBROUTINE getnomenclatureidforthisvisitproblem(null)
   DECLARE sub_start_curtime3 = dq8 WITH private, constant(curtime3)
   DECLARE idx = i4 WITH noconstant(0), protect
   DECLARE pos = i4 WITH noconstant(0), protect
   CALL log_message("In getNomenclatureIdForthisVisitProblem()",log_level_debug)
   SELECT INTO "nl:"
    FROM nomenclature n
    PLAN (n
     WHERE expand(idx,1,triggering_criteria_reply->cnt,this_visit_problem_mean,
      triggering_criteria_reply->qual[idx].criteria_type_mean,
      n.concept_cki,triggering_criteria_reply->qual[idx].triggering_criteria_ident))
    DETAIL
     pos = locateval(idx,1,triggering_criteria_reply->cnt,this_visit_problem_mean,
      triggering_criteria_reply->qual[idx].criteria_type_mean,
      n.concept_cki,triggering_criteria_reply->qual[idx].triggering_criteria_ident)
     IF (pos > 0)
      triggering_criteria_reply->qual[pos].triggering_entity_id = n.nomenclature_id,
      triggering_criteria_reply->qual[pos].triggering_entity_name = "NOMENCLATURE"
     ENDIF
    WITH nocounter, expand = 1
   ;end select
   CALL log_message(build("Exit getNomenclatureIdForthisVisitProblem(), Elapsed time in seconds:",((
     curtime3 - sub_start_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE gettriggeringcriteria(null)
   DECLARE sub_start_curtime3 = dq8 WITH private, constant(curtime3)
   DECLARE triggering_criteria_cnt = i4 WITH noconstant(0), protect
   CALL log_message("In getTriggeringCriteria()",log_level_debug)
   SELECT INTO "nl:"
    FROM cp_triggering_criteria ctc
    PLAN (ctc
     WHERE ctc.cp_pathway_id=cp_pathway_id)
    DETAIL
     triggering_criteria_cnt += 1
     IF (triggering_criteria_cnt > size(triggering_criteria_reply->qual,5))
      stat = alterlist(triggering_criteria_reply->qual,(triggering_criteria_cnt+ 10))
     ENDIF
     triggering_criteria_reply->qual[triggering_criteria_cnt].criteria_type_mean = ctc
     .criteria_type_mean, triggering_criteria_reply->qual[triggering_criteria_cnt].
     triggering_criteria_ident = ctc.triggering_criteria_ident, triggering_criteria_reply->qual[
     triggering_criteria_cnt].triggering_entity_id = ctc.triggering_entity_id,
     triggering_criteria_reply->qual[triggering_criteria_cnt].triggering_entity_name = ctc
     .triggering_entity_name, triggering_criteria_reply->qual[triggering_criteria_cnt].
     triggering_criteria_group_mean = ctc.triggering_criteria_group_mean
    FOOT REPORT
     stat = alterlist(triggering_criteria_reply->qual,triggering_criteria_cnt),
     triggering_criteria_reply->cnt = triggering_criteria_cnt
    WITH nocounter
   ;end select
   CALL log_message(build("Exit getTriggeringCriteria(), Elapsed time in seconds:",((curtime3 -
     sub_start_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE gettriggeringcriteriadisp(null)
   DECLARE sub_start_curtime3 = dq8 WITH private, constant(curtime3)
   DECLARE triggering_criteria_disp = i4 WITH noconstant(0), protect
   DECLARE idx = i4 WITH noconstant(0), protect
   DECLARE pos = i4 WITH noconstant(0), protect
   DECLARE table_access = i4 WITH noconstant(checkdic("DO_CONCEPT","T",0)), protect
   CALL log_message("In getTriggeringCriteriaDisp()",log_level_debug)
   IF (is_cp_build_tool=1)
    CALL getdisplayusingconceptcki(null)
   ELSE
    CALL getdisplayusingnomenid(null)
   ENDIF
   SELECT INTO "nl:"
    FROM cmt_concept cc
    PLAN (cc
     WHERE expand(idx,1,triggering_criteria_reply->cnt,this_visit_problem_mean,
      triggering_criteria_reply->qual[idx].criteria_type_mean,
      cc.concept_cki,triggering_criteria_reply->qual[idx].triggering_criteria_ident))
    DETAIL
     pos = locateval(idx,1,triggering_criteria_reply->cnt,this_visit_problem_mean,
      triggering_criteria_reply->qual[idx].criteria_type_mean,
      cc.concept_cki,triggering_criteria_reply->qual[idx].triggering_criteria_ident)
     IF (pos > 0)
      triggering_criteria_reply->qual[pos].triggering_criteria_disp = cc.concept_name
     ENDIF
    WITH nocounter, expand = 1
   ;end select
   IF (table_access=2)
    CALL gettriggeringcriteriadisplayforontology(null)
   ENDIF
   CALL gettriggeringcriteriadisplayforordersentence(null)
   CALL gettriggeringcriteriadisplayforcodevalue(null)
   CALL log_message(build("Exit getTriggeringCriteriaDisp(), Elapsed time in seconds:",((curtime3 -
     sub_start_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE gettriggeringcriteriadisplayforontology(null)
   DECLARE sub_start_curtime3 = dq8 WITH private, constant(curtime3)
   DECLARE triggering_criteria_disp = i4 WITH noconstant(0), protect
   DECLARE idx = i4 WITH noconstant(0), protect
   DECLARE pos = i4 WITH noconstant(0), protect
   CALL log_message("In getTriggeringCriteriaDisplayForOntology()",log_level_debug)
   SELECT INTO "nl:"
    FROM do_concept dc
    PLAN (dc
     WHERE expand(idx,1,triggering_criteria_reply->cnt,"DISCERN_ONTOLOGY_CONDITION",
      triggering_criteria_reply->qual[idx].criteria_type_mean,
      dc.do_concept_name,triggering_criteria_reply->qual[idx].triggering_criteria_ident))
    DETAIL
     pos = locateval(idx,1,triggering_criteria_reply->cnt,"DISCERN_ONTOLOGY_CONDITION",
      triggering_criteria_reply->qual[idx].criteria_type_mean,
      dc.do_concept_name,triggering_criteria_reply->qual[idx].triggering_criteria_ident)
     IF (pos > 0)
      triggering_criteria_reply->qual[pos].triggering_criteria_disp = dc.do_concept_name
     ENDIF
    WITH nocounter, expand = 1
   ;end select
   CALL log_message(build("Exit getTriggeringCriteriaDisplayForOntology(), Elapsed time in seconds:",
     ((curtime3 - sub_start_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE gettriggeringcriteriadisplayforordersentence(null)
   DECLARE sub_start_curtime3 = dq8 WITH private, constant(curtime3)
   DECLARE triggering_criteria_disp = i4 WITH noconstant(0), protect
   DECLARE idx = i4 WITH noconstant(0), protect
   DECLARE pos = i4 WITH noconstant(0), protect
   CALL log_message("In getTriggeringCriteriaDisplayForOrderSentence()",log_level_debug)
   SELECT INTO "nl:"
    FROM order_sentence os,
     order_catalog_synonym ocs
    PLAN (os
     WHERE expand(idx,1,triggering_criteria_reply->cnt,"ORDER_SENTENCE",triggering_criteria_reply->
      qual[idx].triggering_entity_name,
      os.order_sentence_id,triggering_criteria_reply->qual[idx].triggering_entity_id))
     JOIN (ocs
     WHERE ocs.synonym_id=os.parent_entity_id)
    DETAIL
     pos = locateval(idx,1,triggering_criteria_reply->cnt,"ORDER_SENTENCE",triggering_criteria_reply
      ->qual[idx].triggering_entity_name,
      os.order_sentence_id,triggering_criteria_reply->qual[idx].triggering_entity_id)
     IF (pos > 0)
      triggering_criteria_reply->qual[pos].triggering_criteria_disp = ocs.mnemonic
     ENDIF
    WITH nocounter, expand = 1
   ;end select
   CALL log_message(build(
     "Exit getTriggeringCriteriaDisplayForOrderSentence(), Elapsed time in seconds:",((curtime3 -
     sub_start_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE gettriggeringcriteriadisplayforcodevalue(null)
   DECLARE sub_start_curtime3 = dq8 WITH private, constant(curtime3)
   DECLARE idx = i4 WITH noconstant(0), protect
   DECLARE pos = i4 WITH noconstant(0), protect
   CALL log_message("In getTriggeringCriteriaDisplayForCodeValue()",log_level_debug)
   SELECT INTO "nl:"
    FROM code_value cv
    PLAN (cv
     WHERE expand(idx,1,triggering_criteria_reply->cnt,code_value_mean,triggering_criteria_reply->
      qual[idx].triggering_entity_name,
      cv.code_value,triggering_criteria_reply->qual[idx].triggering_entity_id))
    DETAIL
     pos = locateval(idx,1,triggering_criteria_reply->cnt,code_value_mean,triggering_criteria_reply->
      qual[idx].triggering_entity_name,
      cv.code_value,triggering_criteria_reply->qual[idx].triggering_entity_id)
     IF (pos > 0)
      triggering_criteria_reply->qual[pos].triggering_criteria_disp = cv.display
     ENDIF
    WITH nocounter, expand = 1
   ;end select
   CALL log_message(build("Exit getTriggeringCriteriaDisplayForCodeValue(), Elapsed time in seconds:",
     ((curtime3 - sub_start_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE gettriggeringcriteriaterminology(null)
   DECLARE sub_start_curtime3 = dq8 WITH private, constant(curtime3)
   DECLARE idx = i4 WITH noconstant(0), protect
   DECLARE pos = i4 WITH noconstant(0), protect
   CALL log_message("In getTriggeringCriteriaTerminology()",log_level_debug)
   IF (is_cp_build_tool=1)
    CALL getterminologyusingconceptcki(null)
   ELSE
    CALL getterminologyusingnomenid(null)
   ENDIF
   SELECT INTO "nl:"
    FROM nomenclature n,
     code_value cv
    ...
    DETAIL
     pos = locateval(idx,1,triggering_criteria_reply->cnt,this_visit_problem_mean,
      ...
      n.concept_cki,triggering_criteria_reply->qual[idx].triggering_criteria_ident)
     IF (pos > 0)
      triggering_criteria_reply->qual[pos].triggering_criteria_terminology = cv.display
     ENDIF
    WITH nocounter, expand = 1
   ;end select
   CALL log_message(build("Exit getTriggeringCriteriaTerminology(), Elapsed time in seconds:",((
     curtime3 - sub_start_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getterminologyusingnomenid(null)
   DECLARE sub_start_curtime3 = dq8 WITH private, constant(curtime3)
   CALL log_message("In getTerminologyUsingNomenId()",log_level_debug)
   SELECT INTO "nl:"
    FROM nomenclature n,
     code_value cv
    PLAN (n
     WHERE expand(idx,1,triggering_criteria_reply->cnt,reason_for_visit_mean,
      triggering_criteria_reply->qual[idx].criteria_type_mean,
      n.nomenclature_id,triggering_criteria_reply->qual[idx].triggering_entity_id))
     JOIN (cv
     WHERE cv.code_value=n.source_vocabulary_cd)
    DETAIL
     pos = locateval(idx,1,triggering_criteria_reply->cnt,reason_for_visit_mean,
      triggering_criteria_reply->qual[idx].criteria_type_mean,
      n.nomenclature_id,triggering_criteria_reply->qual[idx].triggering_entity_id)
     IF (pos > 0)
      triggering_criteria_reply->qual[pos].triggering_criteria_terminology = cv.display
    ...
    WITH nocounter, expand = 1
   ;end select
   CALL log_message(build("Exit getTerminologyUsingNomenId(), Elapsed time in seconds:",((curtime3 -
     sub_start_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getterminologyusingconceptcki(null)
   DECLARE sub_start_curtime3 = dq8 WITH private, constant(curtime3)
   CALL log_message("In getTerminologyUsingConceptCKI()",log_level_debug)
   SELECT INTO "nl:"
    FROM nomenclature n,
     code_value cv
    PLAN (n
    ...
    WITH nocounter, expand = 1
   ;end select
   CALL log_message(build("Exit getTerminologyUsingConceptCKI(), Elapsed time in seconds:",((curtime3
      - sub_start_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getdisplayusingnomenid(null)
   DECLARE sub_start_curtime3 = dq8 WITH private, constant(curtime3)
   CALL log_message("In getDisplayUsingNomenId()",log_level_debug)
   SELECT INTO "nl:"
    FROM nomenclature n
    PLAN (n
     WHERE expand(idx,1,triggering_criteria_reply->cnt,reason_for_visit_mean,
    ...
    DETAIL
     pos = locateval(idx,1,triggering_criteria_reply->cnt,reason_for_visit_mean,
      ...
      n.nomenclature_id,triggering_criteria_reply->qual[idx].triggering_entity_id)
     IF (pos > 0)
      triggering_criteria_reply->qual[pos].triggering_criteria_disp = n.source_string
     ENDIF
    WITH nocounter, expand = 1
   ;end select
   CALL log_message(build("Exit getDisplayUsingNomenId(), Elapsed time in seconds:",((curtime3 -
     sub_start_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getdisplayusingconceptcki(null)
   DECLARE sub_start_curtime3 = dq8 WITH private, constant(curtime3)
   CALL log_message("In getDisplayUsingConceptCKI()",log_level_debug)
   SELECT INTO "nl:"
    FROM nomenclature n
    PLAN (n
     WHERE expand(idx,1,triggering_criteria_reply->cnt,reason_for_visit_mean,
    ...
    WITH nocounter, expand = 1
   ;end select
   CALL getdisplayusingsourceidentifier(null)
   CALL log_message(build("Exit getDisplayUsingConceptCKI(), Elapsed time in seconds:",((curtime3 -
     sub_start_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getdisplayusingsourceidentifier(null)
   DECLARE sub_start_curtime3 = dq8 WITH private, constant(curtime3)
   CALL log_message("In getDisplayUsingSourceIdentifier()",log_level_debug)
   SELECT INTO "nl:"
    FROM nomenclature n
    PLAN (n
     WHERE expand(idx,1,triggering_criteria_reply->cnt,reason_for_visit_mean,
      triggering_criteria_reply->qual[idx].criteria_type_mean,
      n.source_identifier,triggering_criteria_reply->qual[idx].triggering_criteria_group_mean,n
      .concept_cki,triggering_criteria_reply->qual[idx].triggering_criteria_ident))
    DETAIL
     pos = locateval(idx,1,triggering_criteria_reply->cnt,reason_for_visit_mean,
      triggering_criteria_reply->qual[idx].criteria_type_mean,
      n.source_identifier,triggering_criteria_reply->qual[idx].triggering_criteria_group_mean)
     IF (pos > 0)
      triggering_criteria_reply->qual[pos].triggering_criteria_disp = n.source_string
     ENDIF
    WITH nocounter, expand = 1
   ;end select
   CALL log_message(build("Exit getDisplayUsingSourceIdentifier(), Elapsed time in seconds:",((
     curtime3 - sub_start_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 CALL log_message(build("Starting Script:",log_program_name),log_level_debug)
 SET triggering_criteria_reply->status_data.status = "F"
 CALL main(null)
 SET triggering_criteria_reply->status_data.status = "S"
#exit_script
 ...
 CALL log_message(build("Total time in seconds:",((curtime3 - script_start_curtime3)/ 100.0)),
  log_level_debug)
 IF (( $OUTDEV != "NOFORMS"))
  CALL putjsonrecordtofile(triggering_criteria_reply)
 ENDIF
END GO
