CREATE PROGRAM bii_wlfw_facilityfilter
 PROMPT
  "Output to File/Printer/MINE" = "MINE"
  WITH outdev, inputjson
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 IF ( NOT (validate(nurse_unit_filter)))
  RECORD nurse_unit_filter(
    1 pt_list[*]
      2 patientid = f8
      2 encounterid = f8
      2 ppr = f8
 ...
 DECLARE deleted_var = f8 WITH constant(uar_get_code_by("MEANING",48,"DELETED")), protect
 DECLARE loadbasepts(null) = null WITH protect
 DECLARE getpatientproviderreltns(null) = null WITH protect
 CALL loadbasepts(null)
 CALL getpatientproviderreltns(null)
 SUBROUTINE loadbasepts(null)
   SET stat = cnvtjsontorec(request->blob_in)
   DECLARE num = i4 WITH noconstant(0), protect
   DECLARE rm_cnt = i4 WITH noconstant(0), protect
   DECLARE vt_cnt = i4 WITH noconstant(0), protect
   DECLARE pos = i4 WITH noconstant(0), protect
   DECLARE room_val_parser = vc WITH noconstant(""), protect
   DECLARE disp_room = vc WITH noconstant(""), protect
   DECLARE bed_cnt = i4 WITH noconstant(0), protect
   SET stat = alterlist(nurse_unit_filter->facilities,size(patient_rec->facilities,5))
   FOR (x = 1 TO size(patient_rec->facilities,5))
     SET nurse_unit_filter->facilities[x].facilityvalues = uar_get_code_display(patient_rec->
      facilities[x].facilitycd)
   ENDFOR
   SET stat = alterlist(nurse_unit_filter->units,size(patient_rec->units,5))
   FOR (x = 1 TO size(patient_rec->units,5))
     SET nurse_unit_filter->units[x].unitvalues = uar_get_code_display(patient_rec->units[x].unitcd)
   ENDFOR
   IF ((patient_rec->beds != ""))
    SET room_val_parser = build("disp_room In(",trim(patient_rec->beds),")")
   ELSE
    SET room_val_parser = "0"
   ENDIF
   SELECT INTO "NL:"
    l_location_group_type_disp = uar_get_code_display(l.location_group_type_cd), l
    .location_group_type_cd, n_loc_building_disp = uar_get_code_display(n.loc_building_cd),
    n_loc_facility_disp = uar_get_code_display(n.loc_facility_cd), r_loc_nurse_unit_disp =
    uar_get_code_display(r.loc_nurse_unit_cd), b_loc_room_disp = uar_get_code_display(b.loc_room_cd),
    b_location_disp = uar_get_code_display(b.location_cd)
    FROM location_group l,
     room r,
     nurse_unit n,
     bed b
    PLAN (l
     WHERE l.location_group_type_cd=nurseunit_var
      AND l.active_ind=1
      AND l.beg_effective_dt_tm <= cnvtdatetime(sysdate)
      AND l.end_effective_dt_tm >= cnvtdatetime(sysdate)
      AND l.active_status_cd != deleted_var
      AND l.sequence=1
      AND expand(num,1,size(patient_rec->units,5),l.parent_loc_cd,patient_rec->units[num].unitcd))
     JOIN (n
     WHERE n.location_cd=l.parent_loc_cd
      AND n.active_ind=1
      AND n.beg_effective_dt_tm <= cnvtdatetime(sysdate)
      AND n.end_effective_dt_tm >= cnvtdatetime(sysdate))
     JOIN (r
     WHERE r.loc_nurse_unit_cd=n.location_cd
      AND r.active_ind=1
    ...
    ORDER BY r_loc_nurse_unit_disp, b_loc_room_disp, b_location_disp
    HEAD REPORT
     rm_cnt = 0, stat = alterlist(rooms->rm_list,100)
    HEAD b_loc_room_disp
     bed_cnt = 0
    DETAIL
     IF ((patient_rec->beds != ""))
      disp_room = uar_get_code_display(b.loc_room_cd)
      IF (parser(room_val_parser))
       rm_cnt += 1, bed_cnt += 1
       IF (mod(rm_cnt,100)=1
        AND rm_cnt > 100)
        stat = alterlist(rooms->rm_list,(rm_cnt+ 99))
       ENDIF
       rooms->rm_list[rm_cnt].pt_bed = b.location_cd, rooms->rm_list[rm_cnt].room_bed = build2(trim(
         b_loc_room_disp),"-",trim(b_location_disp))
      ENDIF
     ELSE
      rm_cnt += 1, bed_cnt += 1
      IF (mod(rm_cnt,100)=1
      ...
      rooms->rm_list[rm_cnt].pt_bed = b.location_cd, rooms->rm_list[rm_cnt].room_bed = build2(trim(
        b_loc_room_disp),"-",trim(b_location_disp))
     ENDIF
    FOOT  b_loc_room_disp
     IF (bed_cnt < 2
      AND bed_cnt > 0)
      rooms->rm_list[rm_cnt].room_bed = trim(b_loc_room_disp)
     ENDIF
    FOOT REPORT
     stat = alterlist(rooms->rm_list,rm_cnt), rm_cnt = 0
    WITH nocounter, expand = 1
   ;end select
   CALL error_and_zero_check_rec(curqual,"LoadBasePts","bii_wlfw_facilityFilter",1,0,
    rooms)
   SELECT INTO "NL:"
    ed.encntr_id, ed.person_id, ed.loc_nurse_unit_cd,
    ed.loc_room_cd, ed.loc_bed_cd, ed.person_id
    FROM encntr_domain ed,
     person p,
     encounter e
    PLAN (ed
     WHERE ed.active_ind=1
      AND expand(num,1,size(patient_rec->units,5),ed.loc_nurse_unit_cd,patient_rec->units[num].unitcd
      )
      AND ed.beg_effective_dt_tm <= cnvtdatetime(sysdate)
      AND ed.end_effective_dt_tm >= cnvtdatetime(sysdate)
      AND expand(num,1,size(patient_rec->facilities,5),ed.loc_facility_cd,patient_rec->facilities[num
      ].facilitycd))
     JOIN (p
    ...
    HEAD REPORT
     vt_cnt = 0
    DETAIL
     pos = locateval(num,1,size(rooms->rm_list,5),ed.loc_bed_cd,rooms->rm_list[num].pt_bed)
     IF (pos > 0)
      vt_cnt += 1
      IF (vt_cnt > size(nurse_unit_filter->pt_list,5))
       stat = alterlist(nurse_unit_filter->pt_list,(vt_cnt+ 5))
      ENDIF
      nurse_unit_filter->pt_list[vt_cnt].patientid = ed.person_id, nurse_unit_filter->pt_list[vt_cnt]
      .encounterid = ed.encntr_id
     ENDIF
    FOOT REPORT
     stat = alterlist(nurse_unit_filter->pt_list,vt_cnt), vt_cnt = 0
    WITH nocounter, expand = 1
   ;end select
   CALL error_and_zero_check_rec(curqual,"LoadBasePts","bii_wlfw_facilityFilter",1,0,
    nurse_unit_filter)
 END ;Subroutine
 SUBROUTINE getpatientproviderreltns(null)
   CALL log_message("In GetPatientProviderReltns()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   ...
   SELECT INTO "nl:"
    ...
    PLAN (epr
     WHERE expand(eidx,1,size(nurse_unit_filter->pt_list,5),epr.encntr_id,nurse_unit_filter->pt_list[
      eidx].encounterid)
      AND (epr.prsnl_person_id=patient_rec->prsnl_id)
    ...
    DETAIL
     ecnt += 1, idx = locateval(y,1,size(nurse_unit_filter->pt_list,5),epr.encntr_id,
      nurse_unit_filter->pt_list[y].encounterid)
     WHILE (idx > 0)
      nurse_unit_filter->pt_list[idx].ppr = epr.encntr_prsnl_r_cd,idx = locateval(y,(idx+ 1),size(
        nurse_unit_filter->pt_list,5),epr.encntr_id,nurse_unit_filter->pt_list[y].encounterid)
     ENDWHILE
    WITH nocounter
   ;end select
   SELECT INTO "nl:"
    ...
    PLAN (ppr
     WHERE expand(pidx,1,size(nurse_unit_filter->pt_list,5),ppr.person_id,nurse_unit_filter->pt_list[
      pidx].patientid)
      AND (ppr.prsnl_person_id=patient_rec->prsnl_id)
      AND ppr.active_ind=1
      AND ppr.beg_effective_dt_tm <= cnvtdatetime(sysdate)
      AND ppr.end_effective_dt_tm >= cnvtdatetime(sysdate)
      AND ppr.person_prsnl_r_cd > 0)
    ORDER BY ppr.person_id
    DETAIL
     idx = locateval(y,1,size(nurse_unit_filter->pt_list,5),ppr.person_id,nurse_unit_filter->pt_list[
      y].patientid)
     WHILE (idx > 0)
      IF ((nurse_unit_filter->pt_list[idx].ppr=0.0))
       nurse_unit_filter->pt_list[idx].ppr = ppr.person_prsnl_r_cd
      ENDIF
      ,idx = locateval(y,(idx+ 1),size(nurse_unit_filter->pt_list,5),ppr.person_id,nurse_unit_filter
       ->pt_list[y].patientid)
     ENDWHILE
    WITH nocounter
   ;end select
   CALL log_message(build("Exit GetPatientProviderReltns(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
#exit_script
 CALL echorecord(nurse_unit_filter)
 CALL ajaxreply(trim(cnvtrectojson(nurse_unit_filter)))
END GO
