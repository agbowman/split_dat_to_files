CREATE PROGRAM dm_mars_suite_activity_rows:dba
 ...
 SET reply->table_name = "MARS_SUITE_ACTIVITY"
 SET reply->rows_between_commit = 100
 SET i18nhandle = 0
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 DECLARE batch_ndx = i4 WITH protect, noconstant(1)
 DECLARE c_audit = i4 WITH protect, noconstant(0)
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(msa.mars_suite_activity_id)
   FROM mars_suite_activity msa
   WHERE msa.mars_suite_activity_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(msa.mars_suite_activity_id)
  FROM mars_suite_activity msa
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    msa.rowid
    FROM mars_suite_activity msa
    WHERE msa.mars_suite_activity_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists("1=1","msa"))
    ...
    DETAIL
     ...
     ENDIF
     reply->rows[v_rows].row_id = msa.rowid
    WITH nocounter, maxqual(msa,value(v_rows_left))
   ;end select
