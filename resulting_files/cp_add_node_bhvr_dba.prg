CREATE PROGRAM cp_add_node_bhvr:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Passed JSON:" = "",
  "Instance Identifier" = ""
  WITH outdev, req, instance_ident
 ...
 DECLARE jind = i2 WITH protect, noconstant(0)
 DECLARE jsonstr = vc WITH protect, noconstant(trim( $REQ))
 IF (validate(request->blob_in))
  IF ((request->blob_in > " "))
   CALL logblobindata("CP_ADD_NODE_BHVR",request->blob_in, $OUTDEV)
 ...
 ENDIF
 IF (jsonstr > " ")
  SET jrec = cnvtjsontorec(trim(jsonstr,3))
  SET jind = 1
 ENDIF
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "CP_ADD_NODE_BHVR"
 DECLARE verifyinputs(null) = null WITH protect
 DECLARE insertrows(null) = i4 WITH protect
 DECLARE lstat = i4 WITH protect, noconstant(0)
 DECLARE instanceident = vc WITH protect, constant(trim( $INSTANCE_IDENT))
 DECLARE max_response_ident_size = i4 WITH protect, constant(1000)
 ...
 SUBROUTINE verifyinputs(null)
   ...
   DECLARE nlistsize = i4 WITH protect, constant(size(request->qual,5))
   DECLARE x = i4 WITH protect, noconstant(0)
   DECLARE codevaluevalid = i2 WITH protect, noconstant(0)
   IF (nlistsize <= 0)
    SET reply->status_data.status = "F"
    SET reply->status_data.subeventstatus.operationname = "PERFORM"
    SET reply->status_data.subeventstatus.operationstatus = "F"
    SET reply->status_data.subeventstatus.targetobjectname = "Validate"
    SET reply->status_data.subeventstatus.targetobjectvalue =
    "Pathway Behavior list must be nonzero in size."
    GO TO exit_script
   ENDIF
   FOR (x = 1 TO nlistsize)
     IF ((request->qual[x].cp_node_id > 0))
      SET codevaluevalid = checknodeid(cnvtreal(request->qual[x].cp_node_id))
      IF (codevaluevalid=0)
       SET reply->status_data.status = "F"
       SET reply->status_data.subeventstatus.operationname = "PERFORM"
       SET reply->status_data.subeventstatus.operationstatus = "F"
       SET reply->status_data.subeventstatus.targetobjectname = "Validate"
       SET reply->status_data.subeventstatus.targetobjectvalue = build2("CP_NODE_ID ",trim(cnvtstring
         (request->qual[x].cp_node_id),3)," does not exist or is inactive.")
       GO TO exit_script
      ENDIF
     ENDIF
     IF (size(trim(request->qual[x].reaction_entity_name,3),1)=0)
      SET reply->status_data.status = "F"
      SET reply->status_data.subeventstatus.operationname = "PERFORM"
      SET reply->status_data.subeventstatus.operationstatus = "F"
      SET reply->status_data.subeventstatus.targetobjectname = "Validate"
      SET reply->status_data.subeventstatus.targetobjectvalue = build2(
       "REACTION_ENTITY_NAME for list seq ",trim(cnvtstring(x),3)," must not be blank.")
      GO TO exit_script
     ENDIF
     IF (size(trim(request->qual[x].response_ident,3),1)=0)
      SET reply->status_data.status = "F"
      SET reply->status_data.subeventstatus.operationname = "PERFORM"
      SET reply->status_data.subeventstatus.operationstatus = "F"
      SET reply->status_data.subeventstatus.targetobjectname = "Validate"
      SET reply->status_data.subeventstatus.targetobjectvalue = build2("RESPONSE_IDENT for list seq ",
       trim(cnvtstring(x),3)," must not be blank.")
      GO TO exit_script
     ENDIF
   ENDFOR
 ...
 END ;Subroutine
 SUBROUTINE (insertlongtextrefrow(node_bhvr_id=f8,long_text=vc) =f8 WITH protect)
   ...
   DECLARE lsetactiveind = i4 WITH protect, noconstant(1)
   SET dnewltextid = getlongtextseqid("LONG_DATA_SEQ")
   INSERT  FROM long_text_reference lt
    SET lt.active_ind = lsetactiveind, lt.active_status_cd = dactive_cd, lt.active_status_dt_tm =
     cnvtdatetime(sysdate),
     lt.active_status_prsnl_id = reqinfo->updt_id, lt.long_text = long_text, lt.long_text_id =
     dnewltextid,
     lt.parent_entity_id = node_bhvr_id, lt.parent_entity_name = "CP_NODE_BEHAVIOR", lt.updt_applctx
   ...
   RETURN(dnewltextid)
 END ;Subroutine
 SUBROUTINE (checkcodevalue(codevalue=f8,codeset=i4) =i4 WITH protect)
   ...
   RETURN(curqual)
 END ;Subroutine
 SUBROUTINE (checknodeid(nid=f8) =i4 WITH protect)
   CALL log_message("In CheckNodeId()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SELECT INTO "nl:"
    FROM cp_node cn
    WHERE cn.cp_node_id=nid
     AND cn.beg_effective_dt_tm <= cnvtdatetime(sysdate)
     AND cn.end_effective_dt_tm >= cnvtdatetime(sysdate)
    WITH nocounter
   ;end select
   CALL log_message(build("Exit CheckNodeId(), Elapsed time in seconds:",((curtime3 - begin_curtime3)
     / 100.0)),log_level_debug)
   RETURN(curqual)
 END ;Subroutine
 SUBROUTINE insertrows(null)
   ...
   DECLARE begin_curtime3 = dq8 WITH private, constant(curtime3)
   DECLARE xsize = i4 WITH protect, constant(size(request->qual,5))
   DECLARE x = i4 WITH protect, noconstant(0)
   DECLARE skipinsert = i2 WITH protect, noconstant(0)
   DECLARE cnb_id = f8 WITH protect, noconstant(0)
   DECLARE cur_response_ident = vc WITH protect, noconstant("")
   DECLARE cur_long_response_ident = vc WITH protect, noconstant("")
   SET lstat = alterlist(reply->qual,xsize)
   IF (checkdic("CP_NODE_BEHAVIOR","T",0) != 2)
    SET reply->status_data.status = "F"
    SET reply->status_data.subeventstatus.operationname = "PERFORM"
    SET reply->status_data.subeventstatus.operationstatus = "F"
    SET reply->status_data.subeventstatus.targetobjectname = "INSERT"
    SET reply->status_data.subeventstatus.targetobjectvalue = build2(
     "CP_NODE_BEHAVIOR Table does not exist ","or Permission is not granted to access.")
    GO TO exit_script
   ENDIF
   FOR (x = 1 TO xsize)
     SET skipinsert = 0
     IF (size(instanceident,5) > 0)
      IF ((request->qual[x].reaction_entity_name="LONG_TEXT_REFERENCE"))
       SELECT INTO "nl:"
        FROM cp_node_behavior cnb,
         long_text_reference ltr,
         dummyt d1
        PLAN (cnb
         WHERE cnb.instance_ident=instanceident
          AND (request->qual[x].response_ident=cnb.response_ident)
          AND (cnb.cp_node_id=request->qual[x].cp_node_id)
          AND cnb.reaction_entity_name="LONG_TEXT_REFERENCE")
         JOIN (ltr
         WHERE ltr.parent_entity_id=cnb.cp_node_behavior_id
          AND ltr.parent_entity_name="CP_NODE_BEHAVIOR")
         JOIN (d1
         WHERE (ltr.long_text=request->qual[x].description))
        HEAD REPORT
         skipinsert = 1
        WITH nocounter
       ;end select
      ELSE
       SELECT INTO "nl:"
        ...
        WHERE cnb.instance_ident=instanceident
         AND (request->qual[x].response_ident=cnb.response_ident)
         AND (cnb.cp_node_id=request->qual[x].cp_node_id)
         AND (cnb.reaction_entity_id=request->qual[x].reaction_entity_id)
        HEAD REPORT
         skipinsert = 1
        WITH noccounter
       ;end select
     ...
     IF (skipinsert=0)
      ...
      ;end select
      IF ((request->qual[x].reaction_entity_name="LONG_TEXT_REFERENCE"))
       SET request->qual[x].reaction_entity_id = insertlongtextrefrow(cnb_id,request->qual[x].
        description)
      ENDIF
      IF (cnb_id=0.0)
       SET reply->status_data.status = "F"
       ...
       SET reply->status_data.subeventstatus.targetobjectname = "INSERT"
       SET reply->status_data.subeventstatus.targetobjectvalue = build2(
        "Unable to generate cp_node_behavior_id ","from REFERENCE_SEQ.")
       ROLLBACK
       GO TO exit_script
      ENDIF
      SET cur_response_ident = trim(request->qual[x].response_ident)
      SET cur_long_response_ident = ""
      IF (validate(debug_ind,0)=1)
       CALL echo(build(" cur_response_ident -- > ",size(cur_response_ident)))
      ENDIF
      IF (size(cur_response_ident) > max_response_ident_size)
       SET cur_long_response_ident = cur_response_ident
       SET cur_response_ident = trim(build(cnvtstring(cnvtdatetime(sysdate)),"_response_ident"),3)
      ENDIF
      INSERT  FROM cp_node_behavior cnb
       SET cnb.cp_node_behavior_id = cnb_id, cnb.cp_node_id = cnvtreal(request->qual[x].cp_node_id),
        cnb.reaction_entity_id = cnvtreal(request->qual[x].reaction_entity_id),
        cnb.reaction_entity_name = trim(request->qual[x].reaction_entity_name), cnb.response_ident =
        trim(cur_response_ident), cnb.long_response_ident = trim(cur_long_response_ident),
        cnb.reaction_type_mean = trim(request->qual[x].reaction_type_mean), cnb.instance_ident = trim
        (instanceident), cnb.updt_applctx = reqinfo->updt_applctx,
        cnb.updt_cnt = 0, cnb.updt_dt_tm = cnvtdatetime(sysdate), cnb.updt_id = reqinfo->updt_id,
        cnb.updt_task = reqinfo->updt_task
       WITH nocounter
      ;end insert
      SET lstat = error_and_zero_check_rec(1,"INSERT","ERROR INSERTING INTO CP_NODE_BEHAVIOR",1,1,
       reply)
      SET reply->qual[x].cp_node_behavior_id = cnb_id
      SET reply->qual[x].reaction_entity_name = request->qual[x].reaction_entity_name
      SET reply->qual[x].reaction_entity_id = cnvtreal(request->qual[x].reaction_entity_id)
      SET reply->qual[x].response_ident = request->qual[x].response_ident
      SET reply->qual[x].cp_node_id = cnvtreal(request->qual[x].cp_node_id)
      SET reply->qual[x].instance_ident = trim(instanceident)
      SET reply->qual[x].description = trim(request->qual[x].description)
     ENDIF
   ENDFOR
...
#set_return
 ...
 ENDIF
#exit_script
 CALL setauditinitreplystatusbyprogramreplystatus(reply)
 IF (validate(debug_ind,0)=1)
