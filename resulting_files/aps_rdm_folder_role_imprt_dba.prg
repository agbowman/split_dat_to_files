CREATE PROGRAM aps_rdm_folder_role_imprt:dba
 ...
 SUBROUTINE insertreqproc(nrequest,spfmt,ntargetreq,ndeststep,sservice,nreprocess,naction,srefpfmt,
  nseq)
   ...
   ENDCASE
 END ;Subroutine
 DECLARE max_list = i4 WITH constant(size(requestin->list_0,5))
 DECLARE x = i4 WITH noconstant(0)
 DECLARE nfailedcnt = i4 WITH noconstant(0)
 DECLARE sreadme_name = c22 WITH constant("APS_RDM_FOLDER_ROLE_IMPRT")
 CALL logscriptstart(sreadme_name)
 RECORD request(
   1 role_name = c40
   1 permission_bitmap = i4
 )
 RECORD reply(
   1 role_id = f8
   1 status_data
 ...
 FOR (x = 1 TO max_list)
   SET request->role_name = requestin->list_0[x].role_name
   SET request->permission_bitmap = cnvtint(requestin->list_0[x].permission_bitmap)
   EXECUTE aps_add_folder_role
   IF ((reply->status_data.status="S"))
    COMMIT
   ELSE
    SET nfailedcnt = (nfailedcnt+ 1)
    CALL logmsg(concat("Could not insert record:",requestin->list_0[x].role_name),e_logtofile)
   ENDIF
 ENDFOR
 FREE RECORD request
 FREE RECORD reply
 IF (nfailedcnt=0)
  CALL logstatus("Data load was successful.","S")
 ELSE
  CALL logstatus("One or more rows of the data load failed.","F")
 ENDIF
