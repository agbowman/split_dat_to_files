CREATE PROGRAM cv_get_observation:dba
 ...
 DECLARE procstatuslistsize = i2 WITH protect, noconstant(size(request->proc_status_list,5))
 ...
 DECLARE mv = f8 WITH protect, noconstant(0.0)
 DECLARE mvu = f8 WITH protect, noconstant(0.0)
 DECLARE buffer[110] = c150 WITH protect, noconstant(fillstring(150," "))
 DECLARE nprocstatussize = i4 WITH protect, noconstant(size(request->proc_status_list,5))
 DECLARE cancelled = f8 WITH protect, noconstant(0.0)
 ...
 DECLARE mu = f8 WITH protect, noconstant(0.0)
 ...
 IF ((request->historical_ind=1))
  EXECUTE cv_get_observation_historical  WITH replace("REQUEST",request), replace("REPLY",reply)
 ELSE
  ...
  SET mu = uar_get_code_by("MEANING",27700,"MU")
  SET stat = alterlist(reply->qual,10)
  SET buffer[1] = "select"
  SET buffer[2] = 'into "nl:" *'
  SET x = 3
  IF ((request->im_study_id != 0))
   SET buffer[x] = "from cv_proc cv,"
  ...
  IF (modalitylistsize > 0)
   ...
   SET x += 1
  ENDIF
  IF (((validcompdate=1) OR ((request->sps_desc != ""))) )
   SET buffer[x] = ", cv_step cvs"
   SET x += 1
   IF ((request->sps_desc != ""))
  ...
  IF ((request->im_study_id != 0))
   ...
   SET buffer[(x+ 1)] = 'and s.orig_entity_name = "CV_PROC"'
   SET x += 2
   IF (matchstudylistsize != 0)
   ...
   SET buffer[(x+ 1)] = "join cv where cv.cv_proc_id = pr.parent_entity_id"
   SET andgiven = 1
   SET x += 2
  ELSE
  ...
  IF ((request->accession != ""))
   ...
   ELSE
    ...
    SET x += 1
   ENDIF
   SET buffer[x] = "cv.accession = request->accession"
   SET x += 1
  ENDIF
  IF (orderlistsize != 0)
   IF (andgiven=0)
  ...
  IF (proclistsize != 0)
   ...
   ENDIF
   SET buffer[x] =
   "expand(num, 1, procListSize, cv.cv_proc_id, request->cv_proc_list[num]->cv_proc_id)"
   SET x += 1
  ELSE
   IF (andgiven=0)
    SET andgiven = 1
   ELSE
    ...
    SET x += 1
   ENDIF
   SET buffer[x] = "cv.cv_proc_id + 0!=0"
   SET x += 1
  ENDIF
  ...
  IF (cnvtdatetime(request->from_dt) > 0)
   ...
   IF (cnvtdatetime(request->to_dt) <= 0)
    SET buffer[x] =
    "(cv.request_dt_tm between cnvtdatetime(cnvtdate(cnvtdatetimeutc(request->from_dt,1)),0)"
    SET buffer[(x+ 1)] = "and cnvtdatetime(curdate,235959)"
    SET buffer[(x+ 2)] =
    "or cv.cv_proc_id = (select c1.cv_proc_id from cv_step c1 where c1.perf_start_dt_tm between "
    SET buffer[(x+ 3)] =
    " cnvtdatetime(cnvtdate(cnvtdatetimeutc(request->from_dt,1)),0) and cnvtdatetime(curdate,235959)))"
    SET x += 4
   ELSE
    SET buffer[x] =
    "(cv.request_dt_tm between cnvtdatetime(cnvtdate(cnvtdatetimeutc(request->from_dt,1)),0)"
    ...
    SET buffer[(x+ 2)] =
    "or cv.cv_proc_id = (select c1.cv_proc_id from cv_step c1 where c1.perf_start_dt_tm between "
    SET buffer[(x+ 3)] = " cnvtdatetime(cnvtdate(cnvtdatetimeutc(request->from_dt,1)),0) and "
    SET buffer[(x+ 4)] = "cnvtdatetime(cnvtdate(cnvtdatetimeutc(request->to_dt,1)),235959)))"
    SET x += 5
   ENDIF
  ENDIF
  IF (cnvtdatetime(request->begin_req_dt_tm) > 0
   ...
   ENDIF
   SET buffer[x] =
   "cv.request_dt_tm between cnvtdatetime(cnvtdate(cnvtdatetimeutc(request->begin_req_dt_tm,1)), 0) "
   SET buffer[(x+ 1)] =
   " and cnvtdatetime(cnvtdate(cnvtdatetimeutc(request->end_req_dt_tm,1)), 235959) "
   SET x += 2
  ENDIF
  IF (procstatuslistsize != 0)
   IF (andgiven=0)
    SET andgiven = 1
   ELSE
    SET buffer[x] = "and "
   ...
   ENDIF
   SET buffer[x] =
   "expand(num, 1, procStatusListSize, cv.proc_status_cd + 0, request->proc_status_list[num]->proc_status_cd)"
   SET x += 1
  ENDIF
  IF (cnvtdatetime(request->study_date) > 0)
   ...
   ENDIF
   SET buffer[x] =
   '(format(cv.request_dt_tm,"YYYYMMDD;;d")=format(cnvtdatetimeutc(request->study_date,1),"YYYYMMDD;;d")'
   SET buffer[(x+ 1)] =
   "or cv.cv_proc_id = (select c1.cv_proc_id from cv_step c1 where c1.perf_start_dt_tm between "
   SET buffer[(x+ 2)] = " cnvtdatetime(cnvtdate(cnvtdatetimeutc(request->study_date,1)),0) and"
   SET buffer[(x+ 3)] = "cnvtdatetime(cnvtdate(cnvtdatetimeutc(request->study_date,1)),235959)))"
   SET x += 4
  ENDIF
  IF ((request->match_type=1))
   ...
   ELSE
    ...
    SET x += 1
   ENDIF
   SET cancelled = uar_get_code_by("MEANING",4000341,"CANCELLED")
   SET buffer[x] = "cv.proc_status_cd+0 != cancelled"
   SET x += 1
  ENDIF
  IF (nprocstatussize > 0)
   IF (andgiven=0)
    SET andgiven = 1
   ELSE
    SET buffer[x] = "and "
   ...
   ENDIF
   SET buffer[x] =
   "expand(num,1,nProcStatusSize,cv.proc_status_cd+0,request->proc_status_list[num].proc_status_cd)"
   SET x += 1
  ENDIF
  IF ((ccldminfo->sec_org_reltn=1))
...
"
  ...
  ENDIF
  IF ((((request->req_proc_desc != "")) OR (modalitylistsize > 0)) )
   SET buffer[x] = "join order_cat where cv.catalog_cd = order_cat.catalog_cd"
   SET x += 1
  ...
  IF ((request->req_proc_id != ""))
   ...
   ENDIF
   SET buffer[x] = "join cv1 where cv1.code_value = cv.catalog_cd"
  ...
  IF (matchstudylistsize != 0
   ...
   SET buffer[x] =
   'join pr where pr.parent_entity_id = cv.cv_proc_id and pr.parent_entity_name="CV_PROC"'
   SET buffer[(x+ 1)] =
   'join s where s.im_study_id = pr.im_study_id and s.orig_entity_name = "CV_PROC" and '
   SET buffer[(x+ 2)] = studyclause
   SET x += 3
  ENDIF
  IF (((validcompdate=1) OR ((request->sps_desc != ""))) )
   SET buffer[x] = "join cvs where cvs.cv_proc_id = cv.cv_proc_id "
   IF (validcompdate=1)
    SET buffer[(x+ 1)] =
"and cvs.perf_start_dt_tm between                                cnvtdatetime(cnvtdate(cnvtdatetimeutc(request->begin_comp_\
dt_tm,1)),0) and \
"
    SET buffer[(x+ 2)] = "cnvtdatetime(cnvtdate(cnvtdatetimeutc(request->end_comp_dt_tm,1)),235959)"
    SET x += 3
   ENDIF
   IF ((request->sps_desc != ""))
    SET buffer[(x+ 1)] = "join dta where cvs.task_assay_cd = dta.task_assay_cd"
    SET buffer[(x+ 2)] = "and cnvtupper(dta.description) = patstring(cnvtupper(request->sps_desc))"
    SET x += 3
   ENDIF
  ENDIF
  IF ((request->only_unpub_flag=1))
   ...
   SET x += 5
  ENDIF
  SET buffer[x] = "order cv.cv_proc_id"
  SET buffer[(x+ 1)] = "head cv.cv_proc_id"
  ...
  SET x += 7
  SET buffer[x] = "reply->qual[count]->cv_proc_id = cv.cv_proc_id "
  SET x += 1
  IF ((request->load_ids_only != 1))
   ...
   SET buffer[(x+ 3)] = "reply->qual[count]->catalog_cd = cv.catalog_cd "
   SET buffer[(x+ 4)] = "reply->qual[count]->accession = cv.accession "
   SET buffer[(x+ 5)] = "reply->qual[count]->priority_cd = cv.priority_cd "
   SET buffer[(x+ 6)] = "reply->qual[count]->proc_status_cd = cv.proc_status_cd "
   SET buffer[(x+ 7)] = "reply->qual[count]->order_physician_id = cv.order_physician_id "
   SET buffer[(x+ 8)] = "reply->qual[count]->refer_physician_id = cv.refer_physician_id "
   SET buffer[(x+ 9)] = "reply->qual[count]->prim_physician_id = cv.prim_physician_id "
   SET buffer[(x+ 10)] = "reply->qual[count]->reason_for_proc = cv.reason_for_proc"
   SET buffer[(x+ 11)] = "reply->qual[count]->request_dt_tm = cv.request_dt_tm"
   SET buffer[(x+ 11)] = "reply->qual[count]->updt_cnt = cv.updt_cnt"
   SET x += 12
  ENDIF
  ...
  IF (count != 0
   ...
   SET x = 1
   SET buffer[x] =
   'select into "nl:" * from cv_proc cv,order_detail ord_dtl, (dummyt d with seq=value(count)), dummyt d1'
   SET x += 1
   ...
   IF ((request->load_link_study_ind=1))
    ...
    SET x += 1
   ENDIF
   IF ((request->load_steps_ind != 0))
    SET buffer[x] = ", cv_step cvs"
    SET x += 1
   ENDIF
   SET buffer[x] = "plan d"
   SET buffer[(x+ 1)] = "join cv where cv.cv_proc_id = reply->qual[d.seq]->cv_proc_id"
   SET x += 2
   IF ((request->load_pertains_study_ind=1))
    SET buffer[x] =
'join st1 where st1.orig_entity_id=outerjoin(cv.cv_proc_id) and                             st1.orig_entity_name=outerjoin(\
"CV_PROC")\
'
    SET x += 1
   ENDIF
   IF ((request->load_link_study_ind=1))
    SET buffer[x] =
'join sr1 where sr1.parent_entity_id=outerjoin(cv.cv_proc_id)                            and sr1.parent_entity_name=outerjo\
in("CV_PROC")\
'
   ...
   ENDIF
   IF ((request->load_steps_ind != 0))
    SET buffer[x] = "join cvs where cvs.cv_proc_id = outerjoin(cv.cv_proc_id)"
    SET x += 1
   ENDIF
   ...
   IF ((request->load_steps_ind != 0))
    SET buffer[x] = "detail"
    SET buffer[(x+ 1)] = "count2 = count2 + 1 "
    SET buffer[(x+ 2)] = "stat = alterlist(reply->qual[count]->cv_steps, count2) "
    SET buffer[(x+ 3)] = "reply->qual[count]->cv_steps[count2]->cv_step_id = cvs.cv_step_id"
    SET x += 4
   ENDIF
