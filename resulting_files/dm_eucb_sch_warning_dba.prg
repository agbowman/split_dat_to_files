CREATE PROGRAM dm_eucb_sch_warning:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_EUCB_SCH_WARNING"
 ...
 SUBROUTINE cust_ucb_upt(dummy)
   UPDATE  FROM sch_warning w
    SET w.updt_id = reqinfo->updt_id, w.updt_dt_tm = cnvtdatetime(sysdate), w.updt_applctx = reqinfo
     ->updt_applctx,
     w.updt_task = reqinfo->updt_task, w.parent_id = request->xxx_uncombine[ucb_cnt].to_xxx_id
    WHERE (w.parent_id=request->xxx_uncombine[ucb_cnt].from_xxx_id)
     AND (w.sch_warn_id=rchildren->qual1[det_cnt].entity_id)
