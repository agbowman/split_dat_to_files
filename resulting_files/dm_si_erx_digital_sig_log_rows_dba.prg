CREATE PROGRAM dm_si_erx_digital_sig_log_rows:dba
 ...
 SET reply->table_name = "SI_ERX_DIGITAL_SIG_LOG"
 SET reply->rows_between_commit = 100
 SET i18nhandle = 0
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 DECLARE months_to_keep = i4 WITH protect, noconstant(- (1))
 DECLARE token_size = i4 WITH protect, noconstant(0)
 DECLARE tok_ndx = i4 WITH protect, noconstant(0)
 DECLARE strunit = vc WITH protect, noconstant(" ")
 ...
 FOR (tok_ndx = 1 TO token_size)
   ...
   ENDIF
 ENDFOR
 IF (months_to_keep < 24)
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1",
   "You must look back at least 24 months. You entered %1 month(s) or did not enter any value.","i",
   months_to_keep)
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(si.si_erx_digital_sig_log_id)
   FROM si_erx_digital_sig_log si
   WHERE si.si_erx_digital_sig_log_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(si.si_erx_digital_sig_log_id)
  FROM si_erx_digital_sig_log si
  DETAIL
   v_max_id = cnvtreal(maxval)
  WITH nocounter
 ;end select
 SET strunit = build(months_to_keep,"M")
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    si.rowid
    FROM si_erx_digital_sig_log si
    WHERE si.si_erx_digital_sig_log_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists(
      "si.sign_dt_tm < cnvtlookbehind(strUnit, cnvtdatetime(curdate,curtime3))","si"))
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = si.rowid
    WITH nocounter, maxqual(si,value(v_rows_left))
   ;end select
