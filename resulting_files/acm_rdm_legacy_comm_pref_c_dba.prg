CREATE PROGRAM acm_rdm_legacy_comm_pref_c:dba
 ...
 DECLARE lclinicalcommchildtotal = i4 WITH protect, noconstant(0)
 DECLARE lapptreminderchildtotal = i4 WITH protect, noconstant(0)
 DECLARE bdebugme = i2 WITH noconstant(false)
 SET ms_child_info_domain_nm = "ACM_RDM_MIGRATE_PERSON_PATIENT_TO_PERSON_PREF_COMM"
 SET ms_range_prefix = "ACM_RDM_MIGRATE_PERSON_PATIENT_TO_PERSON_PREF_COMM_RANGE"
 SET ms_child_max_name = "MAX PERSON_ID EVALUATED"
 ...
 IF (sbr_fetch_min_max(ms_child_info_domain_nm,ms_range_prefix,ms_child_max_name,mf_max_range_id,
  ...
  GO TO exit_program
 ENDIF
 IF (sbr_sync_ccl_tmp_def("TEMP_RDM_MIGRATE_PPC")=0)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  cv.code_value
 ...
 IF (error(ms_errmsg,0) != 0)
  ...
  GO TO exit_program
 ENDIF
 SET lclinicalcommchildtotal = 0
 SELECT INTO "nl:"
  cvg.child_code_value
  FROM code_value_group cvg
  WHERE cvg.parent_code_value=dclinicalcommtypecd
  DETAIL
   lclinicalcommchildtotal += 1
  WITH nocounter
 ;end select
 IF (error(ms_errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("No child value configured for CLINICALCOMM Communication type.",
   ms_errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
 ...
 IF (error(ms_errmsg,0) != 0)
  ...
  GO TO exit_program
 ENDIF
 SET lapptreminderchildtotal = 0
 SELECT INTO "nl:"
  cvg.child_code_value
  FROM code_value_group cvg
  WHERE cvg.parent_code_value=dapptremindertypecd
  DETAIL
   lapptreminderchildtotal += 1
  WITH nocounter
 ;end select
 IF (error(ms_errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("No child value configured for APPTREMINDER Communication type.",
   ms_errmsg)
  GO TO exit_program
 ENDIF
