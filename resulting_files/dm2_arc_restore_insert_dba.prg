CREATE PROGRAM dm2_arc_restore_insert:dba
 ...
 SUBROUTINE update_time_window(null)
  ...
  ENDIF
 END ;Subroutine
 DECLARE v_stmt_rest_ins = vc
 CALL parser("rdb ",0)
 FOR (cnt3 = 1 TO request->temp_size)
   SET v_stmt_rest_ins = arc_replace(pers_arc->statements[request->s_ndx].restore_insert[cnt3].stmt,
    pers_arc->statements[request->s_ndx].restore_insert[cnt3].link_ind,pers_arc->statements[request->
    s_ndx].restore_insert[cnt3].list_ind,pers_arc->statements[request->s_ndx].restore_insert[cnt3].
   ...
   IF ((cnt3=request->temp_size))
    CALL parser(v_stmt_rest_ins,1)
   ELSE
    CALL parser(v_stmt_rest_ins,0)
   ENDIF
   IF ((debug_mover->debug_level > 1))
    CALL echo(v_stmt_rest_ins)
   ENDIF
 ENDFOR
