CREATE PROGRAM afc_get_price_sched_item_hist:dba
 IF ( NOT (validate(afc_get_price_sched_item_hist_script_vrsn)))
  DECLARE afc_get_price_sched_item_hist_script_vrsn = vc WITH constant("464331.000"), private
 ENDIF
 ...
 IF (validate(i18nuar_def,999)=999)
  ...
  persist
 ENDIF
 CALL beginservice(afc_get_price_sched_item_hist_script_vrsn)
 ...
 IF ( NOT (validate(script_name)))
  DECLARE script_name = vc WITH protect, constant("AFC_GET_PRICE_SCHED_ITEM_HIST")
 ENDIF
 ...
 DECLARE failurelogmessage = vc WITH protect, noconstant("")
 SET failurelogmessage = uar_i18ngetmessage(i18nhandle,"psihKey_HistoryRetrievalFailue",nullterm(
   "No Price Schedule Item modifications history is available."))
 DECLARE successlogmessage = vc WITH protect, noconstant("")
 SET successlogmessage = uar_i18ngetmessage(i18nhandle,"psihKey_HistoryRetrievalSuccess",nullterm(
   "Price Schedule Item modifications history fetched successfully!"))
 IF ((request->price_sched_items_hist_qual > 0))
...
#check_error
 IF (failed=false)
  ...
  SET reqinfo->commit_ind = true
  CALL exitservicesuccess(build(" version : ",afc_get_price_sched_item_hist_script_vrsn))
 ELSE
  CASE (failed)
   OF select_error:
  ...
  SET reply->status_data.subeventstatus[1].targetobjectvalue = psi_hist_table_name
  SET reqinfo->commit_ind = false
 ENDIF
 SUBROUTINE (getpricescheditemmodhistory(dummyvar=i4) =null)
   IF ( NOT (validate(get_price_sched_items_hist_sub_name)))
    DECLARE get_price_sched_items_hist_sub_name = vc WITH protect, constant(
     "AFC_GET_PRICE_SCHED_ITEM_HIST.getPriceSchedItemModHistory")
   ENDIF
   IF ((validate(rec_count,- (1))=- (1)))
    DECLARE rec_count = i2 WITH protect, noconstant(0)
   ENDIF
   SET select_begin = 1
   SET selec_end = request->price_sched_items_hist_qual
   IF (size(request->price_sched_items_hist,5) > 0)
    FOR (pishindex = 1 TO size(request->price_sched_items_hist,5))
      SELECT
       IF ((request->price_sched_items_hist[pishindex].price_sched_items_hist_id != 0))
        WHERE (psih.price_sched_items_hist_id=request->price_sched_items_hist[pishindex].
        price_sched_items_hist_id)
         AND psih.active_ind=validate(request->active_ind,true)
         AND psih.task_action_flag != 0
         AND psih.price_sched_id=ps.price_sched_id
         AND p.person_id=psih.updt_id
       ELSEIF ((request->price_sched_items_hist[pishindex].price_sched_items_hist_id IN (0, null))
        AND (request->price_sched_items_hist[pishindex].price_sched_items_id != 0))
        WHERE (psih.price_sched_items_id=request->price_sched_items_hist[pishindex].
        price_sched_items_id)
         AND (psih.bill_item_id=request->price_sched_items_hist[pishindex].bill_item_id)
         AND psih.active_ind=validate(request->active_ind,true)
         AND psih.task_action_flag != 0
         AND psih.price_sched_id=ps.price_sched_id
         AND p.person_id=psih.updt_id
       ELSEIF ((request->price_sched_items_hist[pishindex].price_sched_items_hist_id IN (0, null))
        AND (request->price_sched_items_hist[pishindex].price_sched_items_id IN (0, null))
        AND (request->price_sched_items_hist[pishindex].price_sched_id != 0))
        WHERE (psih.price_sched_id=request->price_sched_items_hist[pishindex].price_sched_id)
         AND (psih.bill_item_id=request->price_sched_items_hist[pishindex].bill_item_id)
         AND psih.active_ind=validate(request->active_ind,true)
         AND psih.task_action_flag != 0
         AND psih.price_sched_id=ps.price_sched_id
       ...
       ELSEIF ((request->price_sched_items_hist[pishindex].price_sched_items_hist_id IN (0, null))
        AND (request->price_sched_items_hist[pishindex].price_sched_items_id IN (0, null))
        AND (request->price_sched_items_hist[pishindex].price_sched_id IN (0, null))
        AND (request->price_sched_items_hist[pishindex].bill_item_id != 0))
        WHERE (psih.bill_item_id=request->price_sched_items_hist[pishindex].bill_item_id)
         AND psih.active_ind=validate(request->active_ind,true)
         AND psih.task_action_flag != 0
         AND p.person_id=psih.updt_id
       ELSE
        WHERE (psih.updt_id=reqinfo->updt_id)
         AND psih.active_ind=validate(request->active_ind,true)
         AND psih.task_action_flag != 0
         AND p.person_id=psih.updt_id
       ENDIF
       INTO "nl:"
       psih.*, ps.price_sched_desc, p.*
       FROM price_sched_items_hist psih,
        price_sched ps,
        person p
       PLAN (psih)
        JOIN (ps
        WHERE ps.active_ind=true)
       ORDER BY psih.updt_dt_tm DESC
       DETAIL
        rec_count += 1, stat = alterlist(reply->qual,rec_count), reply->qual[rec_count].
        price_sched_items_id = psih.price_sched_items_id,
        reply->qual[rec_count].bill_item_id = psih.bill_item_id, reply->qual[rec_count].
        price_sched_id = psih.price_sched_id, reply->qual[rec_count].price = psih.price,
        reply->qual[rec_count].allowable = psih.allowable, reply->qual[rec_count].copay = psih.copay,
        reply->qual[rec_count].deductible = psih.deductible,
        reply->qual[rec_count].percent_revenue = psih.percent_revenue, reply->qual[rec_count].
        charge_level_cd = psih.charge_level_cd, reply->qual[rec_count].interval_template_cd = psih
        .interval_template_cd,
        reply->qual[rec_count].detail_charge_ind = psih.detail_charge_ind, reply->qual[rec_count].
        stats_only_ind = psih.stats_only_ind, reply->qual[rec_count].tax = psih.tax,
        reply->qual[rec_count].exclusive_ind = psih.exclusive_ind, reply->qual[rec_count].
        cost_adj_amt = psih.cost_adj_amt, reply->qual[rec_count].units_ind = psih.units_ind,
        reply->qual[rec_count].billing_discount_priority = psih.billing_discount_priority_seq, reply
        ->qual[rec_count].beg_effective_dt_tm = psih.beg_effective_dt_tm, reply->qual[rec_count].
        end_effective_dt_tm = psih.end_effective_dt_tm,
        reply->qual[rec_count].updt_id = p.name_full_formatted, reply->qual[rec_count].updt_dt_tm =
        psih.updt_dt_tm, reply->qual[rec_count].updt_cnt = psih.updt_cnt,
        reply->qual[rec_count].price_sched_items_hist_id = psih.price_sched_items_hist_id, reply->
        qual[rec_count].history_ind = true, reply->qual[rec_count].price_sched_desc = ps
        .price_sched_desc,
        reply->qual[rec_count].task_action_flag = psih.task_action_flag
       WITH nocounter
      ;end select
      SET reply->price_sched_items_qual = rec_count
      IF (curqual=0)
       SET failed = false
       SET reply->status_data.subeventstatus[1].operationname = "Select"
       ...
       CALL logmessage(get_price_sched_items_hist_sub_name,failurelogmessage,log_info)
      ELSE
       SET failed = false
       SET reply->status_data.status = "S"
       CALL logmessage(get_price_sched_items_hist_sub_name,successlogmessage,log_info)
      ENDIF
    ENDFOR
   ENDIF
 END ;Subroutine
