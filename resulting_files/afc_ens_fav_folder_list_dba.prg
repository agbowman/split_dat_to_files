CREATE PROGRAM afc_ens_fav_folder_list:dba
 RECORD reply(
 ...
 SET hafc_ens_fav_folder_list = 0
 SET istatus = 0
 SET serrmsg = fillstring(132," ")
 SET reply->status_data.status = "F"
 SET table_name = "FAV_FOLDER_LIST"
 IF ((request->fav_folder_list_qual > 0))
  SET reply->fav_folder_list_qual = request->fav_folder_list_qual
  FOR (inx0 = 1 TO request->fav_folder_list_qual)
    CASE (request->fav_folder_list[inx0].action_type)
     OF "ADD":
      SET action_begin = inx0
      SET action_end = inx0
      EXECUTE afc_add_fav_folder_list
      IF (failed != false)
       GO TO check_error
      ENDIF
     OF "UPT":
      SET action_begin = inx0
      SET action_end = inx0
      EXECUTE afc_upt_fav_folder_list
      IF (failed != false)
       GO TO check_error
      ENDIF
     OF "DEL":
      SET action_begin = inx0
      SET action_end = inx0
      EXECUTE afc_del_fav_folder_list
      IF (failed != false)
       GO TO check_error
      ENDIF
     ELSE
