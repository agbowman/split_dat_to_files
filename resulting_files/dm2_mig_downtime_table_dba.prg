CREATE PROGRAM dm2_mig_downtime_table:dba
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_mig_dt_table",".log","dm2_mig_downtime_table")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM2_MIG_DOWNTIME_TABLE"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 ELSEIF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Username ", $1," is not a valid user.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 ...
 SELECT INTO "nl:"
  d.table_name
  FROM dba_tables d
  WHERE d.owner=cnvtupper( $1)
   AND d.table_name=cnvtupper( $2)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) > 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 ELSEIF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Table name ", $2," is not a valid table.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Validating that user supplied is an uptime user."
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2_MIG_USER"
   AND d.info_name=cnvtupper( $1)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) > 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 ELSEIF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("User ", $1," is not a UPTIME user.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Updating DM_INFO exclusion row."
 UPDATE  FROM dm_info di
  SET di.updt_dt_tm = cnvtdatetime(curdate,curtime3), di.info_number = 15, di.info_char = "1"
  WHERE di.info_domain="DM2_MIG_EXCL"
   AND di.info_name=cnvtupper(concat( $1,".", $2))
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc) > 0)
  ROLLBACK
  ...
  GO TO exit_script
 ELSEIF (curqual=0)
  SET dm_err->eproc = "No rows qualified for update.  Inserting DM_INFO exclusion row."
  INSERT  FROM dm_info di
   SET di.info_domain = "DM2_MIG_EXCL", di.info_name = cnvtupper(concat( $1,".", $2)), di.info_number
     = 15,
    di.info_char = "1", di.updt_dt_tm = cnvtdatetime(curdate,curtime3)
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc) > 0)
   ROLLBACK
...
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_mig_downtime_table completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_mig_dt_table")
END GO
