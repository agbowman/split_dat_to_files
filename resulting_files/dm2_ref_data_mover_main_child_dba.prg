CREATE PROGRAM dm2_ref_data_mover_main_child:dba
 ...
 SUBROUTINE dm2_rdds_get_tgt_id(s_gmti_tgt_rs)
   ...
   RETURN(drgmti_return_val)
 END ;Subroutine
 DECLARE translate_column(tc_temp_tbl_cnt=i4,tc_perm_col_cnt=i4) = null
 DECLARE lookup_translations(lt_temp_tbl_cnt=i4,lt_temp_col_cnt=i4) = null
 DECLARE translate_vers_col(tv_temp_tbl_cnt=i4,tv_temp_col_cnt=i4) = null
 DECLARE update_cnt = i4
 DECLARE no_unique_ident = i2
 DECLARE ins_ind = i2
 DECLARE another_loop = i4
 DECLARE drdm_return_var = i4
 DECLARE drdm_srch_str = vc
 DECLARE drdm_from_con = vc
 DECLARE drdm_to_con = vc
 DECLARE drdm_parser_cnt = i4
 DECLARE where_clause = vc
 DECLARE drdm_go_ind = i4
 DECLARE rec_found = i2
 DECLARE short_string = vc
 DECLARE update_error_check_ind = i2
 DECLARE looked_up = i2
 DECLARE been_translated = i4
 DECLARE inserted = i2
 DECLARE drdm_col_value = f8
 DECLARE drdm_col_name = vc
 DECLARE drdm_data_type = vc
 DECLARE no_insert_update = i2
 DECLARE good_rec = i4
 DECLARE rec_struct_loop = i4
 DECLARE final_check_loop = i4
 DECLARE unique_ident_loop = i4
 DECLARE trans_val_loop = i4
 DECLARE cv_query_loop = i4
 DECLARE ins_upd_loop = i4
 DECLARE update_loop = i4
 DECLARE where_update_loop = i4
 DECLARE insert_loop = i4
 DECLARE parse_loop = i4
 DECLARE passed_var = vc
 DECLARE insert_update_reason = vc
 DECLARE nodelete_msg = vc
 DECLARE drdmm_srch_str_s = vc
 DECLARE idcd_msg = vc
 DECLARE zero_rec = i4
 DECLARE tbl_loop = i4
 DECLARE new_pk_where = vc
 DECLARE more_dels = i2
 DECLARE to_this = f8
 DECLARE from_this = f8
 DECLARE vers_cd_value = f8
 DECLARE rpt_missing = vc
 DECLARE rpt_table = vc
 DECLARE rpt_column = vc
 DECLARE rpt_from = f8
 DECLARE rpt_orphan_ind = i2
 DECLARE rpt_oldver_ind = i2
 DECLARE space_loop = i4
 DECLARE space_size = i4
 DECLARE mmc_imt_check = i2
 DECLARE tname_flex = vc
 DECLARE orphaned_col_name = vc
 DECLARE new_seq_ind = i2
 DECLARE alt_del_pk = vc
 DECLARE src_pk_where = vc
 DECLARE mmc_src_tab_name = vc
 DECLARE drdm_table_suffix = vc
 DECLARE drdm_tc_new_seq = f8
 DECLARE merge_loop = i4
 DECLARE missing_xlat_cnt = i4
 DECLARE nomv_ind = i2
 DECLARE nomv_type = vc
 DECLARE drdm_long_check = vc
 SET current_merges = 0
 SET fail_merges = 0
 SET perm_col_cnt = size(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual,5)
 SET new_seq_ind = 0
 IF ((validate(drdm_override_ind,- (1))=- (1)))
  DECLARE drdm_override_ind = i2
  SET drdm_override_ind = 0
 ENDIF
 SET nvp_commit_ind = 1
 CALL echo("")
 CALL echo("")
 CALL echo(build("LOG_ID = ",drdm_chg->log[drdm_log_loop].log_id))
 CALL echo("")
 CALL echo("")
 CALL parser(concat("set RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
   "->log_id = drdm_chg->log[drdm_log_loop].log_id go"),1)
 CALL parser(concat("set RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
   "->From_table_name = drdm_chg->log[drdm_log_loop].table_name go"),1)
 CALL parser(concat("set RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
   "->to_table_name = drdm_chg->log[drdm_log_loop].table_name go"),1)
 SET drdm_table_suffix = dm2_rdds_get_tbl_alias(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix)
 IF ((drdm_chg->log[drdm_log_loop].delete_ind=1))
  IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].insert_only_ind=1))
   CALL merge_audit("FAILREASON",
    "No delete will be done because this table is listed as Target as Master",3)
   SET drdm_mini_loop_status = "MERGED"
   GO TO exit_child
  ENDIF
  SET dm_err->eproc = "Creating delete Pk_where statement"
  IF (findstring("DM_TRANSLATE",cnvtupper(drdm_chg->log[drdm_log_loop].pk_where),1,0) > 0)
   CALL parser(concat("set new_pk_where = ",drdm_chg->log[drdm_log_loop].pk_where," go"),1)
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET nodelete_ind = 1
    SET no_insert_update = 1
    ...
    GO TO exit_child
   ENDIF
   IF (findstring("NOXLAT",new_pk_where)=0
    AND findstring("ORPHAN",new_pk_where)=0)
    SET tname_flex = drdm_chg->log[drdm_log_loop].table_name
    SET drdm_parser->statement[1].frag = concat("delete from ",tname_flex," ",value(new_pk_where),
     " go")
   ...
   ELSE
    SET nodelete = 1
    CALL echo("************NO TRANSLATIONS FOR DELETE YET***************")
    CALL merge_audit("FAILREASON","No translations for delete yet",3)
    IF (findstring("ORPHAN",new_pk_where) > 0)
     SET drdm_mini_loop_status = "ORPHAN"
    ENDIF
    COMMIT
   ENDIF
  ELSEIF (((findstring("DM_TRANS2",cnvtupper(drdm_chg->log[drdm_log_loop].pk_where),1,0) > 0) OR (
  findstring("DM_TRANS3",cnvtupper(drdm_chg->log[drdm_log_loop].pk_where),1,0) > 0)) )
   SET alt_del_pk = replace(drdm_chg->log[drdm_log_loop].pk_where,"<SOURCE_IND>","0",0)
   CALL parser(concat("set new_pk_where = ",alt_del_pk," go"),1)
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET nodelete_ind = 1
    SET no_insert_update = 1
    ...
    GO TO exit_child
   ENDIF
   IF (findstring("=NOXLAT",trim(new_pk_where,4))=0
    AND findstring("=ORPHAN",trim(new_pk_where,4))=0
    AND findstring("=BADLOG",trim(new_pk_where,4))=0
    AND findstring("=OLDVER",trim(new_pk_where,4))=0
    AND findstring("=BADTRN",trim(new_pk_where,4))=0
    AND findstring("=NOMV",trim(new_pk_where,4))=0
    AND findstring("=NOLOG",trim(new_pk_where,4))=0
    AND findstring("=INLOG",trim(new_pk_where,4))=0
    AND findstring("=INSRC",trim(new_pk_where,4))=0
    AND findstring("=UPDTSK",trim(new_pk_where,4))=0)
    SET alt_del_pk = replace(drdm_chg->log[drdm_log_loop].pk_where,"<SOURCE_IND>","1",0)
    CALL parser(concat("set src_pk_where = ",alt_del_pk," go"),1)
    SET mmc_src_tab_name = dm2_get_rdds_tname(drdm_chg->log[drdm_log_loop].table_name)
    SET drdm_parser->statement[1].frag = concat("select into 'NL:' from ",mmc_src_tab_name," ",
     drdm_table_suffix," ",
     src_pk_where," go")
    CALL parse_statements(1)
    IF (curqual > 0)
     SET nodelete_ind = trigger_proc_call(drdm_chg->log[drdm_log_loop].table_name,src_pk_where,
      drdm_chg->log[drdm_log_loop].context_name,"",0.0)
     IF (nodelete_ind=0)
      COMMIT
     ELSE
      ROLLBACK
     ...
     GO TO exit_child
    ELSE
     IF ( NOT ((drdm_chg->log[drdm_log_loop].table_name IN ("LONG_TEXT", "LONG_TEXT_REFERENCE",
     "LONG_BLOB_REFERENCE"))))
      SET drdm_child_del = 0
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_flag=0))
       SELECT INTO "NL:"
        FROM dm_refchg_tab_r d
        WHERE (d.parent_table=drdm_chg->log[drdm_log_loop].table_name)
        ORDER BY d.process_seq
        DETAIL
         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_flag = 2, drdm_child_del = (drdm_child_del+
         1), stat = alterlist(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual,drdm_child_del),
         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual[drdm_child_del].child_name = d
         .child_table, dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual[drdm_child_del].
         parent_id_col = d.parent_id_col, dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual[
         drdm_child_del].parent_tab_col = d.parent_tab_col
        WITH nocounter
       ;end select
       IF (curqual=0)
        SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_flag = 1
       ENDIF
      ENDIF
      SET drdm_child_del = 0
      IF ((drdm_chg->log[drdm_log_loop].table_name="PM_FLX_PROMPT"))
       FOR (drdm_child_del = 1 TO size(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual,5))
         CALL parser(concat("delete from ",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual[
           drdm_child_del].child_name," d where d.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
           parent_qual[drdm_child_del].parent_id_col," in ("),0)
         CALL parser(concat("select prompt_id from pm_flx_prompt t1562 ",new_pk_where,")"),0)
         IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual[drdm_child_del].parent_tab_col !=
         ""))
          CALL parser(concat(" and d.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual[
            drdm_child_del].parent_tab_col," = '",drdm_chg->log[drdm_log_loop].table_name,"'"),0)
         ENDIF
         CALL parser(" go ",1)
       ENDFOR
      ELSEIF ((drdm_chg->log[drdm_log_loop].table_name="PM_FLX_CONV_OPTIONS"))
       FOR (drdm_child_del = 1 TO size(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual,5))
         CALL parser(concat("delete from ",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual[
           drdm_child_del].child_name," d where d.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
           parent_qual[drdm_child_del].parent_id_col," in ("),0)
         CALL parser(concat("select pm_flx_conv_options_id from pm_flx_conv_options t7629 ",
           new_pk_where,")"),0)
         IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual[drdm_child_del].parent_tab_col !=
         ""))
          CALL parser(concat(" and d.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual[
         ...
         CALL parser(" go ",1)
       ENDFOR
      ELSEIF ((drdm_chg->log[drdm_log_loop].table_name="PM_FLX_CONVERSATION"))
       FOR (drdm_child_del = 1 TO size(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual,5))
         IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual[drdm_child_del].child_name !=
         "PM_FLX_PROMPT"))
          CALL parser(concat("delete from ",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual[
            drdm_child_del].child_name," d where d.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
            parent_qual[drdm_child_del].parent_id_col," in ("),0)
          CALL parser(concat("select conversation_id from pm_flx_conversation t2133 ",new_pk_where,
            ")"),0)
          IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual[drdm_child_del].parent_tab_col
           != ""))
           CALL parser(concat(" and d.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].parent_qual[
       ...
       ENDFOR
      ENDIF
      CALL parser(concat("delete from ",value(drdm_chg->log[drdm_log_loop].table_name)," ",
        drdm_table_suffix," ",
        value(new_pk_where)," go"),1)
     ELSE
      IF (findstring("PM_FLX_PROMPT",new_pk_where)=0
       AND findstring("PM_FLX_CONVERSATION",new_pk_where)=0
       AND findstring("PM_FLX_CONV_OPTIONS",new_pk_where)=0)
       IF (((findstring("LONG_TEXT_ID",new_pk_where) != 0) OR (findstring("LONG_BLOB_ID",new_pk_where
        ) != 0)) )
        SET drdm_long_check = ""
        CALL parser(concat("select into 'nl:' from ",drdm_chg->log[drdm_log_loop].table_name," ",
          drdm_table_suffix),0)
        CALL parser(concat(" ",new_pk_where," detail drdm_long_check = ",drdm_table_suffix,
          ".PARENT_ENTITY_NAME with nocounter go"),1)
        IF ( NOT (drdm_long_check IN ("PM_FLX_PROMPT", "PM_FLX_CONVERSATION", "PM_FLX_CONV_OPTIONS"))
        )
         CALL parser(concat("delete from ",value(drdm_chg->log[drdm_log_loop].table_name)," ",
           drdm_table_suffix," ",
           value(new_pk_where)," go"),1)
     ...
     IF (check_error(dm_err->eproc)=1)
      IF ((dm_err->ecode=284)
       AND (drdm_chg->log[drdm_log_loop].table_name="CODE_VALUE"))
       SET drdm_mini_loop_status = "NOMV99"
       CALL disp_msg("Unable to perform deletes on the CODE_VALUE table",dm_err->logfile,0)
       CALL merge_audit("FAILREASON","Unable to perform deletes on the CODE_VALUE table",3)
       SET drdm_error_out_ind = 0
      ELSE
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       ROLLBACK
       CALL merge_audit("FAILREASON",dm_err->emsg,1)
       COMMIT
       SET drdm_error_out_ind = 1
       GO TO exit_child
      ENDIF
   ...
   ELSE
    ...
    ELSEIF (findstring("OLDVER",new_pk_where) > 0)
     SET drdm_mini_loop_status = "OLDVER"
     SET stat = alterlist(missing_xlats->qual,0)
    ELSEIF (findstring("BADTRN",new_pk_where) > 0)
     SET drdm_mini_loop_status = "BADTRN"
     SET stat = alterlist(missing_xlats->qual,0)
    ELSEIF (findstring("BADLOG",new_pk_where) > 0)
     SET drdm_mini_loop_status = "BADLOG"
     SET stat = alterlist(missing_xlats->qual,0)
    ELSEIF (findstring("NOMV",new_pk_where) > 0)
     SET drdm_mini_loop_status = "NOMV99"
     SET stat = alterlist(missing_xlats->qual,0)
    ENDIF
    COMMIT
   ENDIF
  ELSE
   SET new_pk_where = drdm_chg->log[drdm_log_loop].pk_where
   IF (findstring("NOXLAT",new_pk_where)=0)
    SET mmc_src_tab_name = dm2_get_rdds_tname(drdm_chg->log[drdm_log_loop].table_name)
    SET drdm_parser->statement[1].frag = concat("select into 'NL:' from ",mmc_src_tab_name," ",
     drdm_table_suffix," ",
     value(new_pk_where)," go")
    CALL parse_statements(1)
    IF (curqual > 0)
     SET nodelete_ind = trigger_proc_call(drdm_chg->log[drdm_log_loop].table_name,new_pk_where,
      drdm_chg->log[drdm_log_loop].context_name,"",0.0)
     IF (nodelete_ind=0)
      COMMIT
     ELSE
     ...
     GO TO exit_child
    ELSE
     SET drdm_parser->statement[1].frag = concat("delete from ",drdm_chg->log[drdm_log_loop].
      table_name," ",drdm_table_suffix," ",
      value(new_pk_where)," go")
     CALL parse_statements(1)
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSE
  SET good_rec = 0
  SET more_dels = 0
  SET no_unique_ident = 0
  IF (more_dels=0)
   IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].lob_process_type="LOB_LONG"))
    SET fail_merges = (fail_merges+ 1)
    SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
    SET fail_merge_audit->num[fail_merges].text =
    "Cannot process rows for tables with a LOB data type in source, but a a LONG data type in target."
    CALL merge_audit(fail_merge_audit->num[fail_merges].action,fail_merge_audit->num[fail_merges].
     text,1)
    IF (drdm_error_out_ind=1)
     ROLLBACK
    ...
    GO TO exit_child
   ENDIF
   SET where_clause = concat(" ",value(drdm_chg->log[drdm_log_loop].pk_where))
   SET drdm_srch_str = dm2_get_rdds_tname(drdm_chg->log[drdm_log_loop].table_name)
   SET long_blob_tab = 0
   FOR (temp_col_cnt = 1 TO perm_col_cnt)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].check_null = 0
     IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].binary_long_ind=1))
      SET long_blob_tab = 1
     ENDIF
   ENDFOR
   SET col_cnt = 1
   ...
   SET drdm_parser_cnt = 2
   SELECT INTO "NL:"
    FROM (dummyt d  WITH seq = perm_col_cnt)
    DETAIL
     IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].db_data_type != "*LOB"))
      drdm_col_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name,
      passed_var = concat("var",cnvtstring(d.seq))
      IF (drdm_parser_cnt > 2)
       drdm_parser->statement[drdm_parser_cnt].frag = " , ", drdm_parser_cnt = (drdm_parser_cnt+ 1)
      ENDIF
      drdm_parser->statement[drdm_parser_cnt].frag = concat(passed_var," = nullind(",
       drdm_table_suffix,".",drdm_col_name,
       ")"), drdm_parser_cnt = (drdm_parser_cnt+ 1)
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].binary_long_ind=1))
       drdm_parser->statement[drdm_parser_cnt].frag = concat(", bloblen = textlen(",drdm_table_suffix,
        ".",drdm_col_name,")"), drdm_parser_cnt = (drdm_parser_cnt+ 1)
      ENDIF
      dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[zero_rec].check_null = 0
     ENDIF
    WITH nocounter
   ;end select
   SET drdm_parser->statement[drdm_parser_cnt].frag = concat(" from ",value(drdm_srch_str)," t",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix)
   IF (long_blob_tab=1)
    SET drdm_parser->statement[(drdm_parser_cnt+ 1)].frag = concat(value(where_clause),
     " head report outbuf = fillstring(32767,  ' ') offset=0 retlen=0 freers = 0 detail ")
    SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
   ELSE
    SET drdm_parser->statement[(drdm_parser_cnt+ 1)].frag = concat(value(where_clause)," detail ")
    SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
   ENDIF
   SELECT INTO "NL:"
    FROM (dummyt d  WITH seq = perm_col_cnt)
    DETAIL
     IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].db_data_type != "*LOB"))
      drdm_col_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name,
      passed_var = concat("var",cnvtstring(d.seq)), drdm_parser_cnt = (drdm_parser_cnt+ 1),
      drdm_parser->statement[drdm_parser_cnt].frag = concat(
       " dm2_ref_Data_doc->tbl_qual[temp_tbl_cnt].col_qual[",trim(cnvtstring(d.seq)),
       "].check_null = ",passed_var), drdm_parser_cnt = (drdm_parser_cnt+ 1)
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].binary_long_ind=1))
       drdm_parser->statement[drdm_parser_cnt].frag = concat(" if (freers = 0) RS_",dm2_ref_data_doc
        ->tbl_qual[temp_tbl_cnt].suffix,"->from_values.",drdm_col_name," = '' endif "),
       drdm_parser_cnt = (drdm_parser_cnt+ 1), drdm_parser->statement[drdm_parser_cnt].frag = concat(
        "retlen = 1 while (retlen > 0) retlen = ","blobget(outbuf,offset,",drdm_table_suffix,".",
        drdm_col_name,
        ")"),
       drdm_parser_cnt = (drdm_parser_cnt+ 1), drdm_parser->statement[drdm_parser_cnt].frag =
       " offset = offset + retlen ", drdm_parser_cnt = (drdm_parser_cnt+ 1),
       drdm_parser->statement[drdm_parser_cnt].frag = " if (freers = 0)", drdm_parser_cnt = (
       drdm_parser_cnt+ 1), drdm_parser->statement[drdm_parser_cnt].frag = concat(" RS_",
        dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,"->from_values.",drdm_col_name," = "),
       drdm_parser_cnt = (drdm_parser_cnt+ 1), drdm_parser->statement[drdm_parser_cnt].frag = concat(
        "notrim(concat(trim(RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,"->from_values.",
        drdm_col_name,"),notrim(outbuf)))"), drdm_parser_cnt = (drdm_parser_cnt+ 1),
       drdm_parser->statement[drdm_parser_cnt].frag = " else ", drdm_parser_cnt = (drdm_parser_cnt+ 1
       ), drdm_parser->statement[drdm_parser_cnt].frag = concat(" RS_",dm2_ref_data_doc->tbl_qual[
        temp_tbl_cnt].suffix,"->from_values.",drdm_col_name," = "),
       drdm_parser_cnt = (drdm_parser_cnt+ 1), drdm_parser->statement[drdm_parser_cnt].frag = concat(
        "notrim(concat(notrim(RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,"->from_values.",
        drdm_col_name,"),notrim(outbuf)))"), drdm_parser_cnt = (drdm_parser_cnt+ 1),
       drdm_parser->statement[drdm_parser_cnt].frag = " endif  freers = 1 endwhile"
      ELSE
       drdm_parser->statement[drdm_parser_cnt].frag = concat(" RS_",dm2_ref_data_doc->tbl_qual[
        temp_tbl_cnt].suffix,"->from_values.",drdm_col_name," = ",
        drdm_table_suffix,".",drdm_col_name), drdm_parser_cnt = (drdm_parser_cnt+ 1), drdm_parser->
       statement[drdm_parser_cnt].frag = concat(" dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[",
        trim(cnvtstring(d.seq)),"].check_space = 0")
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
   IF (long_blob_tab=1)
    SET drdm_parser->statement[drdm_parser_cnt].frag = " with nocounter, rdbarrayfetch=1 go"
   ELSE
    SET drdm_parser->statement[drdm_parser_cnt].frag = " with nocounter go"
   ENDIF
   CALL parse_statements(drdm_parser_cnt)
   ...
   ENDIF
   IF (curqual=0)
    SET drdm_mini_loop_status = "NO SRC"
    SET dm_err->emsg = concat("A ",drdm_chg->log[drdm_log_loop].table_name," row ",trim(drdm_chg->
      log[drdm_log_loop].pk_where)," wasn't found in the source.")
    SET dm_err->emsg = substring(1,110,dm_err->emsg)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->err_ind = 0
    SET dm_err->emsg = concat("A ",drdm_chg->log[drdm_log_loop].table_name," row ",trim(drdm_chg->
      log[drdm_log_loop].pk_where)," was udpated to be NO SRC.")
    SET dm_err->emsg = substring(1,110,dm_err->emsg)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->err_ind = 0
    GO TO exit_child
   ELSE
    ...
    CALL parser(concat("set RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
      "->to_table_name = drdm_chg->log[drdm_log_loop].table_name go"),1)
    FOR (space_loop = 1 TO perm_col_cnt)
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[space_loop].data_type IN ("VC", "C*"))
       AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[space_loop].binary_long_ind=0))
       CALL parser(concat("set space_size = size(RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
         "->from_values.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[space_loop].column_name,
         ") go"),1)
       IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[space_loop].check_null=0)
        AND space_size=0)
        SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[space_loop].check_space = 1
       ENDIF
      ENDIF
    ENDFOR
    ...
    CALL echo("")
    CALL echo("")
    CALL echo(build("Post RS = ",curmem))
    CALL echo("")
    CALL echo("")
    CALL echo("**************LOOKING UP TRANSLATIONS***************")
    CALL echo("")
    CALL echo("")
    SET dm_err->eproc = concat("Looking up Translations",cnvtstring(drdm_chg->log[drdm_log_loop].
      log_id))
    IF ((((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].custom_script != "")
     AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].custom_script != " ")) OR ((dm2_ref_data_doc->
    tbl_qual[temp_tbl_cnt].version_ind=1))) )
     FOR (another_loop = 1 TO perm_col_cnt)
      SET looked_up = lookup_translations(temp_tbl_cnt,another_loop)
      IF (drdm_error_out_ind=1)
       SET another_loop = perm_col_cnt
      ENDIF
     ENDFOR
     SET dm_err->eproc = concat("Handling the VERSION_NBR column",cnvtstring(drdm_chg->log[
       drdm_log_loop].log_id))
     FOR (another_loop = 1 TO perm_col_cnt)
       IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[another_loop].exception_flg=9)
        AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[another_loop].version_nbr_child_ind=1)
       )
        CALL translate_vers_col(temp_tbl_cnt,another_loop)
       ENDIF
     ENDFOR
    ELSE
     FOR (another_loop = 1 TO perm_col_cnt)
       IF ((((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[another_loop].pk_ind=1)) OR ((
       dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[another_loop].unique_ident_ind=1))) )
        SET looked_up = lookup_translations(temp_tbl_cnt,another_loop)
        IF (drdm_error_out_ind=1)
         SET another_loop = perm_col_cnt
        ENDIF
     ...
     FOR (another_loop = 1 TO perm_col_cnt)
       IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[another_loop].exception_flg=9)
        AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[another_loop].version_nbr_child_ind=1)
        AND (((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[another_loop].pk_ind=1)) OR ((
       dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[another_loop].unique_ident_ind=1))) )
        CALL translate_vers_col(temp_tbl_cnt,another_loop)
       ENDIF
     ENDFOR
    ...
    ENDIF
    IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].custom_script != "")
     AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].custom_script != " "))
     SET dm2_ref_data_reply->error_ind = 0
     SET dm2_ref_data_reply->error_msg = ""
     SET stat = alterlist(dm2_ref_data_reply->qual,0)
     CALL parser(concat("execute ",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].custom_script," go"),1)
     IF (drdm_error_out_ind=1)
      GO TO exit_child
     ENDIF
     IF ((dm2_ref_data_reply->error_ind=0))
      FOR (cust_cnt = 1 TO size(dm2_ref_data_reply->qual,5))
        SET mmc_imt_check = insert_merge_translate(dm2_ref_data_reply->qual[cust_cnt].from_value,
         dm2_ref_data_reply->qual[cust_cnt].to_value,dm2_ref_data_reply->qual[cust_cnt].table_name)
        IF (mmc_imt_check=1)
        ...
        ENDIF
      ENDFOR
      IF (((drdm_error_out_ind=1) OR (nodelete_ind=1)) )
       GO TO exit_child
      ENDIF
      SET drdm_mini_loop_status = "MERGED"
     ...
     ELSE
      ...
      CALL echo("")
      CALL echo("")
      IF ((dm2_ref_data_reply->error_msg="ORPHAN*"))
       CALL orphan_child_tab(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name,"ORPHAN")
       IF (nodelete_ind=0)
        SET drdm_mini_loop_status = "ORPHAN"
       ENDIF
      ELSEIF ((dm2_ref_data_reply->error_msg="OLDVER"))
       SET drdm_no_trans_ind = 1
       SET drdm_mini_loop_status = "OLDVER"
       CALL orphan_child_tab(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name,"OLDVER")
      ELSEIF ((dm2_ref_data_reply->error_msg="NOMV*"))
       SET drdm_no_trans_ind = 1
       SET drdm_mini_loop_status = dm2_ref_data_reply->error_msg
       CALL orphan_child_tab(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name,
        drdm_mini_loop_status)
      ELSEIF ((dm2_ref_data_reply->error_msg="BADLOG"))
       SET drdm_no_trans_ind = 1
       SET drdm_mini_loop_status = "BADLOG"
       CALL orphan_child_tab(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name,"BADLOG")
      ELSE
       SET fail_merges = (fail_merges+ 1)
       SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
       SET fail_merge_audit->num[fail_merges].text = dm2_ref_data_reply->error_msg
       SET drdm_no_trans_ind = 1
       CALL merge_audit(fail_merge_audit->num[fail_merges].action,fail_merge_audit->num[fail_merges].
        text,2)
       IF (drdm_error_out_ind=1)
    ...
    ELSEIF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].version_ind=1))
     ...
     ELSE
      ...
      IF ((dm2_ref_data_reply->error_msg="ORPHAN*"))
       ...
       ENDIF
      ELSEIF ((dm2_ref_data_reply->error_msg="OLDVER"))
       IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].child_flag=1))
        SET drdm_no_trans_ind = 1
        SET drdm_mini_loop_status = "OLDVER"
       ENDIF
      ELSEIF ((dm2_ref_data_reply->error_msg="NOMV*"))
       CALL orphan_child_tab(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name,dm2_ref_data_reply->
        error_msg)
       IF (nodelete_ind=0)
        SET drdm_no_trans_ind = 1
        SET drdm_mini_loop_status = dm2_ref_data_reply->error_msg
     ...
     ENDIF
    ELSE
     SET been_translated = translate_column(temp_tbl_cnt,perm_col_cnt)
     IF (drdm_error_out_ind=1)
      GO TO exit_child
     ENDIF
     FOR (another_loop = 1 TO perm_col_cnt)
       IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[another_loop].pk_ind != 1)
        AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[another_loop].unique_ident_ind != 1))
        SET looked_up = lookup_translations(temp_tbl_cnt,another_loop)
        IF (drdm_error_out_ind=1)
         SET another_loop = perm_col_cnt
        ENDIF
     ...
     FOR (another_loop = 1 TO perm_col_cnt)
       IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[another_loop].exception_flg=9)
        AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[another_loop].version_nbr_child_ind=1)
        AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[another_loop].pk_ind != 1)
        AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[another_loop].unique_ident_ind != 1))
        CALL translate_vers_col(temp_tbl_cnt,another_loop)
       ENDIF
     ENDFOR
     ...
     CALL echo("")
     CALL echo("")
     SET dm_err->eproc = concat("Checking ID and CD columns ",cnvtstring(drdm_chg->log[drdm_log_loop]
       .log_id))
     SET rpt_orphan_ind = 0
     SET rpt_oldver_ind = 0
     FOR (final_check_loop = 1 TO perm_col_cnt)
       IF (value(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].idcd_ind)=1)
        IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].translated=0))
         ...
         IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].parent_entity_col
          != ""))
          CALL parser(concat("set rpt_from = RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
            "->from_values.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].
            column_name," go"),1)
          SET rpt_col_value = value(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
           final_check_loop].parent_entity_col)
          SET rpt_col_pos = locateval(rpt_srch,1,perm_col_cnt,rpt_col_value,dm2_ref_data_doc->
           tbl_qual[temp_tbl_cnt].col_qual[rpt_srch].column_name)
          IF (rpt_col_pos > 0)
           SET rpt_data_type = value(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[rpt_col_pos].
            data_type)
           IF (rpt_data_type IN ("VC", "C*"))
            ...
            IF (rpt_fnd != 0)
             CALL parser(concat("set rpt_parent_col = cnvtupper(RS_",dm2_ref_data_doc->tbl_qual[
               temp_tbl_cnt].suffix,"->from_values.",rpt_col_value,") go"),1)
             IF (rpt_parent_col != ""
              AND rpt_parent_col != " ")
              SET rpt_parent_col = find_p_e_col(rpt_parent_col,final_check_loop)
             ELSE
              SET rpt_i_domain = concat("RDDS_PE_ABBREV:",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
               table_name)
              SET rpt_i_name = concat(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
               final_check_loop].column_name,":",rpt_parent_col)
              SELECT INTO "NL:"
               FROM dm_info d
          ...
          IF (rpt_parent_col != "INVALIDTABLE"
           ...
           ENDIF
          ENDIF
         ELSEIF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].
         root_entity_name != ""))
          SET rpt_table = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].
          root_entity_name
          SET rpt_column = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].
          root_entity_attr
          CALL parser(concat("set rpt_from = RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
            "->from_values.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].
            column_name," go"),1)
         ...
         IF (rpt_table != ""
          AND rpt_from != 0
          AND (((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].root_entity_name
          != dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name)) OR ((((dm2_ref_data_doc->tbl_qual[
         temp_tbl_cnt].col_qual[final_check_loop].root_entity_attr != dm2_ref_data_doc->tbl_qual[
         temp_tbl_cnt].col_qual[final_check_loop].column_name)) OR ((dm2_ref_data_doc->tbl_qual[
         temp_tbl_cnt].col_qual[final_check_loop].pk_ind != 1))) )) )
          SET rpt_srch = locateval(rpt_mult_cnt,1,size(dm2_ref_data_doc->tbl_qual,5),rpt_table,
           dm2_ref_data_doc->tbl_qual[rpt_mult_cnt].table_name)
          IF (rpt_srch=0)
           SET rpt_mult_cnt = temp_tbl_cnt
           SET rpt_srch = fill_rs("TABLE",rpt_table)
           SET temp_tbl_cnt = rpt_mult_cnt
          ENDIF
          IF ((((dm2_ref_data_doc->tbl_qual[rpt_srch].mergeable_ind=0)) OR ((dm2_ref_data_doc->
          tbl_qual[rpt_srch].reference_ind=0)
           AND  NOT ((dm2_ref_data_doc->tbl_qual[rpt_srch].table_name IN ("ACCESSION", "ADDRESS",
          "PHONE", "PERSON", "PERSON_NAME",
          "PERSON_ALIAS", "DCP_ENTITY_RELTN", "LONG_TEXT", "LONG_BLOB", "ACCOUNT",
          "AT_ACCT_RELTN"))))) )
           SET rpt_missing = "NOMV04"
          ELSE
           SET rpt_missing = report_missing(rpt_table,rpt_column,rpt_from)
          ENDIF
          IF (rpt_missing="ORPHAN")
           SET rpt_orphan_ind = 1
           IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].
           defining_attribute_ind != 0))
            SET orphaned_col_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
            final_check_loop].column_name
           ENDIF
          ENDIF
          IF (rpt_missing="OLDVER")
           SET rpt_oldver_ind = 1
          ENDIF
          IF (rpt_missing="NOMV*")
           SET nomv_ind = 1
           SET nomv_type = rpt_missing
          ENDIF
         ENDIF
         IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].
         defining_attribute_ind != 0))
          SET idcd_check = 1
          SET drdm_no_trans_ind = 1
          IF ( NOT (rpt_missing IN ("OLDVER", "NOMV*")))
           SET dm_err->eproc = concat("This log_id ","wasn't translated because of the ",
            dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].column_name,
            " column.")
           CALL echo("")
           CALL echo("")
           CALL echo(dm_err->eproc)
           ...
           SET fail_merges = (fail_merges+ 1)
           SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
           SET fail_merge_audit->num[fail_merges].text = dm_err->eproc
           CALL merge_audit(fail_merge_audit->num[fail_merges].action,fail_merge_audit->num[
            fail_merges].text,2)
          ELSE
           SET dm_err->eproc = concat("A parent value from ",rpt_table,
            " has an exception log_type of ",rpt_missing)
           IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].child_flag=1))
            CALL merge_audit("FAILREASON",dm_err->eproc,1)
           ELSE
            SET drdm_no_trans_ind = 0
            CALL merge_audit("FAILREASON",dm_err->eproc,2)
           ENDIF
          ENDIF
          IF (drdm_error_out_ind=1)
           ROLLBACK
           SET final_check_loop = perm_col_cnt
          ENDIF
          IF (rpt_orphan_ind=1)
           SET final_check_loop = perm_col_cnt
           CALL orphan_child_tab(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name,"ORPHAN")
         ...
         ELSE
          IF (idcd_check=0)
           ...
           ELSE
            SET rpt_orphan_ind = 0
            CALL parser(concat("set RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
              "->to_values.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].
              column_name," = 0 go"),1)
            SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].translated = 1
           ENDIF
          ELSE
           IF (rpt_orphan_ind=0)
            SET idcd_check = 1
        ...
        ENDIF
       ENDIF
       FREE SET rpt_col_value
       FREE SET rpt_fnd
       FREE SET rpt_srch
       FREE SET rpt_parent_col
       FREE SET rpt_i_domain
       FREE SET rpt_i_name
       FREE SET rpt_mult_cnt
       SET rpt_table = ""
       SET rpt_column = ""
       SET rpt_from = 0
     ENDFOR
     FOR (final_check_loop = 1 TO perm_col_cnt)
       IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].exception_flg=9)
        AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].translated=0))
        SET idcd_check = 1
        SET drdm_no_trans_ind = 1
        SET dm_err->eproc = concat("This log_id ","wasn't translated because of the ",
         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].column_name," column.")
        CALL echo("")
        CALL echo("")
        CALL echo(dm_err->eproc)
        CALL echo("")
     ...
     IF (drdm_error_out_ind=1)
      GO TO exit_child
     ENDIF
     IF (((rpt_orphan_ind=1) OR (((rpt_oldver_ind=1) OR (nomv_ind=1)) )) )
      SET no_insert_update = 1
      IF (rpt_orphan_ind=1)
       SET drdm_mini_loop_status = "ORPHAN"
       CALL orphan_child_tab(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name,"ORPHAN")
      ELSEIF (rpt_oldver_ind=1)
       IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].child_flag=1))
        SET drdm_mini_loop_status = "OLDVER"
       ENDIF
       CALL orphan_child_tab(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name,"OLDVER")
      ELSEIF (nomv_ind=1)
       SET drdm_mini_loop_status = nomv_type
       CALL orphan_child_tab(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name,
        drdm_mini_loop_status)
      ENDIF
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].child_flag != 2)
       AND rpt_oldver_ind != 1)
       COMMIT
      ENDIF
      SET insert_update_reason = drdm_mini_loop_status
     ENDIF
     SET dm_err->eproc = concat("Status of translation",cnvtstring(drdm_chg->log[drdm_log_loop].
       log_id))
     IF (no_insert_update=1)
      SET insert_update_reason = concat(insert_update_reason," ",cnvtstring(drdm_chg->log[
        drdm_log_loop].log_id))
      CALL echo("")
      CALL echo("")
      CALL echo(insert_update_reason)
      CALL echo("")
      CALL echo("")
      SET fail_merges = (fail_merges+ 1)
      SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
      SET fail_merge_audit->num[fail_merges].text = insert_update_reason
      CALL merge_audit(fail_merge_audit->num[fail_merges].action,fail_merge_audit->num[fail_merges].
       text,2)
      IF (drdm_error_out_ind=1)
       ROLLBACK
     ...
     ENDIF
     IF (idcd_check=0)
      SET inserted = insert_update_row(temp_tbl_cnt,perm_col_cnt)
      IF (nodelete_ind=0)
       IF (ins_ind=1
        AND (((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name="PRSNL")) OR ((dm2_ref_data_doc->
       tbl_qual[temp_tbl_cnt].table_name="PRSNL0386"))) )
        INSERT  FROM dm_chg_log
         SET log_type = "PRSSEC", log_id = seq(dm_clinical_seq,nextval), table_name = "PRSNL",
          target_env_id = dm2_ref_data_doc->env_target_id, pk_where = concat(rs_0386->to_values.
           username,"&",rs_0386->to_values.name_full_formatted), chg_dt_tm = cnvtdatetime(curdate,
           curtime3)
        ;end insert
        IF (check_error(dm_err->eproc)=1)
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       ...
       ELSE
        SET drdm_mini_loop_status = "MERGED"
       ENDIF
       IF (drdm_mini_loop_status IN ("MERGED", "ORPHAN"))
        FOR (merge_loop = 1 TO current_merges)
         CALL merge_audit(child_merge_audit->num[merge_loop].action,child_merge_audit->num[merge_loop
          ].text,3)
         IF (drdm_error_out_ind=1)
       ...
       ENDIF
      ELSE
       SET nodelete_msg = concat("Could not insert log_id ",cnvtstring(drdm_chg->log[drdm_log_loop].
         log_id))
       CALL echo("")
       CALL echo("")
       CALL echo(nodelete_msg)
       ...
       SET fail_merges = (fail_merges+ 1)
       SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
       SET fail_merge_audit->num[fail_merges].text = nodelete_msg
       CALL merge_audit(fail_merge_audit->num[fail_merges].action,fail_merge_audit->num[fail_merges].
        text,2)
       IF (drdm_error_out_ind=1)
        ROLLBACK
  ...
  ELSE
   SET nodelete = 1
   CALL echo("**********CANNOT BE TRANSLATED YET BECAUSE OF DELETES**********")
  ENDIF
 ENDIF
 SUBROUTINE lookup_translations(lt_temp_tbl_cnt,lt_temp_col_cnt)
   DECLARE drdm_root_name = vc
   DECLARE drdm_parent_col = vc
   DECLARE drdm_old_value = vc
   DECLARE drdm_table_name = vc
   DECLARE lt_data_type = vc
   DECLARE lt_col_pos = i4
   DECLARE lt_col_loop = i4
   DECLARE source = f8
   DECLARE target = f8
   DECLARE drdm_col_name = vc
   DECLARE pe_fnd = i4
   DECLARE pe_srch = i4
   DECLARE sbr_i_domain = vc
   DECLARE sbr_i_name = vc
   DECLARE sbr_new_seq = f8
   DECLARE lt_temp_rm = vc
   DECLARE lt_temp_pos = i4
   DECLARE lt_temp_ndx = i4
   DECLARE lt_temp_loop = i4
   DECLARE lt_temp_colname = vc
   SET drdm_root_name = value(dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].
    root_entity_name)
   IF (drdm_root_name=" ")
    SET drdm_root_name = ""
   ENDIF
   IF (drdm_root_name="PERSON"
    AND (dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].table_name != "PRSNL"))
    SET drdm_root_name = "PRSNL"
    SET dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].root_entity_name =
    "PRSNL"
   ENDIF
   SET drdm_col_name = value(dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].
    column_name)
   SET drdm_old_value = value(dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].
    parent_entity_col)
   IF (drdm_old_value=" ")
    SET drdm_old_value = ""
   ENDIF
   SET drdm_table_name = concat("RS_",dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].suffix)
   SET target = value(dm2_ref_data_doc->env_target_id)
   SET source = value(dm2_ref_data_doc->env_source_id)
   IF ((dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].idcd_ind=0))
    IF ((dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].exception_flg != 9))
     DECLARE sze = i4
     CALL parser(concat("set sze = size(",drdm_table_name,"->from_values.",drdm_col_name,") go"),1)
     IF ((dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].data_type="VC")
      AND (dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].check_null=0)
      AND sze=0)
      CALL parser(concat("set ",drdm_table_name,"->to_values.",drdm_col_name,' = " " go'),1)
     ELSE
      CALL parser(concat("set ",drdm_table_name,"->to_values.",drdm_col_name," = ",
        drdm_table_name,"->from_values.",drdm_col_name," go"),1)
     ENDIF
     SET dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated = 1
    ENDIF
   ENDIF
   IF ((dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated=0))
    IF (value(dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].exception_flg)=1)
     CALL parser(concat("set ",drdm_table_name,"->to_values.",drdm_col_name," = ",
       drdm_table_name,"->from_values.",drdm_col_name," go"),1)
     SET current_merges = (current_merges+ 1)
     SET child_merge_audit->num[current_merges].action = "EXCEPT1"
     SET child_merge_audit->num[current_merges].text = concat(dm2_ref_data_doc->tbl_qual[
      lt_temp_tbl_cnt].table_name,"  ",drdm_col_name)
     SET dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated = 1
    ENDIF
   ENDIF
   IF ((dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated=0))
    IF (value(dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].exception_flg)=6)
     SET drdm_from_con = value(dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].
      sequence_name)
     SET drdm_parser->statement[1].frag = concat("select into 'nl:' y=seq(",drdm_from_con,
      ", nextval) ")
     SET drdm_parser->statement[2].frag = "from dual detail sbr_new_seq = y with nocounter go"
     CALL parse_statements(2)
     CALL parser(concat("set ",drdm_table_name,"->to_values.",drdm_col_name," = sbr_new_seq go"),1)
     SET current_merges = (current_merges+ 1)
     SET child_merge_audit->num[current_merges].action = "EXCEPT6"
     SET child_merge_audit->num[current_merges].text = concat(dm2_ref_data_doc->tbl_qual[
      lt_temp_tbl_cnt].table_name,"  ",drdm_col_name)
     SET dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated = 1
    ENDIF
   ENDIF
   IF ((dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated=0))
    IF (value(dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].constant_value)
     != null)
     SET dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].check_null = 0
     SET drdm_from_con = dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].
     constant_value
     CALL parser(concat("set ",drdm_table_name,"->to_values.",drdm_col_name," = ",
       drdm_from_con," go"),1)
     SET current_merges = (current_merges+ 1)
     SET child_merge_audit->num[current_merges].action = "CONSTANTS"
     SET child_merge_audit->num[current_merges].text = concat(dm2_ref_data_doc->tbl_qual[
      lt_temp_tbl_cnt].table_name,"  ",drdm_col_name)
     SET dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated = 1
    ENDIF
   ENDIF
   IF ((dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].idcd_ind=1))
    CALL parser(concat("set from_con = cnvtstring(",drdm_table_name,"->from_values.",drdm_col_name,
      ") go"),1)
    IF (cnvtreal(from_con) <= 0)
     CALL parser(concat("set ",drdm_table_name,"->to_values.",drdm_col_name," = ",
       from_con," go"),1)
     SET dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated = 1
    ENDIF
   ENDIF
   IF ((dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated=0)
    AND (dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].exception_flg != 9))
    SET drdm_from_con = concat(drdm_table_name,"->from_values.",drdm_col_name)
    CALL parser(build("set drdm_col_value = ",drdm_from_con," go"),1)
    IF (drdm_root_name != "")
     SET drdm_from_con = select_merge_translate(cnvtstring(drdm_col_value),drdm_root_name)
     IF (drdm_from_con != "No Trans")
      CALL parser(concat("set ",drdm_table_name,"->to_values.",drdm_col_name,
        " = cnvtreal(drdm_from_con) go"),1)
      SET dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated = 1
     ENDIF
    ELSE
     IF (drdm_old_value != "")
      SET lt_col_pos = locateval(lt_col_loop,1,perm_col_cnt,drdm_old_value,dm2_ref_data_doc->
       tbl_qual[lt_temp_tbl_cnt].col_qual[lt_col_loop].column_name)
      IF (lt_col_pos > 0)
       SET lt_data_type = value(dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_col_pos].
        data_type)
       IF ( NOT (lt_data_type IN ("VC", "C*")))
        CALL echo(concat(drdm_col_name,
          " has an invalid value in the PARENT_ENTITY_COL of column level metadata."))
        SET fail_merges = (fail_merges+ 1)
        SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
        SET fail_merge_audit->num[fail_merges].text = concat(drdm_col_name,
         "has an invalid value in the PARENT_ENTITY_COL of column level metadata.")
        CALL merge_audit(fail_merge_audit->num[fail_merges].action,fail_merge_audit->num[fail_merges]
         .text,1)
        IF (drdm_error_out_ind=1)
        ...
        ENDIF
        SET drdm_mini_loop_status = "NOMV99"
        CALL orphan_child_tab(dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].table_name,"NOMV99")
        SET drdm_parent_col = "INVALIDTABLE"
       ELSE
        SET pe_fnd = 0
        SET pe_fnd = locateval(pe_srch,1,dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_cnt,
         drdm_old_value,dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[pe_srch].column_name)
        IF (pe_fnd != 0)
         CALL parser(concat("set drdm_parent_col = cnvtupper(",drdm_table_name,"->from_values.",
           drdm_old_value,") go"),1)
         IF (drdm_parent_col != ""
          AND drdm_parent_col != " ")
          SET drdm_parent_col = find_p_e_col(drdm_parent_col,lt_temp_col_cnt)
         ELSE
          SET drdm_parent_col = "INVALIDTABLE"
          SET sbr_i_domain = concat("RDDS_PE_ABBREV:",dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].
           table_name)
          SET sbr_i_name = concat(drdm_col_name,":",drdm_parent_col)
          SELECT INTO "NL:"
           FROM dm_info d
           WHERE d.info_domain=sbr_i_domain
            AND d.info_name=sbr_i_name
           DETAIL
            drdm_parent_col = d.info_char
           WITH nocounter
          ;end select
          IF (drdm_parent_col="INVALIDTABLE")
           CALL parser(concat("set ",drdm_table_name,"->to_values.",drdm_col_name," = 0 go"),1)
           SET dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated = 1
          ENDIF
         ENDIF
        ELSE
         SET drdm_parent_col = "INVALIDTABLE"
         CALL echo("The PARENT_ENTITY_COL value in column level metadata is not valid.")
         SET fail_merges = (fail_merges+ 1)
         SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
         SET fail_merge_audit->num[fail_merges].text =
         "THE PARENT_ENTITY_COL VALUE IN COLUMN LEVEL METADATA IS NOT VALID"
         CALL merge_audit(fail_merge_audit->num[fail_merges].action,fail_merge_audit->num[fail_merges
          ].text,1)
         IF (drdm_error_out_ind=1)
          ROLLBACK
     ...
     ENDIF
     IF (drdm_parent_col != "INVALIDTABLE")
      SET drdm_from_con = select_merge_translate(cnvtstring(drdm_col_value),drdm_parent_col)
      IF (drdm_from_con != "No Trans")
       CALL parser(concat("set ",drdm_table_name,"->to_values.",drdm_col_name,
         " = cnvtreal(drdm_from_con) go"),1)
       SET dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated = 1
    ...
    ENDIF
    IF ((dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated=0))
     IF ((dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].
     defining_attribute_ind=0))
      IF (drdm_root_name != ""
       AND (((drdm_root_name != dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].table_name)) OR ((((
      dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].root_entity_attr !=
      dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].column_name)) OR ((
      dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].pk_ind != 1))) )) )
       SET lt_temp_rm = report_missing(drdm_root_name,dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].
        col_qual[lt_temp_col_cnt].root_entity_attr,drdm_col_value)
      ELSEIF (drdm_parent_col != ""
       AND drdm_parent_col != "INVALIDTABLE")
       SET lt_temp_pos = locateval(lt_temp_ndx,1,dguc_reply->rs_tbl_cnt,drdm_parent_col,dguc_reply->
        dtd_hold[lt_temp_ndx].tbl_name)
       IF (lt_temp_pos != 0)
        IF ((dguc_reply->dtd_hold[lt_temp_pos].pk_cnt >= 1))
         SET lt_temp_ndx = 0
         FOR (lt_temp_loop = 1 TO dguc_reply->dtd_hold[lt_temp_pos].pk_cnt)
           IF ((((dguc_reply->dtd_hold[lt_temp_pos].pk_hold[lt_temp_loop].pk_name="*ID")) OR ((
           dguc_reply->dtd_hold[lt_temp_pos].pk_hold[lt_temp_loop].pk_name="CODE_VALUE"))) )
            SET rpt_column = dguc_reply->dtd_hold[lt_temp_pos].pk_hold[lt_temp_loop].pk_name
            SET lt_temp_ndx = (lt_temp_ndx+ 1)
           ENDIF
         ENDFOR
        ENDIF
        IF (lt_temp_ndx > 1)
       ...
       ENDIF
       IF (lt_temp_colname != "")
        SET lt_temp_rm = report_missing(drdm_parent_col,lt_temp_colname,drdm_col_value)
       ENDIF
      ENDIF
      IF (lt_temp_rm="ORPHAN")
       CALL parser(concat("set ",drdm_table_name,"->to_values.",drdm_col_name," = 0 go"),1)
       SET dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated = 1
      ENDIF
     ENDIF
    ENDIF
    IF ((dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated=1))
     SET current_merges = (current_merges+ 1)
     SET child_merge_audit->num[current_merges].action = "LOOKUPS"
     SET child_merge_audit->num[current_merges].text = concat(dm2_ref_data_doc->tbl_qual[
      lt_temp_tbl_cnt].table_name,"  ",drdm_col_name)
    ENDIF
   ENDIF
   IF ((dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated=0))
    IF ((dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].exception_flg=7))
     SET drdm_from_con = concat(drdm_table_name,"->from_values.",dm2_ref_data_doc->tbl_qual[
      lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].column_name)
     CALL parser(concat("set from_this = ",drdm_from_con," go"),1)
     SET drdm_from_con = value(dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].
      sequence_name)
     SET drdm_parser->statement[1].frag = concat("select into 'nl:' y=seq(",drdm_from_con,
      ", nextval) ")
     SET drdm_parser->statement[2].frag = "from dual detail to_this = y with nocounter go"
     CALL parse_statements(2)
     SET drdm_chg->log[drdm_log_loop].reprocess_ind = 1
     SET mmc_imt_check = insert_merge_translate(from_this,to_this,dm2_ref_data_doc->tbl_qual[
      lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].root_entity_name)
     IF (drdm_error_out_ind=1)
      ROLLBACK
     ENDIF
     CALL parser(concat("set ",drdm_table_name,"->to_values.",drdm_col_name," = to_this go"),1)
     SET dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated = 1
     SET current_merges = (current_merges+ 1)
     SET child_merge_audit->num[current_merges].action = "EXCEPT7"
     SET child_merge_audit->num[current_merges].text = concat(dm2_ref_data_doc->tbl_qual[
      lt_temp_tbl_cnt].table_name,"  ",drdm_col_name)
    ENDIF
   ENDIF
   FREE SET drdm_root_name
   FREE SET drdm_parent_col
   FREE SET drdm_old_value
   FREE SET drdm_table_name
   FREE SET source
   FREE SET target
   RETURN(dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].col_qual[lt_temp_col_cnt].translated)
 END ;Subroutine
 SUBROUTINE translate_column(tc_temp_tbl_cnt,tc_perm_col_cnt)
   DECLARE query_value = f8
   DECLARE translated_value = i4
   DECLARE go_flag = i2
   DECLARE ui_check = i2
   DECLARE drdm_table_name = vc
   DECLARE tc_re_attr = vc
   DECLARE tc_col_name = vc
   SET ui_check = 0
   SET dm_err->eproc = concat("Translate columns",cnvtstring(drdm_chg->log[drdm_log_loop].log_id))
   CALL echo("")
   CALL echo("")
   ...
   SET drdm_return_var = 0
   SET temp_col_cnt = 1
   WHILE (temp_col_cnt <= tc_perm_col_cnt)
     SET go_flag = 0
     SET drdm_table_name = concat("RS_",dm2_ref_data_doc->tbl_qual[tc_temp_tbl_cnt].suffix)
     SET drdm_from_con = value(dm2_ref_data_doc->tbl_qual[tc_temp_tbl_cnt].col_qual[temp_col_cnt].
      root_entity_name)
     SET drdm_to_con = value(dm2_ref_data_doc->tbl_qual[tc_temp_tbl_cnt].table_name)
     SET tc_re_attr = value(dm2_ref_data_doc->tbl_qual[tc_temp_tbl_cnt].col_qual[temp_col_cnt].
      root_entity_attr)
     SET tc_col_name = value(dm2_ref_data_doc->tbl_qual[tc_temp_tbl_cnt].col_qual[temp_col_cnt].
      column_name)
     IF (drdm_from_con=drdm_to_con
      AND tc_re_attr=tc_col_name)
      IF ((dm2_ref_data_doc->tbl_qual[tc_temp_tbl_cnt].col_qual[temp_col_cnt].translated=1))
       SET go_flag = 1
      ELSE
       IF ((dm2_ref_data_doc->tbl_qual[tc_temp_tbl_cnt].merge_delete_ind=1))
        SET ui_check = 1
       ELSE
        FOR (unique_ident_loop = 1 TO tc_perm_col_cnt)
          IF ((dm2_ref_data_doc->tbl_qual[tc_temp_tbl_cnt].col_qual[unique_ident_loop].
          unique_ident_ind=1)
           AND go_flag=0)
           SET ui_check = 1
           IF ((dm2_ref_data_doc->tbl_qual[tc_temp_tbl_cnt].col_qual[unique_ident_loop].translated=0)
           )
            SET drdm_return_var = 1
            SET go_flag = 1
           ENDIF
          ENDIF
        ENDFOR
       ...
       IF (go_flag=0
        AND ui_check=1)
        SET query_value = query_target(tc_temp_tbl_cnt,tc_perm_col_cnt)
        IF (query_value >= 0)
         SET drdm_from_con = dm2_ref_data_doc->tbl_qual[tc_temp_tbl_cnt].col_qual[temp_col_cnt].
         column_name
         CALL parser(concat("set ",drdm_table_name,"->to_values.",drdm_from_con," = query_value go"),
          1)
         SET dm2_ref_data_doc->tbl_qual[tc_temp_tbl_cnt].col_qual[temp_col_cnt].translated = 1
         CALL parser(concat("set to_this = ",drdm_table_name,"->to_values.",drdm_from_con," go"),1)
         CALL parser(concat("set from_this = ",drdm_table_name,"->from_values.",drdm_from_con," go"),
          1)
         SET mmc_imt_check = insert_merge_translate(from_this,to_this,dm2_ref_data_doc->tbl_qual[
          tc_temp_tbl_cnt].table_name)
         IF (drdm_error_out_ind=1)
          ROLLBACK
         ENDIF
        ELSEIF ((query_value=- (3)))
         SET drdm_from_con = dm2_ref_data_doc->tbl_qual[tc_temp_tbl_cnt].col_qual[temp_col_cnt].
         column_name
         SET drdm_to_con = value(dm2_ref_data_doc->tbl_qual[tc_temp_tbl_cnt].col_qual[temp_col_cnt].
          sequence_name)
         SET drdm_parser->statement[1].frag = concat("select into 'nl:' y=seq(",drdm_to_con,
          ", nextval)")
         SET drdm_parser->statement[2].frag =
         " from dual detail drdm_tc_new_seq = y with nocounter go"
         CALL parse_statements(2)
         CALL parser(concat("set ",drdm_table_name,"->to_values.",drdm_from_con,
           " = drdm_tc_new_seq go"),1)
         SET new_seq_ind = 1
         SET current_merges = (current_merges+ 1)
         SET child_merge_audit->num[current_merges].action = "NEWSEQ"
         SET child_merge_audit->num[current_merges].text = concat(dm2_ref_data_doc->tbl_qual[
          tc_temp_tbl_cnt].table_name,"  ",dm2_ref_data_doc->tbl_qual[tc_temp_tbl_cnt].col_qual[
          temp_col_cnt].column_name)
         SET dm2_ref_data_doc->tbl_qual[tc_temp_tbl_cnt].col_qual[temp_col_cnt].translated = 1
         CALL parser(concat("set to_this = ",drdm_table_name,"->to_values.",drdm_from_con," go"),1)
         CALL parser(concat("set from_this = ",drdm_table_name,"->from_values.",drdm_from_con," go"),
          1)
   ...
   ENDWHILE
   FREE SET query_value
   FREE SET translated_value
   FREE SET go_flag
   FREE SET ui_check
   FREE SET drdm_table_name
   RETURN(null)
 END ;Subroutine
 SUBROUTINE translate_vers_col(tv_temp_tbl_cnt,tv_temp_col_cnt)
   DECLARE tv_loop = i4
   DECLARE tv_fk_ndx = i4
   DECLARE tv_par_tab_ndx = i4
   DECLARE tv_par_pk_ndx = i4
   DECLARE tv_par_vers_ndx = i4
   DECLARE tv_vers_num = i4
   DECLARE tv_child_val = f8
   SET tv_fk_ndx = locateval(tv_loop,1,dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].col_cnt,
    dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].col_qual[tv_temp_col_cnt].child_fk_col,
    dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].col_qual[tv_loop].column_name)
   IF (tv_fk_ndx > 0)
    IF ((dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].col_qual[tv_fk_ndx].translated=1))
     CALL parser(concat("set tv_child_val = RS_",dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].suffix,
       "->to_values.",dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].col_qual[tv_temp_col_cnt].
       child_fk_col," go"),1)
     IF (tv_child_val=0)
      CALL parser(concat("set RS_",dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].suffix,"->to_values.",
        dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].col_qual[tv_temp_col_cnt].column_name," = 0 go"),
       1)
      SET dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].col_qual[tv_temp_col_cnt].translated = 1
     ELSE
      SET tv_par_tab_ndx = locateval(tv_loop,1,dm2_ref_data_doc->tbl_cnt,dm2_ref_data_doc->tbl_qual[
       tv_temp_tbl_cnt].col_qual[tv_temp_col_cnt].parent_table,dm2_ref_data_doc->tbl_qual[tv_loop].
       table_name)
      IF (tv_par_tab_ndx=0)
       SET tv_loop = temp_tbl_cnt
       SET tv_par_tab_ndx = fill_rs("TABLE",dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].col_qual[
        tv_temp_col_cnt].parent_table)
       SET temp_tbl_cnt = tv_loop
      ENDIF
      IF (tv_par_tab_ndx > 0)
       SET tv_par_pk_ndx = locateval(tv_loop,1,dm2_ref_data_doc->tbl_qual[tv_par_tab_ndx].col_cnt,
        dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].col_qual[tv_temp_col_cnt].parent_pk_col,
        dm2_ref_data_doc->tbl_qual[tv_par_tab_ndx].col_qual[tv_loop].column_name)
       SET tv_par_vers_ndx = locateval(tv_loop,1,dm2_ref_data_doc->tbl_qual[tv_par_tab_ndx].col_cnt,
        dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].col_qual[tv_temp_col_cnt].parent_vers_col,
        dm2_ref_data_doc->tbl_qual[tv_par_tab_ndx].col_qual[tv_loop].column_name)
       IF (tv_par_vers_ndx > 0
        AND tv_par_pk_ndx > 0)
        CALL parser(concat("select into 'NL:' from ",dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].
          col_qual[tv_temp_col_cnt].parent_table," d "),0)
        CALL parser(concat(" where d.",dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].col_qual[
          tv_temp_col_cnt].parent_pk_col," = tv_child_val"),0)
        CALL parser(concat(" detail tv_vers_num = d.",dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].
          col_qual[tv_temp_col_cnt].parent_vers_col," with nocounter go"),1)
        IF (check_error(dm_err->eproc)=1)
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        ENDIF
        SET dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].col_qual[tv_temp_col_cnt].translated = 1
        CALL parser(concat("set RS_",dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].suffix,
          "->to_values.",dm2_ref_data_doc->tbl_qual[tv_temp_tbl_cnt].col_qual[tv_temp_col_cnt].
          column_name," = tv_vers_num go"),1)
       ELSE
        CALL disp_msg(concat("ERROR: Parent table doesn't contain necessary versioning columns: ",
          trim(cnvtstring(drdm_chg->log[drdm_log_loop].log_id))),dm_err->logfile,1)
        SET drdm_error_out_ind = 1
       ENDIF
      ELSE
       CALL disp_msg(concat("ERROR: Could not obtain parent tables meta-data : ",trim(cnvtstring(
           drdm_chg->log[drdm_log_loop].log_id))),dm_err->logfile,1)
       SET drdm_error_out_ind = 1
      ENDIF
     ENDIF
    ENDIF
   ELSE
    CALL disp_msg(concat("ERROR: The child FK doesn't exist : ",trim(cnvtstring(drdm_chg->log[
        drdm_log_loop].log_id))),dm_err->logfile,1)
    SET drdm_error_out_ind = 1
   ENDIF
 END ;Subroutine
#exit_child
 FREE SET no_unique_ident
 FREE SET another_loop
 FREE SET drdm_return_var
 FREE SET drdm_srch_str
 FREE SET drdm_from_con
 FREE SET drdm_to_con
 FREE SET drdm_parser_cnt
 FREE SET where_clause
 FREE SET drdm_go_ind
 FREE SET short_string
 FREE SET update_error_check_ind
 FREE SET looked_up
 FREE SET been_translated
 FREE SET inserted
 FREE SET drdm_col_value
 FREE SET drdm_col_name
 FREE SET drdm_max_id
 FREE SET no_insert_update
 FREE SET is_valid
 FREE SET last_log_id
 FREE SET good_rec
 FREE SET rec_struct_loop
 FREE SET final_check_loop
 FREE SET unique_ident_loop
 FREE SET trans_val_loop
 FREE SET cv_query_loop
 FREE SET ins_upd_loop
 FREE SET update_loop
 FREE SET where_update_loop
 FREE SET insert_loop
 FREE SET passed_var
 FREE SET insert_update_reason
 FREE SET idcd_msg
 FREE SET zero_rec
 FREE SET last_table
 FREE SET to_this
 ...
 CALL echo("")
 CALL echo("")
 CALL echo(build("All Free Mem = ",curmem))
 CALL echo("")
 CALL echo("")
END GO
