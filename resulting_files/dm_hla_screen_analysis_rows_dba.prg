CREATE PROGRAM dm_hla_screen_analysis_rows:dba
 ...
 DECLARE v_years_to_keep = i4 WITH protect, noconstant(0)
 ...
 SET reply->table_name = "HLA_SCREEN_ANALYSIS"
 SET reply->rows_between_commit = 50
 FOR (v_tok_ndx = 1 TO size(request->tokens,5))
   IF ((request->tokens[v_tok_ndx].token_str="YEARSTOKEEP"))
    SET v_years_to_keep = ceil(cnvtreal(request->tokens[v_tok_ndx].value))
   ENDIF
 ENDFOR
 IF (v_years_to_keep < 1)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"TOKENERR",
   "You must keep at least oen year's worth of data.  You entered %1 years or did not enter any value.",
   "i",v_years_to_keep)
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  hsa.rowid
  FROM hla_screen_analysis hsa
  WHERE hsa.updt_dt_tm < cnvtlookbehind(build(cnvtstring(v_years_to_keep),",Y"))
   AND parser(sbr_getrowidnotexists("hsa.criteria_id != 0","hsa"))
  DETAIL
   v_rows = (v_rows+ 1)
   IF (mod(v_rows,50)=1)
    stat = alterlist(reply->rows,(v_rows+ 49))
   ENDIF
   reply->rows[v_rows].row_id = hsa.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,v_rows)
  WITH nocounter
 ;end select
 SET reply->err_code = error(v_errmsg2,1)
 IF ((reply->err_code != 0))
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"COLLECTERR",
   "Failed in collection of rows for purging: %1","s",nullterm(v_errmsg2))
  GO TO exit_script
 ENDIF
