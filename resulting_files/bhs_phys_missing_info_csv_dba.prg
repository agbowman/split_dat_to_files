CREATE PROGRAM bhs_phys_missing_info_csv:dba
 PROMPT
   ...
   FOR (y = 1 TO size(md_alias->qual[d.seq],5))
     ...
     ->qual[y].b_fax=null)) OR ((((md_alias->qual[y].ez_addr_1=null)
      ...
      xperson_id = format(md_alias->qual[y].personid,"#########"), output_string = build(',"',
       ...
       '","',
       md_alias->qual[y].npi_nbr,'","',md_alias->qual[y].org_id,'","',md_alias->qual[y].ext_id,
       '","',md_alias->qual[y].doc_upin,'","',md_alias->qual[y].license_nbr,'","',
       md_alias->qual[y].b_addr_1,'","',md_alias->qual[y].b_addr_2,'","',md_alias->qual[y].b_addr_3,
       '","',md_alias->qual[y].b_addr_4,'","',md_alias->qual[y].b_city,'","',
       md_alias->qual[y].b_state,'","',md_alias->qual[y].b_zip,'","',format(md_alias->qual[y].b_phone,
        "(###)###-####"),
       '","',format(md_alias->qual[y].b_fax,"(###)###-####"),'","',md_alias->qual[y].ez_addr_1,'","',
       md_alias->qual[y].ez_addr_2,'","',md_alias->qual[y].ez_addr_3,'","',md_alias->qual[y].
       ez_addr_4,
       '","',md_alias->qual[y].ez_city,'","',md_alias->qual[y].ez_state,'","',
       md_alias->qual[y].ez_zip,'","',format(md_alias->qual[y].ez_phone,"(###)###-####"),'",'), col 1,
      output_string
      IF ( NOT (curendreport))
       row + 1
      ENDIF
 ...
 IF (email_ind=1)
  ...
  SET filename_out = concat(format(curdate,"YYYY-MM-DD;;D"),"_PHYS_missing_info.csv")
  DECLARE subject_line = vc
  SET subject_line = concat(curprog,"-V1.x - PHYS missing info ",curnode)
