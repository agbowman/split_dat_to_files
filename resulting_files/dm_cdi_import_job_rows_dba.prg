CREATE PROGRAM dm_cdi_import_job_rows:dba
 ...
 DECLARE complete_status = i2 WITH protect, noconstant(3)
 DECLARE filepurged_status = i2 WITH protect, noconstant(6)
 SET i18nhandle = 0
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 SET reply->table_name = "CDI_IMPORT_JOB"
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(j.cdi_import_job_id)
    FROM cdi_import_job j
    WHERE j.cdi_import_job_id > 0
    DETAIL
  ...
  ENDIF
  SELECT INTO "nl:"
   seqval = max(j.cdi_import_job_id)
   FROM cdi_import_job j
   DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     j.rowid
     FROM cdi_import_job j
     WHERE parser(sbr_getrowidnotexists("j.cdi_import_job_id between curMinID and curMaxID","j"))
      AND j.create_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
      AND ((j.status_flag=complete_status) OR (j.status_flag=filepurged_status))
     DETAIL
      rows += 1
      IF (mod(rows,50)=1)
       stat = alterlist(reply->rows,(rows+ 49))
      ENDIF
      reply->rows[rows].row_id = j.rowid
     WITH nocounter, maxqual(j,value(rowsleft))
    ;end select
