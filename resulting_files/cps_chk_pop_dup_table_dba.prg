CREATE PROGRAM cps_chk_pop_dup_table:dba
 ...
 SET false = 0
 SET continue = true
 SET log_file_name = "CPS_POP_NOMEN_DUP_HOLD.LOG"
 SELECT INTO "nl:"
  err_msg = r.line
  FROM rtl2t r
  HEAD REPORT
 ...
 IF (curqual < 1)
  ...
  SET request->setup_proc[1].error_msg = concat("FAILURE on population of Nomen_Dup_Hold table ",
   format(cnvtdatetime(curdate,curtime3),"dd-mmm-yyyy hh:mm;;q"),"  : Could not find ",trim(
    log_file_name)," look for ",
 ...
 WHILE (knt >= 1
  AND continue=true)
   IF (substring(31,7,err_log->qual[knt].msg)="SUCCESS")
    SET continue = false
    IF (datetimediff(cnvtdatetime(beg_dt_tm),cnvtdatetime(substring(40,20,err_log->qual[knt].msg)))
     < time_dif)
     SET request->setup_proc[1].success_ind = 1
     SET request->setup_proc[1].error_msg = concat("SUCCESS on population of Nomen_Dup_Hold table ",
   ...
   ELSEIF (substring(31,7,err_log->qual[knt].msg)="WARNING")
    ...
    IF (datetimediff(cnvtdatetime(beg_dt_tm),cnvtdatetime(substring(40,20,err_log->qual[knt].msg)))
     < time_dif)
     SET request->setup_proc[1].success_ind = 1
     SET request->setup_proc[1].error_msg = concat("WARNING on population of Nomen_Dup_Hold table ",
