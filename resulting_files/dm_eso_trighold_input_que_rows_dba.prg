CREATE PROGRAM dm_eso_trighold_input_que_rows:dba
 ...
 SET reply->table_name = "ESO_TRIGHOLD_INPUT_QUE"
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    minval = min(etiq.eso_trighold_input_que_id)
    FROM eso_trighold_input_que etiq
    WHERE etiq.eso_trighold_input_que_id > 0
    DETAIL
  ...
  ENDIF
  SELECT INTO "nl:"
   maxval = max(etiq.eso_trighold_input_que_id)
   FROM eso_trighold_input_que etiq
   DETAIL
  ...
  WHILE (v_cur_min_id <= v_max_id
   AND v_rows_left > 0)
    SELECT INTO "nl:"
     eiq.eso_trighold_input_que_id
     FROM eso_trighold_input_que eiq
     WHERE parser(sbr_getrowidnotexists("eiq.updt_dt_tm < cnvtdatetime(v_days_back)","eiq"))
      AND eiq.eso_trighold_input_que_id BETWEEN v_cur_min_id AND v_cur_max_id
      AND eiq.trigger_proc_flag=3
     DETAIL
      row_cnt += 1
      IF (mod(row_cnt,50)=1)
       stat = alterlist(reply->rows,(row_cnt+ 49))
      ENDIF
      reply->rows[row_cnt].row_id = eiq.rowid
     WITH nocounter, maxqual(eiq,value(v_rows_left))
    ;end select
    ...
    SET v_cur_max_id = (v_cur_min_id+ (v_batch_size - 1))
    SET v_rows_left = (request->max_rows - row_cnt)
  ENDWHILE
