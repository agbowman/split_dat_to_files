CREATE PROGRAM acm_add_encntr_alias_hist:dba
 ...
 INSERT  FROM encntr_alias_hist eah,
  ...
  SET eah.encntr_alias_hist_id = seq(encounter_seq,nextval), eah.transaction_dt_tm = cnvtdatetime(
    acm_request->encntr_alias_qual[xref->add[d.seq].idx].transaction_dt_tm), eah.encntr_id =
   acm_request->encntr_alias_qual[xref->add[d.seq].idx].encntr_id,
   eah.encntr_alias_id = acm_request->encntr_alias_qual[xref->add[d.seq].idx].encntr_alias_id, eah
   .pm_hist_tracking_id = acm_request->encntr_alias_qual[xref->add[d.seq].idx].pm_hist_tracking_id,
   eah.alias = acm_request->encntr_alias_qual[xref->add[d.seq].idx].alias,
   eah.tracking_bit = tracking_bit, eah.change_bit = chgbit, eah.active_ind = 1,
