CREATE PROGRAM ags_exe_person_load:dba
 IF (validate(reply,"!")="!")
  FREE RECORD reply
  RECORD reply(
    1 status_data
 ...
 ENDIF
 CALL echo("***")
 CALL echo("***   BEG AGS_EXE_PERSON_LOAD")
 CALL echo("***")
 IF (validate(request,"!")="!")
  GO TO exit_script
 ENDIF
 ...
 DECLARE require_ssn = i2 WITH public, noconstant(1)
 DECLARE consent_cdf = vc WITH public, noconstant("YES")
 CALL echo("***")
 ...
 WHILE (ipos < bs_length)
   ...
   CASE (field_name)
    ...
    OF "REQUIRE_SSN":
     SET require_ssn = cnvtint(field_data)
    OF "CONSENT_CDF":
     SET consent_cdf = trim(field_data)
    ELSE
     CALL ags_set_status_block(eattribute,efailure,"WORKING_BATCH_SELECTION",concat(
 ...
 CALL ags_set_status_block(eattribute,esuccessful,"REQUIRE_SSN",concat("REQUIRE_SSN :: ",trim(
    cnvtstring(require_ssn))))
 CALL ags_set_status_block(eattribute,esuccessful,"CONSENT_CDF",concat("CONSENT_CDF :: ",trim(
    cnvtstring(consent_cdf))))
...
#skip_parse
 ...
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
  CALL ags_set_status_block(eselect,efailure,"GET PERSON DATA",trim(serrmsg))
  GO TO exit_script
 ENDIF
 CALL echo("***")
 ...
 SET rpersonload->consent_cdf = consent_cdf
 SET rpersonload->require_ssn = require_ssn
 FOR (exe_idx = 1 TO work->qual_knt)
   SET rpersonload->ags_task_id = work->qual[exe_idx].ags_task_id
   SET rpersonload->debug_logging = work->qual[exe_idx].timers_flag
   EXECUTE ags_person_load  WITH replace("REQUEST","RPERSONLOAD")
   IF (get_script_status(0)=efailure)
    CALL ags_set_status_block(ecustom,efailure,"AGS_PERSON_LOAD",concat("Load for ags_task_id ",
      cnvtstring(rpersonload->ags_task_id)," FAILED"))
    GO TO exit_script
   ENDIF
 ENDFOR
#skip_person_load
...
#exit_script
 ...
 CALL echo("***")
 CALL echo("***   END AGS_EXE_PERSON_LOAD")
 CALL echo("***")
 SET script_ver = "005 11/22/06"
END GO
