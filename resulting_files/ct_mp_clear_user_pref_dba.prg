CREATE PROGRAM ct_mp_clear_user_pref:dba
 PROMPT
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 SET log_program_name = "ct_mp_clear_user_pref"
 ...
 SET reply->status_data.status = "F"
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_clear_user_pref","Invalid Json Request",reply
   )
  GO TO exit_script
 ENDIF
 SET stat = cnvtjsontorec( $JSONREQUEST)
 IF ((request->prot_id <= 0))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_clear_user_pref","Invalid prot master id",
   reply)
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
